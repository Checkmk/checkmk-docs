...
EXCLUDES = ('check_', 'draft_', 'cma_', 'legacy_', '.', 'internal_', 'missing',
            'index', 'deprecated_', 'training_', 'global_attr')
...
class GitCommits():
    def __init__(self):
        self.Commit = NamedTuple('Commit', [('name', str), ('date', str),
                                            ('id', str), ('message', str)])
        self.Translate = NamedTuple('Translate', [('id', str), ('date', str),
                                                  ('files', dict)])
        self.translated_markers = list()
        self.last_translation = dict()
        self.commits = list()
        self.default_branch = check('git branch --show-current',
    ...
    def _get_log_as_list_of_dicts(self, max_age, pretty, file):
        ...
        for entry in raw_list:
            ...
            commits.append(
                dict(name=name, date=date, id=id[:6], summary=summary))
...
class ColorizedOutput(object):
    ...
    def __init__(self, box_size=90, tty=True):
        ...
        self.languages = dict(
            de="{bg_red}{black} ⚒ {yellow}{de}{black} ⚒ {normal}".format(
                **self.colors),
            en="{bg_blue}{white} ✭ {white}{en}{white} ✭ {normal}".format(
                **self.colors))
        self.colors.update(de=self.languages[u'de'], en=self.languages[u'en'])
    ...
    def article_details(self, commits, lang):
        ...
        for entry in commits:
            ...
            attr = dict(prefix=prefix,
                        commit=commit,
                        message=commit.message[:message_length],
                        commit_date=datetime.utcfromtimestamp(int(
                            commit.date)).strftime('%Y-%m-%d %H:%M'))

            summary = "{magenta}{commit.id} {blue}{commit_date} {yellow}{commit.name}\033[500D|\033[38C {normal}{message}{box_borders}"
            if prefix:
        ...
        if dirty_commits >= 1 and write:
            ...
            if last_translation == "1104537600":
                ...
                date = datetime.utcfromtimestamp(
                    int(last_translation)).strftime('%Y-%m-%d %H:%M')
                hint = f" - last full translation: {date}"
            self.line(
                "| {bold}{filename}{normal} \033[500D|\033[30C{green}clean{normal}{bold}{hint}{normal}{box_borders}",
                **dict(filename=filename, hint=hint))
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Article name or commit hash. The" + \
    ...
    parser.add_argument('-a',
                        '--article',
                        default='',
                        help='A specific article to show')
    parser.add_argument(
        '-c',
        '--commit',
        default='',
    ...
    parser.add_argument(
        ...
        default=85,
        help=
        'Adjust the width to use more of your screen. Default is set to 85 characters'
    )
...
def dirty_commit(commit, test_ids, test_messages, except_marker):
    ...
    if commit.id in test_ids:
        return False
    elif message_to_check.startswith(MARKER) \
            and not message_to_check.startswith(except_marker):
        return False
...
def get_article_diff(article):
    ...
    for combination in [('de', 'en'), ('en', 'de')]:
        ...
        for commit in article.commits.get(lang, []):
            if dirty_commit(commit, article.commits.get(f'{test}_ids', []),
...
def main():
    ...
    if opts.article_or_hash:
        opt_commit = identify_positional(opts.article_or_hash,
                                         for_type='commit')
        opt_article = identify_positional(opts.article_or_hash,
                                          for_type='article')
    else:
    ...
    if opt_commit:
        ...
        for lang in article.lang:
            article.commits[f'{lang}_messages'], article.commits[
                f'{lang}_ids'], article.commits[lang] = repo.get_file_commits(
                    article.filename, lang, max_age=article.last_translation)
        state, commits = get_article_diff(article)

        write.line("{box_top}")
        if state:
            summary_line = "| {bold}{filename}{normal}" + " " * 20 + "{green}clean{normal}{box_borders}"
        else:
            summary_line = "| {bold}{filename}{normal}" + " " * 20 + "{red}dirty{normal}{box_borders}"
        write.line(
            summary_line,
            **dict(filename=article.filename.replace('.asciidoc', ''),
                   clean=state))
        ...
        for article in articles:
            ...
            for lang in article.lang:
                article.commits[f'{lang}_messages'], article.commits[
                    f'{lang}_ids'], article.commits[
                        lang] = repo.get_file_commits(article.filename, lang,
                                                      article.last_translation)
        ...
        if without_translation_marker:
            ...
            write.line("{red}Without translation marker{normal}: {articles}\n",
