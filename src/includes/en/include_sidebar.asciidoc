...
.Sidebar for quick access to frequently used functions
ifdef::onprem[]
image::gui_sidebar_default.png[alt="{CMK} sidebar.",width=42%]
endif::[]
ifdef::saas[]
image::gui_sidebar_default_saas.png[alt="{CMK} sidebar.",width=42%]
endif::[]
...
* [.guihint]#Welcome to {CMK}# -- Show slideout with the content of the welcome page
...
In this preview, the lower area of each snap-in shows a short description about its meaning and function.
...
There are several ways to customize the snap-ins of the sidebar.
...
* Remove from sidebar:
Point your mouse at the lower right corner of the snap-in until the icon:icon_delete[alt=“Delete icon.”] trash can icon appears, and then click the icon.
====
