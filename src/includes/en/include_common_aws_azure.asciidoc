...
You can select the push mode with the [.guihint]#{CMK} agent connection mode# option in the host's properties in the xref:hosts_setup#monitoring_agents[Monitoring agents] section. 
...
Check the link:https://checkmk.com/download?method=cmk&edition=cce&version={current-major}&platform=ubuntu&os=jammy&type=cmk[download page^] regularly for updates and download the updated package with the `wget` command shown there.
...
The first step is to install the package, in the following example the {current} version:
