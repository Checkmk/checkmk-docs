...
[TIP]
====
Javascript evaluates _keycodes_, not resulting characters (_keysyms_).
For this reason the keys shown in the table apply to the US American QWERTZ keyboard layout.
If you are using a different layout (for example AZERTY as used in French speaking countries), other keys might have to be used.
====
