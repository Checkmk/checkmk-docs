...
Prüfen Sie die link:https://checkmk.com/de/download?method=cmk&edition=cce&version={current-major}&platform=ubuntu&os=jammy&type=cmk[Download-Seite^] regelmäßig auf Updates und laden Sie das aktualisierte Paket mit dem dort angezeigten `wget` Kommando herunter.
...
mysite       {current}.cce   default version
mytestsite   {current-major}p3.cce
----
