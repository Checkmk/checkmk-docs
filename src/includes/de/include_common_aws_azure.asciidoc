...
// include_common_aws_azure wird genutzt ininstall_azure und install_aws
...
Stattdessen kommt der Nutzer `ubuntu` zum Einsatz, der über die Berechtigung verfügt, `sudo` mit beliebigen Befehlen ohne Passwortabfrage auszuführen.
...
Created new site mysite with version {current}.ultimate.
...
Die in der obigen Befehlsausgabe gezeigte URL (`\http://cloud/mysite`) verwendet den intern genutzten Host-Namen Ihrer AWS- oder Azure-VM.
...
Prüfen Sie die link:https://checkmk.com/de/download?method=cmk&edition=cce&version={current-major}&platform=ubuntu&os=jammy&type=cmk[Download-Seite^] regelmäßig auf Updates und laden Sie das aktualisierte Paket mit dem dort angezeigten Befehl `wget` herunter.
...
Der erste Schritt ist die Installation des Paketes, im folgenden Beispiel die Version {current}:

[{shell}]
----
{c-ubuntu} sudo apt install ./{pkg-cce}-{current}_0.jammy_amd64.deb
----
...
mysite       {current}.ultimate   default version
mytestsite   {current-major}p3.ultimate
----
...
[{shell}]
----
{c-ubuntu} sudo apt purge {pkg-cce}-{current-major}p3
----
