...
// include_views wird genutzt in user_interface und intro_tools
...
ifdef::onprem[]
.Ansicht der Service-Probleme in der Hauptseite
image::gui_view_service_problems.png[alt="Tabellenansicht 'Service problems'."]
endif::[]
ifdef::saas[]
image::gui_view_service_problems_saas.png[alt="Tabellenansicht 'Service problems'."]
endif::[]
...
Mit [.guihint]#Entries per row# können Sie auswählen, in wie vielen Tabellen die Objekte in der Ansicht nebeneinander angezeigt werden (z.B. um Ihren breiten Bildschirm voll auszunutzen).
Im obigen Beispiel steht der Wert auf `1`.
