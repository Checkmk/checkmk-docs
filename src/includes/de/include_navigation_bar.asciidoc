...
Oben rechts im Menü sehen Sie den aktuellen Benutzernamen und die diesem Benutzer zugewiesene xref:wato_user#roles[Rolle] (`admin`). 
...
ifdef::saas[]
Das [.guihint]#Admin#-Symbol öffnet direkt, d. h. ohne Menü, das xref:admin_panel#[Administration Panel] für die Verwaltung von {CE}.
endif::[]
...
Unter anderem können Sie auch dieses Handbuch öffnen.
Falls es nach einem Update der Instanz inkompatible Änderungen gibt, wird Ihnen die Zahl im Symbol des [.guihint]#Help#-Menüs eingeblendet.
Im [.guihint]#Help#-Menü selbst wird dann ein Link in roter Hintergrundfarbe angezeigt, mit dem Sie die Liste der xref:glossar#werk[Werks] dieser inkompatiblen Änderungen öffnen können. 
endif::[]
...
// TK: Die Shortcuts zur Suche kommen eigentlich zu früh, da die Suche in beiden Artikeln später kommt. Aber dafür 2 Tabellen einzuführen, finde ich noch schlechter. 
