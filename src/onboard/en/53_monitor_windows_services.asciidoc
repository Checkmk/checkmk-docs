...
:description: By default, Checkmk does not monitor any services on your Windows hosts. This is simply because Checkmk does not know which services are important to you. Use the Windows service discovery to tell the service discovery which Windows services are to be monitored.
...
++++3. In the input field enter `EventLog.*` and in the next input field that opens `Dhcp.*`.
...
If you define these values independent of the Windows service discovery with its own, following rule, a change will be valid even without a new service discovery.
...
++++1. Click *Setup > Services > Service monitoring rules > Windows Services* and then click *link:/pass:[$$SITE_NAME$$]/check_mk/index.py?start_url=%2Fpass:[$$SITE_NAME$$]%%2Fcheck_mk%2Fwato.py%3Fcsrf_token%3D728ec448-d5bf-4b62-a6e1-fc08f01efb8a%26filled_in%3Dnew_rule%26ruleset_back_mode%3Drulesets%26rule_folder%3D%26varname%3Dcheckgroup_parameters%253Aservices%26mode%3Dnew_rule%26folder%3D%26_new_dflt_rule%3DSETET[Add rule]*.

++++2. In the *Value* box, check *Services states* and click *Add new element*.
...
// No User Guide link for this topic
