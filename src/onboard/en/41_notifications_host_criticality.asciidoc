...
:description: The number of superfluous notifications sent out by a monitoring system is ideally 0 (in words: zero). In order to get there, one important step is to configure the actual criticality of your hosts and services and sent notifications accordingly.
...
Details on how to configure each of the methods is available in the link:https://docs.checkmk.com/saas/en/notifications.html#notification_method[Checkmk Cloud User Guide^].
