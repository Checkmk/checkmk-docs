...
Before you start with Checkmk, you will have to prepare your Kubernetes cluster and install our Node Collector and our Cluster Collector.
...
++++2. Add the entire content of the certificate you gathered earlier and click *Save*.
...
++++2. In the *Kubernetes* box, you can freely assign the *Cluster name*. It just needs to be unique in your Checkmk site.
...
++++6. Just as before, again select the *Verify the certificate* option for the *SSL certificate verification*.
...
In order to automatically create hosts for all the components within your cluster, you will need to setup the _dynamic host management_.
