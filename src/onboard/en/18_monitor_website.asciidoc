...
:description: Checkmk provides active checks that allow you to monitor network services directly and conveniently from the outside by directly contacting a network protocol at the target host, in this case HTTP(S).


[#configure_active_check]
== Configure the active check

Let's assume you want to monitor the validity of the certificates of all web servers in your infrastructure, ensure response times of less than one second and a status code of 200.
...
++++4. In the *Standard settings for all endpoints* section, check *Response time*, click *Add new entry* and set the fixed levels for *Warning at* to `1` second and *Critical at* to `2` seconds.

++++5. Check *Status code*, click *Add new entry* and set *Expected* to `200`.

++++6. Check *Certificate validity*, click *Add new entry* and set the fixed levels for *Warning below* to `15` days and *Critical below* to `8` days.
...
In this step the `www.checkmk.com` web server is created and configured as host.
...
You receive a list of all hosts and their `Basic website health` services that monitor the websites.
