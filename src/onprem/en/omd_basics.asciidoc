...
:description: {CMK} is built on top of the Open Monitoring Distribution (OMD). Read here how to use the omd command to create and administrate {CMK} sites.    
:experimental:
...
.Features of an OMD-based installation
[%collapsible]
====
...
.In the `omd config` menu you navigate with the arrow and kbd:[Enter] keys
...
The output above is abbreviated for clarity and shows only the first entries.
...
By the way, `omd update` is also used to xref:upgrade#[upgrade] to another edition, e.g. from {RE} to {SE}.
...
The lion's share of the data to be moved during a site backup are the measured values stored in the xref:graphing#rrds[Round Robin Databases (RRDs)] and the log files with historical events.
Another significant part of the data to be moved during a site backup are the agent packages, which are stored by the Agent Bakery in the `~/var/check_mk/agents` directory, but these can be easily recreated if necessary.

`omd backup` offers xref:limit_data[the same options] as `omd cp` when copying to exclude this data.
In the following example, the backup is created completely without this data:
...
When backing up, you can also specify which of the above data you want to exclude.
Use `omd backup --help` to display the available options.


=== Backing up a running site

A backup can also be created from a running site.
To ensure a consistent state of the xref:graphing#rrds[RRDs] used for recording the measurement data, the `omd backup` command automatically alters the Round Robin cache to a mode with which the running updates are written only to the journal, and no longer to the RRDs.
