...
:description: With the ntopng integration, {CMK} can monitor network data flows. Read here how to analyze the network traffic with ntopng.
...
The connection parameters are used to specify which data is fetched from ntopng.
...
{cee-only}
To use the integration of ntopng in {CMK}, you need one of the commercial editions of {CMK} with the add-on for ntopng integration and an ntopng installation in a Professional or Enterprise version >= 5.6,
...
After you have xref:glossar#activate_changes[activated the pending changes], all {CMK} users will see the user interface changes for ntopng
...
Finally, activate the pending changes.
...
The [.guihint]#Alerts#, [.guihint]#Flows# and [.guihint]#Top Talkers# dashboards presented in the last sections all consist of only one widget each.

When editing a xref:dashboards#[dashboard], you can use these ntopng widgets to customize existing dashboards or to create new ones.
...
When editing a dashboard, you will find the ntopng widgets, as well as the other widgets, in the [.guihint]#Add# menu in the [.guihint]#Ntop# section.
