...
* The {CMK} version (e.g. {v24}) of the remote and central sites is the same -- xref:mixed_versions[mixed versions] are supported only for easier updates.
* In the same way as {CMK} supports multiple sites on a server, remote sites can also run on the same server.

Here is an example for creating a remote site with the name `myremote1`:
...
You only need it once: to connect the remote site to the central site.
...
Be sure to check which [.guihint]#Message broker port# you need to specify. You can find this on the respective remote instance - in the [.guihint]#Basic# section via the command line with `omd config`:

image::distributed_monitoring_rabbitmq.png[alt="Display of the message broker port in omd config.", width=80%]

[TIP]
====
Don't be confused, the [.guihint]#Message Broker Port# is called [.guihint]#RABBITMQ_PORT# here, as currently only the RabbitMQ Message Broker software is integrated in {CMK}.
====
...
To enable such communication, you only need to ensure that xref:ports#distributed_monitoring[the message broker port is set correctly on all participating instances] and xref:create_remote[the piggyback hub is enabled].
...
If you run the inventory using {RE} in distributed systems, the directory must
...
If you have many simultaneous users, ensure the configuration
...
may not be visible, a xref:glossar#notification[notification] through the central site is not ideal.
...
`.cfg`, for example `~/etc/nagios/conf.d/config-myremote1.cfg`.
...
Note -- these settings will be automatically inherited by all remotes for
...
In rare cases, however, even these can be incompatible with each other, so in such a situation you must maintain exactly the same version level (e.g. {v23}p11) on all sites to allow the sites to work together without errors.
