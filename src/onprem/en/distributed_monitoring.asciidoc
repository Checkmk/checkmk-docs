...
As a prerequisite for this, both remote sites must also have been configured by the central site -- i.e. [.guihint]#Push configuration to this site# must be configured as described in the section
xref:connect_remote_sites[[Assigning remote sites to the central site]], must be activated for both.
...
If you run inventories using {RE} in distributed systems, the directory must
