// -*- coding: utf-8 -*-
// IGNORE se customer1 customer2 customer3 operator1
...
For these reasons we do not recommend it.
////

The procedure used by {CMK} for implementing a distributed monitoring is based on xref:livestatus[Livestatus] and the automated distribution of configuration.
If requirements mandate highly isolated networks or even _strictly unidirectional data transfer_ from the periphery to the central site, it is possible to use xref:livedump[Livedump or CMCDump].
...
{CMK}'s xref:user_interface#[user interface] uses Livestatus to combine all tethered sites into a general overview, which then _resembles_ a single, large monitoring system.
...
Such a system not only has a common status overview but also a common configuration, and effectively '_feels_ like a large system'.
...
Then xref:glossar#activate_changes[activate the pending changes], as switching the piggyback hub on or off requires the site to be restarted.
...
The most important steps are now to activate Livestatus via TCP for the network and to enable the piggyback hub here as well.
Note that Livestatus is not _per se_ a secure protocol and should only be used within a secure network (secured LAN, VPN, etc.).
...
Set `LIVESTATUS_TCP` to `on`, and for `LIVESTATUS_TCP_PORT` enter an unallocated port number that is unique on this server.
The default is 6557:
...
A check with `netstat` shows that port 6557 is open.
The connection to this port is made with an instance of the Internet super-server `xinetd`, which runs directly on the site:
...
Now log in once to the remote site as user `cmkadmin` and activate the piggyback hub using the [.guihint]#Setup > General > Global# settings in the section [.guihint]#Site management#:

image::distributed_monitoring_site_management.png[alt="Die Option 'Enable piggyback-hub' in den globalen Einstellungen."]

Then activate the pending changes, as switching the piggyback hub on or off requires the site to be restarted.
...
The settings of the [.guihint]#Status connection# determine how the central site queries the status of the remote sites via Livestatus.
...
Here enter the HTTP URL for the remote site's web interface, without the `check_mk/` at the end of the URL.
If you normally access {CMK} per HTTPS, then substitute the `http` here with `https`.
...
This must be installed xref:omd_https#[manually] on the remote site at the Linux level.
...
The `cmkadmin` account on the remote site will subsequently no longer be used.

Click on the key icon of the site to be connected.
In the following form, use as [.guihint]#Login credentials# `cmkadmin` and the password assigned when creating the remote site.
At this point, confirm that you want to overwrite the settings by clicking [.guihint]#Confirm overwrite#.
...
When everything has been successful, xref:glossar#activate_changes[activate the pending changes].
On the overview page for pending changes, you will also see an overview of the Livestatus connections and the synchronization states of the individual sites in the central setup:
...
The following icons show the replication status of the configuration environment:
...
Clicking the icon:icon_activate_changes[] [.guihint]#Activate on selected sites# button will now synchronize all sites and activate the pending changes.
...
==== Executing a fresh service discovery and activating pending changes for migrated hosts
...
To enable such communication, you only need to ensure that the message broker port is xref:ports#distributed_monitoring[set correctly] on all participating sites and xref:setting_up_central_site[the piggyback hub is enabled].
...
Data traffic often creates a bottleneck in communication.
...
A status icon will inform the user of the status of the password sharing to the individual sites.
...
Share Livestatus with TCP, then add the site to via [.guihint]#Setup > General > Distributed monitoring# – and you are done!
...
This is reached via the icon:button_configuration[] icon in the menu under [.guihint]#Setup > General > Distributed monitoring#:

image::distributed_monitoring_site_specific_global_configuration.png[]

Via this icon you will find a selection of all global settings--although anything you define here will only be effective for the chosen site.
...
The open-source program link:https://www.nagvis.org[NagVis^] visualizes status data from monitoring on self-produced maps, diagrams and other charts.
...
It is annoying when a site does not answer at all.
...
* The remote sites have no permanent network connection or no fixed IP-addresses and methods such as link:https://en.wikipedia.org/wiki/Dynamic_DNS[Dynamic DNS^] are not an option.
...
Livedump/CMCDump can help by generating a dump of the remote sites' configuration which can then be loaded into the central site.

Now during the monitoring, on every remote site a dump of the current status will be written to a file at predetermined intervals (e.g. every minute).
...
* Commands such as [.guihint]#Schedule downtimes# and [.guihint]#Acknowledge problems# on the central site _cannot_ be transmitted to the remote site.
...
First, on the remote site, create a dump of its host's and service's configurations in the Nagios-configuration format.
...
– where Nagios expects to find the configuration data for hosts and services.
...
Now log in to the central site and activate the pending changes:
...
* The creation of the dump and the transfer of the configuration must be repeated for every change to hosts or services on the remote site.
...
In contrast to Livedump, CMCDump can replicate the _complete_ status of hosts and services (Nagios does not have the required interfaces for this task).
...
* The timestamps for the last check execution and the last state change
...
When importing the status the CMC does not just simulate a check execution, rather by using an interface designed for this task it transmits an accurate status.
...
The dump of the configuration is made with `cmcdump -C`.
...
You will however see by the icon:icon_shadow[] icon that we are dealing with a _shadow object_.
...
If you have an access from the remote to the central site via SSH without a password all three commands can be combined into a single one
...
Technically speaking, the central notification server does not even need to be literally at the 'center'.
...
Once there the notification rules will evaluate who should actually be notified and how.
...
Notifications are sent from remote sites to the central site.
The central site acknowledges to the remotes that the notifications have been received, which prevents notifications being lost even if the TCP connection is broken.
...
Since remote and central sites require different global settings for the spooler, you must make xref:sitespecific[site specific settings] for _all_ remotes.
...
Step 5: Activate the pending changes.
...
[#mixed_versions]
== Updates in distributed monitoring setups
...
[#temp_diff_patch]
=== Temporarily differing patch levels (but same major version)
...
[#temp_diff_major]
=== Temporarily differing major versions
...
[#mixed_editions]
== Central and remote sites with different editions

Since {CMK} version {v25}, a central site can be connected to remote sites running other editions, as long as the remote sites are running an edition with a more limited feature set.
Up to and including {CMK} {v24}, mixed operation with different editions was only possible if the central site was running {CSE} and the remote sites were running {CRE}.

For remote sites with a more limited feature set, it is not possible to replicate the configuration from the central site.
Therefore, the [.guihint]#Enable replication# setting must be set to [.guihint]#No replication with this site#.
In practice, this means that such a scenario only allows for read-only access.

For xref:license#distributed[license verification and license usage reporting], the edition of the central site is taken into account.

Please also note the instructions for handling xref:upgrade#editiondistributed[upgrades] in distributed monitoring.
