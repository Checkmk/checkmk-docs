...
Set `LIVESTATUS_TCP` to `on` and enter a free port number for `LIVESTATUS_TCP_PORT` that is unique on this server.
The default is 6557:
...
The binding to this port is done with an instance of the Internet super-server `xinetd`, which runs directly in the site:
...
Enter the HTTP URL for the web interface of the remote site here, without the `check_mk/` at the end.
