...
To enable such communication, you only need to ensure that xref:ports#distributed_monitoring[Port 5671 is accessible on all participating sites].
...
Every notification that is generated on a remote site runs through the chain of xref:notifications#rules[notifications rules] there.
...
For the smooth execution of xref:update_major#[major updates] (for example from {v23} to {v24}) in large distributed environments, from version {v20} {CMK} allows mixed operations in which the central and remote sites may differ by one major version.
