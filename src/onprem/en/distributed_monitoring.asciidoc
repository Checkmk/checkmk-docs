...
{cre-only}
In {CRE} the link:http://docs.pnp4nagios.org/[PNP4Nagios]
...
Note that for Apache it _is_ relevant whether a URL ends
with a (/) ‘slash’ or not:
...
automatically activated. Verify this with `omd config` and activate
