...
In order to be able to use EPEL on RedHat and binary compatible distributions, the package sources for optional RPMs are required if these haven’t already been installed during the operating system installation.
...
On RHEL and all binary compatible distributions this is yet another job for `rpm`:
