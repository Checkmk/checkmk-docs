...
= Site services - {CMK} under the hood
:title: Site services - {CMK} under the hood
:description: Operating a {CMK} site requires many individual services/processes. This article provides a brief overview of them.
...
|`apache` |Yes |Yes |Yes |Web server, more details xref:apache [below]
...
|`xinetd` |Yes |Yes |No |TCP access to Livestatus
|===
...
The most prominent example is probably the xref:glossar#activate_changes[activation of pending changes].
...
The `automation-helper` service speeds up some GUI interactions, such as xref:glossar#service_discovery[service discovery] and activating pending changes,
