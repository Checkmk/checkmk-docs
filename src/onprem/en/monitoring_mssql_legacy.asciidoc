...
:description: {CMK} offers state-of-the-art solutions for monitoring Microsoft SQL Server environments. Read here how to configure the legacy plug-in mssql.vbs.
...
and will be removed from {CMK} (see link:https://checkmk.com/werk/17669[Werk pass:q[#]17669^]) with the next major version.
Therefore, switch to the new plug-in and the [.guihint]#Microsoft SQL Server (Linux, Windows)# rule set for the Agent Bakery.
...
In the intended rule in the [.guihint]#Microsoft SQL Server (deprecated)# section, ideally select the [.guihint]#System Authentication# option for a simple and problem-free connection.
