...
// -*- coding: utf-8 -*-
// IGNORE % GBit check_mk MBit lt pre →
...
[TIP]
====
The new registration is made in addition to the existing one, the registration shown in the xref:write_check_plugin[previous chapter] remains unchanged.
====
...
The overall status of the service is then based on the worst partial result in the order {OK} → {WARN} → {UNKNOWN} → {CRIT}.
...
// This block replaces the following complete(!) chapter
// start translation
////
[#rule_set]
...
[TIP]
====
Be aware that the examples presented in this section are _not_ covered by the Check API.
====
...
If you want to give the users of the rule set values instead of empty input fields, you can do this by adding the `default_value` argument to the `percentage()` function.
...
[IMPORTANT]
====
Once everything is ready and working correctly, be sure to remove the `print` commands again, as these can confuse {CMK}'s internal communication.
====
...
[TIP]
====
On GitHub you will find both the file with the
link:https://github.com/Checkmk/checkmk-docs/blob/master/examples/devel_check_plugins/check_plugin_advanced_myhostgroups_parameters.py[rule set^]
as well as the rule set-enhanced
link:https://github.com/Checkmk/checkmk-docs/blob/master/examples/devel_check_plugins/check_plugin_advanced_myhostgroups.py[check plug-in^].
====
...
[TIP]
====
The examples presented in this chapter are _not_ covered by the Check API.
====
