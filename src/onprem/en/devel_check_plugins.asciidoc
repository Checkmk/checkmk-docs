...
{CMK} provides a *Check API* for programming check plug-ins.
In {CMK} version {current-major}, the Check API V2 is the current version.
In this article, we will show you how to use Check API version 2 for the programming of plug-ins.

You can access the Check API documentation at any time via the {CMK} user interface: [.guihint]#Help > Developer resources > Plug-in API references#.
...
[TIP]
====
Even without a running {CMK} site, you can view a copy of the plug-in API documentation at link:https://docs.checkmk.com/plugin-api/{current-major}/[docs.checkmk.com/plugin-api^].
====
...
An executable basic framework (link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_simple_myhostgroups_bare_minimum.py[Download at GitHub^]), which you will expand step by step in the following, looks like this:
...
The link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_simple_myhostgroups.py[check plug-in^]
and as well as the link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/agent_plugin_simple_myhostgroups[agent plug-in^]
...
First, try the xref:wato_services#commandline[service discovery] with the `-I` option.
Adding the `v` option (for _verbose_) will generate detailed output.
...
If everything went as expected, you can xref:glossar#activate_changes[activate the pending changes].
If not, you will find helpful information in the chapter on xref:errors[troubleshooting].

Finally, generate an updated configuration of the monitoring core and xref:cmk_commandline#commands_core[restart it]:
...
The extended link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/agent_plugin_advanced_myhostgroups[agent plug-in^]
and the extended file for the link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_advanced_myhostgroups_step01.py[check plug-ins^]
...
As the new `myhostgroups_advanced` check plug-in provides new services, you must perform a service discovery for this check plug-in and activate the pending changes in order to see these services in the monitoring:
...
To make the rule set visible, you must restart the Python processes that provide the graphical user interface.
This is handled by restarting the Apache web server:
...
After that, the rule set can be found in [.guihint]#Setup# on the page mentioned above.
However, you will only be able to find the default value using the search function in the [.guihint]#Setup# after restarting Redis:
...
link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/ruleset_myhostgroups.py[rule set^]
as well as the 
link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_advanced_myhostgroups.py[check plug-in^] extended by the rule set.
====


[#testing_the_extended_plugin]
=== Testing and enabling the extended check plug-in
...
.Faster restart for experts
[%collapsible]
====
If you have a thorough understanding of the {CMK} process structure and know exactly which components will be affected by changes to your plug-in, you can limit the restart to specific services.
For an overview, see the article xref:cmk_processes#[Site services].
====
...
The graphing file for this chapter can be found at link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/graphing_myhostgroups.py[GitHub^].
...
// MFS: Luckily, no one in the team is working from Büsingen...
...
[TIP]
====
The debug object changed its path between {CMK} {v23} and {v24}.
Instead of the previous location `cmk.utils` it can now be found in `cmk.ccc`.
...
[TIP]
====
Please note that the symbolic links `~/lib/check_mk` [.fa-light.fa-arrow-right-long]#&nbsp;# `~/lib/python3/cmk` and `~/local/lib/check_mk` [.fa-light.fa-arrow-right-long]#&nbsp;# `~/local/lib/python3/cmk` link:https://checkmk.com/werk/17969[do not exist anymore^] since {CMK} {v25}.
In case you have previously deployed extensions to this directories using custom made scripts, adjust these scripts accordingly.
====
