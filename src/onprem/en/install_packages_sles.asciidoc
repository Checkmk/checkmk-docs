...
[%collapsible]
====
{CMK} allows xref:omd_basics#[multiple sites] to run in parallel on a single server.
To achieve this, for each site an Apache web server is used, running on a high port under the site user.
The system Apache, running on the standard ports 80/443 (when xref:omd_https#[HTTPS is enabled]), acts as a _reverse proxy_ to access the site Apaches.
