...
To achieve this, an Apache web server is used for each site, running on a high port under the site user.
...
Use the YaST GUI tool or the `systemctl` command to enable and start Apache:
...
// MFS: This should probably be forked into two: one for yum and one for zypper:
include::include_uninstall_rpm.asciidoc[]
