...
= The {SE}
:revdate: 2024-05-07
:title: The Standard Edition
:description: The different {CMK} editions offer different feature sets. This article explains the Standard Edition.
...
|SNMP monitoring |Thanks to a dedicated xref:snmp#[SNMP engine] _(inline SNMP)_, the commercial editions are more performant than the {RE} with the use of `snmpget`/`snmpbulkwalk`.
...
|{CMK} Synthetic Monitoring |The xref:robotmk#[{CMK} Synthetic Monitoring with Robotmk] is only available in the commercial editions, but requires an additional subscription.
