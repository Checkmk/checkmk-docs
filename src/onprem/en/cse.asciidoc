...
To do so, follow the xref:upgrade#[upgrade description].

A downgrade is also possible: from {UE} to {SE} and from {SE} to {RE}.
To do so, follow the xref:upgrade#down2cee[downgrade description].
...
On a small scale—that is, with a single site and up to 750 services—{CMK} as a commercial edition can also be operated permanently beyond that period in license state 'Free' without requiring a subscription.
