...
{cee-only}
Alert handlers are scripts you write yourself, that are run for you by {CMK} in the commercial editions if a problem is detected--or more precisely--if a host or service changes its status.
...
[TIP]
====
Unlike other places in {CMK} *every* alert handler rule will be executed if the conditions are matching.
...
* Rules in [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Linux/UNIX agent options > Remote alert handlers (Linux)# that generate SSH keys.
...
Define a rule under [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Linux/UNIX agent options > Remote alert handlers (Linux)#.
To create this rule, you will need the [.guihint]#Rule sets# and [.guihint]#Add or modify executables# xref:wato_user#roles[permissions].
