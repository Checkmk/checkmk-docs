...
[TIP]
====
This article is currently being revised and translated for the update to the new APIs of {v23}.
Therefore, currently the text and the linked example files on GitHub do not match.
====
// delete to here
...
Vom gelegentlich gesehenen `import pass:[*]`, raten wir ab, da es zum einen unnötig viel Speicher nutzt und zum anderen verschleiert, welche Namensräume tatsächlich verfügbar gemacht werden.
...
An executable basic framework (link:https://github.com/Checkmk/checkmk-docs/blob/master/examples/devel_check_plugins_snmp/flintstone_setup_check_bare_minimum.py[download from GitHub^]), which you will expand step by step in the following tutorial, looks like this:
...
// This block replaces the following code block: just replace without translation
////
.~/local/lib/python3/cmk_addons/plugins/flintstone/agent_based/flintstone_setup_check.py
[{python}]
----
...
Die Erstellung des Check-Plugins geschieht exakt wie bei den xref:devel_check_plugins#register_check_plug-in[agentenbasierten Check-Plugin] beschrieben.
////
// end translation
// delete from here
[#register_check_plug-in]
