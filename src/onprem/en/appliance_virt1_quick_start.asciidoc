...
:description: Setting up the virtual {CMK} Appliance involves a series of steps. Read here how to prepare your virtual appliance for use.
:experimental:
...
This quick start guide takes you from setting up the virtual appliance using a minimal configuration, to the login screen of a functioning {CMK} site.
...
. Download the {CMK} virt1 as an OVA (Open Virtualization Appliance) file from our link:https://checkmk.com/download[download page^].
For VMware ESXi and Proxmox, you need the OVA file (Open Virtualization Appliance), and for Hyper-V, you need the ZIP file.
Then import the file into your hypervisor.

.. For VMware ESXi: For better performance, you should change the network card to VMXNET 3.
xref:appliance_install_virt1#vmxnet[More ...]
.. General: The virtual hard disk on which {CMK} is ultimately installed and on which all sites and their data are subsequently stored is preset to a size of 100 GB.
Consider carefully whether this size is sufficient for your needs and, if necessary, increase the size of the virtual hard disk *before* starting virt1 for the first time.
...
Press the kbd:[F1] key to call up the configuration menu and edit at least the following settings:
