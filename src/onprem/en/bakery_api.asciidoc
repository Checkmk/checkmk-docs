...
You can therefore find the API documentation directly in your {CMK} site.
If you do not currently have a {CMK} site running, you can view a copy of the Bakery API documentation at link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/[docs.checkmk.com/plugin-api^].
...
Each plug-in explicitly declares the API version on which it is based when xref:access_api[accessing the API].
As of {CMK} {v20}, link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/#version-1-cmk-base-plugins-bakery-bakery-api-v1[version 1^] is the current version of the Bakery API, which is described in this article.

{CMK} {v25} introduces link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/#module-cmk.bakery.v2_unstable[version 2^] of the Bakery API, initially with the suffix _unstable_. 
Therefore, versions 1 and 2 can still be used in parallel in {v25}.
This means that plug-ins created in accordance with this article will still work at least in {CMK} {v25}, and very likely beyond that as well.

[TIP]
====
Since some import paths for APIs in the commercial editions have been moved in {CMK} {v25}, you may need to make modifications to your plug-ins.
This is the case if, contrary to xref:access_api[our recommendations], you are using absolute imports instead of relative ones.
Change these to relative imports before testing your Bakery plug-ins with {CMK} {v25}.
====

The Rulesets API used for the xref:ruleset[configuration] of Bakery plug-ins in the {CMK} GUI follows its own version history.
For this part of your extension packages, refer to our article on xref:devel_check_plugins#[developing agent-based check plug-ins].


[#testing]
== Testing guidelines

If you are developing Bakery plug-ins, you will want to test them regularly.
To make changes available in {CMK}, two steps are required.

First, use `cmk-validate-plugins` to ensure that all plug-ins are syntactically correct and can be loaded by {CMK}:
...
In the case of Bakery plug-ins, {CMK} now recognizes the new files, but cannot determine the bake requirements until the corresponding xref:mkps#[MKP] version numbers have been updated.
Since it can be very tedious to constantly increment version numbers while programming a Bakery plug-in, we recommend switching to the command line and using the `cmk` command there.
The `-f` switch forces the agent for `myserver123` to be baked:
...
You can now copy the newly-created agent to a host and install it there to test the extensions you have added.
...
An example of such a minimal rule set, which only activates the distribution of a plug-in, could look as follows.
...
After saving the rule set, check that all plug-ins are valid:
...
The rule set will now be visible in the {CMK} interface.
The GUI resulting from this rule set is shown in the following screenshot:

image::bakeryapi_settings.png[alt="GUI for the rule set used to configure the plug-in."]
