...
Note, that the Bakery API does not provide functions for configuring the Bakery plug-in, i.e. for creating the associated rule set, nor for the _contents_ of the files provided with the plug-in, e.g. the agent plug-ins.
...
// start translation
////
Sie finden daher die API-Dokumentation direkt in Ihrer {CMK}-Instanz.
Ohne laufende {CMK}-Instanz können Sie sich die Bakery-API-Dokumentation als Kopie auf link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/[docs.checkmk.com/plugin-api^] ansehen.
////
// end translation
...
The API with its documentation is versioned using two-level numbering system based on the basic principles of the link:https://semver.org/[Semantic Versioning 2.x^]  specification in the format `X.Y`, where `X` stands for a _major version_ and `Y` for a _minor version_. 
...
This means, plug-ins created following this guide will continue to function at least in {CMK} {v25}, and most likely beyond that.

[TIP]
====
The Rulesets API which is used for the xref:ruleset[configuration] of Bakery plug-ins in the {CMK} GUI follows its own version history.
...
link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/#version-1-cmk-base-plugins-bakery-bakery-api-v1[Version 1^] ist die seit {CMK} {v20} aktuelle Version der Bakery-API, die in diesem Artikel beschrieben wird.

{CMK} {v25} führt link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/#module-cmk.bakery.v2_unstable[Version 2^] der Bakery-API ein, zunächst mit dem Zusatz _unstable_ (instabil). 
Deshalb können die beiden Versionen 1 und 2 auch in {v25} noch parallel benutzt werden.
...
[TIP]
====
Da in {CMK} {v25} einige Import-Pfade für APIs der kommerziellen Editionen verschoben werden, ist es möglich, dass Sie Anpassungen an Plugins vornehmen müssen.
Das ist dann der Fall, wenn Sie entgegen xref:access_api[unseren Empfehlungen] absolute statt relativer Imports verwenden.
Ändern Sie diese auf relative Imports ab, bevor Sie Ihre Bakery-Plugins mit {CMK} {v25} testen.
====
...
[TIP]
====
If you want to develop and test plug-ins independently of a {CMK} site, you can access a copy provided at link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/[docs.checkmk.com/plugin-api^].
The documentation generated with link:https://www.sphinx-doc.org/[Sphinx^] displays all API documentation relevant for the development of {CMK} plugins, 
that means, in addition to the Bakery API documentation, you will also find documentation for the Check API, for example.
====
// delete to here
...
Wenn Sie Bakery-Plugins programmieren, werden Sie diese immer wieder testen wollen.
Um Änderungen in {CMK} verfügbar zu machen, sind zwei Schritte notwendig.

Zuerst stellen Sie mit `cmk-validate-plugins` sicher, dass alle Plugins syntaktisch korrekt sind und von {CMK} geladen werden können:
...
Anschließend starten Sie Ihre Instanz neu:
...
Im Fall von Bakery-Plugins kennt {CMK} jetzt zwar die neuen Dateien, kann aber keinen Backbedarf ermitteln, solange keine zugehörigen xref:mkps#[MKP]-Versionsnummern geändert wurden.
Da es während der Programmierung eines Bakery-Plugins sehr lästig sein kann, ständig Versionsnummern zu inkrementieren, empfehlen wir, auf die Kommandozeile zu wechseln und dort `cmk` zu verwenden.
Der Schalter `-f` erzwingt hier das Backen des Agenten für `myserver123`:

[{shell}]
----
{c-omd} cmk -fAv myserver123
...
Den frisch gebackenen Agenten können Sie nun zu einem Host kopieren und dort installieren, um die hinzugefügten Erweiterungen zu testen.
////
// end translation
...
* For Linux and Solaris there is an additional program `some_binary`, which should be included.
This is, for instance, a small shell script with which a plug-in can be started by a command, independently of the {CMK} agent.
...
[TIP]
====
If you want to port rule sets that were supported up to {CMK} {v23} to the new programming interfaces, open the link:https://docs.checkmk.com/2.2.0/en/bakery_api.html#ruleset[this article in the User Guide for version {v22}^] in a new tab or window.
...
// start translation
////
Nachdem Sie den Regelsatz gespeichert haben, prüfen Sie die Validität aller Plugins:
...
Starten Sie dann Ihre Instanz neu:
...
Jetzt ist der Regelsatz in der {CMK}-Oberfläche sichtbar.
////
// end translation
...
The `hello_world.py` plug-in file is stored in the local part of the site directory structure at `~/local/lib/check_mk/base/cee/plugins/bakery/s`.
...
Make sure to define keys and data types as defined in the xref:ruleset[rule set created above].
...
Make sure to convert the passed interval to `Integer` since rule set and bakery use different data types.
...
|`~/local/lib/python3/cmk_addons/plugins/<family_name>/rulesets/` |Directory for the rule set files for configuring the agent plug-in (in the example `ruleset_hello_world_bakery.py`) and also the associated check plug-in (e.g. for setting thresholds).
Therefore, choose meaningful names to be able to tell the files apart.
Names of files stored here have to start with `ruleset_`, otherwise {CMK} will ignore those.
|===
