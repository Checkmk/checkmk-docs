...
You can also find a thoroughly commented example of this procedure in the {CMK} Exchange: the link:https://exchange.checkmk.com/p/hello-world[Hello world!^] MKP, which is closely based on the example scenario presented in this article.
...
However, in the example scenario, we have specified that an execution interval should be chosen and the two configuration variables `user` and `content` should be set.
...
|`local/lib/python3/cmk_addons/plugins/<family_name>/rulesets` |Directory for the rule set files for configuring the agent plug-in (in the example `hellobakery_bakery.py`) and also the associated check plug-in (e.g. for setting thresholds). Therefore, choose meaningful names to be able to tell the files apart. Names of files stored here have to start with `ruleset_`, otherwise {CMK} will ignore those.
|===
