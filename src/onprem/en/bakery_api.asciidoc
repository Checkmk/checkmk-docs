...
[TIP]
====
Even if the _Bakery_ is only available in the commercial editions, the _API_ has been available in all editions starting with {CMK} {v23}. For the installation of packages that make use of the Bakery API on {RE}, it means that the additional functionality is simply ignored.
This gives you the option of creating extension packages that can be installed on all editions.
====
