...
* Observability: +
Observability extends the traditional monitoring of complex IT systems by analyzing data types (such as metrics or logs), with the goal of not only identifying _when_ a system is malfunctioning, but also _why_. In {CE-onprem}, data can be obtained via the link:https://opentelemetry.io/[OpenTelemetry^] standard.
...
|{CMK} Relay |The xref:relay#[{CMK} Relay] is a remote unit on a {CMK} site that enables monitoring via xref:glossar#snmp[SNMP] or xref:glossar#special_agent[special agents] of hosts that are not themselves accessible from the Checkmk site, for example because a firewall configuration blocks incoming traffic for a network.
...
|Metric backend |{CMK} uses—currently—_ClickHouse_ as xref:metric_backend#[metric backend] only for OpenTelemetry metrics; its column-based database can process incoming OpenTelemetry metrics very efficiently.
|===
