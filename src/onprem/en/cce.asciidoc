...
In the following short article we would like to introduce {CCE}, one of the commercial xref:glossar#edition[editions].
...
xref:managed#[{CME}] is then based on {CE}.
...
* Only {CE} is available through the xref:install_azure#[Microsoft Azure] and xref:install_aws#[AWS] marketplaces.
...
|Push mode |In {CE} the xref:wato_monitoringagents#agents[{CMK} agent] for Linux and for Windows can be configured and used in xref:glossar#push_mode[push mode].
...
|Testing the Edition |You can test {CE} in full functionality for 30 days without obligation.
