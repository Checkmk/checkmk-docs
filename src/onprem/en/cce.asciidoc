...
{CE-onprem} is the basis for the xref:managed#[{ME}] on the one hand, and on the other for {CCE-saas}, which provides {CMK} as _Software as a Service (SaaS)_ in the cloud.
...
You can upgrade {RE} or {SE} to {UE}.
