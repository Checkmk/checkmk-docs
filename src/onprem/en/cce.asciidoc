...
In the following short article we would like to introduce {CCE-onprem}, one of the commercial xref:glossar#edition[editions].


[#editions]
== Overview of {CE-onprem}

{CE} is available in two variants: as {CE-saas} and {CE-onprem}.
While {CE-onprem} is one of the _on-premises_ editions of {CMK}, all of which require you to set up your own {CMK} server, with {CE-saas} the {CMK} server is already running in the AWS Cloud.

The {CE-saas} software is based on that of {CE-onprem}.
...
As an on-premises edition, {CE-onprem} is based on xref:cse#[{CSE}] in terms of content, but additionally offers extensive cloud functions.
...
In addition, {CE-onprem} also provides cloud-specific xref:glossar#check_plugin[check plug-ins].
In this way you can continue to use {SE} when transitioning from a data center to a cloud.
If you would subsequently also like to use the SaaS and PaaS products that a cloud can additionally provide, you can upgrade to {CE-onprem}.

xref:managed#[{CME}] is then based on {CE-onprem}.
...
[TIP]
====
Whenever there is no possibility of confusion with {CE-saas} -- as in the other articles in this {CMK} User Guide -- we simply call {CE-onprem} the {CE}.
====
