...
.The standard checks in the {RE}
image::analyze_config.png[alt=“Result from the ‘Analyze configuration’.”]

The above screenshot shows the checks that are executed by default for a single site in the {RE}.
However, this is not the whole story, as more checks may well be offered.
What these are depends, for example, on whether you have installed the {RE} or one of the commercial editions, whether your site is used in a distributed monitoring, whether a specific rule has been set up, an LDAP connection exists, a global setting has been defined -- or even the value of an option in a rule.
