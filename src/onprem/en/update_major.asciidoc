...
Furthermore – depending on the proportion to the total number of checks – more extensive checks, especially in the cloud area, can result in further additional load, so that a total of 10-15% (in extreme cases around 20%) more CPU load can be expected.

*What do you have to do?*
Make sure that there is sufficient free capacity available.
As a rule of thumb: If the _load_ is below _number of processor cores × 0.8_ and the CPU Utilization is below 70{nbsp}%, no problems are to be expected.
...
The legacy `check_http` contained basic functionality for checking certificates and the `check_httpv2` mentioned in the previous section improves on this.
However, both have a few things in common:
First of all, they are only applicable if a certificate is used at an HTTPS endpoint, and there is no option for a detailed examination of the certificates, for example for contained _Certificate Subject Alternative Names_ or the issuing authority.

We have implemented the resulting user requests with the new `check_cert`.
All settings for the new [.guihint]#Check certificates# can also be found under [.guihint]#Setup > Services > HTTP, TCP, Email, ...# in the [.guihint]#Networking.# box. 
