...
When using OpenTelemtry, add at least 8GB additional RAM, since the xref:metric_backend#[metric backend] needs this for efficient operation.

|*Disk space* |The hard disk space requirement increases, especially when using OpenTelemetry.
The amount of additional disk space needed depends on the OpenTelemetry data ingested.
