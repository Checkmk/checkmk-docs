...
If you want to use {CMK} version {v24} on the {CMK} Appliance, you will need firmware version 1.7 of the appliance.
Firmware version 1.6 of the {CMK} Appliance is still supported by {CMK} {v23}, but no longer by {CMK} {v24}.
...
.What will you need to do? Update {CMK} and the appliance's firmware in the correct order
[%collapsible]
====
If your appliance is still running firmware version 1.6, proceed as follows:

. Update the appliance's firmware to version 1.6.9.
. Update {CMK} to version {oldstable-patch}.
...
. Replace the now-incompatible {CMK} version {oldstable-patch} for appliance version 1.6 with the same {CMK} version {oldstable-patch} for appliance 1.7.
{CMK} installation packages for appliance version 1.7 include `-4` in their names.
...
Step 4 is optional but strongly recommended, because if you encounter update issues in step 5, you can continue using {CMK} {oldstable-patch} seamlessly for the time being.
