...
You can find out which Linux distribution versions {CMK} supports in the xref:update_matrix#[update matrix for {v23}] and on the link:https://checkmk.com/de/download[download page^] after you have selected the {CMK} version and your Linux distribution.
...
*Does this affect you?*
Yes, as a user of SLES 15SP3/4. The update is somewhat more complex, as package sources for the outdated version 7 must be deactivated and activated for the new version 8.
...
Then prepare the package sources for the update to {v22}p21 (or higher), which is mandatory for the installation of {CMK} {v23}, as described in link:https://checkmk.com/de/werk/16025[Werk pass:q[#]16025^].
...
*Does this affect you?*
This only affects users of {CE}, other forms of distributed monitoring are not affected.

*What do you need to do?*
As the update ultimately corresponds to the xref:update_distributed_monitoring[procedure in distributed environments with distributed setup], it must be ensured that the last updated central site (using {CMK} {v22}) can still transfer the license information to the already updated remote sites (on {CMK} {v23}).
As the necessary changes are only implemented in {CMK} {v22}p17 (link:https://checkmk.com/de/werk/16193[Werk pass:q[#]16193^]), you must therefore update the central site to this version at least.
This ensures the transfer of the xref:license#license_cce[license information] and the remote sites can be updated to {v23}.
We recommend updating all sites to the latest available patch release before updating to {v23}.
...
[#agentwindows]
=== The agent for Windows: Deprecated Plugins

_TBD: Deprecated Plugins!_


Some plugins that were marked as _deprecated_ have been removed and may require manual installation if needed: link:https://checkmk.com/de/werk/16359[Werk pass:q[#]16359^]

[#pythonwindows]
=== The agent for Windows: Python 3.4 removed

Up to version {v22}, {CMK} offered the option of supplying a Python 3.4 runtime environment for Windows agents via an agent bakery rule.
This option is no longer available in {v23}.

*Does this affect you?*
Hopefully not: The removal only affects you if you use agent plugins written in Python on Windows Server 2008 R2 (EOL already in 2020).
Current Python versions run on all newer Windows versions.
All older Windows versions - such as Windows Server 2008 R1 or Windows Vista - no longer support {CMK} in version {v22}.
...
For these systems, navigate to the rule [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Windows modules > Install Python runtime environment# and change the setting [.guihint]#Install Python runtime environment > Installation# to [.guihint]#Never install Python with the agent#.
...
link:https://github.com/Uninett/mod_auth_mellon[mod_auth_mellon^] is a software module for Apache that provides services for authentication (_authentication_) via Secure Assertion Markup Language (SAML).
The connection to SAML authentication services was possible in {CMK} {v22} in two ways:
At Apache level with mod_auth_mellon (the only supported option until version {v21}) or in the commercial editions of {CMK} xref:saml#saml_cee[built-in SAML authentication].
With version {v23} mod_auth_mellon is no longer delivered with the {CMK} software.

*Does this affect you?*
This affects you if you use the {RE} or use SAML authentication in the commercial editions with mod_auth_mellon.
...
If you are using {RE}, we recommend deactivating SAML authentication _before_ the update.
You can then install `mod_auth_mellon` manually from the source code _after_ the update and reactivate it.
...
These have since been replaced by improved counterparts in the Kubernetes agent (see link:https://checkmk.com/de/werk/14572[Werk pass:q[#]14572^]).
In addition, in the [.guihint]#Prometheus# and [.guihint]#Alertmanager# rules, the specification of IP address/host name, port and path prefix has been replaced by a single input field [.guihint]#Custom URL# (see link:https://checkmk.com/de/werk/14573[Werk pass:q[#]14573^]).
...
As central sites with {CMK} {v22} can hold packages for {v23} and distribute them to remote sites, this feature also helps when updating in xref:distributed_monitoring#[distributed environments] with a xref:glossar#distributed_setup[centralized configuration].


[#apis]
=== Programming interfaces
// TK: Something about the migration to the new APIs must be included in this context
...
If changes are necessary, we recommend migrating to the xref:outlook_newapi[new APIs], which are available from 
{CMK} {v24} completely replace the old APIs.


[#rest_api]
=== Discontinuations in the REST-API

Discontinued endpoints or parameters of the {CMK} REST API are marked in the API documentation with the icon icon:icon_rest_api_deprecated[alt="Icon to indicate a discontinuation in the REST API documentation].
...
Search for `Deprecated` in the browser on the page, check whether you are using discontinued elements in your scripts and replace them before updating.
...
Before performing the update is the right time to check for any changes from the previous major version {v22} that may have an impact on the update.
Since {CMK} {v23}, such works are no longer dragged on, but deleted during the update after showing a confirmation dialog (link:https://checkmk.com/de/werk/15292[Werk #15292^]).

It is also a good idea to get an overview of the incompatible changes *before* the update:
Open the list of link:https://checkmk.com/de/werks?search=&cmk_version$$[$$$$]$$=2.3&compatible=0[Werks^] on the {CMK} website.
...
If it is certain that a work has no impact or you have made the required changes, confirm it with [.guihint]#Acknowledge#.
...
The update to {v23} currently requires at least {v22}p23.
...
Until {CMK} {v22}, the Windows agent relied on the additional plugin link:https://checkmk.com/integrations/winperf_if[winperf_if] to correctly transmit the status of network interfaces. Without the plugin, the status was always _up_. As of {CMK} {v23}, the agent can transmit the correct status on its own (link:https://checkmk.com/de/werk/15839[Werk pass:q[#]15839^]).

*Does this affect you?*
Depending on the extent to which you have distributed the plugin and whether you have deactivated the monitoring of the status of network interfaces for hosts without a plugin, this may affect you.
Hosts that have not previously used a plugin and whose network interfaces were included in the monitoring will change many interfaces from _up_ to _down_, resulting in the status {CRIT} and triggering notifications.
...
Take the opportunity to first implement the recommendations of the blog article link:https://checkmk.com/blog/network-monitoring-with-checkmk-2-0[_"Network monitoring with Checkmk: 3 rules to rule them all"_], at least for your Windows hosts.
For Windows, the settings mentioned are usually [.guihint]#Network interfaces on Windows#.


[#cmeccelicense]
=== Synchronize license information

With {CMK} {ME} and {CE} with distributed setup, license information is sometimes not synchronized to remote sites during the update, this must be checked and initiated manually if necessary.
...
{CMK} {v23} contains a new active check for HTTP connections and certificates, which is significantly better performing, more robust and easier to configure than the previous `check_http`.
...
The new check [.guihint]#Check HTTP web service#, which can be found in the [.guihint]#Networking# category, will completely replace the old one ([.guihint]#SetupServices > HTTP, TCP, Email, ... > Check HTTP service#) in the medium term.
For this reason, we recommend that you no longer use the previous check when creating new hosts and gradually migrate existing hosts to the new check.

Migration aids will be provided during the life cycle of {CMK} {v23}, but a completely automated migration tends to be difficult because for many hosts rules from two checks have to be merged.

Further information is provided in link:https://checkmk.com/de/werk/15514[Werk pass:q[#]15514^].

[#outlook_newapi]
=== Programming interfaces

After {CMK} version {v23} only the well-specified APIs for programming check plugins, special agents, graphing... will be supported.
...
An overview of the biggest changes can be found at link:https://checkmk.com/de/werk/16259[Werk pass:q[#]16259^].

As the previous APIs are no longer supported from {CMK} {v24}, all plugins that use the older APIs must be migrated to the new APIs before the update to {v24}.
...
Support for Redfish compatible management boards will be added and improved during the lifecycle of {CMK} {v23}, initially as an optional MKP (link:https://checkmk.com/de/werk/16589[factory pass:q[#]16589^]).
We explain additional background information and alternatives in the link:https://checkmk.com/blog/monitoring-management-boards[blog article "Monitoring management boards"].

[#automation_login]
=== Authentication via GET parameter

Up to {CMK} {v22}, every automation user could be logged in with a user name and password passed via GET parameters.
This option will be reduced from {CMK} {v23} and eventually removed completely (link:https://checkmk.com/de/werk/16223[Werk pass:q[#]16223^]).
In the medium term, you must therefore switch your scripts to authentication via HTTP headers.

First, {v23} makes the option of logging in via GET parameters configurable.
However, the setting [.guihint]#Enable automation user authentication via HTTP parameters# will initially use [.guihint]#enabled# as the default.
In {CMK} {v24}, the default will then change to [.guihint]#disabled#.
Finally, the option of logging in via GET parameters will be completely removed with {CMK} {v25}.

[#ntopng56_deprecation]
=== Ntopng 5.6 and older no longer supported

{CMK} {v23} will be the last version to support Ntopng in versions 5.x and 6.x (link:https://checkmk.com/de/werk/16483[Werk pass:q[#]16483^]).
With {CMK} {v24}, support for Ntopng 5.x will be discontinued and only Ntopng 6.x will be supported.
Please update your Ntopng installations to 6.x during the lifecycle of {CMK} {v23}.
