...
====
_Work in progress:_
This article is subject to constant change until the release of the final version of {CMK} {current-major}.
...
[TIP]
====
Contrary to the previous assumption of the author of these lines, this also affects you if you use the active check `check_sql` to monitor Oracle databases.
Up to {CMK} {v23}p27 you had to install the `oracledb` module in order to be able to use this check. 
Since {v23}p28 the module is link:https://checkmk.com/werk/17370[supplied with {CMK}^].
====
...
[TIP]
====
If you have activated the included MKP for the Redfish special agent, please deactivate it before updating.
As of {v24}b5 the presence of the package causes error messages.
====
...
Existing extensions that will no longer work after an update to {v24} will then lead to notifications being displayed repeatedly for all users with the authorization to manage MKPs (link:https://checkmk.com/de/werk/17649[Werk pass:q[#]17649^]).
...
[TIP]
====
If you use non-public APIs in plugins, test all the more intensively with {v23} and {v24}.
...
*Does this affect you?*
The risk of being affected is present if you monitor hardware with very long lifecycles, especially with special agents.
During the preparations for the update from {v23} to {v24}, for example, we noticed the special agents for many NetApp devices, which we had to switch from the discontinued ZAPI to the more modern REST API (link:https://checkmk.com/de/werk/16767[Werk #16767^]).
For licensing reasons, we had to move the agent for Dell Powerconnect to a separate link:https://exchange.checkmk.com/p/dell-powerconnect[package^] available in the Exchange (link:https://checkmk.com/de/werk/15359[work #15359^]).

*What will you need to do?*
If services disappear after the update, search the factory for the manufacturer of the monitored products.
In most cases, you will have to switch to new APIs or, in rare cases, use packages from the Exchange to monitor a specific hardware or software.
...
Since {v23} {CMK} contains a new active check for HTTP connections and certificates, which is significantly better performing, more robust and easier to configure than the previous [.guihint]#Check HTTP service#.
The new [.guihint]#Check HTTP web service# can be found in the [.guihint]#Networking# box under [.guihint]#Setup > Services > HTTP, TCP, Email, ...#. 
This one is significantly mor performant, robust and easier to configure than the old [.guihint]#Check HTTP service#.
In addition, it is now possible to monitor several things at once with a single rule, for example HTTP response code, response time and certificate validity.

In {CMK} {v24}, some rarely used options of the old check have been added to make migration easier.
Furthermore, we provide a migration script that maps existing calls and resulting services 1:1.
Use this to convert services from the old to the new active check, since with {CMK} {v25}, no new rules using the old check cannot be created anymore.
It will later be completely removed.

Even if the migration script does not cause any performance disadvantages, you can achieve significant performance advantages if you merge several rules, which consequently result in several executions of the check.
In many cases, you can also generate several services from one rule of the new check.
...
Details on possibilities, limitations and best practices for preparation and post-processing will be explained in a detailed blog article.

If you come across applications in which the new check _cannot_ replace the old one, you can display the command line used in the same way as link:https://checkmk.com/blog/how-monitor-servers-broken-tls-checkmk[procedure described here^].
You then have the option of calling the old check or a system-wide installed `check_http` via [guihint]#Setup > Other services > Integrate Nagios plugins#.
...
Up to {CMK} {v24}, management boards can be configured in two ways: As a property of a host or as a separate host.
...
In {CMK} in version {v24}, Redfish monitoring is an integral part of the software (link:https://checkmk.com/werk/17404[Werk pass:q[#]17404^]).
