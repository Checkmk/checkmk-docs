// -*- coding: utf-8 -*-
// IGNORE Alertmanager p17 p21 p23 p8 pre % × b6 0p12 Pre pX
...
[IMPORTANT]
====
When updating from {CMK} {v22}pX to the latest patch level of {v23}, update to version 2.3.0p12 as an intermediate step.
During the update to 2.3.0p12, make sure that any problems reported during the pre-flight check have been resolved.
It is usually sufficient to carry out the action suggested by {CMK}.
Only then update to a higher patch level of {v23}.
====
...
Furthermore -- depending on the proportion to the total number of checks -- the more extensive checks in {v23}, especially in the cloud area, can result in further additional load, so that a total of 10-15{nbsp}% (in extreme cases around 20{nbsp}%) more CPU load can be expected.

*What will you need to do?*
Make sure that there is sufficient free capacity available.
As a rule of thumb: If the _CPU load_ is below the _number of processor cores x 0.8_ and the CPU utilization is below 70{nbsp}%, no problems should be expected.
...
Some obsolete distributions will no longer be supported under {CMK} version {v23}.

*Does this affect you?*
This will affect you if any of the following Linux distributions -- still supported in {v22} -- is installed on your {CMK} server:
...
Make sure that the target version of the Linux distribution of {CMK} is {v22} *and* that {v23} is supported.
...
As central sites with {CMK} {v22} can hold packages for {v23} and distribute these to remote sites, this feature also helps when updating in xref:glossar#distributed_monitoring[distributed environments] with a xref:glossar#distributed_setup[distributed setup].
...
If changes are necessary, we recommend migrating to the xref:outlook_newapi[new APIs], which will be available from {CMK} {v24} and completely replace the old APIs.
...
As in every {CMK} version, there are also changes to the software in the current {v23} version which _may_ have repercussions on your {CMK} installation -- at {CMK} these are organized and documented in what we refer to as our {CMK} _Werks_.
...
Since {CMK} {v23}, such Werks are no longer carried over, but are deleted during the update after displaying a confirmation dialog (link:https://checkmk.com/werk/15292[Werk #15292^]).
...
As a prerequisite, before updating to version {v23}, version {v22} *must* have been installed on the {CMK} site.
...
[#automation_login]
=== Authentication via the GET parameter

Up to {CMK} {v22}, every xref:glossar#automation_user[automation user] could be logged in with a user name and password passed via the GET parameter.
...
First, {v23} makes the option of logging in via the GET parameter configurable.
...
Finally, the option of logging in via the GET parameter will be completely removed under {CMK} {v25}.
