...
====
_Work in progress:_
This article is subject to constant change until the release of {CMK} {v23}.
...
. Especially when updating in xref:distributed_monitoring#[distributed environments] with xref:glossar#distributed_setup[central configuration], you should take care to store both package versions with correct compatibility information for MKPs that require different versions for {CMK} {v22} and {v23}.
During the update, the remote instances will then automatically activate the appropriate version.
...
Take the opportunity to first implement the recommendations of the blog article link:https://checkmk.com/de/blog/network-monitoring-with-checkmk-2-0[_"Network monitoring with Checkmk: 3 rules to rule them all"_], at least for your Windows hosts.
