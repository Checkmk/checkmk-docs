...
As the necessary changes are only implemented in {CMK} {v22}p17 (link:https://checkmk.com/de/werk/16193[Werk pass:q[#]16193^]), you must therefore update the central instance to this version at least.
...
This update affects modules installed in an instance.
...
These files are located in the local part of the instance directory structure, i.e. in `~/local` and can be organized in extension packages or just "lying around".
Local files can cause problems during an update if they no longer match the new {CMK} version.

*Does this affect you?*
Since it is not possible for {CMK} to fully ensure the compatibility of local adaptations during an update, you should check your {CMK} instance before an update to see whether local files are used, which ones they are and how they are organized.
...
. Get an xref:mkps#find_unpackaged[overview] of _unpackaged_ local files of your {CMK} instance by executing the command `mkp find` as instance user.
...
As central instances with {CMK} {v22} can hold packages for {v23} and distribute them to remote instances, this feature also helps when updating in xref:distributed_monitoring#[distributed environments] with a xref:glossar#distributed_setup[centralized configuration].
...
As in every {CMK} version, there are alterations to the software in the current version {v23} that _may_ have repercussions on your {CMK} installation.
...
Open the list of link:https://checkmk.com/werks?search=&cmk_version$$[$$$$]$$=2.3&compatible=0[Werks^].
...
*What do you need to do?*
Under [.guihint]#Setup > General > Distributed Monitoring#, take a look at the overview of remote instances and search for instances marked with [.guihint]#License state: trial#.
To force an immediate synchronization for these instances, check the license under [.guihint]#Setup > Licensing# or save the license settings.
