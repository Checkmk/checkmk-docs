...
[#editions]
=== Selection of the {CMK} edition

// KNW-1291
_TBD: {CME} is now based on {CCE}._

A few things need to be taken into account for license management.
These will be added during the beta phase.
If you want to update an installation already in productive use with {v22} to {v23}, please contact our support.
There are no special things to consider for test installations during the 90-day test period.
...
With {CMK} version {v23} (and {v22}p21) on SUSE Linux Enterprise Server 15 Service Pack 3, the dependency on the PHP programming language runtime environment is increased from version 7 to version 8.

*Does this affect you?*
Yes, as a user of SLES 15SP3. The update is somewhat more complex, as package sources for the outdated version 7 must be deactivated and activated for the new version 8.
