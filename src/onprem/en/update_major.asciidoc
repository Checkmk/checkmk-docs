// -*- coding: utf-8 -*-
// IGNORE Alertmanager p17 p21 p23 p8 pre pX
...
[TIP]
====
From the update to {v23}, changes are rolled back if the update fails or is canceled with `Ctrl+C` (link:https://checkmk.com/werk/16408[Werk pass:q[#]16408^]).
Only when the text [.guihint]#Completed verifying site configuration. Your site now has version {v23}pX# is displayed, the updated configuration is active.
This does not replace a backup, but usually reduces the downtime if something goes wrong.
====
...
Migration assistance will be provided during the lifecycle of {CMK} {v23}, however a fully automated migration tends to be difficult because many hosts require rules from two checks to be merged in order to benefit from the improved efficiency.

Further information can be found in the two Werks link:https://checkmk.com/de/werk/15514[pass:q[#]15514^] and link:https://checkmk.com/de/werk/15515[pass:q[#]15515^].
