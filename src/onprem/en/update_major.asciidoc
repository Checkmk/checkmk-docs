...
You can find it under [.guihint]#SetupAgents > Windows, Linux, Solaris, AIX > Agent rules > Check MS SQL Server (Linux, Windows)#.
...
The configurability has been significantly improved, so you can now choose which sections are to be checked and add your own SQL statements.
Furthermore, databases on different hosts can now be monitored using a plugin.
To keep this clear, the piggyback mechanism can be used to assign databases to other hosts.
All configuration is done via a YAML file, where the stable options are available via Bakery rules.
