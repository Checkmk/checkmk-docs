...
That's why the `tests.robot` is referenced in the `tasks` area.
====

In addition to `conda.yaml`, the `environmentConfigs` area is used to determine the freeze file corresponding to the system.
The freeze files record which dependencies must be installed in which versions to obtain exactly identical environments.
If you want to use them, just move them from the directory `output` one directory up (where also the `conda.yaml` resides).
RCC will determine the correct freeze file automatically.
