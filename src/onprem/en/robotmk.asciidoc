...
The Robotmk keyword `Monitor Subsequent Keyword Runtime` is the marker to monitor (to trigger {CMK}) the following keyword (`Foobar`); more precisely, its runtime.
...
Under Windows, the Robotmk scheduler is executed by default as a sub-process of the {CMK} agent and runs in the `SYSTEM` context.
