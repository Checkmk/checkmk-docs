// -*- coding: utf-8 -*-
// IGNORE MyApp1 MyApplication_mybot Windows|Linux mybot_managed rf
// NONASCII
include::global_attr.adoc[]
= {CMK} Synthetic Monitoring with Robotmk (RCC)
:title: {CMK} Synthetic Monitoring with Robotmk (RCC)
...
By default, {CMK} uses RCC (originally developed by Robocorp) for this purpose--as described here in the article „{CMK} Synthetic Monitoring with Robotmk (RCC)“.
If RCC does not work for you, you can alternatively use our in-house tooling CSM/Micromamba--as described in the article xref:robotmk_mm#[{CMK} Synthetic Monitoring with Robotmk (CSM)].
====
...
{cee-only} {CMK} Synthetic Monitoring is available in the commercial xref:glossar#edition[editions], but it requires an link:https://checkmk.com/request-quote/synthetic-monitoring[additional subscription^].
...
// ML: Added right before conference
[IMPORTANT]
====
Development of the open-source tool RCC was discontinued some time ago by its developer Robocorp.
Nevertheless, RCC remains the recommended way to create environments for Robot Framework for the time being.
For cases where RCC does not work in your environment, a fallback solution developed by Checkmk called "CSM," based on Micromamba, is available and presented xref:robotmk_mm#[in a seperate version of this article].
====
...
image::robotmk_scheduler_00.png[alt="Empty Robotmk scheduler rule.",fullscreen=1]
...
First, the [.guihint]#RCC configuration#.
This allows you to specify proxy and TLS settings in the [.guihint]#RCC profile configuration# section.

Next is the path specification for [.guihint]#Base directory for ROBOCORP_HOME directories#  — for more information on ROBOCORP_HOME, see xref:#rccbackground[RCC--a brief degression.]

One final RCC option: [.guihint]#Allow agent deployment as root#.
Only with this option is the combination of RCC and a root-user agent possible!

.RCC configuration, valid for all sequences
image::robotmk_scheduler_08rcc.png[alt="The combination of a root agent and RCC configuration must be explicitly allowed!"]


[IMPORTANT]
====
By default, agents on Linux are installed under the root account.
If Synthetic Monitoring is enabled, this is prevented for security reasons—building the agent produces an error message (see link:https://checkmk.com/werk/19460[Werk 19460^]).
Accordingly, you should set a different user context using the [.guihint]#Customize agent package (Linux)# rule.
If you absolutely must run RCC under the root account, you can use the [.guihint]#Allow agent deployment as root# option in the Robotmk scheduler rule.
====


Last but not least: [.guihint]#Grace period before scheduler starts#.
The scheduler starts together with the {CMK} agent before the desktop logon--which, of course, means that any tests on the desktop must fail.
...
image::robotmk_monitorng_suite.png[alt="Status of the test suite in monitoring.",fullscreen=1]
...
image::robotmk_monitorng_test.png[alt="Status of the test in monitoring.",fullscreen=1]
...
In principle, you could simply archive the complete robot directory with the standard files (`robot.yaml, conda.yaml, test.robot`).
...
Under Linux, the scheduler runs under the same user as the agent.

[IMPORTANT]
====
By default, agents on Linux are installed under the root account.
If Synthetic Monitoring is enabled, this is prevented for security reasons—building the agent will result in an error message (see link: https://checkmk.com/werk/19460[Werk 19460^]).
Accordingly, you should set a different user context using the [.guihint]#customize agent package (Linux)# rule.
If you absolutely must run RCC under the root account, you can use the [.guihint]#Allow agent deployment as root# option in the Robotmk scheduler rule.
====
...
[{powershell}]
----
PS C:\robots> set ROBOCORP_HOME=C:\robotmk\rcc_home\current_user
----
...
[{powershell}]
----
PS C:\robots> set ROBOCORP_HOME=C:\robotmk\rcc_home\HarryHirsch
----
...
[{powershell}]
----
PS C:\robots> cd myrobot
PS C:\robots\myrobot> rcc holotree vars
----
...
----
PS C:\robots\myrobot> rcc holotree hash conda.yaml
...
[{powershell},subs="+quotes"]
----
PS C:\robots\myrobot> rcc holotree catalogs
...
[{powershell}]
----
PS C:\robots\myrobot> rcc holotree export -r robot.yaml --zipfile myrobot-env.zip
----
...
For container operation, there is now a project on the GitHub pages of Elabit, the developer of the original Robotmk plug-in, called link:https://github.com/elabit/rccremote-docker[RCCRemote-Docker^].
However, RCCRemote and RCCRemote-Docker are _not_ parts of {CMK} Synthetic Monitoring and *are not covered by {CMK} support*!
