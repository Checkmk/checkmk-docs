...
In the following, we would like to introduce {CME} (formerly {CMK} MSP), an extension of the commercial xref:glossar#edition[edition] {CUE}, which allows you, as a _managed service provider (MSP)_, to operate isolated sites for multiple customers using {CMK}'s xref:glossar#distributed_monitoring[distributed monitoring].
...
In terms of content, {ME} is based on xref:cce#[{CUE}] and is its multi-tenant-capable extension.
...
To do so, follow the xref:upgrade#[upgrade description] to {CE}.
For an upgrade to {ME} in distributed environments, only the xref:upgrade#offline_distributed[offline upgrade] can be used.
...
New in the [.guihint]#Main dashboard# is the [guihint]#Customers# widget, which is located to the left of the service problems:

image::managed_dashboard.png[alt="The 'Customers' widget in the 'Main' dashboard.",width=600]
...
Of course, you can also use the filters and data sets as they are used for the widgets and the snap-in for your own views.
