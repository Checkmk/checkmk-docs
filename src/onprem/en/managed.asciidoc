...
In the following, we would like to introduce you to {CME}, one of the commercial xref:intro_setup#editions[editions].
...
In a distributed setup, {CMK} in {RE}, {SE} and {CE} will therefore distribute all configuration data to all participating sites, 
...
With {CME}, via xref:distributed_monitoring#[distributed monitoring] a service provider (i.e. a managed service provider, MSP or provider for short)  for each of its customers integrates one or more remote sites into the provider's central site -- these remote sites can only be accessed by the relevant customer.
...
image::managed_distributed_monitoring_de.png[alt="Illustration of communication in {ME}'s distributed monitoring system."]
...
In terms of content, it is based on xref:cce#[{CCE}] and is its multi-client-capable extension.
