...
= The {ME}
:revdate: 2024-04-22
:title: The Managed Services Edition - Offering {CMK} as a service
:description: With the {CMK} Managed Services Edition you can operate separate sites for several customers via distributed monitoring.
...
In the following, we would like to introduce you to the {CME}, one of the commercial xref:intro_setup#editions[editions].

In xref:glossar#distributed_monitoring[distributed monitoring] with a xref:glossar#distributed_setup[distributed setup], users will generally log on to the _central site_, 
...
In a distributed setup, {CMK} in the {RE}, the {SE} and the {CE} will therefore distribute all configuration data to all participating sites, 
...
The provider can still carry out the configuration conveniently via the distributed setup of its own central site.
The provider also has a central web interface for all its customers to work with the monitoring data.
This works in exactly the same way as in a normal distributed environment, with the only difference being that *all* participating sites must use the {CME}.

image::managed_distributed_monitoring_de.png[alt="Illustration of communication in the {ME}'s distributed monitoring system."]

Important configuration data (e.g. remote sites and users) can be assigned to customers in the {ME}.
...
*Important*: You must use the {CME} if you use {CMK} to monitor not only your own infrastructure but also that of other organizations.


[#editions]
== Overview of the {ME}

The {ME} is currently the most comprehensive edition of {CMK}.
In terms of content, it is based on the xref:cce#[{CCE}] and is its multi-client-capable extension.
...
[#functions]
== Additional functions with the {ME}

The main function of the {ME} that distinguishes it from other editions is the ability to assign the following elements to customers:
...
To switch from one of the other editions to the {ME}, follow the xref:update#upgrade[upgrade description].
For an upgrade to the {ME} in distributed environments, only the xref:update#offline_distributed[offline upgrade] can be used.
