...
[TIP]
====
Unfortunately, we are currently stuck in Azure bureaucracy and cannot provide images with {CMK} {v23} preinstalled.
The latest images available in the Microsoft Azure Marketplace are still on {v22}.
You can use these images and xref:update#[update] them to {v23} immediately after provisioning.
...
Only the xref:cce#[{CE}] is used in such a scenario. 
