...
[TIP]
====
Unfortunately, we are currently stuck in Azure bureaucracy and cannot provide images with {CMK} {current-major} preinstalled.
The latest images available in the Microsoft Azure Marketplace are still on {v22}.
You can use these images and xref:update#[update] them first to {oldstable} and then to {current-major} immediately after installation.
However, a more straightforward solution is to use a vanilla Ubuntu 24.04 image and follow the steps to xref:install_packages_debian#[perform an installation] there.
====
...
{CMK} {v22} takes this a step further and provides link:https://azuremarketplace.microsoft.com/en-us/marketplace/apps/tribe29gmbh1665582614827.checkmk003?tab=Overview[a pre-installed image^] based on Ubuntu 22.04 _(Jammy Jellyfish)_ with all of the required dependencies already included.
