// -*- coding: utf-8 -*-
// IGNORE MyApp1 myapp1_mybot1 Test1 Windows|Linux mybot_managed rf Micromamba CSM RobotCode Venv autologin Notepad++
// NONASCII
include::global_attr.adoc[]
= {CMK} Synthetic Monitoring with Robotmk (CSM)
:title: {CMK} Synthetic Monitoring with Robotmk (CSM)
...
[IMPORTANT]
====
Synthetic Monitoring requires virtual environments on the hosts running the tests.
By default, {CMK} uses RCC (originally developed by Robocorp) for this purpose--as described in the article xref:robotmk#[{CMK} Synthetic Monitoring with Robotmk (RCC)].
If RCC does not work for you, you can alternatively use our in-house tooling CSM/Micromamba--as described here in the article „{CMK} Synthetic Monitoring with Robotmk (CSM)“.
====
...
Such an _automation package_ with the definition of the execution environment (`robotmk-env.yaml`), post-installation instructions (`robotmk-setup.yaml`) and the test suites (`tests.robot`) is also called a robot.
...
image::robotmk_managed_robots_scheduler_csm.png[alt="Configuration of the scheduler with a managed robot."]
...
To do this, the scheduler must first be stopped and an xref:glossar.html#scheduled_downtime[scheduled downtime] should be set in the monitoring to prevent any impact on the monitoring and to prevent the scheduler from restarting automatically.

On Windows, you can stop the service using the Task Manager. In Linux, you can stop and start the service in the terminal:
...
Even for ad hoc execution, you must first stop the currently running scheduler service and set a xref:glossar#scheduled_downtime[scheduled downtime] in the monitoring.
