...
Code examples created for this article are available in our link:https://github.com/Checkmk/checkmk-docs/tree/{gitbranch}/examples/devel_special_agents[GitHub repository^] and the finished Open-Meteo special agent is available as link:https://exchange.checkmk.com/p/ometemp[MKP in the {CMK} exchange^].
...
Once the files have been created, first check whether {CMK} can validate them:
...
If you want the new rule to be visible in the search immediately, you will need to restart the in-memory database _Redis_:
...
So that you will not need any specific hardware for this exercise, we will use the free API from link:https://open-meteo.com/[Open-Meteo.com^].
...
[TIP]
====
In “real life,” you'll frequently find yourself in phases where you'll be experimenting extensively with parameters and how they're passed.
To avoid having to update the rule and call configuration for every single change, no matter how small, you can also call the special agent on a trial basis as xref:datasource_programs#[classic data source program].
You can find the required rule under [.guihint]#Setup > Agents > Other integrations > Individual program call instead of agent access#.
====
...
After checking with `cmk-validate-plugins`, restart the site's web server:
...
Finally, we directly load the string as an object with `json.loads()`.
...
It is always a good idea to run a validation with `cmk-validate-plugins` if you have made changes to elements that are executed within {CMK} processes.
In this article, these include rules, call configuration, and check plug-ins.

If you have only made changes to the call configuration and the special agent itself, it is sufficient to generate a new configuration for the monitoring core and restart it.
...
If you have also modified rules or check plug-ins, first generate the configuration for the monitoring core and then restart the entire site:
...
The scripts described in this article for special agent, rule configuration, and call configuration, when used in combination with this check plug-in, should now provide you with a functioning service in {CMK}:
...
To get an initial idea of possible causes, you should first run `cmk-validate-config`.
The following sections will help you further isolate the source of a problem.
// delete to here
...
The [.guihint]#Configured API integrations and {CMK} agent# value must be selected for the [.guihint]#{CMK} agent / API integrations# parameter.
...
[#activate_changes]
=== Error messages when activating pending changes

There may also be a problem in the call configuration or in the actual special agent.
This becomes visible, for example, because a yellow warning is being displayed during [.guihint]#Activate pending changes#.
...
| `GENERAL` | _Everything that does not fit somewhere else_
