...
{CMK} provides several helper functions that are available in the namespace link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.server_side_programs/index.html#module-cmk.server_side_programs.v1_unstable[`cmk.server_side_programs.v1_unstable`^].
If you implement a special agent in Python 3 and it is always executed by a {CMK} site (some special agents are written in such a way that they can also be used as agent plug-ins) the API offers some interesting extensions.

[TIP]
====
As the import path suggests, this is a programming interface that has not yet been marked as stable.
We plan to release it as stable in {CMK} {v30}.
Currently, we are planning minor enhancements rather than incompatible changes.
Nevertheless, you should allow at least some time for testing when migrating your plug-ins from {v25} to {v30}.
====
...
Some REST APIs use _rate limiting_ to reduce the number of incoming requests to be processed.
Rate limiting is not always static and does not always use exactly the same allowed number of requests per unit of time.
In the case of dynamic rate limiting, the REST API should return the link:https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/429[HTTP status code 429^] and specify a wait time in the header field named _Retry-After_.
If the wait time is acceptable for monitoring purposes, it may be useful to fall back on cached older responses.
The `Storage` class is available for this purpose.

A `Storage` object is a key-value store that uses a tuple of two strings as a key.
The first string is the plug-in's identifier; the second can be, for example, the hostname—or, in cases where a `Storage` object can be shared across hosts, a URL where specific parameters trigger a specific response.
Only strings are allowed as values as well.
To store more complex objects, using JSON is usually a pragmatic approach.

An extension to the Storage API could therefore look like this:
...
If the HTTP status code 429 continues to be returned, the special agent will continue to use the `Storage` object in the future.
The extended special agent therefore also stores the time of the last successful REST API access.
In combination with a constant specifying the maximum allowed age (`MAXAGE` is one hour in the example), this ensures that if the data is too old, the agent simply does not produce any output, and the service status thus changes to {UNKNOWN} after that hour.


[#vcr]
=== Recording and playing back calls and responses

[TIP]
====
The _Video Cassette Recorder (VCR) for REST API Calls_ presented here is intended solely for debugging and testing purposes!
A forgotten trace file in a production environment will cause the same response to be returned indefinitely—and is sure to drive you crazy.
If you make the functionality described here visible to users in _Rule Specs_, clearly state in the inline help that this option should only be enabled upon request from support.
====

The rate limiting described in the last example can be particularly disruptive when developing check plug-ins that evaluate the output of special agents.
If you make the same call repeatedly within a short period of time, not only will rate limiting be a problem, but you'll often incur costs for REST API calls as well.

The library link:https://pypi.org/project/vcrpy/[`vcrpy` (_Video Cassette Recorder for Python_)^] comes in handy here, as it allows you to record HTTP requests and play them back over and over again. To integrate this, {CMK} link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.server_side_programs/index.html#cmk.server_side_programs.v1_unstable.vcrtrace[provides a function^] that you can activate via the argument parser.
If the `--vcrtrace` command-line option is used to specify the path to a trace file in `~/tmp/check_mk/debug`, the special agent will execute each HTTP request only the first time and record the response received.
If exactly the same HTTP request is to be made again later, this previously recorded request is served directly from the file without sending a new request to the server.

Without the `--vcrtrace` command-line parameter, the customized special agent continues to behave unobtrusively.


[#detect_errors]
== Troubleshooting

Even when developing your own custom agents, errors and problems can occur from time to time.
Identifying the causes of these issues is the first step toward resolving them.
For initial clues about possible causes, you should first run `cmk-validate-config`.
The following sections will help you further narrow down the problem.
