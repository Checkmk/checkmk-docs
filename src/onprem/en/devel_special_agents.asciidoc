...
By the way, the https://docs.checkmk.com/master/de/devel_special_agents.html[German version] of this article has already been updated -- if that is an alternative for you.
// delete to here
...
Vorbereitend sollten Sie daher drei Dateiverzeichnisse anlegen, die sich alle unterhalb von `local/lib/python3/cmk_addons/plugins/<name>` befinden.
...
Die Entwicklung dieser Plugins beschreibt der Artikel xref:devel_check_plugins#[Agentenbasierte Check-Plugins schreiben] ausführlich.
...
Für den Parameter [.guihint]#Checkmk agent / API integrations# muss die Option [.guihint]#Configured API integrations and Checkmk agent# gewählt sein.
