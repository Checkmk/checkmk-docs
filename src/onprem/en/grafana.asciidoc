...
The only restriction here is that users of the Grafana Cloud solution must use {CCE} and activate the plug-in xref:installation_marketplace[via the official Grafana plug-in collection].
...
This type of installation requires the use of {CCE}.
...
* The signed variant `checkmk-cloud-datasource-X.Y.Z.zip` xref:installation_cli[mentioned above], which can only be used with {CE}.
