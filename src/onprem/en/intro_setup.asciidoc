...
:description: Here you can find tips for selecting the appropriate {CMK} edition and version, for installing {CMK}, for creating your first site and for logging in.    
...
You can use it to comprehensively monitor small IT infrastructures.
Support is available in our link:https://forum.checkmk.com[Forum^].

xref:cse#[{CSE}] is primarily aimed at professional users and offers a number of interesting features beyond the scope of {RE},
...
a large number of sophisticated widgets to incorporate into xref:glossar#dashboard[dashboards], xref:reporting#[reporting], and much more.
As a {SE} user, you will receive professional support link:https://checkmk.com/product/support[from our team^] or from one of our link:https://checkmk.com/partners/find-partner[partners^].

// This block is outdated
// delete from here
xref:cce#[{CCE}] extends the functionality of {SE}, among others, with features that are useful in cloud environments such as Amazon Web Services (AWS) and Microsoft Azure,
...
More details can be found in the article on xref:cce#[{CE-onprem}].
{CME} is the multi-tenant capable extension of {CE} and has all of the necessary functions to operate isolated sites for each of multiple customers with {CMK} via a xref:glossar#distributed_monitoring[distributed monitoring]. 
If you as a _managed service provider (MSP)_ want to offer these services to your customers, this is your edition.
You can find more details on the concept of managed services in the introduction xref:managed#intro[of this article].

[TIP]
====
In addition to the _on-premises_ editions mentioned above, you can use {CCE-saas} as _Software as a Service (SaaS)_ in the cloud.
The link:https://docs.checkmk.com/saas/en/intro_setup.html[Beginner's Guide] in the Checkmk Cloud User Guide will help you get started with {CE-saas}.
====
// delete to here
...
{cee-only}
In this User Guide, whenever we discuss functions that apply only to the commercial editions -- i.e. to {SE} or {CE} -- we mark this with the icon as in this paragraph.
...
A detailed overview of what other versions are available can be found in xref:cmk_versions.html#[this article].
...
If you do not want to set up your own Linux server, you can also run {CMK} in a number of suitable prepared environments, such as Docker or an appliance. 
Below, we briefly present four common options for the installation. 
...
With the *{CMK} virt1* virtual appliance you get a ready-to-use virtual machine with an OVA (Open Virtualization Archive) file format that you can use in a hypervisor such as VMware ESXi, Microsoft Hyper-V or Proxmox.
...
{cee-only}
The virtual appliance is available for all commercial xref:glossar#edition[editions], also in the license states "Trial" and "Free".
...
This password can be changed at any time later.
...
As `root`, simply enter the command `omd create` and add the option `--admin-password` (with the password you have chosen) as well as the site name.
...
By the way -- whenever we give file path names in the User Guide that *do not* begin with a slash but with a tilde (`~`), they refer to the site directory, i.e., the home directory of the site.
...
The `omd` command provides many more options for controlling and configuring sites, and these are described in the article on xref:omd_basics#[administering sites].
In addition, you can find out how the directory structure for {CMK} is set up in the article on xref:cmk_commandline#folder_structure[{CMK} on the command line].
...
[TIP]
====
As soon as you are running {CMK} as production system, we strongly recommend for security reasons that you only allow access to the interface in a secured manner, i.e., via HTTPS instead of HTTP.
You can find out what you need to do for this in the article on xref:omd_https#[securing the web interface with HTTPS].
====

You will find the {CMK} login dialog at the above URL:
...
This will take you to the start page in {CMK}, which we will look at in more detail in the next chapter:

.The default {CMK} start page is the welcome page
image::intro_welcome.png[alt="The welcome page as the start page.",fullscreen=1]
