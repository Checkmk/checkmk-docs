...
{CRE} is free, 100{nbsp}% open-source software and incorporates Nagios as its core.
...
such as with the xref:cmc#[{CMK} Micro Core (CMC)], the very performant core developed in-house by Checkmk (which replaces Nagios), 
...
xref:cce#[{CCE}] extends the functionality of {SE} with features for the comprehensive monitoring of hybrid environments in which hosts are located both in data centers and in cloud environments such as Amazon Web Services (AWS) and Microsoft Azure.
Examples of these features include cloud-specific check plug-ins and dashboards, the {CMK} agent in xref:glossar#push_mode[push mode], automatic host registration, and application observability via the OpenTelemetry standard.
xref:managed#[{ME}] is the multi-tenant extension of {CE} and has all of the features needed to operate {CMK} sites for multiple customers that are isolated from each other via xref:glossar#distributed_monitoring[distributed monitoring].

[TIP]
====
In addition to the aforementioned on-premises editions, you can also use {CMK} with {CCE-saas} as software as a service (SaaS) in the cloud.
In the {CE-saas} User Guide, you will find an  https://docs.checkmk.com/saas/en/cce.html[overview of the functions] and a link:https://docs.checkmk.com/saas/en/intro_setup.html[Beginner's Guide^], which will help you get started with {CE-saas}.
====
...
For this Beginner's Guide, we are using {RE}.
However, we point out important differences compared to the commercial editions.
...
In the article about xref:cmk_processes#[site services], you can read about the services available in {RE} and in the commercial editions, and what each service is responsible for.
...
[TIP]
====
As soon as you are running {CMK} as a production system, we strongly recommend for security reasons that you only allow access to the interface in a secured manner, i.e., via HTTPS instead of HTTP.
...
.The welcome page is the default start page in {CMK}
