...
a large number of sophisticated dashlets to incorporate into xref:glossar#dashboard[dashboards], xref:reporting#[reporting], and much more.
...
{CCE} extends the functionality of {SE}, among others, with features that are useful in cloud environments such as Amazon Web Services (AWS) and Microsoft Azure,
...
To test the commercial editions of {CMK} without obligation, you can use {CE} or {ME}, because both editions are not limited for the first 30 days (in the 'Trial' license state).
...
In addition, you can find out how the directory structure for {CMK} is set up in the xref:cmk_commandline#folder_structure[article on {CMK} on the command line].
