...
The dynamic configuration of hosts means that, based on information from monitoring
...
For this purpose the DCD works with so-called _connectors_.
Each connector can get information from a very specific type of source, and has its own specific configuration.

With special connectors, it will become even easier in the future, to automatically take hosts into {CMK} from an existing CMDB.
...
As soon as you have saved the changes with icon:icon_save[alt=Icon for saving.”] [.guihint]#Save#, the selected user is used to access the REST-API and you can continue with the dynamic host configuration.
...
The [.guihint]#Setup > Hosts > Dynamic host management# menu entry brings you to the configuration of the DCD respectively its connector:
...
After saving the connector appears in the list.
...
After saving the connector properties, and following an activate changes, the connector will automatically start its operation.
...
|A DCD connector is removed.
|If you shut down a DCD connector ([.guihint]#do not activate this dynamic configuration connection#), or remove it altogether, all hosts created by this connector are preserved.
...
|If your cloud environment is unreachable and the {CMK} service requesting it goes to {CRIT}, the generated hosts will remain in monitoring _indefinitely_.
...
In addition to the question of whether hosts should be removed automatically at all, in the connector properties there are three more options 
