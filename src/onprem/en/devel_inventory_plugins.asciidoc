...
In the new browser window, select [.guihint]#Agent based ("Check API") > Version 2: cmk.agent_based.v2# in the left navigation bar.
...
Key columns have the special feature that they are used to detect changes, which in turn can lead to a status change of the [.guihint]#Check_MK HW/SW Inventory# service in monitoring.
...
If you know the name of a rule (here: [.guihint]#Lsusb inventory demo#), you can type it directly into the xref:user_interface#search[search field].
...
The following output provides an overview of existing sections:
