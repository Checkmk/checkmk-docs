...
[#display]
== Customizing the display of inventory data

When viewing the hardware/software inventory, you'll notice that the columns are displayed in a different order than defined in the plugin—namely, sorted alphabetically.
To improve the presentation, the link:https://docs.checkmk.com/plugin-api/{current-major}/cmk.inventory_ui/index.html#module-cmk.inventory_ui.v1_unstable[Inventory UI API^] from the namespace `cmk.inventory_ui.v1_unstable` comes into play.
...
Files that use the UI API are located in the `inventory_ui` subfolder of your plug-in family:
...
In the example shown, all data is handled as text fields.
This means you only need to change the order of the columns and define the column headers:
For the filename, {CMK} uses `hardware.py`, `software.py`, or `networking.py` as a convention.
You may deviate from this convention if it improves clarity.
...
.After using the UI API, the columns will be displayed in the desired order
image::devel_inventory_plugins_uiapi.png[alt="Inventory tree with Hardware > USB > Devices expanded."]

Assuming you have developed an inventory plug-in for a mass storage device that was not previously supported; in that case, you can use the UI API to ensure the numbers are displayed correctly:
...
In addition to various options for customizing the display of text and numbers, the API also provides ways to change the background color of fields—typically in combination with Boolean values.
Furthermore, you can define your own sorting and rendering functions.
...
In the {CMK} inventory, the resulting nodes look like this:

.Here, a single inventory function evaluates two agent sections
image::devel_inventory_plugins_multiple_sections.png[alt="Inventory tree with the expanded nodes for Software > Fictional > Example and Hardware > Fictional > Example."]
