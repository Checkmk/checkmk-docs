...
:description: With NagVis, {CMK} can visualize relationships between monitored objects. Read here how to create your own maps or diagrams with NagVis.
...
{CMK}, or to be more precise xref:glossar#livestatus[Livestatus], provides backend data such as hosts, services, host groups and service groups.
...
Here select the [.guihint]#NagVis maps# snap-in, and finally start NagVis with the [.guihint]#Edit# button.
