...
Encrypting the file with the secret key would be of no use, after all, you certainly do not want to have to enter the passphrase every time you start the {CMK} server...
...
After saving and xref:glossar#activate_changes[activating pending changes] the host is added to the monitoring.
...
The first step is to go to the [.guihint]#Setup# menu and create a rule in the rule set [.guihint]#Setup > Agents > Access to agents > {CMK} agent > Symmetric encryption (Linux, Windows)#.
...
Or the attacker actually does get a response, but the {CMK} server does not receive any data and will quickly report an error.
