// -*- coding: utf-8 -*-
// IGNORE +49 Infos User_Memory support@checkmk
include::global_attr.adoc[]
= Configuring and using the appliance
:title: Configuring and using the appliance
:description: The physical and the virtual {CMK} Appliance offer a range of options for configuration. Read here how to set up appliances of either type.
:experimental:
...
The appliance supports the editions {SE}, {CE}, and {ME}.
...
From the status view, you can open the configuration menu by pressing the kbd:[F1] key:
...
In the event of problems, error messages or conflicts, you can obtain detailed information on the update process in the xref:update#detailed[article on updates].
...
* Change in the first digit of the version number (_major release_): You must manually back up your appliance's data and import it again following the update. An update without manual data migration is not possible.
...
* Change of the third digit in the version number (_patch_): The update can be carried out without manual data migration. A prior backup is nevertheless recommended.
...
*Note on updating the major or minor version:* For example, if you update the firmware from 1.6.x to 1.7.y, you must also update all {CMK} versions used by any site or install them in the same version (but for the new platform).
The reason: Major and minor versions usually update the operating system version used as foundation or change underlying concepts.
...
In the console, you can access the menu for controlling the unit by pressing kbd:[F2].
...
To do this, press the kbd:[F2] key in the status view, select [.guihint]#Factory Reset# in the following dialog and confirm the following dialog with [.guihint]#yes#.
...
You can reach this terminal with the key combination kbd:[Ctrl+Alt+F2].
...
This terminal can be reached with the key combination kbd:[Ctrl+Alt+F3].
You can switch back to the status view with the key combination kbd:[Ctrl+Alt+F1].
