...
By the way, the https://docs.checkmk.com/master/de/apis_intro.html[German version] of this article has already been updated -- if that is an alternative for you.
// delete to here
...
Darüber hinaus gibt es jedoch noch APIs für xref:glossar#check_plugin[Check-Plugins,] das Inventar, die xref:glossar#agent_bakery[Agentenbäckerei] und so weiter.
...
|HW/SW-API
|xref:inventory#external[Inventurdaten-API]
|n.a.
...
xref:ec#statusinterface[Statusschnittstelle] der Event Console
|n.a.
...
|xref:inventory#external[HW/SW-Inventur] |Beschreibung der Inventar-eigenen Web-API für den externen Zugriff.
|xref:ec#statusinterface[Statusschnittstelle] der Event Console |Per Unix-Socket und über eine Teilmenge des Livestatus-Protokolls können Daten von der Event Console ausgelesen und Kommandos abgesetzt werden.
...
|xref:devel_check_plugins#[Agentenbasierte Check-Plugins schreiben] |Ausführliche Beschreibung zur Entwicklung eines Check-Plugins.
|xref:devel_check_plugins_snmp#[SNMP-basierte Check-Plugins schreiben] |Ausführliche Beschreibung zur Entwicklung eines Check-Plugins für SNMP-Daten.
|===
...
|link:https://checkmk.atlassian.net/wiki/spaces/KB/pages/9473812/REST-API+code+examples[REST-API-Code-Beispiele^] |cURL-Beispiele in der in der {CMK} Knowledge Base für konkrete, beispielsweise UND-verknüpfte Abfragen.
