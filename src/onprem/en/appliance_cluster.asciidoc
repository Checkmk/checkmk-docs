...
//SK: This anchor is a hack, so that the browser jumps to the right places an show the following discrete heading.
[#cluster_virt1]#In# doing so, use the `LAN1` and `LAN2` ports to connect to your network and the `LAN3` and `LAN4` ports for the direct sync connection between the units.
