...
Open the xref:user_interface#setup_menu[Setup menu] and activate the xref:intro_gui#show_less_more[Show more mode]—because only then will the [.guihint]#Setup > Connectors > InfluxDB connections# menu item be displayed.
Select this to display the list of existing InfluxDB connections.
Since you probably have not configured anything here yet, this list will be empty.
...
The [ guihint]#Send metrics to InfluxDB# rule set can be found in [guihint]#Setup > Services > Service monitoring rules#, can be accessed more quickly with the xref:user_interface#search[search] in the [.guihint]#Setup# menu, and quickest by clicking in the xref:influxdb_connection_list[connection list]:
...
Finally, xref:glossar#activate_changes[activate the pending changes] to create the connection and create the rule(s).
...
If you do not want this, you should select the metrics by rule *before* enabling the connection, and then enable both the rules and the connection together.
...
An xref:glossar#activate_changes[activate pending changes] for the rules and connection completes the configuration for Graphite.
