...
:description: Linux upgrades on the {CMK} server can be performed with low downtimes. Read here how to plan and execute your system upgrade.
...
[IMPORTANT]
====
The edition and version number of the {CMK} package for the target version must exactly match the one used previously.
====
...
. Revert the renaming of the soft link pointing to the {CMK} installation you are using. The following command changes it back from `_version` to `version`:
...
[IMPORTANT]
====
The edition and version number of the {CMK} package for the target distribution must exactly match the one used previously.
====
