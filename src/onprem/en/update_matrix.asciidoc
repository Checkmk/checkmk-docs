...
If you want to update to a version of {CMK} older than {current-major}, please read this article for the {CMK} version that is to be the _target_ of the update.
...
In the development branch, nothing is shown here due to missing data.
...
Note that STS versions of Ubuntu are no longer supported since {CMK} {v23}.
