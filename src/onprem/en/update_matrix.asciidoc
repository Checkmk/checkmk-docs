...
[TIP]
====
The complete compatibility matrix will only be available after the release of Checkmk {v23}.
During the beta phase, use the xref:release_notes#[Release notes] and the xref:ossupport[OS support policy] (in this article) as a reference.
====
