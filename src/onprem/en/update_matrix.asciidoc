...
:description: Each {CMK} version supports specific Linux distribution versions. Read here how to plan and perform Linux upgrades in interaction with {CMK} updates.    
...
For an update from Debian 11 with {CMK} {oldstable}p1 to Debian 13 with {CMK} {current}, this will result in the following update sequence:

. On Debian 11, update {CMK} {oldstable}p1 xref:update#[to the current patch version] of {oldstable} ({oldstable-patch}).

. Upgrade from Debian 11 to 12 and finally to 13 as described in the xref:release_upgrade#[Linux upgrade on the {CMK} server] article.

. On Debian 12, update {oldstable-patch} to the xref:update_major#[current patch version {current}].
