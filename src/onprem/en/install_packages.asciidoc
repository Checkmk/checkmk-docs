...
If you do not want to deal with Linux under any circumstances, it is also possible to run {CMK} as xref:appliance_install_virt1#[a virtual appliance] or link:https://checkmk.com/product/appliances[a physical appliance^].
When running {CMK} as xref:introduction_docker#[a Docker-Container], some Linux-specific installation steps are performed automatically for you, as our official images already contain the required environment for the software and will create a {CMK} site for you upon starting a container.
...
Depending on the Linux distribution you want to install {CMK} onto, follow one of the dedicated installation instruction below, in which you will find details about setting up the package source, downloading the appropriate {CMK} package and installing the package:
