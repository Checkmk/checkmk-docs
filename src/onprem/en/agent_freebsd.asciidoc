...
:description: Monitoring FreeBSD systems with {CMK} works similarly to monitoring Linux systems. Read here which differences you should take into account.
...
This article explains the installation and setup of the agent for _FreeBSD_ --
...
A special dependency is `bash`, since the agent script actually uses a syntax which is neither compatible with the link:https://man.freebsd.org/cgi/man.cgi?query=sh&sektion=1&manpath=freebsd-release-ports[Almquist Shell^] nor with link:https://man.freebsd.org/cgi/man.cgi?query=tcsh&sektion=1&manpath=freebsd-release-ports[`tcsh`^].
The `ipmitool` is needed to acquire hardware information.

Install all of these dependencies with the following command:
...
=== Installation from the FreeBSD Ports

The link:https://ports.freebsd.org/[FreeBSD Ports^] contain a link:https://cgit.freebsd.org/ports/tree/net-mgmt/check_mk_agent[port for the {CMK} agent^] maintained by Ian Valentine, which may contain patches to make the agent compatible with newer FreeBSD-STABLE versions.
...
There is also a pre-built package available for the `check_mk_agent` port, which you can install using `pkg`:
...
In most cases, the version you build yourself from the ports is more up-to-date:
...
Now run the agent on the command line and skim over the output in the pager:
...
First comment out the line that allows access from anywhere, then add a line that allows access only to the {CMK} server -- here with the IP address `198.51.100.123`.
...
Run the following command as the site user on the {CMK} server to test the network connection to the agent (in this example, to the host with the IP address `198.51.100.231`).
...
For a 'bare' FreeBSD with no server services configured yet, at least 11 services should be discovered, and ten additional ones for file system information if installed on ZFS:

.The service discovery in {CMK} will find at least 11 services.
...
While we are at it, let's use a two-liner to test the Python interpreter.
