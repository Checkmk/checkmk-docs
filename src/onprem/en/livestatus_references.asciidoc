...
|`log` |All information for logged events. These should be filtered by class and timestamp/time period.
...
|`Localtime` |Unix Time |By using a specified reference time it is attempted to compensate for differing local timestamps in a distributed environment.
