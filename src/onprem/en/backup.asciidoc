...
* Both systems must have the same {CMK} edition installed, for example. the {CRE}.
...
For example, if the local site is the central site in a xref:glossar#distributed_monitoring#[distributed monitoring] with xref:glossar#distributed_setup#[distributed setup], the backup includes the configuration data of this central site and all remote site that are configured from the central site.
