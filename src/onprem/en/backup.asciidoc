...
If the local site is the central site in xref:distributed_monitoring#[distributed monitoring], the backup includes the configuration data of this central site and all remote sites that are configured from the central site.
...
To restore a backup on a remote site, proceed in the same way by first allowing interactive login and configuration via Setup and then resetting the options.
