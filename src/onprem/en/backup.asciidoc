...
[TIP]
====
{CMK} alternatively offers the possibility to perform the basic backup and restore functions via command line with `omd backup` and `omd restore`.
...
* The exact same xref:cmk_versions#[{CMK} version] must be installed on both systems, e.g. `{v23}p1`.
...
If the local site is the central site in a xref:distributed_monitoring#[distributed monitoring], the backup includes the configuration data of this central site and all remote sites that are configured from the central site.
...
This is a little more time-consuming, as you must first allow the configuration via Setup on each remote site, then log in there and configure the backup.
...
. In the [.guihint]#Configuration connection# box activate the option [.guihint]#Users are allowed to directly login into the Web GUI of this site# and *deactivate* the [.guihint]#Disable configuration via Setup on this site# option, because configuration via Setup on a remote site is not possible by default.
...
. Finally, reset on the central site the changed options of the remote site properties to the previous values.
Activate these changes as well.

To xref:backup_restore[restore] a backup on a remote site, proceed in the same way by first allowing interactive login and configuration via Setup and in the end resetting the options.
