...
|{CSE} | -- |link:https://checkmk.com/download?platform=docker&edition=cee[{CMK} downloads page^] |link:https://portal.checkmk.com/[{CMK} customer portal^] |xref:registry_images[{CMK} Docker Registry] 
...
{cce-only}
The official images are also freely available on various platforms for the {CE} and {ME} commercial editions.
These editions of {CMK} can be tested with all features as part of a 30-day trial period.
...
For editions whose images are available in Docker Hub according to the above table, you can jump directly to the section xref:container_start[Starting a {CMK} container].

{cce-only}
[#registry_images]
Images for {SE} are stored in the {CMK} Docker Registry.
Here the Docker Engine can automatically find and download images based on their full name and file path.
However, you must first log in to the registry with your credentials from the {CMK} customer portal.
To log in, use the Docker Credential Store or the following command:
...
After successfully logging in, the Docker Engine can access the {CMK} images via the registry.
...
For version {current} of {SE}, for example, specify the full path for the image when xref:#container_start[starting the container] as follows:
`registry.checkmk.com/enterprise/{pkg-cee}:{current}`
