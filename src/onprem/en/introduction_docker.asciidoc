...
|Commercial editions |link:https://checkmk.com/download?method=docker&edition=cce&version=stable[{CMK} download page^] (for the {CE}), link:https://portal.checkmk.com/[{CMK} customer portal^]
|===
...
[{shell}]
----
{c-root} docker container run -dit -p 8080:5000 -p 8000:8000 --tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 -v monitoring:/omd/sites --name monitoring -v /etc/localtime:/etc/localtime:ro --restart always checkmk/check-mk-raw:{version}-latest
Unable to find image 'checkmk/check-mk-raw:{version}-latest' locally
{version}-latest: Pulling from checkmk/check-mk-raw
...
Status: Downloaded newer image for checkmk/check-mk-raw:{version}-latest
...
|`checkmk/check-mk-raw:{version}-latest` |The {CMK} image label in the `<repository>:<tag>` format.
...
Download the desired edition and version from the link:https://checkmk.com/download?method=docker&edition=cce&version=stable[{CMK} download page^] (for the {CE}) or from the link:https://portal.checkmk.com/[{CMK} customer portal^].
...
Just take care that you use the name of the `Loaded image` from the previous command output in the following start command, so in this example `checkmk/check-mk-cloud:2.2.0p1`:
