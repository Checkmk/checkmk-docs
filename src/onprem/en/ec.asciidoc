// -*- coding: utf-8 -*-
// IGNORE Addons cancelling logfiles Eventlog Finetune Logfile logfile +1 pre
// NONASCII
include::global_attr.adoc[]
= The Event Console
:title: The Event Console - Processing logs and SNMP traps
:description: {CMK} can react to a wide range of events with specified actions. Read here how to configure and use the Event Console for this purpose.
...
* It can perform automated actions and send xref:glossar#notification[notifications] via {CMK}.
...
And neither Syslog nor SNMP traps offer acknowledgments or similar protection against lost messages.
...
It also does not help when messages have already been pre-classified by their originator.
...
And it cannot even be definitely said that the application which has produced the message is even important on this computer.
...
As always in {CMK}, you must first xref:glossar#activate_changes[activate pending changes] in order for them to take effect.
...
Only administrators are allowed to activate pending changes in the EC.
...
Since the protocol is so simple, you do not even need a special program, you can simply send the data via `netcat` or `nc` using UDP.
...
This is analogous to the xref:basics_ackn#[acknowledgment] of host and service problems.

Subsequent invocation of the command with the checkbox unchecked _removes_ the acknowledgment.
...
This goes through the xref:notification_rules#[notification rules] and can generate emails, an SMS, or whatever you have configured accordingly.
...
That is why we do not speak of _deleting_, but rather of _archiving_.
...
Don't make the mistake of using the placeholder `\1` in [.guihint]#Text to cancel events(s)#.
...
Normally an acknowledgment of the event creates a situation that if further messages are received, a new count is started and a new event is triggered.
...
That does not sound very reasonable {nbsp}...
...
|===
|Interval |Alignment
// MFS: https://checkmk.com/werk/19567
...
* Features such as xref:notification_rules#bulk[bulk notifications], HTML emails and other useful functions are available.
...
By the way, the EC and its components are designed to send notifications at a frequency suitable for humans and incident response teams.
There is no hard limit, but think in terms of "notifications per day" rather than "per second"—the EC is not an API for integrating third-party systems!
...
|Host tags |The host will not receive a host tag. If you have host tag groups with empty tags, the host adopts those tags, but otherwise it has no tags from the group. Keep this in mind when you define conditions referring to host tags in the notification rules.
...
If you already have this running, stop it.
...
In the case of SNMPv1 and v2c this is a simple password, which is known as a 'Community'.
...
Regrettably, the advantages of open-source development have not yet spread to the authors of MIB files, and so we from the {CMK} project are unfortunately not able to deliver vendor specific MIB files.
...
The {CMK} agent is able to analyze log files via the _logwatch_ plug-in.
...
Don't confuse this with the _subsequent_ reclassification via the [.guihint]#Logfile patterns# rule set.
...
|`UPDATE` |The event was edited by the operator (change to comment, contact info, acknowledgment).
...
That notification will go through the {CMK} xref:notification_rules#[notification rules].
...
If you use the Event Console only to process real error messages, this cannot really happen.
...
|`UPDATE` |Executes an update from an event. The arguments are in order event id, user abbreviation, acknowledgment (0/1), comment and contact info.
