...
All file paths are specified from the site user's perspective.
...
|`~/var/check_mk/core/config.pb` |The core's configuration with all static data on hosts, services, groups, users and global settings in the data format Protocol Buffers. In Nagios this data equates to the files under `etc/nagios/conf.d`.
|`~/var/check_mk/core/state.pb` |The core's current runtime state. The file stores information on the current status of hosts and services, likewise xref:basics_downtimes#[scheduled downtimes], xref:basics_ackn#[acknowledgments] and commentaries in the data format Protocol Buffers. The data is updated regularly during processing and when the core is stopped. If it is unavailable or incompatible when the core starts, the core will begin with an empty state. This data equates to the `var/nagios/retention.dat` file in Nagios, but it is however binary-coded.
...
We explain how to handle Protocol Buffers in the article xref:cmk_commandline#file_syntax[Checkmk on the command line].
