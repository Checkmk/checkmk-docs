...
When activated -- regardless of whether `force-install` is used -- , the files are installed in a directory hierarchy within `~/local/` and the package description file is placed in `~/var/check_mk/packages/`.
