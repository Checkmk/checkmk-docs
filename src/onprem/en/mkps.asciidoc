...
[TIP]
====
Extensions that use the APIs introduced with {CMK} {v16} and already marked as deprecated in {CMK} {v20} will no longer be supported by {CMK} {v23}.
Enter 2.2.99 here as the maximum supported version.
Extensions that use the APIs introduced in {CMK} {v20} will work up to {CMK} {v23} without adjustments, but not with {CMK} {v24}.
You can enter 2.3.99 as the maximum supported Checkmk version here.
====
