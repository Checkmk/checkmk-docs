...
{CMK} has a highly modular design and it can be extended in many ways using Python programming skills.
To manage these extensions effectively, share them within distributed environments, and make them available to other users, {CMK} provides its own package format: the *{CMK} extension package*—or *MKP* for short.

An MKP can contain any number of extensions—for example, a set of xref:glossar#check_plugin[check plug-ins] including associated manual pages, the configuration environment for thresholds, and related metric definitions.
It can also contain settings for distributing agent plug-ins via the xref:glossar#agent_bakery[Agent Bakery].

In this article, you will learn how to install and manage extensions, as well as how to ensure that updates run smoothly even in distributed environments.
For extension developers, we have created a xref:devel_mkps#[separate article] dedicated to modifying and packaging extensions.


[#exchange]
=== The {CMK} Exchange

On the link:https://exchange.checkmk.com[{CMK} Exchange^], programmers of plug-ins can provide packages for other {CMK} users and share these amongst themselves.
...
In addition, it is possible that an MKP was developed for older versions of {CMK} and thus may not be fully compatible with a version currently in use (e.g., with the update from version {v16} to version {v20} {CMK} changed from Python 2 to Python 3).
In extreme cases there can be a risk of data corruption and loss.
We therefore strongly recommend that before using third-party MKPs in a production environment, they should first be installed and tried on a test site.
...
These are compatible with each other so that you can use both the command and [.guihint]#Extension Packages# without 'messing anything up'.
...
An MKP that you have downloaded from the Exchange, for example, can be uploaded to {CMK} by clicking the [.guihint]#Upload package# button and it will then be made available for installation.
...
Only with a click on the 'plug' button icon:icon_install[] will an available package also be activated.
...
Now perform an xref:wato#activate_changes[activation of pending changes], after which all functions from the package will be anchored in the system and ready for use.
...
Using the icon:icon_delete[] button in the list of all packages, you can again selectively delete installed and unused packages.
...
In the case of a xref:glossar#central_setup[distributed monitoring with a central setup], it is sufficient to make the packages available on the central site.
...
A special situation is the attempted activation of a package that does not match the {CMK} version in use.
...
For clarity, here we have abbreviated the output, which is about 50 lines long, to less than half:
...
Commands for modifying and creating packages can be found in the article xref:devel_mkps#command_reference[MKPs for developers].
