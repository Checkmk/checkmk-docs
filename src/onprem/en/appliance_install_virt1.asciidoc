...
:description: You can set up the virtual {CMK} Appliance within Hyper-V, Proxmox or VMware ESXi. Read here what to keep in mind in each case.
:experimental:
...
With the _{CMK} virt1_ virtual appliance you can run {CMK} as a guest on an existing virtualization platform (_hypervisor_) such as VMware ESXi, Hyper-V or Proxmox.
Supported editions: {SE}, {CE} and {ME}.
...
Back on the [.guihint]#Hyper-V Manager# main screen, select your new VM for {CMK} and then click [.guihint]#Settings#.
...
* Then click on [.guihint]#SCSI Controller# and add a new [.guihint]#Hard drive# via [.guihint]#Add#.
Under the [.guihint]#Virtual hard disk# field you will again find the [.guihint]#Browse# button.
This time, select the virtual hard disk with the file name containing `disk2`.
* *Optional*: {CMK} is installed on this second disk (`disk2`), where all {CMK} sites and their associated data are also stored.
By default, this virtual hard disk had a size of 100 GB.
If you need a larger disk for your sites and data, click [.guihint]#Edit# *now* and select the [.guihint]#Expand# action.
When the virtual appliance is started for the first time, this disk is initialized and cannot be enlarged afterwards.
...
- [.guihint]#System#: Activate the option [.guihint]#Qemu Agent#.
