...
|Assign Memory | Startup memory | min. 4096 MB
|Configure Networking | Connection | _freely selectable_
|Connect Virtual Hard Disk | Use an existing virtual hard disk | Select the disk from the zip with `disk1` in the file name
|Security | Enable Secure Boot | Deactivate checkbox
|SCSI Controller | Add Hard Drive | Select the disk from the zip with `disk2` in the file name
|===
...
The import of the appliance is largely automatic, but you should then take a look at the network configuration.
...
. Next, assign memory to the VM ([.guihint]#Assign Memory#). The minimum requirement here is `4096 MB`.
. In the [.guihint]#Configure Networking# step, you may need to select the correct connection.
. Now in the step [.guihint]#Connect Virtual Hard Disk#, you must select the option [.guihint]#Use an existing virtual hard disk#. Then click on [.guihint]#Browse# and select the virtual hard disk previously extracted from the zip file, which has `disk1` in its name.
. Then click [.guihint]#Finish#, but do *not* start the VM yet.
. Now select your new VM for {CMK} and then click on [.guihint]#Settings.#
. Here you must select the setting [.guihint]#Security# and *deactivate* the [.guihint]#Enable Secure Boot# checkbox.
. Then click on [.guihint]#SCSI Controller# and add a new [.guihint]#Hard drive# via [.guihint]#Add#. Under the [.guihint]#Virtual hard disk# field you will again find the [.guihint]#Browse# button. This time, select the virtual hard disk with the file name `disk2` and close the settings with [.guihint]#Apply#.
