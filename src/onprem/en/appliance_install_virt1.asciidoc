...
If you are using System Center Virtual Machine Manager (SCVMM) instead, you should also be able to follow these instructions, as the differences in setting up a virtual machine are marginal.
====
...
|SCSI Controller | Add Hard Drive | Select the disk from the zip archive with `disk2` in the file name
|===
