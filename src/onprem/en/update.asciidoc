...
// MFS: FIXME, adjust after translation
= Updates and Upgrades
:title: Updates and Upgrades
:description: For updates and upgrades of existing {CMK} sites, some preparations and follow-ups are necessary. Read here how to plan and execute updates and upgrades.
:experimental:
...
// MFS: New signposting, has to be translated completely
////
...
// MFS: Complete translation of this paragraph needed:
////
...
////
Possible parameters for `--conflict=` could be:

[cols="20,~"]
|===
|`--conflict=ignore` |Ignore each and every conflict. In many cases, {CMK} might not start and manual adjustments are required. Did we already mention the relevance of having a recent backup? 
...
Below is an example of the complete command for an automated update to version `{current}.community` of `mysite`:
