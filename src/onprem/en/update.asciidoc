...
|{v21} |{v21} |Yes | State before updating all sites.
|{v21} |{v22} |Yes | Minor functional losses are to be expected during the update, so only operate as a mixed environment for a short period of time. There is no danger for data and configuration.
|{v22} |{v21} |*No* | *Caution:* With a xref:distributed_monitoring#distr_wato[centralized configuration] there is a risk of irreparably damaging remote sites in this situation. Avoid this condition at all costs!
|{v22} |{v22} |Yes | State after updating all sites.
|===
...
[#cre2cee_distributed]
==== Upgrading the {RE} to the {SE} (or the {CE})

{CMK} does not officially support a mixed operation between the {RE} and the commercial editions.
For this reason, we strongly recommend that you upgrade the edition _offline_.
...
[#cee2cce_distributed]
==== Upgrading the {SE} to the {CE}

The feature set in the {CE} is a true superset of the feature set in the {SE}, so the biggest challenge in upgrading the edition is handling the different xref:license#[license and subscription management].
The upgrade can be done _online_ in this situation, i.e. a mixed operation is possible for a short period of time.
In this case, the following sequence must be followed and the entire upgrade must be completed within 30 days:

. Perform the upgrade of the central site.
This will set it to the {CE} "Trial" license state for 30 days.
In this mode, there is no limitation on the number of services and no restriction on communication with other editions.
Remote sites will continue to use the existing licensing.

. Update the remote sites one by one.
Each of these will now also be placed in the {CE} "Trial" license state for 30 days.
. Depending on which cloud-specific services are required in your environment, deactivate any services that are not required and determine the scope of the required services in this move.
. Change your subscription to the {CE} with the required number of services and store the subscription data in {CMK}.
...
. If it is desired (and an abundance of notifications is not a problem), the central site can be restarted immediately.
. Now it's time to downgrade the remote sites. You can do this in parallel and restart the remote sites immediately following their downgrading.
