...
image::update1.png[alt="Three sites and their software versions used.",width=400]

Here the site `mysite1` uses version `2.4.0p3.cee`, but the sites `mysite2` and `mysite3` use version `2.4.0p1.cre`.
Although {CMK}-Version `2.4.0p1.cce` has been installed, it is currently not being used.
...
image::update2.png[alt="The site 'mysite' with the software version used before the update.",width=400]

Here the site `mysite` is to be updated to the {CMK}-version `2.4.0p3.cee`.
...
image::update3.png[alt="The situation after installing the new software version.",width=400]

The second step will now be to update the site from `2.4.0p1.cre` to the `2.4.0p3.cee`.
This is performed with the `omd update` command, which we will discuss in detail below:

image::update4.png[alt="With 'omd update' the site receives the new software version.",width=400]

Following the update, the (likely) no longer required version `2.4.0p1.cre` can be deleted by uninstalling the appropriate package.
...
++++
// image::update_omd-update-2.png[alt="Selecting the target version for the update.",width=55%]
...
++++
// image::update_omd-update-3.png[alt="Confirmation dialog when updating the version.",width=63%]
...
++++
// image::update_raw_to_enterprise.png[alt="Confirmation dialog when upgrading the edition.",width=38%]
...
++++
// image::omd-update.png[alt="Query to resolve a conflict.",width=83%]
...
Below is an example of the complete command for an automated update to version `{current}.cre` of `mysite`:
...
++++
// image::update_downgrade_ce_se.png[alt="Confirmation dialog when downgrading the edition.",width=30%]
