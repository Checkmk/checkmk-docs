...
////
To summarize:
If you want to customize a {CMK} configuration and activate the changes, in Nagios you will subsequently require:
...
[cols="20,~"]
|===
|`agent` |Evaluates the data from a {CMK} agent.
This is (normally) retrieved via TCP Port 6556.
|`snmp` |Serves the monitoring of devices via SNMP.
|`active` |Calls a standard type of Nagios-compatible plug-in for the monitoring.
Here {CMK} actually only adopts the configuration.
|===
