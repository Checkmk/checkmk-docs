...
Assume that the above rule would now be restricted so that a notification would only be sent if a monitoring event for a service starting with the name `NTP` occurred on a host in the [.guihint]#Main# folder:
...
In the notification rule, select the desired parameter definition under [.guihint]#Select parameters# *before* you click on [.guihint]#Create#.
