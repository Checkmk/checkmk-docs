...
eine große Anzahl von ausgeklügelten Dashlets zum Einbau in xref:glossar#dashboard[Dashboards,] ein xref:reporting#[Reporting], und vieles mehr.
...
{CCE} erweitert den Funktionsumfang von {SE} unter anderem um Funktionen, die in Cloud-Umgebungen wie Amazon Web Services (AWS) und Microsoft Azure nützlich sind, 
...
Mehr Details finden Sie im Artikel über xref:cce#[{CE-onprem}].
Die {CME} ist die mandantenfähige Erweiterung von {CE} und verfügt über alle notwendigen Funktionen, um mit {CMK} über das xref:glossar#distributed_monitoring[verteilte Monitoring] voneinander abgeschottete Instanzen für mehrere Kunden zu betreiben. 
...
[TIP]
====
Außer den genannten _On-Premises_-Editionen können Sie {CCE-saas} als _Software as a Service (SaaS)_ in der Cloud nutzen.
...
Zusätzlich erfahren Sie im xref:cmk_commandline#folder_structure[Artikel über {CMK} auf der Befehlszeile], wie die Verzeichnisstruktur von {CMK} aufgebaut ist.
