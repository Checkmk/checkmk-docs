...
{CRE} ist kostenlos, als Open-Source-Software zu 100{nbsp}% quelloffen und enthält Nagios als Kern. 
...
xref:managed#[{ME}] ist die mandantenfähige Erweiterung von {CE} und verfügt über alle Funktionen, um mit {CMK} über das xref:glossar#distributed_monitoring[verteilte Monitoring] voneinander abgeschottete Instanzen für mehrere Kunden zu betreiben.
// Falls Sie als ein _Managed Service Provider (MSP)_ für Ihre Kunden diese Dienste anbieten wollen, ist {ME} die richtige Wahl. 
...
Im {CE-saas} Handbuch finden Sie eine https://docs.checkmk.com/saas/de/cce.html[Übersicht der Funktionen] und einen link:https://docs.checkmk.com/saas/de/intro_setup.html[Leitfaden für Einsteiger^], der Ihnen bei den ersten Schritten mit {CE-saas} hilft.
====

Eine Aufstellung der Unterschiede zwischen den Editionen finden Sie auf unserer link:https://checkmk.com/de/preise[Website^].

Für diesen Leitfaden für Einsteiger nutzen wir {RE}.
Wir weisen Sie aber auf wichtige Unterschiede zu den kommerziellen Editionen hin.

{cee-only}
Wann immer wir in diesem Handbuch Funktionen besprechen, die nur für die kommerziellen Editionen gelten -- also für {SE} oder {CE} -- , kennzeichnen wir dies mit dem Symbol wie in diesem Absatz.
...
Geben Sie als `root` den Befehl `omd create` ein und ergänzen Sie ihn um die Option `--admin-password` (mit dem von Ihnen gewählten Passwort) und um den Instanznamen.
...
Was Sie dafür tun müssen, erfahren Sie im Artikel über die xref:omd_https#[Absicherung der Web-Oberfläche mit HTTPS].
====
...
.Die Startseite von {CMK} ist standardmäßig die Willkommensseite
image::intro_welcome.png[alt="Die Willkommensseite als Startseite.",fullscreen=1]

xref:intro_welcome#[Weiter geht es mit der Willkommensseite]
