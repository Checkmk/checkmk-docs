...
= {CMK} aufsetzen
:title: {CMK} aufsetzen
:description: Hier finden Sie Tipps zur Auswahl der passenden Edition und Version von {CMK}, zur Installation, zur Erstellung einer ersten Instanz und zum Login.
...
Sie können damit kleine IT-Infrastrukturen umfassend überwachen.
Support erhalten Sie in unserem link:https://forum.checkmk.com[Forum^].

xref:cse#[{CSE}] richtet sich vor allem an professionelle Anwender und bietet über den Umfang von {RE} hinaus eine Reihe interessanter Features,
wie zum Beispiel mit dem xref:cmc#[{CMK} Micro Core (CMC)] einen sehr performanten, eigenen Kern (der Nagios ersetzt), 
...
eine große Anzahl von ausgeklügelten Widgets zum Einbau in xref:glossar#dashboard[Dashboards,] ein xref:reporting#[Reporting], und vieles mehr.
Für {SE} erhalten Sie link:https://checkmk.com/de/produkt/support[von uns^] oder einem link:https://checkmk.com/de/partner/partner-finden[unserer Partner^] professionellen Support.

xref:cce#[{CCE}] erweitert den Funktionsumfang von {SE} um Funktionen zur vollständigen Überwachung hybrider Umgebungen, in denen Hosts sowohl in Rechenzentren als auch in Cloud-Umgebungen wie Amazon Web Services (AWS) und Microsoft Azure beheimatet sind.
Beispiele dieser Features sind Cloud-spezifische Check-Plugins und Dashboards, der {CMK}-Agent im xref:glossar#push_mode[Push-Modus], die Autoregistrierung von Hosts und _Observability_ von Anwendungen über den OpenTelemetry-Standard.

xref:managed#[{CME}] ist schließlich die mandantenfähige Erweiterung von {CE} und verfügt über alle Funktionen, um mit {CMK} über das xref:glossar#distributed_monitoring[verteilte Monitoring] voneinander abgeschottete Instanzen für mehrere Kunden zu betreiben. 
Falls Sie als ein _Managed Service Provider (MSP)_ für Ihre Kunden diese Dienste anbieten wollen, ist dies Ihre Edition. 

[TIP]
====
{CMK} können Sie außer in den genannten _On-Premises_-Editionen auch mit {CCE-saas} als _Software as a Service (SaaS)_ in der Cloud nutzen.
Der link:https://docs.checkmk.com/saas/de/intro_setup.html[Leitfaden für Einsteiger^] des Checkmk Cloud Handbuchs hilft Ihnen bei den ersten Schritten mit {CE-saas}.
====

Eine Aufstellung der Unterschiede zwischen den Editionen finden Sie auf unserer link:https://checkmk.com/de/preise[Website^].

Für diesen Leitfaden für Einsteiger nutzen wir {RE} -- weisen Sie aber auf wichtige Unterschiede zu den kommerziellen Editionen hin.
...
// ES: Wäre es hier sinnvoll, CE und ME zusammenfassend zu erwähnen? Zum Beispiel als "oder {CE} mit oder ohne Multi-Tenancy"
// TK: Eher nicht an dieser Stelle, auch wenns sich wirklich holprig liest. Grundsätzliches Problem dabei: Wir umgehen unsere Makros und können die Namen später nicht mehr einfangen.
// TK: Schöner fände ich es, die {ME} klar(er) als Spezialfall der {CE} einzuordnen. Dann könnte man die {ME} im vorherigen Satz ganz weglassen. 1. Schritt in die Richtung war, die {ME} oben in der Liste in den Absatz der {CE} reinzuholen. Aber da hast Du mir ja wieder eine Leerzeile dazwischen gesetzt ;-) 
// TK: Und/oder wir verzichten auf das {CME}-Makro und nehmen nur noch {ME}, damit das Ding eben nicht mehr wie eine eigenständige Edition aussieht?
...
Einen detaillierten Überblick, welche Arten von anderen Versionen es außerdem gibt, zeigt xref:cmk_versions.html#[dieser Artikel].
...
Wenn Sie keinen eigenen Linux-Server aufsetzen möchten, können Sie {CMK} auch in bestimmten eigens dafür eingerichteten Umgebungen, wie beispielsweise in Docker oder in einer Appliance, betreiben. 
Im Folgenden stellen wir vier gängige Möglichkeiten zur Installation vor. 
...
Mit der virtuellen Appliance *{CMK} virt1* erhalten Sie eine komplett eingerichtete virtuelle Maschine im Dateiformat OVA (Open Virtualization Archive), die Sie in einem Hypervisor wie zum Beispiel VMware ESXi, Microsoft Hyper-V oder Proxmox verwenden können.
...
Der Vorteil der Appliance ist, neben einem vorinstallierten System, dass Sie Betriebssystem, Appliance und {CMK} komplett über die grafische Oberfläche konfigurieren können, ohne die Linux-Befehlszeile bemühen zu müssen.
...
{cee-only}
Die virtuelle Appliance ist für alle kommerziellen xref:glossar#edition[Editionen] verfügbar, auch in den Lizenzstatus „Trial“ und „Free“.
...
Dabei unterstützen wir sowohl {RE} als auch die kommerziellen Editionen mit fertigen Container Images, die mit wenigen Befehlen eingerichtet sind.
...
Dieses Passwort kann später jederzeit geändert werden.
...
Geben Sie als `root` den Befehl `omd create` ein und ergänzen Sie ihn um die Option `--admin-password t0p53cr3t` (mit dem von Ihnen gewählten Passwort) und um den Instanznamen.
Um zu verhindern, dass hierdurch das Passwort für den Instanz-Administrator im Klartext in der Historie Ihrer Befehlszeile erscheint, können Sie vor dem Befehl ein Leerzeichen einfügen.

// TK: Ausgabe check-mk-community-2.5.0-2026.01.28
[{shell}]
----
...
Executing post-create script "02_message-broker-certs"...[green]#OK#
...
Übrigens: Immer wenn wir im Handbuch Pfadnamen angeben, die *nicht* mit einem Schrägstrich beginnen, sondern mit der Tilde (`~`), beziehen sich diese auf das Instanzverzeichnis, das heißt das Home-Verzeichnis der Instanz.
...
Zu diesem werden Sie am einfachsten mit dem folgenden Befehl:
...
// Ausgabe check-mk-community-2.5.0-2026.01.28 (wie auch die beiden folgenden)
[{shell}]
----
{c-omd} omd status
...
{cee-only}
Da die kommerziellen Editionen über mehr Features als {RE} verfügen, sehen Sie dort auch mehr Dienste. 
Außerdem finden Sie als Monitoring-Kern `cmc` statt `nagios`.
Welche Dienste es in {RE} und in den kommerziellen Editionen gibt und wofür die einzelnen Dienste zuständig sind, können Sie im Artikel über die xref:cmk_processes#[Instanzdienste] nachlesen.

Der Befehl `omd` bietet noch viele weitere Möglichkeiten zur Steuerung und Konfiguration von Instanzen, die im Artikel über die xref:omd_basics#[Konfiguration der Instanzen] beschrieben sind. 
Zusätzlich erfahren Sie im Artikel über xref:cmk_commandline#folder_structure[{CMK} auf der Befehlszeile], wie die Verzeichnisstruktur von {CMK} aufgebaut ist.
...
[TIP]
====
Sobald Sie {CMK} produktiv betreiben, empfehlen wir Ihnen aus Sicherheitsgründen dringend, den Zugriff auf die Oberfläche ausschließlich gesichert zuzulassen, das heißt über HTTPS statt HTTP.
Was Sie dafür tun müssen, erfahren Sie im xref:omd_https#[Artikel über die Absicherung der Web-Oberfläche mit HTTPS].
====

Unter der oben genannten URL finden Sie den {CMK}-Anmeldedialog:
...
Dadurch landen Sie auf der Startseite von {CMK}, auf die wir im nächsten Kapitel näher eingehen werden:

.Die Startseite von {CMK} ist standardmäßig die Willkommens-Seite
image::intro_welcome.png[alt="Die Willkommens-Seite als Startseite.",fullscreen=1]

xref:intro_setup_monitor#[Weiter geht es mit der Einrichtung des Monitorings]
// TK: Wenn ich „des Monitorings“ lese, krieg ich Minikrise, aber laut Duden ist das nun mal der Genitiv...
