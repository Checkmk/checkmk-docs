...
In vielen Fällen verlangen Sicherheitsrichtlinien den Verzicht der Übertragung von Daten im Klartext oder die Verwendung vorhandener Kommunikationskanäle.
...
Sie übergeben {CMK} einen Befehl.
...
.Wann sollte ich Datenquellenprogramme einsetzen, wann Spezialagenten?
[%collapsible]
====
Datenquellenprogramme und xref:special_agents#[Spezialagenten] sind eng miteinander verwandt.
Letztlich sind Spezialagenten als Evolution von Datenquellenprogrammen zu betrachten.
Spezialagenten wurden zunächst genutzt, um Datenquellenprogramme besser konfigurierbar zu machen.

Heute liefern Spezialagenten in der Regel eigene Agentensektionen und werden mit eigenen Check-Plugins kombiniert.
In der Praxis relevant ist, dass einem Host nur ein Datenquellenprogramm zugeordnet sein kann, aber seit {CMK} {v24} beliebig viele Spezialagenten.
Auch können Spezialagenten die reguläre Agentenausgabe ergänzen, ein Datenquellenprogramm ersetzt sie hingegen.

Wenn Sie also beispielsweise über ein REST-API Daten einholen können, die zusätzlich zur regulären Agentenausgabe Nutzen im Monitoring haben, müssen Sie einen Spezialagenten verwenden.
Falls Sie eine reguläre Agentenausgabe einfach auf anderen Wegen übertragen wollen, sind Datenquellenprogramme der richtige Weg.
Selbstverständlich können Sie jedes Datenquellenprogramm – wenn denn einmal eine bessere Konfigurierbarkeit erforderlich sein sollte – in einen xref:devel_special_agents#[Spezialagenten umwandeln].
====
...
Um zu verhindern, dass {CMK} den Namen `myserver125` per DNS aufzulösen versucht, setzen Sie [.guihint]#IP address family# auf [.guihint]#No IP#.
