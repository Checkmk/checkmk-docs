...
In diesem Artikel stellen wir Ihnen mit {CSE} (vormals {CMK} Enterprise) eine der kommerziellen xref:glossar#edition[Editionen] vor, die im professionellen Umfeld am häufigsten anzutreffen ist.


[#editions]
== Einordnung von {SE}

Aufbauend auf der reinen Open Source Variante {CRE} (vormals {CMK} Raw) bietet {CSE} vor allem Verbesserungen der Performance und der Automatisierung, umfangreiche Möglichkeiten der Nutzung von Dashboards und ein Reporting. 
...
Daneben bietet {SE} einen offiziellen Support mit garantierter Reaktionszeit.

Auf {SE} baut wiederum xref:cce#[{CCE-onprem}] auf.

Ergänzend zum link:https://checkmk.com/de/preise[Editionsvergleich^] und zur Präsentation der link:https://checkmk.com/de/produkt/checkmk-pro[{SE}^] auf unserer Website zeigt dieser Artikel die Unterschiede auf technischer Ebene und versucht so, eine komplementäre Entscheidungshilfe bei der Wahl der Edition zu sein.
...
Sie können ein Upgrade der {RE} auf {SE} durchführen.
Folgen Sie dafür der xref:update#upgrade[Upgrade-Beschreibung].

// Downgrade nach {RE} wird hier nur erwähnt, weil wir keinen Editionsartikel für die {RE} haben.
Auch ein Downgrade ist möglich: von {UE} nach {SE} und von {SE} nach {RE}.
Folgen Sie dafür der xref:update#down2cee[Downgrade-Beschreibung].
...
|CMC |Die kommerziellen Editionen verwenden standardmäßig den {CMK} Micro Core, welcher durch Reduzierung auf die wesentliche Funktionalität und Optimierung dieser performanter ist als andere Monitoring-Kerne. +
Benutzer, die von Nagios-Umgebungen zu {CMK} migrieren, sollten die Artikel zum xref:cmc#[{CMK} Micro Core] und den xref:cmc_differences#[Besonderheiten des CMC] aufmerksam lesen, um zu beurteilen, ob es für eine Übergangszeit sinnvoll sein kann, {CMK} mit dem Nagios-Kern zu betreiben.
...
|Verteilte HW-/SW-Inventur |Die automatische Synchronisierung von Inventardaten im verteilten Monitoring macht Inventardaten zentral verfügbar.
...
|===
|Funktion |Ergänzende Informationen
|Dashboards |Nur in den kommerziellen Editionen können Dashboards im xref:dashboards#layout[Responsive-Layout] erstellt werden.
Das Anchored-Layout steht dagegen in allen Editionen zur Verfügung.
|Standard-Dashboard |Die kommerziellen Editionen verwenden als Standard ein xref:dashboards#usage[Dashboard], welches in {RE} nicht verfügbar ist.
Das [.guihint]#Main dashboard# von {RE} heißt in den kommerziellen Editionen [.guihint]#Problem dashboard#.
|Messwerte und Graphing |Erweiterte Funktionen des xref:graphing#[Graphings] stehen ausschließlich in den kommerziellen Editionen zur Verfügung.
Dazu gehören: PDF-Export, Graphensammlungen, Anpassungen (_graph tunings_) und benutzerdefinierte Graphen.
...
|Wartungszeiten (_scheduled downtimes_) |Nur in den kommerziellen Editionen können regelmäßig wiederkehrende xref:basics_downtimes#[Wartungszeiten] festgelegt werden.
Zudem ist über _Kommandos_ eine komfortable xref:commands#commands[Änderung oder Entfernung einer oder mehrerer Wartungszeiten] möglich.
|Verfügbarkeit |In den kommerziellen Editionen kann die Berechnung der xref:availability#[Verfügbarkeit] von Zeitperioden abhängig gemacht werden.
Verfügbarkeitsdaten können auch als PDF exportiert werden.
|PDF-Export von Ansichten |Der direkte PDF-Export von xref:views#export[Ansichten] im Monitoring ist den kommerziellen Editionen vorbehalten.
...
|===
|Funktion |Ergänzende Informationen
|Agentenbäckerei (_Agent Bakery_) |Die kommerziellen Editionen verfügen mit der xref:wato_monitoringagents#bakery[Agentenbäckerei] über einen Mechanismus, automatisch individuelle Agentenpakete mit Plugins und Konfigurationsdateien zu erstellen.
Einstellungen der Agenten-Konfiguration können mit der xref:bakery_api#[Bakery-API] in der Setup-GUI vorgenommen werden.
...
// TK: Muss man den folgenden Satz erwähnen oder ist das nicht sowieso klar?
Das gilt daher auch für die xref:agent_deployment#one_registration[gleichzeitige Registrierung] von Agent Controller und Agent Updater über die Befehlsschnittstelle `cmk-agent-ctl` des Agent Controllers.
|SNMP-Monitoring |Dank einer eigenen xref:snmp#performance[SNMP-Engine] _(Inline-SNMP)_ sind die kommerziellen Editionen performanter als {RE} mit der Nutzung von `snmpget`/`snmpbulkwalk`.
|{CMK}-Fetcher |In den kommerziellen Editionen sorgen die xref:cmc_differences#aux_processes[{CMK}-Fetcher] für effizientes Einholen der Monitoring-Daten.
|===
...
|Zentrale Benachrichtigungen |Im verteilten Monitoring kann der Benachrichtigungs-Spooler der kommerziellen Editionen so konfiguriert werden, dass er xref:distributed_monitoring#notifications[Benachrichtigungen von den Remote-Instanzen an alle Ziele] verschickt.
|===
...
|===
|Funktion |Ergänzende Informationen
|SAML |Die kommerziellen Editionen können nativ gegen xref:saml#[SAML] authentifizieren, in {RE} ist dies nur auf Apache-Ebene möglich.
...
|===
|Funktion |Ergänzende Informationen
|Test der Edition |Alle kommerziellen Editionen können in den ersten 30 Tagen (im Lizenzstatus „Trial“) in vollem Funktionsumfang unverbindlich getestet werden.
In kleinem Rahmen, d.h. mit _einer_ Instanz und bis zu 750 Services, kann {CMK} auch darüber hinaus dauerhaft ohne Subskription im Lizenzstatus „Free“ betrieben werden.
...
// ES: Schöner Text in der Tabelle. Allerdings verwirren mich die Stichworte "Bereitstellung" und "Test der Edition", verständlicher fände ich was mit "ausprobieren" oder "Testnutzung" oder "Betrieb von kommerziellen Editionen ohne Subskription".
// Warum ist diese Lizenzsache eigentlich unter "Unterschiede der Komponenten" einsortiert, das betrifft doch gar keine spezifische Komponente, sondern die Installation an sich?
