...
|CMC |Die kommerziellen Editionen verwenden standardmäßig den {CMK} Micro Core, welcher durch Reduzierung auf die wesentliche Funktionalität und Optimierung dieser performanter als andere Monitoring-Kerne ist. +
...
|===
|Funktion |Ergänzende Informationen
|Dashboards |Dashboards können nur in den kommerziellen Editionen im xref:dashboards.html#layout[Responsive-Layout] erstellt werden.
...
|PDF-Export von Ansichten |Der direkte PDF-Export von xref:views.html#export[Ansichten] im Monitoring ist den kommerziellen Editionen vorbehalten.
