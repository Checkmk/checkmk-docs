...
= Die {SE}
:revdate: 2024-05-07
:title: Die Standard Edition
:description: Die unterschiedlichen {CMK}-Editionen bieten einen unterschiedlichen Feature-Umfang. Dieser Artikel erklärt die Standard Edition.
...
|SNMP-Monitoring |Dank einer eigenen xref:snmp#[SNMP-Engine] _(Inline-SNMP)_ sind die kommerziellen Editionen performanter als die {RE} mit der Nutzung von `snmpget`/`snmpbulkwalk`.
...
|{CMK} Synthetic Monitoring |Das xref:robotmk#[{CMK} Synthetic Monitoring mit Robotmk] gibt es nur in den kommerziellen Editionen, benötigt jedoch eine zusätzliche Subskription.
//|Verteilte HW/SW-Inventur |Die automatische Synchronisierung von Inventardaten im xref:distributed_monitoring#[verteilten Monitoring] macht Inventardaten zentral verfügbar.
