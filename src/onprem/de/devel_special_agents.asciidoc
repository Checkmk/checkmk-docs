...
if lastsuccess + MAXAGE > now:
    ...
    print(data)
----
// ES: Schön gelöst mit der Erklärung des geteilten Storage-Objekts oben.
// ES: print(None) (in dem Fall, dass wir keine Daten bekommen und auch im Storage-Objekt nichts für uns steht) gibt soweit ich weiß tatsächlich None aus. Kann der Agent damit in der Sektion was anfangen? Falls nicht, sollten wir vielleicht im else-Block als Fallback lieber einen leeren String zuweisen.
...
In Kombination mit einer Konstanten für ein maximal zulässiges Alter (`MAXAGE` ist im Beispiel eine Stunde) sorgt dies dafür, dass bei zu alten Daten schlicht keine Agentenausgabe erfolgt und der Zustand des Service damit nach dieser Stunde zu {UNKNOWN} wechselt.
...
[TIP]
====
Der hier vorgestellte _Videokassettenrekorder (VCR) für REST-API-Aufrufe_ ist ausschließlich zu Debugging- und Testzwecken vorgesehen!
