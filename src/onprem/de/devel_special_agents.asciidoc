...
Sind Eigentümer oder Rechte nicht korrekt gesetzt, kann es zu Problemen und Inkompatibilitäten kommen.
====
...
Vorbereitend sollten Sie daher drei Ordner anlegen, die sich alle unterhalb von `local/lib/python3/cmk_addons/plugins/<Name>` befinden.
Der im Beispiel gezeigte vierte ist optional, er wird gegebenenfalls für einen agentenbasierten Check verwendet.
...
Falls Sie nach weiteren möglichen Fehlerquellen suchen, können Sie dies in den kommerziellen Editionen bequem über die  xref:glossar#mkp[Verwaltung von Erweiterungspaketen] machen.
Alternativ haben wir Ihnen im Abschnitt xref:detect_errors["Fehler beheben"] die wichtigsten Fehlerquellen und mögliche Lösungsansätze aufgelistet. 
...
In vielen Fällen sind Nutzername und Passwort oder ein API-Schlüssel zum Zugriff auf Daten notwendig.
...
Die Entwicklung dieser Plugins beschreibt der Artikel xref:devel_check_plugins#[Entwicklung agentenbasierter Check-Plugins] ausführlich.
...
| `APPLICATIONS` |  Anwendungsmonitoring
// | `CACHING_MESSAGE_QUEUES` | 
| `CLOUD` | Cloud Monitoring
| `CONFIGURATION_DEPLOYMENT` | Konfigurationsmanagement
| `DATABASES` | Monitoring von Datenbanken
| `GENERAL` | Alles, was nicht irgendwo anders reinpasst
...
| `NOTIFICATIONS` | Monitoring von Benachrichtigungsystemen
| `OPERATING_SYSTEM` | Betriebssystemmonitoring allgemein
