...
[IMPORTANT]
====
Wahrscheinlich ist Ihnen dieses schon bekannt, dennoch möchten wir noch einmal darauf hinweisen:
Auch im Zusammenhang mit der Entwicklung von Spezialagenten müssen Sie darauf achten, jeweils den korrekten Benutzer für die Dateierstellung zu verwenden.
Sind die Rechte nicht korrekt gesetzt, so kann es zu Problemen und Inkompatibilitäten kommen.
Achten Sie daher bitte auch bei den Beispielen in diesem Artikel stets darauf, wann welcher Benutzer zu verwenden ist.
====
...
Vorbereitend sollten Sie daher vier Ordner anlegen, die sich alle unterhalb von `local/lib/python3/cmk_addons/plugins/<Name>` befinden.
...
Diese Konfiguration führt den auszuführenden Spezialagenten mit den Einstellungen, die aus GUI und Standardeinstellungen bezogen werden, zusammen.
...
Falls Sie nach weiteren möglichen Fehlerquellen suchen, können Sie dies in den kommerziellen Editionen bequem über die 
xref:glossar#agent_bakery[Agentenbäckerei] machen. Alternativ haben wir Ihnen im Abschnitt xref:detect_errors[die wichtigsten Fehlerquellen] und mögliche 
Lösungsansätze aufgelistet. 
...
Wie im vorherigen Beispiel legen Sie als erstes die xref:prepare_env[benötigten Ordner] an - diesmal mit der Benennung `ometemp` anstelle von `hellospecial``.
...
Eine - nicht abschließende - Übersicht nutzbarer Gruppierungen finden Sie unter xref:files[Dateien und Verzeichnisse] am Ende dieses Artikels.
====
...
Nun starten Sie den Instanz-Webserver neu:
...
Viel macht diese allerdings noch nicht.
Sie enthält aktuell nur die beiden Felder für die Eingabe von Längen- und Breitengrad.

[#use_pwd]
==== Erweiterung: Passwörter verwenden

Für die gesicherte Kommunikation zwischen {CMK} und Open-Meteo können Sie in dem soeben erstellten Skript nun noch ein Passwort einsetzen.
Für die Verwaltung von Passwörtern stehen eigene `formspec`-Elemente zur Verfügung.
Diese erlauben es, entweder Passwörter ad hoc zu definieren, oder auf den xref:password_store#[Password Store] zuzugreifen.
...
Vergessen Sie nicht, nach der Erweiterung den Instanz-Apache erneut zu starten.
...
Die Werte für Längen- und Breitengrad, die Sie in der Regel angeben können, werden nun in das `Dictionary` `params` übertragen.
Gleichzeitig enthält das Objekt `host_config` alle Host-spezifischen Einstellungen, die hier genutzt werden sollen.
So haben Sie zum Beispiel mit `host_config.primary_ip_config.address` den Zugriff auf die primäre IP-Adresse, `host_config.name` enthält den Hostnamen.
...
Einen noch sichereren Weg, auf dem das gestartete Programm das Password direkt aus dem Password Store ausliest, bereiten wir gerade vor.
====
...
Die Entwicklung dieser Plugins beschreibt der Artikel xref:devel_check_plugins#[Entwicklung agentenbasierter Check-Plugins].

Ein Unterschied zum dort erwähnten Beispiel ist hier die Übergabe der JSON-Rückgabe der REST-API:
Wir verwenden die `itertools`, um aus dem zweidimensionalen Array ein eindimensionales zu machen.
Anschließend konkatenieren wir dieses Array mit Leerzeichen.
Längen- und Breitengrad werden also zu einer Variable zusammengefügt, getrennt durch Leerzeichen.
...
Wenn der Service Check_MK nach der Aktivierung des Spezialagenten plötzlich seinen Zustand auf {WARN} oder {CRIT} wechselt, sehen Sie sich die zugehörige [.guihint]#Summary# an.
 
image::devel_special_check_mk.png[alt="Fehlermeldung des Services Check_MK wegen des Spezialagenten."]

Lässt die [.guihint]#Summary# auf einen Zusammenhang mit dem neuen Spezialagenten schliessen, so prüfen Sie die Eigenschaften des Hosts.
In den [.guihint]#Checkmk agent / API integrations# muss die Option [.guihint]#Configured API integrations and Checkmk agent# anstelle des Standards gewählt sein.
...
Hinweis: War Ihre Regel vorher in {CMK} sichtbar und ist nun - nach einer Änderung an der Regelkonfiguration - plötzlich verschwunden, so 
...
Interessant sind nun vor allem die letzte Zeilen der Ausgabe.
Anscheinend enthält die Datei `server_side_calls/special_agent.py` einen ungültigen Parameter.
Dies sollten Sie sich im Dateicode genauer ansehen.
...
[#avail_integration]
=== Verfügbare Gruppierungen der "Other integrations"
