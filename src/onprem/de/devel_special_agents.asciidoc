...
Falls Sie einen Spezialagenten in Python 3 implementieren und dieser immer von einer {CMK}-Instanz ausgeführt wird (manche Spezialagenten sind so geschrieben, dass sie auch als Agentenplugin genutzt werden können) erhalten Sie mit dem API einige interessante Erweiterungen.
...
Einige REST-APIs verwenden Rate-Limiting, um die Zahl der eingehenden Anfragen zu reduzieren.
...
Bei dynamischem Rate-Limiting sollte das REST-API den link:https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/429[Status Code 429^] und die Angabe einer Wartezeit im _Retry-After_ Header-Feld zurückliefern.
...
Es handelt sich bei ihr um einen Schlüssel-Wert-Speicher, der einen String als Schlüssel verwendet, und auch als Werte nur Strings zulässt.
Zur Speicherung komplexerer Objekte ist die Verwendung von JSON meist ein pragmatischer Ansatz.

Eine Erweiterung um das Storage-API könnte so aussehen:
...
if now > nexttry:
    ...
    if response.status_code == 200:
        ...
        nexttry = now + float(response.header["retry-after"])
...
Wird weiterhin ein Status Code 429 zurückgeliefert, wird sich der Spezialagent mitunter ewig im `Storage`-Objekt bedienen.
...
Das im letzten Beispiel beschriebene Rate-Limiting kann besonders bei der Entwicklung von Spezialagenten stören, wo man denselben Aufruf oft zigmal hintereinander tätigt.
...
Wird mit dem Aufrufparameter `--vcrtrace` der Pfad zu einer Datei in `~/tmp/check_mk/debug` angegeben, werden unbekannte HTTP-Requests ausgeführt und deren Rückgaben aufgezeichnet.
Bekannte Requests werden direkt aus der Datei bedient, ohne dass für diese noch Anfragen an den Server stattfinden.
