...
Ein `Storage`-Objekt ist ein Schlüssel-Wert-Speicher, der einen String als Schlüssel verwendet und auch als Werte nur Strings zulässt.
Zur Speicherung komplexerer Objekte ist die Verwendung von JSON meist ein pragmatischer Ansatz.

Eine Erweiterung um die Storage-API könnte so aussehen:
...
Beachten Sie, dass das Beispiel bewusst extrem vereinfacht.
Wird weiterhin der HTTP-Status-Code 429 zurückgeliefert, wird sich der Spezialagent mitunter ewig im `Storage`-Objekt bedienen.
In den meisten Fällen dürfte es daher sinnvoll sein, zusätzlich den Zeitstempel des letzten erfolgreichen Aufrufs zu speichern und nach Erreichen eines maximalen Alters einen Fehler zurückzuliefern.


[#vcr]
=== Aufrufe und Antworten aufnehmen und abspielen

Das im letzten Beispiel beschriebene Rate-Limiting kann besonders bei der Entwicklung von Spezialagenten stören, wo man möglicherweise denselben Aufruf innerhalb kurzer Zeit immer wieder tätigt.
In solchen Fällen ist nicht nur Rate-Limiting problematisch, oft entstehen auch noch Kosten für REST-API-Aufrufe.

Hier hilft die Bibliothek link:https://pypi.org/project/vcrpy/[`vcrpy`^], welche es erlaubt, HTTP-Requests aufzuzeichnen und abzuspielen. Für die Einbindung stellt {CMK} eine Funktion bereit, welche Sie über den Argument-Parser aktivieren:
...
parser.add_argument(
    "--vcrtrace",
    help="Specify a tracefile to record/replay HTTP requests.")
args = parser.parse_args()

# ...
----

Wird mit dem Aufrufparameter `--vcrtrace` der Pfad zu einer Datei in `~/tmp/check_mk/debug` angegeben, wird der Spezialagent jeden HTTP-Request jeweils nur beim ersten Mal ausführen und die erhaltene Rückgabe aufzeichnen.
...
Ohne Aufrufparameter `--vcrtrace` verhält sich der Spezialagent wie gewohnt.
