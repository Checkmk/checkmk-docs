...
Einige REST-APIs verwenden _Rate Limiting_, um die Zahl der zu verarbeitenden eingehenden Anfragen zu reduzieren.
...
Der erste String ist der Identifikator des Plugins, der zweite kann beispielsweise der Host-Name sein -- oder in Fällen, in denen ein `Storage`-Objekt zwischen Hosts geteilt werden kann, eine URL, bei der bestimmte Parameter eine bestimmte Antwort bedingen.
...
Eine vergessene Trace-Datei im Produktiveinsatz wird ewig dieselbe Antwort -- und damit ganz sicher Haare raufen -- verursachen.
Wenn Sie die hier beschriebene Funktionalität Nutzenden gegenüber in _Rule Specs_ sichtbar machen, weisen Sie in der Inline-Hilfe klar darauf hin, dass diese Option nur auf Support-Aufforderung gesetzt werden sollte.
====
