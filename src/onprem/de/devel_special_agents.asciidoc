...
response = requests.get(url.format(fullurl))
print('<<<ometemp:sep(0)>>>')
print(data)
----
...
In den meisten Fällen dürfte es daher sinnvoll sein, zusätzlich den Zeitstempel des letzten erfolgreichen Aufrufes zu speichern und nach Erreichen eines maximalen Alters einen Fehler zurückzuliefern.


[#vcr]
=== Aufrufe und Antworten aufnehmen und abspielen

Das im letzten Beispiel beschriebene Rate-Limiting kann besonders bei der Entwicklung von Spezialagenten stören, wo man denselben Aufruf oft zigmal hintereinder tätigt.
