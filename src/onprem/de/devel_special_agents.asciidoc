...
Die Code-Beispiele dieses Artikels finden Sie in unserem link:https://github.com/Checkmk/checkmk-docs/tree/{gitbranch}/examples/devel_special_agents[GitHub-Repository^] und den fertig gebauten Open-Meteo-Spezialagenten zudem als link:https://exchange.checkmk.com/p/ometemp[MKP in der {CMK}-Exchange^].
...
----
<<<ometemp:sep(0)>>>
{"latitude":48.14,"longitude":11.6,"generationtime_ms":0.022292137145996094,"utc_offset_seconds":0,"timezone":"GMT","timezone_abbreviation":"GMT","elevation":536.0,"current_units":{"time":"iso8601","interval":"seconds","temperature_2m":"°C"},"current":{"time":"2026-07-01T18:15","interval":900,"temperature_2m":18.4}}
----

[TIP]
====
Im „echten Leben“ werden Sie immer wieder Phasen haben, in denen Sie intensiv mit Parametern und deren Übergabe experimentieren werden.
Um nicht bei jeder noch so kleinen Änderung die Regel- und Aufrufkonfiguration nachziehen zu müssen, können Sie den Spezialagenten auch testweise als xref:datasource_programs#[klassisches Datenquellprogramm] aufrufen.
Die benötigte Regel finden Sie unter [.guihint]#Setup > Agents > Other integrations > Individual program call instead of agent access#.
====
...
[{python}]
----
from cmk.rulesets.v1.form_specs import DefaultValue, Dictionary, DictElement, Float, String, Password, migrate_to_password
...
Dies konvertiert die gesamte Agentensektion in einen einzeiligen String.
Schließlich lesen wir diesen String mit `json.loads()` direkt als Objekt ein.
...
Die in diesem Artikel beschriebenen Skripte für Spezialagent, Regelkonfiguration und Aufrufkonfiguration in Kombination mit diesem Check-Plugin sollten Ihnen nun einen funktionierenden Service in {CMK} liefern:
...
{CMK} stellt einige Hilfsfunktionen bereit, die im Namensraum link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.server_side_programs/index.html#module-cmk.server_side_programs.v1_unstable[`cmk.server_side_programs.v1_unstable`^] verfügbar sind.
Falls Sie einen Spezialagenten in Python 3 implementieren und dieser immer von einer {CMK}-Instanz ausgeführt wird (manche Spezialagenten sind so geschrieben, dass sie auch als Agentenplugin genutzt werden können), erhalten Sie mit der API einige interessante Erweiterungen.

[TIP]
====
Wie der Import-Pfad bereits andeutet, handelt es sich hier um eine noch nicht als stabil markierte Programmierschnittstelle.
Wir planen, diese in {CMK} {v30} als stabil zu veröffentlichen.
Derzeit sind eher kleinere Erweiterungen als inkompatible Änderungen geplant.
Dennoch sollten Sie für die Migration Ihrer Plugins von {v25} nach {v30} zumindest etwas Zeit zum Testen einplanen.
====

[#storage]
=== Daten zwischenspeichern

Einige REST-APIs verwenden Rate-Limiting, um die Zahl der zu verarbeitenden eingehenden Anfragen zu reduzieren.
Nicht immer ist das Rate-Limiting statisch und verwendet immer exakt die gleiche zulässige Anzahl von Anfragen pro Zeiteinheit.
Bei dynamischem Rate-Limiting sollte die REST-API den link:https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/429[HTTP-Status-Code 429^] und die Angabe einer Wartezeit im Header-Feld namens _Retry-After_ zurückliefern.
Ist die Wartezeit für das Monitoring akzeptabel, kann es sinnvoll sein, auf zwischengespeicherte ältere Antworten zurückzugreifen.
Hierfür steht die Klasse `Storage` zur Verfügung.

Ein `Storage`-Objekt ist ein Schlüssel-Wert-Speicher, der ein Tupel aus zwei Strings als Schlüssel verwendet.
Der erste String ist der Identifikator des Plugins, der zweite kann beispielsweise der Hostname sein -- oder in Fällen, in denen ein `Storage`-Objekt zwischen Hosts geteilt werden kann, eine URL, bei der bestimmte Parameter eine bestimmte Antwort bedingen.
Auch als Werte sind nur Strings zulässig.
Zur Speicherung komplexerer Objekte ist die Verwendung von JSON meist ein pragmatischer Ansatz.

Eine Erweiterung um die Storage-API könnte demnach so aussehen:
...
fullurl = url.format(lat=args.latitude, long=args.longitude)

MAXAGE = 3600.0 # Consider data not refreshed for more than one hour as too old
now = time.time()
storage = Storage("ometemp", fullurl)
nexttry = float(storage.read("nexttry", "0"))
lastsuccess = float(storage.read("lastsuccess", "0"))
data = None
...
if now > nexttry:
    ...
    if response.status_code == 200:
        ...
        storage.write("nexttry", str(0))
        storage.write("lastsuccess", str(lastsuccess))
    elif response.status_code == 429:
        data = storage.read("data", None)
        nexttry = now + float(response.headers["retry-after"])
        storage.write("nexttry", str(nexttry))
else:
    data = storage.read("data", None)
    
if lastsuccess + MAXAGE > now:
    print('<<<ometemp:sep(0)>>>')
    print(data)
----
// ES: In der Zeile mit storage = Storage("ometemp", fullurl) übergeben wir hier die gesamte URL für ein Argument von Storage, das eigentlich einen Hostnamen erwartet. Ist das so geplant? Alternativ könnte für das Beispiel einfach ein Hostname hardgecodet werden.

// ES: Verstehe ich es richtig, dass die Logik hier keine Schleifen drehen soll? Pro Check-Intervall wird genau einmal versucht, die API abzufragen, und wenn es nicht klappt, schreiben wir einfach die Ergebnisse von der zuletzt erfolgreich durchgeführten Abfrage (also den gespeicherten String aus dem Storage-Objekt) in die Ausgabe?

// ES: Mich verwirrt hier das Feld "nexttry". Im Erfolgsfall erhalten wir Daten und speichern unter nexttry den genauen Zeitpunkt, zu dem wir zuletzt Daten erhalten haben. Im 429-Fall erhalten wir _keine_ Daten und speichern den Zeitpunkt, zu dem es nicht geklappt hat plus etwas Pufferzeit, nach der wir wieder einen Versuch starten können.

Wird weiterhin der HTTP-Status-Code 429 zurückgeliefert, bedient sich der Spezialagent auch künftig im `Storage`-Objekt.
Im erweiterten Spezialagenten wird daher zusätzlich der Zeitpunkt des letzten erfolgreichen REST-API-Zugriffs gespeichert.
In Kombination mit einer Konstanten für ein maximal zulässiges Alter (`MAXAGE` ist im Beispiel eine Stunde) sorgt dies dafür, dass bei zu alten Daten schlicht keine Agentenausgabe erfolgt und der Zustand des Services damit nach dieser Stunde zu {UNKNOWN} wechselt.


[#vcr]
=== Aufrufe und Antworten aufnehmen und abspielen

[TIP]
====
Der hier hier vorgestellte _Videokassettenrekorder (VCR) für REST-API-Aufrufe_ ist ausschließlich zu Debugging- und Testzwecken vorgesehen!
// Möglicherweise wissen nur die älteren Lesenden, was so ein link:https://en.wikipedia.org/wiki/Videotape[Rekorder ist^]...
Eine vergessene Trace-Datei im Produktiveinsatz wird ewig dieselbe Antwort -- und damit ganz sicher Haareraufen -- verursachen.
// "exponieren" oder "sichtbar machen"?
Wenn Sie die hier beschriebene Funktionalität Nutzenden gegenüber in Rulespecs sichtbar machen, weisen Sie bitte in der Inline-Hilfe klar darauf hin, dass diese Option nur auf Support-Aufforderung gesetzt werden sollte.
====

Das im letzten Beispiel beschriebene Rate-Limiting kann besonders bei der Entwicklung von Check-Plugins, welche die Ausgabe von Spezialagenten auswerten, stören.
Tätigt man denselben Aufruf innerhalb kurzer Zeit immer wieder, ist nicht nur Rate-Limiting problematisch, oft entstehen auch noch Kosten für REST-API-Aufrufe.

Hier hilft die Bibliothek link:https://pypi.org/project/vcrpy/[`vcrpy` (_Video Cassette Recorder for Python_)^], welche es erlaubt, HTTP-Requests aufzuzeichnen und immer und immer wieder abzuspielen. Für die Einbindung stellt {CMK} link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.server_side_programs/index.html#cmk.server_side_programs.v1_unstable.vcrtrace[eine Funktion bereit^], welche Sie über den Argument-Parser aktivieren.
Wird mit dem Aufrufparameter `--vcrtrace` der Pfad zu einer Trace-Datei in `~/tmp/check_mk/debug` angegeben, wird der Spezialagent jeden HTTP-Request jeweils nur beim ersten Mal ausführen und die erhaltene Rückgabe aufzeichnen.
Wenn später genau der gleiche HTTP-Request wieder gestellt werden soll, wird dieser bereits aufgezeichnete Request direkt aus der Datei bedient, ohne dass eine neue Anfrage an den Server stattfindet.

Ohne Aufrufparameter `--vcrtrace` verhält sich auch der so angepasste Spezialagent weiterhin unauffällig.
// ES: Hier wäre noch ein Hinweis schön, dass die aufgezeichneten Daten niemals verfallen und das Speichern deshalb nur in bestimmten Anwendungskontexten überhaupt sinnvoll sein wird. Wetterdaten im produktiven Monitoring aus dem Videorekorder zu holen ist eher keine vielversprechende Idee. In der Developer-Doku wird ja auch explizit betont, dass diese Funktion vor allem zum Debuggen gedacht ist.


[#detect_errors]
== Fehler beheben

Auch bei der Entwicklung eigener Spezialagenten kann es immer wieder mal zu Fehlern und Problemen kommen.
Deren Ursachen zu lokalisieren ist dann die Voraussetzung für die folgende Behebung.
Für erste Hinweise auf mögliche Ursachen sollten Sie zunächst `cmk-validate-config` aufrufen.
Die nächsten Abschnitte helfen, weiter einzugrenzen.
...
[#activate_changes]
=== Fehlermeldung bei der Aktivierung ausstehender Änderungen

Möglicherweise gibt es auch ein Problem in der Aufrufkonfiguration oder am eigentlichen Spezialagenten.
Dies wird zum Beispiel sichtbar, weil beim [.guihint]#Activate pending changes# ein gelber Warnhinweis angezeigt wird.
