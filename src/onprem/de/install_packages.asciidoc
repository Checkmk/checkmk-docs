...
xref:install_packages_redhat#[Installation unter Red Hat und Derivaten]
...
xref:update_matrix#[Update-Matrix für Version {v23}]
{related-end}
...
Exakte Versionsnummern unterstützter Distributionen entnehmen Sie bitte der xref:update_matrix#matrix[Kompatibilitätsmatrix], auf derselben Seite finden Sie auch die xref:update_matrix#ossupport[Richtlinie zur Betriebssystemunterstützung (_OS Support Policy_)].
