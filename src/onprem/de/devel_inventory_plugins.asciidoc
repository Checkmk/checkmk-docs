...
Wählen Sie im neuen Browserfenster in der linken Navigationsleiste [.guihint]#Agent based ("Check API") > Version 2: cmk.agent_based.v2# aus.
...
// TK: Klarer machen, dass hier ein neues Agentenplugin geschrieben wird, das eine neue Agentensektion mitliefert.
...
// TK: Das klingt so, als ob Du hier das Check-Plugin baust und nicht das Agentenplugin.
...
Falls der Host, auf dem Sie das Agentenplugin testen, eine virtuelle Maschine ist oder Sie nicht ständig Geräte entfernen und anschließen wollen, speichern Sie das folgende Beispiel einfach in einer Datei im xref:spool_directory#[Spool-Verzeichnis].
...
Der Aufbau ähnelt dem von xref:devel_check_plugins#write_check_plugin[Check-Plugins].
...
{c-omd} cmk -vv --inventory myhost | grep lsusb_demo
...
// MFS: Zweite Iteration, siehe Diskussion
...
Je Zeile entsteht eine Liste von Tokens.
Jede dieser Listen wiederum ist Element einer Liste von Zeilen.
Wird ein Separator angegeben, definiert dieser die Grenze zwischen Tokens.
...
Während die maximal unterstützte USB-Version xref:scaffold[oben] im simplen Beispiel noch einfach hart kodiert wurde, können Sie sich gerne Gedanken machen, wie Sie diese mittelfristig mit regulären Ausdrücken aus der Beschreibung ermitteln.
...
// ES: Offenbar können Inventur-Plugins problemlos geparsete Agentensektionen verarbeiten, es wird nur halt in diesem Beispiel nicht gemacht. Siehe Docstring für die Klasse InventoryPlugin in packages/cmk-plugin-apis/cmk/agent_based/v2/_plugins.py
// ES: Der "Gegensatz" ist hier also eigentlich keiner, sondern nur ein Effekt vom Aufbau des Beispiels, das wir hier durchspielen.
...
Schlüsselspalten haben die Besonderheit, dass sie zur Erkennung von Änderungen herangezogen werden, die wiederum im Monitoring zu einem Statuswechsel des Service [.guihint]#Check_MK HW/SW Inventory# führen können.
...
// ES: Stimmt nicht, sie werden gemerget bzw. im Konfliktfall überschrieben, sodass die jeweils späteste Zeile mit den gleichen Schlüsseln gewinnt.
...
// ES: Hier für das bessere Verständnis evtl. den relevanten Teil von Werk #19313 über status_columns und das Verhalten bei Abwesenheit von inventory_columns einflechten
...
Da die Inventur-API Teil der Check-API ist, gestaltet sich die Erstellung und Anwendung von Regelsätzen exakt wie bei der Entwicklung regulärer Check-Plugins.
...
Daher sollen die Benutzer die Möglichkeit haben, selbst zu entscheiden, ob Hubs erfasst werden sollen oder nicht.
Die Einstellung sollen sie über eine Checkbox in einer Regel vornehmen können.
...
Die Verknüpfung erfolgt im xref:apply_ruleset[nächsten Abschnitt].

Sie können die Regel jetzt in der GUI unter [.guihint]#Setup > Hosts > HW/SW inventory rules# einsehen und anpassen.
Ist der Name einer Regel bekannt (hier: [.guihint]#Lsusb inventory demo#), können Sie diesen direkt ins xref:user_interface#search[Suchfeld] tippen.
...
Um ihn anzuwenden, ergänzen Sie zwei weitere [.notranslate]#_Named Arguments_# bei der Erstellung der `InventoryPlugin`-Klasse.
...
[#display]
== Darstellung von Inventurdaten anpassen

Beim Blick in die HW/SW-Inventur fällt auf, dass die Spalten in einer anderen Reihenfolge als im Plugin definiert dargestellt werden -- nämlich alphabetisch sortiert.
Für die Verbesserung der Darstellung kommt die link:https://docs.checkmk.com/plugin-api/{current-major}/cmk.inventory_ui/index.html#module-cmk.inventory_ui.v1_unstable[UI-API der Inventur^] aus dem Namensraum `cmk.inventory_ui.v1_unstable` ins Spiel. 
...
Dateien, welche die UI-API nutzen, liegen im Unterordner `inventory_ui` Ihrer Plugin-Familie:

[{shell}]
----
{c-omd} mkdir -p ~/local/lib/python3/cmk_addons/plugins/lsusb_demo/inventory_ui
{c-omd} cd ~/local/lib/python3/cmk_addons/plugins/lsusb_demo/inventory_ui
----

Im vorgestellten Beispiel werden alle Daten als Textfelder gehandhabt.
Damit genügt es, die Reihenfolge der Spalten zu ändern und die Spaltentitel zu setzen:
Für den Dateinamen verwendet {CMK} als Konvention `hardware.py`, `software.py` oder `networking.py`.
Von dieser Konvention darf abgewichen werden, wenn es der Übersichtlichkeit dient.

.~/local/lib/python3/cmk_addons/plugins/lsusb_demo/inventory_ui/hardware.py
[{python}]
----
#!/usr/bin/env python3
...
node_hardware_usb_devices = Node(
    name = "hardware_usb_devices",
    path = [ "hardware", "usb", "devices" ],
    title = Title("Devices"),
    table = Table(
        columns = {
            "vendor": TextField(Title("Vendor ID")),
            "device": TextField(Title("Device ID")),
...
.Nach Verwendung der UI-API erscheinen die Spalten in der gewünschten Reihenfolge
image::devel_inventory_plugins_uiapi.png[alt="Inventory tree mit aufgeklapptem Hardware > USB > Devices."]
// ES: Einheitliche Benennung des Inventurbaums/Inventory tree durch den ganzen Artikel, ich bevorzuge die deutsche Variante

Angenommen, Sie haben ein Inventur-Plugin für einen Massenspeicher entwickelt, der bislang nicht unterstützt wurde, dann können Sie mit der UI-API die korrekte Zahlendarstellung erreichen:

.~/local/lib/python3/cmk_addons/plugins/fictional_mass_storage/inventory_ui/hardware.py
[{python}]
----
#!/usr/bin/env python3

from cmk.inventory_ui.v1_unstable import Node, Table, TextField, NumberField, Title, IECNotation, AgeNotation, Unit

UNIT_BYTES = Unit(IECNotation("B"))
UNIT_AGE = Unit(AgeNotation())

node_hardware_somebus_disks = Node(
    name = "hardware_somebus_disks",
    path = [ "hardware", "somebus", "disks" ],
    title = Title("Mass storage devices"),
    table = Table(
        columns = {
            "device": TextField(Title("Device ID")),
            "total_space": NumberField(Title("Total disk space"), render = UNIT_BYTES),
            "total_uptime": NumberField(Title("Uptime"), render = UNIT_AGE),
        },
    ),
)
----

Neben verschiedenen Möglichkeiten, Text- und Zahlendarstellung zu beeinflussen, bringt die API auch Möglichkeiten zur Änderung der Hintergrundfarbe von Feldern -- üblicherweise in Kombination mit booleschen Werten.
Darüber hinaus können Sie eigene Sortier- und Rendering-Funktionen definieren.
...
Im Beispiel haben Sie sich bisher an das Standardverhalten gehalten:
Ein Inventur-Plugin abonniert eine Agentensektion, welche denselben Namen trägt wie das Inventur-Plugin.
Aber was tun, wenn Sie vorhandene Agentensektionen mitverwenden wollen, die bereits von einem vorhandenen Inventur-Plugin ausgewertet werden?
Hier ist die Wahrscheinlichkeit hoch, dass bereits ein Inventur-Plugin existiert, das eben diesen Namen der Agentensektion verwendet.
So verschaffen Sie sich einen Überblick über bereits existierende Sektionen:

[{shell}]
----
{c-omd} cmk -vv --inventory myhost
...
[{python}]
----
inventory_plugin_fictional = InventoryPlugin( 
    name = "fictional",
...
// TK: Der Link auf https://www.gnu.org/licenses/old-licenses/gpl-2.0.html ist ein Suchspiel?
// MFS: ICYKYK... Wer Software entwickelt/Bibliotheken nutzt, hat die GPL wenigstens einmal gelesen und kennt das Beispiel. Der Link ist nur eine Gedächtnisstütze: "Ach, da hab ich den Namen schonmal gelesen!"
...
In der Inventur von {CMK} sehen die entstandenen Knoten dann so aus:

.Hier wertet eine einzige Inventurfunktion zwei Agentensektionen aus
image::devel_inventory_plugins_multiple_sections.png[alt="Inventory tree mit den aufgeklappten Nodes für Software > Fictional > Example und Hardware > Fictional > Example."]
// ES: Screenshot ist nicht committet