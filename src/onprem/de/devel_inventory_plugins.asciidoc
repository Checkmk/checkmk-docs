...
Schlüsselspalten haben die Besonderheit, dass sie zur Erkennung von Änderungen herangezogen werden, die wiederum im Monitoring zu einem Statuswechsel des Services [.guihint]#HW/SW Inventory# führen können.
...
Für die Verbesserung der Darstellung kommt das link:https://docs.checkmk.com/plugin-api/{current-major}/cmk.inventory_ui/index.html#module-cmk.inventory_ui.v1_unstable[UI-API der Inventur^] aus dem Namensraum `cmk.inventory_ui.v1_unstable` ins Spiel. 
...
Dateien, welche das UI-API nutzen, liegen im Unterordner `inventory_ui` Ihrer Plugin-Familie:
...
.~/local/lib/python3/cmk_addons/plugins/lsusb_demo/agent_based/hardware.py
[{python}]
----
#!/usr/bin/env python3
...
.Nach Verwendung des UI-API erscheinen die Spalten in der gewünschten Reihenfolge
...
.~/local/lib/python3/cmk_addons/plugins/lsusb_demo/fictionary_mass_storage/hardware.py
[{python}]
----
#!/usr/bin/env python3

from cmk.inventory_ui.v1_unstable import Node, Table, TextField, NumberField, Title, IECNotation, AgeNotation
...
Neben verschiedenen Möglichkeiten, Text- und Zahlendarstellung zu beeinflussen, bringt das API auch Möglichkeiten zur Änderung der Hintergrundfarbe von Feldern – üblicherweise in Kombination mit booleschen Werten.
...
+ EXECUTING INVENTORY PLUGINS
 ...
 robotmk: skipped (no data)
----
// TK: Schöne Ausgabe. Und jetzt herrscht Klarheit? Nicht für mich. Was lerne ich hier?
// MFS: Einleitender Satz geändert.
