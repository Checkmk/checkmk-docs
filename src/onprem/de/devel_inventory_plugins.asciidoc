...
Die HW/SW-Inventur hilft, den Überblick über vorhandene Hardware zu behalten und jederzeit Kontrolle über installierte Software zu haben.
...
Check-Plugins für sich schnell ändernde Daten, Inventur-Plugins für selten geänderte Daten.
In diesem Artikel verwenden wir das etwas an den Haaren herbeigezogene Beispiel mit USB-Geräten.
Wollen Sie beispielsweise sicherstellen, dass niemand an irgendeinen Rechner im Monitoring USB-Geräte anstöpselt, die nicht auf einer Positivliste stehen, ist dafür ein Check-Plugin geeigneter.
====
...
Die Inventur-API ist Teil der Check-API und daher gilt für den Zugriff auf Dokumentation dasselbe wie bei agentenbasierten Check-Plugins.
...
Es folgt – durch Doppelpunkt getrennt – zuerst die Hersteller-ID, dann die Geräte-IT.
...
Spool-Dateien werden in die Agentenausgabe aufgenommen und mitübertragen.
...
Das An- und Abstöpseln von USB-Geräten können Sie dann simulieren, indem Sie der Textdatei Zeilen hinzu fügen oder welche entfernen.
...
Inventur-Plugins leben in den regulären Verzeichnisstruktur neben den normalen Check-Plugins.
...
Die Inventur-Funktion entspricht der Check-Funktion und die Erstellung des `InventoryPlugin`-Objektes erfolgt analog der eines `CheckPlugin`-Objektes.

Die `Attributes`, welche per `yield` übergeben werden, implementieren das minimal sinnvolle Blatt eines Inventurbaumes:

* Der Pfad (`path`) gibt Hierarchie und Ebene im Baum an, hier [.guihint]#Hardware > Usb > General#
* Die Schlüssel-Wert-Pärchen im Objekt `inventory_attributes` werden als Schlüssel-Wert-Tabelle dargestellt

Testen Sie zunächst, ob das Plugin syntaktisch korrekt ist und bei der Invetur gestartet werden kann:
...
Ist das der Fall, aktualisieren Sie die Konfiguration für den Monitoring-Kern und starten dann Ihre Site neu:
...
Ein Blick in das Inventar des Test-Hosts zeigt nun ein neues Blatt im Inventarbaum:


[#parse_function]
=== Die Parse-Funktion schreiben

Agentensektionen ohne Angabe eines Separators werden in vor der Übergabe an die Inventurfunktion in eine zweidimensionale Liste von Worten umgewandelt:
...
Das erleichtert die Verarbeitung beispielsweise von als CSV formatierten Daten.
...
In unserem Beispiel interessiert nicht, wo ein USB-Gerät angestöpselt ist.
...
Hier können Sie ein wenig abkürzen: Im Gegensatz zu agentenbasierten Check-Plugins wird die Parse-Funktion nicht über Erzeugung eines `AgentSection`-Objektes eingebunden, sondern muss aus der Inventurfunktion selbst aufgerufen werden.
...
Die dritt, rein informative Spalte enthält die Beschreibung.
...
Desweiteren müssen Schlüssel eindeutig sein.
...
Da das Inventur-API Teil des Check-Plugin-APIs ist, gestaltet sich die Erstellung und Anwendung von Regelsätzen exakt wie bei der Entwicklung regulärer Check-Plugins.
...
Abgelegt wird diese im Ordner `rulesets` der Plugin-Familie:
...
Die Verwendung des Regelsatzes funktioniert ebenso über die Ergänzung weiterer [.notranslate]#_Named Arguments_# bei der Erstellung der `InventoryPlugin` Klasse.
...
Analog muss der Inventur-Funktion als erstes Argument das Parameter-Dict übergeben werden:
...
Wenn Sie vorhandene Agentensektionen mitverwenden wollen, die bereits von einem vorhandenen Inventur-Plugin ausgewertet werden, ist die Wahrscheinlichkeit hoch, dass bereits ein Inventur-Plugin existiert, dass den Namen der Agentensektion verwendet.
