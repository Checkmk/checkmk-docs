...
Es folgt – durch Doppelpunkt getrennt – zuerst die Hersteller-ID, dann die Geräte-ID.
...
Falls der Host, auf dem Sie das Agenten-Plugin testen, eine virtuelle Maschine ist, speichern Sie das folgende Beispiel einfach in einer Spool-Datei.
...
Das An- und Abstöpseln von USB-Geräten können Sie dann simulieren, indem Sie der Textdatei Zeilen hinzufügen oder welche entfernen.
...
Ein Blick in das Inventar des Test-Hosts zeigt nun ein neues Blatt im Inventurbaum:

// AI: Screenshot fehlt. Hier sollte ein image::-Makro mit dem Inventurbaum-Screenshot folgen, analog zu anderen Artikeln (z.B. image::devel_inv_tree.png[alt="..."]).
...
// AI: `Label` wird hier und unten noch einmal importiert (doppelter Import). Einen der beiden Imports entfernen. Außerdem wird `Help` importiert, aber im Beispiel nicht verwendet.
...
// AI: Dieser auskommentierte Block (bis zum schließenden `////`) enthält kopierten Inhalt aus `devel_check_plugins`. Bitte prüfen, ob er noch benötigt wird oder gelöscht werden kann.
////
---
