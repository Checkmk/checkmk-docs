...
Wenn Sie das Skript für den Regelsatz abgespeichert haben, validieren Sie es mit `cmk-validate-plugins` und starten dann die Instanz mit `omd restart` neu.
Dass `cmk-validate-plugins` vorhandene aber nicht verknüpfte Regelsätze anmeckert, dürfen Sie an dieser Stelle ignorieren.
Anschließend können Sie diese Regel in der GUI unter [.guihint]#Setup > Hosts > HW/SW inventory rules# einsehen und anpassen.
...
// MFS: Regeln sind sichtbar, bevor sie verknüpft sind. cmk-validate-plugins meckert meist, wenn vorhandene Regeln nicht verknüpft sind.
...
// MFS: vorangestellt:
Im Beispiel haben wir bisher das Standardverhalten verwendet:
