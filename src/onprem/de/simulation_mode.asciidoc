...
:description: Um {CMK} ohne Netzwerkzugriff zu testen, können Sie mit Daten aus dem Cache den Betrieb simulieren. Lesen Sie hier, wie Sie den Simulationsmodus nutzen.
...
Alle anderen Dinge in der xref:glossar#site[Instanz] laufen ganz normal weiter:
...
* Konfigurierte Sicherungsaufträge werden zu ihrem definierten Zeitpunkt ausgeführt.
...
Services, die Metriken zu ihrer Funktion benötigen, werden also xref:monitoring_basics#stale[stale.]


[#activation]
== Aktivierung

Sie können den Simulationsmodus direkt im [.guihint]#Setup# anschalten mit  [.guihint]#Setup > General > Global settings > Execution of checks > Simulation mode#:

image::omd_basics_simulation.png[]

Generell raten wir entschieden davon ab, Konfigurationsdateien von {CMK} zu bearbeiten.
Für den folgenden, speziellen Anwendungsfall kann es aber sinnvoll sein, den Simulationsmodus statt im [.guihint]#Setup# direkt in einer Konfigurationsdatei zu aktivieren:
Und zwar dann, wenn Sie eine Kopie einer Instanz erstellt haben und den Modus aktivieren wollen, bevor Sie die Instanz starten.
In diesem Fall fügen Sie in der kopierten Instanz manuell in der Datei `~/etc/check_mk/conf.d/wato/global.mk` die folgende Zeile hinzu:
...
Ist der `simulation_mode` in dieser Datei nicht gesetzt, wird der Standardwert (off) benutzt.
