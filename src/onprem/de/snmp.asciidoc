...
// MFS: Das doppelte "einfach" liegt wegen des per Präprozessors-Statement entfernten Abschnittes sonst zu eng beieinander.
ifdef::onprem[]
...
nutzt für die SNMP-Überwachung _aktive Anfragen_ – also eine Pull-Methode.
Dabei sendet
ifdef::onprem[]
{CMK}
endif::[]
...
In einer xref:distributed_monitoring#[verteilten Umgebung] oder bei Verwendung des xref:relay#[Relays] ist die IP-Adresse der Maschine entscheidend, von der aus das Gerät überwacht wird.
endif::[]
ifdef::saas[]
. Tragen Sie als erlaubte IP-Adresse die Adresse des verwendeten Relays ein.
endif::[]
...
ifdef::saas[]
Im Kasten [.guihint]#Basic settings# müssen Sie zunächst das Relay auswählen, mit dem der SNMP-Host überwacht werden soll.
endif::[]
ifdef::onprem[]
In {UE} können Sie im Kasten [.guihint]#Basic settings# ein Relay auswählen, mit dem der SNMP-Host überwacht werden soll, wenn er nicht direkt vom {CMK}-Server aus erreichbar ist.
endif::[]
...
ifdef::onprem[]
Treten die Probleme mit den kommerziellen Editionen zu Tage, kann ein Grund darin liegen, dass die dort standardmäßig aktivierte,
endif::[]
ifdef::saas[]
Ein Grund bei Problemen kann sein, dass die standardmäßig aktivierte,
endif::[]
performantere xref:performance[Inline-SNMP] Implementierung sich stärker auf die Einhaltung von Standards verlässt als `snmpget`.
...
Auch das zusammen mit {UE} verwendbare Relay unterstützt beide Abfragemöglichkeiten.
endif::[]
ifdef::saas[]
Das {CMK} Relay unterstützt zwei Abfragemöglichkeiten für SNMP: die Kommandozeilenbefehle `snmpget` bzw. `snmpbulkwalk` und eine eingebaute SNMP-Engine, die SNMP-Anfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen.
