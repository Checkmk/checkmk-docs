...
Sollte allerdings für eine Hard- oder Software-Komponente (beispielsweise einen RAID-Controller) kein {CMK}-Agenten-Plugin zur Überwachung -- dafür jedoch eine SNMP-Schnittstelle -- existieren, können Sie selbstverständlich über SNMP zusätzliche Monitoring-Daten erheben.
...
[TIP]
====
SNMPv3 wird -- vorausgesetzt Sie haben die passende Regel aktiviert -- ausschließlich auf diejenigen Hosts angewendet, die in ihrer Konfiguration Zugangsdaten im SNMPv3-Stil enthalten. SNMPv2c wird nur auf diejenigen Hosts angewendet, für die explizit der Regelsatz [.guihint]#Enable SNMPv2c for hosts# aktiviert wurde. Für alle anderen Hosts gilt automatisch immer SNMPv1.
====
...
Dadurch wird der Host gezwungen, trotz eingestellter Version 1 das Protokoll SNMPv2c zu verwenden, allerdings _ohne Bulkwalk._
...
Hier kann es helfen, auf SNMPv1 zurück zu gehen (was normalerweise viel langsamer ist, aber manchmal immer noch schneller als ein kaputtes SNMP v2c).
...
|`authPriv` |Benutzer-basierte Authentifizierung wie bei `authNoPriv`, zusätzlich werden alle Daten verschlüsselt. Hierzu müssen Sie manuell einen Schlüssel austauschen und ihn sowohl im Gerät als auch in {CMK} hinterlegen.
|===
