...
Auch mit welchem {CMK}-Patch-Release die Unterstützung für neue Linux-Distributionsversionen hinzugekommen ist, zeigt die xref:update_matrix#[Update-Matrix].
...
----
// ES: Sind die Anführungszeichen nötig? Bei mir klappt es auch ohne.
...
// ES: Für mich ist "Rollback" ein Zurückversetzen eines Objekts in einen Zustand, der exakt so schon einmal dagewesen ist - also nicht nur auf eine ältere Version bezogen, sondern auf alle Konfigurationen, überwachten Objekte im Monitoring usw. Ist ein Szenario denkbar, in dem wir unserer Instanz eine ältere Version von {CMK} zuweisen, auch wenn genau diese Instanz niemals zuvor genau unter dieser älteren Version lief?
// Falls du extra betonen wolltest, dass man das Downdate nur machen soll, wenn man direkt davor das Update gemacht hat und irgendwie nicht zufrieden ist, dann ist für mich Rollback der passende Begriff. Ansonsten würde ich hier nicht so konkret über Rollbacks sprechen.
...
. Zuerst werden Standarddateien unter `~/etc/` und `~/var/` aktualisiert, also die Dateien, die bei der Instanz-Erstellung mit `omd create` erzeugt wurden.
...
// ES: Die Aufgabe dieses Abschnitts wird mir anhand der Überschrift nicht so richtig klar. Findest du eine aussagekräftigere Überschrift? Präposition statt Konjunktion könnte schon helfen.
