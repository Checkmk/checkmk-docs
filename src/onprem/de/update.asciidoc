...
Dem _Upgrade_ oder _Downgrade_ der {CMK}-Edition – also dem Wechsel zu einer Edition mit anderem Funktionsumfang, beispielsweise von {RE} auf eine der kommerziellen Editionen oder zurück – ist der separate Artikel xref:upgrade#[Upgrades] gewidmet.
====
...
Die in den folgenden Abschnitten beschriebenen Themen sind bei jedem Update relevant, egal ob innerhalb einer Major-Version von einer niedrigeren auf eine höhere Patch-Version oder von einer Major-Version auf die nächste.
...
. Dann ist es Zeit, xref:local_files[aufzuräumen]: Im lokalen Verzeichnis sollten keine Dateien mehr vorliegen, die keinem Paket mehr zugeordnet sind.
...
Ist dies nicht der Fall, bündeln Sie zusammengehörende Dateien jeweils in xref:devel_mkps#[{CMK}-Erweiterungspaketen (MKPs)].
...
Beim Update auf eine höheres Patch-Version innerhalb einer Major-Version sind bezüglich der Kompatibilität von MKPs selten Überraschungen zu erwarten.
Auszuschließen sind Inkompatibilitäten jedoch nie.
Beispielsweise setzen manche Entwickler von Erweiterungen die höchste Kompatibilität auf eine bestimmte Patch-Version von {CMK}, wenn ein Anbieter einer REST-API eine Version zu einem bestimmten Datum abkündigt.
...
Ist das nicht der Fall – oder die Erweiterung selbst entwickelt –, finden Sie im Artikel xref:devel_intro#[Erweiterungen für {CMK} entwickeln] alle nötigen Informationen, um Ihre Erweiterungen anzupassen. 
...
Detaillierte Beispiele zum Backup mit `omd backup` finden Sie im Artikel zur xref:omd_basics#omd_backup_restore[Instanzverwaltung mit omd].
...
. Als erstes wird mit dem Paketmanager der Linux-Distribution das xref:install_new_versions[neue {CMK}-Paket installiert] – oder die neuen Pakete,
wenn vor dem Update der Major-Version die letzte Patch-Version der vorhandenen Major-Version installiert werden soll.

. Es folgt die xref:execute[Aktivierung der neuen {CMK}-Version] für die zu aktualisierende Instanz mittels `omd update`.
Gegebenenfalls ist der Vorgang für jeden Schritt des Update-Pfades zu wiederholen.
...
Wenn Sie gleichzeitig zum _Versions-Update_ ein xref:upgrade#[_Editions-Upgrade_] von {RE} auf eine der kommerziellen Editionen durchführen, werden Sie sicherheitshalber noch einmal auf diesen Umstand hingewiesen:
...
////
SK:
Auch wenn das Kommando update-apache-config heißt, finde ich es ungünstig hier das Wort "Apache" zu verwenden. Kann sein, dass wir noch 20 Jahre Apache verwenden und der auch für immer so heißt, aber ein generischer Begriff, gefiele mir hier besser. Man könnte da dann sowas wie "akuell: Apache" hinschreiben.

Der ganze Abschnitt ist schon sehr allgemein und ich weiß nicht, ob er der Panik, die manche Nutzer nach einem Update verspüren, gerecht wird.

Setzen wir eigentlich irgendwo voraus, dass ein System-Apache vorhanden ist? Wir das geprüft und ist das ein Requirement?
////
...
Zusätzlich haben Sie mit dem Menüeintrag [.guihint]#Help > About {CMK} > Change log (Werks)# Zugriff auf die komplette Historie der Änderungen in der aktuellen Major-Version.
...
[#mkps_central_setup]
=== Erweiterungspakete in zentralem Setup
...
Zeigt eine Viewer-Instanz nur Daten von Remote-Instanzen, kann deren Aktualisierung erfolgen, sobald die Remote-Instanzen aktualisiert sind.
Zeigt die Viewer-Instanz dagegen auch Daten der Zentralinstanz, darf sie erst als letztes aktualisiert werden.
...
|*t* |Ihre Originaldatei -- ohne die bereits erfolgreich zusammengeführten Änderungen -- wird in einem Editor geöffnet. Editieren Sie nun die Datei, um eventuellen Konflikten aus dem Weg zu gehen. Nach dem Schließen des Editors probiert {CMK} die Zusammenführung erneut.
...
|*i* |Installiert die neue Standardversion der Datei: Ihre Änderungen an der Datei gehen verloren.
...
Für mit vielen Subkommandos von `omd` verwendbare Parameter werfen Sie einen Blick in den Artikel zur xref:omd_basics#[Instanzverwaltung mit `omd`].
...
Wollen Sie sichergehen, dass Ihre Skripte auch auf Servern funktionieren, auf denen {CMK} {v24} das Standard-`omd` bereitstellt, schauen Sie in link:https://docs.checkmk.com/2.4.0/de/update.html#scripted[diesen Artikel im Handbuch von {CMK} {v24}^].
...
|Obsolete |In der neuen Version ist eine Datei weggefallen. Sie haben diese Datei sowieso schon gelöscht. Nichts passiert.
