...
Fehlschlagen kann ein Update durch einen unerwarteten internen Fehler, aber auch durch Benutzereingabe während des Update-Prozesses, zum Beispiel durch Auswahl der Menüoption `abort` oder durch Drücken der Tastenkombination kbd:[Ctrl] + kbd:[C].
...
|*s* |Wenn Sie unsicher sind, können Sie mit *s* eine Shell öffnen. Sie befinden sich im Verzeichnis, in dem die betroffene Datei liegt, und können sich ein Bild von der Lage machen. Beenden Sie die Shell mit kbd:[Ctrl] + kbd:[D], um das Update fortzusetzen.
