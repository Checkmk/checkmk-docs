...
====
// ES: evtl. die Beschreibung im Tipp weniger detailliert halten oder einen zusätzlichen Hinweis ergänzen, sodass klarer wird, dass der verlinkte Artikel auch Downgrades beschreibt
...
// ES: Hier wäre es charmant, einen Querverweis auf eine Übersichtsseite zur Deinstallation platzieren zu können - auch wenn von dort dann nur zu den Deinstallations-Abschnitten in den umgebungsspezifischen Artikeln verlinkt wird.
...
. Zuerst steht xref:local_files[aufräumen] an: Im lokalen Verzeichnis sollten keine Dateien mehr vorliegen, die keinem Paket mehr zugeordnet sind.
. Es folgt die xref:incompatible_mkps[Kompatibilitätsprüfung] installierter Erweiterungen.
. Nach Ermittlung des Update-Pfades laden Sie die benötigten Installationspakete und Erweiterungen herunter.
...
// ES: In der bisherigen Einleitung von Abschnitt 3 und in allen Unterüberschriften fehlten mir konkrete Verben. Gerne auch als zusätzliches Signposting, bevor die Unterabschnitte folgen. Vorschlag:
////
Ermitteln Sie zunächst den für Ihr Szenario benötigten xref:update_major_version[Update-Pfad].

Falls Sie in Ihrer vorhandenen Instanz xref:glossar#mkp[{CMK}-Erweiterungspakete (MKPs)] einsetzen, prüfen Sie die xref:incompatible_mkps[Kompatibilität dieser MKPs] für die Zielversion Ihres geplanten Updates.
Auch lokal geänderte Dateien im Instanzverzeichnis müssen Sie aktiv xref:local_files[sichten], um einen möglichen Handlungsbedarf rechtzeitig zu erkennen.

In jedem Fall sollten Sie vor dem Update für ein frisches, fehlerfreies xref:backup[Backup] Ihrer Instanz sorgen.
////
...
Auszuschließen sind Inkompatibilitäten jedoch nie, beispielsweise setzen manche Entwickler von Erweiterungen die höchste Kompatibilität auf ein bestimmtes Patch-Release von {CMK}, wenn ein Anbieter eines REST-APIs eine Version zu einem bestimmten Datum abkündigt.
...
Mit welcher {CMK}-Major-Version Support für welche Linux-Distributionsversion abgekündigt wurde, zeigt die xref:update_matrix#[Update-Matrix].
So erfahren Sie, ob eine xref:release_upgrade#[Aktualisierung der Linux-Distribution] als Zwischenschritt erforderlich ist.
...
Auch mit welchem {CMK}-Patch-Release Unterstützung für neue Linux-Distributionsversionen hinzugekommen ist, zeigt die xref:update_matrix#[Update-Matrix].
So erfahren Sie, ob mit dem vorliegenden Patch-Release eine xref:release_upgrade#[Aktualisierung der Linux-Distribution] auf die heiß ersehnte neueste Version möglich ist.
...
// MFS: Why? Am Ende hat man zwei Sites und beide auf der neuesten Major und stellt nach ein paar Tagen fest, dass was hakelt. Weil man faul war, hat man aber kein Backup auf die Seite gelegt.
////
...
// MFS: Riesengroßer Block verschobenes und teils neu geschriebenes Zeugs...
////
Falls Sie ein Update der {CMK}-Version {oldstable} auf {current-major} planen, sollten Sie zuerst den Artikel xref:update_major#[Update auf Version {current-major}] lesen, in dem wir die wichtigsten Themen zusammengetragen haben, die Sie vor und nach einem solchen Update beachten sollten.
...
// ES: Falls man nur ein Patch-Update innerhalb einer Major-Version macht, ist der Update-Pfad trivial, oder? Gerne hier einen dahingehenden Hinweis einfügen. Und dann eine aussagekräftige Überschrift wählen wie z.B. "Update-Pfad ermitteln"
...
// MFS: Siehe oben deutlich gekürzt, da in update_major viel besser für konkrete Fälle, inclusive Mainlining beschrieben
[#incompatible_mkps]
=== MKPs auf Kompatibilität prüfen
...
// MFS: Mit neuem MKP-Management wurden so sloppy modifizierte Dateien eh schon überschrieben
*Achtung:*
...
[#backup]
=== Frisches Backup erstellen und testen
...
====
////
// MFS: Ende riesengroßer Block verschobenes und teils neu geschriebenes Zeugs...
...
. Als erstes wird mit dem Paketmanager der Linux-Distribution das xref:install_new_versions[neue {CMK}-Paket installiert].
. Es folgt die xref:execute[Aktivierung der neuen {CMK}-Version] für die zu aktualisierende Instanz mittels `omd update`.
...
// ES: in upgrade.asciidoc habe ich einen neuen Abschnitt "Nach dem Upgrade" eingefügt. Dieser Abschnitt hier findet auch ganz klar *nach* dem eigentlichen Update statt, könnte also unter einer Überschrift wie "Nach dem Update" stehen.
// MFS: Heißt nun Nachbereitungen.
// ES: Dass der folgende Abschnitt "Das Update im Detail" sich hier dann nicht reibungslos einfügt, ist ein Symptom der Vermischung von Anleitung und Erklärung. Ich würde ihn aber erstmal da stehenlassen, wo er ist, und habe mir eine Notiz gemacht, das im Rahmen unserer Diataxis-Pläne umzubauen.
// MFS: Nach unten gerutscht...
...
// ES: Dieser Abschnitt hat nicht den gleichen Stellenwert wie 3.1 und 3.3. Er ist eher ein untergeordneter Hinweis, der logisch noch zu 3.1 gehört, weil er den Lesenden hilft, einen funktionierenden Update-Pfad zu ermitteln (dadurch, dass hier vom Rückwärtsgehen explizit abgeraten wird).
...
Ein Rollback sollte nur vorgenommen werden, wenn eine Sicherung der Konfiguration von unmittelbar vor dem Update vorliegt (beispielsweise mit der globalen Eistellung zur Versionierung mit Git).
...
Haben Sie beim Durchlauf von `omd update` Konflikte in Dateien bekommen, die Sie lösen müssen?
Oder sind Sie einfach neugierig, was beim Update genau „unter der Haube abläuft“?
Dann sollten Sie hier weiterlesen.

Bei `omd update` geschehen vier Dinge:

. Zuerst werden Standarddateien unter `~/etc/` und `~/var/` aktualisiert, also die Dateien, die bei Instanzerstellung mit `omd create` erzeugt wurden.

. Es folgt das Umschalten der Version auf die Zielversion durch Ändern des symbolischen Links `version`, welcher sich im Instanzverzeichnis befindet.

. Danach werden Migrationen von Regeln durchgeführt, beispielsweise für geänderte Datenstrukturen, Datentypen oder hinzugekommene Variablen.

. Schließlich werden automatisch die xref:glossar#activate_changes[Änderungen aktiviert], um eine valide Konfiguration für den Kern zu erzeugen.
...
// MFS: Brauchen wir das? Oben wird alles erklärt und hier schreiben wir: Hat nicht geklaptt, lies oben nochmal!
////
[#diagnose]
...
// ES: Schön sauber gelöst!

// MFS: Ich bin mir unsicher, ob wir hier Zwischenüberschriften brauchen. Mein Bauchgefühl sagt nein.
// ES: Ich finde sie gut, weil man damit den Zweck jeder Tabelle auf einen Blick erkennt. Hätten wir Tabellentitel o.ä., könnten die Überschriften weg, aber ich finde es absolut in Ordnung, wie es jetzt ist.
...
// MFS: Commented out for now, to update in 2.4 branch
////
[#legacy_options]
=== Legacy-Optionen

Die hier gezeigten Optionen sind in {CMK} {v25} noch nutzbar, werden aber in {CMK} {v26} nicht mehr zur Verfügung stehen.
...
[cols="20,~"]
|===
|`--conflict=ignore` |Ignoriert jeglichen Konflikt. Eventuell ist {CMK} dann aber nicht lauffähig und ein manuelles Nacharbeiten erforderlich. Erwähnten wir bereits die Relevanz eines aktuellen Backups?
...
----
{c-root} omd stop mysite ; \
    omd -f -V "{current}.community" update --conflict=install mysite && \
    omd start mysite
----
