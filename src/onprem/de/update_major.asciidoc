...
In diesem Artikel finden Sie die wichtigsten Themen, die für das Update Ihrer {CMK}-Version {oldstable} auf {current-major} relevant sind.
...
Diese ist in Teilen automatisch übersetzt und nicht immer auf dem aktuellsten Stand der deutschen Version.
====
...
[TIP]
====
Skripte im `treasures`-Verzeichnis zählen nicht zum Produkt {CMK} und sind vom Support ausgenommen.
Die Verwendung dieser Skripte erfolgt auf eigenes Risiko, es besteht keine Garantie.
====

// MFS: Selbst drüber gestolpert.
// [TIP]
// ====
...
// ====
// ES: promoted the above to running text to avoid multiple admonitions after adding previous admonition
// TK: Gute Idee: Passt! 
...
Haben Sie sichergestellt, dass ein Werk keine Auswirkungen hat oder Sie haben benötigte Änderungen vorgenommen, bestätigen Sie das Werk mit [.guihint]#Acknowledge#.
...
Nach durchgeführter Migration können Sie deutliche Performance-Vorteile erreichen, indem Sie mehrere Regeln zusammenführen, so dass eine einzige Ausführung des Checks mehrere Ausgaben erzeugen kann, was den CPU-Overhead reduziert.
