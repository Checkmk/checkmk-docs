...
Seit {v24}p24 verwendet {CMK} neue Editionsnamen, die besser den Funktionsumfang repräsentieren.
Wenn Sie zwischenzeitlich eines der Patch-Releases installiert haben, kennen Sie die neuen Editionsnamen bereits aus der Oberfläche.
Während {v24} Releases in Paketnamen das alte Editionskürzel verwenden, wechselt {CMK} {v25} zu Kurznamen.
Für die Paketnamen bedeutet das die folgende Änderung:
...
Falls Sie Installationsskripte erstellt haben, welche das alte Paketnamensschema verwenden, müssen Sie diese auf das neue umstellen.
...
[cols="17,~"]
|===
|*Prozessorlast* |Eine etwas höhere Prozessorlast ist bei Verwendung vieler aktiver Checks und einiger Spezialagenten zu erwarten.
...
Falls Sie OpenTelemetry benutzen, rechnen Sie mit wenigstens 8GB zusätzlich genutztem RAM, da das neue xref:metric_backend#[Metrik-Backend] dies für effizienten Betrieb benötigt.
...
{CMK} {v25} wechselt zu einem neuen xref:metric_backend#[Metrik-Backend] für die Speicherung und Abfrage von OpenTelemetry-Daten.

*Betrifft Sie das?*
Wenn Sie OpenTelemetry-Daten bereits im Monitoring mit {CMK} {v24} benutzen, sind Sie betroffen.

*Was müssen Sie tun?*
Zunächst ist vor dem Update für das Metrik-Backend ein zusätzlicher RAM-Bedarf von wenigstens acht Gigabyte einzuplanen.
...
Einen Überblick liefert link:https://checkmk.com/de/werk/19133[Werk pass:q[#]19133^], die Einrichtung erklärt der xref:opentelemetry#[Artikel zum OpenTelemetry-Monitoring].
...
*Betrifft Sie das?*
Dies betrifft alle Nutzer von {SE}. Nutzer von {RE} und {UE} sind nicht betroffen.

*Was müssen Sie tun?*
Führen Sie vor dem Update auf {v25} eine xref:license#[Lizenzverifizierung] durch.
Wurde der Schritt vergessen, schlägt der Check vor dem Update mit der Meldung
_Update aborted with Error: A valid and non expired license is needed before updating_
fehl.

[#linux_versions]
=== Linux-Distributionsversionen

In {CMK}-Version {current-major} werden einige veraltete Distributionen nicht mehr unterstützt werden.
...
[#updater_32]
=== Agent Updater (Linux) ohne 32 Bit Linux-Unterstützung

Bislang war der Agent Updater für Linux als Binary für die Plattformen `x86_64` (64 Bit) und `i586` (32 Bit) verfügbar, mit {CMK} {v25} fällt die 32-Bit-Unterstützung weg (link:https://checkmk.com/de/werk/18925[Werk pass:q[#]18925^]).

*Betrifft Sie das?*
Wenn Sie Hosts im Monitoring haben, die mit Linux auf 32 Bit `x86` laufen, konnten Sie bislang den Agent Updater als Binary ausliefern.

.Was müssen Sie tun? Python-Laufzeitumgebung für Agent Updater als Skript anpassen
[%collapsible]
====
Hosts, welche bisher Einstellung [.guihint]#Executable format (Linux): 32bit packaged binary# nutzen werden mit Wegfall des 32 Bit Binarys automatisch auf das Agent-Updater-Skript umgestellt.
Damit dieses seine Arbeit verrichten kann, muss auf dem Host Python 3.7 mit den folgenden Modulen verfügbar sein:

* `cryptography`
* `requests`
* `pysocks`

Wir empfehlen, nach Konfiguration dieser Abhängigkeiten noch unter {v24} selbst auf das xref:agent_deployment#executable_format[Python-Skript] umzustellen und das Update zu testen.
====

[#python2]
=== Keine Agentenplugins in Python 2 mehr (Linux/Unix)

Mit {v20} wechselte {CMK} zu Python3 als Standardinterpreter für in Python geschriebene Agentenplugins (link:https://checkmk.com/de/werk/12149[Werk pass:q[#]12149^]).
In {v25} fällt die Unterstützung für Python2 komplett weg (link:https://checkmk.com/de/werk/16908[Werk pass:q[#]16908^]).

*Betrifft Sie das?*
Dies betrifft Sie, wenn Sie ältere Hosts überwachen, die noch keinen Python3-Interpreter installiert haben.

.Was müssen Sie tun? Python-Laufzeitumgebung nachinstallieren oder älteren Agenten verwenden
[%collapsible]
====
Die sicher einfachste Methode, alle Agentenplugins auch mit {CMK} {v25} nutzen zu können, dürfte die Konfiguration einer Python3-Laufzeitumgebung sein.
...
Für diesen Fall müssen Sie automatische Agenten-Updates deaktivieren.

Beachten Sie, dass Agentenplugins ohne Dateinamenserweiterung über den Shebang ausgeführt werden.
Sie können demnach auf eigenes Risiko auch ältere (Python2-) Agentenplugins nach Änderung von Shebang (auf den Pfad zum Python2-Interpreter) und Entfernung der Dateinamenserweiterung zusammen mit einem Agenten in Version {v25} verwenden.
Wenn Sie diesen Weg wählen, müssen Sie die Verteilung der manuell installierten Plugins auf die betroffenen Hosts per Bakery-Regel deaktivieren.
====

[#goodbye_vbs]
=== Keine mitgelieferten Agentenplugins in VBS mehr (Windows)

// KNW-1883
Siehe link:https://checkmk.com/de/werk/17685[Werk pass:q[#]17685^],
link:https://checkmk.com/de/werk/17686[pass:q[#]17686^],
link:https://checkmk.com/de/werk/17687[pass:q[#]17687^],
link:https://checkmk.com/de/werk/17688[pass:q[#]17688^],
link:https://checkmk.com/de/werk/17689[pass:q[#]17689^],
link:https://checkmk.com/de/werk/18111[pass:q[#]18111^] und
link:https://checkmk.com/de/werk/18112[pass:q[#]18112^].
...
Diese Möglichkeit wurde ab {CMK} {v23} zurückgefahren und in Version {v25} abgeschafft (link:https://checkmk.com/de/werk/16223[Werk pass:q[#]16223^] und link:https://checkmk.com/de/werk/17345[pass:q[#]17345^] ).
...
*Was müssen Sie tun?*
Lesen Sie die Hinweise in link:https://checkmk.com/de/werk/19333[Werk pass:q[#]19333^] wie Sie das Teilen einzelner Dashboards in {CMK} ab {v25} aktivieren.
...
{CMK} {v25} führt für bisher _ad-hoc_ definierte interne Programmierschnittstellen (APIs) versionierte Nachfolger ein (link:https://checkmk.com/de/werk/18600[Werk pass:q[#]18600^]).
Dies betrifft `cmk.inventory_ui`, `cmk.password_store` und `cmk.server_side_programs`.
Da wir uns Änderungen vorbehalten, verwenden sie als Versionsnummer `v1_unstable`.
Änderungen bis `v1` planen wir möglichst ohne Brüche vorzunehmen.

Zudem xref:bakery_api#history[zieht das Bakery-API um], was nur in seltenen Fällen Probleme nach sich zieht.

Die internen Programmierschnittstellen bleiben weiterhin verfügbar, wir können jedoch keine Garantie hinsichtlich der Kompatibilität treffen, da ein großer Teil der bei {CMK} mitgelieferten Plugins bereits migriert ist.
...
*Was müssen Sie tun?*
Sollten Probleme mit den alten, ad-hoc definierten APIs auftreten, empfehlen wir die Migration der betroffenen Plugins auf die neuen `v1_unstable` APIs.
Die benötigte Dokumentation ist in jeder Site unter [.guihint]#Help > Developer resources > Plug-in API references#
...
Die Version `1.0` der REST-API, die bis {CMK} {v24} mit ausgeliefert wurde, ist mit der neuen Version `v1` kompatibel.
...
Abkündigungen von API-Versionen -- die mit genügend Vorlauf in Form von Werks bekanntgegeben werden -- treten erst mit der jeweils nächsten Major-Version von {CMK} in Kraft.


[#incompatible]
=== Inkompatible Änderungen

Wie in jeder {CMK}-Version, so gibt es auch in der aktuellen Version {current-major} Änderungen der Software – bei {CMK} in sogenannten xref:glossar.html#werk[Werks] organisiert und dokumentiert –, die Rückwirkungen auf Ihre {CMK}-Installation haben _können_.
...
Seit {CMK} {v23} werden solche Werks nämlich nicht mehr weitergeschleppt, sondern beim Update nach Zustimmung in einem Bestätigungsdialog gelöscht (link:https://checkmk.com/de/werk/15292[Werk #15292^]).
...
Öffnen Sie die Liste der link:https://checkmk.com/de/werks?search=&cmk_version$$[$$$$]$$={current-major-short}&compatible=0[Werks^] auf der {CMK}-Webseite.
...
Am eigentlichen Update der Software hat sich in {CMK} {v24} nichts Grundlegendes geändert,
...
Die Benutzeroberfläche (GUI) von {CMK}, die mit Version {v20} komplett neu gestaltet wurde,
// hat sich auch in der {v25} nicht grundlegend verändert.
hat für {v25} eine grundlegende Überarbeitung erfahren.
So wurde die Suche neu gestaltet, die Aktivierung von Änderungen ist möglich, ohne die jeweils geöffnete Seite zu verlassen und mehr Assistenten sind vorhanden.
Dennoch können Sie die generellen Abläufe, die Sie aus den Versionen {v20} bis {v24} kennen, auch in der {v25} mit moderator Einarbeitungszeit anwenden.
...
////
Bei den Vorbereitungen des Updates von {v23} nach {v24} sind uns beispielsweise die Spezialagenten für viele NetApp-Geräte aufgefallen, die wir vom abgekündigten _ZAPI_ auf das modernere REST-API umstellen mussten (link:https://checkmk.com/de/werk/16767[Werk #16767^]).
...
*Betrifft Sie das?*
Wenn Sie Hosts im Monitoring haben, die mit Linux auf 64 Bit ARM (`aarch64`) laufen, mussten Sie bislang die Agentenausgabe unverschlüsselt übertragen oder zu einem xref:agent_linux_legacy#ssh[SSH-Tunnel] Tunnel greifen.

.Was müssen Sie tun? Agentenpakete mit Linux/aarch64 Agent Controller backen
[%collapsible]
====
Mit {CMK} {v25} können Sie Linux-Hosts auf `x86_64` und `aarch64` gleich behandeln.
Der Agent Controller ist jetzt für beide Plattformen verfügbar.
In der Bakery-Regel [.guihint]#Customize agent package (Linux)# legen Sie fest, in welchen Agentenpaketen Sie den Agent Controller für welche Plattform (oder für beide ausliefern.

Beachten Sie, dass der als Binary verpackte _Agent Updater_ bis auf weiteres auf `x86_64` beschränkt ist.
...
Eine manuelle Konvertierung kann Performance-Gewinne bedeuten (link:https://checkmk.com/de/werk/18032[Werk #18032^], link:https://checkmk.com/de/werk/18033[#18033^] und link:https://checkmk.com/de/werk/18034[#18034^]).

*Betrifft Sie das?*
Dies betrifft Sie nur, wenn Sie intensiv mit der HW/SW-Inventur arbeiten.
...
Da es dennoch eine Weile dauern kann, bis alle Inventarbäume umgewandelt sind, empfehlen wir nach dem Update zumindest für die häufig genutzten Bäume den Befehl `cmk-transform-inventory-files` aufzurufen (link:https://checkmk.com/de/werk/18033[Werk #18033^]).
====
...
// KNW-1878
Der Hoststatus von Hosts die nur per IPv6 erreichbar waren wurde bisher mit dem aktiven Check `check_icmp` ermittelt.
Ab {CMK} {v25} steht für diese Hosts bei Verwendung des CMC auch xref:cmc_differences#smartping[Smart Ping] zur Verfügung (link:https://checkmk.com/de/werk/18050[Werk pass:q[#]18050^]).
...
.Was müssen Sie tun? Serviceerkennung oder `check_icmp` konfigurieren.
[%collapsible]
====
Hosts ohne explizite Konfiguration werden automatisch umgestellt.
Dadurch fallen der aktive Check und seine Metriken weg.
Ist dies das gewünschte Verhalten, müssen Sie lediglich die Änderungen der Serviceliste akzeptieren.
...
[#mssql]
=== Ruleset Microsoft SQL Server (Windows) deprecated

// KNW-2078
Siehe link:https://checkmk.com/de/werk/17669[Werk pass:q[#]17669^].

[#promscraper]
=== Prometheus: Option Prometheus Scrape Targets deprecated

// KNW-2072
Siehe link:https://checkmk.com/de/werk/18709[Werk pass:q[#]18709^].
...
Werk link:https://checkmk.com/de/werk/17725[pass:q[#]17725^] beschreibt das Migrationsskript.
...
// zunächst  als optional zu aktivierendes MKP (link:https://checkmk.com/de/werk/16589[Werk pass:q[#]16589^]).
Seit {CMK} {v24} ist Redfish-Monitoring fester Bestandteil der Software (link:https://checkmk.com/de/werk/17404[Werk pass:q[#]17404^]).
