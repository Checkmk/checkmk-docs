...
// TK: Neuer Absatz für KNW-1558. Ich habe Probleme das Thema auf den Punkt zu bringen: Muss ich was tun? Was muss ich tun? Wenn ja: Gehört es in Kap.2
// TK: Oder ist das einfach nur eine interessante Information? Dann passt es hier aber auch nicht richtig, weil es nichts nachzubereiten gibt.
...
// Wenn für den Host keine Spezialagenten konfiguriert sind, wird -- wie bisher -- der {CMK}-Agent verwendet.
...
Haben Sie beispielsweise Regeln für `agent_acme` konfiguriert und weitere Regeln für `agent_cyberdyne`, die durch eine kleine Nachlässigkeit denselben Hosts zugeordnet wurden, wie `agent_acme`, wird dieser Agent nun auch zusätzlich für diese Hosts ausgeführt. 

*Was müssen Sie tun?*
// Nichts, aber diese Information könnte Sie vielleicht interessieren.
...
Bedenken Sie auch, dass falscher Zuordnung von Spezialagenten bei Hosts plötzlich Services auftauchen, die dort nicht hingehören.

// In vielen Fällen werden wenige, versehentlich häufiger ausgeführte Spezialagenten nicht auffallen.
// Allerdings kann es vorkommen, dass die Last auf dem {CMK}-Server steigt und bei Hosts plötzlich Services auftauchen, die dort nicht hingehören.
