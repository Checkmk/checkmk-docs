...
Wir hoffen, bereits zu Beginn der Betaphase die wichtigsten Themen abgedeckt zu haben.
...
Bis {CMK} {v22} war der Windows-Agent auf ein zusätzliches Plugin angewiesen, um den Status von Netzwerk-Interfaces korrekt zu übermitteln. Ohne Plugin war der Status immer {UP}. Ab {CMK} {v23} kann der Agent alleine den korrekten Status übermitteln (link:https://checkmk.com/de/werk/15839[Werk pass:q[#]15839^]).
...
Hosts, die bisher kein Plugin verwendet haben und deren Netzwerk-Interfaces ins Monitoring aufgenommen waren, werden viele Schnittstellen von {UP} nach {DOWN} wechseln und damit Benachrichtigungen auslösen.

*Was müssen Sie tun?*
Überprüfen Sie, ob der gefundene Zustand der betroffenen Netzwerkschnittstellen der gewünschte ist.
