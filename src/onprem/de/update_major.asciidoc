...
Desweiteren können umfangreichere Checks gerade im Cloud-Bereich je nach Anteil an der Gesamtmenge der Checks für weitere Mehrlast sorgen, so dass insgesamt 10–15{nbsp}% (in Extremfällen um 20{nbsp}%) mehr CPU-Last zu erwarten sind.

*Was müssen Sie tun?*
Stellen Sie sicher, dass genügend freie Kapazitäten vorhanden sind.
Als Faustregel gilt: Liegt die _Load_ unter _Zahl der Prozessorkerne × 0,8_ und die CPU Utilization unter 70{nbsp}%, sind keine Probleme zu erwarten.
...
{CMK} {v23} enthält einen neuen aktiven Check für HTTP-Verbindungen und Zertifikate, der wesentlich performanter, robuster und besser zu konfigurieren ist, als der bisherige `check_http`.
...
Aus diesem Grund raten wir dazu, beim Neuanlegen von Hosts den bisherigen Check nicht mehr zu verwenden und Hosts im Bestand nach und nach auf den neuen Check zu migrieren.
...
Basisfunktionalität für die Prüfung von Zertifikaten enthielt der bisherige `check_http` und der im vorherigen Abschnitt erwähnte `check_httpv2` verbessert sie noch einmal.
...
Die hieraus resultierenden Nutzerwünsche haben wir mit dem neuen `check_cert` umgesetzt.
Alle Einstellungen des neuen [.guihint]#Check certificates# finden Sie ebenfalls unter [.guihint]#Setup > Services > HTTP, TCP, Email, ...# im Kasten [.guihint]#Networking.# 
