...
====
_Work in progress:_
Dieser Artikel ist bis zur Veröffentlichung der finalen Version von {CMK} {current-major} einem beständigen Wandel unterworfen.
...
Seit {v23}p28 wird dieses Modul link:https://checkmk.com/werk/17370[von {CMK} mitgeliefert^].
====
...
[TIP]
====
Wenn Sie das mitgelieferte MKP für den Redfish-Spezialagenten aktiviert haben, deaktivieren Sie dieses bitte vor dem Update.
Stand {v24}b5 verursacht die Anwesenheit des Paketes Fehlermeldungen.
====
...
*Betrifft Sie das?*
Das Thema APIs betrifft Sie, wenn Sie die mit {CMK} ausgelieferten APIs um Ihre eigenen, selbst geschriebenen Checks erweitert haben und/oder wenn Sie Plugins aus anderen Quellen nutzen.

// MFS: KNW-1773
*Was müssen Sie tun?*
Stellen Sie vor dem Update auf {v24} zunächst sicher, dass Sie auf die letzte Patch-Version von {v23} aktualisiert haben.
Vorhandene Erweiterungen, die nach einem Update auf {v24} nicht mehr funktionieren werden, führen dann bei allen Benutzern mit der Berechtigung, MKPs zu verwalten zu Benachrichtigungen, die wiederholt angezeigt werden (link:https://checkmk.com/de/werk/17649[Werk pass:q[#]17649^]).
...
//SP: Absatz 1 und Absatz 3 von "Was müssen Sie tun?" haben den exakt gleichen Anfang. Soll das so sein? Lässt sich der Inhalt eventuell kombinieren und die Dopplung damit abschaffen?
...
Auch wenn das Migrationsskript keine Performance-Nachteile verursacht, können Sie deutliche Performance-Vorteile erreichen, wenn Sie mehrere Regeln, die folglich mehrere Aufrufe des Checks verursachen, zusammenführen.
...
Details zu Möglichkeiten, Limitationen und Best-Practices bei Vor- und Nachbereitung wird ein ausführlicher Blog-Artikel erklären.
...
// Versuchen Sie das bitte in Ihrem eigenen Interesse zu vermeiden, um nach Updates sporadisch erratisches Verhalten ausschließen zu können.
...
In {CMK} in Version {v24} ist Redfish-Monitoring fester Bestandteil der Software (link:https://checkmk.com/de/werk/17404[Werk pass:q[#]17404^]).
