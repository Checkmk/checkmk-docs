...
*Betrifft Sie das?*
Ja, als Nutzer von SLES 15SP3/4. Das Update ist etwas aufwendiger, da Paketquellen für die veraltete Version 7 deaktiviert und für die neue Version 8 aktiviert werden müssen.
...
*Betrifft Sie das?*
Dies betrifft nur Nutzer der {CE}, andere Formen des verteilten Monitorings sind nicht betroffen.

*Was müssen Sie tun?*
Da das Update letztlich der xref:update_distributed_monitoring[Prozedur in verteilten Umgebungen mit verteiltem Setup] entspricht, muss sichergestellt werden, dass die zuletzt aktualisierte Zentralinstanz noch unter {CMK} {v22} die Lizenzinformationen an die bereits aktualisierten Remote-Instanzen unter {CMK} {v23} übertragen kann.
...
Damit ist die Übertragung der xref:license#license_cce[Lizenzinformationen] sichergestellt und die Remote-Instanzen können auf {v23} aktualisiert werden.
Wir empfehlen, alle Instanzen vor dem Update auf {v23} auf das letzte verfügbare Patch-Release zu aktualisieren.
...
Einige Plugins, die als _deprecated_ markiert waren, wurden entfernt und erfordern gegebenenfalls manuelle Installation, falls benötigt: link:https://checkmk.com/de/werk/16359[Werk pass:q[#]16359^].
...
*Betrifft Sie das?*
Hoffentlich nicht: Der Wegfall betrifft Sie nur, wenn Sie in Python geschriebene Agentenplugins auf Windows Server 2008 R2 oder Windows 7 (beide EOL bereits in 2020) nutzen.
...
////
MFS: Die ganz alten, ganz toten, stinkenden APIs sind doch eh weg, oder?
...
In der Version {v23} ist es nicht mehr möglich, Standardwerte von Check-Plugins oder andere globale Variablen mithilfe von mk-Dateien zu überschreiben.
...
Da Zentralinstanzen mit {CMK} {v22} Pakete für {v23} vorhalten und an Remote-Instanzen verteilen können, hilft dieses Feature zudem beim Update in xref:distributed_monitoring#[verteilten Umgebungen] mit xref:glossar#distributed_setup[zentraler Konfiguration].
...
Vor dem Update ist der richtige Zeitpunkt, liegen gebliebene Änderungen der bisherigen Major-Version {v22} darauf hin abzuklappern, welche Auswirkungen auf das Update haben können.
...
Es ist zudem eine gute Idee, sich bereits *vor* dem Update einen Überblick über die inkompatiblen Änderungen zu verschaffen:
...
Bis {CMK} {v22} war der Windows-Agent auf das zusätzliche Plugin link:https://checkmk.com/integrations/winperf_if[winperf_if] angewiesen, um den Status von Netzwerk-Interfaces korrekt zu übermitteln. Ohne Plugin war der Status immer _up_. Ab {CMK} {v23} kann der Agent alleine den korrekten Status übermitteln (link:https://checkmk.com/de/werk/15839[Werk pass:q[#]15839^]).

*Betrifft Sie das?*
Je nachdem, in welchem Umfang Sie das Plugin verteilt haben und ob Sie bei Hosts ohne Plugin das Monitoring des Zustandes von Netzwerk-Interfaces deaktiviert haben, kann Sie das betreffen.
...
Nutzen Sie die Gelegenheit, um zunächst wenigstens für Ihre Windows-Hosts die Empfehlungen des Blogartikels link:https://checkmk.com/de/blog/network-monitoring-with-checkmk-2-0[_"Netzwerk-Monitoring mit Checkmk: 3 rules to rule them all"_] umzusetzen.
...
Bei {CMK} {ME} und {CE} mit verteiltem Setup werden mitunter beim Update keine Lizenzinformationen zu Remote-Instanzen synchronisiert.
Dies muss überprüft und gegebenenfalls manuell angestoßen werden.
...
Der in der Kategorie [.guihint]#Networking# zu findende zu findende neue Check [.guihint]#Check HTTP web service# wird mittelfristig den alten ([.guihint]#SetupServices > HTTP, TCP, Email, ... > Check HTTP service#) komplett ablösen.
...
Migrationshilfen werden während des Lebenszyklus von {CMK} {v23} bereitgestellt werden, allerdings ist eine komplett automatisierte Migration tendenziell schwierig, weil bei vielen Hosts Regeln von zwei Checks zusammen geführt werden müssen.

Weitere Informationen finden Sie im link:https://checkmk.com/de/werk/15514[Werk pass:q[#]15514^].
...
Die Unterstützung von Redfish kompatiblen Management Boards wird während des Lebenszyklus von {CMK} {v23} eingebaut und verbessert werden, zunächst als optional zu aktivierendes MKP (link:https://checkmk.com/de/werk/16589[Werk pass:q[#]16589^]).
Weitere Hintergründe und Alternativen erläutern wir im link:https://checkmk.com/blog/monitoring-management-boards[Blog-Artikel "Monitoring management boards"].

[#automation_login]
=== Authentifizierung per GET-Parameter

Bis {CMK} {v22} konnte jeder Automationsbenutzer mit per GET-Parameter übergebenen Nutzernamen und Passwort angemeldet werden.
...
Die Einstellung [.guihint]#Enable automation user authentication via HTTP parameters# wird jedoch zunächst als Standard [.guihint]#enabled# verwenden.
...
=== Ntopng 5.6 und älter nicht mehr unterstützt

{CMK} {v23} wird die letzte Version sein, die Ntopng in Versionen 5.x und 6.x unterstützt (link:https://checkmk.com/de/werk/16483[Werk pass:q[#]16483^]).
Mit {CMK} {v24} wird die Unterstützung für Ntopng 5.x eingestellt und nur noch Ntopng 6.x unterstützt.
Aktualisieren Sie während des Lebenszyklus von {CMK} {v23} daher Ihre Ntopng-Installationen auf 6.x.
