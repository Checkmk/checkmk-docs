...
Wenn Sie die {CMK}-Version {v24} in der {CMK} Appliance nutzen wollen, benötigen Sie die Firmware-Version 1.7 der Appliance.
Die Firmware-Version 1.6 der {CMK} Appliance wird noch von {CMK} {v23} unterstützt, aber nicht mehr von {CMK} {v24}.
...
[%collapsible]
====
Gehen Sie wie folgt vor, wenn Ihre Appliance noch die Firmware-Version 1.6 nutzt:

. Aktualisieren Sie die Firmware der Appliance auf Version 1.6.9.
. Aktualisieren Sie {CMK} auf Version {oldstable-patch}.
...
. Ersetzen Sie die nunmehr inkompatible {CMK}-Version {oldstable-patch} der Appliance-Version 1.6 durch die gleiche {CMK}-Version {oldstable-patch} für die Appliance 1.7.
...
Schritt 4 ist optional, aber dringend empfohlen, denn im Fall von Update-Problemen in Schritt 5 können Sie {CMK} {oldstable-patch} erst einmal nahtlos weiternutzen.
