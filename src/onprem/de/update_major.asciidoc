// -*- coding: utf-8 -*-
// IGNORE Alertmanager p17 p21 p8 % Prozentbereich × b6 0p12 Pre pX
...
{CMK} {v23} unterstützt zwar noch die {CMK} Appliance in Version 1.6.
Dies ist aber nur der Fall, um flexible Update-Pfade zu ermöglichen.
Die {CMK} Appliance 1.6 selbst wird im Gegensatz zu {CMK} {v23} seit langer Zeit nicht mehr mit Aktualisierungen unterstützt.
...
*Was müssen Sie tun?*
Gehen Sie wie folgt vor, um {CMK} und Firmware der Appliance in der richtigen Reihenfolge aktualisieren:

. Aktualisieren Sie die Firmware der Appliance auf 1.6.9
. Aktualisieren Sie {CMK} auf Version {v22}p47 (Installationspakete für Appliance-Versionen 1.6 tragen `-3` im Namen)
. Aktualisieren Sie die Firmware der Appliance auf die neueste 1.7
. Ersetzen Sie das nun inkompatible {CMK} {v22}p47 durch {v22}p47 für die Appliance 1.7 (Installationspakete für Appliance-Versionen 1.7 tragen `-4` im Namen)
. Aktualisieren Sie {CMK} wie im weiteren Verlauf dieses Artikels beschrieben auf Version {current} (und denken Sie auch hier beim Download an die `-4` im Paketamen)

Schritt 4 ist optional, aber dringend empfohlen:
Im Falle von Update-Problemen in Schritt 5 können Sie {CMK} {v22}p47 erst einmal nahtlos weiternutzen.
...
Daneben wurde die bereits in {v20} als _deprecated_ gekennzeichnete Check-API endgültig entfernt.
...
[IMPORTANT]
====
Mit {CMK} {v23}b6 wurde die bereits in {v20} als _deprecated_ gekennzeichnete Check-API der Versionen bis {v16} endgültig entfernt (link:https://checkmk.com/de/werk/16689[Werk #16689^]).
Checks, die unter Verwendung dieser Programmierschnittstelle erstellt wurden, werden beim Update deaktiviert.
...
Generell empfehlen wir, zuerst {CMK} auf die neueste {v22} Patch-Version zu aktualisieren und dann das Update auf die neueste {v23} Patch-Version durchzuführen.
