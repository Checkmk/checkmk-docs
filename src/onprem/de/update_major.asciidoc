...
{CMK} {v24} erfordert etwas höhere Systemressourcen als {v23}.
...
Gleiches gilt für verwendeten Arbeitsspeicher.
Liegen ein oder mehrere Werte höher, schaffen Sie genügend freie Kapazitäten, beispielsweise indem Sie Test-Instanzen auf demselben Server deaktivieren oder Hosts auf andere Instanzen verschieben.
...
Kurz nach Erscheinen von RHEL 10 werden wir Pakete von {CMK} {v23} und {v24} für diese Distributionsversion bereitstellen.
Dies erlaubt es Ihnen, zunächst auf die neueste Patch-Version von {CMK} {v23} zu aktualisieren, dann das Update von RHEL 9 nach 10 durchzuführen und hier schließlich auf die neueste Patch-Version von {CMK} {v24} zu aktualisieren.
...
Die globale Einstellung [.guihint]#Enable automation user authentication via HTTP parameters# verwendete jedoch als Standard [.guihint]#enabled#.
In {CMK} {v24} wechselt der Standard dann zu [.guihint]#disabled#.
...
*Was müssen Sie tun?*
Falls Sie automatisches Login per GET-Parameter verwenden, müssen Sie die globale Einstellung [.guihint]#Enable automation user authentication via HTTP parameters# auf  [.guihint]#enabled# ändern.
...
{CMK} {v23} ist die letzte Version sein, die xref:ntop#[ntopng] in den Versionen 5.x _und_ 6.x unterstützt (link:https://checkmk.com/de/werk/16483[Werk pass:q[#]16483^]).
Mit {CMK} {v24} wird die Unterstützung für ntopng 5.x eingestellt und nur noch ntopng 6.x unterstützt.

*Betrifft Sie das?*
Diese Änderung betrifft Nutzer der kommerziellen Distributionen, welche die Integration mit ntopng verwenden.
...
Da Zentralinstanzen mit {CMK} {v23} Pakete für {v24} vorhalten und an Remote-Instanzen verteilen können, hilft dieses Feature zudem beim Update in xref:glossar#distributed_monitoring[verteilten Umgebungen] mit xref:glossar#distributed_setup[verteiltem Setup].
...
Mit {CMK} {v24} werden keine Maßnahmen mehr getroffen, Kompatibilität zu den alten APIs zu bewahren (link:https://checkmk.com/de/werk/16259[Werk pass:q[#]16259^]).
Das bedeutet, dass Plugins, welche die alten APIs nutzen, mit hoher Wahrscheinlichkeit mit {CMK} {v24} nicht mehr funktionieren werden.
...
Werden die älteren, unversionierten APIs oder `cmk.agent_based.v1` verwendet, migrieren Sie diese auf die neuen APIs und die neue Ordnerstruktur.
Informationen zur Migration finden in link:https://checkmk.com/de/werk/16259[Werk pass:q[#]16259^] und in den xref:devel_intro#[Artikeln zur Entwicklung von Check-Plugins].
