...
* Ubuntu in Short Term Support (STS) Versionen 22.10 bis 23.10 – ab {v23} unterstützt {CMK} nur noch Ubuntu Long Term Support (LTS) Versionen
* SLES 15 SP1 und SP2
...
[#agentupdater16]
=== Automatische Agenten-Updates von Version {v16}

Wegen einer Änderung des Kommunikationsprotokolls kann {CMK} {v23} keine Agenten-Updates an den xref:agent_deployment#[Agent Updater] in Version {v16} oder früher mehr ausliefern.

*Betrifft Sie das?*
Falls Ihr Deployment vorbereitete Betriebssystem-Images verwendet, die den {CMK} Agent Updater mit ausliefern und sich auf automatische xref:agent_deployment#automatic_installer[Installation/Aktualisierung] verlassen, besteht ein gewisses Risiko.

*Was müssen Sie tun?*
Überprüfen Sie vorbereitete Betriebssystem-Images und aktualisieren Sie den vorhandenen Agent Updater auf die niedrigste im Unternehmen verwendete {CMK}-Version, mindestens jedoch {v20}.

[#agentupdatercert]
=== Zertifikatsprüfung bei Agenten-Updates

Das Verhalten des Kommandozeilenparameters `--trust-cert` wurde geändert.
Bislang wurde die gesamte Zertifikatskette geprüft und dem in der Hierarchie höchsten gefundenen selbst signierten Zertifikat vertraut, in der Regel ist dies das Root-Zertifikat.
Ab {CMK} {v23} wird nur dem Serverzertifikat vertraut.
...
*Was müssen Sie tun?*
Stellen Sie vor dem Update auf {v23} sicher, dass die korrekten Zertifikate per Agentenbäckerei  bereitgestellt werden und führen Sie ein Update Ihrer Agenten durch.

////
[#agentlinux]
=== Der Agent für Linux

In {CMK} {v23} unterstützt der xref:agent_linux#[Linux-Agent] nicht länger Python 2.6 für Plugins.
...
_TBD: Python 3.4 und Deprecated Plugins_
...
Mit link:https://checkmk.com/de/werk/14648[Werk #14648^] wird in der Version {v23} die Unterstützung für alte Bakery Plugins eingestellt.
...
{CMK} {v23} aktualisiert Python von 3.10 auf 3.11.
...
Ab Version {v23} ist die Verwendung bestimmter Sonderzeichen in Benutzernamen nicht mehr zulässig.
Mit ungültigen Benutzernamen ist ein Update auf die {v23} nicht möglich, da `omd update` mit einem Fehler abbricht.
...
Mit dem link:https://checkmk.com/de/werk/14393[Werk pass:q[#]14393^] warnt Sie {CMK} seit Version {v22}p20 bei ungültigen Benutzernamen 
...
Ab Version {v23} ist die Verwendung von unsicheren und veralteten Hashfunktionen für Passwörter nicht mehr erlaubt.
...
Ein lokaler Benutzer, dessen Passwort mit einer der genannten, veralteten Hashfunktionen umgewandelt wurde, kann sich in der Version {v23} nicht mehr anmelden.

*Betrifft Sie das?*
{CMK} warnt Sie seit Version {v22}p16 (siehe link:https://checkmk.com/de/werk/14391[Werk pass:q[#]14391^]), wenn unsichere Passwort-Hashes in der Datei `~/etc/htpasswd` des Instanzverzeichnisses gefunden wurden.
...
In der {v22} wurde dann jeder betroffene Benutzer entweder nach der Anmeldung aufgefordert, ein neues Passwort zu wählen,
...
Dieser Benutzer wird sich nach dem Update auf die Version {v23} nicht mehr anmelden können.
...
*Betrifft Sie das?* 
Seit der Version {v22}p7 werden Sie bei der Ausführung von `omd update` dazu aufgefordert, zusätzlich `omd update-apache-config <sitename>` auszuführen.
Falls Sie dies bereits bei einem Update der {v22} getan haben, ist beim Update auf die {v23} nichts weiter zu tun.
Aus der bisherigen dringenden Empfehlung wird nun eine Voraussetzung beim Update auf die {v23}.
...
Wie bereits in der Version {v22} angekündigt, unterstützt {CMK} {v23} nicht mehr den Nagios Service Check Acceptor (NSCA).
...
In {CMK} {v23} wurden einige intern genutzte Programmierschnittstellen (APIs) umgebaut.
...
Falls Sie in der Version {v22} die {FE} installiert haben, liegt es nahe, mit dem Update auf die {v23} auch das Upgrade auf die {CE} durchzuführen,
da Sie diese xref:editions[{CMK}-Edition] in der {v23} zeitlich begrenzt ausprobieren und in in kleinem Rahmen dauerhaft nutzen können.

*Hinweis:* Das hier beschriebene _direkte Crossgrade_ wird nur für den Wechsel von {v22} {FE} nach {v23} {CE} unterstützt.
...
. Führen Sie das Update auf die Version {v23} so durch, wie es im Artikel zu xref:update#detailed[Updates und Upgrades] beschrieben ist. +
Dabei führen Sie gleichzeitig mit dem _Update_ auf die Version {v23} ein _Upgrade_ auf eine andere Edition durch, 
da es im konkreten Fall keine gemeinsame Version der {v22} {FE} und der {v23} {CE} gibt.
...
Seit {CMK} {v22} benötigen Hosts zur Durchführung der xref:wato_monitoringagents#agents[TLS-Registrierung] Zugriff auf folgende Ports des {CMK}-Servers: Port 443 (respektive 80) für REST-API-Anfragen und Port 8000 für den Agent Receiver.
...
Um den in der {CMK} {v23} eingeführten xref:glossar#push_mode[Push-Modus] der {CE} nutzen zu können, muss Port 8000 des {CMK}-Servers vom überwachten Host aus erreichbar sein.


[#certificate_lifetime]
=== Agent Controller Zertifikatslaufzeit

{CMK} {v22} nutzte für den Aufbau der TLS-Verbindung Zertifikate mit de facto unbegrenzter Gültigkeit.
In {CMK} {v23} ist die Laufzeit der Zertifikate einstellbar und auf einen Standardwert von fünf Jahren gesetzt (siehe link:https://checkmk.com/de/werk/14964[Werk pass:q[#]14964^]).
Zudem kann eine eigene _Certificate Authority_ verwendet werden.
Vor dem Update auf {v23} erstellte Zertifikate werden automatisch nach Ablauf der maximalen Laufzeit durch neu generierte Zertifikate ausgetauscht.

Aktualisieren Sie daher {CMK} {v22} Agenten, welche den Agent Controller mit Verschlüsselung verwenden, zeitnah auf den {v23} Agenten.

In den wenigen Fällen, wo ein Update des Agenten nicht möglich ist – zum Beispiel, weil die Betriebssystemversion vom {v23} Agenten nicht mehr unterstützt wird –, müssen Sie betroffene Hosts nach Ablauf der Zertifikatslaufzeit manuell xref:agent_windows#registration[neu registrieren].
Dasselbe gilt, wenn Hosts mit dem {v23} Agenten längere Zeit inaktiv sind. 
...
Ab der Version {v23} ist die SAML-Authentifizierung in den kommerziellen Editionen von {CMK} eingebaut und die Konfiguration erfolgt über die {CMK}-Benutzeroberfläche, wie es im Artikel zu xref:saml#saml_cee[SAML] beschrieben ist.
Auch die SAML-Authentifizierung per mod_auth_mellon wird in der Version {v23} noch unterstützt.
...
In beiden Fällen funktioniert das alte Verfahren weiterhin in {v23}.
...
Die Unterstützung der alten Plugin-API für die HW/SW-Inventur wird in der nächsten {CMK}-Version {v23} entfernt.
...
Diese Plugins funktionieren in der Version {v22}, werden aber ab der Version {v23} ignoriert werden.
...
Falls Sie benutzerdefinierte Plugins nutzen, migrieren Sie diese auf die neue Check-API bevor Sie auf die Version {v23} wechseln.
