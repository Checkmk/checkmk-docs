...
Für das Monitoring von Microsofts SQL Server steht mit {CMK} {v23} ein neues Agentenplugin zur Verfügung, Sie finden es unter [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Microsoft SQL Server (Linux, Windows)#.
...
Des weiteren können Sie nun mit einem Plugin Datenbanken auf verschiedenen Hosts überwachen.
