// -*- coding: utf-8 -*-
// IGNORE Alertmanager p17 p21 p23 p8 % Prozentbereich × b6
...
Als Faustregel gilt: Liegt die _ CPU load_ unter _Zahl der Prozessorkerne × 0,8_ und die CPU utilization unter 70{nbsp}%, sind keine Probleme zu erwarten.
...
Mit {CMK} {v23} wechselt die {ME} die Basis von der {SE} zur {CE}.
Dies bedeutet nicht nur, dass die {ME} den erweiterten Funktionsumfang der {CE} erhält, sondern auch, dass das Lizenzmanagement an das der {CE} angeglichen wird.

*Betrifft Sie das?*
Dies betrifft nur Benutzer der {ME}, andere Formen des verteilten Monitorings sind nicht betroffen.

*Was müssen Sie tun?*
Das Update einer {ME} auf {v23} folgt der generellen xref:update_distributed_monitoring[Prozedur in verteilten Umgebungen mit verteiltem Setup.]
...
[#agentlinux]
=== Der Agent für Linux
_TBD: Wir prüfen noch, ob hier etwas eingefügt werden muss. Gegenwärtig (5. März 2024) sieht es nicht danach aus._
////
...
*Betrifft Sie das?*
Hoffentlich nicht: Der Wegfall betrifft Sie nur, wenn Sie in Python geschriebene Agentenplugins auf Windows Server 2008 R2 oder Windows 7 (beide End of Life bereits in 2020) nutzen.
...
Falls Sie die {RE} benutzen, können Sie bereits vor dem Update systemweit `mod_auth_mellon` nach der link:https://github.com/latchset/mod_auth_mellon[Anleitung auf der Projektseite^] installieren.
...
////
// Wird von SK weiter bearbeitet:
_TBD: Kopiert aus Ausblick in {v22}, wird noch ausformuliert._
////
...
Bei {CMK} {CE} und {ME} mit verteiltem Setup werden mitunter beim Update keine Lizenzinformationen zu Remote-Instanzen synchronisiert.
...
Weitere Informationen finden Sie im Werk link:https://checkmk.com/de/werk/15516[pass:q[#]15516^].
...
Für das Monitoring von Microsofts SQL Server steht mit {CMK} {v23} ein neues Agentenplugin zur Verfügung, Sie finden es unter [.guihint]#SetupAgents > Windows, Linux, Solaris, AIX > Agent rules > Check MS SQL Server (Linux, Windows)#.
Dieses ist in Rust implementiert und ersetzt langfristig das bisherige, in VBS geschriebene.
...
Die Konfigurierbarkeit wurde deutlich verbessert, so können Sie nun wählen, welche Sektionen geprüft werden sollen und eigene SQL-Statements hinzufügen.
Des weiteren können nun mit einem Plugin Datenbanken auf verschiedenen Hosts überwacht werden.
Damit dies übersichtlich bleibt, ist über den Piggyback-Mechanismus die Zuordnung zu anderen Hosts möglich.
Die Konfiguration erfolgt per YAML-Datei, hier sind die stabilen Optionen per Bakery-Regeln verfügbar.
Weitere Einstellungen, bei denen wir uns Modifikationen vorbehalten, können Sie per Editor konfigurieren. Details erfahren Sie in Werk link:https://checkmk.com/de/werk/15842[pass:q[#]15842^].

Mit der Einführung des neuen Plugins startet die Deprecation des bisherigen Plugins [.guihint]#Microsoft SQL Server (Windows)#.
In {CMK} {v23} erhalten Sie lediglich Hinweise auf die Verwendung eines veralteten Regelsatzes.
Ab {v24} wird es nicht mehr möglich sein, neue Regeln anzulegen und mit {v25} wird das bisherige Plugin voraussichtlich aus {CMK} entfernt werden.
Weitere Informationen liefert Werk link:https://checkmk.com/de/werk/15844[pass:q[#]15844^].
