...
Der unter ... zu findende neue Check wird mittelfristig den alten (unter ...) komplett ablösen.
Aus diesem Grund raten wir dazu, beim Neuanlegen von Hosts den bisherigen Check nicht mehr zu verwenden.

Eine vollautomatisierte Migration ist tendenziell schwierig, weil bei vielen Hosts die Regeln von zwei Checks zu einem zusammen geführt werden müssen.
Wir werden dennoch zeitnah Migrationshilfen bereit stellen, damit Sie von der besseren Performance bei möglichst vielen Hosts profitieren können.
////
