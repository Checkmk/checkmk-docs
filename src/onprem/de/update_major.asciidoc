...
[TIP]
====
Dieser Artikel wird auch nach Veröffentlichung von {CMK} {current-major} weiterhin überarbeitet werden.
...
[cols="17,~"]
|===
|*Prozessorlast* |Eine etwas höhere Prozessorlast ist bei Verwendung vieler aktiver Checks zu erwarten.
...
|*Arbeitsspeicher* |Die Arbeitsspeichernutzung von {CMK} {v24} ist verglichen mit {v23} etwa 30{nbsp}% höher.
...
|*Festplattenplatz* |Der Bedarf an Festplattenplatz steigt mit {CMK} {v24} um bis zu 5{nbsp}MB pro Host,
die direkt nach dem Update belegt werden.
|===
...
In {CMK}-Version {v24} werden einige veraltete Distributionen nicht mehr unterstützt werden.
...
Welche Linux-Distributionsversionen {CMK} unterstützt, erfahren Sie in der xref:update_matrix#[Update-Matrix für Version {v24}] und auf der link:https://checkmk.com/de/download[Download-Seite^], nachdem Sie die {CMK}-Version und Ihre Linux-Distribution ausgewählt haben.

Nach Erscheinen von Red Hat Enterprise Linux 10 werden wir Pakete von {CMK} {v23} und {v24} für diese Distributionsversion bereitstellen.
...
{CMK} {v24} unterstützt die {CMK} Appliance erst ab Version 1.7.
Die von {CMK} {v23} noch unterstützte Appliance Firmware Version 1.6 wird von {CMK} {v24} nicht mehr unterstützt.

*Betrifft Sie das?*
Dies betrifft Sie, wenn Sie die {CMK} xref:appliance_usage#[Appliance] verwenden und diese noch nicht auf Firmware 1.7 aktualisiert haben.

.Was müssen Sie tun? {CMK} und Firmware der Appliance in der richtigen Reihenfolge aktualisieren
[%collapsible]
====
Gehen Sie wie folgt vor, wenn Ihre Appliance noch Firmware Version 1.6 nutzt:

. Aktualisieren Sie die Firmware der Appliance auf 1.6.9
. Aktualisieren Sie {CMK} auf Version {oldstable-patch} (Installationspakete für Appliance-Versionen 1.6 tragen `-3` im Namen)
. Aktualisieren Sie die Firmware der Appliance auf die neueste 1.7
. Ersetzen Sie das nun inkompatible {CMK} {oldstable-patch} durch {oldstable-patch} für die Appliance 1.7 (Installationspakete für Appliance-Versionen 1.7 tragen `-4` im Namen)
. Aktualisieren Sie {CMK} wie in diesem Artikel beschrieben auf Version {current} (und denken Sie auch hier beim Download an die `-4` im Paketamen)

Schritt 4 ist optional, aber dringend empfohlen:
Im Falle von Update-Problemen in Schritt 5 können Sie {CMK} {oldstable-patch} erst einmal nahtlos weiternutzen.
Lesen Sie die Details zur xref:appliance_usage#cma_webconf_firmware[Aktualisierung] im Artikel zur Nutzung der Appliance.
====
...
Diese Möglichkeit wurde ab {CMK} {v23} zurückgefahren und wird in Version {v25} ganz abgeschafft werden (link:https://checkmk.com/de/werk/16223[Werk pass:q[#]16223^]).
...
*Betrifft Sie das?*
Diese Änderung betrifft Benutzer der kommerziellen Editionen, welche die Integration mit ntopng verwenden.
...
*Betrifft Sie das?*
Diese Änderung ist nur relevant für Hosts, in deren Eigenschaften das Attribut [.guihint]#{CMK} agent / API integrations# auf [.guihint]#API integrations if configured, else {CMK} agent# gesetzt ist.

Es ist möglich, dass Sie versehentlich Regeln erstellt haben, die Hosts mehr Spezialagenten zugeordnet haben als benötigt.
Diese zusätzlichen Regeln wurden bisher ignoriert, wenn sie alphabetisch hinter der ersten lagen.
...
.Was müssen Sie tun? Regeln für Spezialagenten korrekt zuordnen
[%collapsible]
====
...
Auch die Nutzung des aktiven Checks `check_sql` zur Überwachung von Oracle-Datenbanken bedarf einer genauen Betrachtung:
Bis {CMK} {v23}p27 mussten Sie das Modul `oracledb` nachinstallieren, um diesen Check nutzen zu können. 
Seit {v23}p28 wird dieses Modul von {CMK} mitgeliefert (link:https://checkmk.com/werk/17370[Werk pass:q[#]17370^]).
...
[.admonition-tip]
****
Wenn Sie das in der Version {v23} mitgelieferte MKP für den Redfish-Spezialagenten aktiviert haben, deaktivieren Sie dieses vor dem Update.
Die Anwesenheit des Pakets verursacht Fehlermeldungen beim Update.
****
====
...
// MFS: KNW-1773
.Was müssen Sie tun? Selbst geschriebene und extern bezogene Plugins überprüfen und migrieren
[%collapsible]
====
Stellen Sie vor dem Update auf {v24} zunächst sicher, dass Sie auf die letzte Patch-Version von {v23} aktualisiert haben.
Vorhandene Erweiterungen, die nach einem Update auf {v24} nicht mehr funktionieren werden, führen dann bei allen Benutzern mit der Berechtigung, MKPs zu verwalten, zu Meldungen in der GUI, die wiederholt angezeigt werden (link:https://checkmk.com/de/werk/17649[Werk pass:q[#]17649^]).

Überprüfen Sie selbst geschriebene Erweiterungen und solche von Drittanbietern auf die vollständige Verwendung der aktuellen APIs (`cmk.agent_based.v2`,  `cmk.server_side_calls.v1`, `cmk.graphing.v1` und `cmk.rulesets.v1`).
...
Zudem finden Sie auf link:https://github.com/Checkmk/checkmk/tree/master/doc/treasures/migration_helpers/[GitHub^] im `treasures`-Verzeichnis von {CMK} Skripte, die bei der Migration helfen können.

// include::include_treasures_admonition.asciidoc[]
// ES: Obacht: Hier war ursprünglich eine ganz normale Admonition eingebunden, aber wenn wir uns in einem Collapsible befinden, muss ein anderes Konstrukt her. Doppelungen von mehrfach vorkommenden Admonition-Texten sind unschön.
...
Falls Sie in Plugins nicht-öffentliche APIs benutzen, testen Sie umso intensiver, mit {v23} und {v24}.
So wurden an der „internen Werkzeugkiste“ einige Änderungen vorgenommen, die beispielsweise xref:devel_check_plugins.html#custom_debug[Auswirkungen auf die Erkennung des Debug-Flags] haben.
====
...
Wie in jeder {CMK}-Version, so gibt es auch in der aktuellen Version {v24} Änderungen der Software – bei {CMK} in sogenannten xref:glossar.html#werk[Werks] organisiert und dokumentiert –, die Rückwirkungen auf Ihre {CMK}-Installation haben _können_.
...
Vor dem Update ist der richtige Zeitpunkt, liegen gebliebene Änderungen der Version {v23} daraufhin abzuklappern, welche Auswirkungen diese auf das Update haben können.
...
Es ist zudem eine gute Idee, wenn Sie sich bereits *vor* dem Update einen Überblick über die inkompatiblen Änderungen der Version {v24} verschaffen:
...
Generell empfehlen wir, zuerst {CMK} auf die neueste {v23} Patch-Version zu aktualisieren und dann das Update auf die neueste {v24} Patch-Version durchzuführen.
...
Die Unterstützung von Redfish-kompatiblen Management Boards wurde in {CMK} {v23} eingebaut und kontinuierlich verbessert, zunächst  als optional zu aktivierendes MKP (link:https://checkmk.com/de/werk/16589[Werk pass:q[#]16589^]).
In {CMK} {v24} ist Redfish-Monitoring fester Bestandteil der Software (link:https://checkmk.com/de/werk/17404[Werk pass:q[#]17404^]).
