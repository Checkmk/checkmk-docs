...
Mit {CMK} {v23} wechselt die {ME} die Basis von {SE} zu {CE}.
Dies bedeutet nicht nur, dass die {ME} den erweiterten Funktionsumfang von {CE} erhält, sondern auch, dass das Lizenzmanagement an das der {CE} angeglichen wird.

*Betrifft Sie das?*
Dies betrifft nur Nutzer der {CE}, andere Formen des verteilten Monitoring sind nicht betroffen.

*Was müssen Sie tun?*
Da das Update letzlich der xref:update_distributed_monitoring[Prozedur in verteilten Umgebungen mit verteiltem Setup] entspricht, muss sichergestellt werden, dass die zuletzt aktualisierte Zentralinstanz noch unter {CMK} {v22} die Lizenzinformationen an die bereits aktualisierten Remote-Instanzen unter {CMK} {v23} übertragen kann.
