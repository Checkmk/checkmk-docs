...
Die Appliance unterstützt die Editionen {SE}, {CE} und {ME}.
...
Von der Statusansicht aus erreichen Sie das Konfigurationsmenü über die Taste kbd:[F1]:
...
image::cma_webconf_login_2.png[alt="Login-Fenster von Checkmk.",width=57%]
...
Bei Problemen, Fehlermeldungen oder Konflikten erhalten Sie im Artikel xref:update#detailed[Updates] genaue Informationen zum Update-Prozess.
...
* Änderung der ersten Stelle der Versionsnummer (_Major Release_): Sie müssen die Daten Ihres Geräts manuell sichern und nach dem Update wieder einspielen. Ein Update ohne manuelle Datenmigration ist nicht möglich.
...
* Änderung der dritten Stelle der Versionsnummer (_Patch_): Das Update kann ohne manuelle Datenmigration durchgeführt werden. Eine vorherige Sicherung wird trotzdem empfohlen.
...
*Hinweis zum Update der Major- oder Minor-Version:* Wenn Sie die Firmware beispielsweise von 1.6.x auf 1.7.y aktualisieren, müssen Sie auch alle {CMK}-Versionen, die von einer Instanz verwendet werden, wahlweise aktualisieren oder in der gleichen Version (aber für die neue Plattform) installieren.
Der Grund: Major- und Minor-Versionen aktualisieren entweder die als Basis verwendete Betriebssystemversion oder ändern grundlegende Konzepte.

Unter [.guihint]#{CMK} versions# können Sie prüfen, ob Versionen inkompatibel sind.
...
In der Konsole erreichen Sie das Menü zur Steuerung des Geräts durch einen Druck auf kbd:[F2].
...
Drücken Sie dazu in der Statusansicht die Taste kbd:[F2], wählen Sie im folgenden Dialog [.guihint]#Factory Reset# und bestätigen Sie den darauf folgenden Dialog mit [.guihint]#yes#.
...
Sie können den Zugriff auf die Befehlszeile der einzelnen Monitoring-Instanzen aktivieren.
...
Als `root` landen Sie jedoch nicht im Menü der Appliance, sondern schlicht auf der Befehlszeile.
...
Dieses Terminal erreichen Sie durch die Tastenkombination kbd:[Ctrl+Alt+F2].
...
Dieses Terminal ist mit der Tastenkombination kbd:[Ctrl+Alt+F3] erreichbar.
Mit Hilfe der Tastenkombination kbd:[Ctrl+Alt+F1] können Sie wieder auf die Statusansicht zurückschalten.
...
[TIP]
====
Die oben angegebenen Werte gelten ab der Firmware-Version 1.7.6.
Bis dahin wurden 100 MB für ein Einzelgerät bzw. 300 MB für Appliances im Cluster-Betrieb reserviert.
====
...
Auf dem Gerät wird ein Service [.guihint]#User_Memory# angelegt, der Ihnen die aktuellen und historischen Auslastungen durch {CMK}-Instanzen anzeigt.
...
Die aktuellen Versionen der Firmware finden Sie auf unserer link:https://checkmk.com/download/archive#appliance[Website.^]
...
Sollten Sie in {CMK} mal mit Problemen konfrontiert sein, die Sie nicht selbsttätig -- unter Zuhilfenahme dieses Handbuchs -- lösen können, so bieten sich Ihnen mit dem offiziellen link:https://checkmk.com/de/produkt/support[{CMK}-Support^] und dem link:https://forum.checkmk.com/[{CMK}-Forum^] zwei exzellente Anlaufstellen.
In beiden Fällen ist eine möglichst präzise Beschreibung des Fehlers oder der Problemstellung äußerst hilfreich.
Zudem sind bestimmte Informationen zu Ihrer {CMK} Appliance für eine schnelle Analyse und Lösung zwingend erforderlich.
Die grundlegendsten Informationen stellen hierbei sicherlich die von Ihnen eingesetzte Firmware-Version und die Plattform dar.
...
Statt dass der Support Ihnen eine „Wunschliste“ vorlegt und Sie damit auf eine Schnitzeljagd durch die unterschiedlichen Bereiche von {CMK} schickt, können Sie mit nur wenigen Klicks in der grafischen Oberfläche ein maßgeschneidertes Päckchen mit Informationen zusammenstellen -- in diesem Artikel _Dump_ genannt.

Wählen Sie im Hauptmenü der Weboberfläche die Option [.guihint]#Support diagnostics#.
Auf der folgenden Seite werden die Informationen angezeigt, die gesammelt werden, sowie die weitere Option [.guihint]#Disk performance#, die Sie aktivieren sollten, wenn Ihr Problem möglicherweise mit den Festplatten zusammenhängt.
Ein Klick [.guihint]#Create dump# startet die Erstellung eines Dumps.
Sie erhalten einen Überblick über die gesammelten Dateien und die ausgeführten Befehle.
Unten auf der Seite finden Sie einen Knopf zum Herunterladen des erstellten Dumps.

Geben Sie diesen Dump auf Anfrage oder proaktiv an den {CMK}-Support weiter.
