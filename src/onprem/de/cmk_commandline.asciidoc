...
:description: Auf viele Funktionen einer {CMK}-Instanz können Sie mit der Befehlszeile (auch Kommandozeile genannt) zugreifen. Lesen Sie hier, wie Sie die Befehlszeile nutzen können.
...
Trotzdem gibt es Situationen, in denen es nützlich ist, sich auf die Untiefen der Befehlszeile (auch Kommandozeile genannt) zu begeben, z.B.
...
// TK: Das Collapsible fällt hier vom Himmel. Kannst Du bitte einen Einführungssatz spendieren, für wen das jetzt relevant ist?
// TK: Kann man hier nicht öffentlicher Schlüssel statt public key schreiben? Oder zumindest: Feinheiten bei der SSH-Public-Key-Authentifizierung (dann hätte man SSH schon vor dem Collapsible)
...
// TK: Wer beachtet hier was nicht? Checkmk oder die Distro? Und was geht dann schief?
Dies ist der Fall, wenn die SSHD-Konfiguration voraussetzt, dass das Home-Verzeichnis nicht _world-readable_ ist.
// TK: Wenn dem so ist, hab ich doch die SSH-Konfiguration selbst genau so aufgesetzt, oder? Der Satz klingt aber eher so, als ob SSH das erwzingt und man da nix dagegen tun kann. 
...
Denken Sie aber daran, dass dies Einfluss auf die Erstellung weiterer Sites haben kann:
...
// TK: Das ist zu kurz: Bis hierher steht kein Wort im Artikel darüber, das omd Ports ausliest? Welche und von wo?
// TK: Inhaltlich versteh ichs auch nicht. Wenn mysite nicht mehr gelesen werden kann, warum hab ich dann Probleme mit der Erzeugung einer anderen Instanz?
...
Folgendes Schaubild zeigt die wichtigsten Verzeichnisse einer {CMK}-Installation mit einer Instanz namens `mysite` und einer `<version>`, die zum Beispiel `{current}.pro` heißt:
...
|Physikalischer Ort |`/omd/versions/{current}.pro/` |`/omd/sites/mysite/`
...
Es gibt folgende Unterverzeichnisse, welche hier im Beispiel physikalisch unterhalb von `/omd/versions/{current}.pro/` liegen und über symbolische Links vom Instanzverzeichnis aus erreichbar sind:
...
[IMPORTANT]
====
Dateien innerhalb von `~/local/` abzulegen hat das Potenzial, Chaos zu schaffen, weil das Überschreiben von bei {CMK} mitgelieferten Funktionen nicht nur die Möglichkeit schafft, Fixes und Erweiterungen zu testen, sondern auch das Risiko, vorhandene Funktionalität zu beschädigen.
// TK: Hmmm, hier fehlt mir der Nebensatz, der mit weil anfängt ;-)
...
In der Regel ist solch ein Test mit dem Hinweis verbunden, in welcher der nächsten Patch-Versionen die geänderten Dateien in der `/omd/`-Verzeichnisstruktur ausgeliefert werden und damit Sie Dateien im `~/local/`-Verzeichnis zum richtigen Zeitpunkt löschen können.
// TK: "und damit" ist missverständlich. Ich versteh das so: ... die geänderten Dateien ausgeliefert werden, damit Sie die Dateien im `~/local/`-Verzeichnis löschen können.

Im Idealfall packen Sie eigene Veränderungen nach jeder Anpassung als xref:devel_mkps#[MKP] und erhalten Anpassungen von Dritten ebenfalls auf MKP.
...
// MFS: Nach dem Picking nach 2.5 und 2.4 ist die Baumdarstellung für 2.5 anzupassen.
...
Dieser Befehl wird in einer {CMK}-Instanz standardmäßig einmal am Tag per Cronjob durchgeführt.
// MFS: Deprecated! |`cmk -P` |xref:mkps#commandline[MKPs verwalten]
