...
[%collapsible]
====
Wenn Sie das Login per Public Key automatisieren wollen, werden Sie auf einigen Distributionen feststellen, dass Ihre `~/.ssh/authorized_keys` nicht beachtet wird.
Dies ist der Fall, wenn die SSH-Konfiguration voraussetzt, dass das Heimatverzeichnis nicht _world-readable_ ist.
...
Desweiteren finden Sie eine „Schattenhierarchie“ der Softwareverzeichnisse `bin/`, `lib/` und `share/`.
...
Falls Sie hier Dateien ablegen um eigene Änderungen an {CMK} zu testen, tun Sie dies bitte in reinen Testinstanzen.
...
[IMPORTANT]
====
Dateien innerhalb von `~/local/` abzulegen hat das Potential, Chaos zu schaffen.
Es gibt daher genau drei legitime Gründe, in diesem Verzeichnis zu arbeiten.

* Sie entwickeln Erweiterungen für {CMK}, dann orientieren Sie sich an den in den xref:devel_intro#[Artikeln für Entwickelnde] beschriebenen Verzeichnissen. Wenn Sie darauf achten, Namenskollisionen zu vermeiden, sind keine Seiteneffekte zu erwarten.

* Sie möchten an der Entwicklung beitragen, indem Sie beispielsweise mitgelieferte Plugins erweitern. In diesem Fall kopieren Sie _in einer Testinstanz_ die zu modifizierenden Dateien in ihren korrespondierenden Pfad in `~/local/` und editieren sie dort.

* Sie haben einen Bug gefunden und mit {CMK} Kontakt aufgenommen. In manchen Fällen erhalten Sie von Support-Personen bei {CMK} dann Dateien, die sie zum Testen in `~/local/` ablegen sollen. In der Regel ist solch ein Test mit dem Hinweis verbunden, in welcher der nächsten Patch-Releases die geänderten Dateien ausgeliefert werden und damit die Testdateien gelöscht werden sollen.
====
