...
.Feinheiten fürs Login mit Public Key
[%collapsible]
====
...
Wenn Sie das Login per Public Key automatisieren wollen, werden Sie auf einigen Linux-Distributionen feststellen, dass Ihre `~/.ssh/authorized_keys` nicht beachtet wird.
// TK: Wer beachtet hier was nicht? Checkmk oder die Distro? Und was geht dann schief?
Dies ist der Fall, wenn die SSH-Konfiguration voraussetzt, dass das Home-Verzeichnis nicht _world-readable_ ist.
...
Falls Sie also mit `chmod 0700 $OMD_ROOT` dem Rest der Welt Leserechte entziehen wollen, denken Sie daran, dass bei Erstellung weiterer Instanzen `omd` die verwendeten Ports nicht auslesen kann.
...
Daher ist der pragmatische Ansatz, vor der Erstellung von Instanzen die Rechte erneut zu vergeben und gleich danach wieder zu entziehen.
====
...
[IMPORTANT]
====
Dateien innerhalb von `~/local/` abzulegen hat das Potenzial, Chaos zu schaffen.
...
In der Regel ist solch ein Test mit dem Hinweis verbunden, in welcher der nächsten Patch-Versionen die geänderten Dateien ausgeliefert werden und damit die Testdateien gelöscht werden sollen.
