...
////
Alternativ können Sie die Log Level auch auf der Befehlszeile in Konfigurationsdateien anpassen.
...
Das Setup führt Buch über solche Änderungen und zeigt diese in der GUI als zu xref:glossar#activate_changes[aktivierende Änderungen] an.
...
Wenn Sie von Hand die {CMK}-Konfiguration anpassen und die Änderungen aktivieren möchten, benötigen Sie anschließend bei Nagios:
...
// TK: Stimmt die Beschreibung für active? Hier werden doch aktive Checks wie check_http oder check_httpv2 gelistet.
[cols="20,~"]
|===
|`agent` |Wertet Daten eines {CMK}-Agenten aus.
// MFS: Eher: Wertet Daten von Agenten-Plugins oder Spezialagenten aus. Der Agent wird...
Dieser wird (normalerweise) per TCP Port 6556 abgerufen.
|`snmp` |Dient zur Überwachung von Geräten via SNMP.
|`active` |Ruft für die Überwachung ein Nagios-kompatibles Plugin nach klassischer Art auf.
Hier übernimmt {CMK} eigentlich nur die Konfiguration.
// MFS: Eher: Ruft einen aktiven Check auf. Dies beinhaltet Nagios-kompatible Plugins von Drittanbietern, bei denen {CMK} nur die Konfiguration übernimmt.
|===
...
// TK: Ich hab mal bei den Drachen das fette rausgenommen.
// TK: Bytheway: Ist das wirklich eine sehr geläufige Redewendung?
// MFS: Hinsichtlich Popkultur ist das Urban Dictionary eine Autorität: https://www.urbandictionary.com/define.php?term=here%20be%20dragons
// TK: Und müsste die dann nicht „Hier sind Drachen“ heißen? https://de.wikipedia.org/wiki/Hic_sunt_dracones
// MFS: Im Deutschen wird recht häufig das "wohnen" verwendet: https://www.tuwien.at/alle-news/news/wo-die-drachen-wohnen
[IMPORTANT]
====
...
Verschiedene Mechanismen zur Automations, die innerhalb von {CMK} arbeiten oder von außerhalb auf {CMK} zugreifen (z.B. via xref:rest_api#[REST-API]), können Konfigurationsänderungen vornehmen.
