...
Das Revisionsdatum gibt in diesem Zeitraum nicht den Stand von {CMK} an, mit dem dieser Artikel synchron ist, sondern das Datum der letzten nicht-trivialen Änderung.
====
...
|`PATH` |Verzeichnisse, in denen ausführbare Programme gesucht werden. {CMK} hängt hier z.B. das `bin/` der Instanz mit ein. Bei Namensgleichheit haben {CMK}-Programme Vorrang. Das ist z.B. wichtig beim Befehl `mail`, den {CMK} in einer ganz bestimmten Version mitliefert.
|`LD_LIBRARY_PATH` |Verzeichnisse, in denen nach zusätzlichen Binärbibliotheken gesucht wird. Durch diese Variable stellt {CMK} sicher, dass mitgelieferte Bibliotheken Vorrang vor den im normalen Betriebssystem installierten haben.
// SK: PYTHONPATH entsprechend Werk #5870 entfernt
// |PYTHONPATH |Suchpfad für Python-Module. Auch hier haben von {CMK} gelieferte Modulvarianten im Zweifel den Vorrang.
|`PERL5LIB` |Suchpfad für Perl-Module. Auch hier haben von {CMK} gelieferte Modulvarianten im Zweifel den Vorrang.
...
Folgendes Schaubild zeigt die wichtigsten Verzeichnisse einer {CMK}-Installation mit einer Instanz namens `mysite` und einer `<version>`, die zum Beispiel `2.3.0p8.cee` heißt:
...
|Physikalischer Ort |`/omd/versions/2.3.0p8.cee/` |`/omd/sites/mysite/`
...
unterhalb von `/omd/versions/2.3.0p8.cee` liegen und über symbolische
...
[TIP]
====
Einige in diesem Kapitel getroffenen Aussagen zum Vorrang von lokalen (instanzspezifischen) Dateien vor gleichnamigen Dateien im Software-Verzeichnis sind nicht mehr korrekt.
...
Setzen Sie die Einträge, wenn diese noch nicht vorhanden sind. Dies ist der Fall, wenn ein
...
WAYS TO CALL:
 ...
 cmk  --cap [pack|unpack|list FILE.cap]  Pack/unpack agent packages (Managed Services Edition, Enterprise Edition)
...
Kenntnis von Lage und Strukturierung von Konfigurationsdateien kann in viele Fällen bei der Lösung von Problemen und der Eingrenzung von Bugs (beispielsweise in aus der link:https://exchange.checkmk.com/[{CMK} Exchange^] bezogenen oder xref:devel_intro#[selbst programmierten] Erweiterungen) hilfreich sein.
Die in diesem Kapitel vorgenommene Gegenüberstellung zwischen Setup und Konfigurationsdateien soll keinesfalls dazu animieren, per Skript Konfigurationsdateien zu ändern.
Sollte es erforderlich sein, Konfigurationsänderungen zu automatisieren, können Sie dies sicher und ohne Seiteneffekte mit dem xref:rest_api[REST-API] tun.
...
====
*Hier wohnen Drachen!*
Nehmen Sie keinesfalls Änderungen an Konfigurationsdateien vor, wenn Sie nicht explizit von einer Support-Person von {CMK} unmissverständlich dazu aufgefordert wurden.
====

=== Wo ist die Dokumentation?

// Falls Sie jetzt allerdings ein umfassendes Kompendium über den genauen Aufbau von allen von {CMK} verwendeten Konfigurationsdateien erwarten, müssen wir Sie enttäuschen:
// MFS: Kein Blabla, gleich auf den Punkt kommen:
Nicht hier.
...
Beachten Sie zudem, dass sich Formate von Konfigurationsdateien auch zwischen Patch-Releases ändern können.
...
_[.notranslate]#WATO#_ ist die Abkürzung für Web Administration Tool: das {CMK}-Konfigurationswerkzeug bis einschließlich zur Version {v16}.
...
// Bei manchen gibt es noch eine Hauptdatei, welche als Erstes eingelesen wird.
// Diese wird vom Setup nie angefasst und ist nur für manuelle Änderungen vorgesehen.
...
Verschiedene Automationsmechanismen, die innerhalb von {CMK} arbeiten oder von außerhalb auf {CMK} zugreifen (z.B. via xref:rest_api[REST-API]) können Konfigurationsänderungen vornehmen.
In einigen Fällen ist es dann zwar erwünscht, dass die so erstellten Hosts und Ordner im Setup sichtbar und überprüfbar sind, Änderungen durch „menschliche“ Nutzer aber nicht erwünscht sind.
...
Das betrifft übrigens _nicht_ die Service-Erkennung.
...
// Rein formal gilt für alle Konfigurationsdateien von {CMK}, dass diese in *Python 3*-Syntax geschrieben sind.
// Dabei gibt es zwei Arten von Dateien:

* Wie ein Skript _ausgeführt_ werden Dateien, die Variablenzuweisungen enthalten (`=`) wie z.B. `hosts.mk`
* Als Variable eingelesen werden Dateien, die nur einfache Werte oder Python-Dictionaries enthalten, beispielsweise `.wato`.

// Die ausführbaren Dateien erkennen Sie daran, dass hier Variablen durch Zuweisungen (`=`) mit Werten belegt werden.
// Die anderen Dateien enthalten meist ein Python-Dictionary, welches mit einer öffnenden geschweiften Klammer beginnt.
// Manchmal sind es auch einfache Werte.
...
Näheres können Sie zum Beispiel der link:https://docs.python.org/3/reference/[offiziellen Referenz von Python 3] entnehmen.

Binärdateien tragen die Endung `.pb`, was kurz für _Protocol Buffers_ (kurz: link:https://protobuf.dev/[_Protobuf_^]) steht.
...
Im _Lieferumfang_ von {CMK} befindet sich `protoc`, das viele einfache Aufgaben erledigt.
...
{CMK} liefert hierfür das Programm `cmk-validate-config` welches im Gegensatz zu dem bei Versionsupdates aufgerufenenen `cmk-update-config` keine Änderungen wie Migrationen von Datenformaten durchführt.
Dieses Programm prüft sowohl Syntax (Klammersetzung, korrekte Zuweisungen...) als auch partiell Semantik (verwendete Datentypen und Vorhandensein bestimmter Schlüssel).
...
The following mk files have passed the validation:
  ...
  etc/check_mk/dcd.d/wato/connections.mk...................... [green]#Passed#
