...
Zwar ist `/omd` ein symbolischer Link auf `/opt/omd`, womit die Daten _physikalisch_ eigentlich unterhalb von `/opt` liegen, aber alle Pfadnamen in {CMK} verwenden immer `/omd`.
...
Hier abgelegte Dateien können gleichnamige, von {CMK} mitgelieferte Dateien _überschatten_.
Beachten Sie, dass hier abgelegte Dateien im xref:glossar#distributed_monitoring[verteilten Monitoring] auf alle Remote-Instanzen verteilt werden.
...
In der Regel ist solch ein Test mit dem Hinweis verbunden, in welcher der nächsten Patch-Versionen die geänderten Dateien in der `/omd/`-Verzeichnisstruktur ausgeliefert werden, damit Sie anschließend Dateien im `~/local/`-Verzeichnis löschen können.
...
Nur solche mit einer Datei `pass:[__init__].py` können überschattet werden und auch dies nur, wenn im gleichnamigen Verzeichnis in `~/local/` ebenfalls eine Datei `pass:[__init__].py` liegt.
====
...
Das [.guihint]#Setup# führt Buch über solche Änderungen und zeigt diese in der GUI als xref:glossar#activate_changes[ausstehende zu aktivierende Änderungen] an.
