// -*- coding: utf-8 -*-
// IGNORE % p2 Sicherheitsgruppen
include::global_attr.adoc[]
= Installation aus dem AWS Marketplace
:title: Installation aus dem AWS Marketplace
:description: Sie können {CMK} als Amazon Machine Image (AMI) betreiben. Lesen Sie hier, wie Sie Ihre Instanz in Amazon Web Services (AWS) einrichten und nutzen.
...
Mittlerweile gehen wir einen Schritt weiter und bieten link:https://aws.amazon.com/marketplace/pp/prodview-gddkal2hfn7yo[ein vorinstalliertes Image^] auf Basis von Ubuntu 22.04 _(Jammy Jellyfish)_, bei dem alle Abhängigkeiten bereits erfüllt sind.
Hierbei kommt ausschließlich die xref:cce#[{CE-onprem}] zum Einsatz.
Nach dem Einrichten der ersten Instanz können Sie diese kommerzielle xref:glossar#edition[Edition] im Lizenzstatus „Trial“ ausprobieren.
...
Wegen der günstigen Traffic-Kosten raten wir zur Nutzung eines _AWS S3 Bucket_.
