...
[TIP]
====
Unser Beispiel verwendet ein sehr einfaches Shellskript als Agenten-Plugin.
...
Fehlt dem Skript das Suffix `.py`, wird der Shebang ausgewertet, ist das Suffix vorhanden, sucht der {CMK}-Agent nach dem Python-Interpreter.
// ES: Erläuterung besser in zwei Sätze aufteilen? Bspw: 
// Der {CMK}-Agent wertet den Shebang nur aus, wenn dem Skript das Suffix `.py` fehlt.
// Ist das Suffix vorhanden und ein Python-Interpreter bekannt, wird dieser verwendet, auch wenn er nicht der Angabe im Shebang entspricht.
// Statt im Shebang können Sie den Pfad zum gewünschten Interpreter auch in der ... siehe unten
Dessen absoluten Pfad können Sie auch in der Konfigurationsdatei `$MK_CONFDIR/python_path.cfg` als Variable `PYTHON3` übergeben.
