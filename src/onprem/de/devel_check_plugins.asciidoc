...
====
// TK: Ich würde hier schon noch den einführenden Satz aus dem Werk spendieren, um zu motivieren, warum das Zeug da liegt, wo es liegt - und den Begriff "Garantie" vermeiden, etwa so:
...
// TK: Da diser [TIP] immerhin 3x genutzt wird, könnte man ihn in eine include-Datei auslagern.
// TK: Brauchen wir da statt [TIP] ein [IMPORTANT]?
