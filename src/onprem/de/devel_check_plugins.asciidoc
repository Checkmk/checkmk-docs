...
Für die Programmierung der Check-Plugins bringt {CMK} eine *Check-API.*
In der {CMK}-Version {current-major} ist die Check-API V2 die aktuelle Version.
Wir zeigen Ihnen in diesem Artikel, wie Sie die Check-API Version 2 für die Plugin-Programmierung nutzen können.

Über die {CMK}-Benutzeroberfläche haben Sie jederzeit Zugriff auf die Dokumentation der Check-API: [.guihint]#Help > Developer resources > Plug-in API references#.
...
[TIP]
====
Auch ohne laufende {CMK}-Instanz können Sie sich die Plugin-API-Dokumentation als Kopie auf link:https://docs.checkmk.com/plugin-api/{current-major}/[docs.checkmk.com/plugin-api/^] ansehen.
====
...
* Übung mit Linux auf der Befehlszeile.
...
Wenn Sie neugierig sind, welche Namen es schon gibt, können Sie diese in einer {CMK}-Instanz auf der Befehlszeile mit `cmk -L` auflisten lassen:
...
Ein lauffähiges Grundgerüst (link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_simple_myhostgroups_bare_minimum.py[Download bei GitHub^]), das Sie im Folgenden Schritt für Schritt weiter ausbauen werden, sieht so aus:
...
Das link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_simple_myhostgroups.py[Check-Plugin^]
und auch das link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/agent_plugin_simple_myhostgroups[Agentenplugin^]
...
Zunächst stellen Sie sicher, dass Ihr Plugin syntaktisch korrekt ist:
...
Weitere Tests und die Aktivierung des Plugins werden auf der Befehlszeile mit dem Befehl `cmk` erledigt.
Probieren Sie zuerst die xref:wato_services#commandline[Service-Erkennung] mit der Option `-I` aus.
...
Wenn soweit alles wie erwartet abgelaufen ist, können Sie xref:glossar#activate_changes[die ausstehenden Änderungen aktivieren].
Falls nicht, finden Sie im Kapitel zur xref:errors[Fehlerbehebung] Hinweise.

Zum Abschluss erzeugen Sie eine aktualisierte Konfiguration des Monitoring-Kerns und xref:cmk_commandline#commands_core[starten diesen neu]:
...
Das erweiterte link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/agent_plugin_advanced_myhostgroups[Agentenplugin^]
und die erweiterte Datei für die link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_advanced_myhostgroups_step01.py[Check-Plugins^]
...
Da das neue Check-Plugin `myhostgroups_advanced` neue Services zur Verfügung stellt, müssen Sie eine Service-Erkennung für dieses Check-Plugin durchführen und die ausstehenden Änderungen aktivieren, um die Services im Monitoring zu sehen:
...
link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_advanced_myhostgroups_step02.py[GitHub^]
...
Der Aufruf von `cmk-validate-plugins` wird nun bestätigen, dass der Regelsatz selbst in Ordnung ist, aber warnen, dass er noch keine Auswirkungen hat.

Um den Regelsatz sichtbar zu machen, müssen Sie die Python-Prozesse neu starten, welche die grafische Oberfläche bereitstellen.
Das wird vom Neustart des Apache-Webserver mit erledigt:
...
Danach ist der Regelsatz im [.guihint]#Setup# auf der oben genannten Seite zu finden.
Mit der Suchfunktion im Setup-Menü finden Sie den Regelsatz aber erst nach einem Neustart von Redis:
...
link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/ruleset_myhostgroups.py[Regelsatz^]
als auch das um den Regelsatz erweiterte
link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/check_plugin_advanced_myhostgroups.py[Check-Plugin^].
====


[#testing_the_extended_plugin]
=== Das erweiterte Check-Plugin testen und aktivieren

Die im letzten Kapitel vorgenommenen Änderungen betreffen sowohl den Monitoring-Kern als auch die grafische Oberfläche und deren Hilfsprozesse.
Daher ist es erforderlich, nach der Prüfung mit `cmk-validate-plugins` zunächst die Konfiguration neu zu erzeugen und dann die Instanz neu zu starten:
...
.Schnellerer Neustart für Experten
[%collapsible]
====
Falls Sie über ein tieferes Verständnis der Prozessstruktur von {CMK} verfügen und exakt wissen, welche Komponenten von Änderungen an Ihrem Plugin betroffen sind, können Sie den Neustart auf bestimmte Dienste beschränken.
Einen Überblick verschafft der Artikel xref:cmk_processes#[Instanzdienste].
====
...
Die Graphing-Datei dieses Kapitels finden Sie wieder auf link:https://github.com/Checkmk/checkmk-docs/blob/{gitbranch}/examples/devel_check_plugins/graphing_myhostgroups.py[GitHub^].
...
[TIP]
====
Beachten Sie, dass die beiden symbolischen Verknüpfungen `~/lib/check_mk` [.fa-light.fa-arrow-right-long]#&nbsp;# `~/lib/python3/cmk` sowie `~/local/lib/check_mk` [.fa-light.fa-arrow-right-long]#&nbsp;# `~/local/lib/python3/cmk` seit {CMK} {v25} link:https://checkmk.com/werk/17969[nicht mehr existieren^].
Falls Sie Erweiterungen mit selbst geschriebenen Skripten in diese Verzeichnisse ausgebracht haben, müssen Sie diese Skripte anpassen. 
====
