...
[TIP]
====
Dieser Artikel wird derzeit überarbeitet für die neuen APIs der Version {v23}.
====
...
// TK: Altes Kap. 1.3 zu folgendem TIP umgeschrieben:
[TIP]
====
...
// TK: In diesem Kapitel durchgehend die Dateien, Pfade, import-Befehle und geänderten Aufrufe (register>discovery) ersetzt.
...
// TK: Formale Änderungen wie im vorherigen Kapitel gemacht.
...
// TK: Ich hab den TIP auskommentiert, da ich im folgenden Code-Schnipsel alt und neu zeige, so dass es (noch) klarer werden sollte, dass das alte Check-Plugin in der Datei drin bleibt.
////
[TIP]
====
...
// TK: In der API-Dok steht nur float, nicht integer.
...
// TK: Ich mag mich täuschen, aaaaaber: Bereits vor diesem Kapitel (also vor der Definition der Metriken) wird das Symbol und 6 leere Graphen angezeigt:
// TK: für die 4 ausgelesenen Werte (num_hosts, num_hosts_up, num_services, num_services_ok) und die 2 berechneten Werte (hosts_up_perc, services_ok_perc)
// TK: Liegt es vielleicht daran, dass ich früher bereits diese Metriken in meiner Testinstanz installiert hatte?
Wenn Sie mit der Maus auf das Symbol zeigen, werden die Graphen als Vorschau eingeblendet.
// TK: Jetzt stimmt es wieder und es werden nur die beiden Graphen angezeigt, allerdings ohne die gelben und roten Linien für die levels_lower. Warum fehlen die jetzt?
...
// TK: Da dieses Kapitel komplett überarbeitet wurde, habe ich fürs Review alle alten Texte auskommentiert stehen lassen, damit man den Update besser nachvollziehen kann.
// TK: Beim Review die Verwendung der API-Begriffe kontrollieren: Klasse, Instanz, Funktion, Objekt, ...
...
// TK: Das da ist obsolet, richtig? i18n geht jetzt anders, aber wie genau?
// TK: Müssten wir nicht _Localizable importieren und vor alle Titel einen Unterstrich setzen?
// TK: Oder lassen wir das Thema i18n unter den Tisch fallen?
////
...
----
// TK: Label und BooleanChoice werden bei uns aber gar nicht verwendet?
...
Der Name darf auf keinen Fall identisch sein mit einem bereits existierenden Regelsatz, weil dieser damit überschrieben würde.
Um dieser Gefahr aus dem Weg zu gehen, verwenden Sie am besten im Namen einen Präfix.
// TK: Stimmen die beiden vorherigen Sätze noch?
...
// TK: Stattdessen eigenes Topic mit CustomTopic definieren? Ich hab ein wenig rumprobiert, aber es nicht hinbekommen.
...
----
// TK: Oder sollen wir hier mit omd restart die Abkürzung vorgeben?
...
====
// TK: IMPORTANT ist schon ganz schön heftig - zumal ich nicht weiß, ob das wirklich (noch) stimmt.
...
// TK: KeyError? Bei cmk --detect-plugins=myhostgroups_advanced -v localhost gibt es ohne Regel für jeden Host-Gruppen-Service: check failed - please submit a crash report!
...
// TK: Dieser Abschnitt ist m.E. mit den neuen APIs obsolet
////
[#reuse_ruleset]
...
[#format_numbers]
== Zahlen formatieren
// TK: Alle Funktionen neu getestet (nur die Ausgabe von date hat sich geändert).
// TK: import-Befehl und date Ausgabe geändert, sonst 1x quergelesen > Review muss nicht sein.
...
// TK: In der Instanz gibt es 2 Skripte, auf GitHub 5 (https://github.com/Checkmk/checkmk/tree/master/doc/treasures/migration_helpers). Nach GitHub verlinken?
// TK: GitHub Diff anbieten für die Migration unserer Beispieldateien? 
