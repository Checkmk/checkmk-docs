...
// TK-RE: Kommentare zum aktuellen Review sind wie dieser gekennzeichnet.
// TK-RE: Um das Review auch ohne git diff nachvollziehen zu können, habe ich sehr viele alte Texte auskommentiert stehen lassen. Bitte beim/nach dem Review löschen.
...
// TK-RE: Altes Kap. 1.3 zu folgendem TIP umgeschrieben:
[TIP]
====
...
// TK-RE: In diesem Kapitel durchgehend die Dateien, Pfade, import-Befehle und geänderten Aufrufe (register>discovery) ersetzt.
...
In einem Unterverzeichnis `<plug-in_family>` werden dann nach Bedarf für die verschiedenen APIs weitere Unterverzeichnisse mit vorgegebenem Namen angelegt, z.B. `agent_based` für die Check-API agentenbasierter Check-Plugins.
...
Damit das Ganze auch etwas bewirken kann, müssen Sie die neue Agentensektion mit der neuen Parse-Funktion erzeugen, damit sie von {CMK} erkannt und berücksichtigt wird.
...
// TK-RE: Formale Änderungen wie im vorherigen Kapitel gemacht.
...
-- im ersten Schritt so wie xref:register_check_plug-in[zuvor] durch die Erstellung einer Instanz der Klasse `CheckPlugin`.

// TK-RE: Ich hab den TIP auskommentiert, da ich im folgenden Code-Schnipsel alt und neu zeige, so dass es (noch) klarer werden sollte, dass das alte Check-Plugin in der Datei drin bleibt.
////
[TIP]
====
// Die neue Registrierung wird zusätzlich zur bereits vorhandenen vorgenommen, die im xref:write_check_plugin[vorherigen Kapitel] gezeigte bleibt unverändert enthalten.
Das neue Check-Plugin wird zusätzlich zum bereits vorhandenen erstellt, das im xref:write_check_plugin[vorherigen Kapitel] gezeigte bleibt unverändert enthalten.
====
////
...
// LT@TK: Womit sich die Frage stellt, ob notice_only hier überhaupt erwähnt werden soll
...
// TK-RE: In der API-Dok steht nur float, nicht integer.
...
// TK-RE: Ich mag mich täuschen, aaaaaber: Bereits vor diesem Kapitel (also vor der Definition der Metriken) wird das Symbol und 6 leere Graphen angezeigt:
// TK-RE: für die 4 ausgelesenen Werte (num_hosts, num_hosts_up, num_services, num_services_ok) und die 2 berechneten Werte (hosts_up_perc, services_ok_perc)
// TK-RE: Liegt es vielleicht daran, dass ich früher bereits diese Metriken in meiner Testinstanz installiert hatte?
Wenn Sie mit der Maus auf das Symbol zeigen, werden die Graphen als Vorschau eingeblendet.
// TK-RE: Jetzt stimmt es wieder und es werden nur die beiden Graphen angezeigt, allerdings ohne die gelben und roten Linien für die levels_lower. Warum fehlen die jetzt?
...
// TK-RE: Beim Review die Verwendung der API-Begriffe kontrollieren: Klasse, Instanz, Funktion, Objekt, ...
...
[TIP]
====
Beachten Sie, dass die in diesem Abschnitt vorgestellten Beispiele _nicht_ durch die Check-API abgedeckt sind.
====
////
...
// TK-RE: Das da ist obsolet, richtig? i18n geht jetzt anders, aber wie genau?
// TK-RE: Müssten wir nicht _Localizable importieren und vor alle Titel einen Unterstrich setzen?
// TK-RE: Oder lassen wir das Thema i18n unter den Tisch fallen?
////
...
----
// TK-RE: Label und BooleanChoice werden bei uns aber gar nicht verwendet?
...
////
Wenn Sie den Benutzern des Regelsatzes statt leerer Eingabefelder bereits Werte vorgeben wollen, können Sie das tun, indem Sie die `Percentage()` Funktion um das Argument `default_value` erweitern.
...
Der Name darf auf keinen Fall identisch sein mit einem bereits existierenden Regelsatz, weil dieser damit überschrieben würde.
Um dieser Gefahr aus dem Weg zu gehen, verwenden Sie am besten im Namen einen Präfix.
// TK-RE: Stimmen die beiden vorherigen Sätze noch?
...
// TK-RE: Stattdessen eigenes Topic mit CustomTopic definieren? Ich hab ein wenig rumprobiert, aber es nicht hinbekommen.
...
----
// TK-RE: Oder sollen wir hier mit omd restart die Abkürzung vorgeben?
...
In der Bedingung finden Sie das per `HostAndItemCondition` definierte Eingabefeld [.guihint]#Host group name# wieder zur Einschränkung der Regel für Host-Gruppen.
...
[IMPORTANT]
====
Wenn alles fertig ist und funktioniert, entfernen Sie unbedingt die `print`-Befehle wieder.
Diese können die interne Kommunikation von {CMK} durcheinanderbringen.
====
// TK-RE: IMPORTANT ist schon ganz schön heftig - zumal ich nicht weiß, ob das wirklich (noch) stimmt.
...
// TK-RE: KeyError? Bei cmk --detect-plugins=myhostgroups_advanced -v localhost gibt es ohne Regel für jeden Host-Gruppen-Service: check failed - please submit a crash report!
...
// TK-RE: Dieser Abschnitt ist m.E. überflüssig, denn die neuen APIs sollen Copy&Paste aus CMK-Soure-Dateien überflüssig machen.
////
[#reuse_ruleset]
...
[TIP]
====
Beachten Sie, dass die in diesem Kapitel vorgestellten Beispiele _nicht_ durch die Check-API abgedeckt sind.
====
////
...
----
// TK-RE: Open wird bei uns aber gar nicht verwendet?
// TK-RE: Keine Erklärung der Module notwendig, oder?
...
Wählen Sie einen eindeutigen Namen, so dass keine vorhandene Metrik „überschrieben“ wird.
// TK-RE: Stimmt der vorherige Satz noch?
...
////
* Die in {CMK} vordefinierten Farbnamen für die Farbdefinition `color` finden Sie auf link:https://github.com/Checkmk/checkmk/blob/master/packages/cmk-graphing/cmk/graphing/v1/metrics.py[GitHub.^]
// TK-RE: Die Klasse Color in der API-Definition lässt mich ratlos zurück. Warum stehen in der API-Dok keine Farbnamen oder wie ich HEX-Werte festlegen kann oder oder oder?
// TK-RE: Ich hab erstmal die metrics.py verlinkt, weil mir sonst zum Thema nichts eingefallen ist.
...
// TK-RE: Ich raffs einfach nicht, wie die Perfometer jetzt funktionieren. Da ist die API-Dok an mir als Zielgruppe vorbeigeschrieben.
// TK-RE: Ich hab das Beispiel auf das einfachst mögliche eingedampft - und den alten Text auskommentiert stehengelassen. Mattias, bitte übernehmen ;-)

Möchten Sie zu einer Metrik noch ein Perf-O-Meter in der Zeile der Service-Liste anzeigen?
Das könnte zum Beispiel so aussehen:

.Das Perf-O-Meter zeigt den Prozentsatz der Services
...
// TK-RE: Wie ist das Mapping der Perfometer-Definition zu den Services bei denen das Perfometer angezeigt wird?
// TK-RE: Wenn ich als name = "myhostgroups" eintrage, bleiben die Perfometer trotzdem an den beiden Services kleben, die aus myhostgroups_advanced kommen.
...
[#format_numbers]
== Zahlen formatieren
// TK-RE: Alle Funktionen neu getestet (nur die Ausgabe von date hat sich geändert).
// TK-RE: import-Befehl und date Ausgabe geändert, sonst 1x quergelesen > Review muss nicht sein.
...
[#errors]
== Fehler beheben
// TK-RE: Kapitel NICHT aktualisiert.
...
// TK-RE: Obacht: In folgendem Beispiel-Output stehen alte Pfade drin wie cmk/base/api/agent_based/register
[{shell}]
----
...
* Im Treasures-Verzeichnis der {CMK}-Instanz finden Sie in `~/share/doc/check_mk/treasures/migration_helpers/` Skripte, die Ihnen bei der Migration auf die neuen APIs helfen.
// TK-RE: In der Instanz gibt es 2 Skripte, auf GitHub 5 (https://github.com/Checkmk/checkmk/tree/master/doc/treasures/migration_helpers). Nach GitHub verlinken?
// TK-RE: GitHub Diff anbieten für die Migration unserer Beispieldateien? 
...
|`~/local/lib/python3/cmk_addons/plugins/<plug-in_family>/agent-based` |Ablageort für selbst geschriebene Check-Plugins.
...
|`/usr/lib/check_mk_agent/plugins/` |Dieses Verzeichnis liegt sich auf einem überwachten Linux-Host. Hier erwartet der {CMK}-Agent für Linux Erweiterungen des Agenten (Agentenplugins).
|===
