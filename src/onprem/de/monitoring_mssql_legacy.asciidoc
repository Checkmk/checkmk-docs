...
und wird mit der nächsten Major-Version aus {CMK} entfernt werden (siehe link:https://checkmk.com/werk/17669[Werk pass:q[#]17669^]).
Wechseln Sie daher auf das neue Plugin und den Regelsatz [.guihint]#Microsoft SQL Server (Linux, Windows)# für die Agentenbäckerei.
...
image::mssql_bakery_ruleset_search.png[alt="Die Regel 'Microsoft SQL Server (deprecated)' in den Agentenregeln."]

Wählen Sie in der geplanten Regel im Abschnitt [.guihint]#Microsoft SQL Server (deprecated)# idealerweise die Option [.guihint]#System Authentication# für eine einfache und problemlose Anbindung.
...
Der Shell bzw. der Befehlszeile muss vorher nur das jeweilige Konfigurationsverzeichnis bekannt gemacht werden.

Um das Plugin manuell anführen zu können, öffnen Sie zuerst eine Befehlszeile mit Admin-Rechten.
...
*Hinweis:* Auch hier ist die gesetzte Umgebungsvariable nicht permanent, sondern besteht nur, solange diese Befehlszeile offen ist.
