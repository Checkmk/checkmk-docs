...
Dafür finden Sie in der {CMK} Exchange ein ausführlich kommentiertes Beispiel: das link:https://exchange.checkmk.com/p/hello-world[Hello world!^] MKP, welches eng an das in diesem Artikel vorgestellte Beispielszenario angelehnt ist.
...
Im einfachsten Fall dient der Regelsatz lediglich der Aktivierung des Plugins: So ordnen Sie Ihr Plugin bestimmten Zielhosts zu.
...
Allerdings haben wir im Beispielszenario spezifiziert, dass ein Ausführungsintervall bestimmt und die beiden Konfigurationsvariablen `user` und `content` gesetzt werden sollen.
