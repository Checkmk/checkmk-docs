...
Die API mit ihrer Dokumentation ist versioniert und nutzt dabei eine zweistufige Nummerierung nach den Grundprinzipien der Spezifikation für link:https://semver.org/[Semantic Versioning 2.x^] im Format `X.Y`, wobei `X` für eine _Major Version_ steht und `Y` für eine _Minor Version_. 
...
Seit {CMK} {v20} ist link:https://docs.checkmk.com/plugin-api/{gitbranch}/cmk.base.plugins.bakery.bakery_api/#version-1-cmk-base-plugins-bakery-bakery-api-v1[Version 1^] die aktuelle Version der Bakery-API, die in diesem Artikel beschrieben wird.
...
Beachten Sie für diesen Teil Ihrer Erweiterungspakete unseren Artikel zur Entwicklung xref:devel_check_plugins#[agentenbasierter Check-Plugins].
...
Da es während der Programmierung eines Bakery-Plugins sehr lästig sein kann, ständig Versionsnummern zu inkrementieren, empfehlen wir, auf die Befehlszeile zu wechseln und dort den Befehl `cmk` zu verwenden.
