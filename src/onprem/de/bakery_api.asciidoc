...
[TIP]
====
Auch wenn die _Bakery_ nur in den kommerziellen Editionen erhältlich ist, existiert die _API_ seit {CMK} {v23} in allen Editionen. Für die Installation von Paketen mit Bakery-API auf {RE} bedeutet das, dass die zusätzliche Funktionalität schlicht ignoriert wird.
Dies gibt Ihnen die Möglichkeit, Erweiterungspakete zu erstellen, welche auf allen Editionen installiert werden können.
====
...
Mehr dazu erfahren Sie in der Einführung zum xref:devel_check_plugins#[Schreiben eigener agentenbasierter Check-Plugins.]
...
Sie finden zu diesem Thema eine Einführung im Artikel zum xref:devel_check_plugins#rule_set[Schreiben eigener agentenbasierter Check-Plugins.]
