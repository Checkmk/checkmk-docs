...
Auf {CE} baut dann xref:managed#[{CME}] auf.
...
* Nur {CE} ist über die Marketplaces von xref:install_azure#[Microsoft Azure] und xref:install_aws#[AWS] verfügbar.
...
|Push-Modus |In {CE} kann der xref:wato_monitoringagents#agents[{CMK}-Agent] für Linux und für Windows im xref:glossar#push_mode[Push-Modus] konfiguriert und genutzt werden.
...
|Test der Edition |Sie können {CE} 30 Tage lang in vollem Funktionsumfang unverbindlich testen.
