...
{CMK} liefert dafür einen OpenTelemetry Collector, mit dem Daten passiv empfangen und aktiv über einen _Prometheus Scraper_ abgefragt werden können.

|Metrik-Backend |{CMK} setzt -- zurzeit -- nur für die OpenTelemetry-Metriken als xref:metric_backend#[Metrik-Backend] _ClickHouse_ ein, dessen spaltenbasierte Datenbank die eingehenden OpenTelemetry-Metriken sehr effizient verarbeiten kann.
|===
