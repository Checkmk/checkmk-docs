...
Im Folgenden möchten wir Ihnen mit {CCE-onprem} eine der kommerziellen xref:glossar#edition[Editionen] vorstellen.


[#editions]
== Einordnung von {CE-onprem}

{CE} gibt in in zwei Varianten: als {CE-saas} und {CE-onprem}.
Während {CE-onprem} eine der _On-Premises_-Editionen von {CMK} ist, die alle das Aufsetzen eines eigenen {CMK}-Servers voraussetzen, läuft bei {CE-saas} der {CMK}-Server bereits in der AWS Cloud.

Die Software von {CE-saas} basiert auf der von {CE-onprem}.
...
Als On-Premises-Edition baut {CE-onprem} inhaltlich auf xref:cse#[{CSE}] auf, bietet aber zusätzlich umfangreiche Cloud-Funktionen.
...
Darüber hinaus bietet {CE-onprem} auch Cloud-spezifische xref:glossar#check_plugin[Check-Plugins.]
So können Sie beim Übergang aus dem Rechenzentrum in eine Cloud weiter {SE} nutzen.
Möchten Sie danach auch SaaS und PaaS Produkte, die eine Cloud Ihnen bietet, nutzen, können Sie auf {CE-onprem} upgraden.

Auf {CE-onprem} baut dann xref:managed#[{CME}] auf.
...
[TIP]
====
Immer dann, wenn eine Verwechslung mit {CE-saas} ausgeschlossen ist -- wie in den anderen Artikeln dieses {CMK} Handbuchs -- nennen wir {CE-onprem} einfach {CE}.
====
