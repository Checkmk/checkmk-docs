...
:revdate: 2024-06-06
:title: Checkmk Cloud
:description: Was Checkmk Cloud ausmacht, vor allem auch im Vergleich zu den anderen {CMK}-Editionen, erfahren Sie hier.
...
Im Folgenden möchten wir Ihnen mit {CCE} eine der kommerziellen xref:intro_setup#editions[Editionen] vorstellen.


[#editions]
== Einordnung von {CE}

Inhaltlich baut {CE} auf xref:cse#[{CSE}] auf, bietet aber zusätzlich umfangreiche Cloud-Funktionen.
...
Darüber hinaus bietet {CE} auch Cloud-spezifische xref:glossar#check_plugin[Check-Plugins.]
So können Sie beim Übergang aus dem Rechenzentrum in eine Cloud weiter {SE} nutzen.
Möchten Sie danach auch SaaS und PaaS Produkte, die eine Cloud Ihnen bietet, nutzen, können Sie auf {CE} upgraden.


[#functions]
== Zusätzliche Funktionen von {CE}

Die wesentlichen Funktionen von {CE}, die diese von anderen Editionen unterscheiden:

* {CE} ist über die Marketplaces von xref:install_azure#[Microsoft Azure] und xref:install_aws#[AWS] verfügbar.
...
Mit {CE} können Sie zusätzlich den xref:glossar#push_mode[Push-Modus] nutzen.

* Autoregistrierung von Hosts: +
In {CE} können Sie xref:hosts_autoregister#[Hosts automatisch erstellen lassen], auf denen ein {CMK}-Agent installiert ist.
...
*Hinweis:* In der {CMK}-Benutzeroberfläche werden Elemente, die es nur in {CE} gibt, durch den Zusatz [.guihint]#(Cloud Edition)# gekennzeichnet, z.B. Regeln, Dashboards oder Parameterwerte, 


[#upgrade]
== Upgrade zu {CE}

Sie können {CE} unverbindlich testen, denn sie ist in den ersten 30 Tagen nicht limitiert.
Durch Eingabe eines Lizenzschlüssels nach spätestens 30 Tagen können Sie {CE} dann ohne Beschränkungen einsetzen.

Für den Wechsel von einer der anderen Editionen zu {CE} folgen Sie der xref:update#upgrade[Upgrade-Beschreibung.]
...
|Cloud-Dashboards |In {CE} finden Sie im Monitoring die spezifischen Dashboards für xref:monitoring_aws#dashboards[AWS,] xref:monitoring_azure#dashboards[Azure] und xref:monitoring_gcp#dashboards[GCP.]
|===
...
|Push-Modus |Nur in {CE} kann der xref:wato_monitoringagents#agents[{CMK}-Agent] für Linux und für Windows im xref:glossar#push_mode[Push-Modus] konfiguriert und genutzt werden.
...
|Autoregistrierung |{CE} bietet die Möglichkeit, Hosts automatisch bei der Registrierung anzulegen mit der sogenannten xref:hosts_autoregister#[Autoregistrierung.]
...
|Agentenbäckerei (_Agent Bakery_) |In {CE} können Sie die Agentenbäckerei zusätzlich nutzen, um Agentenpakete mit einer Konfiguration für die Autoregistrierung zu versehen.
...
|Berechtigung zur Agentenregistrierung | Die xref:wato_user#predefined_roles[vordefinierte Rolle] `agent_registration` enthält in {CE} zusätzliche Berechtigungen, um Hosts automatisch zu erstellen.
|===
...
|Check-Plugins für AWS |In {CE} gibt es zusätzlich Check-Plugins für die AWS Cloud-Produkte
...
|Check-Plugins für Azure |In {CE} gibt es zusätzlich Check-Plugins für die Azure-Produkte
...
|Check-Plugins für GCP |In {CE} gibt es zusätzlich Check-Plugins für die Goggle Cloud-Produkte
