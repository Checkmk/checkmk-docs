...
// TK: Grundsätzlich: Das mehrmalige Abfrühstücken des gleichen Themas in einem Artikel macht den Update ganz schön anstrengend.
// TK: Ein wichtiges Feature wird zuerst in diesem Abschnitt erwähnt, dann im nächsten Abschnitt als Unterscheidungsmerkmal nochmal ausführlicher und dann nochmal unten als Tabelleneintrag.
// TK: Man könnte hier z.B. die 3 nächsten Punkte unter dem Listeneintrag Monitoring-Agenten zusammenfassen und dann kürzer beschreiben.
...
// ES: The following paragraph needs attention for KNW-2113
{CE-onprem} ist einerseits die Basis für xref:managed#[{ME}] und andererseits für {CCE-saas}, die {CMK} als _Software as a Service (SaaS)_ in der Cloud zur Verfügung stellt.
...
* {CMK} Relay: +
Mit dieser abgesetzten Einheit können Sie Hosts überwachen, die selbst nicht von der Checkmk-Instanz erreichbar sind.
...
* Observability: +
Observability erweitert das klassische Monitoring komplexer IT-Systeme durch die Analyse von Datentypen (zum Beispiel Metriken oder Logs), mit dem Ziel, nicht nur festzustellen, _dass_ ein System fehlerhaft arbeitet, sondern _warum_ dem so ist. In {CE-onprem} können Daten über den link:https://opentelemetry.io/[OpenTelemetry^]-Standard bezogen werden.
...
Sie können ein Upgrade der {RE} oder der {SE} auf {UE} durchführen.
...
// TK: Relay passt auch nicht so wirklich zur Überschrift
|{CMK} Relay |Das xref:relay#[{CMK} Relay] ist eine abgesetzte Einheit einer {CMK}-Instanz, die die Überwachung per xref:glossar#snmp[SNMP] oder xref:glossar#special_agent[Spezialagenten] von Hosts ermöglicht, die selbst nicht von der Checkmk-Instanz erreichbar sind, beispielsweise weil eine Firewall-Konfiguration den eingehenden Datenverkehr für ein Netzwerk blockiert.
...
// TK: OpenTelemetry-Daten statt OpenTelemetry-Metriken wg. Support von Logs via EC
|OpenTelemetry |Sie können xref:opentelemetry#[OpenTelemetry-Daten] empfangen und im Monitoring verarbeiten.
{CMK} liefert dafür einen OpenTelemetry-Kollektor, mit dem Daten passiv empfangen und aktiv über einen _Prometheus Scraper_ abgefragt werden können.

// TK: Metrik-Backend erstmal hier geparkt, obwohl es nicht zur Überschrift Observability passt.
|Metrik-Backend |{CMK} setzt -- zurzeit -- nur für die OpenTelemetry-Metriken das xref:metric_backend#[Metrik-Backend] _Clickhouse_ ein, dessen Spalten-basierte Datenbank die eingehenden OpenTelemetry-Metriken sehr effizient verarbeiten kann.
|===
