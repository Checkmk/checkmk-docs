...
// ES: Ich finde die Überschriften, die "online" oder "offline" enthalten, schwierig zu lesen. Können wir sie umstellen? Etwa so:
// 3.2. Lizenzinformationen automatisch online übermitteln lassen
// 3.3. Lizenzinformationen offline übermitteln
// 4.2. Lizenzinformationen online übermitteln
// 4.3. Lizenzinformationen offline übermitteln
...
// ES: Ich weiß, die Artikelstruktur war schon vor KNW-1921 so - aber wäre es möglich, dass wir die Abschnitte tauschen, sodass Erläuterungen zu Checkmk Pro vor den Erläuterungen zu Checkmk Ultimate erscheinen?
[#license_cce]
== Lizenzierung von {CE}
...
Zugleich laden Sie die Verifizierungsantwort in {CMK} hoch.
// ES: Dieses "zugleich" erschließt sich mir beim Lesen nicht. Entweder es passiert automatisch, oder ich führe die Schritte nacheinander aus.
Durch diese beiden Schritte lizenzieren Sie Ihre {CE} bzw. {ME} und schalten diese zur weiteren Nutzung frei.
...
[#submit_usage_cce]
=== Automatisch online Lizenzinformationen übermitteln lassen
...
// ES: Im Screenshot ist von der Checkmk Cloud Edition die Rede, im Satz nach dem Screenshot wird von Checkmk Ultimate gesprochen.
// Wie ist der Stand von diesem Screenshot bzgl. KNW-2023?
// ES: ggf. mit Developer Tools im Browser den Editionsnamen auf dieser Seite ändern?
...
[#manualtrans_cce]
=== Offline Lizenzinformationen übermitteln
...
[#submit_usage_cee]
=== Online Lizenzinformationen übermitteln
...
Ab jetzt meldet {CMK} regelmäßig Ihre aktuelle Lizenznutzung an {com} GmbH/Inc.
Nach jeder erfolgreichen Übertragung sehen Sie eine Bestätigung sowie eine Zusammenfassung der übermittelten Informationen.
// ES: Der obige Satz ist fast identisch zu einem Satz oberhalb vom Screenshot, würde einen davon entfernen
Im Rahmen dieses Prozesses holt {CMK} auch automatisch neue Lizenzinformationen aus dem {CMK}-Kundenportal, falls Sie zwischenzeitlich die Lizenz erweitert oder verlängert haben.
...
// ES: Ist es richtig, dass der Knopf hier anders heißt als in Abschnitt 3.3? Oder wurde ein Abschnitt zwischenzeitlich aktualisiert und der andere nicht?
...
// ES: Ich fände es einfacher zu verstehen, wenn im Folgenden statt von bunten "Spalten" lieber von "Balken" gesprochen wird
