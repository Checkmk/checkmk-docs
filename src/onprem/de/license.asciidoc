...
{cee-only}
Nutzen Sie eine der kommerziellen Editionen, d.h. die {CSE}, die {CME} oder die {CCE} (oberhalb der Grenzen des Lizenzstatus „Free“: 750 Services, eine Instanz), so sind Sie verpflichtet, regelmäßig eine Übersicht über den Einsatz von {CMK} in Ihrem Unternehmen an die {comfull} zu liefern.
...
Den Lizenzschlüssel hinterlegen Sie in {CMK}. Zugleich laden Sie die Verifizierungsantwort in {CMK} hoch. Durch diese beiden Schritte lizenzieren Sie Ihre {CE} und schalten diese zur weiteren Nutzung frei.
...
Als Benutzer der {CE} lassen Sie {CMK} Ihre Lizenzinformationen per Online-Kommunikation übertragen.
...
Die Lizenzierungs-Anmeldedaten müssen vor der ersten Übertragung in {CMK} hinterlegt worden sein (siehe xref:setup_cce[Subskriptionsdaten in {CMK} eingeben]).
...
|Ausgeschlossene Services / Hosts |Services bzw. Hosts, die nicht in die Lizenzgröße eingerechnet werden. Dies sind z.B. Services / Hosts, die nur für interne Testszenarios in Ihrem Unternehmen genutzt werden. Um Services oder Hosts als ausgeschlossen zu markieren, setzen Sie Labels vom Typ `cmk/licensing:excluded` entsprechend der Beschreibung  zur xref:labels#create_labels[Erstellung von Labels].
...
Klicken Sie dann auf den Knopf icon:icon_assume_0[alt="Symbol zur Offline-Übertragung der Lizenzdaten."] [.guihint]#Offline verification# in der {CE} bzw. [.guihint]#Submit license usage offline# in den anderen kommerziellen Editionen.

Sie gelangen auf eine dieser Seiten:

.Offline-Verifizierung der Lizenz in der {CE}
image::license_offline_verification.png[alt="Seite zur Offline-Verifizierung der Lizenzinformationen in der {CE}.", width=85%]

.Offline-Übermittlung der Lizenzinformationen in den anderen kommerziellen Editionen
image::license_offline_submission.png[alt="Seite für Offline-Übermittlung der Lizenzinformationen in den anderen kommerziellen Editionen.", width=85%]
