...
Nach der Ersteinrichtung -- und im Falle der Varianten von {CE} der Lizenzeinspielung zum Ende der Testphase (Lizenzstatus „Trial“) -- kann {CMK} Ihnen den Rest dieser Verwaltungsarbeit komplett abnehmen.
...
Klicken Sie nun auf [.guihint]#Generate Licensing Credentials#, um die Erstellung des Passworts zu beginnen.
...
Die Lizenz-ID hinterlegen Sie in {CMK}. Zugleich laden Sie die Verifizierungsantwort in {CMK} hoch. Durch diese beiden Schritte lizenzieren Sie Ihre {CE} bzw. {ME} und schalten diese zur weiteren Nutzung frei.
...
Da dies jedoch aufwändiger ist, empfehlen wir Ihnen, dies nur zu nutzen, wenn Sie aus technischen Gründen dazu gezwungen sind.
