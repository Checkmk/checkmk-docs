...
[TIP]
====
Die komplette Kompatibilitäts-Matrix steht erst nach Release von Checkmk {v23} zur Verfügung.
Benutzen Sie während der Betaphase die xref:release_notes#[Release notes] und die xref:ossupport[Richtlinie zur Betriebssystemunterstützung] (in diesem Artikel) als Referenz.
====
