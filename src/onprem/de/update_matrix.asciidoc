...
:description: Jede {CMK}-Version unterstützt gewisse Linux-Distributionsversionen. Lesen Sie hier, wie Sie Linux-Upgrades im Zusammenspiel mit {CMK}-Updates planen.    
...
//SP: Hier ist die Rede von Linux-Versionen, warum geht es dann um Upgrades und nicht um Updates?
...
//SP: Ist dafür egal, ob es sich auf CMK-SEite nur um ein Update oder auch um ein Upgrade handelt?
// Aus der Versionsnnummer kann man nicht erkennen welche Variante gemeint ist. Soll das so sein?
Für ein Update von Debian 11 mit {CMK} {oldstable}p1 auf Debian 13 mit {CMK} {current} ergibt sich so die folgende Update-Reihenfolge:

. Unter Debian 11 führen Sie das Update von {CMK} {oldstable}p1 xref:update#[auf die aktuelle Patch-Version] von {oldstable} ({oldstable-patch}) durch.

. Führen Sie das Upgrade von Debian 11 auf 12 und schließlich auf 13 durch, so wie es im Artikel zum xref:release_upgrade#[Linux-Upgrade auf dem {CMK}-Server] beschrieben ist.

. Unter Debian 13 aktualisieren Sie {oldstable-patch} auf die xref:update_major#[aktuelle Patch-Version {current}].
...
Bekanntgegeben wird dies beim Erscheinen einer neuen {CMK}-Version.
