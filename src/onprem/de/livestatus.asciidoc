...
Mit dem Header `OrderBy` können Sie die Ausgabe nach den Inhalten beliebiger Spalten auch auch Werten beliebiger Dictionary-Schlüssel sortieren.
So lässt sich bspw. eine alphabetische Auflistung aller Host erstellen:

// MFS: Evtl. im folgenden Beispiel "asc" weglassen... 
[{shell}]
----
{c-omd} lq "GET hosts\nColumns: name\nOrderBy: name asc"
ahost
...
// MFS: ...und dann hier erklären, dass Standardsortierung "asc", also aufsteigend ist, und man "asc" oder "desc" explizit angeben kann.

Durch die Angabe von `desc` lässt sich die Ausgabe einfach umdrehen:
...
Ein weiterer Anwendungsfall ist die Kombination mit einem xref:livestatus#commands[Kommando.]
...
Diese prüft, ob die Daten des Service {CMK} jünger sind als der definierte Zeitpunkt.
