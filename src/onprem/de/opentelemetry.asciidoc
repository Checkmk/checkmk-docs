...
{cee-only}
Ab {CMK} {v24} können Sie in {CCE} link:https://opentelemetry.io/[OpenTelemetry^]-Metriken empfangen und im Monitoring verarbeiten.
...
[TIP]
====
Dieser Artikel beschreibt den Stand der OpenTelemetry-Integration bei Veröffentlichung von {CMK} {v24}p21.
...
Zunächst müssen Sie in den globalen Einstellungen ([.guihint]#Global settings#) den Kollektor aktivieren.
...
Die Einrichtung des Kollektors nehmen Sie unter  [.guihint]#Setup > Hosts > OpenTelemetry collector (experimental)# vor, wenn der Kollektor sich passiv verhalten und Daten _empfangen_ soll.
Soll sich der Kollektor _aktiv_ verhalten – also Prometheus-Endpunkte abfragen –, verwenden Sie [.guihint]#Setup > Hosts > OpenTelemetry collector: Prometheus scraping (experimental)#.
In beiden Fällen starten Sie mit dem Knopf [.guihint]#Add OpenTelemetry collector configuration# die Einrichtung eines neuen Kollektors:
...
// MFS: Wurde entfernt. Es wird nun immer ein Service erzeugt.
