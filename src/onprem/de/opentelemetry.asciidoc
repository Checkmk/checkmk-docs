...
[TIP]
====
Ist eine etwas komplexere Erstellung der Hostnamen gewünscht, speichern und aktivieren Sie die Kollektorkonfiguration, ohne eine Regel für die [guihint]#Host name computation# hinzugefügt zu haben.
...
====
// ES: Das kann ich nicht reproduzieren. Nach dem Anlegen von dice-server und hello-metric sehe ich keine Vorschläge zur Host name computation, die offensichtlich auf diese beiden neuen Hosts bezogen sind.
...
// ES: Wenn das Dropdown "Monitoring agents:" im Screenshot etwas breiter wird, kann man zumindest erahnen, welcher Punkt ausgewählt ist, auch wenn der Eintrag nicht in voller Breite zu sehen ist.
...
// ES: Hier noch 1-2 Sätze einfügen zur Motivation, warum neben dem ersten Beispiel auch das zweite gezeigt wird
...
Für eine übersichtlichere Darstellung erfolgt daher die Umwandlung unter Berücksichtigung der Zeitkomponente.
// ES: "erfolgt" an welcher Stelle? In app.py? Oder in meinem Spezialagenten? 

// ES: Satz zum besseren Verständnis eingefügt, bitte factchecken:
...
Namen bekannter Metriken sind über die Dropdown-Auswahl beziehungsweise als Vorschlag während des Tippens zugänglich.
Noch unbekannte Metriken, bei denen Sie wissen, dass sie erscheinen werden, können Sie als Freitext eintragen.

Je nachdem, welchen Datentyp der OpenTelemetry-Datapoint liefert, kann es sinnvoll sein, Raten berechnen zu lassen.
Bei Zählern – wie für verworfenene Netzwerkpakete – ist dies sinnvoll.
...
|===
|Pfad |Bedeutung
| `~/var/check_mk/otel_collector/host_monitoring/` |Hier entstehen OpenTelemetry-Daten. Für jeden Host wird ein Unterverzeichnis erstellt.
...
| `~/tmp/check_mk/otel_collector/autocompleter/` |Temporäres Verzeichnis, in dem von der Vorschlagsfunktion (Dropdowns und Vorschläge beim Tippen) benötigte Dateien abgelegt werden.
|===
