...
Sorgen Sie dann dafür, dass Metriken zum Kollektor geschickt werden.
...
OpenTelemetry bietet auch die Möglichkeiten, Log-Meldungen zu übertragen.
...
Diese hat sechs Graphen, `pass:[Value_1__Per_Sec]` bis `pass:[Value_6__Per_Sec]`, welche den Raten der gewürfelten Zahlen eins bis sechs entspricht.
...
Namen bekannter Metriken sind über Dropdown-Auswahl, beziehungsweise Vorschlag während des Tippens zugänglich.
...
Liefern mehrere Quellen Daten, die ein und demselben {CMK}-Service zugeordnet, können Sie darüber hinaus festlegen, ob eine Aggregierung (beispielsweise Summe oder Durchschnitt) vorgenommen werden soll oder schlicht die jüngsten, größten oder kleinsten Datenpunkte eines Intervalls dominieren.
