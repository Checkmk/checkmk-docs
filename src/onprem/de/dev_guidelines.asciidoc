...
* Legen Sie das Plugin nach `~/share/check_mk/agents/plugins` für unixoide Systeme und setzen Sie die Ausführungsrechte auf `755`.
...
|Dateigröße |Byte
|Temperatur |Celsius
|Netzwerkdurchsatz |Oktette pro Sekunde (nicht Bits/s!)
...
Auch in der {RE} werden diese anhand der Metrikdefinition in {CMK} selbst erzeugt.
