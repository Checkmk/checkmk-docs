...
* Vergessen Sie den Hinweis auf das Plugin nicht in der Manual Page des Check-Plugins.
...
Jedes Check-Plugin *muss* eine Manual Page haben.
Falls Sie in einer Check-Plugin-Datei mehrere Plugins programmiert haben muss natürlich jedes davon eine eigene Manual Page haben.

Die Manual Page ist für den Benutzer gedacht!
...
Eine Manual Page besteht aus mehreren, teilweise optionalen, Bereichen:
...
Über den Header `catalog:` legen Sie fest, wo im Katalog der Check-Plugins die Manual Page einsortiert wird.
...
Folgende Informationen müssen in der `description:` der Manual Page enthalten sein:
...
Bei Checks, die ein Item haben (also auch ein `%s` im Service-Namen), muss in der Manual Page unter `item:` beschrieben sein, wie dieses gebildet wird.
