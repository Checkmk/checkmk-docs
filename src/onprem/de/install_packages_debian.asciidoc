...
Sofern nicht anders angegeben, gelten für ältere als den im Folgenden genannten Versionen die gleichen Anforderungen.
// TK: "Sofern nicht anders angegeben": Es werden ja gerade *keine* anderen = älteren Versionen erwähnt > Löschen
// TK: "und _STS-Versionen (Short Term Support)_" rausgenommen, weil STS ja nicht mehr unterstützt wird.
Tauschen Sie in diesem Fall einfach den Code-Namen aus.
...
// TK: Dot Release? Warum nicht Patch Release oder Patch-Version? Und was heißt "früher" als ein Dot Release? Ist ein Dot Release nicht die kleinste Release-Einheit?
...
Die `updates`-Repositories sind empfohlen, aber nicht zwingend erforderlich.
Stabilitäts-Updates werden hier früher als zu den _Dot Releases_ bereitgestellt.
...
// TK: dpkg-sig gibts nicht mehr für 24.04. Vorschlag: install dpkg-sig und später dpkg-sig --verify streichen und die Signatur nur mit gpg --verify prüfen 
...
// TK: Etwas ausführlicher beschreiben?
// TK: Die Meldung `Good signature` ist der Nachweis, dass die Paketdatei mit dem zuvor heruntergeladenen und importierten Schlüssel `Check_MK-pubkey.gpg` signiert wurde.
// TK: Die angezeigte Warnung bezieht sich auf den öffentlichen Schlüssel von {CMK}, nicht auf die Signatur des Pakets.
// TK: Der Grund für die Warnung ist, dass der verwendete Schlüssel mit keinem anderen Schlüssel signiert ist, dem der Benutzer vertraut - was aber in diesem Fall kein Problem darstellt. 
