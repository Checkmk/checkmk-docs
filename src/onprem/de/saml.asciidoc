// -*- coding: utf-8 -*-
// IGNORE UserPrincipalName |
// NONASCII
include::global_attr.adoc[]
= Anmeldung mit SAML
:title: Anmeldung mit SAML
:description: Die Anmeldung in {CMK}-Instanzen kann über eine Single-Sign-On-Lösung (SSO) erfolgen. Lesen Sie hier, wie Sie die Authentifizierung mit SAML einrichten.
...
In diesem Artikel erfahren Sie, wie Sie eine Anmeldung via _Secure Assertion Markup Language_ (SAML) in {CMK} einrichten.
...
Mit Hilfe der Verbindung und Kommunikation zwischen sogenanntem „Service Provider“ (SP) und sogenanntem „Identity Provider“ (IdP) wird es somit den Mitarbeitenden ermöglicht, mit nur einer einzigen Anmeldung auf verschiedene Webanwendungen zuzugreifen.
...
Jeder durch SAML berechtigte Benutzer wird automatisch in {CMK} angelegt, sobald er sich das erste Mal dort anmeldet -- vorausgesetzt, es gibt nicht bereits einen Benutzer mit der gleichen ID.
...
Um SAML in den kommerziellen Editionen nutzen zu können, muss erst der IdP eingerichtet werden.
In unserem Beispiel ist das Entra ID (hieß bis 2023 Azure Active Directory).
...
Sie finden diese in der Ansicht [.guihint]#Enterprise applications | All applications > Browse Microsoft Entra Gallery > checkmk-saml | SAML-based Sign-On# (siehe oben):
...
Als nächstes legen Sie im Kasten [.guihint]#Security# fest, ob Sie die Zugriffsverbindungen mit xref:certificates#[{CMK}- oder mit eigenen Zertifikaten] absichern wollen:
