...
:description: Lesen Sie hier, wie Sie mit dem omd-Befehl {CMK}-Instanzen erstellen und verwalten.    
:experimental:
...
OMD war ein von Mathias Kettner gegründetes Open-Source-Projekt, das zum Ziel hatte, eine aus vielen Komponenten bestehende Monitoring-Lösung einfach und schnell konfigurierbar zu machen.
Der von {CMK} mitgelieferte `omd`-Befehl ist mittlerweile eng mit {CMK} verzahnt. 

// rund um die komfortable und flexible Installation einer Monitoring-Lösung aus diversen Komponenten dreht.

.Die Geschichte von OMD
[%collapsible]
====
In den Anfangstagen war {CMK} keine vollständige Monitoring-Software, sondern zunächst nur ein Agentenskript und ein dazugehöriger Check für Nagios und später kompatible Monitoringkerne.
Die von vielen zu Nagios beitragenden Entwicklern geäußerte Frustration über mangelnde Einbindung der Community führte zwischen 2009 und 2013 zu mehreren Nagios-Forks und eben auch der Eigenentwicklung des xref:cmc#[{CMK} Micro Core] durch Mathias Kettner.

Monitoring-Systeme bestanden zu dieser Zeit meist aus vielen Komponenten: dem Monitoring-Kern, einem Administrationswerkzeug, einem weiteren Frontend für die Visualisierung von Graphen und Dashboards, einer Datenbankkomponente und einer Benachrichtigungskomponente.

Die _Open Monitoring Distribution_ hatte dabei mehrere Ziele.
In erster Linie sollten die ausgewählten Komponenten korrekt aufeinander abgestimmt werden, so dass die Nacharbeit minimal blieb.
Zudem sollte der Aufbau einer Testumgebung – beispielsweise um andere Konfigurations-Frontends oder andere Monitoring-Kerne zu testen – schnell möglich sein.
Die Abstimmung der Arbeit an OMD und vieler enthaltener Komponenten erfolgte nicht nur auf Mailinglisten sondern auch in Person an Monitoring-Stammtischen oder Konferenzen wie der link:https://osmc.de/[OSMC^].
So verwundert es nicht, dass eine Vielzahl der damals beteiligten (und meist noch heute in der Branche aktiven) Entwickler und Unternehmen ihren Sitz in Süddeutschland haben.

Mittlerweile sind die Komponenten von {CMK} sehr eng miteinander verzahnt.
So ist beispielsweise für Nutzende keine Abgrenzung zwischen Verwaltungs-Frontend (_Setup_, ehemals _WATO_) und Monitoring-Frontend mehr wahrnehmbar.
Folge davon ist, link:https://omdistro.org/[keine gemeinsame Entwicklung mehr^] stattfindet, sondern zwei spezialisierte Versionen gepflegt werden:
Die von {CMK} lernen Sie hier kennen, daneben existiert eine weitere des Unternehmens _Consol_, welche den Nagios-4-Fork _Naemon_, die Oberfläche _Thruk_ und den Agenten _SNClient_  kombiniert.
====


[#omd_facts]
== Eigenschaften einer OMD basierten Installation

Eine OMD basierten Installation zeichnet sich durch diese Eigenschaften aus:
...
* und schließlich eine rückstandsfreie Löschmöglichkeit.

OMD wird auf der Befehlszeile verwaltet, mit dem Befehl `omd` -- genauer: mit einer Reihe von `omd`-Befehlen für die unterschiedlichen Aktionen rund um die Verwaltung der Monitoring-Instanzen, z.B. `omd create` für das Anlegen einer Instanz.
...
Sobald Sie als Instanzbenutzer angemeldet sind, brauchen Sie in der Regel bei `omd`-Befehlen keinen Instanznamen mit anzugeben, da so ein Befehl auf die Instanz angewendet wird, unter der Sie angemeldet sind.
...
.Im `omd config`-Menü navigieren Sie mit den Pfeil- und kbd:[Enter]-Tasten
...
Wer den interaktiven Modus nicht liebt oder mit Skripten arbeiten will, kann die einzelnen Einstellungen als Variablen auch per Befehlszeile setzen.
...
Die aktuelle Belegung aller Variablen zeigt als `root` der Befehl `omd config mysite show` und als Instanzbenutzer `omd config show`:
...
Die obige Ausgabe zeigt nur die ersten Einträge.
...
Dieser Befehl eignet sich daher _nicht,_ um eine Instanz, die z.B. durch ein Backup dupliziert wurde, mit dem Original in einem verteilten Monitoring zu betreiben -- auch wenn die beteiligten Instanzen nach Ausführung von `omd mv` unterschiedliche Namen haben.
...
Mit `omd update` wird übrigens auch ein xref:upgrade#[Upgrade] auf eine andere Edition durchgeführt, z.B. von {RE} auf {SE}.
...
*Hinweis:* {CMK} bietet auch die Möglichkeit Backup und Restore ohne Befehlszeile über die GUI durchzuführen unter [.guihint]#Setup > Maintenance > Backups#.
...
=== Daten vom Backup ausschließen

Der Löwenanteil der zu bewegenden Daten bei einer Instanzsicherung sind die in den xref:graphing#rrds[Round-Robin-Datenbanken (RRDs)] gespeicherten Messwerte und die Log-Dateien mit historischen Ereignissen.
Ein weiterer nicht unerheblicher Teil der Daten sind die Agentenpakete, die von der Agentenbäckerei im Verzeichnis `~/var/check_mk/agents` abgelegt werden, jedoch bei Bedarf problemlos neu erstellt werden können.

`omd backup` bietet zum Verzicht auf diese Daten xref:limit_data[die gleichen Optionen] wie `omd cp` beim Kopieren.
Im folgenden Beispiel wird das Backup komplett ohne diese Daten erstellt:
...
Sie können beim Backup auch gezielt festlegen, welche der genannten Daten Sie ausschließen möchten.
Mit `omd backup --help` können Sie sich die verfügbaren Optionen anzeigen lassen.


=== Backup bei laufender Instanz

Ein Backup kann auch von einer laufenden Instanz erstellt werden.
Um einen konsistenten Stand der für das Aufzeichnen der Messdaten verwendeten xref:graphing#rrds[RRDs] zu gewährleisten, versetzt der Befehl `omd backup` den Round-Robin-Cache automatisch in einen Modus, bei dem laufende Updates nur noch in das Journal und nicht mehr in die RRDs geschrieben werden.
...
Man muss wohl nicht extra dazu schreiben, dass hierbei *alle Daten der Instanz gelöscht* werden!
...
OMD kann ungenutzte Versionen mit dem Befehl `cleanup` deinstallieren:
