...
|`CORE` |`nagios` ({RE}), +
 `cmc` (kommerzielle Editionen) |Auswahl des Monitoring-Kerns. In den kommerziellen Editionen kann statt des xref:cmc#[{CMK} Micro Core (CMC)] auch der Nagios-Kern ausgewählt werden. In der {CRE} gibt es nur `nagios` als Monitoring-Kern.
