// -*- coding: utf-8 -*-
// IGNORE MyApp1 myapp1_mybot1 Test1 Windows|Linux mybot_managed rf Micromamba CSM RobotCode Venv nutzerseitiges namen Notepad++ mitzuverfolgen
// NONASCII
include::global_attr.adoc[]
= {CMK} Synthetic Monitoring mit Robotmk (CSM)
...
Sie befinden sich in der *aktuellen Micromamba/CSM-Version* dieses Artikels.
...
Standardmäßig nutzt {CMK} dafür RCC (ursprünglich entwickelt von Robocorp) -- wie im Artikel xref:robotmk#[{CMK} Synthetic Monitoring mit Robotmk (RCC)] beschrieben.
Sollte RCC bei Ihnen nicht funktionieren, können Sie alternativ unser hauseigenes Tooling CSM/Micromamba verwenden -- wie hier im Artikel "{CMK} Synthetic Monitoring mit Robotmk (CSM)" beschrieben.
====
...
CSM können Sie auf einem beliebigen Rechner nutzen.
Das Tool übernimmt das Erstellen der benötigten Konfigurationsdateien sowie das Verpacken dieser plus Test-Suites zu einem Automationspaket.
...
Übrigens, all diese Aufgaben ließen sich auch allein mit Micromamba erledigen, allerdings mit ziemlich komplizierten Befehlen.
Daher haben wir mit CSM einen Wrapper entwickelt, der Ihnen die Arbeit deutlich erleichtert.
...
Die link:https://robotframework-browser.org/[Browser Library von Robot Framework^] nutzt Playwright -- und Playwright läuft nicht auf allen von {CMK} unterstützten Linux-Systemen.
...
- Laden Sie link:https://github.com/Checkmk/csm/releases[CSM^] herunter.
...
- `.robot.toml`: Überschreibt bei Bedarf einzelne `robot.toml`-Einstellung auf dem lokalen Rechner (gehört entsprechend nicht ins Versionskontrollsystem)
...
Im Monitoring taucht der Umgebungsbau später als [.guihint]#RCC/Conda environment build status# auf (zur Erinnerung: RCC war die erste implementierte Lösung vor Micromamba/Conda).
...
Zur Erinnerung: CSM ist ein rein nutzerseitiges Tool, das als Wrapper den Umgang mit Micromamba erleichtert.
...
Auch hier ist der Grund simpel: So können Test-Suites, die eben nicht auf den Desktop angewiesen sind, ohne Verzögerung in eigenen Sequenzen ausgeführt werden -- wie zum Beispiel die hier im Artikel eingesetzte Test-Suite.
...
image::robotmk_scheduler_05_csm.png[alt="Konfiguration der Suite."]
...
Sehr nützlich kann auch [.guihint]#Grace period before scheduler starts# sein: Der Scheduler startet beim Neustart des Test-Hosts zusammen mit dem {CMK}-Agenten noch vor der Desktop-Anmeldung des Nutzers -- was freilich dazu führt, dass etwaige Tests auf dem Desktop fehlschlagen müssen.
Über eine Vorlaufzeit lässt sich der Start manuell verzögern, damit zum Beispiel per Autologin (Windows-Funktion zum Anmelden ohne manuelle Passworteingabe) der Desktop des technischen Benutzers (wieder) aktiv wird.
...
image::robotmk_monitoring_report_01_csm.png[alt="Robot-Framework-Bericht für Test-Suite 'Mybot1'."]
...
image::robotmk_managed_robots_scheduler_csm.png[alt="Konfiguration des Schedulers mit einem gemanagten Robot."]
...
Dafür muss dieser zuvor gestoppt und im Monitoring sollte eine xref:glossar#scheduled_downtime[Wartungszeit] gesetzt werden, um Auswirkungen auf das Monitoring und einen automatischen Neustart des Schedulers zu verhindern.
...
Auch bei der Ad-hoc-Ausführung müssen Sie den laufenden Scheduler-Service zunächst stoppen und sollten im Monitoring eine xref:glossar#scheduled_downtime[Wartungszeit] setzen.
