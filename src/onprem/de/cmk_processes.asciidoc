// -*- coding: utf-8 -*-
// IGNORE Addons Crontab speicherresident OOM
// NONASCII
include::global_attr.adoc[]
= Instanzdienste - {CMK} unter der Haube
:title: Instanzdienste - {CMK} unter der Haube
:description: Für den Betrieb einer {CMK}-Instanz werden viele einzelne Dienste/Prozesse benötigt. Diese werden in diesem Artikel kurz vorgestellt.
...
|`apache` |Ja |Ja |Ja |Webserver, mehr Details xref:apache[weiter unten]
...
|`xinetd` |Ja |Ja |Nein |TCP-Zugriff auf Livestatus
|===
...
Der Instanz-Apache stellt die grafische Benutzeroberfläche von {CMK} bereit und liefert in aktiven Benutzersitzungen Aktualisierungen von Tabellen und Graphen.
Beim Start einer Instanz werden fünf Apache-Prozesse gestartet, die Maximalzahl ist auf 64 eingestellt.
Sind die fünf Prozesse beschäftigt, werden weitere gestartet.
Unbeschäftigt vorgehalten benötigt ein Apache nur einige hundert Kilobyte (kB) RAM, voll beschäftigt hingegen 150 bis 500 Megabyte (MB), davon rund 200 speicherresident.
Ein Apache, der 1000 Anfragen bedient hat, beendet sich automatisch.

In vielen Fällen ist eine Änderung der Standardeinstellung nicht nötig, weil oft auch bei vielen gleichzeitigen Sitzungen ein Apache (oder weniger) pro Sitzung bereits genügt, damit alle Sitzungen ohne Wartezeit bedient werden.
Zudem gilt: Bei einer Instanz, auf die nur wenige Benutzer zugreifen, wird nur ein Bruchteil der 64 Apaches gestartet.
Es gibt jedoch Fälle, in denen das Tuning Sinn macht:

* Auf einer Zentralinstanz im xref:distributed_monitoring#[verteilten Monitoring], auf der relative wenige Monitoring-Daten verarbeitet werden, aber viele Benutzer Dashboards geöffnet haben, kann es mitunter erforderlich sein, die Zahl zu erhöhen.
* Umgekehrt kann es auf einer eher knapp dimensionierten Einzelinstanz, die viele Monitoring-Daten verarbeitet und dies zuverlässig tun soll, erforderlich sein, die Maximalzahl in den Bereich von zehn oder noch weniger zu senken:
Besser, Sie lassen einzelne Nutzer manchmal ein wenig warten, als zu riskieren, dass der OOM-Killer sein Werk tut -- und möglicherweise wichtigere Komponenten des Monitorings beendet.

Um einen Eindruck zu bekommen, wie viele Apaches tatsächlich benutzt werden, werfen Sie einen Blick in die Metriken des Services [.guihint]#Process mysite Apache#.
Für den Start ist es eine gute Idee, ein wenig über der durchschnittlich benötigten Zahl zu bleiben.
Zudem berechnet die xref:analyze_configuration#[Analyse der Konfiguration] eine sinnvolle Maximalzahl basierend auf dem verfügbaren Arbeitsspeicher.

Die Zahl der maximal gestarteten Apache-Prozesse justieren Sie mit der globalen Einstellung [.guihint]#Site management > Apache process tuning#.
Beachten Sie, dass es bei deutlichen Anpassungen der Zahl der Instanz-Apaches auch sinnvoll ist, die Zahl der System-Apache-Prozesse analog anzupassen.
Lesen Sie hierfür die Dokumentation der verwendeten Linux-Distribution.
...
Das wohl prominenteste Beispiel: Die xref:glossar#activate_changes[Aktivierung ausstehender Änderungen].
...
Der Dienst `automation-helper` beschleunigt einige GUI-Interaktionen, beispielsweise die xref:glossar#service_discovery[Service-Erkennung] und die Aktivierung ausstehender Änderungen,
