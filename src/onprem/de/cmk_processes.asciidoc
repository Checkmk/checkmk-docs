...
Der Instanz-Apache stellt die grafische Benutzeroberfläche von {CMK} bereit und liefert in aktiven Nutzersitzungen Aktualisierungen von Tabellen und Graphen.
Beim Start einer Site werden fünf Apache-Prozesse gestartet, die Maximalzahl ist auf 64 eingestellt.
Sind die fünf Prozesse beschäftigt, werden weitere gestartet.
Unbeschäftigt vorgehalten benötigt ein Apache nur einige hundert kB RAM, voll beschäftigt hingegen 150 bis 500 MB, davon rund 200 speicherresident.
Ein Apache, der 1000 Anfragen bedient hat, beendet sich automatisch.

In vielen Fällen ist eine Änderung der Standardeinstellung nicht nötig, weil oft auch bei vielen gleichzeitigen Sessions ein Apache (oder weniger) pro Sitzung bereits genügt, damit alle Sitzungen ohne Wartezeit bedient werden.
Zudem gilt: Bei einer Instanz, auf die nur wenige Nutzer zugreifen, wird nur ein Bruchteil der 64 Apaches gestartet.
Es gibt jedoch Fälle, in denen das Tuning Sinn macht:

* Auf einer Zentralinstanz im xref:distributed_monitoring#[verteilten Monitoring], auf der relative wenige Monitoring-Daten verarbeitet werden, aber viele Nutzer Dashboards geöffnet haben, kann es mitunter erforderlich sein, die Zahl zu erhöhen.
...
Um einen Eindruck zu bekommen, wie viele Apaches tatsächlich benutzt werden, werfen Sie einen Blick in die Metriken des Dienstes [.guihint]#Process mysite Apache#.
