...
Wenn aktiviert, erreichen Sie die Jaeger-Web-Oberfläche über `http://mycmkserver/mysite/jaeger`.
...
Eine ausführliche, technische Erklärung der Funktionsweise zeigt Ihnen der Vortrag link:https://www.youtube.com/watch?v=WQ-3rFQub6U[Piggyback unleashed - a new way for intersite communication^] von der letzten {CMK}-Konferenz.

////
=== ui-job-scheduler
Der `ui-job-scheduler`-Dienst organisiert asynchrone Aufgaben, die in der Web-Oberfläche anfallen.
