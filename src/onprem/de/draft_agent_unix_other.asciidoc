...
:description: Um auch Systeme zu überwachen, für die kein offizielles Agentenpaket existiert, können Sie das Agentenskript auf vielfältigen Wegen starten.
...
Sie können die aktuellste Version des Agentenskriptes im {CMK}-Setup oder direkt in unserem link:https://github.com/Checkmk/checkmk/tree/2.2.0/agents[GitHub-Repository^] herunterladen.
...
[#restrict_access_agent_execution]
...
Nach einem Speichern und einem xref:glossar#activate_changes[Aktivieren der ausstehenden Änderungen] ist der Host ins Monitoring aufgenommen.
