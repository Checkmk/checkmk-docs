...
Diese Schnellstart-Anleitung führt Sie vom Download der virtuellen Appliance über eine Minimalkonfiguration bis hin zum Anmeldebildschirm einer laufenden {CMK}-Instanz.
...
. Laden Sie die {CMK} virt1 von unserer link:https://checkmk.com/de/download[Download-Seite^] herunter.
Für VMware ESXi und Proxmox benötigen Sie die OVA-Datei (Open Virtualization Appliance) und für Hyper-V die Zip-Datei.
Importieren Sie die Datei anschließend in Ihren Hypervisor.
xref:appliance_install_virt1#[Mehr ...]
.. Für VMware ESXi: Für bessere Performance sollten Sie die Netzwerkkarte auf VMXNET 3 ändern. xref:appliance_install_virt1#vmxnet[Mehr ...]
.. Allgemein: Die virtuelle Festplatte, auf der letzlich {CMK} installiert wird und anschließend alle Instanzen und deren Daten gespeichert werden, ist auf eine Größe von 100 GB voreingestellt.
Überlegen Sie bitte genau, ob diese Größe für Sie ausreichend ist und vergrößern Sie die virtuelle Festplatte bei entsprechendem Bedarf unbedingt *bevor* Sie die virt1 zum ersten Mal starten. xref:appliance_install_virt1#[Mehr ...]
