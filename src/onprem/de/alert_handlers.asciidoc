...
{cee-only}
Alert Handlers sind von Ihnen selbst geschriebene Skripte, die {CMK} in den kommerziellen Editionen für Sie ausführt, wenn ein Problem festgestellt wird -- oder genauer ausgedrückt -- wenn ein Host oder Service seinen Zustand ändert.
...
[#activate_alert_handler]
=== Alert Handler aktivieren

Die Aktivierung des Handlers erfolgt über [.guihint]#Setup > Events > Alert handlers#.
...
* Alert Handlers sind erst wirklich nützlich, wenn Sie auf dem Ziel-Host einen Befehl ausführen.
...
[TIP]
====
Anders als an anderen Stellen von {CMK}, wird bei den Alert Handlers *jede* Regel ausgeführt, deren Bedingung erfüllt ist.
...
* Regeln in [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Linux/UNIX agent options > Remote alert handlers (Linux)#, welche SSH-Schlüssel generieren.
...
Legen Sie eine Regel unter [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Linux/UNIX agent options > Remote alert handlers (Linux)# an.
Zur Erstellung dieser Regel benötigen Sie die xref:wato_user#roles[Berechtigungen] [.guihint]#Rule sets# und [.guihint]#Add or modify executables#.
...
pass:[(Reading database ... 515080 files and directories currently installed.)]
