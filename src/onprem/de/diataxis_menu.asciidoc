...
= Diátaxis Dummy TOC
:title: Diataxis-Dummy-TOC
:description:
 


[#getting_started]
== Erste Schritte mit {CMK}
...
// intro_extend wird evtl. abgeschafft/woanders hin verlagert?

=== Praktischer Einstieg
- link:intro_setup.html[{CMK} aufsetzen]
- link:intro_setup_monitor.html[Das Monitoring einrichten]
// intro_setup_monitor: separieren in mehrere Artikel? (Anfang: Explanation, zweiter Teil: How-to)
...
[#how-to]
== How-to guides
// ES: Wie man sieht, ist eine sinnvolle Reihenfolge und/oder Kategorisierung der Einträge innerhalb jedes Diataxis-Typs noch ausstehend. Wir sammeln erstmal für jeden Typ diejenigen Artikel, die wir für eindeutig halten. Wenn die Typen immer mehr gefüllt sind und der Abschnitt TBZ immer kürzer wird, kann es sich lohnen, die Reihenfolge der Einträge pro Typ zu besprechen.

- link:monitoring_podman.html[Podman überwachen]
...
[#toc_rules]
=== Regeln
- link:wato_rules.html[Regeln in {CMK} anwenden]
...
=== TBW (to be written)
// ES 2026-09-18: Wenn wir beim Sortieren vorhandener Artikel eine bedeutende Lücke in der Abdeckung von Themen und User Needs entdecken, notieren wir in diesem Abschnitt, welche neuen Artikel wir uns wünschen. Jede Diataxis-Kategorie enthält diesen Abschnitt, er kann aber auch leer bleiben.

==== Hosts ins Monitoring aufnehmen
// ES: Im 4. Diataxis-Treffen haben wir beschlossen, dass wir How-to-Guides für diese zentralen Aufgaben erstellen wollen. Sie sollen dann hier im ToC erscheinen. Ob es bei den folgenden vier Einträgen bleibt oder welche verschwinden oder dazukommen, schauen wir dann.
- Hosts mit Agenten
- Hosts mit Spezialagenten
- SNMP-Hosts
- Relay
...
[#explanation]
== Explanation
- link:wato_rules_about.html[Die Funktionsweise von Regeln in {CMK}]
...
[#tbd]
== TBZ (to be zerpflücked)
// Artikel in dieser Kategorie beinhalten im aktuellen Zustand mehrere Diataxis-Typen. Sie müssen stark gekürzt oder in mehrere Artikel aufgeteilt werden, um sauber zu einem oder mehreren Diataxis-Typen zugeordnet zu werden.

- link:wato_monitoringagents.html[Monitoring-Agenten]
// zu wato_monitoringagents: Abschnitt 1 (#intro) und 2 (#agents) sind klar Typ "Explanation", danach kommen praktische Anleitungen

[#toc_data]
==== Datenhaltung
- link:metric_backend.html[Das {CMK} Metrik-Backend]

[#tbr]
== TBR (to be repaired)
// Artikel in dieser Kategorie sind im aktuellen Zustand nicht hilfreich oder nützlich für Lesende. Sie müssen möglicherweise komplett aufgelöst werden und/oder teilweise in andere Artikel eingegliedert werden und/oder großflächig neu geschrieben werden und/oder um viel Signposting und Kontext erweitert werden, um einen Mehrwert für die Lesenden zu bieten.

==== Appliance, Container, Cloud

// 2026-09-09 (sebkir, ES): der Artikel install_appliance bedarf grundsätzlicher Verbesserung/Löschung; kein Link zur Anleitung zum Installieren der Appliance selbst; keine sinnvolle Zielsetzung, fehlender Kontext
- link:install_appliance_cmk.html[Installation von {CMK} in der Appliance]

[#tbe]
== TBE (to be einsortiert)
// ES: Inhalte in dieser Kategorie sind noch unsortiert. Bei unseren Diataxis-Meetings sichten wir die hier gelisteten Artikel und verschieben sie in je einen der Diataxis-Typen oder in die Kategorie "Erste Schritte mit Checkmk" oder in eine der Kategorien TBR und TBZ.

[#toc_welcome]
=== Willkommen bei {CMK}
// TBE-Themen von Thomas - START
...
- link:update_major.html[Update auf Version {current-major}]
- link:update_matrix.html[Update-Matrix für Version {current-major}]
...
- link:cmk_processes.html[Instanzdienste - {CMK} unter der Haube]
...
- link:product_usage_analytics.html[Analyse der Produktnutzung]
...
// TBE-Themen von Thomas - ENDE
// TBE-Themen von Esther - START
...
- link:robotmk_mm.html[{CMK} Synthetic Monitoring mit Robotmk]
...
// TBE-Themen von Sebastian - ENDE

[#toc_prognosis]
==== Prognosen
// TBE-Themen von Mattias - START
...
- link:devel_inventory_plugins.html[HW/SW-Inventur-Plugins entwickeln]
