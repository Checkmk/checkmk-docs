...
Da die kommerziellen Editionen jedoch einige exklusive visuelle Elemente (etwa Widgets) enthalten, könnten Sie über eine Meldung folgender Art stolpern: +
`Widget type 'state_host' could not be found. Please remove it from your dashboard configuration.` +
Hier fehlt also ein einzelnes Widget, das übrige Dashboard ist aber weiterhin nutzbar.
