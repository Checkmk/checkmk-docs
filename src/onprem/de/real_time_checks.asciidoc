...
Mit dieser Regel spezifizieren Sie, welche Services von welchen Hosts mit Echtzeit-Checks überwacht werden sollen.
...
Geben Sie unter [.guihint]#Step (precision)# den Wert 1 ein, damit die RRD-Konfiguration mit Ein-Sekunden-Intervallen arbeiten kann.
...
Führen Sie dazu auf dem {CMK}-Server als Instanzbenutzer den Befehl `cmk-convert-rrds` aus.
...
Allerdings müssen Sie Ihre gewünschten Einstellungen auf dem Host hinterlegen, damit der Agent wirklich sekündlich die benötigten UDP-Pakete sendet.
...
Falls Sie aber für einen echtzeitüberwachten Service ein Alter von über 91 Sekunden entdecken, liegt noch ein xref:#debugging[Fehler] in der Einrichtung vor, den es zu beheben gilt.
====
...
Setzen Sie dafür unter [.guihint]#Setup > Global settings > Monitoring core > Logging of the core# das Logging-Level für [.guihint]#Checkmk check helpers# auf den Wert [.guihint]#Informational# und speichern und aktivieren Sie die Änderungen.
