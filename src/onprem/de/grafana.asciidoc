...
Hier sehen Sie noch einmal die detaillierte Beschreibung der {CMK}-Datenquelle und können diese mit Klick auf den Button mit Beschriftung _Install_ in Ihrer Grafana-Instanz verfügbar machen.
Die Buttons ändern sich nun und Sie haben die Möglichkeit, mit Klick auf den Button _Add new data source_ eine xref:activate[Verbindung zu Ihrer {CMK}-Instanz einzurichten].
