...
Einzige Einschränkung hierbei: Benutzer des Grafana Cloud Angebots müssen {CCE} verwenden und das Plugin xref:installation_marketplace[über die offizielle Grafana Plugin-Sammlung] aktivieren.
...
* Die xref:installation_cli[oben] erwähnte signierte Variante `checkmk-cloud-datasource-X.Y.Z.zip`, die nur mit {CE} genutzt werden kann.
