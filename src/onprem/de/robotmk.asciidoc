// -*- coding: utf-8 -*-
// IGNORE MyApp1 MyApplication_mybot Windows|Linux mybot_managed rf namen Entwicklungs
// NONASCII
include::global_attr.adoc[]
= {CMK} Synthetic Monitoring mit Robotmk (RCC)
:title: {CMK} Synthetic Monitoring mit Robotmk (RCC)
...
[IMPORTANT]
====
Synthetic Monitoring benötigt virtuelle Umgebungen auf den Tests ausführenden Hosts.
Standardmäßig nutzt {CMK} dafür RCC (ursprünglich entwickelt von Robocorp) -- wie hier im Artikel "{CMK} Synthetic Monitoring mit Robotmk (RCC)" beschrieben.
Sollte RCC bei Ihnen nicht funktionieren, können Sie alternativ unser hauseigenes Tooling CSM/Micromamba verwenden -- wie im Artikel xref:robotmk_mm#[{CMK} Synthetic Monitoring mit Robotmk (CSM)] beschrieben.
====

[#intro]
== Synthetisches Monitoring mit Robot Framework

{cee-only} {CMK} Synthetic Monitoring ist in den kommerziellen xref:glossar#edition[Editionen] verfügbar, benötigt jedoch eine link:https://checkmk.com/request-quote/synthetic-monitoring[zusätzliche Subskription^].
...
// ML: kurz vor Konferenz eingefügt
[IMPORTANT]
====
Die Entwicklung des Open-Source-Tools RCC wurde vor einiger Zeit von Entwickler Robocorp eingestellt.
Dennoch bleibt RCC bis auf weiteres der empfohlene Weg zur Erstellung von Umgebungen für Robot Framework.
Für Fälle, in denen RCC in Ihrer Umgebung nicht funktionieren sollte, steht als Fallback eine von Checkmk entwickelte Lösung namens „CSM“ auf Basis von Micromamba zur Verfügung; dazu gibt es eine eigenständige xref:robotmk_mm#[Version dieses Synthetic-Monitoring-Artikels].
====
...
Zunächst die [.guihint]#RCC configuration#.
Diese erlaubt im Bereich [.guihint]#RCC profile configuration# Angaben zu Proxy- und TLS-Themen.

Es folgt die Pfadangabe zu [.guihint]#Base directory for ROBOCORP_HOME directories#  -- weitere Informationen zu ROBOCORP_HOME finden Sie im xref:#rccbackground[RCC-Exkurs.]

Eine letzte RCC-Option: [.guihint]#Allow agent deployment as root#.
Nur mit dieser Option ist die Kombination aus RCC und einem root-Nutzer-Agenten möglich!

.RCC-Konfiguration, gültig für alle Sequenzen
image::robotmk_scheduler_08rcc.png[alt="Die Kombination root-Agent plus RCC-Konfiguration muss explizit erlaubt werden!"]


[IMPORTANT]
====
Standardmäßig werden Agenten unter Linux unter dem root-Konto installiert.
Wenn Synthetic Monitoring aktiviert ist, ist dies aus Sicherheitsgründen unterbunden -- das Backen des Agenten produziert eine Fehlermeldung (siehe link:https://checkmk.com/werk/19460[Werk 19460^]).
Entsprechend sollten Sie über die Regel [.guihint]#Customize agent package (Linux)# einen anderen Nutzerkontext setzen.
Falls Sie RCC unbedingt unter dem root-Konto laufen lassen müssen, können Sie die Option [.guihint]#Allow agent deployment as root#  in der Robotmk-Scheduler-Regel nutzen. 
====


Zu guter letzt: [.guihint]#Grace period before scheduler starts#.
Der Scheduler startet zusammen mit dem {CMK}-Agenten noch vor der Desktop-Anmeldung -- was freilich dazu führt, dass etwaige Tests auf dem Desktop fehlschlagen müssen.
...
Unter Linux läuft der Scheduler unter dem gleichen Nutzer wie der Agent.
...
Die Pfade in folgenden Beispielen beschränken sich auf Windows -- lediglich wenn Linux andere Befehle erfordert, geben wir diese explizit an.
