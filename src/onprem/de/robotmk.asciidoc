...
== KPI-Monitoring
Oben haben Sie bereits gesehen, dass sich die Laufzeiten von High-Level-Keywords als Teil der Suite mit überwachen lassen.
...
Für die _Muster-basierte Erkennung_ werden die gewünschten Keywords als reguläre Ausdrücke per Regel hinterlegt.
...
Achtung: Wenn der reguläre Ausdruck auf mehrere Keywords desselben Tests matcht, geht der Service-Status auf {UNKNOWN}.
...
. Robotmk-Bibliothek importieren
. Marker in vor relevante Keywords setzen
...
Das Robotmk-Keyword `Monitor Subsequent Keyword Runtime` ist der Marker, um das folgende Keyword, genauer gesagt dessen Laufzeit, zu überwachen.
