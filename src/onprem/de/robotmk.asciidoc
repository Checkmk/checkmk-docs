...
// MFS: Vier Mal "gibt" in aufeinander folgenden Sätzen. Bitte durch Synonyme ersetzen.
...
// MFS: Modifikation 03/2025, prepare for translation
...
// MFS: Vorschlag: für den Einsatz in {CMK} ist aber die Deklaration nach Robot Framework erforderlich
====
...
// MFS: Von json auf file geändert
...
----
// MFS: in beiden Fällen halte ich erzwungenes Umbrechen (Backslash-Newline unter Linux) für sinnvoller, auch wenn es dann keine Oneliner mehr sind, sondern kurze Script.
// MFS: Man kann unter Windows ab NT 4 Forward Slashes nutzen, unter Linux aber keine Backslashes.
...
// MFS: Ist die Abkürzung KPI so sehr in der RF-Terminologie verankert, dass wir sie nicht ausschreiben müssen?
[#kpi]
== KPI-Monitoring
...
// MFS: Sieht etwas komisch aus. Genügt nicht: == Muster für die Erkennung
...
// MFS: Bei Achtung u.ä. lieber Admonition oder wenigstens bold.
...
// MFS: Sieht etwas komisch aus. Genügt nicht: == Marker für die Erkennung

Für die Erkennung via Marker müssen Sie folgende Schritte durchführen:
// MFS: Bitte ganze Sätze, sonst stolpert DeepL bei der Übersetzung
...
// MFS: Das in der Klammer ist ein Nebensatz. Vielleicht besser so:
// KPIs werden sich dabei häufig auf individuelle Nutzer-Keywords beziehen, daher erweitern wir das Beispiel hir um das Keyword `Foobar`.
// Dieses ruft lediglich das Standard-Keyword `Log` auf und wird wiederum vom Testfall `My Test` aufgerufen.
...
// MFS: Den eben gelesenen Satz verstehe ich überhaupt nicht.
...
// MFS: Nach neuer Rechtschreibung ohne Bindestrich
...
// MFS: Ein wenig sloppy, vielleicht "keinen oder sehr eingeschränkten Internetzugriff haben"
...
// MFS: Hier eher "erklären" oder "aufzuklären" als "klären".

[#rccbackground]
=== RCC-Exkurs

// MFS: Evtl. im folgenden Abschnitt das Wort "Fork" fallen lassen?
==== RCC-Historie
...
// MFS: Siehe oben "ZIP-Archive zur Verteilung" oder so...
