...
Allerdings werden beide exakt gleich verwendet, weshalb hier auch die `tests.robot` im Bereich `tasks` referenziert wird.
====

Im Bereich `environmentConfigs` wird neben der `conda.yaml` die zum System passende Freeze-Datei bestimmt.
Die Freeze-Dateien halten fest, welche Abhängigkeiten in welchen Versionen installiert werden müssen, um exakt identische Umgebungen zu erhalten.
Wenn Sie diese nutzen wollen, verschieben Sie sie einfach aus dem Ordner `output` eine Ebene nach oben (wo auch die `conda.yaml` liegt).
RCC bestimmt die korrekte Datei automatisch.
