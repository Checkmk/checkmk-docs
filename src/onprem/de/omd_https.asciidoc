...
Die Standard-Sicherheitseinstellungen von allen modernen Browsern sperren bei vielen Kombinationen aus HTTP und HTTPS die unverschlüsselte Komponente oder erzwingen ein Upgrade der Verbindung zu HTTPS.
...
Da auf einem reinen {CMK}-Server wird in der Regel nur ein virtueller Host mit einem Servernamen verwendet wird, nehmen Sie die Einstellungen nur im _Standard-Vhost_ respektive in der globalen Apache-Konfiguration vor.
...
Sie können entweder einen virtuellen Host anlegen, indem Sie die Konfigurationsdatei `/etc/apache2/vhosts.d/vhost.template` nach `/etc/apache2/vhosts.d/vhost.template` nach 
`/etc/apache2/vhosts.d/vhost-default.conf` kopieren und diese Datei anpassen.
