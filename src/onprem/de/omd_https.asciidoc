...
+
--
SLES und OpenSuse liefern eine Vorlage einer SSL-Konfiguration mit.
...
Sie können entweder einen virtuellen Host anlegen, indem Sie die Konfigurationsdatei `/etc/apache2/vhosts.d/vhost.template` nach `/etc/apache2/vhosts.d/vhost-default.conf` kopieren und diese Datei anpassen.
