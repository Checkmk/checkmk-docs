...
Hauptgrund für die Absicherung der Weboberfläche von {CMK} ist meistens der Sicherheitsaspekt:
...
Weder sollen diese übertragenen Daten von Dritten eingesehen, noch manipuliert werden können.
...
[TIP]
====
Auch beim Einsatz innerhalb vertrauenswürdiger Umgebungen trägt die Verwendung von HTTPS zur reibungslosen {CMK}-Nutzung bei.
So sperren die Standard-Sicherheitseinstellungen von allen modernen Browsern bei vielen Kombinationen aus HTTP und HTTPS die unverschlüsselte Komponente oder erzwingen ein Upgrade der Verbindung zu HTTPS:
...
Desweiteren sperren die meisten Browser den Zugriff auf die Zwischenablage per Javascript, was Copy-Buttons unbrauchbar macht.
====
