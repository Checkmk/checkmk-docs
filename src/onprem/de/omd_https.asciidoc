...
[TIP]
====
Wenn Sie den vollautomatischen Modus von Let's Encrypt verwendet haben, dürfen Sie diesen Abschnitt überspringen.
====
...
+
--
Bei RHEL, sowie darauf basierenden Distributionen ist `mod_ssl` ein eigenes Paket, das Sie separat installieren müssen:
...
Zunächst müssen Sie in der SSL-Konfigurationsdatei die korrekten Pfade zu Schlüssel, Zertifikat und Intermediate-Zertifikat angeben.
...
+
--
SuSE liefert eine Vorlage einer SSL-Konfiguration mit.
