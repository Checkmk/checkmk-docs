...
{CMK} stellt einen Agenten für FreeBSD bereit, der auf dem Agenten für Linux basiert, aber für FreeBSD-Feinheiten angepasst wurde, wie z.B. unterschiedliche Ausgabeformate der Befehlszeilen-Tools und abweichende Pfad-Konventionen.
...
[TIP]
====
Hinweis zu anderen BSD-Systemen:
Dieser Artikel erklärt Installation und Einrichtung des Agenten für _FreeBSD_,
...
Eine besondere Abhängigkeit ist `bash`, denn tatsächlich verwendet das Agentenskript eine Syntax, die weder mit der link:https://man.freebsd.org/cgi/man.cgi?query=sh&sektion=1&manpath=freebsd-release-ports[Almquist Shell^] noch mit link:https://man.freebsd.org/cgi/man.cgi?query=tcsh&sektion=1&manpath=freebsd-release-ports[`tcsh`^] kompatibel ist.
Das `ipmitool` wird zum Auslesen von Hardwareinformationen benötigt.

Installieren Sie all diese Abhängigkeiten mit dem folgenden Befehl:
...
Die link:https://ports.freebsd.org/[FreeBSD Ports^] enthalten einen von Ian Valentine gepflegten link:https://cgit.freebsd.org/ports/tree/net-mgmt/check_mk_agent[Port des {CMK}-Agenten^], der gegebenenfalls Patches enthält, um den Agenten mit neueren FreeBSD-STABLE Versionen kompatibel zu machen. 
...
Auch für den Port `check_mk_agent` existiert ein fertiges Paket, das Sie mit `pkg` installieren können:
...
Meist ist die aus den Ports selbst gebaute Version aktueller:
...
[{shell-raw}]
----
root@bsd:/root # wget -O /usr/local/bin/check_mk_agent https://raw.githubusercontent.com/checkmk/checkmk/{current-major}/agents/check_mk_agent.freebsd
...
=== Test auf der Befehlszeile

Rufen Sie nun den Agenten auf der Befehlszeile auf und überfliegen Sie die Ausgabe im Pager:
...
Kommentieren Sie zunächst die Zeile aus, die Zugriffe von überall erlaubt, fügen Sie dann eine Zeile ein, die nur dem {CMK}-Server – hier mit der IP-Adresse `198.51.100.123` – den Zugriff erlaubt.
Der erste Parameter ist der Name des vom `inetd` ausgeführten Befehls:
...
Führen Sie folgenden Befehl als Instanzbenutzer auf dem {CMK}-Server aus, um die Netzwerkverbindung zum Agenten zu testen (im Beispiel zum Host mit der IP-Adresse `198.51.100.231`).
Die Befehlsausgabe zeigt nur die ersten Zeilen:

[{shell-raw}]
----
{c-omd} nc "198.51.100.231" 6556
<<<check_mk>>>
Version: {current}
AgentOS: freebsd
----
...
Bei einem „nackten“ FreeBSD, auf dem noch keine Server-Dienste konfiguriert sind, sollten mindestens 11 Dienste erkannt werden, bei Installation auf ZFS zehn zusätzliche für Dateisysteminformationen:

.Die Service-Erkennung in {CMK} findet mindestens 11 Services
