...
Das bedeutet, dass Sie auch eine schon länger bestehende Instanz mit nur einer einzigen Anpassung - für die korrekte Verarbeitung von Piggyback-Daten - zu einem verteilten Monitoring ausbauen können.
Sie müssen lediglich unter [.guihint]#Setup > General > Global settings# im Kasten [.guihint]#Site management# den Piggyback-Hub aktivieren:
...
* Die {CMK}-Version (z.B. {v24}) der Remote- und Zentralinstanz ist dieselbe – ein xref:mixed_versions[Mischbetrieb] wird nur für die leichtere Durchführung von Updates unterstützt.
...
Created new site myremote1 with version 2.4.0p1.cee.
...
Die wichtigsten Schritte sind jetzt, dass Sie Livestatus via TCP auf dem Netzwerk
...
Sie benötigen es nur zweimal: für die Aktivierung des Piggyback-Hub und um die Verbindung der Zentral- zur Remote-Instanz herzustellen.
...
[TIP]
====
Denken Sie an das [.guihint]#Activate changes# bevor Sie weiterarbeiten.
====
...
Prüfen Sie dafür unbedingt, welchen [.guihint]#Message broker port# Sie angeben müssen. Sie finden auch diesen auf der jeweiligen Remote-Instanz - im Abschnitt [.guihint]#Basic# über die Kommandozeile mit `omd config`:

image::distributed_monitoring_rabbitmq.png[alt="Anzeige des Message Broker Ports in omd config.", width=80%]

[TIP]
====
Lassen Sie sich nicht verwirren, der [.guihint]#Message Broker Port# heißt hier [.guihint]#RABBITMQ_PORT#, da derzeit nur die RabbitMQ Message Broker Software in {CMK} eingebunden ist.
====
...
2025-04-30 15:58:30,627 [20] [cmk.liveproxyd] Livestatus Proxy-Daemon (2.4.0p1) starting...
...
2025-04-30 07:11:40,023 [20] [cmk.mknotifyd] Check_MK Notification Spooler version 2.4.0p1 starting
...
In seltenen Fällen sind aber auch diese untereinander inkompatibel, so dass Sie den exakten Versionsstand (z.B. {v23}p11) auf allen Seiten einhalten müssen, damit eine fehlerfreie Zusammenarbeit der Instanzen möglich ist.
...
Für die reibungslose Durchführung von xref:update_major#[Major-Updates] (beispielsweise von {oldstable} auf {current_major}) in großen verteilten Umgebungen erlaubt {CMK} seit {v20} einen Mischbetrieb, bei dem die Differenz genau eine Major-Version sein darf.
