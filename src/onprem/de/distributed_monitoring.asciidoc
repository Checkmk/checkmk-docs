...
// MFS: Evtl. wird so die Intention deutlicher?
// Im verteilten Monitoring werden der der Piggyback-Host und die davon abhängigen Hosts nicht immer von der gleichen Instanz aus überwacht werden.
// Eine instanzübergreifende xref:piggyback#distributed_piggyback[Host-Zuordnung der Piggyback-Daten] kann für diese Fälle aktiviert werden.
Im verteilten Monitoring müssen der Piggyback-Host und die davon abhängigen Hosts nicht von der gleichen Instanz aus überwacht werden.
Eine instanzübergreifende xref:piggyback#distributed_piggyback[Host-Zuordnung der Piggyback-Daten] ist möglich.

// MFS: Auf 5671 korrigiert, der 5672 war von DEV falsch kommuniziert worden.
...
// MFS: Interne Ports erwähnen wir i.d.R. nur im Ports-Artikel, Firewall-Config erlaubt meist localhost ohne Einschränkungen. Dieser Satz wäre daher irreführend.
// Die restlichen benötigten Ports werden automatisch ausgewählt.


===== Peer-to-Peer-Verbindungen

// MFS: Einschub Motivation Traffic bspw. innerhalb eines lokalen Netzes zu belassen.
Um Ihre Zentralinstanz zu entlasten oder Netzwerkverkehr zu optimieren, können Sie Remote-Instanzen auch direkt miteinander kommunizieren lassen (Peer-to-Peer-Verbindung).
Als Voraussetzung dafür müssen beide Remote-Instanzen von der Zentralinstanz mitkonfiguriert werden.
D.h. [.guihint]#Push configuration to this site# muss, wie im Abschnitt 
xref:connect_remote_sites[Remote-Instanzen in die Zentralinstanz einbinden] beschrieben, bei beiden aktiviert sein.
