...
Hier ist ein Beispiel für das Anlegen einer Remote-Instanz mit dem Namen `myremote1`:

[{shell}]
----
{c-root} omd create myremote1
...
Hierfür begeben Sie sich auf der Remote-Instanz auf die Kommandozeile und rufen dort `omd config` auf.
...
In der folgenden Eingabemaske verwenden Sie als [.guihint]#Login credentials# `cmkadmin` und das generierte Passwort der Remote-Instanz.
...
Damit Sie also von der Zentralinstanz weiterhin auf die Remote-Instanz zugreifen können, muss gewährleistet sein, dass im Menü unter [.guihint]#Setup > General > Distributed Monitoring# bei der jeweiligen Instanz unter [.guihint]#Encryption# die Option [.guihint]#Encrypt data using TLS# ausgewählt ist. Prüfen Sie dies und stellen Sie die Option gegebenenfalls, wie im folgenden Screenshot zu sehen ist, um:
...
Um die Kommunikation zu ermöglichen, müssen Sie lediglich sicherstellen, dass xref:ports#distributed_monitoring[der Message Broker Port auf allen beteiligten Instanzen korrekt eingestellt] und xref:create_remote[der Piggyback-Hub aktiviert] ist.
...
eventuell nicht sichtbar sind, ist eine xref:glossar#notification[Benachrichtigung] durch die Zentrale
...
endet, z.B. `~/etc/nagios/conf.d/config-myremote1.cfg`. Wenn ein SSH-Zugang
...
// TK: Hola: Selbst umgeschrieben? Respekt!

Auf link:https://github.com/Checkmk/checkmk/tree/master/doc/treasures/[GitHub^] finden Sie im `treasures`-Verzeichnis von {CMK} das Skript `/livedump/livedump-ssh-recv`, das genau das tut. 
// TK: `livedump/livedump-ssh-recv`, d.h. ohne Slash, weil relativ zum treasures-Verzeichnis.
// TK: Oder Du linkst direkt nach https://github.com/Checkmk/checkmk/tree/master/doc/treasures/livedump, wie Du es auch analog in devel_check_plugins getan hast.
