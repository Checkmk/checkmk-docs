...
Dabei gibt es verschiedene Gründe für das Aufteilen in mehrere xref:glossar#site[Instanzen]:
...
Das von {CMK} benutzte Verfahren zum Aufbau eines verteilten Monitorings basiert auf xref:livestatus[Livestatus] und einer automatischen Konfigurationsverteilung.
Wenn Anforderungen stark abgeschottete Netze oder sogar die _strikt unidirektionale Datenübertragung_ von der Peripherie in die Zentrale vorschreiben, gibt es die Möglichkeit der Verwendung von xref:livedump[Livedump bzw. CMCDump].
...
Anschließend xref:glossar#activate_changes[aktivieren Sie die ausstehenden Änderungen], da ein Ein- oder Ausschalten des Piggyback-Hubs einen Neustart der Instanz erfordert.
...
Die wichtigsten Schritte sind jetzt, dass Sie Livestatus via TCP im Netzwerk freigeben und auch hier den Piggyback-Hub aktivieren.
Beachten Sie dabei, dass Livestatus per se kein abgesichertes Protokoll ist und nur in einem sicheren Netzwerk (abgesichertes LAN, VPN etc.) verwendet werden darf.
Das Freigeben geschieht als Instanzbenutzer bei noch gestoppter Instanz per `omd config`:
...
Setzen Sie `LIVESTATUS_TCP` auf `on` und tragen Sie für `LIVESTATUS_TCP_PORT` eine freie Portnummer ein, die auf diesem Server eindeutig ist.
Der Default dafür ist 6557:
...
Eine Kontrolle mit `netstat` zeigt, dass Port 6557 geöffnet ist.
Die Bindung an diesen Port geschieht mit einer Instanz des Internet Superservers `xinetd`, welcher direkt in der Instanz läuft:
...
Aktivieren Sie anschließend die ausstehenden Änderungen, da ein Ein- oder Ausschalten des Piggyback-Hubs einen Neustart der Instanz erfordert.
...
Tragen Sie hier die HTTP-URL zur Weboberfläche der Remote-Instanz ein, und zwar ohne das `check_mk/` am Ende.
...
Das Konto `cmkadmin` auf der Instanz wird dann nicht mehr verwendet.

Klicken Sie auf das Schlüsselsymbol der anzubindenden Instanz.
In der folgenden Eingabemaske verwenden Sie als [.guihint]#Login credentials# `cmkadmin` und das beim Anlegen der Remote-Instanz vergebene Passwort.
...
==== Service-Erkennung für umgezogene Hosts neu durchführen und ausstehende Änderungen aktivieren
...
Um die Kommunikation zu ermöglichen, müssen Sie lediglich sicherstellen, dass der Message Broker Port auf allen beteiligten Instanzen xref:ports#distributed_monitoring[korrekt eingestellt] und xref:setting_up_central_site[der Piggyback-Hub aktiviert] ist.
...
Dazu kann Livedump/CMCDump einen Dump der Konfiguration der Remote-Instanzen erstellen, der bei der Zentralinstanz eingespielt wird.

Während des Monitorings wird nun auf jeder Remote-Instanz einmal pro definiertem Intervall (z.B. jede Minute) ein Dump des aktuellen Status in eine Datei geschrieben.
...
Als Erstes erzeugen Sie auf der Remote-Instanz einen Dump der Konfiguration Ihrer Hosts und Services im Nagios-Konfigurationsformat.
...
Loggen Sie sich jetzt auf der Zentralinstanz ein und aktivieren Sie die ausstehenden Änderungen:
...
* Das Erstellen des Dumps und das Übertragen der Konfiguration müssen Sie nach jeder Änderung von Hosts oder Services auf der Remote-Instanz wiederholen.
...
Der Dump der Konfiguration geschieht mit `cmcdump -C`.
...
Schritt 5: Aktivieren Sie die ausstehenden Änderungen.
...
[#mixed_versions]
== Updates im verteilten Monitoring
...
[#temp_diff_patch]
=== Vorübergehend abweichende Patchlevel (gleiche Major-Version)
...
[#temp_diff_major]
=== Vorübergehend abweichende Major-Versionen
...
[#mixed_editions]
== Editionsunterschiede zwischen Zentralinstanz und Remote-Instanzen

Seit Version {v25} von {CMK} kann eine Zentralinstanz mit Remote-Instanzen anderer Editionen verbunden werden, solange die Remote-Instanzen mit einer Edition mit geringerem Funktionsumfang betrieben werden.
Bis {CMK} {v24} war ein Mischbetrieb mit unterschiedlichen Editionen nur möglich, wenn die zentrale Instanz mit {CSE} betrieben wurde und die Remote-Instanzen mit {CRE}.

Für Remote-Instanzen mit geringerem Funktionsumfang ist es nicht möglich, die Konfiguration von der zentralen Instanz aus zu replizieren.
Die Einstellung [.guihint]#Enable replication# muss also zwingend auf [.guihint]#No replication with this site# gesetzt werden.
In der Praxis bedeutet das, dass ein solches Szenario ausschließlich rein lesenden Zugriff erlaubt.

Für die xref:license#distributed[Lizenzverifizierung und die Übermittlung der Lizenznutzung] wird die Edition der zentralen Instanz berücksichtigt.

Beachten Sie auch die Hinweise zum Umgang mit xref:upgrade#editiondistributed[Upgrades] im verteilten Monitoring.
