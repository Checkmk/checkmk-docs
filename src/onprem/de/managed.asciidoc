...
Im Folgenden möchten wir Ihnen mit {CME} eine der kommerziellen xref:intro_setup#editions[Editionen] vorstellen.
...
In einem verteilten Setup wird {CMK} in {RE}, {SE} und {CE} daher alle Konfigurationsdaten an alle beteiligten Instanzen verteilen, 
...
Mit {CME} bindet ein Dienstleister (das heißt ein Managed Service Provider, kurz MSP oder Provider) in seiner Zentralinstanz für jeden Kunden über das xref:distributed_monitoring#[verteilte Monitoring] eine oder mehrere Remote-Instanzen ein, die nur diesem Kunden gehören.
...
Inhaltlich baut sie auf xref:cce#[{CCE}] auf und ist deren mandantenfähige Erweiterung.
