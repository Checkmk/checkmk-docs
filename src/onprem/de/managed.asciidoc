...
= {ME}
:title: {ME} - {CMK} als Dienstleistung
:description: Lesen Sie hier, wie Sie mit {ME} voneinander abgeschottete Instanzen für mehrere Kunden betreiben und verwalten.       
...
Im Folgenden möchten wir Ihnen mit {CME} (vormals {CMK} MSP) die Erweiterung der kommerziellen xref:glossar#edition[Edition] {CUE} vorstellen, mit der Sie als ein _Managed Service Provider (MSP)_ über das xref:glossar#distributed_monitoring[verteilte Monitoring] von {CMK} voneinander abgeschottete Instanzen für mehrere Kunden betreiben können.
...
Benutzer mit sehr eingeschränkten Berechtigungen werden in aller Regel keinen direkten Befehlszeilenzugriff auf den Monitoring-Server bekommen 
...
In einem zentralen Setup wird {CMK} in {RE}, {SE} und {CE} _ohne_ Multi-Tenancy daher alle Konfigurationsdaten an alle beteiligten Instanzen verteilen, 
...
Schließlich kann es sein, dass der lokale Monitoring-Server von dem Kunden selbst betrieben wird oder er anderweitig über direkten Zugang zu der Befehlszeile des Servers verfügt.
...
Mit {ME} bindet ein Dienstleister (das heißt ein Managed Service Provider, kurz MSP oder Provider) in seiner Zentralinstanz für jeden Kunden über das verteilte Monitoring eine oder mehrere Remote-Instanzen ein, die nur diesem Kunden gehören.
...
Das funktioniert genauso wie in einer normalen verteilten Umgebung auch, mit dem einzigen Unterschied, dass *alle* beteiligten Instanzen {ME} verwenden müssen.
...
*Wichtig*: Sie müssen {ME} verwenden, wenn Sie mit {CMK} nicht nur Ihre eigene, sondern auch die Infrastruktur von anderen Unternehmen überwachen.
...
{ME} baut inhaltlich auf xref:cce#[{CUE}] auf und ist deren mandantenfähige Erweiterung.
...
Sie können ein Upgrade einer der anderen Editionen auf {ME} durchführen.
Folgen Sie dafür der xref:upgrade#[Upgrade-Beschreibung] auf {CE}.
Für das Upgrade zu {ME} kommt in verteilten Umgebungen nur das xref:upgrade#offline_distributed[Offline-Upgrade] in Frage.
Dagegen ist ein _Downgrade_ der {ME} zu einer anderen Edition nicht vorgesehen und wird von uns nicht unterstützt.
...
Neu im [.guihint]#Main#-Dashboard ist das Widget [.guihint]#Customers#, welches sich links der Service-Probleme befindet:
...
image::managed_dashboard.png[alt="Das 'Customers'-Widget im 'Main'-Dashboard.",width=600]
...
Selbstverständlich können Sie die Filter und Datensätze, so wie sie für das Widget und das Snapin verwendet werden, auch für die eigenen Ansichten benutzen.
