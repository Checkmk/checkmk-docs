...
Im Folgenden möchten wir Ihnen die kommerzielle xref:glossar#edition[Edition] {CME} (vormals {CMK} MSP) vorstellen, mit der Sie als ein _Managed Service Provider (MSP)_ über das xref:glossar#distributed_monitoring[verteilte Monitoring] von {CMK} voneinander abgeschottete Instanzen für mehrere Kunden betreiben können.
...
{CME} ist die derzeit umfangreichste Edition von {CMK}.
Inhaltlich baut sie auf xref:cce#[{CCE-onprem}] auf und ist deren mandantenfähige Erweiterung.
