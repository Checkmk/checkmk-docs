...
[TIP]
====
Leider stecken wir derzeit in der Azure-Bürokratie fest und können keine Images mit vorinstalliertem {CMK} {v23} bereitstellen.
Die neuesten Images, die im Microsoft Azure Marketplace verfügbar sind, basieren noch auf {v22}.
Sie können diese Images verwenden und sie unmittelbar nach Bereitstellung auf {v23} xref:update#[aktualisieren].
Eine einfachere Lösung ist jedoch die Verwendung eines Ubuntu 24.04 Vanilla-Images und die xref:install_packages_debian#[Durchführung der Installation] dort.
====
