...
[TIP]
====
Leider stecken wir derzeit in der Azure-Bürokratie fest und können keine Images mit vorinstalliertem {CMK} {current-major} bereitstellen.
Die neuesten Images, die im Microsoft Azure Marketplace verfügbar sind, basieren noch auf {v22}.
Sie können diese Images verwenden und sie zunächst mit xref:update#[update] auf {oldstable} und dann unmittelbar nach dessen Installation auf {current-major} aktualisieren.
Eine einfachere Lösung ist jedoch die Verwendung eines Vanilla-Ubuntu-24.04-Images und die Ausführung der Schritte unter xref:install_packages_debian#[perform an installation] dort.
====
