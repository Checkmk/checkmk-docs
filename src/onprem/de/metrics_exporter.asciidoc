...
:title: Metriken an InfluxDB und Graphite senden - per InfluxDB v2 API oder das Carbon-Plaintext-Protokoll
...
InfluxDB bietet mit der InfluxDB v2 API eine einheitliche API, mit der unter anderem Metrikdaten in die Datenbank geschrieben werden können.
...
[TIP]
====
Die in diesem Kapitel beschriebene Konfiguration gilt nur für die Anbindung einer InfluxDB mit der InfluxDB v2 API.
...
{CMK} unterstützt zur Zeit _nicht_ die InfluxDB v3 API.
...
Öffnen Sie das xref:user_interface#setup_menu[Setup-Menü] und schalten Sie den xref:intro_gui#show_less_more[Show-more-Modus] an -- denn nur dann wird der Menüeintrag [.guihint]#Setup > Connectors > InfluxDB connections# angezeigt.
...
Den Regelsatz [guihint]#Send metrics to InfluxDB# finden Sie unter [guihint]#Setup > Services > Service monitoring rules#, noch schneller mit der xref:user_interface#search[Suche] im [.guihint]#Setup#-Menü und am schnellsten per Klick in der xref:influxdb_connection_list[Verbindungsliste]:
...
Zum Schluss xref:glossar#activate_changes[aktivieren Sie die ausstehenden Änderungen] für die Erstellung der Verbindung und der Regel.
...
Die xref:glossar#activate_changes[Aktivierung ausstehender Änderungen] für die Regeln und die Verbindung schließt die Konfiguration für Graphite ab.
