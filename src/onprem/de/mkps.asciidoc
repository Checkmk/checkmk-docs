...
Es kann darüber hinaus Einstellungen für die Verteilung von Agenten-Plugins via Agentenbäckerei enthalten.
...
Auf der Befehlszeile lösen Sie ein Paket mit `mkp release` auf.
...
Das Äquivalent auf der Befehlszeile ist `mkp find`:
...
Sie können daher für [.guihint]#Valid until {CMK} version# 2.5.99 als maximal unterstützte {CMK}-Version eintragen.
...
Die Vorgehensweise zum Erstellen von MKPs auf der Befehlszeile ist analog zum Setup-Menü.
...
You may now edit it.
Create the package using pass:[`]mkp package tmp/check_mk/hello_world_ng.manifest.temppass:[`].
----
...
Wenn Sie die Archive neu packen, achten Sie darauf, keine Pfade mit einzupacken, die nicht Teil der Ordnerhierarchie unter `~/local` sind.
