...
Des Weiteren benötigen Sie das Zip-Archiv der Appliance.
...
Falls Sie stattdessen den System Center Virtual Machine Manager (kurz: SCVMM) nutzen, sollten Sie dieser Anleitung ebenfalls folgen können, da die Unterschiede bei der Einrichtung einer virtuellen Maschine marginal sind.
====
...
Sie richtet sich an Nutzer, die Hyper-V im Schlaf beherrschen und auf die nachfolgende xref:hyperv_stepbystep[schrittweise Anleitung] verzichten möchten.
...
* Wählen Sie den Punkt [.guihint]#Security# aus und setzen Sie die Checkbox [.guihint]#Enable Secure Boot# auf *deaktiviert*.
