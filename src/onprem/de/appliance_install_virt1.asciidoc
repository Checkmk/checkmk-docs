...
:description: Die virtuelle {CMK} Appliance können Sie in Hyper-V, Proxmox oder VMware ESXi einrichten. Lesen Sie hier, was es jeweils zu beachten gilt.
:experimental:
...
Mit der virtuellen Appliance _{CMK} virt1_ können Sie {CMK} als Gast auf einer vorhandenen Virtualisierungsplattform (_Hypervisor_) wie VMware ESXi, Hyper-V oder Proxmox betreiben.
Unterstützt werden {SE}, {CE} und {ME}.
...
//SK Der folgende Anker gehört eigentlich vor das folgende "Wichtig". Dann springt der Link aber an eine ungünstige Stelle.
[#vmxnet]
...
|SCSI Controller | Add Hard Drive | Wählen Sie die Disk aus dem Zip-Archiv mit `disk2` im Dateinamen. +
 +
*Optional*: Vergrößern Sie diese Platte, falls Ihnen 100 GB voraussichtlich nicht reichen werden, per [.giuihint]#Edit > Expand#.
|===
...
* Klicken Sie auf [.guihint]#SCSI Controller# und fügen Sie per [.guihint]#Add# eine neues [.guihint]#Hard Drive# hinzu.
Unter dem Feld [.guihint]#Virtual hard disk# finden Sie erneut den Knopf [.guihint]#Browse#.
Wählen Sie dieses Mal die virtuelle Festplatte, deren Dateiname `disk2` enthält.
* *Optional*: Auf dieser zweiten Platte (`disk2`) wird {CMK} installiert und dort werden auch alle {CMK}-Instanzen und deren zugehörige Daten abgelegt.
Standardmäßig hatte diese virtuelle Festplatte eine Größe von 100 GB.
Wenn Sie eine größere Platte für Ihre Instanzen und Daten benötigen, klicken Sie *jetzt* [.guihint]#Edit# und wählen Sie die Aktion [.guihint]#Expand#.
Beim ersten Start der virtuellen Appliance, wird diese Platte initialisiert und kann im Anschluss nicht mehr vergrößert werden.
...
- [.guihint]#OS > Guest OS > Type: Linux#.
- [.guihint]#System#: Aktivieren Sie die Option [.guihint]#Qemu Agent#.
