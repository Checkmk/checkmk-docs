...
:description: Mit der ntopng-Integration kann {CMK} Netzwerkdatenflüsse überwachen. Lesen Sie hier, wie Sie den Netzwerkverkehr mit ntopng analysieren.
...
Mit den Verbindungsparametern wird festgelegt, welche Daten von ntopng abgeholt werden.
...
{cee-only}
Um die Integration von ntopng in {CMK} nutzen zu können, benötigen Sie eine der kommerziellen Editionen von {CMK} mit dem Add-on für die ntopng-Integration sowie eine Installation von ntopng in einer Professional oder Enterprise Version >= 5.6,
...
Nachdem Sie die xref:glossar#activate_changes[ausstehenden Änderungen aktiviert] haben, wird jeder {CMK}-Benutzer die Änderungen der Benutzeroberfläche für ntopng sehen,
...
Aktivieren Sie abschließend die ausstehenden Änderungen.
...
Die in den letzten Abschnitten vorgestellten Dashboards [.guihint]#Alerts#, [.guihint]#Flows# und [.guihint]#Top Talkers# bestehen alle jeweils nur aus einem einzigen Widget.

Beim Editieren eines xref:dashboards#[Dashboards] können Sie diese ntopng-Widget verwenden, um bestehende Dashboards anzupassen oder neue zu erstellen.
...
Beim Editieren eines Dashboards finden Sie die ntopng-Widgets, wie auch die anderen Widgets, im Menü [.guihint]#Add# im Abschnitt [.guihint]#Ntop#.
