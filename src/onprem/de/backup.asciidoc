...
Dies ist etwas aufwändiger, denn auf jeder Remote-Instanz müssen Sie zuerst die Konfiguration via Setup erlauben, sich dann dort anmelden und das Backup konfigurieren.
...
Um die xref:backup_restore[Wiederherstellung (Restore)] eines Backups auf einer Remote-Instanz durchzuführen, gehen Sie analog vor, indem Sie zuerst wieder die interaktive Anmeldung und die Konfiguration via Setup erlauben und abschließend die Optionen wieder zurücksetzen.
