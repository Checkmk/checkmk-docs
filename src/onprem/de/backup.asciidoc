...
:description: Mit Backups sichern Sie Daten und Einstellungen aus Ihrer {CMK}-Instanz und stellen sie wieder her. Lesen Sie hier, wie Sie Backups erzeugen und nutzen.
...
[TIP]
====
{CMK} bietet alternativ die Möglichkeit, die grundlegenden Backup- und Restore-Funktionen per Befehlszeile mit `omd backup` und `omd restore` auszuführen.
...
* Auf beiden Systemen muss die exakt gleiche xref:cmk_versions.html#[{CMK}-Version] installiert sein, also z.B. {current}.
* Auf beiden Systemen muss die gleiche {CMK}-Edition installiert sein, z.B. {CRE}.

[TIP]
====
Die Systemplattform ist nicht relevant, sodass Sie zum Beispiel ein Backup mit Ubuntu als Quellsystem erstellen und mit Red Hat oder einer Appliance als Zielsystem zurückspielen können.
====
...
Dies ist etwas aufwendiger, denn auf jeder Remote-Instanz müssen Sie zuerst die Konfiguration via [.guihint]#Setup# erlauben, sich dann dort anmelden und das Backup konfigurieren.
...
. xref:glossar#activate_changes[Aktivieren Sie die ausstehenden Änderungen].
