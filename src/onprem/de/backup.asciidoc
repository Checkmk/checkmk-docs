...
Ist die lokale Instanz die Zentralinstanz in einem xref:distributed_monitoring#[verteilten Monitoring,] so umfasst das Backup die Konfigurationsdaten dieser Zentralinstanz und aller Remote-Instanzen, die von der Zentralinstanz aus konfiguriert werden.
...
Dies ist etwas aufwändiger, denn auf jeder Remote-Instanz müssen Sie zuerst die interaktive Anmeldung erlauben, sich dann dort anmelden und das Backup konfigurieren.
Abschließend können Sie interaktive Anmeldung auf den Remote-Instanzen wieder deaktivieren.

Dies funktioniert in allen {CMK}-xref:glossar#edition[Editionen] einschließlich der {ME}.
// TK: {CME} wird hier explizit erwähnt, weil das erst in der 2.3.0 funktioniert.
...
. Im Kasten [.guihint]#Configuration connection# deaktivieren Sie die Option [.guihint]#Disable remote configuration# und aktivieren die Option [.guihint]#Direct login to Web GUI allowed.#
...
// TK: Welchen Account nehm ich denn da?

. Abschließend können Sie auf der Zentralinstanz in den Eigenschaften der Remote-Instanz die Optionen [.guihint]#Disable remote configuration# und [.guihint]#Direct login to Web GUI allowed# wieder auf die vorherigen Werte zurücksetzen.
Aktivieren Sie auch diese Änderungen.

Um das Restore eines Backups auf einer Remote-Instanz durchzuführen, gehen Sie analog vor, indem Sie zuerst wieder die interaktive Anmeldung erlauben und abschließend wieder deaktivieren.
