...
:description: Linux-Upgrades auf dem {CMK}-Server sind mit geringen Ausfallzeiten möglich. Lesen Sie hier, wie Sie die Aktualisierung des Systems planen und umsetzen.  
...
* Eine Sicherung mit den von {CMK} bereitgestellten Tools, entweder auf der Befehlszeile mit dem xref:omd_basics#omd_backup_restore[Befehl `omd`] oder im xref:backup#[Setup] erstellt.
...
Die Vorbereitung besteht vor allem aus der Entfernung _aller_ bereits jetzt nicht mehr benötigten {CMK}-Pakete.
...
[IMPORTANT]
====
Die Edition und die Versionsnummer des für die Zielversion heruntergeladenen {CMK}-Paketes müssen exakt der bisher verwendeten entsprechen.
====
...
----
{c-root} omd versions
{current-major}p1.pro
{current-major}p2.pro (default)
----
...
mysite    {current-major}p2.pro    default version
----
...
[{shell}]
----
{c-root} apt purge -y {pkg-cee}-{current-major}p1
Reading package lists... Done
Building dependency tree... Done
Reading state information... Done
The following packages will be REMOVED:
  {pkg-cee}-{current-major}p1*
...
Removing {pkg-cee}-{current-major}p1 (0.jammy) ...
(Reading database ... 127417 files and directories currently installed.)
Purging configuration files for {pkg-cee}-{current-major}p1 (0.jammy) ...
----
...
[{shell}]
----
{c-root} apt remove -y {pkg-cee}-{current-major}p2
Reading package lists... Done
Building dependency tree... Done
Reading state information... Done
The following packages will be REMOVED:
  {pkg-cee}-{current-major}p2*
...
----
...
[{shell}]
----
{c-root} pass:[do-release-upgrade]
----
...
[{shell}]
----
{c-root} apt install /tmp/{pkg-cee}-{current-major}p2_0.noble_amd64.deb
----
...
[IMPORTANT]
====
Die Edition und die Versionsnummer des für die Zieldistribution heruntergeladenen {CMK}-Paketes müssen exakt der bisher verwendeten entsprechen.
====
