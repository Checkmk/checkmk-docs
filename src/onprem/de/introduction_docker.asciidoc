...
[IMPORTANT]
====
Der Betrieb von {CMK} anhand der hier vorgestellten Images wird nur innerhalb von Docker selbst unterstützt.
...
[cols="25,~,~,~"]
|===
|{CRE} |link:https://hub.docker.com/r/checkmk/{pkg-cre}/[Docker Hub^] | link:https://checkmk.com/de/download?platform=docker&edition=cre[{CMK}-Download-Seite^] |-- 
|{CSE} |link:https://hub.docker.com/r/checkmk/{pkg-cee}/[Docker Hub^] |link:https://checkmk.com/de/download?platform=docker&edition=cee[{CMK}-Download-Seite^] |link:https://portal.checkmk.com/de/[{CMK}-Kundenportal^] 
|{CCE} |link:https://hub.docker.com/r/checkmk/{pkg-cce}/[Docker Hub^] |link:https://checkmk.com/de/download?platform=docker&edition=cce[{CMK}-Download-Seite^] |link:https://portal.checkmk.com/de/[{CMK}-Kundenportal^] 
|{CME} |link:https://hub.docker.com/r/checkmk/{pkg-cme}/[Docker Hub^] |link:https://checkmk.com/de/download?platform=docker&edition=cme[{CMK}-Download-Seite^] |link:https://portal.checkmk.com/de/[{CMK}-Kundenportal^] 
|===
...
{cce-only}
Auch für die kommerziellen xref:glossar#edition[Editionen] von {CMK} sind die offiziellen Images auf verschiedenen Plattformen frei verfügbar (im Lizenzstatus „Trial“).
...
Wenn Sie erwarten, dass die Verbindung zum Docker Hub von Ihrem System aus ohne Probleme möglich sein wird, können Sie direkt zum Abschnitt xref:container_start[Start eines {CMK}-Containers] springen.
...
| `image: "checkmk/{pkg-cre}:{current-major}-latest"` | Bezeichnung des {CMK}-Images im Format `<Repository>:<Tag>`.
...
l|environment:
  ...
  Sie können dieses Passwort jederzeit ändern, indem Sie innerhalb des laufenden Containers als Instanzbenutzer `cmk-passwd cmkadmin` ausführen.
...
Falls Sie noch kein Image heruntergeladen und auf Ihrem System hinterlegt haben, dauert das Ausführen von `docker compose up` einige Minuten, da zu diesem Zeitpunkt das angegebene Image aus dem Docker Hub geladen wird.
...
Optional erzwingen Sie wieder mit einem vorangestellten `docker pull checkmk/{pkg-cre}:{current-major}-latest` das Herunterladen des aktuellsten Images.
...
[IMPORTANT]
====
Verwenden Sie die Option `-v` nur, wenn Sie sich sicher sind, dass Sie die Einstellungs- und Monitoring-Daten aus den Volumes des Containers nicht mehr benötigen.
