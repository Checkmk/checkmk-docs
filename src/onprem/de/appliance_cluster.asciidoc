...
[#cluster_virt1]#Dabei# verwenden Sie die Schnittstellen `LAN1` und `LAN2` für den Anschluss an Ihr Netzwerk und die Schnittstellen `LAN3` und `LAN4` für die direkte Sync-Verbindung zwischen den Geräten.
...
. Prüfen Sie, ob Ihre Instanzen kompatible {CMK}-Versionen nutzen und aktualisieren Sie bei Bedarf alle {CMK} Instanzen wie im xref:appliance_usage#update_site[Appliance-Hauptartikel] beschrieben.
