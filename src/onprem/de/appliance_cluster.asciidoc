// -*- coding: utf-8 -*-
// IGNORE Failover LAN1 LAN2 Virtualisierer bond0 bond1 clustern ens32 eth0 eth1 geclustert geclusterten gerätespezifische rack1 rack5 Anschlüsse Hardwareausfälle Netzwerkanschlüsse
...
Funktionieren kann das Cluster im Ernstfall natürlich nur, wenn die Knoten über _separate_ Switches und Stromversorgungen betrieben werden!
...
Dabei verwenden Sie die Schnittstellen `LAN1` und `LAN2` für den Anschluss an Ihr Netzwerk und die Schnittstellen `LAN3` und `LAN4` für die direkte Sync-Verbindung zwischen den Geräten.
...
Öffnen Sie die Weboberfläche des ersten Knotens, wählen Sie die Geräteeinstellungen und oben [.guihint]#Network Settings.#
...
Nur die Standardschnittstelle (auf den Racks [.guihint]#eth0# über die Buchse [.guihint]#LAN1#), hier im Screenshot _ens32_, hat aktuell eine Konfiguration.
...
Der Grund: Die Major-Updates aktualisieren entweder die als Basis verwendete Debian-Version oder ändern grundlegende Konzepte.
Kurz gesagt heißt das, dass Sie das Cluster kurzzeitig komplett offline nehmen müssen - Sie bekommen also eine Wartungszeit (_downtime_).
Bei regulären Updates genügt es, einzelne Knoten des Clusters in den Wartungszustand zu versetzen, um die Aktualisierung durchzuführen.
Bei jedem Major-Update gehen Sie wie folgt vor:

. Trennen Sie die Knoten mit [.guihint]#Clustering > Disconnect Cluster# vom Cluster.
. Aktualisieren Sie die Knoten nacheinander wie im xref:appliance_usage#cma_webconf_firmware[Appliance-Hauptartikel] beschrieben.
. Verbinden Sie die Knoten über [.guihint]#Clustering > Reconnect Cluster# wieder zum Cluster.
. Prüfen Sie, ob Ihre Instanzen kompatible {CMK}-Versionen nutzen und aktualisieren Sie bei Bedarf wie im xref:appliance_usage#update_site[Appliance-Hauptartikel] beschrieben.
