...
//
// ES: Verweis auf Notification-Skripte vorerst entfernt. Falls gewünscht, kann ein Verweis zu einer neuen Seite in der Knowledge Base eingefügt werden, auf der die einzelnen Notification-Skripte in Github verlinkt werden
// TK: OK. Nur in der KB wird nichts von selbst erscheinen. Ich würde alle Kommentare hier löschen.
// TK: Das konntest Du nicht wissen, aber das Update in src/saas/de/notifications.asciidoc hättest Du Dir sparen können. Die notifications-Dateien werden neu sortiert, wenn die SaaS bei uns von 2.3.0 auf 2.4.0 gehoben wird (demnächst).
