...
:revdate: 2024-04-11
:title: Host tags
:description: With host tags, host tag groups and auxiliary tags you can structure folders and hosts in {CMK} according to your wishes.
...
image::hosttags_tags_default.png[alt="List of all predefined host tag groups.",fullscreen=1]

{CMK} sets up multiple host tag groups during an installation:

[cols="15,15,30,25,15",options="header"]
|===
// |ID |Titel |Zweck  |Merkmal |ID des Merkmals
|Group ID |Group Title (Name) |Function | Possible tag | Tag ID
// |Group ID
// |Group Title (Name)
// |Included Tags
// |Function
...
.4+|System criticality. For the `offline` tag the rule [.guihint]#Hosts to be monitored# is included, which disables the monitoring of the host. The other tags are just examples and have no initial function. However, you can assign these to hosts and then use them in rules.
...
.3+|Take this tag group as an example only. For the `wan` tag the example rule [.guihint]#PING and host check parameters# is stored, which adjusts the threshold values for ping response times to the longer runtimes in the WAN.
...
.4+|Specifies how the data is fetched from the host.
...
.3+|`piggyback`
.3+|[.guihint]#Piggyback#
.3+|This tag determines whether and how xref:glossar#piggyback[piggyback data] is expected/processed for the host.
...
.3+|`snmp_ds`
.3+|[.guihint]#SNMP#
.3+|This specifies whether data should (also) be collected via SNMP.
...
.4+|`address_family`
.4+|IP address family
.4+|Determines whether the host should be monitored by IPv4 or IPv6 or both. 'No IP' is relevant for hosts queried via a xref:glossar#special_agent[special agent].
