...
[TIP]
====
If you have already gained experience with the on-premises editions of {CMK}, you may notice that in this introduction there is no mention of legacy agents in pull mode or SNMP traps in the Event Console.
These features are not very relevant in the area of cloud monitoring and are therefore not part of {CE-saas}.
====
endif::[]
...
|Registration |The trust relationship between the Agent Controller in the host and the Agent Receiver in the {CMK} site is established. |All editions
|TLS encryption |After successful registration, data is exchanged in encrypted form using TLS. .3+|All editions
|Compression |Data is exchanged in compressed form.
|Pull mode |The agent sends the data when requested by the {CMK} site.
|Push mode |The agent sends the data to the {CMK} site autonomously. |{CE}
|Individual agent configuration |Per xref:glossar#agent_bakery[Agent Bakery], agents can be individually configured for single or groups of hosts and the agent packages can be created for installation. .2+|Commercial editions
|xref:agent_deployment#[Automatic agent updates] |The package from the Agent Bakery is first installed manually or via script and is automatically updated from then on.
|xref:hosts_autoregister#[Automatic creation of hosts] |The registration of the agent with the {CMK} site and the creation of the host is done automatically. |{CE}
|===
endif::[]
...
In these software packages you will find the agent with Agent Controller.
...
ifdef::onprem[]
.In the Agent Bakery, you can download agent packages
...
image::agent_linux_bakery_saas.png[alt=“Preconfigured agents in the Agent Bakery.”,fullscreen=1]
endif::[]
...
the Vanilla agent and a preconfigured agent for auto-registration.
endif::[]
...
For this you select the [.guihint]#Generic agent options > Allowed agent access via IP address (Linux, Windows)# rule set. 
...
Many agent rules are concerned with the installation of various _plug-ins_.
...
Here, for example, is the [.guihint]#MySQL databases# rule for monitoring MySQL:
...
On the [.guihint]#Properties of host# page, select [.guihint]#Host > Test connection# from the menu, and start the test by clicking [.guihint]#Run tests#:

image::monitoringagents_host_diag.png[alt="Result of the connection test to a host.",fullscreen=1]
