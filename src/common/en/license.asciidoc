...
|Excluded services / hosts |Services or hosts that are not included in the license size. They are intended for instances with which new functions, configuration changes or similar of {CMK} are to be tested. To mark services or hosts as excluded, set labels of type `cmk/licensing:excluded` according to the description in xref:labels#create_labels[Creating labels].
