...
[TIP]
====
Views have been around for almost as long as {CMK} has existed.
The technology used to implement these views has become outdated and has several drawbacks; for example, the views are difficult to scale, and run slowly in large environments.
That's why views in {CMK} are being reimplemented.
This approach involves loading less pre-rendered HTML from the server and instead relying more on asynchronous communication in the background--all with the goal of providing a responsive user interface. +
Starting with {CMK} version {v25}p10, the first new view [.guihint]#All hosts (experimental)# is available for you to try out.
It gives you a preview of the new views that will be introduced in {CMK} version {v30}.
It will take longer than one {CMK} major version to replace the _classic_ views with the _new_ views.
You can find everything you need to know about the new views in the chapter xref:new_view_using[Using the new views] of this article.
All other chapters apply only to the classic views.
====
...
[#using_views]
== Using the classic views

[#view_options]
=== Options, filters and commands

Each view offers various options that allow you to customize it and trigger actions.
...
As soon as you set a filter the icon on icon:icon_filters_set[alt="Icon for a set filter."] changes so that it is clear that under certain circumstances not all data will be displayed.
...
Starting with {CMK} version {v25}p10 (link:https://checkmk.com/werk/16925[Werk pass:q[#]16925^]), you can try out the first _new view_ with [.guihint]#All hosts (experimental)#:

.The new view [.guihint]#All hosts (experimental)#, whose content has been filtered by a search
image::views_all_hosts_exp.png[alt="The new view 'All hosts (experimental)'.",fullscreen=1]

The capabilities provided by the new view are presented in the following sections.

[TIP]
====
Since the new views are a completely new development, features you are familiar with from the classic views are _not yet_ supported in the new views.
For example, it is not possible to customize or clone a new view.
Only this current chapter is relevant for the new views; all other chapters in this article describe options that are available _only_ for the classic views.
====


[#new_view_switch]
=== Switching between the new and the classic views

Each new view is provided _in addition_ to the existing classic view.

Under the familiar name [.guihint]#All hosts#, you will find the classic view as usual, for example in the [.guihint]#Monitor# menu.
The title of the new view is marked with the suffix [.guihint]#(experimental)#.
In both views, you can switch to the other view with the click of a button in the upper-right corner of the page, allowing you to quickly test the new features compared to the classic view.
In the classic view, the button is labeled [.guihint]#Try the new "All hosts" view#, and in the new view, it is labeled [.guihint]#Return to classic view#.


[#new_view_menu_bar]
=== The streamlined menu bar

The menu bar contains only menus and menu items for actions that are relevant to the new view and cannot be performed in the view in any other way.


[#new_view_filter]
=== Filtering table content

You will find a quick way to filter the table to show only the content that is relevant to you below the menu bar:

Search field:: In the search field on the left, you can enter free text and submit it by pressing the kbd:[Enter] key. +
Only those entries that contain the search term will then be displayed.
The search covers _all_ table entries, which means there may be more entries than are currently displayed in the table.

Predefined button for filtering:: The [.guihint]#Unhandled host problems# button in the [.guihint]#All hosts (experimental)# view provides quick access to unhandled host problems. +
This view displays all hosts in the {DOWN} or {UNREACH} state, provided the hosts are not in a scheduled downtime or the problem has already been acknowledged.
This button filters some of the table columns.
You can see which ones by looking for the icon:icon_filters_set[alt="Icon for a set filter."] in the column header--as opposed to the icon:icon_filter[alt="Icon of a filter."] for an unset filter.

Filter columns:: You can use any filter icon in the column header to manually set your own filters in any column.
Click the filter icon icon:icon_filter[alt="Icon of a filter."] to see a list of available filter entries specific to the values displayed in the selected column.

Reset filters:: You can reset all filters set via column icons by clicking [.guihint]#Reset all filters#.

As a general rule, all filters are logically linked with *AND* ('both/and').


[#new_view_actions]
=== Performing actions

Buttons for the most important actions are prominently placed above the table.
In the [.guihint]#All hosts (experimental)# view, for example, these include the buttons for xref:basics_ackn#[acknowledging problems] or for setting xref:glossar#scheduled_downtime[scheduled downtimes].

These buttons are only active if at least one row has been selected via a checkbox.

The other actions are summarized in the [.guihint]#Actions# table column.
By the way, you can also perform actions in a xref:new_view_slideout[slideout].


[#new_view_columns]
=== The table columns

For a new view, the available table columns have been revised and reordered so that all important, frequently used information is clearly displayed.

The checkbox in the first table column for selecting rows for actions is visible--provided you have the necessary permissions to edit.

The [.guihint]#Actions# column contains the available actions that are not already offered by the buttons above the table.
Actions performed on a host are displayed in the [.guihint]#Mode# column, such as scheduled downtimes or acknowledged problems.

The [.guihint]#All hosts (experimental)# view contains, for example, the [.guihint]#IP address# and [.guihint]#All services# columns.
The latter shows the total for all services on a host.
...
Clicking repeatedly sorts the table first in ascending order, then in descending order, and finally removes the sort order.
The sort icon in the column header indicates which sort mode is active:
icon:icon_view_sort_ascending[alt="Icon for ascending sort order."] ascending,
icon:icon_view_sort_descending[alt="Icon for descending sort order."] descending, or
icon:icon_view_sort_unsorted[alt="Icon for no sort order."] unsorted.

Multi-level sorting across multiple table columns is also possible:
To do this, click the header of the table column by which you want to sort first (icon:icon_view_sort_ascending[alt="Icon for ascending sort order."] ascending or icon:icon_view_sort_descending[alt="Icon for descending sort order."] descending),
and then, while pressing the kbd:[Shift] key, click the header of the table column by which you want to sort next (again ascending or descending).
Entries that are identical in the first selected table column are then sorted by the second table column you selected.
The sort icons in the column headers indicate whether sorting is currently active and in which direction.

Multi-level sorting can be extended to additional table columns.
The multi-level sorting is disabled again when you click a table column without pressing the kbd:[Shift] key.


[#new_view_slideout]
=== Opening a slideout

From a new view, you can display contextual information via a slideout--that is, without having to leave the current view.

A table cell that opens a slideout contains the icon:icon-chevron-right[alt="Icon of a right-pointing arrow."] icon of a right-pointing arrow on the right margin.
Click the cell to display the slideout.

In the [.guihint]#All hosts (experimental)# view, you can click the table cell containing the host name to display the [.guihint]#Host details# slideout.
Here you will find additional host-specific information, much of which is otherwise collected in the classic [.guihint]#Status of host# view.


[#new_view_limit]
=== Changing the display limit

By default, a maximum of 1,000 entries are displayed in the table of a new view.
You can increase this value to `5000` or `All` using the [.guihint]#Show# list at the top right of the table.

Note that the global settings [.guihint]#Soft query limit# and [.guihint]#Hard query limit#, described in the section xref:limit[The display limit], apply only to classic views and are _not_ taken into account by the new views.


[#new_view_feedback]
=== Providing feedback on the new view

The new view is already being made available in {CMK} {v25} so that you can provide feedback.
We will try to incorporate your feedback into the further development of the new views for {CMK} {v30}.

Clicking the [.guihint]#Give feedback on the new view# button in the upper-right corner of the view opens a short questionnaire that you can fill out and send to {CMK}.
...
There you can create your own views with icon:button_add_view[], or customize existing ones with icon:icon_clone[alt="Icon for cloning."]:
...
You can return to the standard view later by simply deleting your clone (achieved in the table of views, as you might expect, with icon:icon_delete[alt="Icon for deleting."]).
...
. Filters set interactively by the user with icon:icon_filter[alt="Icon of a filter."] in the view.
...
Firstly, by clicking on icon:icon_filter[alt="Icon of a filter."] you can decide which filter will be available to a user.
...
|`t` |Title bar (with heading, breadcrumbs, countdown icon for page update), menu bar and actions bar are omitted.
