...
In the image above, in addition to the change for the newly-created [.guihint]#Oracle tablespaces# rule, you can also see changes that were not made by you.
These are changes to the password for the user `agent_download`, which is used to download the agent packages for the auto-registration of hosts.
{CE} changes this password regularly -- at least daily -- and uses the xref:glossar#automation_user[automation user] `automation`.

The listed changes can only be activated all at the same time.
Therefore, activate the [.guihint]#Activate changes of other users# checkbox.
Next, click on the [.guihint]#Activate on selected sites# button to apply the changes.
