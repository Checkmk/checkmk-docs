...
:description: With rules and host tags you can configure {CMK} and prevent a range of false alarms. Read here which fine-tuning options we recommend.
...
So basically, what we need to do here is to turn off the preliminary stage of notifications and to avoid the critical states {DOWN}, {WARN} and {CRIT} for non-critical problems.
====

The following sections on configuration will show you what fine-tuning options are available -- so that everything that does not cause problems will be green -- and how to get any occasional drop-outs under control.


[#rules]
== Rule-based configuration

Before you start configuring, we will give you first a brief overview on how configure the settings of hosts and services in {CMK}.
...
The basic idea is that you do not explicitly specify every single parameter for every service, but implement something like: +
...
For example, there is one called [.guihint]#File systems (used space and growth)# that sets the thresholds for all services that monitor file systems.
...
If you are unsure in which category the rule set can be found, you can also search through *all* rules in one go, either by using the xref:intro_gui#search[search] in the [.guihint]#Setup# menu or by opening the rule search page via [.guihint]#Setup > General > Rule search#.
In the following section, which deals with creating a rule, we will take the latter route.
...
.The action menu for the service
...
.The rule sets for the service
image::intro_parameters_of_this_service.png[alt="List of all rule sets for a service.",fullscreen=1]

In the first box entitled [.guihint]#Check origin and parameters#, the [.guihint]#File systems (used space and growth)# entry takes you directly to the set of rules for the file system monitoring thresholds.
...
.The overview page for the rule set
...
.The properties of the new rule
...
.Selecting the parameter for tablespace usage thresholds
...
Next is the [.guihint]#Conditions# box:

.In this condition, host tags and [.guihint]#Explicit tablespaces# are specified
image::intro_rule_ora_condition.png[alt="Dialog for setting the conditions for the new rule."]

In this example we will only go into the parameters that are absolutely necessary for defining this specific rule:
...
[.guihint]#Host tags# are a very important feature in {CMK}, so we will be devoting a separate section to them below.
At this point, you use one of the predefined host tags to specify that the rule should only apply to production systems.
To do this, first enable [.guihint]#Host tags# and then select the host tag group [.guihint]#Criticality# from the list.
In the two lists that appear, select [.guihint]#is# and [.guihint]#Productive system#.
...
[TIP]
====
A detailed description of all of the other parameters for conditions and a detailed explanation of the important concept of rules can be found in the xref:wato_rules#[article on rules].
...
In the previous section, you saw an example of a rule that should only apply to production systems.
More specifically, in that rule you defined a condition using the [.guihint]#Productive system# _host tag_.
Why was the condition defined as a tag rather than simply applied to the folder?
...
image::intro_tags_default.png[alt="List of all predefined host tag groups.",fullscreen=1]
...
.First, the basic settings for the host tag group are defined
...
.Then the individual tags are defined
...
.Host tag groups are displayed in a host's properties
...
* Create your own rules in the [.guihint]#File systems (used space and growth)# rule set.
...
[TIP]
====
An alternative to creating downtimes for hosts, which we have already described in the chapter on xref:intro_monitor#downtimes[{CMK} in monitoring], is the [.guihint]#Recurring downtimes for hosts# rule set in the commercial editions.
...
[TIP]
====
An alternative to creating downtimes for hosts, which we have already described in the chapter on xref:intro_monitor#downtimes[{CMK} in monitoring], is the [.guihint]#Recurring downtimes for hosts# rule set.
This has the great advantage that hosts that are added to the monitoring later automatically receive these scheduled downtimes.
====
endif::[]
...
.This rule specifies how to check whether hosts are {UP}
...
.This rule determines, among other things, the status of the [.guihint]#Check_MK# service in error situations
...
.Defining the valid states _up_ and _down_
...
.Setting a mean value for CPU utilization
...
To make sure you do not miss anything, {CMK} automatically sets up a special service for you on each host -- this service is known as the [.guihint]#Check_MK Discovery#:

.The [.guihint]#Check_MK Discovery# service displays service discovery changes
