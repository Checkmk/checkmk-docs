...
|8000 |TCP |Agent Controller TLS registration, agents in xref:glossar#push_mode[push mode], update of the certificates for mTLS used by the Agent Controller |If multiple sites are running on the {CMK} server, additional ports (8001, 8002...) may be needed.
|===

The TLS registration of agents uses the REST API on port 80/443 to discover the port to register (normally 8000 TCP).
...
|===
|Port |Protocol |Designation |Supplementary information
|80 |TCP |Hypertext Transfer Protocol (HTTP) |For xref:glossar#agent_bakery[Agent Bakery] and xref:dcd#[dynamic host configuration]
...
Communication with the {comfull} server for xref:product_usage_analytics#[product usage analytics], if an administrator has actively consented to usage data transmission (available in {CMK} {v23}p47 and above). +
