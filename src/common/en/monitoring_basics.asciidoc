...
|`no_permissions` |Must not do anything.
|===
...
{CMK} is so designed that whenever a host or service has a problem, an email is sent to the object's contacts (note that the `cmkadmin` user, 
