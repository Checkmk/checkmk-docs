...
Install it in the same network as the SNMP device you want to monitor.
====
endif::[]
...
[TIP]
====
{UE} provides a second method for monitoring SNMP devices using the xref:relay#[{CMK} Relay].
Relays located in the same network as the SNMP devices being monitored can also collect data from SNMP devices that the monitoring server cannot reach directly. 
====
endif::[]
...
[%collapsible]
====
The SNMP terminology uses two keywords, that unfortunately often get confused: _query_ and _request_.
The difference lies in the protocol level:

* _Query_ refers to the user's perspective--that is, _what_ data should be retrieved:
'Hey, SNMP client, get me all the available data on the ports of the switch in the broom closet at 27 Kellerstraße!'
The term _query_ is not included in the SNMP specification, but it appears in the documentation for many tools.
* _Request_ always refers to the specific way in _which_ a device provides data.
What one might colloquially phrase as:
'Hey, SNMP device, give me the available data for OID .1.3.6.1.2.1.1.1.0 and then tell me the next OID under which you have data!' would technically be an _SNMP GETNEXT-REQUEST_ that the _SNMP manager_ sends to the device's _SNMP agent_.

What's perceived as a single _query_ from the user's perspective, can result in one, several, or even thousands of necessary _requests_, depending on what the device supports and how the SNMP library being used operates internally.
====
...
The bulk requests accelerate monitoring by up to a factor of 10.
...
ifdef::saas[]
{CE} uses _active requests_ for SNMP monitoring – a pull method.
The Relay sends a UDP packet (port 161) containing an SNMP request to the device requesting the provision of specific data.
endif::[]
...
ifdef::saas[]
For these reasons, the {CMK} Relay does not support trap reception.
In the on-premises editions of {CMK}, traps can be received using the Event Console.
endif::[]
ifdef::onprem[]
Some of the drawbacks of SNMP traps are:
...
The {CMK} Relay does not support trap reception.
endif::[]
...
. Enable SNMP for _read_ requests.
ifdef::onprem[]
...
In a xref:distributed_monitoring#[distributed environment] or when using the xref:relay#[{CMK} Relay], the IP address of the machine from which the device is being monitored is critical.
endif::[]
ifdef::saas[]
. Enter the address of the {CMK} Relay you are using as the permitted IP address.
endif::[]
...
This certainly makes sense, but you should know that SNMP transmits the community in plain text (except for xref:snmp#v3[SNMP version 3]).
Anyone who can intercept packets can therefore very easily identify the community.
...
ifdef::saas[]
In the [.guihint]#Basic settings# box, you must first select the {CMK} Relay that will be used to monitor the SNMP host.
endif::[]
ifdef::onprem[]
In {UE}, you can select in the [.guihint]#Basic settings# box the {CMK} Relay to monitor the SNMP host if it is not directly accessible from the {CMK} server.
endif::[]
...
ifdef::onprem[]
If the problems occur with the commercial editions, one reason may be
endif::[]
ifdef::saas[]
One possible cause of problems is 
endif::[]
that the more performant xref:performance[inline SNMP] implementation, which is activated there by default, relies more heavily on compliance with standards than `snmpget`.
...
and they will not deliver any results to a service discovery if you do not help them with a special configuration.
...
. In the [.guihint]#Setup > Agents > SNMP rules > Disable bulk walks# rule set, create a rule for the host, and set the value to [.guihint]#Positive match (Add matching hosts to the set)#.
...
If you only want to monitor very few of the ports (only the first two ports, for example), you can manually limit {CMK} to retrieving data only for specified ports. Details on this can be found below in xref:snmp#performance[Performance].
...
Over and over again it is found that for certain devices {CMK} does not provide any specific plug-in, meaning you can only monitor the standard services as mentioned.
ifdef::onprem[]
...
ifdef::saas[]
In that case, the only option is to suggest the development of a corresponding plug-in at link:https://ideas.checkmk.com/[ideas.checkmk.com^]. 
endif::[]
...
If none of the SNMP protocol versions work, there can be several possible causes:
...
A device can show different information at one and the same point in the SNMP tree – depending on which _context ID_ is transmitted with the request.
...
Here you select the check plug-in for which contexts are to be activated, and then the list of contexts for which monitoring data should be retrieved.
...
ifdef::onprem[]
{cee-only}
While {RE} makes SNMP requests in the classic way via the `snmpget` or `snmpbulkwalk` commands, the commercial editions have a built-in SNMP engine that performs SNMP queries very efficiently without generating any extra processes.
...
The {CMK} Relay, which can be used in {UE}, also supports both query methods.
endif::[]
ifdef::saas[]
The {CMK} Relay supports two methods for querying SNMP:
the `snmpget` and `snmpbulkwalk` commands, and a built-in SNMP engine that executes SNMP queries with high performance without spawning additional processes.
With this engine, the CPU load for SNMP processing is about half as high, which is why it is used by default.
endif::[]
...
This is a good thing anyway, since this is normally faster because selective information cannot be retrieved with the efficient bulk requests.
...
In the rule's value, for each particular check plug-in you specify which indexes of the respective table are to be retrieved.

The usual check type for switch ports is called [.guihint]#SNMP interface check with 64 bit counters (using v2c)#.
The following example shows a setting in which only the first two ports are retrieved via SNMP:
...
To prevent the GUI from being blocked indefinitely by such a task, a 15-minute timeout is set by default.
...
Once the file has been generated, you can download it from the [.guihint]#Result# line.
