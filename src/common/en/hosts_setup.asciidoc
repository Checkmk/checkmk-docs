...
This applies to hosts queried via a xref:glossar#special_agent[special agent], and to the push mode of the {CMK} agent in {CCE}.
...
However, it fits particularly well as a counterpart to the option to xref:hosts_autoregister#[have hosts created automatically], which is included in the scope of functions in {CCE}.
