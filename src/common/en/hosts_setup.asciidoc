...
[TIP]
====
Selecting [.guihint]#Configured API integrations, no Checkmk agent# ensures that both of the services [.guihint]#Check_MK# and [.guihint]#Check_MK Discovery# always have the {OK} state.
...
This option is only available from {CCE} onwards, i.e. in {CE} and {ME}, as the push mode is supported only in these editions.
By the way, this option is also available from {CCE} onwards in the properties of a folder and is used there to enable xref:hosts_autoregister#[hosts to be created automatically] via auto-registration.
...
ifdef::saas[]
[.guihint]#Checkmk agent connection mode# specifies that the {CMK} agent should work in the xref:glossar#push_mode[push mode].
...
However, it fits particularly well as a counterpart to the option to xref:hosts_autoregister#[have hosts created automatically], which is included in the scope of functions from {CE} onwards.
endif::[]
ifdef::saas[]
