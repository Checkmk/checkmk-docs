...
ifdef::saas[]
This is especially useful for those attributes that are common to large groups of hosts, such as the host tags that you use to determine whether the host should be monitored by a {CMK} agent.
endif::[]
...
Please note, however, that {CMK} allows a maximum length of 240 characters for the host name.
...
|4. |Use the [.guihint]#Hosts with dynamic DNS lookup# xref:glossar#rule_set[rule set] to specify hosts for a dynamic DNS. The result is analogous to option 3, except that the [.guihint]#Hostname# field is now used for the DNS query. 
...
ifdef::saas[]
With this setting the host will be monitored agentless only with xref:glossar#active_check[active checks].
endif::[]
...
By the way, this option is also available in {CCE} in the properties of a folder and is used there to enable xref:hosts_autoregister#[hosts to be created automatically] via auto-registration.
If you select [.guihint]#Push - {CMK} agent contacts the server# here, additionally set [.guihint]#IP address family# to [.guihint]#No IP# in the xref:dns[Network address box].
...
This is the only available option in {CE-saas}.
...
This prevents the host's accessibility from being checked via xref:cmc_differences#smartping[Smart Ping].
In the on-premises editions of {CMK}, it is also possible to configure the agent for xref:glossar#pull_mode[pull mode].
endif::[]
...
Note that the setting made here must match the settings of the first [.guihint]#{CMK} agent / API integrations# option, otherwise the absence of expected monitoring data may not be noticed.
...
ifdef::saas[]
We therefore recommend monitoring management boards - if available - with  special agents.
endif::[]
====
...
* Username, e.g. [.guihint]#cmkadmin#: the host was created manually by a user, e.g. in xref:create_hosts[Setup].
* Automation username (in the picture above [.guihint]#automation#): the host was created by an xref:glossar#automation_user[automation user], e.g. using the xref:rest_api#[REST API].
...
ifdef::saas[]
This is useful for services that remember a current state on discovery (e.g., the current state of file systems and their mount points).
endif::[]
