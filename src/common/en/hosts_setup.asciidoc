...
{CMK} was designed from the beginning for environments with very many hosts, and has its own specific approach for making the configuration of such environments manageable for the user.
...
Here in the default view you can see an overview of the folders and below these, in tabular form, the hosts already present in the current folder.
...
The [.guihint]#Host name# field is used everywhere within {CMK} to uniquely identify the host.

.The basic settings of a host in {CE}
...
|1. |You do _not_ specify an IP address. The host name must be resolvable by DNS. |When xref:glossar#activate_changes[activate pending changes].
...
|4. |Use the [.guihint]#Hosts with dynamic DNS lookup# xref:glossar#rule_set[rule set] to specify hosts for a dynamic DNS. The result is analogous to option 3, except that the [.guihint]#Host name# field is now used for the DNS query. 
...
ifdef::onprem[]
.Configuring monitoring agents in {CE}
image::hosts_new_host_agents.png[alt="Dialog with the properties of a host: Monitoring agents."]
endif::[]
ifdef::saas[]
.Configuring monitoring agents
image::hosts_new_host_agents_saas.png[alt="Dialog with the properties of a host: Monitoring agents."]
endif::[]
...
Monitoring data is supplied by xref:glossar#api_integrations[API integrations], that is, by special agents or by piggyback from other hosts.
If xref:glossar#special_agent[special agents] are configured, _all_ are executed.
If no API integrations are configured, the output from the {CMK} agent is accessed.
This is the default value.
|[.guihint]#Configured API integrations and {CMK} agent# |
The output from a {CMK} agent is expected.
Data supplied by API integrations will also be used if configured.
...
With this setting the host will be monitored via SNMP or agentless only with xref:glossar#active_check[active checks].
Rules for active checks can be found in [.guihint]#Setup > Hosts > HTTP, TCP, Email, ...#
...
[TIP]
====
Selecting [.guihint]#Configured API integrations, no {CMK} agent# ensures that both of the services [.guihint]#Check_MK# and [.guihint]#Check_MK Discovery# always have the {OK} state.
...
ifdef::saas[]
[.guihint]#{CMK} agent connection mode# specifies that the {CMK} agent should work in the xref:glossar#push_mode[push mode].
This option is also available in the properties of a folder and is used there to enable xref:hosts_autoregister#[hosts to be created automatically] via auto-registration.
Additionally set [.guihint]#IP address family# to [.guihint]#No IP# in the xref:dns[Network address box].
This prevents the host's accessibility from being checked via xref:cmc_differences#smartping[Smart Ping].
endif::[]

Use the following [.guihint]#SNMP# setting to configure monitoring via xref:glossar#snmp[SNMP].
After enabling [.guihint]#SNMP# and selecting the SNMP version, the [.guihint]#SNMP credentials# field appears,
...
.The attributes available by default
...
.An attribute for the hardware type
image::hosts_setup_custom_attributes.png[alt="Form for creating a custom attribute."]
...
[IMPORTANT]
====
Management Boards will no longer be supported as a host property in a future {CMK} version.
...
.This box is only displayed for hosts that have already been created
...
There are the following main choices for the creator name:

* Username:
the host was created manually by a user, e.g. in the xref:create_hosts[Setup].

* Automation username (in the picture above [.guihint]#automation#):
the host was created by an xref:glossar#automation_user[automation user], e.g. using the xref:rest_api#[REST API].

ifdef::onprem[]
* _Network scan_:
The host was found during an automatic xref:folder_network_scan[network scan].
endif::[]

If the host was created by an automation user, it will be locked by its corresponding site ([.guihint]#Locked by#).
...
In contrast, [.guihint]#Save & run connection tests# takes you to the connection test.
This allows you to first test whether you are getting any data at all from the host with the settings you have specified--be it via an agent or whatever you have previously configured.
ifdef::onprem[]
...
* [.guihint]#Detect network parent hosts#: xref:hosts_structure#parents_scan[Have parents created by a scan].
...
* [.guihint]#Remove TLS registration#: Removes the registration for encrypted communication via Transport Layer Security (TLS).
You can find more information on this topic in the articles on the xref:agent_linux#deregister[Linux agent] and xref:agent_windows#deregister[Windows agent].
...
====
ifdef::onprem[]
This rule is available in all {CMK} editions.
However, it fits particularly well as a counterpart to the option to xref:hosts_autoregister#[have hosts created automatically], which is included in the scope of functions of {CE}.
endif::[]
ifdef::saas[]
This rule fits particularly well as a counterpart to the option of xref:hosts_autoregister#[have hosts created automatically].
endif::[]
...
Note that the xref:dcd#automatic_deletion_of_hosts[dynamic host management] also provides the option to have hosts removed automatically.
...
Select [.guihint]#Setup > Hosts > Hosts#.
In the host list, click icon:icon_services[alt="Icon to display the configured services."].

* In Setup, via the properties of a host: +
Select [.guihint]#Setup > Hosts > Hosts#.
In the host list, click the required host. On the [.guihint]#Properties of host# page, select [.guihint]#Host > Save & run service discovery#.
If you select [.guihint]#Host > Run service discovery# from the menu instead, you will also go to the service list--but without saving the host properties.

* In monitoring, via the list of services of a host: +
Select the [.guihint]#Host# menu, and in the [.guihint]#Setup# section select the [.guihint]#Run service discovery# entry.
A small gear symbol at the icon shows that this menu entry leads into a page in the Setup.
This will take you directly to the service configuration.
...
* The [.guihint]#Accept all# button is usually the best action for a new host.
This is also the right choice for existing hosts in which services are found that are not currently being monitored.
These can be found in the [.guihint]#Undecided services (currently not monitored) section#.
[.guihint]#Accept all# adds the missing services, removes vanished services and accepts any host labels found. 
...
* After each modification an xref:glossar#activate_changes[activate pending changes] is required to make them effective.
...
Once the service discovery has been completed, all that remains is to xref:glossar#activate_changes[activate pending changes] as usual, after which all the new hosts will be in the monitoring.
...
|`Host name` |Name of the host (maximum 240 characters)
...
First open [.guihint]#Setup > Hosts > Hosts#.
Then navigate to any folder or stay in the [.guihint]#Main# folder.
...
{CMK} provides a convenient way to access the central xref:user_interface#search[search].
However, you can also search specifically in the [.guihint]#Monitor# and [.guihint]#Setup# menus.
...
For the [.guihint]#Host name# field, a partial word search (_infix search_) is used--the entered text is searched, at any position, in the host name.
...
[IMPORTANT]
====
When making changes to multiple hosts and in multiple locations, it is always possible for something to go wrong.
ifdef::onprem[]
Therefore, make sure you have an up-to-date xref:backup#[backup] of your site--before starting the renaming action.
endif::[]
ifdef::saas[]
Therefore, make sure you have an up-to-date xref:admin_panel#admin_site[backup] of your site--before starting the renaming action.
endif::[]
====
...
In the [.guihint]#Host name matching# field, first optionally specify a regular expression that matches the *beginning* of the host names you want to rename--so here in the example, all hosts whose names start with `lnx`.
...
You can find the reference documentation in your {CMK} site's xref:rest_api#access[web interface].
There you can familiarize yourself with the syntax of the requests and the structure of the responses.
You can access all important REST API entries in the {CMK} web interface via the navigation bar in the [.guihint]#Help > Developer resources > REST API# menu.
...
|Configuration file for all hosts in the [.guihint]#Main# folder.
For hosts in subfolders of [.guihint]#Main#, there is a file with the same name in each subdirectory.
