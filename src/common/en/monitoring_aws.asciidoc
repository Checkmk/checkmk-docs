...
:description: {CMK} can monitor data and costs of Amazon S3, EC2, EBS, RDS, ELB and related products. Read here how to manage the monitoring.
...
After you add the services and after an xref:glossar#activate_changes[activate pending changes] it will look something like this in monitoring:
...
|CE |[.guihint]#Costs & Usage# |At the AWS host
|EBS |[.guihint]#Block Storages# |Appended to the EC2 instance if it belongs to the instance, otherwise to the AWS host
|S3 |[.guihint]#Simple Storages# |At the AWS host
|RDS |[.guihint]#Relational databases# |At the AWS host
|===
...
ifdef::onprem[]
{CCE-only}
For a convenient start into monitoring AWS, {CMK} ships in {CE}
endif::[]
ifdef::saas[]
For a convenient start into monitoring AWS, {CE} ships
endif::[]
