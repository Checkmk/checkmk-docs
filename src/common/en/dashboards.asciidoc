...
An important feature of dashboard__ use:__ Using filters, even dashboards that show values for all hosts or services in the network can be broken down into specific areas. Dashboards are therefore not simply rigid displays, rather they are real tools for finding and analyzing problems and states.
...
|{CMK} |Site overview |no |Hosts as state-hexagons
|{CMK} |Alert statistics |no |Hosts as alerting-hexagons
|{CMK} |Host statistics |yes |Overall statistics for the host state
|{CMK} |Service statistics |yes |Overall statistics for the service state
|{CMK} |Event statistics |yes |Overall statistics for the event state
...
Basically, many dashboards already work exactly like this: The [.guihint]#Host statistics# visualizes the host states and a click on one of the states redirects to a view that lists the associated hosts -- and from there it goes on to the individual services of a single host.
...
image::dashboards_total_dashlets.png[alt="Error message in the 'Total host problems' dashlet."]
