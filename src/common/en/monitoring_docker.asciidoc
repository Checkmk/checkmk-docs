...
:description: {CMK} can monitor Docker nodes and their containers. Read here how to set up and use the monitoring based on the piggyback mechanism.
...
If you now perform the xref:wato_services#[service discovery] in {CMK} and activate the pending changes, you should find some new services that affect the Docker node itself:
...
The plug-in or special agent transports data of other host—'piggybacked'—so to speak--alongside its own data.
...
* For [.guihint]#Monitoring agents# be sure to set [.guihint]#{CMK} agent / API integrations# to [.guihint]#No API integrations, no {CMK} agent#.
...
Additionally, you can prefix the container name with the Docker hostname.
This can be done using the `combined` option.
To determine the Docker hostname, we use the output of `uname -n`.
