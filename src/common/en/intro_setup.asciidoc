...
The {CME} is a multi-tenant capable extension of the {CE} and has all of the necessary functions for operating individual, self-contained sites for each of multiple clients with {CMK} via a xref:glossar#distributed_monitoring[distributed monitoring]. 
