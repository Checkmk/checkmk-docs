...
:description: {CMK} can monitor cronjobs and other time-based processes with the script mk-job. Read here how to set up and use this script.
...
To monitor this job, use an editor you trust to change the line by prefixing the command `/usr/local/bin/backup >/dev/null` with the script name `mk-job` together with a name for the service (in the example, `nightly-backup`).
The latter name will later be used as the service name in {CMK} and must therefore be unique on this host:
...
If `mk-job` is run as an unprivileged user, it will attempt to create missing user directories.
In most cases, this will fail due to insufficient permissions.
Therefore, first create a user directory in the `job` directory `/var/lib/check_mk_agent/job/`.
Then assign ownership to the appropriate user—in the following example command, the user `myuser`:
...
Now `mk-job` can store the results to this directory on behalf of the user.
