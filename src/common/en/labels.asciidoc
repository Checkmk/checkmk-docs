// -*- coding: utf-8 -*-
// IGNORE hw kube Xubuntu Meraki Nutanix
...
An important function of xref:glossar#label[Labels] is the same as for host features:
...
*Tip:* If this causes difficulties in individual cases, it may be better to work with host characteristics.
In the article on xref:hosts_structure#[Structuring the hosts] you will find more information on the various structuring elements.
...
*Important:* The operators are processed in exactly this priority -- `Not`, `And`, `Or` -- i.e. _not_ necessarily in the order in which they appear in the list.
...
*Note:* If you have defined neither [.guihint]#Host tags# nor [.guihint]#Host labels#, the rule in question is always applied to all hosts or services.
...
You can use both labels and host characteristics in a rule.
...
For example, to find Linux hosts that are located in Munich and are not web servers, the filter could look like this:
...
// === Automatically generated service labels