...
{CMK} can create such dynamic hosts automatically with the xref:dcd#[dynamic host configuration], and can also receive information on any labels/tags that are already present.
...
{CCE} and {CME} include the option to xref:hosts_autoregister#[create hosts automatically].
