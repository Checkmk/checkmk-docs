...
= Labels
:title: Labels
:description: Labels for hosts and services in {CMK} help you structure your monitoring. Read here how to create, manage and use labels.
...
There is a [.guihint]#Host labels# rule set for this function, which you can quickly find via a xref:user_interface#search[search] in the [.guihint]#Setup# menu.
...
The discovery check will then be {OK} again the next time it runs (up to a two-hour delay), even if you have not yet xref:glossar#activate_changes[activated the pending changes].
...
|`cmk/device_model` |model name provided by the device
