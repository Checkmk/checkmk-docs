...
We have already introduced the facility for setting thresholds depending on a files system's size in the xref:intro_finetune#filesystems[chapter on fine-tuning the monitoring]
-- and hinted then that {CMK} has another, even cleverer option on offer -- the *magic factor*.
