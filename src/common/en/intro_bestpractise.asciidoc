...
.Option selected to monitor each core individually
...
.Selecting state and start type for Windows Server services
...
Therefore, you need a second step and a second rule, this time from the [.guihint]#Windows service discovery# rule set, with which you define which Windows services {CMK} should monitor as services.
...
.Limiting the Windows Server services to be monitored using regular expressions
...
Enable [.guihint]#Service name# and enter `Internet connection`.
...
If you now activate the pending changes, you will get the new host `internet` with the single service `Internet connection` in the monitoring.
...
.A service that monitors connections to three IP addresses
...
. Save the rule and activate the pending changes.

You will now have a new host with the services you specified that check access via HTTP(S):

.A service that monitors the availability of a web server
image::intro_check_httpv2_services.png[alt="List entry of the service for monitoring the HTTP/HTTPS services on a host."]
...
We have already introduced the facility for setting thresholds depending on a files system's size in the xref:intro_finetune#filesystems[chapter on fine-tuning the monitoring]—and hinted then that {CMK} has another, even cleverer option on offer—the *magic factor*.

You set up the magic factor like this:

. In the [.guihint]#File systems (used space and growth)# rule set, you create just a single rule.
...
. In addition, activate [.guihint]#Magic factor (automatic level adaptation for large file systems)# and confirm the 0.8 default value.

. Also set [.guihint]#Reference size for magic factor# to 20{nbsp}GiB.
Since 20{nbsp}GiB is the default value, it will take effect even without you explicitly activating the option.

The result will look like this:

.The magic factor is configured using two rule options
...
. File systems that are exactly 20{nbsp}GiB in size are given the thresholds 80{nbsp}% / 90{nbsp}%.

. File systems smaller than 20{nbsp}GiB are given lower thresholds.

. File systems larger than 20{nbsp}GiB are given higher thresholds.
...
.The [.guihint]#Summary# displays the thresholds for each file system service 
image::intro_magic_factor_services.png[alt="List with two file system services and their thresholds."]

The following table shows the resulting and actually applied thresholds for warnings, calculated using the respective magic factor for a reference size of 16{nbsp}GiB and an initial threshold of 80{nbsp}%.
Instead of the default value of 20 GiB, the table shows the reference size of 16 GiB, simply because it fits better into the sequence of multiplying example values:
...
On https://github.com/Checkmk/checkmk/tree/master/doc/treasures/[GitHub^], you will find in the `treasures` directory of {CMK} the script `magic_levels.py` which was used to create this table.

include::include_treasures_admonition.asciidoc[]

Some explanations and notes regarding the table above:

* The column for 16{nbsp}GiB shows the reference size itself, at which each factor leaves the threshold unchanged.
To the left of this column, the thresholds decrease; to the right, they increase.
Note that the magic factor never lowers a threshold below the values specified in the rule shown above for the [.guihint]#Minimum levels if using magic factor# option, which are preset to 50{nbsp}% / 60{nbsp}%.

* Given the sizes typical of today's storage systems, it is worth converting the percentages back to free space, since a single percentage point covers a large range.
Using the default factor of 0.8, a 16 TiB file system reaches the {WARN} state at 95{nbsp}%, leaving approximately 819{nbsp}GiB free.
With a factor of 0.5, the same file system does not reach the {WARN} state until 99.4{nbsp}%, leaving only 98{nbsp}GiB of free space.
The lower the factor, the less leeway you have with your largest file systems.
You should keep this effect in mind before setting a magic factor of 0.5 or lower.

////
* If the smallest file system you are monitoring is already several hundred gigabytes in size, the reference size of 16{nbsp}GiB no longer comes close to reflecting your infrastructure, and each file system will be scaled up.
In this case, increase the [.guihint]#Reference size for magic factor# to a size typical for your systems and leave the factor at 0.8.
For example, with a reference size of 1{nbsp}TiB, a 256 GiB file system reaches the {WARN} state at 73.6{nbsp}%, and a 64 TiB file system reaches it at 91.3{nbsp}%.
////

With this section on the magic factor, we conclude our Beginner's Guide.
