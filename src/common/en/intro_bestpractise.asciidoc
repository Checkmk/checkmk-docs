...
// This block replaces the following text until the end of this section
// start translation
////
...
. Create a new host for the web server in {CMK}, for `checkmk.com` for example.
Under [.guihint]#Monitoring agents#, enable [.guihint]#Checkmk agent / API integrations# and select [.guihint]#No API integrations, no Checkmk agent# there.
Save the host without service discovery.

. Create a new rule from the [.guihint]#Check HTTP web service# rule set that will only apply to the new host (e.g. via the [.guihint]#Explicit hosts# condition).
+
--
