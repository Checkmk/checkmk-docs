...
The following table shows the resulting and actually applied thresholds for warnings, calculated using the respective magic factor for a reference size of 20{nbsp}GB and an initial threshold of 80{nbsp}%:
...
* The column for 20{nbsp}GB shows the reference size itself, at which each factor leaves the threshold unchanged.
...
Using the default factor of 0.8, a 10 TB file system reaches the {WARN} state at 94.2{nbsp}%, leaving approximately 580{nbsp}GB free.
With a factor of 0.5, the same file system does not reach the {WARN} state until 99.1{nbsp}%, leaving only 89{nbsp}GB of free space.
...
* If the smallest file system you are monitoring is already several hundred gigabytes in size, the reference size of 20{nbsp}GB no longer comes close to reflecting your infrastructure, and each file system will be scaled up.
...
For example, with a reference size of 1000{nbsp}GB, a 500 GB file system reaches the {WARN} state at 77.0{nbsp}%, and a 50 TB file system reaches it at 90.9{nbsp}%.
