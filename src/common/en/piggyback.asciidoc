...
:description: With the piggyback mechanism, {CMK} can receive data about hosts when querying other hosts. Read here how to use piggyback hosts.
...
The command `cmk-piggyback list sources` returns a list of all piggyback hosts that have provided data for other hosts.
Its counterpart is `cmk-piggyback list piggybacked`, which displays a list of all piggybacked hosts (independently of whether they are already included in the monitoring).

The `cmk-piggyback` command also provides additional options for observing the treatment of piggyback data in the monitoring.
For instance, with `cmk-piggyback track`, you can view all incoming piggyback data.
You can use `cmk-piggyback create-sections` to create sample piggyback data.
endif::[]
...
| `~/bin/cmk-piggyback` |Helper script with analysis tools for piggyback features. Display information about its usage with `cmk-piggyback -h`.
|===
endif::[]
