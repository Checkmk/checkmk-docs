...
For each target host, one directory is present.
The data from each source host is placed as one file to the corresponding target directory.
...
If you are working in an environment where hosts automatically change the source host, we recommend using the xref:dcd#[dynamic host management].
...
Further information on this peer-to-peer connection can be found in the article on xref:distributed_monitoring#[Distributed monitoring].
endif::[]
