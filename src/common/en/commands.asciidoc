...
When you select one of the commands, an area with the parameters of the command is displayed below the menu bar or action bar.
...
Pressing the [.guihint]#Acknowledge problems# button opens a request for confirmation:

image::commands_confirm_acknowledgement.png[alt="Ask for confirmation before executing the command."]
...
Reminder: Available means that the commands can be different depending on the view and permission.
...
Acknowledgements can be removed with the [.guihint]#Remove acknowledgments# command.
