...
{cre-only}
Up to {CMK} version {v22}, SAML was also supported alternatively with the Apache module `mod_auth_mellon` and this was supplied as part of the {CMK} software.
Starting with version {v23}, `mod_auth_mellon` is no longer included in the {CMK} software.
...
Now call [.guihint]#Edit > Signing Option > Sign SAML assertion# to configure Entra ID for responses and verifications:
...
Alternatively, you can download the metadata XML file directly from Entra ID and upload it in the dialog above with the option [.guihint]#Upload XML file# in the [.guihint]#Identity provider metadata#.
...
On the {CMK} server, `mod_auth_mellon` acts as a service provider for authentication.


=== Logging in with Active Directory Federation Services

// This feature is not supported by SUSE Linux Enterprise Server (SLES) versions 12 SP3, 12 SP4 and 15 due to missing dependencies (SLES 15 SP1 and later do allow connectivity).
...
// Currently, SLES 15 SP4 is _not_ supported.

The setup is accomplished in three steps:

. Configuring Apache (one result: XML file with metadata).
. Configuring ADFS: setting up a relying party trust with Mellon metadata.
. Enabling the login in {CMK} itself.
...
As a final step, replace the authentication configuration file `~/etc/apache/conf.d/auth.conf` with the following variant - of course specifying your {CMK} server (here `myserver`) and your instance (here `mysite`).
...
When Entra ID (called Azure Active Directory until 2023) acts as the IdP, there are some differences in the configuration procedure, for example, the user name can be set directly without being rewritten.

Prerequisites for the following sample configuration:

- Set UserPrincipalName in LDAP connection as identifier (more information in the link:https://learn.microsoft.com/en-us/windows/win32/adschema/a-userprincipalname[Microsoft documentation^]).

- Custom Enterprise App in Entra ID with UserPrincipalName as 'name' attribute (for more information see the link:https://learn.microsoft.com/en-us/entra/identity/enterprise-apps/add-application-portal[Microsoft documentation^]).
...
When NetIQ Access Manager acts as the IdP, there are some differences in the configuration procedure, for example, the user name can be set directly without being rewritten.
