...
:description: In {CMK}, you can configure the monitoring via the Setup menu. Read here how to use the setup menu and activate pending changes.
...
ifdef::onprem[]
and the xref:relay#[{CMK} Relays] (only in {UE}).
endif::[]
ifdef::saas[]
and the xref:relay#[{CMK} Relays]
endif::[]
...
and agent registration to xref:hosts_autoregister#[automatically create hosts] (only in {CE}).
endif::[]
ifdef::saas[]
...
ifdef::onprem[]
You can also add and use xref:views#icons[custom icons].
endif::[]
...
ifdef::onprem[]
|[.guihint]#Connectors# |Connections to other systems, for example, for exporting metrics to xref:metrics_exporter#[InfluxDB (and Graphite)].
...
|[.guihint]#Telemetry# |Receiving xref:opentelemetry#[OpenTelemetry] data
ifdef::onprem[]
({CE} only) via the OpenTelemetry Collector or a Prometheus endpoint.
endif::[]
ifdef::saas[]
...
ifdef::onprem[]
.The [.guihint]#Setup shortcuts# snap-in of {CE} in Show more mode
image::wato_setup_shortcuts.png[alt="Setup shortcuts snap-in.",width=50%]
endif::[]

ifdef::saas[]
.The [.guihint]#Setup shortcuts# snap-in in Show more mode
...
You can see the number of pending changes that have not been activated yet in the navigation bar, in the [.guihint]#Changes# menu, indicated by the number in the yellow circle.
If you click this icon, you will see the details of these pending changes:

.Summary of pending changes
image::wato_menu_changes.png[alt="The 'Changes' menu displays the list of pending changes.",width=86%]

Clicking the [.guihint]#Activate pending changes# button applies the changes.
After a short wait, you will receive a confirmation that the changes have been successfully activated and everything is up to date.
The number in the [.guihint]#Changes# menu has disappeared and will remain so until the next change is pending.

The slideout shown above provides a summary of the most important information.
This allows you to see at a glance whether there are any problems with activation or with sites.
Such problems can prevent pending changes from being activated.
In this case, it is advisable to get the full overview using the [.guihint]#Activate pending changes# xref:glossar#view[view].
You can open this page from the slideout by clicking the button showing the three numbers or on [.guihint]#Open full view#:

.All information about pending changes
image::wato_pending_log.png[alt="List of pending changes to be activated."]

On this page, you can start activating the changes by clicking [.guihint]#Activate on selected sites#.

[TIP]
====
You can specify whether clicking the [.guihint]#Changes# icon should display the slideout, or immediately open the [.guihint]#Activate pending changes# page.
You can find the corresponding option [.guihint]#Default behavior for 'Changes'# in your personal settings in the xref:user_interface#user_menu[User Menu].
====

Whichever method you choose, 
when you activate the pending changes, a new configuration for the monitoring core is generated from the configuration files, 
and the monitoring core is instructed to use this configuration immediately:

image::wato_activate_changes.png[alt="Illustration showing how changes are applied from the configuration environment to the monitoring environment.,width=500px]
...
[TIP]
====
If you try to activate pending changes and another activation is currently running, you will receive a corresponding warning:
...
You will encounter the activation of pending changes again and again when setting up your monitoring system, as well as every time you adjust the configuration later on.
