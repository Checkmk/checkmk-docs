...
:description: Programs on monitored hosts can store state information in the spool directory. Read here how to use that data as part of the {CMK} agent output.
...
- when the xref:monitoring_jobs#[`mk-job` plug-in] is not sufficient for the control of cronjobs, 
...
[IMPORTANT]
====
Note that the agent does not provide any content checking of spool files!
Manipulated spool files can overwrite regular agent sections and thus create the illusion of a problem-free operation.
Even without malicious intent, there is the potential risk that a spool file may contain a corrupted agent output, resulting in subsequent parts of the agent output being completely ignored.
====
