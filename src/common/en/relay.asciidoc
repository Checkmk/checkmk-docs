...
This also means, there is no typically no practical use for it in environments using {ME} as intended. 
endif::[]
...
The {CMK} Relay is distributed link:https://hub.docker.com/r/checkmk/check-mk-relay[as container^] for the _x86-64_ platform.
In general, any container runtime can be used, but to facilitate updates of the relay, we strongly recommend running on link:https://podman.io/[Podman^] and installing using the provided installation script.
...
// MFS: Diese kann bald wieder weg, in den nächsten Tagen sollte der Download aus der UI auch in den 2.5 Builds möglich sein.
[TIP]
====
...
Each Relay contacts its associated {CMK} site at least once per minute for updated configuration.
...
When deployed on Podman, the {CMK} Relay installs a service that automatically updates the container image to same version as the associated site and restarts the container afterwards.
...
Changes to this file trigger the oneshot service `checkmk_relay-update-manager`.
