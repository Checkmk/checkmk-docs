...
With {UE} the {CMK} Relay can be deployed to collect monitoring data and send it to the monitoring site.
...
To check the state of the updater manager for the {CMK} Relay, run:
...
Restarting the service (as down before retrieving the status for the output shown) forces a check for updates which might pull a newer image with the same release tag.
