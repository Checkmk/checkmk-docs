...
Since {CMK} with CMC does not rely on an external binary, there is no need to spawn a new process for each ping packet sent, thus dramatically decreasing the resources overhead.
Additionally the default behavior of `icmpsender` differs from its Nagios counterpart.
Instead of multiple packets in rapid series that are waited upon, `icmpsender` sends only _a single_ ICMP packet per host once every n-seconds interval (defaults to 6 seconds, configurable via the [.guihint]#Normal check interval for host checks# rule set).
This behavior drastically reduces the amount of traffic going through the cable.
// MFS: Removed, since no details on number of hosts and exact configuration are mentioned.
//, which is benchmarked to be up to 45 MBit/sec! 
...
It considers incoming ping packets from a host to be successful host checks, thus marking it as being {UP}. 
...
If it receives such packages, the host is considered to be {UP}.
...
Note: `icmpreceiver` will ignore any SNMP packet because SNMP does not communicate using TCP, relying on UDP instead.
...
In such condition multiple service checks could returns problems that depend on the hosts down state, resulting in the bad behavior of sending a notification.
...
In {CRE}, link:http://docs.pnp4nagios.org/[PNP4Nagios^] from Jörg Linge is used for this, 
