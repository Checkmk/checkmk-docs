...
:description: The {CMK} Micro Core (CMC) differs in some details from other Nagios-compatible cores. The most important differences are described in detail here.
...
This helper creates the files either in a format compatible with PNP or, alternatively, in the new {CMK} format (only applies to new installations).
