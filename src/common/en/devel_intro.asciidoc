...
Optionally, special agents—if they are implemented in Python—can access the password store directly.
This is explained in the separate article xref:devel_password_store#[_The password store for developers_].
