// -*- coding: utf-8 -*-
// IGNORE Securables datafile Msdb Tempdb
// NONASCII
...
ifdef::saas[]
The setup has been very simplified in {CE}
endif::[]
...
The plugin is then installed as usual via the xref:wato_monitoringagents#[Agent Bakery]: Bake and sign the agent, and then either download it manually and install it on the relevant hosts, or, if configured, wait for or trigger the xref:agent_deployment#[automatic agent updates].
...
You can use [.guihint]#Connection -> Exclude system databases# (or `exclude_databases` in the `mk-sql.yml` file) to exclude certain system databases that may be inaccessible (currently the databases Model, Master, Msdb, and Tempdb).
