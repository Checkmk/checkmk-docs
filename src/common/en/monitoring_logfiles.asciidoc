...
{cee-only}
Users of our commercial editions can select [.guihint]#Text logfiles (Linux, Solaris, Windows)# during xref:configuration[Configuration of the rule] [.guihint]#Deploy the Logwatch plugin and its configuration# to automatically deploy the agent plug-in with the agent.
...
{cee-only}
In the commercial editions, first call up the rule for the agent plug-in [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Text logfiles (Linux, Solaris, Windows)#.
