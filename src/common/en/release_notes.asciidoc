...
After installing {CMK} a _Software Bill of Materials (SBOM)_ is available  at `share/doc/bill-of-materials.json` or `share/doc/bill-of-materials.csv`.
If you want to check the SBOM before download and installation, you will find a JSON file and a CSV file,  along with the corresponding hashes in the download directory.
These are named according to the edition and version, for example link: https://download.checkmk.com/checkmk/{current}/check-mk-community-{current}-bill-of-materials.json[check-mk-community-{current}-bill-of-materials.json^] for {RE} {current}.
endif::[]
