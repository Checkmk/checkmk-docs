...
[TIP]
====
You can monitor OpenShift clusters with {CE-saas} using the xref:glossar#checkmk_relay[{CMK} Relay].
...
ifdef::saas[]
With {CE}, you can monitor the following objects:
endif::[]
...
ifdef::saas[]
{CE} is supplied with six built-in dashboards for Kubernetes.
endif::[]
