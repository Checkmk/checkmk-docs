// -*- coding: utf-8 -*-
// IGNORE backends siloed
// NONASCII 
include::global_attr.adoc[]
= The {CMK} metric backend
:title: The {CMK} metric backend
:description: The {CMK} metric backend provides efficient background storage for monitoring data in dynamic environments.
...
ifdef::onprem[]
{cee-only} Until now, {CUE}
endif::[]
ifdef::saas[]
Until now, {CMK}
endif::[]
has stored metrics in xref:graphing#rrds[Round Robin Databases ](RRDs for short), event information in SQLite files, and inventory data in JSON files.
...
ifdef::onprem[]
Furthermore, the approach taken in {CMK} {v24}--making OpenTelemetry data available in {CMK} via a special agent using 'traditional' methods--came with a certain overhead in terms of conversion and copying operations.
endif::[]
...
decided to initially use a column-based database as the metric backend for OpenTelemetry data.
...
Currently, the metric backend is used only for xref:opentelemetry#[OpenTelemetry metrics].
For future versions, we reserve the right to use the metric backend for additional monitoring data in addition to RRDs.
Where RRDs satisfactorily fulfill their purpose, {CMK} will continue to use them in the future.

ifdef::saas[]
No explicit setup is required.
{com} operates one ClickHouse cluster per AWS region.
When your {CMK} xref:glossar#site[site] was created, the database associated with it was also automatically created in this cluster.
endif::[]
...
Note that any processing of OpenTelemetry _metrics_ requires an enabled metric backend.
...
Therefore, if you send OpenTelemetry metrics to OpenTelemetry Collectors and thus to metric backends of remote sites, these metrics can only be viewed there as a siloed solution.

For upcoming versions of {CMK}, we are therefore evaluating the possibility of creating a central site for the metric backend enabling the use of a distributed ClickHouse cluster.
Details will be available in the road map documentation in due course.

In {CMK} {v25}, the most pragmatic approach is likely to be using the OpenTelemetry Collector and metric backend solely on the central site.
If this approach is not feasible due to network segmentation, you can also run OpenTelemetry Collectors and metric backends as a standalone solution on remote sites.
...
The `checkmk.metrics_samples_*` tables are usually the ones you want to explore.

.With a basic understanding of SQL you can determine whether expected data is landing in the metric backend
image::metric_backend_sql_console.png[alt="With a basic understanding of SQL, you can determine whether expected data is landing in the metric backend.",fullscreen=1]
endif::[]
...
A _Custom Metric_ corresponds to a time series of data identified by _Service Name_, _Metric Name_ (in the terminology of the metric backend or the OpenTelemetry Collector), and other attributes.
You can view the current number of Custom Metrics at any time via the [.guihint]#OMD mysite metric backend# service in your {CMK} site's self-monitoring.
