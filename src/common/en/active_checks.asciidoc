...
What both have in common is that they only report states as the host sees them from the inside,
...
Therefore always make sure -- also with the aid of the already-mentioned icon:icon_help[] inline help -- whether this option is available in the specific rule set.
...
Depending on whether you want to run such a plug-in on a unix-type system or on a Windows system, place it in the appropriate location in the respective agent’s installation directory.
