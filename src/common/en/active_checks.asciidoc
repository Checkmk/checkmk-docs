...
.Units of measurement for active checks
[%collapsible]
====
{CMK} does not evaluate any returned units of measurement (UOM for short) when Nagios-compatible plug-ins are called.

The documentation from 'monitoring-plugins.org' for link:https://www.monitoring-plugins.org/doc/guidelines.html#AEN197[_Performance data_^] explicitly states that values passed here are forwarded to third-party applications (in this case {CMK}'s metrics and graphing system), and that these applications are free to determine how the data is evaluated.

In {CMK}, however, values are mapped via metric identifiers.
To avoid duplicating effort, and due to the wide variety of units of measurement used in classic monitoring plug-ins, {CMK} has decided to ignore Nagios' units of measurement.

If you implement a classic monitoring plug-in yourself, you can choose the metric name so that it is mapped to appropriate, existing {CMK} metrics.
If that is not possible, you can create your own metric definitions for the 'performance data' output.
We explain the possible approaches in the article on xref:localchecks#metrics_intro[local checks], and in the article on xref:devel_check_plugins#new_metricdefinition[developing check plug-ins].
====
