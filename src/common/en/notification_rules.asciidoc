...
:description: {CMK} can send notifications about relevant monitoring events via different methods. Read here how to set up the notification rules for your purposes.
...
The content of an email notification sent by {CMK} in HTML format looks something like this:
...
Now comes a next rule with the [.guihint]#SMS# method and the [.guihint]#Suppress all previous# selection.
