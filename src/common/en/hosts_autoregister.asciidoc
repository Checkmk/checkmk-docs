...
By the way, when configuring the folder, you can specify whether the agents for the hosts to be created there should work in {CE}'s xref:glossar#pull_mode[pull mode] or xref:glossar#push_mode[push mode].
...
|`/var/lib/cmk-agent/pre_configured_connections.json` |Contains a pre-configured connection integrated into the agent package via the Agent Bakery to a site for the auto-registration in {CE}.
|===
...
|`C:\ProgramData\checkmk\agentregistered_connections.json` |Contains a pre-configured connection integrated into the agent package via the Agent Bakery to a site for the auto-registration in {CE}.
|===
