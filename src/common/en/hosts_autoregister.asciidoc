...
:description: The {CMK} agent can automatically register objects as hosts to be monitored by a {CMK} site. Read here how to set up the auto-registration.
...
Then comes the {CMK} agent:
Here, the information for a registration request is specified by an agent rule and then packed (baked) into an agent package by the Agent Bakery.
ifdef::onprem[]
This includes selecting the {CMK} site that will monitor the host, which in a xref:glossar.html#distributed_monitoring[distributed monitoring] will usually be a remote site.
endif::[]
...
the xref:user[automation user] for auto-registration ([.guihint]#Authorization#)
and the {CMK} site ([.guihint]#Site to register with#) that is to monitor the host. 
ifdef::onprem[]
In a xref:glossar.html#distributed_monitoring[distributed monitoring], this will usually be a remote site.
endif::[]
...
A xref:glossar#service_discovery[service discovery] is then performed and the xref:glossar#activate_changes[pending changes activated],
...
For a fault diagnosis beyond this, for further information see the articles on the xref:agent_linux#test[Linux agent] and the xref:agent_windows.html[Windows agent]—among others—on the topics of communication between an agent and a {CMK} site, as well as on (manual) registration.
