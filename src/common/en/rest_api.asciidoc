...
Die REST-API gibt zu jeder Anfrage den link:https://en.wikipedia.org/wiki/List_of_HTTP_status_codes[HTTP-Status-Code^] zurück, mit dem Sie überprüfen können, ob die Anfrage erfolgreich war.
...
// end translation
// delete from here
Under `links`, the API returns a selection of requests (heavily truncated in the above example) which can be applied to the host just created -- as befits a REST API.
...
The output is shortened and only the important lines are marked: the header line with the ETag, the attributes of the one pending change to create a host and the HTTP status code for `OK`.
