...
Maybe that sounds a bit bold for you.
Is it not risky that when items that should be present but are missing, are just silently omitted from the reporting?
...
Can you actually xref:glossar#notification[notify] of state changes in BI aggregations?
...
and these can in turn of course trigger notifications.
...
=== Notifications via special agents

We will start with the xref:glossar#special_agent[‘special agent‘] method,
because this is always good if you wish to generate more than a handful of aggregations as services.
...
Notification with an active check is more or less the more direct way, and it requires no artificial 'helper host' when executing the special agent,
since it has to query each aggregation individually, but with larger numbers of aggregations it is significantly less efficient and also more complicated to set up.

To put all of this simply:
There is an active check which can retrieve the state of BI aggregations using HTTP from {CMK}'s xref:rest_api#[REST API].
You can easily set this up with the [.guihint]#Setup > Services > Other services > Check State of BI Aggregation# rule set:
...
* The user must be an xref:glossar#automation_user[automation user]
-- only such users may call the REST API.
The `automation` user is ideal for this as it is always created automatically for such purposes.

* At [.guihint]#Automation Secret# enter the user's [.guihint]#Automation secret for machine accounts#, which you can find in the user properties (only if you use another automation user than `automation`).

In the example [.guihint]#Automatically track downtimes of aggregation# is activated.
Strictly speaking, this means the xref:glossar#scheduled_downtime[_scheduled_ downtimes].
This will make the new active service automatically get a scheduled downtime, even if the BI aggregation also does this.
