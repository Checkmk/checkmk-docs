...
_BI_ module in {CMK} pretty well. It is all about deriving the overall state of *business-critical applications* from the many individual state values,
...
[.guihint]#Count the number of nodes in state OK# to get the number of subnodes with state
...
have the {OK} state will be displayed in the trees. This will then look like this:
...
You can move the tree freely by clicking on the background, and scale the entire display using the mouse wheel. As soon as the mouse pointer is over an individual node, you get the node's associated state information
...
switches 1 and 2 so that each has a subnode showing the host state *and* the two uplink interfaces.
...
And never forget that you can always use icon:icon_help[] to get inline help.
...
ensure that the state of an aggregation is always the same for everyone.
...
To understand which rule BI calculates the „in maintenance“ state, it helps when you are
...
Important: the _real_ current state of the hosts/services plays _no_ role in the calculation - what is in maintenance is assumed to be {CRIT} in the BI logic. Why? Because an {UP} or {OK} state during a maintenance period is pure coincidence, for example if a host reports {UP} for a few seconds in between several restarts.
...
For the `Infrastructure` node this has no effect, because `switch-2` is _not_ in maintenance,
and thus `Infrastructure` is also not in maintenance.
There is therefore no icon icon:icon_derived_downtime[alt="Icon of a service's scheduled downtime derived from the host."] for derived scheduled downtimes.
...
least one important problem that has not been acknowledged, and thus the {OK} state
...
The aggregation would then consist of 4 services with state {OK}, but the aggregation itself would be {WARN} or {CRIT} -- without providing any indication of the reason.
...
In other words, while the rules of an aggregation indicate its state, frozen aggregations inform of state changes.
...
If you, for example, have used [.guihint]#worst# everywhere, the state overall
...
=== Active checks or special agents

[{image-left}]
image::bi_large_icon_notifications.png[width=80]

Can you actually xref:notifications#[notify] of state changes in BI aggregations?
...
* Using the special agent [.guihint]#Check state of BI Aggregations#
* With active checks of the [.guihint]#Check State of BI Aggregation# type


=== Notifications via a special agent

We will start with the xref:special_agents#[special agent] method,
...
You do not necessarily have to stick to the host which the special agent is assigned to
...
*Important:* If the host to which you assign this rule should continue to be monitored through the normal agent, ensure in its settings that regular agent *and* data special agents are run:
