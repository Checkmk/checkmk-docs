...
Or you have a queue for current requests (_Current requests queue_), which you simply want to call `current` - the result would be the assignment of the metric definition for link:https://github.com/Checkmk/checkmk/blob/{current-major}/cmk/plugins/collection/graphing/current.py[amperage^].
