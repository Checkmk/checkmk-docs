...
Or you have a queue for current requests (_Current requests queue_), which you simply want to call `current`&mdash;the result would be the assignment of the metric definition for link:https://github.com/Checkmk/checkmk/blob/{current-major}/cmk/plugins/collection/graphing/current.py[amperage^].
...
[TIP]
====
While the exit code is irrelevant for local checks executed synchronously, in asynchronous execution it determines whether the check was successful and thus whether the result may be cached.
An exit code other than `0` will trigger the check to run again at the next check interval.
====
