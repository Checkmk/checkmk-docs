...
:description: {CMK} can monitor files with respect to their age, size and count. Read here how to achieve this by working with the rule sets mk_filestats and fileinfo.
...
Both variants of service rules are additionally available as xref:wato_services#enforced_services[enforced services].
...
Once completed add the new service to the affected hosts and activate the pending changes as usual.
