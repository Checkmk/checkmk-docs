...
{cee-only}
In the commercial editions, first call the rule for the agent plug-in [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Count, size and age of files - mk_filestats (Linux)#.
...
{cre-only}
In the {CRE} you configure the plug-in as usual using a text file:
...
{cre-only}
In the {CRE} with fileinfo you must also configure using files, which vary according to the operating system:
