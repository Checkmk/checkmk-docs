...
ifdef::onprem[]
On some systems, the {CMK} agent cannot be installed and SNMP is not -- satisfactorily -- available.
endif::[]
ifdef::saas[]
On some systems, the {CMK} agent cannot be installed.
endif::[]
Nevertheless, {CMK} can also monitor such a system -- as long as there is an application programming interface (API) with which {CMK} can retrieve the monitoring data.
Such an API is usually based on Telnet, SSH or HTTP/XML.

ifdef::onprem[]
You do not have to worry about querying these APIs yourself, because {CMK} supplies the necessary so-called xref:datasource_programs#[datasource programs] for frequently required systems,
endif::[]
ifdef::saas[]
You do not have to worry about querying these APIs yourself, because {CMK} supplies the necessary so-called _datasource programs_ for frequently required systems,
endif::[]
...
image::special_agents_program_vcenter.png[alt="Query type options in the VMware ESXi configuration."]
