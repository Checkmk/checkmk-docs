...
Using the Vanilla agents can be useful for debugging or if you want to create operating system images that can be used independently of a {CMK} edition or site.
See xref:post_install[more later].
endif::[]
...
There you will find the packages prepared for auto-registration in the folders that have been configured for auto-registration.
By default, this is the `preconfigured_agent` folder.
The agent is registered here automatically after the agent package has been installed.
Manual registration, as described in the following xref:registration[Registration chapter], is _not_ necessary.
...
ifdef::saas[]
You have the option of creating your own packages for the xref:hosts_autoregister#[auto-registration] in addition to the supplied packages.
In addition to a user with the authorization to register hosts, you need at least one folder that is configured to contain the hosts to be created automatically.
endif::[]
