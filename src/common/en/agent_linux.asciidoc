...
:description: The Agent Controller sends the monitoring data collected by the Linux agent to {CMK}. Here you can learn how to set up the monitoring for Linux hosts.
:experimental:
...
The *agent script* is accompanied by the *Agent Controller*, a component that communicates securely with the *Agent Receiver*, a process running on the {CMK} server.
To do this, the Agent Controller registers itself with the *Agent Receiver*.
The Linux agent can perform functions with the Agent Controller such as TLS encryption of communication or data compression, and is designed to accommodate future enhancements.
...
However, the agent also masters a so-called *legacy mode* to support Linux systems with a different computer architecture than `x86_64` or `aarch64`, without RPM or DEB package management and without the `systemd` init system.
...
Because push mode requires the Agent Controller, installation is currently only possible on modern Linux systems that use `systemd` and which are based on the `x86_64` or `aarch64` architectures.
...
Both package types are available for different CPU architectures:

- Packages with names containing `all` or `noarch` can be installed on any Linux system, however they only contain the Agent Controller for `x86_64` (also called `amd64`), when installed on other CPU architectures, the Agent will only work in xref:agent_linux_legacy#[legacy mode]

- Packages with names containing  `aarch64` or  `arm64` can only be installed on Linux systems that are running on a 64 bit ARM CPU, on such systems they also activate the Agent Controller if `systemd` is used as init system
...
For example, the included DEB package with the Linux agent can be downloaded using the following `curl` command, which authenticates using an xref:glossar#automation_user[automation user].
...
==== CPU architecture and folder hierarchy

The CPU architecture (`aarch64`/`arm64` or `x86_64`/`amd64`, or neither, or both) of the Agent Controller included in packages created with the Agent Bakery can be specified using the rule  
[.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Customize agent package (Linux)#.
Similarly, you can configure settings for installation in a single directory or for unprivileged execution under [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Linux/Unix agent options#.
endif::[]
...
Registering with the Agent Controller requires a Linux system with an init system `systemd` version 219 or later and an `x86_64` or `aarch64` computer architecture.
...
The reason for this is that your Linux uses a different computer architecture than `x86_64` or `aarch64`, for example the older _32-bit x86_.
...
An _out of the box_ monitoring with {CE-saas} is thus not possible in such a case.
====
endif::[]
...
The biggest difference here is the modified `register-new` subcommand, which is used to request registration _and_ creation of a new host in the {CMK} site.
...
Responsible for this is the rule set [.guihint]#Setup > Agents > Windows, Linux Solaris, AIX > Agent Rules > Generic agent options > Execute MRPE checks#.
...
. Create a rule in [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Generic agent options > Deploy custom files with agent# .
