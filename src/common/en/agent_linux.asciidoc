...
// MFS: Change sentence structure for LLM translation.
...
An exception to this are the packages preconfigured for the xref:hosts_autoregister#[auto-registration] and downloaded via the Agent Bakery (feature only available in {CE} and {ME}).
...
// This block replaces the rest of this section 
// start translation
////
...
For push hosts configured on a site running the {CCE} this soft link points to the folder with the agent output, for pull hosts it points to a non-existent file with the name of the host as used in the monitoring.
