...
|`--checkmk-crashes` | Latest crash report from each subdirectory of `~/var/checkmk/crashes/`.
|`--checkmk-overview` |Information from the HW/SW inventory node [.guihint]#Software > Applications > {CMK}# of the {CMK} servers.
|`--checkmk-config-files FILE,FILE ...` |Configuration files from the `~/etc/checkmk/` directory. All desired files must be specified explicitly. The file names must be specified relative to `~/etc/checkmk/`.

Example: `--checkmk-config-files main.mk,apache.d/wato/global.mk`
|`--checkmk-log-files FILE,FILE ...` |Log files (`.log` and `.state`) from `~/var/log/`. All desired files must be specified explicitly. The file names must be specified relative to `~/var/log/`.

Example: `--checkmk-log-files apache/error_log,update.log,web.log`
|`--performance-graphs H` |Performance graphs (e.g. CPU load, CPU utilization) of the {CMK} server -- only available in the commercial editions. For the parameter `H`, you must specify the host name of the Checkmk server.
|===
