...
* The icon:icon_ack[alt="Icon indicating acknowledgement."] xref:basics_ackn#[acknowledgement of a problem] by a user.
...
Because there are numerous different implementations for this command under Linux, for standardization {CMK} provides the version
from the link:https://heirloom.sourceforge.net/mailx.html[Heirloom mailx^] project directly in the site user’s search path (as `~/bin/mail`).
...
For testing notifications, {CMK} offers a smart tool with [.guihint]#Test notifications#.
...
In addition, there are other options for calling from some table views in Monitoring (service list and service details) and in Setup (properties of a host), in each case in the menu [.guihint]#Host > Test notifications.#
...
You can select a state change or the start of a xref:glossar#scheduled_downtime[maintenance time] as the event.
...
If you have selected sending, the corresponding message 'Notifications have been sent' is displayed here.
This must then immediately lead to an e-mail for this problem.

The line below summarizes the notifications generated from your entries.
By clicking on the icon icon:icon_toggle_context[alt="Icon for displaying the notification context."], you can show the notification context.
This allows you to see the environment variables valid in the context of this notification and their values. 
...
Under [.guihint]#Global notification rules# the notification rules are listed, which are presented in more detail in xref:rules[next chapter].
At this point, only the first column of the table is important, which uses symbols to indicate which of the rules apply icon:icon_checkmark[alt="Symbol for displaying a positive status."] and which do not icon:icon_hyphen[alt="Symbol for displaying a negative status."].
In the example, the first rule does not apply because it evaluates status changes of hosts, but the event is a status change of a service.

[TIP]
====
As usual, you can continue to trigger notifications directly via the GUI as an alternative to testing notifications via simulation, e.g. with the xref:commands#[commands] [.guihint]#Send custom notification# and [.guihint]#Fake check results#.
====
...
*Note:* When you call up the [.guihint]#Notification configuration# page for the first time, you will see a warning about the non-configured _"fallback email address"_. You can ignore this warning for the moment. We will go into this xref:fallback[below].
...
This rule defines the above-described default behavior. It is structured as follows:
...
*Note:* For many notification methods for forwarding to other systems, you will find more detailed information in separate articles.
The list of articles can be found in the chapter on xref:includedscripts[notification scripts].
...
*Note:* When using a ticket system, a messenger or an event engine as notification method, you should also observe the notes on these xref:ticketsystem[special cases].
...
{cre-only}
If you are using the {RE} (or one of the commercial editions with a Nagios core), in isolated cases it can nonetheless occur that a host problem
...
* In the commercial editions, using [.guihint]#Global settings > Monitoring Core > Tuning of flap detection# you can define the parameters for flapping detection and set them to be more or less sensitive:
...
{CMK} can be set up so that successive notifications are issued at fixed intervals, until either the problem has been icon:icon_ack[alt="Icon to indicate acknowledgement."] xref:basics_ackn#[acknowledged] or solved.
...
You can manually trigger this state change with the xref:commands#[command] [.guihint]#Fake check results# for testing purposes.
...
|xref:cmc#[{CMK} Micro Core (CMC)] |The monitoring core in the commercial editions that performs the same function as Nagios in the {RE}. |`~/var/log/cmc.log`
...
|`?r` |any ➤ OK |Service status changed from any status to {OK}
|`?w` |any ➤ WARN |Service status changed from any status to {WARN}
|`?c` |any ➤ CRIT |Service status changed from any status to {CRIT}
|`?u` |any ➤ UNKNOWN |Service status changed from any status to {UNKNOWN}
|===
...
Now to test, set the service `CPU load` on the host `myserver` to {CRIT} -- with the xref:commands#[command] [.guihint]#Fake check results# as described xref:notification_testing[above].
