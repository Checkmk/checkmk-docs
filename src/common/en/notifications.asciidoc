...
[TIP]
====
In {CMK} {v23} [.guihint]#Test notifications# can test all notifications *rules* and send out *email* notifications.

If you want to test the actual sending of anything other than email, you can still use xref:#fake_check_results[Fake check results].
====
...
In addition to the simulation, you can also have the email notification sent.
...
Use the [.guihint]#Send out HTML/ASCII email notification according to notification rules# checkbox to specify whether the notification is only simulated or actually sent.
...
Click on [.guihint]#Test notifications# to start the test -- and also the email dispatch, if you have selected this option.
...
[#fake_check_results]
== Testing notifications with [.guihint]#Fake check results#

To test the notification methods other than email you could simply set an {OK} service to {CRIT} manually.
...
If the command is not visible in the list, please click the button [.guihint]#Show more# icon:button_showmore[] once.

image::notifications_fake_check_results.png[alt="Dialog for manual state change."]

If all the conditions of one of your notification rules are met, the notification method set for this rule is executed.
