...
:description: Notifications in {CMK} actively inform individual users or groups about monitoring events. Read here how to configure and use notifications.
...
The configuration is performed using the [.guihint]#Monitoring Configuration > Notification period for hosts#, or respectively the [.guihint]#Notification period for services# rule set, which you can quickly find via the xref:user_interface#search[search] in the [.guihint]#Setup# menu.
...
Since commands--in contrast to rules--require neither configuration permissions nor an xref:glossar#activate_changes[activate pending changes], they can be a quick workaround for reacting promptly to a situation.
...
The raw notification does not yet contain details of the actual contacts or of the notification method.
...
* `check-mk-notify` is given as the command.
...
|`rd` | [.guihint]#UP ➤ DOWN# |Host state changed from {UP} to {DOWN}
|`ru` | [.guihint]#UP ➤ UNREACHABLE# |Host state changed from {UP} to {UNREACH}
|`dr` | [.guihint]#DOWN ➤ UP# |Host state changed from {DOWN} to {UP}
|`du` | [.guihint]#DOWN ➤ UNREACHABLE# |Host state changed from {DOWN} to {UNREACH}
|`ud` | [.guihint]#UNREACHABLE ➤ DOWN# |Host state changed from {UNREACH} to {DOWN}
|`ur` | [.guihint]#UNREACHABLE ➤ UP# |Host state changed from {UNREACH} to {UP}
|`?r` | [.guihint]#any ➤ UP# |Host state changed from any state to {UP}
|`?d` | [.guihint]#any ➤ DOWN# |Host state changed from any state to {DOWN}
|`?u` | [.guihint]#any ➤ UNREACHABLE# |Host state changed from any state to {UNREACH}
|`f`  | [.guihint]#Start or end of flapping state# |
|`s`  | [.guihint]#Start or end of a scheduled downtime# |
|`x`  | [.guihint]#Acknowledgment of problem# |
|`as` | [.guihint]#Alert handler execution, successful# |
|`af` | [.guihint]#Alert handler execution, failed# |
|===
...
|*Abbreviation* |*Meaning* |*Description*
|`rw` | [.guihint]#OK ➤ WARN# |Service state changed from {OK} to {WARN}
|`rr` | [.guihint]#OK ➤ OK# |Service state changed from {OK} to {OK}
|`rc` | [.guihint]#OK ➤ CRIT# |Service state changed from {OK} to {CRIT}
|`ru` | [.guihint]#OK ➤ UNKNOWN# |Service state changed from {OK} to {UNKNOWN}
|`wr` | [.guihint]#WARN ➤ OK# |Service state changed from {WARN} to {OK}
|`wc` | [.guihint]#WARN ➤ CRIT# |Service state changed from {WARN} to {CRIT}
|`wu` | [.guihint]#WARN ➤ UNKNOWN# |Service state changed from {WARN} to {UNKNOWN}
|`cr` | [.guihint]#CRIT ➤ OK# |Service state changed from {CRIT} to {OK}
|`cw` | [.guihint]#CRIT ➤ WARN# |Service state changed from {CRIT} to {WARN}
|`cu` | [.guihint]#CRIT ➤ UNKNOWN# |Service state changed from {CRIT} to {UNKNOWN}
|`ur` | [.guihint]#UNKNOWN ➤ OK# |Service state changed from {UNKNOWN} to {OK}
|`uw` | [.guihint]#UNKNOWN ➤ WARN# |Service state changed from {UNKNOWN} to {WARN}
|`uc` | [.guihint]#UNKNOWN ➤ CRIT# |Service state changed from {UNKNOWN} to {CRIT}
|`?r` | [.guihint]#any ➤ OK# |Service state changed from any state to {OK}
|`?w` | [.guihint]#any ➤ WARN# |Service state changed from any state to {WARN}
|`?c` | [.guihint]#any ➤ CRIT# |Service state changed from any state to {CRIT}
|`?u` | [.guihint]#any ➤ UNKNOWN# |Service state changed from any state to {UNKNOWN}
|===
...
A synchronous delivery is then feasible if the notification script runs quickly, and above all cannot lead to some sort of timeout. With notification methods that access existing spoolers that is a given.
...
If the email really cannot be delivered the SMTP server creates a nice error message in its log file and attempts to generate an error email to the 'sender'.
...
|`NOTIFY_CONTACTNAME` |Username (login) of the contact.
...
For this reason a delivery using environment variables also does not function practicably.
...
|`~/var/log/mknotifyd.log` |The notification spooler's log file.
|`~/var/log/mknotifyd.state` |The current state of the notification spooler. This is primarily relevant for xref:distributed_monitoring#notifications[notifications in distributed environments].
