...
*Note:*
ifdef::onprem[]
When using a ticket system, a messenger or an event engine
endif::[]
ifdef::saas[]
When using a ticket system or a messenger
endif::[]
...
In order to reduce false alarms, but still process genuine notifications, 
ifdef::onprem[]
it 
endif::[]
ifdef::saas[]
...
pays very close attention to the exact times of the
...
keeps the module on standby as a permanent auxiliary process (_notification helper_), thus reducing process-creation and saving machine time.
