...
These are the Open Source {CRE} (formerly {CMK} Raw), for professional users the {CSE} (formerly {CMK} Enterprise) available by subscription, the {CCE} (formerly {CMK} Cloud (Self-hosted)) based on the {SE}, and finally the {CME} (formerly {CMK} MSP).
