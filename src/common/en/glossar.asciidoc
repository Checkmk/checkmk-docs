...
The activation of pending changes is accessed via the [.guihint]#Changes# icon in the navigation bar.

More under xref:wato#activate_changes[Activate pending changes].
...
[#dashboard]
==== Dashboard
A dashboard is a freely-configurable overview consisting of views and/or so-called _widgets_.
...
These are the open-source variant {CRE} (formerly {CMK} Raw), for professional users the {CSE} (formerly {CMK} Enterprise) available by subscription, and the {CCE} (formerly {CMK} Cloud (Self-hosted)) which is based on {SE}.
...
All commercial editions can be tested for the first 30 days (in license state 'Trial') in full functionality without obligation.
On a small scale—that is, with a _single_ site and up to 750 services—{CMK} as a commercial edition can also be operated permanently beyond that period in license state 'Free' without requiring a subscription.
...
Supplied classic views can be customized, and they can also serve a basis for creating your own views.
It is also possible to create classic views from scratch.
