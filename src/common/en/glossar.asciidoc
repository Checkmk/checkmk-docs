...
ifdef::saas[]
This agent can be a small program installed on the host (the {CMK} agent), a special agent that obtains the information through an API provided by the target system -- or an active check that queries network-based services.
endif::[]
...
ifdef::saas[]
With the Agent Bakery, agents can be individually packaged and optionally also distributed automatically.
endif::[]
...
Several automation users are already set up in a fresh {CMK} site, e.g. for web services and for registering the agent with the {CMK} server for TLS-encrypted data transfer.
...
[#check]
==== Check
A check in the context of {CMK} is a script or program that checks a host or service according to predefined rules,
...
ifdef::onprem[]
For example, the check plug-in _df_, found within a site at `~/local/lib/python3/cmk_addons/plugins/<plug-in_family>/agent_based/` (when using the Check API V2) or `~/local/lib/check_mk/base/plugins/agent_based/` (when using the Check API V1), creates services for a host's existing mounted file systems from data from an agent in the site, and checks those services against the data, such as how much free space is left.
endif::[]
ifdef::saas[]
For example, the check plug-in _df_ creates services for a host's existing mounted file systems from data from an agent in the site, and checks those services against the data, such as how much free space is left.
endif::[]
...
They are the Open Source {CRE}, for professional users the {CSE} available by subscription, the {CCE} based on it, and the multi-tenant {CME}.
...
For example, {CE} is an enhanced and optimized {SE} for cloud environments.
...
// This block is new
// start proofreading
If an object (host or service) changes its state several times over a short time, it is regarded as flapping and is marked with the icon:icon_flapping[alt=“Icon for displaying the flapping state.”] icon.
For a flapping object successive state changes trigger no further notifications.
...
ifdef::saas[]
More under xref:admin_panel#[The admin panel.]
endif::[]
