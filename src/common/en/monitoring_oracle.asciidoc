...
We extend these plug-ins regularly, so it is always worth looking in the catalog.
...
For this reason, we will first describe the general steps, then the specific steps for the respective operating system group and finally the xref:glossar#agent_bakery[Agent Bakery] of the commercial editions.


[#first_steps]
== First steps for initial setup

The configuration files with sample content presented in this and the following chapters can be found on the {CMK} server --
either via xref:files_cmk[command line] or via the {CMK} web interface.
...
Provided that you are not using xref:rac[Real Application Cluster (RAC)], create a user for each database to be monitored.
How to access an instance differs depending on the installed operating system.
For Linux, you can do this for example, by always first setting the relevant instance as an environment variable for which a user is to be created.
...
The assignment of permissions is a bit different as for regular users as you need to specify them for all current containers and for all in the future:
...
Now create on the Oracle host a new configuration file `mk_oracle.cfg` for xref:linux[Linux, AIX, Solaris] or `mk_oracle_cfg.ps1` for xref:windows[Windows].
In the following example the file for unix-type systems can be seen:
...
All other options that you can set in xref:adv_linux[unix-type systems] or under xref:adv_windows[Windows] are optional.


[#oracle_wallet]
=== Using the Oracle Wallet

As an alternative to specify the user directly and with a password in a configuration file, you can also use the _Oracle Wallet_.
...
With this last step you have created the prerequisites for removing the access data from the xref:user_config[agent plug-in's configuration file].
...
Under unix-type systems {CMK} uses a uniform agent plug-in.
...
[IMPORTANT]
====
The agent plug-in for unix-type systems `mk_oracle` does not work well with `systemd` (see link:https://checkmk.com/werk/13732[Werk pass:[#]13732^]).
...
We recommend execution every minute, i.e. `/usr/lib/check_mk_agent/plugins/60/`.
...
However, depending on the application scenario, you will quickly need further possibilities for better, individual control of the monitoring per instance.
...
With the standard login you can use regular or maybe even all instances of a database.
...
* If an optional field is omitted in the middle of the string, a colon must be coded, as with the `DBUSER_MYINST2` entry, where no special role is specified.
* If after a certain point, no more optional fields are needed, you can omit the colons, as with the `ASMUSER` entry, where neither the host nor the port is specified.
...
*Important:* For ASM SIDs you cannot use this concept but you can use `SKIP_SIDS="+ASM1 ..."` instead.
|===
...
The operational purposes are diverse, and it is especially practical when it is undesirable to have certain long-running sections to be included in everything, or only basic information from test instances is required.
...
|`SYNC_ASM_SECTIONS` |Here the same mechanism for ASM sections applies that is generally described for the `SYNC_SECTIONS` variable.
|`ASYNC_ASM_SECTIONS` |Here the same mechanism for ASM sections applies that is generally described for the `ASYNC_SECTIONS` variable.
...
In the instance `INST2` in turn only three of the four asynchronous sections are queried, since `jobs` was additionally excluded.
...
`TNS_ADMIN` is an environment variable that points to the directory where Oracle configuration files such as `sqlnet.ora` and `tnsnames.ora` are located.
By default, `TNS_ADMIN` is set to `$ORACLE_HOME/network/admin`.
You can set `TNS_ADMIN` in the configuration file as follows:
...
[#access_linux]
==== Access rights during execution
...
The non-privileged user requires the following permissions:

* Read the file `$TNS_ADMIN/sqlnet.ora`.
* Read the file `$TNS_ADMIN/tnsnames.ora`.
* Execute the wallet folder. +
The wallet folder is `/etc/check_mk/oracle_wallet/` if you have proceeded as described in the section on xref:oracle_wallet[Oracle Wallet].
* Read the files in the wallet folder.

As a rule, the non-privileged user will be `oracle`.
However, this depends on your Oracle installation.
...
Under unix-type systems, you can not only retrieve locally running instances, but also log on to remote ones and retrieve the databases running there.
...
For monitoring remote databases, there are the following requirements:
...
As a rule, the conditions are already fulfilled when an Oracle installation is present on the host on which the agent plug-in is to be executed.
...
In the example, two remote instances are being configured in two lines.
So that each text line is unique, each variable is given an ID.
...
The first new value `PIGGYBACKHOST` is set to `myOracleHost` for the instance `MYINST3`.
...
The version specification therefore also cannot be omitted, and it determines which SQL queries can be passed to the instance.
...
* You cannot -- understandably -- xref:skip_include[exclude] remote instances with `SKIP_SIDS`, and in return you do not need to include them using `ONLY_SIDS`.

* Instances with the same name (`SID`) must not be assigned to the same host.
...
The functionality is similar to the agent plug-in under xref:linux[unix-type systems], but contains only a part of this.
To use the agent plug-in under Windows you need the following paths, if you have not changed them.
...
Administrators under Windows are the `Local System` account and members of the built-in `Administrators` group.
...
Therefore if you want to monitor remote databases, you need a host with a compatible xref:linux[unix-type operating system].
...
On the one hand, when you are dealing with obsolete configuration options that are only available for compatibility reasons.
These can then be replaced with the current options accordingly.
On the other hand, if these are functions that require a major intervention and in-depth specialist knowledge.
Accordingly, the xref:custom_sqls[custom SQL queries] cannot be configured directly in the Agent Bakery.
...
In the xref:user_config[simplest configuration] for a unix-type operating system, the rule will look something like this:
...
|[.guihint]#Host uses xinetd or systemd (Linux/AIX/Solaris only)# |This option must be activated for unix-type systems with `xinetd`/`systemd`.
...
|[.guihint]#Sqlnet Send timeout# |This option is added to the `sqlnet.ora` file and sets a timeout for the instance itself.
...
These only affect the specified ID and must be unique.
...
|[.guihint]#sqlnet.ora permission group (Linux/AIX/Solaris only)# |Enter here the Linux group of the non-privileged user who executes the Oracle binaries so that this user can read the `sqlnet.ora` file created by the Agent Bakery.
For a standard Oracle installation, `oinstall` can be used as group. +
Further information can be found in the section xref:access_linux[Access rights during execution] for Linux.
...
You can find more information on this topic in the section xref:access_windows[Access rights during execution] for Windows.
|===
...
Note that the plug-in must be provided with the path to its configuration file beforehand, as it relies on this.
...
Again, you will get information about how the plug-in tries to access the instances. An output may then look like this, for example:
...
Again, you will need the path to the configuration file, and also the path to the plug-in's cached data.
...
You can call up debugging with the `-d` option.
Do not forget the necessary environment variable here either: 
...
However, since you cannot pass any arguments to the agent plug-in, you have to switch on tracing manually:
...
|===
|File path |Description
|`~/share/check_mk/agents/plugins/mk_oracle` |The agent plug-in for unix-type systems, which fetches the data on the Oracle host.
|`~/share/check_mk/agents/plugins/mk_oracle_crs` |This agent plug-in for unix-type systems provides data to an Oracle Cluster Manager.
...
|`~/share/check_mk/agents/cfg_examples/` |Here are sample configuration files for unix-type systems in the files `mk_oracle.cfg`, `sqlnet.ora` and `tnsnames.ora`.
