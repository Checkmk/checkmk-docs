...
Make the owner and group of the Oracle binaries (such as `$ORACLE_HOME/bin/sqlplus`) executed by the `mk_oracle` agent plug-in the owner and group of the manually created files and directories in `/etc/check_mk/`.
For example, the following commands change the owner to `oracle` and the group to `oinstall`:
...
The permissions for the owner and group should then look something like this:
...
You should not have any problems with the access rights as long as you have changed the owner and group of the files and directories as described at the end of the section on xref:oracle_wallet[Oracle Wallet].
