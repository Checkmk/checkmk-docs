...
In the {RE} select [.guihint]#Setup > Agents# and in the commercial editions [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Related#.
...
So that the agent plug-in can access these files you can specify the path under Linux, Solaris and AIX using the xref:tns_config[`TNSALIAS` extended variable].
...
You can also determine whether the user is a `SYSDBA` or `SYSASM`, on which combination of address and port the instance listens to, and even which `TNSALIAS` should be used.
...
[TIP]
====
Since the legacy agent is only needed in very rare, exceptional cases, it is not considered in the following description.
We therefore always refer to the current agent architecture here.
====
////
...
|[.guihint]#ORACLE_HOME to use for remote access (Linux/AIX/Solaris only)# |Here you can determine where the agent plugin finds the `sqlplus` program.
...
|[.guihint]#sqlnet.ora permission group (Linux/AIX/Solaris only)# |
|[.guihint]#Oracle binaries permissions check (Windows only)# |
|===
...
|`SQLS_DIR` |The path name of the directory in which the files with the cutsom SQL queries are stored. |No
|`SQLS_SQL` |The file that contains the SQL statements for a section. |No
// |`SQLS_PARAMETERS` |tbd |Yes
|`SQLS_SECTION_NAME` |The name of the section that you evaluate with an xref:own_check_plug-in [own check plug-in] for the cutsom SQL queries. |Yes
...
|`/etc/check_mk/tnsnames.ora` |The configuration file which, if you specify it manually, determines an alias for a schema.
...
|`C:\ProgramData\checkmk\agent\config\tnsnames.ora` |The configuration file which, if you specify it manually, determines an alias for a schema.
Sample files are also located in the Oracle installation, but since the path differs from installation to installation, it cannot be specified in a standardized way.
|===
