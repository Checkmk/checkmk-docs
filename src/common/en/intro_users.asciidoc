...
:description: One {CMK} site can be used by many users. Read here how to configure roles for permissions and contact groups for responsibilities.
...
|Empty template for least privilege roles |`no_permissions` |This role has no permissions at all. It is not intended for direct assignment. Instead, it can be used to create new roles with only the minimum required permissions.
|===
...
.The new contact group will apparently be responsible for Windows and Linux servers
...
So create your own new rules here and select the contact group you want to assign to the hosts:

.The contact group is selected in the [.guihint]#Assignment of hosts to contact groups# box
image::intro_rule_assignment_hosts_contact_groups.png[alt="Dialog for assigning contact groups to hosts in a rule."]

You can then specify the hosts to be assigned to this contact group in the following [.guihint]#Conditions# box.

[TIP]
====
If several rules apply to a host, all of them will be evaluated and the host will be assigned to several contact groups in this way.
====
...
.Multiple contact groups can be assigned to a host
...
[TIP]
====
You can set the [.guihint]#Permissions# attribute in the same way at the folder level instead of at the host level.
...
Before we move on to creating users, let’s briefly introduce the associated application: the admin panel.
...
.The {CE} admin panel
image::intro_admin_panel_saas.png[alt=“The admin panel with the overview of the sites.”,fullscreen=1]

Like the {CMK} user interface, the admin panel also has a navigation bar on the left-hand side.

A click on [.guihint]#Sites# shows the xref:glossar#site[sites] set up in {CE}.
During the xref:intro_setup#signup_trial[registration] process, you entered the name of the site, which was then created automatically.
This site is displayed here in the admin panel.
...
Similar to the sites, clicking [.guihint]#User management# in the navigation bar displays a table listing all users.
...
To learn how to create new users here, see the xref:create_users[next section].

By the way, you can find a description of the admin panel functions in the xref:admin_panel#[article of the same name].
endif::[]
...
.Automatically and manually created users in the user table
...
.The new user's identity settings
...
.The new user's security settings
...
.Contact groups are assigned by checking the boxes
...
.A new user needs an email address and one of the predefined xref:roles[roles]	
image::intro_new_user_saas.png[alt=“Dialog for entering the email address and role for the new user.”,width=63%]

Confirm the dialog by clicking on [.guihint]#Create user#.
