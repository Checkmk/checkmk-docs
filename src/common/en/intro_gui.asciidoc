...
[TIP]
====
The [.guihint]#Main dashboard# shown above with problems and statistics of all hosts and services is that of the {RE}.
...
[TIP]
====
The search in the monitoring environment will not return any results until you add the first host to the monitoring.
====
