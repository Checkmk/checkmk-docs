...
ifdef::onprem[]
*Tip:* With the `find_piggy_orphans` script from the `treasures` directory you can find all piggybacked hosts for which there is data but which have not yet been created as hosts in {CMK}:
