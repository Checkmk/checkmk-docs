...
ifdef::onprem[]
*Tip:* With the command xref:piggyback#orphaned_piggyback_data[`cmk-piggyback list orphans`] you can find all piggybacked hosts for which there is data but which have not yet been created as hosts in {CMK}.
endif::[]
