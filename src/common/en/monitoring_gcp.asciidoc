...
:description: {CMK} offers an extensive module for monitoring Google Cloud products and their costs. Read here how to configure and use the integration.
...
In addition to a descriptive name, you can optionally enter a description--a _service account_ description.
...
When you have added the services, it will look something like this after xref:glossar#activate_changes[activating the pending changes] in the monitoring:
...
ifdef::onprem[]
{CCE-only}
For a convenient start into monitoring GCP, {CMK} ships in {CE}
endif::[]
ifdef::saas[]
For a convenient start into monitoring GCP, {CE} ships
endif::[]
