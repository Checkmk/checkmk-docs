...
ifdef::onprem[]
You can find the options for configuring certificates in the [.guihint]#Global settings#.
endif::[]
You can view the certificates generated (or later replaced) in {CMK} when the site was created via the [.guihint]#Certificate overview# page.
...
In some cases, external applications need to communicate with {CMK} components that are secured with the site certificate.
Such applications often refuse to communicate if the host name used to establish the connection is not included in the certificate.
For {UE} version {v25} and later, this only affects the xref:opentelemetry#collector_configuration[OpenTelemetry integration].
However, future versions will include additional components that use the site certificate. 

For this reason, it is advisable that the host names used are included in the certificate.

In the [.guihint]#Global settings#, you can use [.guihint]#Site certificate subject alternative names# to set all host names for which the server certificate should be valid.
endif::[]
