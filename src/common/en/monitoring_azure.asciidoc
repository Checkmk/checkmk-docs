// -*- coding: utf-8 -*-
// IGNORE + % BuiltInRole hyperscalers
...
[IMPORTANT]
====
Before updating to {CMK} {v24}, you must check whether you are using the labels without the `cmk/azure/` prefix in any form.
Be sure to change these to the labels with the prefix *before* the update.
The labels without the prefix will be removed during the first service discovery after the update and rules that rely on these labels, for example, will then become ineffective.
====
