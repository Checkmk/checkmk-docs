...
{CMK} includes a extensive module for monitoring Microsoft Azure, consisting of a xref:glossar#special_agent[special agent] to Azure and a comprehensive collection of check plug-ins that retrieve and evaluate various metrics and statuses for you.
...
== Azure-Monitoring starting with {CMK} {v25}

With {CMK} {v25}, we have completely revamped the monitoring of Microsoft Azure.
In addition to the new special agent, all existing check plug-ins have also been renewed.

[TIP]
====
The special agent and check plug-ins for the new Azure monitoring are *not* compatible with the old version.
It is not possible to carry over existing data, metrics and performance data.
====

For more information on the new special agent, see the corresponding link:https://checkmk.com/werk/19467[Werk #19467^].

Information on setting up and using the old Azure monitoring can be found in the corresponding manual article for {CMK} {v24}: link:https://docs.checkmk.com/2.3.0/de/monitoring_azure.html[Monitoring Microsoft Azure up to {CMK} {v24}].
...
As of today, the API queries that {CMK} requires for monitoring are free of charge with Azure (in contrast to AWS). However, there is a limit on the number of queries per time ('throttling limits').
...
ifdef::onprem[]
{CCE-only}
For a convenient start into Azure monitoring, {CMK} ships in {CE}
endif::[]
ifdef::saas[]
