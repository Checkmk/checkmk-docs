...
[TIP]
====
The [guihint]#Onboarding Guide# of {CE} (SaaS) contains a quick how-to for setting up Microsoft Azure monitoring.
...
ifdef::onprem[]
{CCE-only}
For a convenient start into Azure monitoring, {CE} and above
endif::[]
ifdef::saas[]
For a convenient start into Azure monitoring, {CE} (SaaS)
endif::[]
includes the two built-in xref:glossar#dashboard[dashboards], [.guihint]#Azure VM instances# and [.guihint]#Azure storage accounts#.
