...
You assign email address and contact groups via the user's properties, as we showed earlier in the xref:intro_users#create_users[chapter on user administration], for instance by adding your email address to the `cmkadmin` user account and to the [.guihint]#Everything# contact group.
