...
:description: {CMK} reports state changes of monitored objects via notifications. Read here how to prepare, activate and test notification dispatch by email.
:experimental:
...
{CE} is delivered with a notification rule that via HTML email notifies every contact for important hosts about important status changes for these hosts and their services.
endif::[]
...
The question "Why did not {CMK} notify here?" will therefore be asked more often than you might expect, especially at the beginning.
...
If you are not yet familiar with `less`—with the key combination `Shift+G` you can jump to the end of a file (which is useful for log files), and quit `less` with the kbd:[Q] key.
...
[TIP]
====
If you want to know exactly when notifications are generated--and when they are not--you can learn the whole truth in the article on xref:notifications#system[notifications].
====
