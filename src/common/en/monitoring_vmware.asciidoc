...
:description: With {CMK} you can monitor a vCenter, its ESXi hosts and also the virtual machines (VMs). Read here how to set up the monitoring.
...
[TIP]
====
You can monitor VMware ESXi with {CE-saas} using the xref:glossar#checkmk_relay[{CMK} Relay].
To do this, the host with the VMware ESXi xref:glossar#special_agent[special agent] must be monitored via the {CMK} Relay.
Monitoring with {CE-saas} without the relay requires opening ports to allow access to the VMware Management API from the internet.
...
Tip: To verify whether the user meets the requirements, simply log in to the ESXi server's web interface (VMware Host Client) using that user account.
If the login fails, the user may not have sufficient permissions.

* You must have defined the ESXi server as the host in {CMK}, and configured its monitoring agent as [.guihint]#API integrations if configured, else {CMK} agent#.
...
Activate the pending changes as usual.
ifdef::onprem[]
...
* The vCenter has been defined as a host in {CMK} and its monitoring agent is configured as [.guihint]#API integrations if configured, else {CMK} agent#
...
When searching for the source of an error, there are several places to start.
