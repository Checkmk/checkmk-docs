...
on a host it is possible to query Disk-IO, data store performance, the status
...
Thus the VM related data is found right where it is actually
...
standard agent the xref:glossar#special_agent[special agent]
...
xref:hosts_setup#services[service discovery]. This should find a number of
services:
...
searching for the error. Later it is important that the data gets to the
...
|`~/tmp/check_mk/piggyback/` |{CMK} saves the piggyback data here. For each host a subfolder is created with the host’s name -- this subfolder contains a text file with the host’s data. The filename is the name of the host providing the data.
