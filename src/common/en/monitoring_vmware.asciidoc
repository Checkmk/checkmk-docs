...
The command xref:piggyback#orphaned_piggyback_data[`cmk-piggyback list orphans`] finds all unassigned piggybacked hosts:
...
For each object listed, {CMK} can’t find a host with the same name to which it can allocate the data.
The piggyback names can however be xref:rename_piggyback[altered] in a number of ways.
endif::[]
