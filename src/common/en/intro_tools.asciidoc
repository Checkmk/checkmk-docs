...
:description: {CMK} shows you monitoring data in multiple places, such as snap-ins, dashboards and views. Read here how to use these elements.
...
// delete from here
Now that your monitoring system has something to do, it is time to take a closer look at the elements of the {CMK} user interface that help you in your daily monitoring or operating life. 
// delete to here
// This block is new
// start translation
////
...
This includes dashboards and views that you call up via the [.guihint]#Monitor# menu, the snap-ins in the sidebar, as well as the display and processing of metrics.
...
ifdef::saas[]
image::intro_view_service_filter_saas.png[alt="View with open filter bar."]
endif::[]
...
Take the service that checks the file system `C:` on a Windows server as an example:

.A service for a Windows file system
image::intro_filesystem_c.png[alt="List entry of a service for a Windows server file system."]

In addition to the {OK} status, you can see that approximately, of the file system's total of 200{nbsp}gibibytes, 65 has been used -- which is around 32{nbsp}%.
...
.A time series graph for visualizing metrics
