...
:description: The HW/SW inventory function of {CMK} collects information about hardware and software on monitored hosts. Read here how to perform the inventory check.
...
Do not confuse this with the [.guihint]#HW/SW Inventory (Linux, Windows, Solaris, AIX)# rule set which is for the xref:inventory_plugin[agent plug-in], as we will see later.
