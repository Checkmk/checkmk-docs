...
The three checkboxes below offer exactly the same options that you have in the service list in the setup and that we have already explained above.
In the drop-down menu, you also have the option of selecting [.guihint#Refresh all services (tabula rasa)#.


In [.guihint]#Mode# you will find exactly the same options as in the setup's service list that we have previously discussed:

Im Drop-down-Menü [.guihint]#Parameters# ist die Option [.guihint]#Custom service configuration# vorausgewählt.
Die drei Checkboxen darunter bieten genau die Möglichkeiten, die Sie auch in der Serviceliste im Setup haben und die wir schon weiter oben erläutert haben.
