...
It ensures that the daily changes in a data center are always promptly
...
a combination of the *host name* (`myserver01`), the xref:checkplugins[*check
...
This is however only intended for temporary and smaller actions, as the services deselected in this way will be highlighted as [.guihint]#missing# by {CMK}, and the xref:discovery_check[discovery check] (which we will show you later below) will likewise be unhappy.
...
. Search for hosts with xref:hosts_setup#search[setup host search], select all hosts
...
|[.guihint]#Only include hosts that failed on previous discovery# |Hosts for which an earlier service discovery via bulk actions has failed (e.g. because the host was not accessible at the time), are flagged with the icon:icon_inventory_failed[] symbol. This option allows the discovery to be repeated only for these hosts.
|[.guihint]#Only include hosts with a failed discovery check# |This restricts the discovery to such hosts for which the xref:discovery_check[discovery check] failed. When you work with discovery check this is a good method for greatly accelerating a discovery on many hosts. The combination with the [.guihint]#Refresh all services (tabula rasa)# option makes less sense in this case however as it can distort the status of existing services.
...
We have xref:disabled_services[earlier] shown how you can configure the
...
actively find items, or collect them into groups.
The naming of items is sometimes also an issue -- e.g. for
ifdef::onprem[]
those switchports 
endif::[]
ifdef::onprem[]
...
xref:enforced_services[enforced services] or -- and this is
...
expected, then you can also work with xref:enforced_services[enforced services].
...
The host’s service list in the Setup can be easily accessed via the icon:icon_menu[] action menu of the [.guihint]#Check_MK Discovery# service
...
ifdef::onprem[]
For example, the check for switchports
endif::[]
ifdef::saas[]
For example, some checks for network interfaces
endif::[]
will only take ports that are {UP} into the monitoring.
...
ifdef::onprem[]
xref:views#[views] and NagVis maps, and to switch targeted xref:glossar#notification[notifications] and xref:alert_handlers#[alert handlers].
endif::[]
ifdef::saas[]
xref:views#[views], and to switch targeted xref:notifications#[notifications].
endif::[]

By the way – you could almost always construct corresponding views
...
ifdef::onprem[]
xref:views#[views], NagVis maps, xref:glossar#notification[notifications] and xref:alert_handlers#[alert handlers].
endif::[]
ifdef::saas[]
