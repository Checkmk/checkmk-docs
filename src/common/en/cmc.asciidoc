...
:description: The monitoring core is a central part of the {CMK} system. The most important properties and features of the {CMK} Micro Core are described here.
...
Even though Nagios can perform exceptionally--being flexible, fast, stable and well-proven--there are still situations in which their limits are reached.
...
* *Rapid activation of pending changes* -
...
* The seamless activation of pending changes allows frequent configuration changes.
...
The CMC is included as a component in the commercial xref:glossar#edition[editions].
endif::[]
