...
{CE} provides you {CMK} as _Software as a Service_ (SaaS).
You can find out what characterizes {CE} (SaaS), also in comparison to its self-hosted siblings such as {CE} (Self-hosted), in a separate article, which will be linked here as soon as it is written.
endif::[]
...
ifdef::saas[]
If you are reading this User Guide for the first time you can also start directly with the first article from the Beginner's Guide (coming soon), 
and if necessary, later return to the descriptions in the more advanced chapters.
endif::[]
