...
. Store the subscription data and check the license data.
The services provided by all remote sites are treated in the same way when calculating the subscription.
. Gradually update the remote sites.
. For each remote site that has received the upgrade, you can reactivate the synchronization of the configuration.

We recommend waiting until all sites have been upgraded to {SE} before activating {SE} specific features.
...
. Upgrade the remote sites to the {CE}.
*Important:* Mixed operation with {SE} as the central site is only possible in 30-day trial mode, so do not store any subscription data yet!
. Store the subscription data for the {CE} in the central site under the {SE}
. Finally, upgrade the central site to {CE}.

If you want to upgrade directly from {RE} to {CE}, you must upgrade the central site to {SE} as an intermediate step: First upgrade the central site from {RE} to {SE}, followed by the upgrade of the remote sites directly from {RE} to {CE}. Finally, upgrade the central site from {SE} to {CE}.
