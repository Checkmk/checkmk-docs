...
This feature is available in versions {v25}, 2.4.0p26 and above, and 2.3.0p47 and above.

No usage data will be transmitted without your active consent (opt-in as described xref:#consent[below]).
As a {CMK} xref:wato_user#predefined_roles[administrator], you decide for yourself whether to activate this data transmission in order to influence the further development of the software in this way.
endif::[]
ifdef::saas[]
...
For an overview of the type of data that will be collected when usage data transmission is active, have a look at the link:https://analytics.checkmk.com/manifest[_product usage analytics manifest_^].
endif::[]
ifdef::saas[]
For an overview of the type of data that will be collected, have a look at the link:https://analytics.checkmk.com/manifest[_product usage analytics manifest_^].
endif::[]
...
After another 30 days, the modal dialog will appear again.
...
After selecting the option [.guihint]#Do not collect product usage data#, you will not receive any further requests to transmit data.
...
Allow outgoing https connections from your {CMK} server to `\https://analytics.checkmk.com/upload` on port 443.
endif::[]
...
A complete overview of the data that is collected can be found in the link:https://analytics.checkmk.com/manifest[_product usage analytics manifest_^].
A link to this document can also be found in your {CMK} site under [.guihint]#Help > About {CMK} > Product usage analytics manifest#.
...
Your site transmits the collected data with an encrypted POST request from your {CMK} server via https to `\https://analytics.checkmk.com/upload` on port 443.
endif::[]
ifdef::saas[]
...
We store usage data we receive without associating it with the sites from which it originates -- the data is processed anonymously.
We run statistical analyses over all received data in order to discover trends concerning the usage of the different features of {CMK}.
...
* If you have set the option [.guihint]#Enable replication# to the value [.guihint]#Push configuration to this site# for a remote site, this is a xref:glossar#central_setup[distributed monitoring with central setup].
