...
{cee-only}
{CMK} {v24} offers in {CCE} experimental support for processing link:https://opentelemetry.io/[OpenTelemetry^] metrics.
...
The OpenTelemetry data is always available in the site if the OpenTelemetry Collector has been set up correctly.
This can be checked via the command line, as the OpenTelemetry data is stored in the `~/tmp/check_mk/otel_collector/` directory:
...
The options for creating a piggyback and OpenTelemetry connection, which can be set up in {CE}, are almost identical.
