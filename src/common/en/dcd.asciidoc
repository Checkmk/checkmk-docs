...
:description: You can have {CMK} automatically add or delete hosts using its dynamic host management capabilities. Read here how to use this function.
...
Subsequently, a xref:glossar#service_discovery[service discovery] is (optionally) performed on the hosts, and finally the xref:glossar#activate_changes[pending changes are activated] so that the current status can be seen in the xref:glossar#monitoring_environment[monitoring environment].
...
support for processing link:https://opentelemetry.io/[OpenTelemetry^] metrics.
To do this, an OpenTelemetry Collector in {CMK} collects metric data that the collector receives via the OpenTelemetry Protocol (OTLP) or retrieves via a Prometheus endpoint.
The OpenTelemetry Collector writes the received data to the xref:metric_backend#[metric backend].
Dynamic host management accesses the data stored there.

The setup of OpenTelemetry, including the OpenTelemetry Collector
ifdef::onprem[]
and the metric backend
endif::[]
is described in the article xref:opentelemetry#[Monitoring OpenTelemetry metrics].
...
Create a new connection using [.guihint]#Add connection#.
...
The options for creating a xref:connection_piggyback[piggyback connection] and an xref:connection_opentelemetry[OpenTelemetry connection] differ in one key aspect:
While piggyback connections use the host names provided with the piggyback data, host names for OpenTelemetry data can be generated more flexibly.
We will discuss this point in detail; all other properties of an OpenTelemetry connection will be covered in a quick overview.

image::dcd_connection_ot_01.png[alt="Properties of an OpenTelemetry connection."]

For an xref:connection_opentelemetry[OpenTelemetry connection], select [.guihint]#OpenTelemetry data - Metric backend# as the [.guihint]#Connector type#.

Use [.guihint]#Sync interval# to specify how often the connection should search for new hosts.

The biggest difference from piggyback connections is [.guihint]#Metric backend filter criteria#.
Here, you can first use the [.guihint]#Attributes# to specify which data should be included when creating a host, and then determine which attribute should be used to generate the host name.
The image above shows the [.guihint]#Resource attribute for hostname lookup# field filled in with the `service.name` attribute, which generates a host from each Telemetry service name.
This is the default setting if you used the [.guihint]#OpenTelemetry Quick Setup#.
Open the help for the input field to get more detailed information.
...
By checking the box next to [.guihint]#Service discovery#, you specify that a xref:glossar#service_discovery[service discovery] be performed on the automatically generated hosts.
However, this will only produce the desired result if the xref:opentelemetry#special_agent[special agent for OpenTelemetry] is configured.
...
These options function as described in the section on xref:host_delete_options[piggyback connection].
...
To view these anyway, click on the clock symbol icon:icon_dcd_history[alt="Icon of the execution history."] in the [.guihint]#Actions# column, which will bring you to the xref:execution_history[Execution history] of this connection.
...
This action determines how and when dynamic host management should activate pending changes and how long it can take to do so.
...
If you want to watch the DCD at work, you will find the clock icon icon:icon_dcd_history[alt="Icon of the execution history."] in the list of connections for each entry,
...
You can specify the scope of the logged data using the rule [.guihint]#Setup > Hosts > Dynamic Host Management > Host Manager Settings#.
