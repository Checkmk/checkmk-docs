...
The push mode function in {CCE} is also dependent on the Agent Controller and is therefore not available in the legacy mode.
...
You can use xref:datasource_programs#[datasource programs] to connect from the monitored host and use this to transmit the agent output to the {CMK} server.
