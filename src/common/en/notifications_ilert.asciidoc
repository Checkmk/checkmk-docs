...
[TIP]
====
In ilert, only the xref:notifications#environment_variables[{CMK} notification types] `PROBLEM`, `ACKNOWLEDGEMENT` and `RECOVERY` are processed -- all others are ignored.
====
