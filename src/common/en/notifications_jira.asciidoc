...
Finding the `projectid` (which refers to the Space ID—unfortunately, Atlassian confuses the terms project and space; while projects were renamed spaces for the GUI, this change wasn't implemented in the technical infrastructure) is a bit of a hassle: Open the [.guihint]#Space settings# and click on [.guihint]#Toolchain# in the navigation bar on the left—the URL that opens will then contain an entry at the very end like `projectId=10000`.
