...
*Note:* The visible menus and menu entries on the navigation bar depend on your permissions.
...
*Note:* You can also load the contents of the [.guihint]#Setup# menu into the sidebar with the [.guihint]#Quick setup# snap-in.
...
|[.guihint]#Language# of the user interface |By default, you can choose between German and English, the languages officially supported by {CMK}.
If you activate the option [.guihint]#Enable community translated languages (not supported)# in [.guihint]#Setup > Global settings > User interface#, the languages translated by the community are also available.
