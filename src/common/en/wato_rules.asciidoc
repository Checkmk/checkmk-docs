...
= Rules
:title: Rules
:description: Rules are used to configure the monitoring of objects and many settings in {CMK}. Read here how rules are defined and evaluated.
...
and it is therefore set to the [.guihint]#Smart PING (only with {CMK} Micro Core)# default value of the commercial editions.
...
The following image shows the [.guihint]#File systems (used space and growth)# rule set with four rules configured:
