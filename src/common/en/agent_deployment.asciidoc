...
// MFS: translation needed, current state of this section in German
////
...
////
// MFS: Proofreading necessary, see German text above

=== Disabling automatic updates for a host

If a host should be removed from the automatic updates, adjust the [.guihint]#Agent updater (Linux, Windows, Solaris)# rule set so that the update plug-in is deactivated there.
With the next regular update the agent then removes its updater itself!

It goes without saying that the update can then only be reactivated by manually installing of a new agent package.
