...
ifdef::onprem[]
image::agent_deployment_activation_of_automatic_agent_updates.png[alt="Global setting for activating the agent updater."]
endif::[]
ifdef::saas[]
image::agent_deployment_activation_of_automatic_agent_updates_saas.png[alt="Global setting for activating the agent updater."]
endif::[]
////
...
[IMPORTANT]
====
This passphrase can later neither be changed nor restored.
...
You can find this rule by clicking on the [.guihint]#Configuration of update plug-in# entry on the [.guihint]#Automatic agent updates# page.
...
The remote site also requires a previously-created automation user to be able to log on to the central site. This can also be selected here. If none is specified, the remote site searches for an automation user with the name 'Automation'.
...
It has occasionally occurred -- especially on Red Hat/CentOS systems -- that the
