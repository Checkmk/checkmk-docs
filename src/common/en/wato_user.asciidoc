...
You can use hardware authenticators such as the YubiKey or another similar USB token.
...
Click the icon:icon_2fa_add[alt="Button to add a new credential."] [.guihint]#Add credential# icon: 

image::wato_user_2fa_2.png[alt="Two-factor authentication setup page."]

{CMK} detects the authentication options already set up on your computer. A small dialog will open in the browser window where you can set the authenticator.
After that action you will be logged out automatically.

Next, you can directly log in to {CMK} for the first time using two-factor authentication. First, enter your username and password as usual. Then another login screen will appear:

image::wato_user_2fa_login.png[alt="Login with the second authentication factor.", width=55%]
...
To do this, create a list of backup codes in advance on the [.guihint]#User > Two-factor authentication# page using icon:icon_regenerate[alt="Button to create backup codes."] [.guihint]#Regenerate backup codes#. 

image::wato_user_2fa_backup.png[alt="Display of the generated backup codes.", width=52%]

Keep these backup codes in a secure place.

Then, if you want to log in to the {CMK} website, click [.guihint]#Use backup code# on the second login screen. Enter one of your backup codes and log in with [.guihint]#Use backup code#.

image::wato_user_2fa_login2.png[alt="Prompt to activate authenticator.", width=55%]
