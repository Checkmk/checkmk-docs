...
:description: In {CMK} you control the responsibility for each monitored object via users, roles and contact groups. Read here how to administrate users in your site.
ifdef::onprem[]
:keywords: 2fa
endif::[]
...
ifdef::saas[]
An example of this is the user with administrator rights, which is automatically created after registration.
endif::[]
...
ifdef::onprem[]
If a contact was created for the sole purpose of xref:glossar#notification[notification] (e.g. to forward notifications to a ticket system), then it may be useful to create it in such a way that no login to the interface is possible.
endif::[]
...
This email contains the username (i.e. the email address), a temporary password and a link for the first login to {CE}.
...
An xref:glossar#automation_user[automation user], followed by the single user (`cmkadmin`) for the interactive login with password.
...
* You choose this user's password when creating the site. You can also change this password at any time later on.

The form for creating a new user (with icon:button_new_user[alt="Button for creating a new user."]) or editing an existing user (with icon:icon_edit[alt="Icon for editing."]) is divided into five sections.
...
As always in {CMK}, the ID of a dataset ([.guihint]#Username#) cannot be changed later.
It is used for logging-in and also as an internal key in all files and data structures.

The email address is optional and is only required if the user is to become a contact who is to be notified by email (xref:notification_rules#smtp[SMTP configuration] required).
Likewise, the [.guihint]#Pager address# field is intended for notification via SMS or similar systems.
If you write your own notification scripts you can access the values in these fields and use them for any purpose.

With [.guihint]#Monitored sites# you can optionally restrict which of the existing sites may be accessed.
This is particularly practical for very large environments, such as a distributed monitoring with hundreds of sites.
If a user only needs a certain number of these sites for his hosts, the GUI will only contact the selected sites to set up views
-- which in turn greatly improves performance.
endif::[]


[#user_config_security]
=== Security
...
In the [.guihint]#Notifications# box, you can use the [.guihint]#Receive fallback notifications# option to specify that this contact will receive notifications when xref:notification_rules#fallback[no notification rule applies].
endif::[]
...
In {CCE} this role contains additional permissions for xref:hosts_autoregister#[creating hosts automatically].
...
ifdef::onprem[]
An additional note for xref:glossar#distributed_monitoring[distributed monitoring]:
In a distributed environment, any new settings are immediately transferred to all monitoring sites.
This is the only way to ensure that, in particular, a newly-assigned password also works immediately everywhere--and not only following the next activation of pending changes.
However, this only works for sites that are accessible via the network at that time.
All other sites receive the updates at the next successful xref:glossar#activate_changes[activation of pending changes].
endif::[]
...
Up to this point, we have described how to create a 'normal user’.
However, for various reasons, there are also individual users who have special rights or functions.
...
automation users.
We will go into their special features below.
...
When installing {CMK}, an administrator is created by default--as already mentioned xref:user_config[above].
This administrator is named `cmkadmin`.
You can now continue to work with this administrator as you wish, however you may not want to use this default administrator.
For example, due to internal company regulations, because you want to define multiple administrators, or to comply with security recommendations such as the link:https://owasp.org/www-project-application-security-verification-standard/[OWASP Application Security Verification Standard (ASVS)^].

First of all, an administrator is a user like any other.
It is therefore created as described in the chapter xref:user_config[User administration in the configuration environment].
However, the rights that are assigned to the user via the selected role are decisive.

image::wato_user_admin.png[alt="Selecting the Administrator role for a user."]

Select the [.guihint]#Administrator# role when creating the user.
All further settings are also made for the administrator according to personal preferences or internal company specifications.
endif::[]
...
For other tasks, you can create additional automation users, usually with the role [.guihint]#administrator#.
...
[IMPORTANT]
====
HTTP basic authentication requires the automation password in plain text by default.
To do this, set the option [.guihint]#Store the secret in cleartext#.
The password is then saved in the site directory under `~/var/check_mk/web/<username>/automation.secret`.
This is necessary for all rules and scripts that use the automation password.
====
...
You can easily store the contents of this file in a variable in the shell, for instance so that it can be read out with a script later on:

[{shell}]
----
{c-omd} SECRET=pass:[$](cat var/check_mk/web/automation/automation.secret)
{c-omd} echo "$SECRET"
a8075a39-e7fe-4b5c-9daa-02635
----
endif::[]
...
because the credentials (username and password) passed via URL are stored in the log files of the site-specific Apache (see link:https://checkmk.com/werk/14261[Werk pass:q[#]14261^]).
...
So that you retain control of _who_ is permitted to make changes, and _what_ they are allowed to do, and so that people do not get in each other's way, you can assign permissions for the {CMK} xref:wato#[Setup] on the basis of folders.
...
Alternatively, it is possible to use authenticator apps (for example, on the smartphone) that generate a _time-based one-time password (TOTP)_.
...
After your approval of the confirmation dialog, the user will be able log in to the {CMK} web interface again using 'only' the username and password.
...
A user who should activate two-factor authentication but does not yet have it will be redirected to the [.guihint]#Two-factor authentication# page when logging in after entering the username and password:
...
In the article describing xref:notification_rules#[notification rules], we go into great detail about how {CMK} can inform contacts about problems with hosts or services.
...
|The message is displayed in an xref:dashboards#builtin_widgets[widget] of the type [.guihint]#User messages#.
|===
...
A special feature of the option [.guihint]#Show hint in the 'User' menu#: Via [.guihint]#User messages > Received messages# users can find their collected messages and acknowledge and/or delete them.

image::wato_user_user_messages.png[alt="Overview of the user notifications."]

ifdef::onprem[]
{CMK} automatically notifies users of security-relevant alterations to their accounts.
These include changes to the password and two-factor authentication (activation, deactivation, login via backup code, and the creation and revocation of backup codes).
If an e-mail address is stored for the user, notifications are sent by e-mail, otherwise they are sent via the message tool.

Security messages are triggered only by actions within the {CMK} GUI.
They cannot be deleted by the user, however the display duration of these messages can be configured via [.guihint]#Setup > General > Global settings > User management > User security notification duration#.
By default, the display duration is 7 days.
endif::[]
