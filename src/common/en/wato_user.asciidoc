...
In this article we will show you everything on the subjects of user management and permissions in {CMK}.
...
[#user_config]
== User management via the configuration environment
...
The [.guihint]#Automation secret for machine accounts# option is meant for accounts that access {CMK} via HTTP and authenticate themselves via the URL.
...
With the predefined roles (see xref:roles[below]), however, this would make little sense.
...
There are two methods for including hosts in contact groups: via xref:hosts_setup#folder[folder] and
...
*Caution:* The xref:hosts_setup#inheritance[inheritance] of the [.guihint]#Permissions# attribute over the folders is overridden at this point.
...
It is important that you understand that roles define the _level_ of permissions 
and not a reference to any hosts or services.
...
|`admin` |All permissions -- especially the right to change permissions. |The {CMK} administrator who maintains the monitoring system itself.
|`user` |May only see his own hosts and services, may only make changes in the web interface in xref:folder_permissions[folders shared for them] and generally may not do anything that affects other users. |The normal {CMK} user, who uses the monitoring and reacts to notifications.
|`agent_registration` |The permission to register a host's xref:wato_monitoringagents#agents[{CMK} agent] with the {CMK} server for TLS-encrypted data transmission -- nothing else. |This role is assigned to the xref:glossar#automation_user[automation user] `agent_registration` to perform a registration with minimal rights. In the {CCE}, this role contains additional permissions for xref:hosts_autoregister#[creating hosts automatically].
|`guest` |May see everything but not change anything. |Guest is intended simply for 'looking', for which all guests share a common account. Also useful for 'public' status monitors hanging on a wall.
|===
...
By the way, when creating a new {CMK} site, only one user (`cmkadmin`) for the `admin` role is created.
...
The usual way via the xref:intro_setup#login[login dialog] is cumbersome and requires the retrieval of several URLs in succession and the saving of a cookie.
...
Two automation users are already set up in each {CMK} site: for web services and for registering the agent with the {CMK} server for TLS-encrypted data transfer.
...
You can have this created automatically with the icon:icon_random[] cube:
...
*Important:* The automatic login via the URL in the browser described below has been disabled for security reasons since {CMK} {v22},
...
you must explicitly enable this with the global setting [.guihint]#Setup > General > Global settings > User interface > Enable login via GET requests.#
...
=== Function of the 'user' role in {CMK}
...
As an admin, you can unlock other users via [.guihint]#Setup > Users >Users# and then the properties of the locked user.
...
To do this, edit the file `etc/htpasswd` as a site user and remove the exclamation mark from the line of the affected user, here `myuser`:
...
In the [.guihint]#Session management# box, you can set two different ways of ending a session and combine them with each other:
...
Once the set session duration has expired, the user must log in again, regardless of whether they were active at the end of the session or not.
At the same time, you can use [.guihint]#Advise re-authentication before termination# to specify when the user should be notified to save their entries and log in again before the "hard" termination of their session:
...
Alternatively, it is possible to use software authenticators (authenticator apps on the smartphone) that generate a time-based one-time password (TOTP).
// MFS: I deliberately formulated this in general terms. On devices where the cryptographic module can communicate with a biometric device via a secure channel, the fingerprint scanner, for example, fulfills the requirements.
// MFS: Background at Matthew Garret https://mjg59.dreamwidth.org/62746.html
...
* xref:omd_https#[Securing the {CMK} web interface with HTTPS]
...
image::wato_user_2fa.png[alt="Selecting two-factor authentication from the User menu.", width=67%]

You will now be given two options marked with the key icon icon:icon_2fa_add[alt="Button for adding a new credential."] to add the second factor: 
...
// MFS: I don't quite understand this sentence, I have changed from "set up" to "available", as browser access to a USB token depends on many factors, e.g. is not always possible in RDP sessions.
...
When using one-time passwords, you must enter a generated password for confirmation.
...
Then another login screen appears:

image::wato_user_2fa_login.png[alt="Logging in with the second authentication factor.", width=53%]
...
To do this, create a list of backup codes in advance on the [.guihint]#User > Two-factor authentication# page using icon:icon_regenerate[alt="Button for creating backup codes."] [.guihint]#Generate backup codes#. 

image::wato_user_2fa_backup.png[alt="Display of created backup codes.", width=85%]

Keep these codes in a secure place.

If you then want to log in with a backup code on the {CMK} website with a backup code, click [.guihint]#Use backup code# on the second login screen. Enter one of your backup codes and log in with [.guihint]#Use backup code#.

image::wato_user_2fa_login2.png[alt="Prompt to enter the backup code.", width=53%]
...
In the user's view, remove the two-factor authentication for this user via [.guihint]#User > Remove two-factor authentication#.

image::wato_user_2fa_remove.png[alt="Removing the two-factor authentication.", width=39%]

After your confirmation of the security prompt, the user will be able log in to the {CMK} web interface again using 'only' the user name and password.
...
|[.guihint]#Show popup message# |The next time the user opens the page, a popup window with the message will be opened.
...
|[.guihint]#Send email# |Sends an email. However, this will only reach users who also have an email address configured.
|[.guihint]#Show in the dashboard element 'User messages'# |The message is displayed in an xref:dashboards#builtin_dashlets[dashlet] of the type [.guihint]#User messages#.
|===
...
|`~/etc/auth.secret` |This file contains a random secret used to sign login cookies.
...
|`~/etc/auth.serials` |Serial numbers of passwords per user.
...
|`~/etc/check_mk/multisite.d/wato/users.mk` |Contains the users set up with the configuration environment.
...
|`~/var/check_mk/web` |Every user who has logged in to the GUI at least once has a subfolder here where things like custom views and reports, the current sidebar configuration and much more are stored in small individual files with the extension `.mk`.
These files have Python format.
|`~/var/log/web.log` |Log file of the user interface.
