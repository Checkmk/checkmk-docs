...
By the way, the https://docs.checkmk.com/saas/de/wato_user.html[German version] of this article has already been updated -- if that is an alternative for you.
====
endif::[]
...
This user is allowed to see all hosts and services, but only because the 'Administrator' role contains the permission [.guihint]#See all hosts and services#, and not necessarily because they would be a contact for everything.
...
* This user has the 'Administrator' role and thus all permissions.
* They are not responsible for any contact and do not receive any notifications.
* They can still see everything (because of their 'Administrator' role).
* You should definitely change the password that was assigned when the site was created!

The form for creating a new user is opened with icon:button_new_user[alt="Button for creating a new user."] -- or to edit an existing user with icon:icon_edit[alt="Icon for editing a list entry."], is divided into five sections.
The first section is concerned with the identity.
endif::[]
...
If you *lock* users with the [.guihint]#disable the login to this account# option, they will be appear in the table with the icon:icon_user_locked[] icon.
...
ifdef::onprem[]
Users can also change all of the settings in this box xref:personal_settings[themselves] via [.guihint]#User > Edit profile# (except with the 'Guest user' role).
endif::[]
ifdef::saas[]
...
The fact that normal users (the 'Normal monitoring user' role) only see those objects for which they are a contact is all the more important the larger your monitoring environment is.
This not only provides an uncluttered overview, but also prevents users from intervening where they have no business.

As an administrator (the 'Administrator' role) you are of course always allowed to see everything.
...
ifdef::onprem[]
The `'Guest user' role is preset so that its users can also see everything.
...
|`user`
|Normal monitoring user
|May only see his own hosts and services, may only make changes in the web interface in xref:folder_permissions[folders shared for them] and generally may not do anything that affects other users.
|The normal {CMK} user, who uses the monitoring and reacts to notifications.
...
Should new permissions be added or existing ones change in future {CMK} versions, you can still be sure that a role derived from `no_permissions` will not receive any new, unexpected permissions.
...
By the way, when creating a new {CMK} site, only one user (`cmkadmin`) with the 'Administrator' role is created for the login to the {CMK} interface.
...
As usual, the icon:icon_edit[alt="Icon for editing a list entry."] takes you to the edit mode for a role:
...
Each time we then decide in which of the three roles 'Administrator', 'Normal monitoring user'' and 'Guest user' the new permission should take.
...
but also see where permissions have been explicitly set (icon:icon_perm_yes[]) or removed (icon:icon_perm_no[]).
...
You can have this created automatically with the icon:icon_random[alt="Icon for rolling a password."] cube:
...
The 'Guest user' role is well-suited for this function because it grants all read rights, but does not allow any changes or interventions.
...
=== Function of the 'Normal monitoring user' role
...
The first step to this is to have your administrator colleagues work with their own users based on the 'Normal monitoring user' role.
...
As long as you have not yet released any folders or BI packs, this means that users with the 'Normal monitoring user' role cannot initially make any changes.
...
Unlocking is then only possible by a user with the 'Administrator' role.
...
Then another login dialog appears:
...
If you then want to log in with a backup code on the {CMK} website, click [.guihint]#Use backup code# on the second login dialog.
Enter one of your backup codes and log in with [.guihint]#Use backup code#.
...
To do this, open the relevant entry in the user management by clicking on icon:icon_edit[alt="Icon for editing a list entry."].
