...
Dazu zählen zum Beispiel Administratoren und der Automationsbenutzer.
...
Bei der Installation von {CMK} wird - wie oben bereits erwähnt - standardmäßig ein Administrator angelegt.
...
Beispielsweise aufgrund firmeninterner Vorschriften, weil Sie verschiedene Administratorenrollen definieren wollen oder auch um Sicherheitsempfehlungen wie dem OWASP Application Security Verification Standard (ASVS) zu genügen.
...
Wählen Sie beim Anlegen des Benutzers nun die Rolle [.guihint]#Administrator# aus.
