...
-- provided both the {CMK} server and agent versions have at least {v21}.
...
The agent program is minimalistic, secure, easily extendable and comprehensive, and provides access to important data that is not accessible via WMI or SNMP.
...
.Options when updating a very old agent
...
Via the command line, Windows provides administrators with `msiexec` the possibility of installing MSI packages automatically without user interaction.
...
In such a case the agent will switch to the unencrypted *legacy pull mode*, so that the {CMK} server is not cut off from the monitoring data and can continue to display it.
This applies to a new installation as well as to an update of an agent of version {v20} or older.
...
The situation will be different when using a *legacy agent* on a very old xref:windows_versions[Windows system].
...
In the {CCE} you will find the [.guihint]#Checkmk agent connection mode# option in the host properties in the chapter on xref:hosts_setup.html#monitoring_agents[monitoring agents].
...
Since version {v21}, with the introduction of the two new components -- the Agent Controller on the host, and the Agent Receiver on the {CMK} server -- the number of points where something can go wrong has increased.
