...
ifdef::onprem[]
The *agent program* is accompanied by the *Agent Controller*, a component that communicates securely with the *Agent Receiver*, a process running on the {CMK} server.
To do this, the Agent Controller registers itself with the *Agent Receiver*.
The Windows agent can perform functions with the Agent Controller such as TLS encryption of communication or data compression, and is designed to accommodate future enhancements.
...
The xref:glossar#push_mode[push mode] is available in {CCE}.
Reversing the communication direction makes it easier to monitor hosts that are located behind firewalls.
Push mode is usually combined with xref:hosts_autoregister#[automatic registration] of the {CMK} agent, which is also available in {CE}.
...
{UE} is the most flexible and supports both modes.
====
endif::[]
...
ifdef::onprem[]
You can find out exactly what these are in the xref:release_notes.html#windows[System requirements] article.
endif::[]
ifdef::saas[]
...
In pull mode, it listens on TCP port 6556 for incoming connections from the {CMK} xref:glossar#site[site] and queries the agent program via a Windows-specific mechanism for process communication called link:https://learn.microsoft.com/en-us/windows/win32/ipc/about-mailslots[Mailslot^].
...
ifdef::onprem[]
{CMK} guides you through the user interface to download, install, and register the {CMK} agent on a host, thereby adding it to the xref:glossar#configuration_environment[configuration environment].
An assistant guides you through the necessary steps in a _slideout_.
In the slideout, you can select Windows, Linux, Solaris, or AIX as the operating system for the host.
The xref:intro_setup_monitor#linux[Beginner's Guide] describes the process in detail for a Linux host in pull mode.
The process is identical for a Windows host--only the commands are different.

The main advantage of the slideout is that it displays directly in the GUI the appropriate commands for downloading, installing, and registering a Windows host.
You can copy any command to the clipboard and then run it on the host.
For a Windows host, the agent package is downloaded using curl via the REST API, installed with the `msiexec` command, and finally, the host is registered with `cmk-agent-ctl`.
endif::[]
...
This uses the agent packages prepared for auto-registration for Linux and Windows.
...
The main advantage of the slideouts is that they display directly in the GUI the appropriate commands for downloading, installing, and registering a Windows host.
You can copy any command to the clipboard and then run it on the host.
endif::[]
...
ifdef::saas[]
In {CE-saas}, it is particularly easy to install and use the preconfigured agent packages for auto-registration.
In addition to these, so-called 'Vanilla agents' are available without preconfiguration, though they require more effort to set up.
...
The Vanilla agents without preconfiguration can be found in the first row [.guihint]#Default configuration# of the table.

.In the Agent Bakery you can download preconfigured and Vanilla agent packages
image::agent_linux_bakery_saas.png[alt="Agent packages in the Agent Bakery."]
...
For example, the included MSI package with the Windows agent can be downloaded using the following `curl` command, which authenticates using an xref:glossar#automation_user[automation user].
...
For details on this and other REST API endpoints, see the API documentation available in {CMK} via [.guihint]#Help > Developer resources > REST API > Version 1 > Documentation#.
...
This applies to both a new installation and an update of a very old agent (older than version {v21}).
...
*Note:* In the [.guihint]#{CMK} Agent installation auditing# rule set you will find various settings to check the state of the agent and make it visible in monitoring.
...
Immediately after installing the agent (including when updating a very old agent, i.e., one older than version {v21}), only unencrypted communication is possible in the legacy pull mode.
...
ifdef::onprem[]
In {CE} you will find the [.guihint]#{CMK} agent connection mode# option in the properties of the host in the section on xref:hosts_setup.html#monitoring_agents[monitoring agents].
...
ifdef::onprem[]
{CE} offers the facility to create hosts automatically at registration.
...
ifdef::saas[]
In addition to the packages available in the Agent Bakery, you have the option to create your own packages for xref:hosts_autoregister#[auto-registration].
...
The biggest difference here is the modified `register-new` subcommand, which is used to request the registration _and_ creation of a new host in the {CMK} site.
...
First specify the access mode in the Setup, in the properties of the host, with the [.guihint]#{CMK} agent connection mode# option.
...
We will work our way from the agent program, through the Agent Controller
ifdef::onprem[]
and TCP port 6556, 
endif::[]
...
After executing the `cmk-agent-ctl.exe register` command, the Agent Controller first queries the {CMK} server via the REST API to determine the port used by the Agent Receiver.
...
[TIP]
====
The behavior described here mainly affects users who add hosts into a {CE-saas} monitoring (for example for evaluation purposes) that are already in a monitoring by on-premises editions of {CMK}.
====
endif::[]
...
In the first step, deactivate the encryption in the existing rule under [.guihint]#Setup > Agents > Access to agents > {CMK} agent > Symmetric encryption (Linux, Windows)#.
...
ifdef::onprem[]
As a user of one of the commercial editions, you can simply create a rule via [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Windows agent options > Disabled sections (Windows agent)# which will then be taken into account by the xref:glossar#agent_bakery[Agent Bakery].
endif::[]

ifdef::saas[]
You can simply create a rule via [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Windows agent options > Disabled sections (Windows agent)# which will then be taken into account by the xref:glossar#agent_bakery[Agent Bakery].
endif::[]
...
Why are these plug-ins not simply hard-coded into the agent?
...
* The plug-in is so special that it is not needed by most users (example: `citrix_licenses.ps1`).
...
.The service discovery finds the new service
image::agent_windows_service_discovery.png[alt="Service discovery page showing the new service."]
