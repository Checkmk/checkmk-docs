...
An exception to this are the packages preconfigured for the xref:hosts_autoregister#[auto-registration] and downloaded via the Agent Bakery in {CCE}.
...
When using the push mode of {CCE}, it is usually not necessary to change its settings.
...
For push hosts under {CCE} this soft link points to the folder with the agent output, for pull hosts it points to a non-existent file with the name of the host as used in the monitoring.
...
|`C:\ProgramData\checkmk\agent\pre_configured_connections.json` |Contains a pre-configured connection to a site for the xref:hosts_autoregister#[auto-registration] in {CE}, integrated into the agent package via Agent Bakery.
