...
[TIP]
====
For more information on certificates, see the article xref:certificates#[_Certificate overview in {CMK}_].
...
Next we transfer the `/tmp/mynewhost3.json` file to the host we registered and import that file:
...
*Note:* The command has been split into multiple lines for readability.
endif::[]
...
Alternatively, you can use CIM Cmdlets to uninstall:
...
If the uninstallation was successful, you will receive a `ReturnValue` of `0` as confirmation.
Otherwise, you will receive an MSI error code that hints at why the uninstallation failed.
...
|`~/tmp/check_mk/cache` |This directory stores cache data from hosts monitored by the {CMK} agent in pull mode.
|`~/tmp/check_mk/data_source_cache/push-agent/` | Cache data from hosts using the agent in push mode is stored below this directory.
|===
endif::[]
