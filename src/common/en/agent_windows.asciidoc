...
For a Windows host, the agent package is downloaded using curl via the REST API, installed with the `msiexec` command, and the host is finally registered with `cmk-agent-ctl`.
endif::[]
