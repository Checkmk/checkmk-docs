...
For the [.guihint]#CPU load# service on the Linux host in our example, you can create a new rule with the [.guihint]#CPU load (not utilization!)# rule set under [.guihint]#Service monitoring rules#, which you can find most quickly by xref:user_interface#search[searching] the [.guihint]#Setup# menu.
...
If you have set up prediction-based monitoring for a service, xref:glossar#activate_changes[activate the pending changes], and once {CMK} has performed a check for this service, the new icon icon:icon_prediction[] will appear in the service list:
