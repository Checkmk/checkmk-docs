...
|[.guihint]#unmonitored# |N/A |Time ranges during which the object was not being monitored. There are two possible reasons for this: the object was not in the monitoring configuration, or the monitoring itself was not running during the specified time range.
...
The time range query can be customized with the [.guihint]#Availability > Change display options > xref:option_timerange[Time Range]# menu item.
More on this later...
...
[{image-left}]
image::avoption_format_timestamps.png[alt="Option for formatting timestamps.",width=350]

The formatting of timestamps applies to settings in the [.guihint]#Timeline#.
...
_availability_. Often {WARN} does not mean a real problem exists yet,
...
service times. These hidden times then do not count towards the 100{nbsp}%.
...
With an xref:glossar#activate_changes[activate pending changes] the cache is retained!
Only with an actual (re)start of {CMK} will it need to be newly generated--for example, following a server reboot or an update of {CMK}.
