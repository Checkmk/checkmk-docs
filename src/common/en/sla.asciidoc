...
:description: {CMK} can evaluate the fulfillment of service level agreements (SLAs) in a detailed way. Read here how to configure and use complex SLAs.
...
{cee-only}
 If you are using one of the commercial editions {CMK} has a separate function for Service Level Agreements (SLA).
