...
:description: Der Monitoring-Kern ist ein zentraler Teil des {CMK}-Systems. Hier werden die wichtigsten Eigenschaften und Features des {CMK} Micro Core beschrieben.
...
Auch wenn Nagios hervorragende Dienste leistet, flexibel, stabil und gut erprobt ist, so gibt es doch Situationen, in denen man damit an Grenzen stößt. 
...
* *Schnelles Aktivieren ausstehender Änderungen* - 
...
* Das unterbrechungsfreie Aktivieren ausstehender Änderungen ermöglicht häufige Konfigurationsänderungen.
...
Der CMC ist verfügbar als Bestandteil der kommerziellen xref:glossar#edition[Editionen].
endif::[]
