...
// TK: Ungeeignet sind z. B. Namen von Piggyback-Hosts, die nur aus einem Punkt bestehen oder mit einem beginnen wie `.myhostname`, da diese in {CMK} nicht verarbeitet werden.
// TK: Für die Umbennenung von Piggyback-Hosts gibt es einen speziellen ...
Dafür gibt es den speziellen xref:wato_rules#[Regelsatz] [.guihint]#Hostname translation for piggybacked hosts#, den Sie im Setup-Menü unter [.guihint]#Setup > Agents > Agent access rules# finden.
...
// TK: Formal: Huckepack-Dateien > Piggypack-Daten. Host-Namen in Kommandotext: `.`
// TK: Inhaltlich: Das Thema passt doch viel besser in den Absatz davor. Ich hab es oben mal auskommentiert eingefügt.
...
// TK: Hm, die Regel heißt anders: ... Piggybacked Host Data. Auch wenn Kleinschreibung angesagt wäre: Wollen wir das im Handbuch verbessern oder nicht doch lieber die GUI ändern (lassen)?
...
// TK: Das Bild oben und den vorherigen Satz würde ich einfach weglassen.
// TK: Das einzig interessante daran ist der Inline-Hilfetext, dessen Inhalt man evtl. verkürzt übernehmen könnte.
...
// TK: Ich hatte ne halbstündige Session mit Masl zu dem Thema: Das ganze hat nix mit dem DCD zu tun und Hosts werden auch nicht gelöscht.
// TK: Der DCD ist ein Enterprise Feature, aber diese Regel gibt es in allen Editionen (also auch Raw).
// TK: Marsellus hat sich geirrt und der Text "Keep hosts while" ist auch komplett missverständlich.
// TK: Hier geht es um den Zeitraum, nachdem die Piggyback-Daten gelöscht werden: im Beispiel im folgenden Kapitel "Transport der Piggyback-Daten" wäre das die Datei A.
// TK: Also etwa so:
// Mit der Option [.guihint]#Keep hosts while piggyback source sends piggyback data# legen Sie fest, nach welcher Zeit die Dateien mit Piggyback-Daten gelöscht werden.
// Sie können die globale Einstellung [.guihint]#Maximum age for piggyback files# übernehmen oder den Zeitraum selbst festlegen.
// Achten Sie darauf, dass dieses dieser Zeitraum mindestens so groß ist wie das Check-Intervall für die Piggyback-Hosts.
...
// TK: Zeitspanne > Zeit oder Zeitraum
// TK: Daten können nicht stale werden, nur Hosts und Services. Hier werden alle Services stale, die aus den Piggyback-Daten gespeist werden.
Außerdem legen Sie den Zustand des Services Check_MK fest.
// TK: ...des Services [.guihint]#Check_MK# in diesem Zeitraum fest.
...
// TK: In der Inline-Hilfe steht noch: The source host names have to be set in the condition field Explicit hosts.
// TK: Erinnerung ist vielleicht hilfreich, dass man nach dem ganzen Regelgedöns für die Piggyback-Hosts nicht die Quell-Hosts vergisst, für die die Regel gelten soll.
