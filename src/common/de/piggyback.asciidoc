...
Mit dem Befehl `cmk-piggyback list sources` erhalten Sie eine Liste aller Piggyback-Hosts, die Daten über andere Hosts bereitgestellt haben.
Das Gegenstück dazu ist der Befehl `cmk-piggyback list piggybacked`, um eine Liste aller Piggybacked-Hosts zu erhalten (unabhängig davon, ob diese im Monitoring bereits eingerichtet sind).

Mit `cmk-piggyback` stehen Ihnen noch weitere Möglichkeiten zur Verfügung, um die Behandlung von Piggyback-Daten im Monitoring nachzuvollziehen.
Sie können beispielsweise mit `cmk-piggyback track` nachverfolgen, welche Piggyback-Daten auf dem {CMK}-Server eintreffen.
Mit `cmk-piggyback create-sections` erzeugen Sie beispielhafte Piggyback-Daten.
endif::[]
...
Mit dem Befehl `cmk-piggyback list orphans` erhalten Sie eine Liste alle Objekte, für die es zwar Piggyback-Daten gibt, die aber noch nicht als Hosts in {CMK} angelegt sind.
...
| `~/bin/cmk-piggyback` |Hilfsskript mit Analysewerkzeugen für Piggyback-Funktionen. Informationen zur Verwendung des Skripts erhalten Sie mit `cmk-piggyback -h`.
|===
endif::[]
