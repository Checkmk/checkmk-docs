...
. Legen Sie eine Regel an und stellen Sie die Bedingung so ein, dass Sie auf den _Piggback-Host_ greift -- also quasi auf Host A.
...
Es gibt dann pro Zeile den Namen von einem nicht überwachten Piggybacked-Host aus:
