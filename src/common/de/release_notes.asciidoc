...
Eine _Software Bill of Materials (SBOM)_ ist nach der Installation von {CMK} unter `share/doc/bill-of-materials.json` respektive `share/doc/bill-of-materials.csv` verfügbar.
Falls Sie die SBOM vor Download und Installation überprüfen wollen, finden Sie im Download-Verzeichnis je eine JSON- und eine CSV-Datei plus die dazugehörigen Hashes.
Diese sind nach der Edition und Version benannt sind, beispielsweise link:https://download.checkmk.com/checkmk/{current}/check-mk-community-{current}-bill-of-materials.json[check-mk-community-{current}-bill-of-materials.json^] für {RE} {current}.
endif::[]
