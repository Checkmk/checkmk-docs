...
|8000 |TCP |Agent Controller TLS-Registrierung, Agenten im xref:glossar#push_mode[Push-Modus], Aktualisierung der mTLS-Zertifikate des Agent Controller |Wenn mehrere Instanzen auf dem {CMK}-Server laufen, sind eventuell weitere Ports (8001, 8002…) nötig.
|===
...
Sind beide nicht erreichbar, kann der Port per xref:agent_linux#networkrequirements[Befehlszeilenoption] angegeben werden.
...
|5672 |TCP |Advanced Message Queuing Protocol (AMQP) |Ein Message Broker wird für die Weiterleitung von xref:piggyback#distributed_piggyback[Piggyback-Daten im verteilten Monitoring] benötigt. Die Portnummer wird für jede neu eingebundene Remote-Instanz hochgezählt.
...
[#relay]
== Monitoring mit dem {CMK} Relay

Das xref:glossar#checkmk_relay[{CMK} Relay] erlaubt das Monitoring in {CE} mit xref:glossar#special_agent[Spezialagenten] und xref:glossar#snmp[SNMP] auch, wenn die {CMK}-Instanz keinen direkten Zugriff auf die überwachten Geräte hat.
...
|443 |TCP |Hypertext Transfer Protocol over SSL/TLS (HTTPS) |Installation und Update von Relays via `hub.docker.com` (bei Verwendung des Installationsskriptes)
...
Kommunikation mit dem Server der {comfull} für die xref:product_usage_analytics#[Analyse der Produktnutzungsdaten], falls ein Administrator die Zustimmung dazu erteilt hat (ab {CMK} {v25}). +
URL: `\https://analytics.checkmk.com/upload`.
|===

[TIP]
====
Beachten Sie, dass sich die IP-Adresse von `license.checkmk.com` jederzeit ändern kann.
Mitunter haben aufgelöste IP-Adressen nur wenige Stunden Gültigkeit.
Falls Ihre Firewall keine Möglichkeit bietet, ausgehenden Verkehr anhand von Host-Namen zu erlauben, ziehen Sie in Erwägung, xref:license#license_cmk[einen Proxy] für die Kommunikation mit dem Lizenzserver zu verwenden.
====
