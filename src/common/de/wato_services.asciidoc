...
Sie erreichen die Serviceliste auf verschiedene Arten:
...
Sie erreichen die Regel über [.guihint]#Setup > Services > Discovery rules > Discovery and Checkmk settings > Disabled Services#.
...
[#bulk_discovery]
== Service-Erkennung für viele Hosts gleichzeitig
...
. Mit der xref:hosts_setup#search[Host-Suche] Hosts suchen, alle gefundenen mit einem Klick auf das Kreuz icon:button_select_all[] auswählen und wieder in der Menüleiste über [.guihint]#Hosts > On selected hosts > Run bulk service discovery# gehen.
...
Die vier Checkboxen darunter bieten genau die Möglichkeiten, die Sie auch in der Serviceliste im Setup haben und die wir schon weiter oben erläutert haben.
...
|[.guihint]#Only include hosts that failed on previous discovery# |Hosts, bei denen eine frühere Service-Erkennung per Bulk-Operation fehlgeschlagen ist (z.B. weil der Host zu dem Zeitpunkt nicht erreichbar war), werden von {CMK} mit dem Symbol icon:icon_inventory_failed[] markiert. Diese Option erlaubt, die Erkennung nur genau für diese Hosts zu wiederholen.
...
wo Sie sich entscheiden können, anstelle der Interface-ID dessen Description oder Alias
...
Servicegruppen ermöglichen Ihnen auf einfach Art, 
ifdef::onprem[]
über xref:views#[Ansichten] und NagVis-Karten deutlich mehr Ordnung ins Monitoring zu bringen und gezielte xref:notifications#[Benachrichtigungen] und xref:alert_handlers#[Alert Handler] zu schalten.
endif::[]
ifdef::saas[]
über xref:views#[Ansichten] deutlich mehr Ordnung ins Monitoring zu bringen und gezielte xref:notifications#[Benachrichtigungen] zu schalten.
endif::[]
...
ifdef::onprem[]
xref:views#[Ansichten], NagVis-Karten, xref:alert_handlers#[Alert Handler] und xref:notifications#[Benachrichtigungen].
endif::[]
ifdef::saas[]
zum Beispiel xref:views#[Ansichten] und xref:notifications#[Benachrichtigungen].
endif::[]
