...
ifdef::saas[]
Dieser Check ist derzeit nur für On-Premises-Editionen mit verteiltem Monitoring relevant.
endif::[]
...
ifdef::saas[]
Dieser Check ist derzeit nur für On-Premises-Editionen relevant. In {CE} (SaaS) stellt {com} sicher, dass nur kompatible Plugins installiert sind.
endif::[]

|[.guihint]#Deprecated check plug-ins# |
ifdef::onprem[]
Im Handbuch finden Sie Artikel über die xref:devel_intro#[Entwicklung von Erweiterungen für {CMK}] und das xref:devel_check_plugins#[Entwickeln von agentenbasierten Check-Plugins] mit der in Version {v20} eingeführten Check-API.
...
ifdef::saas[]
Dieser Check ist derzeit nur für On-Premises-Editionen relevant. In {CE} (SaaS) stellt {com} sicher, dass Backups auf Block- oder Dateisystemebene erfolgen. Backup und Restore sind über das xref:admin_panel#admin_site[Admin Panel] zugänglich.
endif::[]
...
ifdef::saas[]
Dieser Check ist derzeit nur für On-Premises-Editionen relevant. In {CE} (SaaS) stellt {com} sicher, dass Backups auf Block- oder Dateisystemebene erfolgen.
endif::[]
...
ifdef::saas[]
Dieser Check ist derzeit nur für On-Premises-Editionen relevant. In {CE} (SaaS) ist die Kommunikation für die Benachrichtigungskomponente immer verschlüsselt.
endif::[]
...
ifdef::saas[]
Dieser Check ist derzeit nur für On-Premises-Editionen relevant. In {CE} (SaaS) wird die Kommunikation des Web Frontends mittels AWS WAF v2 abgesichert.
endif::[]
