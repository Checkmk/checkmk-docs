...
[TIP]
====
Das Monitoring von VMware ESXi mit {CE-saas} ist per xref:glossar#checkmk_relay[{CMK} Relay] möglich.
Dabei muss der Host des xref:glossar#special_agent[Spezialagenten] für VMware ESXi über das {CMK} Relay überwacht werden.
Eine Überwachung mit {CE-saas} ohne Relay erfordert, Ports für den Zugriff auf die VMware Management API aus dem Internet freizugeben.
...
Tipp: Um zu prüfen, ob der Nutzer den Anforderungen entspricht, genügt es, sich mit diesem Benutzer in die Weboberfläche des ESXi-Servers (VMware Host Client) einzuloggen.
Sollte der Login fehlschlagen, verfügt der Nutzer womöglich nicht über ausreichende Rechte.
...
Aktivieren Sie die ausstehenden Änderungen wie üblich.
ifdef::onprem[]
