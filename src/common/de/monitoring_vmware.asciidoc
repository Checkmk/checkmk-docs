...
// TK: Es spricht nichts dagegen, bei so einer Gelegenheit die Beschreibung zu vereinheitlichen, denn warum wird hier die Ausführung des Kommandos gezeigt, bei azure, aws & Co aber nicht?
Das Kommando xref:piggyback#orphaned_piggyback_data[`cmk-piggyback list orphans`] zeigt Ihnen alle Piggybacked-Hosts ohne Zuordnung:
...
Zu den aufgelisteten Objekten kann {CMK} keinen Host mit dem gleichen Namen finden, um die Daten zuzuordnen.
Sie können aber die Piggyback-Namen auf verschiedene Weisen xref:rename_piggyback[anpassen].
endif::[]
