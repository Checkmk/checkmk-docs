...
Seit {CMK} Version {v23} gibt es für diesen Zweck mit [.guihint]#Check HTTP web service# einen komplett neu geschriebenen aktiven Check, der den bisher einzig verfügbaren [.guihint]#Check HTTP service# ersetzen wird.

Das Vorgehen mit [.guihint]#Check HTTP web service# ist wie folgt:
...
Den Service-Namen (z. B. `Webserver`) und gegebenenfalls einen Präfix (`HTTP` oder `HTTPS`) legen Sie beim Endpunkt fest.
...
Im obigen Beispiel wurden in der Regel zwei Endpunkte definiert:
Der HTTP-Endpunkt überprüft die Antwortzeit und der HTTPS-Endpunkt die Zertifikatsgültigkeit.
