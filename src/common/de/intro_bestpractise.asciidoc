...
Wenn Sie jetzt die ausstehenden Änderungen aktivieren, erhalten Sie im Monitoring den neuen Host `internet` mit dem einzigen Service `Internet connection`.
...
. Speichern Sie die Regel und aktivieren Sie die ausstehenden Änderungen.
...
Statt des Standardwerts von 20 GiB zeigt die Tabelle die Referenzgröße von 16 GiB, weil das einfach besser in die Reihe der sich vervielfachenden Beispielwerte passt:
...
// Auskommentiert, da der Ratschlag nur sehr schwer (wenn überhaupt) umzusetzen ist.
////
