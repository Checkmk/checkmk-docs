...
// TK: Hier gibt es ganz schön viele Abschnitte und Unterabschnitte für die paar ollen Optionen...
...
vom einfachen [.guihint]#2 hours#, welches die Wartung *ab sofort* definiert, bis hin zu der Angabe eines expliziten Zeitraums, mit dem auch eine Wartung in der Zukunft definiert werden kann.
...
// TK: Host-Übersicht? Meiner Meinung nach gibt es nur Unterschiede bei Hosts und bei Services - und auch nur in den Advanced options.
// TK: Wenn es eine Option nur hier oder dort gibt, dann bei der Option beschreiben.
...
// TK: Gibt es Repeat nur in den kommerziellen Editionen? Dann sollte das hier stehen.


=== Advanced options

Im Abschnitt [.guihint]#Advanced options# finden Sie weitere Möglichkeiten, um Wartungszeiten zu definieren:
// TK: Hier bereits den Screenshot einfügen. Da unten ist er zu spät.
// TK: Im Screenshot alle Optionen zeigen, also auch die "Include indirectly connected hosts (recursively)", da man sonst nicht weiß, wo die zu finden ist.


==== Only for hosts: Set child hosts in downtime
// TK: Ich fand es noch nie so dolle, als Überschriften die englischen GUI-Texte zu nehmen, aber egal...
// TK: Das ist doch die einzige Option, die es nur für Hosts gibt.

Diese Option ist für Router und Switche nützlich, aber z.B. auch für Virtualisierungs-Hosts.
{CMK} wird so auch automatisch auf alle Hosts eine Wartungszeit setzen, welche über den betroffenen Host direkt (oder bei ausgewähltem [.guihint]#Include indirectly connected hosts (recursively)# auch indirekt) erreichbar sind.
...
// TK: Es gibt aber nicht nur Schedule downtime on host, sondern für Services auch noch Schedule downtime on service
// TK: Diese etwas komplizierte Beschreibung in diesem Abschnitt ist durch das GUI-Redesign eigentlich obsolet - bzw. Beschreibung muss angepasst werden.
// TK: Ich kann jetzt schon im Dialog mit dem passenden Knopf die richtige Entscheidung fällen.
// TK: Früher war alles besser - quatsch: ... gab es gar keine Knöpfe "Schedule on...", sondern man hat die Wartungszeit durch Klick auf den Zeitraum gestartet (das waren alles Buttons).
// TK: Nur durch eine Rückfrage konnte man dann entscheiden ob man für den Host oder die Services setzten wollte. In der Rückfrage gibt es diese Entscheidung jetzt nicht mehr (ist ja überflüssig).
...
Wenn Sie, beispielsweise in der Ansicht [.guihint]#Services of Host,# Wartungszeiten gesetzt haben, würden sich die geplanten Wartungszeiten auch tatsächlich direkt auf alle bzw. alle ausgewählten Services beziehen.
Sollte dies einmal nicht gewünscht sein, müssen Sie nicht zuerst in die Ansicht [.guihint]#Status of Host# navigieren, sondern können mithilfe dieser Option die Wartungszeit direkt auf den Host setzen.
...
Wie in jeder Ansicht können Sie über die icon:icon_filter[] Filter die Auswahl einschränken.
Über die icon:commands[] Kommandos in dieser Ansicht können Sie eine oder mehrere Wartungszeiten entfernen und sogar auch nachträglich ändern (nur kommerzielle Editionen), z.B. wenn diese verlängert werden müssen, da die eigentliche Wartung länger dauert als zunächst vorgesehen.

image::basics_downtimes_editdowntimes.png[alt="Ansicht der Wartungszeiten."]
...
image::basics_downtimes_downtimes_history.png[alt="Übersicht über vergangene Wartungszeiten."]
...
Wenn es nur um die xref:notifications#[Benachrichtigungen] geht, können Sie hierzu mit xref:timeperiods#[Zeitperioden] und dem Regelsatz [.guihint]#Notification period for Hosts/Services# arbeiten.
// TK: Hier besser 2x ins Glossar verlinken.
...
// TK: Warum Plural? Ist doch nur ein Kommando.

Die erste Methode ist der Weg, den wir bereits für einmalige
Wartungszeiten kennengelernt haben -- über ein Kommando -- aber mit der Option [.guihint]#Repeat#

image::basics_downtimes_periodic.png[alt="Die Wiederholungsperiode auswählen."]
// TK: Der Screenshot sieht echt schlimm aus. Ich weiß, es geht nicht besser ... trotzdem ;-)

Dabei wählen Sie die Periode aus, mit der sich die Wartung wiederholen soll. Das
*erste* Auftreten tragen Sie wie gewohnt ein. Von dem hier eingetragenen Startzeitpunkt aus wird die
...
Eine alternative, sehr elegante Methode für das Einrichten von periodischen Wartungszeiten ist das Anlegen über xref:wato_rules#[Regeln.]
// TK: Hier auch ins Glossar verlinken.
Mithilfe von
