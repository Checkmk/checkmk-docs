...
[#bilder]
== Bilder, Alt-Texte, Bildunterschriften

Alle Bilder sollen einen Alt-Text enthalten und alle Bilder in Blockelementen eine Bildunterschrift.

Alle Bilder in Blockelementen können in einer Lightbox mit Karussell angesehen werden.
Normale Bilder, d. h. solche ohne spezielles Attribut, behalten dabei ihre Größe:
...
[#aktionsknoepfe]
== Aktionsknöpfe, Buttons, Ikonen allgemein
...
Dann muss aber zunächst in Docserve und anschließend in den CI-Builds sichergestellt werden, dass Highligting und Kopierverhalten stimmt.

=== Eine traditionelle POSIX-Shell

Der Typ `shell` lässt Makroersetzungen und Span-Klassen zu.
Aus diesem Grund müsst Ihr mit Backticks, dem Dollarsymbol und Hashes vorsichtig sein.
Dieser Typ aktiviert den Copybutton per Default:
...
Wird nur eine Ausgabe gezeigt, sollte kein Copybutton angezeigt werden:
...
=== Eine POSIX-Shell ohne Ersetzungen

Da dieser Typ eine Variante des eben gezeigten ist, ebenso mit Copybutton:
...
Auch die bekommt einen Copybutton:
...
=== DOS-Sessions (cmd.exe)

Für die CMD existiert ein eigener Typ. Für diesen muss der Copybutton ggf. von Hand aktiviert werden:
...
=== Ein interaktiver Python-Prompt

Das Öffnen des Python-Prompts geschieht noch in einer normalen Shell-Umgebung:
