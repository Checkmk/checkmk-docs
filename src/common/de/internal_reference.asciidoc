...
|4317 |TCP |OpenTelemetry (über gRPC) | Empfang von xref:opentelemetry#[OpenTelemetry] Metriken _(optional aktivierbar in {CE})_
|4318 |TCP |OpenTelemetry (über HTTP/HTTPS) | Empfang von xref:opentelemetry#[OpenTelemetry] Metriken _(optional aktivierbar in {CE})_
|6559 |UDP | link:https://checkmk.com/werk/8350[Realtime-Checks^] |Empfang von UDP-Paketen für die Realtime-Checks einzelner Dienste (selten verwendet, _optional aktivierbar_)
endif::[]
ifdef::saas[]
...
.[#thisisanid]#Setzen wir Collapsibles sparsam genug ein?#
[%collapsible]
====
Keine Ahnung.
...
// TK: Wie meinen?
Dann muss aber zunächst in docserve und anschließend in den CI-Builds sichergestellt werden, dass Highligting und Kopierverhalten stimmen.
...
Für die Windows-CMD gibt es den Source-Block `cmd`.
Der hat per Default keinen Kopierknopf.
Im folgenden Beispiel wurde er mit `[.copybutton]` manuell hinzugefügt:

[.copybutton]
[{cmd}]
----
C:\Windows\System32> shutdown.exe /s /t 30 /c "damned thing"
Going to halt now!
----
...
Und ein Beispiel mit Highlighting für Substrings (in gelb):
...
Auch einzelne xref:tab_linux[Tabs einer Tabgruppe] und ein xref:thisisanid[Collapsible] können verlinkt werden.
