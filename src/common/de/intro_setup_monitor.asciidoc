...
Bevor wir mit den ersten Schritten ins Monitoring beginnen, werden wir kurz einige wichtige Begriffe erläutern.
...
Eine Abkürzung und mehr Sicherheit bietet hier das Slideout an, welches die Authentisierung für diese Aktionen jeweils mit einem Token durchführt.
// Es handelt sich dabei um ein Einmal-Token (_one-time token_ oder _OTT_), das nach der Generierung eine Woche gültig ist.
// temporär gültig?
...
Stattdessen erfolgt auch bei dieser Aktion, wie schon zuvor bem Herunterladen des Agentenpakets, die Authentisierung per Token.
...
.Das Slideout zur Erstellung eines On-Premises-Hosts
