// -*- coding: utf-8 -*-
// IGNORE 2c muc OTT Slideout Slideouts Willkommens
// NONASCII
...
////
TK: Fürs Review
TK: Die alten Kapitel habe ich erstmal auskommentiert stehenlassen.
TK: Anders als zuerst geplant, hab ich für saas die Erstellung des 1. Hosts noch nicht umgeschrieben:
TK: Der alte Onboarding Guide ist zurzeit noch da und ob in der saas die automatische Hostregistrierung bleibt oder auch wegkommt, weiß ich noch nicht.
////
...
Die Aktionen, die mit der Willkommens-Seite ausgeführt werden können, benötigen Berechtigungen, die in der Regel nur {CMK}-Administratoren haben.
...
Wenn Sie einen Schritt erledigt haben, wird dieser als abgeschlossen markiert und Sie können mit dem nächsten Schritt fortsetzen.
...
Wenn Sie ausführlichere Informationen suchen, dann finden Sie diese, wie von {CMK} gewohnt, in diesem Handbuch und insbesondere auch im diesem Leitfaden für Einsteiger.
...
Über das Snapin [.guihint]#Welcome to {CMK}# öffnet sich ein Slideout, dass die [.guihint]#Topics to explore# zeigt.
...
// TK: Mit dem Relay ist diese Unterscheidung onprem und saas nicht mehr notwendig.
////
ifdef::onprem[]
...
// TK: Mit dem Relay ist diese Unterscheidung onprem und saas nicht mehr notwendig. Der Satz stimmt auch für saas, da der Relay Teil von {CMK} ist.
////
ifdef::onprem[]
...
Falls es Sie interessiert, können Sie sich den Rest ansehen, indem Sie bei jedem der geöffneten Kästen die drei Auslassungspunkte icon:button_showmore[alt="Symbol zum Wechsel in den Show-more-Modus."]  anklicken und die beiden zugeklappten Kästen am Ende der Seite öffnen. Wie am Anfang erwähnt, ist {CMK} ein komplexes System, das auf jede Frage eine Antwort hat.
...
Diesen Namen können Sie recht frei vergeben und er darf bis zu 240 Zeichen lang sein.
...
Falls Sie die Datei in ein anderes Verzeichnis heruntergeladen haben, ersetzen Sie im folgenden Installationsbefehl das Verzeichnis `/tmp/` mit eben diesem Verzeichnis.
...
Falls sich die heruntergeladene Datei nicht auf dem Ziel-Host für die Installation des Agenten befinden sollte, müssen Sie die Datei zuerst auf den Ziel-Host kopieren, zum Beispiel mit dem Befehlszeilentool `scp`.
...
Die Installation erfolgt als `root` auf der Befehlszeile, für die RPM-Datei mit `rpm`, am besten mit der Option `-U`, die für _Update_ steht und dafür sorgt, dass die Installation auch dann fehlerlos durchläuft, wenn bereits eine alte Version des Agenten installiert ist:
...
Die Befehlszeile können Sie geöffnet lassen.
...
Nach Klick auf [.guihint]#Download {CMK} agent# öffnet sich das Slideout [.guihint]#Install Checkmk agent#, das Sie durch die drei Schritte führen wird, um den {CMK}-Agenten zum Laufen zu bringen:

.Das Slideout zur Installation des {CMK}-Agenten mit dem aktiviertem ersten Schritt
...
Dadurch wird zuerst der {CMK}-Agent auf dem Host heruntergeladen und anschließend installiert.
...
Die Registrierung erfolgt durch Aufruf des Agent Controllers `cmk-agent-ctl` als `root` auf der Befehlszeile.
Für den Befehl benötigen Sie die Namen des {CMK}-Servers (im Beispiel `myserver`), der {CMK}-Instanz (`mysite`) und des Hosts (`localhost`), wie er xref:create_host[soeben] in {CMK} eingerichtet wurde.
...
Standardmäßig benötigt der Befehl den Namen und das Passwort eines {CMK}-Benutzers mit der Berechtigung, diese Registrierung durchzuführen.
Eine Abkürzung und Vereinfachung bietet hier das Slideout mit der Möglichkeit ein Einmal-Token (_one-time token_ oder _OTT_) zu generieren und damit zu registrieren.
...
// TK: Folgender Satz ist neu.
Es kann übrigens bis zu maximal einer Minute dauern, bis ein frisch konfigurierter Agent die Informationen zu den gefundenen Services liefert.
...
Informationen dazu, wie man ausstehende Änderungen wieder zurückzieht finden Sie im xref:wato#revert_changes[Artikel über die Konfiguration von {CMK}].
////
...
Die Nummer im Menü [.guihint]#Changes# ist verschwunden und bleibt es auch solange, bis die nächsten Änderung ansteht.
...
Zur Erfolgskontrolle wechseln Sie abschließend einmal in die Monitoring-Umgebung, zum Beispiel in dem Sie in der Navigationsleiste den Eintrag [.guihint]#Monitor > Overview > Main dashboard# auswählen.
...
// TK: Noch nicht überarbeitet, noch nicht ready for review!
ifdef::saas[]
[#first]
== Den ersten Host aufnehmen
...
Das Thema zeigt, wie Sie in wenigen Schritten Ihren _On-Premises_-Rechner (Linux- oder Windows) in das Monitoring aufnehmen.
...
Die beiden Befehle zum Herunterladen und zur Installation können Sie mit den beiden grünen Knöpfen rechts von den Befehlen in die Zwischenablage kopieren.

Nach der Ausführung des Befehls zur Installation des {CMK}-Agenten finden Sie den neuen Host nach wenigen Minuten in der xref:glossar#monitoring_environment[Monitoring-Umgebung] via [.guihint]#Monitor > Overview > All hosts#:
...
. Agent herunterladen +
Das passiert in Schritt 2 und 3 im [.guihint]#Welcome#-Thema des Onboarding Guide.
Alternativ zu den gezeigten Befehlen, die die {CMK} xref:rest_api#[REST-API] nutzen, kann ein Installationspaket auch über die {CMK}-Oberfläche heruntergeladen werden.
...
. Agent installieren +
Die Installation erfolgt in Schritt 4 des [.guihint]#Welcome#-Themas im Onboarding Guide.
...
Gleich im ersten Thema [.guihint]#Add your first host# finden Sie Knöpfe, mit denen Sie die Erstellung von Hosts für On-Premises- und Cloud-Umgebungen starten können.
...
Sobald der Agent installiert ist, können Sie den Host in {CMK} erstellen, per Befehl registrieren und ins Monitoring aufnehmen.
...
Bevor SNMP-Geräte für die Nutzung eines {CMK} Relay konfiguriert werden können, muss der Relay zuerst einmal eingerichtet werden.
...
// TK: Der Link und der Verweis auf den Ordner Network in Schritt 3 passt (noch) nicht für die saas.
...
ifdef::onprem[]
. Falls der Host über einen {CMK} Relay überwacht wird, aktivieren Sie in den Eigenschaften des Hosts, im Kasten [.guihint]#Basic settings# das Attribut [.guihint]#Monitored on relay# und wählen Sie den Namen des zuvor eingerichteten {CMK} Relay aus.
endif::[]
...
+
.Ein {CMK} Relay kann erst dann ausgewählt werden, wenn er zuvor eingerichtet wurde
...
ifdef::saas[]
// TK: für saas Onboarding Guide überprüfen
...
Im nächsten Kapitel werden die Teile der Benutzeroberfläche genauer vorgestellt, die wir in diesem Kapitel nur gestreift haben.
