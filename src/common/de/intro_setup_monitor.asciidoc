...
// TK: Schritte sind die Unterpunkte eines Themas. Aufgabe ist Quatsch. Ich habs geändert.
...
// TK: Da hast Du ja gleich den Finger in die Wunde gelegt:
// TK: Ich will hier komplett unabhängig von der Willkommens-Seite beschreiben. Das steht auch oben so ähnlich im Kap. zur Willkommens-Seite, vielleicht nicht klar genug...
// TK: Das konkrete Problem hier ist: Wenn ich aus der Welcome page "Server (Linux, Windows, Solaris, ...)" anklicke, erstelle ich einen Host immer im Main folder, auch wenn ich zuvor via Welcome page eine hübsche Ordnerstruktur kreiert habe. Das Problem ist übrigens im SPN-Team bekannt, gibt aber keine Lösung. Also führe ich den Leser hier manuell in den Linux-Ordner, damit der Host auch dort erstellt wird, wo er hingehört.
...
// TK: Ja. Ich habs umformuliert und weiter unten ausgewählt statt aktiviert geschrieben.
...
// TK: Ja, ich hab tatsächlich die 2 Schritte manuell abgehakt, weil sie dann zu dem passen, was hier manuell durchgezogen wurde. Ist aber fragwürdig. Soll ich das ändern und auf 0 setzen?
// TK: Ja, der Hinweis zum manuellen Abhaken sollte rein, oben ins Kap. zur Willkommens-Seite?
