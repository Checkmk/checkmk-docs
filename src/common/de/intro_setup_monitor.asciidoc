...
:description: Nach der Einrichtung von {CMK} nehmen Sie Hosts ins Monitoring auf. Lesen Sie hier, wie Sie Services von Hosts konfigurieren und ausstehende Änderungen aktivieren.
...
Erläuterungen zu den zentralen Begriffen „Hosts“ und „Services“ finden Sie im Artikel xref:monitoring_basics#hosts_services[Grundlagen des Monitorings].

Das Konzept der „Agenten“ in {CMK} ist im Artikel xref:wato_monitoringagents#[Monitoring-Agenten] beschrieben.

// ES: Der folgende Abschnitt ist auskommentiert, weil wir die Erklärung auslagern möchten. Ich lösche nichts, damit wir später sicherstellen können, dass die Erklärung im verlinkten Artikel wirklich äquivalent ist zu dem, was hier ursprünglich stand.
////
Bevor Sie mit den ersten Schritten ins Monitoring beginnen, werden wir kurz einige wichtige Begriffe erläutern.
...
Informationen zur Ordnerstruktur finden Sie in den Artikeln über die xref:hosts_setup#folder[Verwaltung] und die xref:hosts_structure#folder[Strukturierung] von Hosts.
...
Falls Sie Host-Namen verwenden, die nicht per DNS auflösbar sind, können Sie die IP-Adresse auch von Hand im Feld [.guihint]#IPv4 address# eintragen.
...
image::intro_ipv4_inlinehelp.png[alt="Die Eigenschaften eines Hosts mit Inline-Hilfe für die IPv4-Adresse."]
...
Eine Abkürzung und mehr Sicherheit bietet hier das Slideout an, welches die Authentisierung für diese Aktionen jeweils mit einem _Einmal-Token_ durchführt, das für eine erfolgreiche Ausführung innerhalb eines  begrenzten Zeitraums gültig ist.

Bevor das Token genutzt werden kann, muss es erstellt werden.
Klicken Sie also zuerst auf [.guihint]#Generate one-time token#.
...
Stattdessen erfolgt auch bei dieser Aktion, wie schon zuvor beim Herunterladen des Agentenpakets, die Authentisierung per Einmal-Token.
Nachdem Sie [.guihint]#Generate one-time token# angeklickt haben, sieht das Slideout etwa so aus:
...
Das Einmal-Token wird im gezeigten Befehl mit der Option `-ott` genutzt.
...
Dabei wird der gleiche Test der Agentenverbindung durchgeführt, der im Abschnitt xref:download_agent[Agent herunterladen und installieren] beschrieben ist.
====
...
Für den Anfang ist es am einfachsten, wenn Sie jetzt auf den Knopf [.guihint]#Accept all# klicken,
der alles auf einmal macht: fehlende Services hinzufügen, verschwundene entfernen -- und zusätzlich gefundene Änderungen an Host- und Service-xref:glossar#label[Labels] übernehmen.
...
Diese Services erscheinen dann als [.guihint]#Undecided#, und Sie können sie einzeln oder alle auf einmal in das Monitoring aufnehmen.
...
Wie viele Änderungen sich bisher angesammelt haben, die noch nicht aktiviert wurden, sehen Sie in der Navigationsleiste, im Symbol [.guihint]#Changes#, und dort an der Nummer im gelben Kreis.
Wenn Sie das Symbol anklicken, sehen Sie die Details dieser noch ausstehenden Änderungen:
...
In diesem Slideout werden die wichtigsten Informationen angezeigt.
...
Das ist zum Beispiel dann notwendig, falls das Slideout Probleme mit der Aktivierung oder mit Instanzen anzeigt, die die Aktivierung verhindern können.
...
Die Nummer im Symbol [.guihint]#Changes# ist verschwunden und bleibt es auch solange, bis die nächste Änderung ansteht.
...
ifdef::saas[]
[#first_auto]
== Den ersten Host automatisch aufnehmen

Jetzt ist alles dafür vorbereitet, um den ersten Host in das Monitoring aufzunehmen.
Eine Möglichkeit ist, den Rechner zu überwachen, von dem aus Sie die Weboberfläche von {CE} aufrufen.

Dies können Sie bequem mit Hilfe der xref:intro_welcome#[Willkommensseite] erledigen.
...
Eine Abkürzung und mehr Sicherheit bietet hier das Slideout an, welches die Authentisierung für diese Aktion mit einem _Einmal-Token_ durchführt, das für eine erfolgreiche Ausführung innerhalb eines begrenzten Zeitraums gültig ist.
...
Im Slideout können Sie mit [.guihint]#Next step# den dritten und letzten Schritt auswählen, mit [.guihint]#View hosts# das Slideout schließen und die Seite [.guihint]#Main > preconfigured_agent# in der xref:glossar#configuration_environment[Konfigurationsumgebung] öffnen:
...
Im Artikel über die xref:wato#activate_changes[Konfiguration von {CMK}] erfahren Sie übrigens mehr über den Unterschied zwischen Konfigurations- und Monitoring-Umgebung.
...
Dass das alles so einfach funktioniert, liegt daran, dass in {CE} für alle unterstützten Betriebssysteme die Pakete mit dem {CMK}-Agenten vorkonfiguriert bereitstehen --
für die xref:hosts_autoregister#[Autoregistrierung eines Hosts], das heißt für die Registrierung des Agenten, die Erstellung des Hosts, die Service-Erkennung und die Aktivierung der daraus resultierenden Änderungen.
...
Bei der Einrichtung von Hosts für gängige Anwendungsfälle erhalten Sie Unterstützung in der xref:intro_welcome#[Willkommensseite].
