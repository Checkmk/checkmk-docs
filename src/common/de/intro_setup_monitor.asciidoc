...
// MFS: Man kann keine Namen anpingen, erst wird die Adresse aufgelöst, dann die Erreichbarkeit geprüft.
...
// MFS: Müssen wir da den Usecase von Ad-Hoc-Namen in kleinen Netzen legitimieren? Wahrscheinlich nicht... Leider harmoniert Checkmk nicht wirklich gut mit DHCP Client Option 12.
Falls Sie kein DNS verwenden möchten, können Sie die IP-Adresse auch von Hand im Feld [.guihint]#IPv4 address# eintragen.
// MFS: Ich würde eher formulieren:
// Falls Sie Hostnamen verwenden, die nicht per DNS auflösbar sind, ...
Wenn Sie eine IP-Adresse eintragen, pingt {CMK} nicht länger den Host-Namen, sondern diese IP-Adresse.
// MFS: Ich würde eher formulieren:
// Wenn Sie eine IP-Adresse eintragen, überspringt {CMK} die Prüfung auf Auflösbarkeit und testet direkt die Erreichbarkeit dieser IP-Adresse.
