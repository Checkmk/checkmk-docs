// -*- coding: utf-8 -*-
// IGNORE 2c muc preconfigured_agent Willkommensseite
// NONASCII
...
Dass dass alles so einfach funktioniert, liegt daran, dass in {CE} für alle unterstützten Betriebssysteme die Pakete mit dem {CMK}-Agenten vorkonfiguriert bereitstehen --
...
Das prinzipielle Vorgehen ist für einen Linux- und für einen Windows-Hosts identisch und wird im Folgenden nur skizziert:
...
Schiefgehen kann vor allem dann etwas, wenn man es zum ersten Mal probiert.
Daher sind gute Möglichkeiten zur Fehlerdiagnose wichtig. +
Die Artikeln zu xref:agent_linux#test[Linux] und xref:agent_windows#test[Windows] widmen dem Thema Test und Diagnose ein eigenes, ausführliches Kapitel. +
+
