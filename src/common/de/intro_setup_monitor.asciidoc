...
Dass das alles so einfach funktioniert, liegt daran, dass in {CE} für alle unterstützten Betriebssysteme die Pakete mit dem {CMK}-Agenten vorkonfiguriert bereitstehen -- für die automatische Registrierung des Agenten, die Erstellung des Hosts und die Aktivierung der daraus resultierenden Änderungen.
...
[#activate_changes]
Vor allem für die manuelle Variante ist es von Vorteil, die einzelnen Schritte im Prozess zu kennen, mit der ein Host in das Monitoring aufgenommen wird.
