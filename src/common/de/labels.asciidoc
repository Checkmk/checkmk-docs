...
////
MFS: Alt, hochgezogen, weil nicht mehr aktuell. Soll stattdessen ein Teil des auskommentierten Textes drüber wieder aktiviert werden? IMHO: Nein. Das sollte die Übersicht der Check-Plugins resp. die Artikel monitoring_azure etc. leisten.
...
Führen Sie hier am besten die xref:hosts_setup#bulk_operations[Bulk-Aktion] zur Service-Erkennung durch ([.guihint]#Run bulk service discovery#)
...
Eine wichtige Funktion von xref:glossar#label[Labels] ist die gleiche wie bei Host-Merkmalen:
