...
{CMK} kann mit der xref:dcd#[dynamischen Host-Konfiguration] solche dynamischen Hosts automatisch anlegen und bekommt dabei auch die Information über die dort bereits vorhandenen Labels/Merkmale.
...
{CCE} und {CME} verfügen über die Möglichkeit, xref:hosts_autoregister#[Hosts automatisch erstellen] zu lassen.
