...
= Labels
:title: Labels
:description: Labels für Hosts und Services in {CMK} helfen Ihnen, die Überwachung zu strukturieren. Lesen Sie hier, wie Sie Labels erstellen, verwalten und nutzen.
...
Dazu gibt es den Regelsatz [.guihint]#Host labels#, den Sie schnell über die xref:user_interface#search[Suche] im [.guihint]#Setup#-Menü finden können.
...
Der Discovery Check wird bei der nächsten Ausführung (in bis zu zwei Stunden) dann wieder {OK}, selbst wenn Sie noch nicht xref:glossar#activate_changes[die ausstehenden Änderungen aktiviert] haben.
...
Haben Sie mehrere Regeln erstellt, so werden nachfolgende Regeln dadurch unter Umständen nicht mehr ausgewertet, siehe xref:wato_rules_about#matching[Arten der Regelauswertung].
====
...
|`cmk/device_model` |Vom Gerät übertragene Modellbezeichnung
