...
:description: Der {CMK} Micro Core (CMC) hat einige Besonderheiten gegenüber anderen Nagios-kompatiblen Kernen. Die wichtigsten Unterschiede werden hier beschrieben.
...
In allen Editionen kommen als Speicher für Daten, die den Monitoring-Kern passiert haben, xref:graphing#rrds[Round-Robin-Datenbanken (RRDs)] zum Einsatz.

Bei Verwendung des Nagios-Kerns haben zur Aktualisierung geschriebene Daten einen recht langen Weg. 
Es kommen dabei Spool-Dateien, Perl-Skripte und ein Hilfsprozess (`npcd`) zum Einsatz, der in C geschrieben ist. 
Am Ende werden leicht umgewandelte Daten in das Unix-Socket des RRD-Cache-Daemons geschrieben.

Der CMC kürzt diese Kette ab, indem er _direkt_ zum RRD-Cache-Daemon schreibt -- alle Zwischenschritte werden ausgelassen. 
