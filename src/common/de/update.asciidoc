// -*- coding: utf-8 -*-
// IGNORE CTRL+D Benachrichtigungs XYZ p22 p23
...
====
// TK: Etwas zugefüttert. Ich find es ungünstig, wenn in update_major mehr steht als in update, denn in update_major fliegt die Info ja tendenziell in der nächsten Version raus.
// TK: Strg durch CTRL ersetzt, wie sonst im Handbuch auch.
...
Finished update.
----
// TK: Neuen Test-Output generiert und die folgende Zeile geändert.
...
// TK: Die beiden Screenshots müssen neu gezogen werden, wenn die 2.3.0p1 rauskommt.
...
Sie können in einem Schritt auch zur {CE} upgraden, beachten Sie in diesem Fall jedoch auch die Hinweise im folgenden Abschnitt.
...
das heißt während des Mischbetriebes werden Services der {RE} Remote-Instanzen bereits als {SE} abgerechnet.
...
. Im verteilten Monitoring ohne xref:glossar#distributed_setup[verteiltes Setup] müssen auch die Remote-Instanzen gleich nach dem Upgrade lizenziert werden.
...
// TK: Sind schon sichtbar. In diesem Fall finde ich die Formatierung sehr passend, weil der Text im Tip zusammengehalten wird.
[TIP]
====
...
. Im verteilten Monitoring ohne xref:glossar#distributed_setup[verteiltes Setup] müssen nun auch die Remote-Instanzen gleich nach dem Upgrade lizenziert werden.
...
Auch Downgrades zwischen den Editionen sind möglich.
...
Andere Downgrades als die im folgenden beschriebenen sind nicht vorgesehen und werden von uns nicht unterstützt, beispielsweise ein Downgrade der {ME}.
// TK: Satz oben umformuliert
...
Eine Übersicht der Unterschiede zwischen {CE} und {SE} liefert der xref:cce#[Artikel zur {CE}.]
...
// TK: unter Umständen? Warum sollten die noch da sein?
Im xref:cse#[Artikel zur {SE}] können Sie ermitteln, wie viel {SE}-Funktionalität im Falle des Downgrades nach {RE} wegfällt und wo Sie gegebenenfalls weitere Anpassungen vornehmen müssen.
