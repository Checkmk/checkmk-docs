// -*- coding: utf-8 -*-
// IGNORE Benachrichtigungs Ctrl+D XYZ p22 p23
include::global_attr.adoc[]
= Updates und Upgrades
:revdate: draft
:title: Updates und Upgrades
:description: Egal, ob {RE} oder eine der kommerziellen Editionen: {CMK} zu aktualisieren ist so einfach, wie möglich. Die Prinzipien dazu werden hier erklärt.
...
Dies ersetzt kein vollständiges Backup, hilft aber in vielen Fällen, Ausfallzeiten zu verkürzen.
====
...
Wenn Sie gleichzeitig zum Versions-Update ein Editions-Upgrade von einer {RE} auf eine der kommerziellen Editionen durchführen, werden Sie sicherheitshalber noch einmal auf diesen Umstand hingewiesen:
...
[{shell}]
----
2024-03-10 16:49:42 - Updating site 'mysite' from version 2.2.0p23.cee to 2.3.0p1.cee...
...
Noch praktischer ist aber die in {CMK} eingebaute Funktion zur Recherche in den Release Notes.
Nachdem Sie sich an der Instanz angemeldet haben, finden Sie im [.guihint]#Help#-Menü den rot hinterlegten Link:
//SP: Nur eine Kleinigkeit, aber hier ist ein Logikbruch: du hast im vorherigen Abschnitt auf 2.2.0 aktualisiert, aber im Screenshot steht jetzt wieder 2.1.0. Gilt eventuell auch für den nachfolgenden Screenshot.
...
Sie können jedes „Werk“, d.h. jede Änderung, einzeln aufrufen, ansehen, per Mausklick bestätigen -- und damit die Zahl der offenen, inkompatiblen Änderungen sukzessive verringern.
Zusätzlich können Sie mit dem [.guihint]#Filter#-Knopf die Filterleiste einblenden und haben so Zugriff auf die komplette Historie der Änderungen.
...
|{v23} |{v22} |*Nein* | *Achtung:* Bei xref:distributed_monitoring#distr_wato[zentraler Konfiguration] besteht hier die Gefahr, Remote-Instanzen irreparabel zu beschädigen. Vermeiden Sie diesen Zustand unbedingt!
...
// MFS: Abschaffung = removal, was removed in version 2.2.0?
...
Upgrades der {CRE} auf eine der kommerziellen Editionen oder von einer der kommerziellen Editionen auf eine andere mit größerem Funktionsumfang sind jederzeit unkompliziert möglich.
...
[#updateraw]
=== Upgrade der {RE} auf eine der kommerziellen Editionen

// ==== Einleitung
// Auch ein Upgrade der {CRE} auf eine der kommerziellen Editionen ist möglich. Auch hier ist das
...
Instanzen der {RE} werden auch hier beim Upgrade nicht automatisch umgestellt.
...
Um vollen Nutzen aus der {SE} ziehen zu können, beachten Sie die Übersicht der Unterschiede zwischen {RE} und xref:cse#[{SE}].
...
Eine vollständige Übersicht der zusätzlichen Funktionalität bietet der Artikel zur xref:cce#[{CE}.]
...
// TK: Diese Übertragung wird nur bei der Aktivierung der Änderungen durchgeführt, richtig?
// TK: Wenn ja, Hinweis hinzufügen, dass man die Änderungen der Konfiguration (und damit natürlich auch deren Aktivierung) per Setup > General > Read only mode unterbinden kann?
...
. Führen Sie das Upgrade der Zentralinstanz auf die {SE} durch.
. Hinterlegen Sie die Subskriptionsdaten und führen Sie eine Prüfung der Lizenzdaten durch.
// TK: xref:license#[Lizenzieren] Sie die {SE}, indem Sie die Subskriptionsdaten eingeben und anschließend übermitteln.
Die von allen Remote-Instanzen bereitgestellten Services werden bei der Berechnung der Subskription gleich behandelt.
// TK: den oder allen?
// TK: Inhalt versteh ich nicht: Was wird hier gleich behandelt?
// MFS:
D.h. während des Mischbetriebes werden Services der {RE} Remote-Instanzen bereits als {SE} abgerechnet.
. Aktualisieren Sie nach und nach die Remote-Instanzen.
. Falls Sie die Übertragung der Konfiguration nicht global, sondern für jede Remote-Instanz separat deaktiviert haben, können diese für jede Remote-Instanz, welche das Upgrade erhalten hat, gleich nach dem Upgrade wieder aktivieren.
// TK: Im verteilten Monitoring ohne verteiltes Setup (im aktuellen Sprachgebrauch) müssen auch die Remote-Instanzen lizensiert werden.
...
// MFS: Am Stylesheet hier arbeite ich noch. Momentan sind Absätze nicht erkennbar.
[TIP]
====
...
Nehmen wir an, Sie aktivieren eine {SE} spezifische Einstellung – beispielsweise xref:basics_downtimes#[Wartungszeiten] –, bevor alle Remote-Instanzen das Upgrade erhalten haben.
...
////
Wir empfehlen, mit der Aktivierung von {SE} spezifischen Features zu warten, bis alle Instanzen das Upgrade auf {SE} erhalten haben.
////
...
. Führen Sie das Upgrade der Remote-Instanzen auf die {CE} durch.
*Wichtig:* Nur im 30 Tage währenden Lizenzstatus „Trial“ ist der Mischbetrieb mit {SE} als Zentralinstanz möglich, hinterlegen Sie also noch keine {CE}-Subskriptionsdaten!
// TK: Der Hinweis gilt dann doch auch nur im verteilten Monitoring ohne verteiltes Setup, denn im verteilten Setup brauch ich die Remote-Instanzen gar nicht lizensieren.
// MFS: {CE}-Subskriptionsdaten ergänzt.
. Hinterlegen Sie in der Zentralinstanz noch unter der {SE} die Subskriptionsdaten für die {CE}.
// MFS: Mit/ohne zentraler Konfiguration unterscheidet sich der Zeitpunkt, zu dem Lizenzdaten hinterlegt werden sollten.
. Führen Sie das Upgrade der Zentralinstanz auf {CE} durch.
// TK: Wenn zuvor die Remote-Instanzen (noch nicht) lizensiert werden sollten, dann muss man es wohl jetzt als Abschluss tun. 
...
Soll ein Upgrade direkt von der {RE} auf {CE} erfolgen, müssen Sie auf der Zentralinstanz als Zwischenschritt auf die {SE} hieven:
...
[#down2cee]
== Downgrades
// TK: Beim Downgrade gibt es keinen Hinweis auf Lizenzierung. Beabsichtigt? Für mich ok. Ich wüsste auch gar nicht, was lizenztechnisch beim/nach dem Downgrade zu tun ist.
// MFS: Kommt drauf an: CEE läuft mit einer CCE Lizenz, AFAIK kann dann auch Sales eine Lizenz dowgraden, wenn bei Erneuerung in einen niedrigeren Tarif gewechselt wird, so dass nichts zu tun oder erst spät etwas zu tun ist.

// MFS: Aus Gründen der Konsistenz entfernt
// === Einleitung

Auch Downgrades zwischen den Editionen sind vorgesehen.
// TK: Einschränken: Downgrades zwischen den Editionen sind nur für die im folgenden beschriebenen Szenarien vorgesehen.
...
Andere Downgrades als die hier beschriebenen (beispielsweise von {ME} nach {CE}) sind nicht vorgesehen und werden von uns nicht unterstützt.
Stattdessen empfehlen wir, in diesem Fall mit einer frischen Instanz zu beginnen.
// MFS: reinnehmen?
// Selbstverständlich können Sie `omd -f` auf eigene Gefahr nutzen und gegebenenfalls die Konfiguration nach dem Downgrade manuell anzupassen. 
...
Falls Sie unter der {CE} das xref:grafana#[Grafana-Plugin] aus dem „Grafana Store“ genutzt haben, müssen sie dieses durch ein aus dem Zip-Archiv installiertes ersetzen.
...
Mit der xref:cse#[{SE} Übersicht] können Sie ermitteln, wie viel {SE}-Funktionalität im Falle des Downgrades nach {RE} wegfällt und wo Sie gegebenenfalls weitere Anpassungen vornehmen müssen.


[#editiondistributed_down]
=== Editions-Downgrades in verteilten Umgebungen

// TK: Einschränken: Die im vorherigen Kapitel vorgestellen Downgrade-Szenarien können auch in verteilten Umgebungen durchgeführt werden.
Die in den beiden vorherigen Kapiteln vorgestellten Downgrade-Szenarien können auch in verteilten Umgebungen durchgeführt werden.
