...
:description: Ihre {CMK}-Instanz kann vielfältige Diagnoseinformationen zur Fehleranalyse bereitstellen. Lesen Sie hier, wie Sie solche Dumps für den Support erzeugen.
...
ifdef::onprem[]
Die grundlegendsten Informationen stellen hierbei sicherlich die von Ihnen eingesetzte xref:cmk_versions.html#[Version] und xref:cmk_versions.html#suffix[Edition] von {CMK} dar.
...
[IMPORTANT]
====
Die Informationen, die Sie hier auswählen, *können* vertrauliche Daten enthalten.
Bitte gehen Sie entsprechend umsichtig vor!
====
...
ifdef::saas[]
Das Drop-down-Menü für die Instanzauswahl ist in {CE-saas} irrelevant, da hier bis auf weiteres nur die Arbeit mit einer einzigen Instanz möglich ist.
endif::[]
...
----
{
    ...
    "python_paths": [
        "/opt/omd/versions/2.5.0p1.community/bin",
        "/omd/sites/mysite/local/lib/python3",
        ...
        "/omd/sites/mysite/lib/python3"
    ],
    "python_version": "3.13.9 (main, Feb 12 2026, 15:52:20) [GCC 14.2.0]",
    "time": 1772037865.9526536,
    "time_human_readable": "2026-02-25 17:44:25.952654",
    "version": "2.5.0p1"
}
----
...
ifdef::onprem[]
Diese entsprechen genau dem, was Sie sich auf der Befehlszeile mit dem Befehl `omd config show` ausgeben lassen können.
endif::[]
...
[TIP]
====
Um in den Log-Dateien möglichst viele detaillierte Informationen über das Verhalten von {CMK} finden zu können, kann es mitunter notwendig sein, das sogenannte Log-Level in {CMK} kurzzeitig zu erhöhen.
...
[IMPORTANT]
====
In den Dateien, die Sie hier auswählen, können je nach Konfiguration streng vertrauliche Informationen, wie beispielsweise Passwörter, enthalten sein.
...
ifdef::onprem[]
[#commandline]
==  Support Diagnostics über die Befehlszeile
...
[cols="32,~"]
|===
|`--local-files` |Liste aller installierter, unpaketierter und optionaler Dateien unterhalb von `~/local/`.
Dies beinhaltet auch Informationen über installierte MKPs.
|`--omd-config` |Inhalt der Datei `~/etc/omd/site.conf`.
|`--checkmk-crashes` |Neuster Absturzbericht aus jedem Unterverzeichnis von `~/var/check_mk/crashes/`.
|`--checkmk-overview H`  |Informationen aus dem HW-/SW-Inventurknoten [.guihint]#Software > Applications > {CMK}# des {CMK}-Servers.
Für den Parameter `H` müssen Sie den Host-Namen des {CMK}-Servers angeben.
|`--checkmk-config-files FILE,FILE ...` |Konfigurationsdateien (`.mk` und `.conf`) aus dem Verzeichnis `~/etc/check_mk/`.
Alle gewünschten Dateien müssen explizit angegeben werden, und zwar alle Dateinamen relativ zu `~/etc/check_mk/`.
Beispiel: `--checkmk-config-files main.mk,apache.d/wato/global.mk`
|`--checkmk-log-files FILE,FILE ...` |Log-Dateien (`.log` und `.state`) aus `~/var/log/`.
Alle gewünschten Dateien müssen explizit angegeben werden, und zwar alle Dateinamen relativ zu `~/var/log/`.
Beispiel: `--checkmk-log-files apache/error_log,update.log,web.log`
|`--performance-graphs H` |Performance-Graphen (bspw. CPU load, CPU utilization) des {CMK}-Servers -- nur verfügbar in den kommerziellen Editionen.
Für den Parameter `H` müssen Sie den Host-Namen des {CMK}-Servers angeben.
|===
