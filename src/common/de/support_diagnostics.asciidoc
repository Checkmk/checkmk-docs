...
|`--checkmk-crashes` | Neuster Absturzbericht aus jedem Unterverzeichnis von `~/var/checkmk/crashes/`.
|`--checkmk-overview`  |Informationen aus dem HW-/SW-Inventurknoten [.guihint]#Software > Applications > {CMK}# der {CMK}-Server.
|`--checkmk-config-files FILE,FILE ...` |Konfigurationsdateien (`.mk` und `.conf`) aus dem Verzeichnis `~/etc/check_mk/`. Alle gewünschte Dateien müssen explizit angegeben werden. Die Dateinamen müssen dabei relativ zu `~/etc/checkmk/` angegeben werden.

Beispiel: `--checkmk-config-files main.mk,apache.d/wato/global.mk`
|`--checkmk-log-files FILE,FILE ...` |Log-Dateien (`.log` und `.state`) aus `~/var/log/`. Alle gewünschte Dateien müssen explizit angegeben werden. Die Dateinamen müssen dabei relativ zu `~/var/log/` angegeben werden.

Beispiel: `--checkmk-log-files apache/error_log,update.log,web.log`
|`--performance-graphs H` |Performance-Graphen (bspw. CPU load, CPU utilization) des {CMK}-Servers -- nur verfügbar in den kommerziellen Editionen. Für den Parameters `H` müssen Sie den Host-Namen des Checkmk-Servers angeben.
|===
