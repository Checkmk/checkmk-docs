...
:description: In {CMK} können Sie die Überwachung über das Setup-Menü konfigurieren. Lesen Sie hier, wie Sie das Setup-Menü nutzen und ausstehende Änderungen aktivieren.
...
ifdef::onprem[]
und die xref:relay#[{CMK} Relays] (nur in {UE}).
endif::[]
ifdef::saas[]
und die xref:relay#[{CMK} Relays].
endif::[]
...
und zur Agentenregistrierung um xref:hosts_autoregister#[Hosts automatisch zu erstellen] (nur in {CE}).
endif::[]
ifdef::saas[]
...
ifdef::onprem[]
Außerdem können Sie xref:views#icons[benutzerdefinierte Symbole] (_custom icons_) hinzufügen.
endif::[]
...
ifdef::onprem[]
|[.guihint]#Connectors# |Verbindungen zu anderen Systemen, zum Beispiel für den Export von Metriken an xref:metrics_exporter#[InfluxDB (und Graphite)].
...
|[.guihint]#Telemetry# |Empfang von xref:opentelemetry#[OpenTelemetry]-Daten
ifdef::onprem[]
(nur in {CE}) über den OpenTelemetry Collector oder einen Prometheus-Endpunkt. 
endif::[]
ifdef::saas[]
per OpenTelemetry Collector. 
endif::[]
|===
...
ifdef::onprem[]
.Das Snapin [.guihint]#Setup shortcuts# der {CE} im Show-more-Modus
image::wato_setup_shortcuts.png[alt="Snapin Setup shortcuts.",width=50%]
endif::[]

ifdef::saas[]
.Das Snapin [.guihint]#Setup shortcuts# im Show-more-Modus
...
Die Zahl der ausstehenden Änderungen, die noch nicht aktiviert sind, sehen Sie in der Navigationsleiste, im Menü [.guihint]#Changes#, und dort an der Nummer im gelben Kreis.
Wenn Sie das Symbol anklicken, sehen Sie die Details dieser noch ausstehenden Änderungen:

.Die Zusammenfassung der ausstehenden Änderungen
image::wato_menu_changes.png[alt="Das Menü 'Changes' zeigt die Liste der ausstehenden Änderungen.",width=86%]

Durch Klick auf den Knopf [.guihint]#Activate pending changes# werden die Änderungen angewendet.
...
Im oben gezeigten Slideout erhalten Sie eine Zusammenfassung der wichtigsten Informationen.
So können auf einen Blick feststellen, ob es bereits Probleme bei der Aktivierung oder mit Instanzen gibt.
Solche Probleme können verhindern, dass die ausstehenden Änderungen aktiviert werden.
In diesem Fall ist es ratsam, dass Sie sich den vollen Überblick mit der xref:glossar#view[Tabellenansicht] [.guihint]#Activate pending changes# verschaffen.
Diese Seite öffnen Sie aus dem Slideout durch Klick auf den Knopf, der Ihnen die drei Zahlen zeigt oder auf [.guihint]#Open full view#:

.Alle Informationen über die ausstehenden Änderungen
image::wato_pending_log.png[alt="Liste der ausstehenden Änderungen zur Aktivierung."]

Auf dieser Seite starten Sie Aktivierung der Änderungen durch Klick auf [.guihint]#Activate on selected sites#.

[TIP]
====
Sie können festlegen, ob ein Klick auf das Symbol [.guihint]#Changes# das Slideout oder sofort die Seite [.guihint]#Activate pending changes# anzeigen soll.
Die zugehörige Option [.guihint]#Default behavior for 'Changes'# finden Sie bei Ihren persönlichen Einstellungen im xref:user_interface#user_menu[User-Menü].
====

Für welchen Weg Sie sich auch entscheiden haben:
durch das Aktivieren der ausstehenden Änderungen wird aus den Konfigurationsdateien eine neue Konfiguration für den Monitoring-Kern erzeugt und diesem der Befehl gegeben, diese Konfiguration ab sofort zu verwenden:
...
Die Liste der ausstehenden Änderungen wird dadurch geleert.
...
[TIP]
====
Wenn Sie versuchen, ausstehende Änderungen zu aktivieren und gerade eine andere Aktivierung läuft, erhalten Sie eine entsprechende Warnung:
...
Die Aktivierung von ausstehenden Änderungen wird Ihnen bei der Einrichtung Ihrer Überwachung ebenso wie bei jeder späteren Anpassung der Konfiguration immer wieder begegnen.
