...
:description: Prometheus kann in {CMK} integriert werden, um schnelllebige Containerumgebungen zu überwachen. Hier erfahren Sie, wie Sie die Überwachung einrichten.
...
=== Regel für den Spezialagenten anlegen
...
==== Integration über unterschiedliche Scrape Targets

Der Spezialagent für Prometheus kann Daten von unterschiedlichen Scrape Targets abrufen.
Diese haben sehr unterschiedliche Anwendungszwecke und werden deshalb im Folgenden kurz vorgestellt.

Mit dem *Node Exporter* können die die Hardware-Komponenten bla bla bla....
...
Legen Sie nun über [.guihint]#Setup > Hosts > Hosts > Dynamic host management > Add connection# eine neue Verbindung an, wählen Sie als Verbindungstyp [.guihint]#Piggyback data# und legen Sie über [.guihint]#Add new element# die Bedingungen fest, unter denen neue Hosts dynamisch erstellt werden sollen.
