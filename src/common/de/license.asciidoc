...
[#license_cce]
== Lizenzierung der {CE} und der {ME}
...
Dies gilt bei der {CE} natürlich nur, wenn Sie die Grenzwerte des Lizenzstatus „Free“ überschritten haben.
...
Als Benutzer der {CE} oder {ME} lassen Sie {CMK} Ihre Lizenzinformationen per Online-Kommunikation übertragen.
...
Wenn Sie mit einer {SE} starten, schalten Sie anfangs nur Ihre Lizenz frei.
...
.Offline-Übermittlung der Lizenzinformationen in der {SE}
image::license_offline_submission.png[alt="Seite für Offline-Übermittlung der Lizenzinformationen in der {SE}.", width=85%]
