...
|Ausgeschlossene Services / Hosts |Services bzw. Hosts, die nicht in die Lizenzgröße eingerechnet werden. Sie sind für Instanzen gedacht, mit denen neue Funktionen, Konfigurationsänderungen, oder ähnliches von {CMK} getestet werden soll. Um Services oder Hosts als ausgeschlossen zu markieren, setzen Sie Labels vom Typ `cmk/licensing:excluded` entsprechend der Beschreibung  zur xref:labels#create_labels[Erstellung von Labels].
