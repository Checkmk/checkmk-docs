// -*- coding: utf-8 -*-
// IGNORE UOM Metrikidentifikatoren
// NONASCII
...
:description: Mit aktiven Checks können Sie Dienste von außen auf Funktion oder Antwortzeiten überwachen. Hier erfahren Sie, wie Sie {CMK} dafür einrichten.
...
Allen agentenbasierten Methoden ist gemein, dass sie einem nur Zustände mitteilen, wie sie der Host von innen heraus sieht.
...
An vielen Stellen ist es möglich, diese Option leer zu lassen, sodass der Host-Name oder dessen IP genommen wird.
...
Nachdem Sie die eben erstellte Regel aktiviert haben, xref:user_interface#search[suchen] Sie im [.guihint]#Monitor#-Menü nach dem soeben gesetzten Service-Namen `Basic webserver health`.
...
Die beiden wichtigsten Optionen sind hier die Angabe einer Service-Beschreibung und die Befehlszeile.
...
.Maßeinheiten bei aktiven Checks
[%collapsible]
====
{CMK} wertet beim Aufruf Nagios kompatibler Plugins keine zurückgegebenen Maßeinheiten (_units of measurement_ oder kurz _UOM_) aus.

Die Dokumentation von „monitoring-plugins.org“ für link:https://www.monitoring-plugins.org/doc/guidelines.html#AEN197[_Performance data_^] erwähnt ausdrücklich, dass hier übergebene Einheiten an Drittanwendungen (hier: das Metrik- und Graphing-System von {CMK}) weitergegeben werden, und es diesen Anwendungen freigestellt ist, was wie ausgewertet wird.

In {CMK} werden Einheiten jedoch über die Metrikidentifikatoren zugeordnet.
Um keinen doppelten Aufwand zu verursachen, und wegen der Vielzahl der in klassischen Monitoring-Plugins verwendeten Maßeinheiten hat {CMK} beschlossen, Nagios-Maßeinheiten zu ignorieren.

Falls Sie ein klassisches Monitoring-Plugin selbst implementieren, können Sie den Metriknamen so wählen, dass passende, bereits vorhandene {CMK}-Metriken zugeordnet werden.
Ist das nicht möglich, können Sie für die ausgegebenen „Performance-Daten“ selbst Metrikdefinitionen erstellen.
Die möglichen Vorgehensweisen erklären wir im Artikel zu xref:localchecks#metrics_intro[lokalen Checks] und im Artikel zur xref:devel_check_plugins#new_metricdefinition[Entwicklung von Check-Plugins].
====
...
{CMK} wertet beim Aufruf Nagios kompatibler Plugins keine zurückgegebenen Maßeinheiten (_Units of Measurement_ oder _UOM_) aus.
Grund ist vor allem die recht vage Dokumentation der verwendeten Einheiten und die fehlende Festlegung auf die Basiseinheiten.

Wenn Sie zurückgegebene Einheiten korrekt in Graphen dargestellt haben wollen, genügt der Aufruf eines aktiven Checks auf der Befehlszeile nicht.
Sie müssen stattdessen ein Plugin erstellen, welches die link:https://docs.checkmk.com/plugin-api/{current-major}/cmk.server_side_calls/index.html[_Server Side Calls_ API] nutzt und darin die zurückgegebenen UOM selbst auswerten.

Die Programmierung eines solchen Plugins ist nicht Scope dieses Artikels, sondern wird in naher Zukunft in einem eigenen Artikel beschrieben.
endif::[]
...
Zusätzlich benötigen Sie noch eine Konfigurationsdatei, welche bestimmt, in welcher Art und Weise das Plugin ausgeführt werden soll und wie die konkrete Befehlszeile für den Aufruf aussieht.
