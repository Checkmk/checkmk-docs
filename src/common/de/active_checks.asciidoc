...
Die Dokumentation von „monitoring-plugins.org“ zu link:https://www.monitoring-plugins.org/doc/guidelines.html#AEN197[_Performance data_^] erwähnt ausdrücklich, dass hier übergebene Einheiten an Drittanwendungen (hier: das Metrik- und Graphing-System von {CMK}) weitergegeben werden, und es diesen Anwendungen freigestellt ist, was wie ausgewertet wird.
...
Die möglichen Vorgehensweisen erklären wir im xref:localchecks#metrics_intro[Artikel zu lokalen Checks] und im xref:devel_check_plugins#new_metricdefinition[Artikel zur Entwicklung von Check-Plugins].
====
