...
._Units of Measurement_ bei aktiven Checks
[%collapsible]
====
...
Die Dokumentation des Monitoring-Plugins-Projektes zu link:https://www.monitoring-plugins.org/doc/guidelines.html#AEN197[_Performance data_^] erwähnt ausdrücklich, dass hier übergebene Einheiten an Drittanwendungen weitergegeben werden (hier das Metrik- und Graphing-System von {CMK}) und es diesem freigestellt ist, was wie ausgewertet wird.

In {CMK} werden Einheiten jedoch über die Metrik-Identifikatoren zugeordnet.
Um keinen doppelten Aufwand zu verursachen und wegen der Vielzahl der in klassischen Monitoring-Plugins verwendeten Maßeinheiten hat {CMK} beschlossen, Nagios-UOMs zu ignorieren.

Falls Sie ein klassisches Monitoring-Plugin selbst implementieren, können Sie den Metrik-Namen so wählen, dass passende, bereits vorhandene {CMK}-Metriken zugeordnet werden.
Ist das nicht möglich, können Sie für die ausgegebenen Performance-Daten selbst Metrikdefinitionen erstellen.
Die möglichen Vorgehensweisen erklären wir im Abschnitt xref:localchecks#metrics_intro[_Metriken_ im Artikel zu lokalen Checks] und im Abschnitt xref:devel_check_plugins#new_metricdefinition[_Metrikdefinitionen erstellen_ im Artikel zur Entwicklung von Check-Plugins].
====
