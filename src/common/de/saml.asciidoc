...
// TK: Wenn die Connection ID schon required ist, sollte sie auch im Screenshot zu sehen sein.
...
// TK: Meiner Meinung nach ist der obige Satz überflüssig. Der folgende Satz ist wichtig und reicht hier aus.
Der [.guihint]#Name# wird hinterher für die xref:use_saml[Benennung des {CMK}-Anmelde-Buttons] genutzt.
// TK: {CMK}-Anmelde-Buttons oder {CMK}-Anmeldeknopfs?
...
// TK: Der obige Pfad ist falsch: Wenn es ein Verzeichnis innerhalb der Instanz ist, dann so: `~/etc/ssl/saml2/custom/`. Der Slash am Ende kennzeichnet ein Verzeichnis.
// TK: "Ordner" nur im GUI-Zusammenhang nutzen. Für Pfade und Kommandozeile "Verzeichnis", also: 
...
// TK Upload XML file seh ich im Screenshot aber nicht. Vielleicht so:
// ... Dialog hochladen durch Auswahl des Eintrags [.guihint]#Upload XML file# unter [.guihint]#Identity provider metadata.#
...
// TK: Reihenfolge: Die Beschreibung sollte zur Abfolge in der GUI passen, also zuerst Contact groups beschreiben, dann Roles.
...
Um die Zuständigkeiten für alle Benutzer, die sich mit SAML authentisieren, in {CMK} zu regeln, kann jeder xref:wato#contact_groups[Benutzer einer bzw. mehreren Kontaktgruppen zugeordnet] werden.
// TK: falscher Link. Richtig ist: wato_user#contact_groups
Sie haben verschiedene Möglichkeiten, die Zuordnung in den [.guihint]#Contact groups# zu definieren:
// TK: Doofe(?) Frage: Warum stehen diese Details nicht in der Inline-Hilfe, wo sie meiner Meinung nach hingehören?
...
// TK: Heavy Stuff: Versteh kaum ein Wort. Aber egal :-)
// TK: Nur zum letzten Punkt: Was ist der Attribut-String? Empfängt CMK auch hier was und vergleicht das mit der Eingabe? Dann vielleicht so: {CMK} nimmt aus dem empfangenen String ...
// TK: Im 3. und 4. Punkt steht Checkmk. Bitte durch {CMK} ersetzen.
