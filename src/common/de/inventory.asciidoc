// -*- coding: utf-8 -*-
// IGNORE mk_inventory cfg check_mk
// NONASCII
...
ifdef::onprem[]
Das können Sie entweder manuell oder mit der xref:wato_monitoringagents#bakery[Agentenbäckerei] (nur kommerzielle Editionen) machen.
endif::[]
ifdef::saas[]
Das können Sie entweder manuell oder mit der xref:wato_monitoringagents#bakery[Agentenbäckerei] machen.
endif::[]
...
|Windows |`mk_inventory.ps1`
|Linux |`mk_inventory.linux`
|AIX |`mk_inventory.aix`
|Solaris |`mk_inventory.solaris`
|===

Sie finden diese Dateien auch auf der Befehlszeile in der {CMK}-Instanz im Unterverzeichnis
...
Eine sehr wichtige ist z.B. die Tabelle [.guihint]#Software  > Packages#:
...
In Umgebungen, in denen häufige Änderungen an Hardware und Software auftreten, können obsolete Inventurdaten nennenswerten Speicherplatz belegen.
Aus diesem Grund verwendet {CMK} eine konfigurierbare Aufräumstrategie.
Standardmäßig ist diese auf das Behalten der Historie über wenigstens 400 Tage und wenigstens 100 Änderungen konfiguriert.
Dies bedeutet, dass wenn über 400 Tage weniger als 100 Änderungen aufgelaufen sind, mitunter Daten deutlich _länger_ vorgehalten werden und im Falle von mehr als 100 Änderungen innerhalb der 400 Tage deutlich _mehr_ als 100 Änderungen vorgehalten werden.

Aus diesem Grund können Sie die Löschstrategie über [.guihint]#Global settings > HW/SW inventory cleanup# einstellen.
Hier können Sie abweichende Einstellungen für verschiedene Hosts einstellen und dabei auch die Bedingungen ändern, beispielsweise ausschließlich auf Alter oder Anzahl einschränken oder nur Daten vorhalten, die _weder_ ein bestimmtes Alter _noch_ eine bestimmte Anzahl übersteigen.

[TIP]
====
{CMK} Instanzen, die
ifdef::onprem[]
mit einer Version vor {v24}p12 beziehungsweise {v25}
endif::[]
ifdef::saas[]
vor Oktober 2025 erstellt wurden
endif::[]
haben keine Standard-Löschstrategie gesetzt!
Sie müssen diese manuell mit der oben erwähnten globalen Einstellung setzen.
Details erläutert link:https://checkmk.com/werk/18049[Werk pass:q[#]18049^].
====
