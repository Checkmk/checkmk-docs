...
Spezialagenten und
ifdef::saas[]
die in den On-Premises-Editionen von {CMK} nutzbaren Datenquellprogramme
endif::[]
ifdef::onprem[]
xref:datasource_programs#[Datenquellprogramme]
endif::[]
...
ifdef::saas[]
In {CE} sind Datenquellprogramme aus Sicherheitsgründen deaktiviert.
// ES: Bitte den ganzen Absatz mit dem Vergleich zwischen Spezialagenten und Datenquellprogrammen auf onprem beschränken. Für saas ist vor allem relevant, wann es sinnvoll ist, Spezialagenten zu benutzen. Ich denke, das ist oberhalb des Collapsible schon abgedeckt.
endif::[]
====
...
// ES: Im saas-Kontext bitte so formulieren, dass der Begriff Datenquellprogramm gar nicht vorkommt
