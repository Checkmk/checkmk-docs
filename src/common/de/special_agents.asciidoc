...
ifdef::onprem[]
Sie müssen sich um die Abfragen dieser APIs nicht selbst kümmern, denn {CMK} liefert für häufig benötigte Systeme die notwendigen, sogenannten xref:datasource_programs#[Datenquellenprogramme] mit aus,
endif::[]
ifdef::saas[]
Sie müssen sich um die Abfragen dieser APIs nicht selbst kümmern, denn {CMK} liefert für häufig benötigte Systeme die notwendigen, sogenannten _Datenquellenprogramme_ mit aus,
endif::[]
