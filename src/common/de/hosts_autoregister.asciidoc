...
Die Berechtigungen, die zur Registrierung und Erstellung von Hosts benötigt werden, sind in der xref:wato_user#roles[Rolle] [.guihint]#Agent registration user# enthalten.
Dadurch können Sie gezielt einen Benutzer erstellen, der nichts anderes darf, als Hosts zu registrieren.
...
Die Berechtigungen zur Registrierung _neuer_ Hosts sind in dieser Rolle nur in der {CE} verfügbar.
...
Dabei sind in dieser Rolle die Berechtigungen zur Registrierung _neuer_ Hosts nur in der {CE} verfügbar.
...
// TK: Im Rest des Artikels kleine Anpassungen durchgeführt und jeden 2. Screenshot aktualisiert
...
Starten mit [.guihint]#Add rule# die Erstellung einer neuen Regel und vergeben Sie im ersten Kasten [.guihint]#Properties,# wie bei {CMK} gewohnt, zuerst eine ID und einen Titel.
...
Hierfür nutzen Sie den [.guihint]#Agent controller auto-registration# Regelsatz der Agentenbäckerei, der nur in der {CE} vorhanden ist.
...
Damit bietet die {CE} die beiden wichtigsten Komponenten für das _Lifecycle Management_ von Hosts.
...
|`/var/lib/cmk-agent/pre_configured_connections.json` |Enthält eine vorkonfigurierte und per Agentenbäckerei in das Agentenpaket integrierte Verbindung zu einer Instanz für die Autoregistrierung in der {CE}.
|===
...
|`C:\ProgramData\checkmk\agent\pre_configured_connections.json` |Enthält eine vorkonfigurierte und per Agentenbäckerei in das Agentenpaket integrierte Verbindung zu einer Instanz für die Autoregistrierung in der {CE}.
|===
