...
Da die automatische Host-Erstellung vor allem für die Cloud-Umgebungen benötigt wird, ist die Autoregistrierung nur in {CCE} verfügbar.
...
Bei der Konfiguration des Ordners können Sie übrigens festlegen, ob die Agenten der dort zu erstellenden Hosts im xref:glossar#pull_mode[Pull-Modus] oder im xref:glossar#push_mode[Push-Modus] von {CE} arbeiten sollen.
...
Damit bietet {CE} die beiden wichtigsten Komponenten für das _Lifecycle Management_ von Hosts.
