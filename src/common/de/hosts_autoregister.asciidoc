...
:description: Der {CMK}-Agent kann Objekte automatisch als zu überwachende Hosts in einer Instanz registrieren. Lesen Sie hier, wie Sie die Autoregistrierung umsetzen.
...
Dann folgt der {CMK}-Agent:
Hier werden per Agentenregel die Informationen für eine Registrierungsanfrage festgelegt und dann mit der Agentenbäckerei in ein Agentenpaket verpackt.
ifdef::onprem[]
Dazu gehört die Auswahl der {CMK}-Instanz, die den Host überwachen soll, was in einem xref:glossar.html#distributed_monitoring[verteilten Monitoring] in der Regel eine Remote-Instanz sein wird.
endif::[]
...
Im Kasten [.guihint]#Agent controller auto-registration# geben Sie dem Agent Controller alle Informationen mit, 
...
den xref:user[Automationsbenutzer] für die Autoregistrierung ([.guihint]#Authorization#)
und die {CMK}-Instanz ([.guihint]#Site to register with#), die den Host überwachen soll.
ifdef::onprem[]
In einem xref:glossar.html#distributed_monitoring[verteilten Monitoring] wird dies in der Regel eine Remote-Instanz sein.
endif::[]
...
Auf dem Host können Sie sich den Verbindungsstatus des Agent Controllers per Befehl ausgeben lassen,
...
|===
|Ort |Pfad |Bedeutung
|Linux-Host |`/var/lib/cmk-agent/registered_connections.json` .2+|Enthält eine Liste der mit dem Agent Controller registrierten Verbindungen.
|Windows-Host |`C:\ProgramData\checkmk\agent\registered_connections.json`
|Linux-Host |`/var/lib/cmk-agent/pre_configured_connections.json` .2+|Enthält eine vorkonfigurierte und per Agentenbäckerei in das Agentenpaket integrierte Verbindung zu einer Instanz für die Autoregistrierung ab {CE}.
|Windows-Host |`C:\ProgramData\checkmk\agent\pre_configured_connections.json` |
|===

// ES: Vorschlag für möglicherweise lesbarere dreispaltige Tabellen
[cols="~,15,60",options="header"]
|===
|Datei-Inhalt |Ort |Pfad 
.2+|Eine Liste der mit dem Agent Controller registrierten Verbindungen. |Linux-Host |`/var/lib/cmk-agent/registered_connections.json`
|Windows-Host |`C:\ProgramData\checkmk\agent\registered_connections.json`
.2+|Eine vorkonfigurierte und per Agentenbäckerei in das Agentenpaket integrierte Verbindung zu einer Instanz für die Autoregistrierung ab {CE}. |Linux-Host |`/var/lib/cmk-agent/pre_configured_connections.json`
|Windows-Host |`C:\ProgramData\checkmk\agent\pre_configured_connections.json` |
|===

// ES: Spalte 3 und 1 vertauschen, um zu vergleichen, welche Variante lesbarer ist