...
Der große Vorteil des Slideouts ist, dass Ihnen die passenden Befehle zum Download, zur Installation und zur Registrierung eines Linux-Hosts in der GUI angezeigt werden.
...
Der große Vorteil der Slideouts ist, dass Ihnen die passenden Befehle zum Download, zur Installation und zur Registrierung eines Linux-Hosts in der GUI angezeigt werden.
Sie können jeden Befehl in die Zwischenablage kopieren, und ihn dann auf dem Host ausführen.
endif::[]
...
Beide Paketformate stehen für verschiedene Prozessorarchitekturen zur Verfügung:

- Pakete mit `all` oder `noarch` im Namen lassen sich auf allen Linux-Systemen installieren, sie bringen den Agent Controller aber nur für `x86_64` (auch `amd64` genannt) mit, auf anderen Prozessoren arbeitet der Agent im xref:agent_linux_legacy#[Legacy-Modus]

- Pakete mit `aarch64` oder  `arm64` im Namen lassen sich nur auf Linux-Systemen installieren, die einen 64-Bit-ARM-Prozessor verwenden, dort aktivieren Sie auch den Agent Controller, wenn Systemd als Init-System verwendet wird
...
Das mitgelieferte DEB-Paket des Linux-Agenten lässt sich beispielsweise mit dem folgenden `curl`-Befehl holen, in dem die Authentifizierung mit einem xref:glossar#automation_user[Automationsbenutzer] erfolgt:
...
// Ausgabe für dpkg -i check-mk-agent_2.5.0b4-1_all.deb
[{shell}]
----
...
==== Prozessorarchitektur und Verzeichnishierarchie

Die in gebackenen Agentenpaketen enthaltene Prozessorarchitektur (`aarch64`/`arm64` oder `x86_64`/`amd64` oder keine oder beide) des Agent Controllers legen Sie über die Regel [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Customize agent package (Linux)# fest.
Analog legen Sie Einstellungen zur Installation in einem xref:agent_single_folder#[einzigen Verzeichnis], beziehungsweise zur unprivilegierten Ausführung, unter [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Linux/Unix agent options# fest.
endif::[]
...
. Legen Sie eine Regel in [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Generic agent options > Deploy custom files with agent# an.
