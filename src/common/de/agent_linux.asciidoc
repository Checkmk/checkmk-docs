...
// MFS: Nein, auch in der SaaS kann an zwei Stellen ein Agent heruntergeladen werden, der zunächst an 6556 lauscht. 
...
// MFS: Diskussion?
Neben diesen stehen sogenannte Vanilla-Agenten ohne Vorkonfiguration zur Verfügung, die auch mit den On-Premises-Editionen von {CMK} benutzt werden können, aber mehr Aufwand bei der Einrichtung erfordern.
...
Dort finden Sie die für die Autoregistrierung vorbereiteten Pakete bei den Ordnern, die für die Autoregistrierung konfiguriert wurden.
Im Auslieferungszustand ist das der Ordner [.guihint]#preconfigured_agent#.
...
////
kommen Sie mit dem Menüeintrag [.guihint]#Related > Linux, Solaris, AIX files# zur Liste der Agentendateien.
Alles was Sie brauchen, finden Sie gleich im ersten Kasten mit dem Namen [.guihint]#Packaged Agents#:
die fertigen RPM- und DEB-Paketdateien für die Installation des Linux-Agenten mit Standardeinstellungen.
...
// MFS: Wenn man mehrere Agentenkonfigurationen hat, ist es sinnvoll, zu wissen, wo man nachsehen kann.
