...
Um die Ausgabe des Agentenskripts zu analysieren, sollten Sie daher nach Möglichkeit den xref:agent_ctl_dump[Dump-Modus des Agent Controller] verwenden.
====
...
Vorbereitend sollten Sie vorher prüfen, ob Das Agentenskript und der Agent Controller im xref:agent_ctl_dump[Dump-Modus] eine identische Ausgabe liefern und gegebenenfalls eine identische _Environment_ sicherstellen.
Dies kann durch Setzen vor Variablen in einem Plugin/lokalen Check oder der Shell geschehen.
Einen Dump der Environment können Sie erstellen, indem Sie die Zeile
...
in zu einer Plugin-Datei hinzufügen und dann den Inhalt der Datei nach Ausführung durch den Agent Controller ansehen.

Die Ausgabe der zusätzlichen Debug-Informtationen erledigt dann Option `-d`.
