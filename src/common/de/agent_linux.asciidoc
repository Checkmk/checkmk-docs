...
ifdef::saas[]
Im xref:intro_setup#onboarding_guide[Onboarding Guide] unter [.guihint]#Add hosts > Command line download and install# können Sie sich den `curl`-Befehl anzeigen lassen, mit dem Sie den für Autoregistrierung im Ordner `preconfigured_agent` vorbereiteten Agenten herunterladen.
...
DEBUG [reqwest::connect] starting new connection: pass:[https://cmkserver/]
INFO [cmk_agent_ctl::site_spec] Failed to discover agent receiver port using https.
DEBUG [cmk_agent_ctl::site_spec] https error: "Failed to discover agent receiver port from pass:[https://cmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke\nerror] sending request for url (pass:[https://cmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke])\nclient error (Connect)\ndns error\nfailed to lookup address information: Try again"
DEBUG [reqwest::connect] starting new connection: pass:[http://cmkserver/]
INFO [cmk_agent_ctl::site_spec] Failed to discover agent receiver port using http.
DEBUG [cmk_agent_ctl::site_spec] http error: "Failed to discover agent receiver port from pass:[http://cmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke\nerror] sending request for url (pass:[http://cmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke])\nclient error (Connect)\ndns error\nfailed to lookup address information: Try again"
...
// ES: In einer früheren Version dieses Abschnitts wurde auch noch das Problem angedeutet, dass der von der REST-API verwendete Port von den Standard-Ports 443 (https) oder 80 (http) abweichen könnte. Mir ist nicht klar, unter welchen Bedingungen das so sein sollte, weil ich nirgends eine Einstellung für diesen Port finden kann. Wie kann ich diese Situation 1. provozieren und 2. einen Umgang damit empfehlen? Die Antworten interessieren mich auch für die aktualisierte Fassung von rest_api.asciidoc.

ifdef::onprem[]
Ist der {CMK}-Server erreichbar und die Registrierung schlägt dennoch fehl, dann liegt das Problem an einer anderen Stelle.
Nach der Ermittlung des Ports für den Agent Receiver wird im zweiten Schritt eine Verbindung zum Agent Receiver aufgebaut, um das Zertifikat abzurufen.

Ist der Agent Receiver unter dem ermittelten Port nicht erreichbar, wird die Ausgabe etwa folgendermaßen aussehen:
...
INFO [cmk_agent_ctl] starting
INFO [cmk_agent_ctl] Loaded config from '"/var/lib/cmk-agent/cmk-agent-ctl.toml"', connection registry from '"/var/lib/cmk-agent/registered_connections.json"'
Attempting to register at cmkserver, port 8005. Server certificate details:

ERROR [cmk_agent_ctl] Connection refused (os error 111)
----

Im gezeigten Beispiel versucht der Agent Controller, den Agent Receiver auf Port 8005 zu erreichen.
// ES: Das Beispiel ist fingiert, kann diese Situation in der Realität wirklich unprovoziert entstehen?
Die Fehlermeldung drückt eindeutig aus, dass diese Verbindung abgelehnt wurde.
...
Den korrekten Port können Sie bei der Registrierung zusammen mit der {CMK}-Server-Adresse explizit angeben (`--server cmkserver:8000`), um sicherzustellen, dass der Agent Receiver erreicht wird.
