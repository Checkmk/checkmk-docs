// -*- coding: utf-8 -*-
// IGNORE se Ver Monitoringlösung Onprem preconfigured_agent
// NONASCII →
...
Da der Push-Modus zwingend den Agent Controller benötigt, ist die Installation derzeit nur auf modernen Linux-Systemen, die Systemd nutzen und auf der `x86_64`-Architektur aufbauen, möglich.
Unterstützung für `aarch64` (ARM 64 Bit) ist in Arbeit, beziehungsweise kann der Agent Controller für diese Plattform selbst kompiliert werden.
Legacy-Systeme ohne Systemd oder auf `i686`-Prozessoren werden nicht unterstützt.
endif::[]

//
// Ende separate Einleitungen
//
...
Sollten Sie einen Vanilla-Agenten benötigen, kommen Sie mit dem Menüeintrag [.guihint]#Related > Linux, Solaris, AIX files# zur Liste der Agentendateien.
Alles was Sie brauchen, finden Sie gleich im ersten Kasten mit dem Namen [.guihint]#Packaged Agents#:
die fertigen RPM- und DEB-Paketdateien für die Installation des Linux-Agenten mit Standardeinstellungen.
...
// MFS: Nein, ist sehr wichtig an dieser Stelle, weil häufig benötigt
// MFS: Für SaaS Reihenfolge nach Priorität geändert
...
ifdef::saas[]
In {CE} (SaaS) können Sie sich im [.guihint]#Onboarding Guide# unter [.guihint]#Add hosts > Command line download and install# das `curl`-Kommando anzeigen lassen, mit dem Sie den für Autoregistrierung im Ordner [.guihint]#preconfigured_agent# vorbereiteten Agenten herunterladen.
...
Auf diese gehen wir xref:post_install[gleich] ein.
endif::[]
...
// TK: Ist der Hinweis auf die REST-API für den neuen agent_registration Benutzer überhaupt noch interessant?
...
// MFS: Da der Checkmk-Server nicht beeinflusst werden kann, verschieben wir den Fokus auf die Firewall!
...
// MFS: Zumindest für SaaS sollten diese beiden Schritte vertauscht werden:
...
ifdef::saas[]
In diesem Fall erfüllt der Host nicht die Mindestanforderungen für das Monitoring mit {CE} (SaaS).
endif::[]
...
// MFS: Kein Unterschied bei manueller Registrierung
...
// MFS: Hier sollte die Einleitung vereinheitlicht werden, SaaS kann hier als Template dienen, muss dazu aber abgeschwächt werden.
ifdef::onprem[]
...
ifdef::saas[]
Ein modulares System kann an vielen Stellen nicht wie vorgesehen funktionieren.
Da in {CE} (SaaS) der Agent zwingend mit den beiden Komponenten Agent Controller auf dem Host und Agent Receiver auf dem {CMK}-Server verwendet werden muss, steigt die Zahl der Stellen, an denen etwas schiefgehen kann.
endif::[]
...
können Sie über [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Disabled sections (Linux agent)# einfach eine Regel anlegen, welche dann von der xref:glossar#agent_bakery[Agentenbäckerei] berücksichtigt wird.
...
ifdef::onprem[]
Nutzer von {CRE} können die
endif::[]
oben genannte Datei `/etc/check_mk/exclude_sections.cfg` manuell anlegen und dort die Sektionen eintragen, die deaktiviert werden sollen.
