...
[TIP]
====
Mit dem Befehl `cmk-agent-ctl` kann die Registrierung des Agent Controllers und des Agent Updaters _gleichzeitig_ erfolgen.
...
ifdef::saas[]
Sie haben die Möglichkeit, zusätzlich zu den in der Agentenbäckerei verfügbaren eigene Pakete für die xref:hosts_autoregister#[Autoregistrierung] zu erstellen.
