...
:description: Der Agent Controller übermittelt die gesammelten Monitoring-Daten vom Linux-Agentenskript an {CMK}. Lesen Sie hier, wie Sie die Überwachung einrichten.
:experimental:
...
// TK: Den folgenden Absatz umgeschrieben wg. PR 106: https://github.com/Checkmk/checkmk-docs/pull/106/changes/
...
Der Linux-Agent kann mit dem Agent Controller Funktionen realisieren wie die TLS-Verschlüsselung der Kommunikation oder die Datenkomprimierung und ist vorbereitet für zukünftige Erweiterungen.
...
Allerdings beherrscht der Agent auch einen sogenannten *Legacy-Modus*, um Linux-Systeme zu unterstützen mit einer anderen Rechnerarchitektur als `x86_64` oder `aarch64`, ohne RPM- bzw. DEB-Paketmanagement und ohne init-System `systemd`.
...
// ES: The following admonition needs attention for KNW-2113
[TIP]
====
...
Da der Push-Modus zwingend den Agent Controller benötigt, ist die Installation derzeit nur auf modernen Linux-Systemen möglich, die `systemd` nutzen und auf der `x86_64`- oder `aarch64`-Architektur aufbauen.
...
// TK: Neues Kap. wg. neuem Slideout: KNW-2021
[#slideout]
== Schnellinstallation
// TK: Oder länglicher: Installation und Registrierung per Assistent?
...
// TK: Folgender Satz für agent_windows:
// Der Ablauf ist für einen Windows-Host identisch, nur die Befehle sind andere.

Der große Vorteil des Slideouts ist, dass Ihnen die passenden Befehle zum Download, zur Installation und zur Registrierung in der GUI angezeigt werden.
Sie können jeden Befehl in die Zwischenablage kopieren, und ihn dann auf dem Host ausführen.
Für einen Linux-Host wird dabei das Agentenpaket mit Curl über die REST-API heruntergeladen, mit dem `dpkg`- oder `rpm`-Befehl installiert und der Host schließlich mit `cmk-agent-ctl` registriert.
// TK: Folgender Satz für agent_windows:
...
Hier werden die Agentenpakete ohne Vorkonfiguration (auch _Vanilla_-Agenten genannt) verwendet, die es für Windows, Linux, Solaris und AIX gibt. 

Beide Slideouts für die xref:intro_setup_monitor#first_auto[automatische Aufnahme] und für die xref:intro_setup_monitor#first[manuelle Aufnahme] werden im Leitfaden für Einsteiger vorgestellt.

Der große Vorteil der Slideouts ist, dass Ihnen die passenden Befehle zum Download, zur Installation und zur Registrierung in der GUI angezeigt werden.
Sie können jeden Befehl in die Zwischenablage kopieren, und ihn dann auf dem Host ausführen.
endif::[]
...
Ausführliche Informationen über alle möglichen Wege zur Installation eines Vanilla-Agenten (zum Beispiel ohne Einmal-Token, mit xref:glossar#agent_plugin[Agentenplugins] und mit Fehlersuche) erhalten Sie in den folgenden Kapiteln.
...
ifdef::saas[]
// TK: Absatz umgeschrieben: Standardweg weggelassen, Agentenpakete wurden bereits im vorigen Kapitel vorgestellt.
...
Neben diesen stehen die Vanilla-Agenten ohne Vorkonfiguration zur Verfügung, die mehr Aufwand bei der Einrichtung erfordern.
...
[IMPORTANT]
====
Die von {CMK} mitgelieferten Agentenpakete enthalten (Stand 31. März 2026) noch keinen Agent Controller für `aarch64` (ARM mit 64-Bit-Befehlssatz).
Verwenden Sie die xref:glossar#agent_bakery[Agentenbäckerei], um angepasste Pakete zu erstellen, die den Agent Controller für `aarch64` oder beide Architekturen enthalten.
Die enthaltene Architektur des Agent Controllers legen Sie über die Regel [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Customize agent package (Linux)# fest.
====
...
// TK: Vanilla-Agenten umgeschrieben
...
// TK: Uff. Und wieso brauch ich dann per Curl + REST-API eine Authentisieruing (im Slideout per Einmal-Token und im nächsten Abschnitt per Authorization: Bearer)? 
...
Hier können Sie Download und xref:install_package[Installation] mit einem Befehl durchführen:
...
ifdef::onprem[]
Das mitgelieferte DEB-Paket des Linux-Agenten lässt sich beispielsweise mit dem folgenden `curl`-Befehl holen:

[{shell}]
----
{c-root} curl -OJG "\http://mycmkserver/mysite/check_mk/api/v1/domain-types/agent/actions/download/invoke" \
...
ifdef::saas[]
// TK: Hier war bisher für saas der im Onboarding Guide sichtbare `curl`-Befehl als Screenshot drin.
// TK: Den Onboarding Guide gibts nicht mehr und die in den neuen Slideouts enthaltenen Befehle verwenden das Einmal-Token, die nicht weiterhelfen, wenn man sich das Paket manuell holen will.
// TK: Ich hab mich also dafür entschieden, wie bei onprem auch bei saas den kompletten Befehl zu zeigen.
// TK: Muss man hier noch zusätzliche Hilfestellung geben?
// TK: mycmkserver ersetzen durch die saas-Domain? myautomationsecret für den api_user vorher festlegen und kopieren, damit man das Secret auf der Befehelszeile einsetzen kann?

Das mitgelieferte DEB-Paket des Linux-Agenten lässt sich beispielsweise mit dem folgenden `curl`-Befehl holen:

[{shell}]
----
{c-root} curl -OJG "\https://mycmkserver/mysite/check_mk/api/v1/domain-types/agent/actions/download/invoke" \
--header 'Accept: application/octet-stream' \
--header 'Authorization: Bearer api_user myautomationsecret' \
--data-urlencode 'os_type=linux_deb'
----

*Hinweise:* Der Befehl wurde zugunsten der Lesbarkeit in vier Zeilen aufgeteilt.
...
Nachdem Sie das RPM- oder das DEB-Paket geholt und -- falls nötig -- mit `scp`, WinSCP oder anderen Mitteln auf den zu überwachenden Host kopiert haben, ist die Installation mit einem Befehl erledigt.
...
// TK: Die folgende Unterscheidung zwischen onprem und saas lag an dem curl-Kommando im Onboarding Guide, mit welchem ein Paket unter dem Namen check-mk-agent.rpm bzw. check-mk-agent.deb heruntergeladen wird.
// TK: Mit dem geänderten Befehl aus dem obigen REST-API-Kapitel gibt es nun Pakete unter den gewohnten Namen, von daher kann jetzt auch in der saas ein check-mk-agent-{current}-1.noarch.rpm installiert werden.
////
[{shell}]
----
ifdef::onprem[]
...
// TK: Auch im folgenden ist daher eine Unterscheidung zwischen onprem und saas nicht mehr nötig
////
ifdef::onprem[]
...
pass:[(]Reading database ... 739920 files and directories currently installed.)
...
WARNING: The agent controller is operating in an insecure mode! To secure the connection run pass:[`]cmk-agent-ctl registerpass:[`].
...
Created symlink /etc/systemd/system/multi-user.target.wants/cmk-agent-ctl-daemon.service → /lib/systemd/system/cmk-agent-ctl-daemon.service.
----
// TK: Ausgabe getestet: Agent deinstalliert und neuen installiert per dpkg -i check-mk-agent_2.5.0-2026.04.14-1_all.deb
// TK: 2 wesentliche Unterschiede: Insecure WARNING gibts nicht mehr. (Hab ich beim Test was vergessen?) Neu dagegen: Deploying agent controller: /usr/bin/cmk-agent-ctl
// TK: Meine Ausgabe folgt:
////
Vormals nicht ausgewähltes Paket check-mk-agent wird gewählt.
(Lese Datenbank ... 559075 Dateien und Verzeichnisse sind derzeit installiert.)
Vorbereitung zum Entpacken von check-mk-agent_2.5.0-2026.04.14-1_all.deb ...
Entpacken von check-mk-agent (2.5.0-2026.04.14-1) ...
check-mk-agent (2.5.0-2026.04.14-1) wird eingerichtet ...
...
Hier wurde das Paket auf einem zuvor agentenlosen Host erstmalig installiert.
Der Benutzer `cmk-agent` wurde erstellt und `systemd` konfiguriert.
// TK: Ohne Insecure WARNING passt der folgende Satz nicht mehr.
Auf die dazwischen angezeigte Warnung zum `insecure mode`, also dem Legacy-Pull-Modus, gehen wir xref:post_install[gleich] ein.
...
ifdef::onprem[]
Einen Hinweis auf den aktivierten Legacy-Pull-Modus erhalten Sie bereits in der Befehlsausgabe bei der xref:install_package[Installation des Agenten].
...
Die Registrierung mit dem Agent Controller setzt ein Linux-System voraus mit einem init-System `systemd` ab Version 219 und einer `x86_64`- oder `aarch64`-Rechnerarchitektur.
...
Die obige Befehlsausgabe zeigt, dass `systemd` in der richtigen Version installiert ist.
...
Ursache hierfür ist, dass Ihr Linux eine andere Rechnerarchitektur als `x86_64` oder `aarch64` nutzt, zum Beispiel das ältere _32-Bit x86_.
...
Monitoring mit {CE-saas} ist hier _out of the box_ nicht möglich.
...
====
// TK: Letzten Satz im vorherigen Tip auskommentiert
endif::[]
...
// TK: Hier könnte man noch einen Hinweis einfügen, dass per cmk-agent-ctl auch die gleichzeitige Registrierung des Agent Updaters möglich ist mit Verweis auf agent_deployment.html#one_registration
// TK: Ob hier doppelt registriert wird, hängt am Wert Registration der Regel Agent updater (Linux, Windows, Solaris).
// TK: Nur wenn Registration auf Manual steht, kann man die neue Befehlsoption cmk-agent-ctl register --automatic-updates verwenden.
// TK: Ich hab mich erstmal dagegen entschieden, bin aber unsicher. Je nachdem, wie die Regel konfiguriert ist, könnte der Befehl mehr tun als das, was man vielleicht erwartet. MFS: Was ist Deine Meinung?
...
Ist dies der Fall, können Sie die Registrierung inklusive automatischer Erstellung des Hosts auch per Befehl vornehmen.
...
Größter Unterschied hier ist der geänderte Sub-Befehl `register-new`, mit dem die Registrierung _und_ die Erstellung eines neuen Hosts in der {CMK}-Instanz angefragt wird.
...
Sie können es direkt auf der Befehlszeile aufrufen.
...
Dadurch werden Ihnen nicht nur Fehler, sondern auch zusätzliche Informationen und Debug-Nachrichten ausgegeben.
...
Den korrekten Port können Sie bei der Registrierung zusammen mit der {CMK}-Server-Adresse explizit angeben (`--server mycmkserver:8000`), um sicherzustellen, dass der Agent Receiver erreicht wird.
...
// ES: evtl. Querverweis auf Relay hier einfügen?


[#agent_ctl_dump]
=== Agent Controller im Dump-Modus

Der Agent Controller stellt einen eigenen Sub-Befehl `dump` bereit, das die vollständige Agentenausgabe anzeigt, wie sie im Monitoring ankommt:
...
Des Weiteren können Sie auf dem Host mit dem Befehl `systemctl status cmk-agent-ctl-daemon` den Status der letzten Übertragungen und Übertragungsversuche auslesen.
Zeilen wie die folgende deuten auf Verbindungsprobleme hin:
endif::[]

ifdef::saas[]
Auf dem Host können Sie mit dem Befehl `systemctl status cmk-agent-ctl-daemon` den Status der letzten Übertragungen und Übertragungsversuche auslesen.
Zeilen wie die folgende deuten auf Verbindungsprobleme hin:
endif::[]
...
Zuständig dafür ist der Regelsatz [.guihint]#Setup > Agents > Windows, Linux Solaris, AIX > Agent rules > Generic agent options > Execute MRPE checks#.
...
ifdef::onprem[]
=== Zusätzliche Überwachung über das Management Board
// TK: Sollen wir das Kapitel rausschmeissen?
// TK: Minimum Verweis in update_major#management_board

Man kann zu jedem Host ein Management Board konfigurieren und zusätzliche Daten per SNMP holen.
Die dadurch erkannten Services werden dann ebenfalls dem Host zugeordnet.

Die Einrichtung des Management Boards ist dabei sehr einfach.
...
.In den Eigenschaften des Hosts im Setup wird das Management Board für SNMP konfiguriert 
image::agent_linux_snmp_management_board.png[alt="Die Konfiguration des Management Boards für SNMP in den Eigenschaften des Hosts."]
...
Sollte es erwünscht sein, wieder den unverschlüsselten Legacy-Pull-Modus zu aktivieren, müssen Sie dies xref:deregister[per Befehlsparameter anweisen].
endif::[]
