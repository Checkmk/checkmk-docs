...
====
// TK: Dieser Ausflug zu den On-Premises_-Editionen ist (an dieser Stelle) merkwürdig. Muss das sein?
// TK: Ich hab einen Verbindungssatz eingeführt.
...
Unterstützung für `aarch64` (64-Bit-ARM) ist in Arbeit, beziehungsweise kann der Agent Controller für diese Plattform selbst kompiliert werden.
// TK: Vorschlag: Unterstützung für `aarch64` (64-Bit-ARM) ist in Arbeit. Bis es soweit ist, können Sie sich helfen, indem Sie den Agent Controller für diese Plattform selbst kompilieren.
...
// TK: Der folgende Satz sollte onprem-only sein.
...
// TK: Warum hier erneut {CE} (SaaS)? Wir befinden uns doch im {CE} (SaaS) Handbuch.
...
// TK: Im vorherigen Satz: die mit allen {CMK}-Editionen > die auch mit den On-Premises-Editionen von {CMK}
...
Dazu später mehr.
// TK: Link für "später": xref:post_install[später]?
endif::[]
...
// TK: Der obige Satz ist unklar. Es reicht ja nicht aus, einen Ordner für die Autoregistrierung anzulegen. Vielleicht so:
// TK: Dort finden Sie die für die Autoregistrierung vorbereiteten Pakete bei dem Ordner, der für die Autoregistrierung konfiguriert wurde.
...
Alles was Sie brauchen, finden Sie gleich im ersten Kasten mit dem Namen [.guihint]#Packaged Agents#:
die fertigen RPM- und DEB-Paketdateien für die Installation des Linux-Agenten mit Standardeinstellungen.
// TK: Warum dieser Umweg in der SaaS? Der ist in onprem nötig, um die RE miteinzufangen. Die Vanilla-Agenten gibt es doch bereits auf der vorher gezeigten Seite "Windows, Linux, Solaris, AIX".
// TK: Der folgende Screenshot kann dann auch weg (die BU ist auch verdammt lang ;-)
...
// Dieses REST-API-Beispiel sollte zusätzlich in den REST-API-Artikel aufgenommen werden.
...
ifdef::saas[]
In {CE} (SaaS) können Sie sich im [.guihint]#Onboarding Guide# unter [.guihint]#Add hosts > Command line download and install# das `curl`-Kommando anzeigen lassen, mit dem Sie den für Autoregistrierung im Ordner `preconfigured_agent` vorbereiteten Agenten herunterladen.
// TK: Schon wieder {CE} (SaaS). Vorschlag:
// Im xref:intro_setup#onboarding_guide[Onboarding Guide] unter [.guihint]#Add hosts > Command line download and install# können Sie sich das `curl`-Kommando anzeigen lassen...
...
// TK: Obacht: Im Windows-Artikel verwendest Du den gleichen Screenshot, der aber das Kommando für das DEB-Paket zeigt.
endif::[]
ifdef::onprem[]
...
// TK: Obigen Satz würde ich nach onprem verschieben - ist für SaaS nicht wichtig an dieser Stelle.
...
----
// TK: Hm: Ich kann es ja momentan nicht testen, aber das obige scheint mir nur das onprem-Kommando zu sein. Das RPM-Paket hat wahrscheinlich einen kürzeren Namen (wie unten beim DEB-Paket).
// TK: Vor dem 1. Kommando hinzufügen: Die in diesem Abschnitt gezeigten Kommandos installieren ein für die Autoregistrierung vorkonfiguriertes Paket. Bei der Installation eines Vanilla-Agenten müssen Sie den Paketnamen anpassen.  
...
// TK: In agent_windows hab ich das Kapitel für saas mit einem Verweis aufgenommen, weil in den späteren Kapiteln immer wieder die Konfigurationsdateien genannt werden.
...
// TK: Owei, ich verlier langsam den Überblick: Warum ältere? Vorschlag:
...
// Wie bereits oben erwähnt, ist ein Vanilla-Agent sowohl in {CE} (SaaS) als auch in den On-Premises-Editionen nutzbar.
endif::[]
...
// TK: Obacht: Die entsprechende Admonition im Windows-Artikel für saas muss weg. Der Windows Legacy-Agent spielt für SaaS keine Rolle!
...
[TIP]
====
Dieses Kapitel ist vor allem relevant, wenn Sie einen nicht für die Autoregistrierung vorkonfigurierten Vanilla-Agenten installiert haben oder vorhandene Hosts aus dem Monitoring anderer {CMK}-Editionen zu {CE} (SaaS) migrieren wollen.
...
ifdef::onprem[]
// TK: Wieso gibt es diesen Abschnitt nicht in agent_windows?
...
// Da der Checkmk-Server nicht beeinflusst werden kann, verschieben wir für die saas den Fokus auf die Firewall:
ifdef::saas[]
...
// TK: Sehe ich genauso. Ich hab getauscht.
...
// TK: Ergänzen: ... mit {CE} (SaaS), und Sie können sich den folgenden Test sparen.
endif::[]
...
// Kein Unterschied bei manueller Registrierung zwischen onprem und saas.
...
// TK: Ja Ich hab einen neuen Absatz geschrieben (auch in agent_windows).
ifdef::onprem[]
...
// MFS: Ja, ich weiss...
// TK: Jaja, keine Regel ohne Ausnahme ;-)
Ob der Host
ifdef::onprem[]
im Pull-Modus,
endif::[]
...
// TK: Obiger Satz ist onprem-only.
...
// TK: Ich hab einen generischen Absatz geschrieben (auch in agent_windows).
ifdef::onprem[]
...
// TK: Bitte lösch den auskommentierten Abschnitt in agent_windows zu Ausgabe des Agentenprogramms (die Datei ist schon unübersichtlich genug).
...
// TK: Sorry, aber der folgende Teil ist onprem-only. Der Port wird doch auch durch das obige Kommando curl -v --insecure geliefert.
// TK: Vielleicht kriegst Du es hin mit Verweis auf oben einen generischen Text zu schreiben ohne omd config show.
...
// TK: Hm, Port 80 ist doch wieder onprem-only, oder?

Im Falle der Registrierung im Push-Modus gilt:
// TK Obigen Satz auch onprem-only?
...
// TK: Du stellst Fragen... Ist m.E. ok, aber abschwächen: ..., ist {CE} (SaaS) keine adäquate Lösung für das Monitoring dieses Hosts.
ifdef::saas[]
Sollten Sicherheitsrichtlinien in Ihrer Umgebung den Zugriff auf den Agent Receiver nicht zulassen, ist {CE} (SaaS) keine adäquate Monitoring-Lösung für Sie.
endif::[]
...
// TK: In agent_windows hast Du den Abschnitt mit dem Kommando netsh advfirewall firewall add rule als onprem-only getaggt. Warum?
...
// TK: Warum IMPORTANT? TIP reicht hier doch.
ifdef::saas[]
[IMPORTANT]
====
...
// TK: Wie wurde denn Keep existing connections in den mitgelieferten Agentenpaketen gesetzt? Ach, vergiss die Frage einfach ;-)
...
// TK: Achja, der vorherige Satz beschreibt den Pull-Modus.
...
Sollte es erwünscht sein, wieder den unverschlüsselten Legacy-Pull-Modus zu aktivieren, z.B. um die Kommunikation einer {v20} Instanz mit einem {current-major} Agenten zu ermöglichen, müssen Sie diesen Modus xref:deregister[explizit aktivieren.]
// TK: Das Beispiel {v20} Instanz mit {current-major} Agent ist inzwischen auch ziemlich weit weg von den Realitäten.
endif::[]
