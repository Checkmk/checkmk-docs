...
[TIP]
====
Wenn Sie mit den On-Premise-Varianten von {CMK} bereits Erfahrung gesammelt haben, vermissen Sie in dieser Einleitung möglicherweise die Erwähnung von SNMP, Legacy-Agenten im Pull-Modus und SNMP-Traps in der Event Console.
Diese Features sind im Bereich des Cloud-Monitoring wenig relevant und daher nicht Teil der initialen Release.
Dennoch evaluieren wir Features wie SNMP-Monitoring, können aber noch keine Auskunft dazu geben, ob und wann diese verfügbar sein werden.
====
endif::[]
...
[TIP]
====
In diesem Kapitel haben wir Informationen zum Pull-Mode belassen, auch wenn in {CE} (SaaS) nur der Push-Mode verfügbar ist.
Sowohl bei Fehlkonfigurationen als auch beim Wechsel zwischen On-Premise-Varianten von {CMK} und {CE} (SaaS) hilft die Kenntnis beider Kommunikationswege bei der Fehlersuche.
====
endif::[]
...
ifdef::saas[]
[#download_page]
== Vorbereitete Agenten herunterladen

In {CE} (SaaS) gelangen Sie mit dem Menüeintrag [.guihint]#Windows, Linux, Solaris, AIX# zu einer Seite, die Ihnen Zugang zur xref:bakery[Agentenbäckerei] bietet.
Bei einer frischen Instanz finden Sie hier Agentenpakete in zwei Konfigurationen:
...
Oft ist es umständlich, Agentenpakete zuerst vom {CMK}-Server auf Ihren Arbeitsplatzrechnur herunterzuladen um sie dann wieder per `scp` auf die zu überwachenden Server hochzuladen.
Um abzukürzen können Sie sich im [.guihint]#Onboarding Guide# unter [.guihint]#Add hosts > Command line download and install# das `curl`-Kommando anzeigen lassen, mit dem Sie den für Autoregistrierung im Ordner [.guihint]#preconfigured_agent# vorbereiteten Agenten herunterladen.
Analog können Sie auch das Powershell-Kommando zum Download von Windows-Agenten anzeigen lassen.
...
ifdef::saas[]
In {CE} (SaaS) können Sie mit der Agentenbäckerei (_Agent Bakery_) Agenten individuell paketieren.
endif::[]
...
ifdef::onprem[]
// MFS: Sollte IMHO woanders hin wandern, für SaaS zunächt entfernt, Onprem wegen Updates belassen.
[TIP]
====
...
Diesen fehlt noch der Agent Controller, damit sind sie nur mit den On-Premise-Varianten von {CMK} nutzbar.
...
Bitte beachten Sie, dass wir für selbst kompilierte Komponenten keinen Support bieten können.
====
endif::[]
...
ifdef::saas[]
Wenn Sie noch keine Einstellungen für bestimmte Hosts vorgenommen haben, gibt es nur die beiden eingangs erwähnten Standardagentenkonfigurationen.
endif::[]
