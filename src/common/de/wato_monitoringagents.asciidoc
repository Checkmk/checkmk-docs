...
Der Push-Modus ist nur in {CCE} verfügbar.
...
Für den Push-Modus von {CE} müssen {CMK}-Server und Agent mindestens Version {v22} haben.
...
// ML: Reviewer-Hinweis: Satz 3 habe ich umformuliert, weil die "automatische Bäckerei", "automatische Backerei" hätte sein müssen - und das ist laut Duden abwertend zu verstehen ... hach, Sprache ;)
*Hinweis:* Bei jedem Backvorgang kann die fortlaufende Back-Revision -- für die Unterscheidbarkeit verschiedener Backvorgänge -- erhöht werden.
Diese ist nur in den Metadaten des gebackenen Pakets sichtbar.
Die Funktion ist ab {CMK} 2.3.0 standardmäßig deaktiviert, um Probleme beim automatischen Backen von Agenten zu vermeiden.
Wenn Sie die Erhöhung der Back-Revision dennoch aktivieren möchten, zum Beispiel für die eindeutige Verarbeitung in einem Paketmanager, schalten Sie die Option [.guihint]#Setup > Global settings > Setup > Apply bake revision# ein.

////
ML: Reviewer-Hinweis 2: Inhaltlich: Wenn ich das Werk richtig verstehe, ist das Problem nicht nur beim automatischen Backen relevant, sondern immer - daher Back-Revision per Default aus. Auch wird nicht die Erhöhung aktiviert, sondern die ganze Revision-Geschichte. Beim automatischen Backen - sofern Revision eingeschaltet - bleibt die Revision vorhanden, wird aber nicht hoch gezählt. Hier mal ein leicht erweiterter Vorschlag:

*Hinweis:* Bei jedem Backvorgang kann eine fortlaufende Back-Revision -- für die Unterscheidbarkeit verschiedener Backvorgänge -- erzeugt werden.
Diese ist nur in den Metadaten des gebackenen Pakets sichtbar.
Die Funktion ist ab {CMK} 2.3.0 standardmäßig deaktiviert, um zu verhindern, dass gebackene Agenten ihre gültige Signatur verlieren.
Wenn Sie die Back-Revision dennoch aktivieren möchten, zum Beispiel für die eindeutige Verarbeitung in einem Paketmanager, schalten Sie die Option [.guihint]#Setup > Global settings > Setup > Apply bake revision# ein.
Und sofern eingeschaltet: Bei später automatisch über die Regel [.guihint]#Automatically create monitoring agents# gebackenen Agenten bleibt die vorige Revision bestehen und wird nicht weiter inkrementiert -- abermals, um die Signatur nicht zu verlieren.
Wenn Sie automatisch gebackene Agenten _mit_ fortlaufende Revisionen benötigen, können Sie zum Beispiel die REST-API statt der Regel einsetzen.

Ich meine, das müsste so mit dem Werk in dem Ticket übereinstimmen, aber lies mal besser sehr sorgfältig durch!
////
...
Das xref:agent_deployment#[automatische Agenten-Update] der kommerziellen Editionen ist zwar eine Erleichterung, doch trotzdem sollten Sie den Agenten immer nur dann aktualisieren, wenn das Update:
