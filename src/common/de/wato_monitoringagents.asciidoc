...
[TIP]
====
Wenn Sie mit den On-Premises-Editionen von {CMK} bereits Erfahrung gesammelt haben, vermissen Sie in dieser Einleitung möglicherweise die Erwähnung von Legacy-Agenten im Pull-Modus und SNMP-Traps in der Event Console.
Diese Features sind im Bereich des Cloud-Monitoring wenig relevant und daher nicht Teil von {CE-saas}.
====
endif::[]
...
|Registrierung |Das Vertrauensverhältnis zwischen dem Agent Controller des Hosts und dem Agent Receiver der {CMK}-Instanz wird hergestellt. |Alle Editionen 
|TLS-Verschlüsselung |Nach erfolgreicher Registrierung werden die Daten mit TLS verschlüsselt ausgetauscht. .3+|Alle Editionen 
|Komprimierung |Die Daten werden komprimiert ausgetauscht.
|Pull-Modus |Der Agent versendet die Daten auf Anforderung der {CMK}-Instanz.
|Push-Modus |Der Agent versendet die Daten von sich aus an die {CMK}-Instanz. |{CE}
|Individuelle Agentenkonfiguration |Per xref:glossar#agent_bakery[Agentenbäckerei] können Agenten für einzelne oder Gruppen von Hosts individuell konfiguriert und die Agentenpakete für die Installation erstellt werden. .2+|Kommerzielle Editionen
|xref:agent_deployment#[Automatische Agenten-Updates] |Das Paket aus der Agentenbäckerei wird zuerst manuell oder per Skript installiert und wird von da an automatisch aktualisiert.
|xref:hosts_autoregister#[Automatische Erstellung von Hosts] |Die Registrierung des Agenten bei der {CMK}-Instanz und die Erstellung des Hosts erfolgt automatisch. |{CE}
|===
endif::[]
...
In diesen Softwarepaketen finden Sie den Agenten mit Agent Controller.
...
Wenn Sie automatisch gebackene Agenten _mit_ fortlaufenden Revisionen benötigen, sollten Sie zum Beispiel die xref:rest_api_bakery[REST-API] statt der Regel einsetzen.
====
endif::[]
...
ifdef::onprem[]
.In der Agentenbäckerei können Sie die Agentenpakete herunterladen
...
Den Vanilla-Agenten und einen für die Autoregistrierung vorkonfigurierten.
endif::[]
...
Dazu wählen Sie den Regelsatz [.guihint]#Generic agent options > Allowed agent access via IP address (Linux, Windows)#.
...
Sehr viele Agentenregeln befassen sich mit der Installation verschiedener _Plugins._ 
...
Hier als Beispiel die Regel [.guihint]#MySQL databases# für die Überwachung von MySQL:
...
Ohne aktiviertes Logging sehen Sie diese Informationen nur, wenn Sie Agenten mit `cmk --bake-agents -v` xref:cmk_commandline#bake_agents[auf der Befehlszeile backen].
endif::[]


[#rest_api_bakery]
=== Agenten über die REST-API verwalten

Alle Funktionen der Agentenbäckerei stehen auch über die xref:rest_api#[REST-API] zur Verfügung.
Das ermöglicht die Einbindung in automatisierte Deployment-Pipelines und CI/CD-Systeme sowie den skriptgesteuerten Download von Agentenpaketen.
Dieser Abschnitt zeigt einige Beispiel-Anfragen mit Bash und Curl.

Bei allen API-Anfragen müssen Sie Informationen zu Ihrer {CMK}-Instanz und geeignete xref:rest_api#auth[Zugangsdaten] angeben.
Legen Sie daher zunächst die folgenden Variablen fest:
...
Diesen Block mit den von Ihnen gesetzten Werten können Sie in allen Beispielen in den folgenden Abschnitten einfügen, um die vorgegebenen Werte aus dem Beispiel-Code zu ersetzen.

==== Agentenpakete auflisten

Der REST-API-Endpunkt [.guihint]#Show all agents# gibt eine Liste aller vorhandenen Agentenpaketen mit ihren Hashes zurück.
// ES-blocked by KNW-1719: Direkten Link an diesen Abschnitt der API-Doku einfügen
Kopieren Sie den Beispiel-Code aus der Dokumentation der REST-API und ersetzen Sie den Block mit den Variablen durch den oben vorbereiteten Block:
...
Als Rückgabe erhalten Sie ein JSON-Objekt mit allen verfügbaren Agentenpaketen.
Jeder Agent ist an mehreren Stellen mit seinem Hash gekennzeichnet, den Sie auch in der xref:bakery_download[Weboberfläche der Agentenbäckerei] finden.
Anhand der Hash-Werte können Sie zuordnen, welche Konfiguration jedem Agentenpaket zugrunde liegt.


==== Agentenpakete herunterladen

Sie können ein Agentenpaket entweder direkt über seinen Hash oder über einen Hostnamen herunterladen.

Für den Download anhand des Hash-Wertes verwenden Sie den REST-API-Endpunkt [.guihint]#Download an agent by agent hash and operating system#.
// ES-blocked by KNW-1719: Direkten Link zu diesem Endpunkt in der API-Doku einfügen
Kopieren Sie den Beispiel-Code aus der REST-API-Dokumentation und ersetzen Sie sowohl den Block mit den Variablen als auch den Hash durch Ihre Werte:

.download_agent_by_hash.sh
[{bash},highlight=7-14;22]
----
#!/bin/bash
...
curl \
  ...
  --data-urlencode 'agent_hash=40dbb28a816134d4' \
  --data-urlencode 'os_type=linux_tgz' \
  "$API_URL/domain-types/agent/actions/download_by_hash/invoke"
----

Alternativ können Sie mit dem REST-API-Endpunkt [.guihint]#Download an agent by host or folder name and operating system# ein Agentenpaket über den Hostnamen herunterladen, ohne den Hash zu kennen.
// ES-blocked by KNW-1719: Direkten Link zu diesem Endpunkt in der API-Doku einfügen
Passen Sie den kopierten Code aus der REST-API-Dokumentation so an, dass genau die benötigten Felder für den gewünschten Agenten angegeben werden.

Im Beispiel wird der Agent für den Linux-Host `myhost` heruntergeladen:

.download_agent_by_host.sh
[{bash},highlight=7-14;22-24]
----
#!/bin/bash
...
curl \
  ...
  --data-urlencode 'host_name=myhost' \
  --data-urlencode 'os_type=linux_tgz' \
...
Um einen Backvorgang auszulösen, nutzen Sie den Endpunkt [.guihint]#Bake all agents#:
// ES-blocked by KNW-1719: Direkten Link zu diesem Endpunkt in der REST-API-Doku einfügen
...
Da der Backvorgang asynchron abläuft, können Sie über den Endpunkt [.guihint]#Show the baking status# den aktuellen Status abrufen.
Gehen Sie vor wie bei den zuvor kopierten Code-Beispielen.
...
----
{
  "links": [
    {
      "domainType": "link",
      "rel": "self",
      "href": "http://myserver/mysite/check_mk/api/1.0/domain-types/agent/actions/baking_status/invoke",
      "method": "GET",
  ...
  "result": {
    ...
    "value": {
      ...
      "is_active": false,
      "duration": 0.12719297409057617,
      "title": "Baking agents...",
      "stoppable": false,
      "deletable": true,
      "user": "automation",
      "estimated_duration": 0.3183622360229492,
      "ppid": 269419,
...
Sobald Sie in diesem Ergebnis den Status `finished` entdecken, können Sie die fertig gebackenen Agentenpakete herunterladen.
...
ifdef::onprem[]
=== Agent über die Befehlszeile testen

Sie können einen korrekt installierten Agenten sehr einfach von der Befehlszeile aus abfragen. 
...
[TIP]
====
Weitere Diagnosebefehle zur Ausführung auf dem Host des Agenten finden Sie im Artikel zum xref:agent_linux#test[Linux-Agenten] und xref:agent_windows#test[Windows-Agenten.]
====
endif::[]
...
Wählen Sie auf der Seite [.guihint]#Properties of host# im Menü [.guihint]#Host > Test connection# und starten Sie den Test durch Klick auf [.guihint]#Run tests#:

image::monitoringagents_host_diag.png[alt="Ergebnis des Verbindungstests zu einem Host.",fullscreen=1]
