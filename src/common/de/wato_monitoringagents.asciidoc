...
Legen Sie zunächst die folgenden Variablen mit den Werten Ihrer {CMK}-Instanz fest:
...
*Agentenkonfigurationen auflisten*

Eine Liste aller vorhandenen Agentenkonfigurationen mit ihren Hashes erhalten Sie mit:

[{bash}]
----
curl \
  --header "Authorization: Bearer $USERNAME $PASSWORD" \
  "http://$HOST_NAME/$SITE_NAME/check_mk/api/v1/domain-types/agent/collections/all"
----

Jeder Eintrag enthält ein Feld `id` mit dem vollständigen 16-stelligen Hash der Konfiguration.
In der Agentenbäckerei werden nur die ersten 8 Zeichen des Hashs angezeigt.
...
Beim Download über den Hash geben Sie den vollständigen 16-stelligen Hash und das Zielformat an:

[{bash}]
----
curl \
  --header "Authorization: Bearer $USERNAME $PASSWORD" \
  "http://$HOST_NAME/$SITE_NAME/check_mk/api/v1/domain-types/agent/actions/download_by_hash/invoke?agent_hash=<hash>&os_type=linux_deb" \
  --output check-mk-agent.deb
----

Alternativ können Sie ein Agentenpaket auch über den Hostnamen herunterladen, ohne den Hash zu kennen:

[{bash}]
----
curl \
  --header "Authorization: Bearer $USERNAME $PASSWORD" \
  "http://$HOST_NAME/$SITE_NAME/check_mk/api/v1/domain-types/agent/actions/download_by_host/invoke?agent_type=host_name&host_name=myhost&os_type=linux_deb" \
  --output check-mk-agent.deb
----

Der Parameter `agent_type` entspricht den Spaltentypen in der Agentenbäckerei: `host_name` für Host-spezifische Pakete, `vanilla` für den Vanilla-Agenten und `generic` für ordnerspezifische Pakete.
Für ordnerbasierte Pakete verwenden Sie statt des Parameters `host_name` den Parameter `folder_name`.

Als Werte für `os_type` stehen `linux_deb`, `linux_rpm`, `linux_tgz`, `windows_msi`, `aix_tgz`, `solaris_pkg` und `solaris_tgz` zur Verfügung.

*Agenten backen*

Um einen Backvorgang auszulösen, senden Sie eine POST-Anfrage:
...
curl \
  --request POST \
  --header "Authorization: Bearer $USERNAME $PASSWORD" \
  "http://$HOST_NAME/$SITE_NAME/check_mk/api/v1/domain-types/agent/actions/bake/invoke"
----

Da der Backvorgang asynchron abläuft, können Sie den aktuellen Status abrufen:

[{bash}]
----
curl \
  --header "Authorization: Bearer $USERNAME $PASSWORD" \
  "http://$HOST_NAME/$SITE_NAME/check_mk/api/v1/domain-types/agent/actions/baking_status/invoke"
----

Warten Sie, bis der Status `done` zurückgegeben wird, bevor Sie die fertig gebackenen Agentenpakete herunterladen.
