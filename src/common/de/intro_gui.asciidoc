...
image::intro_empty_dashboard.png[alt="{CMK}-Startseite mit noch leerem 'Main dashboard'.",fullscreen=1]
endif::[]
ifdef::saas[]
.Die Startseite von {CE}
image::intro_empty_dashboard_saas.png[alt="{CMK}-Startseite mit noch leerem 'Main dashboard'.",fullscreen=1]
endif::[]
...
.Drei Punkte (...) reduzieren Komplexität in {CMK}
ifdef::onprem[]
...
Außer den beiden in den xref:search_tips[Tipps für die Suche] erwähnten Filtern `h:` und `s:` können Sie weitere nutzen. Welche das sind, und wie Sie auch reguläre Ausdrücke verwenden können, erfahren Sie im xref:user_interface#search_monitor[Artikel über die Benutzeroberfläche].
