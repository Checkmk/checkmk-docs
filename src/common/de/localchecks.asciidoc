...
|`0` |Status |Der Zustand des Services wird als Ziffer angegeben: `0` für {OK}, `1` für {WARN}, `2` für {CRIT} und `3` für {UNKNOWN}.
Alternativ ist es möglich, den xref:dynamic_state[Status dynamisch berechnen] zu lassen, dann wird die Ziffer durch ein `P` ersetzt.
...
|`My output text which may contain spaces` |Statusdetail |Details zum Status, wie sie in {CMK} im Feld [.guihint]#Summary# angezeigt werden.
...
// TK: temp/temperature wird im Artikel aber nicht verwendet.
...
// TK: Warum hier feste Links auf die 2.4.0 und nicht auf {current-major} wie in den folgenden Beispielen?
...
// TK: Die 3 folgenden Abschnitte passen nicht so richtig ins Kapitel Metriken, aber sei's drum.
