...
Zwischen den vier Teilen dieser Ausgabe muss immer *genau ein Leerzeichen* stehen.
Innerhalb der Statusdetails können Leerzeichen beliebig verwendet werden.
...
[TIP]
====
Beim direkten Aufruf des Agentenskripts oder des Plugins in einer Shell unter Linux stehen möglicherweise andere link:https://wiki.debian.org/EnvironmentVariables[Umgebungsvariablen^] zur Verfügung als beim Aufruf durch den Agent Controller.
Unter Windows kommt hinzu, dass der Agent Controller unter dem Systembenutzer läuft, der Aufruf im Terminal aber unter einem normalen Nutzer oder Administrator erfolgt.
Zusätzlich zur anderen Umgebung kann dies fehlende Berechtigungen bedeuten.
Um die Ausgabe des Agentenskripts möglichst nahe an den Bedingungen zu analysieren, unter denen der {CMK}-Agent aufgerufen wird, sollten Sie daher nach Möglichkeit den xref:agent_linux#agent_ctl_dump[Dump-Modus des Agent Controller] verwenden.
====
...
{CMK} als Linux-Anwendung erwartet aber ohne wenn und aber _UTF-8_ und einfache _Line Feeds_.
Der Zeichensatz bezogenen Fehlersuche widmen wir im Artikel zu xref:spool_directory#[Spool-Dateien] ein xref:spool_directory#charset[ganzes Kapitel].
====
