...
Es kann legitim sein, in dem Skript, das den lokalen Check bereitstellt, zwischen dynamischer und statischer Zustandsermittlung zu wechseln.
Grundlage der Entscheidung sollte sein, dass wer etwas sicherer weiss, den Status bestimmt.
...
[{shell}]
----
P "Backup stuff" duration=2342;1800;3600 Successfully created the backup
----

Mit der Auswertung im Monitoring-System und nicht auf dem überwachten Host selbst können Monitoring-Admins die Standard-Schwellwerte überschreiben.
Schließlich sind die Schwellwerte der Entwickler der lokalen Checks oft zurecht nur grobe Vorschläge.
Ist ein bestimmtes Netzwerk-Share wegen Wartungsarbeiten langsamer, sind die Schwellwerte schneller im Monitoring als im Skript angepasst.

Schlägt das Backup aber nach kurzer Zeit fehl, weiss das niemand besser als das Backup-Skript selbst.
In diesem Fall _muss_ es den Status setzen und _darf_ kein Überschreiben durch das Monitoring zulassen.

[{shell}]
----
2 "Backup stuff" duration=123;1800;3600 Backup failed.
----

Dass in diesem Fall keine Schwellwerte im Graphen angezeigt werden, ist nur sinnvoll.
Denn nicht die benötigte Dauer bis zum (fehlgeschlagenen) Backup  ist hier relevant, sondern die Tatsache, dass es nicht erfolgreich war.
...
// MFS: KNW-1782 Limitations durch was weiss ich was
[TIP]
====
...
Das Metrik- und Graphing-System von {CMK} ist aufgrund von (historischen) Abwägungen hinsichtlich Performance und Robustheit auf obere Schwellwerte eingeschränkt. 
Dies bedeutet, dass zwar die Ermittlung des Status eines Dienstes – welche in der _Check Engine_ vor der Übergabe an den Monitoring-Kern stattfindet –, erwartungsgemäß funktioniert, aber die in der Metrik- und Graphing-Komponente dargestellten Informationen untere Schwellwerte ignorieren.
Daher fehlen in der {CMK}-GUI bei ausschließlicher Verwendung unterer Schwellwerte Dinge wie gelbe und rote Linien in Graphen, Perf-O-Meter und [guihint]#Service performance data# komplett.

Wir evaluieren derzeit, ob die Vorteile durch die Einschränkungen weiterhin deren Nachteile überwiegen.
====
