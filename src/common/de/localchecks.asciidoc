...
Mit den _lokalen Checks_ (_local checks_) sind Sie in der Lage, durch kleine Erweiterungen des Agenten auf dem Ziel-Host schnell und einfach eigene Services zu erstellen.
...
Zudem geben lokale Checks den _Admins von Servern_ weitreichende Freiheiten.
...
In diesem Artikel verwenden einige Beispiele die Bezeichner `humidity` oder `temp`.
Für beide existieren vordefinierte Metrikdefinitionen, welche die Metriken mit den korrekten Einheiten versehen.
In link:https://github.com/Checkmk/checkmk/blob/2.4.0/cmk/plugins/collection/graphing/humidity.py[beiden^] link:https://github.com/Checkmk/checkmk/blob/2.4.0/cmk/plugins/collection/graphing/temperature.py[Fällen^] stellt die Metrikdefinition ein Perf-O-Meter bereit und die Legenden der Graphen zeigen dann _Grad Celsius_ und _relative Luftfeuchte in Prozent_.
...
Unter Windows können Sie dafür die Regelsätze [.guihint]#Set execution mode for plug-ins and local checks# und [.guihint]#Set cache age for plug-ins and local checks# nutzen. Diese und weitere Regelsätze für lokale Checks unter Windows finden Sie in der Agentenbäckerei unter [.guihint]#Agent rules > Windows Agent#.
...
Service-Ausgabe darauf hinweisen.
