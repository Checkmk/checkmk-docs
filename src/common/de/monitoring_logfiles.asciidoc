...
:description: In {CMK} können Sie mit mk_logwatch Log-Dateien auf bestimmte Inhalte prüfen und überwachen. Hier erfahren Sie, wie Sie die Überwachung einrichten.
...
In {CMK} übernimmt unser Agenten-Plugin `mk_logwatch.py` (meist einfach „Logwatch“ genannt) diese Aufgabe.

Grundsätzlich sollten Sie bei der Verwendung von Logwatch Maß halten.
...
Eine entsprechende Cabinet-Datei liegt allerdings bereits in Ihrer {CMK}-Instanz bereit.
...
Diese Datei müssen Sie mit der Version aus der {CMK}-Instanz überschreiben.
...
// ES: Dieser Block muss ein Block vom Typ powershell werden, außerdem sollte Stop-Service statt net stop verwendet werden -> KNW-2269
[{shell}]
----
...
ifdef::onprem[]
Kopieren Sie dazu die Datei `~/share/check_mk/agents/plugins/mk_logwatch.py` von Ihrer {CMK}-Instanz auf den Host in das Verzeichnis `/usr/lib/check_mk_agent/plugins/` (Linux) bzw. `C:\ProgramData\checkmk\agent\plugins` (Windows).
endif::[]
ifdef::saas[]
Das Plugin `mk_logwatch.py` finden Sie in der Weboberfläche Ihrer {CMK}-Instanz unter [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX  > Related#.
...
können während der xref:configuration[Konfiguration der Regel] [.guihint]#Text log files (Windows, Linux, Solaris, AIX)# die Option [.guihint]#Deploy the Logwatch plug-in and its configuration# auswählen und das Agentenplugin so automatisch mit dem Agenten ausrollen.
...
Deshalb ist es nach der Installation den Agentenplugins unerlässlich, eine Konfigurationsdatei für den überwachten Host anzulegen.
...
Sie zunächst die Regel für das Agentenplugin [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Text log files (Windows, Linux, Solaris, AIX)# auf.
Die Voreinstellung [.guihint]#Deploy the Logwatch plug-in and its configuration# sollte im Regelfall so belassen werden.
Falls Sie die Konfigurationsdatei `logwatch.cfg` allerdings auf einem anderen Weg auf den Host übertragen wollen oder müssen, steht hier noch die Option [.guihint]#Deploy the Logwatch plug-in without configuration# zur Verfügung.
...
Jetzt folgt die Sektion der Regel, wo es endlich richtig losgeht -- [.guihint]#Configure a log file section#.
Und das beginnen wir direkt mit dem größten Stolperstein der vergangenen Jahre.
In dem Feld [.guihint]#Patterns for log files to monitor (allowing * and ?)# müssen Sie die Log-Dateien, die Sie überwachen wollen, benennen.
...
Die folgende Tabelle gibt Ihnen noch ein paar hilfreiche Beispiele, wie Sie Globs verwenden können, um auch wirklich nur die Dateien zu überwachen, die der Überwachung bedürfen, ohne alle einzeln angeben zu müssen:
...
Wenn Sie jetzt allerdings weiter aussieben müssen, können Sie in einem zweiten Schritt mit der Option [.guihint]#Regular expression for log file filtering# reguläre Ausdrücke auf die Treffer aus Schritt 1 anwenden.
...
Die maximale Anzahl der Zeilen, die Logwatch pro Durchlauf und Datei prüft, lässt sich mit [.guihint]#Restrict number of processed messages per cycle# einschränken, und mit [.guihint]#Restrict runtime of log file parsing# können Sie dafür sorgen, dass Logwatch sich nicht zu lang an einer einzelnen Datei aufhält, die womöglich seit der letzten Prüfung mit abertausenden neuen Einträgen geflutet wurde.
...
Wenn aber auf einem Host 50 Log-Dateien überwacht werden, die plötzlich 100 000 neue Zeilen mit einer Länge von 500 Zeichen aufweisen, landen wir schon im Gigabyte-Bereich.
...
[.guihint]#Process new log files from the beginning# ist standardmäßig abgeschaltet.
...
Andernfalls informieren Sie Logwatch mithilfe der Option [.guihint]#Character encoding that should be used to decode the matching files# explizit darüber, welche Kodierung für die jeweilige Log-Datei erwartet wird.
In jedem Fall kodiert Logwatch seine eigene _Ausgabe_ immer in UTF-8 -- falls Umwandlungen nötig sind, beruft Logwatch sich für die Übersetzung auf die von Ihnen angegebene Kodierung.
...
Jede Zeile einer Log-Datei, die ausgewertet wird, wird auf jeden von Ihnen angegebenen regulären Ausdruck in der Reihenfolge von oben nach unten geprüft, bis ein Match gefunden wurde.
Bei einem Match wird der gewünschte Zustand zugeordnet und es geht sofort mit der jeweils nächsten Zeile weiter.
...
Alternativ können Sie aber erstmal alle interessanten Meldungen an den {CMK}-Server übertragen (beispielsweise eben mit dem Zustand {OK}) und diese dann im Anschluss mit der Regel [.guihint]#Log file patterns# direkt in der {CMK}-Instanz (um-)klassifizieren.
...
ifdef::onprem[]
{cre-only}
In {CRE} konfigurieren Sie das Agentenplugin wie üblich über eine Textdatei.
endif::[]
ifdef::saas[]
...
Eine umfangreiche Beispielkonfiguration finden Sie als Instanzbenutzer in der Datei `~/share/check_mk/agents/cfg_examples/logwatch.cfg`.
...
|`overflow` |[.guihint]#In case of an overflow...# |`overflow=W` | Keine Unterscheidung zwischen Groß- und Kleinschreibung des Wertes
...
Durch die Definition von Gruppierungen mithilfe der Regel [.guihint]#Log file grouping# können Sie auf den Check `logwatch_groups` umschalten.
...
//SK: TODO
// ES: In KNW-1673 ging es unter anderem auch um Werk #15672. Falls dieser Abschnitt mal ausformuliert wird, wird damit auch dieses Werk erledigt. Eine explizite Erwähnung halte ich nicht für notwendig.
...
Neben der direkten Verarbeitung von Log-Meldungen in {CMK} und einer etwaigen Umklassifizierung mit der Regel [.guihint]#Log file patterns# gibt es auch noch die Möglichkeit, durch Logwatch gewonnene Log-Zeilen an die xref:glossar#ec[Event Console] weiterzuleiten.
...
// ES: mk_logwatch_2.py existiert bis einschließlich {CMK} 2.4.0. Die übrigen Änderungen aus dieser Bearbeitungsrunde sollten nach 2.4.0 gepickt werden, daher lasse ich diesen Kommentar erstmal so stehen und entferne die Tabellenzeile dann nur für 2.5.0.
endif::[]
