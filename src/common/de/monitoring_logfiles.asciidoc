...
Alternativ können Sie aber erstmal alle interessanten Meldungen an den {CMK}-Server übertragen (beispielsweise eben mit dem Status {OK}) und diese dann im Anschluss mit der Regel [.guihint]#Logfile patterns# auf dem {CMK}-Server (um-)klassifizieren.
