...
:description: {CMK} kann Benachrichtigungen zur weiteren Verarbeitung an die Incident-Management-Plattform Opsgenie senden. Lesen Sie hier, wie Sie Opsgenie anbinden.
...
+
image::notifications_parameter_opsgenie.png[alt="Die Einstellungen zur Benachrichtigungsmethode für Opsgenie."]
+
...
Besonders interessant ist die Option [.guihint]#Integration team:# Hinterlegen Sie hier den Namen des Opsgenie-Teams, in dessen Namen die Integration erstellt wurde.
Dadurch können weitere Informationen eingeholt sowie Nachricht und Beschreibung einer Benachrichtigung später aktualisiert werden.
