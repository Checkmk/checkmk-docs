...
Das Monitoring übersichtlicher AWS-Umgebungen lässt sich am schnellsten mit unserem [.guihint]#Quick Setup# einrichten.
...
Eine detaillierte Anleitung, wie Sie an die notwendigen Vorbereitungen in Ihrer AWS-Umgebung vornehmen können, finden Sie im Kapitel xref:#preparationp[AWS für {CMK} vorbereiten].
...
Sollte Ihre AWS-Umgebung hingegen eher komplex sein, empfiehlt sich die Einrichtung weiterhin nach den folgenden Schritten durchzuführen.
====
