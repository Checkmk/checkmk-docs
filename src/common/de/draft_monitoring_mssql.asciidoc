...
Nun gibt es weitere Optionen, sowohl für die Überwachung der Datenbank auf dem gleichen Host als auch für die Überwachung auf einem entfernten Host.
Diese sind prinzipiell für beide Verbindungstypen gleich.

[cols="30,5,~", options="header"]
|===
|Option | | Beschreibung
|Connection | |
|Data to collect (Sections) | |Hier können Sie abschnittweise einschränken, welche Daten eingesammelt werden sollen bzw. ob diese synchron oder asynchron gesammelt werden sollen.
|Cache age for asynchronous checks | |Für die zuvor festgelegten asynchronen Checks können Sie den Caching-Zeitraum ändern.
|Map data to specific host (Piggyback) | |
|Discovery mode of databases | |
|Custom instances | |
|Options | |Die maximale Anzahl an SQL-Server-Verbindungen kann hier eingestellt werden.
|===
