...
Mindestens Ihre die Directory-ID (auch Tenant-ID genannt) und eine Application-ID (auch Client-ID genannt) werden zwingend benötigt.
...
An dieser Stelle ist zu sagen, dass sich Web-Portale von Hyperscalern und Anbietern von Cloud-Diensten mit schöner Regelmäßigkeit ändern.
...
Nachdem die App angelegt wurde, sollten Sie in einer Übersicht zu dieser neuen App landen.
...
In den Details der App finden Sie nun sowohl die [.guihint]#Application (client) ID,# als auch die [.guihint]#Directory (tenant) ID#, welche Sie gleich in {CMK} eintragen müssen.
...
Nun brauchen Sie noch einen geheimen Client-Schlüssel (im Englischen schlicht [.guihint]#Client secret# genannt), mit dem sich {CMK} bei der API anmelden kann.
Um einen solchen Schlüssel zu erzeugen, klicken Sie in der Übersicht der App auf [.guihint]#Certificates & secrets#, achten dann darauf das der Reiter [.guihint]#Client secrets# ausgewählt ist und klicken dann auf [.guihint]#New client secret.#
...
Wenn Sie später, in der Regel für den Spezialagenten, die Option [.guihint]#App registrations# aktiveren, bekommen Sie einen praktischen Service, der Sie daran erinnert, wenn sich diese Gültigkeitsdauer dem Ende nähert.
...
Da Sie sich ja noch in der Übersicht Ihrer neuen App befinden, können wir diesen optionalen Punkt bearbeiten.
Wenn Sie nämlich die folgenden Services mit {CMK} überwachen möchten, müssen Sie der App zusätzliche API-Berechtigungen erteilen.

* Users in the Active Directory
* AD Connect Sync
* App Registrations

Klicken Sie dafür auf [.guihint]#API permissions# und anschließend auf [.guihint]#Add a permission#.
In dem Dialog der sich jetzt geöffnet hat, müssen Sie den Punkt [.guihint]#Microsoft Graph# finden und anklicken.
Wählen Sie anschließend [.guihint]#Application permissions# und tippen Sie `Directory.Read.All` in die Suche ein.
Aktivieren Sie zugehörige Checkbox und klicken Sie auf [.guihint]#Add permissions#.
...
Wählen Sie dazu in der Hauptnavigation auf der linken Seite den Punkt [.guihint]#All services# und hier dann unter [.guihint]#General# den Punkt [.guihint]#Subscriptions.#
...
In der folgenden Übersicht finden Sie auch die [.guihint]#Subscription ID#, die Sie gleich in die Regel des Spezialagenten eintragen müssen.
...
Im Dialog [.guihint]#Select members# geben Sie im Suchfeld den Namen der App ein, wie Sie ihn vorhin angelegt haben, wählen die App aus, klicken auf [.guihint]#Select#.
Nach zwei weiteren Klicks auf [.guithint]#Review + assign# ist die Einrichtung im Azure-Portal abgeschlossen.
