// -*- coding: utf-8 -*-
// IGNORE + % Subskriptions Directorys
...
Das Monitoring übersichtlicher Azure-Umgebungen lässt sich am schnellsten mit unserem [.guihint]#Quick Setup# einrichten.
...
Eine detaillierte Anleitung, wie Sie an die notwendigen Vorbereitungen in Ihrer Azure-Umgebung vornehmen können, finden Sie im Kapitel xref:#preparationp[Azure für {CMK} vorbereiten].
...
ifdef::onprem[]
{CCE-only}
Zum komfortablen Einstieg in die Überwachung von Azure liefern {CE} und höher
endif::[]
ifdef::saas[]
