...
// TK: Ich war zuerst etwas verwirrt, welche Services denn wo erzeugt werden. Ich hab etwas umgeschrieben, damit ich es besser verstehe.
...
Die Gesamtkosten auf Ebene Ihres Azure-Directorys, werden bei dem Azure-Host angezeigt, den sie xref:create_host[zuerst] angelegt haben.
...
// TK: Das "dann" im vorherigen Satz, um klarer zu machen, dass das ein anderer Service ist (hoffe, ich habs richtig verstanden).
...
// TK: Der Screenshot oben (azure_wato_no_ip.png) hat leider fast nix mehr mit der aktuellen 2.3.0 GUI zu tun. Daher ist auch diese Option nicht zu sehen.
