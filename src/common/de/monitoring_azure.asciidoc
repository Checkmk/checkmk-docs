// -*- coding: utf-8 -*-
// IGNORE + % BuiltInRole Subskriptions Hyperscalern
...
Da Azure nicht über den normalen {CMK}-Agenten abgefragt werden kann, richten Sie jetzt den xref:glossar#special_agent[Spezialagenten] für Azure ein, welcher auch als xref:datasource_programs#[Datenquellenprogramm] bezeichnet wird.
Hierbei kontaktiert {CMK} den Ziel-Host nicht wie üblich über TCP Port 6556, sondern ruft stattdessen ein Hilfsprogramm auf, welches mit dem Zielsystem über ein die anwendungsspezifische API von Azure kommuniziert.
...
ifdef::onprem[]
Übrigens: mit dem Hilfsskript `find_piggy_orphans` aus dem `treasures`-Verzeichnis finden Sie alle Piggybacked-Hosts, für es Daten gibt, die aber noch nicht als Host im {CMK} angelegt sind:
...
[IMPORTANT]
====
Vor dem Update auf {CMK} {v24} müssen Sie unbedingt prüfen, ob Sie die Labels ohne das Präfix `cmk/azure/` in irgendeiner Form nutzen.
Stellen Sie diese unbedingt *vor* dem Update auf die Labels mit dem Präfix um.
Die Labels ohne Präfix fallen bei der ersten Service-Erkennung nach dem Update weg und bspw. Regeln, die auf diese Labels setzen, werden dann wirkungslos.
====
