...
:description: {CMK} bietet ein umfangreiches Monitoring von Microsoft Azure-Produkten und ihren Kosten. Lesen Sie hier, wie Sie die Integration einrichten und nutzen.
...
{CMK} bietet ein umfangreiches Monitoring von Microsoft Azure, welches aus einem xref:glossar#special_agent[Spezialagenten] und einer stattlichen Sammlung von Check-Plugins besteht, die für Sie verschiedenste Metriken und Zustände abrufen und auswerten.
...
ifdef::saas[]
Neben den allgemeinen Informationen zu den link:https://checkmk.com/integrations/azure_usagedetails[Kosten^] Ihrer Azure-Umgebung und dem aktuellen link:https://checkmk.com/integrations/azure_status[Status^] der Azure-Dienste in Ihrer Region, können Sie mit {CE} die folgenden Microsoft Azure-Produkte überwachen:
endif::[]
...
== Azure-Monitoring ab {CMK} {v25}

Mit {CMK} {v25} haben wir das Monitoring von Microsoft Azure vollständig überarbeitet.
Neben dem neuen Spezialagenten, wurde auch alle bisherigen Check-Plugins erneuert.

[TIP]
====
Der Spezialagent und die Check-Plugins des neuen Azure-Monitorings sind mit dem alten *nicht* kompatibel.
Eine Weiterführung der bisherigen Daten, Metriken und Performance-Daten ist nicht möglich.
====

Weitere Informationen zum neuen Spezialagenten finden Sie auch im zugehörigen link:https://checkmk.com/werk/19467[Werk #19467^].

Informationen zur Einrichtung und Verwendung des alten Azure-Monitorings finden Sie im entsprechenden Handbuchartikel zu {CMK} {v24}: link:https://docs.checkmk.com/2.3.0/de/monitoring_azure.html[Microsoft Azure überwachen bis {CMK} {v24}].
...
sondern ruft stattdessen ein Hilfsprogramm auf, welches mit dem Zielsystem über die anwendungsspezifische API von Azure kommuniziert.
...
Konfigurieren Sie die Ressourcengruppen-Hosts ohne IP-Adresse (analog zum Azure-Host) und wählen Sie als Agent [.guihint]#No API integrations, no {CMK} agent# und als Piggyback [.guihint]#Always use and expect piggyback data# aus.
...
ifdef::onprem[]
{CCE-only}
Zum komfortablen Einstieg in die Überwachung von Azure liefert {CMK} in {CE}
endif::[]
ifdef::saas[]
Zum komfortablen Einstieg in die Überwachung von Azure liefert {CE}
endif::[]
