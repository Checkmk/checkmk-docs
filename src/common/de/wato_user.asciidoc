...
Sie können diesen Automationsbenutzer nutzen -- oder auch einen neuen erstellen.
...
[IMPORTANT]
====
Wichtig: Die HTTP-Basisauthentifizierung benötigt das Automationspasswort standardmäßig im Klartext.
Setzen Sie dazu die Option [.guihint]#Store the secret in cleartext.# 
Gespeichert wird es dann im Instanzverzeichnis unter `/var/check_mk/web/$username/automation.secret`.
