...
Dieser darf zwar alle Hosts und Services sehen, aber nur, weil in seiner Rolle „Administrator“ die Berechtigung [.guihint]#See all hosts and services# enthalten ist und nicht, weil er für alles ein Kontakt wäre.
...
Nach der erstmaligen Registrierung sieht diese Seite etwa so aus:
...
* Er hat die Rolle „Administrator“ und damit alle Berechtigungen.
* Er ist für nichts Kontakt und bekommt keine Benachrichtigungen.
* Er darf trotzdem alles sehen (wegen seiner Rolle „Administrator“).
* Das Passwort, das beim Erstellen der Instanz vergeben wurde, sollten Sie auf jeden Fall ändern!

Das Formular zum Anlegen eines neuen Benutzers mit icon:button_new_user[alt="Knopf zum Erstellen eines neuen Benutzers."] oder zum Editieren eines bestehenden Benutzers mit icon:icon_edit[alt="Symbol zum Bearbeiten eines Listeneintrags."] ist in mehrere Abschnitte unterteilt.
...
Das Formular zum Editieren eines bestehenden Benutzers mit icon:icon_edit[alt="Symbol zum Bearbeiten eines Listeneintrags."] ist in mehrere Abschnitte unterteilt.
...
// MFS: In SaaS haben wir nur HTTPS, evtl. allgemein ändern auf HTTP/HTTPS?
Die Option [.guihint]#Automation secret for machine accounts# ist für Konten gedacht, die skriptgesteuert per HTTP auf {CMK} zugreifen und sich über die URL authentifizieren.
...
Wenn Sie einen Benutzer mit der Option [.guihint]#disable the login to this account# *sperren,* wird er in der Tabelle mit dem Symbol icon:icon_user_locked[] dargestellt.
...
=== Benachrichtigungen
// TK: Der Kasten ist in der SaaS noch drin, soll aber raus.
...
ifdef::onprem[]
Alle Einstellungen in diesem Kasten kann der Benutzer über [.guihint]#User > Edit profile# auch xref:personal_settings[selbst ändern] (außer in der Rolle „Guest user“).
endif::[]
ifdef::saas[]
...
// MFS: Für Rollen sollten wir guihint verwenden, damit deren Namen nicht von Deepl übersetzt werden.
Die Tatsache, dass ein normaler Benutzer (Rolle „Normal monitoring user“) nur solche Objekte sieht, für die er ein Kontakt ist, ist umso wichtiger, je größer Ihre Monitoring-Umgebung ist.
Das sorgt nicht nur für Übersicht, sondern verhindert auch, dass Benutzer dort eingreifen, wo sie nichts zu suchen haben.

Als Administrator (Rolle „Administrator“) dürfen Sie natürlich immer alles sehen.
...
// MFS: Gibt es diese Doppelrollen nicht nur onprem?
...
ifdef::onprem[]
Die Rolle „Guest user“ ist so voreingestellt, dass auch ihre Benutzer alles sehen können.
...
Sollten in zukünftigen {CMK}-Versionen neue Berechtigungen hinzukommen oder sich bestehende ändern, können Sie trotzdem sicher sein, dass eine von `no_permissions` abgeleitete Rolle keine neuen, unerwarteten Berechtigungen erhält.
...
Übrigens: Beim Erzeugen einer neuen {CMK}-Instanz wird für die Anmeldung an der {CMK}-Oberfläche nur ein Benutzer (`cmkadmin`) der Rolle „Administrator“ angelegt.
...
Wie üblich gelangen Sie über das Symbol icon:icon_edit[alt="Symbol zum Bearbeiten eines Listeneintrags."] in den Editiermodus für eine Rolle:
...
Neue Rollen erschaffen Sie durch ein Ableiten von bestehenden Rollen mittels icon:icon_clone[] [.guihint]#Clone#.
...
Jedes mal entscheiden wir dann, in welcher der drei Rollen „Administrator“, „Normal monitoring user“ und „Guest user“ die neue Berechtigung enthalten sein soll.
...
sondern auch die Stellen sehen, an denen explizit Berechtigungen gesetzt (Symbol icon:icon_perm_yes[]) bzw. entfernt (Symbol icon:icon_perm_no[]) wurden.
...
// TK: Was ist davon für die SaaS relevant?
...
Dieses können Sie mit dem icon:icon_random[alt="Symbol zum Auswürfeln eines Passworts."] Würfel automatisch erstellen lassen:
...
// TK: Die folgende URL passt nicht zur SaaS
// MFS: Halte ich hier für unproblematisch
...
Die Rolle „Guest user“ ist dafür gut geeignet, weil diese alle Leserechte einräumt, aber keine Veränderungen oder Eingriffe zulässt.
...
=== Bedeutung der Rolle „Normal monitoring user“
...
Der erste Schritt dazu ist, dass Ihre Administrator-Kollegen mit eigenen Benutzern arbeiten, die auf der Rolle „Normal monitoring user“ basieren. 
...
Solange Sie noch keine Ordner oder BI-Pakete freigegeben haben, bedeutet das, dass die Benutzer der Rolle „Normal monitoring user“ zunächst keinerlei Änderungen machen können.
...
Ein Entsperren ist dann nur noch durch einen Benutzer mit der Rolle „Administrator“ möglich.
...
Nach dem Aktivieren dieser globalen Einstellung *kann* es Ihnen passieren, dass Sie vom System abgemeldet werden und sich nicht mehr einloggen können
-- wenn noch eine alte Sitzung auf einem anderen System als aktiv gilt.
Sollten Sie kein weiteres Administratorkonto für die Korrektur haben, können Sie die Einstellung händisch aus der zugehörigen Konfigurationsdatei entfernen.
Anschließend können Sie sich wieder normal in der Weboberfläche einloggen.
Die globalen Einstellungen finden Sie als Instanzbenutzer in der Datei `~/etc/check_mk/multisite.d/wato/global.mk`.
...
* Angabe der Webadresse als einfacher Host-Name oder als voll qualifizierter Domainname, auf jeden Fall eine gültige Domainadresse.
...
Dann erscheint ein weiterer Anmeldedialog:
...
Wollen Sie sich dann mit einem Backup-Code auf der {CMK} Website anmelden, klicken Sie im zweiten Anmeldedialog auf [.guihint]#Use backup code#.
// TK: Zweiter Anmeldedialog? Ich versteh nicht, wie ich an den Dialog im folgenden Screenshot komme.
Geben Sie einen Ihrer Backup-Codes ein und melden Sie sich mit [.guihint]#Use backup code# an.
...
Öffnen Sie dazu in der Benutzerverwaltung den betreffenden Eintrag mit einem Klick auf icon:icon_edit[alt="Symbol zum Bearbeiten eines Listeneintrags."].
