...
// Sie sehen hier den einzigen Benutzer `cmkadmin`, welcher automatisch beim Erzeugen der Instanz angelegt wurde.
...
// TK: Da im verlinkten Glossareintrag das Wichtigste steht, lass ich an dieser Stelle die Automationsbenutzer links liegen.
Bei der {CMK}-Appliance kann dieser Benutzer anders heißen, da Sie dessen Namen und Passwort selbst festlegen.
Dieser erste Benutzer hat folgende Eigenschaften:
...
von allen diesen Rollen. Mit den vordefinierten Rollen (siehe
...
handelt es sich um selten benötigte Einstellungen.
...
// |`agent_registration` |Die Berechtigung, den xref:wato_monitoringagents#agents[{CMK}-Agenten] eines Hosts (präziser: dessen Agent Controller) beim {CMK}-Server zu registrieren -- sonst nichts. |Gedacht für einen Automationsbenutzer, um eine Registrierung mit minimalen Rechten durchzuführen. In der {CCE} enthält diese Rolle zusätzliche Berechtigungen, um xref:hosts_autoregister#[Hosts automatisch zu erstellen.]
...
// TK: Die folgenden beiden Sätze sind neu:
...
Beachten Sie dabei, dass auch die Administratorkonten gesperrt werden können!
Sollten Sie endgültig ausgesperrt sein, so können Sie Ihr Konto nur noch auf der Kommandozeile entsperren.
...
Sie können Hardware-Authentifikatoren wie den YubiKey oder einen anderen USB-Token verwenden.
...
Klicken Sie auf den Knopf icon:icon_2fa_add[alt="Knopf zum Hinzufügen eines neuen Berechtigungsnachweises."] [.guihint]#Add credential#: 

image::wato_user_2fa_2.png[alt="Einrichtungsseite zur Zwei-Faktor-Authentifizierung."]

{CMK} erkennt die auf Ihrem Computer bereits eingerichteten Authentifizierungsmöglichkeiten. Es öffnet sich ein kleiner Dialog im Browserfenster, in dem sie den Authentifikator festlegen.
...
image::wato_user_2fa_backup.png[alt="Anzeige der erstellten Backup-Codes.", width=52%]
...
image::wato_user_2fa_login2.png[alt="Aufforderung zur Aktivierung des Authentifikators.", width=55%]
