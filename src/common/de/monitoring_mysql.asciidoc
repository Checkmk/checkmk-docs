...
// MFS: Muss das Komma vor "kann" dahin?
Zwar ist die Angabe eines Datenbanknutzers optional, wir empfehlen diese allerdings, da der Agent das Plugin in der Regel als _root_ ausführt.
// MFS: Hier würde ich deutlicher erklären, da Monitoring-Admins meist keine Datenbank-Admins sind.
// MFS: Zudem ist der Weg über die .mylogin.cnf sicherer, zumindest wenn ein Nutzer mit hohen Berechtigungen genutzt wird, da kein für jedermensch lesbares Klartext-Passwort hinterlegt werden muss.
// MFS: ...da der Agent das Plugin in der Regel als Systembenutzer _root_ ausführt.
// MFS: Wird kein Name eines Datenbankbenutzers angegeben, versucht der MySQL-Client den Zugriff auf die Datenbank mit dem Nutzername des ausführenden Systembenutzers ohne Verwendung eines Passwortes oder dem in `.mylogin.cnf` hinterlegten Passwort.
// MFS: Je nach Konfiguration der Datenbank (u.a. distributionsabhängig) kann die `.mylogin.cnf` bei der Installation automatisch erstellt worden sein.
// MFS: Daneben auch das hier verlinken, oder? https://dev.mysql.com/doc/refman/9.1/en/option-files.html
