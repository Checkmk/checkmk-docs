...
* link:https://checkmk.com/de/integrations/mysql_slave[MySQL: Slave Sync Status^]
...
Wird kein Name eines Datenbankbenutzers angegeben, versucht der MySQL-Client den Zugriff auf die Datenbank mit dem Nutzername des ausführenden 
Systembenutzers ohne Verwendung eines Passwortes oder dem in `.mylogin.cnf` hinterlegten Passwort.
Weitere Informationen finden Sie z.B. unter https://dev.mysql.com/doc/refman/9.1/en/option-files.html.
...
Tragen Sie User ID und Password den neuen Datenbanknutzer entsprechend ein. Über
