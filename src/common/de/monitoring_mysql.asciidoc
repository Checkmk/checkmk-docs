...
+
--
Um das Plugin unter Windows asynchron auszuführen, passen Sie die Konfiguration des Agenten an und erweitern die Sektion `_execution` unter `plugins` um den folgenden Eintrag:
...
plugins:
    _execution:
        - pattern: mk_mysql.ps1
          ...
          async: yes
----
// ES: Ich finde diese YAML-Datei für Windows nur mit dem Keyword `_execution`, nie ohne Underscore. 
