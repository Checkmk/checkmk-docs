...
:description: Mit {CMK} können Sie auch die Kubernetes-basierte OpenShift Container Platform überwachen. Lesen Sie hier, wie Sie die Überwachung einrichten und nutzen.
...
[TIP]
====
Das Monitoring von OpenShift-Clustern mit {CE-saas} ist per xref:glossar#checkmk_relay[{CMK} Relay] möglich.
...
ifdef::saas[]
Mit {CE} können Sie die folgenden Objekte überwachen:
endif::[]
...
Rechts oben sollte jetzt wiederum der Befehl [.guihint]#Create Namespace# auftauchen.
...
Mit dem Befehlszeilen-Tool `oc` können Sie nun auch alle Informationen aus Ihrem Cluster auslesen, welche Sie in der Regel zur Einrichtung des Spezialagenten angeben müssen.
...
Mit dem folgenden Befehl können Sie die FQDN womöglich auch auf der Befehlszeile ermitteln.
...
ifdef::saas[]
{CE} wird mit sechs eingebauten Dashboards für Kubernetes ausgeliefert.
endif::[]
