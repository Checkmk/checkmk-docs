...
Für den Service [.guihint]#CPU load# des Linux-Hosts unseres Beispiels können Sie eine neue Regel mit dem Regelsatz [.guihint]#CPU load (not utilization!)# unter [.guihint]#Service monitoring rules# erstellen, den Sie am schnellsten mit der xref:user_interface#search[Suche] im [.guihint]#Setup#-Menü finden.

Im Abschnitt [.guihint]#Value# finden Sie Parameter auf Service-Ebene, für die Sie den Wert [.guihint]#Predictive levels (only on CMC)# auswählen können:
...
Beispiel: [.guihint]#Warning at# `2.0` wird dazu führen, dass bei einem Wert über 12 und unter 8 eine Warnung angezeigt wird.
...
Beispiel: [.guihint]#Warning at# `1.0` als Vielfaches der Standardabweichung wird dazu führen, dass bei einem Wert über 10,41 und unter 9,59 eine Warnung angezeigt wird.
...
Beispiel: Ein [.guihint]#Warning level# von `2.0` wird dazu führen, dass eine Warnung nur bei einem Wert über 2 angezeigt wird, selbst wenn der obere Schwellwert für eine Warnung bei 1,5 liegt.
...
Haben Sie das prognosebasierte Monitoring für einen Service eingerichtet, xref:glossar#activate_changes[die ausstehenden Änderungen aktiviert] und hat {CMK} danach den Check für den Service einmal ausgeführt, wird das neue Symbol icon:icon_prediction[] in der Service-Liste angezeigt:
