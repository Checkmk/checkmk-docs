...
= Host-Merkmale
:title: Host-Merkmale
:description: Mit Host-Merkmalen (host tags), Merkmalsgruppen und Hilfsmerkmalen können Sie in {CMK} Ordner und Hosts nach Ihren Wünschen strukturieren.
...
ifdef::onprem[]
Und auf der Befehlszeile können Sie bei vielen Befehlen mit der Syntax `@foo` alle Hosts mit dem Merkmal `foo` auswählen.
endif::[]
...
image::hosttags_tags_default.png[alt="Liste aller vordefinierten Host-Merkmalsgruppen.",fullscreen=1]
...
.4+|`agent`
.4+|[.guihint]#{CMK} agent / API integrations#
.4+|Legt fest, auf welche Art die Daten vom Host geholt werden.
| [.guihint]#API integrations if configured, else {CMK} agent# | `cmk-agent`
| [.guihint]#Configured API integrations and {CMK} agent# | `all-agents`
| [.guihint]#Configured API integrations, no {CMK} agent# | `special-agents`
| [.guihint]#No API integrations, no {CMK} agent# | `no-agent`
