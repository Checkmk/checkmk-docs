...
:no-toc:
// TK: Hier unter {related-start} intro_setup nachtragen, wenn es denn geschrieben ist.
endif::[]
...
{CE} stellt Ihnen {CMK} als _Software as a Service_ (SaaS) zur Verfügung.
// TK: Hier Link einfügen, wenn cce geschrieben ist: ... erfahren Sie in xref:cce#[diesem Artikel.]
Was {CE} (SaaS) auszeichnet, auch im Vergleich zu dessen selbst gehosteten Geschwistern wie {CE} (Self-hosted), erfahren Sie in einem eigenen Artikel, der hier verlinkt wird, sobald er geschrieben ist.
endif::[]
...
// TK: Hier die ifdefs auflösen, wenn intro_setup geschrieben ist.
ifdef::onprem[]
...
ifdef::saas[]
Sollten Sie das erste Mal dieses Handbuch lesen, können Sie direkt mit dem (demnächst verfügbaren) ersten Artikel aus dem Leitfaden einsteigen
und gegebenenfalls später zu der Beschreibung der weiterführenden Kapitel zurückkehren.
endif::[]
...
ifdef::saas[]
Der Hauptteil erklärt von Kapitel 3 bis 8 dann die Funktionen und deren Nutzung in {CMK}:
