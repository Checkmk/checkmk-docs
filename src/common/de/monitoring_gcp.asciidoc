...
ifdef::onprem[]
// TK: Hier kannst Du gerne die Admonition verwenden, auch in monitoring_aws + monitoring_azure.
...
// TK: Oder so (damit der Link-Anker besser zum Ziel passt): Mit dem Kommando `cmk-piggyback list orphans` finden Sie alle xref:piggyback#orphaned_piggyback_data[verwaisten Piggybacked-Hosts,] für die ...
endif::[]
