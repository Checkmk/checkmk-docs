...
{cee-only}
Als Nutzer einer unserer kommerziellen Editionen können Sie die Erstellung und Löschung von Hosts für Ihre VM-Instanzen einfach der xref:dcd#[dynamischen Host-Verwaltung] überlassen.
...
{CCE-only}
Zum komfortablen Einstieg in die Überwachung von GCP liefert {CE} von {CMK} die beiden eingebauten xref:glossar#dashboard[Dashboards] [.guihint]#GCP GCE instances# und [.guihint]#GCP storage buckets# mit aus.
