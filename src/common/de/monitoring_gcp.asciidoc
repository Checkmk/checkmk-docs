// -*- coding: utf-8 -*-
// IGNORE Tabellen Arbeitsbereich
// The above IGNORE tokens appear in guihints and therefore need special treatment
// NONASCII
include::global_attr.adoc[]
= Google Cloud Platform (GCP) überwachen
:title: Google Cloud Platform (GCP) überwachen
:description: {CMK} bietet ein umfangreiches Monitoring von Google Cloud-Produkten und ihren Kosten. Lesen Sie hier, wie Sie die Integration einrichten und nutzen.
...
Wenn dies nicht der Fall ist, aktivieren Sie diese API über den Knopf [.guihint]#Enable APIs and services#.
Nach der Aktivierung dauert es noch einige Minuten, bis die API auch tatsächlich ansprechbar ist.
...
Nachdem Sie die Services hinzugefügt haben, sieht das nach einem xref:glossar#activate_changes[Aktivieren der ausstehenden Änderungen] im Monitoring etwa so aus:
...
ifdef::onprem[]
{CCE-only}
Zum komfortablen Einstieg in die Überwachung von GCP liefert {CMK} in {CE}
endif::[]
ifdef::saas[]
Zum komfortablen Einstieg in die Überwachung von GCP liefert {CE}
endif::[]
