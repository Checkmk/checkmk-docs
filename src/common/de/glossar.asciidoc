...
ifdef::saas[]
n einer frischen {CMK}-Instanz sind bereits mehrere Automationsbenutzer eingerichtet, z. B. für Webdienste und für die Registrierung des Agenten beim {CMK}-Server zur TLS-verschlüsselten Datenübertragung.
endif::[]
