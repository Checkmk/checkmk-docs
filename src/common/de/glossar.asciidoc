...
==== Agent Updater
ifdef::onprem[]
Der Agent Updater ist ein Agentenplugin in den kommerziellen Editionen, das die automatischen Aktualisierungen der Agenten ermöglicht. Der Updater wird separat vom Agent Controller oder gleichzeitig mit ihm bei der {CMK}-Instanz registriert.
endif::[]
ifdef::saas[]
Der Agent Updater ist ein Agentenplugin, das die automatischen Aktualisierungen der Agenten ermöglicht. Der Updater wird separat vom Agent Controller oder gleichzeitig mit ihm bei der {CMK}-Instanz registriert.
endif::[]
...
[#activate_changes]
==== Aktivierung ausstehender Änderungen
...
Zur Aktivierung der ausstehender Änderungen geht es über das [.guihint]#Changes#-Symbol in der Navigationsleiste.

Mehr unter xref:wato#activate_changes[Ausstehende Änderungen aktivieren.]
...
[#dashboard]
==== Dashboard
Ein Dashboard ist eine frei konfigurierbare Übersicht, bestehend aus Tabellenansichten und/oder so genannten Dashboard-Elementen (Widgets).
...
[#edition]
==== Edition
{CMK}-Editionen sind die unterschiedlichen Software-Varianten von {CMK}.
Also die Open-Source-Variante {CRE} (vormals {CMK} Raw), für professionelle Anwender die per Subskription erhältliche {CSE} (vormals {CMK} Enterprise) und die darauf aufbauende {CCE} (vormals {CMK} Cloud (Self-hosted)).
{ME} (vormals {CMK} MSP) ist die mandantenfähige Erweiterung von {CE}.
...
Alle kommerziellen Editionen können in den ersten 30 Tagen (im Lizenzstatus „Trial“) in vollem Funktionsumfang unverbindlich getestet werden.
...
// ES: Hier und auch in cse.asciidoc stört mich, dass im letzten Halbsatz einfach von {CMK} die Rede ist. Wenn man {CMK} dauerhaft betreiben möchte, kann man ja eh {CMK} Community wählen.
...
Mehr unter xref:wato_rules_about#rule_set_types[Arten von Regelsätzen.]
...
Mitgelieferte klassische Tabellenansichten können angepasst werden, und sie können als Basis für eigene Ansichten dienen.
Es ist auch möglich, klassische Tabellenansichten von Grund auf zu erstellen.
