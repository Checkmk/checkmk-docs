...
:description: In {CMK} können Sie Hosts mit Ordnern, Host-Merkmalen, Labels, Parents oder Host-Gruppen strukturieren. Lesen Sie hier, wie Sie eine Struktur aufsetzen.
:experimental:
...
Schlüssel-Wert-Paar eingeben, kbd:[Enter]-Taste drücken: Fertig!
...
Wie üblich, müssen Sie anschließend noch die ausstehenden Änderungen aktivieren.
