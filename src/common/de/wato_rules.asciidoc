...
// MFS: Wir hatten anderswo schon die Diskussion mit Consulting, dass die Erwähnung boolescher Algebra als zu abstrakt wahrgenommen wird.
// Daher evtl. Einführungsreihenfolge ändern und erläutern? `And` (sowohl als auch), `Or` (nicht ausschließendes "oder") und `Not` zur Verfügung.
// Dabei ist `Not` die Abkürzung für `And Not`, also "sowohl A als auch nicht B".
// Problematisch am Not ist, dass es alleine stehend als Vorzeichen zu betrachten ist (Not A And B), aber zwischen Bedingungen die Kurzform für "And Not" ist (A Not B).
// MFS: Wenn Du Änderungen klein halten magst, füg lediglich die Erläuterungen in Klammern ein.
...
// MFS: Ist der harte Umbruch hier im Absatz sinnvoll?
