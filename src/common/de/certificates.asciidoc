...
ifdef::onprem[]
Konfigurationsmöglichkeiten für Zertifikate finden Sie in den [.guihint]#Global settings#.
endif::[]
Die in {CMK} bei der Erstellung der Instanz generierten (oder später ersetzten) Zertifikate können Sie über die Seite [.guihint]#Certificate overview# einsehen.
...
In einigen Fällen sollen externe Anwendungen mit {CMK}-Komponenten kommunizieren, welche mit dem Instanz-Zertifikat abgesichert sind.
Häufig verweigern solche Anwendungen die Kommunikation, wenn der zum Verbindungsaufbau benutzte Host-Name nicht im Zertifikat enthalten ist.
Bei {UE} ab Version {v25} betrifft dies nur die xref:opentelemetry#collector_configuration[OpenTelemetry-Anbindung].
In künftigen Versionen werden aber weitere Komponenten hinzukommen, welche das Instanz-Zertifikat nutzen. 

Aus diesem Grund ist es sinnvoll, die verwendeten Host-Namen im Zertifikat zu hinterlegen.
In den [.guihint]#Global settings# haben Sie die Möglichkeit, mit [.guihint]#Site certificate subject alternative names# alle Host-Namen zu setzen, für welche das Server-Zertifikat gültig sein soll.
endif::[]
...
ifdef::saas[]
// ES: This sentence, and multiple entries in the following table, may need attention for KNW-2113
...
| Server-Zertifikat 
| _Instanz-Zertifikat_ u.a. zur Absicherung der Kommunikation zwischen Instanz (Agent Receiver) und Host (Agent Controller). 
Es ist ein SSL-Zertifikat und bestätigt die Echtheit des Schlüssels für die Netzwerkkommunikation.
Es wird an mehreren Stellen verwendet. 
...
| _CA-Zertifikat_ zum Signieren der individuellen Message-Broker-Zertifikate.
// SH: Oder ist dies hier das "CA-Zertifikat zum Signieren des lokalen Message Brokers"?
Dieser Eintrag ist nur für die On-Premises-Editionen von {CMK} relevant.
| Trifft nicht zu.

| `rabbitmq/ssl/cert.pem` 
| Server-Zertifikat 
| _Server-Zertifikat_ zur Absicherung der Netzwerkkommunikation des lokalen Message Brokers. 
Dieser Eintrag ist nur für die On-Premises-Editionen von {CMK} relevant.
| Trifft nicht zu.
|===
endif::[]
    ...
    Data:
        ...
        Subject Public Key Info:
            Public Key Algorithm: rsaEncryption
                Public-Key: (4096 bit)
                Modulus:
                    00:b3:6d:d4:f9:35:d6:2d:fd:1This entry is currently only relevant in the on-premises editions of Checkmk.

2.1. Certificates for Checkmk componentsf:aa:5d:ec:26:0b:
