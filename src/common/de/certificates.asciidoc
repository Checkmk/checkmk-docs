// -*- coding: utf-8 -*-
// IGNORE Herausgeberzertifikate Signierungsanfragen Mouseover Einzelmonitoring systeminterne Erstellungs Entität 
// NONASCII
// NO-LUNR
...
Jeder Eingriff in ein Zertifikat verändert den Fingerprint und fällt beim Abgleich mit einer vertrauenswürdigen Stelle auf.
...
Nach der Überprüfung der Echtheit eines Schlüssels bestätigen CAs die digitale Identität der überprüften Entität.
{CMK} sichert Ihre xref:glossar#site[{CMK}-Instanzen] über eigene Certificate Authorities ab:
Indem die CAs die eingesetzten Zertifikate auf Gültigkeit und Übereinstimmung mit den für sie hinterlegten Werten überprüfen, authentifizieren sie die systeminterne Kommunikation – beispielsweise zwischen einer Instanz und den ihr zugeordneten xref:glossar#host[Hosts] oder im verteilten Monitoring zwischen den Instanzen.

Ob Sie Systeme im verteilten Monitoring oder mit einer Einzel-Instanz überwachen, ist dabei unerheblich:
{CMK} stattet alle Monitoring-Instanzen mit den CAs und Zertifikaten aus, die für den Betrieb und für die Kommunikation zwischen den Komponenten eines {CMK}-Setups unerlässlich sind, etwa zwischen den xref:glossar#agent[Agenten] und der Instanz.
Vereinfacht gesagt prüft die CA die Identität des Antragstellers (das kann eine Website, aber auch eine Softwarekomponente sein) und bestätigt dessen öffentlichen Schlüssel in einem Zertifikat, das zur Kommunikation berechtigt.
...
CA-Zertifikate bilden das Fundament sicherer Internetverbindungen durch verschlüsselte Datenübertragung zwischen Browser und Website – dabei ist HTTPS als die sichere Variante des Internet-Protokolls HTTP und verwendet für die Datenübertragung TLS- oder SSL-Verschlüsselung.
...
// TK: Etwas umständlich: Vielleicht so: Der vollständige Fingerprint wird beim jeweiligen Fingerprint-Eintrag als Tooltipp angezeigt.
// SH: Ist "Tooltipp" an der Stelle nicht missverständlich? Passt das zur Darstellung des Fingerprints?
...
// TK: Das kann meiner Meinung nach so nicht stimmen: je ein CA-Zertifikat zum Signieren des Instanz-Zertifikats, der Agentenschlüssel bei der mTLS-Authentifizierung und der Message-Broker-Zertifikate.
// TK: (Herausgeberzertifikate) ist hier überflüssig.
// TK: Dieses Feature soll verteiltes Piggyback heißen, auch wenn der Begriff im Linkziel leider fehlt.
// SH: Dem muss ich morgen nochmal in Ruhe nachgehen. 

Standardmäßig sind für bei einer einzelnen Instanz fünf Zertifikate aufgelistet.
...
// TK: Bei der Tabelle hab ich noch diverse Kommentare. Mehr auf der Tonspur.
// TK: Details in der Tabelle: mysite_remote_cert.pem > Wenn die Datei so heißt wie die Remote-Instanz, dann myremotesite.pem verwenden
// SH: Das ist der Originalname der Datei aus meinem Setup, unsere Software vergibt den Zertifikatsnamen nach folgendem Prinzip: 
// SH: An den Namen der Remote-Instanz wird _cert.pem angehängt. 
// SH: Wenn ich meine Remote-Instanz "myremotesite" nenne, heißt das dazugehörige Zertifikat "myremotesite_cert.pem": So habe ich es jetzt eingetragen. 
...
| `ssl/agents/ca.pem` | _CA-Zertifikat für Signierung der Agentenschlüssel_: signiert den Agenten und sichert mTLS ab – zuständig für die beidseitige (mutual) sichere Authentifizierung von Client und Server, die das Standard-TLS-Protokoll erweitert. Der Certificate Signing Request von der Gegenseite (Agent Controller von Hosts im Monitoring) wird mit diesem Zertifikat signiert.  | CA-Zertifikat  | xref:glossar#agent[Monitoring-Agenten], xref:hosts_setup#hosts_autoremove[Hosts erstellen und bearbeiten] und xref:glossar#agent_bakery[Agenten-Bäckerei]
...
In einem xref:glossar#central_setup[verteilten Monitoring mit zentralem Setup] läuft die Weitergabe von Einstellungen über eine REST-API.
// TK: Ich finde den vorherigen Satz überflüssig.
...
// TK: Dafür: Wofür? 
// TK: Ich find die Admonition für das Thema zu heftig. Den Inhalt der Admonition könnte man problemlos im vorherigen Satz unterbringen.
// SH: Da gehen die Meinungen auseinander. 
// SH: Die Admonition ist ein Vorschlag von Mattias und schien ihm wichtig zu sein - ich lasse den Kommentar zwecks Transparenz noch kurz stehen.
