...
Nach der Überprüfung der Echtheit eines Schlüssels bestätigen sie dessen digitale Identität. 

Vereinfacht gesagt prüft die Zertifizierungsstelle (CA) die Identität des Antragstellers (das kann auch eine Softwarekomponente sein) und bestätigt dessen öffentlichen Schlüssel in einem Zertifikat, das zur Kommunikation berechtigt.
{CMK} sichert xref:glossar#site[{CMK}-Instanzen] über eigene Certificate Authorities ab:
Indem diese Softwarekomponenten die eingesetzten Zertifikate auf Gültigkeit und Übereinstimmung mit den für sie hinterlegten Werten überprüfen, authentifizieren sie die systeminterne Kommunikation -- beispielsweise zwischen einer Instanz und den ihr zugeordneten Hosts oder im verteilten Monitoring zwischen den Instanzen. 
...
{CMK} stattet beim Implementieren alle Monitoring-Instanzen mit den CAs und Zertifikaten aus, die für den Betrieb und für die Kommunikation zwischen den Komponenten eines {CMK}-Setups unerlässlich sind, etwa zwischen den Agenten und der Instanz. 

Eine Besonderheit bei {CMK} stellt das Agenten-Signieren dar: Die Kommunikation zwischen Agent Receiver (in der Instanz) und Agent Controller (auf dem überwachten Host) findet per mTLS statt, asynchron und in Ende-zu-Ende-Verschlüsselung. 
Das "m" in mTLS steht für mutual, also das gegenseitige Bestätigen einer Vertrauensbasis:  
Ein dafür zuständiges Zertifikat beantwortet Signierungsanfragen des Agent Controller von Hosts im Monitoring. 
Kompromittierte Schlüssel lassen sich bei diesem Verfahren leicht einzeln austauschen.
Bei {CMK} sind Zertifikate im PEM-Format hinterlegt -- auch für die {CMK}-seitigen Certificate Authorities, die sich mit je einem CA-Zertifikat (Herausgeberzertifikat) jeweils die eigene Vertrauenswürdigkeit attestieren.
