...
// TK: Allgemein: Bitte Kommentare löschen, bei denen nichts mehr zu tun ist (wie der folgende) oder offene Fragen und Pläne für zukünftige Erweiterungen im Wiki parken
...
// TK: Bei xref:glossar#central_setup[Zentralinstanz und Remote-Instanzen] ist der Linksourcetext zu weit weg vom Linkziel. Warum verlinkst Du nicht verteiltes Monitoring? Und warum nimmst Du das Linkziel central_setup und nicht distributed_monitoring?
...
// TK: Im folgenden Collapsible geht es nur zum geringsten Teil um Zertifikatsketten, also besser und allgemeiner: Wie funktionieren Zertifikate?
// TK: Wollten wir mit Fragen im Collapsible-Heading arbeiten? Ich glaub nicht, hab aber keine Regel zum Wording gefunden: Vielleicht besser so: So funktionieren Zertifikate
...
// TK: Kommunikation wird erst ... übertragen: Wording. Besser: findet statt, wird initiiert, ...
...
// TK: Jeder Eingriff in ein Zertifikat verändert dessen Prüfwert: Nicht zu viele Synonyme: Hier Fingerprint verwenden.
// TK: und fällt beim Abgleich mit einer vertrauenswürdigen Quelle auf: Hier vertrauenswürdige Stelle (den Begriff verwendest Du im nächsten Satz)
...
// TK: bestätigen sie die digitale Identität: bestätigen CAs ... (Verwechslungsgefahr mit der direkten Anrede, die im nächsten Satz wieder folgt)

{CMK} sichert Ihre xref:glossar#site[{CMK}-Instanzen] über eigene Certificate Authorities ab:
Indem diese Softwarekomponenten die eingesetzten Zertifikate auf Gültigkeit und Übereinstimmung mit den für sie hinterlegten Werten überprüfen, authentifizieren sie die systeminterne Kommunikation – 
// TK: diese Softwarekomponenten: Unklar, was Du meinst: Instanzen oder CAs. besser: die CAs
// TK: Software wird immer mit Bindestrich gekoppelt (Wörterbuch)
beispielsweise zwischen einer xref:glossar#site[Instanz] und den ihr zugeordneten xref:glossar#host[Hosts] oder im verteilten Monitoring zwischen den Instanzen.
// TK: Nicht zu oft ins Glossar linken. In diesem kurzen Artikel reicht 1x, evtl. nochmal in der Tabelle unten.
...
// TK: {CMK} stattet beim Implementieren: beim Implementieren: überflüssig
...
// TK: die Zertifizierungsstelle: Obacht mit Synonymen: die CA
...
// TK: Herausgeberzertifikate bilden das Fundament: CA-Zertifikate bilden das Fundament
// TK: dabei gilt HTTPS... : dabei ist HTTPS... (oder gibts da Zweifel?)
...
// TK: Einzelmonitoring: Was ist das? Meinetwegen so: Einzel-Instanz-Konfiguration oder Standardmäßig sind bei einer einzelnen Instanz ...
...
// TK: Details in der Tabelle: ssl/agent/ca.pem > ssl/agents/ca.pem
