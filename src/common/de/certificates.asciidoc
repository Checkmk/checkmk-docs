...
// SH: Livestatus und Agent Monitoring zu verlinken. MFS recherchiert, was da weiter zu referenzieren ist.
| ? | [.guihint]#ssl/agent/ca.pem#| signiert den Agenten und sichert mTLS ab – Certificate Signing Request von der Gegenseite wird signiert mit diesem Zertifikat| CA-Zertifikat  | xref:wato_monitoringagents#[Monitoring-Agenten]
// SH: Ist es ein Intermediate-Zertifikat? Laut Maximilian haben wir in der Overview aber keine. Dann müsste es ein Root-Zertifikat sein. Wir klären die korrekte Bezeichnung noch. Zur Komponente: auf die Monitoring-Agenten verweisen.
| Site-Zertifikat | [.guihint]#​sites/mysite.pem# | sichert die Netzwerkkommunikation verschiedener Komponenten | sichert Kommunikation ab | Agent Receiver und Livestatus, xref:wato_monitoringagents#[Monitoring-Agenten] (Links folgen)
// Agent Receiver und Livestatus, verweisen auf Monitoring-Agenten
...
// SH: Die Tabelle ist noch fertig zu befüllen. Vollständige Liste als Referenztabelle: alle 5, Links ergänzen.
...
// Dann Link zu der Komponente im Handbuch ergänzen. 

// SH: Abgrenzung zu omd_https, separat:
