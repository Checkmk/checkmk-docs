...
// ML: Das Wording in der Tabelle oben kommt nicht so ganz hin, "CA als lokaler Message Broker" - da ist wohl die CA FÜR den Message-Broker gemeint, damit der mit den Gegenstellen plaudern darf. Zu sloppy: Ich würde auch eher schlicht von Zertifikatskette reden und den Rest Lesern/Entwicklern überlassen ... (bei mysite.pem zum Beispiel ist "mysite_pro" aus der Spalte "subject name" der Common Name (CN))
// Oder mach es Dir einfach, lies eine der pem-Dateien aus und füge den Kram als Terminal-Session ein - würde dem Artikel was Hübsches hinzufügen ;) Etwa so:
...
Die CAs und CA-Zertifikate sind in der Übersicht als Herausgeberzertifikate in die Rubriken [.guihint]#Subject Name# (die ausgegebenen Zertifikate) und [.guihint]#Issuer# (deren ausgebende Instanz, die Ursprungs-CA) eingeteilt mit ihrer jeweiligen Bezeichnung samt Erstellungs- und Ablaufdatum.
...
| Root-Zertifikat | [.guihint]#ssl/ca.pem# | sichert die Instanz | CA-Zertifikat | Livestatus, Monitoring-Agenten, ... (Link folgt)
// SH: MFS recherchiert, was da weiter zu referenzieren ist.
| ? | [.guihint]#ssl/agent/ca.pem#| signiert den Agenten und sichert mTLS ab – Certificate Signing Request von der Gegenseite wird signiert mit diesem Zertifikat| CA-Zertifikat  | auf die Monitoring-Agenten verweisen (Link folgt)
// SH: Ist es ein Intermediate-Zertifikat? Laut Maximilian haben wir in der Overview keine. Dann müsste es ein Root-Zertifikat sein.
| Site-Zertifikat | [.guihint]#​sites/mysite.pem# | sichert die Netzwerkkommunikation verschiedener Komponenten | sichert Kommunikation ab | Agent Receiver und Livestatus, verweisen auf Monitoring-Agenten (Link folgt)
| CA-Zertifikat für RabbitMQ | [.guihint]#rabbitmq/ssl/ca_cert.pem# | sichert die Zertifikate der individuellen Message Broker, etwa für Distributed Piggyback-Kommunikation | CA-Zertifikat | Piggyback und Distributed Monitoring (Link folgt) 
| SSL-Zertifikat | [.guihint]#​rabbitmq/ssl/cert.pem# | dient der SSL-Verschlüsselung: sichert Netzwerkkommunikation des Message Brokers ab |  sichert Kommunikation ab | Piggyback und Distributed Monitoring (Link folgt) 
