...
ifdef::onprem[]
{cee-only}
Während {RE} SNMP-Anfragen auf klassische Weise über die Befehle `snmpget` bzw. `snmpbulkwalk` macht, haben die kommerziellen Editionen eine eingebaute SNMP-Engine, die SNMP-Anfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen.
...
die Befehle `snmpget` bzw. `snmpbulkwalk` und eine eingebaute SNMP-Engine, die SNMP-Anfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen.
