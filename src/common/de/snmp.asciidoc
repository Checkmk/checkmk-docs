...
. Vergeben Sie eine _Community,_ wenn die Protokollversionen v1 und v2c verwendet werden.
...
SNMP v2c erfordert die Eingabe der oben besprochenen Community.
...
// TK: Benamsung: Inline-SNMP in de und inline SNMP in en. In dem Artikel hab ich es bereits korrigiert.
// TK: Ich hab festgestellt, dass es in cse "Inline SMTP" genannt wurde. Ich hab das jetzt in der 2.2.0 korrigiert.
// TK: Bitte die Option auch in the Tabelle in http://localhost:8088/latest/de/cmk_commandline.html#options aufnehmen bei den anderen SNMP-Optionen.
In einigen Fällen genügt es, die in den kommerziellen Editionen vorhandene xref:performance[Inline-SNMP] Implementierung für betroffene Geräte abzuschalten und damit das etwas robustere und deutlich langsamere  `snmpget` zu aktivieren.
// TK: In einigen Fällen genügt es: Hier fehlt, was schiefgehen kann (wie bei den anderen noch folgenden Abschnitten), damit man weiß, warum man umstellen soll.
Hierfür steht für Tests auf der Kommandozeile dem Befehl `cmk` die Option `--snmp-backend` zur Verfügung, welche als Parameter `classic`, `inline` oder `stored-walk` (Verwendung eines hinterlegten SNMP-Walks) akzeptiert.
...
// TK: Was macht das Kommando denn? Und wie erkenne ich, dass der Test erfolgreich war?
...
{cee-only}
Während die {RE} SNMP-Anfragen auf klassische Weise über die Kommandozeilenbefehle `snmpget` bzw. `snmpbulkwalk` macht, haben die kommerziellen Editionen eine eingebaute SNMP-Engine, die SNMP-Anfragen sehr performant durchführt ohne weitere Prozesse zu erzeugen.
...
// MFS: Hochgezogen zu #cursedhardware
