// -*- coding: utf-8 -*-
// IGNORE % Kellerstraße GETNEXT Querys herauszukitzeln
// NONASCII
include::global_attr.adoc[]
= Überwachen via SNMP
:title: Überwachen via SNMP - Monitoring von SNMP-Geräten mit {CMK}
:description: {CMK} unterstützt Sie flexibel bei der Überwachung zahlreicher Geräte mit SNMP. Hier erfahren Sie, wie Sie die SNMP-Überwachung einrichten und nutzen.
:experimental:
...
SNMP (_Simple Network Management Protocol_) ist ein Netzwerkprotokoll, über das Sie bestimmte Typen von Objekten mit {CMK} überwachen können, auf denen die Installation eines {CMK}-Agenten nicht möglich ist.
Geräte wie Router, Switches, Firewalls, Drucker, Appliances, USVs, Hardwaresensoren und viele andere haben bereits vom Hersteller aus eine eingebaute Schnittstelle für das Monitoring: einen _SNMP-Agenten_.
{CMK} kann SNMP nutzen, um solche Geräte zu überwachen.

Dieser Artikel beschreibt, wie Sie SNMP-fähige Hosts mit wenig Aufwand ins Monitoring aufnehmen und konfigurieren können.
...
ifdef::onprem[]
// MFS: Theoretisch funktioniert das auch mit SaaS, dürfte aber sehr frickelig einzurichten sein
In einigen Fällen kann allerdings die Überwachung per SNMP _zusätzlich_ zum {CMK}-Agenten sinnvoll sein.
Im Abschnitt zur xref:add_device[Aufnahme von Geräten ins Monitoring] finden Sie mehr zu diesem Thema.

////
Sollte allerdings für eine Hardware- oder Software-Komponente (beispielsweise einen RAID-Controller) kein {CMK}-Agentenplugin zur Überwachung -- dafür jedoch eine SNMP-Schnittstelle -- existieren, können Sie selbstverständlich über SNMP zusätzliche Monitoring-Daten erheben.
...
[TIP]
====
{CE} benötigt das xref:relay#[{CMK} Relay] zur Überwachung von SNMP-Geräten.
Installieren Sie dieses im selben Netzwerk wie das zu überwachende SNMP-Gerät.
====
endif::[]
...
[TIP]
====
{UE} erlaubt mit dem xref:relay#[{CMK} Relay] eine zweite Art der Überwachung von SNMP-Geräten.
Relays, die sich im selben Netzwerk wie die zu überwachenden SNMP-Geräte befinden, können auch Daten von solchen SNMP-Geräten einsammeln, die der Monitoring-Server nicht direkt erreichen kann. 
====
endif::[]


[#snmp_communication]
=== Zwei mögliche Kommunikationsrichtungen

ifdef::onprem[]
{CMK} nutzt für die SNMP-Überwachung eine Pull-Methode.
endif::[]
ifdef::saas[]
{CE} nutzt für die SNMP-Überwachung eine Pull-Methode.
endif::[]
Die SNMP-Terminologie verwendet dafür zwei Begriffe, die leider häufig vertauscht werden: _Query_ (oft als _Abfrage_ übersetzt) und _Request_ (oft als _Anfrage_ übersetzt).
Der Unterschied liegt in der Ebene des Protokolls:

* _Query_ bezeichnet die Nutzersicht, also _welche_ Daten eingeholt werden sollen: „Hey, SNMP-Client, besorge mir mal alle vorhandenen Daten zu den Ports des Switches in der Besenkammer der Kellerstraße 27!“
Der Begriff _Query_ ist nicht in der SNMP-Spezifikation enthalten, aber in der Dokumentation vieler Tools. 
* _Request_ bezieht sich immer auf die konkrete Art, _wie_ ein Gerät Daten liefert: „Hey, SNMP-Gerät, liefere mir vorhandene Daten zur OID `.1.3.6.1.2.1.1.1.0` und sag mir dann die nächste OID, unter der du Daten hast!“
Eine solche Aufgabe wird beispielsweise technisch umgesetzt als _SNMP GETNEXT-REQUEST_, den der _SNMP Manager_ zum _SNMP Agent_ des Gerätes schickt.

Was aus Nutzersicht als eine einzige _Query_ betrachtet wird, kann je nachdem, was das Gerät unterstützt und wie die verwendete SNMP-Bibliothek intern arbeitet, zu einem, mehreren oder tausenden notwendigen _Requests_ führen.

Für solch einen Request sendet 
ifdef::onprem[]
{CMK} beziehungsweise
endif::[]
das Relay ein UDP-Paket (Port 161) mit einer SNMP-Anfrage an das Gerät mit der Bitte, bestimmte Daten zu liefern.
// ES: Stimmt das so mit den ifdefs?
...
// ES: Hier den Begriff aktive Anfrage entfernen/durch eine inhaltliche Beschreibung ersetzen
ifdef::saas[]
Das {CMK} Relay unterstützt aus diesen Gründen keinen Empfang von Traps.
In den On-Premises-Editionen von {CMK} können Traps mit der Event Console empfangen werden.
endif::[]
ifdef::onprem[]

.Einige der Nachteile von SNMP-Traps
[%collapsible]
====
...
Dadurch ist das Monitoring genau dann überlastet, wenn man es am meisten brauchen würde.
...
Das {CMK} Relay unterstützt keinen Empfang von Traps.
====
endif::[]
...
Die Bulk-Requests beschleunigen das Monitoring um bis zu Faktor 10.
...
*Achtung:* Da die „echte“ Version 2 keine Relevanz hat, finden sich an vielen Stellen in {CMK} Erwähnungen von v2, die dann aber immer v2c meinen.
|*v3* |Security, +
...
// ES: Siehe Werk #16382, bei frischen Instanzen gibt es immer eine eingerichtete Regel dieses Regelsatzes, die für alle Hosts v2c anwendet außer für diejenigen, die explizit anders gekennzeichnet sind; diese Regel kannst du ändern, löschen oder deaktivieren, um die Nutzung von v1 zu erzwingen/zum Standard zu machen
====

[#enable_snmp]
== Geräte für das Monitoring vorbereiten

Der erste Schritt ist das Vorbereiten des Gerätes.
Jedes Gerät, das SNMP unterstützt, hat in seiner Setup-Oberfläche dafür irgendwo eine Konfigurationsmöglichkeit.
Nehmen Sie dort folgende Einstellungen vor:

. Gehen Sie zur Konfiguration für _aktive_ Abfragen (SNMP GET).
// ES: Oben war noch die Rede von aktiven *Anfragen*. Können wir hier umformulieren? Welche Begriffe würde man in so einer "Maske" in der Realität erwarten?
Leider verwendet jeder Hersteller seine eigene Terminologie.
Achten Sie auf jene Einstellmöglichkeiten, die _keine Traps_ konfigurieren und irgendwo _SNMP community_ erwähnen.
. Aktivieren Sie SNMP für _lesende_ (_read-only_ oder _readable_) Anfragen.

. Falls Ihr SNMP-Gerät Zugriffe auf IP-Adressen beschränkt:
ifdef::onprem[]
...
In einer xref:distributed_monitoring#[verteilten Umgebung] oder bei Verwendung des xref:relay#[{CMK} Relays] ist die IP-Adresse der Maschine entscheidend, von der aus das Gerät überwacht wird.
endif::[]
ifdef::saas[]
Tragen Sie als erlaubte IP-Adresse die Adresse des verwendeten {CMK} Relays ein.
endif::[]
. Vergeben Sie eine Community, wenn die Protokollversionen v1 und v2c verwendet werden.

Die _SNMP community_ ist eine Art Passwort, nur mit dem Unterschied, dass es bei SNMP keinen Benutzernamen gibt.
...
oder zumindest überall in einer Region, einer Abteilung, einem Rechenzentrum und so weiter.

// MFS: Das ist allgemeinene Netzwerk Best Practise, würde ich hier eigentlich komplett entfernen wollen. 
// ES: Stimme zu, weg damit.
////
...
ifdef::saas[]
Im Kasten [.guihint]#Basic settings# müssen Sie zunächst das {CMK} Relay auswählen, mit dem der SNMP-Host überwacht werden soll.
endif::[]
ifdef::onprem[]
In {UE} können Sie im Kasten [.guihint]#Basic settings# das {CMK} Relay auswählen, mit dem der SNMP-Host überwacht werden soll, wenn er nicht direkt vom {CMK}-Server aus erreichbar ist.
endif::[]
...
Die Auswahl von Protokollversion v1 ist nur für _sehr_ alte Geräte eine Notlösung.
...
// MFS: Das folgende geht mit dem Relay nur, wenn die SNMP-Services einem anderen Host im Monitoring zugeordnet sind, ist also eher unpraktisch und daher für SaaS deaktiviert.
ifdef::onprem[]
[%collapsible]
.[#snmp_cmk_agent]#Überwachung per SNMP und {CMK}-Agent?#
====
// ES: Ich finde das hier zu redundant zu dem Abschnitt unter 1.1. Wenn du möchtest, schau, ob hier irgendwas steht, was oben fehlt, und übertrage das dorthin. Mein Vorschlag wäre dann, dass du hier alles bis einschließlich "braucht deutlich mehr Ressourcen" entfernst. Dann im einleitenden Satz der Box nur noch auf 1.1 querverweisen und in der restlichen Box ausschließlich das Szenario, dass beides genutzt wird, erläutern.
...
Es gibt allerdings ein paar wenige Situationen, in denen eine Überwachung per SNMP _zusätzlich_ zum {CMK}-Agenten sinnvoll sein kann.
...
// ES: Hier ist glaube ich eine andere Bedeutung von Abfrage gemeint. Wie wäre es mit "Kommunikation"?

Falls Sie in so einem Fall den Linux- oder Windows-Host per {CMK}-Agent _und_ SNMP überwachen wollen, gehen Sie wie folgt vor:
...
Im gleichen Kasten aktivieren Sie die Option [.guihint]#SNMP# und setzen den Wert auf [.guihint]#SNMPv2 or v3# bzw. [.guihint]#SNMPv1#, wie oben beschrieben:
...
SNMPv3 bietet xref:security[Verschlüsselung und eine echte Authentifizierung].
Daneben führt es das xref:contexts[Konzept der _Kontexte_] ein.
Dabei kann ein Gerät an derselben Stelle im SNMP-Baum unterschiedliche Informationen zeigen -- je nachdem, welche _Kontext-ID_ bei den Requests mitgegeben wird.
...
// ES: Das ist hier die erste Erwähnung von "Passwort", bitte explizit erwähnen, dass die vorher relevante Community durch ein Passwort ersetzt wird
...
Falls Sie ein Gerät haben, das mit Kontexten arbeitet, benötigen Sie in {CMK} zwei Einstellungen:

* Zunächst muss das Gerät mit SNMPv3 abgefragt werden.
* Dann benötigen Sie noch eine Regel im Regelsatz [.guihint]#SNMPv3 contexts to use in requests#.
Hier wählen Sie das Check-Plugin aus, für das Kontexte aktiviert werden sollen, und dann die Liste der Kontexte, für die Monitoring-Daten eingeholt werden sollen.
...
Dazu speichern Sie mit dem Aktionsknopf [.guihint]#Save & run connection tests#.
...
{CMK} kann bei letzteren einfach in die Ausgabe des Agenten schauen und darin -- mithilfe der einzelnen Check-Plugins -- die interessanten
Punkte finden.
...
// ES: Die folgenden drei Absätze würde ich gerne löschen und durch eine Ein-Satz-Zusammenfassung ersetzen. Die Details, was hinter der Kulisse passiert, sind für die meisten Lesenden irrelevant, oder?
...
Danach folgen je nach Bedarf daraus resultierende weitere Requests.
Anhand der Ergebnisse entscheidet dann jedes der um die 1000 mitgelieferten SNMP-Check-Plugins, ob das Gerät dieses Plugin überhaupt unterstützt.
...
Die gefundenen Plugins rufen per örtlich begrenzten SNMP-Requests gezielt genau die Daten ab, die sie benötigen, und ermitteln daraus die zu überwachenden Services.
Die abgerufenen Daten sind genau die gleichen, die später auch regelmäßig für die Überwachung geholt werden.

// MFS: Dieser Absatz ist faktisch richtig, widerspricht aber etwas den typischen Einsatzszenarien des Relays.
// Ich würde ihn dennoch zunächst unverändert drinlassen, bis wir mehr darüber wissen, wie unsere Kunden das Relay für SNMP nutzen.
...
Der Knopf [.guihint]#Rescan# erzwingt einen SNMP-Scan und anschließendes Holen von frischen Daten via SNMP.
...
Der erste Service im Bild ist ein Check, der die Netzwerkports überwacht.
Mindestens einen muss das Gerät haben (und der muss auch aktiv sein) -- sonst würde SNMP nicht funktionieren.
...
Sie können das mit dem Regelsatz [.guihint]#Setup > Services > Discovery rules > Network interface and switch port discovery# beeinflussen.
...
Der zweite Service heißt [.guihint]#SNMP Info# und zeigt die wichtigsten vier Informationen über das Gerät an, nämlich die Werte der Felder `sysDescr`, `sysName`, `sysLocation` und `sysContact`.
...
// MFS: FIXME: Link zum core_tuning Artikel setzen, wenn der fertig ist.
Es ist nicht ungewöhnlich, dass ein größeres SNMP-Gerät die 20- oder 50-fache Fetcher-Auslastung eines mittels {CMK}-Agenten überwachten Hosts verursacht.
Aus diesem Grund ist es wichtig, dass Sie sich mit der Performance überwachter Geräte vertraut machen, problematische Geräte identifizieren und proaktiv Optimierungen durchführen.
...
ifdef::onprem[]
{cee-only}
Während {RE} SNMP-Abfragen auf klassische Weise über die Befehle `snmpget` bzw. `snmpbulkwalk` umsetzt, haben die kommerziellen Editionen eine eingebaute SNMP-Engine, die SNMP-Abfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen.
...
Auch das in {UE} verwendbare {CMK} Relay unterstützt beide Abfragemöglichkeiten.

Falls Ihre {CMK}-Installation mit einer sehr alten kommerziellen Version aufgesetzt wurde oder von {RE} zu einer kommerziellen Version migriert wurde, stellen Sie Ihre SNMP-Geräte auf Inline-SNMP um.
// ES: Bitte checken, ob meine Umformulierung hier den ursprünglichen Sinn des Satzes korrekt wiedergibt.
// ES: Das ist hier die erste Erwähnung von Inline-SNMP abgesehen von der Überschrift. Sollte das besser ein Guihint sein, weil die Option dazu in CMK so heißt? Auf jeden Fall sollte die Bezeichnung Inline-SNMP dort geliefert werden, wo das Konzept erwähnt wird (aus meiner Sicht am besten direkt bei "eingebaute SNMP-Engine")
Lediglich bei xref:defective[defekten Implementierungen] sollten Sie das robustere `snmpget` bzw. `snmpbulkwalk` verwenden.
endif::[]
ifdef::saas[]
Das {CMK} Relay unterstützt zwei Abfragemöglichkeiten für SNMP:
die Befehle `snmpget` bzw. `snmpbulkwalk` und eine eingebaute SNMP-Engine, die SNMP-Abfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen.
Die CPU-Last für die SNMP-Verarbeitung ist mit dieser etwa halb so groß, weshalb sie als Standard verwendet wird.
...
Falls Sie mit Ihren Ressourcen an die Grenzen stoßen bzw. die Abfrage eines einzelnen Gerätes länger als 60 Sekunden dauert, können Sie das Intervall erhöhen, mit dem {CMK} den oder die Hosts kontaktiert.
Mit dem Regelsatz [.guihint]#Normal check interval for service checks#, den Sie gezielt auf die {CMK}-Services von Hosts anwenden, können Sie das generelle Intervall von einer Minute auf beispielsweise zwei, fünf oder zehn Minuten verlängern.
...
Insgesamt empfehlen wir aber, das Monitoring so auszulegen, dass das Standardintervall von einer Minute beibehalten werden kann und das Abfrageintervall nur in Ausnahmefällen für einzelne Hosts oder Checks erhöht wird.


[#timing_settings]
=== Timing settings for SNMP access

Standardmäßig erwartet {CMK} auf einen SNMP-Request eine Antwort innerhalb von einer Sekunde.
Außerdem verschickt die Monitoring-Software jeden Request bis zu dreimal, bevor sie aufgibt.
// ES: Standardmäßig steht Number of retries auf 5, d.h. insgesamt bis zu 6 Versuche (siehe Voreinstellung in der Regel "Timing settings for SNMP access"). Das Inline-Backend verwendet standardmäßig den Wert 3 (d.h. bis zu 4 Versuche), falls keine Regel aktiv ist.
// ES: Alles ein bisschen verwirrend. Meine Empfehlung wäre, sich hier mit genauen Zahlen bedeckt zu halten, da das genaue Verhalten ja auch unter anderem vom verwendeten Backend abhängt.
...
Beachten Sie, dass sich diese Einstellungen auf einen _einzelnen SNMP-Request_ beziehen.
Der komplette Überwachungsvorgang eines Hosts besteht aus vielen einzelnen Requests.
...
SNMP überträgt pro `GetBulk`-Request in der Voreinstellung zehn Antworten in einem Paket.
Mit dem Regelsatz [.guihint]#Bulk walk: number of OIDs per bulk# können Sie ausprobieren, ob ein höherer Wert (beispielsweise 20 oder 50) eine bessere Performance bringt.
...
In der Praxis kann es auch vereinzelt vorkommen, dass Geräte mit dem voreingestellten Wert von zehn OIDs per Bulk Probleme haben.
Dann kann es sinnvoll sein, die Anzahl beispielsweise auf fünf zu senken.

.Nicht immer gilt: viel hilft viel!
[%collapsible]
====
Dass die Performance von Bulk-Requests bei sehr hohen Zahlen von übertragenen OIDs einbricht, liegt auch daran, dass SNMP Pakete mit Dummy-Daten auffüllt.
Wurden beispielsweise 20 OIDs angefragt, das Gerät hat aber nur 17 zu liefern, werden drei aufgefüllt.
Bei 50 OID würden 33 aufgefüllt werden.
Ein weiterer Aspekt ist der Arbeitsspeicherbedarf beim angefragten Gerät.
Schwache Geräte wie einfache Netzwerkdrucker werden eher mit kleineren Bulk-Größen gut klar kommen.
====
...
// ES: Diese Regel gibt es in {RE} nicht.
{cee-only}
...
Das ist auch gut so, denn im Normalfall ist das schneller -- punktuelle Informationen können nicht mit den effizienten Bulk-Requests eingeholt werden.
Zudem ist es aus unserer Sicht sowieso empfehlenswert, grundsätzlich alle Ports zu überwachen, um defekte Ports oder Kabel mit hohen Fehlerraten zu finden.
...
Für solche Fälle gibt es in den kommerziellen Editionen den Regelsatz [.guihint]#Bulk walk: limit SNMP OID Ranges#.
Mit diesem können Sie die Liste der abgefragten Daten (z.B. Ports) statisch begrenzen.
Im Wert der Regel legen Sie jeweils für ein bestimmtes Check-Plugin fest, welche Indizes der jeweiligen Tabelle eingeholt werden sollen.

Der übliche Check-Typ für Switch Ports heißt [.guihint]#SNMP interface check with 64-bit counters (using v2c)#.
// ES: Diesen Wert kann ich auswählen in einem Dropdown mit dem Titel "Section name". Die Formulierung "Check-Typ" könnte hier verwirrend sein. Genauer formulieren?
Folgendes Beispiel zeigt eine Einstellung, bei der nur Daten für die ersten beiden Ports per SNMP eingeholt werden:

image::snmp_limit_oid_ranges.png[alt="Bulk walk: limit SNMP OID ranges."]

[TIP]
====
Diese Filterung findet  _vor_ der Service-Erkennung und dem Monitoring statt.
...
Die ersten RFCs zu SNMP datieren auf 1988.
Im breiten Einsatz ist SNMP seit Mitte/Ende der 1990er.
Gerade in den Anfangsjahren haben viele Hersteller das Protokoll nur teilweise oder falsch implementiert, und bis heute ist die Einhaltung von Konventionen nicht bei jedem Gerät garantiert.
{CMK} hält eine Reihe von Maßnahmen bereit, um dennoch Monitoring-Daten aus solchen Geräten herauszukitzeln.

[#defective]
=== Defekte SNMP-Implementierungen

Es gibt Geräte, bei denen SNMP zwar einigermaßen funktioniert, aber bestimmte Teile des Protokolls nicht oder falsch umgesetzt wurden.
Ein besonders häufig gesehenes Problem sind Implementierungen, die mit bestimmten Abfrage-Tools getestet wurden, nicht aber auf Konformität zur Spezifikation.

ifdef::onprem[]
Treten die Probleme mit den kommerziellen Editionen von {CMK} auf, kann ein Grund darin liegen, 
endif::[]
ifdef::saas[]
Ein Grund bei Problemen kann sein,
endif::[]
dass die dort standardmäßig aktivierte, performantere xref:performance[Inline-SNMP-Implementierung] sich stärker auf die Einhaltung von Standards verlässt als `snmpget`.
...
ifdef::onprem[]
Hierfür steht für Tests auf der Befehlszeile dem Befehl `cmk` für einige Optionen die Zusatzoption `--snmp-backend` zur Verfügung, welche als Parameter `inline` (Verwendung von Inline-SNMP), `classic` (Verwendung von `snmpget`) oder `stored-walk` (Verwendung eines hinterlegten xref:simulation[SNMP-Walks]) akzeptiert.
War der Test auf der Befehlszeile erfolgreich, können Sie mit dem Regelsatz [.guihint]#Hosts not using Inline-SNMP# die Hosts bestimmen, welche dauerhaft kein Inline-SNMP nutzen sollen.
// ES: Deprecated, Ersatz ist #Hosts using a specific SNMP backend#, siehe Werk #12381. In dieser Regel gibt es ein Dropdown mit den Auswahlmöglichkeiten "Use Inline SNMP backend"/"Use Classic backend"
endif::[]


==== Keine Antwort auf Anfrage nach `sysDescr`

Die wichtigste Identifikationsmöglichkeit von SNMP-Geräten ist der String im Feld `sysDescr`.
Antwortet ein Gerät nicht auf die Abfrage dieser OID, ist ein solches Gerät wie tot.

Auch in der Service-Erkennung wird es keine Resultate liefern, wenn Sie nicht durch eine spezielle Konfiguration nachhelfen:
Legen Sie für die betroffenen Hosts eine Regel an unter [.guihint]#Setup > Agents > SNMP rules > Hosts without system description OID# mit der Option [.guihint]#Positive match (Add matching hosts to the set)#.
...
==== SNMPv2c geht, aber Bulk-Requests scheitern

Einige Geräte unterstützen zwar Version v2c (und werden in der Diagnose darauf auch eine Antwort liefern), allerdings fehlt im Protokoll die Umsetzung des Request-Befehls `GetBulk`.
...
Bei einem solchen Host werden auch einige einfache SNMP-Checks funktionieren, wie z.B [.guihint]#SNMP Info# oder [.guihint]#Uptime#.
Aber andere Services fehlen -- insbesondere die Netzwerkschnittstellen, die eigentlich bei jedem Gerät vorhanden sein müssen.

Falls Sie tatsächlich einen Host haben, bei dem das so ist, können Sie diesen mit v2c, aber ohne Bulk-Requests betreiben.
Nutzen Sie für die betroffenen Hosts die Regel [.guihint]#Setup > Agents > SNMP rules > Disable bulk walks#.
...
Falls Sie von den Ports nur sehr wenige überwachen möchten (z.B. nur die ersten beiden), können Sie {CMK} manuell auf die Abfrage von Daten einzelner Ports begrenzen.
Details finden Sie im Abschnitt zu xref:snmp#performance[Performance].


[#standard_services]
=== Es werden nur die Standard-Services gefunden

Wenn Sie ein SNMP-Gerät in die Überwachung aufnehmen und {CMK} erkennt lediglich die Services [.guihint]#SNMP Info#, [.guihint]#Uptime# und die Interfaces, so kann das verschiedene Ursachen haben:


==== Es gibt keine Plugins

{CMK} liefert etwa 1000 Check-Plugins für SNMP-Geräte aus, aber natürlich ist selbst diese Liste nie vollständig.
...
ifdef::saas[]
In diesem Fall bleibt als einzige Möglichkeit, unter link:https://ideas.checkmk.com/[ideas.checkmk.com^] die Entwicklung eines entsprechenden Plugins anzuregen. 
endif::[]

==== Die Erkennung der Plugins funktioniert nicht

Manchmal kommt es vor, dass eine neuere Firmware von einem Gerät dazu führt, dass {CMK}-Plugins das Gerät nicht mehr erkennen -- zum Beispiel weil sich in der Systembeschreibung des Geräts ein Text geändert hat.
In diesem Fall müssen die bestehenden Plugins angepasst werden.
Erstellen Sie einen xref:simulation[SNMP-Walk] und kontaktieren Sie unseren Support.


==== Das Gerät liefert die benötigten Daten nicht aus

Manche Geräte haben in ihrer SNMP-Konfiguration die Möglichkeit, den Zugriff auf bestimmte Informationsbereiche einzeln zu konfigurieren.
...
Falls keine einzige SNMP-Protokollversion funktioniert, gibt es mehrere mögliche Ursachen:
...
In bestimmten Situationen kann es nützlich sein, einen kompletten Dump aller SNMP-Daten eines überwachten Geräts in eine Datei schreiben zu lassen (einen _SNMP-Walk_).
...
Wir verwenden diese Funktionalität intensiv in Support und Entwicklung, um bestehende Plugins zu verbessern und neue Check-Plugins zu entwickeln.
So benötigen unsere Entwickler keinen Zugriff auf Ihre Geräte, sondern lediglich SNMP-Walks.
...
Um die GUI nicht ewig mit einem solchen Job zu blockieren, ist ein Timeout von 15 Minuten voreingestellt.
Erhöhen können Sie dieses bei Bedarf über [.guihint]#Setup > General > Global settings > Execution of checks > SNMP walk download timeout#.

Sobald das Erstellen abgeschlossen ist, können Sie die Datei in der Zeile [.guihint]#Result# herunterladen.


[#snmpwalk_cli]
=== Erstellen eines Walks auf der Befehlszeile

Alternativ können Sie Walks auch auf der Befehlszeile erzeugen.
Melden Sie sich dazu auf der Instanz an, von der aus das Gerät überwacht wird.
// ES: Braucht es hier Details über Relay-Szenarien?
...
Legen Sie jetzt eine Regel im Regelsatz [.guihint]#Simulating SNMP by using a stored SNMP walk# an, die für den betroffenen Host greift.
...
Diese können Sie einfach auf den {CMK}-Server umbiegen, indem Sie dem Host die Einstellung [.guihint]#IP address family: No IP# geben.
endif::[]
