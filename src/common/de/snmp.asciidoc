// -*- coding: utf-8 -*-
// IGNORE % Kellerstraße GETNEXT Querys
// NONASCII
...
Im Kasten zu xref:snmp_cmk_agent[SNMP und {CMK}-Agent] finden Sie mehr zu diesem Thema.
...
// ES: An dieser Stelle traf mich das Collapsible vollkommen unvorbereitet. Ich wurde doch bis hierher noch gar nicht mit der konkreten Funktionsweise und den erklärten Begriffen konfrontiert.
// ES: Habe deshalb den Einstieg leicht umstrukturiert und das Collapsible nach unten verschoben und umbetitelt.
...
// .Was ist der Unterschied zwischen Query und Request?
// ES: Egal wo ich das Collapsible hinschiebe, der ursprüngliche Titel beantwortet immer eine Frage, die in dieser Form vom umgebenden Text eigentlich nicht aufgeworfen wird. Daher mein Vorschlag für einen alternativen Titel:

////
.Abfragen (_Querys_) und Anfragen (_Requests_) in SNMP
[%collapsible]
====

====
////
// ES: Ich habe versucht, innerhalb des Collapsible die sprachliche Komplexität zu reduzieren, ohne die Aussage zu sehr zu verändern.

ifdef::onprem[]
{CMK} nutzt für die SNMP-Überwachung eine Pull-Methode.
// ES: Den Begriff "aktive Anfrage" oder "active request" finde ich nirgends außer hier, auch nicht in {CMK}. Die Kursivschreibung hier deutet an, dass das ein etablierter Begriff ist. Wenn du mir zustimmst, dass das nicht so ist, sollten wir einfach umformulieren /vereinfachen.
...
// Dabei sendet das Relay ein UDP-Paket (Port 161) mit einer SNMP-Anfrage an das Gerät mit der Bitte, bestimmte Daten zu liefern.
endif::[]
...
ein UDP-Paket (Port 161) mit einer SNMP-Anfrage an das Gerät mit der Bitte, bestimmte Daten zu liefern.
Das Gerät antwortet dann seinerseits mit einem UDP-Paket, das die Daten (oder eine Fehlermeldung) enthält.
...
// ES: Auch hier den Begriff aktive Anfrage entfernen/durch eine inhaltliche Beschreibung ersetzen
ifdef::saas[]
...
Falls Ihre {CMK}-Installation mit einer sehr alten kommerziellen Version aufgesetzt wurde oder Sie von {RE} migriert haben, stellen Sie Ihre SNMP-Geräte auf Inline-SNMP um.
...
Mit dem Regelsatz [.guihint]#Normal check interval for service checks#, den Sie gezielt auf die {CMK}-Services von Hosts anwenden, können Sie das generelle Intervall von einer Minute auf beispielsweis zwei, fünf oder zehn Minuten verlängern.
...
In der Praxis auch vereinzelt vorkommen, dass Geräte mit dem voreingestellten Wert von zehn OIDs per Bulk Probleme haben.
...
Das ist auch gut so, denn im Normalfall ist das schneller, denn punktuelle Informationen können nicht mit den effizienten Bulk-Requests eingeholt werden.
...
Gerade in den Anfangsjahren haben viele Hersteller das Protokoll nur teilweise oder falsch implementiert oder ignorieren Konventionen.
{CMK} hält eine Reihe von Maßnahmen bereit, um dennoch Monitoringdaten aus solchen Geräten herauszukitzeln.
...
Eine besonders häufig gesehenes Problem sind Implementierungen, die mit bestimmten Abfrage-Tools getestet wurden, nicht aber auf Konformität zur Spezifikation.

ifdef::onprem[]
Treten die Probleme mit den kommerziellen Editionen zu Tage, kann ein Grund darin liegen, 
endif::[]
ifdef::saas[]
...
Die wichtigste Identifikationsmöglichkeit von SNMP-Geräten ist der String `sysDescr`.
...
Weisen Sie den betroffenen Hosts die Regel [.guihint]#Setup > Agents > SNMP rules > Disable bulk walks# zu.
...
Diese können Sie einfach auf den {CMK}-Server umbiegen, indem Sie dem Hosts die Einstellung [.guihint]#IP address family: No IP# geben.
endif::[]
