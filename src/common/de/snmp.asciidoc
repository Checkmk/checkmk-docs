...
* _Request_ bezieht sich immer auf die konkrete Art, wie ein Gerät Daten liefert. Was man umgangssprachlich formulieren würde als: „Hey, SNMP-Gerät, liefere mir vorhandene Daten zur OID `.1.3.6.1.2.1.1.1.0` und sag mir dann die nächste OID, unter der Du Daten hast!“ wäre technisch ein _SNMP GETNEXT-REQUEST_ den der _SNMP Manager_ zum _SNMP Agent_ des Gerätes schickt.
