...
[%collapsible]
====
Die SNMP-Terminologie verwendet zwei Schlüsselworte, die leider häufig vertauscht werden -- besonders gerne in Übersetzungen: _Query_ (oft mit _Abfrage_ übersetzt) und _Request_ (meist als _Anfrage_ übersetzt).
Der Unterschied liegt in der Ebene des Protokolls:

* _Query_ bezeichnet die Nutzersicht, also _welche_ Daten eingeholt werden sollen: „Hey, SNMP-Client, besorge mir mal alle vorhandenen Daten zu den Ports des Switches in der Besenkammer der Kellerstraße 27!“. Der Begriff _Query_ ist nicht in der SNMP-Spezifikation enthalten, aber in der Dokumentation vieler Tools. 
* _Request_ bezieht sich immer auf die konkrete Art, _wie_ ein Gerät Daten liefert. Was man umgangssprachlich formulieren würde als: „Hey, SNMP-Gerät, liefere mir vorhandene Daten zur OID `.1.3.6.1.2.1.1.1.0` und sag mir dann die nächste OID, unter der Du Daten hast!“ wäre technisch ein _SNMP GETNEXT-REQUEST_ den der _SNMP Manager_ zum _SNMP Agent_ des Gerätes schickt.

Was aus Nutzersicht als ein einziger _Query_ betrachtet wird, kann je nachdem, was das Gerät unterstützt und wie die verwendete SNMP-Bibliothek intern arbeitet, zu einem, mehreren oder tausenden notwendigen _Requests_ führen.
====
...
ifdef::onprem[]
{cee-only}
Während {RE} SNMP-Abfragen auf klassische Weise über die Befehle `snmpget` bzw. `snmpbulkwalk` macht, haben die kommerziellen Editionen eine eingebaute SNMP-Engine, die SNMP-Abfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen.
