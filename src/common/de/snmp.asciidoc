...
Traps haben viele Nachteile gegenüber Abfragen im Pull-Verfahren, weswegen sie für das Monitoring keine große Bedeutung haben.
ifdef::saas[]
...
In der Praxis wird in der überwiegenden Zahl der Fälle v2c und v3 eingesetzt, v1 spielt keine Rolle mehr.
...
|*v1* | |ja |Findet man nur bei sehr alten Geräten (etwa 20 Jahre und älter), die v2c nicht unterstützen oder deren v2c-Unterstützung Fehler hat.
|*v2c* |Bulk-Requests, +
...
*Achtung:* Da die „echte“ Version 2 keine Relevanz hat, finden sich an manchen Stellen in {CMK} Erwähnungen von v2, die dann aber immer v2c meinen.
|*v3* |Security, +
...
SNMPv2c wird auf diejenigen Hosts angewandt, für die der Regelsatz [.guihint]#Enable SNMPv2c for hosts# aktiviert wurde.
Standardmäßig ist solch ein Regelsatz aktiviert, der für Hosts greift, die nicht explizit für SNMPv1 konfiguriert sind. 
Mit einer früher greifenden Regel für andere Hosts können Sie Ausnahmen definieren.
====
...
[TIP]
====
Die Verwendung mehrerer Kontexte auf den selben OID-Ästen hat zur Folge, dass nur Daten des ersten Kontextes eingeholt -- und dafür Dienste erzeugt -- werden.
Werden die Daten derart überschneidender Kontexte vollständig im Monitoring benötigt, ist es erforderlich einen Host pro Kontext anzulegen.
====
...
Es ist nicht ungewöhnlich, dass ein größeres SNMP-Gerät
ifdef::onprem[]
die 20- oder 50-fache Fetcher-Auslastung eines mittels {CMK}-Agenten überwachten Hosts verursacht.
endif::[]
ifdef::saas[]
eine relative hohe Fetcher-Auslastung beim Relay erzeugt.
endif::[]
Aus diesem Grund ist es wichtig, dass Sie sich mit der Performance überwachter Geräte vertraut machen,
ifdef::onprem[]
problematische Geräte identifizieren und proaktiv xref:core_tuning#[Optimierungen] durchführen.
endif::[]
ifdef::saas[] 
problematische Geräte identifizieren und proaktiv Optimierungen -- an SNMP-Abfragen und Relay-Einstellungen -- durchführen.
endif::[]
...
ifdef::onprem[]
{cee-only}
Während {RE} SNMP-Abfragen auf klassische Weise über die Befehle `snmpget` bzw. `snmpbulkwalk` umsetzt, haben die kommerziellen Editionen eine eingebaute SNMP-Engine (_Inline-SNMP_), die SNMP-Abfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen.
...
Außerdem verschickt die Monitoring-Software jeden Request mehrfach, bevor sie aufgibt.
...
Der komplette Abfrage eines Hosts besteht aus vielen einzelnen Requests.
...
den Regelsatz [.guihint]#Bulk walk: limit SNMP OID Ranges#.
Mit diesem können Sie die Liste der abgefragten Daten (z.B. Ports) statisch begrenzen.
Im Wert der Regel legen Sie jeweils für einen bestimmten Ast des OID-Baumes ([.guihint]#Section name#) fest, welche Indizes eingeholt werden sollen.

Die Identifikation kann sich auf generische oder herstellerspezifische Äste beziehen.
Die Standardeinstellung für Switch Ports ist generisch und heißt [.guihint]#SNMP interface check with 64-bit counters (using v2c)#.
...
War der Test auf der Befehlszeile erfolgreich, können Sie mit dem Regelsatz [.guihint]#Hosts using a specific SNMP backend# die Einstellung [.guihint]#Use Classic backend# für Hosts setzen, welche dauerhaft kein Inline-SNMP nutzen sollen.
endif::[]
...
ifdef::saas[]
[#simulation]
== SNMP-Walks für den {CMK}-Support
endif::[]
