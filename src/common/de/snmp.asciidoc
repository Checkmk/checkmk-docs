...
[%collapsible]
====
Die SNMP-Terminologie verwendet zwei Schlüsselworte, die leider häufig vertauscht werden: _Query_ (oft als _Abfrage_ übersetzt) und _Request_ (oft als _Anfrage_ übersetzt).
...
ifdef::saas[]
{CE} nutzt für die SNMP-Überwachung _aktive Anfragen_ – also eine Pull-Methode.
...
// MFS: Geht mit dem Relay nur, wenn die SNMP-Services einem anderen Host im Monitoring zugeordnet sind, ist also eher unpraktisch und daher für SaaS deaktiviert.
ifdef::onprem[]
[#snmp_cmk_agent]
...
. Legen Sie im Regelsatz [.guihint]#Setup > Agents > SNMP rules > Disable bulk walks# eine Regel für den Host an und stellen Sie in der Regel den Wert auf [.guihint]#Positive match (Add matching hosts to the set)#.

Dadurch wird der Host gezwungen, trotz eingestellter Version 1 das Protokoll SNMPv2c zu verwenden, allerdings _ohne Bulk-Walk_.
...
Falls Sie mit Ihren Ressourcen an die Grenzen stoßen bzw. die Abfrage eines einzelnen Gerätes länger als 60 Sekunden dauert, können Sie das Intervall reduzieren, mit dem {CMK} den oder die Hosts kontaktiert.
...
SNMP überträgt pro `GetBulk`-Request in der Voreinstellung 10 Antworten in einem Paket.
