// -*- coding: utf-8 -*-
// IGNORE Logfile reaktionsschnelle
// NONASCII
...
// TK: Soll Vue.js erwähnt werden oder ist das eine Implementierungsdetail?
// MFS: Ich würde dieses Details eher unterschlagen und darauf verweisen, dass mit den neuen Tabellenansichten weniger fertiges HTML vom Server geladen wird, stattdessen mehr asynchrone Kommunikation im Hintergrund stattfindet. Und das Ziel des Ganzen ist, dass das Interface "snappier" ist.
...
====
// TK: Da die neuen mit den klassischen Views wenig bis nichts gemeinsam haben (jedenfalls, was hier im Artikel beschrieben ist), könnte man für die neuen Views in der 3.0 einen eigenen Artikel schreiben.
// TK: Allerdings gibt es zu dem Thema neue Views im Vergleich zu den klassischen nicht wirklich viel zu beschreiben. Meinungen?
// MFS: Ich vermute, dass die neuen in 3.0 Default werden und in 3.1 die klassischen rausfallen. Dann kann man den Artikel für 3.0 einmal umstellen und für 3.1 wieder ausdünnen. Zweiter Artikel ist IMHO nicht notwendig.
...
&hellip; und bringen Sie zu (kontextabhängigen) Detailansichten.
...
.[.guihint]#Service#-Menü in der Ansicht eines Services
image::views_context_buttons.png[alt="Ansicht mit geöffnetem Menü 'Service'."]

Sie können Host- und Service-spezifische Tabellenansichten auch über die xref:user_interface#search[Suche] im [.guihint]#Monitor#-Menü erreichen.
...
.Ansicht der Services eines Hosts
...
.Menüleiste der Ansicht [.guihint]#All hosts#
...
.Menüleiste der Ansicht [.guihint]#Services of host#
...
.Darstellungsoptionen einer Ansicht
...
.Sortierung einer Tabelle durch Klick auf eine Spaltenüberschrift
image::views_sort_by_state.png[alt="Sortierung einer Tabelle nach Spaltenüberschriften."]
...
Hierbei wird ein sogenannter Sofortbericht (_instant report_) erzeugt, der die aktuelle Tabellenansicht in ein PDF-Dokument überführt, abzüglich Symbole, Navigation und so weiter.
...
.Hinweis bei Überschreitung des Anzeigelimits
...
.Hinweis bei Überschreitung des zweiten Anzeigelimits
...
.Globale Einstellung der Anzeigelimits
...
// MFS: An dieser Stelle könnte sinnvoll sein, zu erwähnen: Da jetzt die Tabelleneinträge in Batches vom Server geladen werden, dauert es zwar eine Weile, bis wirklich alle Einträge geladen sind, aber es treten keine Timeouts mehr auf.
// TK: Finde ich hier schwierig, da ich ja nicht die alten mit den neuen Views vergleichen will - und dass es bei den alten Timeouts geben kann, steht ja auch nirgendwo.
...
{CMK} erlaubt Ihnen, die mitgelieferten Tabellenansichten anzupassen und sogar Ihre eigenen zu erzeugen, welche Sie dann auch in xref:reporting#[Berichte] und xref:glossar#dashboard[Dashboards] einbauen können.
...
* Wie viele Datensätze werden angezeigt (*Zeilenanzahl*)?
...
. Im Menü [.guihint]#Customize > Visualization > Views#.
Dort können Sie mit icon:button_add_view[] Ihre eigenen Tabellenansichten erzeugen oder wiederum mit icon:icon_clone[] bestehende klonen und anpassen:

.Eine Ansicht erstellen
image::views_edit_views.png[alt="Ansicht zur Erstellung einer Ansicht."]
...
.Allgemeine Eigenschaften der Ansicht
...
.Spezifische Eigenschaften der Ansicht
image::views_edit_view_view_properties.png[alt="Spezifische Eigenschaften der Ansicht."]

Mit [.guihint]#Row limit# legen Sie die maximal angezeigte Anzahl an Zeilen in dieser Ansicht fest, um z.B. Top-N-Listen zu erstellen.
Bedenken Sie dabei aber, dass die potentiellen Begrenzungen des xref:limit[Anzeigelimits bei großen Monitoring-Umgebungen] dennoch gelten.

Die Einstellung [.guihint]#Automatic page reload# bestimmt das Aktualisierungsintervall, [.guihint]#Number of Columns# die Anzahl der Spalten, 
...
.Ergebnis der geänderten Ansicht
...
.Eigenschaften für die Spalten der Ansicht
...
.Ergebnis der geänderten Ansicht mit einem Tooltipp
...
.Host-Ansicht mit Perf-O-Metern für die Services
...
.Eigenschaften einer [.guihint]#Joined column# mit expliziter Angabe des Services
image::views_edit_view_columns_2.png[alt="Eigenschaften einer 'Joined column' mit expliziter Angabe des Services."]
...
.Eigenschaften einer [.guihint]#Joined column# mit regulärem Ausdruck für den Service
...
.Eigenschaften zur Sortierung der Ansicht
...
.Eigenschaften zum Filtern der Ansicht
...
.Die Option für die Konfiguration einer Suchansicht
image::views_edit_view_view_properties_show_data.png[alt="Die Option 'Show data only on search'."]


[#new]
== Tabellenansichten erstellen

Das Erstellen einer Tabellenansicht mit icon:button_add_view[] geht im Prinzip genauso wie das Editieren einer bestehenden Tabellenansicht -- mit einem kleinen Unterschied:
...
.Ansicht mit Gruppierung nach Host-Gruppen
...
.Auswahl eines spezifischen Objekttyps
image::views_create_view_2.png[alt="Auswahl eines spezifischen Objekttyps."]

Hier entscheidet sich, ob Ihre Tabellenansicht einen Kontext hat, oder ob es eine globale Tabellenansicht werden soll.
...
Durch das Aktivieren von [.guihint]#Show information of a single Host# legen Sie fest, dass die Tabellenansicht genau einen Host beschreibt.
...
Starten Sie mit der xref:new[Erstellung einer Ansicht] und wählen Sie als grundlegende xref:datasource[Datenquelle] die gewünschte Inventurtabelle aus.
...
.Dialog zur Auswahl der Spalten
...
.Auswahl einer [.guihint]#Joined column# unter Verwendung eines Makros
...
.Auswahl einer [.guihint]#Joined inventory column# unter Verwendung eines Makros
...
Nachdem alle Einstellungen vorgenommen wurden, speichern Sie mit icon:button_save_and_go_to_view[] [.guihint]#Save & go to view# die Ansicht ab und werden direkt zu dem Ergebnis geleitet.
...
.Ergebnis der Service-Suche in einer Matrixansicht
...
.Matrixansicht mit zusätzlichen Zeilen
...
.Matrixansicht mit zusätzlichen Informationen in den Zellen
...
.Farbige Darstellung von Ausreißern
...
.Globale Einstellung zum Einschalten von Alarmtönen in Ansichten
...
.Aktivierung von Alarmtönen in einer Ansicht
...
Damit ist das Symbol im System, findet aber noch keinerlei Verwendung.

.Dialog zur Auswahl und zum Hochladen eines eigenen Symbols
image::views_manage_icons_upload.png[alt="Dialog zur Auswahl und zum Hochladen eines eigenen Symbols."]
endif::[]

ifdef::onprem[]
Nun müssen Sie das Symbol noch 
endif::[]
ifdef::saas[]
Sie müssen das Symbol 
endif::[]
...
.Eigenschaften für eigene Symbole
...
.Die Regel für die Nutzung eines eigenen Symbols
image::views_custom_icons_or_actions.png[alt="Die Regel für die Nutzung eines eigenen Symbols."]

Zum anderen filtern Sie im Kasten [.guihint]#Conditions# wie üblich auf die gewünschten Hosts/Services:

.Bedingungen in der Regel für die Nutzung eines eigenen Symbols 
image::views_custom_icons_or_actions_b.png[alt="Bedingungen in der Regel für die Nutzung eines eigenen Symbols."]
...
.Host-Ansicht mit dem eigenen Symbol
image::views_view_with_custom_icon.png[alt="Host-Ansicht mit dem eigenen Symbol."]
