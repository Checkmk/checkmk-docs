...
Die wichtigste Aufgabe der xref:user_interface#[Benutzeroberfläche] von {CMK} ist das Anzeigen des aktuellen Status von Hosts und Services.
...
Einen Kontext benötigt z.B. die Tabellenansicht [.guihint]#Status of Host ...# -- nämlich die Angabe desjenigen Hosts, dessen Status angezeigt werden soll.
...
.Das [.guihint]#Monitor#-Menü der {RE} im Show-less-Modus
image::monitor_menu_less.png[alt="Monitor-Menü der {RE} im Show-less-Modus."]
...
image::views_tactical_overview.png[width=42%, align=center, alt="Ansicht des Overview aus dem rechten Seitenmenü."]
...
image::views_snapin_views.png[width=42%, align=center, alt="Das Snapin Views."]
...
image::views_context_buttons.png[alt="Tabellenansicht des Kontextmenüs."]
...
image::views_monitor_search.png[width=60%, align=center, alt="Die Suche im Monitor-Menü."]

Ein weiterer Weg zu Detailansichten ist die Schnellsuche mit dem Snapin [.guihint]#Quicksearch# in der Seitenleiste:

image::views_snapin_quicksearch.png[width=42%, align=center, alt="Die Schnellsuche."]
...
image::views_menu_context_service.png[alt="Menüleiste der Ansicht eines Services eines Hosts."]
...
Sobald Sie einen Filter gesetzt haben, ändert sich das Symbol auf icon:button_filters_set_lo[], damit klar ist, dass unter Umständen nicht alle Daten angezeigt werden.
// TK: Das Icon wurde in der 2.2 ersetzt.
...
image::views_time_date.png[alt="Änderung der Darstellung."]
...
Um eine solche Liste nach dem aktuellen Status der Services zu sortieren genügt ein Klick auf [.guihint]#State.#
...
|[.guihint]#Reports > This view as PDF# |Nur in den kommerziellen Editionen: Hierbei wird ein sogenannter Sofortbericht (_instant report_) erzeugt, der die aktuelle Tabellenansicht in ein PDF-Dokument überführt, abzüglich Icons, Navigation und so weiter.
...
Die Limitierung wird nämlich bereits an der Quelle der Daten durchgesetzt: in den Monitoring-Kernen der angebundenen Instanzen.
Das ist sehr wichtig:
Denn wenn wir erst eine Million Datensätze aus Ihrer weltweit verteilten Umgebung zusammensammeln würden, um gleich danach 99,9{nbsp}% der Daten wieder wegzuwerfen,
dann wäre das Kind ja schon in den Brunnen gefallen.
...
Sie können die beiden Stufen unter [.guihint]#Global settings > User interface# anpassen:
...
image::views_edit_views.png[alt="Neue Tabellenansichten erzeugen."]
...
Sie können später einfach zur Standardansicht zurückkehren, indem Sie Ihren Klon löschen (das geht dann in der Tabelle der Ansichten wie erwartet mit icon:icon_delete[]).
...
. Danach gelten Tabellenansichten, die ein Administrator angepasst und freigegeben hat (genau gesagt jemand mit dem Recht [.guihint]#Modify builtin views#).
...
image::views_edit_view_general_properties.png[alt="Festlegung der Anzeige einer Tabellenansicht in den General Properties."]
...
image::views_edit_view_view_properties.png[alt="Einstellungen in den Ansichtseigenschaften."]
...
-- beide Werte können in der Tabellenansicht selbst [.guihint]#Display > Modify display options# noch verändert werden.
...
image::views_layout_single_dataset.png[alt"Ansicht der modifizierten Tabellenansicht."]
...
image::views_edit_view_columns.png[alt="Anpassung der Spaltenansicht."]

Je nach gewählter Spalte kann es dafür spezifische Optionen geben.
Mit der Spalte [.guihint]#Hosts: Hostname# ist es zum Beispiel möglich, den Host-Namen über die Option [.guihint]#Coloring# entsprechend seines Status farblich zu hinterlegen.
...
Die Spalte [.guihint]#Hostname# ist hier anklickbar und bringt den Benutzer dann zu [.guihint]#Services of Host# -- von diesem Host.
...
image::views_hover_address.png[alt="Beispiel einer Spaltenansicht."]
...
image::views_joined_columns_example.png[alt="Beispiel von Serviceinformationen in einer Host-Ansicht."]
...
image::views_edit_view_columns_2.png[alt="Einstellung der Spalten."]
...
image::views_edit_view_columns_3.png[alt="Weitere Einstellungen zu den Spalten."]
...
Sie können eine _mehrstufige_ Sortierung einstellen, z.B. zuerst nach dem Service-Status, und bei gleichem Status nach dem Namen des Services.
...
image::views_edit_view_sorting.png[alt="Sortier-Optionen."]
...
Ein gutes Beispiel ist die Tabellenansicht [.guihint]#Service problems#, welche Sie bequem über das Snapin [.guihint]#Overview# erreichen.

Hier sehen Sie diese Tabelle gruppiert nach dem Service-Status ([.guihint]#State#): zuerst alle {CRIT}, dann {UNKNOWN}, dann {WARN} - jeweils mit einem eigenen Gruppentitel:

image::views_edit_view_grouping.png[alt="Ansicht einer gruppierten Tabelle."]
...
Übrigens: Ein Umsortieren nach einer Spalte durch den Benutzer hat keinen Einfluss auf die Gruppierung:
...
Zum einen legen Sie so fest, welche Filter dem Benutzer beim Klick auf icon:icon_filter[] angeboten werden.
...
image::views_edit_view_context_filter.png[alt="Einstellungen zu den Suchfiltern."]
...
image::views_hostgroups_summary.png[alt="Beispiel für Host- und Service-Gruppen."]
...
image::views_hostgroups.png[alt="Exemplarische Ansicht einer Host-Gruppenkombination."]
...
// TK: Meinem Verständnis hilft es, wenn man am Anfang kurz skizziert, was man vorhat. Dann kämen auch die Themen Instanzen und Editionen nicht so überraschend daher wie zurzeit.
...
Sie finden diese Tabellenansicht über [.guihint]#Customize > Visualization > Views.#
// TK: aber nur, wenn ich die Begrenzung auf 100 Ergebnisse aufhebe. Hinweis zur Umgehung dieses Anfängerfehlers wäre nett.

Folgende Abbildung zeigt eine Suche nach dem Serviceausdruck `CPU|^memory|Filesystem /opt/omd/sites/mysite`:

image::views_matrix_filter_service.png[width=55%, align=center, alt="Filter für die Service-Suche."]
...
image::views_matrix_view_1.png[alt="Ergebnis der obigen Filtersuche."]
...
* Bei [.guihint]#Grouping# ist als einzige Spalte der Host-Name ([.guihint]#Hosts: Hostname#) eingetragen.
...
image::views_matrix_view_2.png[alt="Exemplarische Ansicht von Host-Informationen."]
...
image::views_matrix_view_3.png[alt="Zweite exemplarische Ansicht von Host-Informationen."]


=== Erkennen von Ausreißern

Warum sind eigentlich manche Zellen farbig hinterlegt?
// TK: Aber in den Screenshots oben sind ja gar keine Zellen farbig hinterlegt. Von daher ist die Frage etwas merkwürdig.
...
// TK: Der Screenshot ist dann auch nicht wirklich passend.
...
Über den globalen Schalter [.guihint]#Setup > General > Global settings > User interface > Enable sounds in views# können Sie diese einschalten.
Wie (fast) immer hilft hier die Suche im [.guihint]#Setup#-Menü:

image::global_settings_enable_sound.png[alt="Das Einschalten von Alarmtönen in Views."]
...
image::view_properties_sounds.png[alt="Die Aktivierung der Alarmtöne."]
...
|`f` |Menüeintrag [.guihint]#Display > Filter# und der zugehörige Aktionsknopf icon:icon_filter[] zum Öffnen der Filterleiste werden entfernt.
...
|`e` |Menüeintrag [.guihint]#Display > Customize View# wird entfernt.
...
In Tabellenansichten von Hosts und Services sehen Sie auch eine Spalte für Icons und darin das icon:icon_menu[] Aktionsmenü, aus dem Sie Host- beziehungsweise Service-Aktionen auswählen können.
Sie dürfen Tabellenansichten aber auch um eigene Icons erweitern.
...
image::views_manage_icons_upload.png[alt="Eigene Icons hochladen."]

Nun müssen Sie das Icon noch als über xref:glossar#rule[Regeln] ansprechbares Objekt und optional eine zugehörige Aktion definieren.
...
der Titel wird übrigens später per On-Mouse-Over-Effekt als Tooltipp direkt am Icon angezeigt und ist für die Benutzer damit sehr hilfreich.

image::views_global_settings_custom_icons_and_actions.png[alt="Einstellungen für eigene Icons."]
...
Über ein Häkchen bei [.guihint]#Show in column# können Sie das Icon dann als eigenständiges Bildchen neben icon:icon_menu[] anzeigen lassen,
andernfalls landet Ihre Aktion im zugehörigen Aktionsmenü.

Im letzten Schritt bestimmen Sie nun, bei welchen Hosts oder Services das neue Icon angezeigt werden soll -- und das natürlich über Regeln.
...
Zum einen wählen Sie unter [.guihint]#Custom icons or actions for hosts in status GUI# das just angelegte Icon:
...
image::views_custom_icons_or_actions_b.png[alt="Setzen der Bedingungen für das Icon."]
...
image::views_view_with_custom_icon.png[alt="Exemplarische Host-Ansicht mit dem eigenen Icon."]
