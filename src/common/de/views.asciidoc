...
====
// TK: Wahrscheinlich ist es die sauberste Lösung für die 3.0.0 einen eigenen Artikel für die neuen Views zu schreiben. 
// TK: Allerdings gibt es zu dem Thema neue Views nicht wirklich viel zu beschreiben. Meinungen?
...
.Die Menüleiste zu "Services of host"
...
.Die neue Tabellenansicht [.guihint]#All hosts (experimental)# mit durch Suche eingeschränktem Inhalt
...
Es ist zum Beispiel nicht möglich, eine neue Ansicht anzupassen oder eine Ansicht manuell zu erstellen, die auf einer neuen Ansicht basiert.
Für die neuen Ansichten ist nur dieses Kapitel relevant, alle anderen Kapitel dieses Artikels beschreiben Möglichkeiten, die _nur_ für klassische Ansichten verfügbar sind.
====
...
Den schnellen Zugriff zum Filtern der Tabelle auf die für Sie relevanten Inhalte finden sich unterhalb der Menüleiste:
...
Durchsucht werden dabei _alle_ Tabelleneinträge, das heißt eventuell mehr als aktuell angezeigt werden.
...
* Filter zurücksetzen +
Alle per Spaltensymbole gesetzten Filter können Sie per Knopfdruck auf [.guihint]#Reset all filters# wieder zurücksetzen.
...
Für die wichtigsten Aktionen gibt es Knöpfe, die prominent über der Tabelle platziert sind, in der Ansicht [.guihint]#All hosts (experimental)# zum Beispiel zur xref:basics_ackn#[Quittierung von Problemen] oder zur Festlegung von xref:glossar#scheduled_downtime[Wartungszeiten].
Die Knöpfe sind nur dann aktiv, wenn mindestens eine Zeile per Checkbox ausgewählt wurde.

Alle anderen Aktionen finden Sie zusammengefasst in der Tabellenspalte [.guihint]#Actions#.  
...
Die Checkbox in der ersten Tabellenspalte zur Auswahl der Zeilen für Aktionen ist immer sichtbar -- sofern Sie über die Berechtigung zur Bearbeitung verfügen.

Die Spalte [.guihint]#Actions# enthält die ausführbaren Aktionen, die nicht bereits durch Knöpfe oberhalb der Tabelle hervorgehoben platziert wurden.
...
Die Ansicht [.guihint]#All hosts (experimental)# enthält zum Beispiel die Spalten [.guihint]#IP address# und [.guihint]#All services#, die die Summe bildet über alle Services eines Hosts.
...
Aus einer neuen Ansicht können Sie kontextbezogene Information über ein Slideout einblenden lassen, das heißt, ohne die aktuelle Ansicht verlassen zu müssen.
...
Die neue Ansicht wird bereits deshalb in {CMK} {v25} verfügbar gemacht, um Ihnen die Möglichkeit zum Feedback zu geben -- und so die Ausgestaltung der neuen Ansichten in der {CMK}-Version {v30} zu beeinflussen -- in begrenztem Rahmen, aber immerhin.
...
.Ansicht der neu erstellten, eigenen Ansicht
