...
// TK: Soll im folgenden Tipp Vue.js erwähnt werden oder ist das eine Implementierungsdetail?
...
// TK: Tabellenansichten gibt es schon solange, wie es Checkmk gibt: Stimmt das überhaupt?
// MFS: Streng genommen: Nein. Das allererste Checkmk waren Skripte zur Konfiguration von Nagios, dann dürfte das Monitoring-Frontend gekommen sein.
// MFS: Immerhin sollten _da_ Tabellenansichten zu Beginn dabeigewesen sein.
[TIP]
====
Tabellenansichten gibt es schon solange, wie es {CMK} gibt.
...
.Das Service-Menü in der Ansicht eines Services
...
.Übersicht der Services zu einem Host
...
.Die Menüleiste der Ansicht [.guihint]#All hosts#
...
.Die Menüleiste der Ansicht zu [.guihint]#Services of host#
...
.Die Menüleiste zu einem einzelnen Service
...
.Einstellung der Optionen für die Ansicht
...
.Warnhinweis bei Überschreitung des Anzeigelimits
...
.Anzeige bei zweiter Überschreitung des Anzeigelimits
...
[TIP]
====
Da die neuen Ansichten eine komplette Neuentwicklung sind, werden Features, die Sie von den klassischen Ansichten kennen, in neuen Ansichten _nicht_ unterstützt.
// MFS: ...werden einige Features ... noch nicht...
// MFS: Halte ich für besser, weil da nach und nach mehr kommt.
...
// MFS: Hier halte ich die Description List für besser geeignet
* Suchfeld +
Im linken Suchfeld können Sie Freitext eingeben und mit der kbd:[Enter]-Taste absenden. +
...
* Vordefinierter Knopf zum Filtern +
Den schnellen Zugriff auf unbehandelte Host-Probleme bietet in der Ansicht [.guihint]#All hosts (experimental)# der Knopf [.guihint]#Unhandled host problems#. +
...
* Spalten filtern +
Jedes Filtersymbol im Spaltentitel können Sie nutzen, um in einer beliebigen Spalte manuell Ihre eigenen Filter zu setzen.
...
* Filter zurücksetzen +
Alle über Spaltensymbole gesetzten Filter können Sie per Knopfdruck auf [.guihint]#Reset all filters# wieder zurücksetzen.

Grundsätzlich gilt, dass alle Filter logisch mit UND verbunden werden.
// MFS: Vorschlag: dass alle Filter mit logischem "und" ("sowohl als auch") verknüpft werden.
...
Übrigens lassen sich Aktionen auch in einem xref:new_view_slideout[Slideout] ausführen.
// MFS: Hier halte ich eine Aktivkonstruktion als Handlungsaufforderung für passender:
...
. Im Menü [.guihint]#Customize > Visualization > Views#. Dort können Sie mit icon:button_add_view[] Ihre eigenen Tabellenansichten erzeugen oder wiederum mit icon:icon_clone[] bestehende klonen und anpassen:
...
.Generelle Einstellungen der Tabellenansichten
...
.Beispiel der Spaltendarstellung
...
.Filtereinstellungen für eine Tabellenansicht
...
.Weitere Filtereinstellungen für eine Tabellenansicht
...
.Filter zur Auswahl angezeigter Informationen
...
.Auswahl der spezifischen Objekteigenschaften
...
.Einstellung der Eigenschaften eines Makros
...
.Einschränkung der auszuwählenden Spalten
...
.Die neu erstellte, eigene Ansicht
...
.Verwendung des Filters für die Suche
...
.Ansicht de Matrix mit weiteren Informationen
...
.Globale Einstellung von Alarmtönen
...
.Aktivierung der Alarmtöne in den Eigenschaften
...
.Ergänzung eines eigenen Symbols
...
.Eigenschaften des eigenen Symbols ergänzen
...
.Verwendung des eigenen Symbols in einer Regel
...
.Bedingung in einer Regel für das eigene Symbol 
...
.Host-Ansicht unter Verwendung des eigenen Symbols
