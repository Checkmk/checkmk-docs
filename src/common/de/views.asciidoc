...
Suchfeld:: Im Suchfeld auf der linken Seite können Sie Freitext eingeben und mit der kbd:[Enter]-Taste absenden. +
Es werden daraufhin nur noch die Einträge angezeigt, die den Suchtext enthalten.
...
Sie können den Wert mit der Liste [.guihint]#Show# rechts oberhalb der Tabelle auf `5000` oder `All` erhöhen.
...
Dort können Sie mit icon:button_add_view[] Ihre eigenen Tabellenansichten erzeugen oder wiederum mit icon:icon_clone[alt="Symbol für das Klonen."] bestehende klonen und anpassen:
...
Sie können später einfach zur Standardansicht zurückkehren, indem Sie Ihren Klon löschen (das geht dann in der Tabelle der Ansichten wie erwartet mit icon:icon_delete[alt="Symbol für das Löschen."]).
