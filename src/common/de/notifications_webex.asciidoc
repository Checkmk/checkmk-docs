...
+
image::notifications_webex4.png[alt="Die Einstellungen zur Benachrichtigungsmethode für Cisco Webex Teams."]
+
...
. Erstellen Sie neue Parameter für Mattermost mit [.guihint]#Setup > Events >  Notifications > Parameters for notification methods > Parameters for Cisco Webex Teams > Add parameter.#
. Geben Sie die kopierte Webhook-URL ein.
+
