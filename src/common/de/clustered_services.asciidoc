...
:description: Bestimmte Services können Sie auf wechselnden Hosts in einem Cluster bereitstellen. Lernen Sie hier, wie Sie in {CMK} solche Cluster-Services überwachen.
...
sodass sich der Ausfall nach außen gar nicht bemerkbar macht.
...
In einem Cluster werden die Hosts, aus denen er besteht, als Knoten (_nodes_) bezeichnet.
...
Um einen Failover transparent zu machen, stellen einige Cluster eine eigene Cluster-IP-Adresse zur Verfügung, die manchmal auch als virtuelle IP-Adresse bezeichnet wird.
...
Dem Client, der mit dem Cluster kommuniziert, kann ein interner Failover egal sein:
...
Beispiele dafür sind CPU- und Speichernutzung, lokale Festplatten, physische Netzwerkschnittstellen und so weiter.
...
Bei denjenigen Checks, die die Cluster-Unterstützung implementiert haben, können Sie dies in der Handbuchseite des Plugins nachlesen.
Zu den Handbuchseiten geht es im Menü [.guihint]#Setup > Services > Catalog of check plug-ins#.
...
Schließen Sie die Erstellung mit [.guihint]#Save & view folder# ab und xref:glossar#activate_changes[aktivieren Sie die ausstehenden Änderungen].
...
Aktivieren Sie [.guihint]#Explicit hosts# und tragen Sie den aktiven Knoten-Host `mssql-node01` und den passiven Knoten-Host `mssql-node02` ein.
...
Schließen Sie die Erstellung der Regel mit [.guihint]#Save# ab und aktivieren Sie die ausstehenden Änderungen.
...
Nach erfolgreichem Abschluss, erkennbar an der Meldung `Bulk discovery successful`, verlassen Sie die Seite und aktivieren Sie die ausstehenden Änderungen.
...
Im Fall eines Failover wird die Aufgabe des ausgefallenen Knotens vom anderen Knoten übernommen, auf dem dann beide Datenbankinstanzen laufen.
...
Den xref:monitoring[1. Schritt zur Einrichtung des Aktiv/Passiv-Clusters] müssen Sie für einen Aktiv/Aktiv-Cluster nur leicht abwandeln:
...
Dann aktivieren Sie [.guihint]#Services# und machen Sie dort zwei Einträge:
