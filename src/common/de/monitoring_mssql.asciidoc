...
:description: Mit dem {CMK}-Plugin für Windows und Linux können Sie Microsoft SQL Server überwachen. Lesen Sie hier, wie Sie die Überwachung vorbereiten und nutzen.
...
== Konto für die Überwachung einrichten
...
[#setupuser]
=== Benutzer einrichten / Konten verwenden
...
+
--
Im Falle von MSSQL unter Windows gibt es zwei Möglichkeiten.
...
Die Installation des Plugins läuft anschließend wie gehabt über die xref:wato_monitoringagents#[Agentenbäckerei]: Agent backen und signieren und anschließend entweder manuell herunterladen und  auf den betroffenen Hosts installieren oder, sofern eingerichtet, die xref:agent_deployment#[automatischen Agenten-Updates] abwarten/auslösen.
...
Über [.guihint]#Connection -> Exclude system databases# (oder `exclude_databases` in der `mk-sql.yml`-Datei) lassen sich teils nicht erreichbare Systemdatenbanken ausnehmen, derzeit die Datenbanken Model, Master, Msdb und Tempdb).
