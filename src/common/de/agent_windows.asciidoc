...
ifdef::saas[]
Im xref:intro_setup#onboarding_guide[Onboarding Guide] unter [.guihint]#Add hosts > Command line download and install# können Sie sich nach Anklicken des Windows-Icons den PowerShell-Befehl anzeigen lassen, mit dem Sie den für Autoregistrierung im Ordner `preconfigured_agent` vorbereiteten Agenten herunterladen.

// FIXME Schriftgröße im Screenshot
// FIXME Screenshot mit Windows PowerShell-Befehl
image::agent_linux_onboard_curl.png[alt="Kompletter Download-Befehl, der bereits Nutzername und Passwort enthält."]
endif::[]

ifdef::onprem[]
Das mitgelieferte MSI-Paket des Windows-Agenten lässt sich beispielsweise mit dem folgenden `curl`-Befehl holen.
