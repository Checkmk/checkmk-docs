...
//
// MFS: Änderungen
// * Annahme, dass Push default ist
// * Auf Port 6556 und unverschlüsselte Agentenausgabe wird erst später eingegangen
...
Neben der initialen Authentifizierung und Registrierung ist Aufgabe des Agent Controller einmal pro Minute das Agentenskript aufzurufen und die Agentendaten zur Checkmk-Site zu übertragen.  

[TIP]
====
Dieser Artikel behandelt den Agenten von {CE} (SaaS).
Diese {CMK}-Version erwartet aus Gründen von Performance und Sicherheit ausschließlich den verschlüsselten Push-Mode zur Kommunikation zwischen Agent und Instanz.
In {RE} und {SE} steht ausschließlich der Pull-Mode (verschlüsselt und – wo es aus Kompatibilitätsgründen erforderlich ist – unverschlüsselt) zur Verfügung.
...
|===
|Edition |Version
// |Windows 8, Windows Server 2012 (R1) |6.2
...
|Windows 10, 11, Windows Server 2016, 2019, 2022 |10.0
|===
...
//
// MFS: Änderungen
// * Annahme, dass Agentenbäckerei default ist
// * Auf Vanilla-Agents wird nur für Sonderfälle eingegangen
// * Tabelle aufgelöst, es muss nur grob erklärt werden, was es sonst so gibt

ifdef::saas[]
In {CE} (SaaS) ist der Standardweg, einen Agenten zu installieren, die Verwendung der für Autoregistrierung und Push-Mode vorbereiteten Agentenpakete.
Neben diesen stehen sogenannte Vanilla-Agenten ohne Vorkonfiguration zur Verfügung, die mit allen {CMK}-Editionen benutzt werden können, aber mehr Aufwand bei der Einrichtung erfordern.

Die Verwendung der Vanilla-Agenten kann beim Debugging von Konfiguration sinnvoll sein, oder wenn Sie Betriebssystem-Images erstellen wollen, die unabhängig von einer {CMK}-Edition oder -Instanz verwendet werden können.
Dazu später mehr.
endif::[]
...
// MFS: Vanilla ist Default, Bakery erfordert Mehraufwand
...
// MFS: Bakery ist Default, Vanilla erfordert Mehraufwand
...
Dort finden Sie für jeden angelegten Ordner für die Autoregistrierung in diesem Ordner vorkonfigurierte Pakete:
Hier erfolgt die Registrierung des Agenten automatisch nach der Installation des Agentenpakets und eine manuelle Registrierung, wie im folgenden xref:registration[Kapitel] beschrieben, ist _nicht_ mehr notwendig.

// FIXME Schriftgröße im Screenshot
.In der Agent Bakery laden Sie für Push-Mode und Autoregistrierung vorkonfigurierte Agenten herunter
image::agent_linux_bakery_saas.png[alt="Vorkonfigurierte Agenten in der Agent Bakery."]
...
Sollten Sie einen Vanilla-Agenten benötigen, kommen Sie mit dem Menüeintrag [.guihint]#Related > Windows files# zu Agentendateien und -plugins.
Alles was Sie brauchen, finden Sie gleich im ersten Kasten mit dem Namen [.guihint]#Packaged Agents#:
die fertige MSI-Paketdatei für die Installation des Windows-Agenten mit Standardeinstellungen.

.Auf der Download-Seite, welche Agentenplugins enthält, finden Sie den Vanilla-Agenten, der mit allen {CMK}-Editionen nutzbar ist, aber einen höheren Konfigurationsaufwand hat
image::agent_windows_agent_files.png[alt="Download-Seite mit dem MSI-Paket."]
endif::[]
...
ifdef::saas[]
In {CE} (SaaS) können Sie sich im [.guihint]#Onboarding Guide# unter [.guihint]#Add hosts > Command line download and install# nach Anklicken des Windows-Icons das PowerShell-Kommando anzeigen lassen, mit dem Sie den für Autoregistrierung im Ordner [.guihint]#preconfigured_agent# vorbereiteten Agenten herunterladen.
...
Die anderen angezeigten Optionen betreffen den kaum anzutreffenden Fall, dass Sie ein Windows-System ins Monitoring aufnehmen wollen, auf dem vor langer, langer Zeit ein Agent der Onprem-Variante von {CMK} installiert wurde.
endif::[]
...
Bei dem für Autoregistrierung vorkonfigurierten Agentenpaket wird nun die Checkmk-Instanz kontaktiert, nach wenigen Minuten steht dann der Host mit ersten Monitoring-Daten bereit.
endif::[]
...
[TIP]
====
Dieser Abschnitt ist für Sie nur relevant, wenn Sie einen nicht nicht für Autoregistrierung vorkonfigurierten Vanilla-Agenten installiert haben. 
====
endif::[]
...
ifdef::saas[]
damit auch ältere {CMK}-Server (Onprem) nicht von den Monitoring-Daten abgeschnitten werden und diese weiterhin anzeigen können.
endif::[]
...
Anders liegt der Fall bei Nutzung eines *Legacy-Agenten* auf einem sehr alten xref:windows_versions[Windows-System].
...
[IMPORTANT]
====
Windows-Systeme, bei denen nur ein Legacy-Agent installiert werden kann, können _nicht_ ins Monitoring von {CE} (SaaS) aufgenommen werden!
====
endif::[]
...
[TIP]
====
Dieses gesamte Kapitel ist vor allem relevant, wenn Sie einen nicht für Autoregistrierung vorkonfigurierten Vanilla-Agenten installiert haben oder vorhandene Hosts aus dem Monitoring mit anderen {CMK}-Editionen zu {CE} (SaaS) migrieren wollen.
Er kann auch hilfreich sein, wenn die Autoregistrierung – aus welchen Gründen auch immer – einmal fehlschlägt.
====
endif::[]
...
ifdef::onprem[]
*Hinweis:* Da es auf sehr alten xref:windows_versions[Windows-Systemen] keinen Agent Controller, und da mit keine Registrierung und TLS-Verschlüsselung gibt, 
müssen Sie bei Bedarf andere Wege der Verschlüsselung wählen.
Wir empfehlen, in diesem Fall die eingebaute (symmetrische) Verschlüsselung zu nutzen, sie wird mit der Regel [.guihint]#Symmetric encryption (Linux, Windows)# eingerichtet.
endif::[]

// MFS: Da der Checkmk-Server nicht beeinflusst werden kann, hier mal was zur Firewall! Aus agent_linux kopiert
...
// MFS: Evtl. den ersten Satz ändern, der Nutzer kennt Autoregistrierung bereits als Default.
...
// MFS: Hier ist Klärungsbedarf. Wird ggf. _CLUSTER_NETWORK_NAME_ genutzt?  
...
[TIP]
====
Dieser Abschnitt hat dank Verfügbarkeit der Autoregistrierung für {CE} (SaaS) wenig Relevanz.
Falls Sie die _Registrierung im Auftrag_ beispielsweise zusammen mit Infrastruktur-Management-Werkzeugen bereits mit den Onprem-Editionen von {CMK} verwenden, können Sie dies auch weiterhin mit {CE} (SaaS) tun.
====
endif::[]
...
// MFS: Auch SaaS-Nutzer können das tun, but why?
ifdef::onprem[]
...
Da die Ausgabe etwas länger sein kann, ist der Pager `more` hier sehr praktisch.
...
// MFS: War da nicht ein Werk, welches statt TCP Port auf das Äquivalent zum Unix Domain Socket geändert hat?
// TK: Werk #14360 (Controller communicates with Windows agent using mailslot)?
// MFS: Auskommentiert, mag bei Updates von 2.1 nach 2.2 stellenweise bleiben, ist bei Neuinstallationen nicht mehr nutzbar.
...
// MFS: FIXME: Ganz weglassen?
ifdef::saas[]
Sollten Sicherheitsrichtlinien in Ihrer Umgebung den Zugriff auf den Agent Receiver nicht zulassen, ist {CE} (SaaS) keine adäquate Monitoringlösung für Sie.
endif::[]
...
// TK: Bei einem Legacy-Agenten müsste der Pfad so lauten: %ProgramFiles% (x86)\checkmk\service\check_mk_agent.exe
...
Bei Hosts im Push-Modus zeigt dieser Softlink auf den Ordner mit der Agentenausgabe, bei Pull-Hosts auf eine nicht vorhandene Datei mit dem Namen des Hosts, wie er im Monitoring verwendet wird.
...
// MFS: Evtl. weitere Debugging-Möglichkeiten mit der Entwicklung besprechen?
...
[IMPORTANT]
====
Das hier beschriebene Verhalten betrifft vor allem Nutzer, die im Monitoring bei anderen {CMK}-Editionen befindliche Hosts _zusätzlich_ (zum Beispiel zu Evaluationszwecken) bei {CMK} (SaaS) ins Monitoring aufnehmen.
====
endif::[]
...
Die Schrägstriche (_forward slashes_) als Pfadtrenner sind in beiden Beispielen eine zulässige und bevorzugte (erspart Escaping) Ausnahme von den sonst üblichen Backslashes:
...
ifdef::onprem[]
Als Nutzer einer der kommerziellen Editionen
endif::[]
ifdef::saas[]
Als Nutzer von {CE} (SaaS)
endif::[]
können Sie über [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Disabled sections (Windows agent)# einfach eine Regel anlegen, welche dann von der xref:glossar#agent_bakery[Agentenbäckerei] berücksichtigt wird.
...
ifdef::saas[]
Alternativ sind die Plugins für Windows auch über die Download-Seite des Agenten im Setup-Menü (wie im Kapitel xref:download_gui[Installation] beschrieben) im Kasten [.guihint]#Plugins# verfügbar:
endif::[]
...
// TK: Folgenden Satz auskommentiert, weil die Fettung nicht angezeigt wird.
...
ifdef::saas[]
In {CE} (SaaS) können Sie die mitgelieferten Plugins über die xref:glossar#agent_bakery[Agentenbäckerei] konfigurieren.
endif::[]
