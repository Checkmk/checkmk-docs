...
:description: Der Agent Controller übermittelt die gesammelten Monitoring-Daten vom Windows-Agenten an {CMK}. Hier erfahren Sie, wie Sie die Überwachung einrichten.
:experimental:
...
ifdef::onprem[]
Dem *Agentenprogramm* ist mit dem *Agent Controller* eine Komponente zur Seite gestellt, die mit dem *Agent Receiver*, einem Prozess, der auf dem {CMK}-Server läuft, verschlüsselt kommuniziert.
Dazu registriert sich der Agent Controller beim *Agent Receiver*.
Der Windows-Agent kann mit dem Agent Controller Funktionen realisieren wie die TLS-Verschlüsselung der Kommunikation oder die Datenkomprimierung und ist vorbereitet für zukünftige Erweiterungen.
...
Den xref:glossar#push_mode[Push-Modus] gibt es in {CCE}.
Die Umkehrung der Kommunikationsrichtung erleichtert die Überwachung von Hosts, die sich hinter Firewalls befinden.
Kombiniert wird der Push-Modus meist mit der xref:hosts_autoregister#[automatischen Registrierung] des {CMK}-Agenten, die ebenfalls in {CE} verfügbar ist.
...
{UE} ist am flexibelsten und unterstützt beide Modi.
====
endif::[]
...
ifdef::onprem[]
Welche das genau sind, steht im Artikel xref:release_notes.html#windows[Systemvoraussetzungen].
endif::[]
ifdef::saas[]
...
Im Pull-Modus lauscht er am TCP-Port 6556 auf eingehende Verbindungen der {CMK}-xref:glossar#site[Instanz] und fragt das Agentenprogramm über einen Windows-eigenen Mechanismus für die Prozess-Kommunikation namens link:https://learn.microsoft.com/en-us/windows/win32/ipc/about-mailslots[Mailslot^] ab.
...
ifdef::onprem[]
{CMK} unterstützt Sie in der Benutzeroberfläche dabei, den {CMK}-Agenten auf einen Host herunterzuladen, zu installieren und zu registrieren und damit in die xref:glossar#configuration_environment[Konfigurationsumgebung] aufzunehmen.
Ein Assistent führt Sie dabei in einem _Slideout_ durch die notwendigen Schritte.
Im Slideout können Sie als Betriebssystem für den Host zwischen Windows, Linux, Solaris und AIX auswählen.
Im xref:intro_setup_monitor#linux[Leitfaden für Einsteiger] ist der Ablauf ausführlich für einen Linux-Host im Pull-Modus beschrieben.
Der Ablauf ist für einen Windows-Host identisch, nur die Befehle sind andere.

Der große Vorteil des Slideouts ist, dass Ihnen die passenden Befehle zum Download, zur Installation und zur Registrierung eines Windows-Hosts in der GUI angezeigt werden.
Sie können jeden Befehl in die Zwischenablage kopieren, und ihn dann auf dem Host ausführen.
Für einen Windows-Host wird dabei das Agentenpaket mit Curl über die REST-API heruntergeladen, mit dem `msiexec`-Befehl installiert und der Host schließlich mit `cmk-agent-ctl` registriert.
endif::[]


ifdef::saas[]
{CMK} unterstützt Sie in der Benutzeroberfläche dabei, den {CMK}-Agenten auf einen Host herunterzuladen, zu installieren und zu registrieren -- und sogar ins Monitoring aufzunehmen.
Dafür gibt es in {CE} zwei Assistenten, die Sie dabei jeweils in einem _Slideout_ durch die notwendigen Schritte führen.

* Das eine Slideout hilft dabei, einen Host automatisch in die xref:glossar#configuration_environment[Konfigurationsumgebung] und in die xref:glossar#monitoring_environment[Monitoring-Umgebung] aufzunehmen. +
Dabei werden die für die Autoregistrierung vorbereiteten Agentenpakete für Linux und Windows verwendet.

* Das andere Slideout führt Sie durch den manuellen Ablauf, einen Host in die Konfigurationsumgebung aufzunehmen.
Hier werden die Standard-Pakete verwendet, die es für Windows und Linux gibt. 

Beide Slideouts für die xref:intro_setup_monitor#first_auto[automatische Erstellung] und für die xref:intro_setup_monitor#first[manuelle Erstellung] werden im Leitfaden für Einsteiger vorgestellt.

Der große Vorteil der Slideouts ist, dass Ihnen die passenden Befehle zum Download, zur Installation und zur Registrierung eines Windows-Hosts in der GUI angezeigt werden.
Sie können jeden Befehl in die Zwischenablage kopieren, und ihn dann auf dem Host ausführen.
endif::[]


Um den Ablauf so einfach wie möglich zu gestalten, wird die Authentisierung in den Befehlen, die für Download und Registrierung notwendig sind, mit einem Einmal-Token (_one-time token_) durchgeführt.
Dieses Token ist für eine erfolgreiche Ausführung innerhalb eines begrenzten Zeitraums gültig.
Es wird dabei zuerst generiert, bevor es in den genannten Befehlen verwendet wird.

Download, Installation und Registrierung im Schnelldurchlauf per Slideout eignen sich besonders gut dazu, schnell die ersten Hosts in {CMK} zu erstellen.
Ausführliche Informationen über alle möglichen Wege zur Installation eines Standard-Agenten (zum Beispiel ohne Einmal-Token, mit xref:glossar#agent_plugin[Agentenplugins] und mit Fehlersuche) erhalten Sie in den folgenden Kapiteln.
...
ifdef::saas[]
In {CE-saas} ist es besonders einfach, für die Autoregistrierung vorkonfigurierte Agentenpakete zu installieren und zu nutzen.
Neben diesen stehen die sogenannten „Vanilla-Agenten“ ohne Vorkonfiguration zur Verfügung, die mehr Aufwand bei der Einrichtung erfordern.
...
Die Vanilla-Agenten ohne Vorkonfiguration finden Sie in der ersten Zeile [.guihint]#Default configuration# der Tabelle.

.In der Agentenbäckerei können Sie vorkonfigurierte und Vanilla-Agentenpakete herunterladen
image::agent_linux_bakery_saas.png[alt="Agentenpakete in der Agentenbäckerei.",fullscreen=1]
...
Das mitgelieferte MSI-Paket des Windows-Agenten lässt sich beispielsweise mit dem folgenden `curl`-Befehl holen, in dem die Authentifizierung mit einem xref:glossar#automation_user[Automationsbenutzer] erfolgt.
In neueren Windows-Versionen wird `curl` bereits mitgeliefert, in älteren müssen Sie sich vorher die `curl`-Befehlsumgebung per link:https://curl.se/windows/[curl for Windows^] extra installieren. 

// ES: HTML-Darstellung angelehnt an die vorige Version des Code-Blocks (siehe https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L213)
ifdef::onprem[]
++++
<div class="listingblock copybutton">
    <div class="content">
        <pre class="pygments highlight"><code data-lang="doscon"><span></span><span class="tok-gp">C:\Users\hhirsch\Downloads\&gt;</span> curl.exe -OJG <span class="tok-s2">"http://mycmkserver/mysite/check_mk/api/v1/domain-types/agent/actions/download/invoke"</span> ^
...
<div class="listingblock copybutton">
    <div class="content">
        <pre class="pygments highlight"><code data-lang="doscon"><span></span><span class="tok-gp">C:\Users\hhirsch\Downloads\&gt;</span> curl.exe -OJG <span class="tok-s2">"http://mycmkserver/mysite/check_mk/api/v1/domain-types/agent/actions/download/invoke"</span> ^
--header <span class="tok-s2">"Accept: application/octet-stream"</span> ^
--header <span class="tok-s2">"Authorization: Bearer api_user myautomationsecret"</span> ^
--data-urlencode <span class="tok-s2">"os_type=windows_msi"</span></code></pre>
    </div>
</div>
++++
...
Details zu diesem und anderen REST-API-Endpunkten finden Sie in der API-Dokumentation, die Sie in {CMK} über [.guihint]#Help > Developer resources > REST API > Version 1 > Documentation# aufrufen können.
...
Nachdem Sie das MSI-Paket geholt und -- falls nötig -- mit `scp`, WinSCP oder anderen Mitteln auf den zu überwachenden Host kopiert haben, starten Sie die Installation entweder per Doppelklick auf die MSI-Datei oder wie folgt von der Befehlszeile:
...
// ES: The following sentence may need attention for KNW-2113
...
Windows bietet über die Befehlszeile für Administratoren mit `msiexec` die Möglichkeit, MSI-Pakete automatisiert, ohne Benutzerinteraktion zu installieren.
...
Für jede Option gibt es einen Bezeichner, den Sie für den Installationsbefehl verwenden können:
...
In {CE} können Sie die Agentenbäckerei zusätzlich nutzen, um Agentenpakete mit einer Konfiguration für die Autoregistrierung zu versehen,
...
ifdef::saas[]
// ES: This fragment may need attention for KNW-2113
...
Das betrifft sowohl eine Neuinstallation als auch das Update eines sehr alten Agenten (älter als Version {v21}).
...
Allerdings kann er im Bedarfsfall xref:deregister[per Befehl] wieder eingeschaltet werden.
...
*Hinweis:* Im Regelsatz [.guihint]#{CMK} Agent installation auditing# finden Sie verschiedene Einstellungen, um den Zustand des Agenten zu prüfen und im Monitoring sichtbar zu machen.
...
Unmittelbar nach der Installation des Agenten (auch als Update eines sehr alten Agenten, d.h. älter als Version {v21}) ist nur unverschlüsselte Kommunikation im Legacy-Pull-Modus möglich.
...
ifdef::onprem[]
In {CE} finden Sie in den Eigenschaften des Hosts im Abschnitt zu den xref:hosts_setup.html#monitoring_agents[Monitoring-Agenten] die Option [.guihint]#{CMK} agent connection mode#.
...
Die Registrierung erfolgt mit dem Agent Controller `cmk-agent-ctl`, der für die Konfiguration der Verbindungen eine Befehlsschnittstelle bietet.
Sie können sich mit `cmk-agent-ctl help` die Befehlshilfe anzeigen lassen, auch spezifisch für die verfügbaren Sub-Befehle, z.B. mit `cmk-agent-ctl help register`.
...
// ES: HTML-Darstellung für den Code-Block angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L501
...
Dies kann (von Hand oder per Skript) durch Kontrolle der xref:files[Datei] `C:\ProgramData\checkmk\agent\registered_connections.json` erfolgen.

[TIP]
====
Mit dem Befehl `cmk-agent-ctl` kann die Registrierung des Agent Controllers und des xref:glossar#agent_updater[Agent Updaters] _gleichzeitig_ erfolgen.
Wie das funktioniert, erfahren Sie im Artikel zu den xref:agent_deployment#one_registration[automatischen Agenten-Updates].
====
...
ifdef::onprem[]
{CE} bietet die Möglichkeit, Hosts automatisch bei der Registrierung anzulegen.
...
ifdef::saas[]
Sie haben die Möglichkeit, zusätzlich zu den in der Agentenbäckerei verfügbaren Paketen, eigene Pakete für die xref:hosts_autoregister#[Autoregistrierung] zu erstellen.
...
Ist dies der Fall, können Sie die Registrierung inklusive automatischer Erstellung des Hosts auch per Befehlszeile vornehmen.
...
welche die Konfigurationsdatei `C:\ProgramData\checkmk\agent\pre_configured_connections.json` ins Agentenpaket integrieren und die Registrierung automatisch bei der Installation vornehmen.
Der hier vorgestellte Befehl dient daher primär dem Test und der Fehlersuche, beispielsweise dem Ausprobieren eigener _Agenten-Labels_ mit der Option `--agent-labels <KEY=VALUE>`.

// ES: HTML-Darstellung angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L576
...
Größter Unterschied hier ist der geänderte Sub-Befehl `register-new` in Zusammenspiel mit Angabe des Benutzers für die Autoregistrierung.
...
Der Befehl `cmk-agent-ctl status` zeigt nun genau ein Vertrauensverhältnis zum {CMK}-Server:
...
// ES: HTML-Darstellung angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L655
...
Dabei ist im folgenden Befehl der anzugebende Universally Unique Identifier (UUID) derjenige, der beim Befehl `cmk-agent-ctl status` ausgegeben wird:

// ES: HTML-Darstellung angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L685
...
ifdef::onprem[]
Um alle Verbindungen des Hosts zu entfernen *und* zusätzlich den Legacy-Pull-Modus wiederherzustellen, geben Sie folgenden Befehl ein:

// ES: HTML-Darstellung angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L695
...
Stellen Sie zunächst im Setup, in den Eigenschaften des Hosts, den Zugriffsmodus mit der Option [.guihint]#{CMK} agent connection mode# um.
...
<span class="tok-go">[2026-05-18 23:13:32.722947 -07:00] INFO [cmk_agent_ctl] D:\y5\winagt-build\checkout\packages\cmk-agent-ctl\src\main.rs:16: starting</span>
<span class="tok-go">[2026-05-18 23:13:32.734893 -07:00] INFO [cmk_agent_ctl] D:\y5\winagt-build\checkout\packages\cmk-agent-ctl\src\lib.rs:51: Loaded config from '"C:\\ProgramData\\checkmk\\agent\\cmk-agent-ctl.toml"', connection registry from '"C:\\ProgramData\\checkmk\\agent\\registered_connections.json"'</span>
<span class="tok-go">[2026-05-18 23:13:32.736471 -07:00] DEBUG [reqwest::connect] C:\Users\jenkins\.cargo\registry\src\index.crates.io-1949cf8c6b5b557f\reqwest-0.12.23\src\connect.rs:882: starting new connection: https://mycmkserver/</span>
<span class="tok-go">[2026-05-18 23:13:53.755340 -07:00] INFO [cmk_agent_ctl::site_spec] D:\y5\winagt-build\checkout\packages\cmk-agent-ctl\src\site_spec.rs:254: Failed to discover agent receiver port using https.</span>
<span class="tok-go">[2026-05-18 23:13:53.767766 -07:00] DEBUG [cmk_agent_ctl::site_spec] D:\y5\winagt-build\checkout\packages\cmk-agent-ctl\src\site_spec.rs:255: https error: "Failed to discover agent receiver port from https://mycmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke\nerror sending request for url (https://mycmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke)\nclient error (Connect)\ntcp connect error\nA connection attempt failed because the connected party did not properly respond after a period of time, or established connection failed because connected host has failed to respond. (os error 10060)"</span>
<span class="tok-go">[2026-05-18 23:13:53.768905 -07:00] DEBUG [reqwest::connect] C:\Users\jenkins\.cargo\registry\src\index.crates.io-1949cf8c6b5b557f\reqwest-0.12.23\src\connect.rs:882: starting new connection: http://mycmkserver/</span>
<span class="tok-go">[2026-05-18 23:14:14.815083 -07:00] INFO [cmk_agent_ctl::site_spec] D:\y5\winagt-build\checkout\packages\cmk-agent-ctl\src\site_spec.rs:254: Failed to discover agent receiver port using http.</span>
<span class="tok-go">[2026-05-18 23:14:14.815471 -07:00] DEBUG [cmk_agent_ctl::site_spec] D:\y5\winagt-build\checkout\packages\cmk-agent-ctl\src\site_spec.rs:255: http error: "Failed to discover agent receiver port from http://mycmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke\nerror sending request for url (http://mycmkserver/mysite/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke)\nclient error (Connect)\ntcp connect error\nA connection attempt failed because the connected party did not properly respond after a period of time, or established connection failed because connected host has failed to respond. (os error 10060)"</span>
<span class="tok-go">[2026-05-18 23:14:14.815937 -07:00] ERROR [cmk_agent_ctl] D:\y5\winagt-build\checkout\packages\cmk-agent-ctl\src\main.rs:35: Failed to discover agent receiver port from Checkmk REST API, both with http and https. Run with verbose output to see errors.</span></code></pre></div></div>
</code></pre>
    </div>
</div>
++++
...
Nach Absetzen des Befehls `cmk-agent-ctl.exe register` fragt der Agent Controller zunächst über die REST-API den {CMK}-Server nach dem Port des Agent Receivers.
...
// TK: Alter Inhalt des Abschnitts folgt:
////
Schlägt die Registrierung eines Hosts im Monitoring fehl, ohne dass überhaupt das Zertifikat angezeigt wird, hilft Kenntnis der involvierten Netzwerkkommunikation bei der Identifizierung des Problems -- und natürlich bei der Behebung.

Nach Absetzen des Befehls `cmk-agent-ctl register` fragt der Agent Controller zunächst mittels REST-API den {CMK}-Server nach dem Port des Agent Receivers.
...
// ES: HTML-Darstellung angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L846
...
Wenn Sie die Agentenausgabe lokal untersuchen wollen, verwenden Sie den Agent Controller im _Dump-Modus_ (Sub-Befehl `dump`).
...
Sie können ihn mit der Taste kbd:[Q] verlassen:

// ES: HTML-Darstellung angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L890
...
// Anders als im Linux-Artikel wird hier der Befehl auf dem {CMK}-Server ausgeführt, da es unter Windows kein nc gibt.
[{shell}]
----
...
erhalten Sie mit diesem Befehl statt der `16` die komplette unverschlüsselte Agentenausgabe.
...
In aktuellen Versionen von Windows können Sie die [.guihint]#Windows Defender Firewall with Advanced Security# über die Windows Einstellungen ([.guihint]#Settings > Windows Security#) finden oder über den Aufruf von `wf.msc` über die Befehlszeile starten:
...
// Bei der Agent-Installation wird das Profile All gewählt (Befehlszeile: profile=any)
|[.guihint]#Name# |Geben Sie der Regel einen prägnanten und kurzen Namen.
|====

ifdef::onprem[]
Alternativ können Sie auch diesen Schritt automatisieren und die Regel direkt auf der Befehlszeile setzen.
Passen Sie den folgenden Befehl gegebenenfalls Ihrem angepassten Installationspfad an:

// ES: HTML-Darstellung angelehnt an vorigen Block-Inhalt https://github.com/Checkmk/checkmk-docs/blob/5765a90fbd731ea099e6be2e9d29cc355f06c919/src/common/de/agent_windows.asciidoc?plain=1#L989
...
[TIP]
====
Das hier beschriebene Verhalten betrifft vor allem Benutzer, die Hosts ins Monitoring von {CE-saas} aufnehmen (zum Beispiel zu Evaluationszwecken), die sich bereits bei On-Premises-Editionen von {CMK} im Monitoring befinden.
====
endif::[]
...
Im ersten Schritt schalten Sie in der vorhandenen Regel unter [.guihint]#Setup > Agents > Access to agents > {CMK} agent > Symmetric encryption (Linux, Windows)# die Verschlüsselung ab.
...
ifdef::onprem[]
Als Nutzer einer der kommerziellen Editionen können Sie über [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Windows agent options > Disabled sections (Windows agent)# einfach eine Regel anlegen, welche dann von der xref:glossar#agent_bakery[Agentenbäckerei] berücksichtigt wird.
endif::[]

ifdef::saas[]
Sie können über [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Windows agent options > Disabled sections (Windows agent)# einfach eine Regel anlegen, welche dann von der xref:glossar#agent_bakery[Agentenbäckerei] berücksichtigt wird.
endif::[]
...
* Das Plugin ist so speziell, dass es von den meisten Anwendern nicht benötigt wird (Beispiel: `citrix_licenses.ps1`).
...
// ES: mssql.vbs ist legacy und wurde deshalb nicht zu .ps1 portiert (https://checkmk.com/werk/17669). Wer kennt ein anderes Beispiel für optionale Konfigurationsdateien, um diesen Satz zukunftssicherer zu machen?
...
----
// ES: Ab {CMK} 2.6.0 sollen keine vbs-Plugins mehr ausgeliefert werden, schon in 2.5.0 sind nur noch Legacy-Plugins auf vbs. Die Dateiendung hier in der Konfigurationsdatei darf so lange noch bleiben, sollte aber zum nächsten Major-Release entfernt werden.

*Wichtig*: Der Einsatz solcher Plugins setzt voraus, dass die Dateien auch in einer regulären Befehlszeile ohne spezielle Pfade aufgerufen werden können.
...
.Die Service-Erkennung findet den neuen Service
image::agent_windows_service_discovery.png[alt="Seite der Service-Erkennung mit dem neuen Service."]
...
Über die Befehlszeile für Administratoren haben Sie mehrere Möglichkeiten den Agenten zu entfernen.
