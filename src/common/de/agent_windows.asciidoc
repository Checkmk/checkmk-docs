...
// 6.4 statt 10.0 wurde nur bei Windows 10 Technical Preview 1-3 ausgegeben, das sollte für uns keine Rolle spielen.
...
// Dieses REST-API-Beispiel sollte in den REST-API-Artikel verschoben werden.
...
ifdef::saas[]
In {CE} (SaaS) können Sie sich im [.guihint]#Onboarding Guide# unter [.guihint]#Add hosts > Command line download and install# nach Anklicken des Windows-Icons das PowerShell-Kommando anzeigen lassen, mit dem Sie den für Autoregistrierung im Ordner `preconfigured_agent` vorbereiteten Agenten herunterladen.
...
// Das ist nicht ganz nett, weil im Linkziel zwar die Unterschiede zwischen {v15} und {v16} erklärt werden, aber nicht explizit die 3 angebotenen Auswahlmöglichkeiten.
...
// Da der Checkmk-Server nicht beeinflusst werden kann, hier mal was zur Firewall. Aus agent_linux kopiert.
ifdef::saas[]
...
////
Kaum noch relevant, da hier Verschlüsselung einsetzt, damit ist nichtmal DoS sinnvoll denkbar?
