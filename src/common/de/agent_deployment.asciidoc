...
weiter unten unter xref:scenarios[Einsatzszenarien]. Bei einem verteilten Setup mit zentraler
...
Voraussetzung dafür ist ein Setup mit xref:distributed_monitoring#[zentraler Konfiguration] 
...
// MFS: Alter Text (Verhalten vor 2.3.0:) als Referenz:
...
// Dies funktioniert nur, wenn entweder ein selbst signiertes Zertifikat eingesetzt wird oder wenn die komplette Zertifikatskette inklusive CA-Root-Zertifikat auf dem Server hinterlegt wurde.
...
// Einen ausführlichen Einblick in die gesamte Thematik bietet auch das folgende Video, das auf der {CMK} Konferenz #4 (2018) entstanden ist:
...
Wenn Sie bei der Ausführungs von `cmk-update-agent` auf einem Host die Fehlermeldung "No valid signature found." erhalten, bedeutet das im Regelfall, dass das der Agent *nicht* mit einem der xref:signature_keys[akzeptierten Schlüssel signiert] wurde.
In diesem Fall müssen Sie das xref:bakery[Backen und signieren] mit einem unter [.guihint]#Signature keys the agent will accept# ausgewählten Schlüssel wiederholen, den gebackenen Agenten im Anschluss manuell neu installieren und auch den Agent Updater erneut registrieren.

Wenn Sie `cmk-update-agent -v` ausführen, können erhalten Sie auch noch weitere Details zu diesem Verhalten.
In der detailierten Fehlermeldungen werden explizit die Signaturen aus der Kofiguration des Agent Updaters (unter Linux: `/etc/check_mk/cmk-update-agent.cfg`) angegeben, die kein akzeptiertes (sprich: konfiguriertes) Gegenstück auf Ihrem {CMK}-Server aufweisen
...
Die folgenden Berechtigungen sind für die Verteilung individuell konfigurierter Agentenpakete erforderlich.

include::include_permission_add_or_modify_executables.asciidoc[]
////
