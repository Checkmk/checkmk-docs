...
// TK: Es wird doch nur noch das Serverzertifikat importiert (und diesem vertraut). Dann stimmt doch der obige Absatz nicht mehr und muss umgeschrieben werden...

[IMPORTANT]
====
Beachten Sie:

. Wird ein Zertifikat auf diese Weise importiert, muss der Anwender die Authentizität des Servers selbst sicherstellen, da das Zertifikat aus keiner separaten Quelle stammt.
. Da nur das Serverzertifikat importiert wird, das mitunter sehr kurzlebig ist, sollten Sie gegebenenfalls rechtzeitig Zertifikate über die Agentenbäckerei bereitstellen.
====
