...
// TK: Auskommentiert, denn warum soll ich zuerst umständlich über die Global settings die Einstellung setzen, die ich in Kap. 1.5 durch einen einfachen Klick auf Master switch erledigen kann?
////
...
ifdef::onprem[]
image::agent_deployment_activation_of_automatic_agent_updates.png[alt="Globale Einstellung zur Aktivierung des Agent Updaters."]
endif::[]
ifdef::saas[]
image::agent_deployment_activation_of_automatic_agent_updates_saas.png[alt="Globale Einstellung zur Aktivierung des Agent Updaters."]
endif::[]
////
...
Wie Sie dabei vorgehen, wird im Folgenden nur skizziert - mehr Informationen zur Erstellung einer xref:wato_user#roles[Rolle] und eines xref:wato_user#user_config[Benutzers] erhalten Sie im Artikel zur Benutzerverwaltung.
// TK: Braucht es den vorherigen Satz? Die Beschreibung unten ist jetzt doch sehr ausführlich geworden.
...
// Obacht: Der folgende Code-Block(!) enthält mehrere ifdefs! Oder doch besser 2 separate Code-Blocks für onprem und saas?
[{shell}]
----
...
----
////
// TK: In der vorherigen Ausgabe werden die folgenden 3 Zeilen nur dann gezeigt, wenn mit einem Benutzer mit eingeschränkten Berechtigungen registriert wird (so hab ich es jedenfalls verstanden):
Note: The host "localhost" is currently not known in the active monitoring configuration.
You can however add this host to the monitoring later on without having to register again.
Please check the exact spelling if you intended to register an existing host.
// TK: Matthew schreibt dazu in der KB: This is totally fine, the warning is generated due to the fact that the registration role you have used have very limited permissions.
// TK: Wer kann in den 3 Zeilen Ausgabe den Zusammenhang zu eingeschränkten Berechtigungen erkennen?
// TK: Vor allem ist mein Host localhost sehr wohl im Setup und im Monitoring vorhanden.
// TK: Müssen wir auch was dazu schreiben, und wenn ja, was?
////
// TK: Nochwas: Obwohl ich mit agent_updater_registration als Automationsbenutzer registriert habe, steht in der generierten Ausgabe oben (letzte Zeile) die Option -P für Password und nicht -S für Secret.
// TK: Getestet: Ich kann -P und -S verwenden, sowohl mit cmkadmin als auch mit agent_updater_registration. 
...
// TK: Obacht: Auch der folgende Code-Block enthält mehrere ifdefs! Oder doch besser 2 separate Code-Blocks für onprem und saas?
[{shell}]
----
...
// TK: Obacht: Auch dieser Code-Block enthält wieder mehrere ifdefs! Oder doch besser 2 separate Code-Blocks für onprem und saas?
[{shell}]
----
C:\WINDOWS\system32> "C:\Program Files (x86)\checkmk\service\check_mk_agent.exe" updater register ^
...
-v
----
// TK: Obacht: Es ist mir nicht gelungen, die Registrierung unter Windows zu testen (meine Kommandos laufen beim Verbindungsaufbau nach 120sec in einen Timeout).
// TK: Da jetzt in onprem und saas per Automationsbenutzer registriert wird, hab ich im obigen Kommando einfach -P durch -S ersetzt.

Mit der Option `-S` wird das Automationspasswort des Automationsbenutzers übergeben.


==== Generelle Hinweise zur Registrierung
// TK: Ich hab die alte Überschrift (Einmalige Registrierung und generelle Hinweise) umgeschrieben.

// TK: Absatz auskommentiert, da jetzt eh nur noch per Automationsbenutzer registriert wird.
////
...
// TK: Folgenden Listeneintrag überarbeitet:
* Die Anmeldung zur Registrierung ist nur einmal erforderlich.
...
. Registrieren Sie den Host am {CMK}-Server mit dem Aufruf von `cmk-update-agent register`. Hier bietet es sich an, die benötigten Registrierungsinformationen per Befehlszeile direkt zu übergeben. Vor allem, wenn Sie ein Installationsskript verwenden wollen. Die entsprechenden Optionen können Sie sich beim Aufruf von `cmk-update-agent register --help` anzeigen lassen.
...
An dieser Stelle muss die URL angegeben werden, unter der die Zentralinstanz von der Remote-Instanz aus erreichbar ist, inklusive Protokoll und `/check_mk/`, also z.B. `\https://myserver.org/mysite/check_mk/`. Während die {CMK}-Instanz versucht, alle anderen fehlenden Einstellungen selbst herauszufinden, ist die Angabe dieser URL nicht optional, da diese Verbindungsrichtung bei einer zentralen Konfiguration  sonst nicht zustande kommt.
...
// TK: Den Rest des Absatzes auskommentiert. Das Feld heißt jetzt "Automation user for connection to central site (deprecated)" und ist für die 2.4.0 nicht mehr relevant.
////
...
selbst und fügen Sie die folgende Zeile der Datei mit folgendem Befehl hinzu:
...
Es ist vereinzelt aufgetreten -- insbesondere auf Red Hat/CentOS-Systemen --
