...
Wenn Sie bei der Ausführung von `cmk-update-agent` auf einem Host die Fehlermeldung "No valid signature found." erhalten, bedeutet das im Regelfall, dass das der Agent *nicht* mit einem der xref:signature_keys[akzeptierten Schlüssel signiert] wurde.
...
In der detaillierten Fehlermeldungen werden explizit die Signaturen aus der Konfiguration des Agent Updaters (unter Linux: `/etc/check_mk/cmk-update-agent.cfg`) angegeben, die kein akzeptiertes (sprich: konfiguriertes) Gegenstück auf Ihrem {CMK}-Server aufweisen
