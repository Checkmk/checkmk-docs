...
ifdef::saas[]
Belassen Sie diese Einstellung für {CE} auf [guihint]#64bit packaged binary#.
endif::[]
...
die Sie am schnellsten finden, wenn Sie mit [.guihint]#Find on this page# auf der langen Seite der Berechtigungen danach suchen:
...
Klicken Sie [.guihint]#Automation secret for machine accounts#, um den neuen Benutzer als xref:glossar#automation_user[Automationsbenutzer] zu erstellen und würfeln Sie ein Automationspasswort aus, das dabei in die Zwischenablage kopiert wird.
Dieses Automationspasswort wird von Ihnen bei der Registrierung angefordert.
...
// Vorsicht beim Update der Befehlsausgabe: Der folgende Code-Block enthält mehrere ifdefs!
[{shell}]
----
...
[cols="15,60,~",options="header"]
|===
|Ort |Pfad |Bedeutung
|Linux-/Unix-Host |`/usr/lib/check_mk_agent/plugins/3600/cmk-update-agent` |Das Agent-Updater-Plugin als Binärdatei oder Skript, abhängig von der Konfiguration als xref:executable_format[ausführbares Format].
Das Unterverzeichnis `3600` steht dabei für das xref:interval_for_update_check[Intervall für den Update-Check] in Sekunden (hier für den Standardwert von einer Stunde).
|Windows-Host |`C:\ProgramData\checkmk\agent\plugins\cmk_update_agent.checkmk.py` |Das Agent-Updater-Plugin als Python-Datei.
|Linux-/Unix-Host |`/usr/bin/cmk-update-agent` |Skript zum Aufruf des Agent-Updater-Plugins und zur Registrierung des Agenten mit dem Befehl `cmk-update-agent register`.
|Windows-Host |`C:\Program Files (x86)\checkmk\service\check_mk_agent.exe` |Agentenprogramm zur Registrierung des Agenten mit dem Befehl `check_mk_agent.exe updater register`.
|Linux-/Unix-Host |`/etc/check_mk/cmk-update-agent.cfg` .2+|Konfigurationsdatei des Agent-Updater-Plugins, die die Einstellungen der Regel [.guihint]#Agent updater (Linux, Windows, Solaris)# enthält.
Editieren Sie diese Datei nicht, denn Sie wird bei der Installation und beim Update geschrieben.
|Windows-Host |`C:\ProgramData\checkmk\agent\config\cmk-update-agent.cfg`
|Linux-/Unix-Host |`/etc/cmk-update-agent.state` .2+|Datei mit Registrierungsinformationen inklusive des _Host Secrets_.
|Windows-Host |`C:\ProgramData\checkmk\agent\config\cmk-update-agent.state`
|Linux-/Unix-Host |`/var/lib/check_mk_agent/cmk-update-agent.log` .2+|Ausführliche Log-Datei mit DEBUG-Meldungen.
|Windows-Host |`C:\ProgramData\checkmk\agent\log\cmk-update-agent.log`
|Windows-Host |`C:\ProgramData\checkmk\agent\bakery\check_mk.bakery.yml` |Von der Agentenbäckerei erstellte Konfigurationsdatei, die u.a. das Intervall für den Update-Check definiert.
|===
