...
Im einfachsten Fall genügt es, wenn Sie Ihre Agenten mit Hilfe der Funktion [.guihint]#Sign agents# in der Agentenbäckerei mit einem der Schüssel, die unter [.guihint]#Signature keys the agent will accept# angezeigt werden, signieren.
...
In der detaillierten Fehlermeldungen werden explizit die Signaturen aus der xref:#files[Konfigurationsdatei des Agent-Updater-Plugins] angegeben, die kein akzeptiertes (sprich: konfiguriertes) Gegenstück auf Ihrem {CMK}-Server aufweisen.
