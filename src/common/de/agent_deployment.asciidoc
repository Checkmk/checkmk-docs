...
:description: In diesem Artikel erfahren Sie, wie Sie individuelle Installationspakete für Agenten mit {CMK} erstellen und bei Bedarf automatisch verteilen.
...
Der Abschnitt xref:#host_selection[Begrenzung des Updates auf bestimmte Hosts] zeigt, wie Sie festlegen und testen können, welche Hosts automatische Updates erhalten sollen.
...
// ES-todo: Screenshot neu erzeugen, um die Option "Registration" mit zu zeigen und zu erläutern (KNW-2021)
...
// ES-todo: Prüfen, ob sich hier im Zusammenhang mit KNW-2021 etwas ändert
...
// ES-todo: Abschnitt zur Option Registration einfügen (KNW-2021)

Nach dieser letzten Anpassung könnte Ihre Regel wie im folgenden Screenshot aussehen:

// ES-todo: Screenshot austauschen (KNW-2021) -> evtl. Trennung zwischen onprem/saas auflösen, wenn möglich
ifdef::onprem[]
...
// ES-todo: Alternativen beschreiben, möglicherweise mit Tabs? Die Option --automatic-updates beim Registrieren des Agent Controllers sollte der Default sein, die separate Registrierung des Agent Updaters soll aber ebenfalls beschrieben werden
...
// ES-todo: Admonition aktualisieren, da mit KNW-2021 `agent_registration` auch die Berechtigungen für das Registrieren des Updates bekommt
...
// ES-todo: checken, welche Auswirkungen KNW-2021 auf diesen SaaS-Absatz hat
ifdef::saas[]
...
ifdef::onprem[]
// ES-todo: checken, ob die folgenden Absätze überhaupt noch nötig sind. Falls nein: im gesamten Artikel agent_updater_registration durch agent_registration ersetzen
...
// ES: Ist es richtig, dass da oben mehrfach `- U` steht und nicht `-U` (ohne Leerzeichen)?
...
// ES-todo: Dies ändert die globale Einstellung [.guihint]#Enable automatic agent updates# unter [.guihint]#Setup > General > Global settings > Activation of automatic agent updates#
...
// ES-todo: neuer Screenshot nötig? (KNW-2021)
...
// ES-todo: kann dieser Hinweis verständlicher formuliert werden?
...
// ES-todo: ggf. darauf hinweisen, dass diese Felder im Screenshot nicht zu sehen sind, aber Teil der gleichen Tabelle sind
...
// ES-todo: wirklich? Es ist kein {OK} möglich??
...
// ES-todo: kann man hier mit wenig Aufwand als protocol den Wert https angeben, nachdem wir im restlichen Artikel immer wieder diese Empfehlung geben?
[{shell}]
----
...
// ES-todo: dieser Absatz ist gedoppelt, ist das Absicht? Er taucht genauso auch weiter oben unter "1.6. Automatische Updates für einen Host abschalten" auf.
Soll ein Host aus den automatischen Updates entfernt werden, so passen Sie über den Regelsatz [.guihint]#Agent updater (Linux, Windows, Solaris)# dessen Einstellung so an, dass das Update-Plugin dort deaktiviert ist.
Beim nächsten regelmäßigen Update entfernt der Agent seinen Updater dann selbst!
...
// ES-todo: Hat die neue Option "Registration" mit dem Wert "On every agent controller registration" hierauf einen Einfluss? (KNW-2021)
...
// ES-todo: Hat sich dieser Abschnitt mit KNW-2021 erübrigt, kann er gelöscht werden? Bedient er genau das gleiche Ziel?
ifdef::onprem[]
[#automatic_installer]
...
// ES-todo: Gilt das weiterhin? Warum (nicht)?
...
// ES-todo: Besser: sie befinden sich in [.guihint]#Setup > General > Global settings# im Kasten mit dem Titel [.guihint]#Automatic agent updates#. Den Pfad auch überall im restlichen Artikel aktualisieren!
...
// ES-todo: checken, ob das im verteilten Monitoring tatsächlich so konfiguriert wird oder anders
...
// ES-todo: Ich zweifle daran, dass dieses Problem tatsächlich noch auftreten kann. Kann ich das testen? Verwandt: Werk #5507
