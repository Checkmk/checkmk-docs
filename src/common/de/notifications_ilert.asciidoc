...
[TIP]
====
In ilert werden nur die xref:notifications#environment_variables[{CMK}-Benachrichtigungstypen] `PROBLEM`, `ACKNOWLEDGEMENT` und `RECOVERY` verarbeitet - alle anderen werden ignoriert.
====
