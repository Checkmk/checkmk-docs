...
//SK: Muss hier nicht trotzdem irgendwo {cee-only} hin? Zumindest in den onprem-Teil?
ifdef::onprem[]
...
// icon_derived_downtime.png abgelegt in check_mk zeigt das blaue Pause-Zeichen. In 2.3.0 ist aber ein anderes Symbol zu sehen, das ich nicht in git finde: blaues Dokument mit Pylone
// SK: Das Icon wird "live" aus zwei svg zusammengebaut. Der Text mit dem Pause-Symbol muss aber weg. Das Icon soll halt symbolisieren, dass nicht der Service selbst in Downtime ist, sondern eben der Host zu dem der Service gehört. Eigentlich eine sehr gute Darstellung, wie ich finde.
// SK: Ich fummel das mal als Bild zurecht. Meld dich mal, wenn du hier bist und ich das Icon noch nicht angepasst habe.
