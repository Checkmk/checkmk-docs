...
* Ein Eintrag erfolgt in die Objekt-History, sodass die Aktion später nachvollzogen werden kann.
...
Dazu dient die Option [.guihint]#Expire on#, die aber nur in den kommerziellen Editionen wirksam ist.
...
// TK: Wir hatten uns entschieden, in jedem Artikel eigene Screenshots zu nutzen, weil der Austausch von einem Screenshot, der öfter verwendet wird, manchmal zu unerwünschten Ergenissen führt.
// TK: Das ist ein gutes Beispiel, denn der Screenshot zeigt den Dialog für Services (denn nur dort gibt es den Knopf "Schedule downtime on service") und nicht für Hosts.
// TK: Bitte hier einen passenden Screenshot intro_command_downtime.png verwenden. Wenn du diesen Namen nimmst, braucht man die englische Datei nicht anzufassen. Dort hab ich schon den passenden Update gemacht.
...
* In Tabellenansichten wird der ausgewählte Host oder Service mit dem icon:icon_downtime[alt="Symbol zur Anzeige der Wartungszeit."] Leitkegel markiert.
Wird ein Host mit allen seinen Services in die Wartung geschickt, erhalten die Services das icon:icon_derived_downtime[alt="Symbol zur Anzeige einer abgeleiteten Wartungszeit bei einem Service."] blaue Pause-Zeichen.
