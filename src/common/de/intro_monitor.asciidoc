...
Zwar zeigen Ihnen bereits die standardmäßig verfügbaren xref:glossar#view[Tabellenansichten] oder z.B. das Snapin [.guihint]#Overview# sehr genau, wie viele und welche Probleme es gerade gibt.
...
Sie bleiben aber auf der Seite mit der Liste, da Sie hier alle Optionen haben, um nur ein Problem oder gleich mehrere auf einmal zu quittieren.
...
[TIP]
====
Wenn Sie auf einer Seite mit einer Liste eine Aktion durchführen, ohne dass Sie Checkboxen eingeblendet haben, dann wird diese Aktion für *alle* Listeneinträge durchgeführt.
====
...
.Die Rückfrage vor der Anwendung
...
ifdef::onprem[]
Dazu dient die Option [.guihint]#Expire on#, die aber nur in den kommerziellen xref:glossar#edition[Editionen] verfügbar ist.
endif::[]
ifdef::saas[]
