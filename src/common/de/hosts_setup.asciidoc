...
ifdef::saas[]
Das ist vor allem bei solchen Attributen nützlich, die bei großen Gruppen von Hosts gleich sind, wie z.B. die Host-Merkmale (_host tags_), mit denen Sie unter anderem festlegen, ob der Host per {CMK}-Agent überwacht werden soll.
endif::[]
...
|4. |Über den xref:glossar#rule_set[Regelsatz] [.guihint]#Hosts with dynamic DNS lookup# bestimmen Sie Hosts für ein dynamisches DNS. Das Resultat ist analog zu Möglichkeit 3, nur dass für die DNS-Anfrage jetzt das Feld [.guihint]#Hostname# verwendet wird. |Beim Ausführen der Checks
|===
...
ifdef::saas[]
Mit dieser Einstellung wird der Host agentenlos nur mit xref:glossar#active_check[aktiven Checks] überwacht.
endif::[]
...
Diese Option gibt es in {CCE} übrigens auch in den Eigenschaften eines Ordners und wird dort verwendet, um per Autoregistrierung xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen.]
Wenn Sie hier [.guihint]#Push - {CMK} agent contacts the server# auswählen, setzen Sie zusätzlich im xref:dns[Kasten Network address] die [.guihint]#IP address family# auf [.guihint]#No IP.#
...
Dies ist die einzig verfügbare Option in {CE-saas}.
...
In den On-Premises-Editionen von {CMK} ist es möglich, den Agenten auch für den xref:glossar#pull_mode[Pull-Modus] zu konfigurieren.
endif::[]
...
ifdef::saas[]
Wir raten daher, Management Boards – wenn vorhanden – mit Spezialagenten zu überwachen.
endif::[]
====
...
* Name eines Benutzers, z.B. [.guihint]#cmkadmin#: Der Host wurde manuell durch einen Benutzer erstellt, z.B. im xref:create_hosts[Setup.]
* Name eines Automationsbenutzers (im Bild oben [.guihint]#automation#): Der Host wurde durch einen xref:glossar#automation_user[Automationsbenutzer] erstellt, z.B. mit der xref:rest_api#[REST-API.]
...
ifdef::saas[]
Das ist bei Services nützlich, die sich bei der Erkennung einen aktuellen Zustand merken (z.B. den aktuellen Zustand von Dateisystemen und ihren Mountpoints).
endif::[]
