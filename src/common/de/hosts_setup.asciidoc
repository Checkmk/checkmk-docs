...
Dies gilt für Hosts, die über einen xref:glossar#special_agent[Spezialagenten] abgefragt werden und für den Push-Modus des {CMK}-Agenten in {CCE}.
...
Sie passt aber besonders gut als Gegenstück zur Möglichkeit, xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen], die im Funktionsumfang von {CCE} enthalten ist.
