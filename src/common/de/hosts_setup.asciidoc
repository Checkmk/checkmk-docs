...
{CMK} wurde von Anfang an für Umgebungen mit sehr vielen Hosts entworfen und hat einen ganz eigenen Ansatz, um die Konfiguration solcher Umgebungen für den Benutzer handhabbar zu machen.
...
In der Standardansicht sehen Sie hier die Übersicht der Ordner und darunter tabellarisch die bereits im aktuellen Ordner vorhandenen Hosts.
...
SEBKIR: In einem richtigen Tutorial würden wir uns wahrscheinlich an dieser Stelle entscheiden, wie es weitergeht. Vielleicht ist eine Weiche aber auch in einem Tutorial mal okay. Was meint ihr?
...
Dagegen bringt Sie [.guihint]#Save & run connection tests# zum Verbindungstest.
Damit können Sie zuerst einmal testen, ob Sie mit den festgelegten Einstellungen überhaupt Daten von dem Host bekommen -- sei es per Agent oder was auch immer Sie vorher konfiguriert haben.
ifdef::onprem[]
...
Öffnen Sie zuerst [.guihint]#Setup > Hosts > Hosts#.
Navigieren Sie dann in einen beliebigen Ordner oder verbleiben Sie im Ordner [.guihint]#Main#.
...
SEBKIR: Egal welchen Chrakter dieser Abschnitt aktuell wirklich hat, sollte er eine Explanation werden, weil es mit ganz dolle viel Vorsicht zu genießen ist.
...
[IMPORTANT]
====
Bei Änderungen an vielen Stellen und für mehrere Hosts kann auch einmal etwas schief gehen.
ifdef::onprem[]
Vergewissern Sie sich daher, dass Sie über ein aktuelles xref:backup#[Backup] Ihrer Instanz verfügen -- bevor Sie die Umbenennungsaktion starten.
endif::[]
ifdef::saas[]
Vergewissern Sie sich daher, dass Sie über ein aktuelles xref:admin_panel#admin_site[Backup] Ihrer Instanz verfügen -- bevor Sie die Umbenennungsaktion starten.
endif::[]
====
...
Im Feld [.guihint]#Host name matching# geben Sie zunächst optional einen regulären Ausdruck an, der mit dem *Anfang* der Host-Namen übereinstimmt, die Sie umbenennen möchten -- hier im Beispiel also alle Hosts, deren Namen mit `lnx` beginnen.
...
Dabei wird auch das gegenseitige Vertrauensverhältnis (mTLS) gelöscht, so dass im Falle eines Wiedererscheinens die Registrierungsprozedur erneut durchlaufen werden muss.
...
Hosts, deren Service [.guihint]#Check_MK# über einen bestimmten Zeitraum {CRIT} ist, können Sie automatisch entfernen lassen.
...
====
ifdef::onprem[]
Diese Regel ist in allen {CMK}-Editionen verfügbar.
Sie passt aber besonders gut als Gegenstück zur Möglichkeit, xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen], die im Funktionsumfang der {CE} enthalten ist.
endif::[]
ifdef::saas[]
Diese Regel passt besonders gut als Gegenstück zur Möglichkeit, xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen].
endif::[]
...
Beachten Sie, dass auch die xref:dcd#automatic_deletion_of_hosts[dynamische Host-Verwaltung] die Möglichkeit bietet, Hosts automatisch entfernen zu lassen.
...
[{file}]
----
hostname;alias;agent;snmp_ds
lnx17.example.com;Webserver;cmk-agent
lnx18.exmpl.com;Backupserver;cmk-agent
...
// TK: Der Import der Beispieldaten funktioniert nicht (mehr): Invalid value in column 4 (tag_snmp_ds) of row 0: The selected element '' is not longer available. Please
...
|===
|Attribut |Bedeutung
|`Host name` |Name des Hosts (maximal 240 Zeichen)
...
SEBKIR: Für mich sollte der Titel "How to search for hosts in the setup" lauten und deshalb habe ich es hier einsortiert. Als Tutorial wäre m.E. etwas übertrieben.

{CMK} bietet Ihnen die komfortable Möglichkeit zum Einstieg in die zentrale xref:user_interface#search[Suche].
Sie können aber auch gezielt im [.guihint]#Monitor#-Menü und im [.guihint]#Setup#-Menü suchen.
...
Beim Feld [.guihint]#Host name# wird eine Wortteilsuche (_Infix-Suche_) verwendet -- der eingegebene Text wird, an beliebiger Stelle, im Host-Namen gesucht.
...
=== All die schönen Boxen erklärt, die mensch sieht, wenn ein Host erzeugt oder bearbeitet wird
...
Das Feld [.guihint]#Host name# dient überall innerhalb von {CMK} zur eindeutigen Identifizierung des Hosts.

.Die Grundeinstellungen eines Hosts in {CE}
...
Beachten Sie allerdings, dass {CMK} für den Host-Namen maximal eine Länge 240 Zeichen erlaubt.
...
.Dem Host einen Alias und eine IP-Adresse zuweisen
...
|1. |Sie geben _keine_ IP-Adresse an. Der Host-Name muss per DNS auflösbar sein. |Beim xref:glossar#activate_changes[Aktivierung ausstehender Änderungen]
...
|4. |Über den xref:glossar#rule_set[Regelsatz] [.guihint]#Hosts with dynamic DNS lookup# bestimmen Sie Hosts für ein dynamisches DNS. Das Resultat ist analog zu Möglichkeit 3, nur dass für die DNS-Anfrage jetzt das Feld [.guihint]#Host name# verwendet wird. |Beim Ausführen der Checks
|===
...
Dies gilt für Hosts, die über einen xref:glossar#special_agent[Spezialagenten] abgefragt werden und für den xref:glossar#push_mode[Push-Modus] des {CMK}-Agenten.
...
ifdef::onprem[]
.Konfiguration der Monitoring-Agenten in {CE}
image::hosts_new_host_agents.png[alt="Dialog mit Eigenschaften eines Hosts: Monitoring-Agenten."]
endif::[]
ifdef::saas[]
.Konfiguration der Monitoring-Agenten
image::hosts_new_host_agents_saas.png[alt="Dialog mit Eigenschaften eines Hosts: Monitoring-Agenten."]
endif::[]
...
[cols="40,~"]
|===
|[.guihint]#API integrations if configured, else {CMK} agent# |
Monitoring-Daten werden per xref:glossar#api_integrations[API-Integrationen], das heißt von Spezialagenten oder per Piggyback von anderen Hosts geliefert.
Falls xref:glossar#special_agent[Spezialagenten] konfiguriert sind, werden _alle_ ausgeführt.
Sind keine API-Integrationen konfiguriert, wird auf die Ausgabe des {CMK}-Agenten zugegriffen.
Dies ist der Standardwert.
|[.guihint]#Configured API integrations and {CMK} agent# |
Die Ausgabe eines {CMK}-Agenten wird erwartet.
Per API-Integrationen gelieferte Daten werden – falls konfiguriert – zusätzlich herangezogen.
|[.guihint]#Configured API integrations, no {CMK} agent# |
Ausschließlich per API-Integrationen gelieferte Daten werden für das Monitoring herangezogen.
|[.guihint]#No API integrations, no {CMK} agent# |
Mit dieser Einstellung wird der Host per SNMP oder agentenlos nur mit xref:glossar#active_check[aktiven Checks] überwacht.
Regeln für aktive Checks finden Sie u.a. in [.guihint]#Setup > Hosts > HTTP, TCP, Email, ...#.
Wenn Sie nicht mindestens einen aktiven Check definieren, erzeugt {CMK} automatisch einen [.guihint]#PING#-Service.
|===
...
[TIP]
====
Die Auswahl von [.guihint]#Configured API integrations, no {CMK} agent# sorgt dafür, dass die beiden Services [.guihint]#Check_MK# und [.guihint]#Check_MK Discovery# immer den Zustand {OK} haben.
...
ifdef::onprem[]
Mit [.guihint]#{CMK} agent connection mode# entscheiden Sie, ob der {CMK}-Agent im xref:glossar#pull_mode[Pull-Modus] oder im xref:glossar#push_mode[Push-Modus] arbeiten soll.
Diese Option gibt es nur in {CCE}, da nur in dieser Edition der Push-Modus zur Verfügung steht.
Diese Option gibt es in {CE} übrigens auch in den Eigenschaften eines Ordners und wird dort verwendet, um per Autoregistrierung xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen].
...
// TK: Muss man im obigen Satz noch was dazu sagen, warum der Knopf ausgewechselt wird? Der Absatz ist eh schon so länglich.
...
ifdef::saas[]
[.guihint]#{CMK} agent connection mode# legt fest, dass der {CMK}-Agent im xref:glossar#push_mode[Push-Modus] arbeiten soll.
Diese Option gibt es übrigens auch in den Eigenschaften eines Ordners und wird dort verwendet, um per Autoregistrierung xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen].
Setzen Sie zusätzlich im xref:dns[Kasten Network address] die [.guihint]#IP address family# auf [.guihint]#No IP#.
Damit verhindern Sie, dass die Erreichbarkeit des Hosts per xref:cmc_differences#smartping[Smart Ping] überprüft wird.
endif::[]
...
Das anschließende Feld [.guihint]#Piggyback# bezieht sich auf die Verwendung von xref:glossar#piggyback[Piggyback-Daten], welche durch andere Hosts „huckepack“ geliefert und als Agentenausgabe diesem Host zugeordnet werden.
Beachten Sie, dass die hier vorgenommene Einstellung zu den Einstellungen der ersten Option [.guihint]#{CMK} agent / API integrations# passen muss.
...
.Die standardmäßig verfügbaren Attribute
...
.Ein Attribut für den Hardware-Typ
...
.Ankündigung zur Abkündigung
image::hosts_setup_management_board.png[alt="Einrichtung eines Management Boards."]

[IMPORTANT]
====
Management Boards werden in einer zukünftigen {CMK}-Version nicht mehr als Eigenschaft eines Hosts unterstützt werden.
...
.Dieser Kasten wird nur für bereits erstellte Hosts angezeigt
...
Für den Namen des Erstellers gibt es im Wesentlichen die folgenden Möglichkeiten:

* Name eines Benutzers:
Der Host wurde manuell durch einen Benutzer erstellt, z.B. im xref:create_hosts[Setup].

* Name eines Automationsbenutzers (im Bild oben [.guihint]#automation#):
Der Host wurde durch einen xref:glossar#automation_user[Automationsbenutzer] erstellt, z.B. mit der xref:rest_api#[REST-API].

ifdef::onprem[]
* _Network scan_:
Der Host wurde bei einem automatischen xref:folder_network_scan[Netzwerk-Scan] gefunden.
endif::[]

Wenn der Host durch einen Automationsbenutzer erstellt wurde, ist er durch die zugehörige Instanz gesperrt ([.guihint]#Locked by#).
...
Wie Sie einen solchen Netzwerk-Scan einrichten erfahren Sie im Kapitel xref:#setup_network_scan[Netzwerk-Scan einrichten].
...
* [.guihint]#Detect network parent hosts#: xref:hosts_structure#parents_scan[Parents per Scan anlegen lassen].
...
* [.guihint]#Remove TLS registration#: Entfernt die Registrierung zur verschlüsselten Kommunikation per Transport Layer Security (TLS).
Mehr Informationen zum Thema finden Sie in den Artikeln zum xref:agent_linux#deregister[Linux-Agenten] und xref:agent_windows#deregister[Windows-Agenten].
...
SEBKIR: Haben wir uns bereits darauf geeinigt, wo wir "Dateien und Verzeichnisse" ablegen?
...
|Konfigurationsdatei für alle Hosts im Ordner [.guihint]#Main#.
Für Hosts in Unterordnern von [.guihint]#Main# gibt es jeweils eine gleichnamige Datei in den zugehörigen Unterverzeichnissen.
...
Wählen Sie [.guihint]#Setup > Hosts > Hosts#.
In der Host-Liste klicken Sie auf icon:icon_services[alt="Symbol zur Anzeige der konfigurierten Services."].

* Im Setup über die Eigenschaften eines Hosts: +
Wählen Sie [.guihint]#Setup > Hosts > Hosts#.
In der Host-Liste klicken Sie den Host an. Auf der Seite [.guihint]#Properties of host# wählen Sie im Menü [.guihint]#Host > Save & run service discovery#.
Wenn Sie stattdessen im Menü [.guihint]#Host > Run service discovery# wählen, kommen Sie ebenfalls zur Service-Liste -- ohne allerdings die Host-Eigenschaften zu speichern.

* Im Monitoring über die Liste der Services eines Hosts: +
Wählen Sie das Menü [.guihint]#Host# und dort im Abschnitt [.guihint]#Setup# den Eintrag [.guihint]#Run service discovery#.
Ein kleines Zahnradsymbol am Icon zeigt, dass dieser Menüeintrag in eine Seite im Setup führt.
So kommen Sie direkt zur Konfiguration der Services.
...
* Der Knopf [.guihint]#Accept all# ist meist die beste Aktion für einen neuen Host.
Dies ist auch die richtige Wahl für bestehende Hosts, bei denen Services gefunden wurden, die aktuell nicht überwacht werden.
Diese finden Sie im Abschnitt [.guihint]#Undecided services (currently not monitored)#.
[.guihint]#Accept all# fügt die fehlenden Services hinzu, entfernt verschwundene Services und übernimmt gefundene Host-Labels. 
...
* Nach jeder Änderung ist ein xref:glossar#activate_changes[Aktivierung ausstehender Änderungen] nötig, um diese wirksam zu machen.

[#rest_api]
=== REST-API-Anfragen für Hosts und Ordner

SEBKIR: Vielleicht genügt hier auch ein Hinweis in der Einleitung und dann ein Link zu einem Tutorial "Alle Hostnamen über die REST-API anzeigen lassen" oder so.
...
Wenn Befehlszeile, Skripte und APIs nicht Ihre Werkzeuge der ersten Wahl sind, reicht es an dieser Stelle aus, zu wissen, dass es diese API gibt.
...
Die Referenz-Dokumentation finden Sie in der xref:rest_api#access[Weboberfläche] Ihrer {CMK}-Instanz.
Dort können Sie sich mit der Syntax der Anfragen und der Struktur der Antworten vertraut machen.
Auf alle wichtigen Einträge zur REST-API können Sie in der {CMK}-Weboberfläche über die Navigationsleiste im Menü [.guihint]#Help > Developer resources > REST API# zugreifen.
