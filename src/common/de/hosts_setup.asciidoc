...
// FIXME 2.4.0 SaaS: Alle Spezialagenten werden ab 2.4.0 ausgeführt.
ifdef::onprem[]
...
Wenn Sie nicht mindestens einen aktiven Check definieren, erzeugt {CMK} automatisch einen PING-Service.
|===

[TIP]
====
Die Auswahl von [.guihint]#Configured API integrations, no Checkmk agent# sorgt dafür, dass die beiden Services [.guihint]#Check_MK# und [.guihint]#Check_MK Discovery# immer den Zustand {OK} haben.
...
Diese Option gibt es nur ab {CCE}, d. h. in {CE} und {ME}, da nur in diesen Editionen der Push-Modus zur Verfügung steht.
Diese Option gibt es ab {CCE} übrigens auch in den Eigenschaften eines Ordners und wird dort verwendet, um per Autoregistrierung xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen.]
...
ifdef::saas[]
[.guihint]#Checkmk agent connection mode# legt fest, dass der {CMK}-Agent im xref:glossar#push_mode[Push-Modus] arbeiten soll.
...
Sie passt aber besonders gut als Gegenstück zur Möglichkeit, xref:hosts_autoregister#[Hosts automatisch erstellen zu lassen], die im Funktionsumfang ab {CE} enthalten ist.
endif::[]
ifdef::saas[]
...
|===
|Attribut |Bedeutung
|`Host name` |Name des Hosts (maximal 253 Zeichen)
