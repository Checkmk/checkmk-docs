...
// MFS: Da zudem Ping (also das ICMP-Ding) z.B. bei Windows nicht funktioniert, werden da gerade smartere Tests evaluiert (Route, ARP Cache...), daher würde ich eher formulieren:
...
Präziser geschrieben, nutzt {CMK} den Befehl `nslookup` und nur dann, wenn dieser nicht verfügbar ist, den Befehl `ping`.
// MFS: Ich würde diesen Satz weglassen. Implementierungsdetails, die mglw. bereits jetzt schon nicht mehr stimmen, weil wir für sowas normalerweise eher verfügbare Python-Module verwenden, als Systemtools zu verwenden, die jede Distribution in einer anderen Version mitbringt.
// MFS: Zudem liest nslookup auch die /etc/hosts aus. Um sicher auf DNS zu gehen, müsste man dig verwenden.
...
// TK: Folgender Absatz passt nicht zur Überschrift, aber für diese Kurzinfo wollte ich keinen neuen Abschnitt aufmachen.
...
Wenn Sie aber eine IP-Adresse eintragen, pingt {CMK} nicht länger den Host-Namen, sondern diese IP-Adresse.
...
Dazu wird Ihnen ein Slideout eingeblendet, das Sie durch die notwendigen Schritte führt.
// TK: Das muss im Leitfaden noch umgeschrieben werden, dann kann dorthin verlinkt werden.
// Dazu wird Ihnen ein Slideout eingeblendet, das Sie durch die notwendigen Schritte führt, die im xref:intro_setup_monitor#first[Leitfaden für Einsteiger] genau beschrieben sind.
endif::[]
...
// TK: Hier fehlt noch die neue Option OpenTelemetry data. Im Artikel opentelemetry hab ich noch nichts dazu gefunden.
...
* Name eines Benutzers: Der Host wurde manuell durch einen Benutzer erstellt, z.B. im xref:create_hosts[Setup].
