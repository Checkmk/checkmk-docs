...
. Öffnen Sie mit [.guihint]#Create# die Eingabe für die Parameter.
. Geben Sie unter [.guihint]#Parameter properties# die kopierte Webhook-URL ein:
+
...
+
Hinweis: Hier dürfen nur URLs, die mit `https://` beginnen verwendet werden.
