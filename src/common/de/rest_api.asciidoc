...
// ES-review: Swagger bzw. die interaktive GUI der REST-API erwähne ich hier absichtlich nicht, weil es den zuständigen Devs sehr wichtig ist, dass die in ReDoc einsehbare Doku die "source of truth" ist.
...
.Im Hilfe-Menü zur REST-API finden Sie Ressourcen zu allen von Ihrer Instanz unterstützten Versionen der REST-API
...
// ES-review: Hab den Rahmen weggemacht, weil er mit der BU doof aussah. OK?
// [{image-border}]
...
Damit Sie dennoch Zugriff auf derartige Backports haben, liefert {CMK} ab Version {v25} die REST-API nicht nur in allen unterstützten Versionen (z.B. `v1`) aus, sondern zusätzlich in einer weiteren, _nicht unterstützten_ Version namens `unstable`.
...
ifdef::saas[]
Im Zuge der ständigen Weiterentwicklung von {CMK} wird auch die REST-API immer wieder erweitert und angepasst.
...
.Im Hilfe-Menü zur REST-API finden Sie auch Ressourcen zur `unstable`-Version
image::restapi_help_menu.png[alt="Help-Menü in der Navigationsleiste.",width=60%]
