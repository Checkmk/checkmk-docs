...
// ES-review: Please review for KNW-1797 in comparison to the version of this article at commit b625d657b314fa9e1e65864463e989aa5b926c18

// ES-review: In einer früheren Version dieses Artikels (https://github.com/Checkmk/checkmk-docs/blob/b625d657b314fa9e1e65864463e989aa5b926c18/src/common/de/rest_api.asciidoc?plain=1#L207) gab es Unstimmigkeiten zwischen onprem und saas in Bezug auf die verwendeten Prompts.
// Im Abschnitt "Anfragen stellen per Skript" wurden in beiden Handbüchern Instanzbenutzer-Prompts ({c-omd}) verwendet, die in der saas aber gar nicht nutzbar sind.
// Ich habe mich entschieden, hier eine neue lokale Variable einzuführen, um nicht jeden einzelnen Prompt im Artikel doppeln zu müssen.
// Die folgenden Zeilen haben den Effekt, dass das Makro {localprompt} im onprem-Handbuch genauso aufgelöst wird wie {c-omd}, während das Makro {localprompt} im saas-Handbuch so aufgelöst wird wie {c-user}.
ifdef::onprem[]
:localprompt: {c-omd}
endif::[]
ifdef::saas[]
...
// ES-review: Wer mit meiner Lösung gar nicht leben kann, möge bitte ein neues Ticket aufmachen "Remove {c-omd} prompts from rest_api". Es ist mit relativ geringem Aufwand und überschaubarem Verlust möglich, alle Prompts auf {c-user} umzustellen.
// ES-review @MFS: Initiale Tests deuten darauf hin, dass Variablen, die innerhalb eines Artikels definiert sind, auch nur innerhalb dieses Artikels verfügbar sind. Darf ich mich darauf verlassen? Sonst packe ich einfach ein Präfix hier dran, das den Artikel eindeutig kennzeichnet.
// TK: Als ich mir im Pleistozän den Artikel ausgedacht hab, hatte die Evolution die saas noch gar nicht auf ihrem Zettel.
// TK: Wenn wir für die sass jetzt user@host brauchen, dann mach das doch jetzt auch für onprem und schmeiss OMD[mysite] raus. Das erleichert den Pflegeaufwand.
// TK: Allerdings brauchst Du dann die schlau ausgedachten lokalen Variablen nicht mehr. Entscheide Du, was leichter umzusetzen ist.
...
// ES-review: Ich finde die Formulierung hier nicht so toll, weiß aber nicht, wie ich stattdessen die Parallele zu "bis v24" ausdrücken könnte. Ideen?
// TK: Erstmal super, dass Du überhaupt an die saas gedacht hast. Leider keine besseren Ideen. Natürlich könnte man versuchen zu begründen, dass eine saas-Version entweder 2.4 oder 2.5 ist. Aber das wollen wir ja eigentlich nicht. Also lass es so. 
endif::[]
...
// TK: Der vorherige Satz ist schwer verständlich. Ist folgender besser? 
// TK: Eine Major-Version der REST-API und damit die URL ändert sich nur bei inkompatiblen Änderungen. 
...
// TK: und danach > und auch folgende Major-Versionen wie `v2`, `v3` etc.
...
// TK: als neue Major-Versionen > als eine neue Major-Version (weil in einer CMK Patch-Version wird es sicher nur eine neue REST-API Major geben)
...
// ES-review: Bisher nicht möglich, aber vielleicht eines Tages: WD-1345
endif::[]
...
// TK: Sicher? Wenn nicht, dann könnte man den Satz auch einfach streichen.
endif::[]
...
// TK: über den empfohlenen Umgang? In update_major steht bisher nichts über den Umgang. Vielleicht so: erfahren Sie, welche REST-API-Versionen in der {CMK}-Version {current-major} unterstützt werden.
endif::[]
...
Der Funktionsumfang dieser API-Version kann in jeder {CMK}-Version unterschiedlich sein.

Mehr über die `unstable`-Version der REST-API erfahren Sie im entsprechenden xref:#unstable[Abschnitt] dieses Artikels.
====
// TK: Ab dem 2. Satz im vorherigen TIP steig ich aus.
// TK: Mir ist nicht klar, was in diesem Zusammenhang Backports sein können.
// TK: in älteren {CMK}-Versionen? Warum jetzt {CMK}-Versionen? Müsste hier nicht stehen: in den unterstützten REST-API-Versionen?
// TK: Update: Nachdem ich Kap. 5. Die unstable-Version der API nutzen gelesen hab, seh ich etwas Licht im API-Tunnel.
// TK: Vielleicht hab ja nur ich ein Problem mit Backports, aber ich würde an dieser Stelle kürzen und die Backports exklusiv Kap. 5 überlassen.
// TK: Mein TIP-Vorschlag:
////
Zusätzlich zu den unterstützten Major-Versionen der REST-API haben Sie auch die Möglichkeit, die Version `unstable` mit Ihren API-Anfragen anzusteuern.
Diese Version bietet Ihnen beispielsweise Zugriff auf Funktionen, die sich aktuell noch in der Entwicklung befinden,
Wie der Name schon andeutet, wird die `unstable`-Version nicht unterstützt und bietet keine Garantie auf Kompatibilität.
Mehr über die `unstable`-Version der REST-API erfahren Sie im entsprechenden xref:#unstable[Kapitel] dieses Artikels.
////
...
ifdef::onprem[]
Wenn Ihre {CMK}-Version mehrere Versionen der REST-API unterstützt, stehen hier alle unterstützten Versionen der REST-API zur Auswahl.
endif::[]
ifdef::saas[]
Hier stehen alle zum aktuellen Zeitpunkt von {CE} unterstützten Versionen der REST-API zur Auswahl.
// TK: Warum hier ifdef::saas? Der onprem-Satz passt doch auch für die saas, auch wenn da {CMK}-Version steht.
endif::[]
...
// TK: von Ihrer Instanz eher streichen, weil die Abhängigkeit kommt ja von der CMK-Version
...
Sie wurde beim Anlegen Ihrer Instanz mitgeliefert, wurde aber separat mit ReDoc generiert und ist deshalb nicht als Unterseite der {CMK}-Web-GUI zu verstehen.
// ES-review: Interessiert das die Lesenden hier? Ich will eigentlich nur begründen, warum die Seite extern geöffnet wird und komplett anders aussieht als sowohl die Web-GUI als auch das Handbuch.
// TK: Ok, aber dann schreib es doch hin: und sieht daher anders aus als die Seiten der {CMK}-Web-GUI. 
...
// TK: Achja, ich glaub für die saas musst Du Webserver rauskicken.
...
// TK: Nunja, bei onprem findet man da jetzt nur noch den Automationsbenutzer agent_registration, mit dem man aber wenig anfangen kann, weil der quasi nix darf. Vielleicht im nächsten onprem-Teil hinzufügen, dass man sich erstmal einen schönen Automationsbenutzer schnitzen soll?
ifdef::saas[]
...
Die API-Dokumentation, die Sie mit [.guihint]#Help > Developer resources > REST API > Version 1 > Documentation# öffnen können, enthält im Abschnitt „Queries through the REST API“ ein Beispiel eines Skripts für diese Aufgabe.
// TK: Der GUI-Pfad kann hier weg. Man könnte im ersten Satz API-Dokumentation verlinken: xref:access[API-Dokumentation]. Oder Thema erledigt sich eh mit KNW-1719.
...
[{shell}]
----
{localprompt} touch create_host.py service_discovery.py show_pending_changes.py activate_changes.py
{localprompt} chmod +x create_host.py service_discovery.py show_pending_changes.py activate_changes.py
----
--
...
[{shell}]
----
{localprompt} touch create_host.sh service_discovery.sh show_pending_changes.sh activate_changes.sh
{localprompt} chmod +x create_host.sh service_discovery.sh show_pending_changes.sh activate_changes.sh
----
--
====
...
Die im weiteren Verlauf dieses Artikels gezeigten API-Aufrufe verwenden den in der API-Dokumentation enthaltenen Beispiel-Code für Python mit dem Modul `requests`.
Alternativ dazu finden Sie für jedes Beispiel auch eine Curl-Variante.
Inhaltlich sind die alternativen Implementierungen deckungsgleich.
Zu möglichen Unterschieden im Verhalten der verwendeten Werkzeuge, zum Beispiel in Bezug auf Redirects, empfehlen wir einen Blick in die Dokumentation des jeweiligen Tools.
...
+
--
// ES-review: Installationshinweise zu Python muss ich hier aber nicht geben, oder?
// TK: Nein. Aber ein grober Hinweis wär schon schön, dass man eine Python-Umgebung braucht (im 1. Absatz nach dem 1. Satz). Wie man das formuliert, weisst Du viel besser als ich.
Für die `requests`-Beispiele legen Sie eine Reihe von Python-Skripten an und machen diese ausführbar.
...
// TK: Ich hoffe, Du verzeihst mir, dass ich die ganzen Beispiele nicht getestet habe ;-)
...
// TK: Mattias hatte vorgeschlagen Beispiel-IPs aus für die Doku reservierten Adressbereiche zu verwenden:
// TK: https://docs.google.com/document/d/1S948ptXcQ6Bbncs00ZENvmAXXD1GSSmLkmvnK7pts0U/edit?pli=1&tab=t.0#heading=h.3uc1qcy4nf6m
// TK: Ist aber nicht wichtig.
...
----
{localprompt} python3 create_host.py 
...
// TK: Dunkle Erinnerung: Es gibt/gab(?) Probleme mit dem mode tabula_rasa. Hab aber in Slack nix Vernünftiges gefunden. SK fragen?
// TK: Im Beispiel-Code wird jetzt(?) refresh statt tabula_rasa verwendet. Mein Vorschlag: Hier nichts verändern und auch refresh nehmen.
...
[{shell}]
----
{localprompt} python3 service_discovery.py
Done
----
...
[{shell}]
----
{localprompt} ./service_discovery.sh 
...
Ergänzen Sie außerdem eine Zeile mit dem Befehl `pprint.pprint(dict(resp.headers))`, um ein Python-Dictionary mit allen Header-Informationen ausgeben zu lassen.
Eine weitere Zeile mit dem Befehl `print("xxx-status_code={}".format(resp.status_code))` gibt zusätzlich den HTTP-Status-Code aus.
// ES-review: Hilfe, die Beschreibung der zwei zusätzlichen Befehle ist zu holprig. Irgendwelche Vorschläge?
// TK: Hilfe naht! Vielleicht so: Ergänzen Sie außerdem zwei Zeilen mit `print`/`pprint`-Befehlen um ein Python-Dictionary mit allen Header-Informationen und den HTTP-Status-Code ausgeben zu lassen.
...
----
{localprompt} python3 show_pending_changes.py 
{...
...
----
{localprompt} ./show_pending_changes.sh 
HTTP/1.1 200 OK
...
[{shell},highlight=30]
----
{localprompt} python3 activate_changes.py 
{'domainType': 'activation_run',
...
[{shell},highlight=21]
----
{localprompt} ./activate_changes.sh
{
    "links": [
        {
            "domainType": "link",
...
// TK: Vielleicht hier einen Abschlussatz, dass hiermit das total einfache Beispiel abgeschlossen ist und man den Erfolg in der CMK-GUI bewundern kann?


[#rest_api_gui]
=== Anfragen stellen mit der interaktiven GUI der REST-API
// ES-review: In früheren Versionen stand in diesem Abschnitt immer "die (interaktive) REST-API GUI". Ich finde die Formulierung wie hier in der Überschrift lesbarer - Feedback? In der Weboberfläche wird diese Ressource als "Interactive GUI" im Menü für die REST-API verlinkt.
// TK: Passt!
// TK: Vielleicht noch an einer passenden Stelle einflicken, dass die interaktive GUI mit Curl arbeitet?
...
die interaktive GUI der REST-API, die einen direkten API-Zugriff auf eine {CMK}-Instanz aus dem Browser heraus ermöglicht.
// TK: eine {CMK}-Instanz? die {CMK}-Instanz
...
// TK: Ach ne: Swagger zeigt ja nur noch "attributes": {} an ohne ipadress. Das war früher auch mal anders (da war ja eh alles besser).
// TK: Von daher ist die Anweisung im folgenden 3. Schritt nicht mehr so ganz richtig: Überschreiben Sie im [.guihint]#Request body# die Beispielwerte für `host_name` und `ipaddress`.
// TK: Vielleicht im Screenshot statt der Defaultwerte die Beispielwerte mit ipaddress zeigen?
...
// TK: Ich bekomm doch nur bei Curl ein JSON-Objekt zurück, oder? Wo schau ich bei bei Python rein?
...
Aber auch bei fehlgeschlagenen Anfragen lohnt sich ein Blick in das JSON-Objekt, das die API zurückgibt.
Hier können Sie hilfreiche Informationen ablesen, die Ihnen helfen, dem zugrundeliegenden Problem auf die Spur zu kommen:

// ES-review: Es wäre schöner, wenn hier eine lesbarere Reihenfolge der Felder präsentiert würde, aber die API spuckt das JSON-Objekt nun einmal leider alphabetisch sortiert aus. Logische Reihenfolge fingieren für bessere Nachvollziehbarkeit, oder bei der originalen Reihenfolge bleiben im Sinne der Konsistenz mit der tatsächlichen Ausgabe? Die Frage gilt für beide Beispiele in diesem Abschnitt.
// TK: Ich würd beim Original bleiben.
[{shell}]
----
{
...
// ES-review: Ist diese Zusatzinfo hier zuviel?
// TK: Nö, ist hilfreich

// ES-review: Ergebnisse meiner Tests in einer Checkmk Pro (formerly Enterprise) 2.5.0-2026.02.16:
////
* cmkadmin2 legt Host via GUI an, cmkadmin ruft Skript auf -> fail
* cmkadmin2 legt Host via GUI an, cmkadmin2 ruft Skript auf -> fail
* cmkadmin legt Host via Skript an, cmkadmin2 ruft Skript auf -> fail
* cmkadmin legt Host via Skript an, cmkadmin ruft Skript auf -> success
* cmkadmin legt Host via Skript an, cmkadmin2 aktiviert via GUI -> success
////
...
// TK: Vielleicht tue ich mich mit dem Thema so schwer, weil das alles ungelegte Eier sind. Das erinnert mich daran, dass ich in der 2.0 geschrieben hab, das die REST-API Semantic Versioning folgt und in der 2.5 tut sie es nun.

ifdef::onprem[]
Im xref:cmk_versions#[Entwicklungszyklus von {CMK}] kommt es gelegentlich vor, dass neue Features oder Korrekturen für bestehende Features auch in Patch Releases für ältere, noch im Support-Zeitraum befindliche Major-Versionen von {CMK} zurückportiert werden.
// TK: Wollen wir hier "neue Features" wirklich nennen? Das sollte doch eigentlich nicht sein.
In solchen Fällen sollen die entsprechenden Änderungen allerdings nicht in der älteren Version der REST-API vorgenommen werden.
// TK: Die entsprechenden Änderungen werden aber _nicht_ in älteren REST-API-Versionen verfügbar gemacht.
...
Sie erhalten also in diesem Fall mit dem Update auf das nächste Patch-Level einer älteren Major-Version von {CMK} Zugriff auf Änderungen an der REST-API, die eigentlich für die neueste Major-Version der REST-API bestimmt sind.
// TK: Den vorherigen Satz hab ich mehrmals gelesen, versteh ihn aber immer noch nicht. Ich probiers mal:
// TK: Die `unstable`-Version macht solche Backports und damit Funktionen verfügbar, die nicht in den unterstützten REST-API-Versionen Ihrer {CMK}-Version enthalten sind.

Auch in den Patch-Updates für die jeweils aktuelle Major-Version von {CMK} kann die `unstable`-Version der REST-API neue, experimentelle Features enthalten.
// TK: Sie kann auch neue, experimentelle Features enthalten.
...
// TK: Und wie erkenne ich, welcher Endpunkt experimentell ist? Ah, da gibts ja CMK-31526 für :-)
...
// ES-blocked by CMK-31526: woran erkennt man, welche Endpunkte "unstable" sind
...
// TK: ein sehr langes > ein sehr langes und komplexes?
