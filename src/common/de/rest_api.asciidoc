...
|Versionierung |Die API ist versioniert und nutzt dabei eine Nummerierung nach dem *Semantic Versioning 2.x* Standard.
...
Die Versionsnummern der Major- und Minor-Version sind Bestandteil der URL, mit dem eine Anfrage an den Server gesendet wird.
...
[TIP]
====
Sicher ist Ihnen aufgefallen, das es im [.guihint]#Help#-Menü noch weitere Einträge zur REST-API gibt.
...
Die xref:api_doc[API-Dokumentation] enthält im Abschnitt „Queries through the REST API“ ein Beispiel eines Skripts für diese Aufgabe.
...
Mit HTTPie steht eine Alternative zur Verfügung, die konsistent, leicht zu verstehen und gut für die Verwendung in Skripten geeignet ist.
====
...
Im mittleren Bereich sehen Sie die Details zur gewählten Anfrage: welche HTTP-Authentifizierung gefordert ist (diese ist identisch für alle Anfragen über die REST-API) und die notwendigen und optionalen Parameter. Notwendig (_required_) ist der Name des Hosts und der Ordner, in dem er angelegt werden soll.
