...
Änderungen von API-Funktionen -- in Form von Bug Fixes, kompatiblen Änderungen oder sogar inkompatiblen Änderungen -- können in jeder Patch-Version von {CMK} ausgeliefert werden.
// TK: Von den inkompatiblen Änderungen bin ich verwirrt:
// TK: Das klingt nach einem Widerspruch zum Semantic Versioning: Für eine inkompatible Änderung muss es eine neue Major-Version geben. Ist das bei der CMK REST-API etwa anders?
// TK: Oder ist das hier gemeint: Mit einer CMK-Patch-Version kann eine REST-API-Major-Version ausgeliefert werden? 
Hierüber werden Sie wie gewohnt über link:https://checkmk.com/de/werks[Werks^] informiert.
// ES: Gerne Feedback dazu, ob das ein Link oder besser ein Glossareintrag für Werks sein sollte, was hilft den Lesenden hier am besten weiter?
...
Im Artikel über xref:update_major#rest_api[Updates auf eine höhere {CMK}-Major-Version] finden Sie jederzeit aktuelle Informationen über den empfohlenen Umgang mit den jeweils unterstützten REST-API-Versionen.
// TK: Im Artikel zum xref:update_major#rest_api[Update auf Version {current-major}] ... über den empfohlenen Umgang mit den in einer {CMK}-Major-Version jeweils unterstützten REST-API-Versionen.
...
====
// TK: Vielleicht reinschreiben, dass `unstable` einen frühzeitigen Zugriff auf in Entwicklung befindliche REST-API-Funktionen bietet (wenn das denn wahr ist).
