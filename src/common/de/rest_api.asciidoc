...
Mit der {CMK} REST-API als Anwendungsprogrammierschnittstelle können Sie die Aufgaben, die Sie sonst in {CMK} über die GUI erledigen, per Befehl oder Skript mit HTTP-Anfragen an den {CMK}-Server übermitteln und ausführen lassen.
...
Neben diesen generellen Komfortfunktionen soll die REST-API die komplette Funktionalität abdecken, die {CMK} über die GUI und über die Befehlszeile bietet.
...
////
Beachten Sie, dass Sie mit dem Befehl xref:omd_basics#omd_config[omd config] die Möglichkeit haben, die HTTP-Basic-Authentication der {CMK}-Instanz einzuschalten ([.guihint]#Web GUI > MULTISITE_COOKIE_AUTH > off#).
...
Zur Aufbereitung des JSON-Formats gibt es diverse Websites und Werkzeuge, zum Beispiel den Befehlszeilen-JSON-Prozessor `jq`.
...
. Antwort überprüfen:
Unter [.guihint]#Responses# sehen Sie zunächst den gesendeten Curl-Befehl und die URL des Endpunkts.
...
Über die API können standardmäßig nur ausstehende Änderungen aktiviert werden, die erstens auch über die API durchgeführt wurden und zweitens vom gleichen Benutzer stammen wie dem, der die Aktivierung via API ausführt.
