...
// ES: Dein Satz drückt was anderes aus als meiner. Ich wollte darauf hinaus, dass die API nur mit einer einzigen Nummer gekennzeichnet wird, nämlich mit der Major-Versionsnummer. (Das ist schwer zu rechtfertigen, nachdem ich mich auf die "Grundprinzipien von SemVer" bezogen habe, zu denen diese Vorgehensweise nicht passt...)
// ES: Anderer Formulierungsvorschlag:
Die Versionsbezeichnungen der REST-API von {CMK} drücken allerdings ausschließlich die Ebene der Major-Version aus.
Das bedeutet, dass Bug Fixes (Patch-Versionen) und kompatible Änderungen (Minor-Versionen) ab der Version `v1` nur „unter der Haube“ stattfinden und keine Änderung des Pfads für API-Anfragen erfordern.
...
// ES: Das ist nicht zwingend so, siehe ADR
...
// ES: Wohl steht da was über empfohlenen Umgang! Wir empfehlen, API-Pfade in Ihren Skripten in Bezug auf die neue Versionierung zu aktualisieren! Wenn eine neue Major-Version rauskommt, werden wir sicher auch Empfehlungen abgeben wie "Wenn Ihre Skripte die Endpunkte x und y nutzen, sollten Sie schnellstmöglich auf die neue Major-Version wechseln, die seit dem Patch-Release 2.5.0p99999 mit {CMK} ausgeliefert wird"
endif::[]
...
// ES: Hier steht es doch (re: TK-Kommentar oben). Eine ausführlichere Anleitung wäre aber natürlich schön! Gibt es dafür irgendwo best practices, die ich berücksichtigen könnte?
endif::[]
...
// ES-blocked by CMK-31526: Sämtliche Code-Snippets werden sich in 2.5.0 Anfang April ändern, sodass die API-Version als Variable deklariert wird. Dann müssen hier alle Code-Blöcke aktualisiert werden.
...
// ES: Ich weiß! Ich hatte schon mal danach gefragt, ob ich mich an diesem Vorschlag orientieren soll oder lieber Konsistenz mit den Code-Schnipseln aus der API-Doku priorisieren soll (dort wird nämlich die 192.168.0.123 angegeben, warum auch immer). Die Mehrheit war für Konsistenz mit der API-Doku.
...
// ES: kann ich machen, oder vielleicht genügt es, die Beschreibung unten anzupassen
...
// ES: Tut sie ehrlich gesagt immer noch nicht...
