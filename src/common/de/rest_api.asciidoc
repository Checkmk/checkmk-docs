...
// ES-blocked: CMK-29243 is related to the recent changes in this article but is not currently blocking KNW-1797. Once CMK-29243 is resolved, make sure that all claims here are consistent with the results of CMK-29243.
...
// ES-todo: Swagger hier auch erwähnen
...
// ES-todo: Konkretisieren, was "es gibt Beispiele" bedeutet (in der API-Doku sind Beispiele hinterlegt, die du kopieren, anpassen oder direkt interaktiv ausführen kannst)
...
// ES-blocked: Es gibt in CMK 2.5.0 nur eine einzige API-Doku, die mit allen Editionen ausgeliefert wird. Man muss dann für jede Funktion, die man nutzen will, nachlesen, ob sie für die vorhandene Edition verfügbar ist. Ich habe Zweifel daran, ob die Informationen akkurat sind, weil von 217 derartigen Hinweisen bei insgesamt 5 steht "all commercial editions", die Funktion "Get a custom graph" ist markiert als Available in the following editions: Pro, Ultimate, Ultimate with multi-tenancy, und alle 211 anderen Funktionen sind angeblich in jeder Edition vorhanden
ifdef::onprem[]
{cee-only}
...
// ES-blocked: CMK-29526 blockiert hier nicht direkt, wir sollten aber dennoch sicherstellen, dass das Ticket erledigt wird, damit der obige Satz wirklich korrekt ist
endif::[]
...
// ES-review: Komplizierte ifdefs in diesem Abschnitt bitte besonders gründlich korrekturlesen
...
// ES-blocked: Verweis auf den Mirror der API-Doku auf docs.checkmk.com einfügen
// ES-blocked: blocked by KNW-1719
...
// ES-blocked: todo in update_major (nachdem alle inhaltlichen Blocker von KNW-1797 weg sind und dieser Artikel hier stabil ist)
endif::[]
...
[#access]
=== Zugriff
// ES-blocked: blocked by KNW-1719: Link einfügen zur API-Doku, die auf docs.checkmk.com gehostet ist
...
// ES-blocked: blocked by CMK-29126 - Pfad ist evtl anders
...
// ES-review: Schrifthöhe im Bild bei "Introduction": 17 px
image::restapi_help_menu.png[alt="Help-Menü in der Navigationsleiste.",width=60%]
// ES-blocked: blocked by CMK-29126. Sieht in der SaaS evtl. anders aus, dann muss ich einen weiteren Screenshot machen. Außerdem sicherstellen, ob der Klickpfad zur Doku gleich ist oder anders (und dann dafür sorgen, dass sämtliche Vorkommen des Pfads hier im Artikel stimmen)
...
// ES-blocked: blocked by CMK-29126. prüfen, ob das auch für saas gilt
...
// ES-blocked: blocked by CMK-29126. prüfen, ob die Doku für die saas die gleiche ist wie für andere Editionen
...
// ES-blocked: aktuell stimmt das nicht, siehe CMK-31473
...
// ES-blocked: blocked by KNW-1719: hier auch den Link zum Mirror platzieren mit Anker auf genau diesem Abschnitt
...
// ES-blocked: Außer anscheinend bei Redirects (204/303)... blocked by CMK-31473
...
{localprompt} chmod +x create_host.sh service_discovery.sh show_pending_changes.sh activate_changes.sh
----
// ES-review: Die Admonition zu "bitte httpie statt curl verwenden" habe ich entfernt, weil curl jetzt sowieso ziemlich in den Hintergrund rückt.
--
====
endif::[]
...
// ES-blocked: Hier einen Direktlink zu diesem Anker im API-Docs-Mirror einfügen
// ES-blocked: blocked by KNW-1719

[{image-border}]
image::restapi_redoc_2pane.png[alt="Der Eintrag in der API-Dokumentation zum Erstellen eines Hosts.",fullscreen=1]
// ES-todo: Neuen Screenshot erzeugen - not blocked
...
// ES-blocked: blocked by CMK-31473
// ES-blocked: Seltsame Ergebnisse. Bei korrekten Parametern, vorhandenem Host usw. erhalte ich hier den Response Code 204. Der ist in der API-Doku für diesen Endpunkt gar nicht vorgesehen.
// Es scheint was mit den Redirects zu tun zu haben bzw. allgemein mit dem Verhalten von requests.
// Verwandt dazu ist möglicherweise Werk #16663 (Christopher Krause), in dem der Code 302 zu 303 geändert wurde?
...
// ES-todo: ETags überall kürzen für bessere Lesbarkeit der Code-Blöcke ohne Scrollbalken
...
// ES-blocked: blocked by KNW-1719: Link einfügen und sinnvoll darstellen, sobald das möglich ist
...
// ES-todo: Screenshot sauber zurechtschneiden, nicht blockiert!
...
Einen Sonderfall stellt der HTTP-Status-Code `204` dar, der besagt, dass eine Anfrage zwar erfolgreich war, aber zu keiner Antwort geführt hat.
...
// ES-blocked: blocked by CMK-31526
// ES-blocked: blocked by CMK-29126: unstable in Checkmk Cloud nicht verfügbar? oder nur nicht verlinkt?
// ES-blocked: motivieren, warum
// ES-blocked: erklären, wie
// ES-blocked: abgrenzen, dass wir nicht supporten
// ES-blocked: Hinweise, wo man herausfindet, was die unstable so kann
...
// ES-blocked: Sicherstellen, dass alle eingehenden Links zu diesem Artikel korrekt sind und funktionieren. Erst daran arbeiten, wenn alle offenen todos und Blocker hier erledigt sind.