...
:description: Zustandsänderungen von Objekten meldet {CMK} Ihnen mit Benachrichtigungen. Lesen Sie hier, wie Sie den E-Mail-Versand vorbereiten, aktivieren und testen.
:experimental:
...
Wenn der E-Mail-Versand korrekt eingerichtet ist, sollten Sie in der Lage sein, auf der Befehlszeile eine E-Mail zu versenden, z.B. über diesen Befehl:
...
Mit der Tastenkombination kbd:[Shift+G] springen Sie ans Ende der Datei (was bei Log-Dateien nützlich ist), und mit der Taste kbd:[Q] beenden Sie `less`.

Mit dem Befehl `tail -f` können Sie den Dateiinhalt auch fortlaufend beobachten.
...
[TIP]
====
Wenn Sie ganz genau wissen wollen, wann Benachrichtigungen erzeugt werden -- und wann eben nicht -- erfahren Sie die ganze Wahrheit wieder im Artikel zu den xref:notifications#system[Benachrichtigungen].
====
...
xref:intro_extend#[Weiter geht es mit dem Ausbau des Monitoring]
