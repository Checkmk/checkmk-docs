// -*- coding: utf-8 -*-
// IGNORE AZERTY Keycode Keysym QWERTZ
// IGNORE livestatus
...
// Review: Headline, intro and the definition as an "Exkurs" make clear beyond doubt that the following is an off-topic. However, we could improve this further by putting it into a collapsable box to enable our readers to either skip or dive into this extra material as they please.
Aus jeder Suchabfrage wird eine xref:glossar#livestatus[Livestatus]-Abfrage erstellt.
Welche das ist, können Sie sich in der {CMK}-GUI anzeigen lassen, wenn Sie die globale Einstellung [.guihint]#Setup > Global settings > User interface > Debug livestatus queries# einschalten.
// Review: The guihint looks incomplete, please check. Do we deliberately omit the "General" before "Global settings"? On my machine, it's #Setup > General > Global settings > User interface > Debug livestatus queries# --- Ignore my remark if this is self-evident.
Mit aktivierter Einstellung wird die Livestatus-Abfrage unten in der xref:views[Tabellenansicht] eingeblendet, die das Resultat der Suchabfrage ist.
// Review: 2 remarks. 
// 1) Visualization: We need a screenshot of the Checkmk-GUI here, I think. Sebkir thinks so as well. 
// 2) Ambivalent reference "unten eingeblendet". The sudden shift of focus from GUI to CLI in the following sentence and in the example got me confused. It seems a bit abrupt? My brains took the CLI-image below ( = unten in the User Guide) as point of reference. Wrong. After trial and error I realized that I have to *look into the Checkmk-GUI instead*, and in there check out the lower parts of my screen (= unten in the GUI). What sent me offtrack: the sentence ending by "die Livestatus-Abfrage unten ... eingeblendet" --> wo "unten" and which "Tabellenansicht", the reference only works with a screenshot displaying those items. 
Diese und ähnliche Abfragen können Sie auch auf der Befehlszeile verwenden, so wie es im xref:livestatus#lql_shell[Livestatus-Artikel] beschrieben ist.

Zum Beispiel sieht die Eingabe von `h: localhost s: mem s: cpu h:switch-intern` im Suchfeld des Monitor-Menüs auf der Befehlszeile etwa so aus, wobei im Beispiel `Columns` auf die beiden Spalten beschränkt werden, die die beiden folgenden Filterzeilen auch verwenden:
// Review: Why does this shell start with the OMD[mysite] site user prompt? Do we want the users to turn to the shell for an active LQL session now, or is this only a means to depict something similar to the Checkmk-GUI "Debugging livestatus query" box? Focus on GUI or CLI?
// Review conclusion: A GUI-screenshot would sort things and save words. It's too complex without.
...
Beachten Sie dabei, dass der Befehl `lq` und die vier folgenden Zeilen die Eingabe sind, die Sie durch zweimaliges Drücken der Taste kbd:[Enter] abschließen.
Auf die Leerzeile folgen dann die fünf Zeilen der Ausgabe.
endif::[]
