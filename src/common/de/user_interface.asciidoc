...
Wir werden uns in diesem Artikel mit allen Elementen beschäftigen, die Sie im obigen Bild der {CMK}-Benutzeroberfläche sehen -- und arbeiten uns dabei von links nach rechts durch die {CMK}-Benutzeroberfläche: von der Navigationsleiste über die xref:main_page[Hauptseite] zur xref:sidebar[Seitenleiste].
...
[#navigation_bar]
== Navigationsleiste
// TK: Zur Struktur: Die Reihenfolge der Unterkapitel ist nicht so, wie die Reihenfolge der Icons in der Navigationsleiste, aber ich will erst die Menüs erklären, bevor ich die Suche in den Menüs beschreibe.
// ES: Liest sich gut.
// TK: Das neue Changes-Symbol wird nicht hier beschrieben, sondern weiterhin als Änderungen aktivieren in wato. Soll das hierhin verschoben werden?
// ES: In diesem Artikel hier wird ja auch nicht auf das Help-Menü und auf den Toggle-Knopf für die Sidebar eingegangen. Deshalb würde ich hier keine Vollständigkeit erwarten. Dennoch wäre wahrscheinlich ein kleiner Hinweis (Signposting mit Querverweisen) zu Beginn der Section gut: In diesem Artikel erklären wir die Funktionsweise der Menüs x, y, z. Erläuterungen zu den verbleibenden Elementen a, b, c finden Sie im Artikel xy.
...
// TK: Ist der folgende Screenshot überflüssig?
// ES: Ich finde ihn gut, er stellt die Sache mit den Filtern viel übersichtlicher dar als der Fließtext untendrunter.
...
////
TK: Monitor-Such-Infos, die es (aus verschiedenen Gründen) nicht mehr in den Artikel geschafft haben:
- Die Liste der Suchoperatoren/Filter, da die jetzt in der GUI verfügbar sind. Eine Mini-Zusatzinformationen, die durch das Löschen der Tabelle auch weg ist: Bei Host-Merkmalen kann man sowohl nach Merkmal und Wert suchen (tg: agent:cmk-agent) als auch nur nach dem Wert (tg: cmk-agent) und erhält das gleiche Ergebnis.
- Tipps für die Suche: Groß- und Kleinschreibung ist bei der Suche nicht relevant -- mit Ausnahme der Suche nach Labels, die nur dann gefunden werden, wenn sie in der Syntax `Schlüssel:Wert` exakt so eingegeben werden, wie sie heißen. > Das steht jetzt als Info in der GUI, wenn man hl: auswählt.
- Die Suchanfrage können Sie in einem Lesezeichen speichern. > Achja...
////
...
// ES: Hier habe ich mich gefragt, was denn mit Groß- und Kleinschreibung passiert. Da die Info dazu gelöscht wurde, ist es vielleicht weniger verwirrend zu lesen, wenn wir beim Host einfach mit "my" und "host" als Beispielsequenzen arbeiten statt mit "My" und "Host". Oder an dieser Stelle im Text erscheint doch ein knapper Hinweis, dass die Großschreibung nicht berücksichtigt wird.
...
// TK: Wann so ein Button angezeigt wird, muss ich noch rauskriegen.
// ES: Ich sehe ihn in drei verschiedenen Sites nicht (Pro, Community, verschiedene Dailys aus dem Februar)
// TK: Oder kann das weg, weil selbsterklärend?
// ES: Für mich ist es jedenfalls nicht selbsterklärend, weil ich den Knopf nicht sehen kann. Deshalb: entweder genauer erklären oder lieber den Hinweis komplett entfernen.
[{image-left}]
...
Für zwei ausgezeichnete Seiten gibt es spezielle Menüeinträge: [.guihint]#Default start URL# für das Standard-xref:dashboards[Dashboard] und [.guihint]#Welcome page# für die xref:intro_setup_monitor#welcome[Willkommens-Seite].
...
Das Snapin [.guihint]#Welcome to {CMK}# öffnet ein Slideout, dass die [.guihint]#Topics to explore# der xref:intro_setup_monitor#welcome[Willkommens-Seite] zeigt.
