...
.Die {CMK}-Startseite der {SE}
image::gui.png[alt="Die {CMK}-Startseite der {SE}."]
...
*Hinweis:* Die Anzeige der sichtbaren Optionen in der Konfigurationsumgebung ist abhängig von Ihren Rechten.
Sollten Sie einzelne Optionen oder auch Gruppen von Optionen in Ihrem {CMK} nicht sehen, so fehlen Ihnen die entsprechenden Zugriffsrechte.
// TK: Anzeige der sichtbaren Optionen riecht nach Pleonasmus. Warum Optionen/Gruppen von Optionen? Es geht doch um Menüs und Menüeinträge. Und nicht nur um die Konfigurationsumgebung. Rechte > Berechtigungen
...
// TK: Wer braucht an dieser Stelle die Livestatus-Abfrage? Stimmt die überhaupt noch?
...
.Das [.guihint]#Customize#-Menü der {SE} im Show-More-Modus
...
xref:views#[Tabellenansichten (_views_)] und
...
.Das [.guihint]#Setup#-Menü der {RE} im Show-Less-Modus
...
|Farbthema ([.guihint]#Color theme#) |Die Umschaltung zwischen [.guihint]#Dark# und [.guihint]#Light# ist direkt im [.guihint]#User#-Menü möglich.
...
|Sprache der Benutzeroberfläche ([.guihint]#Language#) |Standardmäßig können Sie hier zwischen den offiziell von {CMK} unterstützten Sprachen Deutsch und Englisch wählen.
Aktivieren Sie in [.guihint]#Setup > Global settings > User interface# die Option [.guihint]#Enable community translated languages (not supported)#, so stehen Ihnen zusätzlich die von der Community übersetzten Sprachen zur Verfügung.
...
|Benachrichtigungsregeln ([.guihint]#Notification rules#) |Regeln zu xref:notifications#personal[benutzerdefinierten Benachrichtigungen].
...
Dafür benötigt Ihr aktuelles Benutzerkonto allerdings eine Berechtigung, die standardmäßig nur die xref:wato_user#roles[Rolle] `admin` besitzt.
