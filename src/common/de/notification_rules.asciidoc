...
:title: Benachrichtigungen per Regel einrichten
:description: {CMK} kann Benachrichtigungen über relevante Monitoring-Ereignisse auf diversen Wegen verschicken. Lesen Sie hier, wie Sie die Regeln dafür einstellen.
...
Über [.guihint]#Setup > Events > Notification# gelangen Sie zur Übersichtsseite der Benachrichtigungen.
...
Im linken Bereich der Seite sehen Sie erst einmal eine Übersicht über den aktuellen Status der Benachrichtigungen:
...
|[.guihint]#Core status of notifications# |Zeigt an, auf wie vielen {CMK}-xref:glossar#site[Instanzen] Benachrichtigungen aktiviert sind.
...
Wie gewohnt, können Sie die Regel icon:icon_edit[alt="Symbol für das Bearbeiten."] bearbeiten, icon:icon_clone[alt="Symbol für das Klonen."] klonen, icon:icon_delete[alt="Symbol für das Löschen."] löschen oder eine neue Regel anlegen.
Sobald Sie mehr als eine Regel haben, können Sie die Reihenfolge der Regeln per Drag & Drop über das Symbol icon:icon_drag[alt="Symbol zum Verschieben eines Eintrags in der Liste."] verändern.
...
Der Inhalt einer von {CMK} versendeten Benachrichtigung per E-Mail im HTML-Format sieht etwa so aus:

[{image-border}]
image::html_notification.png[alt="Eine Benachrichtigung per E-Mail."]

Wie im Beispiel zu sehen ist, enthält die E-Mail auch die aktuellen xref:glossar#metric[Metriken] des betroffenen Services.
...
* Der Benutzer ist Mitglied einer xref:wato_user#contact_groups[Kontaktgruppe] und somit ein xref:glossar#contact[Kontakt].
...
Sie können das Versenden von E-Mails auf der Befehlszeile mit dem Befehl `mail` testen.
...
Der Inhalt der E-Mail wird von der Standardeingabe gelesen, der Betreff mit `-s` angegeben und die Zieladresse einfach als Argument ans Ende der Befehlszeile gestellt:
...
Hierfür gibt es die Regel [.guihint]#Setup > Notifications > Parameters for notification methods > HTML Email# und in Benachrichtigungsregeln die xref:notification_method[Benachrichtigungsmethode] HTML-E-Mail.
...
Falls Sie aber *beide* Bedingungen aktivieren, greift die Regel, sobald ein Ereignis in *einer* der beiden Bedingungen festgelegt ist.
...
Im 2. Schritt nutzen Sie diverse Filter, um die Hosts und Services zu bestimmen, für die die Benachrichtigungsregel gelten soll.

Angenommen, Sie würden den Filter so setzen, dass nur eine Benachrichtigung erfolgen würde, wenn ein Monitoring-Ereignis für einen Service, der mit dem Namen `NTP` beginnt, auf einem Host im Ordner [.guihint]#Main# einträte:

image::notification_rules_step2.png[alt="Regel mit den Bedingungen für die Erstellung einer Benachrichtigung."]

Das Ergebnis dieser Benachrichtigungsregel mit den drei Einzelbedingungen (Statusänderungen, Service-Name, Ordner) wäre, dass *nur* Benachrichtigungen erfolgen würden, wenn der Service `NTP` auf {CRIT} ginge -- und die Zustandsänderungen des Hosts würden ignoriert, weil kein Monitoring-Ereignis die Zustandsänderung eines Hosts *und* den Service-Namen mit `NTP` enthalten würde.

Ein Hinweis zur Option [.guihint]#Contact group filters# im obigen Bild:
...
In der Benachrichtigungsregel wählen Sie dann unter [.guihint]#Select parameters# die gewünschte Parameterdefinition aus, *bevor* Sie auf [.guihint]#Create# klicken, um die ausgewählte Parameterdefinition für diese Benachrichtigungsregel abzuändern..
...
Näheres dazu finden Sie im Abschnitt Thema xref:cancel[Benachrichtigungen durch Regeln löschen].
...
image::notification_rules_step4.png[alt="Regel mit den Optionen zur Kontaktauswahl."]
...
==== Bedingungen für den Benachrichtigungsversand

Den Schritt [.guihint]#Sending conditions# können Sie für einfache Regeln erst einmal überspringen.
...
Mit der Option [.guihint]#Allow users to deactivate this notification# können Sie Benutzern erlauben, Benachrichtigungen „abzubestellen“, die von dieser Regel erzeugt werden.
Wie das geht, zeigen wir bei den xref:personal_notifications#[persönlichen Benachrichtigungen].
...
Unter [.guihint]#Test results# erfahren Sie, wie viele Benachrichtigungsregeln greifen und wie viele Benachrichtigungen daraus resultieren.
Haben Sie den Versand gewählt, wird Ihnen hier die entsprechende Meldung `notifications have been sent out` angezeigt.
...
Durch Klick auf das Symbol icon:icon_toggle_context[alt="Symbol zum Ein- oder Ausblenden des Benachrichtigungskontexts."] können Sie den Benachrichtigungskontext einblenden.
...
Sie können {CMK} so einrichten, dass es in einem festen Intervall immer weitere Benachrichtigungen versendet, solange bis das Problem entweder icon:icon_ack[alt="Symbol einer Quittierung."]xref:basics_ackn#[quittiert] oder behoben wurde.
...
Mit der Option [.guihint]#Throttling of 'Periodic notifications'# können Sie die Rate der wiederholten Benachrichtigungen nach einer bestimmten Zeit reduzieren. So können Sie z.B. zunächst jede Stunde eine E-Mail senden lassen und später das Ganze auf einmal am Tag beschränken.
...
image::notification_rules_enable_sync_smtp.png[alt="Benachrichtigungsmethode mit den Optionen für die synchrone E-Mail-Zustellung."]
...
Sie sehen dabei die vier Einzelschritte in der zeitlichen Abfolge von unten nach oben, wie wir sie bereits im Artikel über die Grundlagen zu Benachrichtigungen im Kapitel zur xref:notifications#history[Benachrichtigungshistorie] vorgestellt haben.
...
Die einzelnen Schritte können Sie auch in der Log-Datei `~/var/log/notify.log` nachverfolgen.
...
image::notifications_smtp_failed.png[alt="Liste aufgelaufener Benachrichtigungen für eine fehlgeschlagenen E-Mail-Zustellung."]


Doch was tun bei fehlgeschlagenen Benachrichtigungen?
Diese wiederum per E-Mail zu benachrichtigen ist augenscheinlich keine gute Lösung.
Anstelle dessen zeigt {CMK} einen deutlichen Warnhinweis mit roter Hintergrundfarbe im Snapin xref:user_interface#overview[Overview] an:

image::notifications_overview_failed.png[alt="Anzeige fehlgeschlagener Benachrichtigungen im Snapin 'Overview'.",width=50%]

Hier können Sie:

* durch Klick auf den Text [.guihint]#... failed notifications# zu einer Liste der fehlgeschlagenen Benachrichtigungen kommen und
* durch Klick auf icon:icon_delete[alt="Symbol für das Löschen."] diese Meldungen bestätigen und mit einem Klick auf icon:icon_confirm[alt="Symbol zur Bestätigung."] [.guihint]#Confirm# in der sich öffnenden Übersicht den Hinweis wieder entfernen.
...
*Wichtig:* Für xref:bulk[Bulk-Benachrichtigungen] steht die nachvollziehbare Zustellung per SMTP nicht zur Verfügung!
endif::[]
...
image::notification_rules_bulk.png[alt="Benachrichtigungsmethode mit den Optionen zur Bulk-Benachrichtigung."]
...
* Sie können die Größe des Topfs begrenzen ([.guihint]#Max. notifications per bulk#).
...
[TIP]
====
Alternativ können Sie auch einen Benutzer in seinen xref:wato_user#user_config_notifications[persönlichen Einstellungen] zum Empfänger machen. 
...
Wie wir im Abschnitt zum xref:cancel[Löschen von Benachrichtigungen] gezeigt haben, ist das explizite Löschen von Benachrichtigungen ja erwünscht und kein Konfigurationsfehler.
...
[TIP]
====
Wenn Sie Benachrichtigungen nur für sich persönlich abschalten wollen, können Sie dies über die xref:personal_notifications#[persönlichen Benachrichtigungsregeln] machen.
====
endif::[]
