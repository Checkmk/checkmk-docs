...
:description: Eine {CMK}-Instanz können viele Benutzer nutzen. Lesen Sie hier, wie Sie Rollen für Berechtigungen und Kontaktgruppen für Zuständigkeiten verwalten.
...
|Empty template for least privilege roles |`no_permissions` |Diese Rolle hat gar keine Berechtigungen. Sie ist nicht für die direkte Zuweisung gedacht. Stattdessen kann sie verwendet werden, um neue Rollen nur mit der erforderlichen Mindestberechtigung zu erstellen.
|===
...
.Die neue Kontaktgruppe soll offensichtlich für Windows- und Linux-Server zuständig sein
...
Legen Sie hier selbst neue Regeln an und wählen Sie die Kontaktgruppe aus, die Sie den Hosts zuweisen wollen:

.Die Kontaktgruppe wird im Kasten [.guihint]#Assignment of hosts to contact groups# ausgewählt
image::intro_rule_assignment_hosts_contact_groups.png[alt="Dialog zur Zuweisung von Kontaktgruppen zu Hosts in einer Regel."]

Die Hosts, die dieser Kontaktgruppe zugeordnet werden sollen, legen Sie dann im folgenden Kasten [.guihint]#Conditions# fest.

[TIP]
====
Greifen für einen Host mehrere Regeln, so werden alle ausgewertet und der Host bekommt auf diese Art mehrere Kontaktgruppen.
====
...
.Mehrere Kontaktgruppen können einem Host zugeordnet werden
...
[TIP]
====
Sie können das Attribut [.guihint]#Permissions# analog statt auf Host- auch auf Ordnerebene festlegen.
...
Bevor wir mit dem Erstellen von Benutzern weitermachen, wollen wir kurz die zugehörige Anwendung vorstellen: das Admin Panel.

Das Admin Panel ist die Zentrale für die Verwaltung von Instanzen, Benutzern und Lizenzen in {CE}.
...
image::intro_admin_panel_saas.png[alt="Das Admin Panel mit der Übersicht der Instanzen.",fullscreen=1]

Wie die {CMK}-Benutzeroberfläche hat auch das Admin Panel eine Navigationsleiste am linken Rand.

Ein Klick auf [.guihint]#Sites# zeigt die in {CE} eingerichteten xref:glossar#site[Instanzen].
Während der xref:intro_setup#signup_trial[Registrierung] hatten Sie den Namen der Instanz eingegeben, die anschließend automatisch erstellt wurde.
Diese Instanz wird hier im Admin Panel angezeigt.
...
Analog zu den Instanzen zeigt ein Klick in der Navigationsleiste auf [.guihint]#User management# eine Tabelle mit allen Benutzern.
Aktuell enthält die Benutzertabelle nur einen Benutzer mit dem Namen der von Ihnen während der Registrierung eingegebenen E-Mail-Adresse und der zugewiesenen Rolle [.guihint]#Administrator#.
Wie Sie hier neue Benutzer erstellen, erfahren Sie im xref:create_users[nächsten Abschnitt].

Eine Beschreibung der Funktionen des Admin Panel finden Sie übrigens im xref:admin_panel#[gleichnamigen Artikel].
endif::[]
...
.Automatisch und manuell erstellte Benutzer in der Benutzertabelle
...
.Die Einstellungen zur Identität des neuen Benutzers
...
.Die Sicherheitseinstellungen des neuen Benutzers
...
.Durch Ankreuzen werden die Kontaktgruppen zugewiesen
...
.Ein neuer Benutzer benötigt eine E-Mail-Adresse und eine der vordefinierten xref:roles[Rollen]	
image::intro_new_user_saas.png[alt="Dialog zur Eingabe von E-Mail-Adresse und Rolle für den neuen Benutzer.",width=63%]

Bestätigen Sie den Dialog durch Klick auf [.guihint]#Create user#.
...
.Die Einladung wurde abgeschickt, aber noch nicht akzeptiert
