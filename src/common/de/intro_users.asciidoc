...
|===
|Rolle |Bedeutung 
//SK: "Anpassen von Rollen" klingt für mich so, als könnte man die Rollen bearbeiten. Ich kann ja nur Anpassen, welche Rolle einem Nutzer zugewiesen ist.
|Administrator |Ein Administrator kann alle Funktionen in {CMK} ausführen. Seine Hauptaufgabe ist die generelle Konfiguration von {CMK}, *nicht* das Monitoring. Dies schließt auch das Anlegen von Benutzern und Anpassen von Rollen ein.
...
//SK: In Setup > Users sieht man leider noch zwei weitere Rollen. Sagen wir dazu an anderer Stelle etwas? Wenn man diese Rolle in Setup > Users anklickt, gibt es einen Fehler statt einer Erklärung.
...
* Ein Benutzer der Rolle „Normal monitoring user“ sieht im Monitoring genau die Objekte, die sich in seinen Kontaktgruppen befinden.
...
Wundern Sie sich nicht, wenn es dort außer dem Eintrag `cmkadmin` auch noch zwei weitere Benutzer `automation` und `agent_registration` gibt.
...
Wundern Sie sich nicht, wenn es dort außer den beiden im Admin Panel sichtbaren Einträgen auch noch die Benutzer `agent_download`, `agent_registration` und `automation` gibt.
Diese xref:glossar#automation_user[Automationsbenutzer] sind für Zugriffe von außen gedacht, z.B. per Skript oder xref:rest_api#[REST-API.]
