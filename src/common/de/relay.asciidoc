...
Werden kompatible Versionen von Podman und Systemd vorgefunden, wird Sie das Skript vor die Wahl stellen, entweder abzubrechen oder nicht unterstützte Installation durchzuführen.

.Das {CMK} Relay auf Windows
[%collapsible]
====
...
In Standardkonfiguration (13 Fetcher) sollte ein Arbeitsspeicherbedarf von etwa einem Gigabyte und die Auslastung eines Prozessorkerns aktueller Hardware mit weniger als 20{nbsp}% eingeplant werden.
...
[TIP]
====
Die zum Kopieren und Einfügen angebotenen Parameter für Installation und Registrierung enthalten ein Einmal-Token (_one-time token_).
