...
:description: Mit Regeln und Host-Merkmalen können Sie {CMK} konfigurieren und Fehlalarme vermeiden. Lesen Sie hier, welche Optionen zur Feinjustierung wir empfehlen.
...
Worum es im Folgenden also geht, ist es die Vorstufe der Benachrichtigungen abzustellen und die kritischen Zustände {DOWN}, {WARN} und {CRIT} für unkritische Probleme zu vermeiden.
====
...
Bevor Sie mit der Konfiguration starten, geben wir Ihnen eine kurze Einführung zum Thema, wie Sie die Einstellungen von Hosts und Services in {CMK} festlegen.
...
So gibt es etwa einen mit dem Namen [.guihint]#File systems (used space and growth)#, der die Schwellwerte für alle Services festlegt, die Dateisysteme überwachen.
...
Wenn Sie sich unsicher sind, in welcher Kategorie der Regelsatz zu finden ist, können Sie auch gleich *alle* Regeln durchsuchen, indem Sie entweder die xref:intro_gui#search[Suche] im [.guihint]#Setup#-Menü nutzen oder über [.guihint]#Setup > General > Rule search# die Seite zur Regelsuche öffnen.
Wir werden den letztgenannten Weg im folgenden Abschnitt gehen, in dem es um die Erstellung einer Regel geht.
...
.Das Aktionsmenü für den Service
...
.Die Regelsätze für den Service
image::intro_parameters_of_this_service.png[alt="Liste aller Regelsätze für einen Service.",fullscreen=1]

Im ersten Kasten mit dem Titel [.guihint]#Check origin and parameters# führt Sie der Eintrag [.guihint]#File systems (used space and growth)# direkt zum Regelsatz für die Schwellwerte der Dateisystemüberwachung.
...
Sie landen dann auf dieser Seite:

.Die Übersichtsseite zum Regelsatz
...
.Die Eigenschaften der neuen Regel
...
.Auswahl des Parameters für die Schwellwerte der Tablespace-Nutzung
...
Nun folgt der Kasten [.guihint]#Conditions# zur Festlegung der Bedingungen:

.In dieser Bedingung werden Host-Merkmale und [.guihint]#Explicit tablespaces# festgelegt
...
Die Host-Merkmale ([.guihint]#Host tags#) sind ein ganz wichtiges Feature von {CMK}, deshalb widmen wir ihnen im Folgenden noch einen eigenen Abschnitt.
...
Dazu aktivieren Sie zuerst [.guihint]#Host tags# und wählen dann in der Liste die Host-Merkmalsgruppe [.guihint]#Criticality# aus.
In den beiden Listen, die dann eingeblendet werden, wählen Sie [.guihint]#is# und [.guihint]#Productive system# aus.
...
[TIP]
====
Die detaillierte Beschreibung aller anderen Parameter für Bedingungen und eine ausführliche Erklärung zum wichtigen Konzept der Regeln finden Sie im xref:wato_rules#[Artikel über Regeln].
...
Im vorherigen Abschnitt haben Sie ein Beispiel für eine Regel gesehen, die nur für „produktive“ Systeme gelten soll.
Genauer gesagt haben Sie in der Regel eine Bedingung über das Host-Merkmal (_host tag_) [.guihint]#Productive system# definiert.
Warum wurde die Bedingung als Merkmal definiert und nicht einfach für den Ordner festgelegt?
...
Nehmen Sie als Beispiel den Standort (_location_).
...
image::intro_tags_default.png[alt="Liste aller vordefinierten Host-Merkmalsgruppen.",fullscreen=1]
...
.Zuerst werden die Grundeinstellungen der Host-Merkmalsgruppe festgelegt
...
.Dann werden die einzelnen Merkmale definiert
...
.Host-Merkmalsgruppen werden in den Eigenschaften eines Hosts angezeigt
...
* Legen Sie Ihre eigenen Regeln an im Regelsatz [.guihint]#File systems (used space and growth)#.
...
[TIP]
====
Eine Alternative zur Erstellung von Wartungszeiten für Hosts, die wir im Kapitel über xref:intro_monitor#downtimes[{CMK} im Monitoring] schon beschrieben haben, ist in den kommerziellen Editionen der Regelsatz [.guihint]#Recurring downtimes for hosts#.
...
[TIP]
====
Eine Alternative zur Erstellung von Wartungszeiten für Hosts, die wir im Kapitel über xref:intro_monitor#downtimes[{CMK} im Monitoring] schon beschrieben haben, ist der Regelsatz [.guihint]#Recurring downtimes for hosts#.
Dieser hat den großen Vorteil, dass auch Hosts, die erst später in die Überwachung aufgenommen werden, automatisch diese Wartungszeiten erhalten.
====
endif::[]
...
.Diese Regel legt fest, wie geprüft werden soll, ob Hosts {UP} sind
...
.Diese Regel bestimmt unter anderem den Zustand des Services [.guihint]#Check_MK# in Fehlersituationen
...
.Festlegung der gültigen Zustände _up_ und _down_
...
.Einen Mittelwert für die CPU-Auslastung festlegen
...
.Der Service [.guihint]#Check_MK Discovery# zeigt Änderungen bei der Service-Erkennung
