...
Wir kommen jetzt ausführlich zu einem wichtigen Thema zurück, das wir bei der xref:intro_setup_monitor#first[Einrichtung des Monitorings] nur gestreift haben: die Aktivierung von Änderungen.
...
//SK: Uff. Das Password ändert sich also "regelmäßig". Ich könnte mir vorstellen, dass es Leute gibt, die das genauer wissen wollen.
//SK: Des Weiteren ist die Inline-Hilfe ist an der Stelle recht ungünstig, weil da etwas von "Colleagues" steht.
...
{CE} ändert das Passwort regelmäßig -- und nutzt dabei den xref:glossar#automation_user[Automationsbenutzer] `automation`.
//SK: Den folgenden Satz könnte man so interpretieren, dass man das nur explizit bestätigen muss, wenn man die Änderungen der "Kollegen" ebenfalls aktivieren will. Ich finde, dass deutlicher werden muss, dass der Haken auf jeden Fall gesetzt werden muss, wenn man hier weiterkommen möchte.
Die Aktivierung von Änderungen anderer Benutzer muss von Ihnen explizit bestätigt werden -- mit der Checkbox [.guihint]#Activate changes of other users.# 
...
//SK: Ich aus dem Kontext nicht, warum der Link auf #test geht. Soll das so?
Dazu mehr im Kapitel zu den xref:intro_notifications#test[Benachrichtigungen.]
====
endif::[]
...
ifdef::onprem[]
// TK: Drucker werden über SNMP überwacht. Daher passt das Beispiel nicht. Gibt es ein anderes Beispiel ohne SNMP? Vorerst kommt das Thema nicht in die saas rein.
[#hosts_down]
...
//SK: Es gibt ja auch noch Delay host/service notifications. Der Anwendungsfall mag ein anderer sein, aber "Maximum number" ist zumindest nicht die "letzte Methode".
//SK: Delay kann(!) auch besser sein, weil der Service ja auf WARN oder CRIT geht und somit die entsprechenden Counter erhöht werden, aber es gehen halt keine Benachrichtigungen raus.
//SK: Da dieser Artikel ja mit "Fehlalarme sind der Tod" beginnt, finde ich das hier schon erwähnenswert.
