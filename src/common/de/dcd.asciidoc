...
:title: Dynamische Host-Verwaltung – Überwachen dynamischer Infrastrukturen
...
Anschließend wird auf den Hosts (optional) eine xref:glossar#service_discovery[Service-Erkennung] durchgeführt und abschließend die xref:glossar#activate_changes[ausstehenden Änderungen aktiviert], damit der aktuelle Zustand in der xref:glossar#monitoring_environment[Monitoring-Umgebung] sichtbar ist.
...
Unterstützung für die Verarbeitung von link:https://opentelemetry.io/[OpenTelemetry^]-Metriken.
Dafür sammelt in {CMK} ein OpenTelemetry Collector Metrikdaten, die der Kollektor per OpenTelemetry Protocol (OTLP) erhält oder über einen Prometheus-Endpunkt abruft.
// ES: Den Rest dieses Abschnitts bitte noch prüfen/aktualisieren für KNW-1928
// 
...
Der OpenTelemetry Collector schreibt empfangene Daten in das xref:metric_backend#[Metrik-Backend].
Auf die dort hinterlegten Daten greift die dynamische Host-Verwaltung zu.

Die Einrichtung von OpenTelemetry einschließlich des OpenTelemetry Collectors
ifdef::onprem[]
und des Metrik-Backends
endif::[]
wird im Artikel xref:opentelemetry#[OpenTelemetry-Metriken überwachen] beschrieben.

////
Die OpenTelemetry-Daten sind in der Instanz immer dann verfügbar, wenn der OpenTelemetry Collector korrekt aufgesetzt wurde.
...
Die Optionen bei der Erstellung einer xref:connection_piggyback[Piggyback-Verbindung] und einer xref:connection_opentelemetry[OpenTelemetry-Verbindung] unterscheiden sich in einem wesentlichen Punkt:
Während bei Piggyback-Verbindungen die Host-Namen verwendet werden, die mit den Piggyback-Daten mitgeliefert werden, können bei OpenTelemetry-Daten die Host-Namen flexibler erzeugt werden.
Auf diesen Punkt gehen wir detailliert ein, alle weiteren Eigenschaften einer OpenTelemetry-Verbindung werden im Schnelldurchlauf vorgestellt.

image::dcd_connection_ot_01.png[alt="Eigenschaften einer OpenTelemetry-Verbindung."]

Für eine xref:connection_opentelemetry[OpenTelemetry-Verbindung] wählen Sie [.guihint]#OpenTelemetry data - Metric backend# als [.guihint]#Connector type# aus.

Mit [.guihint]#Sync interval# bestimmen Sie, wie oft die Verbindung nach neuen Hosts suchen soll.

Der größte Unterschied zu Piggyback-Verbindungen ist [.guihint]#Metric backend filter criteria#.
// 3.0.0 GUI change: Metric backend filter criteria > Telemetry Host and Service configuration
Hier können Sie zunächst mit den [.guihint]#Attributes# festlegen, welche Daten bei der Host-Erstellung überhaupt berücksichtigt werden sollen und dann, aus welchem Attribut der Host-Name entstehen soll.
Das obige Bild zeigt das Feld [.guihint]#Resource attribute for hostname lookup#  ausgefüllt mit dem Attribut `service.name`, welches aus jedem Telemetry-Service-Namen einen Host erzeugt.
// 3.0.0 GUI change: Resource attribute for hostname lookup > Host name template used to name the hosts
Dieses ist Vorgabe, wenn Sie das [.guihint]#OpenTelemetry Quick Setup# verwendet haben.
// MFS: Wird in 3.0 dann gehen:
// Wenn beispielsweise verschiedene Cluster gleiche Service-Namen liefern, aber nach diesen differenziert werden soll, können Sie verschiedene Attribute kombinieren.
Öffnen Sie die Hilfe beim Eingabefeld, um detaillierte Informationen zu erhalten.
...
// TK: Der Screenshot oben zeigt keine der beiden beschrieben Attribute. Die gibts auch nicht mehr bei der OTel connection. Dafür gibts jetzt Maximum number of hosts.
...
Hier geht es darum, wie und wann die dynamische Host-Verwaltung ausstehende Änderungen aktivieren soll und wie lang sie dafür maximal brauchen darf.
...
Den Umfang der geloggten Daten können Sie über die Regel [.guihint]#Setup > Hosts > Dynamic host management > Host manager settings# festlegen.
