...
Die Optionen bei der Erstellung einer Piggyback- und OpenTelemetry-Verbindung, die in {CE} eingerichtet werden kann, unterscheiden sich in einem wesentlichen Punkt:
...
Der größte Unterschied zu Piggyback-Verbindungen ist [.guihint]#Telemetry Host and Service configuration#.
Hier können Sie zunächst mit den [.guihint]#Attribute filters# festlegen, welche Daten bei der Host-Erstellung überhaupt berücksichtigt werden sollen und dann, aus welchem Attribut der Host-Namen entstehen soll.
...
// TK: Ich krieg Deine Beschreibung nicht mit der GUI zusammen: Ich sehe zuerst Attributes und dann Host name template used to name the hosts.
// TK: Deine Beschreibung lese ich so, dass es nur um den Wert bei Host name template used to name the hosts geht. Richtig?
// MFS: Welche Daten berücksichtigt werden sollen (Filterung etc.) ist optional, ins Hostname template muss was eingetragen werden. Genau darum geht est.
// TK: Wie hängen denn die beiden Einstellungen zusammen? Muss/kann ich denn Attribute auswählen und haben diese ausgewählten Attribute was zu tun mit dem Host name template?
// TK: Bytheway: Ich seh die GUI nur im master. Kommt diese GUI-Änderung in die nächste 2.5.0p?
// MFS: Leider nicht, geändert auf das einfachere Resource Attribute.
