...
Während bei Piggyback-Verbindungen die bei den Piggyback-Daten mitgelieferten Hostnamen verwendet werden, können bei OpenTelemetry-Daten die Hostnamen sehr flexibel erzeugt werden.
...
Hier legen Sie zunächst fest, welche Daten bei der Host-Erzeugung berücksichtigt werden sollen und dann, aus welchen Attributen in welcher Form Hostnamen entstehen sollen.
Der Screenshot zeigt das Makro `$RESOURCE_ATTR.service.name$`, welches aus jedem Telemetry-Servicenamen einen Host erzeugt.
Dieses ist Vorgabe, wenn Sie das [.guihint]#OpenTelemetry quick setup# verwendet haben.
Wenn beispielsweise verschiedene Cluster gleiche Servicenamen liefern, aber nach diesen differenziert werden soll, können Sie verschiedene Attribute kombinieren.
