...
{cee-only}
{CMK} {v24} bietet in {CCE} eine experimentelle Unterstützung für die Verarbeitung von link:https://opentelemetry.io/[OpenTelemetry^]-Metriken.
...
Die Einrichtung von OpenTelemetry einschließlich des OpenTelemetry Collectors wird im Artikel xref:opentelemetry#[OpenTelemetry-Metriken überwachen] beschrieben.
...
Die Optionen bei der Erstellung einer Piggyback- und OpenTelemetry-Verbindung, die in {CE} eingerichtet werden kann, sind nahezu identisch.
