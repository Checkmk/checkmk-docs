...
Während bei Piggyback-Verbindungen die Host-Namen verwendet werden, die mit den Piggyback-Daten mitgeliefert werden, können bei OpenTelemetry-Daten die Host-Namen sehr flexibel erzeugt werden.
...
Hier legen Sie zunächst fest, welche Daten bei der Host-Erstellung berücksichtigt werden sollen und dann, aus welchen Attributen in welcher Form Host-Namen entstehen sollen.
Das obige Bild zeigt das Makro `$RESOURCE_ATTR.service.name$`, welches aus jedem Telemetry-Service-Namen einen Host erzeugt.
