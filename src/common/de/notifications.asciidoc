...
:title: Grundlagen zu Benachrichtigungen
:description: Benachrichtigungen in {CMK} informieren Benutzer aktiv bei Monitoring-Ereignissen. Lesen Sie hier, wie Sie Benachrichtigungen konfigurieren und nutzen.
...
Eine Benachrichtigung bedeutet, dass Benutzer im Falle von Problemen oder anderen Ereignissen im Monitoring von {CMK} aktiv informiert werden.
...
* Start oder Ende einer icon:icon_downtime[alt="Symbol einer Wartungszeit."] xref:glossar#scheduled_downtime[Wartungszeit]
* Die icon:icon_ack[alt="Symbol zur Anzeige der Quittierung."] xref:basics_ackn#[Quittierung eines Problems] durch einen Benutzer
...
Wenn Sie stattdessen direkt mit der Umsetzung anfangen wollen:
{CMK} unterscheidet generell zwei Formen der Einstellung von Benachrichtigungen.
...
Die Erstellung dieser Benachrichtigungen wird im im Artikel xref:notification_rules#[Benachrichtigungen per Regel einrichten] beschrieben.
...
Wie diese persönlichen Einstellungen umgesetzt werden können, lesen Sie im Artikel xref:personal_notifications#[Persönliche Benachrichtigungsregeln].
...
Manche große Unternehmen haben eine Art Leitstand, in der das Monitoring-Team die {CMK}-Oberfläche ständig im Blick hat und keine zusätzlichen Benachrichtigungen benötigt.
...
Die Konfiguration geschieht über die Regelsätze [.guihint]#Notification period for hosts# bzw. [.guihint]#Notification period for services#, die Sie schnell über die xref:user_interface#search[Suche] im [.guihint]#Setup#-Menü finden können.
...
xref:glossar#active_check[Aktive Checks] werden dann in der Regel {CRIT} oder {UNKNOWN}, weil diese gezielt versuchen, den Host zu erreichen und dabei in einen Fehler laufen.
...
Denn wenn z. B. ein Webserver nicht erreichbar ist -- und dies auch schon als Benachrichtigung kommuniziert wurde --, wäre es wenig informativ, wenn nun auch für jeden einzelnen HTTP-Dienst, den dieser bereitstellt, eine E-Mail generiert würde.
...
Wenn Sie nichts anderes konfiguriert haben, geschieht dies durch einen xref:cmc_differences#smartping[Smart Ping] bzw. Ping.
...
{CMK} ist „ab Werk“ so eingerichtet, dass es bei einem Monitoring-Ereignis an jeden xref:glossar#contact[Kontakt] des betroffenen Hosts oder Services eine Benachrichtigung per E-Mail versendet.
...
* Eventuell _keine_ Benachrichtigungen für {WARN} oder {UNKNOWN} erhalten.
...
Auch können Sie Benachrichtigungen wieder xref:notification_rules#cancel[löschen], welche vorherige Regeln generiert haben.
...
Es ist grundsätzlich möglich, die Benachrichtigungen bezogen auf einzelne Hosts und Services auch per xref:commands#[Kommando] vorübergehend abzuschalten.
...
Da Kommandos im Gegensatz zu Regeln weder eine Konfigurationsberechtigung noch eine xref:glossar#activate_changes[Aktivierung ausstehender Änderungen] benötigen, können sie daher eine Lösung sein, um auf eine Situation schnell zu reagieren.
...
ifdef::onprem[]
Im xref:glossar#distributed_monitoring[verteilten Monitoring] gibt es diesen Schalter einmal pro Instanz.
...
* Als Befehl wird `check-mk-notify` angegeben.
...
|`rd` | [.guihint]#UP ➤ DOWN# |Setzt den Host-Zustand von {UP} auf {DOWN}
|`ru` | [.guihint]#UP ➤ UNREACHABLE# |Setzt den Host-Zustand von {UP} auf {UNREACH}
|`dr` | [.guihint]#DOWN ➤ UP# |Setzt den Host-Zustand von {DOWN} auf {UP}
|`du` | [.guihint]#DOWN ➤ UNREACHABLE# |Setzt den Host-Zustand von {DOWN} auf {UNREACH}
|`ud` | [.guihint]#UNREACHABLE ➤ DOWN# |Setzt den Host-Zustand von {UNREACH} auf {DOWN}
|`ur` | [.guihint]#UNREACHABLE ➤ UP# |Setzt den Host-Zustand von {UNREACH} auf {UP}
|`?r` | [.guihint]#any ➤ UP# |Setzt jeden beliebigen Host-Zustand auf {UP}
|`?d` | [.guihint]#any ➤ DOWN# |Setzt jeden beliebigen Host-Zustand auf {DOWN}
|`?u` | [.guihint]#any ➤ UNREACHABLE# |Setzt jeden beliebigen Host-Zustand auf {UNREACH}
|`f`  | [.guihint]#Start or end of flapping state# |Anfang oder Ende eines unstetigen Zustands
|`s`  | [.guihint]#Start or end of a scheduled downtime# |Anfang oder Ende einer Wartungszeit
|`x`  | [.guihint]#Acknowledgment of problem# |Quittierung eines Problems
|`as` | [.guihint]#Alert handler execution, successful# |Erfolgreiche Ausführung eines Alert Handlers
|`af` | [.guihint]#Alert handler execution, failed# |Gescheiterte Ausführung eines Alert Handlers
|===
...
|*Kürzel* |*Bedeutung* |*Beschreibung*
|`rw` | [.guihint]#OK ➤ WARN# |Setzt den Service-Zustand von {OK} auf {WARN}
|`rr` | [.guihint]#OK ➤ OK# |Setzt den Service-Zustand von {OK} auf {OK}
|`rc` | [.guihint]#OK ➤ CRIT# |Setzt den Service-Zustand von {OK} auf {CRIT}
|`ru` | [.guihint]#OK ➤ UNKNOWN# |Setzt den Service-Zustand von {OK} auf {UNKNOWN}
|`wr` | [.guihint]#WARN ➤ OK# |Setzt den Service-Zustand von {WARN} auf {OK}
|`wc` | [.guihint]#WARN ➤ CRIT# |Setzt den Service-Zustand von {WARN} auf {CRIT}
|`wu` | [.guihint]#WARN ➤ UNKNOWN# |Setzt den Service-Zustand von {WARN} auf {UNKNOWN}
|`cr` | [.guihint]#CRIT ➤ OK# |Setzt den Service-Zustand von {CRIT} auf {OK}
|`cw` | [.guihint]#CRIT ➤ WARN# |Setzt den Service-Zustand von {CRIT} auf {WARN}
|`cu` | [.guihint]#CRIT ➤ UNKNOWN# |Setzt den Service-Zustand von {CRIT} auf {UNKNOWN}
|`ur` | [.guihint]#UNKNOWN ➤ OK# |Setzt den Service-Zustand von {UNKNOWN} auf {OK}
|`uw` | [.guihint]#UNKNOWN ➤ WARN# |Setzt den Service-Zustand von {UNKNOWN} auf {WARN}
|`uc` | [.guihint]#UNKNOWN ➤ CRIT# |Setzt den Service-Zustand von {UNKNOWN} auf {CRIT}
|`?r` | [.guihint]#any ➤ OK# |Setzt jeden beliebigen Service-Zustand auf {OK}
|`?w` | [.guihint]#any ➤ WARN# |Setzt jeden beliebigen Service-Zustand auf {WARN}
|`?c` | [.guihint]#any ➤ CRIT# |Setzt jeden beliebigen Service-Zustand auf {CRIT}
|`?u` | [.guihint]#any ➤ UNKNOWN# |Setzt jeden beliebigen Service-Zustand auf {UNKNOWN}
|===
...
// Merker: Befehl für CMK < 1.6 zum Start von mknotifyd: omd -f config set MKNOTIFYD on
...
Die praktische Einrichtung von Benachrichtigungen mit nachvollziehbarer Zustellung per SMTP finden Sie in den xref:notification_rules#sync_html[globalen Benachrichtigungsregeln] und den xref:personal_notifications#sync_html[persönlichen Benachrichtigungsregeln] beschrieben.
...
`DOWNTIMESTART`: Beginn einer xref:glossar#scheduled_downtime[Wartungszeit] +
...
`ALERTHANDLER (...)`: xref:alert_handlers#[Alert Handler]-Ausführung (nur kommerzielle Editionen) +
...
Wenn Ihr Skript xref:notification_rules#bulk[Bulk-Benachrichtigungen] unterstützen soll, müssen Sie es speziell dafür präparieren, da hier dem Skript _mehrere Benachrichtigungen auf einmal_ übergeben werden.
...
Testen Sie das Skript wie xref:example_test[oben beschrieben] und aktivieren Sie zusätzlich in der Benachrichtigungsregel [.guihint]#Notification Bulking#.
