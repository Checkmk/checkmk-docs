...
// TK: Alter Text auskommentiert. Einige der Infos finden sich jetzt in commands##commands
// TK: Der neue Test ist hier nicht 100%ig passend, da im Kontext dieses Kapitels nur schnell eine Benachrichtigung per E-Mail abgesetzt werden soll - und nicht jede Einzelheit erklärt werden sollte, z.B. zu Benachrichtigungsregeln, die erst im nächsten Kapitel beschrieben werden.
////
...
Für den Test von Benachrichtigungen bietet {CMK} mit [.guihint]#Test notifications# ein smartes Werkzeug.
...
Mit der Checkbox [.guihint]#Send out notification# legen Sie fest, ob die Benachrichtigung nur simuliert oder auch tatsächlich versendet wird.
...
Die Zeile darunter fasst die aus Ihren Eingaben erzeugten Benachrichtigung zusammen.
Durch Klick auf das Symbol icon:icon_toggle_context[alt="Symbol zur Anzeige des Benachrichtigungskontexts."] können Sie den Benachrichtigungskontext einblenden, das heißt, der im Kontext dieser Benachrichtigung gültigen Umgebungsvariablen und ihrer Werte. 
...
An dieser Stelle ist nur die erste Spalte der Tabelle wichtig, die mit Symbolen anzeigt, welche der Regeln icon:icon_checkmark[alt="Symbol zur Anzeige eines positiven Status."] greifen und welche icon:icon_hyphen[alt="Symbol zur Anzeige eines negativen Status."] nicht.
