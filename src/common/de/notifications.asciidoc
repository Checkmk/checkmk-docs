...
[TIP]
====
In {CMK} {v23} können Sie mit [.guihint]#Test notifications# alle *Benachrichtigungsregeln* testen und Benachrichtigungen per *E-Mail* versenden.

Wenn Sie den tatsächlichen Versand von etwas anderem als E-Mails testen möchten, können Sie weiterhin xref:#fake_check_results[Fake check results] verwenden.
====
...
Zusätzlich zur Simulation können Sie Benachrichtigungen per E-Mail auch testweise versenden lassen.

Sie erreichen den Benachrichtigungstest am schnellsten über [.guihint]#Setup > Events > Notifications# und den Knopf [.guihint]#Test notifications.#
...
Eine Beschreibung des Ereignisses können Sie bei [.guihint]#Plugin output# hinzufügen.
Als Ereignis können Sie einen Zustandswechsel oder den Beginn einer xref:glossar#scheduled_downtime[Wartungszeit] wählen.
Mit der Checkbox [.guihint]#Send out HTML/ASCII email notification according to notification rules# legen Sie fest, ob die Benachrichtigung nur simuliert oder auch tatsächlich versendet wird.
...
Durch Klick auf [.guihint]#Test notifications# starten Sie den Test -- und auch den E-Mail-Versand, sofern Sie diese Option ausgewählt haben.
Der Dialog [.guihint]#Test notifications# wird aus- und die Ergebnisse eingeblendet:

image::notifications_test_results1.png[alt="Die Zusammenfassung der Simulationsresultate."]
...
image::notifications_test_results2.png[alt="Die Details der Simulationsresultate."]

Unter [.guihint]#Resulting notifications# sehen Sie, an wen und über welchen Weg Benachrichtigungen abgesetzt werden.
...
Unter [.guihint]#Global notification rules# werden die Benachrichtigungsregeln aufgelistet, die im xref:rules[nächsten Kapitel] genauer vorgestellt werden.
...
Im Beispiel greift die erste Regel nicht, da in dieser Zustandswechsel von Hosts ausgewertet werden, es sich aber beim Ereignis um einen Zustandswechsel eines Services handelt.
...
*Hinweis:* Für viele Benachrichtigungsmethoden zur Weiterleitung an andere Systeme erhalten Sie genauere Informationen in separaten Artikeln.
Die Liste der Artikel finden Sie im Kapitel zu den xref:includedscripts[Benachrichtigungsskripten.]
...
Falls Sie in der Benachrichtigungsmethode [.guihint]#Cancel previous notifications# gewählt haben, werden nur Benachrichtigungen an die hier gewählten Kontakte entfernt.

*Hinweis:* Falls Sie als Benachrichtigungsmethode ein Ticketsystem, einen Messenger oder eine Event Engine nutzen, beachten Sie zusätzlich die Hinweise zu diesen xref:ticketsystem[Sonderfällen.]
...
[#fake_check_results]
=== Benachrichtigungen mit [.guihint]#Fake check results# testen

Um andere Benachrichtigungsmethoden als E-Mail zu testen, können Sie einen {OK}-Service einfach von Hand auf {CRIT} setzen.
...
Sollten Sie das Kommando in der Liste nicht sehen, klicken Sie bitte vorher noch auf den Knopf [.guihint]#Show more# icon:button_showmore[].

image::notifications_fake_check_results.png[alt="Dialog zum manuellen Zustandswechsel."]

Wenn so alle Bedingungen einer Ihrer Benachrichtigungsregeln erfüllt sind, wird für diese Regel die eingestellte Benachrichtigungsmethode ausgeführt.
...
Allerdings ist die so erzeugte Benachrichtigung dann von einem anderen Typ und kann sich -- abhängig von Ihren Benachrichtigungsregeln -- anders verhalten.
