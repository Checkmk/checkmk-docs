...
Auch der Push-Modus von {CCE} ist auf den Agent Controller angewiesen und daher im Legacy-Modus nicht verfügbar.
...
[{shell-raw}]
----
{c-root} wget http://mycmkserver/mysite/check_mk/agents/check-mk-agent-{latest}-1.noarch.rpm
----
...
[{shell}]
----
{c-root} rpm -U check-mk-agent-{latest}-1.noarch.rpm
----
...
[{shell}]
----
{c-root} dpkg -i check-mk-agent_{latest}-1_all.deb
(Reading database ... 739920 files and directories currently installed.)
Preparing to unpack .../check-mk-agent_{latest}-1_all.deb ...
Unpacking check-mk-agent ({latest}-1) ...
Setting up check-mk-agent ({latest}-1) ...
----
...
[{shell-raw}]
----
{c-root} tar -C / --no-overwrite-dir -xvf /tmp/check-mk-agent_{latest}.tar.gz
----
