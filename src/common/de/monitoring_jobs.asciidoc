...
:description: {CMK} kann Cronjobs und andere zeitbasierte Prozesse mit dem Skript mk-job überwachen. Hier lesen Sie, wie Sie die Überwachung einrichten und nutzen.
...
Um diesen Job zu überwachen, ändern Sie mit einem Editor Ihres Vertrauens die Zeile ab, indem Sie dem Befehl `/usr/local/bin/backup >/dev/null` den Skriptnamen `mk-job` zusammen mit einem Namen für den Service (im Beispiel `nightly-backup`) voranstellen.
Der letztgenannte Name wird dann später als Service-Name in {CMK} genutzt und muss daher auf diesem Host einzigartig sein:

./etc/cron.d/backup
[source,subs=macros]
----
# Syntax:
# pass:c[<Minute> <Stunde> <Tag> <Monat> <Wochentag> <Benutzer> mk-job <Service-Name> <Befehl>]
...
Wird `mk-job` als unpriviligierter Benutzer ausgeführt, versucht dieses, fehlende Benutzerverzeichnisse anzulegen.
Dies wird in den meisten Fällen an fehlenden Berechtigungen scheitern.
Legen Sie daher zunächst ein Benutzerverzeichnis im `job`-Verzeichnis `/var/lib/check_mk_agent/job/` an.
Weisen Sie dann die Eigentümerschaft dem entsprechenden Benutzer zu, im folgenden Beispielbefehl dem Benutzer `myuser`:
...
Jetzt ist es `mk-job` möglich, im Namen des Benutzers die Ergebnisse in dieses Verzeichnis abzulegen.
...
ifdef::onprem[]
Sie können sich die Agentenausgabe mit dem folgenden xref:cmk_commandline#dump_agent[Befehl] anzeigen und die Ausgabe in den Befehl `less` weiterleiten:
...
Mit dem folgenden Befehl ändern Sie nur die Gruppenberechtigung.
