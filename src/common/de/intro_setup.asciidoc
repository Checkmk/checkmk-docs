...
Für die {SE} können Sie optional link:https://checkmk.com/de/produkt/support[von uns^] oder einem link:https://checkmk.com/de/partner/partner-finden[unserer Partner^] professionellen Support erhalten.
...
Die {CME} ist eine mandantenfähige Erweiterung der {CE} und verfügt über alle notwendigen Funktionen, um mit {CMK} über das xref:glossar#distributed_monitoring[verteilte Monitoring] voneinander abgeschottete Instanzen für mehrere Kunden zu betreiben. 
