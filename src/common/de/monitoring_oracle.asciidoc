...
*Achtung:* Das Oracle-Plugin verträgt sich nicht gut mit systemd (siehe link:https://checkmk.com/de/werk/13732[Werk pass:[#]13732^]).
...
Wie Sie bereits im vorherigen Abschnitt gelernt haben, können Sie Instanzen nicht nur komplett abschalten, sondern auch lediglich teilweise überwachen.
...
Um nur bestimmte Instanzen einzuschränken, können Sie die Variable `EXCLUDE_<sid>` nutzen, die Sie im xref:monitoring_oracle#skip_include[vorherigen Abschnitt] bereits kennengelernt haben.
...
* `long`: Wenn der Service eine lange Ausgabe für die xref:wato_services#discovery_auto[[.guihint]#Details#] haben soll, können Sie diese hier angeben.
