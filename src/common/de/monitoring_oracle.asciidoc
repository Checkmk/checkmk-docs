...
+
--
Unter Unix-artigen Systemen (Linux, Solaris, AIX) verwendet {CMK} ein einheitliches Agentenplugin.
...
[cols="~,~",options="header"]
|===
|Plugin-Pfad |Konfigurationspfad
|`C:\ProgramData\checkmk\agent\plugins` |`C:\ProgramData\checkmk\agent\config`
|===
...
Sie können auch bestimmen, ob es sich bei dem Benutzer um einen `SYSDBA` oder `SYSASM` handelt, auf welcher Kombination von Adresse und Port die Instanz lauscht und -- für Unix-artige Systeme -- sogar, welcher xref:tns_config[TNS-Alias] (`TNSALIAS`) benutzt werden soll.
