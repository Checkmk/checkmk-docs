...
Wir erweitern diese Plugins regelmäßig, sodass sich ein Blick in den Katalog immer wieder lohnt.
...
Das Agentenplugin für die Unix-artigen Betriebssysteme Linux, Solaris, AIX heißt `mk_oracle` und für Windows `mk_oracle.ps1`.
// TK: Oben Unix-artige eingeführt
...
+
--
Legen Sie auf dem Oracle-Host die Konfigurationsdatei `mk_oracle.cfg` an:
...
+
--
Legen Sie auf dem Oracle-Host die Konfigurationsdatei `mk_oracle_cfg.ps1` an.
Diese Datei ist ein PowerShell-Skript, das die benötigten Variablen setzt:
...
// ES: Auskommentierter Block ersetzt durch Tabs
////
...
Sie können die Überwachung unter Unix-artigen Systemen und unter Windows einrichten.
Allerdings stehen einige Möglichkeiten unter Windows nicht oder nur eingeschränkt zur Verfügung.
In den folgenden Abschnitten finden Sie alle Informationen zur Einrichtung in den verschiedenen Umgebungen.
...
Die Funktionalität ist an das Agentenplugin unter Unix-artigen Systemen angelehnt, enthält aber nur einen Teil davon.
...
// ES: xref für "Unix-artige Systeme" zum Tabwechsel einbauen, sobald diese Funktionalität verfügbar ist?
// TK: Links zwischen Tabs der gleichen Gruppe? Ich würd sagen: nein.
// TK: Hier wird aber auch deutlich, dass Unix-artige Systeme als Synonym für Linux, Solaris, AIX nur so halbgut ist, weil auf den Reitern eben nicht Unix-artige Systeme steht.
// TK: Eventuell an dieser Stelle ergänzen: ... an das Agentenplugin unter Unix-artigen Systemen (Linux, Solaris, AIX) angelehnt ...
...
|===
// TK: Hier ist die 1. Spalte eigentlich überflüssig. Kann man aber drinlassen wg. Symmetrie.
...
*Besonderheiten*
// ==== Besonderheiten // ES: kann innerhalb von Tab-Boxen nicht als H4-Überschrift dargestellt werden, daher hier zunächst nur in fettgedruckter Schrift
// TK: Man könnte auch den ganzen Abschnitt wieder aus dem Tab ziehen und als H4 Besonderheiten (nur Windows) einsortieren.
...
// TK: Der Halbsatz mit dem TNS-Alias gilt nicht für Windows: und sogar > und sogar für Unix-artige Systeme ...
// TK: Bin bei der letzten Bearbeitung dran gescheitert, herauszufinden, ob TNS-Alias nun ein Thema für Windows ist oder nicht. Ich hab einen Kommentar dazu ins Wiki geschrieben.
...
// ES: Letzter Satz kam ursprünglich nur aus dem Windows-Abschnitt - gilt er wirklich für beide Umgebungen? TK?
// TK: Ich hab keine Ahnung. Das weiß nur MaSl (vielleicht). Erschwerend kommt hinzu, dass ich den Satz gar nicht (mehr?) verstehe. Jede Angabe muss ausgefüllt sein, wenn sie benutzt wird? Hä?

Die zu Beginn angelegte xref:user_config[Konfigurationsdatei] kann also beispielsweise wie folgt erweitert werden:
...
// ES: Gehört das nachfolgende nicht eigentlich logisch noch zum Inhalt der Admonition? TK?
// TK: Ja, es gehört zur Admonition. Ich habs rausgezogen, weil ich in der Admonition keinen Dateiinhalt haben wollte. Kannst Du aber gerne ändern.
...
TNS-Alias werden in Unix-artigen Umgebungen in der Datei `tnsnames.ora` festgelegt.
...
Diese sind ähnlich zu den Angaben für `DBUSER`, die Sie bereits in der xref:advanced_user[erweiterten Benutzerkonfiguration] kennengelernt haben.
...
// ES: Nachfolgender auskommentierter Block ersetzt durch Tabs
////
// -->
[#linux]
...
Der wesentliche Unterschied zu einer manuellen Konfiguration ist, dass Sie nur noch dann auf dem Oracle-Host auf der Befehlszeile arbeiten müssen, wenn Sie spezielle Oracle-spezifische Konfigurationen vornehmen wollen.
...
Die Optionen, die in der xref:advanced_user[Konfigurationsdatei] mit Doppelpunkt (für Linux & Co.) oder als Listeneinträge (für Windows) separiert sind,
...
Mehr dazu im Abschnitt über die xref:sections[zu holenden Daten].
|[.guihint]#Exclude some sections on certain instances# |Wenn Sie mit `EXCLUDE_<SID>` nicht die gesamte Instanz, sondern nur ein paar Sektionen ausschließen wollen, können Sie dies mit dieser Option festlegen, so wie im Abschnitt über die xref:sections[zu holenden Daten] beschrieben.
...
Weitere Informationen finden Sie im Abschnitt xref:access[Zugriffsrechte bei der Ausführung].
Dort werden auch andere Dateien und Verzeichnisse für Linux genannt, z.B. `tnsnames.ora`, die nicht von der Agentenbäckerei erstellt werden.
...
Mehr zum Thema finden Sie im Windows-Bereich des Abschnitts xref:access[Zugriffsrechte bei der Ausführung].
|===
...
Das Agentenplugin `mk_oracle` (Linux, Solaris, AIX) beziehungsweise `mk_oracle.ps1` (Windows) bietet Ihnen verschiedene Möglichkeiten zur Fehlerdiagnose.
In den folgenden Abschnitten finden Sie Hinweise zur Diagnose in unterschiedlichen Umgebungen.

[#connection_test]
=== Verbindungstest

Sollten Sie Probleme mit der Verbindung zu einer oder mehreren Instanzen auf einem Oracle-Server haben, können Sie als Erstes grundlegende Parameter prüfen lassen.

Der im Folgenden beschriebene Verbindungstest überprüft auch, ob die notwendigen xref:access[Zugriffsrechte bei der Ausführung] gesetzt sind.
...
Beachten Sie, dass dem Agentenplugin zuvor die Pfade zu seiner Konfigurationsdatei und zu den zwischengespeicherten Daten des Plugins bekannt gemacht werden müssen.
In der Ausgabe wurden die _Dummy-Sektionen_ zugunsten der Lesbarkeit weggelassen.

Im Folgenden das Beispiel für einen Linux-Server mit `systemd`, auf dem das Agentenplugin alle 60 Sekunden xref:install_plugin[asynchron] ausgeführt wird:
...
Hier werden Sie Informationen darüber bekommen, wie das Plugin versucht, auf die Instanzen zuzugreifen.
...
--
====
// TK: Obacht: Im obigen powershell-Block falsche Umsetzung von <<<oracle_instances>>> und <<<oracle_instance>>> in [{powershell}]
...
Hier der Aufruf für einen Linux-Server mit `systemd`, auf dem das Agentenplugin alle 60 Sekunden xref:install_plugin[asynchron] ausgeführt wird:
...
// TK: Der Satz oben gehört zur Linux & Co. Ich sehe nicht, dass unter Windows überhaupt eine Log-Datei erzeugt wird. Ich glaube, dass die ganzen Log-Meldungen in der Befehlsausgabe stecken. Aber wer kennt sich schon mit Oracle aus...
...
Diese Ausgabe ist entsprechend auch die umfangreichste und sicherlich auch die am schwersten zu lesende Möglichkeit, an die Ursache eines Problems zu kommen, und sollte daher auch nur als letztes Mittel eingesetzt werden.
...
+
--
Im Folgenden das Beispiel für das Debugging auf einem Linux-Server mit `systemd`, auf dem das Agentenplugin alle 60 Sekunden xref:install_plugin[asynchron] ausgeführt wird:
...
+
--
Unter Windows können Sie dem Agentenplugin keine Argumente übergeben und müssen das Tracing daher manuell anschalten, um die vollständige Ausgabe aller Schritte zu erhalten:
