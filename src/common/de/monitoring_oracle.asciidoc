...
Sofern Sie keine xref:rac[Real Application Cluster (RAC)] nutzen, legen Sie dafür auf jeder zu überwachenden Datenbank einen Benutzer an.
Wie Sie sich auf einer Instanz anmelden, unterscheidet sich je nachdem, welches Betriebssystem Sie einsetzen.
Für Linux können Sie das zum Beispiel erledigen, indem Sie zuerst immer die jeweilige Instanz als Umgebungsvariable setzen, für die ein Benutzer angelegt werden soll.
...
// TK: Hier fehlt ein Satz dazu, was Multi-Tenant-Datenbanken sind.
...
// TK: Hier fällt der Begriff Container vom Himmel.
...
Mit diesem letzten Schritt haben Sie die Voraussetzungen geschaffen, um die Zugangsdaten aus der xref:user_config[Konfigurationsdatei des Agentenplugins] herauszunehmen.
...
// TK: So steht's 1:1 in der mk_oracle help, aber wirft Fragen auf: Wenn es für ASM-SID diese spezielle Syntax gibt, muss das dann nicht bei SKIP_SIDS beschrieben werden?
|===
...
Der nicht privilegierte Benutzer benötigt die folgenden Berechtigungen:

* Lesen der Datei `$TNS_ADMIN/sqlnet.ora`.
* Lesen der Datei `$TNS_ADMIN/tnsnames.ora`.
* Ausführen des Wallet-Ordners. +
Der Wallet-Ordner ist `/etc/check_mk/oracle_wallet/`, falls Sie so vorgegangen sind, wie im Abschnitt zur xref:oracle_wallet[Oracle-Wallet] beschrieben.
* Lesen der Dateien im Wallet-Ordner.

In der Regel wird der nicht privilegierte Benutzer `oracle` sein.
Allerdings hängt dies von Ihrer Oracle-Installation ab.
...
|[.guihint]#sqlnet.ora permission group (Linux/AIX/Solaris only)# |Tragen Sie hier die Linux-Gruppe des nicht privilegierten Benutzers ein, der die Oracle-Binärprogramme ausführt, damit dieser Benutzer die Datei `sqlnet.ora` lesen kann, die von der Agentenbäckerei erstellt wird.
...
====
// TK: Wo finde ich elapsed wieder? Wird mir das in meine MySQL.sql geschrieben?
...
// TK: Infos (wenn auch dürftig) in der mk_oracle help und hier: https://checkmk.com/werk/7095
...
// TK: Wird nicht in der mk_oracle help erwähnt, sondern nur hier: https://checkmk.com/de/werk/10671
