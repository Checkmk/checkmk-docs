...
// SK: Ich habe mal angefangen anzufragen, was hier mit der Suche im Katalog los ist. Ich würde eigentlich erwarten, dass die folgende URL vernünftige Ergebnisse bringt und bspw. alles Tablespace-plugins anzeigt. Geht aber derzeit nicht. https://checkmk.com/integrations?distributions%5B%5D=check_mk&distributions%5B%5D=check_mk_cloud&search=oracle+tablespaces
// SK: Mit solchen Suchen habe ich auch in monitoring_gcp gearbeitet. Das geht eigentlich sehr gut.
...
[#first_steps]
== Ersteinrichtung
// SK: Die erstmalige Installation des Agentenplugins gehört wohl nicht zur Ersteinrichtung. Die wird ja in einem anderen Kapitel beschrieben. Vielleicht sollten wir uns mal einheitliche Kapitelnamen für diese vorbereitenden Schritte überlegen.
...
Danach melden Sie sich auf der Instanz an und legen einen Benutzer für das Monitoring an.
...
//SK: Werden diese ganzen Alternativen denn ausschließlich dann geprüft und verwendet, wenn dieser Schrägstrich in der mk_oracle.cfg steht? Muss bei der Verwendung der Wallet die u.a. Zeile zwingen vorhanden sein?
...
[#linux]
== Linux, Solaris, AIX
// SK: Eigentlich ist die Überschrift schon seltsam. "Linux, Solaris, AIX" was? In 3.2 erfährt man dann, dass das Agentenplugin installiert wird und besinnt sich dann zurück auf die Überschrift darüber.
...
//SK: Stimmt die Anzahl der Doppelpunkte bei DBUSER_MYINST2? Und was ist das INST2 am Ende?
...
//SK: Eigentlich klar, aber vielleicht könnte man in einem Halbsatz erwähnen, wo diese Voraussetzungen erfüllt sein müssen. Auch der erste Satz unter der Liste deutet das nur an und sagt es nicht explizit.
...
//SK: Jeder Variablen wird eine ID "übergeben"? Wo wird die übergeben? Ist damit der Suffix '_myinst1' gemeint? Das ist für mich kein "übergeben".
...
// SK: Ich tippe mal auf die mk_oracle.cfg. Wie das allerdings mit der Wallet geht, bleibt erstmal offen.
