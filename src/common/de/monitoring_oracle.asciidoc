...
// TK: Die Liste ist nicht vollständig und soll es auch nicht sein. Sie ist trotzdem zu lang und sollte auf die wichtigsten Check-Plugins reduziert werden.
// TK: Die Typos in den Check-Plugin Namen sollten wir korrigieren lassen (perfomance > performance, ORACLE tablespaces > Oracle Tablespaces). Stehen nicht in Weblate, weil die nicht übersetzt werden. 
...
Sofern Sie keine xref:rac[Real Application Cluster (RAC)] nutzen, legen Sie dafür in jeder zu überwachenden Datenbank einen Benutzer an.
...
Danach melden Sie sich auf der Instanz ein und legen einen Benutzer für das Monitoring an.
...
//TODO: Hier ausführlich die Unterschiede beschreiben. Siehe Werk #6480.
...
In dem folgenden Beispiel sehen Sie die Datei für die unixoiden Systeme:
...
Alle anderen Einstellungen, die Sie unter xref:adv_linux[unixoiden Systemen] oder unter xref:adv_windows[Windows] verwenden können, sind optional.
...
// TK: Oracle-Wallet gibt es auch unter Windows, die Einrichtung ist aber nur für Linux beschrieben.

Legen Sie zunächst die Datei `sqlnet.ora` an.
// TK: Das liest sich wie eine ausschließlich manuelle Konfiguration. Aber die sqlnet.ora wird auch von der Bakery geschrieben (die tnsnames.ora dagegen nicht). Finde nur ich das überraschend?
...
----
// TK: Den Sqlnet Send timeout, den man in der Oracle-Regel per Bakery setzen kann, lässt sich auch manuell in der sqlnet.ora setzen (SQLNET.SEND_TIMEOUT = 30) - auch wenns mit der Wallet nichts zu tun hat.
...
Machen Sie Eigentümer und Gruppe der vom Agentenplugin `mk_oracle` ausgeführten Oracle-Binärprogramme (wie z.B. `$ORACLE_HOME/bin/sqlplus`) zum Eigentümer und zur Gruppe der manuell erstellten Dateien und Verzeichnisse in `/etc/check_mk/`.
Die folgende Kommandos ändern zum Beispiel den Eigentümer zu `oracle` und die Gruppe zu `oinstall`:

[{shell}]
----
{c-root} chown oracle:oinstall /etc/check_mk/sqlnet.ora /etc/check_mk/tnsnames.ora
{c-root} chown -R oracle:oinstall /etc/check_mk/oracle_wallet
----

Anschließend sollten die Berechtigungen für Eigentümer und Gruppe etwa wie folgt aussehen:

[{shell}]
----
{c-root} tree -ugpR /etc/check_mk
[drwxr-xr-x root     root    ]  /etc/check_mk
├── [drwxr-x--- oracle   oinstall ]  oracle_wallet
│   └── [-rw-r----- oracle   oinstall ]  cwallet.sso
│   └── [-rw------- oracle   oinstall ]  cwallet.sso.lck
│   └── [-rw-r----- oracle   oinstall ]  ewallet.p12
│   └── [-rw------- oracle   oinstall ]  ewallet.p12.lck
├── [-rw-r--r-- oracle   oinstall ]  sqlnet.ora
├── [-rw-r--r-- oracle   oinstall ]  tnsnames.ora
----
...
Unter unixoiden Systemen verwendet {CMK} ein einheitliches Agentenplugin.
...
[IMPORTANT]
====
Das Agentenplugin für unixoide Systeme `mk_oracle` verträgt sich nicht gut mit `systemd` (siehe link:https://checkmk.com/de/werk/13732[Werk pass:[#]13732^]).
...
* Wird ein optionales Feld mittendrin ausgelassen, muss der Doppelpunkt geschrieben werden, wie beim Eintrag `DBUSER_MYINST2`, bei dem keine spezielle Rolle angegeben wurde.
* Werden ab einem bestimmten Punkt keine optionalen Felder mehr gebraucht, können Sie die Doppelpunkte weglassen, wie beim Eintrag `ASMUSER`, bei dem weder der Host, noch der Port angegeben wurde.
...
// TK: So steht's 1:1 in der mk_oracle help, aber wirft Fragen auf: Wenn es für ASM-SID diese spezielle Syntax gibt, muss das dann nicht bei SKIP_SIDS beschrieben werden? Siehe Werk #6767.
|===
...
|===
// TK: MAX_TASKS passt nicht wirklich zur Überschrift dieses Abschnitts.
// TK: OLRLOC Parameter in Konfigdatei (nur für Linux) ist nicht beschrieben. Siehe Werk #7006.
...
// TK: TNS alias und TNS_ADMIN gibt es auch unter Windows - und beide werden in mk_oracle.ps1 auch benutzt. Fehlt hier was?
...
// TK: Die Option Add pre or postfix to TNSALIASes in der Oracle-Regel kann auch in der Konfigurationsdatei konfiguriert werden. Siehe Werk #6492 (nicht selbsterklärend).
...
Standardmäßig ist `TNS_ADMIN` auf `$ORACLE_HOME/network/admin` gesetzt.
In der Konfigurationsdatei können Sie `TNS_ADMIN` wie folgt setzen:
...
----
// TK: Der Beispielwert ist unglücklich gewählt, weil /etc/check_mk/ der Default ist, wenn TNS_ADMIN *nicht* gesetzt ist.
...
Sie sollten keine Probleme mit den Zugriffsrechten haben, sofern Sie Eigentümer und Gruppe der Dateien und Verzeichnisse so geändert haben, wie es am Ende des Abschnitts zur xref:oracle_wallet[Oracle-Wallet] beschrieben ist.
...
Unter unixoiden Systemen können Sie nicht nur lokal laufende Instanzen abrufen, sondern sich auch auf entfernten anmelden und die dort laufenden Datenbanken abrufen.
...
Um entfernte Datenbanken zu überwachen, gibt es die folgenden Voraussetzungen:
...
In der Regel sind die Voraussetzungen schon erfüllt, wenn sich auf dem Host, auf dem das Agentenplugin ausgeführt werden soll, eine Oracle-Installation befindet.
...
Damit jede Zeile eindeutig ist, wird jeder Variablen eine ID übergeben.
...
* Sie können -- logischerweise -- entfernte Instanzen nicht per `SKIP_SIDS` xref:skip_include[ausschließen] und brauchen sie im Gegenzug auch nicht mit `ONLY_SIDS` einzuschließen.
// TK: Warum logischerweise? Ich finds nicht selbsterklärend.
...
[#windows]
== Windows
// TK: Das Windows-Kapitel steht nicht für sich, sondern verlinkt ins Linux-Kapitel (bei den Themen Instanzen ein- oder ausschließen, Zu holende Daten bestimmen): Nicht zu schlimm, aber auch nicht schön.

=== Plugin- und Konfigurationspfade

Unter Windows wird PowerShell als Skriptsprache benutzt, um Oracle-Datenbanken zu überwachen.
Die Funktionalität ist an das Agentenplugin unter xref:linux[unixoiden Systemen] angelehnt, enthält aber nur einen Teil davon.
...
[TIP]
====
Da der Legacy-Agent nur noch in sehr seltenen Ausnahmefällen benötigt wird, wird er bei der folgenden Beschreibung nicht beachtet.
Es wird sich daher immer auf die aktuelle Architektur des Agenten bezogen.
====
////
...
// TK: Wie zuvor bei Linux & Co. wird auch hier implizit mit +ASM eine andere Syntax verwendet, ohne dass diese erklärt wird. Oder muss die ASM-SID immer mit einem + beginnen?
|===
...
Als Administratoren unter Windows gelten dabei das lokale Systemkonto (`Local System`) und Mitglieder der eingebauten Gruppe `Administrators`.
...
Wenn Sie entfernte Datenbanken überwachen wollen, benötigen Sie daher einen Host mit einem kompatiblen xref:linux[unixoiden Betriebssystem.]
...
Zum einen dann, wenn es sich um veraltete Konfigurationsoptionen handelt, die nur noch aus Kompatibilitätsgründen vorhanden sind.
Diese lassen sich dann entsprechend durch die aktuellen Optionen ersetzen.
// TK: Die beiden Sätze verstehe ich nicht: Um welche veralteten Konfigurationsoptionen geht es hier?
Zum anderen, wenn es Funktionen sind, welche einen größeren Eingriff benötigen und ein fundiertes Fachwissen voraussetzen.
Entsprechend sind die xref:custom_sqls[benutzerdefinierten SQL-Abfragen] nicht direkt in der Agentenbäckerei konfigurierbar.
// TK: Was heißt hier "nicht direkt"?

// MA: Hier fehlt noch eine Anleitung, wie man das mit Custom-Dateien und mk_oracle.d machen kann. Siehe Werk #10848.
...
In der xref:user_config[einfachsten Konfiguration] für eines der unixoiden Betriebssysteme sieht die Regel etwa so aus:
...
|===
|Option |Beschreibung
|[.guihint]#Host uses xinetd or systemd (Linux/AIX/Solaris only)# |Diese Option muss für unixoide Systeme mit `xinetd`/`systemd` aktiviert werden.
...
|[.guihint]#Sqlnet Send timeout# |Diese Option wird der Datei `sqlnet.ora` hinzugefügt und setzt ein Timeout für die Instanz selbst.
// TK: Was meint "Instanz selbst"? Welche Instanz ist gemeint? Da der Eintrag (SQLNET.SEND_TIMEOUT = 30) nicht instanzspezifisch ist, gilt er dann nicht vielmehr global für alle Instanzen?
...
Diese wirken sich lediglich auf die erwähnte ID aus und müssen lediglich einzigartig sein.
// TK: Was ist mit dem vorherigen Satz gemeint?
...
// TK: Hier fehlt das Beispiel für die Windows-Konfigurationsdatei.
...
====
// TK: Wo finde ich elapsed wieder? Wird mir das in meine MySQL.sql geschrieben? Siehe Werk #10504 (sehr dünne).
...
Die Möglichkeiten, mit benutzerdefinierte SQL-Abfragen zu überwachen, gehen natürlich über das einfache, oben gezeigte Beispiel hinaus.
...
// TK: Infos (wenn auch dürftig) in der mk_oracle help und in Werk #7095.
...
// TK: Wird nicht in der mk_oracle help erwähnt, sondern nur hier: Werk #10671
...
Beachten Sie, dass dem Agentenplugin zuvor der Pfad zu seiner Konfigurationsdatei bekannt gemacht werden muss, da es sich darauf verlässt.
In der Ausgabe wurden die _Dummy-Sektionen_ zugunsten der Lesbarkeit weggelassen.
Im Folgenden also das Beispiel auf einem Linux-Server:
...
Auch hier benötigen Sie den Pfad zu der Konfigurationsdatei und zusätzlich auch den Pfad zu den zwischengespeicherten Daten des Plugins.
...
eingesetzt werden.
Sie können das Debugging mit der Option `-d` aufrufen.
Vergessen Sie auch hier nicht die notwendige Umgebungsvariable:
...
|===
|Pfad |Bedeutung
|`~/share/check_mk/agents/plugins/mk_oracle` |Das Agentenplugin für unixoide Systeme, welches auf dem Oracle-Host die Daten holt.
|`~/share/check_mk/agents/plugins/mk_oracle_crs` |Dieses Agentenplugin für unixoide Systeme liefert Daten zu einem Oracle Cluster Manager.
...
|`~/share/check_mk/agents/cfg_examples/` |Hier befinden sich beispielhafte Konfigurationsdateien für unixoide Systeme in den Dateien `mk_oracle.cfg`, `sqlnet.ora` und `tnsnames.ora`.
