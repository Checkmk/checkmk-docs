...
Bevorzugte Installationsmethode ist das Paketmanagementwerkzeug Ihrer Linux-Distribution.
...
[TIP]
====
In manchen Fällen sind Sie für die Installation einer hinreichend aktuellen Version des Python-Moduls `docker` auf eine link:https://docs.python.org/3/library/venv.html[Python Virtual Environment (`venv`)] angewiesen.
