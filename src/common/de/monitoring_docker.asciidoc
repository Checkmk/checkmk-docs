...
[TIP]
====
Die Pakete `docker-py` beziehungsweise `python-docker-py` dürfen nicht installiert sein.
...
// MFS: Auch hier gilt, dass man mit pip3 nicht an mit dem Paketmanager installierten Modulen herumfummeln sollte:
////
...
// MFS: Versionsnennung entfernt, da mutmaßlich derzeit ({CMK} {v24}) 5.x benötigt.
...
'5.0.3'
----
// ES: Ich erhalte (innerhalb eines venv) als Ausgabe bei `docker.version` nur das Modul als Objekt, keinen Versions-String:
// >>> import docker
// >>> docker.version
// <module 'docker.version' from '/.../pydocker_env/lib/python3.12/site-packages/docker/version.py'>
// ES: Um tatsächlich eine Versionsnummer zu sehen, gebe ich docker.__version__ ein - unsicher, ob es hierfür eine schönere Abkürzung gibt
// >>> docker.__version__
// '7.1.0'
...
// MFS: Standardweg sollte die Installation per Paketmanager sein, da sonst die Gefahr besteht, mitgelieferte Pakete zu überschreiben und so u.U. sogar Systemwerkzeuge (do-release-upgrade) unbrauchbar zu machen.
////
...
// ES: Hier wäre ein Hinweis gut, woran wir erkennen, dass der Paketmanager das richtige Paket installiert. In `apt list` finde ich python3-docker -- ist das kompatibel oder nicht?
// ES: Falls wir keinen konkreten Paketnamen angeben können/wollen, würde ich sogar bevorzugen, die Installation im venv als empfohlenen Weg zu bezeichnen. Da hat man einfach die volle Kontrolle, macht nix anderes kaputt und kann bequem pip benutzen. Und der Befehl `pip install docker` kann 1:1 vom Handbuch ins venv übernommen werden.
