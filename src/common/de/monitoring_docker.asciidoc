...
Wenn Sie jetzt in {CMK} die xref:wato_services#[Service-Erkennung] durchführen und die ausstehenden Änderungen aktivieren, sollten Sie zunächst einige neue Services finden, welche den Docker-Node selbst betreffen:
...
Des Weiteren gibt es die Möglichkeit, vor den Container-Namen noch den Namen des Docker-Hosts zu setzen.
Das geht mit der Option `combined`.
Zur Ermittlung des Namen des Docker-Hosts ziehen wir den Output von `uname -n` heran.
