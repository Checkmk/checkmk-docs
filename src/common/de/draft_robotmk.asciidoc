...
Sie können die Funktion allerdings mit bis zu drei Tests ausgiebig testen.
...
Das Besondere an Robot Framework: Tests werden nicht in einer echten Programmiersprache, sondern simplem Low-Code geschrieben, einfach zu verwendenden Keywords wie `Open Browser`.
...
Robotmk kann nun diese Robot-Framework-Test-Suites auf dem Host triggern und ihre Ausführung und Resultate als Services in {CMK} monitoren.
...
==== {CMK}-Server
{CMK} Synthetic Monitoring wird über ein Agentenplugin realisiert, das über die Regel [.guihint]#Robotmk Scheduler# aktiviert und konfiguriert wird.
Hier legen Sie fest, welche Test-Suites ausgeführt werden sollen und wie exakt Robot Framework diese verarbeiten soll -- zusammengefasst in einem _Plan._
...
Verantwortlich dafür ist das quelloffene Kommandozeilen-Werkzeug link:https://github.com/robocorp/rcc[RCC:^] 
Dieses baut anhand Ihrer Konfigurationsdateien im YAML-Format virtuelle Python-Umgebungen samt Abhängigkeiten und Robot Framework selbst -- und sorgt auch für die Ausführung der Tests.
...
Die Beispiel-Suite können Sie als ZIP-Datei link:https://github.com/elabit/robotmk-examples/blob/main/minimal.zip[über GitHub herunterladen.^]
...
[TIP]
====
Wichtig: RCC kann grundsätzlich auch Test-Suites auf Basis von zum Beispiel Python verarbeiten, für den Einsatz in {CMK} muss es aber die Deklaration nach Robot Framework sein.
====
...
Nur wenn die Umgebung erfolgreich gebaut wird, können auch die Tests abgearbeitet und folglich gemonitort werden.
...
Der Grund ist simpel: In der Praxis geht es häufig um Tests, die auf dem Desktop ausgeführt werden und da könnten sich mehrere Test-Suites gleichzeitig in die Quere kommen (sich gegenseitig "die Maus klauen").

Die Plangruppen sind nun eine Kapselung für Pläne -- und werden selbst parallel ausgeführt.
...
Die Pfadangabe ist relativ zum oben angegebenen Basisverzeichnis, also beispielsweise schlicht `mybot` für ein Verzeichnis `C:\robots\mybot`.
Alternativ kann hier auch direkt eine robot-Datei angegeben werden, etwa `C:\robots\mybot\test.robot` -- je nach Aufbau Ihres Robot-Framework-Projekts.
...
Diese Angabe ist abhängig davon, ob Sie oben als [.guihint]#Relative path to test suite file or folder# ein Verzeichnis oder eine robot-Datei angegeben haben.
...
// ML: Passt die LocalSystem-Angabe so?
...
Das geht, weil RCC clever ist: Einmal gebaute Umgebungen werden ganz oder teilweise wiederverwendet -- der erste Bau hätte hier eher 30+ Sekunden gedauert.
...
Nun hat Windows 11 standardmäßig Aliasse für `python.exe` und `python3.exe` vorgegeben, die auf den Microsoft-Store verweisen.
