...
==== {CMK}-Server
{CMK} Synthetic Monitoring wird über Robotmk realisiert, das ein Agentenplugin als Datensammler nutzt und den Robotmk-Scheduler für das Triggern von Robot-Framework-Projekten.
...
Der Scheduler stößt diesen Bau an und sorgt anschließend selbst für die Ausführung der Tests.
...
Nützlich ist dies insbesondere dann, wenn Ihre Test so organisiert sind, dass die übergeordneten Keywords das "Was" beschreiben und die darunter liegenden Keywords das "Wie" -- so bekommen Sie abstraktere Auswergunten.
...
Der erste Bau hätte hier eher 30+ Sekunden gedauert.
