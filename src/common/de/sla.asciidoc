// -*- coding: utf-8 -*-
// IGNORE Aggr Ansichts Filesystems MyBiPack MyTopLevelRule mins subresults timeperiod timerange
...
:description: {CMK} kann die Erfüllung von Service Level Agreements (SLAs) detailliert auswerten. Lesen Sie hier, wie Sie komplexe SLAs konfigurieren und nutzen.
...
In {CMK} können Sie xref:availability#[Verfügbarkeiten] auswerten und für diese auch eine rudimentäre xref:availability#thresholds[Auswertung von Service Level Agreements (SLAs)] konfigurieren.
...
Und auch die SLAs stehen den Aggregaten zur Verfügung -- über einen kleinen Umweg:
Der Zustand eines BI-Aggregats kann über den BI-Spezialagenten als ganz normaler Service überwacht werden.
Dieser erscheint dann beispielsweise als [.guihint]#Aggr Disk & Filesystems# in den Ansichten
und kann wiederum über die bereits xref:link_sla_service[oben] genutzte Regel [.guihint]#Assign SLA definition to service# mit einem SLA verknüpft werden.
Im Ergebnis sieht das dann etwa so aus:

.Der Service zeigt die SLA-Informationen eines BI-Aggregats
image::sla_view_bi_2.png[alt="Eine Ansicht mit den SLA-Informationen eines BI-Aggregats.",width=97%]

Sie finden die Spezialagentenregel unter [.guihint]#Setup > Agents > Other integrations > BI Aggregations# und eine Beschreibung des Regelsatzes im xref:bi#biasservice[BI-Artikel.] 

Der Spezialagent benötigt hier zum einen die betroffene Instanz und dazu gegebenenfalls Login-Daten, sofern es sich um eine entfernte Instanz handelt.
Die wesentliche Konfiguration findet unter [.guihint]#Filter aggregations# statt.
Hier geben Sie alle Aggregate an, die Sie als separate Services ins Monitoring holen wollen.
Unter [.guihint]#By aggregation name (exact match)# geben Sie den Titel der obersten Regel eines Aggregats an -- also zum Beispiel [.guihint]#Disk & Filesystems# (welche dann ergänzt um [.guihint]#Aggr# als neuer Service [.guihint]#Aggr Disk & Filesystems# entdeckt wird).

.Konfiguration des Spezialagenten 
image::sla_bi_agent_2.png[alt="Konfiguration des BI-Spezialagenten."]

Die exakten Namen der gewünschten Regeln/Aggregate finden Sie -- beispielsweise -- unter [.guihint]#Setup > Business Intelligence > Business Intelligence > Default Pack - Aggregations:#

.Die Titel der Regeln in Aggregaten
image::sla_bi_rules_2.png[alt="Liste von BI-Regeln zur Auswahl einer Regel."]

*Hinweis:* Hier besteht *Verwechslungsgefahr*:
In der BI-Konfiguration erstellen Sie die eigentliche Aggregation, also die Logik, über Regeln -- und diese Regeln werden hier über ihre Titel als [.guihint]#Aggregation Name# eingetragen.
