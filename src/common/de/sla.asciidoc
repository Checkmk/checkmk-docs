...
Dieser erscheint dann beispielsweise als [.guihint]#Aggr Disk & Filesystems# in den Ansichten und kann wiederum über die bereits xref:link_sla_service[oben] genutzte Regel [.guihint]#Assign SLA definition to service# mit einem SLA verknüpft werden.
...
Hier geben Sie alle Aggregationen an, die Sie als separate Services ins Monitoring holen wollen.
Unter [.guihint]#By aggregation name (exact match)# geben Sie den Titel der obersten Regel einer Aggregation an -- also zum Beispiel [.guihint]#Disk & Filesystems# (welche dann ergänzt um [.guihint]#Aggr# als neuer Service [.guihint]#Aggr Disk & Filesystems# entdeckt wird).

.Konfiguration des Spezialagenten 
image::sla_bi_agent_2.png[alt="Konfiguration des BI-Spezialagenten."]

Die exakten Namen der gewünschten Regeln/Aggregationen finden Sie -- beispielsweise -- unter [.guihint]#Setup > Business Intelligence > Business Intelligence > Default Pack - Aggregations:#

.Die Titel der Regeln in Aggregationen
