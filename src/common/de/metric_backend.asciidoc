...
ifdef::onprem[]
{cee-only} Bislang speicherte {CUE}
endif::[]
ifdef::saas[]
Bislang speicherte {CMK}
endif::[]
Metriken in xref:graphing#rrds[Round Robin Databases] (kurz RRDs), Informationen zu Ereignissen (_events_) in SQLite-Dateien und Inventardaten in JSON-Dateien.
...
die Entscheidung, zunächst für OpenTelemetry-Daten eine spaltenbasierte (_column based_) Datenbank als Metrik-Backend zu verwenden.
...
Derzeit kommt das Metrik-Backend nur für xref:opentelemetry#[OpenTelemetry-Metriken] zum Einsatz.
...
ifdef::saas[]
Eine explizite Einrichtung ist nicht notwendig.
{com} betreibt pro AWS-Region ein ClickHouse Cluster.
Bei der Erstellung Ihrer {CMK}-xref:glossar#site[Instanz] wurde die ihr zugeordnete Datenbank in diesem Cluster bereits erstellt.
endif::[]
...
Die aktuelle Zahl der Custom Metrics können Sie jederzeit über den Service [.guihint]#OMD mysite metric backend# im Selbst-Monitoring Ihrer {CMK}-Instanz einsehen.
...
// Haben wir eine absolute Höchstgrenze?
