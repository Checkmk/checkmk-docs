...
{cee-only}
In den kommerziellen Editionen rufen Sie zunächst die Regel für das Agentenplugin [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Count, size and age of files - mk_filestats (Linux)# auf.
...
{cre-only}
In der {CRE} konfigurieren Sie das Plugin wie üblich über eine Textdatei:
...
{cre-only}
In der {CRE} müssen Sie auch bei fileinfo wieder über Dateien konfigurieren, unterschiedlich nach Betriebssystem:
