...
:description: {CMK} kann Dateien bezüglich Alter, Größe und Anzahl überwachen. Lesen Sie hier, wie Sie dafür mit den Regelsätzen mk_filestats und fileinfo arbeiten.
...
Beide Varianten von Service-Regeln gibt es zusätzlich als xref:wato_services#enforced_services[erzwungene Services].
...
Fügen Sie anschließend wie üblich den neuen Service zu den betroffenen Hosts hinzu und aktivieren Sie die ausstehenden Änderungen.
...
Ausschließlich auf Windows-Hosts können Sie mit dem sogenannten Globstar (zwei aufeinanderfolgende Sternchen) auch gleich alle Dateien in *allen* Unterverzeichnissen einbeziehen.
