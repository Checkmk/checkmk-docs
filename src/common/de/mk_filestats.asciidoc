...
ifdef::onprem[]
{cee-only}
In den kommerziellen Editionen rufenl
endif::[]
ifdef::saas[]
Rufen
endif::[]
...
ifdef::saas[]
Die Konfiguration des Agenten über die Agent Bakery mittels [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Count, size and age of files (Linux, Windows)# ist deutlich einfacher gehalten:
endif::[]
