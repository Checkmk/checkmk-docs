...
Ersetzen Sie dazu die Beispielwerte der folgenden Variablen:
...
- `jira_url`: Die URL zur Jira-Instanz (häufig etwa _pass:[https://MY_JIRA_URL.atlassian.net]_)
...
====
// ES: Nach dieser Admonition würde ich erwarten, dass das Skript unten nochmal drauf hinweist, was man bei onprem anders machen soll - nicht darauf, was das besondere an Cloud ist. Schließlich stellen wir Cloud ja jetzt als de-facto-Standard dar. Mein Vorschlag:
// # Jira (self-hosted) or API version <= 2: use username + password instead
...
// ES: Es soll wohl etwas einfacher über Project -> Details bzw. Project -> Settings -> URL angucken gehen. Kannst du das bitte verifizieren und dann die Erklärung hier anpassen?
// ES: Außerdem wäre es aus meiner Sicht viel besser, wenn die Erklärung der Begriffe Space/Project hier nicht als störender Einschub erscheint. Gerne als Tip oder Collapsible oder einfach als Ein-Satz-Absatz separat einfügen, danach erklären, was zu tun ist.
