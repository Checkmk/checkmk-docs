...
*Projekt vs. Space:* Atlassian hat vor einiger Zeit den Begriff `Project` durch `Space` ersetzt -- allerdings nur in der GUI, nicht im technischen Unterbau, sprich URLs und APIs.
Daher entspricht die Space-ID in technischen Ausgaben der `projectid`.

Die `projectid` zu finden ist etwas umständlich: Öffnen Sie die [.guihint]#Space settings# und klicken Sie in der Navigationsleiste links auf den Punkt [.guihint]#Toolchain# -- die aufgerufene URL enthält dann ganz am Ende einen Eintrag in der Art `projectId=10000`.
Alternativ können Sie auch die URL `MY_JIRA_URL.atlassian.net/rest/api/3/project/MY_SPACE_KEY` aufrufen und im angezeigten JSON-Dokument nachschauen.
