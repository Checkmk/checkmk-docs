...
:description: {CMK} erlaubt Installation und Ausführung von Linux-/Unix-Agenten als unprivilegierter Nutzer. Das reduziert Folgen defekter oder manipulierter Plugins.
...
Seit {CMK} {v25} ist auch der xref:agent_deployment#[Agent Updater] als Agentenplugin mit der unprivilegierten Ausführung kompatibel.
...
Es ist geplant, beide in einer künftigen Version von {CMK} zu entfernen.

// ES: Ist das hier nach einem Jahr immer noch ein technical preview?
...
Beachten Sie, dass Ihre Auswahl für den Benutzer des Agent Controllers seit {CMK} {v25} auch den xref:agent_deployment#[Agent Updater] betrifft, da dieser unter dem Benutzer des Agent Controllers ausgeführt wird.
Nur zum Installieren von Paketen wechselt der Updater vorübergehend zur privilegierten Ausführung.
...
Betroffen davon sind in {CMK} {v25} `mdadm` (zum Auslesen des Zustandes verschiedener Software-RAIDs und verschlüsselter Laufwerke), `mailq` (zum Auslesen der E-Mail-Warteschlange des Postfix-MTA), die Skripte zum Monitoring von {CMK}-Instanzen, sowie vom link:https://checkmk.com/integrations/veritas_vcs[Veritas Cluster Service^] Plugin benötigte Befehle.
...
Mit längeren Intervallen als einer Minute zwischen Ausführung decken Sie auch Plugins ab, die sonst asynchrone Ausführung erfordern würden.

* Wenn Sie Agentenpakete bereits mit der Regel [guihint]#Plug-ins, local checks and MRPE for non-root users# xref:config_runas[paketiert haben],
dann müssen Sie als Root auszuführende Plugins in die konfigurierten Verzeichnisse verschieben und Konfiguration für `sudo` ausbringen.
