...
//SK: Oben hast du die Dinger noch Proxmox-Knoten genannt. In der Proxmox-GUI heißen die Dinger auch "Nodes". Ich würde dabei bleiben.
Gleichzeitig sollten Sie das Monitoring explizit auf Ihre Proxmox-Hosts einschränken.
...
Damit hieraus Hosts entstehen, müssen Sie als nächstes diverse Hosts mit absolut korrekten Namen händisch anlegen.
...
//SK: Die Host müssen in Checkmk genauso heißen, wie sie in Proxmox heißen. Den Teil "nach IP, Region und Instanz-ID" verstehe ich nicht ganz.
Für die Benamsung dieser Piggybacked-Hosts wählen Sie die eindeutige Benamsung nach IP, Region und Instanz-ID.
...
//SK: "Proxmox-Instanzen" ist hier missverständlich. In Proxmox gibt es VMs und Container. Das würde ich hier auch so schreiben.
Konfigurieren Sie die Hosts für diese Proxmox-Instanzen und wählen Sie als Monitoring-Agent [.guihint]#No API integrations, no {CMK} agent# aus.
