...
:description: With {CE-saas}, setting up your first {CMK} site only takes minutes. Read here how to register an account and get started.           
...
[TIP]
====
Whenever it is clear that there will be no confusion with link:https://docs.checkmk.com/master/en/cce.html[{CE-onprem}^] -- as in this Beginner's Guide -- when discussing {CE-saas} we will simply call it {CE}.
====
