...
= {CE-saas}
:title: {CE-saas}
:description: {CE-saas} offers low-maintenance, reliable monitoring. Read here what characterizes this edition in contrast to {CE-onprem}.                  
...
{CCE-saas} (formerly also known as {CMK} Cloud (SaaS)) provides you with {CMK} as _Software as a Service (SaaS)_ in the cloud.
...
To help you to set up your infrastructure quickly, {CE-saas} provides you with preconfigured agent packages, which with only a few minutes work enable you to create your hosts with standard settings and add these to your monitoring.
You can therefore concentrate on IT monitoring right away -- and not on operating {CMK}.

In this article you will learn what {CE-saas} has to offer.


[#editions]
== Overview of {CE-saas}

Unlike the on-premises editions of {CMK}, which all require you to set up your own {CMK} server, with {CE}, the {CMK} server is already running in the cloud and just waiting for you to use it.

The {CCE-saas} software is based on the on-premises edition {CCE-onprem}.
Thus, in principle, both offer close to the same range of functions.
An overview of the functions of link:https://docs.checkmk.com/master/en/cce.html[{CE-onprem}^] can be found in the {CMK} User Guide.

The 'in principle' in the previous paragraph indicates that there are actually some differences between {CE-saas} and {CE-onprem}.
These result from the technical requirements to be met for providing the software in the cloud and the implementation of security guidelines, but also the prioritization of functions into those that are absolutely necessary for {CE-saas} and those that can wait for the next release.
...
The following table shows the main functions provided by {CE-saas} in addition to those offered by {CE-onprem}:
...
|The xref:intro_setup_monitor#first_auto[Beginner's Guide] shows how a new host can be added to the monitoring with one of these agent packages.

|AI system [.guihint]#Explain with AI#
|The xref:explain_with_ai#[AI system] allows you to obtain additional information about hosts and services. 
Our integrated Large Language Model (LLM) helps you get to the root of problems in your cloud infrastructure and also provides general knowledge about {CMK}. |
...
The following table shows the main functions of {CE-saas} that are implemented differently compared to {CE-onprem}:
...
|Testing the edition
|{CE} can be tested for the first 30 days (in license state 'Trial') without obligation.
The following restrictions apply during this trial period: +
A maximum of 2500 xref:glossar#service[services] can be monitored, the frequency of xref:opentelemetry#[OpenTelemetry metrics] received via the xref:metric_backend#[metrics backend] is limited to one data point every 30 seconds (instead of 15 seconds), and OpenTelemetry data is retained for two days (instead of 14 days). +
If you do _not_ license {CE} during the trial period, the {CMK} site will be automatically deleted after 30 days. |
...
|SNMP monitoring
|The xref:glossar#checkmk_relay[{CMK} Relay] is required for monitoring via SNMP.
The relay collects monitoring data from the SNMP hosts and forwards it to the {CMK} site.
|Other hosts that need to be queried for monitoring can also be integrated via a {CMK} relay, e.g., using xref:glossar#special_agent[special agents] or xref:glossar#active_check[active checks].
...
|OpenTelemetry
|xref:opentelemetry#[OpenTelemetry data] can only be passively received using the OpenTelemetry collector.
Active retrieval via a _Prometheus scraper_ is not possible.
|Push mode is the default in a cloud environment.

|Product usage analytics
|xref:opentelemetry#[Product usage analytics] is always enabled in {CE}. |
...
The following table shows the main functions of {CE-onprem} that are not available in {CE-saas}:
