...
In this article you will learn what {CE-saas} has to offer -- compared to its sister {CE-onprem}.
...
{CE-onprem} is one of the _on-premises_ editions of {CMK}, all of which require you to set up your own {CMK} server.
...
[TIP]
====
Whenever there is no possibility of confusion with {CE-onprem} -- as in the other articles in this {CE} User Guide -- we simply call {CE-saas} the {CE}.
====
