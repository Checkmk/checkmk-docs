...
:description: The admin panel is the central point for managing {CE-saas}. Read here how to set up and manage sites and users.
...
The page heading shows the name of the _tenant_, in this example `My Organization`.
...
You can also log in to {CE} at a later time and carry out the licensing actions described below via the admin panel.
