...
Auf der xref:open[Hauptseite] des Admin Panel zeigt der Titel den Namen des Tenant.
...
Die Anforderung eines Angebots ist der Auftakt im Bestellprozess, in dem Sie unter anderem einen Auftrag erhalten, diesen akzeptieren und darüber eine Bestätigung erhalten.
...
Falls also die E-Mail mit dem Aktivierungsschlüssel nicht bei Ihnen angekommen ist, dann lassen Sie sich die E-Mail von Ihrer Kollegin weiterleiten, die für Ihre Firma die Subskription erworben hat.
====

Der Aktivierungsschlüssel kann auch jederzeit im {CMK}-Kundenportal ausgelesen werden.
...
Klicken Sie [.guihint]#Activate license,# geben Sie in die folgende Dialogbox den Schlüssel ein und bestätigen Sie.
