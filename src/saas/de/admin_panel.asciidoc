...
Wie Sie an Benutzernamen und Passwort kommen, steht im xref:intro_setup.html#signup_trial[Leitfaden für Einsteiger]. 
...
Um die Seite zur Verwaltung der {CMK}-xref:glossar.html#site[Instanzen] zu öffnen, klicken Sie in der Navigationsleiste des Admin Panel auf [.guihint]#Sites:#
...
Die Überschrift der Seite zeigt den Namen des Mieters (_tenant_), im Beispiel `Checkmk GmbH`.
Dies ist der Name der Organisation, den Sie bei der xref:intro_setup.html#login_first[ersten Anmeldung] angegeben haben.
