...
.Zur Auswahl stehen die zuletzt freigegebenen {CMK}-Versionen
image::admin_panel_create_site.png[alt="Dialogbox zur Erstellung einer Instanz.",width=88%]

Geben Sie in der Dialogbox den Namen der zu erstellenden Instanz ein, wählen Sie die {CMK}-Version und die Zeitzone aus und bestätigen Sie mit [.guihint]#Create site.#
