...
:description: Das Admin Panel ist die Zentrale für die Verwaltung von {CE-saas}. Hier erfahren Sie, wie Sie Instanzen und Benutzer einrichten und verwalten.         
...
Die Überschrift der Seite zeigt den Namen des _Tenant_, im Beispiel `My Organization`.
...
Sie können daher eine neue Instanz nur dann erstellen, wenn die Tabelle der Instanzen leer ist, d.{nbsp}h., wenn Sie die bestehende Instanz gelöscht haben.
...
{CE} aktualisiert die Instanz automatisch für Sie auf eine neue Patch-Version, also z. B. von {v24}p27 auf {v24}p28.
...
Wenn Sie den rot hinterlegten Link im [.guihint]#Help#-Menü öffnen, werden Ihnen nur die xref:glossar#werk[Werks] (d.{nbsp}h. die Änderungen) angezeigt, bei denen etwas zu tun ist, und die daher mit [.guihint]#Incompatible - TODO# gekennzeichnet sind.
...
Sie erhalten eine Tabelle mit allen verfügbaren Backups für diese Instanz, d.{nbsp}h. denjenigen, die Sie selbst erstellt haben (Backup-Typ [.guihint]#MANUAL#), und den von {CE} automatisch erstellten (Backup-Typ [.guihint]#SCHEDULE#).
...
Die Benutzerverwaltung mit den in den folgenden Abschnitten beschriebenen Aktionen kann nur von Administratoren, d.{nbsp}h. Benutzern mit der Rolle [.guihint]#Administrator#, aufgerufen werden.
...
Diese E-Mail enthält den Benutzernamen (d.{nbsp}h. die E-Mail-Adresse), ein temporäres Passwort und einen Link zur ersten Anmeldung bei {CE}.
...
Nach Eingabe der gewünschten E-Mail-Adresse wird eine E-Mail versendet mit einem Link zur Auswahl eines neues Passworts.
...
Um {CE} aus der Testphase (Lizenzstatus „Trial“) heraus zu lizenzieren, benötigen Sie eine _Subskription_.
Nachdem Sie die Subskription erworben haben, erhalten Sie einen Aktivierungsschlüssel, den Sie in {CE} eingeben.
Das war es auch schon.

Die lizenzierbare Einheit ist dabei der _Tenant_, der die Instanzen und Benutzer zusammenfasst, die im Admin Panel erstellt und verwaltet werden können.
...
Die Lizenzverwaltung mit den in den folgenden Abschnitten beschriebenen Aktionen kann nur von Administratoren, d.{nbsp}h. Benutzern mit der Rolle [.guihint]#Administrator#, aufgerufen werden.

Der Kauf der Subskription und die Eingabe des Aktivierungsschlüssels sind auch nach Ablauf der Testphase möglich.
Sie können sich auch später noch bei {CE} anmelden und über das Admin Panel die im Folgenden beschriebenen Aktionen zur Lizenzierung durchführen.
