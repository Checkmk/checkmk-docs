...
In diesem Artikel erfahren Sie, was {CE-saas} zu bieten hat -- im Vergleich zu dessen Schwester {CE-onprem}.


[#editions]
== Einordnung von {CE-saas}

{CE} gibt es in zwei Varianten: als {CE-saas} und {CE-onprem}.
{CE-onprem} ist eine der _On-Premises_-Editionen von {CMK}, die alle das Aufsetzen eines eigenen {CMK}-Servers voraussetzen.
Anders {CE-saas}:
...
[TIP]
====
Immer dann, wenn eine Verwechslung mit {CE-onprem} ausgeschlossen ist -- wie in den anderen Artikeln dieses {CE} Handbuchs -- nennen wir {CE-saas} einfach {CE}.
====
