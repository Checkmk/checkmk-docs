...
{related-start}
xref:cce#[{CE} (SaaS)]
xref:admin_panel#[Das Administration Panel]
{related-end}
...
Im Vordergrund sehen ein Thema des _Onboarding Guide,_ das Ihnen erklärt, wie Sie in wenigen Schritten Ihren ersten xref:glossar#host[Host] ins Monitoring von {CE} aufnehmen können.
...
Allerdings finden Sie dort Zugriff auf zwei Elemente, die es nur in {CE} gibt: den Onboarding Guide, von dem bereits die Rede war, und das Administration Panel.
...
Wenn Sie sich fragen, wo diese Eingaben in {CE} gelandet sind, dann lautet die Antwort: im Administration Panel.

Das Administration Panel ist die Zentrale für die Verwaltung von {CE}.
...
Sie öffnen das Administration Panel per Klick auf [.guihint]#Admin# in der xref:glossar#navigation_bar[Navigationsleiste.]

image::intro_admin_panel_saas.png[alt="Das Administration Panel mit der Übersicht der Instanzen."]

Auch das Administration Panel hat eine Navigationsleiste am linken Rand.
Durch Klick auf [.guihint]#User management# erhalten Sie z. B. eine Tabelle mit allen Benutzern.
Aktuell enthält die Tabelle nur einen Benutzer mit dem Namen der von Ihnen eingegebenen E-Mail-Adresse.
Analog zeigt ein Klick auf [.guihint]#Sites# die in {CE} eingerichteten Instanzen.

Eine detaillierte Beschreibung der Funktionen des Administration Panel finden Sie im xref:admin_panel#[gleichnamigen Artikel.]
...
Aus dem Administration Panel wechseln Sie in die {CMK}-Oberfläche der Instanz, indem Sie den Instanznamen anklicken (im Beispiel `mysite`).
