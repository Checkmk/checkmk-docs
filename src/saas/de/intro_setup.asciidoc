...
[TIP]
====
Beachten Sie, dass es in dieser Testphase einige xref:cce#functions_different[Einschränkungen] gibt:
So können zum Beispiel maximal 2500 xref:glossar#service[Services] überwacht werden.
Falls Sie {CE} während der Testphase _nicht_ lizenzieren, wird die {CMK}-xref:glossar#site[Instanz] nach Ablauf der 30 Tage automatisch gelöscht.
====
// ES: Schön. Sollte hier und in cce.asciidoc explizit darauf hingewiesen werden, dass nur eine Instanz zur Zeit erlaubt ist, solange keine Lizenz vorliegt?
...
image::intro_welcome_saas.png[alt="Die Willkommensseite als Startseite.",fullscreen=1]
