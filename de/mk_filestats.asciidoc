...
.[.guihint]#mk_filestats# offenbart im Monitoring, welche Dateien für den Status verantwortlich sind
...
.Überwacht alle Dateien unterhalb des Pfads
...
Damit ist die Konfiguration abgeschlossen und Sie können das Plugin xref:agent_linux#manualplugins[unter Linux] oder xref:agent_windows#_manuelle_installation_von_plugins[unter Windows] manuell installieren.
...
._yourfile_ wird von [.guihint]#fileinfo# angezeigt, obwohl nicht für die Statusänderung verantwortlich
...
.Bei [.guihint]#fileinfo# sind es zu offenherzige Globbing-Muster, die für riesige Listen sorgen können
