// -*- coding: utf-8 -*-
// IGNORE Sicherungsfähige AUTORITÄT\SYSTEM Datafiles MSSQL datafile mssql msoledbsql sqlncli11 ini vbs sqloledb
...
Die einzige Voraussetzung für die Einrichtung der Überwachung innerhalb von MSSQL ist, dass ein Datenbanknutzer und dessen Passwort vorhanden sein müssen.
Dieser Benutzer benötigt die Berechtigung _Verbindung mit beliebiger Datenbank (Connect any database)_ auf die MSSQL-Instanzen.
Idealerweise verwenden Sie daher direkt den Systembenutzer (NT AUTORITÄT\SYSTEM), der diese Eigenschaften in der Regel aufweist.
Dazu ist keine weitere Einstellung auf dem MSSQL-Host nötig.
...
Instanzen einen passenden Datenbankbenutzer anlegen.
Melden Sie sich dazu mit einem Benutzer mit ausreichenden
...
Ist hier zum Beispiel `SQL_Latin1_General_CP1_CS_AS` gesetzt, so steht „CS“ für „Case Insensitive“.
