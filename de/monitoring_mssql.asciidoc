...
Benutzer benötigt folgende Rechte auf die MSSQL-Instanzen:
...
Zugriffsberechtigungen in MSSQL an und erzeugen Sie anschließend einen neuen
Datenbanknutzer:

[{sql}]
----
mssql> CREATE USER checkmk WITH PASSWORD = 'MyPassword123';
----

Dieser neu angelegte Benutzer benötigt die oben genannten Berechtigungen. Prüfen Sie daher, ob der Benutzer die notwendigen Berechtigungen
hat, oder fügen Sie diese über einen Befehl hinzu. Im folgenden Beispiel
wird das für den Benutzer `checkmk` erledigt:

[{sql}]
----
mssql> GRANT CONNECT ANY DATABASE TO checkmk;
----
//SP: Ich bin nicht sicher, ob der Befehl korrekt ist. Ich habe verschiedene Infos in den MSSQL Docs gefunden.

//SP: Neben der Arbeit auf der Befehlszeile gibt es (anders als in MySQL?) für die Benutzereinrichtung eine bequeme Lösung über das Microsoft SQL Server Management Studio. Macht es Sinn, diese zu ergänzen?
...
==== Kommunikation mit MSSQL über Socket einrichten

Statt den Agenten über TCP mit MSSQL kommunizieren zu lassen, können Sie {CMK}
anweisen den Socket anzusprechen. Dazu definieren Sie in der mssql.cfg lediglich die
...
user=checkmk
password=MyPassword123
*socket=/var/run/mssqld/mssqld.sock*
----


==== Host manuell definieren

Des Weiteren ist es über die Konfigurationsdateien auch möglich den MSSQL-Host
...
Auch für die Größe einzelner Datein in MSSQL können Sie Schwellwerte festlegen. Dazu verwenden Sie die Regel [.guihint]#MSSQL datafile sizes.#
