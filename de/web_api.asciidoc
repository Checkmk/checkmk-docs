...
:description: Die {CMK} HTTP-API (oder Web-API) ist eine Schnittstelle, um {CMK} über HTTP zu konfigurieren - und dieser Artikel führt in die Nutzung dieser API ein.
...
Die Web-API ermöglicht Ihnen, Konfigurationsaufgaben, die Sie normalerweise manuell in der xref:wato#[{CMK}-Konfigurationsumgebung] mit dem [.guihint]#Setup#-Menü durchführen, über HTTP-Aufrufe zu automatisieren. Zwar ist nicht die komplette Funktionalität des Setup-Menüs via API verfügbar, doch die wichtigsten Dinge, wie die Verwaltung von Hosts, Ordnern, Instanzen, Benutzern, Gruppen und etlichem mehr ist möglich. Eine vollständige Liste aller API-Funktionen finden Sie xref:web_api_references#[hier].
...
xref:web_api#format[Datenstrukturen] nötig. Natürlich ist der Artikel dennoch so einfach wie
...
Um von einem Client aus die Web-API nutzen zu können, muss auf dem {CMK}-Server ein xref:wato_user#automation[Automationsbenutzer] eingerichtet sein. Nur dieser ist berechtigt, Aktionen über die API auszuführen.
...
Bei einer neu erstellten Instanz (site) ist der Benutzer [.guihint]#automation# bereits angelegt. Sie finden ihn, wie andere Benutzer auch, unter [.guihint]#Setup > Users > Users#. Achten Sie darauf, dass die Rollen und die damit verbundenen Berechtigungen für den Automationsbenutzer so gesetzt sind, dass Sie die Ausführung Ihrer Anfragen erlauben.
...
[{shell}]
----
{c-root} curl "\http://myserver/mysite/check_mk/webapi.py?action=get_all_hosts&_username=automation&_secret=mysecret&output_format=python"
...
[{shell}]
----
{c-root} curl "\http://myserver/mysite/check_mk/webapi.py?action=get_host&_username=automation&_secret=mysecret&output_format=python" -d 'request={'hostname':'myserver123'}'
...
xref:web_api_references#[Befehlsreferenz]
...
[{shell}]
----
{c-omd} curl "\http://localhost/$OMD_SITE/check_mk/webapi.py?action=get_all_hosts&_username=automation&_secret=$(cat $OMD_ROOT/var/check_mk/web/automation/automation.secret)&output_format=python"
----
...
xref:web_api_references#[Befehlsreferenz], um eine Beschreibung zu allen Befehlen
zu erhalten.

Den Umgang mit der API zeigt ein einfaches Beispiel:
Sie erstellen einen Host mit seinen Services über die Web-API mit nur
drei Befehlen. Prinzipiell gehen Sie dabei genauso vor, wie im {CMK} xref:wato#[Setup]:
...
Mit dem Befehl xref:web_api_references#hosts[`add_host`]
...
[{shell}]
----
{c-root} curl "\http://myserver/mysite/check_mk/webapi.py?action=add_host&_username=automation&_secret=myautomationsecret&request_format=python" -d 'request={"hostname":"myserver123","folder":"","attributes":{"ipaddress":"192.168.0.42","site":"mysite","tag_agent":"cmk-agent"}}'
----
...
xref:web_api_references#discover_services[die Art der Serviceerkennung].
...
Zuletzt werden die Änderungen, wie im {CMK} xref:wato#activate_changes[Setup] auch, aktiviert:
...
Wie bereits weiter xref:web_api#result_code[oben] beschrieben, gibt die Anfrage
...
aber prinzipiell jede verfügbare xref:wato_user#roles[Rolle] zuweisen können, müssen
gegebenenfalls auch die xref:wato_user#wato_permissions[Kontaktgruppen] angepasst
...
|etc/check_mk/conf.d/wato/ |Alle hier angelegten Verzeichnisse stellen die im [.guihint]#Main directory# sichtbaren Verzeichnisse für Hosts dar.
