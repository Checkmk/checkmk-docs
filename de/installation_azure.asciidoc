// -*- coding: utf-8 -*-
// IGNORE Downloadseite Monitoringdaten Nullmailer SMB Snakeoil TBD WebDAV
...
Egal, ob Sie bereits langjähriger Nutzer von {CMK} sind, oder erst mit der Verfügbarkeit fertiger Images in den Marketplaces von AWS und Azure zu uns gefunden haben, Sie finden in diesem Handbuchartikel alle Ressourcen, um das vorbereitete VM Image zum für Ihre Bedürfnisse passenden Monitoring zu vervollständigen.
...
Neben der Wahl des Images sollten Sie sich Gedanken um ben Speicherort von Backups machen.
{CMK} unterstützt von Haus aus die BLOB-Speicher von Azure und AWS, daneben können Backups in Dateisystempfaden abgelegt werden, was Backups auf SMB- oder WebDAV-Mounts ermöglicht oder den regelmäßigen Transfer per `rsync` erlaubt.
