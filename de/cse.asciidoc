...
|PDF-Export der Verfügbarkeitsdaten |In den {EE} können xref:availability#[Verfügbarkeitsdaten] auch als PDF exportiert werden.
...
|Jira |Die xref:notifications_jira#[Benachrichtigungsmethode für Jira] ist den {EE} vorbehalten.
|ServiceNow |Die xref:notifications_servicenow#[Benachrichtigungsmethode für ServiceNow] gibt es nur in den {EE}.
|===
