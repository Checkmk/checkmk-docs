// -*- coding: utf-8 -*-
// IGNORE 0° 1° Festkommazahl menschenlesbare vorzeichenbehaftete Benachrichtigungs rev23 Routertyp snmp
...
//SP: Arbeiten wirklich die Plugins mit SNMP? Oder arbeiten die Plugins mit SNMP-basierten Geräten? Oder, was mir am meisten einleuchten würde,  kommunizieren sie über SNMP mit den Geräten?
Der Unterschied liegt sowohl im Ablauf der Service-Erkennung (Discovery) als auch des Checks selbst.
Bei den agentenbasierten Checks wird mit dem Agentenplugin festgelegt, welche Daten an die Checkmk-Instanz _gesendet_ werden, zudem findet dort oft bereits eine Vorfilterung (aber keine Auswertung) statt.
//SP: Müsste das nicht ein "agentenbasiertes Check-Plugin" sein? Bei der Unterscheidung Check-Plugin vs. Check bin ich mir noch immer unsicher.
// MFS: Hier ist es tatsächlich die Gesamtheit beider Komponenten.
...
//SP: Mit Discovery führst du hier ein Fremdwort ein, das bislang nicht verwendet wurde. Einen Link oder eine kurze Erklärung fände ich hilfreich.
...
//SP: Hier wieder: Check oder Check-Plugin?
// MFS: Same.
...
//SP: Könnte man hier Objekte statt Items schreiben? Oder wird unter Items etwas anderes verstanden?
// MFS: Das greift auf die Checks mit Items vor, welche mehrere Services erzeugen, die im Hauptartikel bereits eingeführt wurden, hier sollten wir konsistent bleiben.

Beim *Check* ist dann schon bekannt, welche Plugins für das Gerät ausgeführt werden sollen und eine erneute SNMP-Detection entfällt.
//SP: Ist der Check dann Schritt 3?
// MFS: Da der Check alle Minute ausgeführt wird, dat sich der ursprüngliche Autor wohl das "Schritt" erspart.
...
//SP: Check oder Check-Plugin?
// MFS: Check in seiner Gesamtheit.
...
. Sie entscheiden, welche Äste des SNMP-Baumes für das Monitoring geholt werden müssen.
...
Ist gerade kein Gerät zur Hand, finden Sie im Kapitel zur xref:errors[Fehlerbehebung] verschiedene Simulationsmöglichkeiten.

Der erste Schritt dahin ist, einen kompletten SNMP-Walk auszuführen -- falls das Gerät das nicht verweigert.
//SP: Heißt es für den Walk tatsächlich ziehen? Wird ein Walk nicht ausgeführt, angestossen oder so?
// MFS: Das bezeichnet sowohl die Tätigkeit als auch das Ergebnis. "Ziehen" ist ziemlich Admin-Slang, ich habe hier auf "ausführen" geändert.
...
//SP: Sollte? Und was ist, wenn sie es aus meiner Sicht nicht tut?
// MFS: Dann haste Pech gehabt und Du musst weitere, u.U. herstellerspezifische OIDs auswerten. Das möchte ich daher hier nur andeuten.
...
Im Vergleich zum agentenbasierten Check-Plugin sind hier `startswith` und `SNMPTree` neu.
...
. Sie identifizieren die Hosts, für welche das Check-Plugin ausgeführt werden soll.
...
Das Beispiel enthält noch den Parameter `name` mit dem die erzeugte SNMP-Sektion identifiziert wird und eine Parse-Funktion, auf die wir später zu sprechen kommen.
...
Die OIDs, die Sie den Erkennungsroutinen eines Check-Plugins übergeben, werden von *jedem* Gerät geholt, welches per SNMP überwacht wird.
// SP: Warum werden die OIDs von jedem Gerät geholt?
// MFS: Weil ermittelt werden muss, welches SNMP-Plugin auf welches SNMP-Gerät anzuwenden ist.
Seien Sie daher sehr sparsam bei der Verwendung von herstellerspezifischen OIDs.
Versuchen Sie, Ihre SNMP-Detection so zu gestalten, dass bevorzugt die `sysDescr` (`.1.3.6.1.2.1.1.1.0`) und die `sysObjectID` (`.1.3.6.1.2.1.1.2.0`) verwendet werden.
...
Angenommen, der Routertyp kann mit einer variablen Zahl von Netzwerkkarten bestückt werden, deren Name, Verbindungsstatus und Geschwindigkeit unterhalb von `.1.3.6.1.4.1.424242.2.3.23` ausgelesen werden könnten...
...
Alle unter einer OID erhältlichen Blätter werden in eine Tabellenspalte geschrieben. Damit dürfte es offensichtlich sein, dass wegen der Darstellung der Daten nur zusammenpassende OIDs abgefragt werden dürfen.
...
Die Registrierung des Plugins geschieht exakt wie bei den xref:devel_check_plugins#register_check_plug-in[agentenbasierten Check-Plugins beschrieben].
...
Auch die Discovery-Funktion entspricht xref:devel_check_plugins#discovery_function[dem Beispiel des agentenbasierten Check-Plugins].
...
Test und Aktivierung erfolgen xref:devel_check_plugins#test[wie beim agentenbasierten Pendant].
...
Nach dem Neustart des Monitoring-Kerns ist der neue Check im Monitoring sichtbar:
...
Da die Fehlerbehebung im wesentlichen der im gleichnamigen Kapitel im Artikel zu xref:devel_check_plugins#errors[agentenbasierten Plugins entspricht], gehen wir hier nur auf SNMP-Spezifika ein.
//SP: Wenn sie "exakt entspricht", bräuchte es hier m.E. keine weiteren Beschreibungen. Wie wäre es mit "großteils" oder "vorwiegend"?
// MFS: im wesentlichen.
...
Im Artikel zur _Überwachung via SNMP_ xref:snmp#simulation[zeigen wir detailliert], wie Sie SNMP-Walks aus der GUI erstellen und wie Sie diese zur Simulation einsetzen können.
Der Simulationsmodus erlaubt es auch, die Plugin-Entwicklung auf Testsystemen vorzunehmen, welche die SNMP-Hosts, für die Sie ein Plugin entwickeln, nicht erreichen können.
In unserem GitHub-Repository finden Sie einen link:https://github.com/Checkmk/checkmk-docs/blob/master/examples/devel_check_plugins_snmp/flintstones_fred_router.txt[Beispiel-SNMP-Walk^], den Sie zur Entwicklung und zum Testen des Plugins nutzen können.
...
Ein Hilfsmittel bei der Programmierung eines solchen kann das link:https://pypi.org/project/snmp-agent/[Python-Modul _snmp-agent_^] sein.

[#hwbugs]
=== Unkooperative Hardware

Bevor ein Gerät mit einem neuen Check-Plugin überwacht werden kann, muss es erst überhaupt überwacht werden können.
Die xref:snmp#cursedhardware[Übersicht bekannter Probleme mit Lösungsvorschlägen] finden Sie daher im Artikel zur _Überwachung via SNMP_.
