...
// TK: Benennung der OIDs ohne führenden Punkt: http://oidref.com/1.3.6.1.2.1.1
...
// TK: ob das Check-Plugin denn überhaupt für das jeweilige Gerät interessant ist.
// TK: oder noch besser:
// TK: ob das jeweilige Gerät für das Check-Plugin denn überhaupt interessant ist.
...
// TK: Für jedes dieser Items wird dann ein Service erzeugt. 
...
// TK: ob das Plugin für das Gerät ausgeführt werden soll, und eine erneute SNMP-Detection entfällt.
...
// TK: Da steht das gleiche wie oben bei der Discovery-Funktion. Die Monitoring-Daten wurden doch gerade schon geholt. Jetzt nochmal?
...
. Sie entscheiden, welche Äste des SNMP-Baums für das Monitoring geholt werden müssen.
...
// TK: Ist eigentlich nicht Fehlerbehebung. Eigenes Kapitel Simulationsmöglichkeiten?
...
// TK: = der Name, den ich bei register.check_plugin als name vergebe, also der Name des Check-Plugins?
...
// TK: Die OIDs, die Sie der SNMP-Detection eines Check-Plugins übergeben, werden von *jedem* Gerät geholt, welches per SNMP überwacht wird.
// TK: Nur so kann {CMK} feststellen, auf welche Geräte das Check-Plugin anzuwenden ist.
...
Angenommen, der Router kann mit einer variablen Zahl von Netzwerkkarten bestückt werden, deren Name, Verbindungsstatus und Geschwindigkeit unterhalb von `.1.3.6.1.4.1.424242.2.3.23` ausgelesen werden könnten...
