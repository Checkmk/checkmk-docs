...
// MA: Das würde ich so nicht machen, weil man dann alle Inhalte unterhalb von
// MA: /omd bewegt. Eigentlich muss aber die Software nicht bewegt werden, sondern
// MA: lediglich die Instanzen, sprich: alles unterhalb von /omd/sites. Alles andere
// MA: führt möglicherweise zu Problemen, wenn dann wieder checkmk installiert
// MA: wird: 1.6.0p22 ist bereits unter /omd/versions/ vorhanden, soll aber als
// MA: Paket installiert werden, etc.
...
==== mkp remove interface-checks-inv-plugins

==== Größe von checkmk/iventory/archive prüfen und ggf. leeren
...
//SK: Ich würde hier gerne auf den entsprechenden Abschnitt im Artikel cmc_differences verlinken, aber der beschreibt natürlich noch die Situation in der 1.6. Das würde ich gerne bald ändern, damit ich hier nicht zu sehr ins Detail gehen muss. -> link:cmc_differences.html#checkhelper[Helper/Fetcher/Checker]
...
=== neue Sachen discovern

*Betrifft Sie das?* MySQL am Start? Dann ja.

*Was müssen Sie tun?* Für alle möglichen Host eine Re-Discovery durchführen. Details folgen.

==== diverse if neu discovern

Sonderzeichen in if-Names werden jetzt anders behandelt was ein Re-Discovery nötig macht.
...
//SK: Hier geht es morgen noch mal weiter.
...
[{shell}]
----
{c-omd} mkp remove Ex2010_MBDB_Info
----
