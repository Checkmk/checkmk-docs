...
Wie vor dem Update jeder produktiven Software sollten Sie auch vor dem Update von {CMK} die Aktualität Ihrer Backups prüfen. Wenn Sie Ihre Backups automatisiert über das Modul WATO > Backups erstellen, prüfen Sie dort, ob die letzten Backup Jobs fehlerfrei durchgelaufen sind. Weitere Informationen zu diesem Thema finden Sie im Kapitel link:omd_basics.html#backup[Instanzen sichern und wiederherstellen].
...
*Hinweis:* Bitte geben Sie bei der Installation per `apt install` den vollständigen Pfad zur .deb-Datei an.
...
[#forecast_graphs]
=== Python für Forecast Graphs entfernen

*Betrifft Sie das?* Falls Sie die in {CMK} {v16} eingeführten Forecast Graphs verwendet haben, ist es sehr zu empfehlen, das dafür installierte Python und FB Prophet wieder aus Ihren Sites zu entfernen. Ab {v20} verfügt {CMK} über eine komplett eigenständige Implementierung dieser Vorhersagefunktion.

*Was müssen Sie tun?* Das Skript welches Sie für die Installation von Prophet und seinen Abhängigkeiten verwendet haben, erledigt für Sie auch die Entfernung dieser Pakete. Rufen Sie es dazu einfach mit der Option -u auf. In einem verteilten Monitoring müssen Sie diesen Schritt auf jeder Site, auf der Sie FB Prophet installiert haben, wiederholen.
...
[#incompatible_mkps]
=== Inkompatible und obsolete MKPs identifizieren
...
*Betrifft Sie das?* Aus diesem Grund ist eine Prüfung der von Ihnen installierten MKPs, besonders vor einem so umfangreichen Update, drigend geboten. So verhindern Sie, dass inkompatible Pakete das Update behindern oder im Anschluss an das Update doppelte oder zumindest sehr ähnliche Services entstehen.
...
[#dry_run]
=== Best practices zum Update

Im folgenden beschreiben wir Best practices, welches wir selbst bei Updates von großen {CMK}-Umgebungen befolgen. Diese sind sicherlich nicht in jeder Umgebung Pflicht, Sie können Ihnen den Prozess des Updates jedoch erleichtern.

==== OS Patches

Das Betriebssystem auf einem {CMK}-Server sollte ohnehin immer auf einem aktuellen Stand sein. Vor einem Update von {CMK} schadet es allerdings bestimmt nicht, sich dessen nochmal mithilfe von apt, yum oder zypper zu versichern.

==== Probelauf des Updates

In großen Umgebungen, in denen auch das Zurückspielen eines selbstverständlich vorhandenen Backups Ihrer {CMK}-Umgebung mit einem gewissen zeitlichen Aufwand verbunden wäre, empfielt es sich, vor dem Update der produktiven Umgebung, einen Test mit einer geklonten Site durchzuführen. Zu diesem Zweck können Sie beispielsweise das letzte reguläre Backup Ihrer Site unter einem anderen Namen wiederherstellen.
...
*Alternativ* können Sie Ihre Site auch per omd cp kopieren. Dafür muss die Site allerdings kurzzeitig gestoppt werden.
...
Führen Sie das Update im Anschluss erstmal auf dieser neuen geklonten Site durch um hier bspw. Ihre oben angesprochenen lokalen Änderungen in der neuen Umgebung zu prüfen.
...
Auf diese Weise wird der neue Agent nicht gleich über all Ihre Server ausgerollt und Sie können sich auf eingen wenigen Systemen mit den neu angeliferten Daten vertraut machen. Auch aufgrund der deutlich gestiegenen Zahl an mitgeliferten Checkplugins könnten Sie eine ganze Reihe neuer Services finden, welche Sie dann auf den von Ihnen gewählten Testsystemen richtig einstellen können. Eventuell sind für neue Services auch neue Schwellwerte vonnöten. Wenn Sie dies erstmal im Kleinen angehen, ersparen Sie sich ggf. einige Fehlalarme.

Nach dem Abschluss dieser Prüfungen sollten Sie dann allerdings die eingeschränkten Kopien Ihrer eigentlichen Regeln für den Agent Updater wieder löschen und die anfangs deaktivierten Regeln wieder einschalten.
...
In {CMK} {v20} verrichten diese Helper im Grunde nur noch spezialisierter und zielgerichteter ihre Aufgaben. Statt wie bisher sowohl für das Abholen und anschließende Auswerten der Informationen zuständig zu sein, wird der eine Teil der Helper nur für das Abholen der Informationen zuständig sein. Diese Helper heißen jetzt _Fetcher_. Der andere Teil hingegen kümmert sich darum, die vom _Fetcher_ übergebenen Informationen auszuwerten. Diese Prozesse nennen wir _Checker_.
...
=== neue Service discovern

{CMK} {v20} bringt eine ganze Reihe neuer Check-Plugins mit sich. Sollte Sie das automatische Update der Servicekonfiguration über die periodische Serviceerkennung nicht einsetzen, werden Sie auf einer ganzen Reihe von Hosts Service-Suchen durchführen müssen. Wenn Ihre Hosts entsprechend organisiert (bspw. in Ordnern) sind, können Sie hierfür zumeist mit der Funktion [.guihint]#Servies erkennen# (Bulk Discovery) arbeiten. Diese finden Sie unter Setup > Hosts > Hosts > Services erkennen. Im Folgenden listen wir einige Bereiche it den meisten Neuerungen auf.

==== MySQL-Server

Insbesondere auf Servern auf denen MySQL läuft, wird Ihnen {CMK} einge ganze Reihe neuer Services für die Überwachung anbieten. 

==== Netzwerkgeräten

In den Bezeichnungen von Netzwerkgeräten haben sich in vielen IT-Landschaften in der Vergangenheit Sonderzeichen eingeschlichen. Von Größer- und Kleinerzeichen, über Pipes bis hin zu Umlauten wird {CMK} {v20} diese Sonderzeichen gänzlich anders behandeln, damit es hier zu weniger Problemen kommt. Unabhängig davon, ob eine solche Benamung gut und sinnvoll ist, muss auf den entsprechenden Geräten über die [.guihint]#Servicekonfiguration# eine [.guihint]#Service-Suche# gestartet werden.

==== Qnap bietet viele neue Services

altes Plugin entfernen und cmk -Iv gegen die Hosts.

==== Kernel-Services

Die drei Services [.guihint]#Kernel Process Creations, Kernel Major Page Faults und Kernel Context Switches# sind in {CMK} {v20} zu einem Service mit dem Namen [.guihint]#Kernel Performance# zusammengefasst. Auf den {CMK}-Servern selbst muss also auch eine Re-discovery der Services angestoßen werden. Dies sollte natürlich auf jeder Site gemacht werden. Auf der Kommandozeile können Sie dies mit dem folgenden Befehl erledigen:
...
Die Synchronization der Konfigurationsdateien zwischen den Instanzen im verteilten Monitoring hat sich in der Version {v20} geändert.
...
Python 3 ist - auch bei den Anforderungen an eine korrekte Syntax - deutlicher strikter als sein Vorgänger. In der Beta-Phase von {CMK} {v20} sind uns immer wieder Plugins untergekommen, in denen schon kleine Fehler ein erfolgreiches Update behindern konnten. Wenn beim Update oder beim anschließenden Start der Site der Fehler [.guihint]#inconsistent use of tabs# auftritt, empfehlen wir das betroffene Plugin mit folgenden Befehl zu entfernen.
