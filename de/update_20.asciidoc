...
Weitere Informationen finden Sie in den Artikeln zum link:wato.html#_backups[Backup über WATO] und zum Thema link:omd_basics.html#backup[Instanzen sichern und wiederherstellen].
...
////
// TK: Auskommentiert solange nicht klar ist, wie vorgegangen wird.
[#update_distributed_monitoring]
=== Update im verteilten Monitoring

// MA: Uff... keine Änderungen an der config, solange nicht alle up-to-date
// MA: sind? Das wird für ein paar Installationen schwierig bis unmöglich,
// MA: wenn man mehrere hundert entfernte Instanzen hat. Bisher war es immer so,
// MA: dass man von der vorherigen auf die neue übertragen konnte, aber umgekehrt
// MA: nicht. Das sollten wir nachprüfen.

Im verteilten Monitoring sollten Sie unbedingt zuerst die fernen Instanzen (_remote sites_) mit dem Update versehen und erst zum Abschluss die Zentralinstanz (_central site_) auf {CMK} {v20} bringen. Außerdem dürfen während der Updates keine Konfigurationsverteilungen stattfinden. Da sich die Konfigurationen der Versionen {v16} und {v20} stark unterscheiden, würde deren Verteilung zwischen unterschiedlichen Hauptversionen zu Fehlern führen. Sorgen Sie dafür, dass während der gesamten Zeit des Updates keine Änderungen vorgenommen werden, welche an Ihre fernen Instanzen verteilt werden müssten. Prüfen Sie dahingehend ggf. auch Ihre Verbindungen unter [.guihint]#WATO > Distributed Monitoring#.
////
...
Die Synchronisation der Konfigurationsdateien zwischen den Instanzen im verteilten Monitoring hat sich in der Version {v20} geändert.
...
. Alle Instanzen herunterfahren.
. Updates durchführen.
. Die aktualisierten Instanzen neu starten.

==== Sukzessives Vorgehen 
Das Standardvorgehen verlangt bei größeren Installationen gute Planung und ein wenig Zeit -- das Monitoring wird einige Zeit offline sein. Alternativ können Sie jegliche Konfigurationsänderungen über eine globale Einstellung unterbinden, so dass nur noch der Admin manuell Änderungen durchführen kann. Auf diese Weise können Sie dann nach und nach Instanzen aktualieren. Rufen Sie dazu in der zentralen {v16}-Instanz [.guihint]#WATO > Global Settings# auf und aktivieren Sie den icon:button_readonly[] Nur-Lesen-Modus:
...
In einem Mischbetrieb mit verschiedenen Versionen sind Grundfunktionalitäten gegeben. Einige Features von Checkmk funktionieren in diesem Modus gar nicht oder nur eingeschränkt. Bevor Sie sich für diesen Migrationspfad entscheiden, müssen Sie die Migration für Ihr individuelles Set an Features testen. Stellen Sie zudem sicher, dass Ihre Datensicherung läuft und funktioniert. Es kann bei diesem Vorgehen durchaus passieren, dass größere Probleme auftreten.

Um das Update aller im verteilten Monitoring beteiligten Instanzen im laufenden Betrieb auf die {v20} grundsätzlich zu ermöglichen, funktioniert die Synchronisation für eine Übergangszeit auch in dieser gemischten Umgebung, d.&thinsp;h.,  **von** einer Zentralinstanz mit Version {v20} **zu** fernen Instanzen mit Version {v16}.

Dieser Kompatibilitätsmodus wird in der nächsten {CMK}-Version (zur Zeit geplant als 2.1.0) entfernt. Wir empfehlen, alle im verteilten Monitoring beteiligen Instanzen zügig auf die Version {v20} upzudaten.
