...
//SK: Ich hab das Thema Backup jetzt doch mal kurz angesprochen. Mal sehen ob wir es drin lassen.
[#backup]
=== Backup
...
//SK: Problem ist halt, dass die Befehle zum Durchführen des selben eine andere Sprache sprechen:
//SK: Ubuntu: do-release-upgrade
//SK: Debian: apt full-upgrade bzw. dist-upgrade
//SK: RedHat: yum upgrade
//SK: Update meint bei den OSen immer etwas "kleineres".

//TK: Der folgende Teil würde dann durch einen Link auf den neuen Artikel ersetzt werden, richtig?
//SK: Kann man überlegen. Das ganze ist ordentlich formatiert doch länger als gedacht. Eigentlich wollte ich den Teil hier so lassen und dann für Alternativen und ausführlichere Beschtreibung der Schritte auf den Detailartikel verweisen.
...
//SK: Wo bringen wir diesen Spezialfall von "Lokale Dateien" unter? Oder sollen wir es vorziehen. Es sollte eher nicht nach dem Befehl find -L ~/local -type f stehen, weil einem die ganzen Sachen sonst dort auf die Füße fallen.
...
In größeren Umgebungen, in denen auch das Zurückspielen eines selbstverständlich vorhandenen Backups Ihrer {CMK}-Umgebung mit einem gewissen zeitlichen Aufwand verbunden wäre, empfielt es sich, vor dem Update der produktiven Umgebung, einen Test mit einer geklonten Site durchzuführen. Zu diesem Zweck können Sie bspw. das letzte reguläre Backup Ihrer Site unter einem anderen Namen wiederherstellen.
...
//SK: Ist das zu lang und ausführlich?
...
Aus diesem Grund ist eine Prüfung der von Ihnen installierten MKPs, besonders vor einem so umfangreichen Update, drigend geboten. So verhindern Sie, dass inkompatible Pakete das Update behindern oder im Anschluss an das Update doppelte oder zumindest sehr ähnliche Services entstehen.

Prüfen Sie hierzu Ihre installierten MKPs gegen unseren link:https://checkmk.com/de/integrations[Katalog der Checkplugins] und entfernen Sie Pakete, welche inzwischen nativ von {CMK} bereitsgestellt werden. Bei dieser Gelegenheit können Sie auch MKPs entfernen, die eventuell nur mal für einen Probelauf installiert worden sind. Eine Auflistung finden Sie über das WATO-Modul [.guihint]#Extension Packages#. Auf der Kommandozeile können Sie sich installierte Erweiterungen und auch unpaketierte Dateien mit den folgenden Befehlen anzeigen lassen:
...
Sollte Sie über die Prüfung der installierten MKPs und dem Abgleich mit unserem Katalog nicht alle Pakete zuordnen können, empfiehlt sich der oben beschriebene link:update20.html#dry_run[Probelauf des Updates] um dabei Inkompatibilitäten zu identifizieren und dann in Ihrem produktiven Monitoring vor dem Update zu entfernen.
...
//SK: Dagegen. Kein Backup - kein Mitleid.
...
//SK: Wollen wir an dieser Stelle noch beschreiben warum wir das so machen? Interessant wäre das eigentlich schon, aber uneigentlich sind die beiden Absätze hierüber schon fast zu lang.
...
//SK: Ich nenne diesen Abschnitt aus Tradition so. Vielleicht fällt uns dafür in diesem Kontext etwas besseres ein.

=== inconsistent use of tabs

Python 3 ist - auch bei den Anforderungen an eine korrekte Syntax - deutlicher strikter als sein Vorgänger. In der Beta-Phase von {CMK} {v20} sind uns immer wieder Plugins untergekommen, in denen schon kleine Fehler ein erfolgreiches Update berhindern konten. Wenn beim Update oder beim anschließenden Start der Site der Fehler [.guihint]#inconsistent use of tabs# auftritt, empfehlen wir das betroffene Plugin mit folgenden Befehl zu entfernen.
