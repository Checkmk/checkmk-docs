...
// TK: Fehlt hier nicht, welche Plugin-Version hier aktuell beschrieben ist?
...
// TK: Auch in "Installation auf der Kommandozeile" installierst Du aus dem Marketplace. Siehe auch Kommentar in Zeile 70.
// TK: Marketplace heißt das ja bei Grafana nicht: Grafana Plugins statt Marketplace? Oder Zumindest Grafana Marketplace?
// MFS: Geändert auf offizielle Grafana Plugin-Sammlung
...
// TK: Das Kapitel versteh ich nicht: Gerade schreibst du, dass On-Premises-Installationen mit allen Editionen möglich sind: Aber ein eigener Grafana-Server ist doch On-Premises?
// MFS: Sind sie, aber bei On-Premises muss nach CCE und allen anderen differenziert werden, da das signierte Plugin nur mit CCE läuft, 

Diese Installationsart setzt die Verwendung von {CCE} voraus.
Bei Betrieb von Grafana auf einem eigenen Server ist dort die Installation aus dem Marketplace via Kommandozeile möglich:
...
// TK: Das würde ich zum 1. Unterkapitel machen: Von einfach zu schwierig. Ausserdem bleibt dann in jedem der 3 Unterkapitel wenigstens 1 Komponente stabil.
// MFS: ???
...
// TK: Das ist jetzt aber nicht der {CMK}-Benutzer für Grafana, oder?
// MFS: So besser?
...
Sollten Sie Grafana in einem (Docker-) Container einsetzen, haben Sie verschiedene Möglichkeiten, Änderungen an der Grafana-Konfiguration vorzunemen.
// TK: Möglichkeiten wozu?
// MFS: Nebensatz ergänzt.
...
// TK: Nach der Installation des Plugins, Updates oder einer Änderung der Konfigurationsdatei starten Sie den Grafana-Serverdienst neu:
// TK: wobei: Was ist mit Updates gemeint? Updates sind in diesem Artikel gar nicht beschrieben.
// MFS: Da Grafana über das Paketmanagement installiert wird und Updates des Grafana-CMK-Plugins wie die Installation laufen, muss dies nicht extra beschrieben werden.
...
// TK: Uff: Ich wusste gar nicht, dass guest manche Rechte hat, die user nicht hat.
// MFS: It's complicated...
...
Geben Sie hier die URL zu Ihrer Instanz, den Typ Ihrer Edition und den soeben für Grafana angelegten xref:create_user[Nutzer] ein.
// TK: Du hast doch gerade vorher einen eigenen Grafana-Benutzer in {CMK} erstellt. Muss nicht der hier eingetragen werden?
// MFS: Exakt, fixed that!
...
// MFS: Exakt.
// TK: Und wenn nicht hier: Wozu brauch ich dann den Benutzer? 
...
// MFS: Hier ist innerhalb KNW noch die Diskussion nötig, ob wir darauf hinweisen müssen, dass das, was da in Grafana ankommt auch Unterschiede zwischen den CMK-Editionen aufweist, weil ja die Graphing Engine angezapft wird.
// MFS: Alte Review-Kommentare aus der 2.0 Zeit:
...
// TK: Mir würde es reichen, dass erwähnt wird, das nicht alle Graphing-Funktionen der kommerziellen Editionen in der CRE verfügbar sind. Link auf den Graphing-Artikel reicht da aus. Und ja, wenn dann in die Einleitung.
// TK: Die Beispiele unten sollten auch in der CRE funktionieren - bzw. wenn nicht, sollte es einen Hinweis darauf geben.
...
// TK: Wie heissen die Biester denn in Grafana? Visualisierung oder Panel?
// MFS: Ein Panel ist ein Container für eine Visualisierung.
...
// TK: Der Screenshot zeigt aber Maximum.
...
// TK: Im Screenshot gibt es keine Checkboxen.
...
// TK: Satz hinzufügen, dass das Pfade auf dem Grafana-Server sind.
