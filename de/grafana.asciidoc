...
Screenshots wurden mit Grafana 10.1 erstellt, frühere Versionen weichen in der Nutzerführung etwas davon ab.
...
Einzige Einschränkung hierbei: Nutzer des Grafana Cloud Angebotes müssen die {CCE} verwenden und das Plugin xref:installation_marketplace[aus dem Marketplace installieren].
...
Die Grafana-Anbindung benötigt auf Seite von {CMK} einen Nutzer, der über ausreichende Berechtigungen verfügt und sich zudem über ein _Automation secret_ statt eines Passwortes authentifiziert.
...
Der einfachste Weg zu einem "passenden" Nutzer ist, die xref:wato_user#roles[Nutzerrolle] des Gastnutzers (`guest`) zu klonen und mit der zusätzlichen Berechtigung [.guihint]#User management# (lesender Zugriff auf Nutzerinformationen) zu versehen.
// MFS: User management ist sehr irreführend für eine read-only Berechtigung!
Falls Sie Ihre Grafana-Nutzerrolle auf einer anderen Rolle basieren lassen, achten Sie darauf, dass neben [.guihint]#User management# auch die Berechtigung [.guihint]#See all host and services# gesetzt ist.

Ist die Rolle erstellt, legen Sie einen oder mehrere Grafana-Nutzer mit _Automation secret_ an.
