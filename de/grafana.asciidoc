...
Beginnend mit Version {v20} ist es in allen {CMK}-Editionen möglich,
...
link:https://github.com/Checkmk/grafana-checkmk-datasource/tree/old-main[Archiv auf GitHub]
einsehen.
...
* Sie laden die 1.x.x Zip-Datei aus dem link:https://github.com/Checkmk/grafana-checkmk-datasource/tags[GitHub-Repository] herunter und legen die Inhalte manuell in das Plugin-Verzeichnis von Grafana.
...
[{shell-raw}]
----
{c-root} wget -O grafana-checkmk-datasource-1.1.0.zip https://github.com/Checkmk/grafana-checkmk-datasource/archive/1.1.0.zip
----
...
direkt aus dem Git-Archiv beziehen,
geht die Aktualisierung auf eine neue Version schnell über diesen Befehl: `git pull`
...
[{shell-raw}]
----
{c-root} cd /var/lib/grafana/plugins/ && git clone https://github.com/Checkmk/grafana-checkmk-datasource.git --branch old-main
...
an. *Wichtig*: Wenn Sie eine xref:distributed_monitoring#[verteilte Umgebung]
...
==== Besonderheiten ab Grafana 8.0

Ab Grafana 8.0 ist es nicht mehr ohne Weiteres möglich, externe Plugins einzubinden, solange sie nicht signiert sind. Das trifft auf das {CMK}-Plugin zu, so dass Sie einen zusätzlichen Schritt benötigen.
...
xref:grafana#predefined[vordefinierten Graphen]; Sie ändern lediglich den [.guihint]#Mode#
...
oder wegfällt. Ab Version {v16}p2 haben Sie daher zusätzlich die
...
Ab Version {v16}p2 und der Version 1.1.0 des Plugins ist zusätzlich noch möglich,
