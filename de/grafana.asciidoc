...
// MA: Wir setzen pro prompt (= in sich geschlossenes Kommandozeilenbeispiel) eigentlich immer nur ein Kommando. Daher entweder in zwei Prompts aufteilen oder mit einem && verknüpfen.
...
// MA: Den letzten Satz hätte ich nach dem Screenshot gestellt, da er sich vom Beispiel wieder entfernt, bevor es gezeigt wird.
