...
Einzige Einschränkung hierbei: Benutzer des Grafana Cloud Angebots müssen die {CCE} verwenden und das Plugin xref:installation_marketplace[aus dem Marketplace installieren.]
...
Diese Installationsart setzt die {CCE} voraus.
...
// TK: ... und kann aktiviert und eingerichtet werden?

[#installation_marketplace]
=== Installation aus dem Marketplace ({CE}, Grafana Cloud)
// TK: Vielleicht besser: Installation aus der Grafana Cloud GUI
...
Erstellen Sie ein Plugin-Verzeichnis, falls dieses noch nicht existiert, und machen Sie den Grafana-Benutzer (meist `grafana`) zu dessen Eigentümer:
...
Sollten Sie Grafana in einem (Docker-) Container einsetzen, haben Sie verschiedene Möglichkeiten.
