...
// IGNORE Check_MK NVME Pluszeichen 0b1 mappen matcht unsignierten INI wegfällt
...
== Vorraussetzungen in {CMK} prüfen

Wenn Sie Ihre Instanz mit {CMK} {v21} erzeugt haben -- also den Befehl `omd create` mit der Version {v21} ausgeführt haben -- ist die {CMK} Web API in den globalen Einstellungen standardmäßig deaktiviert.
In diesem Fall müssen Sie die Web API durch das Entfernen des Hakens in [.guihint]#Setup > General > Global Settings > Site management > Disable Web API# wieder aktivieren.

image::grafana_reenable_web_api.png["Aktivieren der Web API in {CMK} {v21"}."]

Ab der Version {v22} von {CMK} wird das Plugin unsere REST-API unterstützen.
...
Laden Sie einfach die link:https://github.com/tribe29/grafana-checkmk-datasource/releases[aktuellste Version] als Zip-Datei herunter und kopieren Sie sie, z.B. mit `scp`, auf den Grafana-Server.
