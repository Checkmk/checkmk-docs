...
Bei der Verwendung des Development-Trees fehlen noch ein paar notwendige Dateien und Verzeichnisse, um das Plugin in Grafana nutzen zu können. Über folgendes Kommando erledigt das Programm `yarn` das Bauen der fehlenden Dateien für Sie:
...
----
// MA: Ich würde hier den heute gängigen Standard systemd nehmen: systemctl restart grafana-server
...
Geben Sie hier die URL zu Ihrer Instanz und einen Automations-Nutzer an, der Rechte hat, auf das Web-API zuzugreifen.
...
// MA: Auch hier: Man muss kein Titel angeben und der Button heißt (inzwischen?) "Apply"
...
Frühere Versionen des Plugins haben zugelassen, die angezeigten Metriknamen mittels Variablen zu steuern.
...
Diese Funktion wurde mit der Version 2.0 des Grafana-Plugins entfernt, weil sie letztlich Funktionalität dupliziert, die inzwischen Teil von Grafana ist.


=== Umbenennung über reguläre Ausdrücke

Mit dem Reiter [.guihint]#Transform# und dem hier vorhandenen Unterpunkt [.guihint]#Rename by regex# können 
