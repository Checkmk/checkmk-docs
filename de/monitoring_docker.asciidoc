...
Das Docker-Monitoring wurde in {CMK} Version {v15} eingeführt und in Version {v15} massiv umgebaut.
...
* Falls kein Agent im Container installiert ist, wird der Agent des Docker-Nodes im Container ausgeführt (diese Funktion wurde aus Gründen der Performance in der Version {v15} entfernt).
