...
[.relateddoc]
wato_monitoringagents, agent_linux, monitoring_kubernetes
...
Dinge finden Sie wie immer im link:check_plugins_catalog.html[Katalog der Check-Plugins].
...
Ab Version VERSION[1.6.0] der {CEE} können Sie mithilfe der link:dcd.html[dynamischen Konfiguration]
...
* In der Version VERSION[1.5.0] die Regelsätze [.guihint]#Docker node}} und {{Piggybacked Docker containers#
* Ab Version VERSION[1.6.0] der Regelsatz [.guihint]#Docker node and containers#
...
* Falls die Container keine eigene IP-Adresse haben (was meist der Fall ist), stellen Sie das Attribut [.guihint]#bei IP-Address-Family}} auf {{No IP# ein.
* Bei [.guihint]#Data sources}} stellen Sie {{Check_MK Agent}} unbedingt auf {{No agent# ein.
...
[.guihint]#Host Check Command}} an und setzen Sie die Option {{Use the status of the service#
...
erkannt. Für den Service [.guihint]#Docker node info# wird in Version VERSION[1.5.0]
der folgende Befehl
...
Ab Version VERSION[1.6.0] von {CMK} gibt es sogenannte link:labels.html[Hostlabels]. Das
