...
auf. {CMK} kann ab der Version {v15} Docker-Container
direkt über den xref:agent_linux#[Linux-Agenten] überwachen. Dabei werden nicht nur
...
Ab Version {v16} der {CEE} können Sie mithilfe der xref:dcd#[dynamischen Konfiguration]
...
Dazu benötigen Sie noch folgende xref:agent_linux#plugins[Agent-Plugins]:

* In der Version {v15} die beiden Plugins `mk_docker_node` und `mk_docker_container_piggybacked`
* Ab Version {v16} nur das Plugin `mk_docker.py`, das Sie hier finden: [.guihint]#Setup > Agents > Other operating systems > Plugins#

Installieren Sie die Plugins bzw. das Plugin wie gewohnt nach `/usr/lib/check_mk_agent/plugins`.
In den {EE} können Sie natürlich alternativ auch mit der xref:wato_monitoringagents#bakery[Agentenbäckerei]
arbeiten, welche für Docker entsprechende Regelsätze bereitstellt:

* In der Version {v15} die Regelsätze [.guihint]#Docker node# und [.guihint]#Piggybacked Docker containers#
* Ab Version {v16} der Regelsatz [.guihint]#Docker node and containers#

Bitte beachten Sie, das ab der {v16} die Python-Bibliothek
...
Ab der {v16} können Sie verschiedene Parameter des Plugins konfigurieren.
...
Ab Version {v16} der {EE} können Sie diese Hosts automatisch anlegen lassen.
Verwenden Sie dazu in der xref:dcd#[dynamische Konfiguration] den Konnektor [.guihint]#Piggyback#.
...
Falls Sie in dem Container einen link:agent_linux.html#Installation[Linux-Agenten] installiert haben,
...
und viele weitere Betriebssystemparameter), wurden diese bei Version {v16} entfernt.
...
xref:wato_monitoringagents#bakery[Agentenbäckerei] einstellen.
...
{v15} von {CMK} dazu automatisch der auf der Node installierte
...
ab Version {v16} notwendig, dass Sie den normalen {CMK}-Agenten
...
von Docker erst ab Version {v15}. Prüfen Sie daher, ob
...
erkannt. Für den Service [.guihint]#Docker node info# wird in Version {v15}
der folgende Befehl
...
Ab Version {v16} von {CMK} gibt es sogenannte xref:labels#[Hostlabels]. Das
...
für Ihre xref:wato_rules#[Regeln] verwenden, um Ihre Monitoringkonfiguration abhängig
