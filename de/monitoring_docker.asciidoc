...
Docker hat sich zu einem der am häufigsten verwendeten Container der Welt entwickelt. So notwendig eine durchgängige und transparente Überwachung der Container ist, so komplex ist sie aber auch aufgrund deren dynamischer und vielschichtiger Architektur.
...
Legen Sie dazu eine Konfigurationsdatei nach `/etc/check_mk/docker.cfg`.
...
//Frage SP: Weiß der normale Anwender, wie das über die Konfigurationsdatei geht? Ich bräuchte mehr Details. Gleiches gilt aus meiner Sicht für Nutzung der Agentenbäckerei. Auch hier weiß ich nicht wirklich (oder finde ich intuitiv) was ich einstellen muss.//
...
//Frage SP: Und was sind geeignete Translation-Regeln? Ich sehe die Optionen. Kann ich davon ausgehen, dass ein Docker-Anwender jetzt weiß, was er einstellen kann/muss? Oder liegt das außerhalb unserer Zuständigkeiten?//


==== Überwachen des Hoststatus

//Das muss ich mir noch einmal ansehen. Völlig unklar, wie und wo ich mich befinde//
...
image::docker_container_hoststatus.png[alt="?????????"]
