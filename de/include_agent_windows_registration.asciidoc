...
////
MFS: Text aus Linux-Agent, wird möglicherweise während Beta-Phase umgestellt:
...
Wie weiter oben beschrieben, ist der Windows-Client beim Update über den herkömmlichen, unverschlüsselten Weg erreichbar.
Dabei zeigt Ihnen der Service [.guihint]#Check_MK# im Zustand {WARN}, dass eine Umstellung auf verschlüsselte Kommunikation angeraten ist.
Bei einer Neuinstallation dagegen ist Registrierung Pflicht, unverschlüsselte Kommunikation nicht möglich.
...
*Hinweis:* Auf sehr alten Windows-Varianten (Windows Server 2008 R1, Windows Vista) mag die Installation des Agenten erfolgreich sein, falls der neue Agent Controller wegen API-Kompatibilitäten nicht gestartet werden kann, wird als Fallback der alte Modus mit unverschlüsselter Übertragung verwendet. Sichern Sie in diesem Fall – falls nötig – mit symetrischer Verschlüsselung oder SSH-Tunnel ab.

// MFS: Benötigen wir auch unter Windows die Unterscheidung zwischen Push- und Pull-Modus? Derzeit deutet nichts darauf hin, dass Push unter Windows Relevanz bekommt.
...
// MFS: Das ist bereits Beta 2!
// MFS: Forward Slashes in Pfaden zu Executables sind seit NT 4 erlaubt (wegen POSIX Konformität). 
[{shell}]
----
...
Dabei ist der Host-Name hinter dem Parameter `--hostname` exakt so anzugeben, wie zuvor beim Erstellen im Setup.
...
Bestätigen Sie mit `Y` und geben Sie gegebenenfalls das Passwort ein, um den Vorgang abzuschließen.
...
// MFS: Wird mglw. auch für den Windows-Agenten relevant!
