...
image::hosts_setup_management_board.png[alt="Einrichtung separater Management-Schnittstellen."]
...
. Zuletzt werden die Hosts in dem Ordner erzeugt. Ausgelassen werden dabei alle Hosts mit Namen, die es in der Instanz bereits gibt.
...
Es gibt einen komfortablen Weg, um in der xref:user_interface#search_setup[Konfigurationsumgebung] (im [.guihint]#Setup#-Menü) gezielt nach Hosts zu suchen. Dies hat zwei Vorteile:
...
Wenn Kommandozeile, Skripte und APIs nicht Ihre Werkzeuge der ersten Wahl sind,
