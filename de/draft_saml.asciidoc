...
////
MFS: Ein paar Kritikpunkte:

1. Zertifikatshandling ist recht rustikal, hier sollte auf Best Practices verwiesen werden und erwähnt werden, dass in Produktivumgebungen eine interne CA aufzusetzen sinnvoll ist und die verwendeten Beispielskripte eher für Testumgebungen geeignet sind. Viele Admins machen sich wenig Kopf um "chains of trust" und wollen nur, dass alles funktioniert. Ein von einem Script vergessener Schlüssel ohne Passphrase, der verwendet werden kann, gültige Zertifikate zu signieren, ist a bissele a Problem, wenn das Root-Zertifikat bei 1000 Servern in den Trust Store aufgenommen wurde.

2. Auch wenn Du wenig Einfluss auf die Screenshots hast: Ein paar sollten der Übersichtlichkeit zuliebe zumindest unten beschnitten werden, damit auch auf kleinen Bildschirmen wenigstens ein Screenshot und die Texte drüber und drunter sichtbar sind. Ein Screenshot mit sehr kleiner Schrift sollte noch mit target="_blank" separat verlinkt werden, um lesbar zu sein.

3. Am Ende sollte noch ein Abschnitt "Logging und Debugfiles" hinzugefügt werden. Es gibt sicher Möglichkeiten, für mod_mellon das Logging alleine hochzudrehen oder gar in ein separates Logfile zu schreiben. Gerade vor dem Hintergrund der Konfiguration für andere IDPs ist Logging/Debugging sicher sinnvoll. Evtl. nochmal im Consulting/Forum nachfragen?

Desweiteren ist genau das ein Beispiel für Funktionalität, die nicht Kernfunktionalität von CMK ist und daher eher sporadisch getestet wird. D.h. es wäre sinnvoll, gleich in der Einleitung exakt eine Systemumgebung zu nennen, mit der das Setup funktionierte, bspw. Ubuntu 20.04 mit Checkmk 2.0.0p16 und Anbindung von Windows Server 2012 als IDP. D.h. sinnvollerweise einen der Consultants fragen, der dieses Setup zuletzt aufgesetzt hat, wie die Systemumgebung aussah.
...
In diesem Artikel erfahren Sie, wie Sie eine Anmeldung via _Secure Assertion Markup Language_ (SAML) einrichten. Die Implementierung in {CMK} läuft auf Apache-Level über link:https://github.com/latchset/mod_auth_mellon[mod_auth_mellon^].
...
*Hinweis:* Dieses Feature wird von den SUSE-Linux-Enterprise-Server-Versionen 12 SP3, 12 SP4 und 15 aufgrund fehlender Abhängigkeiten nicht unterstützt (ab SLES 15 SP1 ist die Anbindung möglich).

////
MFS: Hier gehört meiner Ansicht nach noch der weitere Hinweis rein, dass alles SSL relevante hier Adhoc geschieht und ALLES gefährlich ist, wo Root-Zertifikate nicht sauber mit Passwort versehen und weggeschlossen sind. Evtl. bereits an dieser Stelle Verweis auf omd_https?
////
...
// MFS: Umstellung von mir, da URL kein Argument eines Parameters ist
[{shell}]
----
...
// MFS: ...das öffentliche Serverzertifikat...
Nun benötigen Sie den öffentlichen Schlüssel des AD-FS-Servers, der in der Datei `idp-public-key.pem` gespeichert wird:
// MFS: Diese Methode extrahiert nur das Zertifikat des Servers, ignoriert den Rest der Zertifikatskette. Sich die Zertifikatskette ggf. selbst zusammenzufrickeln, ist ziemlich PITA. Eigentlich sollte man sich das Root-Zertifikat und alle weiteren Zertifikate der Kette besorgen, um sauber Vertrauensverhältnisse aufzubauen. D.h. Admin fragen: "Gib mal Zertifikatskette rüber, gerne in einer konkatenierten Datei."
...
// MFS: Das -n kann weg und Du kannst gerne anfügen, das in der folgenden SSL-Session interaktiv HTTP GET etc. möglich wäre, s_client ist eine Art Debug-Modus für SSL verschlüsselte Protokolle.
...
// MFS: Tendenziell verlinken auf omd_https.html, wo komplettes Zertifikatsmanagement beschrieben wird, wir haben bei dem gesamten Themenkomplex das Problem, dass ein Root-Zertifikat, welches Ad-Hoc erzeugt wurde und ohne Schlüssel irgendwo™ herumliegt, mehr Schaden anrichten kann, wenn es in den systemweiten Store installiert wurde, als wenn nur eine Anwendung einem Zertifikat vertraut.
////
...
// MFS: [source,apache] müsste funktionieren, bitte testen.
// MFS: Das Error-Dokument ist semantisch falsches HTML. Es müsste wie folgt aufgebaut sein um die Fallback-Reihenfolge 1. Redirect per HTTP-Header, 2. Redirect per Meta-Direktive, 3. Nutzer muss klicken, zu respektieren: 
////
<html>
  <head>
...
// MFS:  Screenshot unten beschneiden: 
...
// MFS:  Screenshot unten beschneiden/zusammenschieben: 
...
// MFS: WTF? Das ist bei einer CMK-Testinstanz OK, aber nicht bei einem Produktivsystem!? D.h. bitte anders formulieren, bspw. "Falls Sie zunächst Permit everyone wählen, denken Sie unbedingt daran, die Rechte nach den ersten Tests auf Permit specific group ändern.
...
// MFS:  Screenshot unten beschneiden und Link auf Screenshot im Vollbild einfügen: 
image::saml_adfs_10.png[]

Fügen Sie im folgenden Dialog über [.guihint]#Add Rule...# eine neue Regel hinzu:

// MFS:  Screenshot Größe anhand von Schriftgröße berechnen: 
...
Damit ist auch die AD-FS-Konfiguration abgeschlossen, FS kann nun aus der Windows-Authentifizierung die Authentifizierung für {CMK} ableiten, das Sie im nächsten Schritt anweisen, Nutzer über HTTP-Anfragen zu authentifizieren.
...
// MFS: Syntax-Highlighting für Apache aktivieren, Error Document wie oben
.~/etc/apache/conf.d/auth.conf
[{file}]
----
#Set this to the Name of your Checkmk site, e.g.
