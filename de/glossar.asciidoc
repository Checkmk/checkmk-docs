...
[#api_integrations]
==== API-Integrationen
Wenn im Setup von {CMK} von [.guihint]#API integrations# die Rede ist, sind Monitoring-Daten gemeint, die zwar das Datenformat des {CMK}-Agenten nutzen, aber aus anderer Quelle stammen. 
...
// TK: Es gibt ja 3 Werte, die API integrations enthalten: Einer von den dreien muss ausgewählt sein, richtig?
// MFS: Ja… da gibt es verschiedene Kombinationen, das sollte man hier nicht austreten.
...
Host-Gruppen ermöglichen eine zweite Ebene der Gruppierung quer über die Ordnerstruktur, um Hosts im Monitoring z.B. in Tabellenansichten auswählen zu können.
Host-Merkmale, Labels und Ordner werden genutzt, um Hosts über eine Regel solchen Gruppen zuzuordnen.
Hosts können aber auch explizit einer Gruppe zugeordnet werden.
...
[#rule]
==== Regel
Regeln sind die Grundlage der Konfiguration von Hosts und Services in {CMK}. Regeln in einem Regelsatz steuern immer einen einzelnen, fokussierten Aspekt eines Hosts oder Services. Sie lassen sich mit Bedingungen versehen, sowie innerhalb eines Regelsatzes beliebig aufeinander "stapeln". Die Auswertung erfolgt dann von oben nach unten, so dass es Standardregeln geben kann, wenn keine Bedingung greift, aber auch sehr spezielle Regeln, die nur einen ganz bestimmten Host betreffen. Viele Regelsets in {CMK} haben bereits vordefinierte Standardwerte, so dass nur für abweichende Anforderungen Regeln erstellt werden müssen.
...
{CMK} unterstützt dieses Protokoll -- da es aber vergleichsweise ineffizient ist, sollten Sie SNMP nur bei Geräten einsetzen, die keine effizienteren Möglichkeiten zur Überwachung anbieten.
// TK: keine anderen Möglichkeiten > keine effizienteren Möglichkeiten?
// MFS: Hier könnte man noch ergänzen "beispielsweise Spezialagenten"…
// MFS: Und das würde uns gleich zum nächsten Abschnitt bringen: …und SNMP steht nicht (mit befriedigender Performance oder ausreichendem Datenumfang) zur Verfügung…
...
[#wato]
==== WATO
Das „Web Administration Tool“ war bis zur {CMK}-Version {v16} das GUI-Werkzeug zur Konfiguration von Checkmk. WATO wurde in der Version {v20} ersetzt durch das [.guihint]#Setup#-Menü in der Navigationsleiste.

Mehr unter xref:wato#setup_menu[Setup-Menü.]
// MFS: Historie einarbeiten? "Der Begriff WATO stammt aus der Zeit, als {CMK} eine Script-Sammlung und kein integriertes Produkt war, mit der Einführung von WATO hatten Nutzer arstmals die Möglichkeit, {CMK} optional über eine Weboberfläche statt über Konfigurationsdateien anzupassen.
