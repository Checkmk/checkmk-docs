...
// TK: Auch wenn es den deutschen Begriff in der GUI nicht gibt, würde ich ihn trotzdem als Titel verwenden.
// TK: Eigenschaften eines Hosts hinzugefügt, um das Finden einfacher zu machen (GUI Suche im Setup nach API integrations liefert kein Resultat)
// TK: Datenformat des Agenten: Agent durch {CMK}-Agent ersetzt
...
[#snmp]
==== SNMP
Das „Simple Network Management Protocol“ dient der Überwachung und Konfiguration von Netzwerkgeräten wie Routern, Switches oder Firewalls. {CMK} unterstützt dieses Protokoll -- da es aber vergleichsweise ineffizient ist, sollten Sie SNMP nur bei Geräten einsetzen, die keine anderen Möglichkeiten zur Überwachung anbieten.
