...
[#api_integrations]
==== API integrations
Wenn im Setup von {CMK} von "API integrations" die Rede ist, sind Monitoringdaten gemeint, die zwar das Datenformat des Agenten nutzen, aber aus anderer Quelle geliefert werden. Diese Quellen können Datenquellenprogramme, Spezialagenten oder Piggyback-Daten (huckepack von anderen Hosts gelieferte Daten) sein. Sollen derartige Daten im Monitoring verwendet werden, ist die entsprechende Option zu aktivieren. 

Mehr unter xref:datasource#[Datenquellenprogramme.]
...
[#snmp]
==== SNMP
Das "Simple Network Management Protocol" dient als Protokoll zur Überwachung und Konfiguration von Netzwerkgeräten wie Routern, Switches oder Firewalls. {CMK} unterstützt dieses Protokoll. Da SNMP vergleichsweise ineffizient ist, sollten Sie dieses Protokoll nur bei Geräten einsetzen, die keine anderen Monitoring-Möglichkeiten anbieten.
