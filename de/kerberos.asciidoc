...
Um {CMK} auf die Authentifizierung über Kerberos umzustellen, sichern Sie als Instanzbenutzer im Apache-Verzeichnis die Datei `cookie_auth.conf` weg.
...
Danach wird die Datei `auth.conf` gelöscht und neu geschrieben.
Im folgenden dargestellten Dateiinhalt sind die Einträge in den markierten Zeilen lediglich Beispiele.
Passen Sie diese daher entsprechend Ihrer Umgebung an:
...
Define SERVICENAME myservice.mydomain.tld@${REALM}
...
<Location /${SITE}>
  ...
  KrbAuthRealm ${REALM}
  KrbServiceName HTTP/${SERVICENAME}
  Krb5Keytab /omd/sites/${SITE}/etc/apache/cmk_http.keytab
...
Wenn Sie zusätzlich zu dem Logins über SSO auch reguläre zulassen möchten, aktivieren Sie die Cookie-Authentifizierung.
Ändern Sie dazu entsprechend in der `auth.conf` der folgende Zeile den Wert auf `on`:
