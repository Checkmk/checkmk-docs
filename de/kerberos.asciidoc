...
Ein Single Sign-On auf die GUI von {CMK} mit Kerberos wird von {CMK} zwar nicht offiziell unterstützt, aber Sie können es sich selbst einrichten, sofern Sie grundsätzliches Wissen rund um Kerberos mitbringen. Was auf der Seite von Checkmk benötigen, zeigt Ihnen diese Anleitung.
...
* Für die {CMK}-Instanz ist Cookie-Authentifizierung eingeschaltet (zu überprüfen mit `omd config show` > Web GUI > `MULTISITE_COOKIE_AUTH: on`).

Der letzte Punkt stellt sicher, dass sich Nutzer ohne SSO über die reguläre Login-Seite anmelden können.
Sie können diese Möglichkeit auch xref:kerberos#cookie[deaktivieren].
In diesem Fall gilt diese Voraussetzung natürlich nicht.

[#configuration]
== Integration von Kerberos

Um {CMK} auf die Authentifizierung über Kerberos umzustellen, löschen Sie die Datei `auth.conf` und schreiben sie neu.
Im folgenden dargestellten Dateiinhalt sind die Einträge am Anfang lediglich Beispiele.
Passen Sie diese daher alle Definitionen am Anfang beginnend mit `Define` entsprechend Ihrer Umgebung an:
...
Sofern Sie nicht `basic auth` mit Cookies zulassen wollen,
benötigen Sie den Cookie-Auth der Instanz nicht mehr und deaktivieren ihn entsprechend.
Andernfalls können Sie den nachfolgenden Schritt überspringen.
Achten Sie darauf, dass Sie diesen Wert nur ändern können, wenn die Instanz gestoppt ist:
...
Anschließend löschen Sie die Datei `cookie_auth.conf` aus dem Apache-Verzeichnis der Instanz:
...
[#cookie]
== Basic auth und Cookies zulassen

Wenn Sie zusätzlich zu dem Login über SSO auch reguläre über `basic auth` zulassen möchten, aktivieren Sie diese über eine Option in der `auth.conf`. Ändern Sie dazu in der folgenden Zeile den Wert auf `on`:
...
Wenn Sie zusätzlich auch die Authentifizierung per Cookie zulassen wollen, stellen Sie sicher,
dass die Option am Ende von xref:#configuration["dem vorherigen Kapitel"] nicht in der {CMK}-Instanz abgeschaltet oder
die dafür notwendige Konfiguration gelöscht haben.
