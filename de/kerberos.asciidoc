...
Ein Single Sign-On auf die GUI von {CMK} mit Kerberos wird von {CMK} zwar nicht offiziell unterstützt, aber Sie können das recht einfach selbst einrichten.
Wie das geht, zeigt Ihnen diese Anleitung.
...
* Eine Keytab wurde -- z.B. unter `/etc/krb5.keytab` -- erstellt und darf von dem Instanzbenutzer gelesen werden.
...
Wenn Sie nur noch Logins über SSO zulassen möchten, deaktivieren Sie die Cookie-Authentifizierung.
