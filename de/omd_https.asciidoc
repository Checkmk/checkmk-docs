...
// MFS: Aufgabe des privaten Schlüssels ergänz.
...
// MFS: Ich habe nun eine kurze Erläuterung ergänzt.
...
// MA: Warum finde ich es in Ordnung, wenn man Let's Encrypt explizit erwähnt und
...
// MFS: Steckt ja auch mittlerweile viel OSS Hirnschmalz drin, damit das alles reibungslos geht.
...
// *Hinweis:* Certbot 0.40 aus Ubuntu 20.04 war die früheste Certbot-Version, mit der wir Stand März 2022 ohne Änderung der Konfiguration Let's Encrypt-Zertifikate anfordern konnten. Ältere Versionen mögen ACME v2 fähig sein, decken aber unter Umständen nicht den kompletten Funktionsumfang von ACME v2, lassen DNS-Plugins vermissen oder erfordern teils manuelle Konfiguration.
...
// MFS: Die Reihenfolge entspricht nun den eher pragmatischen Präferenzen vieler Admins. Ich möchte nicht dazu schreiben, dass 0.40 aus Ubuntu 20.04 funktionieren könnte, weil es durchaus sein kann, dass dem bald die Funktion abgedreht werden muss.
...
// MFS: Die Datei wird dann angelegt, wenn mod_ssl aktiviert wird, siehe ganz oben.
...
Wenn Sie Let's Encrypt verwendet haben, um Zertifikate zu generieren, aber Certbot die Konfiguration nicht aktualisieren haben lassen, finden Sie die Pfade unterhalb `/etc/letsencrypt/live` heraus und tragen Sie diese ein:
...
// MFS: In der Tat ist verwirrend, dass Apache den Begriff Sites anders verwendet als Checkmk, ich habe daher versucht, mit kurzer Erklärung der VirtualHosts dies abzufangen.
