...
// MA: Besser? Wichtig ist ja dass HTTPS nett ist, aber nicht das Ende der Geschichte.
...
// MA: Das sehe ich nicht so. Da ja oben ein redirect von HTTP auf HTTPS gemacht wird, braucht es 
// MA: das nicht. Aber so ist es tatsächlich unabhängiger von der Beschreibung oben.
...
// MA: Ich hab den Punkt entfernt und darunter hinzugefügt. Dann wird klarer, wo die Gefahr liegt.
