...
:revdate: 2019-06-18
:title: Nagstamon erfolgreich mit {CMK} verbinden
:description: Nagstamon ist ein Tool für Administratoren, um auf dem Desktop auch ohne Browser Probleme sofort zu sehen. Diese Anleitung hilft bei der Anbindung an ihre {CMK}-Installation.
...
link:http://nagstamon.ifw-dresden.de/[Nagstamon] ist ein kleines Tool
für den Desktop, welches ursprünglich für Nagios entwickelt wurde. In der
heutigen Version unterstützt der Client unter anderem iaber auch {CMK}. Es
übersetzt in sehr kompakter Form eine Ansicht (View) aus {CMK} und fasst
...
Folgenden beschreiben wir, wie man Nagstamon mit {CMK} verbinden kann.


== Konfiguration

Die einfachste Methode ist es, Nagstamon direkt mit der Weboberfläche von {CMK}
zu verbinden. Damit diese Anbindung nahtlos klappt, liefert {CMK}
...
Einrichtung eines neuen Servers über den Menüpunkt [.guihint]#Settings#. Hier
erstellen Sie dann eine neue Verbindung über [.guihint]#New server...#.

Wählen Sie in dem darauf erscheinenden Dialog unter [.guihint]#Monitor type# den Eintrag
[.guihint]#{CMK} Multisite# aus, vergeben einen menschenlesbaren Namen und tragen
die URL der Anbindung in das Feld [.guihint]#Monitor URL# ein. *Wichtig:*
...
Über die Option [.guihint]#Show more options# können Sie dann noch Feinheiten
