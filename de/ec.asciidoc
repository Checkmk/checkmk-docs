...
* Insgesamt sehr lang, evtl. in 2 oder 3 Artikel splitten?
  ...
  TK: Ja, viel zu lang! Aber SNMP-Traps bringen nur 150 Zeilen und Logwatch nur 80 Zeilen.
  TK: Logwatch sollte in den neuen Logwatch Artikel wandern (KNW-344)
  TK: Ich wäre eher für Aufteilung EC für Einsteiger und EC für Experten (mit allen Regeldetails)
...
// TK: Diese ganzen Punkte sollten in ein EC-Folgeticket verschoben werden. Als Kommentare gehen sie hier unter.
// TK: plus einige der Detailkommentare im Artikel: die Auswahl überlass ich Dir ;-)
...
// TK: Finde ich unschön: zuerst den Begriff „Ereignis“ einführen und dann aber quasi im ganzen Artikel mit Event weiterzumachen.
// MFS: So besser? Event muss man nicht ersetzen, das Wort ist mittlerweile eingedeutscht, auch wenn der Duden als Gebrauch "Jargon" angibt: https://www.duden.de/rechtschreibung/Event
...
* Künstliche Events können erzeugt werden, wenn bestimmte link:ec.html#expect[Meldungen ausbleiben].
...
// TK: GUI-Ablauf umgeschrieben, z.B. da die Liste ab Werk nicht (mehr) leer ist.
...
// TK: Da der „Regelsimulator“ in der GUI „Event Simulator“ heißt, hab ich das geändert.
...
// TK: Mit diesem und dem folgenden Screenshot bist Du schon weiter als in der Beschreibung, denn da müsste noch ein Foobar-Daemon die Meldung produzieren.
// MFS: Mißverständnis? 
...
Auch bei den link:wato_user.html#roles[Rollen und Berechtigungen] hat die Event Console einen
...
// TK: Hier und da überarbeitet, um die Verständlichkeit und Nachvollziehbarkeit zu erhöhen ;-)
[#hostnames]
...
`\1.mydomain.test`. 
// TK: Würde helfen, diese simple Regel auch im obigen Screenshot zu sehen.
// MFS: Müsste man einen zweiten Screenshot machen, der Text deckt ja hostname translation generell ab...
...
Events filtern, Kommandos ausführen usw. Einzelheiten erfahren Sie im Artikel über die link:views.html[Ansichten].
...
Mit dem Kommando [.guihint]#Custom Action# können Sie auf Events frei definierbare link:ec.html#actions[Aktionen]
...
von einem aktiv überwachten Service. Diese durchläuft die link:notifications.html[Benachrichtigungsregeln]
...
// TK: In dem Screenshot ist ein hilite angebracht.
...
link:regexes.html#matchgroups[Match-Gruppen] beim Feld [.guihint]#Text to match#. Damit sind
...
Service-Vereinbarung verbunden sein. In {CMK} können Sie per link:wato_rules.html[Regeln]
...
erfahren Sie im link:ec.html#visibility[Kapitel über das Monitoring].
...
Beim Umschreiben sind die oben beschriebenen link:ec.html#matchgroups[Match-Gruppen] besonders
...
für eine beliebige Zeichenfolge, die in einer link:ec.html#matchgroups[Match-Gruppe]
...
// TK: Das Feld Application gibt es nicht; Syslog application to cancel event?
...
* Host
* Anwendung
* link:ec.html#matchgroups[Match-Gruppen]
...
erzeugt werden.
// TK: Ich finde „aufgehen" (und „öffnen“ im nächsten Absatz) merkwürdig. 
// MFS: So schöner?
...
// TK: Dann machen Regelpakete Dinge unübersichtlicher? Dieser Satz ist unglücklich formuliert.
// MFS: Da fehlte "nur"...
...
oder beim Öffnen oder link:ec.html#cancelling[Aufheben] von Events ausführen lassen können:
...
Die Zuordnung geschieht in Echtzeit durch eine link:livestatus.html[Livestatus]-Abfrage an den
...
|===
// TK: Warum sind diese Felder mit guihint ausgezeichnet (auch die in der folgenden Tabelle)? Wo in der GUI sind die zu sehen?
// TK: Die GUI-Felder in den Event-Details kann ich nur zu einem kleinen Teil zu den genannten Feldern mappen.
...
werden, wenn aufgrund der Regel ein Event _geöffnet_ oder link:ec.html#cancelling[aufgehoben]
...
oder link:ec.html#timing][Verzögerung] kann es nämlich dazu kommen, dass Events aufgehoben
...
// TK: Ach ja? Wo finde ich denn diese GUI?
// MFS: Neu formuliert. So besser?
...
// TK: Regelsatz Logwatch patterns gibt es nicht, sondern nur Logfile patterns
// TK: In Logfile patterns gibt es aber keine Option Reclassify messages before forwarding them to the EC
// MFS: Besser?
...
xref:ec#outcome[Setzen von Status und Service-Level] möglich.
// TK: Überarbeitet und Link hinzugefügt
...
|DELAYOVER |Das Event wurde geöffnet, da die in der Regel konfigurierte link:ec.html#timing][Verzögerung] abgelaufen ist.
...
link:ec.html#globalsettings[Einstellungen der Event Console] können Sie die Rotation
...
// TK: Was ist denn mit "von alt nach neu löschen" gemeint? zuerst die ältesten löschen?
// MFS: Jepp!
...
// TK: übrigens? Typo? "übrigen" macht hier aber auch keinen Sinn.
// MFS: Da war schon "übrigen" im Sinne von "weiteren" gemeint. Das ist aber nur ein kleiner Hack, um auch Dinge zu archivieren, die sonst nicht archiviert worden wären. Hätte ich den Artikel geschrieben, wäre das nicht dokumentiert worden – CMK ist kein Syslog-Server.
...
// TK: Da steht doch gar nix von brauchen?
// MFS: Ursprünglich stand "Speicherverbrauch" drin. Klingt wie Reifenverschleiss. Uh, mein RAM Ist nur noch zur Hälfte da, muss neues kaufen gehen. ;-) 
...
// TK: gespeicherten Regelzählen? oder: die per Regel gezählten und gespeicherten Meldungen?
// MFS: Zählerstand ist besser hier...
...
link:notifications.html#rules[Benachrichtigungsregeln] von {CMK}. Diese Regeln müssen sich nicht
...
[#expire]
// TK: Anker gesetzt (best guess), die beiden internen Links in diesem Artikel waren schon in der 1.6 tot.
=== Zu großes Archiv
...
|`var/mkeventd/history/` |Ablageort des link:ec.html#archive[Archivs].
