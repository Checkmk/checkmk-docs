...
Die Event Console empfängt *Meldungen* (_messages_). Eine Meldung ist
...
direkt ein *Ereignis* (_event_) mit den gleichen Attributen, aber:
...
// TK: Kleinigkeit: Im Screenshot müsste "Add rule pack" stehen und nicht "Edit rule pack test".
...
Regelpakete haben nicht den Sinn, Dinge übersichtlicher zu machen, sondern können
...
Event Console unabhängige Überwachung von Log-Dateien -- inklusive einer
...
xref:ec.html#outcome[Setzen von Status und Service-Level] möglich.
