...
* Meldungen können aktuelle Events wieder xref:ec#cancelling[aufheben].
* Künstliche Events können erzeugt werden, wenn bestimmte xref:ec#expect[Meldungen ausbleiben].
...
jetzt die Meldung *tatsächlich erzeugt.* Eventuell definierte xref:ec#actions[Aktionen]
...
stehen Ihnen Events sowie xref:ec#archive[Event-Historie] als Datenquellen zur Verfügung.
...
Mit dem Kommando [.guihint]#Custom Action# können Sie auf Events frei definierbare xref:ec#actions[Aktionen]
...
Einzelheiten zur Benachrichtigung durch die EC erfahren Sie xref:ec#notifications[weiter unten].

==== Archivieren

Der Knopf [.guihint]#Archive event# löscht den Event endgültig aus den offenen Events. Da
alle Aktionen auf Events -- inklusive dieses Löschvorgangs -- auch im xref:ec#archive[Archiv]
...
* Wird der Host jedoch *nicht* gefunden, so werden die Kontaktgruppen -- falls xref:ec#contactgroups[dort konfiguriert] -- aus der Regel übernommen, welche den Event erzeugt hat.
...
Meldungen, wenn Sie in den xref:ec#globalsettings[Einstellungen der EC] den Wert
...
* Umschreiben von Events (xref:ec#rewriting[Rewriting])
* Automatisches Aufheben von Events (xref:ec#cancelling[Cancelling])
* Zählen von Meldungen (xref:ec#counting[Counting])
...
Zu den Regelpaketen erfahren Sie xref:ec#rulepacks[weiter unten] mehr.
...
xref:ec#expire[Herausaltern] dieser Events interessant.
...
Die Kontaktgruppen für die xref:ec#visibility[Sichtbarkeit] werden
auch bei der
xref:ec#notifications[Benachrichtigung] von Events verwendet. Sie können hier per
Regel Events explizit Kontaktgruppen zuordnen. Einzelheiten
erfahren Sie im xref:ec#visibility[Kapitel über das Monitoring].
...
Beim Umschreiben sind die oben beschriebenen xref:ec#matchgroups[Match-Gruppen] besonders
...
für eine beliebige Zeichenfolge, die in einer xref:ec#matchgroups[Match-Gruppe]
...
funktionieren nur beim xref:ec#rewriting[Rewriting].
...
Sie können beim Aufheben eines Events auch automatisch xref:ec#automatic_actions[Aktionen] ausführen
...
* Host
* Anwendung
* xref:ec#matchgroups[Match-Gruppen]
...
automatischen xref:ec#cancelling[Aufheben] von Events arbeiten. Setzen
...
oder beim Öffnen oder xref:ec#cancelling[Aufheben] von Events ausführen lassen können:

* Ausführen von selbstgeschriebenen Shell-Skripten
* Versenden von selbstdefinierten E-Mails
* Erzeugen von {CMK}-xref:ec#notifications[Benachrichtigungen]
...
Dabei wird das gleiche Verfahren wie bei der xref:ec#visibility[Sichtbarkeit von Events]
...
|[.guihint]#Datum/Uhrzeit# |Bei Events, die xref:ec#counting[zählen], ist das der Zeitpunkt des _letzten_ Auftretens einer zum Event gehörigen Meldung.
...
oben bei den xref:ec#commands[Kommandos] gesehen. Spannender ist das automatische
...
werden, wenn aufgrund der Regel ein Event _geöffnet_ oder xref:ec#cancelling[aufgehoben]
...
Event schon die Phase [.guihint]#open# erreicht hat. Bei Verwendung von xref:ec#counting[Zählen]
...
Wie im Abschnitt über das xref:ec#setup[Aufsetzen] der Event Console beschrieben,
...
|CANCELLED |Das Event wurde durch eine OK-Meldung automatisch xref:ec#cancelling[aufgehoben].
...
|EXPIRED |Das Event wurde automatisch archiviert, da seine konfigurierte xref:ec#expire[Lebenszeit] abgelaufen ist.
...
xref:ec#globalsettings[Einstellungen der Event Console] können Sie die Rotation
...
* Arbeiten Sie mit xref:ec#rulepacks[Regelpaketen], um Sätze von verwandten Regeln zusammenzufassen. Die erste Regel in jedem Paket sollte das Paket sofort verlassen, wenn die gemeinsame Grundbedingung nicht erfüllt ist.
...
|pro Host |Die Kontakte zum Host, welche genauso ermittelt werden, wie bei der xref:ec#notifications[Benachrichtigung von Events] über {CMK}.
...
Wie xref:ec#archive[oben] gezeigt, hat die Event Console ein Archiv von allen Events
...
aufzeichnet und Sie auch warnt, wenn es zu xref:ec#overflow[Überläufen] kommt.
...
|`history` |Zugriff auf das xref:ec#archive[Archiv]. Eine Anfrage auf diese Tabelle führt zum Zugriff auf die Textdateien des Archivs. Verwenden Sie auf jeden Fall einen Filter über die Zeit des Eintrags, um einen Vollzugriff auf alle Dateien zu vermeiden.
...
|`ACTION` |Führt eine benutzerdefinierte Aktion auf einem Event aus. Argumente sind Event-ID, Benutzerkürzel und Aktions-ID. Die spezielle ID `@NOTIFY` steht für eine xref:ec#notifications[Benachrichtigung] über {CMK}.
|===
...
|`var/mkeventd/history/` |Ablageort des xref:ec#archive[Archivs].
