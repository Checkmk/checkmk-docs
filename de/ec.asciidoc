...
Vorweg: Keine massiven strukturellen Eingriffe, weitgehend:

* neue Screenshots
* Setup-GUI statt WATO
* Benachrichtigung statt Alarmierung
* 4 Tickets eingearbeitet: KNW-51, KNW-49, KNW-139, KNW-140

Diskussionsbedarf bei Verwendung des Begriffes "Operating", dieser ist sonst
nur in BI in diesem Umfang zu finden.

Auffälligkeiten:

* Gleich an den Anfang müsste, dass derzeit EC nur auf einer
  CMK Instanz pro Host möglich ist!
* Wie steht es um die IPv6-Fähigkeit der EC?
* Insgesamt sehr lang, evtl. in 2 oder 3 Artikel splitten?
  Hier böte sich an, SNMP Traps und Logwatch auszulagern.
* Shellscripte sollten unter Sicherheitsgesichtspunkten überarbeitet werden.
  ACHTUNG: In der Inline-Hilfe stehen noch die gefährlicheren Macros!
* Während der Laufzeit des Skripts ist die Verarbeitung weiterer Events
  angehalten!
* Erklärung der Regelpakete vorziehen?
* Stellenweise Sprünge im Artikel, bspw. werden Aktionen beim Löschen von
  Events nicht zusammen mit Aktionen beim Entstehen erfasst.
* Best Practices fehlen IMHO: Wann schicke ich komplettes Syslog zur EC?
  Wann lasse ich den rsyslogd vorfiltern und schicke nur Teile?
  Wann ist Logwatch performant genug?
  Wann sollte man auf Logwatch verzichten?
////
...
// MFS: Evtl. hioer besser "Hauptaufgabe von CMK ist das aktive Überwachen von Zuständen…"?
...
von {CMK} sogar im [.guihint]#Overview# des Sidebars mit angezeigt.
...
* Sie kann automatisiert Aktionen ausführen und über {CMK} link:notifications.html[Benachrichtigungen] versenden.
...
// MFS: Das stimmt so seit Linux 3.9 nicht mehr.
...
image::ec_omd_config.png[width=330]
...
herauszufischen und zu informieren. Nun ist es leider so, dass die meisten
...
Die Konfiguration der EC finden Sie wenig überraschend in der Setup-GUI
unter [.guihint]#Events > Event Console#. Dieses ist ab Werk leer -- es
...
// MFS: Das mag technisch noch stimmen, aus Nutzersicht wurde beide Formen der Aktivierung von Änderungen aber zusammengelegt.
...
Klicken Sie anschließend rechts oben auf die [.guihint]#Pending changes# und dann auf
[.guihint]#Activate on selected sites# um die Änderung
...
// MFS: Weiter oben bereits hingewiesen.
...
in der obersten Ebene der Setup-GUI der EC, so werden die Regelpakete
...
[.guihint]#Settings# in der Hauptebene des EC-Moduls:
...
icon:icon_help[] Inlinehilfe und an den jeweils passenden Stellen in
diesem Artikel.
...
// MFS: Vorgezogen und erweitert
[#hostnames]
=== Hostzuordnung in der Event-Konsole

Ein Unterschied der EC zu den {CMK}-Gepflogenheiten ist, dass im Gegensatz zum
regulären Monitoring nicht Hosts im Zentrum stehen: Events können ohne
explizite Hostzuordnung auftreten, was oft sogar gewünscht ist. Allerdings
...
xref:#eventsgostate[Zustände werden] sollen, ist korrekte Zuordnung zwingend.

Grundsätzlich gilt bei per Syslog empfangenen Nachrichten, dass der in der
Nachricht verwendete Hostname dem Hostnamen im Monitoring entsprechen sollte,
dies erreichen Sie durch die Verwendung des FQDN/FQHN sowohl in Ihrer
Syslog-Konfiguration als auch in der Benennung der Hosts in {CMK}. Neben dem
Hostnamen wird mit dem Hostalias verglichen. Steht hier der per Syslog
...
zwischengeschaltete Log-Server genutzt werden. Ist die Umstellung der Clients
auf FQDN/FQHN oder das Nachtragen vieler Aliase zu aufwendig, können Sie mit
der EC-Einstellung [.guihint]#Hostname translation for incoming messages#
...
Am flexibelsten ist die Arbeit mit link:regexes.html[regulären Ausdrücken],
welche intelligentes Suchen und Ersetzen in den Hostnamen erlauben.
Insbesondere, wenn zwar Hostnamen eineindeutig sind, aber nur der in {CMK}
mitverwendete Domainpart fehlt, hilft eine simple Regel: `(.*)` wird zu
`\1.mydomain.test`. In Fällen, wo das alles nicht genügt, können Sie noch
...
direkt in der [.guihint]#Overview# des Sidebars angezeigt:
...
image::ec_commands_menu.png[width=500]

image::ec_commands.png[width=500]
...
verfügbar. Diese sendet eine {CMK}-Benachrichtigung, die genauso behandelt wird wie eine
Nachricht von einem aktiv überwachten Service. Diese durchläuft die link:notifications.html[Benachrichtigungsregeln]
und führt dann entsprechend zu E-Mails, SMS oder was auch immer Sie konfiguriert haben.
Einzelheiten zur Benachrichtigung durch die EC erfahren Sie link:ec.html#notifications[weiter unten].
...
In anderen Fällen hat der Hostname eine andere Schreibweise als in der Setup-GUI.
...
Im vorherigen Abschnitt haben Sie schon gesehen, wie Sie mit dem Simulator die Auswertungen
...
image::ec_match_groups_2.png[width=650]
...
Die Kontaktgruppen für die link:ec.html#visibility[Sichtbarkeit] werden
auch bei der
link:ec.html#notifications[Benachrichtigung] von Events verwendet. Sie können hier per
...
Aktionen sind den link:alert_handlers.html[Alert Handlern] für Hosts und Services sehr ähnlich.
...
Folgende Abbildung zeigt eine Regel, in der nach Meldungen mit dem Text `DB
...
eingefangen wird. Der Ausdruck `DB Instance (pass:[.*]) is recovering`, welcher
...
wenig sinnvoll, wenn die Meldung `DB Instance TEST recovered` ein
Event aufheben würde, das von der Meldung `DB Instance PROD failed`
stammt, oder?
...
image::ec_phase_filter_counting.png[width=400]
...
* Erzeugen von {CMK}-link:ec.html#notifications[Benachrichtigungen]


=== Shellskripte und E-Mails

E-Mails und Skripte müssen Sie zunächst in den Einstellungen der Event
Console definieren. Sie finden diese unter dem Eintrag [.guihint]#Actions (E-Mails & Scripts)#:
...
können. Im Skript können Sie Umgebungsvariablen verwe
Dem Skript stehen über Umgebungsvariablen Details zu den Events zur Verfügung,
beispielsweise die `$CMK_ID` des Events, der `$CMK_HOST`, Volltext `$CMK_TEXT` oder die erste Matchgruppe als `$CMK_MATCH_GROUP_1`. 
Eine vollständige Liste der verfügbaren Umgebungsvariablen erhalten Sie in der
icon:icon_help[] Inlinehilfe.
...
Skripten bei den link:notifications.html#methods[Benachrichtigungsmethoden]). Da diese
...
Ältere Versionen von {CMK} haben neben Umgebungsvariablen auch Macros wie
...
sollten Sie von Macros keinen Gebrauch machen. Macros sind derzeit noch aus
...
kennt die EC noch eine dritte Art von Aktion: Das Versenden von Nachrichten über das
{CMK}-link:notifications.html[Benachrichtigungssystem]. Die dabei von der EC erzeugten
...
* Funktionen wie link:notifications.html#bulk[Sammelbenachrichtigungen], HTML-E-Mails und andere nützliche Dinge stehen zur Verfügung.
* Benutzerdefinierte Benachrichtigungsregeln, ein Abschalten der Nachrichten und Ähnliches funktionieren wie gewohnt.
...
* Der Ordner im Setup-GUI
* Die Liste der Kontakte und Kontaktgruppen

Dadurch kann es dazu kommen, dass der Hostname in der Benachrichtigung nicht exakt mit
...
einheitlichen Benachrichtigungsregeln, welche Bedingungen auf den Hostnamen enthalten.
...
Falls die Abfrage nicht klappt oder Host nicht gefunden werden kann, werden Ersatzdaten
angenommen:
...
|[.guihint]#IP-Adresse# |Das Feld IP-Adresse enthält die original Absenderadresse der Meldung.
|[.guihint]#Hostmerkmale# |Der Host bekommt kein Tag. Falls Sie Taggruppen mit Leertags haben, nimmt der Host dort diese Merkmale an. Ansonsten hat er kein Tag der Gruppe. Bitte beachten Sie das, wenn Sie in den Benachrichtigungsregeln Bedingungen über Tags definieren.
...
die typischen Datenfelder einer Monitoringsnachricht so sinnvoll wie möglich
...
|[.guihint]#Vorheriger Zustand# |Da Events keinen früheren Status haben, wird hier bei normalen Events immer {OK}, bei Aufhebungsevents (Cancelling) immer {CRIT} eingetragen. Diese Regelung kommt dem am nächsten, was man für Benachrichtigungsregeln braucht, die eine Bedingung auf den genauen Zustandswechsel haben!
|===
...
Im Element [.guihint]#Master Control# gibt es einen zentralen Schalter für Benachrichtigungen.
...
Benachrichtigungen. Im Logfile sieht das so aus:
...
Falls Sie ein eigenes link:notifications.html#scripts[Benachrichtigungsskript] schreiben, haben Sie
...
image::ec_config_traps.png[width=330]
...
=== Servicelevel und Syslog-Priority

Da weitergeleitete Logfiles je nach verwendetem Format oft die
Syslog-Klassifizierung fehlt, können Sie die Neu-Klassifizierung in den Regeln
des [.guihint]#Log Forwarding# vornehmen. Zudem ist in den Regelsätzen, die Sie
als Teil von [.guihint]#Rule packs# definieren, immer das individuelle Setzen
von Zustand und Service-Level möglich.
...
|E-Mail |Eine E-Mail wurde versendet.
...
// MFS: Speicher wird nicht verbraucht, sondern gebraucht!
...
link:notifications.html[Benachrichtigungsregeln] von {CMK}. Diese Regeln müssen sich nicht
...
|pro Host |Die Kontakte zum Host, welche genauso ermittelt werden, wie bei der link:ec.html#notifications[Benachrichtigung von Events] über {CMK}.
...
Syslogarchiv einzusetzen, kann dies allerdings zu sehr großen Dateien führen.
...
Teilmenge von link:livestatus.html[Livestatus]. Auf diese Schnittstelle greift
der Monitoringkern zu und reicht die Daten an die GUI durch, um so ein
link:distributed_monitoring.html[verteiltes Monitoring] auch für die Event Console zu
ermöglichen.

Für den vereinfachten Live-Status der Event Console gelten folgende Einschränkungen:
...
|`RESETCOUNTERS` |Setzt die Trefferzähler der Regeln zurück (entspricht dem Eintrag [.guihint]#Event Console > Reset counters# in der Setup-GUI).
...
|`ACTION` |Führt eine benutzerdefinierte Aktion auf einem Event aus. Argumente sind Event-ID, Benutzerkürzel und Aktions-ID. Die spezielle ID `@NOTIFY` steht für eine link:ec.html#notifications[Benachrichtigung] über {CMK}.
|===
...
|`etc/check_mk/mkeventd.d/wato/global.mk` |Hier Speichert das Setup-GUI die globalen Einstellungen der Event Console in Python-Syntax.
...
|`tmp/run/mkeventd/status` |Ein Unix-Socket, das die Abfrage des aktuellen Status und das Senden von Kommandos erlaubt. Die Anfragen der GUI gehen zunächst zum Monitoringkern, der dann auf diesen Socket zugreift.
