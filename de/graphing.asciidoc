...
In den {CEE} können Sie mit dem icon:icon_menu[] Menüsymbol den Graphen an
...
Sie gelangen zu Ihren Graphsammlungen über [.guihint]#Monitor > Workplace > My graphs.# Der Eintrag [.guihint]#My Graphs# taucht erst auf, wenn Sie auch
...
Diese Änderung ließe sich dann auch auf bestimmte Vorkommen des Graphs beschränken, beispielsweise in Dashboards; hier im Bild sehen Sie eine Skalierung auf den Bereich _0.5 bis 0.9_ in einem PDF-Bericht:
...
zu kombinieren. Zu den benutzerdefinierten Graphen gelangen Sie z.B. über [.guihint]#Customize > Graphs > Custom graphs.#
...
Metrik zu einem benutzerdefinierten Graphen hinzufügen können:
...
In der Spalte [.guihint]#Actions# finden Sie zu jeder angelegten Metrik einen Knopf zum icon:button_clone[] klonen derselben. So können Sie fix Kurven kopieren und schlicht etwa den Hostnamen austauschen.
...
Standardmäßig werden über die globale Einstellung alle Daten gesendet.
Sie können diese über die Regel [.guihint]#Send metrics to Graphite/InfluxDB,# zu finden über das Menü [.guihint]#Setup,# weiter
einschränken:
