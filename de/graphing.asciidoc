...
Messdaten basiert auf HTML5 und ist in den kommerziellen Editionen und in der {RE} identisch.
Ausschließlich in den kommerziellen Editionen können Sie erweiterte Funktionen wie
...
{cee-only}
In den kommerziellen Editionen können Sie mit dem icon:icon_menu[] Menü, das links unten im Graphen angezeigt wird, diesen an
...
{cee-only}
In den kommerziellen Editionen können Sie kleine Anpassungen an den eingebauten Graphen vornehmen und zwar über [.guihint]#Customize > Graphs > Graph tunings.# Diese [.guihint]#Graph tunings# ermöglichen Ihnen zum Beispiel, die Skalierung der Y-Achse für einen bestimmten Graphen zu verändern, über die Option [.guihint]#Vertical axis scaling:#
