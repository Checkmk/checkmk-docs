...
// IGNORE Graphensammlungen + 1m 88% @ Tunings Seitenabrufe Tobi Oetiker qualität tunings NPCD Skalare Zeiträume
...
image::example_graph.png[alt="Beispiel für eine Verlaufsgrafik."]

Neben dem eigentlichen System-Monitoring -- nämlich der Erkennung von Problemen
...
können. Um den Bedarf an Plattenplatz in Grenzen zu halten, werden die
...
PDF-Export, Graphensammlungen, benutzerdefinierte Graphen
...
wenn Sie mit der Maus über das icon:icon_pnp[] Graphsymbol fahren oder
draufklicken. Der @-Wert rechts oberhalb der Grafik zeigt dabei an, in welchem Intervall neue Daten geholt und ergänzt werden. _@ 1m_ steht zum Beispiel für ein Abfrageintervall von einer Minute.

Die gleichen Graphen finden Sie zudem auch ganz einfach in
...
image::graphing_metrics_table.png[alt=Ausschnitt der Servicemetriken."]
...
image::graphing_pin.png[alt="Visualisierung möglicher Interaktionen in der Verlaufsgrafik."]
...
image::graphing_pause.png[alt="Ansicht des Pause-Symbols in einer Verlaufsgrafik."]
...
image::graphing_monitor_menu.png[alt="Auswahl der Graphensammlung in der Ansicht des Hauptmenüs."]
...
image::graphing_tunings.png[alt="Ansicht der Einstellungen für das Anpassen eines Graphen."]
...
Darüber hinaus stehen Ihnen drei weitere Optionen zur Verfügung: Über [.guihint]#Graph visibility# lässt sich ein Graph an bestimmten Orten explizit ein- oder ausblenden. Und [.guihint]#Vertical axis mirroring# ist nützlich bei Graphen, die Daten ober- und unterhalb der Zeitleiste (X-Achse) zeigen, wie etwa der Graph xref:disk_throughput[Disk throughput] weiter unten: Dort lassen sich die Daten spiegeln, so dass also die vormals oberen Daten unterhalb der Zeitleiste angezeigt werden und umgekehrt.

Besonders wenn Sie in einem internationalen Unternehmen arbeiten, kann die dritte Einstellung hilfreich sein: die Anzeige von Temperaturwerten in Grad Fahrenheit oder Grad Kelvin anstelle von Grad Celsius. Hierfür nutzen Sie die Option [.guihint]#Convert unit to# in den [.guihint]#Graph tunings:#

image::graphing_tunings_temperature.png[alt="Auswahl einer Temperaturmaßeinheit."]

*Hinweis:* Wollen Sie nicht nur die Graphen auf eine andere Temperaturmaßeinheit umstellen, sondern alle Ansichten, 
so verwenden Sie den xref:wato_rules#checkparameters[Service-Regelsatz] [.guihint]#Temperature.#
...
image::graphing_new_custom.png[alt="Ansicht der Auswahloption für benutzerdefinierte Graphen."]
...
[#disk_throughput]
image::graphing_input_output.png[alt="Ansicht eines Input/Output-Graphen."]
...
image::graphdesigner_ops_2.png[alt="Ansicht eines Verlaufsgraphen für kombinierte Werte."]
...
{cee-only}
Wenn Sie eine der {CEE} einsetzen, so können Sie parallel zum in {CMK} eingebauten Graphing auch externe Metrikdatenbanken anbinden und die xref:metrics_exporter#[Metriken an InfluxDB oder Graphite senden.]
...
sogenanntes Round-Robin-Archiv (RRA). Im Standardaufbau gibt es also insgesamt
...
einfach `RRD` in das Suchfeld ein. So finden Sie die Regel
...
image::graphing_rrd_configuration.png[alt="Ansicht der Regeleinstellungen für eine RRD."]
