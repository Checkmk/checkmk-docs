...
* Anwendungsspezifische Daten (z.&ThinSpace;B. Länge von E-Mail-Warteschlangen von MS Exchange)
...
* Manchmal ist es sinnvoll, Konstanten hinzuzufügen, um z.&ThinSpace;B. den Wert einer Kurve von der Zahl 100 abzuziehen.
...
zeigt die Meldungen im Fall, dass eine Verbindung zum Graphite-Server nicht klappt:
...
sondern gehen verloren (bzw. sind dann nur in den RRDs von {CMK}
verfügbar).

Wenn die Verbindung über die globalen Einstellungen konfiguriert ist, werden standardmäßig *alle* Daten gesendet.
