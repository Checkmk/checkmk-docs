...
[.relateddoc]
user_interface, views, dashboards
...
== Graphensammlungen (Graph Collections)#graph_collections
...
== Freiformgraphen (Custom Graphs)#custom_graphs
...
[.guihint]#Views|EDIT# und dann mit dem Knopf ICON[button_custom_graphs.png].
...
Die weiteren drei Möglichkeiten [.guihint]#Mirrored Line}}, {{Mirrored Area}} und {{Mirrored Stacked# funktionieren analog, nur dass
...
=== Formular zum Hinzufügen einer Metrik#adding_metrics
...
*Konsolidierungsfunktion*.  Zur Auswahl stehen [.guihint]#Minimum}}, {{Maximum#
...
=== Rechnen mit Formeln#calculation
...
zwei Kurven: CPU utilization [.guihint]#User}} und {{System#.
...
Durch einen Klick auf ICON[button_dissolve_operation.png] machen Sie die
...
<a href="http://www.pnp4nagios.org">PNP4Nagios von Jörg Linge</a> die Grundlage
...
* Im Kasten [.guihint]#Timeranges}} können Sie einen von fünf Standardzeiträumen wählen (z.B. {{One Month#).


=== Das Basket

In Ihrem [.guihint]#Basket# können Sie mit dem Icon ICON[pnp_add.png] mehrere Graphen
...
== Graphite, Grafana und InfluxDB#graphing_api
...
Dies ist seit Version VERSION[1.6.0] von {CMK} möglich. Sie können in einer Tabelle
...
In folgendem Beispiel habe ich eine Kopie der Ansicht [.guihint]#All hosts# gemacht
und die Spalten für die Anzahl der Services in den verschiedenen Zuständen
entfernt. Damit die Ansich [.guihint]#All hosts# nicht durch die neue Ansicht ersetzt
...
== Hintergründe, Tuning, Fehlerdiagnose#rrds
...
dieses über link:wato_rules.html[Konfigurationsregeln] ändern (sogar pro Host oder Service
unterschiedlich). Den nötigen Regelsatz finden Sie
am einfachsten über die Regelsuche -- also [.guihint]#WATO|Host & Service Parameters|Search for rules sets#.
Und dort geben Sie einfach [.guihint]#RRD# ein. So finden Sie die Regel
...
In den Abschnitten [.guihint]#Consolidation Functions}} und {{RRA Configuration#
...
=== RRD-Speicherformat#rrdformat

[CEE] Die oben gezeigte Regel hat noch eine weitere Einstellung: [.guihint]#RRD storage format#.
Mit dieser können Sie zwischen zwei Methoden
wählen, wie {CMK} die RRDs erzeugt. Diese Einstellung existiert ab Version
VERSION[1.2.8]. Hier wurde das neue Format [.guihint]#One RRD per host/service#
...
=== Der RRD-Cache-Daemon (rrdcached)#rrdcached
