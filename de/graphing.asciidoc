...
* Qualitätskennzahlen von Anwendungen (z.&ThinSpace;B. Antwortzeiten von Webseiten)
* Stromverbrauch und -qualität im Rechenzentrum (Ströme, Spannungen, Leistungen, Batteriegüte,{nbsp}...)
* Anwendungsspezifische Daten (z.&ThinSpace;B. Länge von Mailqueues von MS Exchange)
...
werden von den einzelnen Check-Plugins ermittelt. Die Plugins legen somit auch fest,
welche Metriken genau bereitgestellt werden.

Die Oberfläche für die Visualisierung der historischen
Messdaten basiert auf HTML5 und ist in den {EE} und in der {RE} identisch.
Ausschließlich in den {EE} können Sie erweiterte Funktionen wie
PDF-Export, Graphensammlung, benutzerdefinierte Graphen
...
z.&ThinSpace;B. der prozentual belegte Platz:
...
In den Details gibt es zudem eine *Tabelle mit den aktuellen präzisen Messwerten*
für alle Metriken:
...
In den {CEE} können Sie mit dem icon:icon_menu[] Menü, das links unten im Graphen angezeigt wird, diesen an
verschiedenen Stellen einbetten, z.&ThinSpace;B. in Berichte oder Dashboards. Sehr
...
In den {CEE} können Sie kleine Anpassungen an den eingebauten Graphen vornehmen und zwar über [.guihint]#Customize > Graphs > Graph tunings.# Diese [.guihint]#Graph tunings# ermöglichen Ihnen zum Beispiel, die Skalierung der Y-Achse für einen bestimmten Graphen zu verändern, über die Option [.guihint]#Vertical axis scaling:#
...
Zu den benutzerdefinierten Graphen gelangen Sie z.&ThinSpace;B. über [.guihint]#Customize > Graphs > Custom graphs.# 
...
In der Spalte [.guihint]#Actions# finden Sie zu jeder angelegten Metrik einen Knopf zum icon:button_clone[] klonen derselben. So können Sie fix Kurven kopieren und schlicht etwa den Host-Namen austauschen.
...
verdichtet ist. In einem Bereich, wo z.&ThinSpace;B. nur noch ein Wert pro halber
...
nötig zur Bildung von Berechnungsformeln. Dazu xref:calculation[später] mehr.


[#graph_options]
=== Graphoptionen

Hier finden Sie Optionen, die den Graphen als Ganzes betreffen. Die Einheit ([.guihint]#Unit#) beeinflusst die Beschriftung der Achsen und der Legende. Sie wird
...
Graphen für z.&ThinSpace;B. einen Prozentwert entwerfen, könnten Sie sich aber auch
...
* Manchmal ist es sinnvoll, Konstanten hinzuzufügen, um z.&ThinSpace;B. den Wert einer Kurve von der Zahl 100 abzuzuiehen.
...
selbst hat z.&ThinSpace;B. die link:https://influxdata.com/[InfluxDB] eine
derartige Schnittstelle.

Die Anbindung konfigurieren Sie in [.guihint]#Setup > General > Global Settings# unter
...
Präfix konfigurieren, der jedem Hostnamen vorangestellt wird, um z.&ThinSpace;B. eindeutige
...
zeigt die Meldungen im Fall, dass ein eine Verbindung zum Graphite-Server nicht klappt:
...
Wenn die Verbindung über die globale Einstellungen konfiguriert ist, werden standardmäßig *alle* Daten gesendet.
Sie können dies aber einschränken mit der Regel [.guihint]#Send metrics to Graphite/InfluxDB# (zu finden über die Suchfunktion im [.guihint]#Setup#-Menü):
...
Zusammenfassung wie: _Der durchschnittliche CPU-Verbrauch der letzten 7 Tage_.
Das Ergebnis wäre dann einfach eine Zahl als Prozentwert wie _88%_.
...
nach diesen Spalten sortieren und so z.&ThinSpace;B. die Liste derjenigen xref:monitoring_vmware#[ESXi-Hosts]
...
Zunächst benötigen Sie eine Ansicht, zu welcher die Spalten hinzufügen wollen.
...
* Sie können natürlich auch mehrere Spalten auf diese Art hinzufügen, z.&ThinSpace;B. für unterschiedliche Metriken oder unterschiedliche Zeiträume.
...
z.&ThinSpace;B. den Temperaturverlauf in einem Rechenzentrum über vier Jahre beobachten,
...
sogenanntes Round Robin Archive (RRA). Im Standardaufbau gibt es also insgesamt
...
Ein interessantes alternatives Schema wäre z.&ThinSpace;B. das Speichern von einem Wert
...
können Sie auf die Konsolidierung verzichten und z.&ThinSpace;B. nur noch _Average_
...
dieses über xref:wato_rules#[Regelsätze] ändern (sogar für Hosts oder
...
Auch hier benötigen Sie anschließend
den Befehl `cmk --convert-rrds` -- und auch hier gilt: Stoppen Sie stets Ihre Instanz (mit `omd stop`) bevor Sie mit `cmk --convert-rrds` existierende RRDs konvertieren.  
...
Alle neuen Messwerte für die RRDs werden zunächst vom {CMK} Micro Core ({EE}) bzw. von NPCD ({RE})
...
mit Messdaten und RRDs zu tun haben (alle bezogen auf das Homeverzeichnis der Instanz):
