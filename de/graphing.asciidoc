...
Besonders wenn Sie in einem internationalen Unternehmen arbeiten, kann eine weitere Einstellung hilfreich sein: die Anzeige von Temperaturwerten in Grad Fahrenheit oder Grad Kelvin anstelle von Grad Celsius. Hierfür nutzen Sie die Option [.guihint]#Convert unit to# in den [.guihint]#Graph tunings#:

image::graphing_tunings_temperature.png[alt="Auswahl einer Temperaturmaßeinheit."]

*Hinweis:* Wollen Sie nicht nur die Graphen auf eine andere Temperaturmaßeinheit umstellen, sondern dazu passend auch die anderen Anzeigen der Services-Übersicht, so verwenden Sie den xref:wato_rules#checkparameters[Service-Regelsatz] _Temperature._
