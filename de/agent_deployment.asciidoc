...
// IGNORE Check_MK [ ] TMPDIR check_mk
...
{cee-only}
Die {CEE} können ihre Agenten auf Linux, Windows und Solaris automatisch aktualisieren.
...
//TK: Ein kurzer Überblick über das Prinzip wäre schon angebracht: Realisierung als Agentenplugin, Konfiguration am Server inkl. Backen und Signieren, Download des Agenten, Transport zum Host, Installation des Agenten, Registrierung...
...
//TK: Warum wird hier der letzte Schritt der Voraussetzungen (Master switch) zuerst ausgeführt?
...
Beachten Sie, dass der Regelsatz hier das Prinzip xref:wato_rules#matching["Erste Regel pro Parameter gewinnt"] verwendet.
...
Bei einem verteilten Monitoring mit zentraler Konfiguration erhält der Agent Updater standardmäßig seinen zuständigen Update-Server von der {CMK}-Instanz, zu der er sich verbindet.
...
Hier konfigurierte CA- oder selbst signierte Zertifikate stehen dem Agent Updater
...
* Windows: Für Windows wird das Plugin immer als 32-Bit-Executable ausgeliefert. Diese Regel wird dementsprechend standardmäßig ignoriert. *Hinweis:* Etwaig vorhandene 64-Bit-Binaries stammen aus älteren Versionen von {CMK} und entsprechen nicht mehr dem aktuellen Standard.
* Solaris: Hier müssen Sie ebenfalls nichts anpassen. {CMK} wird hier das Python-Skript verwenden, auch wenn Sie den Standardwert auf dem 64-Bit-Binary lassen.
* Andere Architekturen: Wenn Sie Pakete für andere Architekturen, wie beispielsweise arm oder ppc, verwenden möchten, setzen Sie diese Option manuell auf [.guihint]#Python#. Dies ist notwendig, da diese Architekturen von {CMK} nicht automatisch abgefangen werden und keine Binaries dafür angeboten werden.
...
Nach dieser letzten Anpassung könnte Ihre Regel wie im folgenden Screenshot
...
Bevor Sie einen neuen Agenten auf eine größere Zahl von Host ausrollen, möchten Sie ihn sicherlich zuerst mit einer kleineren Anzahl von Hosts ausprobieren.
...
Nachdem Sie hier Bedingungen für die Auswahl von Hosts getroffen haben, können Sie in dem Feld [.guihint]#Test hostname before activation# einzelne Host-Namen eintippen und kontrollieren, ob die Updates für diese Hosts nun aktiviert sind oder nicht.
Die Bedingungen werden dabei immer mit _UND_ verknüpft.
...
Eine detailliertere Log-Datei `cmk-update-agent.log` inklusive Debug-Ausgaben und eventuellen Tracebacks findet sich unter Linux und unter Windows:
...
melden, ist dies über einen fest eingestellten Update-Server im
...
die im Setup zur Verfügung stehenden CA- oder selbst signierte Zertifikate
...
// MFS: Inhaltliche Überarbeitung Spetember/Oktober 2022 hier:
...
Daher wird der Certificate Store des Betriebssystems ignoriert, sobald ein oder mehrere Zertifikate über eine der folgenden Mechanismen importiert wurden.
...
können Zertifikate mit in das Agentenpaket verbacken werden und stehen dem Agent Updater ab Installation (oder Update) zur Verfügung.
...
Die Remote-Instanz kann sich daher aus dem {CMK} eigenen Certificate Store unter
...
[#certificate_change]
==== Vorgehen beim Austausch eines Zertifikates

Wenn Sie mit eigener Zertifikatsinfrastruktur arbeiten, verwenden Sie im Idealfall ein sehr lange gültiges Root-Zertifikat, mit dessen zugehörigem Schlüssel regelmäßig Intermediate Zertifikate signiert werden, welche dann wiederum zum Signieren der Serverzertifikate verwendet werden.
In diesem Fall rollen Sie Intermediate-Zertifikate als Zertifikatskette _(Certificate Chain)_ auf dem {CMK}-Server aus.
Den Hosts im Monitoring muss nun lediglich das Root-Zertifikat bekannt gemacht werden.

Sind Sie unsicher, ob ein neues Serverzertifikat ein neues Root-Zertifikat erfordert, verwenden Sie den folgenden Befehl, um den Identifier des Root Keys, mit dem ein Zertifikat signiert wurde, zu ermitteln:
...
Wurde bislang ein selbst signiertes Zertifikat, ein kurzlebiges Root-Zertifikat einer internen CA verwendet oder der Root-Schlüssel Ihrer internen CA kompromittiert, ist wie folgt vorzugehen:

. Fügen Sie das neue Zertifikat über die Regel [.guihint]#Certificates for HTTPS verification# hinzu. Verwenden Sie im Zweifel die komplette Zertifikatskette.
. Backen Sie die Agentenpakete neu und führen Sie ein Update der Hosts im Monitoring durch. Stellen Sie sicher, dass dieses Update für alle Hosts durchgelaufen ist, bevor Sie fortfahren.
. Tauschen Sie nun das Serverzertifikat aus.
. Testen Sie mit wenigen Hosts, ob das Update über das neue Zertifikat möglich ist.
...
verloren gehen. Eine neue Installation findet diese Datei ebenfalls wieder und
...
// Folgende Aufstellung zeigt Ihnen, welche Dateien und Verzeichnisse auf dem {CMK}-Server und den überwachten Hosts mit den automatischen Agenten-Updates zu tun haben.
