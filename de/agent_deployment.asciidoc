...
Sie für jeden diesen Punkte über [.guihint]#Help => Toggle inline help# weitere
...
in den folgenden fünf Abschnitten beschriebenen Einstellungen vorgenommen und
...
[.guihint]#Agent updater (Linux, Windows)# um dieses Plugin. Bitte beachten Sie, dass
...
hier die Onlinehilfe ([.guihint]#Help => Toggle inline help#) weitere Informationen zu
...
Updater standardmäßig seinen zuständigen Updateserver von der {CMK}-Site, zu der
...
Hier geben Sie den DNS-Namen an, unter dem {CMK}-Server erreichbar ist.
Wichtig ist hier, dass der zu überwachende Host diesen Namen auflösen kann und
er im {CMK} als Host konfiguriert ist. Achten Sie bei dem Einsatz von HTTPS
...
diesem Fall tragen sie hier einmalig eine andere Site ein. Siehe hier auch
weiter unten unter link:agent_deployment.html#scenarios[Einsatzszenarien]. Bei einem verteilten Setup mit zentraler
Konfiguration kann dieSite, an der Sie sich registrieren wollen, ebenfalls von
der zentralen Site, auf der diese Regel konfiguriert wird, abweichen.
=======
==== Name der {CMK}-Site
...
auch Zertifikate von der {CMK}-Site zur Verfügung gestellt werden, oder diese
...
signiert, kann die Verbindung im Normalfall ohne importiere Zertifikate
...
Seit der Version VERSION[1.5.0] können Sie über die Option [.guihint]#Executable format (Linux)#
...
icon:context_button_save_1px_border[].
...
Als nächstes können Sie bereits die so konfigurierten Agenten backen. Navigieren
Sie dazu wieder nach [.guihint]#Setup => Agents => Windows, Linux, Solaris AIX# und klicken
Sie dann auf [.guihint]#Bake agents#. Die erstellten und angepassten Regeln finden sich
nämlich erst dann in den Installationspaketen wieder, wenn Sie diese neu
erstellen/backen. Wenn dieser Prozess abgeschlossen ist, werden Sie darüber
informiert:
...
Als nächstes signieren Sie die Agenten mit dem in
link:agent_deployment.html#create_signature_key[Schritt 1.1] erstellten Schlüssel.
Klicken Sie dafür auf [.guihint]#Sign agents#, wählen Sie den gewünschten Schlüssel aus,
geben Sie die zum Schlüssel passende Passphrase ein und klicken Sie auf
[.guihint]#Sign#. Alle so signierten Agenten werden dem einem einem entsprechenden
Symbol icon:icon_signature_key[] gekennzeichnet. Wenn Sie mehrere Schlüssel
angelegt haben, signieren Sie mit jedem Schlüssel separat. *Wichtig:* Einem
...
`msiexec` (bzw. per Doppelklick). Das Agent-Updater-Plugin finden Sie nun
im Pluginsverzeichnis des Hosts:
...
* Unter Windows: Die Agent-Updater-Executable unter `C:\ProgramData\Checkmk\Agent\plugins\`.
...
<<<<<<< HEAD
cmk-update-agent register -s mymonitoring.example.intern -i heute -H myserver -p http -U cmkadmin -P *** -v
----

*Hinweis:* Seit {CMK} VERSION[2.0] wird der Agent-Updater für Windows-Hosts
...
link:ttps://www.youtube.com/watch?v=S7TNo2YcGpM&#t=12m47[Die neuen automatischen Agent Updates.]
...
Die Onlinehilfe ([.guihint]#Help => Toggle inline help#) gibt weitere Erklärungen. Ein
...
<<<<<<< HEAD
[#check_mk_agent]
=== Der Check Check_MK Agent bei jedem betroffenen Host
=======
...
Seit {CMK} VERSION[2.0] ist es möglich, gebackene Agenten auch
...
Dabei wird einem Host der Server zugeteilt, von dessen Site aus er gemonitort
...
die im Setup zur Verfügung stehenden CA- oder self-signed Zertifikate
...
Um eine {CMK}-Site per HTTPS anprechen zu können, muss zunächst einmal der
...
link:ttps://www.youtube.com/watch?v=ZF2MpQtdLg[SSL und Zertifikate]
