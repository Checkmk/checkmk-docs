...
Folgende Aufstellung zeigt Ihnen, welche Dateien und Verzeichnisse auf dem {CMK}-Server mit den automatischen Agenten-Updates zu tun haben. Wie immer sind alle Angaben hier relativ zum Instanzverzeichnis (z.B. /omd/sites/mysite).

==== Auf dem Host (Linux/Unix)
[cols="45,~",options="header"]
|===
|Pfad |Bedeutung 

|/etc/cmk-update-agent.state |Statusdaten (solche Daten, die man vielleicht eher unter /var vermuten würde), wird nicht vom Packaging Tool erfasst, muss manuell gelöscht werden.
|/etc/check_mk/cmk-update-agent.cfg |Die Konfiguration des Agent Updater Plugins, wie sie aus dem bakelet kommt. Wird nur bei der Installation oder beim Update geschrieben.
|/usr/bin/cmk-update-agent |Starter-Script, setzt Umgebungsvariablen, die das Plugin erkennt und daran festmacht, dass es manuell läuft.
|/usr/lib/check_mk_agent/plugins/XXXX/cmk-update-agent |Das eigentliche Plugin. Je nach Regeleinstellung Binary Executable oder Script.
|/var/lib/check_mk_agent/cache/plugins_XXXX\cmk-update-agent.cache |Cache-File, das die aktuell gültige Agent Sektion vom Agent Updater enthält. Gilt, bis die XXXX Sekunden abgelaufen sind.
|/var/lib/check_mk_agent/cmk-update-agent.log |Ausführliches Logfile (DEBUG-Level)
|/etc/prelink.conf bzw. /etc/prelink.conf.d/cmk-update-agent |Der prelink-Blacklist-Eintrag.
|/tmp/_MEIXXXXX |Temporäre Dateien vom PyInstaller, während der Binary-Agent-Updater läuft.
|/tmp/check-mk-agent-XXXX |Temporär abgelegter Agent, der während des Updates installiert werden soll.
|===

==== Auf dem Host (Windows)
[cols="45,~",options="header"]
|===
|Pfad |Bedeutung 
...
|die temporären Dateien (PyInstaller Folder und heruntergeladenes MSI)|gibt es auch hier, Speicherort hängt von Windows ab
|Update Cycle |ist in der check_mk.ini definiert, caching findet komplett im Memory statt
|===
...
|~/var/check_mk/agents |Die gebackenen Agents. Eingeordnet nach System und zutreffenden Hosts (letzteres per Soft-Link). Unter "system"/_PACKAGES liegen die gebackenen Agenten und sind nach ihrem Hash benannt, nebst Signatur und config.
|===
