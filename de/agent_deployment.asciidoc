...
Sie für jeden dieser Punkte über [.guihint]#Help > Show inline help# weitere
...
[.guihint]#Agent updater (Linux, Windows, Solaris)# im Abschnitt [.guihint]#Agent rules > Automatic Updates# um dieses Plugin. Bitte beachten Sie, dass
...
Agent Updaters zum {CMK}-Server angeben. Werden die zugehörigen Einträge nicht
...
===== DNS-Name oder IP-Adresse des Updateservers (DNS name or IP address of update server)
...
darauf, dass der Name des Zertifikats mit dem Namen des {CMK}-Servers
übereinstimmt, der dem Host bekannt ist.


[#cmk_site_update_server]
===== Name der {CMK}-Instanz des Updateservers (Name of {CMK} site of update server)
...
==== Intervall für den Updatecheck (Interval for update check)
...
[.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX# das für den Host passende Paket herunter. Achten Sie darauf, dass das Paket auch das
Agent-Updater-Plugin enthält.

Kopieren Sie nun das Paket auf den Host und xref:wato_monitoringagents#bakery_download[installieren Sie es] mit `rpm`, `dpkg` oder
`msiexec`. Den Agent-Updater bzw. das Plugin
...
können Sie in dem Feld [.guihint]#Test hostname before activation# einzelne Hostnamen
...
Folgende Aufstellung zeigt Ihnen, welche Dateien und Verzeichnisse auf dem {CMK}-Server, einem Remote-Server bzw. den überwachten Hosts mit den automatischen Agenten-Updates zu tun haben.
...
|`/etc/check_mk/cmk-update-agent.cfg` |Die Konfiguration des Agent Updater Plugins; wird nur bei der Installation oder beim Update geschrieben.
|`/usr/lib/check_mk_agent/plugins/XXXX/cmk-update-agent` |Der Agent Updater. XXXX steht dabei für das festgesetzte xref:interval_for_update_check#[Zeitintervall in Sekunden]. Je nach Regeleinstellung Binary Executable oder Script.
|`/var/lib/check_mk_agent/cmk-update-agent.log` |Ausführliches Logfile (DEBUG-Level)
|===
...
|`.state` - und `.config` File |liegen im Verzeichnis "config"
|cmk-update-agent Plugin |liegt im Verzeichnis "plugins"
|`cmk-update-agent.log` |liegt im Verzeichnis "logs"
|`check_mk.ini` |Hier ist der Aktualisierungszyklus definiert
|===

=== Pfade auf dem überwachenden Server
[cols="45,~",options="header"]
|===
|Pfad |Bedeutung
|`~/var/check_mk/agent_deployment` |Registrierungsinformationen (Host Secret) und letzter Status der registrierten Hosts. Die Dateien sind nach dem betreffenden Host benannt.
|`~/var/check_mk/agents` |Die gebackenen Agents. Eingeordnet nach System und zutreffenden Hosts (letzteres per Soft-Link).
|===
