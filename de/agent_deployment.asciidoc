...
// IGNORE Check_MK TMPDIR check_mk [ ] CAs
// Eckige KLammern waren hier zur Erklärung von Auslassungen im Text
...
Wenn Sie mit eigener Zertifizierungsinfrastruktur arbeiten, verwenden Sie im Idealfall ein sehr lange gültiges Root-Zertifikat, mit dessen zugehörigem Schlüssel regelmäßig Intermediate Zertifikate erstellt werden, welche dann wiederum zum Signieren der Serverzertifikate verwendet werden.
...
Sind Sie unsicher, ob ein neues Serverzertifikat ein neues Root-Zertifikat erfordert, verwenden Sie den folgenden Befehl, um den Identifier des Root Keys, mit dem ein Server-Zertifikat signiert wurde, zu ermitteln:
...
. Backen Sie die Agentenpakete neu und führen Sie ein Update aller Hosts im Monitoring durch. Stellen Sie sicher, dass dieses Update für alle Hosts durchgelaufen ist, bevor Sie fortfahren.
. Tauschen Sie nun das Serverzertifikat aus.
. Testen Sie mit wenigen Hosts, bei denen bei Fehlschlag ein manuelles Update leicht möglich ist, ob das Update über das neue Zertifikat möglich ist.
