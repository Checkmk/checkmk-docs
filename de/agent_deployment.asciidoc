...
Nach einer erfolgreichen Registrierung wird das _Host Secret_ beim Agenten in der Datei `/etc/cmk-update-agent.state` gespeichert.
...
welcher zuletzt vom Host herruntergeladen wurde ([.guihint]#Downloaded Agent#) und
...
kürzlich zurückgerufene CAs können oft nicht berücksichtigt werden. Daher wird
