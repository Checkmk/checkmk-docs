...
// IGNORE GCE GCP IAM Konnektor Memorystore VMs hineinkopieren zurückliefert
...
Neben den allgemeinen Informationen zu den Kosten Ihrer Google Cloud und dem aktuellen Status der link:https://checkmk.com/de/integrations/gcp_status[Google-Dienste in Ihrer Region^], können Sie mit allen Editionen von {CMK} ab Version {v22} die folgenden Google Cloud-Produkte überwachen:
