...
|[.guihint]#out of service period# | |Das Objekt war außerhalb seiner icon:icon_outof_serviceperiod[] Service-Periode -- also in einem Zeitraum, in dem die Verfügbarkeit „egal“ war. Mehr zu den Service-Perioden erfahren Sie link:availability.html#serviceperiod[weiter unten].
...
oder link:bi.html[BI-Aggregaten] auf. 
...
Mithilfe eines link:wato_user.html#automation[Automationsbenutzers], der sich über die URL
...
* Auch in der Zeitleiste können Sie mit dem Menüeintrag [.guihint]#Availability > Change display options# bzw. [.guihint]#Availability > Change computation options# die link:availability.html#options[Optionen] für die Darstellung und Auswertung anpassen.
...
link:views.html[Ansicht] der Monitoring-Historie mit einem vorausgefüllten Filter
...
link:views.html#limit[Ansichten] funktioniert. Wenn die Anfrage an den Monitoring-Kern
