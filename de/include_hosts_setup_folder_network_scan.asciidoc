...
//TK: Das braucht aber echt nicht im Handbuch zu stehen.
...
//TK: Aha, nach dem Einrichten (wahrscheinlich auch nach einem Sichern?) ist der Scan bereits scharf. Für das "also" fehlt im Satz davor eine Info: Vielleicht so: Unmittelbar nach dem Einrichten (Sichern?) oder wenn das Intervall abgelaufen ist ...

Sobald ein Scan startet, passieren im Grunde drei Dinge:

. {CMK} ermittelt zuerst die IP-Adressen, die gescannt werden sollen.
Zusätzlich zu den eingestellten Adressen und Adressbereichen, ist es wichtig zu wissen, dass niemals bereits bekannte Adressen gescannt werden.
//TK: Besser so?: Aus den konfigurierten Adressbereichen werden dabei alle Adressen entfernt, die bereits bekannt sind.
. Die so ermittelten Adressen werden nun gepingt. Wenn es auf einer Adresse eine Antwort auf diesen Ping gibt, wird versucht einen Host-Namen zu ermitteln.
...
// TK: Hab ich mich natürlich gleich gefragt, wann denn diese Dubletten angelegt werden könnten - aber steht ja am Ende des Kapitels :-)
...
* {CMK} kann Ihnen zu den gefundenen Hosts nur dann saubere Daten liefern, wenn Ihr Netzwerk und Ihre Instanz entsprechend konfiguriert ist.
Widersprüchliche Einträge im DNS und Ihrer {CMK}}-Instanz können zu Dubletten führen, d.h., Sie werden womöglich Hosts über den Netzwerk-Scan finden, welche unter anderen Namen bereits durch {CMK} überwacht werden.
//TK: Hab ich das jetzt so richtig verstanden?:
//TK: Eine Dublette wird angelegt, wenn der Host mit Namen (aber ohne IP-Adresse) in {CMK} bereits eingerichtet ist und nun über seine IP-Adresse unter einem anderen Namen gefunden wird.
//TK: Den Satz finde ich klarer, als der Teil nach dem d.h., 
