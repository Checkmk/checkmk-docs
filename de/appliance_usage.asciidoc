...
// ML: KNW-810
_Achtung:_ Bei Aktualisierungen auf neue {CMK}-Major-Versionen sollten Sie den xref:update_major#[expliziten Artikel] dazu beachten. Wichtig ist hier, dass Sie immer nur auf die nächste Major-Version aktualisieren und keine überspringen.
...
Im Gegensatz zum folgend beschriebenen Firmware-Update innerhalb kompatibler Versionen, also beispielsweise 1.5.1 auf 1.5.2, müssen Sie beim Update von 1.4-Versionen auf die aktuelle 1.5 etwas anders vorgehen. Der Grund: Die 1.5 basiert auf einer neuen Debian-Version. Kurz gesagt heißt das, dass Sie das Cluster kurzzeitig komplett komplett offline nehmen müssen - Sie bekommen also eine Downtime. Bei regulären Updates genügt es, einzelne Nodes des Clusters in den Wartungsmodus zu versetzen, um die Aktualisierung durchzuführen. Bei diesem Major-Update auf 1.5 gehen Sie wie folgt vor:
