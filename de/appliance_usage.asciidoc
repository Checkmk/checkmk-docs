...
Wir beginnen hier mit dem Erststart der Appliance -- die virtuelle Appliance muss also bereits xref:introduction_virt1#[installiert], beziehungsweise das Rack xref:appliance_rack_config#[angeschlossen], sein.
...
Außerdem werden alle Log-Einträge, die an einen Syslog-Server gesendet werden, mit den konfigurierten Hostnamen als Quellhost ergänzt, um die Einträge leichter zuordnen zu können.
...
Weiterhin müssen Sie noch die Hostadresse des Syslog-Servers konfigurieren, der die Log-Meldungen annehmen soll.
...
Nach der Anmeldung können Sie {CMK} wie gewohnt einrichten -- die ersten Schritte sind im Artikel zum xref:intro_setup_monitor#[Das Monitoring einrichten] beschrieben.
...
Sie landen wieder in der Instanzverwaltung und können die importierte Instanz starten und verwalten:
...
Wenn ein Nutzer auf die Weboberfläche per HTTPS zugreift und das Zertifikat von einer Zertifizierungsstelle signiert ist, der er nicht vertraut, führt das zunächst zu einer Warnung im Webbrowser.
...
Nutzer, die versehentlich direkt oder über Bookmarks per HTTP auf die Weboberfläche zugreifen, werden also automatisch auf HTTPS umgeleitet.
...
Aktuelle Supportinformationen zur Appliance sind zudem jederzeit im Internet über unsere link:https://checkmk.de/appliance.html[Webseite^] abrufbar.
...
Die aktuellen Versionen der Firmware finden Sie auf unserer link:https://checkmk.com/de/download[Webseite.^]
