...
###Serverraum oder im Falle der Checkmk virt1 durch Zugriff auf die Management-Oberfläche
...
Wählen Sie nun die Checkmk-Version mit der die Monitoring-Instanz erstellt
werden soll.
...
einloggen. Nach dem Login können Sie Checkmk wie gewohnt einrichten. In
allen Monitoring-Instanzen ist für alle Administratoren in der Sidebar
das Snapin [.guihints]#Checkmk Appliance# verfügbar. Damit gelangen Sie aus Ihren
...
laufende Monitoring-Instanz auf eine Checkmk-Appliance migriert werden soll. Die
...
[#manage_cmk]
== Checkmk-Versionen verwalten

Es ist möglich, auf dem Gerät mehrere Checkmk-Versionen parallel
...
Zum Verwalten der Checkmk-Versionen wählen Sie im Hauptmenü der Weboberfläche
den Eintrag [.guihints]#Checkmk versions}}.# 

Im folgenden Dialog werden Ihnen alle installierten Checkmk-Versionen
...
Außerdem können Sie hier neue Checkmk-Versionen auf das Gerät hochladen,
um diese in neuen Instanzen zu verwenden oder bestehende Instanzen zu
aktualisieren.

Laden Sie dazu zunächst die gewünschte Checkmk-Version von unserer <a
...
Nun wird die Checkmk-Version auf das Gerät hochgeladen. Dies kann, je nach
...
=== Zugriff auf Checkmk-Agenten anpassen

Auf dem Gerät ist ein Checkmk-Agent installiert, der in der Grundeinstellung
...
Es ist ebenfalls möglich, den Checkmk-Agenten von einem entfernten Gerät aus
...
konfigurieren, die den Checkmk-Agenten kontaktieren dürfen.
...
auf dem Gerät mindestens Checkmk 1.2.8p6, 1.4.0i1 oder einen Daily-Build ab
...
Die nicht komprimierte Sicherung wird erst ab Checkmk-Version 1.2.8p5
...
von Checkmk ab Version 1.4.0i1 auf dem Gerät einen neuen Service [.guihints]#Backup [JOB-ID]}}.# 
...
kompatible Checkmk-Appliances. Folgende Modelle können miteinander geclustert
werden:
...
erstellen, die alle vier Netzwerkanschlüsse der Checkmk rack1 nutzt.
...
*Virtuelle Maschinen:* Wenn Sie die Cluster-Funktion mit zwei „Checkmk
...
Im Folgenden wird die Konfiguration eines Clusters mit zwei Checkmk rack1
...
rechts auf das Icon neben dem Checkmk-Logo. Daraufhin erscheint im Dialog eine
...
Management-Interfaces des Checkmk rack1, die Sie zu jeder Zeit auf den einzelnen
...
z.B. bei kritischen Problemen, von Checkmk SMS-Benachrichtungen verschicken
zu lassen.
...
abzüglich des Speichers, der von den Systemprozessen der Checkmk-Appliance
...
können Sie Ihr Gerät von Checkmk überwachen lassen. Auf dem Gerät wird ein
...
Im Fall eines Softwarefehlers, egal ob Firmware oder Checkmk-Monitoring-Software,
