...
Dies können Sie z.&ThinSpace;B. mit einem `ping` von einem anderen Gerät im Netzwerk testen.
...
Wenn von Ihrem Gerät aus E-Mails verschickt werden sollen, z.&ThinSpace;B. Benachrichtigungen bei Problemen, müssen Sie die Option [.guihint]#Outgoing Emails# konfigurieren. 
...
Sie können unter dieser Einstellung auch konfigurieren, dass alle E-Mails, die vom Betriebssystem des Gerätes erzeugt werden, z.&ThinSpace;B. bei kritischen Fehlern, an eine separate E-Mail-Adresse geschickt werden sollen.
...
Sorgen Sie dafür, dass die Instanz nicht läuft, d.&ThinSpace;h. wenn der [.guihint]#Status# gleich [.guihint]#running# ist, stoppen Sie die Instanz ([.guihint]#Control > Stop#). 
...
Dies kann sinnvoll sein, wenn Sie kein 1:1-Abbild der Instanz brauchen, sondern diese nur, z.&ThinSpace;B. zu Testzwecken, duplizieren wollen.
...
Sie können die Software Ihres Gerätes, d.&ThinSpace;h. die Firmware der Appliance, im laufenden Betrieb auf eine neuere Version aktualisieren -- oder auch auf eine ältere Version zurück wechseln. 
...
Die CFW-Datei finden Sie im Download-Bereich beim Produkt Checkmk Appliance. 

*Hinweis:* Achten Sie darauf, dass Sie die Firmware passend zur installierten Edition Ihrer Appliance auswählen. 
Wenn Sie allerdings von der {FE} auf eine Vollversion der {EE} upgraden wollen, ist auch das möglich, indem Sie die Firmware der Vollversion auswählen und auf diese aktualisieren.  
...
Nur wenn Sie diese Option konfiguriert haben, können Sie z.&ThinSpace;B. bei der
Konfiguration der NTP- oder Mailserver Host- und Domainnamen verwenden.


=== Zeitsynchronisation konfigurieren

Die Systemzeit des Gerätes wird an vielen Stellen, z.&ThinSpace;B. zum Erfassen von
...
Damit Sie von dem Gerät aus E-Mails verschicken können, z.&ThinSpace;B. bei Ereignissen
...
gibt, z.&ThinSpace;B. ein Job nicht ausgeführt werden kann oder ein Hardwareproblem
...
{CMK}-System überwacht werden kann, z.&ThinSpace;B. in einer verteilten Umgebung
...
(z.&ThinSpace;B. PuTTY) mit dem Gerät verbinden.
...
dass alle Änderungen, die Sie am Gerät vorgenommen haben, z.&ThinSpace;B. Ihre
...
z.&ThinSpace;B. einem Fileserver, abgelegt werden. Hierzu konfigurieren Sie zunächst
...
Freigabe, wie z.&ThinSpace;B. die Netzwerkadresse des Fileservers und den exportierten
...
Knoten seine Aufgaben nicht mehr erfüllen kann, z.&ThinSpace;B. aufgrund eines Ausfalls,
...
`192.168.178.110`) auf die Ressourcen des Clusters, z.&ThinSpace;B. Ihre
...
_(M)_ für den Main-Host und _(S)_ für den Slave-Host dargestellt.
...
Alle Anfragen an die Monitoring-Instanzen, wie z.&ThinSpace;B. Zugriffe auf die
Web-Oberfläche, aber auch eingehende Meldungen, wie z.&ThinSpace;B. SNMP-Traps oder
...
Nur in Ausnahmefällen, wie z.&ThinSpace;B. Fehlerdiagnosen oder Updates eines bestimmen
Knotens, sollten Sie direkt auf die einzelnen Knoten zugreifen müssen.


==== Geräteeinstellungen

Die Einstellungen, wie z.&ThinSpace;B. Zeitsynchronisation oder Einstellungen zur
...
Es gibt einige gerätespezifische Einstellungen, wie z.&ThinSpace;B. die des
...
Konfiguration weiter nutzen, oder z.&ThinSpace;B. eines der Geräte wieder auf den
...
z.&ThinSpace;B. bei kritischen Problemen, von {CMK} SMS-Benachrichtungen verschicken
zu lassen.
...
z.&ThinSpace;B. das link:https://www.multitech.com/models/92507087LF[MTD-H5-2.0],
...
Die Telefonnummer muss dabei ohne führende Nullen und ohne führendes Plus-Zeichen eingegeben werden, also z.&ThinSpace;B. `491512345678` für eine Mobilfunknummer in Deutschland.
