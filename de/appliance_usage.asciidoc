...
Das Passwort muss mindestens 8 Zeichen lang sein, und muss mindestens einen Kleinbuchstaben, einen Großbuchstaben und eine Ziffer enthalten.
...
Anschließend sehen Sie im Startbildschirm im Kasten [.guihint]#Access# den Eintrag [.guihint]#Console Login: on#.
...
Das bedeutet, dass jeder, der direkten Zugriff auf die Konsole hat (z.B. durch Zutritt zum Serverraum oder im Falle der {CMK} virt1 durch Zugriff auf die Management-Oberfläche der Virtualisierungslösung), die Grundkonfiguration des Gerätes ändern kann.

Um den Zugriff zu beschränken, könnnen Sie eine Passwortabfrage aktivieren, an dem man sich mit dem Gerätepasswort anmelden muss, bevor man Einstellungen vornehmen oder den aktuellen Status einsehen kann.

Wenn Sie die Weboberfläche aktiviert haben, haben Sie damit auch ein Gerätepasswort gesetzt und müssen kein neues vergeben. Andernfalls
öffnen Sie an der Konsole über `F1` das Konfigurationsmenü und wählen [.guihint]#Device Password#, um das Passwort festzulegen.

Anschließend wählen Sie aus dem Konfigurationsmenü die Option _Konsolenlogin_ um diese zu aktivieren.

Wenn Sie diese Schritte abgeschlossen haben, sehen Sie auf dem Statusbildschirm der Konsole, wie im obigen Screenshot, _Konsolenlogin: an_ im Kasten _Zugriff_.
////
...
Damit ist die Grundkonfiguration des Gerätes abgeschlossen und Sie können mit der Einrichtung der ersten Monitoring-Instanz fortfahren.
...
Öffnen Sie das Hauptmenü der Web-Oberfläche und klicken auf den
...
URL zur Monitoring-Instanz aufrufen, hier `192.168.178.60/mysite`.
...
Monitoring-Instanzen direkt zur Web-Oberfläche der Appliance.
...
In der Web-Oberfläche finden Sie unter dem Punkt [.guihint]#Site Management# einen
...
Zum Verwalten der {CMK}-Versionen wählen Sie im Hauptmenü der Web-Oberfläche
...
(suchen Sie in der Spalte _Distribution_ nach _{CMK} Appliance_).
...
sogenannte Firmware-Update in der Web-Oberfläche.
...
Computer herunter. Öffnen Sie im Anschluss die Web-Oberfläche des Gerätes
