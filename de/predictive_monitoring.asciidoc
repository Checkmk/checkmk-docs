...
Für unser Beispiel bedeutet das, zuerst eine neue xref:timeperiods#[Zeitperiode] für die Zeit mit hoher Auslastung zu definieren (montags bis freitags von 0:00 bis 7:00 Uhr).
...
Allerdings ist die manuelle Bindung von Schwellwerten an Zeiträume auch etwas unflexibel und manchmal auch einfach viel zu umständlich.

{cee-only}
Falls Sie eine der {CEE} einsetzen, steht Ihnen aber noch eine weitere Möglichkeit zu Verfügung, um das Problem zu lösen.
...
Eine ganze Reihe von {CMK}-Plugins unterstützen das prognosebasierte Monitoring. Im folgenden finden Sie einige wichtige Beispiele:
...
link:https://checkmk.com/de/integrations/statgrab_net[Network Interfaces: Traffic^] +
...
Die Einstellungen für das prognosebasierte Monitoring finden Sie an der gleichen Stelle, an der Sie auch sonst Schwellwerte für einen Service
einstellen.
...
Obwohl die Messdaten in den RRD-Dateien 4 Jahre lang gespeichert werden, macht es keinen Sinn, zu weit in die Vergangenheit zurückzugehen.
...
Das liegt daran, dass {CMK} die minütlich vorliegenden Messdaten in den RRD-Dateien _standardmäßig_ in drei Phasen verdichtet, um Platz zu sparen: nach 2, 10 und nach 90 Tagen.
...
Für unseren Beispiel-Server mit der hohen Auslastung Montag bis Freitag nachts bietet es sich an, die wöchentliche Referenzperiode auszuwählen und einen Zeitraum von (maximal) 90 Tagen.
...
*Hinweis:* Die Werte, die zur Berechnung des Durchschnitts für die Referenzperiode hergenommen werden, können selbst bereits berechnete (d.h. nicht gemessene) Werte sein - je nach Auflösung der historischen Daten in den RRD-Dateien.
...
Um die Auswirkung der verschiedenen Parameterwerte zur Festlegung der Schwellwerte zu verdeutlichen, sehen wir uns einen einzigen Wert auf der
Referenzkurve genau an.
...
Beispiel: [.guihint]#Warning at# `10.0` % wird dazu führen, dass bei einem Wert über 11 und unter 9 eine Warnung angezeigt wird.
...
Schließlich haben Sie mit [.guihint]#Limit for upper bound dynamic levels# die Möglichkeit für die oberen Schwellwerte absolute Mindestwerte
vorzugeben.
...
Im nächsten Kapitel erfahren Sie, wie sich die Einrichtung im Monitoring bemerkbar macht und wie Sie sich den Prognose-Graphen anzeigen
lassen können.
...
*Hinweis:* Insbesondere nach der Ersteinrichtung für einen Service kann es vorkommen, dass das Symbol fehlt, da noch nicht genügend Daten für die konfigurierte Prognose bereitstehen.
In diesem Fall wird in der Spalte [.guihint]#Summary# eine Meldung der Art `(no reference for prediction yet)` angezeigt.
Sobald genügend Daten vorhanden sind, wird dich das Problem von selbst erledigen.
...
Die dargestellte Zeitperiode orientiert sich an der ausgewählten Referenzperiode.
...
*Tipp:* Wie beim erstmaligen Einrichten wird auch jede Änderung des prognosebasierten Monitoring erst nach einem neuen Check des Services
wirksam.
Sie brauchen auf den nächsten regelmäßigen Check nicht zu warten, sondern können ihn manuell in der Service-Liste anstoßen mit dem Symbol icon:icon_menu[] und dem Menüeintrag [.guihint]#Reschedule '{CMK}' service.#
