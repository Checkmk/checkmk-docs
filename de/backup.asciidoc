...
== Technische Voraussetzung für Backups

Um Backups erstellen und später auch wieder einspielen zu können, gibt es zwei wesentliche Grundvoraussetzungen:

* Auf beiden Systemen, sowohl dem Quell- als auch dem Zielsystem des Backups muss die exakt gleiche Version von {CMK} installiert sein.

* Auf beiden Systemen muss die exakt gleiche Edition von {CMK} installiert sein.

Hinweis: Die Systemplattform ist nicht relevant, so dass Sie zum Beispiel auch mit Ubuntu auf dem Quellsystem und Redhat oder einer Appliance als Zielsystem Backups erstellen können.
