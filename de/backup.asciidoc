...
Im Laufe der Zeit werden Sie einiges an Arbeit in die Konfiguration Ihrer Hosts und Services, Grenzwerte, Alarmierungen und so weiter stecken -- daher sollten Sie Backups erstellen.
...
image::backup_key_warning.png[alt="Liste der Backup-Schlüssel mit Meldung, dass die Schlüssel noch nicht heruntergeladen wurden."]
