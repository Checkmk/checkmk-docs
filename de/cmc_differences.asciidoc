...
Die Fetcher übernehmen damit die Netzwerkkommunikation mit dem {CMK}-Agenten, SNMP-Agenten und den xref:datasource_programs#specialagents[Spezialagenten.] 
Das Sammeln dieser Informationen benötigt etwas Zeit, verbraucht aber nur wenig Arbeitsspeicher.
...
Dafür verursachen sie keine Netzwerklast und sind sehr schnell in der Ausführung.
...
Es ist standarmäßig aktiv, d.h. die Option [.guihint]#Global settings > Monitoring Core > Use separate fetchers and checkers# ist eingeschaltet.
...
welches wiederum auf dem link:http://www.rrdtool.org/RRDtool[RRDTool] aufsetzt. 
