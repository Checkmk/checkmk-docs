...
Das Erzeugen der Prozesse für den Aufruf der Plugins verbraucht wertvolle CPU-Ressourcen. 
...
Host-Checks dienen nicht nur der Benachrichtigung bei einem Totalausfall eines Hosts, sondern auch dazu, Benachrichtigungen zu Service-Problemen während der Zeit des Host-Ausfalls zu unterdrücken. 
...
Für Hosts, bei denen Sie diese benötigen, können Sie einfach einen xref:active_checks#[aktiven Check] per Ping einrichten mit dem Regelsatz [.guihint]#Check hosts with PING (ICMP Echo Request#.
...
// ML: In der en-Version heißen die Dinger "Checkmk fetcher", "Checkmk helper" und "check helpers" - im Deutschen "Checkmk Fetcher", "Checkmk Helper" und "Check Helper". Wenn es Eigennamen sein sollen: In en auch groß. Wenn nicht: In de Bindestriche.
...
Bei jeder Ausführung eines xref:active_checks#[aktiven Checks] muss zunächst der komplette Nagios-Prozess dupliziert werden (_fork_), bevor er durch den neuen Prozess -- das Check-Plugin -- ersetzt wird. 
...
// TK: Den folgenden 3 Sätze hab ich auskommentiert, weil sie nicht zum folgenden Intro der neuen Fetcher und Checker passen:
// {CMK} berechnet die Ergebnisse von allen Services ganz ohne weitere Prozesserzeugungen. 
// Doch auch das {CMK}-Plugin selbst braucht etwas CPU-Zeit. 
// Dabei wird der Großteil beim Starten des Plugins benötigt.
...
Die Fetcher übernehmen damit die Netzwerkkommunikation mit den {CMK}-Agenten, SNMP-Agenten und den xref:datasource_programs#specialagents[Spezialagenten.] 
Das Sammeln dieser Informationen benötigt etwas Zeit, verbraucht aber nur wenig Arbeitsspeicher (ca. 30 MByte pro Prozess).
...
Ein Checker-Prozess verbraucht mindestens ca. 90 MByte -- es kann aber auch ein Vielfaches davon notwendig sein, je nachdem wie die Checks konfiguriert sind. Dafür verursachen die Checker keine Netzwerklast und sind sehr schnell in der Ausführung.
Die Anzahl der Checker sollte nur so groß sein, wie Ihr {CMK}-Server parallel Prozesse abarbeiten kann. 
Im Regelfall entspricht diese Zahl der Anzahl an Rechenkernen Ihres Servers. 
Da die Checker nicht IO-gebunden sind, sind sie am effektivsten, wenn jeder Checker seine eigene CPU erhält. 

// ML: Fände ich verständlicher: "... jeder Checker seinen eigenen Kern erhält."

// TK: Den folgenden Part "Weiterentwicklung 1.6 nach 2.0" hab ich (noch) dringelassen, weil man die alten CMK-Helper auch in der 2.0 wieder aktivieren kann.
...
Sie ist standardmäßig aktiv, d.h. die Option [.guihint]#Global settings > Monitoring Core > Use separate fetchers and checkers# ist eingeschaltet.
...
In größeren Installationen wurden aber für viele zu überwachende Hosts viele {CMK} Helper benötigt, die dann viel Speicher verbrauchten,
da jeder {CMK} Helper die gesamte {CMK}-Konfiguration im Arbeitsspeicher hielt. 

Mit dem neuen Fetcher/Checker-Modell der Version {v20} können nunmehr beide Aufgaben auf zwei separate Pools von Prozessen aufgeteilt werden: 
...
Dadurch verbraucht ein CMC bei gleicher Leistung (Checks pro Sekunde) bis zu viermal weniger Arbeitsspeicher!
...
Achten Sie jedoch darauf, nur so viele Hilfsprozesse zu erzeugen, wie wirklich benötigt werden, da jeder Prozess Ressourcen verbraucht. 
...
Alternativ zum Snapin [.guihint]#Core statistics# können Sie sich auch Ihre Konfiguration von {CMK} analysieren lassen, mit [.guihint]#Setup > Maintenance > Analyze configuration#. 
Der Vorteil: Hier erhalten Sie von {CMK} sofort die Bewertung, wie es um den Zustand der Hilfsprozesse steht:
// TK: Auch hier gibt es wieder einen Eintrag für die CMK Helper aus der 1.6: "Checkmk helper usage", den ich aus dem Screenshot weggeschnitten habe
// TK: Bei der Gelegenheit: Warum ist im Handbuch eigentlich die Analyze configuration nicht beschrieben?

image::cmc_differences_analyze_configuration.png[alt="Dialog mit dem Resultat der Konfigurationsanalyse für Hilfsprozesse."]

Durch Klick auf icon:icon_toggle_details[alt="Symbol zum Ein-und Ausblenden von Detailinformationen."] können Sie sich für jeden Eintrag einen passenden Hilfetext einblenden lassen. 
Sehr praktisch ist: 
Sollte einer der Hilfsprozesse nicht [.guihint]#OK# sein, können Sie aus dem Hilfetext auch gleich die zugehörige Option in den [.guihint]#Global settings# öffnen, um den Wert zu ändern. 
...
. Das Anlegen und Aktualisieren der xref:graphing.html#rrds[Round-Robin-Datenbanken (RRDs).]
