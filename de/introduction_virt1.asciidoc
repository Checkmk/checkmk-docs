...
* OVA-Datei: Die virtuelle Appliance {CMK} virt1 zum Import in den Hypervisor. OVA steht für Open Virtualization Archive und ist das Archivformat des Open Virtualization Format (OVF). Die OVA-Datei finden Sie im Download-Bereich beim Produkt {CMK} Appliance. Diese OVA-Datei benötigen Sie für die Installation.

* CFW-Datei: Die Firmware der Appliance. Damit können Sie die Appliance-Version updaten, z.B. von Version 1.4.13 auf 1.4.14. Ein xref:appliance_usage#cma_webconf_firmware[Update der Firmware] erfolgt mit der Appliance Web-GUI. Die CFW-Datei finden Sie ebenfalls beim Produkt {CMK} Appliance. Die-CFW-Datei benötigen Sie nicht für die Erstinstallation, da in der OVA-Datei die Firmware bereits enthalten ist.

* CMA-Datei: Die {CMK}-Software zur Installation in der Appliance. Beginnend mit der Appliance-Version 1.4.14 ist in der Appliance keine {CMK}-Software mehr vorinstalliert. Auch die xref:appliance_usage#manage_cmk[Installation der {CMK}-Software] führen Sie mit der Appliance Web-GUI durch. Die CMA-Datei wird Ihnen im Download-Bereich angeboten nach der Auswahl der {CMK} Edition, Version und der Plattform Appliance. Die CMA-Datei benötigen Sie erst *nach* abgeschlossener Installation -- wenn Sie die Grundkonfiguration der Appliance durchführen.
