...
{CMK} erwartet die Agentenausgabe ausschließlich link:https://de.wikipedia.org/wiki/UTF-8[UTF-8^] kodiert (ohne link:https://de.wikipedia.org/wiki/Byte_Order_Mark#In_UTF-8[BOM^]) mit einem einfachen Newline (`0x0A` oder `\n`) als Zeilenumbruch.
...
* Verzichten Sie auf das link:https://de.wikipedia.org/wiki/Byte_Order_Mark[_Byte Order Mark_^].
...
* Verwenden Sie den Unix-Zeichenumbruch Newline (`0x0A` oder `\n`).
...
`0xFF 0xFE` zeigt eine UTF-16 kodierte Datei an: Ändern Sie in diesem Fall Ihre Skripte auf Ausgabe in UTF-8.
