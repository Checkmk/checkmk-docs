...
Damit Sie einen direkten Eindruck bekommen, wie diese Dashboards aussehen, finden Sie nachfolgend zwei Beispiele, wie diese Dashboards aufgebaut sind. Zuerst das Dashboard zu den VM-Instanzen, bei Sie auf der linken Seite den aktuellen Zustand und auf der rechten Seite den zeitlichen Verlauf der wichtigsten Metriken vergleichen können:

image::monitoring_azure_dashboard_vm.png[alt="Eine Übersicht zu den VM-Instanzen"]

Das Dashboard zu den Storage Accounts ist ganz ähnlich aufgebaut. Auf der rechten Seite finden Sie aktuelle Daten der jeweiligen Buckets. Auf der linken werden wieder die wichtigsten Metriken im zeitlichen Verlauf dargestellt:

image::monitoring_azure_dashboard_storage.png[alt="Eine Übersicht zu den Storage Accounts"]