...
|MSI-Paket aus der link:wato_monitoringagents.html#bakery[Agentenbäckerei]
...
[.guihint]#Monitoring Agents#. Falls Sie die {EE} und eine Version einsetzen, die älter als VERSION[1.4.0] ist, gehen Sie vorher
noch den Umweg über den Knopf [.guihint]#Unpackaged agents}} bzw. {{Agent files#:
...
== Installation des alten Agenten#legacy
...
== Test und Fehlerdiagnose#testing
...
== Einbinden von klassischen Check-Plugins#mrpe
...
== Erweitern um Agenten-Plugins#plugins
...
=== Ausführung eines speziellen Plugins anpassen#customizeexecution
...
im Kasten [.guihint]#DATA SOURCES}} die Einstellung {{SNMP# auf die geeignete
Verbindungsart (snmpv2/3 oder snmpv1). In Versionen älter als VERSION[1.5.0]
heißt der Kasten [.guihint]#Host tags}} und wird auf {{Dual: Check_MK Agent + SNMP#
...
|Das Verzeichnis für eigene link:localchecks.html[local-Skripten]
