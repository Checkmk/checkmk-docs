...
// IGNORE R1 R2 Ver benutzerspezifischen eherner erstmalig mitinstalliert mitinstallierten pre programmspezifischen v1 v2 v3
// IGNORE Anwendungsfälle Forwardslashes Pfadtrenner Skriptbestandteile
...
Im Pull-Modus lauscht er am TCP-Port 6556 auf eingehende Verbindungen der {CMK}-xref:glossar#site[Instanz] und fragt das Agentenprogramm über eine TCP-Verbindung ab.
...
|Mitgeliefertes MSI-Paket |Einfache Installation eines Standard-Agenten mit manueller Konfiguration über Konfigurationsdateien. |X |X |X
|MSI-Paket aus der xref:glossar#agent_bakery[Agentenbäckerei] |Konfiguration über die GUI, individuelle Konfiguration pro Host möglich. | |X |X
...
In den {EE} und der {CE} gelangen Sie im [.guihint]#Setup#-Menü über [.guihint]#Agents > Windows, Linux, Solaris, AIX# zunächst in die xref:wato_monitoringagents#bakery[Agentenbäckerei], wo Sie die gebackenen Pakete finden.
...
// TK: Screenshot-Update für 2.2 nötig
...
{cee-only}
Die {EE} und die {CE} verfügen mit der xref:glossar#agent_bakery[Agentenbäckerei] über ein Software-Modul zum automatischen Paketieren von individuell angepassten Agenten.
...
// MFS: Müssen wir hier hinsichtlich der Auto-Registrierung noch Nachfragen stellen?
// MFS: Hier keine Änderungen.
...
*Vorsicht, Falle:* Falls Sie vorrangig Unix-Maschinen administrieren, sind Sie es gewohnt, Pfade oder Parameter mit Leer- oder Sonderzeichen in einfachen Anführungszeichen (Apostrophe, `0x27`) zu umschließen.
Dieses Zeichen interpretiert Windows als Teil des Aufrufs – hier des Passwortes –, und die Registrierung wird fehlschlagen.
Verwenden Sie stattdessen doppelte Anführungszeichen (Quotation Mark, `0x22`).
...
Wir arbeiten uns im Folgenden vom Agentenprogramm über den Agent Controller und den TCP-Port 6556 bis zur {CMK}-Instanz durch.
...
Falls die Kommunikation zwischen Agent und {CMK}-Server _noch_ unverschlüsselt ist (wie im Legacy-Pull-Modus) oder unverschlüsselt ist und auch bleibt (wie im Legacy-Modus),
...
[#windows_firewall]
=== Windows Firewall
// MFS: Müssen wir hier noch auf den Push-Modus verweisen? Outbound Ports sind bei Windows in der Regel erlaubt.
// MFS: Falls ja, muss die Änderung analog in den Linux-Artikel.
// TK: Ein Hinweis wäre schon sinnvoll, dass es im Push-Modus um die Outbound Rules geht + dass Windows die Kommunikation für ausgehende Ports in der Regel erlaubt. 
// MFS: Nach Rücksprache mit Sergey hinzugefügt.
...
Sie können dieses Verhalten temporär ändern, indem Sie auf dem Host in der Datei `C:\ProgramData\checkmk\agent\pre_configured_connections.json` die Variable `keep_vanished_connections` auf `true` setzen.
...
|`C:\ProgramData\checkmk\agent\pre_configured_connections.json` |Enthält eine vorkonfigurierte und per Agentenbäckerei in das Agentenpaket integrierte Verbindung zu einer Instanz für die xref:hosts_autoregister#[Autoregistrierung] in der {CE}.
...
// TK: Hier fehlt die C:\ProgramData\checkmk\agent\log\check_mk.log
|`C:\ProgramData\checkmk\agent\log` |Hier finden Sie Logdateien. Neben der ständig im Betrieb aktualisierten `check_mk.log` (in allen Editionen) können hier weitere Protokolle von automatischer Installation oder Updates vorhanden sein.
|===
