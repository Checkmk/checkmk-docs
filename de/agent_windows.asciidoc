...
Archivierte Agenten älterer {CMK}-Versionen ohne Agent Controller, die Sie auf eigenes Risiko mit nicht aufgeführten Windows-Versionen nutzen können, stellen wir link:https://download.checkmk.com/legacy-agents/[hier zum Download^] zur Verfügung.
Für den Legacy-Modus sind einige Besonderheiten zu beachten, die im xref:post_install[Installationskapitel] zusammengefasst sind.
...
Anders liegt der Fall bei Nutzung eines archivierten Agenten auf einem sehr alten xref:windows_versions[Windows-System].
