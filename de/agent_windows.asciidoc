...
image::agent_windows_communication_failed.png[width=64%]
//SK: Hier wäre eine BU mit erneutem Bezug auf den Connection Test sehr hilfreich.
...
{cee-only}
Die {CEE} verfügt auch für den Agenten unter Windows über die
...
Downloadseite der Agenten im Setup (wie am Anfang des Artikels beschrieben)
...
im Regelsatz [.guihint]#Setup > Agents > Agent access rules > Encryption#. Die
...
erreichen die entsprechende Regel in den {EE} über [.guihint]#Setup > Agents >
Access to Agents > Encryption#. Mit dem Erstellen einer solchen Regel sind Sie
...
// ML: Auch unter Win 10 ist der Agent noch in der Systemsteuerung zu finden.
...
// ML: Als Ergebnis bekommt man in Powershell und CMD eine ewig lange Liste ohne Sortierung - und in CMD kann man nichtmal an den Anfang scrollen, da der Output beschränkt ist. Da müsste vielleicht noch irgendwas Hilfreiches zum Filtern dazu. Ich mache es mit grep, aber das ist ja standardmäßig nicht verfügbar - und wie man wmic filtert weiß ich gerade nicht auswändig.
