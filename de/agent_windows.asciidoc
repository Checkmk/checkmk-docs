...
// Archivierte Legacy Agenten älterer {CMK}-Versionen ohne Agent Controller, die Sie auf eigenes Risiko mit nicht aufgeführten Windows-Versionen nutzen können, stellen wir link:https://download.checkmk.com/legacy-agents/[hier zum Download^] zur Verfügung.
// Für den Legacy-Modus sind einige Besonderheiten zu beachten, die im xref:post_install[Installationskapitel] zusammengefasst sind.
// TK: Neu formuliert (Legacy-Modus entfernt und durch Legacy-Agent ersetzt):
...
{cee-only}
Die {EE} verfügen mit der xref:glossar#agent_bakery[Agentenbäckerei] über ein Software-Modul zum automatischen Paketieren von individuell angepassten Agenten.
...
// TK: Leicht überarbeitet: Legacy-Modus entfernt
