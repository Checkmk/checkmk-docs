...
Sollten Sie allerdings bei der link:wato_services.html#discovery[Service-Erkennung] oder bei einem link:wato_monitoringagents.html#diagnosticpage[Verbindungstest] zum Host überhaupt keine Informationen bzw. nur Fehlermeldungen über Timeouts erhalten, so sollten Sie auf dem Host die [.guihint]#Inbound Rules# der Windows-Firewall prüfen.
...
{cee-only}
Die {CEE} verfügen auch für den Agenten unter Windows über die Möglichkeit, diesen über die link:wato_monitoringagents.html#bakery[Agentenbäckerei] individuell über das [.guihint]#Setup#-Menü zu konfigurieren.
Eine ausführliche Beschreibung finden Sie im allgemeinen Kapitel über die link:wato_monitoringagents.html[Agenten].
...
Der notwendige Eintrag wird dann automatisch in der link:agent_windows.html#files[Konfigurationsdatei der Agentenbäckerei] erzeugt.
...
link:wato_monitoringagents.html#bakery[Agentenbäckerei] konfiguriert werden. Diese sorgt
...
link:agent_windows.html#files[Konfigurationsdatei der Agentenbäckerei] erzeugt.
...
Da die verschiedenen Routinen von Windows nur Dateien entfernen, welche auch durch den Installationsprozess dort hin gekommen sind, ist es vollkommen normal, dass in den link:agent_windows.html#files[Verzeichnissen des Agenten] noch Dateien übrig bleiben.
...
|`C:\ProgramData\checkmk\agent\local` |Das Verzeichnis für link:localchecks.html[lokale Checks].
