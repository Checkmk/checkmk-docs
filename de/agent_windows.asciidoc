...
In der {CMK}-Version {v21} wurde dem *Agentenprogramm* mit dem *Agent Controller* eine neue Komponente zur Seite gestellt.
...
In diesem Modus arbeitet der der Agent nur als Agentenprogramm, d.h. ohne Agent Controller und damit auch ohne Registrierung am {CMK}-Server.
...
Manche davon stehen nur in den {EE} bzw. der {CE} zur Verfügung:
...
// |xref:hosts_autoregister#[Automatische Erstellung von Hosts] |Über Einstellungen in der Agentenbäckerei kann sich der Host automatisch an einer {CMK}-Instanz registrieren, Hosts werden automatisch in für sie vorbereiteten Ordnern erstellt. | | |X
|===
...
Die Installation des gebackenen MSI-Pakets geschieht genauso wie es für die mitgelieferten Pakete xref:install_paket[oben] beschrieben ist.

// MFS: So präziser?
In der {CCE} können Sie die Agentenbäckerei zusätzlich nutzen, um Agentenpakete mit einer Konfiguration für Autoregistrierung zu versehen, was die xref:hosts_autoregister#[automatische Erstellung von Hosts] ermöglicht.
In diesem Fall erfolgt die Registrierung des Agenten automatisch nach der Installation des Agentenpakets und eine manuelle Registrierung, wie im folgenden xref:registration[Kapitel] beschrieben, ist _nicht_ mehr notwendig.
...
In der {CEE} finden Sie in den Eigenschaften des Hosts im Abschnitt zu den xref:hosts_setup.html#monitoring_agents[Monitoring-Agenten] die Option [.guihint]#Checkmk agent connection mode.#
Hier können Sie alternativ zum Pull-Modus, der in allen Editionen verfügbar ist, für den {CMK}-Agenten den Push-Modus der {CE} aktivieren.
...
// TK: Mit neuem 2.2 Agenten checken. cmk-agent-ctl help (oder gar Subkommandohilfe) versteht mein Windows-Agent der 2.1.0p9 nicht.
...
// TK: Hier kommt der neue Abschnitt rein: === Host beim Server automatisch registrieren lassen
...
// TK: Hier kommt der neue Abschnitt rein: === Umstellung zwischen Push- und Pull-Modus
...
// TK: Das mit der nicht vorhandenen Datei klingt schon ziemlich merkwürdig. Ich habs aber nicht getestet.
...
Des Weiteren können Sie auf dem Host einen Blick in die Logdatei `C:\ProgramData\checkmk\agent\log\check_mk.log` werfen (Pfade können abweichend konfiguriert sein). 
...
Wurde ein Host für die xref:hosts_autoregister#rule_autoregister_bakery[Autoregistrierung] in der {CCE} mit dem Regelsatz [.guihint]#Agent controller auto-registration# konfiguriert und die Option [.guihint]#Keep vanished connections# auf [.guihint]#no# gesetzt, werden bei jedem Neustart des Dienstes `cmk-agent-ctl-daemon` (beispielsweise beim Neustart eines Hosts) alle anderen Verbindungen entfernt -- außer der für Autoregistrierung konfigurierten Verbindung.
...
Eine dauerhafte Änderung über ein Update der Agentenpakete hinweg erreichen Sie, indem Sie im oben genannten Regelsatz [.guihint]#Keep vanished connections# auf [.guihint]#yes# setzen.
