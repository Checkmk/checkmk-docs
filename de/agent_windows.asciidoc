...
Firewall an, damit er über den Port 6556 von aussen erreichbar ist. Sollten Sie
...
Host überhaupt keine Informationen bzw. nur Fehlermeldung über Timeouts
erhalten, so sollten Sie auf dem Host die [.guihint]#Inbound Rules# der Windows
Firewall prüfen.
...
Daraufhin öffnet sich ein Wizard für die Erstellung einer neuen Firewall-Regel.
...
| Profile | Dieser Punkt kommt start auf diese Konfiguration Ihres Netzwerks an. Es empfiehlt sich allerdings in den meisten Fällen hier nur [.guihint]#Domain# und [.guihint]#Private# zu aktivieren.
...
In älteren Versionen von Windows finden Sie einen Eintrag in der Systemsteuerung unter Programme > Programme deinstallieren. In neueren Versionen finden Sie den Eintrag für den {CMK}-Agenten
in den Einstellungen unter Apps.
