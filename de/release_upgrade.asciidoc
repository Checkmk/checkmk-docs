...
Die unterschiedliche Methoden mit Ihren jeweiligen Voraussetzungen stellen wir hier vor.
...
Selbstverständlich sind auch Kombinationen der vorgestellten Methoden möglich, zum Beispiel, wenn Sie `/opt/omd` oder `/opt/omd/sites` auf einem eigenen Mount-Punkt verwenden.
...
Diese Methode (englisch _in-place_ oder lateinisch _in situ_) ist oft beim Einsatz von {CMK} auf dedizierter Hardware mit großen Datenbeständen sinnvoll, wo Backup und Restore die Ausfallzeit deutlich erhöhen würde.
...
[#exsitu]
== Backup, Neuinstallation und Restore _(ex situ)_
...
. Führen Sie unter der alten Distributionsversion ein xref:omd_basics#omd_backup_restore[Backup] der Instanz durch.
...
. Führen Sie nun als Instanzbenutzer die Wiederherstellung der Sicherung durch:
