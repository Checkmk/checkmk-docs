...
xref:update_matrix#[Update-Matrix für Version {v22}]
{related-end}
...
Wenn Sie ein Release-Upgrade der Linux-Distribution mit einem xref:update_major#[Versions-Update] kombinieren wollen oder müssen (zum Beispiel wenn eine ältere {CMK}-Version mehr nicht für eine neuere Distributionsversion angeboten wird), bestimmen Sie bitte anhand unserer xref:update_matrix#[Update-Matrix] die geeignete Reihenfolge der durchzuführenden Schritte.
