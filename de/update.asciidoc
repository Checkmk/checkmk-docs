...
Wenn Sie xref:monitoring_aws#[Amazon Web Services (AWS),] xref:monitoring_azure#[Microsoft Azure] oder xref:monitoring_gcp#[Google Cloud Platform (GCP)] überwachen, führen Sie eine Service-Erkennung betroffener Hosts durch, um nur in der {CE} verfügbare Dienste zu finden.
