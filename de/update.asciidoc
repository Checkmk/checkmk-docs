...
Aber auch, wenn Sie bereits eine {v20}-Version installiert haben und auf eine neue stabile Patch-Version der {v20} updaten wollen, können die in den folgenden Abschnitten beschriebenen Themen relevant sein.
...
// TK: Ich würde das Unterkapitel von 2.3 nach 2.1 verschieben - wg. Wichtigkeit und Logik im Ablauf
...
Wenn Sie also beispielsweise von {v15} auf {v20} aktualisieren wollen, aktualisieren Sie zunächst auf die {v16}.
...
{c-omd} omd -f update
----
// TK: Ich würde kein Kommando zeigen, dass man nicht ausführen soll.
// TK: Wenn schon was zeigen, dann evtl. die Fehlermeldung: ERROR: You are trying to update from 1.6.0p19.cre to 2.1.0b1.cre which is not supported.
// TK: Muss man -f überhaupt erwähnen? Wenn man es (ohne -f) versucht, steht die Option ja in der Kommandoausgabe.
...
// TK: Dass es nach einem Downgrade keinen Support mehr gibt, steht zwar nicht im Werk, aber tatsächlich auf der Kommandozeile ;-)
// TK: Wäre schön, wenn man in das Achtung von oben auch den Downgrade reinzwängen könnte, etwa so:
// TK: Wir unterstützen keine Instanzen, bei der durch ein erzwungenes Update Major-Versionen ausgelassen worden oder die auf eine ältere Major-Version aktualisiert wurden.
