...
installierte link:cmk_versions.html[Software-Versionen.] Dabei ist jede Instanz
...
Falls Sie einen Update der {CMK}-Version {v16} auf {v20} planen, sollten Sie zuerst den Artikel xref:update_20#[Update auf Version 2.0.0] lesen, in dem wir die wichtigsten Themen zusammengetragen haben, die Sie vor und nach einem solchen Update beachten sollten.

//SK: Der Abschnitt "Vorbemerkung" ist im Zuge von KNW-319 an dieser Stelle in den Artikel eingefügt worden. Das ganze jetzt wieder komplett in einen Artikel auszulagern, der eigentlich nur ein ganz bestimmtes Update (das von 1.6 nach 2.0) beschreibt, finde ich nicht gut. Die Behandlung lokaler Anpassungen *können* bei *jedem* Update eine Rolle spielen. Für mich würde eher andersherum ein Schuh draus. In update_20 sollte das grob angerissen werden und dann ggf. hier en detail beschrieben werden.

Aber auch, wenn Sie bereits eine {v20}-Version installiert haben und auf eine neue stabile Patch-Version der {v20} updaten wollen, können einige der im Artikel genannten Punkte relevant sein, zum Beispiel, das Kapitel zu xref:update_20#local_files[lokalen Dateien] Ihrer Instanz.
...
Bestätigen Sie in der folgenden Dialogbox den gewählten Update auf die neue Version:
...
. Nachbearbeitungen durch verschiedene Pakete (z.B. {CMK}). Insbesondere werden automatisch die xref:wato#[Änderungen aktiviert], um eine valide Konfiguration für den Kern zu erzeugen.
...
// TK: Update-Beschreibung aus managing_docker rübergeholt und die update_complex Beschreibung für Version kleiner als {v15}p13 gelöscht.
// SK: Passt.
...
=== Alarmierungs-Spooler

Die {RE} hat keinen Alarmierungs-Spooler. Deswegen ist dieser nach dem Umstieg auf
...
* Es befinden sich inkompatible Skripte im lokalen Teil der Instanzverzeichnisstruktur, d. h. in `~/local`.

Im Artikel zum Update der {CMK}-Version {v16} auf {v20} finden Sie Hinweise zur Handhabung xref:update_20#incompatible[inkompatibler Änderungen], xref:update_20#local_files[lokaler Dateien] und
xref:update_20#incompatible_mkps[inkompatibler und obsoleter MKPs].
