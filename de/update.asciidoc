...
Auf der Kommandozeile können Sie sich installierte Erweiterungen mit dem Befehl `mkp find` anzeigen lassen.
...
Um die phasenweise Aktualisierung zu erleichtern, bietet {CMK} seit {v21}p21 die Möglichkeit, gleichnamige Erweiterungspakete in verschiedenen Versionen – eines passend zur älteren Zentralinstanz, eines passend zu den neueren Remote-Instanzen – zu hinterlegen.
...
Fällt mit der Umstellung der Trennung {CFE}/{CSE} respektive Überführung in eine _mit Lizenzschlüssel_ betriebene {CSE} und die auf 750 Services beschränkte Version ohne Lizenzschlüssel weg.

Dieses Upgrade wird daher im Artikel xref:update_major#[Aktualisierung auf {v22}] abgefrühstückt.
...
Sie xref:distributed_monitoring#notifications[hier.]
...
Daneben sind unter Umständen nicht mehr alle Dashboards, Graphen-Einstellungen, Benachrichtigungs-Plugins, Spezialagenten, yadayadayada verfügbar.
Anhand dieser Tabelle (in Arbeit) können Sie ermitteln, wieviel {SE}-Funktionalität im Falle des Downgrades auf {RE} wegfällt.
