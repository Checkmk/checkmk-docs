...
// IGNORE Benachrichtigungs Checkmks Ctrl+D erstmal mitausgelieferten pX p21 unpaketierte
// IGNORE zurückgerollt Defaultversion Instanzverwaltung Instanzverzeichnisstruktur Vorgabedateien
// IGNORE Benachrichtigungssystem p22 wegfällt
...
Wenn Sie gleichzeitig zum Versions-Update ein Editions-Upgrade von einer {RE} auf eine der {CEE} durchführen, werden Sie sicherheitshalber noch
...
|*y* |Dies ist ähnlich der vorherigen Funktion, zeigt aber ausgehend von der früheren Defaultversion, welche Änderungen Sie an der Datei gemacht haben.
...
[#updatedistributed]
== Aktualisierung in verteilten Umgebungen
...
Ist dies nicht möglich – beispielsweise, weil die Umgebung auf Sites in verschiedenen Zeitzonen und betreuenden Teams verteilt ist – kann unter strengen Auflagen ein vorübergehender Mischbetrieb erfolgen.
...
Die Zentralinstanz greift auf die Daten der Remote-Instanzen primär lesend via Livestatus zu.
...
Upgrades der {CRE} auf eine der {CEE} oder von einer {EE} auf eine andere mit größerem Funktionsumfang sind jederzeit unkompliziert möglich.
...
An ihre Stelle trat die Möglichkeit, die {CSE} mit bis zu 750 Services ohne Lizenzschlüssel zu betreiben.
Statt eines Upgrades muss beim Überschreiten der Schwelle lediglich ein Lizenzschlüssel hinterlegt werden.

Das Upgrade {FE} {v21} auf {EE} {v22} wird daher vom Artikel xref:update_major#[_Aktualisierung auf {v22}_] vollständig abgedeckt.
...
Führen Sie in diesem Fall das xref:#cee2cre[Downgrade nach {RE}] +
_vor_ dem Update auf {v22} durch.
...
Sie können in einem Schritt zur {CE} upgraden, beachten Sie in diesem Fall jedoch auch die Hinweise im folgenden Kapitel.

// Da der {RE} etliche Module und Features der {EE} fehlen, gibt es nach der Umstellung ein paar Dinge zu beachten. 
//SP: Könnte man den Satz auch positiv formulieren, ohne den Inhalt zu verfälschen? Also "Da die {EE} etliche zusätzliche Module und Features hat, ..."
Da die {EE} etliche zusätzliche Module und Features hat, gibt es nach der Umstellung ein paar Dinge zu beachten.
...
Sie xref:distributed_monitoring#notifications[im Artikel zum verteilten Monitoring].
...
Je nach Schwerpunkt des Einsatzes werden Sie den umfangreicheren Funktionsumfang oft erst beim Hinzufügen neuer Hosts nutzen.
An einigen Stellen ist dennoch die Überprüfung der vorhandenen Einstellungen angeraten.

Eine vollständige Übersicht der zusätzlichen Funktionalität bietet der xref:cce#[Handbuch-Artikel zur {CE}].

==== Check-Plugins für Cloud-Dienste

Wenn Sie xref:monitoring_aws#[Amazon Web Services (AWS),] xref:monitoring_azure#[Microsoft Azure] oder xref:monitoring_gcp#[Google Cloud Platform (GCP)] überwachen, führen Sie eine Service-Erkennung betroffender Hosts durch, um nur in der {CE} verfügbare Dienste zu finden.
...
. Stellen Sie vorhandene Push-Hosts xref:agent_linux#changepush[auf Pull-Modus um].
Andernfalls empfängt {CMK} von diesen keine Monitoring-Daten und betroffene Hosts nehmen den Status {UNREACH} an.
...
Ferner stehen nicht mehr alle Spezialagenten für Cloud-Dienste und weniger Dashboards zur Verfügung.
Als Folge werden Sie xref:wato_services#vanished[verschwundene Dienste (_vanished services_)] aufräumen müssen.
...
. Änderung des RRD-Datenbankformates (Configuration of RRD databases of hosts) auf Multiple RRDs per host/service. Neben leichten Performance-Nachteilen ist hier anzumerken, dass eine Konvertierung nicht vorgesehen ist, demnach historische Monitoring-Daten nicht mehr sichtbar sind.
. Umschalten des Monitoring-Cores von CMC auf Nagios – von dieser Änderung sind in erster Linie Performance-Nachteile zu erwarten.
