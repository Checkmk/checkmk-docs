// -*- coding: utf-8 -*-
// IGNORE Benachrichtigungs Ctrl+D XYZ p22 Subskriptionsmanagements
include::global_attr.adoc[]
= Updates und Upgrades
:revdate: 2023-05-11
:title: Updates und Upgrades
:description: Egal, ob {RE} oder eine der {EE}: {CMK} zu aktualisieren ist so einfach, wie möglich. Die Prinzipien dazu werden hier erklärt.
...
Falls Sie ein Update der {CMK}-Version {v21} auf {v22} planen, sollten Sie zuerst den Artikel xref:update_major#[Update auf Version 2.2.0] lesen, in dem wir die wichtigsten Themen zusammengetragen haben, die Sie vor und nach einem solchen Update beachten sollten.

Aber auch, wenn Sie bereits eine {v22}-Version installiert haben und auf eine neue stabile Patch-Version der {v22} updaten wollen, können die in den folgenden Abschnitten beschriebenen Themen relevant sein.
...
Wenn Sie also beispielsweise von {v20} auf {v22} aktualisieren wollen, aktualisieren Sie zunächst auf die {v21}.
...
Ein Beispiel für den kompletten Befehl für ein automatisches Update der Instanz `mysite` auf die Version `2.2.0p1.cee`:
...
An deren Stelle kann nun die {CCE} ohne Lizenzierung betrieben werden, sofern mit ihr maximal 750 Services in einer Instanz betrieben werden.
Statt eines Upgrades muss beim Überschreiten der Schwelle lediglich ein Lizenzschlüssel hinterlegt werden.

Das Upgrade der {FE} {v21} wird daher vom Artikel xref:update_major#editions[Update auf Version {v22}] vollständig abgedeckt.
...
Eine Umstellung auf den CMC erfolgt mit `omd config` und wird in dem Artikel zur xref:cmc_migration#[Migration auf den CMC] beschrieben.
...
Bitte beachten Sie, dass in verteilten Umgebungen immer zuerst das Update der Version durchgeführt werden muss, bevor das Upgrade der Edition folgen kann.
Eine andere Reihenfolge oder ein _Crossgrade_ (Upgrade der Edition und Update der Version in einem Schritt) ist nicht vorgesehen.

[#cre2cee_distributed]
==== {RE} nach {SE} (oder {CE})

{CMK} unterstützt offiziell keinen Mischbetrieb zwischen der {RE} und den kommerziellen Editionen.
Aus diesem Grund empfehlen wir dringend, das Upgrade der Edition _offline_ durchzuführen.
...
. Nun steht das Upgrade der Remote-Instanzen an. Dieses können Sie parallel durchführen und Sites gleich nach dem Upgrade wieder starten.
...
Der Funktionsumfang der {CE} ist eine echte Obermenge des Funktionsumfangs der {SE}, weshalb die größte Herausforderung beim Upgrade der Edition in der Handhabung des unterschiedlichen xref:license#[Lizenz- und Subskriptionsmanagements] liegt.
Das Upgrade kann hier _online_ stattfinden, d.h. es ist für einen kurzen Zeitraum ein Mischbetrieb möglich.
Hierfür gilt es, die folgende Reihenfolge einzuhalten und das gesamte Upgrade innerhalb von 30 Tagen abzuschließen:

. Führen Sie das Upgrade der Zentralinstanz durch.
Diese wird dadurch in den 30 Tage laufenden Lizenzstatus „Trial“ der {CE} versetzt.
In diesem Modus existiert zum einen keine Limitierung der Anzahl der Services, zum anderen keine Einschränkung der Kommunikation mit anderen Editionen.
Remote-Instanzen verwenden weiterhin die vorhandene Lizenzierung.
. Aktualisieren Sie nach und nach die Remote-Instanzen.
Jede dieser wird nun ebenfalls in den 30 Tage laufenden Lizenzstatus „Trial“ der {CE} versetzt.
. Je nachdem, welche Cloud-spezifischen Dienste in Ihrer Umgebung benötigt werden, deaktivieren Sie ggf. nicht benötigte Dienste und ermitteln Sie in diesem Zug den Umfang der benötigten Services.
// TK: Der Satz lässt mich etwas ratlos zurück: ich soll Cloud-spezifischen Dienste deaktivieren nachdem ich zur CE gewechselt bin?
// TK: Braucht es das überhaupt: Das Servicezählen für den Subskriptionsumfang hatten wir bisher noch nicht im Handbuch. Ist das wirklich nötig? Selbst wenn man sich verzählt hat, meldet CMK doch eh regelmäßig die Zahlen und dann wird nachberechnet (wenn ichs recht verstanden habe).
. Stellen Sie Ihre Subskription auf {CE} mit der benötigten Zahl an Services um und hinterlegen Sie die Subskriptionsdaten in {CMK}.
...
Bitte beachten Sie, dass in verteilten Umgebungen immer zuerst das Update der Version durchgeführt werden muss, bevor das Downgrade der Edition folgen kann.
Eine andere Reihenfolge oder ein _Crossgrade_ (Downgrade der Edition und Update der Version in einem Schritt) ist nicht vorgesehen.

{CMK} unterstützt offiziell weder Mischbetrieb zwischen der {RE} und den kommerziellen Editionen, noch den Mischbetrieb einer {SE} Zentralinstanz mit {CE} Remote-Instanzen.
...
. Wenn Sie möchten (und eine Fülle von Benachrichtigungen kein Problem darstellt), können Sie die Zentralinstanz sofort wieder starten.
. Nun steht das Downgrade der Remote-Instanzen an. Dieses können Sie parallel durchführen und Sites gleich nach dem Downgrade wieder starten.
