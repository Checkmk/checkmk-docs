...
// {CMK} unterstützt offiziell weder einen Mischbetrieb zwischen der {RE} und den kommerziellen Editionen noch den Mischbetrieb einer {SE} Zentralinstanz mit {CE} Remote-Instanzen.
// TK: Obwohl es passt, wenn man sich noch an das obige Kapitel zum Upgrade erinnert, liest sich der Satz schon sehr sperrig. Wie wärs damit:
// In allen Downgrade-Szenarien unterstützt {CMK} keinen Mischbetrieb zwischen unterschiedlichen Editionen.
// MFS:
