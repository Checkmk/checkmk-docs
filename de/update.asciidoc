...
Um vollen Nutzen aus der {SE} ziehen zu können, beachten Sie die xref:cse#[Übersicht der Unterschiede zwischen {RE} und {SE}].
...
Falls Sie unter der {CE} das xref:grafana#[Grafana-Plugin] aus dem Store genutzt haben, müssen sie dieses durch ein aus dem Zip-Archiv installiertes ersetzen.
...
Anhand xref:cse#[dieser Übersicht] können Sie ermitteln, wie viel {SE}-Funktionalität im Falle des Downgrades nach {RE} wegfällt und wo Sie gegebenenfalls weitere Anpassungen vornehmen müssen.
