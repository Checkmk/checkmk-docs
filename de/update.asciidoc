...
Im Idealfall sind lokale Dateien bereits vollstädig in xref:#incompatible_mkps[MKPs] gebündelt.
...
Statt eines Upgrades muss bei überschreiten der Schwelle lediglich ein Lizenzschlüssel hinterlegt werden.

Das Upgrade {FE} {v21} auf {EE} {v22} wird daher vom Artikel xref:update_major#[_Aktualisierung auf {v22}_] vollständig abgedeckt.

Sollte in ihrem Fall die Limitierung auf 25 Hosts der {FE} ausreichend gewesen sein, aber 750 kostenlose Services genügen nicht, so haben Sie neben Erwerb einer Subskription die Möglichkeit des Downgrades auf die {RE}.
...
Anhand xref:draft_feature_table#[dieser Übersicht] können Sie ermitteln, wieviel {SE}-Funktionalität im Falle des Downgrades auf {RE} wegfällt und wo sie gegebenenfalls weitere Anpassungen vornehmen müssen.
