...
// IGNORE Benachrichtigungs Checkmks Ctrl+D Versionsunterscheid erstmal mitausgelieferten pX unpaketierte
...
So wurde die xref:web_api#[Web-API] intern bereits von {CMK} {v21} nicht mehr benutzt, die Entfernung erfolgte aber erst in Version {v22}.
...
Um die phasenweise Aktualisierung zu erleichtern bietet {CMK} seit {v21}pX die Möglichkeit, gleichnamige Erweiterungspakete in verschiedenen Versionen – eines passend zur älteren Zentralinstanz, eines passend zu den neueren Remote-Instanzen zu hinterlegen.
