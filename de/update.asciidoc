...
:revdate: 2020-06-16
:title: {CMK} updaten
:description: Egal ob {FE}, Open-Source- oder Enterprise Edition: {CMK} zu aktualisieren ist so einfach, wie möglich. Die Prinzipien dazu werden hier erklärt.
...
[{shell}]
----
{c-root} apt install /tmp/check-mk-enterprise-1.6.0p13_0.focal_amd64.deb
----
...
Die Aktualisierung von {CMK} ab Version {v15}p13 lässt sich ebenfalls
sehr einfach bewerkstelligen und wird in dem Artikel
link:managing_docker.html#update_easy[{CMK}-Server im Docker-Container]
beschrieben.

Sollten sie einen {CMK}-Container vor Version {v15}p13 updaten wollen,
sind einige zusätzliche Schritte notwendig die in
link:managing_docker.html#update_complex[Detailliertes Update für frühere {CMK}-Images]
beschrieben werden.
