...
// IGNORE Benachrichtigungssystem p22 wegfällt XYZ
...
An dieser Stelle relevant ist, dass ein reguläres Backup auch gute Dienste für den Trockenlauf eines Updates leisten kann.
So können Sie das Backup unter einem anderen Namen wieder herstellen – und anschließend mit der Site `newsite` das Update testen:
...
Wenn Sie xref:monitoring_aws#[Amazon Web Services (AWS),] xref:monitoring_azure#[Microsoft Azure] oder xref:monitoring_gcp#[Google Cloud Platform (GCP)] überwachen, sind die der {CE} vorbehaltenen Dienste bestehender Hosts zunächst nicht aktiviert.
...
Führen Sie anschließend eine Service-Erkennung bei diesen Hosts durch, um die nun verfügbaren Dienste zu finden.
...
Ferner stehen nicht mehr alle Cloud-Dienste und weniger Dashboards zur Verfügung.
