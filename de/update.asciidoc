...
Falls Sie einen Update der {CMK}-Version {v16} auf {v20} planen, sollten Sie zuerst den Artikel xref:update20[Update auf Version 2.0.0] lesen, in dem wir die wichtigsten Themen zusammengetragen haben, die Sie vor und nach einem solchen Update beachten sollten.
...
image::update_help_menu_incompatible_werks.png[width=75%]
...
// TK: Referenz auf {v15}p13 rausgenommen
Die Aktualisierung von {CMK} lässt sich sehr einfach bewerkstelligen und wird im Artikel link:managing_docker.html#update_easy[{CMK}-Server im Docker-Container] beschrieben.

////
Sollten sie einen {CMK}-Container vor Version {v15}p13 updaten wollen, sind einige zusätzliche Schritte notwendig die in link:managing_docker.html#update_complex[Detailliertes Update für frühere {CMK}-Images] beschrieben werden.
////
...
möglich. Dabei gelten die gleichen Grundsätze wie bei einem xref:update#detailed[normalen Update von {CMK}].
...
Im Artikel zum Update der {CMK}-Version {v16} auf {v20} finden Sie Hinweise zur Handhabung xref:update20##incompatible[inkompatibler Änderungen], xref:update20#local_files[lokaler Dateien] und
xref:update20#incompatible_mkps[inkompatibler und obsoleter MKPs].
