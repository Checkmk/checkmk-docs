...
:description: Mit (CMK) können Sie ein vCenter, seine ESXi-Hosts und auch die VMs ohne extra Agenten überwachen. Einzelheiten zu der Konfiguration sind hier beschrieben.

[.relateddoc]
wato_monitoringagents, cma_virt1_basic_esx, datasource_programs
...
link:check_plugins_catalog.html[Katalog der Check-Plugins] unter
...
* Sie haben den ESXi-Server als Host im {CMK} angelegt und als Agent [.guihint]#Check_MK Agent# eingestellt. Tipp: Wählen Sie den Hostnamen so, wie der Server sich selbst auch kennt.

Sind die Voraussetzungen erfüllt, können Sie eine
link:wato_rules.html#create_rules[Regel]
im Regelsatz [.guihint]#Datasource programs|Check state of VMWare ESX via vSphere# erstellen.  Diese wird dem angelegten
Host zugewiesen, damit der link:wato_monitoringagents.html#special_agent[Spezial-Agent]
für die VMware-Überwachung statt des normalen Agenten für die Abfrage der Daten benutzt
wird.

image::vmware_rulesearch.png[]
...
link:wato_hosts.html#services[Serviceerkennung] durch. Dabei sollte eine Reihe
...
link:monitoring_vmware.html#diagnose[Diagnosemöglichkeiten] nach Fehlern in der
...
|Einfacher Aufzusetzen in Situationen, in denen die Zuordnung der VMs über vMotion dynamisch geschieht.
|Keine Überwachung wenn vCenter nicht verfügbar ist.

|Überwachung der Gesamtnutzung des RAM im Cluster möglich.
|Keine Überwachung von sonstigen hardwarespezifischen Daten der Clusterknoten (z.B. RAM-Disks und Netzwerkkarten).
...
ist es, die [.guihint]#Datastores}} und die {{Virtual Machines# über das vCenter
...
mit Hilfe der Regel [.guihint]#Access to agents|Hostname translation for piggybacked hosts# geschehen.
...
link:monitoring_vmware.html#datasource_combination[Daten] aktiviert ist.
...
verschiedene Weisen link:monitoring_vmware.html#rename_piggyback[anpassen].
...
|Der Spezial-Agent, um die Abfrage von ESXi- und vCenter-Servern auszuführen. Dieses Skript kann zu Testzwecken auch manuell ausgeführt werden.
