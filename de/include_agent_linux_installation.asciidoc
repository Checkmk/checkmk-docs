...
// TK: Wie Sie dabei vorgehen, ist im Artikel xref:agent_linux_legacy#[Linux überwachen im Legacy-Pull-Modus] beschrieben.
...
Anders liegt der Fall, wenn der Agent sich während der Installation nicht an `systemd` registrieren konnte, weil dieser Superserver auf dem Host nicht vorhanden ist.
