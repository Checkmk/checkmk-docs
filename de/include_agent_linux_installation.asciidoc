...
Damit die Registrierung des Agent Controllers klappt, setzt dies eine Linux-Distribution mit `systemd` voraus,
...
Falls `systemd` installiert ist und läuft, wie es die obige Kommandoausgabe zeigt, installieren Sie das Agentenpaket und führen anschließend die xref:registration[Registrierung] durch, um mit dem Agenten im Pull-Modus zu arbeiten.

Falls `systemd` nicht laufen sollte, so können Sie den Agenten trotzdem installieren, wie im Rest dieses Kapitels beschrieben, und ihn dann im Legacy-Pull-Modus nutzen.
...
Im Monitoring sieht das dann so aus:
...
Die {CMK} Instanz erkennt an der Agentenausgabe, dass der Agent Controller vorhanden und damit die TLS-Verschlüsselung möglich, aber noch nicht eingeschaltet ist.
...
Anders liegt der Fall, wenn der Agent sich während der Installation nicht an `systemd` koppeln konnte, weil dieser Superserver auf dem Host nicht vorhanden ist.
Als Ersatz für `systemd` versucht die Installationsroutine `xinetd` zu konfigurieren.
Da der Agent Controller aber `systemd` voraussetzt, ist die Registrierung nicht möglich und der einzige Kommunikationsweg verbleibt der Legacy-Pull-Modus.
Lesen Sie in diesem Fall im Artikel xref:agent_linux_legacy#[Linux überwachen im Legacy-Pull-Modus] weiter.
