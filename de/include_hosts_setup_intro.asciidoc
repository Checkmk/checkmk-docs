...
* Host-Merkmale (_host tags_) und eine darauf aufbauende xref:wato_rules#[regelbasierte] Konfiguration.
* xref:wato_services#[Automatische Erkennung] der zu überwachenden Services.

Generell hat es sich bewährt, sich als erstes ein Ordnungssystem zu überlegen und dieses danach mit Inhalt zu füllen. Konkret bedeutet das also, sich erst mit dem Aufbau der Ordner und ihrer Hierarchie zu befassen und danach die Hosts "einzusortieren".