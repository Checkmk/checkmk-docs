...
Eine Ausnahme wäre, wenn Sie einen Host _gleichzeitig_ mit dem normalen {CMK}-Agenten _und_ SNMP überwachen möchten.
Dafür gibt es gelegentlich einen Grund, etwa wenn Sie auf einem Server einen Herstelleragenten für die Hardwareüberwachung installiert haben, der seine Daten per SNMP bereitstellt, wie das z.B. bei Fujitsu ServerView der Fall ist.
...
Für die Konfiguration der SNMP-Credentials gibt es noch einen alternativen Weg, falls Sie diese nicht einfach über ihre Ordnerstruktur vererben können: den xref:wato_rules#[Regelsatz] [.guihint]#[.guihint]#Setup > Agents > SNMP rules > SNMP credentials of monitored hosts#. 
