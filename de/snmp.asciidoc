...
Traps haben viele Nachteile gegenüber aktiven Anfragen, weswegen sie für das Monitoring keine große Bedeutung
...
image::host_snmp_configuration.png[alt="Aufnahme eines Hosts ins Monitoring per SNMP."]
...
anhand von Host-Merkmalen, xref:labels#[Labels] und ähnlichen Eigenschaften vergeben. Dabei gilt
...
Nach dem Speichern der Host-Eigenschaften (und optional der Diagnose) ist wie
...
Service-Erkennung intern ganz anders als bei Hosts, die mit dem {CMK}-Agenten
...
Kandidaten für die eigentliche Service-Erkennung dienen.
...
Switch Ports, Festplatten, Sensoren, VPNs usw.), aber keine ganz neuen _Plugins._
...
der Service-Erkennung aktiv sind (operational status „up“). Sie können
...
xref:intro_finetune#switchports[Konfigurieren von Switch Ports.]
...
sind in der Diagnose wie tot. Und auch in der Service-Erkennung werden sie
...
Eine zweite Ursache kann sein, dass das Gerät sehr viele Switch Ports hat und
...
und Sie dann nur die besagten Standard-Services überwachen können. Hier
...
die Community eingestellt haben, also entweder bei den Host-Eigenschaften oder im Regelsatz
...
Paket. Mit dem experimentellen Regelsatz [.guihint]#Bulk walk: Number of OIDs per bulk#
...
zu allen Switch Ports holt, auch wenn nicht alle überwacht werden. Das
...
Für solche Fälle gibt es den Regelsatz [.guihint]#Bulk walk: Limit SNMP OID Ranges.# Mit
...
Der übliche Check-Typ für Switch Ports heißt [.guihint]#SNMP interface check with 64 bit counters.#
...
Ping für den Host Check und eventuell konfigurierte aktive Checks. Diese
