...
* Legen Sie in der Regelkette link:.guihint].html#Access to agents[Legacy SNMP devices using SNMP v2c# eine Regel für den Host an und stellen in der Regel den Wert auf [.guihint]#Positive match (Add matching hosts to the set)#.
