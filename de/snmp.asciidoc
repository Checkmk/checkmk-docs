...
der kurze Abschnitt über SNMP im xref:intro_setup_monitor#snmp[Leitfaden für Einsteiger.] Dieser
...
Aber SNMP hat noch eine zweite Spielart: _SNMP-Traps._ Das sind spontane
Meldungen, die Geräte an konfigurierte Adressen per UDP (Port 162) im Push-Verfahren versenden. Traps
...
Falls Sie dennoch mit Traps arbeiten wollen oder müssen, bietet die xref:ec#snmp[Event Console] dafür eine Lösung: Sie kann Traps empfangen und daraus Events generieren.
...
. Tragen Sie als erlaubte IP-Adressen die Adressen Ihrer {CMK}-Server ein. Es kann auch nützlich sein, hier noch eine Testinstanz von {CMK} vorzusehen. Wichtig: Falls Sie mehrere redundante {CMK}-Server betreiben, vergessen Sie nicht, auch die IP-Adressen anzugeben, die nach einem Failover verwendet werden. Speziell bei der {CMK}-Appliance ist es so, dass diese im Cluster-Betrieb als Quell-IP-Adresse bei ausgehenden Verbindungen jeweils die IP-Adresse des aktiven Nodes verwendet -- und nicht die Service-IP-Adresse. In einer xref:distributed_monitoring#[verteilten Umgebung] ist die IP-Adresse derjenigen Remote-Instanz entscheidend, von der aus das Gerät überwacht wird.
...
`public` lautet. Das ist bei vielen Geräten -- und auch bei {CMK} -- der Standard. Nun kann
...
(außer bei xref:snmp#v3[SNMP Version 3]). Jeder, der Pakete mithören kann,
...
zu einer sehr umständlichen Handhabung. Denn diese müssen ja dann nicht nur in
...
image::host_snmp_configuration.png["Screenshot: Aufnahme eines Hosts ins Monitoring per SNMP im Abschnitt Monitoring Agents"]
...
einen Host _gleichzeitig_ mit dem normalen {CMK}-Agenten _und_ SNMP überwachen
...
auf dem Gerät dafür defekt ist (dies kann in der Praxis vereinzelt vorkommen). SNMP in Version 1 ist vor allem sehr langsam, da sie keine Bulk-Zugriffe unterstützt – der Unterschied ist gravierend.

Die dritte Einstellung heißt [.guihint]#SNMP credentials#. Hier ist
zunächst wieder eine Wahl der Protokollversion notwendig, da sich v2c und
v3 voneinander unterscheiden. Die Version 3 erläutern wir
xref:snmp#v3[weiter unten]. Wenn Sie nicht sehr hohe Sicherheitsanforderungen haben, liegen
...
diese nicht einfach über ihre Ordnerstruktur vererben können: den xref:wato_rules#[Regelsatz]
[.guihint]#Access to Agents > SNMP credentials of monitored hosts#. Damit können Sie die Credentials
anhand von Hostmerkmalen, xref:labels#[Labels] und ähnlichen Eigenschaften vergeben. Dabei gilt
...
Diagnoseseite an. Dazu speichern Sie mit dem Menüeintrag [.guihint]#Save & go to connection test.# Hier ist
...
image::snmp_diagnostics.png["Screenshot: Ausgabe der SNMP v2c Diagnose"]
...
|sysDescr |Die Beschreibung des Geräts, wie sie vom Hersteller in der Firmware fest eingebrannt ist. Dieser Text ist für {CMK} sehr wichtig für die automatische Serviceerkennung.
...
gewohnt der nächste Schritt die xref:wato_services#[Konfiguration der Services.]
...
überhaupt unterstützt. Diese Phase nennt {CMK} den _SNMP-Scan._
...
Daher überspringt das Setup den Scan im Normalfall und macht die Erkennung
...
image::snmp_standard_services.png[alt="Screenshot: Anzeige der drei Standarddienste, die jedes SNMP Gerät aufweisen sollte"]
...
Im Leitfaden für Einsteiger finden Sie übrigens ein Kapitel mit Handlungsempfehlungen zum 
xref:intro_finetune#switchports[Konfigurieren von Switchports.]
...
Und schließlich gibt es den Service [.guihint]#Uptime,# der Ihnen zeigt, wann das Gerät zum letzten
...
[.guihint]#Setup > Agents > SNMP rules > Hosts without system description OID# mit der Option [.guihint]#Positive match (Add matching hosts to the set#
...
* Legen Sie in der Regelkette [.guihint]#Setup > Agents > SNMP rules > Legacy SNMP devices using SNMP v2c# eine Regel für den Host an und stellen Sie in der Regel den Wert auf [.guihint]#Positive match (Add matching hosts to the set)#.

Dadurch wird der Host gezwungen, trotz eingestellter Version 1 das Protokoll
SNMP v2c zu verwenden, allerdings _ohne Bulkwalk._  Wir empfehlen
...
langsamer ist, aber manchmal immer noch schneller als ein kaputtes
...
Sie weiter unten im Abschnitt zu xref:snmp#performance[Performance.]
...
* Sie entwickeln selbst Plugins. Dazu finden Sie im Handbuch xref:devel_check_plugins#[mehrere Artikel.]
...
Bei einigen wenigen Geräten müssen Sie SNMP v3 und xref:snmp#contexts[Kontexte] verwenden,
...
die Community eingestellt haben, also entweder bei den Hosteigenschaften oder im Regelsatz
...
image::snmp_credentials_v3.png[alt="Screenshot: Konfiguration der SNMP v3 Sicherheitseinstellungen"]
...
Hosts. Und die Überwachung mit SNMP benötigt mehr CPU und Speicher als die
mit {CMK}-Agenten.
...
die SNMP-Anfragen sehr performant durchführt, ohne weitere Prozesse zu erzeugen. Die CPU-Last
...
// MFS: Komplett auskommentiert, aber zunächst als Referenz dringelassen – neimand möchte CMK ohne Not langsamer machen
...
image::snmp_timing_settings.png[alt="Screenshot: Erhöhen des Response timeout"]
...
Für solche Fälle gibt es die Regelkette [.guihint]#Bulk walk: Limit SNMP OID Ranges.# Mit
...
Der übliche Check-Typ für Switchports heißt [.guihint]#SNMP interface check with 64 bit counters.#
Folgendes Beispiel zeigt eine Einstellung, bei der nur die ersten beiden Ports
per SNMP geholt werden:

image::snmp_limit_oid_ranges.png[alt="Screenshot: Bulk walk: limit SNMP OID ranges"]

*Hinweis:* Diese Filterung findet dann quasi _vor_ der Serviceerkennung und dem Monitoring statt.
Je nach Einstellung von [.guihint]#Network interface and switch port discovery# bedeutet das noch nicht automatisch,
...
um die Überwachung des Geräts auf einem anderen {CMK}-Server zu simulieren, auch
...
image::snmp_download_walk.png[alt="Screenshot Download des SNMP Walks im Kontextmenü des Hosts in der Monitoring-Übersicht"]
