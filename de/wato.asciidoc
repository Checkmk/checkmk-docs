...
xref:dcd#[dynamische Host-Konfiguration], die
...
|[.guihint]#Services# |Die xref:wato_services#[Verwaltung der Services], die auf den überwachten Hosts laufen. Hier finden Sie die
...
Hier gibt es die
xref:wato_monitoringagents#agents[{CMK}-Agenten] für Linux, Windows und andere Betriebsysteme zum Download und in den {CEE} Zugang zur 
...
|[.guihint]#Events# |Die Konfiguration von Meldungen zu Ereignissen -- mit Regeln für
...
xref:wato_user#[Benutzer, Rollen und Berechtigungen]. Sie können die Listen der Benutzer, Kontaktgruppen und Rollen aufrufen und die
...
// TK: Ich finde das Thema Konfigurationsdateien hier nicht passend, wenn wir nicht zumindest einen Link anbieten, in dem steht, wie es geht. Bytheway: Wollen wir, dass hier händisch editiert wird?
// TK: Ich hab es jetzt dringelassen, weil die Konfigurationsdateien in der Grafik weiter unten wieder vorkommen.
...
Wenn Sie schon länger mit dem [.guihint]#Setup#-Menü arbeiten, werden Sie die Symbole im Menü wahrscheinlich schon auswendig kennen -- insbesondere dann, wenn Sie sich die Symbole in den Mega-Menüs bei jedem Menüeintrag anzeigen lassen, wie Sie es im xref:user_interface#user_menu[User-Menü] einstellen können.

Dann empfiehlt sich für den Schnellzugriff das Snapin [.guihint]#Quick setup# für die xref:user_interface#sidebar[Seitenleiste] mit Symbolen für jeden [.guihint]#Setup#-Menüeintrag:

image::wato_quick_setup.png[alt="Snapin Quick setup.",width=50%]

Übrigens gibt es auch das komplette [.guihint]#Setup#-Menü für die Seitenleiste. Dieses Snapin hört auf den Namen [.guihint]#Setup#.


[#activate_changes]
== Änderungen aktivieren

{CMK} speichert alle Änderungen, die Sie vornehmen, zunächst nur in einer vorläufigen *Konfigurationsumgebung*, in der Sie Hosts, Services und Einstellungen verwalten, und die das aktuell laufende Monitoring noch nicht beeinflusst, in der die operative Überwachung stattfindet. Erst durch das „Aktivieren der aufgelaufenen Änderungen“ werden diese in die *Monitoring-Umgebung* übernommen.
...
// TK: In diesem Bild muss WATO durch Setup ersetzt werden.
image::wato_activate_changes.png[alt="Illustration zur Übernahme von Änderungen aus der Konfigurationsumgebung in die Monitoring-Umgebung.",width=400px]
