...
xref:wato_monitoringagents#bakery[Agentenbäckerei] (nur in den {EE}), mit der Agenten konfiguriert, paketiert und automatisch aktualisiert werden können, 
...
xref:alert_handlers#[Alert Handler] (nur in den {EE}).
...
xref:saml#[SAML] konfigurieren (nur in den {EE}).
