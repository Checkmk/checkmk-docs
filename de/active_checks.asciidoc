...
Im Setup können Sie -- bereits weiter oben erwähnt -- die wichtigsten und am
...
[.guihint]#Quicksearch# nach dem soeben gesetzten Servicenamen `Certificate expiry`. In
...
so ist es möglich, mehrere Services auf einem Host aktiv zu überwachen.
...
icon:icon_help[] Inlinehilfe. Nachdem Sie die Änderungen aktiviert haben,
...
// MFS: IMHO sinnvoller Hinweis auf Abgrenzungskriterien – Diskussion im Review
////
Als Faustregel sollte gelten: Aktive Checks sollten die Erreichbarkeit,
Antwortzeit oder den Antwortstatus eines über das Netzwerk erreichbaren
Dienstes auf dem überwachten Host prüfen. Zwar kann theoretisch jedes Skript
für aktive Checks verwendet werden, aber gerade, wenn mehrere Hosts mit
demselben Mechanismus überwacht werden sollen oder auch quantitative Daten
gesammelt werden sollen, lohnt sich der Blick auf die Möglichkeiten des
xref:piggyback#[Piggyback-Mechanismus] oder der
xref:datasource_programs#[Datenquellenprogramme] – oder einer Kombination
aus beiden.
////
...
// MFS: Ich hab das reingenommen, weil agent_linux eher in die Richtung geht, mit lokalen Nagios-Plugins bspw. die Datenbank zu überwachen, die nur auf 127.0.0.1 erreichbar ist.
Angenommen, Sie überwachen von Host A (Ihrer {CMK}-Instanz) einen Host B (bspw. einen Webserver), der wiederum auf Dienste von Host C (bspw. eine Datenbank) zurückgreift.
Eine Überwachung der Dienste auf Host C direkt von Host A aus würde möglicherweise durch Paketlaufzeiten etc. verfälscht werden.
Hier ist es praktisch, ein Nagios-Plugin vom Agenten des überwachten Hosts (hier B) ausführen zu lassen, welches direkt Dienste auf Host C überprüft.
