...
// MFS: Neuer Abschnitt zu Abgrenzungskriterien hinzugefügt:
Wenn Sie Nagios-Plugins integrieren, behalten Sie den Hauptzweck der aktiven Checks im Auge:
Aktive Checks sollten im Sinne eines End-to-End Monitorings die Erreichbarkeit, Antwortzeit oder den Antwortstatus
...
Eine Übersicht finden Sie im Handbuchartikel
xref:devel_check_plugins.html#_muss_es_immer_ein_echtes_plugin_sein[Eigene Check-Plugins schreiben].
...
// MFS: Neuer Abschnitt Erklärung "über Bande"
...
Eine Überwachung der Dienste auf Host C direkt von Host A aus würde möglicherweise durch andere Paketlaufzeiten etc. verfälscht werden.
