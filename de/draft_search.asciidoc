...
[cols="20,~"]
|===
|  `find=suchbegriff` | Der Suchbegriff (zwingende Angabe), mehrere Suchbegriffe bitte mit `+` trennen. Verwenden Sie link:https://de.wikipedia.org/wiki/URL-Encoding#Prozentdarstellung[Prozentdarstellung^] für alle anderen Sonderzeichen und Umlaute (`+` ist `%2B`, `-` ist `%2D`).
