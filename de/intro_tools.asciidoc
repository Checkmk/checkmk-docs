...
:description: Wichtige Werkzeuge für das Monitoring mit {CMK} sind Tactical overview, Quicksearch, Bookmarks und Master Control. Um diese Themen geht es im aktuellen Kapitel des Leitfadens für {CMK} Einsteiger, der kompakt durch die ersten Schritte führt, um das IT-Monitoring mit {CMK} optimal einzurichten.
...
(oder Operating) helfen. In {CMK} wird dieser Teil auch manchmal als
*Statusoberfläche* bezeichnet, weil es meist darum geht, den aktuellen
Status von allen Hosts und Services zu sehen. Dazu gehören Snapins der
Statusleiste, Dashboards, aber auch die Darstellung und Aufbereitung von
Messwerten.
...
In der ersten Spalte dieser kleinen Tabelle sehen Sie zunächst die Anzahl Ihrer
...
Dafür hat {CMK} ein eigenes sehr mächtiges Modul: die link:ec.html[Event Console], die
aber im Leitfaden für Einsteiger nicht besprochen wird.

Die zweite Spalte zeigt die Probleme. Das sind die Objekte, die gerade im Status
{WARN}/{CRIT}/{UNKNOWN} bzw. {DOWN}/{UNREACH} sind. Sie können auf die Zahl in
der Zelle klicken und kommen dann direkt zu einer Liste der Objekte, die hier
gezählt wurden. Das funktioniert übrigens bei allen Zellen der
[.guihint]#Tactical overview#.

Die Zahlen in der dritte Spalte können nie größer werden als die der zweiten,
denn sie zeigen von allen Problemen diejenigen, die noch nicht quittiert wurden.
Die Quittierung (_acknowledgment_) ist eine Art „zur Kenntnisnahme“ eines
Problems, das wir im link:intro_monitor.html#ack[nächsten Kapitel] besprechen werden.

Die letzte Spalte zeigt Hosts oder Services, deren Information „veraltet“
(_stale_) ist, da über sie zurzeit keine aktuellen Monitoring-Daten
vorliegen. Wenn z.B. ein Host aktuell gar nicht erreichbar ist, kann {CMK} auch
keine Neuigkeiten über dessen Services ermitteln. Das bedeutet aber nicht
automatisch, dass diese ein Problem haben. Deswegen nimmt {CMK} nicht einfach
einen neuen Status für diese Services an, sondern setzt sie auf den Pseudostatus
„Stale“. Die Spalte [.guihint]#Stale# wird von {CMK} weggelassen, wenn sie überall 0
zeigen würde.

//SK: Der Hinweis kommt mir hier ein wenig spät. Sonst beschreiben wir in intro_* auch immer den Zustand von {CMK} nach einer Neuinstallation und dann wird oben die TO einfach mit allen 4 Spalten gezeigt.
//SK: Ich würde mir wünschen erst die beiden Spalten ohne "Show more". Dann Klick auf "Show more" und den Rest erklären.
//SK: button_showmore.png stammt aus dem Light-Theme und passt somit nicht zum Bild der TO oben.
...
//SK: Die Quicksearch ist in einer heute-Site vom 20.12.2020 auch nicht standardmäßig aktiviert. Der Hinweis von unten aus dem Abschnitt Views müsste entweder wiederholt oder hoch geholt werden.
...
//SK: Den folgenden Absatz finde ich unnötig verwirrend. Dieses "einerseits,
//SK: andererseits" hilft m.E. nicht weiter. Lesezeichen werden in Listen gespeichert.
//SK: Innerhalb dieser Listen, kann ich die Lesezeichen thematisch organisieren, wenn
//SK: es sein muss. Danach braucht es noch den Link zu zum weitergehenden Artikel und die Kiste ist hier fertig.
//SK: Das teilen von Bookmarks mit anderen Nutzern, obwohl wir noch kaum von weiteren Nutzern gesprochen haben hilft hier nicht.

Einerseits gehört ein Lesezeichen zu einem Thema (so wie es im Snapin
[.guihint]#Bookmarks# zu sehen ist), andererseits zu einer *Liste*. Thema und Liste
können den gleichen Namen haben, aber es handelt sich um unterschiedliche
Objekte. Intern werden die Lesezeichen nämlich als Listen organisiert. In die
Verwaltung der Lesezeichenlisten steigen Sie übrigens mit dem Knopf [.guihint]#Edit#
ein.

Sie können pro Liste entscheiden, ob diese anderen Benutzern bereitgestellt wird
oder für Sie privat bleibt. Jedem Lesezeichen einer Liste können Sie individuell
ein Thema zuweisen. Damit haben Sie die Möglichkeit auch in einer freigegebenen
Liste die Lesezeichen zu Themen zu gruppieren.

Sofern Sie (noch) nicht die Freigabe von Lesezeichen an andere Benutzer planen,
reicht es für den Anfang aus, sich zu merken, dass Ihre persönlichen Lesezeichen
unter [.guihint]#My Bookmarks# gespeichert werden. Wenn Sie dann tiefer in das Thema
einsteigen wollen, können Sie das im
link:user_interface.html#bookmarks[Artikel über die Benutzeroberfläche] tun.
...
//SK: Ich gehe mal davon aus, dass der folgende Abschnitt der mit "das wichtigste Snapin" beginnt, mal von Mathias geschrieben wurde.
//SK: Es wirkt schon sehr (sehr sehr) seltsam, wenn wir dieses Snapin nicht mehr standardmäßig aktivieren, es hier aber weiter als "das wichtigste [...] neben" beschreiben.
//SK: Meines Erachtens wurden (fast) alle Inhalte aus diesem Snapin and andere Stellen gepackt. Der Großteil ist über das Nav-Menü "Monitor" errichbar.
//SK: Isch weiß et nich, aber ich denke der Abschnitt Views muss weg und durch eine Beschreibung der neuen Wege zu den gleichen Zielen ersetzt werden.
//SK: In glaube ja, dass das Snap-In nur für die Alt-User drin geblieben ist. In meinem Monitoring-Alltag habe ich davon immer und immer wieder die gleichen fünf Knöpf benutzt und der Rest war eigentlich nur nervig.
//SK: Ich bin froh, dass das Ding weg ist und wir sollten es nicht mehr (für Neueinsteiger) beschreiben.
...
//SK: Im nächsten Satz stimmt was nicht. "in eine beliebigen Stelle"
