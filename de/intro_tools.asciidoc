...
image::overview_less.png[alt=Snapin Overview im Show-less-Modus,width=50%]
...
Auch im [.guihint]#Overview# können Sie entscheiden, ob sie weniger oder mehr Informationen sehen wollen. Mit dem Knopf icon:button_showmore[alt=Symbol zum Wechsel in den Show-more-Modus] schalten Sie in den Show more-Modus:
...
image::monitor_menu_less.png[alt=Monitor-Menü im Show-less-Modus]
...
image::dashboard_problem.png[alt=Problem dashboard]
...
image::view_service_problems.png[alt=Tabellenansicht Service problems]
...
image::empty_bookmarks.png[alt=Snapin Bookmarks noch ohne Lesezeichen,width=50%]
...
image::bookmarks.png[alt=Snapin Bookmarks mit einem Lesezeichen,width=50%]
...
image::filesystem_c.png[alt=Liste mit einem Service für ein Dateisystem eines Windows-Server]
...
image::service_metrics.png[alt=Details eines Services mit der Tabelle aller Messwerte]
...
image::example_graph.pn[alt=Details eines Services mit einem Graphen zum Zeitverlauf der Messwerte]
...
* Mit dem Symbol icon:resize_graph[alt=Symbol zur Größenänderung des Graphen] in der Ecke rechts unten können Sie den Graphen in seiner Größe ändern.
