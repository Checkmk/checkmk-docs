...
// TK: Ich bekomm das Identity File immer noch nicht mit dem Schlüsselpärchen/Schlüsselpaar zusammen. Evtl. Brücke im Kap. SSH-Schlüssel erstellen bauen?
Die IP-Adresse ist natürlich auf die anzupassen, unter der die VM von außen erreichbar ist:
// TK: Um welche IP-Adresse handelt es sich? Die öffentliche?
// MFS: Ja, im Checkmk Kontext dürfte es keinen Sinn machen, Maschinen aufzusetzen, die nur über private IP-Adressbereiche von Amazon/Azure erreichbar sind.
...
// TK: Die Instanznamenskonvention sollte dort stehen: omd_basics.html#omd_create
...
// TK: Hier, wie in den meisten anderen Stellen dieses Handbuches, verwenden wir als Instanznamen `mysite`.
...
////
MFS: Weitere Recherche nötig. Nullmailer verschluckt sich gerne mal und stellt die Arbeit komplett ein. Dafür ist mutmaßlich GnuTLS in erbindung mit Nebenläufigkeiten verantwortlich.
