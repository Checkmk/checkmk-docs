...
Geben Sie beim Login das _Identity File_ des für diese virtuelle Maschine erzeugten Schlüsselpärchens an.
...
Die IP-Adresse ist natürlich auf die anzupassen, unter der die VM erreichbar ist:
...
Der exakte Prompt kann als Hostname den beim Anlegen der VM angegebenen Hostnamen oder eine IP-Adresse enthalten.
Wir verwenden im Laufe des weiteren Artikels den Hostnamen `cloud`:
...
// MFS: Steht schon in der Ausgabe... Created new site mysite with version 2.2.0p1.cce.
...
==== Relay-only oder vollwertiger Mail Transport Agent

Im Regelfall werden Sie wegen der höheren Flexibilität ein Mailsystem einrichten wollen.
Für kleinere Umgebungen hat sich der _relay-only Mail Transport Agent_ link:https://manpages.ubuntu.com/manpages/jammy/man7/nullmailer.7.html[Nullmailer^] bewährt.
...
Für größere Installationen, bei denen unvorhergesehene Ereignisse auch einige Hundert E-Mails zur Folge haben können, empfehlen wir die Installation eines vollwertigen Mail Transport Agents wie Postfix.
...
// MFS: Derzeit hinken wir immer ein paar Tage hinterher.
...
----
// TK: sudo omd sites
// MFS: Benötigt keine Root-Rechte!
