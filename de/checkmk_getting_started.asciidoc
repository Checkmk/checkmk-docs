...
Als Instanzbenutzer können Sie das Passwort des Benutzers `cmkadmin` - ab dem Patch-Release 2.1.0p16 - wie folgt ändern:
...
In allen vorherigen Patch-Releases von {CMK} {v21} müssen Sie für die Änderung des Passworts eines Nutzers noch `htpasswd` bemühen:
