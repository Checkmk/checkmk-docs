...
Dieser Referenzartikel enthält Beispiele für alle (Asciidoc-)Formatierungen, die wir im Checkmk Handbuch verwenden.
...
Eine Mischung _kursiver,_ *gefetteter,* und `diktengleicher` Schrift gefolgt von einem [.guihint]#GUI-Hint.#
...
Von {CMK} gibt es die folgenden unterschiedlichen Editionen: {CRE}, {CSE}, {CFE}, {CME} und die Gesamtmenge aller {CEE}.
...
Ein Sternchen innerhalb von diktengleichem Text -> `pass:[.*]temp`
...
include::{base}/contrib/de/dev.adoc[]
=======
Eckige Klammern innerhalb einer URL innerhalb von diktengleichem Text -> `\http://myserver/mysite/check_mk/host_inv_api.py?request={"hosts":\pass:[[]"myhost","myhost2"pass:[]]}&output_format=json`

Eine URL, die nicht zum Link werden soll -> \https://www.tribe29.com
>>>>>>> 62cdc8be... added another example
