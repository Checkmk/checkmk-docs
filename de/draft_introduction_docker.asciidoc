...
Sie können den Container auch mit Hilfe des Tools `docker-compose` starten. Das erleichtert u.a. die Angabe der Parameter und ermöglicht zudem den einfachen Start von weiteren Containern. In vorliegenden Fall braucht wird erst einmal nur der grundlegende Start mit [.guihint]#Docker Compose# gezeigt. Legen Sie dafür in in einem Testverzeichnis, die Datei `docker-compose.yml` an und wechseln Sie danach in genau dieses Verzeichnis:
...
Beachten Sie, dass es immer nur _eine_ Datei mit dem Namen `docker-compose.yml` pro Verzeichnis geben kann. Legen Sie nun diese Datei an und beschreiben Sie, wie Checkmk als Container gestartet werden soll. Der Einfachheit halber sehen Sie in dem nachfolgenden Beispiel dieselben Optionen, wie aus dem manuellen Start:
...
Dieses Beispiel können Sie als funktionierende Ausgangsbasis nutzen, um Checkmk in Ihre eigene Docker-Compose-Umgebung einzubinden. Für einen ersten Test starten Sie den Container mit dem Befehl `docker-compose`. Sie sehen in diesem Fall automatisch die Logs in der Ausgabe und haben damit auch das Passwort direkt wie oben zu sehen verfügbar. Aus Übersichtsgründen ist die Ausgabe weggelassen:
...
*Hinweis:* Auf ganz ähnliche Weise gehen Sie vor, wenn Sie einen Container mit der {CCE} starten möchten. Ersetzen Sie in den Beispielen dazu `enterprise` durch `cloud`.
