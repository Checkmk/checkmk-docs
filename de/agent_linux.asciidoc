...
Zum anderen ergänzt er das Skript so, dass neue Funktionen hinzugefügt werden können wie die TLS-Verschlüsselung der Kommunikation und Datenkomprimierung.
...
Mit der {CMK}-Version {v22} wurde zudem ein *Push-Modus* für die {CPE} eingeführt, welcher die Überwachung von Hosts hinter Firewalls erleichtert.
Kombiniert wird dieser meist mit der xref:draft_hosts_autoregister#[automatischen Registrierung.]
...
|===
|Methode |Beschreibung |CRE |CEE|CCE
...
Die Installationsroutine überprüft und konfiguriert in allen Editionen `systemd` und `xinetd` -- in dieser Reihenfolge. |X |X |X
|RPM/DEB-Paket aus der xref:glossar#agent_bakery[Agentenbäckerei] |Konfiguration über die GUI, individuelle Konfiguration pro Host möglich. | |X |X
|xref:agent_deployment#[Automatisches Updaten] |Das Paket aus der Agentenbäckerei wird erstmalig von Hand oder per Skript installiert und von da an automatisch aktualisiert. | |X |X
|xref:draft_hosts_autoregister#[Automatische Erstellung von Hosts] |Über Einstellungen in der Agentenbäckerei kann sich der Host automatisch an einer {CMK}-Site registrieren, Hosts werden automatisch in für sie vorbereiteten Ordnern erstellt. | | |X
|===
...
Eine Ausnahme hiervon sind via Agentenbäckerei heruntergeladene Pakete in {CPE}, die über die Regel [.guihint]#Agent controller auto-registration# für xref:draft_hosts_autoregister#[Autoregistrierung] vorkonfiguriert sind.
Diese führen die Registrierung automatisch nach der Installation durch.

Führen Sie in allen anderen {CMK}-Editionen die Registrierung daher zeitnah nach Installation/Update durch.
...
*Achtung:* Sowohl Push-Agent als auch Autoregistrierung ({PE}) sind zwingend an den Agent Controller gebunden und daher im Legacy-Modus, auf den wir in diesem Kapitel mehrfach verweisen, nicht nutzbar.
...
Ob der Host dabei für Push-Modus (nur {PE}) oder im Push-Modus (andere Editionen) konfiguriert wird, macht für die Befehlsbeispiele keinen Unterschied.
...
// (Autoregistrierung)
// TK: Vielleicht besser: Host beim Server automatisch registrieren lassen

{CPE} bietet die Möglichkeit, Hosts automatisch bei der Registrierung anzulegen.
Die folgenden Voraussetzungen müssen für die xref:draft_hosts_autoregister#[Autoregistrierung] erfüllt sein:

* Ein Benutzer mit der Berechtigung Hosts zu registrieren ist vorhanden
* Wenigstens ein Ordner für automatisch erstellte Hosts existiert
...
// TK: Die (neue) Berechtigung ist wirklich die, Hosts zu registrieren (nicht nur zu erstellen)
// TK: Die 2 Punkte in der Klammer machen den Satz schwer lesbar
// MFS: Umgestellt

In der Regel werden Sie den Weg über die xref:draft_hosts_autoregister#rule_autoregister_bakery[Einstellungen der Agentenbäckerei] gehen, welche eine Konfigurationsdatei `/var/lib/cmk-agent/pre_configured_connections.json` ins Agentenpaket integrieren und die Registrierung automatisch bei der Installation vornehmen.
Der hier vorgestellte Kommandozeilenaufruf dient daher primär dem Test und dem Debugging, beispielsweise dem Ausprobieren eigener _Agenten-Labels_ mit der Option `--agent-labels <KEY=VALUE>`.
// TK: Dann sollte aber auch im Kommando -agent-labels <KEY=VALUE> drinne sein.
// TK: Den letzten Satz im Abschnitt hab ich auskommentiert: der kam mir zu spät.
...
Ob der Host im xref:glossar#push_mode[Push-Modus] oder im xref:glossar#pull_mode[Pull-Modus] oder überhaupt nicht angelegt wird, entscheiden Ihre Einstellungen des Regelsatzes [.guihint]#Agent registration#.
...
In {CPE} können Sie Hosts vom Push- zum Pull-Modus und umgekehrt umstellen.
Dies kann in Einzelfällen nötig sein, wenn Änderungen an der Netzwerktopologie anstehen oder ein Downgrade auf {CEE} durchgeführt werden soll, wo nur Pull-Hosts möglich sind.

Stellen Sie zunächst im Setup den Zugriffsmodus um.
Innerhalb der nächsten Minute werden alle Dienste den Status {UNKNOWN} annehmen, weil keine Monitoring-Daten eingehen.
Führen Sie anschließend eine erneute Registrierung durch.
Bei dieser erneuten Registrierung teilt der Agent Receiver des {CMK}-Servers dem Agent Controller mit, ob er Daten im Push- oder Pull-Modus erwartet.
...
Beim Agent Controller im Push-Modus ({PE}) überspringen Sie Tests auf Port 6556 – selbst wenn Port 6556 vor der Registrierung geöffnet ist, wird der Port nach der Registrierung als Push-Host geschlossen.
...
Analog zu Port 443 (respektive 80) oben können Sie jetzt die Routing- oder Firewall-Einstellungen so anpassen, dass der zu registrierende Host den {CMK}-Server auf dem Port des Agent Receiver (8000 oder 8001…) erreichen kann. Im Falle der Registrierung eines Push-Hosts ({PE}) gilt: Hat die Registrierung funktioniert, wird auch der minütliche Transfer der Agentenausgabe erfolgreich sein.
...
Bei Push-Hosts ({PE}) zeigt dieser Softlink auf den Ordner mit der Agentenausgabe, bei Pull-Hosts auf eine nicht vorhandene Datei mit dem Namen des Hosts, wie er im Monitoring verwendet wird.
...
Desweiteren können Sie auf dem Host mit dem Kommando `systemctl status cmk-agent-ctl-daemon` den Status der letzten Übertragungen und Übertragungsversuche auslesen. Zeilen wie die folgende deuten auf Verbindungsprobleme hin:
...
// MFS: FIXME, die Option wird wahrscheinlich noch umbenannt:
Wurde ein Host im Monitoring der {CPE} für Autoregistrierung (Regelsatz [.guihint]#Agent controller auto-registration#) konfiguriert und die Einstellung _Keep vanished connections_ auf _no_ gesetzt, werden bei jedem Neustart des Dienstes `cmk-agent-ctl-daemon` (beispielsweise beim Neustart eines Hosts) alle anderen Verbindungen außer der für Autoregistrierung konfigurierten Verbindung entfernt.
Dies betrifft beispielsweise Hosts, auf denen vor Installation des gebackenen Agentenpaketes Verbindungen zu mehreren Sites eingerichtet waren oder nach der Installation des Agentenpaketes manuell hinzugefügte Verbindungen.

Sie können dieses Verhalten temporär ändern, indem Sie in der Datei `/var/lib/cmk-agent/pre_configured_connections.json` die Variable `keep_vanished_connections` auf `true` setzen.
Eine dauerhafte Änderung über ein Update der Agentenpakete hinweg erreichen Sie, indem Sie im Setup im Regelsatz  [.guihint]#Agent controller auto-registration# den Wert _Keep vanished connections_ auf _yes_ setzen.

[#howlong]
=== Zeitdauer bis Änderungen sichtbar werden

Bei Autoregistrierung eines Hostes in {CPE} vergehen typischerweise etwa zwei Minuten, bis ein Host im Monitoring auftaucht.

// MFS: FIXME, wird möglicherweise in der Betaphase noch geändert.
Wurde einem Host, der zuerst für eine Push-Verbindung konfiguriert wurde, nachträglich eine Pull-Verbindung zu einer anderen Site hinzugefügt, vergehen bis zu fünf Minuten, bis Port 6556 geöffnet wird.
Sofortige Öffnung des Ports erreichen Sie durch Neustart des Dienstes `cmk-agent-ctl-daemon`.
...
|`/var/lib/check_mk_agent/spool` |Enthält Daten, die z.B. von Cronjobs erstellt werden und eine eigene Sektion beinhalten. Diese werden ebenfalls der Agentenausgabe angehängt. Mehr dazu erfahren Sie im Artikel xref:spool_directory#[Das Spool-Verzeichnis.]
...
// MFS: Neu in CPE/CCE
|`/var/lib/cmk-agent/pre_configured_connections.json` |Enthält eine vorkonfigurierte Verbindung für die Autoregistrierung bei einer Site ({PE}). 
