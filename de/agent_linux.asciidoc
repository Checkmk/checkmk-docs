...
[#autoregister]
=== Host beim Server registrieren (Autoregistrierung)
// TK: Vielleicht besser: Host beim Server automatisch registrieren lassen

Falls Sie die {CPE} nutzen und die Voraussetzungen für eine xref:draft_hosts_autoregister#[Autoregistrierung] erfüllt sind (zumindest der Benutzer mit der Berechtigung Hosts zu registrieren und wenigstens ein Ordner für automatisch erstellte Hosts existieren), können Sie die Registrierung inklusive automatischer Erstellung des Hosts auch per Kommandozeile vornehmen.
...
// Zusätzliche Agent Labels, welche diese Entscheidung beeinflußen können, übergeben Sie mit dem Parameter  `--agent-labels <KEY=VALUE>`.
