...
Der erste Schritt nach dem Absetzen des Kommandos `cmk-agent-ctl register` ist eine Anfrage an das REST-API des {CMK}-Servers: "Bitte teile mir den Port Deines Agent Receiver mit!"
Sie können diese Anfrage auf dem Host, der ins Monitoring aufgenommen werden soll mit einem Programm wie `curl` simulieren:
...
In vielen Fällen dürfte es einfacher sein, auf dem {CMK}-Server den Port des Agent Receiver auszulesen und sich zu notieren:
...
Sie können nun den ermittelten Port bei der Registrierung direkt angeben und so die Anfrage beim REST-API überspringen:
...
Analog zu Port 443 (respektive 80) oben können Sie jetzt die Routing- oder Firewall-Einstellungen so anpassen, dass der {CMK}-Server von den neu zu registrierenden Hosts auf Port 8000 (oder 8001…) erreicht werden kann.
