...
In der {CRE} finden Sie die Linux-Pakete des Agenten über [.guihint]#Setup > Agents > Linux#. In den {EE} gelangen Sie im Setup-Menü über icon:icon_agents[]
[.guihint]#Agents > Windows, Linux, Solaris, AIX# zunächst in die
...
Achtung: Im Kasten [.guihint]#Agents# finden Sie die reinen Agentenskripten
...
SK: Das können wir hier bei Gelegenheit mal nachtragen.
...
{cee-only}
In den {CEE} können die mitgelieferten Plugins über die xref:wato_monitoringagents#bakery[Agentenbäckerei]
...
Regelsätze über [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Agent plugins#:
