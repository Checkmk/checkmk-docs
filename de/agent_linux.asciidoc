...
Bei der Deinstallation des Agenten bleibt der vom Installationsscript angelegte Nutzer `cmk-agent` erhalten.
...
Sollte es erwünscht sein, wieder unverschlüsselten Legacy-Pull-Modus zu aktivieren, z.B. um die Kommunikation einer {v20} Site mit einem {v22} Agenten zu ermöglichen, müssen Sie dies explizit aktivieren:
...
Anschließend starten Sie den Agent Controller neu:

[{shell}]
----
{c-root} cmk-agent-ctl systemctl restart cmk-agent-ctl-daemon
----

Alternativ zu einem Neustart können Sie einen Kaffee oder Tee trinken gehen:
Der Daemon liest alle paar Minuten seine Konfiguration automatisch neu ein und startet sich falls nötig selbst neu.
