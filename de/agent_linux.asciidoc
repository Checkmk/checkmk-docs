...
|RPM/DEB-Paket aus der link:wato_monitoringagents.html#bakery[Agentenbäckerei] |Konfiguration über die GUI, individuelle Konfiguration pro Host möglich. | |X
...
dem Knopf [.guihint]#Agent files# zur Liste der Agentendateien:
...
{cee-only}
Die {CEE} verfügen mit der link:wato_monitoringagents.html#bakery[Agentenbäckerei] über ein
...
Agentenseite (im Kasten [.guihint]#z/OS#) und installieren Sie es ebenfalls nach `/usr/local/bin`. `timeout`
...
über die Agenten link:wato_monitoringagents.html#diagnostics[beschrieben]. Aber
...
Diagnosebefehls. Sektionen beginnen immer mit einem Sektionsheader. Dies ist
...
anlegen, welche dann von der link:wato_monitoringagents.html#bakery[Agentenbäckerei]
berücksichtigt wird.
...
link:wato_services.html#available[verfügbar] angezeigt. Das sieht dann so aus:
...
{cee-only}
Stolze Besitzer der {EE} können MRPE auch mit der
link:wato_monitoringagents.html#bakery[Agentenbäckerei] konfigurieren. Zuständig
dafür ist der Regelsatz
[.guihint]#Monitoring Agents > Rules > Generic Options > Execute MRPE Checks#. Dort können
...
Zu allen von uns mitgelieferten Agentenplugins gibt es auch die passenden Checkplugins,
...
* Die Dokumentation der zugehörigen Check-Plugins im WATO-Modul [.guihint]#Check plugins#
...
Die von {CMK} mitgelieferten Plugins können über die link:wato_monitoringagents.html#bakery[Agentenbäckerei]
...
Regelsätze in [.guihint]#Monitoring Agents > Agent plugins#:
...
keine Ausnahme sein. Da der Monitoring-Agent auf jedem überwachten Server
...
integrations> Individual program call instead of agent access#. Erstellen Sie
...
die im link:cmk_commandline.html#cmk[Artikel über die Kommandozeile] erklärt werden.
...
Encryption#. Die Regel soll auf alle Hosts greifen, für die Sie Verschlüsselung
...
* Der Host ist in {CMK} für die zusätzliche Überwachung per link:agent_linux.html#snmp[SNMP] konfiguriert.
...
|/usr/lib/check_mk_agent/local |Eigene link:localchecks.html[„Localchecks“].
...
|/etc/check_mk/encryption.cfg |Konfiguration für die link:agent_linux.html#encryption[Verschlüsselung] der Agentendaten.
