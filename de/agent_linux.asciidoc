...
*Achtung:* Sowohl Push-Agent als auch Autoregistrierung der {CE} sind zwingend an den Agent Controller gebunden und daher nicht nutzbar im Legacy-Modus, auf den wir in diesem Kapitel mehrfach verweisen.
...
Beim Agent Controller im Push-Modus der {CE} überspringen Sie Tests auf Port 6556 – selbst wenn der Port 6556 vor der Registrierung geöffnet ist, wird er nach der Registrierung als Push-Host geschlossen.
...
Im Falle der Registrierung eines Push-Hosts in der {CE} gilt:
...
// MFS: Ins Leere zeigende Softlinks sind ein Mittel, Information unterzubringen, ohne Inhalt in eine Datei schreiben zu müssen. Ein smarter Kniff vieler Programmierer.
...
Sie können dieses Verhalten temporär ändern, indem Sie auf dem Host in der Datei `/var/lib/cmk-agent/pre_configured_connections.json` die Variable `keep_vanished_connections` auf `true` setzen.
...
// MFS: Muss dieses Hilfsmittel hier dokumentiert werden? Der wird ja regulär gar nicht genutzt und sieht aus, als wurde er noch nicht groß mit dem Agent Controller getestet.
...
// MFS: Evtl. als FIXME REMOVE AFTER BETA kennzeichnen?
