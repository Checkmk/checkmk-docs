...
Der erste Schritt ist eine Anfrage nach dem Port des Agent Receivers, hier wird eine URL für den Zugriff auf das REST-API generiert und diese dann abgerufen.
Sie können die Anfrage auf dem Host, der ins Monitoring aufgenommen werden soll mit einem Programm wie `curl` simulieren:

[{shell-raw}]
----
{c-root} curl -v https://cmk.m-app.mobi/app/check_mk/api/1.0/domain-types/internal/actions/discover-receiver/invoke
----

Der Inhalt der Antwort ist lediglich wenige Byte groß und besteht aus der Port-Nummer des Agent-Receiver.
...
In vielen Fällen dürfte es einfacher sein, auf dem {CMK}-Server den Port des Agent Receiver herauszufinden und sich zu notieren:
...
Sie können nun den ermittelten Port bei der Registrierung direkt angeben und so die Anfrage beim REST-API umgehen:
...
Analog zu Port 443 (respektive 80) oben können Sie die Routing- oder Firewall-Einstellungen so anpassen, dass der {CMK}-Server von den neu zu registrierenden Hosts auf Port 8000 (oder 8001…) erreicht werden kann.
