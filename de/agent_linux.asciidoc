...
{cee-only}
Die {CEE} verfügen mit der link:wato_monitoringagents.html#bakery[Agent-Bakery] über ein
...
Diagnosebefehls. Sektionen beginnen immer mit einem Sektionsheader. Dies sind
die Zeilen, die in <<< und >>> eingeschlossen sind.
...
bestimmer Host fehlerhafte Werte liefert und Sie den Abruf dieser Daten
kurzzeitig aussetzen wollen.

Als Nutzer einer der {CEE} können Sie über Setup > Agents > Windows, Linux,
Solaris, AIX > Agent rules > Disabled sections (Linux agent) einfach eine Regel
anlegen, welche dann von der link:wato_monitoringagents.html#bakery[Agent-Bakery]
berücksichtigt wird.
...
Nutzer der {CRE} können eine solche Datei einfach manuell anlegen.
Alle deaktivierbaren Sektionen finden Sie in der Datei
...
einer Regel im Regelsatz [.guihint]#Host & Service Parameters > Access to agents > Check_MK Agent > Encryption#.
...
SNMP. Damit wird eine Doppeltübertragung automatisch vermieden.
