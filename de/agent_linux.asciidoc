...
Mit der {CMK}-Version {v22} wurde zudem ein *Push-Modus* für {CPE} eingeführt, welcher die Überwachung von Hosts hinter Firewalls erleichtert.
Kombiniert wird dieser meist mit xref:draft_hosts_autoregister#[automatischer Registrierung].
...
Falls Sie  {CPE} nutzen und die Voraussetzungen für eine xref:draft_hosts_autoregister#[Autoregistrierung] erfüllt sind (zumindest der Nutzer mit der Berechtigung Hosts anzulegen und wenigstens ein Ordner für automatisch erstellte Hosts existieren), können Sie die Registrierung inclusive automatischer Erstellung des Hosts auch per Kommandozeile vornehmen.

In der Regel werden Sie den Weg über xref:draft_hosts_autoregister#rule_autoregister_bakery[Bakery-Einstellungen] gehen, welche eine Konfigurationsdatei `/var/lib/cmk-agent/pre_configured_connections.json` ins Agentenpaket integrieren und die Registrierung automatisch bei der Installation vornehmen.
Der hier vorgestellte Kommandozeilenaufruf dient daher primär Test- und Debugging-Zwecken, beispielsweise dem Ausprobieren eigener _Agent Labels_.
...
Größter Unterschied hier ist das geänderte Subkommando `register-new` in Zusammenspiel mit Angabe des Autoregistrierungs-Nutzers.
Beide zusammen fragen an der {CMK}-Site die Erstellung eines neuen Hosts an.
...
Ob der Host als Push- oder Pull-Host oder überhaupt nicht angelegt wird, entscheiden Ihre Einstellungen des Regelsatzes [.guihint]#Agent registration#.
