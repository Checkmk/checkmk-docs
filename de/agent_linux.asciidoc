...
|xref:draft_agent_autoregister#[Automatische Erstellung von Hosts] |Über Einstellungen in der Agentenbäckerei kann sich der Host automatisch an einer {CMK}-Site registrieren, Hosts werden automatisch in für sie vorbereiteten Ordnern erstellt. | | |X
|===
...
Eine Ausnahme hiervon sind via Agentenbäckerei heruntergeladene Pakete in {CPE}, die über die Regel [.guihint]#Agent controller auto-registration# für xref:draft_hosts_autoregistration#[Autoregistrierung] vorkonfiguriert sind.
...
*Achtung:* Sowohl Push-Agent als auch Autoregistrierung ({PE}) sind zwingend an den Agent Controller gebunden und daher im Legacy-Modus nicht nutzbar.
