...
dem Knopf [.guihint]#Unpackaged agents}} bzw. {{Agent files# (Version VERSION[1.4.0])
...
== Agent um Plugins erweitern#plugins
...
=== Manuelle Installation von Plugins#manualplugins
...
=== Konfiguration der Plugins#pluginconfig
...
=== Asynchrone Ausführung#async_plugins
...
=== Eingebaute Verschlüsselung#encryption
...
== Überwachen von Linux per SNMP#snmp
...
Kasten [.guihint]#Host tags}} die Einstellung {{Agent type}} auf {{Dual: Check_MK Agent + SNMP#.
...
* Der Host ist in {CMK} für die zusätzliche Überwachung per link:agent_linux.html#snmp[SNMP] konfiguriert ( [.guihint]#Agent type}} auf {{Dual: Check_MK Agent + SNMP# ).
