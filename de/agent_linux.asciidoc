...
// Hosts im Monitoring werden rechtzeitig mit dem neuen Zertifikat vertraut gemacht, so dass bei Ablauf der Zertifikatsgültigkeit kein Eingreifen erforderlich ist.
// TK: Hosts im Monitoring: Das ist mir nicht deutlich genug. Ich hab nochmal ins Werk geschaut. Was hältst Du davon:
// MFS: Ist wahrscheinlich aus Nutzersicht hinreichend. Technisch tauscht der Agent Receiver sein Zertifikat aus, der Agent Controller fragt dann an, ob ein neues vorliegt. So sehr müssen wir aber nicht ins Detail gehen. Ich habe wieder auf Singular zurückgesetzt.
...
Die Laufzeit des Zertifikats kann über die globale Einstellung [.guihint]#Agent Certificates > Lifetime of certificates# eingestellt werden.
// TK: Bytheway: Müsste man nicht oben eine Anmerkung hinzufügen (in 4.3. Host beim Server registrieren), dass man die in der Kommandoausgabe stehende Validity bis 3020 nicht ernst nehmen soll, da die Gültigkeit eben durch die globale Einstellung festgelegt wird? Ich stolpere bei jeder Registrierung wieder über die 3020 oder inzwischen 3021...
// MFS: Das ist als Bug zu betrachten, hier sollten bei Erstellung der Site 5 Jahre (default) oder 50 Jahre (max) drinstehen. Ich vermute, das wird noch geändert, wenn im nächsten Schritt auch auf eigene Zertifikate umgestellt werden kann.
// TK: Und in agent_windows muss das auch noch rein, oder?
