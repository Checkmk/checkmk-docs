...
// TK: Hier evtl. kurz erläutern, dass die zuerst erstellte Instanz des {CMK}-Servers die 8000 erhält, die zweite 8001 usw. Das steht explizit so noch nicht da.
...
// TK: "In diesem Fall" > Falls die obige Anfrage via REST-API nicht die Port-Nummer zurückliefert, ...
Oft dürfte es jedoch einfacher sein, auf dem {CMK}-Server den Port des Agent Receiver auszulesen und zu notieren.
...
// TK: Der Rest dieses Abschnitts gehört in den Abschnitt "Im Auftrag registrieren".
