...
xref:notifications#[Alarmierung (Notifications)]
...
// TK: Tactical Overview durch Overview ersetzt (auch in en).
...
// TK: Soll www.example.com wirklich als Link scharfgemacht werden? Ist er nämlich momentan.


=== Flexible with max. duration

Diese Option ist z.&ThinSpace;B. nützlich, wenn Sie wissen, dass ein Host für ein
...
// TK: blaue Mond erscheint: War das mal so? Müsste das nicht das Pausenzeichen von oben sein?
...
Diese Option ist für Router und Switche nützlich, aber z.&ThinSpace;B. auch für
...
// TK: Dieser Abschnitt passt hier nicht hin, weil er nichts mit den Optionen im Kasten zu tun hat. Ich würde ihn hinter "Repeat this downtime on a regular basis" verschieben
...
// TK: icon:commands gibt es nicht mehr: Das Kommando heisst jetzt Schedule downtimes mit dem Verkehrs-Warnhütchen.
...
(nur {CEE}), z.&ThinSpace;B. wenn diese verlängert werden müssen, da die eigentliche
...
{cee-only}
Manche Wartungen werden regelmäßig ausgeführt -- wie z.&ThinSpace;B. der
...
nur um die xref:notifications#[Alarmierung] geht, können Sie hierzu mit
...
//TK: Der Screenshot ist sehr unübersichtlich. Vorschlag: Ohne ausgeklapptes Menü, dafür week rot hiliten.
...
xref:wato_rules#hosttags[Host-Merkmalen] können Sie damit z.&ThinSpace;B. Dinge bestimmen wie:
...
|[.guihint]#Exclude scheduled downtimes# |Wartungszeiten werden bei der Berechnung der 100% komplett ausgeklammert. Alle prozentualen Angaben über Verfügbarkeit beziehen sich also nur auf die restlichen Zeiten, quasi: Wie viel Prozent der Nicht-Wartungszeit war das Objekt verfügbar?
...
//TK: Im 1. Satz Phases hinzugefügt (auch in en), da die Option im Screenshot gar nicht zu sehen ist.
