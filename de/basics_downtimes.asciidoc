...
(nur {CEE}), z.B. wenn diese verlängert werden müssen, da die eigentliche
...
xref:wato_rules#hosttags[Host-Merkmalen] können Sie damit z.B. Dinge bestimmen wie:
...
phases of UP/OK as non-downtime#. Wenn diese angehakt ist, dann werden Zeiten,
