...
Hier können Sie einen anderen Benutzer autorisieren und dessen Zugriffsdaten angeben (Name, Passwort). Sobald Sie die Änderungen mit icon:icon_save[alt="Symbol für das Speichern."] [.guihint]#Save# gespeichert haben, wird der definierte Benutzer für die Automatisierung genutzt und Sie können mit der dynamischen Host-Konfiguration fortfahren.
