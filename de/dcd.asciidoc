...
Der Menüpunkt [.guihint]#Setup > Hosts > Dynamic host management# bringt Sie zur Konfiguration
des DCD beziehungsweise dessen Konnektoren:

image::dcd_connections_empty.png[alt="Leere Konnektorenliste im Bereich Dynamic host management."]
...
// ML Eigener Kasten, Extrarand kann weg.
image::dcd_connection_general.png[alt="General properties beim Hinzufügen einer neuen Connection."]
...
// ML: Bild zu breit und Ränder falsch. Zudem sieht es so aus, als würde der Text darüber zu diesem Bild gehören, nicht dem darüber - vielleicht Formatierung oder Text ein klein wenig anpassen?
...
image::dcd_pending_changes.png[alt="Liste der Pending changes unmittelbar vor der automatischen Übernahme ins Monitoring."]
...
kompletten Neustart des {CMK}-Servers selbst. Denn in dieser Situation fehlen erstmal Piggyback-Daten
