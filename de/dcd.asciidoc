...
In {CMK} gibt es standardmässig den Benutzer xref:wato_user#automation[automation]. Dieser Benutzer wird von {CMK} angelegt, um automatisierte Abrufe durchführen zu können. Er ist auch für die dynamische Host-Konfiguration zwingend notwendig.
...
Hier können Sie auf einen anderen Benutzer mit Zugriffsrechten wechseln und dessen Zugriffsdaten angeben (Name, Passwort). Sobald Sie die Änderungen mit icon:icon_save[alt="Symbol für das Speichern."] [.guihint]#Save# gespeichert haben, wird der definierte Benutzer für die Automatisierung genutzt und Sie können mit der dynamischen Host-Konfiguration fortfahren.
