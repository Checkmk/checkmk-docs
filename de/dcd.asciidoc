...
Er ist auch für die dynamische Host-Konfiguration zwingend notwendig.
// MFS: IMHO besser, weil man da mittlerweile einen anderen Nutzer nehmen kann, sogar cmkadmin, daher Betonung auf "ein"
// Ein existierender Automatisierungs-User ist für die dynamische Host-Konfiguration zwingend notwendig.

Haben Sie, aus welchen Gründen auch immer, in Ihrem System diesen Benutzer nicht (mehr), so müssen Sie einen anderen Benutzer mit automatisiertem Zugriff auf die API schaffen.
// MFS: Evtl. so?
// Haben Sie … diesen Nutzer gelöscht oder geändert (beispielsweise von #[.guihint]Automation secret for machine accounts# auf #[.guihint]Normal user login with password#), so müssen Sie…
...
// MFS: Da oft nur auf normales Passwort umgestellt wird, etvl. besser
// Hier können Sie einen anderen Benutzer eintragen oder Änderungen am bestehenden Automationsnutzer eintragen.
Hier können Sie einen anderen Benutzer autorisieren und dessen Zugriffsdaten angeben (Name, Passwort).
