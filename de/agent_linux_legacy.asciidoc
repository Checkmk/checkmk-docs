...
// IGNORE Ver Inetutils Rückwärtsauflösung check_mk_agent distributionsunabhängige 
...
// MFS: Hinweis auf Emulation des Push-Modus mit Datenquellenprogrammen
Auch der Push-Modus der {CCE} ist auf den Agent Controller angewiesen und daher im Legacy-Modus nicht verfügbar.
...
Je nach Paketmanagement stehen drei Installationsmöglichkeiten zur Auswahl: Entweder DEB- oder RPM-Pakete für Debian, Ubuntu, RHEL, SLES (und deren Derivate), ein TGZ-Archiv für alle anderen Distributionen ({CEE}) oder ebenfalls für weitere Distributionen ein Shellskript ({CRE}).
...
// TK: Die 2.1 Regel "Encryption (Linux, Windows)" ist in der 2.2 neu strukturiert (siehe Werks ##14982 und #14652):
// TK: Mit der Regel "Symmetric encryption (Linux, Windows)"" schalte ich die Verschlüsselung ein: Der Agent sendet dann mit dem shared secret verschlüsselte Daten. 
// TK: Mit der Regel "Enforce agent data encryption" lege ich fest, wie mit unverschlüsselten Daten umgegangen wird.
// TK: Die RTC relevanten Einstellungen stecken jetzt alle in der Regel "Real-time checks" (in der 2.1 "Send data for real-time checks"). Diese Regel wird in diesem Kapitel aber nicht erwähnt.
// TK: Ich hab die Abschnitte neu strukturiert und umgeschrieben. Bitte überprüfen.
// TK: Im windows_agent hatten wir in der 2.1 die detaillierte Beschreibung zur symmetrischen Verschlüsselung rausgekickt und nur die Regel "Encryption (Linux, Windows)" erwähnt: an 4 Stellen.
// TK: Reicht dieser Verweis im windows_agent auf *eine* Regel auch noch in der 2.2?
...
.Mit dieser Auswahl werden symmetrisch verschlüsselte Daten vom Agenten akzeptiert
image::agent_linux_enforce_encryption.png[alt="Regel zur Festlegung, welche Daten des Agenten der {CMK}-Server annimmt."]

Sinnvoll ist es, zunächst mit [.guihint]#Accept all incoming data, including unencrypted# zu beginnen.
Sobald Sie meinen, dass alle Agenten auf Verschlüsselung umgestellt sind, stellen Sie auf [.guihint]#Accept all types of encryption#, um dadurch Hosts zu finden, die noch Daten im Klartext senden.
...
// TK: Folgender Satz neu, da diese Regel ja nicht eingebacken wird:
