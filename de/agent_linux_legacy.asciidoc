...
Seit der {CMK}-Version {v21} beherrscht der Linux-Agent mit dem *Agent Controller* den registrierten, TLS-verschlüsselten und komprimierten xref:glossar#pull_mode[Pull-Modus.]
...
Auch der Push-Modus der {CPE} ist auf den Agent Controller angewiesen.
Soll bei Nutzung des Legacy-Modus ein Host ins Monitoring aufgenommen werden, der vom {CMK}-Server nicht erreichbar ist, können Sie beispielsweise dessen Agentenausgabe auf eine Maschine kopieren, die vom {CMK}-Server aus erreichbar ist und mit einem xref:datasource_programs#[Datenquellenprogramm] darauf zugreifen – {CMK} erlaubt vielfältiges Scripting.
