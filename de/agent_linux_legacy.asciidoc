...
Soll im Legacy-Modus ein Host ins Monitoring aufgenommen werden, der vom {CMK}-Server nicht erreichbar ist, 
verwenden Sie xref:datasource_programs#[Datenquellenprogramme], um eine Verbindung vom überwachten Host ausgehend aufzubauen und die darüber übertragene Agentenausgabe einzulesen.
// TK: Deutlicher machen, dass hier selbst gebaut werden muss? Vielleicht so:
// müssen Sie einen anderen Weg finden. 
// Sie können xref:datasource_programs#[Datenquellenprogramme] verwenden, um eine Verbindung vom überwachten Host ausgehend aufzubauen und die darüber übertragene Agentenausgabe einzulesen.
// TK: oder so?
