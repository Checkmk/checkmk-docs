...
Ein anderer Weg geht über den Knopf icon:context_button_rulesets[]
...
Das Symbol icon:button_check_parameters[] bringt Sie direkt zu demjenigen
...
image::service_context_menu.png[align=center, width=550]
...
mit icon:button_edit[] bearbeiten können. Eine über den Knopf [.guihint]#Create rule in folder#
...
icon:button_top[], icon:button_bottom[], icon:button_up[], und
icon:button_down[] ändern. Die Reihenfolge ist wichtig, weil immer
...
* Das Feld [.guihint]#Comment# können Sie für eine längere Beschreibung verwenden. Es erscheint nur im Editiermodus einer Regel. Über das Symbol icon:button_insertdate[] können Sie einen Zeitstempel und Ihren Loginnamen in den Text einfügen lassen (hier im Beispiel `2016-05-06 mk:`).
* Die [.guihint]#Documentation-URL# ist für einen Link auf interne Dokumentation gedacht, die Sie in einem anderen System (z.B. einer CMDB) pflegen.  Sie wird in der Regeltabelle über das Symbol icon:button_url[] anklickbar dargestellt.
...
[.guihint]#Folder#, [.guihint]#Explicit hosts# und [.guihint]#Explicit services# sind unverändert
...
icon:button_new_taggroup[] und bringt Sie zu folgender Eingabemaske:
...
icon:button_top[], icon:button_bottom[], icon:button_up[] und
icon:button_down[] ändern können, hat nicht nur eine optische Funktion:
...
icon:button_sidebar_addsnapin[] unten links in der Seitenleiste zu dieser hinzu.

image::virtual_host_tree_snapin_modern.png[align=center, width=280]
