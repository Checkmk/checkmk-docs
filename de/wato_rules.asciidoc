// -*- coding: utf-8 -*-
// IGNORE rulesets % DB2 Filesystems JVM preselected timespecific zeitbezogenen zeitspezifisch
...
und er steht deswegen auf [.guihint]#Smart PING (only with Checkmk Micro Core),# dem Standardwert in den {EE}.
...
Der letzte Fall -- das icon:icon_rulepmatch[] partielle Greifen einer Regel -- kann nur bei solchen Regelsätzen auftreten, 
...
Diese und verschiedene weitere Regeln können Sie auch zeitspezifisch steuern. So können Sie zum Beispiel die Schwellwerte für die DB2 Tablespace-Nutzung während der Geschäftszeiten anders setzen als am Wochenende.

Klicken Sie erst auf den Knopf [.guihint]#Enable timespecific parameters# und dann auf [.guihint]#Add new element#, werden Ihnen die zeitbezogenen Optionen angezeigt:

image::wato_rules_time_period.png[alt="Ansicht der Regelwerte bei Auswahl zeitspezifischer Parameter."]

Nun wählen Sie in der Liste [.guihint]#Match only during time period# eine Zeitperiode aus. Wie Sie weitere Zeitperioden erstellen, können Sie im Kapitel xref:timeperiods#[Zeitperioden (Time Periods)] nachlesen. 
...
Fügen Sie Bedingungen mit [.guihint]#Add label condition# hinzu.
Wählen Sie nun entweder [.guihint]#has# oder [.guihint]#has not# um eine positive oder negative Bedingung zu formulieren 
...
image::rules_props_labels.png[alt="Bedingung für Service-Labels."]
...
// TK: Stimmt das? Was ist mit Folder oben und den folgenden Bedingungen für Explicit hosts und Services?
