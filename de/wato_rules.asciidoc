...
Host-Merkmale (_Englisch: _Host tags_) sind in *Gruppen* organisiert.
...
(z.B. weil Sie viele verschieden Anwendungen abbilden), können Sie die Merkmalsgruppen
...
Hosts, die das betroffen Merkmal gesetzt hatten, werden automatisch auf den Defaultwert gesetzt.
...
xref:wato_rules#hosttags[Host-Merkmalen] erstellen und so eine „virtuelle“ Hierarchie
abbilden -- und zwar über das Snapin [.guihint]#Virtual Host Tree#. Neben den
Host-Merkmalen dürfen Sie auch die Ordnerstruktur in derlei Bäume einbauen, wobei
