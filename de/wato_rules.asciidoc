...
nur, welche Hosts _drin_ sind und welche nicht.
...
Es es _kein_ zusätzliches Filterkriterium zur Auswahl der Hosts.
...
Angenommen, Sie haben drei Hosts und wollen für jeden dieser Hosts (und auch für alle künftig hinzukommenden Hosts) mit der Regel _Periodic notifications during host problems_ bei Host-Problemen unterschiedliche Benachrichtigungszyklen festlegen:
...
// TK: Hier fehlt die Motivation, warum die Regel jetzt für alle Hosts statt für Host-3 angelegt wird.
...
Wird nun die Regelkette erneut von oben nach unten für die einzelnen Hosts durchlaufen, so ändert sich das Ergebnis:
