...
Stellen Sie nun die Reihenfolge der Regeln um, so ergibt sich daher ein anderes Ergebnis!

* (Neue) Regel A: Host 1 alle 10 Minuten
* (Neue) Regel B: alle Hosts alle 30 Minuten
* (Neue) Regel C: Host 2 alle 20 Minuten

Wird nun die Regelkette erneut von oben nach unten durchlaufen, so ergibt sich:

* Regel A trifft auf Host 1 zu und wird angewendet: die Benachrichtigung für Host 1 erfolgt künftig im 10-Minuten-Takt. Damit ist die Bearbeitung für Host 1 abgeschlossen.

* Regel A trifft auf Host 2 nicht zu. Weiter geht es mit Regel B. Diese trifft für Host 2 zu und wird angewendet: die Benachrichtigung für Host 2 erfolgt künftig im 30-Minuten-Takt. Damit ist die Bearbeitung für Host 2 abgeschlossen.

* Regel A trifft auf Host 3 nicht zu. Regel B aber trifft für Host 3 zu und wird angewendet: die Benachrichtigung für Host 3 erfolgt künftig im 30-Minuten-Takt. Damit ist die Bearbeitung für Host 3 abgeschlossen.

* Obwohl Regel C für Host 2 relevant wäre, wird diese Regel nicht mehr angewendet.
