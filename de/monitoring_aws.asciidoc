...
Um die Überwachung per {CMK} zu ermöglichen, legen Sie am besten dafür einen speziellen AWS-User unterhalb Ihres Root-Accounts an.
link:https://console.aws.amazon.com[Loggen Sie sich^] dafür bei AWS als Root-User ein und navigieren Sie unter [.guihint]#All services# zu [.guihint]#Security, Identity, & Compliance > IAM#
...
Wir empfehlem diesem Benutzer ausschließlich die Policy [.guihint]#ReadOnlyAccess# zuzuweisen.
...
In der Liste unter dem Suchfeld müssen Sie dennoch weit nach unten scrollen, da es eine ganze Reihe Policies gibt, die diesen String enthält:
...
Hier tragen Sie [.guihint]#Access Key ID# des angelegten AWS-User `check-mk` ein.
...
In dem Screenshot sehen Sie außerdem die Option [.guihint]#Use STS AssumeRole to assume a different IAM role#. Sollten Sie einen oder mehrere weitere Accounts bei AWS haben, können Sie mit einem einzigen Monitoring-User alle anderen ebenfalls überwachen.
...
werden und diese ohne eigene Monitoring-Agenten arbeiten.  Dabei wird jeder
...
optional mit dem neuen xref:dcd#[Dynamic Configuration Daemon (DCD)].  Wichtig dabei
