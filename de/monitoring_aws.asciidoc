// -*- coding: utf-8 -*-
// IGNORE AssumeRole ReadOnlyAccess STS Storages EntireBucket Balancer
...
Diesen Filter *müssen* Sie _EntireBucket_ nennen, damit dieser von {CMK} erkannt wird.
...
Wie Sie den sogenannten _(Filter) Scope_ in AWS wählen steht Ihnen ebenfalls frei.
...
Führen Sie also gegebenenfalls eine neuerliche xref:wato_services#discovery[Service Discovery]auf dem AWS-Host durch.
