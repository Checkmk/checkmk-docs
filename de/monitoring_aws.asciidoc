...
* link:https://checkmk.com/de/integrations?distributions%5B%5D=check_mk&distributions%5B%5D=check_mk_cloud&search=aws_s3[Simple Storage Service (S3)^] und 
link:https://checkmk.com/de/integrations?distributions%5B%5D=check_mk&distributions%5B%5D=check_mk_cloud&search=aws_glacier[S3 Glacier^]
...
// TODO: Wenn wir bessere Screenshots haben, kann man auch besser auf die Anwendungsfälle zu diesen Dashboards besser eingehen.

{CCE-only}
Zum komfortablen Einstieg in die Überwachung von AWS liefert die {CE} von {CMK} die beiden eingebauten xref:glossar#dashboard[Dashboards] [.guihint]#AWS EC2 instances# und [.guihint]#AWS S3# mit aus.
Beide finden Sie im Monitoring als Menüeinträge unter [.guihint]#Monitor > Cloud.#

Damit Sie einen direkten Eindruck bekommen, wie diese Dashboards aussehen, finden Sie nachfolgend zwei Beispiele, wie diese Dashboards aufgebaut sind. Zuerst das Dashboard zu den EC2-Instanzen, bei Sie auf der linken Seite den aktuellen Zustand und auf der rechten Seite den zeitlichen Verlauf der wichtigsten Metriken vergleichen können:

image::monitoring_aws_dashboard_vm.png[alt="Eine Übersicht zu den EC2-Instanzen"]

Das Dashboard zu den S3-Buckets ist ganz ähnlich aufgebaut. Auf der rechten Seite finden Sie die aktuelle Speicherauslastung der jeweiligen Buckets. Auf der linken werden wieder die wichtigsten Metriken im zeitlichen Verlauf dargestellt. :

image::monitoring_aws_dashboard_storage.png[alt="Eine Übersicht zu den S3-Buckets"]