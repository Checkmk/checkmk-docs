...
{CMK}-Instanz _Zentralen Instanz_. Von dieser aus werden zentrale Systeme
direkt überwacht.  Außerdem gibt es die Instanzen _entfernte Instanz Site 1_
und _entfernte Instanz Site 2_, welche sich in anderen Netzen befinden und die
...
* *Skalierung*: Das Monitoring selbst erzeugt keinerlei Netzwerkverkehr zwischen der zentralen und entfernten Instanz. Dadurch können hundert und mehr Standorte angebunden werden.
* *Zuverlässigkeit*: Fällt die Netzwerkverbindung zu einer entfernten Instanz aus, so geht das Monitoring dort trotzdem völlig normal weiter. Es gibt keine Lücke in der Datenaufzeichnung und auch keinen Datenstau. Eine lokale Alarmierung funktioniert weiterhin.
* *Einfachheit*: Instanzen können sehr einfach eingebunden und wieder entfernt werden.
* *Flexibilität*: Die entfernten Instanzen sind weiterhin eigenständig und können an dem jeweiligen Standort für das Operating genutzt werden. Das ist insbesondere dann interessant, wenn der „Standort“ auf keinen Fall Zugriff auf den Rest des Monitorings haben darf.
...
eigenverantwortlich gepflegt werden und die Zentrale Instanz lediglich die Aufgabe hat,
...
Alarme u.s.w. auf der zentralen Instanz per WATO zentral gepflegt und dann automatisch
nach Ihren Vorgaben auf die entfernten Instanzen verteilt.
...
. Zentrale Instanz zunächst ganz normal wie eine Einzelinstanz aufsetzen
. Entfernte Instanzen aufsetzen und Livestatus per Netzwerk freigegen
. Auf der zentralen Instanz die entfernten per WATO-Modul icon:icon_sites[] [.guihint]#Distributed monitoring# einbinden
...
==== Zentrale Instanz aufsetzen

An der Zentrale werden keine speziellen Anforderungen gestellt. Das bedeutet,
...
Die entfernten Instanzen werden zunächst als neue Instanzen wie üblich mit `omd
create` erzeugt.  Dies geschieht dann natürlich auf dem (entfernten)
Server, der für die jeweilige entfernte Instanz vorgesehen ist.

*Hinweise*:

* Verwenden Sie für die entfernten Instanzen IDs, die in Ihrem verteilten Monitoring _eindeutig_ sind.
* Die {CMK}-Version der Instanzen darf sich von der Version der zentralen Instanz maximal im Patchlevel unterscheiden (also die Ziffer nach dem `p` bei stabilen Versionen). Andere Versionen _können_ kompatibel sein, müssen aber nicht. Hinweise zu dem Schema der {CMK}-Versionsnummern finden Sie in einem link:cmk_versionen.html[eigenen Artikel].
* Da {CMK} mehrere Instanzen auf einem Server unterstützt, kann die entfernte Instanz auch auf dem gleichen Server laufen.

Hier ist ein Beispiel für das Anlegen einer entfernten Instanz mit dem Namen `remote1`:
...
Created new site remote1 with version 2.0.0.
...
Merken Sie sich das Passwort für `cmkadmin`. Sobald die entfernte Instanz der zentralen untergeordnet wurde,
werden sowieso alle Benutzer durch die von der zentralen Instanz ausgetauscht.
...
==== Entfernte Instanzen in die zentrale einbinden

Die Konfiguration des verteilten Monitorings wird ausschließlich auf der zentralen Instanz
...
entfernten Instanz an:

image::dm_basic_settings.jpg[]

Bei den [.guihint]#Basic settings# ist es wichtig, dass Sie als Site-ID
exakt den Namen der entfernten Instanz verwenden, so wie diese mit `omd create`
erzeugt wurde. Den Alias können Sie wie immer frei vergeben und auch später
ändern.

image::dm_livestatus_settings.jpg[]

Bei den [.guihint]#Livestatus settings# geht es darum, wie die Zentrale Instanz den Status
der entfernten per Livestatus abfragt. Das Beispiel im Screenshot zeigt eine Verbindung
...
gesondert ein. Tragen Sie hier die HTTP-URL zu der Weboberfläche der entfernten Instanz ein und
...
Sie diese, wenn Sie die entfernte Instanz von der zentralen aus mitkonfigurieren möchten. In diesem
...
entfernten Instanz HTTPS unterstützt. Dies muss auf Linux-Ebene auf des entfernten Servers
...
Der Monitoringstatus der (noch leeren) entfernten Instanz ist jetzt schon korrekt
eingebunden. Für das verteilte WATO benötigen Sie noch einen [.guihint]#Login# auf das
WATO der Instanz. Dabei tauscht die zentrale Instanz per HTTP mit der entfernten ein zufällig
...
Verwenden Sie zum Login die Zugangsdaten `cmkadmin` und dem entsprechenden Passwort auf der entfernten Instanz:
...
. Die entfernte Instanz ist gerade gestoppt.
. Die [.guihint]#Multisite-URL of remote site# ist nicht korrekt eingestellt.
. Die entfernte Instanz ist unter dem in der URL eingestellten Hostnamen _von der zentralen aus_ nicht erreichbar.
. Zentrale und entfernte Instanz haben (zu) unterschiedliche {CMK}-Versionen.
. Benutzer und/oder Passwort sind falsch.

Punkte eins und zwei können Sie einfach testen, indem Sie die URL der entfernten Instanz von Hand in Ihrem Browser
aufrufen.
...
|icon:icon_need_restart[] |Diese Instanz hat ausstehende Änderungen. Die Konfiguration stimmt mit der zentralen Instanz überein, aber es sind nicht alle Änderungen aktiviert. Mit dem Knopf [.guihint]#Restart# können Sie diese gezielt für diese Instanz aktivieren.
...
|icon:button_sitestatus_disabled[] |Die Livestatusverbindung zu dieser Instanz ist vorübergehend durch den Administrator (der zentralen Instanz ) deaktiviert worden. Die Einstellung entspricht der Checkbox [.guihint]#Temporarily disable this connection# in der Einstellung dieser Verbindung.
|===
...
überwacht werden soll. Per Default ist die zentrale Instanz eingestellt.
...
und auch die Serviceerkennung von der jeweiligen entfernten Instanz durchgeführt wird,
...
Ab {v16} können Livestatusverbindungen zwischen der zentralen und einer
entfernten Instanz verschlüsselt werden. Bei neu erzeugten Instanzen müssen nichts weiter
...
also von der zentralen weiterhin auf die entfernte Instanz zugreifen können, aktivieren Sie
...
Das ist sehr wichtig, denn es ist überhaupt nicht gesagt, dass die zentrale Instanz
...
`var/check_mk/inventory` von den entfernten zu der zentralen Instanz übertragen werden.
...
müssen Sie das Verzeichnis mit eigenen Mitteln regelmäßig zur zentralen Instanz
...
dies genau wie beim Einrichten einer neuen entfernten Instanz. Geben Sie Livestatus per
...
Es versteht sich von selbst, dass die in der entfernten Instanz verwendeten Hostmerkmale
(Tags) auch in der zentralen Instanz  bekannt sein müssen, damit diese übernommen
...
Als Zweites ziehen die Hosts und Regeln in das zentrale WATO auf der zentralen Instanz
...
entfernten Instanz einfach vorher per WATO in einen Unterordner verschieben.
...
Instanz an die gleiche Stelle in die zentrale Instanz kopieren. Falls es dort bereits ein
...
Nach dem Kopieren sollten die Hosts im zentralen WATO auf der zentralen Instanz auftauchen
...
Damit die Vererbung von Attributen von Elternordnern der zentralen Instanz korrekt
...
*Hinweis*: instanzspezifische Einstellungen für die _zentrale Instanz_ sind nur
...
hat, müssen Sie in jeder einzelnen Instanz eine sitespezifische Einstellung
...
* Syslogziele und SNMP-Trap-Destinations der betroffenen Hosts auf der entfernten Instanz umstellen. Das ist der aufwendigste Teil.
...
den entfernten Instanzen.  Das ist sehr wichtig, da so eine ständige Übertragung
...
. Die PNP4Nagios-Daten werden von der zentralen Instanz abgerufen und an den Benutzer weitergeleitet.
...
des Anwenders aus funktioniert. Ein Zugriff auf die entfernte Instanz von der zentralen aus
...
* Da der Browser die PNP4Nagios-Daten von einem anderen Host als die {CMK}-GUI holt, wird das {CMK}-Sitzungscookie nicht gesendet. Für jede entfernte Instanz muss sich der Benutzer daher einmal neu anmelden. Beim ersten Zugriff auf einen Graphen erscheint dann eine Anmeldemaske.
* Die entfernte Instanz ist eventuell vom Browser des Benutzers gar nicht erreichbar -- sondern nur von der zentralen aus. In diesem Fall funktioniert die Methode gar nicht.
...
==== Abruf durch die zentrale Instanz
...
in Ihrem Netzwerk eindeutig sind, denn Apache muss anhand der ID der entfernten Instanz
...
Setzen Sie nun in der Verbinungseinstellung als URL-Präfix lediglich `/remote1/` ein:

image::dm_url_prefix_proxy.png[]

Dadurch gehen Anfragen an PNP4Nagios zunächst an die zentrale Instanz an der URL `/remote1`.
Sollte die Instanz `remote1` zufällig auf dem gleichen Server laufen wie die
Zentrale, sind Sie jetzt schon fertig und brauchen auch keine Proxyregel, da die Daten
dann direkt ausgeliefert werden können.

Im allgemeinen Fall, dass die entfernte Instanz auf einem anderen Host läuft, benötigen Sie
...
Die Datei besteht pro angebundener entfernter Instanz aus fünf Zeilen. Ersetzen Sie
...
auf der entfernten Instanz ab, von dem aus der entsprechende Server überwacht wird. Im
verteilten Monitoring hat die Zentrale aber keinen direkten Zugriff auf diese Dateien. Die
Lösung ist die gleiche wie bei PNP4Nagios: Die Logwatchwebseite der entfernten Instanz wird eingebettet und separat per HTTP dieser geholt.
...
einer anderen entfernten Instanz umziehen, müssen Sie danach Ihre NagVis-Karten
...
. Nehmen Sie den Host, auf dem die entfernte Instanz läuft, auf der zentralen ins Monitoring auf.
. Tragen Sie diesen bei der Verbindung zur entfernten Instanz als Statushost ein:

image::dm_status_host_2.png[]

Eine ausgefallene Verbindung zur entfernten Instanz kann jetzt nur noch für kurze
...
|icon:button_sitestatus_unreach[] |Der Rechner, auf dem die entfernte Instanz läuft, ist für das Monitoring gerade nicht erreichbar, weil ein Router dazwischen down ist (Statushost hat den Zustand {UNREACH}).
|icon:button_sitestatus_waiting[] |Der Statushost, der die Verbindung zu der entfernten Instanz überwacht, wurde noch nicht vom Monitoring geprüft (steht noch auf {PEND}).
...
dazu veranlassen, einmal aufgebaute Livestatusverbindungen zu entfernten Instanzen
...
Sie bitte bei der Konfiguration des Nagioskerns der entfernten Instanz für eine
...
Wählen Sie nun bei den Verbindungen zu der entfernten Instanz anstelle von „Connect via TCP“ die Einstellung
„[.guihint]#Use Livestatus Proxy-Daemon#“:

image::dm_livestatusproxy.jpg[]

Die Angaben zu Host und Port sind wie gehabt. Auf den entfernten Instanzen müssen Sie
...
erhöhen. Beachten Sie dabei jedoch, dass auf der Gegenstelle (dem entfernten Instanz)
...
einfache Livestatus-Anfrage, welche von der entfernten Instanz in der eingestellten Zeit
...
Das [.guihint]#Caching# sorgt dafür, dass statische Anfragen nur einmal von der entfernten Instanz
...
finden. Bei einem korrekt eingerichteter entfernter Instanz mit fünf Kanälen (Standard), sieht das
etwa so aus:
...
_von der zentralen Instanz auf die entfernten_. Es gibt Situtationen, in denen das entweder nicht
...
* die Instanzen keine permanente Netzwerkverbindung und keine festen IP-Adressen haben.

Das verteilte Monitoring mit Livedump bzw. CMCDump geht einen ganz anderen Weg.
Zunächst einmal sind die entfernten Instanzen so aufgesetzt, dass sie völlig unabhängig von
der zentralen arbeiten und _denzentral administriert_ werden. Auf ein verteiltes
WATO wird verzichtet.

Dann werden in der zentralen Instanz alle Hosts und Services der entfernten Instanzen als _Kopie_ angelegt.
Dazu kann Livedump/CMCDump einen Abzug der Konfiguration der entfernten Instanzen erstellen, der
bei der zentralen eingespielt wird.

Während des Monitorings wird nun auf jeder entfernten Instanz einmal pro definiertem Intervall
...
* Eine Berechnung der Verfügbarkeit wird auf der zentralen Instanz, im Vergleich mit der entfernten, geringfügig abweichende Werte ergeben.
* Zustandswechsel, die schneller geschehen als das Aktualisierungsintervall, sind für die Zentrale unsichtbar.
* Ist eine entfernte Instanz „tot“, so veralten die Zustände auf der zentralen, die Services werden „stale“, sind aber immer noch sichtbar. Messdaten und Verfügbarkeitsdaten gehen für diesen Zeitraum verloren (auf der entfernten Instanz sind sie noch vorhanden).
* Kommandos wie Downtimes und Acknowledgements auf der zentralen Instanz können _nicht_ auf die entfernte übertragen werden.
* Zu keiner Zeit erfolgt ein Zugriff von der zentralen auf die entfernte Instanz.
...
Da kurze Zustandswechsel bedingt durch das gewählte Intervall für die zentrale Instanz
...
kann der Livedump auch in einen der entfernten Livestatus-Instanzen eingespielt werden.
...
* die entfernte Instanz bereits voll eingerichtet ist und fleißig Hosts und Services überwacht,
* die zentrale Instanz gestartet ist und läuft und
* auf dem Zentrale _mindestens ein Host_ lokal überwacht wird (z.B. weil sie sich selbst überwacht).

==== Übertragen der Konfiguration

Als Erstes Erzeugen Sie auf der entfernten Instanz einen Abzug der Konfiguration ihrer
...
Übertragen Sie die Datei zu der zentralen Instanz (z.B. mit `scp`) und legen Sie sie
...
von der entfernten auf die zentrale Instanz möglich ist, geht das z.B. so:
...
Loggen Sie sich jetzt auf der zentralen Instanz ein und aktivieren Sie die Änderungen:
...
Nun sollten alle Hosts und Services der entfernten Instanz in der zentralen auftauchen --
...
* Durch die Option `-T` bei `livedump` erzeugt Livedump Template-Definitionen, auf die sich die Konfiguration bezieht. Ohne diese kann Nagios nicht gestartet werden. Sie dürfen jedoch _nur einmal_ vorhanden sein. Falls Sie auch von einem zweiten entfernten Instanz eine Konfiguration übertragen, so dürfen Sie die Option `-T` dort *nicht* verwenden!
* Der Dump der Konfiguration ist auch auf einem link:cmc.html[CMC-Kern] möglich, das Einspielen benötigt Nagios. Wenn auf Ihrer zentralen Instanz der link:cmc.html[CMC] läuft, dann verwenden Sie link:distributed_monitoring.html#cmcdump[CMCDump].
* Das Abziehen und Übertragen der Konfiguration müssen Sie nach jeder Änderung von Hosts oder Services auf der entfernten Instanz wiederholen.


==== Übertragung des Status

Nachdem die Hosts in der zentralen Instanz sichtbar sind, geht es jetzt an die (regelmäßige)
Übertragung des Monitoringstatus der entfernten Instanz. Wieder erzeugen Sie mit `livedump`
...
Übertragen Sie diese Datei auf die zentrale Instanz in das Verzeichnis `~/tmp/nagios/checkresults`. *Wichtig:* Der Name der Datei muss mit `c` beginnen und sieben Zeichen lang sein. Mit `scp` würde das etwa so aussehen:
...
Anschließend erzeugen Sie auf der zentralen Instanz eine leere Datei mit dem gleichen Namen
...
Der Zustand der Hosts/Services der entfernten Instanz wird jetzt auf die zentrale sofort
aktualisiert:
...
um Livedumpupdates (auch solche von der Konfiguration) per SSH auf der zentralen Instanz
...
Sie diese Datei auf der zentralen Instanz unterhalb von `etc/check_mk/conf.d/`. Die
...
Aktivieren Sie auf der zentralen Instanz die Konfiguration:
...
direkt auf der zentralen Instanz oder einer „normalen“ entfernten Instanz überwachten
...
Zum Einspielen des Status auf der zentralen Instanz muss der Inhalt der Datei mithilfe des
...
Falls Sie per SSH einen passwortlosen Zugang von der entfernten zur zentralen Instanz haben,
...
enternten Instanzen oder von der zentralen aus. Für beide gibt es Argumente.

Argumente für den Versand von den entfernten Instanzen aus:

* Einfacher einzurichten.
* Alarmierung vor Ort geht auch dann, wenn die Verbindung zur Zentrale nicht verfügbar ist.
* Geht auch mit der {CRE}.

Argumente für den Versand von der zentralen Instanz aus:

* Alarme können an einer zentralen Stelle weiterbehandelt werden (z.B. für Weiterleitung in Ticketsystem).
* Entfernte Instanzen benötigen kein Setup für Email oder SMSl
...
der auf einer entfernten Instanz entsteht, durchläuft dort die Kette der
...
zentralisiertes Alarmieren, welcher pro entfernter Instanz einzeln aktiviert werden
kann. Solche entfernten Instanzen leiten dann alle Alarme zur weiteren Verarbeitung
an die zentrale weiter. Dabei ist die zentrale Alarmierung unabhängig
...
„Zentrale Instanz“ sein. Diese Aufgabe kann jede {CMK}-Instanz übernehmen.

Ist eine enternte Instanz auf Weiterleitung eingestellt, so werden alle Alarme
direkt wie Sie vom Kern kommen -- quasi in _Rohform_ -- an die zentrale
...
notwendigen Alarmierungsskripte werden auf der zentralen Instanz aufgerufen.
...
Instanzen. Dies ist ein Hilfsprozess, der _sowohl auf der zentralen, als
auch auf den entfernten Instanzen_ benötigt wird. In neueren {CMK}-Versionen ist
...
Die Alarmspooler von enternter und zentraler (Alarmierungs-)Instanz tauschen sich untereinander
per TCP aus. Alarme werden von der enternten zur zentralen Instanz gesendet. Die Zentrale quittiert
empfangene Alarme an die entfernte Instanz, was verhindert, dass Alarme verloren gehen,
selbst wenn die TCP-Verbindung abbrechen sollte.

Für den _Aufbau_ der TCP-Verbindung haben Sie zwei Möglichkeiten:

. TCP-Verbindung wird von zentraler zur entfernten Instanz aufgebaut. Hier ist die entfernte Instanz der _TCP-Server_.
. TCP-Verbindung wird von entfernter zur zentralen Instanz aufgebaut. Hier ist die zentrale Instanz der _TCP-Server_.
...
Da entfernte und zentrale Instanz für den Spooler unterschiedliche globale Einstellungen
brauchen, müssen Sie für _alle_ entfernten Instanzen
...
Einstellungen für die lokale Instanz (= zentrale Instanz) unterstützt. Bitte
beachten Sie, dass diese automatisch an alle entfernten Instanzen vererbt wird, für die
Sie _keine_ spezifischen Einstellungen definiert haben.

Betrachten Sie zuerst den Fall, dass die zentrale Instanz die TCP-Verbindungen zu den
entfernten aufbauen soll.

Schritt 1: Editieren Sie *bei der entfernten Instanz* die instanzspezifische globale Einstellung
...
Schritt 2: Setzen Sie nun ebenfalls nur *auf der entfernten Instanz* die Einstellung [.guihint]#Notification Spooling#
auf den Wert [.guihint]#Forward to remote site by notification spooler#.

image::mknotifyd_spool.png[]

Schritt 3: Auf dem *zentralen Instanz* -- also in den normalen globalen Einstellungen -- richten
Sie nun zu der entfernten (und später dann auch eventuell zu weiteren entfernten) die
Verbindungen ein:
...
==== Verbindungsaufbau von der entfernten Instanz aus

Soll die TCP-Verbindung von der entfernten Instanz aus aufgebaut werden, so ist das Vorgehen
identisch, bis auf die Tatsache, dass Sie die oben gezeigten Einstellungen
einfach zwischen zentraler und entfernter Instanz vertauschen.
...
zu entfernten Instanzen aufbaut. Für jede Beziehung zwischen zentraler und entfernter Instanz darf aber
...
sollten Sie auf der zentralen Instanz etwa Folgendes sehen:
...
Die gleiche Datei gibt es auch auf der entfernten Instanz. Dort sieht die Verbindung
etwa so aus:
...
Zum Testen wählen Sie z.B. einen beliebigen Service, der auf der entfernten Instanz
...
Das gleiche Ereignis sieht bei der entfernten Instanz so aus:
...
sowohl auf der zentralen, als auch auf den entfernten Instanzen jeweils ein neuer Service gefunden
...
überwacht: einmal durch die zentrale und einmal durch die entfernte Instanz:
...
|etc/check_mk/conf.d/distributed_wato.mk |Wird auf den entfernten Instanzen vom verteilten WATO erzeugt und sorgt dafür, dass sie nur ihre eigenen Hosts überwacht.
|etc/nagios/conf.d/ |Ablageort für selbst erstellte Nagios-Konfigurationsdateien mit Hosts und Services. Dies wird beim Einsatz von link:distributed_monitoring.html#livedump[Livedump] auf der zentralen Instanz benötigt.
...
|var/check_mk/autochecks/ |Von der Serviceerkennung gefundene Services. Dieses werden immer lokal auf der entfernten Instanz gespeichert.
