// -*- coding: utf-8 -*-
// IGNORE % Benachrichtigungs Huckepackdaten Konfigurations betreiber1 kunde1 kunde2 kunde3 p11 se ⇄
...
Manche Check-Plugins verwenden Piggyback-Daten („Huckepackdaten“), um z.B. Überwachungsdaten,
...
Falls Sie mit der {RE} in einem verteilten System Inventarisierung verwenden,
...
Dies geschieht analog zu den aktiven Zuständen über Livestatus und funktioniert sowohl mit der {CRE} als auch mit den kommerziellen Editionen.
...
{cre-only}
Die Konfiguration von _Status-Hosts_ ist der bei der {CRE} empfohlene Weg, defekte Verbindungen zuverlässig zu erkennen.
...
[cols=2]
|===
|icon:icon_status_label_unreach[width=70px] |Der Rechner, auf dem die Remote-Instanz läuft, ist für das Monitoring gerade nicht erreichbar, weil ein Router dazwischen down ist (Status-Host hat den Zustand {UNREACH}).
|icon:icon_status_label_waiting[width=70px] |Der Status-Host, der die Verbindung zur Remote-Instanz überwacht, wurde noch nicht vom Monitoring geprüft (steht noch auf {PEND}).
|icon:icon_status_label_unknown[width=70px] |Der Zustand des Status-Hosts hat einen ungültigen Wert (sollte nie auftreten).
|===
...
{cre-only}
Wenn Sie die {CRE} einsetzen (oder die kommerziellen Editionen mit Nagios als Kern), dann verwenden Sie das Werkzeug `livedump`.
