...
|etc/check_mk/conf.d/ |Konfiguration von Hosts und Regeln für {CMK}. Legen Sie hier Konfigurations­dateien ab, die per link:distributed_monitoring.html#cmcdump[CMCDump] erzeugt wurden. Nur das Unterverzeichnis `wato/` wird per WATO verwaltet und ist dort sichtbar.
