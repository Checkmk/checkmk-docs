...
== Verteiltes Monitoring mit Livestatus#livestatus

=== Grundprinzip

==== Zentraler Status

[livestatus|Livestatus] ist eine in den link:cmc.html[Monitoringkern] integrierte
...
=== Verteiltes Monitoring Aufsetzen#distr_wato_config
...
. Auf dem Master Slaveinstanzen per WATO-Modul ICON[icon_sites.png] [.guihint]#Distributed monitoring# einbinden
...
image::livestatus_tcp_1.png[center, width=380]

Setzen Sie [.guihint]#LIVESTATUS_TCP}} auf {{on# und tragen Sie für
[.guihint]#LIVESTATUS_TCP_PORT# eine freie Portnummer ein, die auf diesem Server
eindeutig ist. Der Default dafür ist 6557:

image::livestatus_tcp_3.png[center, width=380]
...
[.guihint]#Distributed monitoring# und dient dem Verwalten der Ver&shy;bindungen zu den
...
[omd_https|von Hand] aufgesetzt werden. Bei der link:index.html#cma[{CMK} Appliance] kann
...
|Diese Instanz hat ausstehende Änderungen. Die Konfiguration stimmt mit dem Master überein, aber es sind nicht alle Änderungen aktiviert. Mit dem Knopf [.guihint]#Restart# können Sie diese gezielt für diese Instanz aktivieren.

|ICON[icon_need_replicate.png]
|Die WATO-Konfiguration dieser Instanz ist nicht synchron und muss übertragen werden. Danach ist dann natürlich auch ein Neustart notwendig, um die Konfiguration zu aktiveren. Beides zusammen erreichen Sie mit dem Knopf [.guihint]#Sync & Restart#.
|===
...
|Die Instanz ist gerade nicht erreichbar. Livestatus-Anfragen laufen in einen _Timeout_. Dies verögert den Seitenaufbau. Statusdaten dieser Instanz sind in der GUI nicht sichtbar.
...
|Die Livestatusverbindung zu dieser Instanz ist vorübergehend durch den Administrator (des Masters) deaktiviert worden. Die Einstellung entspricht der Checkbox [.guihint]#Temporarily disable this connection# in der Einstellung dieser Verbindung.
...
image::folder_monitored_on.png[center, width=550]
...
* Historische [graphing|Metriken] gehen verloren. Dies können Sie vermeiden, indem Sie betroffene RRD-Dateien von Hand kopieren. Die Lage der Dateien finden Sie unter link:distributed_monitoring.html#files[Dateien und Verzeichnisse].
...
=== Livestatus verschlüsselt anbinden#livestatus_tls
...
image::distributed_monitoring_tls.png[center, width=69%]
...
[notifications|Alarmierung], link:alert_handlers.html[Alerthandler] und alles andere.
...
image::dm_service_details.png[center, width=480]
...
image::dm_filter_site.png[center, width=270]
...
image::snapin_site_status.png[center, width=240]
...
image::dm_master_control.png[center, width=240]
...
image::dm_change_password.png[left, width=150]
...
=== Instanzspezifische globale Einstellungen#sitespecific
...
ICON[icon_sites.png] [.guihint]#Distributed monitoring#:
...
* Falls Sie den Regelsatz [.guihint]#Check event state in Event Console}} verwenden, diesen wieder auf {{Connect to the local Event Console# umstellen.
...
* In den [.guihint]#Settings}} der Event Console den {{Access to event status via TCP}} wieder auf {{no access via TCP# zurückschalten.


=== PNP4Nagios#pnp4nagios

[CRE] In der {CRE} kommt zur Visualisierung von link:graphing.html[Messwerten] das Open-Source-Projekt
...
image::nagvis.png[left, width=170]
...
einzelnen Sites nennt man [.guihint]#Backends}} (Deutsch: {{Datenquellen#).
...
== Instabile oder langsame Verbindungen#wan
...
[CRE] Mit der Checkbox [.guihint]#Use persistent connections# können Sie die GUI
...
=== Der Livestatus-Proxy#livestatusproxy
...
* Der Dump der Konfiguration ist auch auf einem [cmc|CMC-Kern] möglich, das Einspielen benötigt Nagios. Wenn auf Ihrem Master der [cmc|CMC] läuft, dann verwenden Sie link:distributed_monitoring.html#cmcdump[CMCDump].
...
* ICON[icon_ack.png] link:basics_ackn.html[Quittierung], falls vorhanden und
* ob das Objekt gerade in einer ICON[icon_downtime.png] link:basics_downtimes.html[geplanten Wartungszeit] ist.
...
== Alarmierung in verteilten Umgebungen#notifications
...
link:notifications.html#rules[Alarmierungs&shy;regeln].  Falls Sie verteiltes WATO
...
image::omd_config_mknotifyd.png[center, width=300]
...
[cols=<th style="width: 44%">Pfad</th>,options="header"]
|===
...
|Wird auf den Slaves vom verteilten WATO erzeugt und sorgt dafür, dass der Slave nur seine eigenen Hosts überwacht.



|`etc/nagios/conf.d/`
|Ablageort für selbst erstellte Nagios-Konfigurationsdateien mit Hosts und Services. Dies wird beim Einsatz von link:distributed_monitoring.html#livedump[Livedump] auf dem Master benötigt.
...
|`etc/check_mk/conf.d/`
|Konfiguration von Hosts und Regeln für {CMK}. Legen Sie hier Konfigurations&shy;dateien ab, die per link:distributed_monitoring.html#cmcdump[CMCDump] erzeugt wurden. Nur das Unterverzeichnis `wato/` wird per WATO verwaltet und ist dort sichtbar. 


|`var/check_mk/autochecks/`
|Von der Serviceerkennung gefundene Services. Dieses werden immer lokal auf dem Slave gespeichert.


|`var/check_mk/rrds/`
|Ablage der Round-Robin-Datenbanken für die Archivierung der Messwerte beim Einsatz des {CMK}-RRD-Formats (Default bei den {EE}).
