...
// IGNORE % Ausfälle Backends Benachrichtigungs Benachrichtigungsregeln Benachrichtigungsserver Benachrichtigungsskripte
// IGNORE Benachrichtigungslog Benachrichtigungssystems Huckepackdaten Instanzspezifische Checkausführung Checkversuch
// IGNORE Checkversuchs Konfigurations LIVESTATUS_TCP LIVESTATUS_TCP_PORT PNP4Nagios Portnummer Replikationsstatus
// IGNORE Roundtrips Spoolers Spoolings Statushost Statushosts Syslogziele Verfügbarkeitsdaten betreiber1 durchreichen
// IGNORE geclusterten instanzspezifische kunde1 kunde2 kunde3 mitkonfigurieren p11 passwortlosen se selbstsigniertes
// IGNORE spooler vCenters ⇄
...
* Die {CMK}-Version (z.B. {v20}) der Remote- und Zentralinstanz ist dieselbe –- ein xref:mixed_versions[Mischbetrieb] wird nur für die leichtere Durchführung von Updates unterstützt.
...
Instanz des Internet Super Servers `xinetd`, welcher direkt in der Instanz läuft:
...
// ML: Bitte prüfen: "auf 1.6" durch "als 1.6" ersetzt - falls der Absatz auch für 1.6 auf 2.0 gilt, gegebenenfalls komplett umschreiben.

Die Option `LIVESTATUS_TCP_TLS` wird bei einem Update von einer älteren Version als {v16} aus Kompatibilitätsgründen nicht automatisch aktiviert, da danach die Verbindung nur noch verschlüsselt möglich ist.
...
In den {CEE} geschieht die Synchronisation automatisch auf allen Instanzen, bei
...
[cols=2]
|===
|icon:icon_status_label_unreach[width=70px] |Der Rechner, auf dem die Remote-Instanz läuft, ist für das Monitoring gerade nicht erreichbar, weil ein Router dazwischen down ist (Statushost hat den Zustand {UNREACH}).
|icon:icon_status_label_waiting[width=70px] |Der Statushost, der die Verbindung zur Remote-Instanz überwacht, wurde noch nicht vom Monitoring geprüft (steht noch auf {PEND}).
|icon:icon_status_label_unknown[width=70px] |Der Zustand des Statushosts hat einen ungültigen Wert (sollte nie auftreten).
|===
...
* Ein Zugriff auf Details der von Logwatch überwachten Logdateien ist nicht möglich.
...
_Livestatus_ und _Status-Dump_.
...
[#mixed_versions]
== Versionsunterschiede zwischen Master und Remote
// MA: Ich finde den Titel ungünstig, weil ich als Leser anhand des Inhaltsverzeichnis auf der rechten Seite nicht direkt erkennen kann, worum es geht (ggf. ja genau das Thema, was ich suche)
// MFS: So besser?

// MFS: Mischbetrieb weiter hoch ziehen?
// MFS: Technische Hintergründe evtl. in update_major? Weekly 2023-02-02
...
Hier ist ist zwischen folgenden Fällen zu unterscheiden:

=== Abweichende Patchlevel (gleiche Major-Version)

Die Patchlevel (z.B. p11) können zwar in den meisten Fällen unterschiedlich sein.
...
Für die reibungslose Durchführung von xref:update_major#[Major-Updates] (beispielsweise von {v21} auf {v22}) in großen verteilten Umgebungen erlaubt {CMK} 
// MA: Kommt nach "Umgebungen" eigentlich ein Komma?
seit {v20} einen Mischbetrieb, bei dem die Differenz genau eine Major-Version sein darf.
// MA: Hier muss man beim Lesen korrekt betonen, damit der Inhalt klar wird: Die Differenz darf nicht eine Major-Version sein, sondern nur *eine* Major-Version.
Dabei ist darauf zu achten, dass Sie die Remote-Instanzen zuerst aktualisieren,
// MA: Lieber "Dabei ist darauf zu achten, dass Sie die Remote-instanzen zuerst aktualisieren, ...". Es muss immer klar sein, wer das macht.
// MFS: Done.
die Zentralinstanz aber erst, wenn alle Remote-Instanzen auf dem höheren Versionsstand sind.

Auch hier gilt: Prüfen Sie unbedingt die Werks, sowie die Hinweise zum Versions-Update, _bevor_ sie mit dem Update beginnen!
// sollten Sie die Werks auf die exakten Anforderungen zu prüfen.
// MA: "sollten" sollten wir vermeiden. ;) Entweder gilt es, oder es gilt nicht. Wenn es nur unter bestimmten Umständen gilt, dann gilt es immernoch zusammen mit den Bedingungen.
// MFS: "unbedingt"!
So ist immer ein bestimmtes Patchlevel der älteren Version auf der Zentralinstanz erforderlich, um einen reibungslosen Mischbetrieb zu gewährleisten.
...
// MA: Für beide Versionen von was?
