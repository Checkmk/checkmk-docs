...
xref:omd_basics#[Die Monitoring-Instanzen (Sites)]
{related-end}
...
Dieses zeigt den Status der einzelnen Instanzen und bietet darüber hinaus die Möglichkeit, vorübergehend einzelne Instanz durch einen Klick auf den Status aus- und wieder einzublenden. Diese werden dann mit dem Status icon:icon_status_label_disabled[width=70px] angezeigt.
Sie können so auch eine Instanz, die icon:icon_status_label_dead[width=70px] ist und somit Timeouts erzeugt, abschalten und die Timeouts damit vermeiden:
