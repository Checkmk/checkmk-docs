...
Verwenden Sie als [.guihint]#Login credentials# `cmkadmin` und das generierte Passwort der Remote-Instanz. Bestätigen Sie an dieser Stelle noch per [.guihint]#Confirm overwrite,# dass Sie mit die Einstellungen überschreiben wollen.
...
Auf der Übersichtsseite der noch nicht aktivierten Änderungen sehen Sie auch eine Übersicht der Status der Livestatus-Verbindungen sowie des Synchronisationszustands der verteilten Konfigurationsumgebung der einzelnen Instanzen:
...
[cols=2]
|===
|icon:status_label_online[width=80px] |Die Instanz ist per Livestatus erreichbar.
|icon:status_label_dead[width=80px] |Die Instanz ist gerade nicht erreichbar. Livestatus-Anfragen laufen in einen _Timeout_. Dies verzögert den Seitenaufbau. Statusdaten dieser Instanz sind in der GUI nicht sichtbar.
|icon:status_label_down[width=80px] |Die Instanz ist gerade nicht erreichbar, aber das ist aufgrund der Einrichtung eines Statushosts oder durch den xref:livestatusproxy[Livestatus-Proxy] bekannt (siehe xref:wan[unten]). Die Nichterreichbarkeit führt *nicht* zu Timeouts. Statusdaten dieser Instanz sind in der GUI nicht sichtbar.
|icon:status_label_disabled[width=80px] |Die Livestatus-Verbindung zu dieser Instanz ist vorübergehend durch den Administrator (der Zentralinstanz) deaktiviert worden. Die Einstellung entspricht der Checkbox [.guihint]#Temporarily disable this connection# in der Einstellung dieser Verbindung.
|===
...
* Automatisch erkannte Services werden nicht übernommen. Führen Sie daher nach dem Umziehen eine xref:wato_services#[Serviceerkennung] durch.
...
* Daten zur Verfügbarkeit und zu historischen Ereignissen gehen verloren. Diese sind leider nicht so einfach zu migrieren, da diese Daten sich in einzelnen Zeilen im Monitoringlog befinden.
...
das Instanz-Zertifikat durch Ihr eigenes aus und stellen sicher, dass die CA,
...
Damit Sie also von der Zentralinstanz weiterhin auf die Remote-Instanz zugreifen können, muss gewährleistet sein, dass im Menü unter [.guihint]#Setup > General > Distributed Monitoring# bei der jeweiligen Instanz unter [.guihint]#Encryption# die Option [.guihint]#Encrypt data using TLS# ausgewählt ist. Prüfen Sie dies und stellen Sie die Option gegegenenfalls, wie im folgenden Screenshots zu sehen ist, um:
...
Dieses zeigt den Status der einzelnen Instanzen und bietet darüber hinaus die Möglichkeit, vorübergehend einzelne Instanz durch einen Klick auf den Status aus- und wieder einzublenden. Diese werden dann mit dem Status icon:status_label_disabled[width=80px] angezeigt.
Sie können so auch eine Instanz, die icon:status_label_dead[width=80px] ist und somit Timeouts erzeugt, abschalten und die Timeouts damit vermeiden:

image::distributed_monitoring_snapin_site_status.png[width=50%]

Dies entspricht *nicht* dem Abschalten der Livestatus-Verbindung über die Verbindungskonfiguration im [.guihint]#Setup#.
...
TCP-Pakete), wartet die GUI einen gewissen Timeout ab, bevor eine Instanz als icon:status_label_dead[width=80px] deklariert wird und mit den Antworten der übrigen Instanzen fortgefahren wird.
...
Immerhin haben wir ein Monitoringsystem zur Verfügung!
Die GUI kennt dann nicht erreichbare Instanzen und kann diese sofort ausklammern und als icon:status_label_down[width=80px] werten.
...
[cols=2]
|===
|icon:status_label_unreach[width=80px] |Der Rechner, auf dem die Remote-Instanz läuft, ist für das Monitoring gerade nicht erreichbar, weil ein Router dazwischen down ist (Statushost hat den Zustand {UNREACH}).
|icon:status_label_waiting[width=80px] |Der Statushost, der die Verbindung zur Remote-Instanz überwacht, wurde noch nicht vom Monitoring geprüft (steht noch auf {PEND}).
|icon:status_label_unknown[width=80px] |Der Zustand des Statushosts hat einen ungültigen Wert (sollte nie auftreten).
|===
