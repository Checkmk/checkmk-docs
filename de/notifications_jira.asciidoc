...
// SK: Wenn ich dieses Thema richtig verstehe, liegt diese Einschränkung ja in der jeweiligen Jira App begründet. Ich würde es an dieser Stelle deshalb nicht ansprechen.
...
// SK: Korrekt. Dieses Feld bedarf meines Erachtens nicht der weiteren Beschreibung.
...
Sollten nach der Einrichtung der Benachrichtigungsregel in {CMK} keine Tickets in Jira ankommen, prüfen Sie bitte die Entsprechende Log-Datei unter ~/var/log/notify.log. Von Jira kommen hier im Regelfall recht brauchbare Fehlermeldungen zurück, die Ihnen bei der Diagnose tatsächlich helfen können. Im folgenden listen wir einige Beispiele auf:

==== Fehlermeldung: Unable to create issue, JIRA response code 400, Field 'labels' cannot be set.
Eventuell verfügt Ihr verwendetes Jira-Produkt nicht über Labels. Schalten Sie die Verwendung von Labels in Ihrer Benachrichtigungsregel in {CMK} einfach ab, indem Sie den Haken vor [.guihint]#Label# wieder entfernen.

==== Fehlermeldung: Unable to create issue, JIRA response code 400, b'project is required'.
Diese Fehlermeldung weißt darauf hin, dass die ID, welche Sie in der Benachrichtigungsregel für das monierte Feld (hier: Project ID), eingetragen haben, nicht korrekt ist.

==== Fehlermeldung: Unable to resolve https://myjira.mycompany.com/browse/ISSUE-123, JIRA response code 500, b'Internal server error'

Wenn diese Fehlermeldung auftritt, wenn ein Ticket in Jira von {CMK} wieder automatisch geschlossen bzw. in einen anderen Status versetzt werden soll, dann *kann* diese Fehlermeldung ein Hinweis darauf sein, das die Transaction ID, welche Sie in der Benachrichtigungsregel in das Feld [.guihint]#Activate resolution with following resolution transition ID# eingetragen haben, nicht korrekt ist. Gleichen Sie die eingetragene ID in der Regel dann erneut mit der Weboberfläche von Jira ab.
