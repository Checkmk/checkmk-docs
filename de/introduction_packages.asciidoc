...
Installieren Sie nun das Paket *inklusive seiner Abhängigkeiten*. Verwenden Sie
dazu `apt install` (Debian, Ubuntu),`zypper install` (SLES) oder `yum install`
(RedHat, CentOS). Stellen Sie vorher sicher, dass Pakete vom Netz
