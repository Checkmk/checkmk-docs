...
In xref:wato_hosts#parents[{CMK} konfiguriert] werden dabei nur _direkte_ Parents.
...
Nehmen wir an, dass in der oben gezeigten beispielhaften Netzwerktopologie die Hosts _myhost_ und _myhost4_ ausfallen.
Dann geht das Monitoring automatisch davon aus, dass der Ausfall von _myhost4_ einfach dadurch erklärbar ist, dass dieser vom Monitoring nicht mehr erreicht werden kann, da der _einzige_ Parent _myhost_  {DOWN} ist.
Daher wird _myhost4_ im Monitoring anstelle von {DOWN} als {UNREACH} klassifiziert.
Dann greift die wichtige Regel, dass (per Default) zu Hosts mit dem Status {UNREACH} _keine_ xref:notifications#[Benachrichtigungen] erfolgen.
Denn das ist die wichtigste Aufgabe des Konzepts der Parents:
Die Vermeidung von massenhaften Fehlalarmen, falls ein ganzes Netzwerksegment für das Monitoring nicht mehr erreichbar ist.
// MA: Sprachlich etwas schwierig, da oft "Dann" oder "Denn" vorkommt. Mein Vorschlag:
////
Nehmen wir an, dass in der beispielhaften Netzwerktopologie die Hosts _myhost_ und _myhost4_ nicht mehr erreichbar sind.
Der Ausfall von _myhost4_ ist dabei aber auch dadurch erklärbar, dass _myhost_ ausgefallen ist.
Daher wird _myhost4_ im Monitoring als {UNREACH} klassifiziert.
Es ist schlicht nicht eindeutig feststellbar, weswegen {CMK} _myhost4_ nicht mehr erreichen kann und
der Status {DOWN} wäre daher unter Umständen irreführend.
Stattdessen bewirkt das {UNREACH} standardmäßig die Unterdrückung einer xref:notifications#[Benachrichtung].
Denn das ist die wichtigste Aufgabe, des Konzepts der Parents:
Die Vermeidung massenhafter Benachrichtigungen,
...
Hier werden Benachrichtigungen über einen ausgefallenen Host teilweise kurz zurückgehalten und erst dann versendet, wenn _gesichert_ ist, dass der Parent noch erreichbar ist.
// MA: Etwas irreführend, weil ja nicht die Benachrichtigung, sondern der Alarm (bzw. Statuswechsel kurz zurückgehalten wird). Vorschlag:
////
...
Übrigens findet die Überwachung von _myhost4_ trotzdem weiterhin statt.
Sollte es dieser Host schaffen zu antworten, obwohl sein Parent in {CMK} nicht erreichbar ist, wird er als {UP} eingestuft.
// MA: Man könnte das {DOWN} durch "nicht erreichbar" ersetzen. Das rückt die Farbgebung mehr auf den Child-Host.

In manchen Fällen hat ein Host mehrere Parents. Zum Beispiel, wenn ein Router hochverfügbar in einem Cluster betrieben wird. Für {CMK} reicht es, wenn einer dieser Parents erreichbar ist, um den Status eines Host eindeutig zu bestimmen. Falls ein Host also _mehrere_ Parents hat und zumindest einer dieser Parents {UP} ist, wird der Host im Monitoring als erreichbar betrachtet.
...
xref:notifications#[Benachrichtigung] führen. {CMK} ist so voreingestellt, dass im Falle eines
...
* für den Host/Service ein Benachrichtigungszeitraum (_notification period_) definiert wurde, die gerade nicht aktiv ist (siehe unten),
...
// ML: Das erste Icon galt mal für Service UND Host - hier angepasst. Die Klammern beim zweiten Punkt, weil es trotz "derived" auch beim Host selbst so zu sehen ist. Beide Punkte vorläufig - in der Tabelle unten nochmal die gleiche Anpassung.
// TK: Service ohne Host nach Downtime: Pylone. Host nach Downtime: blaues Pausezeichen (für Host und alle seine Services): Beschreibung angepasst, auch unten in der Tabelle
...
einigen Services im icon:icon_menu[] Menü ein gelber Pfeil steht (icon:icon_reload[]),
bei den meisten anderen aber ein grauer Pfeil (icon:icon_reload_cmk[]).
Die Services mit dem gelben Pfeil sind _aktive Checks_. Diese
