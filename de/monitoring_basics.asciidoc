...
|{UNREACH}
|orange
|Der _Weg_ zu dem Host ist aktuell für das Monitoring versperrt, weil ein Router oder Switch auf dem Weg dorthin ausgefallen ist.
...
=== Alarme und Benachrichtigungen#notifications
...
=== Unstetige Hosts und Services (Flapping)#flapping
...
=== Wartungszeiten (Scheduled Downtimes)#downtimes
...
== Checkintervall, Checkversuche und Checkperiode#checkintervall
...
einigen Services im icon:icon_menu.png] Menü ein oranger Doppelpfeil steht (ICON[icon_reload[]),
