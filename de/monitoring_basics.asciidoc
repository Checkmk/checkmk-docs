...
Dagegen beschreibt ein _Zustand_ eine anhaltende Situation, z.&thinsp;B.
...
* Auch nach chaotischen Situationen  --  z.&thinsp;B. Stromausfall im RZ  --  hat man immer einen zuverlässigen Gesamtzustand.
...
Denn ein Host kann vieles sein, z.&thinsp;B.:
...
konfiguriert werden, z.&thinsp;B.:
...
angeben. Wenn z.&thinsp;B. ein Server A _vom Monitoring aus gesehen_ nur über
...
Wenn in diesem Beispiel also der Host _myhost_ den
Zustand {DOWN} annimmt, dann geht das Monitoring automatisch davon aus, dass
ein eventueller Ausfall von _myhost4_ einfach damit
...
Übrigens: Die Überwachung von _myhost4_ findet trotzdem noch statt!
...
die Ordnerstruktur z.&thinsp;B. nach geographischen Gesichtspunkten aufgebaut, dann kann
...
(z.&thinsp;B. `harri`) einer Kontaktgruppe (z.&thinsp;B. `linux-admins`) zugeordnet.
...
image::tactical_overview.png[width=43%, align=center]
...
Wann immer sich der Zustand eines Hosts oder Services ändert (z.&thinsp;B. von
...
* der jeweilige Zustand bei dem Host/Service für Alarmierung abgeschaltet ist (z.&thinsp;B. keine Benachrichtigung bei {WARN}),
...
festlegen und entscheiden, wer auf welchem Wege alarmiert werden soll (E-Mail,
SMS etc.).
...
[cols="5,~"]
|===
|icon:icon_downtime[] |Der Service befindet sich in einer Wartungszeit.
...
Zeitperioden können auch Ausnahmen für bestimmte Kalendertage enthalten -- z.&thinsp;B.
...
wird. Setzen Sie z.&thinsp;B. das Intervall auf 24 Stunden und die Checkperiode auf
...
sensibel. Sind die Checkversuche z.&thinsp;B. auf 3 eingestellt, und der entsprechende
...
|icon:icon_downtime[] |Dieser Service ist gerade in einer Wartungszeit.
