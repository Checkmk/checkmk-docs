...
Normalerweise wird der Zustand eines jeden Services von {CMK} regelmässig überprüft und das Ergebnis interpretiert. Wie oben beschrieben ist jeder Zustand, der nicht {OK} ist, für {CMK} per Definition ein Problem, so zum Beispiel auch ein Host, der für eine Weile nicht erreichbar ist.
...
Bei allen passiven Checks, also denjeingen, die vom Agenten ausgeführt werden, werden stattdessen die Checks ausgelassen. Die Checks, und mit ihnen die zugehörigen Services, verharren in ihrem alten Zustand und werden mit der Zeit icon:icon_stale[] [.guihint]#stale#.

Wodurch aber wird der Status _stale_ festgelegt?
{CMK} kennt das xref:checkintervall[Checkintervall] zwischen zwei Checks. {CMK} weiß, welche Checks Informationen liefern und welche nicht. Und in {CMK} ist ein xref:checkintervall_passive[Faktor] für die Berechnung des _stale_-Eintritts festgelegt. Ist nun die Zeitspanne seit dem letzten erfolgreichen Check um diesen Faktor länger als ein reguläres Checkintervall, so wird der Service auf _stale_ gesetzt.
...
Dieser Faktor stellt das n-fache des normalen Checkintervalls dar. Ist also Ihr Checkintervall auf den Standard von einer Minute eingestellt, so geht ein Problem-Check mit der Standardeinstellung nach der 1,5fachen Zeit, somit nach 90 Sekunden, auf _stale_.
