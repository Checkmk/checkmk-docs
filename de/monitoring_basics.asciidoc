...
////
TK:
Zum Start was einfaches :-) : Für das Kapitel würde ich einen Anker definieren.
Da dieses Kapitel von hinten nach vorne gezogen wurde, wird hier jetzt in der Basiseinführung das 1. Mal der Begriff Check erwähnt.
Also muss erstmal erklärt werden, was ein Check ist, bevor es in die Unterscheidung zwischen aktiv und passiv geht.
Da die Begriffe Check und Service gerne durcheinandergeworfen werden (wie in diesem kleinen Kapitel auch), ist Genauigkeit und Unterscheidung für das Verständnis besonders wichtig:
Was man in der Oberfläche sieht, sind Services.
Der dem Service zugeordnete Check (oder: das Checkplugin, welches auch den Service definiert) ermittelt den Zustand dieses Services.
Sehr empfehlenswert ist Robins neues Video zu genau diesem Thema: https://www.youtube.com/watch?v=xCyTKNlvN9Y
////

Wenn Sie auf die {CMK}-Oberfläche schauen, werden Sie sehen, dass bei einigen Services im icon:icon_menu[alt="Symbol zum Öffnen des Aktionsmenüs."] Menü ein gelber Pfeil steht (icon:icon_reload[]), bei den meisten anderen aber ein grauer Pfeil (icon:icon_reload_cmk[]).
Die Services mit dem gelben Pfeil sind _aktive Checks_.
// TK: Nein, sind sie nicht: Service ungleich Check.
Diese werden von {CMK} direkt ausgeführt.
Services mit dem grauen Pfeil sind solche, bei denen die Check-Ergebnisse von dem aktiven Check {CMK} ermittelt werden.
// TK: Nach der Erklärung eines aktiven Checks hätte ich jetzt die eines passiven Checks erwartet. Aber nein, jetzt geht es wieder um einen (speziellen) aktiven Check: {CMK}, womit wohl der Service [.guihint]#Check_MK# gemeint ist, wie ja auch im Screenshot gezeigt.

Dies geschieht aus Gründen der Performance und stellt eine Besonderheit von {CMK} dar:
// TK: Das ist jetzt auch ein merkwürdiger Einführungssatz zum Screenhot.

image::monitoring_basics_check_mk_service.png[]

Damit das Zielsystem (Server, Netzwerkgerät, etc.) nicht für jeden einzelnen Service aufs Neue kontaktiert werden muss, holt {CMK} einmal pro Intervall alle wichtigen Daten in einem Rutsch und berechnet daraus die neuen Resultate für alle passiven Checks auf einmal.
...
// TK: Plus Verlinkung ins Kapitel 8. Checkintervall, Checkversuche und Checkperiode
...
Neben den xref:problems[oben] beschriebenen Zuständen _Problem_ und _unbehandelt_ gibt es einen dritten Status, der im [.guihint]#Overview# in der Seitenleiste angezeigt wird: _veraltet_ (stale).
// TK: Problem und _unbehandelt_ sind keine Zustände:
// TK: Problem ist die Zusammenfassung mehrerer Zustände (alle, die nicht OK sind), aber unbehandelt/behandelt sind nur zusätzliche Eigenschaften eines Objekts, analog zu veraltet. 
// TK: Zwar nachvollziehbar, dass Du Stale aus den benachbarten Spalten in der GUI motivieren willst, aber das führt hier eher in die Irre.
// TK: In intro_tools#overview gibt es übrigens eine High Level-Einführung, die sich hier auch ganz gut machen würde ... inkl. des Hinweises, dass im Overview die Spalte "Stale" nur angezeigt wird, wenn es auch veraltete Hosts und/oder Services gibt.

image::overview_more.png[alt="Snapin Overview im Show-more-Modus.",width=50%]

Normalerweise wird der Zustand eines jeden Services von {CMK} regelmässig überprüft und das Ergebnis interpretiert.
// TK: Normalerweise? 
// TK: Mir ist nicht so ganz klar, warum Du diesen Weg wählst, um zum Ziel "Stale" zu kommen.
Wie oben beschrieben ist jeder Zustand, der nicht {OK} ist, für {CMK} per Definition ein Problem, so zum Beispiel auch ein Host, der für eine Weile nicht erreichbar ist.
// TK: Auch hier: Stimmt zwar, aber wo führt uns das hin?

xref:active_checks#[Aktive Checks] werden {CRIT} oder {UNKNOWN}, wenn sie einen Host nicht erreichen können, da diese gezielt versuchen, den Host zu erreichen und dabei in einen Fehler laufen.
// TK: oder? Ich denke, ent- oder weder. Und: Nicht die Checks werden {CRIT} oder {UNKNOWN}, sondern die Services.

Bei allen passiven Checks, also denjenigen, die vom Agenten ausgeführt werden, werden stattdessen die Checks ausgelassen.
// TK: Die Checks werden nicht vom Agenten ausgeführt. Der Agent liefert die Daten.
Die Checks, und mit ihnen die zugehörigen Services, verharren in ihrem alten Zustand und werden mit der Zeit icon:icon_stale[] [.guihint]#stale#.
// TK: Wie oben: Service vs. Check: ein Check hat keinen Zustand.

Wodurch aber wird der Status _stale_ festgelegt?
// TK: Ich mag ja (inzwischen ;-) Fragen, finde die Frage aber hier nicht gelungen.
{CMK} kennt das xref:checkintervall[Checkintervall] zwischen zwei Checks.
{CMK} weiß, welche Checks Informationen liefern und welche nicht.
// TK: Checks, die keine Informationen liefern? Und: Spielt das wirklich eine Rolle?
Und in {CMK} ist ein xref:checkintervall_passive[Faktor] für die Berechnung des _stale_-Eintritts festgelegt.
Ist nun die Zeitspanne seit dem letzten erfolgreichen Check um diesen Faktor länger als ein reguläres Checkintervall, so wird der Service auf _stale_ gesetzt.
// TK: Kaum zu verstehen ohne Beispiel.
// TK: Warum verschiebst Du die Einstellung zu Stale aus Kap. 8.2 nicht hierhin?
...
// TK: Nach Wörterbuch werden Begriffe, die mit Check anfangen, durch Bindestrich verbunden.
// TK: Und warum heisst das Kap. nicht Checkintervall, Checkperiode und Checkversuche, in der Reiehenfolge, in der die Begriffe im Text abgehandelt werden?

Das Ausführen von Checks geschieht beim zustandsbasierten Monitoring in festen Intervallen.
{CMK} verwendet als Standard eine Minute.
// TK: verwendet für Service-Checks ...: Bei Host Checks mit Smart Ping sind es 6 Sekunden.
Jeder Check wird also einmal pro Minute ausgeführt.
Per Konfiguration kann dies geändert werden:
...
// TK: Hier fragt man sich schon, wie man das ändern kann: Mit den Regeln: Normal check interval for host checks, Normal check interval for service checks

=== Aktive Checks verändern
// TK: Ist die Überschrift wirklich richtig? Gelten nicht Checkversuche (hard und soft states) auch für Hosts und passive Checks? 

Durch Definition einer anderen Checkperiode als 24X7 kann das Ausführen von _aktiven_ Checks in bestimmten Zeitfenstern unterbrochen werden.
// TK: Das geht zu schnell: Was ist eine Checkperiode? Zeitperiode, die ich per Regel als Checkperiode definiere mit: Check period for hosts, Check period for active services,Check period for passive Checkmk services.
Der Zustand der Services wird dann nicht mehr aktualisiert und diese werden als _veraltet_ angezeigt (stale), symbolisiert durch icon:icon_stale[].
// TK: Hier fehlt der Verweis auf das Kapitel zu Stale.
...
[#max_check_attempts]
Mit Hilfe der _Checkversuche_ (max check attempts) können Sie Benachrichtigungen
...
[#checkintervall_passive]
=== Passive Checks verändern
// TK: Die GUI zeigt, dass es hier auch um Hosts geht. Warum also unter der Überschrift passive Checks?

Wollen Sie beeinflussen, wie schnell ein passiver Check in den _stale_-Zustand (icon:icon_stale[]) geht, verändern Sie den Faktor, auf dem die Einstufung beruht.
// TK: Auch Hosts können Stale werden (sonst gäbe es das Feld im Overview ja nicht). Wann wird ein Host Stale?

In den [.guihint]#globalen Einstellungen (Global settings)# ist im Abschnitt [.guihint]#Benutzerschnittstelle (User interface)# unter anderem der [.guihint]#Veraltungswert, ab dem Hosts/Services als veraltet gelten (Staleness value to mark hosts / services stale)# definiert:
// TK: gui-hint Auszeichnung nur für englische GUI verwenden. Die deutschen GUI-Texte weglassen.

image::monitoring_basics_staleness.png[alt="Einstellungen der Staleness."]

Dieser Faktor stellt das n-fache des normalen Checkintervalls dar.
Ist also Ihr Checkintervall auf den Standard von einer Minute eingestellt, so geht ein Problem-Check mit der Standardeinstellung nach der 1,5fachen Zeit, somit nach 90 Sekunden, auf _stale_.
// TK: Problem-Check?
