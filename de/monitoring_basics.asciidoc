...
////
SP: Kann der folgende abschnitt nicht komplett heir raus? Für einen Basics-Atikel führt das meines Erachtens zu weit:
In {CMK} werden zwei Arten von Checks unterschieden:

* xref:glossar#active_check[Aktive Checks]

* xref:glossar#check[Checks]
...
Die Checks zu den Services mit dem gelben Pfeil werden von {CMK} direkt ausgeführt.
Services mit dem grauen Pfeil sind solche, bei denen die Check-Ergebnisse von dem aktiven Check {CMK} ausgewertet werden.
...
Wenn Sie eine Weile mit {CMK} gearbeitet haben, kann es passieren, dass in Ihren Service- oder Hostansichten Spinnennetze angezeigt werden:

image::monitoring_basics_stale.png[alt="."]

Diese Spinnennetze symbolisieren den Zustand _stale_.
...
Doch was bedeutet der Zustand _stale_?
Der Zustand eines jeden Services wird von {CMK} regelmässig überprüft und das Ergebnis interpretiert.
// TK: Normalerweise? 
