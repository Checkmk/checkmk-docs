...
Wenn z.B. ein Server A _vom Monitoring aus gesehen_ über einen Router B erreichbar ist, dann ist B ein Parent-Host von A.
...
Sollte es dieser Host schaffen zu antworten, obwohl sein Parent in {CMK} {DOWN} ist, wird er als {UP} eingestuft. 

Falls ein Host _mehrere_ Parents hat und zumindest einer dieser Parents {UP} ist, wird der Host im Monitoring als erreichbar betrachtet.
In diesem Fall wird der Host nicht automatisch in den Zustand {UNREACH} wechseln.
