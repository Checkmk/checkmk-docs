...
// MA: Führt es mal zur Beschreibung und mal zur Manual page?
...
// MA: Hängt das Phänomen davon ab, dass ich "eine Weile" damit gearbeitet habe? Vorschlag: "Wenn Sie mit {CMK} arbeiten, kann es manchmal passieren, dass ..."
...
// MA: Die Ursache kann das Fehlen von Agentendaten insgesamt sein, oder das Fehlen von Daten eines einzelnen Plugins.
...
// MA: Ganz so ist es leider nicht. Das muss man noch einmal nachprüfen, aber die einzigen Fälle, die mir bekannt sind, sind die, wo der Host den Status eines bestimmten Services benutzt, um zu ermitteln, ob der Host up/down ist. Bei Smartping kann es kein stale geben. Ob es bei Ping (check_icmp) ein Stale geben kann? Glaube ich nicht, aber könnte u.U. sein.
// MA: Meine Hypothese ist, dass ein Host nur dann stale werden kann, wenn sein Zustand über einen Service bestimmt wird und dieser stale wird.
