...
//und kümmert sich sowohl um die Datenerhebung als auch um die Auswertung.
Services mit dem grauen Pfeil basieren auf passiven Checks, deren Daten von einem anderen Service, dem Service Check_MK, geholt werden.
Dies geschieht aus Gründen der Performance und stellt eine Besonderheit von {CMK} dar.
////
Damit das Zielsystem (Server, Netzwerkgerät, etc.) nicht für jeden einzelnen Service aufs Neue kontaktiert werden muss, holt der Service Check_MK einmal pro Intervall alle wichtigen Daten in einem Rutsch und die passiven Checks berechnen daraus die neuen Resultate.
// TK: Wer macht hier was? Mein Verständnis: Der aktive Check holt die Daten und die passiven Checks werten aus.
//SP: Rücksprache mit Marcel: genau genommen holen Fetcher die Daten und die Checks (sowohl aktiv als auch passiv) werten diese aus. Die weiteren Kommunikationswege machen dann den Unterschied zwischen aktiven und passiven Checks - das führt an dieser Stelle aber zu weit.
...
Services können nicht aktualisiert werden, deren Zustand von passiven Checks ermittelt wird, die auf die Daten des Agenten angewiesen sind.  
...
* Begrenzung der Zeiten, innerhalb derer Checks ausgeführt werden (xref:checkinterval[Check-Zeitraum], _check period_).
