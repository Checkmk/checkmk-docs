...
Während über Kontakte und Kontaktgruppen gesteuert wird, welche Personen für einen bestimmten Host oder Service zuständig sind,
werden die Berechtigungen über _Rollen_ gesteuert.
...
|`agent_registration` |Darf nur den xref:wato_monitoringagents#agents[{CMK}-Agenten] eines Hosts beim {CMK}-Server registrieren -- sonst nichts.
