...
Sie können die Auslöser, die für den Zustand _stale_ verantwortlich sind, anpassen. Lesen Sie hierzu den Abschnitt xref:checkintervall#[Check-Intervall und Check-Versuche.]
...
* Begrenzung der Zeiten, innerhalb derer Checks ausgeführt werden (xref:checkintervall#[Check-Zeitraum, check period]).
