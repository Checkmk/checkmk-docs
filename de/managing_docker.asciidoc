...
:description: Nach dem Start eines Docker-Containers folgt seine Pflege. Dieser Artikel beschreibt, welche Besonderheiten zu beachten sind, wenn {CMK} im Container läuft.
...
In {CMK} wird beim xref:intro_setup#create_site[Erstellen einer Instanz] ein zufälliges Passwort für den Standardbenutzer
`cmkadmin` gewählt. 
