...
Die {CSE} (*CEE*) richtet sich vor allem an professionelle Anwender und bietet über den Umfang der {RE} hinaus eine Reihe von interessanten Features,
...
Auch die {CCE} (*CCE*) richtet sich an professionelle Anwender und erweitert den Funktionsumfang der {SE} um Cloud-Funktionen wie fortgeschrittene Checks zur Überwachung von Cloud-Systemen und Cloud-spezifische Dashboards.
Zusätzlich bietet sie die in Cloud-Umgebungen sehr nützliche Kombination von Autoregistrierung und Konfiguration des {CMK}-Agenten im xref:glossar#push_mode[Push-Modus.]
Mehr Details zur {CE} finden Sie in xref:draft_cce#[diesem Artikel.]

Die {CFE} (*CFE*) ist für Sie die richtige, wenn Sie die {SE} oder {CE} unverbindlich testen wollen oder {CMK} im kleinen Rahmen mit einer xref:glossar#site[Instanz] mit bis zu 25 überwachten Hosts einsetzen möchten. 
Die {FE} enthält alle Features der {CE}, ist kostenlos und ist in den ersten 30 Tagen nicht limitiert bzgl. Instanzen und Hosts. 
Sowohl die {FE} als auch die {RE} können Sie später ohne Umwege direkt auf die {SE} oder die {CE} upgraden.

Die {CME} (*CME*) ist eine mandantenfähige Erweiterung der {SE} und verfügt über alle notwendigen Funktionen, um mit {CMK} über das xref:glossar#distributed_monitoring[verteilte Monitoring] voneinander abgeschottete Instanzen für mehrere Kunden zu betreiben. 
...
// TK: Auf der Homepage wird nur noch CRE vs. Enterprise (ohne Standard!) gefeatured.

{cee-only}
Wann immer wir in diesem Handbuch Funktionen besprechen, die nur für eine der {EE} -- also für die CFE, CEE oder CME -- oder für die {CE} gelten, kennzeichnen wir dies mit dem Symbol wie in diesem Absatz.
...
Der Vorteil der Appliance ist, neben einem vorinstalliertem System, dass Sie Betriebssystem, Appliance und {CMK} komplett über die grafische Oberfläche konfigurieren können, ohne die Linux-Kommandozeile bemühen zu müssen.
...
{cee-only}
Die virtuelle Appliance ist für alle {EE} verfügbar, für die {FE} als kostenlose Demo-Version.
Wie Sie bei der Installation vorgehen müssen, erfahren Sie in der xref:cma_virt1_quick_start#[Schnellstart-Anleitung.]
...
{cee-only}
Da die {EE} über mehr Features als die {RE} verfügen, sehen Sie dort mehr Dienste. 
