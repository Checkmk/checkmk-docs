...
Die {CFE} (*CFE*) ist für Sie die richtige, wenn Sie die {SE} unverbindlich testen wollen oder {CMK} im kleinen Rahmen mit einer xref:glossar#site[Instanz] mit bis zu 25 überwachten Hosts einsetzen möchten. 
...
eine flexible Verteilung der xref:glossar#agent[{CMK} Monitoring-Agenten] (die die Informationen der überwachten Zielsysteme beschaffen),
eine große Anzahl von ausgeklügelten Dashlets zum Einbau in xref:glossar#dashboard[Dashboards,]
ein Reporting, und vieles mehr.
...
{cee-only}
Wann immer wir in diesem Handbuch Funktionen besprechen, die nur für eine der {EE} gelten -- also für die CFE, CEE oder CME -- kennzeichnen wir dies mit dem Symbol wie in diesem Absatz.
...
// Dateiname ausnahmesweise ohne Präfix intro_
