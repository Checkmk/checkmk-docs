...
Die {CFE} (CFE) ist für Sie die richtige, wenn Sie die {SE} oder {CE} unverbindlich testen wollen oder {CMK} im kleinen Rahmen dauerhaft mit bis zu 750 Services einsetzen möchten. 
Die {FE} enthält alle Features der {CE}, ist kostenlos und in den ersten 30 Tagen nicht limitiert.
Aus der {FE} -- wie übrigens auch aus der {RE} -- können Sie später eine {SE} oder {CE} machen.
