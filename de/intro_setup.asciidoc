...
Die {CFE} (*CFE*) ist für Sie die richtige, wenn Sie die {PE} unverbindlich testen wollen oder {CMK} im kleinen Rahmen mit einer xref:glossar#site[Instanz] mit bis zu 25 überwachten Hosts einsetzen möchten. 
Die {FE} enthält alle Features der {PE}, ist kostenlos und ist in den ersten 30 Tagen nicht limitiert bzgl. Instanzen und Hosts. 
Sowohl die {FE} als auch die {RE} können Sie später ohne Umwege direkt auf die {SE} oder die {PE} upgraden.

Die {CPE} (*CPE*) richtet sich vor allem an professionelle Anwender, die Cloud-Funktionen nutzen und bietet über den Umfang der {SE} hinaus eine Reihe von interessanten Features,
wie z.B. Cloud-spezifische Dashboards, die Möglichkeit, Hosts automatisiert anlegen zu lassen, und den Push Agent für die zentral gesteuerte Agenteninstallation.
Für die {SE} können Sie optional link:https://checkmk.com/de/produkt/preise#/[von uns^] oder einem link:https://checkmk.com/de/partner/partner-finden[unserer Partner] professionellen Support erhalten.
