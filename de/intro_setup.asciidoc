...
:revdate: 2021-05-06
:title: {CMK} aufsetzen
:description: Beim Aufsetzen von {CMK} geht es um die Auswahl der passenden Edition und Version, Software-Installation, Erstellung und Start einer Instanz und Anmeldung an der Oberfläche.
...
Die {CRE} (*CRE*) ist kostenlos, zu 100{nbsp}% Open Source und enthält Nagios als Kern. 
...
Die {CFE} (*CFE*) ist für Sie die richtige, wenn Sie die {SE} unverbindlich testen wollen oder {CMK} im kleinen Rahmen mit einer Instanz mit bis zu 25 überwachten Hosts einsetzen möchten. 
Die {FE} enthält alle Features der {SE}, ist kostenlos und ist in den ersten 30 Tagen nicht limitiert bzgl. Instanzen und Hosts. 
Sowohl die {FE} als auch die {RE} können Sie später ohne Umwege direkt auf die {SE} upgraden.

Die {CSE} (*CEE*) richtet sich vor allem an professionelle Anwender und
bietet über den Umfang der {RE} hinaus eine Reihe von interessanten Features, wie z.B. mit dem xref:cmc#[{CMK} Micro Core (CMC)] einen sehr
performanten eigenen Kern (der Nagios ersetzt), ein Reporting, ein ausgefeiltes System für die Visualisierung von Messwerten, eine flexible Verteilung der xref:wato_monitoringagents#[{CMK}-Agenten] (die die Informationen der überwachten Zielsysteme beschaffen), und vieles mehr.
...
Die {CME} (*CME*) ist eine mandantenfähige Erweiterung der {SE} und verfügt über alle notwendigen Funktionen, um mit {CMK} über das xref:distributed_monitoring#[verteilte Monitoring] voneinander abgeschottete Instanzen für mehrere Kunden zu betreiben. 
Falls Sie für Ihre Kunden diese Dienste anbieten wollen, ist diese Ihre Edition. 
Genaueres zum Konzept der Managed Services finden Sie in der Einführung xref:managed#[dieses Artikels].
...
der {EE} gelten -- also für die CEE, CFE oder CME -- kennzeichnen wir dies mit dem Symbol wie in diesem Absatz.
...
{CMK} unterstützt die folgenden Linux-Distributionen: Red Hat Enterprise Linux (RHEL)/CentOS, SUSE Linux Enterprise Server (SLES), Debian und Ubuntu. 
Für jede {CMK}-Edition, {CMK}-Version und Linux-Distribution gibt es ein eigenes angepasstes Paket von uns, das Sie mit dem Paketmanager Ihrer Linux-Distribution installieren können.
...
Der Vorteil der Appliance ist, neben einem vorinstalliertem System, dass Sie Betriebssystem, Appliance und {CMK} komplett über die grafische Oberfläche
konfigurieren können, ohne die Linux-Kommandozeile bemühen zu müssen. Auch die Installation der {CMK}-Software und die Erstellung von Instanzen erfolgt über die Web-GUI der Appliance.
...
{cee-only}
Die virtuelle Appliance ist für alle {EE} verfügbar, für die {FE} als kostenlose Demo-Version. Wie Sie bei der Installation vorgehen müssen, erfahren Sie in der xref:cma_virt1_quick_start#[Schnellstart-Anleitung].
...
{cee-only}
Die physische Appliance gibt für die {SE} und {ME}. 
...
Die Anleitung zur Inbetriebnahme finden Sie in der xref:cma_rack1_quick_start#[Schnellstart-Anleitung].
...
Dabei unterstützen wir sowohl die {RE} als auch die {EE} mit fertigen Container-Images, die mit wenigen Kommandos eingerichtet sind.
...
Dieser darf nur aus Buchstaben und Ziffern bestehen. 
Konvention sind dabei Kleinbuchstaben. 
Im Handbuch verwenden wir in allen Beispielen den Namen `mysite`.
...
* Für die Web-Oberfläche von {CMK} wird ein Benutzer mit dem Namen `cmkadmin` und einem zufälligen Passwort angelegt. Notieren Sie sich dieses Passwort. Sie können das Passwort auch ändern, wie es weiter unten beschrieben ist.
...
Am geänderten Prompt sehen Sie, dass Sie in der Instanz eingeloggt sind. 
Wie der Befehl `pwd` zeigt, befinden Sie sich danach automatisch im Home-Verzeichnis der Instanz:
...
Übrigens: Immer wenn wir im Handbuch Pfadnamen angeben, die *nicht* mit einem Schrägstrich beginnen, beziehen sich diese auf das Home-Verzeichnis der Instanz. 
...
{cre-only}
Da die {RE} nicht über alle Features der {EE} verfügt, sehen Sie dort weniger Dienste. 
Außerdem finden Sie für den Kern `nagios` statt  `cmc`:
...
Diese setzt sich zusammen aus dem Host-Namen oder der IP-Adresse des {CMK}-Servers, einem Schrägstrich und dem Namen der Instanz, z.B. `\http://mycmkserver/mysite`. 
Unter dieser Adresse finden Sie diesen Anmeldedialog:

image::login.png[alt=Anmeldung bei {CMK} mit Benutzername und Passwort,width=60%]
...
[{image-border}]
image::omd_site_not_started.png[alt="Fehlermeldung im Webbrowser zu einer nicht gestarteten Instanz"]
...
[{image-border}]
image::omd_site_not_found.png[alt=Fehlermeldung im Webbrowser zu einer nicht vorhandenen Instanz]

*Wichtig:* Sobald Sie {CMK} produktiv betreiben, empfehlen wir Ihnen aus
Sicherheitsgründen den Zugriff auf die Oberfläche ausschließlich gesichert zuzulassen. 
Was Sie dafür tun müssen, erfahren Sie im
xref:omd_https#[Artikel über die Absicherung der Web-Oberfläche mit HTTPS].
