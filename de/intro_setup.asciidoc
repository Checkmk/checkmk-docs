...
In kleinen Rahmen, d.h. mit einer Instanz und bis zu 750 Services, können Sie {CMK} mit der {CE} auch dauerhaft ohne Lizenzierung betreiben.
...
{cee-only}
Die virtuelle Appliance ist für alle {EE} verfügbar.
// TK: Hinweis auf FE gelöscht: ...,für die {FE} als kostenlose Demo-Version.
...
{cee-only}
Die physische Appliance gibt für alle {EE}. 
