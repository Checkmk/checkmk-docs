...
= {CMK} aufsetzen
:revdate: draft
:title: Checkmk aufsetzen
:description: Das Aufsetzen von Checkmk umfasst die Auswahl der passenden Checkmk Edition und Version, die Installation der Software, die Erstellung und den Start einer Instanz und die Anmeldung an der Oberfläche. Um diese Themen geht es im aktuellen Kapitel des Leitfadens für Checkmk Einsteiger, der kompakt durch die ersten Schritte führt, um das IT-Monitoring mit Checkmk optimal einzurichten.
...
link:intro_welcome.html[Was zuletzt geschah]
...
wie z.B. statt Nagios mit dem link:cmc.html[{CMK} Micro Core (CMC)] einen sehr
//SK: "wie z.B. statt Nagios mit dem" holpert ein wenig.
performanten eigenen Kern, ein Reporting, ein ausgefeiltes System für die
Visualisierung von Messwerten, eine flexible Agentenverteilung und vieles mehr.
//SK: Im Artikel intro_welcome werden explizit die Checkmk-Anfänger angesprochen. Ob diese Anfänger hier schon etwas mit dem Wort "Agentenverteilung" anfangen können, ziehe ich mal in Zweifel.
Für die {SE} können Sie optional von uns oder einem unserer
//SK: Wollen wir hier auch noch unseren Support verlinken? Da das Handbuch ja nicht mehr direkt in der Homepage liegt, ist der Weg dahin durch die fehlende Navigation ja jetzt etwas weiter. https://checkmk.de/support.html
...
//SK: Der folgende Satz hat mir noch nie gefallen, weil wir hier die Entwickler (Red Hat) einfach mit den Distributionen (SLES) mischen. Es gibt schlicht keine "Distribution von SLES". SLES ist die Distribution von SUSE.
{CMK} unterstützt die Distributionen von Red Hat/CentOS, SLES, Debian und
Ubuntu. Für jede {CMK}-Edition, {CMK}-Version und Linux-Distribution gibt es ein
eigenes angepasstes Paket von uns, das Sie mit dem Paketmanager Ihrer
Linux-Distribution installieren können. Wie dies genau geht, erfahren Sie im
link:install_packages.html[Artikel über die Linux-Installation].
...
//SK: Wenn wir hier explizit auf Debian 9 hinweisen, "müssen" wir das 2021 ggf. ändern.
Die virtuelle Appliance enthält neben {CMK} auch das Linux-Betriebssystem Debian
9 (Stretch). Der Vorteil der Appliance ist, neben einem vorinstalliertem System,
...
und Konfiguration von Instanzen, die im link:omd_basics.html[Artikel über Instanzen]
beschrieben sind. Für noch tiefergehende Informationen unter anderem zur
Verzeichnisstruktur der Instanz gibt es den [cmk_commandline|Artikel über {CMK}
auf der Kommandozeile].
//SK:Wenn überhaupt, würde ich hier nur auf die Verzeichnisstruktur verweisen und dann auch direkt dahin verlinken. Es gibt in jedem Artikel des Handbuch "unter anderem" tiefergehende Informationen.
//SK:Im Leitfaden für Eisnteiger braucht es diesen letzten Satz meines Erachtens nicht.


[#login]
== Anmelden

//SK: "Nachdem die Instanz läuft"-> Die Instanz läuft doch noch...
Nachdem die Instanz läuft, kann es auch schon losgehen. Jede Instanz hat eine
...
link:intro_gui.html[Hier geht es weiter]
