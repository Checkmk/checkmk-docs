...
// TK: Ich hab den Satz umgestellt, weil ich über "FreeBSD Linux" gestolpert bin und zuerst dachte, dass wäre ein Begriff.
...
// TK: 1) Den Satz versteh ich nicht. 2) Dragonfly > DragonFly BSD?
...
// TK: den Github-Repositories > Warum Plural?
Der einfachste Weg ist der Download mittels `wget` mit Speicherung direkt im Zielverzeichnis:

// MFS: Wie bekomme ich die erste Zeile fett?
// TK: Jedenfalls nicht mit shell-raw ;-) Ich hab jetzt mal beide Kommandos gefettet:
[{shell}]
----
...
// TK: Schon klar, was Du sagen willst, aber ist natürlich schon etwas merkwürdig, wenn Du oben explizit die Installation der bash als Voraussetzung einführst.
...
// TK: Ist nicht der Fall wahrscheinlicher, dass Port 6556 von einem anderee Dienst belegt ist? Und was mach ich dann?
...
// TK: Die beiden folgenden Punkte würde ich zusammenziehen und nur 1x {file} verwenden.
...
Fügen Sie dann eine Zeile ein, die nur dem {CMK}-Server – hier mit der IP-Adresse `10.2.3.4` – den Zugriff erlaubt.
...
Die xref:checkmk_getting_started.html#add_server[Aufnahme ins Monitoring] verläuft wie bei allen anderen Systemen über die automatische Service-Erkennung. Bei einem „nackten“ FreeBSD, auf dem noch keine Server-Dienste konfiguriert sind, sollten etwa 12 Dienste erkannt werden:
...
*Achtung:* Für die Datenverzeichnisse (unter Linux `/var/lib/check_mk_agent`) sind keine Standardpfade hinterlegt. 
// TK: Vielleicht besser: Für FreeBSD sind keine Standardpfade hinterlegt, wie für Linux unterhalb von `/var/lib/check_mk_agent`.
