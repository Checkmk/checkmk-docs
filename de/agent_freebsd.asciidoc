...
// TK: Folgenden Satz umgeschrieben:
...
// TK: Wobei das "etwas höher liegt" bezüglich der Hauptversion etwas merkwürdig bleibt: Wenn ich eine 1.6-Version installiert habe, kann ich den 2.0-Agenten installieren, weil 2.0 etwas höher liegt als 1.6?
...
Der erste Parameter (`check_mk_agent`) in dieser Zeile ist dabei der Name des vom `inetd` ausgeführten Kommandos:
...
----
// TK: Jetzt funkioniert's! Hat ja auch lange genug gedauert ;-)
// TK: Es funktioniert bei mir übrigens auch ohne die sshd-Zeile.
// TK: Wenn die sshd-Zeile drinbleiben soll, solltest Du das oben kurz beschreiben.
