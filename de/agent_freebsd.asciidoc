...
Wenn die Major-Version des Agenten in den Ports Ihrer aktuell verwendeten{CMK}-Version entspricht oder etwas höher liegt, können Sie diese installieren.
// TK: Hm, was ist jetzt mit "dessen" gemeint? FreeBSD-STABLE Version? Der Patch? Die Patches?
// MFS: Agent in den Ports, siehe oben. Ports sind immer Rolling, während das Basis FreeBSD am Stück aktualisiert wird - großer Vorteil ggü. Linux.
// TK: Was meinst Du mit "Major-Version" - ist bei uns (leider) nicht definiert)? 2.0.0? Oder doch 2.0.0.pxy als Patch Release?
// TK: Anders rum: Ich hab 2.0.0p10 installiert und der Patch gilt für 2.0.0p13: Patch installieren: Ja oder Nein?
// MFS: Laut MaSl sollen Patches nur Fehler korrigieren, wenn Du keine Probleme mit CMK 2.0.0p10 hast, benötigst Du an keiner Stelle ein Update. 
...
Ist die im FreeBSD Ports verfügbare Version älter als die Ihres {CMK}-Servers, installieren Sie den aktuellen Agenten aus dem GitHub-Repository.
// TK: Der Satz ist sehr schön und könnte angepasst auch im vorherigen Kapitel stehen - mit kurzer Zusatzinfo was mit älter/neuer gemeint ist.
// MFS: Darf ich hier bewusst schwammig bleiben?  
...
// MFS: Im Fall des FreeBSD Agents ist nicht garantiert, dass Anpassungen für FreeBSD 13.0 auf CMK 2.0 zurückportiert wurden, daher ist es in diesem Fall sinnvoller, aus "master" zu nehmen. Erklärung hinzugefügt. Der Idealfall wäre natürlich, Mark Peek dazu zu bekommen, checkmk-agent20 und checkmk-agent21 parallel zu pflegen…
...
Die einfachste Möglichkeit, auf den Agenten zuzugreifen, ist der bei FreeBSD mitgelieferte `inetd`. Alternativ steht der von Linux bekannte `xinetd` aus der FreeBSD Ports Sektion `security` zur Verfügung. Dessen Konfiguration entspricht exakt der im xref:agent_linux#manual[Artikel zum Linux-Agenten] beschriebenen.
...
// TK: Danke für die Erklärung! Hab den Link oben eingebaut. 
...
// TK: Mal wieder anders als gedacht: Ok!
...
Kommentieren Sie zunächst die Zeile aus, die Zugriffe von überall erlaubt, fügen Sie dann eine Zeile ein, die nur dem {CMK}-Server – hier mit der IP-Adresse `10.2.3.4` – den Zugriff erlaubt. Der erste Parameter ist der Name des vom `inetd` ausgeführten Kommandos:
...
// TK: Ich kriegs nach wie vor nicht hin - auch nicht mit reboot :-(
// TK: reboot m.E. auch nicht nötig, da ja service inetd restart die Datei neu auswertet: es bleibt dabei: mit ALL gehts und mit checkmk-agent gehts nicht.
