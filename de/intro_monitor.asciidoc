...
= {CMK} im Monitoring
:revdate: draft
:title: Checkmk im Monitoring
:description: Probleme zu quittieren und Wartungszeiten einzurichten sind wichige Aufgaben im Monitoring mit Checkmk. Um diese Themen geht es im aktuellen Kapitel des Leitfadens für Checkmk Einsteiger, der kompakt durch die ersten Schritte führt, um das IT-Monitoring mit Checkmk optimal einzurichten.
...
link:intro_tools.html[Was zuletzt geschah]
...
Es ist gar nicht so selten, dass Sie eine Reihe (zusammengehöriger) Probleme auf
einmal quittieren wollen. Das geht einfach, indem Sie sich mit einem Klick auf
//SK: Achtung: "Show checkboxes" gibt es bei der Ansicht "Host & service problems" nicht. Vielleicht sollten man hier im Beispiel einfach erstmal auf "Service problems" fokussieren. Irgendein Service wird sicherlich ein Problem haben.
[.guihint]#Show checkboxes# eine neue erste Spalte in der Liste einblenden lassen, die
eine Checkbox vor jeder Zeile  enthält. Die Checkboxen sind alle nicht markiert,
denn die Auswahl treffen Sie: Markieren Sie die Checkbox für jeden gewünschten
Host oder Service.
...
einrichten wollen. Zum Beispiel können Ihnen wieder die vordefinierten
Statusansichten gute Dienste leisten: in
//SK: Die "Host und Services search" würde ich jetzt nicht als "ordefinierte Statusansicht" bezeichnen. Ich glaube hier ist irgendwas anderes gemeint.
//SK: "All hosts" oder so. Nach einem Klick auf "Host search" würde ich ja erstmal außer der Suche nicht viel sehen. Auf jeden Fall keine Möglichkeit mir Checkboxen anzeigen zu lassen.
[.guihint]#Monitor# > [.guihint]#Overview# > [.guihint]#Host search# oder [.guihint]#Service search#.
...
//SK: Dieser Screenshot zeigt des Dialog "Schedule Downtime" für einen Service. Das sieht man an der vorletzten Option "Schedule downtime on affected hosts...".
//SK: Etwas weiter drunter wird gesagt, dass man eventuell besser jeweils den ganzen Host in eine Downtime schickt. Vielleicht sollte man dann hier auch den entsprechenden Screenshot verwenden, damit die anderslautende Option niemanden verwirrt.
...
link:intro_finetune.html[Hier geht es weiter]
