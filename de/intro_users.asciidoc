...
auch noch einen Benutzer `automation` gibt. Dieser „Automationsbenutzer“ ist für Zugriffe von außen gedacht (z.B. per Skript oder xref:rest_api.html[REST-API]) und wird im link:wato_user#automation[Artikel über die Benutzerverwaltung] genau beschrieben.
