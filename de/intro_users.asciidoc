...
:description: Benutzer in {CMK} werden Rollen für Berechtigungen und Kontaktgruppen für Zuständigkeiten zugewiesen.
...
den Service `Tablespace FOO` auf dem Host `ora012`? Wer soll den Host und Service im Monitoring sehen und eventuell benachrichtigt werden, wenn es ein Problem gibt?

Zuständigkeiten werden in {CMK} nicht über Rollen, sondern über *Kontaktgruppen* definiert. Das Wort „Kontakt“ ist im Sinne einer Benachrichtigung gemeint: Wen soll das Monitoring kontaktieren, wenn es ein Problem gibt?
...
image::contactgroup_example.png[alt="Illustration der Beziehung zwischen Benutzern, Hosts/Services und Kontaktgruppen.",width=50%]
...
* Gibt es ein Problem mit einem Host oder Service, werden (per Default) alle Benutzer benachrichtigt, die in mindestens einer seiner Kontaktgruppen sind.
...
image::wato_new_contact_group.png[alt="Dialog zur Festlegung der Eigenschaften für die neue Kontaktgruppe."]
...
Der benötigte Regelsatz heißt [.guihint]#Assignment of hosts to contact groups#. Sie finden ihn z.B. auf der Seite [.guihint]#Contact groups#, die Sie im vorherigen Kapitel geöffnet hatten, im Menü [.guihint]#Contact groups > Rules#.
...
image::host_group_assignment_rule.png[alt="Dialog zur Zuweisung von Kontaktgruppen zu Hosts in einer Regel."]
...
image::host_permissions.png[alt="Dialog zur Zuweisung von Kontaktgruppen zu Hosts in den Host-Eigenschaften."]
...
image::wato_module_users.png[alt="Liste der {CMK}-Benutzer."]

Wundern Sie sich nicht, wenn es dort außer dem Eintrag `cmkadmin` auch noch einen Benutzer `automation` gibt. Dieser „Automationsbenutzer“ ist für Zugriffe von außen gedacht (z.B. per Skript oder xref:rest_api.html[REST-API]) und wird im xref:wato_user#automation[Artikel über die Benutzerverwaltung] genau beschrieben.

Einen neuen Benutzer legen Sie mit dem Knopf [.guihint]#Add user# auf der Seite gleichen Namens an:

image::wato_new_user_identity.png[alt="Dialog zur Festlegung der Identität für den neuen Benutzer."]

Im Kasten [.guihint]#Identity# geben Sie die interne ID [.guihint]#(Username#) und einen Titel [.guihint]#(Full name)# ein -- hier den ausgeschriebenen Namen des Benutzers. Die Felder [.guihint]#Email address# und [.guihint]#Pager address# sind optional und dienen den Benachrichtigungen via E-Mail bzw. SMS.

*Hinweis:* Tragen Sie hier vorerst noch keine E-Mail-Adresse ein. Lesen
Sie zunächst die Hinweise im xref:intro_notifications#[Kapitel über Benachrichtigungen].

image::wato_new_user_security.png[alt="Dialog zur Festlegung der Sicherheitseinstellungen für den neuen Benutzer."]
...
image::wato_new_user_contact_groups.png[alt="Dialog zur Auswahl der Kontaktgruppen für den neuen Benutzer."]
...
und Gruppen aus diesen Diensten einzubinden. Die Details dazu finden Sie im xref:ldap#[Artikel über LDAP/Active Directory].
