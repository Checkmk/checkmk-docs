...
Eine Besonderheit der {CMK}-Oberfläche ist, dass sie mehrere Monitoring-Instanzen gleichzeitig visualisieren und konfigurieren kann und sich dann wie ein großes System verhält.
////
...
Im [.guihint]#Monitor#-Menü können Sie Hosts und Services suchen. Die Suche ist interaktiv: Sobald Sie etwas getippt haben, sehen Sie sofort unterhalb des Suchfelds die Ergebnisse, die zu Ihrer Eingabe passen:
...
Die Filter können Sie auch mehrfach und kombiniert nutzen. Mehrere Instanzen eines Filters werden dabei mit *ODER* verknüpft, unterschiedliche Filter mit *UND*. Ausnahme: Mehrere Host-Merkmalsfilter (`tg:`) werden immer mit *UND* verbunden.

Folgende Filter stehen Ihnen zur Verfügung:

//TK: In der folgenden Tabelle die Spalte Verknüpfung gelöscht, da sie zusammen mit den dahinter stehenden Beispielen eher verwirrt als nutzt.

[cols="20,10,~"]
|===
|Filter |Befehl |Beispiel 
...
|Host-Merkmal |`tg:` |`tg: windows tg: testing`
...
xref:views#[Statusansichten (_views_)],
...
Beim Aufruf eines der Menüeinträge wird Ihnen die Liste der bereits existierenden Objekte angezeigt. In einigen Listen finden Sie Objekte, die {CMK} mitausliefert, und die Sie bearbeiten oder als Vorlage für Ihre eigenen Objekte verwenden können.
...
|Benachrichtigungsregeln ([.guihint]#Notification rules#) |Regeln zur xref:notifications#personal[benutzerdefinierten Alarmierung]. Der Eintrag wird nur angezeigt, wenn [.guihint]#Setup > Global Settings > Rule based notifications# angeschaltet ist (was der Default ist).
...
|Startseite ([.guihint]#Start URL to display in main frame#) |URL der Startseite festlegen. Wird auf der Hauptseite ein xref:dashboards[Dashboard] angezeigt, können Sie dieses alternativ über das Menü [.guihint]#Dashboard# zur Startseite machen.
...
*Hinweis:* Für einige Einstellungen gibt es Default-Werte, die global per [.guihint]#Setup > Global Settings# für alle Benutzer geändert werden können, meist unter dem Thema [.guihint]#User Interface#. Außerdem bestimmt die Ihrem Benutzerkonto zugewiesene xref:wato_user#roles[Rolle mit ihren Berechtigungen], ob bestimmte Einstellungen sichtbar sind und geändert werden können.
...
Nach Auswahl eines der Einträge im [.guihint]#Monitor#-Menü wird Ihnen auf der Hauptseite die angeforderte Information in der Regel entweder als xref:dashboards[Dashboard] oder als xref:views[View] angezeigt, auf die wir in den folgenden Kapiteln näher eingehen werden.
...
Wird eines dieser Dashboards angezeigt, können Sie schnell zu den anderen umschalten: in der Menüleiste rechts neben dem [.guihint]#Help#-Menü finden Sie die Symbole für die verfügbaren Dashboards. Sie können auch das aktuell angezeigte Dashboard zur Startseite befördern: im Menü [.guihint]#Dashboard > Set as start URL#.
...
Die Statusansichten haben noch viele weitere Möglichkeiten -- zur Anpassung und zur Erstellung eigener Ansichten. Wie das geht, erfahren Sie im xref:views#[Artikel über Views].
...
Dafür benötigt Ihr aktuelles Benutzerkonto allerdings eine Berechtigung, die per Default nur die xref:wato_user#roles[Rolle] `admin` besitzt.

*Hinweis* Das Prinzip der Sichtbarkeit wird bei {CMK} an vielen Stellen verwendet und im Artikel über die xref:views#edit[Ansichten] näher erklärt.
...
// TK: Es sollten entweder alle Optionen (wenigstens kurz) beschrieben werden - oder auf den ausführlichen Artikel verwiesen werden. Diesen Artikel gibt es aber nicht. Am ehestens wäre im user_interface der richtige Ort für die Master Control Opionen.
