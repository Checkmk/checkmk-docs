...
image::gui.png[alt={CMK}-Startseite]
...
So findet etwa `h:^myhost s:myservice` alle Services, deren Service-Beschreibung „myservice“ enthält und auf Hosts laufen, deren Name mit „myhost“ beginnt.
...
image::customize_menu_more.png[alt=Customize-Menü in der Navigationsleiste,width=65%]
...
In einigen Listen finden Sie Objekte, die {CMK} mitausliefert, und die Sie bearbeiten oder als Vorlage für Ihre eigenen Objekte verwenden können.
...
Das Konzept der regelbasierten Konfiguration ist zentral in {CMK} und sehr leistungsfähig, um die xref:wato_rules#[Host- und Serviceparameter] anzupassen.

*Hinweis:* Auch den Inhalt des [.guihint]#Setup#-Menüs können Sie sich in die Seitenleiste laden: 
als Snapin mit dem Titel [.guihint]#Setup# (mit Titeln und Symbolen) oder [.guihint]#Quick Setup# (nur mit Symbolen).
...
Häufig verwendete Einstellungen können per Klick direkt im [.guihint]#User#-Menü umgeschaltet werden ([.guihint]#Quick toggle#), manche durch Aufruf einer spezifischen Seite, z.B. zum Ändern des Passworts. 
...
*Hinweis:* Für einige Einstellungen gibt es Default-Werte, die global per [.guihint]#Setup > Global Settings# für alle Benutzer geändert werden können, meist unter dem Thema [.guihint]#User Interface#. 
...
* Kontext-sensitive Hilfe (_inline help_): 
Sie können per [.guihint]#Help#-Menü auf der aktuellen Seite Hilfetexte einblenden lassen mit [.guihint]#Help > Show inline help#, was sehr hilfreich ist auf Seiten, die viele Parameter enthalten. 
Der gewählte Modus bleibt aktiv auch für weitere Seiten, die Sie aufrufen, bis Sie ihn mit [.guihint]#Hide inline help# wieder deaktivieren.
...
* Zahl noch nicht aktivierter Änderungen: 
In der Konfigurationsumgebung wird rechts oben auf der Seite statt des Countdowns die Zahl der noch nicht aktivierten Änderungen angezeigt. 
...
image::user_interface_bookmark_lists.png[alt=Lesezeichenlisten]

Sie können nun entweder mit icon:icon_edit[alt=Symbol zum Bearbeiten eines Listeneintrags] eine bestehende Liste bearbeiten -- oder mit [.guihint]#New list# eine neue anlegen:

image::user_interface_new_bookmark_list_general_properties.png[alt=Eigenschaften beim Erstellen einer Lesezeichenliste]
...
image::user_interface_new_bookmark_list_bookmarks.png[alt=Festlegung der Eigenschaften für die neuen Lesezeichen]
