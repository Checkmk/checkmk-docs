...
// ML: Was mir noch etwas fehlen würde ist der Umgang mit einem erstellten Bericht im Monitoring, also, dass ein Bericht (sofern nicht intern Hosts/Services gefiltert werden) erst mit Kontext versehen werden muss - sprich: View aufsuchen, View filtern, Report aus gefilterter Ansicht erstellen erzeugen. 
...
//ML: Wenn ich mich recht entsinne, liegt der Unterschied darin, WANN gefiltert wird: Wird hier gefiltert, werden die nicht benötigten Daten gar nicht erst angefragt. Wird erst im Report gefiltert, werden erst alle Daten geholt und dann gefiltert. Sprich: Eine Performance-Angelegenheit. So zumindest erklärte Marcel das mal - mir schien das nämlich auch etwas redundant.
...
ML: Ich würde sagen nein, die Filter existieren ja CMK-weit  - das müsste irgendwann mal zentral gemacht werden.
Was ich aber noch ergänzen würde: Tabelle mit Inhalts- und eventuell auch fixen Gestaltungselementen - vielleicht als "Anhang".
////
...
Um eine eigene Berichtsvorlage zu erstellen, kopieren Sie die Standardvorlage mit icon:icon_clone[alt="Symbol zum Kopieren"]. Ändern Sie als erstes [.guihint]#Unique ID# und [.guihint]#Title#. Ab dem ersten Speichern finden Sie diesen Bericht dann bei den eigenen Berichtsvorlagen:

// ML: Hier fehlt noch eine wichtige Info: Damit ein Bericht eine Berichtsvorlage (frontmatter template) wird, muss in den Eigenschaften "This is a front matter Template" gesetzt werden. Und zwar auch dann, wenn man die Standardvorlage kopiert! Die Option wird beim Klonen nämlich deaktiviert. Sprich: Eine Standardvorlage ist nur ein normaler Report mit gesetzter "This is a front matter Template"-Option. Zudem vielleicht noch als Ergänzung: Table of Content einfügen - das kann nämlich nur in Frontmatter-Templates gemacht werden.

image::reporting_customized3.png[alt="Auswahl der Berichtsvorlage im Kasten 'Customized'."]

Nun können Sie diese Vorlage entsprechend der obere Beschreibungen genau wie jeden anderen Bericht an Ihre Bedürfnisse anpassen. Verändern Sie die Seiteneinteilung, die Schrift, die Linienfarben etc.
