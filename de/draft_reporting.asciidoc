...
Um Berichte mit Daten zu füllen, gibt es verschiedene Möglichkeiten. Daraus resultiert dann auch, dass die Berichte unterschiedlich aufgerufen werden müssen, um sinnvolle Informationen zu enthalten. Fehlen Spezifikationen, so bekommen Sie statt einer korrekten Ausgabe nur rot hervorgehobene Fehlermeldungen: 

image::reporting_output_red.png[alt="Ausgabe eines Berichts mit unspezifizierten Daten.", width=79%]

Der Unterschied besteht darin, an welcher Stelle die anzuzeigenden Informationen festgelegt werden:
...
//SP: Bild folgt noch entweder hier oder einen Absatz weiter unten (muss ich noch erstellen).
...
|Combined graph | |Verwendung einer Vorlage, Auswahl der Datenquelle und diverse Layout-Einstellungen möglich
|Custom Graph | |
