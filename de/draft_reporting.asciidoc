include::global_attr.adoc[]
= Berichte (Reports)
:revdate: draft
:title: Berichte (Reports)
:description: {CMK} kann Berichte (Reports) nicht nur automatisiert/ad hoc erstellen, sondern diese auch direkt verschicken. Lernen Sie hier, wie Sie diese Funktion nutzen.

{related-start}
xref:views#[Ansichten von Hosts und Services (Views)]
xref:graphing#[Messwerte und Graphing]
xref:dashboards#[Dashboards]
{related-end}


== Was sind Berichte?

{cee-only}
Ein Bericht in {CMK} soll in druckbarer Form einen Überblick über die Daten, Inhalte und Zustände in {CMK} geben. Er ist eine Zusammenstellung unterschiedlicher Elemente (xref:views#[Tabellenansichten], xref:availability#[Verfügbarkeitstabellen], xref:graphing#[Verlaufsgraphen], Fließtext, Logos) zu einem druckfähigen PDF-Dokument.

image::reporting_report.png[alt="Beispiel eines Berichts.", width=70%]


== Berichte in Checkmk

Die {CEE} liefern einige vorgefertigte Berichte. Diese können Sie entweder direkt nutzen, um Auswertungen aus {CMK} zu erhalten oder auch jederzeit als Vorlage für xref:create_reports[eigene Berichte] nutzen.


=== Kennzeichen von Berichten

* Es sind keine externen Werkzeuge wie Jasper, DB oder Ähnliches notwendig.
* Die PDF-Datei wird sauber mit Vektorgrafiken erstellt.
* Berichte können über Schablonen und Vererbung verwaltet werden.
* Der xref:reporting#scheduler[Berichtsplaner] verschickt auf Wunsch periodisch Berichte.
* Sofortberichte werden über [.guihint]#Export > This view as PDF# erzeugt.

.Ansichten können jederzeit als PDF exportiert werden
image::reporting_menu_export.png[alt="Aufgeklapptes 'Export'-Menü mit dem Eintrag zur PDF-Erstellung."]


=== Vorgefertigte Berichte - Inhalte und Nutzung

Ebenso wie xref:views#[Tabellenansichten] und xref:dashboards#[Dashboards] können in {CMK} mitgelieferte Berichte nicht verändert werden.
Aber Sie können alle bereits vorhandenen Berichte beliebig häufig klonen/kopieren und jeweils an Ihre Bedürfnisse anpassen. Das Vorgehen bei der Erstellung xref:create_reports[einfacher] aber auch xref:complex_reports[komplexer] Berichte wird im Folgenden beschrieben.


==== Welche Berichte gibt es?

Einen Überblick über alle Berichte, vorgefertigte und eigene, in {CMK} finden Sie unter [.guihint]#Customize > Business reporting > Reports# (eventuell müssen Sie vorher noch [.guihint]#show more# anklicken):

.Einige der integrierten Berichte
image::reporting_list.png[alt="Liste mit Berichten."]

Auf der linken Seite stehen folgende Symbole für die Bearbeitung zur Verfügung:

[cols="10,~"]
|===

|icon:icon_clone[alt="Symbol zum Kopieren eines Berichts."] |Bericht kopieren
|icon:icon_pdf[alt="Symbol zur Erstellung einer Berichtsvorschau."] |Vorschau anzeigen
|icon:icon_reportscheduler[alt="Symbol zum Öffnen des Berichtsplaners."] |xref:scheduler[Berichtsplaner] öffnen
|===


=== Berichte aufrufen
//SP: Einleitung fehlt noch

==== Schnellaufruf der Berichte - Das Snapin Reporting in der Seitenleiste
Über das Snapin [.guihint]#Reporting# in der xref:user_interface#sidebar[Seitenleiste] erhalten Sie raschen Zugriff auf alle Berichte und den xref:scheduler[Berichtsplaner] [.guihint]#(Scheduler):#

//ML: Stimmt nicht ganz: Hier landen nicht alle Berichte, sondern nur die, die nicht über "Hide this report in the monitor menu" ausgeschlossen sind; und ausgeschlossen werden sollten wohl alle, die keinen HOst-/Service-Kontext in den Eigenschaften haben, da man ansonsten im PDF mit Fehlermeldungen landet. Vielleicht wäre hier ein Screenshot hilfreich, der nicht nur das leere Snapin zeigt, sondern auch ein paar Berichte innerhalb eines Topics (also standardmäßig "Workplace"). Ich habe das Thema unten in einem Vorschlag-Absatz nochmal aufgegriffen.

.Den Schnellzugriff können Sie der Seitenleiste hinzufügen
image::reporting_sidebar.png[alt="Snapin 'Reporting' in der Seitenleiste.",width=55%]

In diesem Snapin befinden sich drei Knöpfe:

[cols="30,~"]
|===

|[.guihint]#Change Timerange# |Zeitfenster für Sofortberichte ändern
|[.guihint]#Scheduler# |xref:scheduler[Berichtsplaner] öffnen
|[.guihint]#Edit# |Liste vorhandener Berichte öffnen
|===


==== Kontext für Berichte
//==== Unterschiedliche Datenquellen
//ML: Datenquellen würde ich hier nicht verwenden - Datasources gibt es ja als CMK-spezifischen Begriff. Die Festlegung auf Services/Hosts ist der Kontext, der auch in den roten Fehlermeldungen auftaucht. Ich habe mal ein wenig nach Kontext umgeschrieben.
//ML: In den beiden folgenden Screenshots ist das zweite Bild mit den Zahnrädern drin - ich denke, das ist an der Stelle hier etwas verwirrend; würde ich einfach rausretuschieren.

Um Berichten einen Kontext zu geben, sie also mit Daten bestimmter Hosts und Services zu füllen, gibt es verschiedene Möglichkeiten. Daraus resultiert dann auch, dass die Berichte unterschiedlich aufgerufen werden müssen, um sinnvolle Informationen zu enthalten. Fehlt der Kontext, also die Angabe, welche Hosts und Services in den Bericht einfließen sollen, so bekommen Sie statt einer korrekten Ausgabe nur rot hervorgehobene Fehlermeldungen:

image::reporting_output_red.png[alt="Ausgabe eines Berichts mit unspezifizierten Daten.", width=79%]

Der Unterschied besteht darin, an welcher Stelle der Kontext festgelegt wird:

////
ML: Ich VERSUCHE mal einen Vorschlag für eine etwas kürzere Variante, mit scheint das etwas umständlich - kann aber gut sein, dass meine Variante dann zu kurz ist ... :
* Kontext vorgegeben (über Berichteigenschaften): Kann von überall aufgerufen werden, Bericht ist immer mit korrekten Informationen gefüllt.

* Kontext nicht vorgegeben: Aufruf über das [.guihint]#Export-#Menü in Ansichten, Kontext wird durch (gefilterte) Ansicht geliefert.

Denken Sie nun nochmal an das oben erwähnte Snapin: Hierüber gelangen Sie direkt zu den gerenderten Berichten - sinnvoll sind hier also eigentlich nur solche Berichte, die einen _eingebauten_ Kontext haben. Berichte, deren Kontext erst über (gefilterte) Ansichten entsteht, können Sie getrost über die Option [.guihint]#Hide this report in the monitor menu# aus dem Hauptmenü sowie dem Snapin ausschließen.
////

* Direkt in einem Bericht ist angeben, welcher Host, welcher Service etc. für das jeweilige Berichtselement herangezogen werden sollen. Dann sehen Sie diese Informationen immer, wenn Sie den Bericht oder dessen Vorschau aufrufen.

* Der Bericht ist ohne Kontext, also ohne Festlegung auf einen Host, einen Service etc definiert. Dann wird der Bericht erst beim Aufruf und abhängig davon, wo in {CMK} Sie sich befinden, mit Daten gefüllt. D.h. die verwendeten Daten orientieren sich daran, bei welchem Host und welchem Service Sie sich gerade befunden haben als Sie den Bericht aufgerufen haben.

image::reporting_output_nored.png[alt="Ausgabe eines Berichts mit spezifizierten Daten.", width=79%]



=== Eigene Berichte erstellen - Grundlagen

Auf Basis der vorhandenen Berichte und Berichtsvorlagen können Sie relativ schnell und einfach einen ersten eigenen Bericht erstellen. Wie Sie dabei am Besten vorgehen, erklären wir Ihnen gleich. Tiefergehende Informationen rund um komplexere Berichte folgen dann xref:complex_reports[weiter unten] im Text.

[#create_reports]
==== Einen ersten eigenen Bericht erstellen

Die schnellste Methode einen Bericht zu erstellen ist, einen vorhandenen Bericht zu kopieren und dann anzupassen.
Wollen Sie zum Beispiel eine Übersicht über die Verfügbarkeit der Services auf einem spezifischen Host, dem Host namens mycmkserver, erstellen, so gehen Sie folgendermaßen vor.

Öffnen Sie als erstes die Berichtsübersicht mit [.guihint]#Customize > Business reporting > Reports#:

image::reporting_list_selection.png[alt="Auswahl aus der Liste mit Berichten."]

Mit einem Klick auf icon:icon_clone[alt="Symbol zum Kopieren eines Berichts."] vor dem Bericht [.guihint]#service_availability# gelangen Sie in die Grundeinstellungen Ihrer Berichtskopie:

image::reporting_general_properties_simple.png[alt="Eingabe der Grundeinstellungen eines Reports."]

Geben Sie die gewünschten Texte für [.guihint]#Unique ID# und [.guihint]#Title# ein. Mit den weiteren Optionen in diesem Kasten sowie den [.guihint]#Report Properties# befassen wir uns erst bei den xref:complex_reports[komplexeren Berichten.]

Für den Moment reicht es, wenn Sie nach unten zum Kasten [.guihint]#Context / Search Filters# scrollen. Wählen Sie bei [.guihint]#Host# die Option [.guihint]#Hostname# aus. Sobald Sie jetzt auf [.guihint]#Add filter# klicken, können Sie aus der Liste der Hostnamen den passenden Host wählen:

image::reporting_filter_host.png[alt="Auswahl des passenden Hosts für den Bericht."]

Mit icon:icon_save[alt="Symbol für das Speichern."] speichern Sie Ihren Bericht und kehren zur Übersichtsseite der Berichte zurück. Unter [.guihint]#Customized# können Sie nun direkt Ihren neuen Bericht aufrufen, indem Sie auf den Titel klicken:

image::reporting_customized.png[alt="Ansicht des eigenen Berichts."]

==== Bearbeitungsfunktionen
Für die Bearbeitung eigener Berichte können Sie auf folgende Funktionen zurückgreifen:

[cols="10,~"]
|===

|icon:icon_clone[] |Bericht kopieren
|icon:icon_delete[] |Bericht löschen
|icon:icon_edit[] |Berichtseinstellungen bearbeiten
|icon:button_report_element_lo[] |Berichtsinhalte bearbeiten
|icon:icon_pdf[] |Vorschau anzeigen
|icon:icon_reportscheduler[] |Regelmäßigen Bericht planen
|===

Eine Beschreibung der unterschiedlichen Möglichkeiten folgt in den weiteren Kapiteln.


==== Ohne Vorlage einen neuen Bericht erstellen
Ein wenig komplexer wird es nun, wenn Sie anstelle einer Berichtskopie einen komplett neuen Bericht erstellen wollen. Als Beispiel erstellen Sie im Folgenden eine einfache Übersicht über alle problematischen Services Ihrer Instanz, d.h. alle Services im Status {WARN}, {CRIT} oder {UNKNOWN}. Diese Übersicht wird den Titel "critical_services" tragen.

Einen neuen Bericht legen Sie auf der Seite [.guihint]#Edit Reports# über [.guihint]#Reports > Add report# an.

image::reporting_add_report.png[alt="Öffnen eines neuen Berichts.", width=38%]

Im ersten Schritt lassen Sie den Filter [.guihint]#Select specific object type# auf der Auswahl [.guihint]#No restrictions to specific objects# stehen und wechseln direkt mit icon:icon_save[alt="Symbol zum Fortfahren."] [.guihint]#Continue# zur nächsten Auswahl.

image::reporting_no_specific_objects.png[alt="Auswahl 'Keine Einschränkung auf spezifische Objekte' beibehalten."]

Wie beim Kopieren eines bestehenden Berichts geben Sie auch hier zuerst [.guihint]#Unique ID# und [.guihint]#Title# an.

image::reporting_general_properties2.png[alt="Vergabe von eindeutiger ID und Titel für den neuen Bericht."]

Im Abschnitt [.guihint]#Context / Search Filters# wählen Sie nun unter [.guihint]#Service# die [.guihint]#Service states# aus. Nach einem Klick auf [.guihint]#Add filter# können Sie die gewünschten Zustände festsetzen:

image::reporting_search_filters.png[alt="Auswahl der Servicezustände für den Report."]

Mit icon:icon_save[alt="Symbol zum Speichern."] [.guihint]#Save & go to list# wird der neue Bericht in den Abschnitt _Customized_ der Übersichtsliste der Berichte aufgenommen.

image::reporting_customized2.png[alt="Ansicht des eigenen Berichts in der Übersicht."]

Wenn Sie diesen Bericht jetzt in freudiger Erwartung aufrufen, ist das Ergebnis ernüchternd. Die Berichtsseite hat zwar ein erstes Layout und auch eine Überschrift, ist aber ansonsten leer. Nicht gerade das Ergebnis, mit dem man seinen Chef beeindrucken kann ...

image::reporting_empty_report.png[alt="Amnsicht des leeren Berichts."]

Deshalb muss nun die Gestaltung des Inhalts für den Bericht definiert werden. Über das Symbol icon:icon_edit_report[alt="Symbol für die Bearbeitung des Berichtsinhalts."] gelangen Sie zur Inhaltsfestlegung für den Bericht. Die Auswahl von Berichtselementen erfolgt dreistufig, jeweils mit einem Klick auf icon:icon_save[alt="Symbol für das Fortfahren im Prozess."] [.guihint]#Continue# dazwischen.

Beginnen Sie mit einem Klick auf [.guihint]#Add content#. Wählen Sie für dieses Beispiel in [.guihint]#Step 1# den Elementtyp _Availability table._ Setzen Sie [.guihint]#Step 2# auf die Auswahl _All services_ und [.guihint]#Step 3# auf die Auswahl _No restrictions to specific objects_. Die weiteren Optionen der nachfolgenden Seite lassen Sie für den Moment auch erst einmal unverändert. Stattdessen klicken Sie direkt auf icon:icon_save[alt="Symbol zum Speichern."] [.guihint]#Save#. Betrachten Sie nun die Vorschau auf Ihren Bericht:

image::reporting_content_output.png[alt="Ansicht des ersten eigenen Elements."]

Auf der linken Seite sehen Sie die Vorschau, hier für den Bericht [.guihint]#critical_services.#
Rechts daneben werden die Elemente des Berichts in einer Liste angezeigt.
Diese Elemente werden mit den Symbolen icon:icon_edit[] bearbeitet, icon:icon_clone[] kopiert, icon:icon_delete[] gelöscht und icon:icon_drag[] umsortiert.


Diesen einfachen, selbst erstellten Bericht können Sie nun flexibel um weitere Elemente erweitern (siehe unten) und jederzeit, z. B. aus der Seitenleiste heraus, aufrufen.

[#complex_reports]
=== Aufbau komplexer Berichte
In den vorherigen Abschnitten haben Sie gesehen, wie man mit geringem Aufwand erste Berichte erstellen kann, entweder als Kopie eines bestehenden Berichts oder auch komplett neu. Diese sind toll, um erste Erfolge zu erzielen. Meist bleibt es jedoch nicht bei so einfach strukturierten Berichten. Komplexere Darstellungen, eine Mischung verschiedener Inhalte, ein eigenes Layout usw. sind der nächste logische Schritt.

Die vielen Möglichkeiten lassen sich am Besten an einem Beispiel aufzeigen.


//ML: Wenn das Bild das Endergebnis von 2.5/Komplexe Berichte zeigen soll, fehlt noch das zweite Bild, das später eingefügt wird.
image::reporting_complex_report.png[alt="."]

==== Grundeinstellungen setzen
In beinahe jedem Schritt der Berichtserstellung gibt es die Möglichkeit, die Inhalte an Ihre Bedürfnisse anzupassen.

Im ersten Schritt können Sie einen Filter setzen und den Bericht von vornherein auf einen bestimmten Host oder eine bestimmte Datenquelle beschränken, etwa Netzwerkschnittstellen, BI-Aggregate, Sensoren und so weiter:

.Meist werden Sie die Filter auf dieser Ebene jedoch nicht benötigen
image::reporting_specific_objects.png[alt="Liste mit Filtern für die Beschränkung des Berichts."]

Als nächstes tragen Sie wieder die [.guihint]#Unique ID# und den [.guihint]#Title# ein. Wählen Sie ein [.guihint]#Icon#, mit dem der Bericht in den Menüs sichtbar sein soll. Durch die Einstellung der Option [.guihint]#Make this report available for other users# auf die Kontaktgruppe, im Beispiel: check-mk-notify, geben Sie den zugehörigen Gruppenmitgliedern die Möglichkeit, den Bericht jederzeit eigenständig aufzurufen und sich selbstständig zu informieren.


image::reporting_general_properties_complex.png[alt="Einstellung der komplexeren Grundeigenschaften für den Bericht."]

Im Kasten [.guihint]#Context / Search Filters# stehen Ihnen wie üblich umfangreiche Filtermöglichkeiten zur Verfügung, von bestimmten Hosts über einzelne Docker-Container bis hin zu Werten aus der Oracle-Überwachung.

image::reporting_context_search_filters.png[alt="Auswahl für die Gestaltung der Berichtsinhalte."]

Für dieses Beispiel ist die Host-Auswahl auf einen Ordner beschränkt und zusätzlich auf festgelegte Host-Probleme. Diese Einschränkung beschleunigt, gerade in komplexeren Umgebungen, die Datenabfrage.

Mit [.guihint]#Save & go to list# gelangen Sie zur Berichtsübersicht. Als nächstes geht es an die Ausgestaltung des Berichtsinhalts.

==== Berichtsinhalt definieren
Mit Hilfe des Symbols icon:button_report_element_lo[] gelangen Sie zur inhaltlichen Gestaltung.

image::reporting_edit_content.png[alt="Aufruf der Inhaltsgestaltung."]

Öffnen Sie mit [.guihint]#Reports > Add content# die Bearbeitung des Berichtsinhalts. Hiermit fügen Sie beispielsweise Tabellen, Graphen, Inhaltsverzeichnisse oder Texte (auch via Makros oder HTTP) in den Bericht ein. Diese Inhalte erscheinen nacheinander und bestimmen so auch die finale Länge des Berichts.

.Eine Auflistung aller Seitenelemente finden Sie in der xref:further_elements[Übersicht der Inhalts- und Seitenelemente].
image::reporting_add_content.png[alt="Hinzufügen eines Inhaltselements.", width=58%]

Wählen Sie in Schritt 1 das gewünschte Element aus, für dieses Beispiel [.guihint]#Multiple graphs#. Damit erhalten Sie, abhängig von Ihren weiteren Einstellungen, mehrere gleichartige Grafiken untereinander. So können Sie zum Beispiel den direkten Vergleich zwischen den Zuständen mehrerer Hosts abbilden.

image::reporting_step1.png[alt="Auswahl einer Grafik."]

Fahren Sie mit icon:icon_save[alt="Symbol zum Weiterspringen."] [.guihint]#Continue# fort. Als Datenquelle für die Grafiken wählen Sie in Schritt 2 für dieses Beispiel [.guihint]#All services#, d.h. alle auf den Hosts laufenden Services:

image::reporting_step2.png[alt="Auswahl der Datenquelle."]

Fahren Sie mit icon:icon_save[alt="Symbol zum Weiterspringen."] [.guihint]#Continue# fort. In Schritt 3 könnten Sie nun die anzuzeigende Datenmenge beschränken. Dies wird für das aktuelle Beispiel nicht benötigt. Wieder wechseln Sie mit icon:icon_save[alt="Symbol zum Weiterspringen."] [.guihint]#Continue# zur nächsten Auswahl.

image::reporting_properties_context.png[alt="Festlegung verschiedener Eigenschaften."]

Für den Beispielbericht legen Sie im Kasten [.guihint]#Properties# unter [.guihint]#Graph layout# fest, dass Sie für die Grafiken jeweils einen Titel sehen wollen, bestehend aus dem Grafiktitel, dem Hostnamen und der Servicebeschreibung. Zudem setzen Sie die angezeigte Zeitspanne unter [.guihint]#Graph time range# auf vier Stunden.

Im Kasten [.guihint]#Context / Search Filters# schränken Sie den Bericht auf festgelegte Host- oder Servicemerkmale ein. Hier wählen Sie diesmal die CPU-Auslastung für den [guihint]#Service# aus.

Mit einem Klick auf icon:icon_save[alt="Symbol zum Speichern."] [.guihint]#Save# haben Sie das erste Berichtselement fertiggestellt.

Für die zweite Grafik im Beispiel beginnen Sie wieder mit [.guihint]#Reports > Add content# und wählen in [.guihint]#Step 1# die Option [.guihint]#Metrics graph of a single service#. Wechseln Sie mit icon:icon_save[alt="Symbol zum Weiterspringen."] [.guihint]#Continue# zu [.guihint]#Step 2# und beschränken Sie die Suchfilter auf Host und Servicebeschreibung:

image::reporting_context_search_filters2.png[alt="Auswahl von Host und Servicebeschreibung."]

Mit einem Klick auf icon:icon_save[alt="Symbol zum Speichern."] [.guihint]#Save# ist das zweite Berichtselement fertig.

Als drittes Element erstellen Sie eine Übersicht aktueller Alarmierungen. Das Vorgehen bleibt wie gehabt:

* Aufruf von [.guihint]#Reports > Add content#
* Step 1: Auswahl des Elements [.guihint]#Availability table#
* Step 2: Datenquelle [.guihint]#Alert Statistics#
* Step 3: Einschränkung auf [.guihint]#Restrict to a single host#
* keine weiteren Optionen im nachfolgenden Schritt

Mit einem Klick auf icon:icon_save[alt="Symbol zum Speichern."] [.guihint]#Save# ist auch dieses Berichtselement fertig.

Um die Darstellung übersichtlicher zu gestalten, fügen Sie noch Überschriften ein:

Öffnen Sie mit [.guihint]#Reports > Add content# die Bearbeitung des Berichtsinhalts. Wählen Sie in [.guihint]#Step 1# als Elementtyp [.guihint]#Heading#. Wechseln Sie mit icon:icon_save[alt="Symbol zum Weiterspringen."] [.guihint]#Continue# zu [.guihint]#Step 2#. Optional können Sie eine Hierarchieebene für die Überschrift angeben. Tragen Sie die Überschrift bei [.guihint]#The text# ein.

image::reporting_add_heading.png[alt="Eingabe der Überschrift."]

Mit einem Klick auf icon:icon_save[alt="Symbol zum Speichern."] [.guihint]#Save# kehren Sie zur Übersicht zurück. Die neue Überschrift befindet sich jetzt jedoch in der Reihenfolge unterhalb der Grafiken. Klicken Sie daher in der Liste der Inhaltselemente auf das entsprechende icon:icon_drag[] und verschieben Sie die Überschrift an die gewünschte Position in der Liste.

image::reporting_content_overview.png[alt="Bearbeitungsansicht des Berichts mit roten Elementen."]

*Tipp:* In der Vorschau sehen Sie möglicherweise rote Zeilen und Fehlermeldungen statt echter Daten -- denn an diesen Stellen fehlt dem Bericht im Editor der Kontext, hier also etwa die Angabe eines Hosts, der sich erst über den Aufruf des Berichts aus einer entsprechenden Ansicht ergeben würde.
Wenn Sie eine brauchbare Vorschau sehen wollen, filtern Sie einfach in den Berichtseinstellungen im Bereich [.guihint]#Context / Search Filters# zwischenzeitlich auf einen bestimmten Host, das macht das Gestalten deutlich einfacher.

image::reporting_content_overview2.png[alt="Bearbeitungsansicht des Berichts ohne rote Elemente."]

[#views]
===== Optische Gestaltung des Berichts

Um den Bericht zu strukturieren, gibt es ergänzende Gestaltungslelemente: Horizontale Linie, Bild, Seitenrahmen und eine einzelne Textzeile. Diese Bausteine können Sie auf allen oder bestimmten Seiten Ihres Berichts anzeigen lassen.

Für den Beispielbericht fügen Sie einen Rahmen um den Bericht ein. Auch hier erfolgt die Definition wieder schrittweise, jeweils fortgesetzt mit einem Klick auf icon:icon_save[alt="Symbol zum Weiterspringen."]. Beginnen Sie mit [.guihint]#Reports > Add page element#.

.Eine Auflistung aller Seitenelemente finden Sie in der xref:further_elements[Übersicht der Inhalts- und Seitenelemente].
image::reporting_step1_fixed.png[alt="Auswahl des Rahmens."]

Da der Rahmen auf allen Seiten sichtbar sein soll, erfordert der nächste Schritt keine Änderungen:

image::reporting_placement.png[alt="Auswahl der Positionierung."]

Der neue Seitenrand wird nun in der Übersicht, sowohl in der Vorschau links als auch in der Tabelle der statischen Seitenelemente rechts, angezeigt:

image::reporting_content_overview3.png[alt="Aktuelle Übersicht des Berichts."]

//ML: An dieser Stelle könnte man noch die Makros erwähnen, beispielsweise, um eine Fußzeile (Line of Text mit Position From Bottom ...) mit dem Titel und der Seitenzahl einzufügen: "Seite $PAGE$ von $REPORT_TITLE". Vielleicht einfach "Tipp:" voranstellen, spart das Ändern obiger Screenshots.

==== Das Layout anpassen
Mit einem hochwertigeren Erscheinungsbild, z.B. mit dem Firmenlogo versehen, wirken Berichte gleich ansprechender.

Öffnen Sie mit [.guihint]#Reports > Edit properties# die Berichtseinstellungen:

image::reporting_edit_properties.png[alt="Auswahl der Option 'Edit properties'.", width=62%]

Im zweiten Kasten finden Sie hier die [.guihint]#Report Properties.#
An dieser Stelle konfigurieren Sie Werte für Schrift, Seitenformat, Zeit- und Datumseinstellungen, das Berichtslayout und den Dateinamen bei Downloads.

.An dieser Stelle können Sie grundlegende Gestaltungsmerkmale vorgeben
image::reporting_report_properties2.png[alt="Formular für Layout-Voreinstellungen eines Berichts."]

Kopieren Sie Ihr Firmenlogo in das Verzeichnis `local/share/check_mk/reporting/images` und speichern Sie es als Datei `logo_mk.png`.
Ab sofort wird dieses Logo auf allen Berichten angezeigt.

image::reporting_own_logo.png[alt="Ansicht des Berichtskopfes mit eigenem Logo.", width=80%]

*Hinweis:* Aktuell passen Sie die bestehende Berichtsvorlage an Ihre Bedürfnisse an. Alternativ können Sie auch eine xref:new_template[komplette, neue Berichtsvorlage] entsprechend Ihren Firmenvorgaben erstellen, z.B. um das Corporate Design des Unternehmens abzubilden.

===== Zusätzliches Bild einfügen

Bilder, die Sie in Ihren Berichten als Gestaltungselemente verwenden möchten, müssen vorher im Verzeichnis `local/share/check_mk/reporting/images` abgelegt worden sein.

Klicken Sie in der Berichtserstellung auf den Knopf [.guihint]#Add page element#.

//ML: Der Mauszeiger müsste da noch raus; optional vielleicht noch die Tabelle unten rechts wegretuschieren.
image::reporting_add_page_element.png[alt="Aufruf der Auswahl von Bildelementen.", width=71%]

Wählen Sie in dem folgenden [.guihint]#Step 1# als Typ [.guihint]#Embedded image# und setzen Sie mit icon:icon_save[alt="Symbol zum Fortfahren."] [.guihint]#Continue# fort. (Eine Auflistung aller Elemente)

.Eine Auflistung aller Seitenelemente finden Sie in der xref:further_elements[Übersicht der Inhalts- und Seitenelemente].
image::reporting_image_add.png[alt="Dialog zur Auswahl eines Bildelements."]

Im folgenden Dialog werden Detaileinstellungen wie Position, Dateiname und Größe gesetzt.

.Größenangaben in Millimetern für einfache Druckgestaltung
image::reporting_image_properties.png[alt="Dialog für Bildeigenschaften."]

Wenn Sie mit allen Einstellungen fertig sind, speichern Sie mit icon:icon_save[alt="Symbol zum Speichern."] [.guihint]#Save.#
Danach gelangen Sie in die Berichtsübersicht und sehen das eingefügte Bild im Bericht.

image::reporting_own_pic.png[alt="Ansicht des Berichtskopfes mit eigenem Logo.", width=80%]



[#scheduler]
=== Report zeitlich planen

//ML: Vielleicht irgendwo noch "Stored Reports" erklären, also dass geplante Reports lokal gespeichert werden können und dann dort landen. Über den Scheduler ergibt sich das zwar eigentlich von selbst, aber wenn man nur den Menüpunkt "Stored Reports" sieht, müsste man als Nutzer erstmal lange suchen.

Über den Berichtsplaner können Sie vorhandene Berichte automatisiert erstellen und versenden lassen.
Sie erreichen den Planer auf der Seite [.guihint]#Edit Reports# über [.guihint]#Reports > Scheduler.#

Erstellen Sie mit [.guihint]#Add schedule# einen neuen Plan, wählen Sie dann den Bericht aus und klicken Sie [.guihint]#Continue#.

Im Kasten [.guihint]#General Options# legen Sie Grundeinstellungen wie Titel, Erstellungszeit, Berichtszeitraum, Empfänger und so weiter fest:

.Sie können sich zum Beispiel automatisch freitagnachmittags einen Wochenbericht per PDF zuschicken lassen
image::reporting_schedule.png[alt="Einstellungen für den Berichtsplaner."]

Unter [.guihint]#Context / Search Filters# finden Sie die üblichen Filtermöglichkeiten, abhängig vom gewählten Bericht.
Speichern Sie den Bericht anschließend, um zur Liste Ihrer geplanten Berichte zurückzukehren.

In der Liste finden Sie auch Informationen darüber, wann der Bericht zum letzten Mal erzeugt/versandt wurde, ob dabei ein Fehler aufgetreten ist, wann er das nächste Mal
erzeugt/versandt wird und einiges mehr (hier leicht gekürzt):

.Ein geplanter Bericht mit ausführlichen, hier leicht gekürzten, Informationen
image::reporting_schedule_list.png[alt="Liste mit geplanten Berichten."]

Einträge in der Liste werden über die fünf Symbole am Listenanfang verwaltet:

[cols="10,~"]
|===

|icon:icon_edit[] |Berichtseinstellungen bearbeiten
|icon:icon_clone[] |Berichtseinstellungen kopieren
|icon:icon_delete[] |Geplanten Bericht löschen
|icon:icon_pdf[] |Bericht erzeugen
|icon:icon_pdfpreview[] |Vorschau anzeigen
|===

*Hinweis:* Versendete Berichte werden nicht in der Instanz gespeichert.


=== Globale Vorgaben für Berichte

Einige grundlegende Einstellungen für Berichte können Sie global festlegen, zum Beispiel um ein Corporate Design zentral zu erstellen und zu verwalten. Einstellungen zu einem einzelnen Bericht legen dann nur noch die Abweichungen zum Corporate Design fest.
Die globalen Voreinstellungen für Berichte konfigurieren Sie über [.guihint]#Setup > General > Global settings > Reporting.#

.Einstellungen wie Vorlagen und Schriftgrößen können Sie übergreifend festlegen
image::reporting_global_settings.png[alt="Globale Einstellungen für Berichte."]

[#new_template]
=== Eine eigene Berichtsvorlage erstellen
{CMK} stellt eine vorgefertigte, übersichtliche Berichtsvorlage bereit, die Sie, wie oben beschrieben, beliebig ergänzen und nutzen können. Gerade Berichte, die an das Management weitergereicht werden, wirken aber noch einmal so gut, wenn sie dem eigenen Firmenlayout entsprechen. Dazu gibt es die Möglichkeit, eine eigene Berichtsvorlage zu erstellen.

Standardmäßig wird für alle Berichte in {CMK} das [.guihint]#Default template for all reports# verwendet:

image::reporting_list_selection2.png[alt="Auswahl aus der Liste mit Berichten."]

Um eine eigene Berichtsvorlage zu erstellen, kopieren Sie die Standardvorlage mit icon:icon_clone[alt="Symbol zum Kopieren"]. Ändern Sie als erstes [.guihint]#Unique ID# und [.guihint]#Title#.

Um diese Berichtsvorlage als solche zu markieren, aktivieren Sie im Kasten [.guihint]#Report Properties# die Option [.guihint]#This is a front matter Template#:

image::reporting_report_properties4.png[alt="Festlegung als Berichtsvorlage."]

Ab dem ersten Speichern finden Sie diese Vorlage dann bei den eigenen Berichtsvorlagen:

image::reporting_customized3.png[alt="Auswahl der Berichtsvorlage im Kasten 'Customized'."]

Nun können Sie diese Vorlage entsprechend der obere Beschreibungen genau wie jeden anderen Bericht an Ihre Bedürfnisse anpassen. Verändern Sie die Seiteneinteilung, die Schrift, die Linienfarben etc. Legen Sie auch gleich die Inhaltselemente fest. Exklusiv für Berichtsvorlagen gibt es zudem das Berichtselement [.guihint]#Table of Contents#:

image::reporting_step1_content.png[alt="Auswahl des Inhaltsverzeichnisses."]

Um einen Ihrer Berichte nun mit der neuen Vorlage zu erstellen, ändern Sie noch die Einstellungen in den [.guihint]#Report Properties# des Berichts. Stellen Sie die Option [.guihint]#Use settings and page layout from# auf den Namen Ihrer neuen Vorlage um:

image::reporting_report_properties3.png[alt="Wechsel der Berichtsvorlage."]

Ihr Bericht wird ab dem Speichern künftig immer mit dieser Vorlage erstellt werden.


[#further_elements]
== Übersicht der Inhalts- und Seitenelemente

//SP: Zu Combined graph / Custom graph / Graph collection / Metrics graph of a single service fehlen mir noch Informationen.
[cols="~,~,~",options="header"]
|===
|Inhaltselement |Funktion |Möglichkeiten
|A Multisite view |komplexe Ansicht zu definierbarem Thema |mehrschichtige Auswahl
|Availability table |Liste der Verfügbarkeitszustände |Auswahl der Datenquelle und weitere Einschränkungen der Daten möglich
|Combined graph |Graphen auf Basis mehrerer Datenreihen |Verwendung einer Vorlage, Auswahl der Datenquelle und diverse Layout-Einstellungen möglich
|Custom Graph |Individuell erstellte Graphen |Standard-Layout-Optionen
|Embedded image |statisches Bild | Größe kann festgelegt werden. _Die Position dieses Bildes ist dynamisch, relativ zu den umgebenden Inhaltselementen!_
|Forecast Graph |Prognoseansicht |Vorgabe der zu berücksichtigenden Hosts, Services und Zeiträume
|Graph collection |Alle Graphen einer Sammlung |Standard-Layout-Optionen
|Heading |Überschrift |Abstufung in Überschriften verschiedener Hierarchien möglich
|Host report for multiple hosts |Statusberichte für verschiedene Hosts |Definition der einzubindenden Sub-Berichte inklusive Strukturierung
|Metrics graph of a single service |Metriken eines einzelnen Services als Graph |Standard-Layout-Optionen
|Multiple graphs |Sammlung mehrerer Grafiken zu einem Thema |Auswahl der Datenquelle, verschiedene Gestaltungsmerkmale der Grafiken
|Page break |Seitenumbruch |
|Paragraph of text |statischer Text |_Die Position dieses Textes ist dynamisch, relativ zu den umgebenden Inhaltselementen._
|Service report for multiple services |Serviceberichtssammlung für mehrere Services |Vorgabe der einzubindenden Sub-Berichte, verschiedene Gestaltungsmerkmale der Einzelberichte
|Table of available text makros |Übersicht aller verfügbaren Makros |Name und aktueller Output des Makros werden angezeigt
|Table of Contents (only active in frontmatter templates) |Inhaltsverzeichnis |_nur in Berichtsvorlagen einsetzbar_
|Vertical space |Freifläche |Höhe sowie Verhalten an einem Seitenanfang können definiert werden
|===

[cols="~,~,~",options="header"]
|===
|Gestaltungselement |Funktion |Möglichkeiten
|A frame around the page |Rahmen um die gesamte Seite |wahlweise um alle oder um spezifische Seiten
|Embedded image |statisches Bild | wahlweise auf einer oder auf spezifischen Seiten, Position, und Größe können festgelegt werden. _Dieses Bild bewegt sich nicht mit den Berichtsinhalten mit!_
|Horizontal rule |waagrechte Linie| wahlweise auf allen oder auf spezifischen Seiten, Position, Farbe und Dicke können festgelegt werden
|One line of text |statische Textzeile |wahlweise auf allen oder auf spezifischen Seiten, Position und Schrifteigenschaften können festgelegt werden
|===



== Dateien und Verzeichnisse

[cols="40,~"]
|===
|Pfad/Datei |Bedeutung

|`var/check_mk/web/_ID_des_Eigentümers_/user_reports.mk` |In diesen Dateien werden die angepassten und neu erstellten Berichte der jeweiligen [.guihint]#Eigentümer (Owner)# der Berichte gespeichert.
|===