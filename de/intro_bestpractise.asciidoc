...
Die entscheidende Option in der neuen Regel lautet [.guihint]#Service states#.
...
Gleichzeitig erhalten Sie beim Service für jede der eingetragenen IP-Adressen Messdaten für die typische Paketumlaufzeit (_round trip time_) und den Paketverlust.
...
Der letzte und vierte Schritt ist notwendig, damit der Host nicht den Zustand {DOWN} erhält, falls die erste IP-Adresse nicht per `ping` erreichbar ist.
Stattdessen übernimmt der Host den Status seines einzigen Services.

*Wichtig:* Da ein Service grundsätzlich nicht benachrichtigt, wenn sein Host {DOWN} ist, ist es wichtig, dass Sie die Benachrichtigungen über den Host steuern -- und nicht über den Service.
...
Die Lösung ist ein sogenannter *aktiver Check.*
...
. Legen Sie zuerst in {CMK} einen neuen Host für den Webserver an, z.B. für `checkmk.com`.
Aktivieren Sie unter [.guihint]#Monitoring agents# die Option [.guihint]#Checkmk agent / API integrations# und wählen dort [.guihint]#No API integrations, no Checkmk agent# aus.
Speichern Sie den Host ohne Service-Erkennung.
...
+
--
Im Kasten [.guihint]#Check HTTP service# finden Sie zahlreiche Optionen zur Durchführung des Checks. Beachten Sie dabei die folgenden Punkte:
...
* Mit [.guihint]#Mode of the Check > Fixed string to expect in the content# können Sie prüfen lassen, ob in der Antwort -- also in der gelieferten Seite -- ein bestimmter Text vorkommt.
...
Sie bekommen jetzt einen neuen Host mit einem Service, der einen Zugriff per HTTP(S) prüft.

image::intro_http_service.png[alt="Listeneintrag eines Services zur Überwachung der HTTP/HTTPS-Dienste auf einem Host."]
...
. In dieser Regel aktivieren Sie [.guihint]#Levels for filesystem# und lassen den Standard der Schwellwerte 80{nbsp}% bzw. 90{nbsp}% unverändert.
...
. Setzen Sie ferner [.guihint]#Reference size for magic factor# auf 20{nbsp}GByte.
Da 20{nbsp}GByte der Standardwert ist, wird er wirksam, auch ohne dass Sie die Option explizit aktivieren.
...
Die in diesem Kapitel angewendeten Standardwerte von {CMK} haben sich in der Praxis bei sehr vielen Installationen bewährt.
