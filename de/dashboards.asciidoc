...
[#builtin_dashlets]
=== Dashlets
Hier sehen Sie zunächst alle 23 Dashlets in der Übersicht, anschließend zeigen wir Ihnen noch ein paar Besonderheiten, die im obigen Beispiel-Dashboard nicht mit abgehandelt wurden. 
...
image::dashboards_total_dashlets.png[alt="Dialog mit Fehlermeldung bezüglich nicht überwachter entfernter Instanz.]

Grundsätzlich heißt das also, dass {CMK} den Server der entfernten Instanz nicht als Host im Monitoring sieht. Zunächst sollten Sie also prüfen, ob nicht bloß die Verbindung für das verteilte Monitoring aufgesetzt, sondern auch der entfernte Server als Host aufgenommen wurde.

Nun kann es sein, dass der Server der erntfernten Instanz aber sehr wohl bereits im Monitoring ist und die Fehlermeldung dennoch auftaucht. In dem Fall liegt es in der Regel an der Version der entfernten Instanz: Wenn die zentrale Instanz eine {v20} ist, muss auch die entfernte eine {v20} sein. In diesem Fall müssen Sie entsprechend aktualisieren -- beachten Sie aber die xref:update_20#[Update-Hinweise.]

Und noch eine Ursache kann für die Fehlermeldung verantwortlich sein: In der Verbindung zu einer entfernten Instanz wird für diese eine [.guihint]#Site ID# vergeben. Wenn diese ID nicht **exakt** mit dem Namen der Instanz übereinstimmt, die IP-Adresse aber korrekt ist, wird die Verbindung selbst zwar als okay angezeigt, aber es wird im Dashlet diese Fehlermeldung provoziert. Die Lösung ist einfach: Neue Verbindung mit korrekter ID anlegen.
