...
Nun kann es sein, dass der Server der Remote-Instanz aber sehr wohl bereits im Monitoring ist und die Fehlermeldung dennoch auftaucht. In dem Fall liegt es in der Regel an der Version der Remote-Instanz: Wenn die Zentralinstanz eine {v20} ist, muss auch die entfernte eine {v20} sein. In diesem Fall müssen Sie entsprechend aktualisieren -- beachten Sie aber die xref:update_20#[Update-Hinweise.]
