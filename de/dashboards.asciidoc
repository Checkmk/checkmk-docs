...
Neben diesem [.guihint]#Main Overview}}-Dashboard bringt {CMK} noch die Varianten {{Host & Services Problems# und
[.guihint]#Network Topology}} mit, die Sie beide in der Seitenleiste unter {{Views|Overview# finden.
...
image::dashboarddashlet.png[center, width=60%]
...
image::addtodashboard.png[center, width=60%]
...
und [.guihint]#Service Statistics}} im Dashboard {{Main Overview#) auch solche gibt,
...
* In jeder Ecke befindet sich ein Anker-Element icon:dashlet_anchor_off.png]. Es kann nur eines aktiv ICON[dashlet_anchor_lo[] sein. In der Standardeinstellung ist der Anker oben links aktiv.
* Indikatoren zeigen vom aktiven icon:dashlet_anchor_lo.png] zu den inaktiven ICON[dashlet_anchor_off[] Ankern. Diese Indikatoren zeigen den Größenmodus des Elements an: Grau (fixe Größe), Gelb (Element wächst), Rot (Maximum).


== Ein neues Dashboard erstellen#new

Um ein ganz neues Dashboard zu erstellen, klicken Sie in der Seitenleiste im
Element [.guihint]#Views}} auf {{EDIT#.  Sie gelangen dann zunächst zu der Liste
