...
Dieses radikal einfache Prinzip ermöglicht auch kreative Abwandlungen, mit denen Sie auch sehr ausgefallene Anforderungen erfüllen können, wie z.B. solche, die sich aus speziellen Architekturen für Sicherheit oder Netzwerk ergeben.
...
welche für diesen Host gilt und tragen Sie `myds` als aufzurufendes Programm ein:

image::ds_program.png[alt="Eingabemaske für einen individuellen Befehl."]

Wenn Sie jetzt in der Setup-GUI zur Service-Konfiguration des Hosts wechseln, sollten Sie
genau einen Service sehen, der für die Überwachung bereitsteht:

image::ds_program_discovery.png[alt="Der neue Dienst wurde erkannt."]
...
Form eines xref:localchecks#[lokalen Checks] aus. Es greift per `$1` auf das erste Argument
zu und speichert es zum Zwecke der Übersicht in der Variable `$HOST_NAME`.
Diese wird dann in die Pluginausgabe des lokalen Checks eingesetzt:
...
image::ds_program_discovery_host.png["Die Service-Erkennung findet den neuen Dienst, der nun den übergebenen Host-Namen als Information ausgibt."]
...
|`$_HOSTTAGS$` |Die Liste aller Host-Merkmale durch Leerzeichen getrennt. Setzen Sie dieses Argument auf jeden Fall in Anführungszeichen, um es vor einem Aufteilen durch die Shell zu schützen.
|===
...
setzt den Exit Code des Programms auf 1:
...
Sobald ein Befehl fehlschlägt (Exit Code ungleich 0), bricht die Shell sofort
ab und beendet das Skript mit dem Exit Code 1. Damit haben Sie eine generische
...
konfigurieren können. Diese Regelsätze finden Sie gruppiert nach Anwendungsfällen unter
...
{CMK} {v20} gruppiert nicht mehr nach Zugriffstechnik, sondern nach Art des überwachten Objektes.
Dies ist insbesondere deshalb sinnvoll, weil in vielen Fällen den Anwender nicht interessiert, mit welcher Technik Daten eingeholt werden und zudem oft Datenquellen und xref:piggyback#[Piggyback] miteinander kombiniert werden, hier also keine klare Abgrenzung möglich ist.
