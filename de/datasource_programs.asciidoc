...
// MFS: Ich finde die Erklärung etwas eng gefasst "nur über API erreichbar" IMHO besser:
// Mit Datenquellenprogrammen werden Hosts überwacht, bei denen der CMK-Agent nicht direkt erreichbar ist und das Einholen der Agenten-Ausgabe über ein API oder einen Netzwerktunnel stattfindet. 
...
// MFS: START-NEW Neuer Text referenziert den Agent Controller
Der {CMK}-Server erreicht Hosts im Monitoring üblicherweise über eine TCP-Verbindung auf Port 6556.
Seit {v21} lauscht auf diesem Port in den meisten Fällen der neue Agent Controller, welche die Agentenausgabe über eine TLS verschlüsselte Verbindung weiterleitet.
Doch in einigen Fällen – zum Beispiel bei sehr schlanken Containern, Legacy- oder Embedded-Systemen kann der Agent Controller nicht verwendet werden.
...
Doch in einigen Fällen verlangen Policies, dass Sicherheitsaspekte wie der Verzicht auf die Übertragung von Daten im Klartext berücksichtigt werden müssen.
...
In solchen Fällen profitieren Sie von der Modularität von {CMK}.
...
// MFS: REFERENCE-OLD
////
{CMK} erreicht seine Agenten üblicherweise über eine einfache TCP-Verbindung auf Port 6556.
Das „Protokoll“ ist trivial:
Sobald der Server die Verbindung öffnet, sendet der Agent seine Daten und schließt den Port wieder.
Die übertragenen Daten sind lesbarer Unicode-Text.
Letztendlich geschieht hier nichts anderes, als dass eine Textdatei vom Agenten zum Server übertragen wird.
...
Eine häufig benötigte Spielart davon -- die Übertragung der Daten über SSH -- wird im Artikel über den xref:agent_linux_legacy#ssh[Linux-Agenten] beschrieben.
