...
// IGNORE 100kB NetApp Filern WebAPI
// MFS: Ich habe NetApp-Filer damit mal temporär zugelassen, evtl. sollten wir ein besseres Wort finden, "Netzwerkspeicher" o.ä.
// MFS: NetApp ggf. in unser Brandnames Dictionary?

== Einleitung

// MFS: Hinweis auf Push hinzugefügt, soll implizieren: CEE Nutzer barauchen Datasource seltener.
...
{CCE} {v22} führte darüber hinaus mit dem Push-Modus die zusätzliche Möglichkeit ein, die Übertragungsrichtung zu wählen.
...
Letztlich kann die Klartextausgabe des Agentenskriptes über beliebige Wege transportiert werden -- direkt oder indirekt, per Push oder Pull oder einer Kombination von beidem.
Hier sind einige Beispiele, mit denen {CMK}-Anwender Daten vom Agenten zum {CMK}-Server bekommen:

// MFS: Push-Zaunpfahl (scp/rsync) deutlicher
* per E-Mail
* per HTTP-Zugriff vom Server aus
* per HTTP-Upload vom Hosts aus
...
////
=== Datenquellenprogramme und {CMK} Enterprise Editions

Der neue Agent ab {CMK} {v21} führte die standardmäßige Verschlüsselung der Übertragung des Agenten-Outputs sein, {CCE} {v22} darüber hinaus die Möglichkeit, für Linux- und Windows-Clients in den meisten Konstellationen die Übertragungsrichtung zu wählen (Pull oder Push).

Für Nutzer dieser Versionen dürfte damit in vielen Fällen die Notwendigkeit des Einsatzes von Datenquellenprogrammen wegfallen, zudem profitieren Nutzer der {CCE} von der Möglichkeit, Hosts automatisch erstellen zu lassen.
////
