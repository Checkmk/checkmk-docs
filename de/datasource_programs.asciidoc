...
:revdate: draft
:title: Datenquellenprogramme - Geräte ohne Betriebssystemzugriff überwachen
...
link:piggyback.html[Piggyback-Daten]
{related-end}
...
den Port wieder. Die übertragenen Daten sind lesbarer Unicode-Text.
Letztendlich geschieht hier nichts anderes, als dass eine Textdatei vom Agenten
zum Server übertragen wird.

Dieses radikal einfache Prinzip ermöglicht auch kreative Abwandlungen, mit denen Sie auch sehr ausgefallene Anforderungen erfüllen können.
Wie z.B. solche, die sich aus speziellen Architekturen für Sicherheit oder Netzwerk ergeben.
Eine häufig benötigte Spielart davon -- die Übertragung der Daten über SSH -- wird im Artikel über den xref:agent_linux#ssh[Linux-Agenten] beschrieben.
Hier sind einige weitere Beispiele, wie man die Daten der Agenten zum {CMK}-Server bekommen kann:
...
// MFS: * per Skript, das via MQTT-Broker Daten aus einem Sensornetz übernimmt

Die universelle Methode, um solche Transportwege an {CMK} anzudocken, sind die [.guihint]#Datasource programs# (Datenquellenprogramme).
Die Idee ist sehr einfach: Sie geben {CMK} die Kommandozeile eines Befehls.
...
Da Änderungen der Datenquellen in der Regel nur Transportwege betreffen, ist es wichtig, dass Sie den Host in der Setup-GUI auf der Einstellung [.guihint]#API integrations if configured, else {CMK} agent# belassen.
...
fiktive Monitoring-Daten: Anstatt einen neuen Transportweg zu integrieren,
erzeugt es Monitoring-Daten selbst. Diese enthalten als einzige Sektion `+<<<df>>>+`
...
Dieser benötigt keine IP-Adresse. Um zu verhindern, dass {CMK} den Namen `myserver125` per DNS aufzulösen versucht, tragen Sie diesen Namen als explizite „IP-Adresse“ ein.

Legen Sie dann eine Regel im Regelsatz
[.guihint]#Setup > Agents > Other integrations > Individual program call instead of agent access# an,
welche für diesen Host gilt und tragen Sie `myds` als aufzurufendes
Programm ein:

image::ds_program.png["Eingabemaske für einen individuellen Befehl."]

Wenn Sie jetzt in der Setup-GUI zur Servicekonfiguration des Hosts gehen, sollten Sie
genau einen Service sehen, der für die Überwachung bereitsteht:

image::ds_program_discovery.png["Der neue Dienst wurde erkannt."]
...
image::ds_program_host.png["Übergabe des Hostnamens mit dem Makro $HOSTNAME$."]
...
image::ds_program_discovery_host.png["Service-Erkennung findet den neuen Dienst, der nun den übergebenen Hostnamen als Information ausgibt."]
...
|$HOSTNAME$ |Der Hostname, wie er im Setup konfiguriert ist.
...
image::ds_program_error.png["Wenn ein Skript von 0 verschiedene Exit-Codes liefert, wird der Dienst sofort CRI/rot."]
...
wir dafür spezielle Regelsätze in der Setup-GUI definiert, mit denen Sie diese direkt
konfigurieren können. Diese Regeln finden Sie gruppiert nach Anwendungsfällen unter
[.guihint]#Setup > Agents > Other integrations# und [.guihint]#Setup > Agents > VM, Cloud, Container#.

Wer von älteren {CMK} Versionen umgestiegen ist, wird sich möglicherweise über
die neue Gruppierung wundern: {CMK} {v20} gruppiert nicht mehr nach
Zugriffstechnik, sondern Art des überwachten Objektes. Dies ist insbesondere
dehalb sinnvoll, weil in vielen Fällen den Anwender nicht interessiert, mit
welcher Technik Daten eingeholt werden und zudem oft Datenquellen und
xref:piggyback#[Piggyback] miteinander kombiniert werden, hier also keine klare
Abgrenzung möglich ist.

image::ds_program_rules.png["Regelsätze für die Überwachung von Anwendungen per Datenquelle und Piggyback."]
...
Der Spezialagent `agent_netapp` greift über diese Schnittstelle zu und wird über den Regelsatz [.guihint]#Check NetApp via WebAPI# als Datenquellenprogramm eingerichtet.
...
image::ds_program_netapp.png["Eingabemaske der Zugangsdaten für die Netapp-Überwachung."]
...
Überwachung von xref:monitoring_vmware[VMWare ESXi] über das vCenter. Letzteres
ist auf einer (meist virtuellen) Windows-Maschine installiert, auf welcher
sinnvollerweise auch ein {CMK}-Agent läuft.

image::ds_program_vcenter.png["Auswahlmöglichkeiten Host system vs. vCenter in der VMware ESXi Konfiguration."]
