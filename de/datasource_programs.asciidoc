...
// MFS: ich würde "verrückteste" durch "sehr kreative" ersetzen, IMHO etwas over the top… 
...
Eine häufig benötigte Spielart davon -- die Übertragung der Daten über SSH -- wird im Artikel über den
...
die Kommandozeile eines Befehls. Anstelle der Verbindung auf Port 6556 führt
...
Da Änderungen der Datenquellen in der Regel nur Transportwege betreffen, ist es wichtig,
dass Sie den Host in der Setup-GUI auf der Einstellung
[.guihint]#{CMK} Agent (Server)# belassen.
// MFS: Ist der Hinweis aus Zeile 293 so korrekt und verständlich integriert?
...
Legen Sie nun im Setup zum Test einen Host 
...
image::ds_program.png["Screenshot der Eingabemaske für einen individuellen Befehl"]
...
image::ds_program_discovery.png["Screenshot: Der neue Dienst wurde erkannt."]
...
image::ds_program_host.png["Screenshot: Übergabe des Hostnamens mit dem Makro $HOSTNAME$"]
...
image::ds_program_discovery_host.png["Screenshot: Service-Erkennung findet den neuen Dienst, der nun den übergebenen Hostnamen als Information ausgibt"]
...
image::ds_program_error.png["Screenshot: Wenn ein Skript von 0 verschiedene Exit-Codes liefert, wird der Dienst sofort CRI/rot."]
...
die neue Gruppierung wundern: {CMK} 2.0.0 gruppiert nicht mehr nach
...
image::ds_program_rules.png["Screenshot: Regelsätze für die Überwachung von Anwendungen per Datenquelle und Piggyback."]
...
eingerichtet. Wichtig ist, dass Sie den Host in der Setup-GUI auf der Einstellung
[.guihint]#{CMK} Agent (Server)# belassen.
...
image::ds_program_netapp.png["Screenshot: Eingabemaske der Zugangsdaten für die Netapp-Überwachung."]
...
image::ds_program_vcenter.png["Screenshot: Auswahlmöglichkeiten Host system vs. vCenter in der VMware ESXi Konfiguration."]

// MFS: Sieht mir etwas weird aus. Wenn was modifiziert wird, sollte es
// IMHO umbenannt werden, sonst klappt nach dem Update was nicht und $KUNDE
// denkt, wir sind schuld.
