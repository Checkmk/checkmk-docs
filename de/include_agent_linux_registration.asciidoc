...
Der Grund ist, dass ein gegenseitiges Vertrauensverhältnis fehlt – Vertrauen und Verschlüsselung ist mit {CMK} {v21} Pflicht.
Wie Sie das herstellen, zeigt dieser Abschnitt.
...
=== Den Host ins Monitoring aufnehmen
...
Der Parameter `--site-address` gibt den Namen des {CMK} Servers und der Instanz an.
Der Server-Name darf auch die IP-Adresse sein, der Instanzname (hier `mysite`) entspricht demjenigen, den Sie im URL-Pfad der WebiWeboberfläche sehen. 
...
Zunächst wird auf einem im Monitoring befindlichen Host die Registrierung stellvertretend durchgeführt.
Die JSON-Ausgabe leiten wir dabei in eine Datei um:
