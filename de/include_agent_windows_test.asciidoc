...
Da die Ausgabe etwas länger sein kann, ist `more` hier sehr praktisch, das Sie in der Ausgabe mit der Taste Q verlassen können:
...
*Hinweis:* Falls die Kommunikation zwischen Agent und {CMK} Server _noch_ unverschlüsselt ist (wie im Legacy-Pull-Modus) oder unverschlüsselt ist, erhalten Sie mit diesem Kommando statt der `16` die komplette unverschlüsselte Agentenausgabe.
