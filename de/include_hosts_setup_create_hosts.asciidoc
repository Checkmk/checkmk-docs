...
Mit den [.guihint]#Monitoring agents# entscheiden Sie, aus welchen Quellen Daten fürs Monitoring verwendet werden. Insbesondere das erste Dropdown-Menü entscheidet, welche Daten bei Vorhandensein mehrerer Quellen herangezogen und welche verworfen werden.

// MFS: FIXME, kompakter?
image::hosts_new_host_3.png[alt="Dialog mit Eigenschaften eines Hosts: Monitoring-Agenten und benutzerdefinierte Attribute."]

Im Dropdownmenü [.guihint]#{CMK} agent / API integrations# haben Sie folgende Auswahlmöglichkeiten:
...
Es kann sonst vorkommen, dass das Ausbleiben erwarteter Monitoring-Daten sonst nicht bemerkt wird.
...
Bei Hosts ohne eigener IP-Adresse genügt dies jedoch nicht, da kein Ping durchgeführt werden kann.
Sie müssen hier über eine Regel [.guihint]#Setup > HostsHost monitoring rules > Host Check Command# bestimmen, wie geprüft werden soll, dass der Host als {UP} bewertet wird. 


[#customattributes]
=== Custom attributes
 
Mit den "Custom attributes" können Sie beliebige Freitext-Felder definieren.
In den Standardeinstellungen sind hier _Labels_, _Criticality_ und _Networking segment_ vorhanden, Sie können jedoch über [.guihint]#Setup > Hosts > Custom host attributes# eigene definieren.
Dies kann ein Vorort-Kontakt sein, eine Filialnummer, Informationen zur Hardware, Inventarnummern, geographische Koordinaten…
