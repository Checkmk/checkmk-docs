...
// SP: Kann der Bearbeitungshinweis jetzt nicht raus?
...
// SP: Was ist mit "Liste der Ordner" gemeint? Ich sehe zwar die Ordner in Form von Symbolen, aber keine Liste.
...
// MFS: FIXME, kompakter, Custom attributes rausnehmen?
image::hosts_new_host_3.png[alt="Dialog mit Eigenschaften eines Hosts: Monitoring-Agenten und benutzerdefinierte Attribute."]
//SP: Ich würde die Custom attributes hier herausnehmen und in den nächsten Abschnitt auslagern. Dieser Screenshot hat als einziger auch noch einen dunklen Rahmen - ist das Absicht?
...
Da diese meist für viele Hosts gleich ist, empfiehlt es sich aber eher, diese Einstellung im Ordner zu hinterlegen.
//SP: Würde es Sinn manchen, hier setup_folder zu verlinken?
...
//SP: Ich würde den unteren Screenshot direkt hierhin setzen, da der gezeigte Dialog hiermit aufgerufen wird.
Dies können ein Vor-Ort-Kontakt, eine Filialnummer, Informationen zur Hardware, Inventarnummern, geographische Koordinaten… sein.
...
// MFS: Technisch könnte die Funktionalität auch über Piggyback oder Spezialagenten bereitgestellt werden, die "erschwerte Zuordnung" bezieht sich also eher auf die Übersichtlichkeit für Admins…
//SP: Wäre es dann nicht sinnvoll, hinzuschreiben, dass die Übersichtlichkeit für Admins gemeint ist?
...
//SP: Dies ist der einzige Abschnitt der Host-Eigenschaften ohne eigenen Screenshot.
Da Management-Boards über eine eigene IP-Adresse verfügen, würde separates Monitoring die Zuordnung erschweren.
...
* automation: der Host wurde durch den xref:wato_user#automation[Automationsbenutzer] erstellt, z.B. auf der Kommadozeile
* Network scan: der Host wurde bei einem automatischen xref:folder_network_scan[Netzwerk-Scan] gefunden
* Benutzername, z.B. cmkadmin: der Host wurde manuell durch einen Benutzer, zum Beispiel im xref:wato_hosts#create_hosts[Setup], erstellt
...
//SP: Man kann im dcd festlegen, welche Attribute hierfür übergeben und damit blockiert werden. Sollte ich das hier beschreiben? Oder verlinken? Oder lenkt das nur ab?
