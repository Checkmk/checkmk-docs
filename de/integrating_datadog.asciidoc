...
Unsere Integration von Datadog richtet sich an alle unsere Nutzer, die Datadog in Ihren Unternehmen bereits im Einsatz haben.
...
Zum Zeitpunkt, zu dem dieser Artikel entstanden ist, finden Sie die entsprechenden Schlüssel bzw. die Möglichkeit solche Schlüssel neu zu erzeugen, in Datadog über den Knopf unten links in der Ecke, auf dem Ihr Nutzername steht.
...
Kopieren Sie als Erstes die beiden zuvor erzeugten Schlüssel in die dafür vorgesehen Felder oder verwenden Sie alternativ den Passwortspeicher von {CMK}.
Prüfen Sie danach, ob der voreingetragene [.guihint]#API host# dem Ihrer Datdog-Instanz entspricht.
...
Hier stehen die sogenannten [.guihint]#Monitors# und [.guihint]#Events# zur Auswahl.
...
Gerade in großen Umgebungen empfiehlt es sich, Monitore, die Sie in {CMK} sehen möchten, in Datadog mit einem entsprechenden Tag (bspw. `monitored_by_checmk:yes`) zu versehen.
...
Da es aber auch vorkommen kann, dass Datadog Events in der Vergangenheit anlegt, sollte der Zeitraum nicht zu knapp bemessen werden.
