...
Damit die Monitore und Events, die Sie mit dieser Regel aus Datadog abholen, nur auf einem Host ankommen, *müssen* Sie den xref:create_host[zurvor erzeugten Host] zum Abschluss noch unter [.guihint]#Conditions > Explicit hosts# eintragen.
Klicken Sie anschließend auf [.guihint]#Save.# Sobald Sie für diesen Host eine Serviceerkennung durchführen, sehen Sie Ihre ersten Monitore und Events aus Datadog in {CMK}.
