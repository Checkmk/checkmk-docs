...
image::icon_aggr.png[left, width=80]
...
== Konfiguration Teil 1: Das erste Aggregat#first
...
image::bi_example.png[center, width=90%]
...
image::button_bi_aggregations.png[left, width=150]
...
image::button_bi_new_aggregation.png[left, width=150]
...
Wichtig ist jedoch, dass Sie bei [.guihint]#Rule to call}} die Einstellung auf {{Call a rule# belassen
...
image::bi_button_create.png[left, width=60]
...
== BI im Operating Teil 1: Die Statusansicht#statusgui
...
und icon:icon_availability.png]. Mit dem ersten Symbol -- ICON[icon_showbi[] -- kommen
...
icon:icon_assume_1.png] ({WARN}), ICON[icon_assume_2[] ({CRIT}) und
icon:icon_assume_3.png] ({UNKNOWN}) wieder zu ICON[icon_assume_none[] zurück.
...
image::master_control_checks_off.png[right, width=280]
...
über das Seitenleisten&shy;element [.guihint]#Master Control#. Bitte vergessen Sie
...
image::bi_tree_less_minimal.png[center, width=240]
...
== BI im Operating Teil 2: Alternative Darstellungen#displayoptions
...
=== Visualisierung von BI-Aggregaten#visualization
...
im Bereich [.guihint]#Rule Properties}} die Layouts {{Hierarchical}}, {{Radial#
...
== Konfiguration Teil 3: Variablen, Schablonen, Suche#config3
...
sollen [.guihint]#Switch 1}} bzw. {{Switch 2# heißen.
...
[.guihint]#Infrastructure}} soll es einen Knoten mit dem Namen {{Mystery Servers#
geben.
...
Beispiel können Sie darauf verzichten und stattdessen den regulären Ausdruck [.guihint]#srv-mys-(.*)#
...
== Die vordefinierte Hostaggregation#hostaggr
...
image::bi_host_tree_tree.png[center, width=40%]

image::button_aggregations.png[left, width=30%]
...
== Berechtigungen und Sichtbarkeit#permissions
...
== BI im Operating Teil 3: Wartungszeiten, Quittierung#operating
...
dem Symbol icon:icon_downtime[] -- ist: _Am betreffenden Objekt
...
ist -- genau dann wird es von {CMK} als icon:icon_downtime[] „in Wartung“ markiert. Wobei auch
...
image::icon_ack.png[left, width=40]
...
== Alarmierung, BI als Service#biasservice

=== Aktive Checks oder Datenquellenprogramm

image::icon_notifications.png[left, width=80]
...
* Der Benutzer muss ein link:wato_user.html#automation[Automationsuser] sein. Nur solch einer darf die Web-API abrufen. Der Benutzer `automation` bietet sich an, da dieser immer automatisch für solche Zwecke angelegt wird.
* Tragen Sie bei [.guihint]#Passwort}} das {{Automation secret for machine accounts# des Benutzers ein, welches Sie in der Konfigurationsmaske der Benutzereigenschaften finden.
