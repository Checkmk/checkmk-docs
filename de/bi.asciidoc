...
[{image-left}]
image::icon_aggr_240.png[width=80]
...
Wenn Sie die Aggregation jetzt mit [.guihint]#Save# speichern sind Sie fertig!
...
image::command_fake_check_results.png[]
 +
[{image-left}]
image::master_control_checks_off.png[width=42%]
 +
...
Die erste Regel ist die Regel für die Wurzel des Baums. Der Knopf
icon:button_aggregations[] oben auf der Seite bringt Sie zur Liste der Aggregationen.
Hier können Sie unter anderem sehen, wie die Regeln untereinander verschachtelt sind:

image::bi_host_tree_tree.png[width=42%]


Standardmäßig sehen Sie hier nur eine Aggregation.
Entfernen Sie in den icon:icon_edit[] Details einfach die
Checkbox bei [.guihint]#Currently disable this aggregation# und sofort bekommen Sie pro
Host eine Aggregationsansicht mit dem Titel `Host myhost123` -- zu finden
in der Seitenleiste unter [.guihint]#Views > Business Intelligence > Single-Host Problems#.
Diese sieht dann
z.B. so aus:
