...
[#aggregationfunction]
Im dritten und letzten Kasten, [.guihint]#Aggregation Function#, geben Sie an, wie der Monitoring-Status des Knotens berechnet werden soll.
...
der den Host-Status *und* die beiden Uplink-Interfaces zeigt. Diese beiden Unterknoten
...
den Services auf jeden Fall auch den Host-Status aufnehmen.
...
// ML: Ich habe das Feature mal Frozen BI genannt - vielleicht lieber ohne Namen von "eingefrorenen Aggregationen" sprechen?
// TK: Kurz im Daily besprochen: Aggregate statt BI, also: eingefrorene Aggregate (_frozen aggregations_). Ich habs bereits geändert.
...
Das Aggregat bestünde dann aus 4 Services mit Status {OK}, wäre selbst aber {WARN} oder {CRIT} -- ohne einen Hinweis auf den Grund zu liefern.
...
Soll heißen: Während die Regeln eines Aggregats dessen Status erklären, erklären eingefrorene Aggregate Status-Änderungen.
// ML: Der Soll-heißen-Satz wäre vielleicht ein Kandidat zum Löschen - ordnet imho ganz nett ein, aber verwirrt er auch ...
// TK: Nö, würd ich stehenlassen.
...
Damit wird das Aggregat beim Speichern eingefroren (natürlich auch, wenn es nicht neu ist).
// TK: Die Klammer hab ich zuerst gar nicht verstanden. Vermutung: Das bezieht sich auf das "New" in "New aggregations are frozen". Versteht man (= ich) ohne Verweis auf den Text in der GUI aber nicht.
// TK: Und warum heißt das dann "New", wenn es das aktuelle Aggregat einfriert?
// TK: Durch den Plural (aggregrations) scheint die Checkbox mehr zu tun als das aktuelle Aggregat einzufrieren.
...
Darüber gelangen Sie zu der Differenzansicht:
...
// TK: Es gibt aber keine Einfrier-Historie, oder? Nochmaliges Einfrieren überschreibt das letzte Einfrieren? Wäre einen Satz wert...
