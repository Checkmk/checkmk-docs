...
Für das Beispiel legen Sie am besten ein neues Paket an (Knopf [.guihint]#Add BI pack#), welches Sie _Mystery_ nennen.
...
Schreibung, inklusive der Groß-/Kleinbuchstaben) sowie den `NTP Time`-Service per Regex an:
...
dafür ist die Funktion [.guihint]#Modify display options#, welche Sie im Menü [.guihint]#Display#
...
4. [.guihint]#Free-Floating style#: Dynamisches Layout nach Optionen wie Anziehung, Abstände, Länge der Äste.

//ML: Hübscher mit Icons - bislang gibt's aber nur die alten, daher obige Listenvariante; später gegebenenfalls anpassen.
////
[cols="5,~"]
|===
...
Diese Idee kann man 1:1 nach BI überträgen: Im Aggregat gibt es vielleicht ein paar Hosts und
...
[.guihint]#Setup > Agents > Other integrations > Applications >	BI Aggregations# den passenden Regelsatz:
...
[.guihint]#HSetup > Services > Other services > Check State of BI Aggregation# einrichten:
