...
{related-start}
xref:views.html[Ansichten von Hosts und Services (Views)]
xref:availability.html[Verfügbarkeit (Availability)]
xref:basics_downtimes.html[Wartungszeiten]
{related-end}
...
//SP: Eventuell den "Create"-Knopf mit anführen.//
...
//SP: "Add BI rule" bzw. "Add rule" explizit erwähnen?//
...
am oberen Rand jeder Statusansicht finden. Diese öffnet einen Kasten mit Optionen. Der
...
////
SP: Müsste die letzte 1 nicht auch in einfache Hochkommata gesetzt werden?
Den Absatz finde ich insgesamt sehr komplex. Mir ist z. B. nicht klar, was mit "einfangen" gemeint ist.
////
...
////
SP: Hier ist meiner Meinung nach nicht die BI-Lernkurve als ganzes gemeint. Es handelt sich nur um einen zu lernenden Aspekt. Vielleicht: "Zugegeben, dieses Thema/dieser Teil ist anspruchsvoll/schwierig. ..."
////
...
//SP: Ist der hier neu auftauchende Begriff "child nodes" gleichbedeutend mit "Unterknoten"?//
...
Dies erledigen Sie in den icon:icon_edit[] Details
des Regelpakets.
//SP: Was soll ich im Folgenden erledigen?//
...
//SP: Hier wird nur BI-Verhalten erklärt, ja? Es wird keine Aktion/Einstellung von mir erwartet?//
...
//SP: Wo befinde ich mich hier? Der obige Screenshot enthält kein "Passwort"//
