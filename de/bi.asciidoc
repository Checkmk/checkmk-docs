...
Die Knoten einer Aggregation können sich im laufenden Betrieb schon mal ändern.
Mit dem Feature _Frozen BI_ können Sie solche Änderungen sichtbar machen. 
Hier ein Beispiel: 
...
Und genau hier setzt das Feature Frozen BI an: Sie frieren den Ist-Zustand ein und können später jederzeit per Klick aufführen lassen, was sich seitdem geändert hat, also welche Knoten hinzugekommen beziehungsweise herausgefallen sind.
Soll heißen: Während die Regeln einer Aggregation deren Status erklären, erklärt Frozen BI Status-Änderungen.
...
Aktivieren Sie die Option [.guihint]#New aggregations are frozen# in den Einstellungen von Aggregationen.

image::bi_freeze_option.png["Option zum Einfrieren einer Aggregation."]

Damit wird die Aggregation beim Speichern eingefroren (natürlich auch, wenn sie nicht neu ist).
Im Monitoring sehen Sie nun neben der Aggregation ein neues icon:icon_bi_freeze[alt="Schneeflocken-Icon"] Schneeflocken-Icon.
Darüber gelangen Sie zu der Differenzansicht: Links der eingefrorene Baum, rechts der aktuelle Baum mit hervorgehobenen Veränderungen (hier der Wegfall des Services [.guihint]#backup3)#:

.Ohne eingefrorenen Zustand bliebe der Wegfall von [.guihint]#backup3# unbemerkt
image::bi_freeze_diff.png["Differenz zwischen eingefrorenem und aktuellem Zustand einer Aggregation."]
