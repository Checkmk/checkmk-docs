// -*- coding: utf-8 -*-
// IGNORE Downloadseite Monitoringdaten Nullmailer SMB Snakeoil TBD WebDAV Benachrichtigungs ED25519 p2 B12MS B2S B4MS B8MS 15% burstable 2xlarge 4xlarge c6a xlarge
...
Egal, ob Sie bereits langjähriger Nutzer von {CMK} sind, oder erst mit der Verfügbarkeit fertiger Images im Marketplace von AWS zu uns gefunden haben, Sie finden in diesem Handbuchartikel alle Ressourcen, um das vorbereitete _Amazon Machine Image_ zum für Ihre Bedürfnisse passenden Monitoring zu vervollständigen.
...
Falls Sie Nutzer des AWS Marketplaces sind, hatten Sie schon immer die Möglichkeit, ein im Marketplace link:https://aws.amazon.com/marketplace/pp/prodview-o5bowpuwmx3ng?sr=0-10&ref_=beagle&applicationId=AWSMPContessa[vorhandenes Ubuntu-Image^] mit {CMK} zu versehen und so "Monitoring in der Cloud" einzurichten.
{CMK} {v22} geht den nächsten Schritt und bietet link:https://aws.amazon.com/marketplace/pp/prodview-gddkal2hfn7yo[vorinstallierte Images^] auf Basis von Ubuntu 22.04 _(Jammy Jellyfish)_, bei denen alle Abhängigkeiten bereits erfüllt sind.
...
Aus diesem Grund enthält das AM Image weder eine vorbereitete Site, noch Mail- oder Firewall-Konfiguration.
...
AWS unterstützt derzeit Schlüssel in den link:https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/create-key-pairs.html[Formaten ED25519 und RSA^]. 
Sie können für das erste Login auf der virtuellen Maschine ein ED25519-Schlüsselpärchen erstellen, dessen Public Key Sie beim Anlegen der VM hochladen.
...
Die Freigaben dieser Ports sind in unseren _Standard-Sicherheitsgruppen_ bereits vorbereitet.
...
|===
|Typ |CPU Cores |RAM (GB) |{CMK} Services Empfehlung
//| B2S | 2 | 4 | 3.000
...
Als Faustregel für die Kalkulation eines vollständigen Backups gilt, dass die _Round Robin Databases_ nach 10 Tagen etwas mehr als ein Drittel ihrer maximalen Größe erreicht haben, das bedeutet, nach dieser Zeit ist es sinnvoll, die Größe des gebuchten Backup-Speichers noch einmal zu überprüfen.
...
{CMK} bietet eine komfortable xref:backup#[Backup-Funktion], die Sie direkt im Setup unter [.guihint]#Setup > Maintenance > Site backup > Site backup targets# konfigurieren.
...
Neben den Credentials müssen Sie einen Ordnerpfad angeben, in dem die Zwischenspeicherung des Archivs vorgenommen wird, bevor dieses in den Objektspeicher kopiert wird.
...
Selbst wenn {CMK} Ihr erstes oder einziges AWS-Projekt sein sollte, lohnt bereits Überwachung der Leistung der Instanz, Zustand der Backup-Buckets und Höhe der anfallenden Kosten.
