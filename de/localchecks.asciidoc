...
{related-start}
xref:wato_monitoringagents[Monitoringagenten]
...
Wird ein Wert nicht benötigt, so bleibt das Feld leer bzw. wird am Ende weggelassen:
...
Sie können diese Zahl entsprechend der Anzahl der erwarteten Ausgaben anpassen:

[{shell-raw}]
----
{c-root} check_mk_agent | grep -v grep | grep -A7 "<<<local"
<<<local:sep(0)>>>
0 "My 1st service" - This static service is always OK
0 "My 2nd service" count1=42|count2=21;23;27 A service with 2 graphs
P "My 1st dynamic service" count=40;30;50 Result is computed from two threshold values
P "My 2nd dynamic service" - Result is computed with no values
P "My 3rd dynamic service" humidity=37;40:60;30:70 A service with lower and upper thresholds
P "My service" humidity=37;40:60;30:70 My service output\\nA line with details\\nAnother line with details
...
Unter Windows können Sie die Agentenausgabe in eine Textdatei umleiten, diese dann
...
[#folders_script]
=== Skript-Verzeichnisse auf dem Ziel-Host
...
[#folders_cache]
=== Cache-Verzeichnisse auf dem Ziel-Host
