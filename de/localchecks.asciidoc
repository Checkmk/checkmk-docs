...
// MA: Warum maskieren? => Weil der Befehl `echo` sie sonst interpretiert. Ich finde es wichtig, dass sie nicht maskiert werden müssen, weil sonst der localcheck etwas anders macht. Oder anders: Die Maskierung ist wichtig für die Eingabe, weil es sich um Bash handelt, aber nicht für die Ausgabe (die später ausgewertet werden soll).
...
// MA: Sehr cool. Vielleicht hab ich es schon damals vergessen, aber: Was mache ich denn, wenn ich nur untere Schwellwerte konfigurieren möchte?
...
// z.b.? Hier wären ja die beiden Regelsätze "Set cache age for plugins and local checks" und "Set execution mode for plugins and local checks" genau diejenigen, die man für das Beispiel benötigt. Vorteil: Wenn sie mal erwähnt sind, kann man in der Setup-Suche direkt nach ihnen suchen und muss nicht den Pfad gehen.
...
[{shell-raw}]
----
{c-root} check_mk_agent | grep -v grep | grep -A3 "<<<local"
<<<local:sep(0)>>>
P "My service" humidity=37;40:60;30:70 My service output\\nA line with details\\nAnother line with details
2 "My cached service" count=4 Some output of a long running script
cached(1618580356,600)
----

// MA: Ich vermute, dass die Ausgabe so richtig ist? Da sind ja zwei Services und die letzte Zeile.
...
// MA: Hier würde ich inzwischen eher auf eine PS-Session verweisen und dort auch mit etwas grep-ähnlichem arbeiten. Z.B.:
//[{shell}]
//----
//PS> "C:\Program Files (x86)\checkmk\service>check_mk_agent.exe" test | Select-String -Pattern "" -Context 0,3
//
//> <<<local:sep(0)>>>
//  P "My service" humidity=37;40:60;30:70 My service output\\nA line with details\\nAnother line with details
//  2 "My cached service" count=4 Some output of a long running script
...
//----
//
//Beim Kontext wählt man, wieviele Zeilen man vor und nach dem Match anzeigen lassen will. In dem Fall keine vorher und drei danach => 0,3
...
// MA: Ich hatte auch mal versucht das einzuführen und Mathias wünschte sich, dass man das auslassen von Ausgabezeilen nicht unkommentiert mit `...` abkürzt. Daher würde ich sowas vorschlagen, wie "Da die Ausgabe des Befehls sehr lang werden kann, ist sie auf die letzten n Zeilen gekürzt."
