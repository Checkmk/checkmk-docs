...
In fast allen {CMK} Editionen können Sie die SAML-Einstellungen direkt in {CMK} vornehmen. Nutzen Sie jedoch eine {CRE}, so führt der Weg stattdessen zum Beispiel über Active Directory Federation Services (AD FS) als Identity Provider (IDP).
Beide Varianten werden im Folgenden ausführlich beschrieben.
...
//das Apache-Modul link:https://github.com/Uninett/mod_auth_mellon[mod_auth_mellon^] sorgt als Service Provider via _Security Assertion Markup Language_ (SAML) für die Authentifizierung.
