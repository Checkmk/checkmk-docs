...
Neu hinzugekommen sind Debian 11 _Bullseye_ und Ubuntu 22.04 _Yammy_. Für beide Distributionen bauen wir auch Installationspakete von {v20}, um den empfohlenen Update-Pfad zu vereinfachen.
...
// MA: Sollte hier nicht 2.0.0p23 stehen? Bisher hat man ja noch kein Update gemacht, sondern selbiges nur vorbereitet.
+
...
// MA: Müssen? Was passiert denn, wenn ich es nicht mache? Besser: "Auf diesen Hosts registrieren Sie nach der Aktualisierung beim Monitoring-Server, um die TLS verschüsselte Übertragung zu nutzen. Das machen Sie mit dem Befehl xyz. Detailiertere Informationen finden sie in der ausfürhlichen Anleitung für xref:agent_linux#registration[Linux] oder xref:agent_windows#registration[Windows]."
// MA: Zusätzlich scheint mir das ein Kapitel für die Nachbereitungen zu sein.
...
{c-omd} mkp find
----
// MA: `mkp list` ist ja schon über die lokalen Dateien behandelt. Passt dort aus meiner Sicht besser.
...
// MA: Versteh ich nicht. Das Flag "inkompatibel" wird doch genau deshalb gesetzt, weil ich als Nutzer manuelle Anpassungen vornehmen muss?
...
// MA: Spricht etwas dagegen, direkt den richtigen Link zu nehmen? => https://checkmk.com/de/werks?search=&cmk_version[]=2.1&compatible=0
...
// MA: Da das -f-Flag auch in dem verlinkten Werk erwähnt wird, kann man den Link weglassen.
...
Durch Klick auf den Knopf icon:icon_edit[alt="Symbol zum Bearbeiten eines Listeneintrags."] vor dem [.guihint]#Master switch# können Sie den Agenten-Update komplett abschalten:
...
Beachten Sie, dass auch beim Update im verteilten Monitoring gilt: Hauptversionen dürfen nicht ausgelassen werden, so dass z.B. eine Verbindung zwischen einer {v16} und einer {v21} Instanz nicht unterstützt wird.
...
Sie sollten daher den Zeitraum eines Mischbetriebs mit verschiedenen Versionen möglichst kurz halten und nach dem Start alle Instanzen zügig aktualisieren.
...
// MA: Vielleicht eher "Entfernte Snapins aus der Sidebar"?
...
Der Service [.guihint]#Check_MK# kümmert sich nun ausschließlich um den Zustand des Agenten im
// MA: Er zeigt aus meiner Sicht "den Zustand der verwendeten Agenten im laufenden Betrieb"
laufenden Betrieb (_operational state_). Er meldet z.B. Verbindungsfehler oder fehlende Daten und zeigt die Ausführungszeit.
...
Betroffene Dienste nehmen den Zustand {UNKNOWN} an und erfordern eine Anpassung der Konfiguration.

Legacy-Plugins, welche die alte Check-API nutzen, verfügten bislang über eine implizite Cluster-Möglichkeit, bei der die Ausgabe der zugeordneten Hosts lediglich der Reihe nach ausgewertet wurde und der letzte erhaltene Einzelzustand den Gesamtzustand des Clusters bestimmte.
Da dieses Verhalten nicht verlässlich war, wurde es in {v21} entfernt
und Plugins für Dienste, die in Clustern genutzt werden, müssen den _nativen Cluster-Modus_ unterstützen.
Wir haben daher eigene Check-Plugins auf das neue Check-API portiert und dabei für den nativen Cluster-Modus angepasst.
link:https://checkmk.com/werk/12908#[Diese Liste] zeigt betroffene Plugins und das neue Standardverhalten.
Bei Verwendung dieser Plugins ist ihrerseits eine Überprüfung des verwendeten Standardverhaltens sinnvoll, in wenigen Fällen werden Anpassungen erforderlich sein.
// MA: Ich bin mir nicht sicher, ob das so stimmt. Es gibt Plugins, die einen nativen
// MA: Clustermodus haben. Dieser ist explizit hinterlegt und verhält sich auf das
// MA: Plugin angepasst, wenn der Output auf einem Cluster-Host verwendet wird. Alle
// MA: andere Plugins haben nun immer einen einfachen Cluster-Modus. Die Plugins aus
// MA: dem Werk wurden zur 2.0 mit einem nativen Clustermodus ausgestattet und wurden
// MA: mit der 2.1 auf die einfachen Cluster-Modus zurückgesetzt.

Mehr Aufmerksamkeit erfordern selbst programmierte oder aus der link:https://exchange.checkmk.com[{CMK}-Exchange] bezogene Plugins, die keine API-Integration für den nativen Cluster-Modus verwenden.
Diese Dienste nehmen nach dem Update auf {v21} den Zustand {UNKNOWN} an.
Verwenden Sie die Regel [.guihint]#Aggregation options for clustered services#, um einen der drei Cluster-Modi _worst_, _best_ oder _failover_ zu definieren.
...
Ändern Sie nach Migration aller beteiligten Sites auf {v21} die Option [.guihint]#Global settings > Notification spooler configuration > Encryption# auf die Einstellung [.guihint]#Encrypt communication with TLS#.
...
Die in der REST-API noch fehlenden Funktionen zum Zugriff auf Instanzen im verteilten Monitoring und auf Metriken und Graphen werden in {v21} Patch-Versionen nachgeliefert, so dass im Laufe der Zeit alle Funktionen zur Verfügung stehen, die auch in der Web-API genutzt werden konnten.
