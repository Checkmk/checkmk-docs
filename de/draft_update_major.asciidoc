...
Für Ubuntu 21.10 (_Impish Indri_) wurde während der Beta-Phase von {v21} die Unterstützung mit Sicherheits-Updates eingestellt.
...
[{shell}]
----
{c-root} apt install /tmp/check-mk-enterprise-2.1.0p23_0.buster_amd64.deb
----
+
...
Nach der Aktualisierung bestehender Hosts im Monitoring auf {v21} nimmt der Dienst [.guihint]#Check_MK Agent# den Status {WARN} an und zeigt die Meldung _TLS is not activated on monitored_ Host.
