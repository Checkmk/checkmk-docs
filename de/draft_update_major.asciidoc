...
Die Benutzeroberfläche (GUI) von in {CMK} wurde mit Version {v20} komplett neu gestaltet.
...
Die Web-API (auch HTTP-API genannt) wird in der nächsten {CMK}-Version (zur Zeit geplant als {v22} entfer
...
Diese veraltete Verfahren zum Aufbau eines xref:distributed_monitoring.html[verteilten Monitoring] bietet gegenüber den {CMK}-eigenen Verfahren keine Vorteile.
