...
Betroffene Dienste nehmen den Zustand {UNKN} an und erfordern Nachkonfiguration.

Legacy-Plugins, welche die alte Check-API nutzen, verfügten bislang über eine implizierte Cluster-Möglichkeit, bei der die Ausgabe der zugeordneten Hosts lediglich konkateniert wurde und der letzte erhaltene Einzelzustand den Gesamtzustand des Clusters bestimmte.
Da dieses Verhalten nicht verlässlich war, wurde es in {v21} entfernt. 
Plugins für Dienste, die in Clustern genutzt werden, müssen nun den _nativen Cluster-Modus_ unterstützen.
...
Diese Dienste nehmen nach dem Update auf {v21} den Zustand {UNKN} an.
...
Ändern Sie nach Migration aller beteiligten Sites auf {v21} [.guihint]#Global settings > Notification spooler configuration > Encryption# auf die Einstellung [.guihint]#Encrypt communication with TLS#.
Neue Verbindungen verwenden bei der Erstellung TLS-Verschlüsselung als Standard.
