...
Eines der wichtigsten Themen ist dabei das xref:distributed_monitoring#[verteilte Monitoring].
Dabei arbeiten Sie mit mehreren {CMK}-Instanzen, die zu einem großen System zusammengeschaltet sind und bei Bedarf global verteilt sein können.
...
PDF-Berichte können nur in den {CEE} erstellt werden.
...
Auch wenn {CMK} fast 2000 Check-Plugins mit ausliefert, kann es trotzdem vorkommen, dass ein konkretes Plugin fehlt.
Eine Einführung in die Entwicklung eigener Plugins erhalten Sie im xref:devel_check_plugins#[Artikel über das Schreiben von Check-Plugins.]
