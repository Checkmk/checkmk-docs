...
// TK: Ja, das generelle Vorgehen ist genau richtig - nur in diesem Fall hab ich eine Ausnahme gemacht und zuerst die Abkürzung genommen, weil REST-API in den ersten beiden Absätzen schon eingeführt war.
...
// TK: done: Tabelle statt Liste plus Hervorhebungen. Internen Merker gesetzt für ggf. erneute Umwandlung in AsciiDoc description-list.
...
// TK: done: siehe vorherigen Kommentar
...
Beachten Sie, dass die REST-API einer anderen Versionierung folgt als die {CMK}-Software. Da ein neues Main Release der API dann notwendig ist, wenn es inkompatible API-Änderungen gibt, passt das in der Regel nicht zum Release-Zyklus der {CMK}-Software.
...
Wenn Sie die API-Dokumentation über die {CMK} GUI öffnen, wird Ihnen automatisch die passende API-Dokumentation angezeigt. Im
...
// TK: Hmm, eigentlich schon - bis auf die ersten Zeilen mit den angepassten Variablen. Alles, was im Vergleich zum Beispiel-Code geändert wurde, hab ich fett formatiert - in allen 3 Beispielen. Oder geht es um was anderes?
...
.  Autorisieren: Knopf [.guihint]#Authorize# > Eingabe von `Bearer automation theautomationsecret` > Bestätigen per Knopf [.guihint]#Authorize#. Nach Abschluss Die Autorisierung bleibt so lange gültig, bis die Seite neu geladen wird.

.  Endpunkt auswählen: [.guihint]#Hosts# > [.guihint]#Create a host# > Knopf [.guihint]#Try it out# 

.  Parameterwerte eingeben: Unter [.guihint]#Request body# die Beispielwerte für `host_name` und `ipaddress` ersetzen.

.  Anfrage senden: Knopf [.guihint]#Execute#

.  Antwort überprüfen: Unter [.guihint]#Responses# wird zunächst das gesendete cURL-Kommando und die URL des Endpunkts gezeigt. Anschließend wird unter [.guihint]#Server response# die Antwort angezeigt mit HTTP-Status-Code, Titel und Details.
...
// TK: Die Fehlerausgaben sind inzwischen ordentlich durch den Wolf gedreht worden (leider nicht immer zum besseren). Ich hab mich jetzt auf zwei Beispiele beschränkt und die Beschreibung der Parameter, die (fast) immer mitgeliefert werden.
