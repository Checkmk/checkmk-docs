...
link:livestatus_references.html.html#list_operators[Operatoren für Listen].
...
|Siehe link:livestatus.html#csv[`csv` anpassen]. Das ist das Standardformat, wenn nichts angegeben wird und an das offizielle CSV-Format angelehnt.
...
== Kopfzeilen aktivieren (ColumnHeaders)#columnheader
...
== Statuscodes (ResponseHeader)#response_header
...
|Hier wird festgelegt, in welchem Zeitintervall die Historie in einer neuen Dadtei weitergeführt wird. 
...
== Livestatus über das Netzwerk nutzen#network
