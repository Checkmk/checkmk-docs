...
Ein Angreifer, dessen Host Zugriff hat, kann die Zertifikatsprüfung abschalten und so auch ohne Zertifikatsdatei beliebige LQL-Kommandos absetzen.



=== Verbindung über SSH

Wenn Zugriff auf Livestatus von außerhalb Ihres lokalen Netzwerkes erforderlich ist, mag Zugriffsschutz alleine auf Basis von IP-Adressen nicht mehr praktikabel sein.
Der einfachste Weg, Authentifizierung vorzunehmen und Zugriff zu erlangen, ist hier die Verwendung der _Secure Shell_.
