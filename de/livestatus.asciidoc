...
// IGNORE AuthUser ColumnHeaders Eingabestream Eingabestreams Einträgen ResponseHeader KeepAlive lq Nagios
...
Sie Skripte über das LAN ausführen und die Daten direkt dort erheben,
...
Des Weiteren können Sie auch den Zugriff über den `omd`-Befehl auf bestimmte IP-Adressen einschränken:
...
In {CMK} {v21} ist SSL-Verschlüsselung der Livestatus-Kommunikation standardmäßig aktiviert.
Diese und weitere Einstellungen können Sie im Setup vornehmen:
...
Livestatus nutzt ein selbst signiertes Zertifikat, welches beim Erstellen der Site generiert wird.
Es liegt in `~/etc/ssl/sites/sitename.pem`.
Damit per Kommando `openssl s_client` eine Verbindung zustande kommen kann, muss dieses Zertifikat als "CA Zertifikat" bekannt gemacht werden.
Wegen der Zugriffsrechte auf die Zertifikatsdatei empfehlen wir, den ersten Test als Site-User vorzunehmen.
Die Ausgabe haben wir hier stark gekürzt, `[...]` zeigt die Auslassungen an:

[{shell}]
----
{c-omd} *openssl s_client -CAfile etc/ssl/sites/mysite.pem -connect localhost:6557*
CONNECTED(00000003)
...
Die mit `read` beginnende Zeile zeigt an, dass Sie nun LQL-Kommandos absetzen können.
Klappt dies, können Sie Livestatus-Abfragen per Pipeline übergeben:

[{shell}]
----
{c-omd} echo -e "GET hosts\nColumns: name\n\n" | \
    openssl s_client -quiet  -CAfile ./etc/ssl/sites/mysite.pem -connect localhost:6557
Can't use SSL_get_servername
depth=1 CN = Site 'mysite' local CA
verify return:1
depth=0 CN = mysite
...
Die Ausgabe vor den vier Hostnamen schreibt `openssl` auf STDERR, Sie können diese Ausgabe durch anhängen von `2>/dev/null` unterdrücken.

==== Remote-Zugriff auf Livestatus

Wenn Sie von einem entfernten Rechner aus auf Livestatus zugreifen wollen, müssen Sie diesem das Zertifikat bekannt machen.
Aus Sicherheitsgründen sollten Sie jedoch nicht die komplette PEM-Datei zum Remote-System kopieren.
Denn diese enthält neben dem Zertifikat auch den privaten Schlüssel, mit dem Livestatus die SSL-Verbindung absichert.
Kopieren Sie die Datei `etc/ssl/sites/sitename.pem` in eine temporäre Datei und entfernen Sie dort mit dem Editor den kompletten Block, der mit `BEGIN PRIVATE KEY` eingeleitet und mit `END PRIVATE KEY` abgeschlossen wird.
Geben Sie dann diese Datei weiter.

Wenn der entfernte Host nun für den Livestatus-Zugriff freigeschaltet wurde, sind Livestatus-Abfragen per Skript möglich:

[{shell}]
----
{c-user} echo -e "GET hosts\nColumns: name\n\n" | \
    openssl s_client -quiet  -CAfile /tmp/cert.pem -connect cmkserver:6557
----

Beachten Sie, dass die Zertifikatsdatei hier keinen Zugriffsschutz bietet, sie dient lediglich der Bestätigung der Transportverschlüsselung.
Ein Angreifer, dessen Host Zugriff hat, kann die Zertifikatsprüfung abschalten und beliebige LQL-Kommandos absetzen.


=== Verbindung über SSH

Wenn Sie sich aus Ihrem lokalen Netzwerk herausbewegen, mag der Zugriffsschutz per IP-Adressen-Beschränkung angesichts dynamischer Client-IP-Adressen nicht mehr praktikabel sein.
Der einfachste Weg, Zugriff zu erlangen, ist hier die Verwendung der _Secure Shell_.

Hier haben Sie die Möglichkeit, ein Kommando zu übergeben, welches auf dem entfernten Server ausgeführt wird:
...
[{shell}]
----
{c-user} openssl s_client -CAfile /tmp/cert.pem -connect localhost:6557
----
