...
// MA: Das ist nich erst seit Checkmk 2.1.0 so, oder? Ich würde hier die Versionsnummer weglassen.
...
Damit das Kommandozeilentool `openssl s_client` das vom Livestatus-Server genutzte Zertifikat validieren kann, muss diese Datei als _Certificate Authority File_ bekannt gemacht werden.
...
Klappt dies, können Sie Livestatus-Abfragen auch per Pipeline übergeben:
// MA: Sollte hier nicht die zusätzliche Option '-quiet' noch erwähnt werden? Die wird ja erst ab hier verwendet.
...
Begeben Sie sich hierfür in die [.guihint]#Global Settings#.
Unter [.guihint]#Trusted certificate authorities for SSL# können Sie das verwendete Zertifikat der Site-CA per Copy and Paste übernehmen.
// MA: Besser den kompletten Pfad? 'Global Settings > Site management > Trusted certificate authorities for SSL'
// MA: Hier würde ich mir einen Screenshot wünschen, da es den Pfad noch einmal besser visualisiert und Missverständnisse vermeidet.
...
Beachten Sie, dass die Zertifikatsdatei hier keinen Zugriffsschutz bietet, sie dient lediglich der Validierung der Transportverschlüsselung.
// MA: Vielleicht etwas deutlicher? => Das Zertifikat bietet Transportverschlüsselung und keine Authentifizierung.
// MA: Unbefugter Zugriff erfolgt ausschließlich über Transportverschlüsselung und Einschränkung der IP-Adressen, die auf den Port zugreifen dürfen.
Ein Angreifer, dessen Host Zugriff hat, kann die Zertifikatsprüfung abschalten und so auch ohne Zertifikatsdatei beliebige LQL-Abfragen absetzen.

==== SSL im Skript

Falls Sie geskriptet via SSL auf Livestatus zugreifen wollen, verwenden Sie möglichst nicht `openssl s_client`.
Primärer Zweck dieses Tools ist der Test des Verbindungsaufbaus und das Debugging von Zertifikatsketten.
Um im Falle von Verbindungsabbrüchen zu erkennen, ob die erwartete Ausgabe vollständig war, empfehlen wir, den xref:responseheader[`ResponseHeader`] auszuwerten.
