...
*Tipp:* Eine Alternative zur Erstellung von Wartungszeiten für Hosts, die wir im xref:intro_monitor#downtimes[Kapitel über Wartungszeiten] schon beschrieben haben, ist in den kommerziellen Editionen der Regelsatz [.guihint]#Recurring downtimes for hosts.#
...
*Tipp:* Wenn Sie in der Service-Konfiguration eines Hosts einen einzelnen Service durch Klick auf icon:icon_move_to_disabled[alt="Symbol zur Deaktivierung eines Services."] deaktivieren, wird für den Host automatisch eine Regel in eben diesem Regelsatz angelegt.
...
// TK: Die Beschreibungen zu hard state/soft state in anderen Artikeln (monitoring_basics.html#max_check_attempts und availability.html#softstates) sind nicht ausführlicher. Insbesondere fehlt, ob und wie sich hard state und soft state in der GUI bemerkbar machen.
