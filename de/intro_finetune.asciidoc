...
:description: Regeln und Hostmerkmale sind wichtige und mächtige Werkzeuge, um eine ganze Reihe von Fehlalarmen in Checkmk zu vermeiden. Um diese Themen geht es im aktuellen Kapitel des Leitfadens für Checkmk Einsteiger, der kompakt durch die ersten Schritte führt, um das IT-Monitoring mit Checkmk optimal einzurichten.

{related-start}
link:intro_users.html[Benutzer einrichten]
link:wato_rules.html[Host- und Serviceparameter]
{related-end}


link:intro_monitor.html[Was zuletzt geschah]
...
`/var/ora/db01` bei 96{nbsp}% Füllgrad_ {WARN} _ und bei
98{nbsp}% _ {CRIT}_._“
...
Wenn Sie sich unsicher sind, in welcher Kategorie der Regelsatz zu finden ist, können Sie auch gleich *alle* Regeln durchsuchen, indem Sie über
[.guihint]#Setup# > [.guihint]#General# > [.guihint]#Rule search# die Seite zur Regelsuche öffnen. Wir werden diesen Weg im folgenden Kapitel gehen, in der wir die Erstellung einer Regel vorstellen.
...
//SK: Die relevante Schrift im Screenshots ist zu klein.
...
Wie sieht das mit den Regeln nun in der Praxis aus? Am besten starten Sie damit, die Regel, die Sie umsetzen wollen, in einem Satz zu formulieren, etwa in diesem: „_Auf allen produktiven Oracle-Servern werden die Tablespaces DW20 und DW30 bei 90{nbsp}% Füllgrad {WARN} und bei 95{nbsp}% {CRIT}._“
...
Alarmierung auslösen und auch in der [.guihint]#Tactical overview# noch nicht als
Problem angezeigt werden. Erst wenn der Status in drei aufeinanderfolgenden
Checks (was insgesamt dann knapp über zwei Minuten dauert) nicht {OK} ist, wird
das hartnäckige Problem gemeldet.
//SK: Hmm.... "Hartnäckiges Problem" ist eine gute Umschreibung allerdings auch etwas unklar. Wir unterschieden im Kontext dieser Regel eigentlich immer von Hard- und Soft-State. Das sollte hier zumindest erwähnt werden,
//SK: weil diese Regel häufig eingesetzt und genauso häufig falsch verstanden wird.
...
//SK: Hier gehört unbedingt noch ein Hinweis auf den weiterführenden Artikel wato_services und Abschniott Discovery Check hin: https://docs.checkmk.com/latest/de/wato_services.html#discovery_check
//SK: Evnetuell sollte man hier auch schon kurz erwähnen, dass man in großen Umgebungen natürlich nicht ständig manuell discovern muss, sondern Checkmk neue Services auch automatisch hinzufügen kann. Siehe selber Link.
...
link:intro_users.html[Hier geht es weiter]
