...
|eventconsoleevents |Listet alle Events der xref:ec#[Event Console]. Zusätzlich verfügt diese Tabelle über alle Spalten von `hosts`.
...
|Filter |Spaltenname, Operator und Objektname |Schränkt die Ausgabe auf das definierte Objekt ein. Siehe auch xref:livestatus_references#filter[Livestatus-Filter].
...
|ENABLE_EVENT_HANDLERS |link:https://assets.nagios.com/downloads/nagioscore/docs/externalcmds/cmdinfo.php?command_id=47[Die Ausführung von Alert Handlern/Event Handlern global erlauben]
|DISABLE_EVENT_HANDLERS |link:https://assets.nagios.com/downloads/nagioscore/docs/externalcmds/cmdinfo.php?command_id=48[Die Ausführung von Alert Handlern/Event Handlern global verhindern]
