...
// TK: Einstellung Host oder Hostgruppen spezifisch setzen: Ist das ein Bsp.? 
// TK: Agent Based Plugins macht es jetzt aber auch nicht gerade verständlicher, oder? Agent Based Plugins vs. Agentenplugin vs. Check-Plugin? 
...
// TK: aber die gesamte GUI-Komponente ist nicht Teil dieser API, sondern einer, die es noch nicht gibt. 
Nicht abgedeckt von der Bakery-API ist die Konfiguration des Bakery Plugins, d.h. der zugehörige Regelsatz, und die _Inhalte_ der mit dem Plugin bereitgestellten Dateien, also z.B. die Agentenplugins.
...
* Unter Linux und Solaris soll nach der Installation des Agenten per Paketmanager-Routine in das Syslog geschrieben werden, dass `my_example` installiert wurde. 
...
// TK: Offensicchtlich? Ich steig aus ;-)
...
Im allgemeinen Fall kann die Konfiguration innerhalb der Funktionen `files_function`, `scriptlets_function` und `windows_config_function` beliebig ausgewertet werden und auf die zurückgegebenen xref:artefacts[Artefakte] Einfluss nehmen, um z.B. Benutzernamen und Passwörter in die Konfiguration zu schreiben.
...
// TK: Erstmal 1000 Dank für den Umbau des Beispiels!
// TK: Vorher steht doch: "Der Dateiname muss so heißen wie das Plugin und mit `.py` Suffix enden."?
// TK: Zu dieser Unterscheidung musst Du mir noch ein paar Takte erzählen.

.my_example_plugin_bakery.py
[{python}]
----
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
