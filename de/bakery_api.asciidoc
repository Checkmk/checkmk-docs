...
Auch dafür finden Sie in der {CMK} Exchange ein ausführlich kommentiertes Beispiel: das Hello bakery! MKP, welches eng an das in diesem Artikel vorgestellte Beispielszenario angelehnt ist.
...
Die Auswertung übernimmt die Agent Bakery mit Callback-Funktionen.
...
Die Klasse `HelloBakeryConfig`, die übrigens auch in den beiden folgenden Funktionen genutzt wird, richtet einen typisierten Zugriff auf die Konfiguration ein, die über den xref:ruleset[Regelsatz] in der Setup-GUI festgelegt wird:
