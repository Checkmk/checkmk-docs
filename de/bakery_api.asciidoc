...
Zur Spezifikation der einzelnen Plugin-xref:artefacts[] stehen `enum.Enums` zur Verfügung, mit denen verschiedene Eigenschaften, meist in Form eines Arguments, angegeben werden können:
...
Zu einem Bakery Plugin muss es einen Regelsatz für das Setup geben, mit dem es über die GUI konfiguriert werden kann. 
...
Im nächsten und letzten Kapitel finden Sie die Zusammenstallung aller Verzeichnisse.
