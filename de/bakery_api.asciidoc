...
Zu Demonstrationszwecken schreiben wir einfach ins Syslog. 
...
Die Klasse `HelloBakeryConfig`, die übrigens auch in den beiden folgenden Funktionen genutzt wird, definiert das Zeitintervall für die erneute Ausführung bei zwischengespeicherter Ausgabe (`interval`) und die beiden Variablen, mit denen das Plugin konfiguriert werden kann (`user`, `content`).
...
image::bakery_api_settings.png[alt="GUI des Regelsatzes zur Konfiguration des Plugins."]
