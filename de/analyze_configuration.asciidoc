...
Geht das nicht schneller? Lässt sich das optimieren? Kann man das System sicherer machen?

Ein Ansatz zur Beantwortung dieser Fragen ist die Überprüfung und 
nachfolgende Optimierung der auf dem {CMK} Server eingerichteten xref:glossar#site[Instanz]. Je performanter Ihr 
...
Durch die Quittierung verschwindet die gelbe bzw. rote Farbe des Zustands. Der Eintrag an sich bleibt erhalten, wird aber optisch dezenter. Ist der Check qittiert, so wechselt der Knopf auf die Darstellung icon:button_unackn_test[alt="Knopf zur Rücknahme der Quittierung."]. Mit diesem Knopf heben Sie die Quittierung dann auch wieder auf.
...
.7+|Performance |Check helper usagefootnote:Cee_only[nur in den {CEE} vorhanden] .5+|xref:cmc_differences#aux_processes[{CMK} Hilfsprozesse]
|Checkmk checker countfootnote:Cee_only[]
|Checkmk checker usagefootnote:Cee_only[]
|Checkmk fetcher usagefootnote:Cee_only[]
|Checkmk helper usagefootnote:Cee_only[]
|Livestatus usage |xref:livestatus#_verbindung_aufrecht_erhalten_keepalive[Verbindung aufrecht erhalten (KeepAlive)]
|Number of users |xref:ldap#[Benutzerverwaltung mit LDAP/Active Directory]
|Reliability |Backups configured |xref:backup#[Backups]
.2+|Security |Encrypt backups |xref:backup#backup_config[Verschlüsselte Backups konfigurieren]
