...
Durch die Quittierung verschwindet die gelbe bzw. rote Farbe des Zustands. Der Eintrag an sich bleibt erhalten, wird aber optisch dezenter. Ist der Check quittiert, so wechselt der Knopf auf die Darstellung icon:button_unackn_test[alt="Knopf zur Rücknahme der Quittierung."]. Mit diesem Knopf heben Sie die Quittierung dann auch wieder auf.
...
.7+|[.guihint]#Performance# |[.guihint]#Check helper usagefootnote:Cee_only[Dieser Check ist nur in den {CEE} vorhanden.]# .5+.^|xref:cmc_differences#aux_processes[{CMK} Hilfsprozesse]
