...
// FEHLT: cmk -I verwendet implizit Cache, es sei denn es sind Hostnamen angegeben!
// TK: Hat sich erledigt: cmk -I wird nicht mehr beschrieben, sondern nur noch verlinkt nach wato_services#commandline und da steht genau das.
...
|`cmk --discover-marked-hosts` |Führt eine Service-Erkennung durch für alle Hosts, für die ein zuvor mit `cmk --check-discovery` ausgeführter Discovery Check Änderungen gefunden -- und den betroffenen Host markiert hat. Dieser Befehl wird in einer {CMK}-Instanz standardmäßig alle 5 Minuten per Cronjob durchgeführt. Die Cronjobs könenn Sie sich mit `crontab -l` anzeigen lassen.
2+|*Agenten*
...
Zum Backen der Agenten dient die Option `-A`, gefolgt vom Namen eines Hosts (oder auch von mehrereren):
