...
Denn was für den Befehl selbst gilt, stimmt auch für seine Optionen: Je weniger zu tippen ist, um so schneller.
...
|Option |Wirkung
2+|*Monitoring-Kern*
|`cmk -R` |xref:commandline[Kern neu starten]
|`cmk -O` |xref:commands_core[Neue Konfiguration in den Kern laden]
|`cmk -U` |xref:commandline[Neue Konfiguration für den Kern erstellen]
|`cmk -N` |xref:commands_core[Nagios-Konfiguration des Kerns ausgeben]
2+|*Checks*
|`cmk myserver123` |xref:execute_check[Checks ausführen] auf dem Host `myserver123`
2+|*Diagnose*
...
|`cmk -d myserver123` |xref:dump_agent[Agentenausgabe holen und anzeigen]
...
|===
|Option |Wirkung 
|`cmk -R` |Erzeugt eine neue Konfiguration für den Kern und startet diesen dann neu (analog zu `omd restart core`). Das ist die bei Nagios vorgesehende Methode.
...
[#dump_agent]
=== Agentenausgabe holen und anzeigen
