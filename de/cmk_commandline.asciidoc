...
// TK: Im folgenden die 2 Optionen --cache und --usewalk aus dem Kapitel zu "Checks ausführen" hierhin verschoben, da es sich um allgemeine Optionen handelt. 
// TK: Die Option --no-cache neu aufgenommen
// TK: Die Option --no-tcp ebenfalls verschoben, aber auskommentiert, da sie nach Kommandohilfe nur für die Service Discovery mit -I gilt. 
// TK: Test mit -I und --no-tcp: Wenn keine Cache-Datei existiert, wird trotzdem "[TCPFetcher] Execute data source" ausgeführt, d.h. der Agent wird kontakiert: anders als beschrieben.
[cols="35,~",options=header]
|===
|Option |Wirkung 
