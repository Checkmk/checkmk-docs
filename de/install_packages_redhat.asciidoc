...
// TK: Vielleicht so:
// Bei Red Hat Enterprise Linux (RHEL) und allen seinen Derivaten wie CentOS, AlmaLinux oder Rocky Linux...
// TK: Im ganzen Artikel zuerst RHEL und dann die anderen?
...
// TK: Der Befehl ist doch bei allen Red Hat&Co-Distros gleich, oder? Dann finde ich das verständlicher:
// TK: Hier ein Beispiel für die Installation der Paketquelle für die Version 8:
// TK: oder meinetwegen so 
// TK: Hier ein Beispiel für die Installation der Paketquelle für die Version 8 unter AlmaLinux 8: 
...
// TK: Welche sind denn nicht-freie Derivate? Wenns die nicht gibt, den Begriff weglassen.
Dies geht z.B. mit folgenden Befehlen:
// TK: Die beiden vorherigen Sätze kann man auch nach unten unter die Red Hat/CentOS 7.x Kommandos schieben, da es erst dann um 8.x geht.
...
// MFS: Alle Centos 8 außer Stream seit Ende 2021 EOL, das erlaubt Vereinfachungen https://access.redhat.com/support/policy/updates/errata
...
Zum Testen oder für kleinere Installationen von {CMK} können Sie auf eine der beiden link:https://checkmk.com/de/download[kostenlosen Editionen^] (die {RE}, welche Open Source ist, und die bis 750 Services gratis nutzbare {CE}) zurückgreifen.
