...
*Hinweis:* Das oben abgebildete [.guihint]#Main dashboard# mit Problemen und Statistiken aller Hosts und Services ist das der {RE}.
Die {EE} bieten standardmäßig ein anderes, mit Graphen erweitertes xref:dashboards#usage[Main dashboard.]
...
Das generelle Verhalten setzen Sie in den Einstellungen Ihres Benutzerprofils ([.guihint]#User > Edit profile > Show more / Show less > Set custom show mode#), indem Sie einen der beiden Modi als Default setzen -- oder auf beide Modi verzichten und sich der ungefilterten Komplexität stellen.
