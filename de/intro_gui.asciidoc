...
Oben rechts im Menü sehen Sie den aktuellen Benutzernamen (zurzeit also `cmkadmin`) und die diesem Benutzer zugewiesene link:wato_user.html#roles[Rolle] (`admin`). In diesem Menü können Sie das Passwort ändern, sich von der {CMK}-Oberfläche abmelden und Ihr Profil persönlich anpassen. Interessante Profileinstellungen für die Oberfläche sind u.a. die Auswahl der Sprache, die Darstellung der Symbole in der Navigationsleiste (mit oder ohne Titel), wo Symbole in den „Mega-Menüs“ angezeigt werden (bei den Themen oder bei den Einträgen), und ob {CMK} eher weniger oder mehr anzeigen soll.
...
Außer den beiden oben erwähnten Filtern `h:` und `s:` können Sie weitere nutzen. Welche das sind, und wie Sie auch reguläre Ausdrücke verwenden könnnen, erfahren Sie im link:user_interface.html#quicksearch[Artikel über die Benutzeroberfläche].
