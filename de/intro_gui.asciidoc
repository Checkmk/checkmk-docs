...
*Hinweis:* Das oben abgebildete [.guihint]#Main dashboard# ist das der {RE}. Die {EE} bieten standardmäßig ein anderes Haupt-xref:intro_tools#dashboards[Dashboard], das den Zustand der Instanz anzeigt.
...
Was Sie auf der Hauptseite sehen, hängt davon ab, wo Sie in {CMK} gerade unterwegs sind. Nach der Anmeldung sehen Sie zunächst das Standard- oder Haupt-xref:intro_tools#dashboards[Dashboard], das einen  Überblick über den aktuellen Zustand und die kürzlichen Ereignisse der überwachten Objekte zeigt:
...
Außer den beiden oben erwähnten Filtern `h:` und `s:` können Sie weitere nutzen. Welche das sind, und wie Sie auch reguläre Ausdrücke verwenden könnnen, erfahren Sie im xref:user_interface#search_monitor[Artikel über die Benutzeroberfläche].
