...
image::license_license_usage_ausschnitt.png[alt="Aktionsknopf zum Öffnen der Seite mit den Subskriptionseinstellungen."]
...
Ihre Lizenzinformationen sind für Sie jederzeit transparent einsehbar. Es werden die Angaben, die Sie auf den Seiten [guihint]#License usage# und [guihint]#Edit subscription settings# sehen sowie gegebenenfalls auf der Seite [guihint]#Submit or download license usage report# ergänzter Kommentar verwendet.

=== Manuelle Übermittlung
Sie können Ihre Lizenzinformationen ganz einfach herunterladen und manuell in das tribe29-Kundenportal hochladen.
...
Mit dem Knopf icon:icon_download[alt="Symbol für das Herunterladen der Daten."] [guihint]#Download report# starten Sie, nach der Festlegung des gewünschten Pfades, den Download des gz-Archivs (Tar-Archiv). Darin enthalten ist eine JSON-Datei, die alle Lizenzinformation, die Sie auf der Seite [guihint]#License usage# angezeigt bekommen, enthält.
// MA: Es handelt sich hier um ein tar-gz-Archiv. Tar ist der Teil, der mehrere Dateien in einer einzelnen zusammenfasst. gz (Gzip) ist der Teil, der die Daten komprimiert. Es sind also eigentlich zwei Techniken, die in einer Datei münden: DATEINAME.tar.gz - Unter diesem Gesichtspunkt wirkt "Download des gz-Archivs (Tar-Archiv)" etwas fremd.
...
Die xref:directtrans[Direktübermittlung durch {CMK}] der kundenbezogenen Informationen entspricht exakt dem oben beschriebenen Vorgehen. Lediglich bei der manuellen Übermittlung gibt es kleine Unterschiede.
...
Die xref:directtrans[Direktübermittlung durch {CMK}] der kundenbezogenen Informationen entspricht exakt dem oben beschriebenen Vorgehen. Lediglich bei der manuellen Übermittlung gibt es kleine Unterschiede.
////
// MA: Ich unterstütze den Vorschlag von Thomas.
