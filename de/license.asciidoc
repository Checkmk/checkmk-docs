...
{cee-only} Nutzen Sie die {CSE} oder die {CME}, so führen Sie mindestens einmal pro Jahr einen Lizenzaudit durch, d.h. Sie erstellen eine Übersicht über die von Ihrem Unternehmen genutzten Lizenzen und geben diese an {com} oder den Sie betreuenden Partner von {com} weiter.
...
Als Administrator von {CMK} können Sie die Lizenzinformationen dann aktiv herunterladen und danach manuell in das Kundenportal von {com} hochladen.
Alternativ können Sie diese ganz bequem direkt von {CMK} an das Kundenportal von {com} übermitteln lassen.
...
== Lizenzinformationen an {com} übermitteln
...
Sie können Ihre Lizenzinformationen direkt herunterladen und manuell in das {com}-Kundenportal hochladen.
...
*Hinweis:* Ihre Zugangsdaten für das {com}-Kundenportal müssen vor der ersten Übertragung in {CMK} hinterlegt worden sein (siehe xref:setup[Subskriptionsdaten eingeben]).
...
Ihre Lizenzinformationen werden dann direkt in Ihren Bereich des {com}-Kundenportals übertragen und Sie erhalten eine entsprechende Bestätigung.
...
Melden Sie sich danach mit Ihren Zugangsdaten im link:https://customer.checkmk.com/[{CMK} Kundenportal^] an ({com}-Partnern stehen hier zusätzliche Optionen zur Verfügung).
