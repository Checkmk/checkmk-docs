...
{cee-only} Nutzen Sie eine der {CEE} oder die {CCE} (oberhalb der Grenzen der {FE}), so sind Sie verpflichtet, regelmässig eine Übersicht über den Einsatz von {CMK} in Ihrem Unternehmen an {com} zu liefern.
...
Melden Sie sich dort mit Ihren Zugangsdaten an, um die [.guihint]#Licensing Credentials# zu erstellen, die Sie im xref:setup[nächsten Schritt] in {CMK} eintragen müssen.
...
Ab jetzt meldet {CMK} regelmässig Ihre aktuelle Lizenznutzung an die {comfull}. Nach jeder erfolgreichen Übertragung sehen Sie eine Bestätigung sowie eine Zusammenfassung der übermittelten Informationen.
...
Als Benutzer der {CEE} lassen Sie {CMK} den Großteil der Arbeit zur Übermittlung Ihrer Lizenzinformationen übernehmen. Sie müssen lediglich zum jeweiligen Ende einer Nutzungsphase einmal die Datenübertragung anstossen.
Nur falls dies in Ihrem Unternehmen nicht möglich ist, müssen Sie xref:manualtrans[die Daten manuell übermitteln].
...
Folgen Sie den Schritten, wie sie im Ablauf auf der jeweiligen Seite dargestellt werden.
