...
{cee-only} Nutzen Sie die {CSE}, die {CME} oder die {CCE} (oberhalb der Grenzen des Lizenzstatus „Free“: 750 Services, eine Instanz), so sind Sie verpflichtet, regelmäßig eine Übersicht über den Einsatz von {CMK} in Ihrem Unternehmen an die {comfull} zu liefern.
...
image::license_offline_verification.png[alt="Seite zur Offline-Verifizierung der {CE}-Lizenzinformationen.", width=85%]

.Offline-Übermittlung der Lizenzinformationen in den anderen {EE}
image::license_offline_submission.png[alt="Seite für Offline-Übermittlung der {EE}-Lizenzinformationen.", width=85%]
