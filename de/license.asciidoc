...
{cee-only} Nutzen Sie eine der {CEE}, so führen Sie mindestens einmal pro Jahr einen Lizenzaudit durch, d.h. Sie erstellen eine Übersicht über den Einsatz von {CMK} in Ihrem Unternehmen und geben diese an {com} weiter.
...
Als Administrator von {CMK} lassen Sie die Lizenznutzungsinformationen dann ganz bequem direkt von {CMK} an das Kundenportal von {com} übermitteln.
...
////
=== Ergänzende Informationen für {com}-Partner

Als Partner von {com} können Sie nicht nur Ihre eigenen Lizenzinformationen einfach und schnell an {com} melden, sondern ebenso die Lizenzinformationen Ihrer Kunden.
...
Melden Sie sich danach mit Ihren Zugangsdaten im link:https://portal.checkmk.com/de/[{CMK}-Kundenportal^] an ({com}-Partnern stehen hier zusätzliche Optionen zur Verfügung).
