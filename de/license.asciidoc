...
{cee-only} Nutzen Sie die {CEE}, so führen Sie mindestens einmal pro Jahr einen Lizenzaudit durch, d.h. Sie erstellen eine Übersicht über den Einsatz von {CMK} in Ihrem Unternehmen und geben diese an tribe29 oder den Sie betreuenden Partner von tribe29 weiter.
...
Loggen Sie sich dort mit Ihren Zugangsdaten ein, um die Lizenzinformationen zu erhalten.
Wählen Sie die passende Subskription aus. Die Ansicht sieht dann beispielsweise so aus:
...
Kopieren Sie den [.guihint]#Application Account# und das Passwort und schliessen Sie dieses Fenster mit [.guihint]#Confirm#. Kehren Sie zu {CMK} zurück und tragen Sie beide Informationen im Abschnitt [.guihint]#Application account crendentials# (siehe oben) als [.guihint]#Username# und [.guihint]#Password# ein.
...
In einer Tabelle unterhalb der Grafik, d. h. unterhalb des oben abgebildeten Ausschnitts, sind die einzelnen Nutzungen der Sites pro Tag aufgelistet.


== Lizenzinformationen an tribe29 übermitteln

Als Benutzer der {CEE} lassen Sie {CMK} den Großteil der Arbeit zur Übermittlung Ihrer Lizenzinformationen übernehmen. Nur falls dies in Ihrem Unternehmen nicht möglich ist, müssen Sie die Daten manuell übermitteln.
...
Um Ihnden die Arbeit zu vereinfachen, definieren Sie die Übermittlung der Lizenzinformationen vor.
...
Sollten Sie gezwungen sein Ihre Lizenzinformationen manuell in das tribe29-Kundenportal hochzuladen, so gehen sie dabei folgendermassen vor:
