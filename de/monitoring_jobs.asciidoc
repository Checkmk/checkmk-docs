...
sind Sie sehr wahrscheinlich bereits mit den sogenannten _Cronjobs_ vertraut.
...
weswegen diese in die Überwachung des Hosts einbezogen werden sollten.
In {CMK} können Sie das mit dem Skript `mk-job` erreichen.
// TK: Plugin-Programm durch Agentenplugin ersetzt.
// MA: Es ist kein Agentenplugin, da es nicht unter MK_LIBDIR/plugins liegt. Es muss wirklich unter /usr/bin oder /usr/local/bin liegen.
Das kleine Programm wird dem eigentlichen Job vorangestellt und führt dann diese Aufgabe aus.
...
Das Programm `mk-job` ist -- wie in {CMK} oft üblich -- ein einfaches Shell-Skript, welches Sie jederzeit überprüfen können. Sie haben also auch bei den wichtigen Jobs auf Ihrem Host jederzeit die maximale Transparenz und Kontrolle.
...
Richten Sie als erstes das kleine Programm auf dem überwachten Host ein, um es nutzen zu können.
Dabei ist es die einfachste Methode,
das Programm mit `wget` direkt von Ihrem {CMK}-Server zu holen und ausführbar zu machen,
im folgenden Beispiel für einen Linux-Server:
...
laden Sie stattdessen `mk-job.aix` oder `mk-job.solaris`.
Wenn der {CMK}-Server nicht erreichbar ist,
oder auf dem Server das Programm `wget` nicht verfügbar ist,
können Sie es natürlich auch auf einen Zwischenzpeicher ablegen
und dann zum Beispiel mit mit `scp` zu dem Host kopieren.
// TK: Wenn der Server nicht erreichbar ist, wie komme ich dann mit scp ran?
// MA: Vielleicht so? Wenn der CMK-Server nicht direkt erreichbar ist, muss man es zuerst auf einen Rechner kopieren, der an den Host rankommt. Das könnte auch der eigene Rechner sein. Und dann geht es mit WinSCP oder scp weiter.
...
und muss daher auf diesem Host einzigartig sein:

.crontab
[source,subs=macros]
----
# Syntax:
# 5 0 * * * pass:c[<Benutzer> mk-job <Service-Name> <Kommando>]
...
Da das Verzeichnis `job` ebenfalls dem Benutzer `root` gehört,
ist es für `mk-job` auch kein Problem, das Benutzerverzeichnis `root` anzulegen, sollte es nicht vorhanden sein.

Bei jedem Aufruf wird der Agent in den Verzeichnissen unterhalb von `/var/lib/check_mk_agent/job/` schauen,
welche Daten dort liegen und diese der Ausgabe hinzufügen.

So ein Ergebnis könnte also etwa wie das folgende aussehen, wobei der Übersicht halber hier nur der relevante der Teil der Agentenausgabe gezeigt wird:
...
In {CMK} werden Sie den Service automatisch finden, sobald der Job einmal ausgeführt und entsprechend die Ergebnisse gespeichert wurden. Gehen Sie also wie üblich in die Service-Erkennung und aktivieren Sie den Service:
...
Dadurch ist es `mk-job` überhaupt erst möglich,
im Namen des Benutzers die Ergebnisse in dieses Verzeichnis abzulegen.
...
um dem Problem oder den Problemen auf die Spur zu kommen.
...
wie in den xref:first_steps[ersten Schritten] beschrieben.
Mögliche Fehlerquellen sind:

* `mk-job` kann von `cron` nicht gefunden werden, weil es in einem Pfad liegt,
...
ob die Ergebnisse auch vom Agenten korrekt weitergegeben werden.

Sie können sich die Agentenausgabe mit dem folgenden xref:cmk_commandline#utilities[Kommando] anzeigen und die Ausgabe in das Kommando `less` weiterleiten:
...
Sind die Ergebnisse hier nicht zu sehen, kann das daran liegen,
dass der Agent nicht über die nötigen Berechtigungen verfügt,
um die entsprechenden Dateien zu lesen.
Das kann zum Beispiel auftreten, wenn Sie den Agenten nicht mit dem Benutzer `root` aufrufen,
die Ergebnisse aber nicht von anderen Benutzern gelesen werden dürfen:
...
Der Besitzer wird nicht geändert, da dieser unangetastet bleibt,
wenn vor dem Doppelpunkt nichts eingetragen wird:
...
// TK: Wie korrigiere ich das denn? Steht das irgendwo?
// MA: Vielleicht so? Ist dann jetzt sehr ausführlich geworden. Aber dafür auch weniger voraussetzungsreich.
...
|`/usr/local/bin/mk-job` | Das Programm `mk-job` sollte hier abgelegt werden, damit es unkompliziert aufgerufen werden kann.
|`/var/lib/check_mk/job/` |Das übliche Verzeichnis unter dem die Ergebnisse geordnet nach Benutzern gespeichert werden. Beachten Sie, dass der Pfad unter *AIX* anders ist: `/tmp/check_mk/job/`
|===
