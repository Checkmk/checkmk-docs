...
// TK: Server durch Host ersetzt, auch im folgenden, wenn es um den Rechner geht, wo mk-job ausgeführt wird.
In {CMK} können Sie das mit dem Agentenplugin `mk-job` erreichen.
...
Wenn der {CMK}-Server nicht erreichbar ist, oder auf dem Server das Programm `wget` nicht verfügbar ist, können Sie es natürlich auch mit `scp` kopieren.
...
// TK: geändert zu "ist es für `mk-job` kein Problem", damit klarer wird, dass man das Verzeichnis nicht manuell erstellen muss.
...
// TK: Was bedeutet "nun" hier genau? Schaut der Agent nicht immer in dieses Verzeichnis?
...
Korrigieren Sie das gegebenenfalls.
...
// TK: Hier soll wohl /usr/local/bin stehen: siehe die Bsp-Kommandos oben
