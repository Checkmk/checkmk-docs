...
Wenn das Programm `wget` nicht verfügbar ist,
können Sie die Datei natürlich auch auf anderem Weg,
zum Beispiel mit `scp`, kopieren.
...
|`/usr/local/bin/` | Das Programm `mk-job` sollte in diesem Verzeichnis abgelegt werden, damit es unkompliziert aufgerufen werden kann.
