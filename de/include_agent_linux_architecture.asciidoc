...
Der *Agent Receiver* `agent-receiver` ist eine neue Komponente des {CMK} Servers und der generelle Endpunkt für die Kommunikation des Agent Controller, z.B. für die Registrierung der Verbindung.
