...
* wird mit beschränkten Rechten unter der User ID `cmk-agent` ausgeführt, die während der Installation des Agentenpakets angelegt wird,
...
Alle neuen Features werden über den Agent Controller und den Agent Receiver eingerichtet/realisiert, d.h. {CMK}-Server und Agent müssen V2.1 haben.
...
Die Registrierung beinhalt bereits die TLS-Verschlüsselung der Kommunikation.
