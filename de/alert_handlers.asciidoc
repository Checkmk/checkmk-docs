= Alerthandler
:revdate: 2017-02-13
[.seealso][#notifications] [#commands] [#wato_monitoringagents]MT:Auf Probleme automatisiert reagieren
MD:Manche Probleme tauchen nur selten für kurze Zeit auf. Checkmk kann dann Skripte ausführen, um ad hoc Informationen für eine weitere Analyse zu sammeln.
...
[CEE] Alerthandler sind von Ihnen selbst geschriebene Skripten, die (CMK)
...
* Alerthandler sind erst wirklich nützlich, wenn Sie auf dem Zielhost ein Kommando ausführen. Für Linux bietet (CMK) eine fertige Lösung, die [alert_handlers#linux_remote|weiter unten] erklärt wird.
...
* Der Alerthandler `linux_remote` mit dem Titel [.guihints]#„Linux via SSH“# auf dem (CMK)-Server.
