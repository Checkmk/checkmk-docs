= Alerthandler
:revdate: 2017-02-13
[.seealso][#notifications] [#commands] [#wato_monitoringagents]:title: Auf Probleme automatisiert reagieren
...
Man möchte meinen, es sei offensichtlich, dass ein Moni&shy;toringsystem
...
=== Alerthandler in Checkmk

[CEE] Alerthandler sind von Ihnen selbst geschriebene Skripten, die Checkmk
...
Alerthandler sind sehr ähnlich zu [notifications|Alarmierungen] und werden
auch ähnlich konfiguriert, aber es gibt ein paar wichtige Unterschiede:

* Alerthandler sind unabhängig von [basics_downtimes|Wartungszeiten], Alarmierungsperioden, [basics_ackn|Quittierungen] und ähnlichen Einschränkungen.
...
[CRE] Noch ein Hinweis für Benutzer der (CRE): Auch Sie können
...
Hinweise dazu finden Sie durch einfaches <a href="https://www.google.de/search?q=nagios+event+handlers">Googeln</a>.
...
Alerthandler sind Skripten, die auf dem Checkmk-Server
...
Wie bei den [notifications|Alarmierungen] bekommt das Skript alle Informationen
über den Host oder Service als Umge&shy;bungs&shy;variablen, wobei diese hier alle
mit dem Präfix `ALERT_` beginnen.
Um zu testen, welche Umgebungsvariablen genau beim Skript ankommen,
können Sie folgenden Alert&shy;handler zum Testen verwenden:
...
Das Scharfschalten des Handlers geht über das WATO-Modul ICON[icon_alert_handlers.png] [.guihints]#Alert handlers}}.# 
Gehen Sie wie folgt vor:

. Legen das obige Skript unter `~/local/share/check_mk/alert_handlers/debug` an.
. Machen Sie es mit `chmod +x debug` ausführbar.
. Rufen Sie das WATO-Modul ICON[icon_alert_handlers.png] [.guihints]#Alert handlers# auf.
. Legen Sie dort mit ICON[button_new_rule.png] eine neue Regel an.
...
({{Call with the following paramaters}}).# 
...
Zum Testen setzen Sie z.B. einen Service mit [.guihints]#Fake check results# von Hand
...
* Alerthandler sind erst wirklich nützlich, wenn Sie auf dem Zielhost ein Kommando ausführen. Für Linux bietet Checkmk eine fertige Lösung, die [alert_handlers#linux_remote|weiter unten] erklärt wird.
...
analog zu den [notifications|Alarmierungen], nur etwas vereinfacht.
...
notwendig, dass per Regel [.guihints]#Maximum number of check attempts for service}}# 
...
Noch ein Hinweis: Anders als an anderen Stellen von Checkmk, wird bei den
...
Hilfsprozess -- der Alerthelper --  welcher vom [cmc|CMC] automatisch gestartet wird. Um
...
oder Services eine Alert&shy;benachrichtigung mit allen Informationen zu dem
...
Ab Version VERSION[1.4.0i1] verhindert Checkmk die gleichzeitige
...
[alert_handlers#notification|unten]).
...
Das [.guihints]#Logging of the alert processing in the core# bezieht sich auf die Phase,
bis der CMC eine Nachricht an den Alerthelper schickt (`cmc.log`). Das
[.guihints]#Alert handler log level# hingegen beeinflusst das Logging im Alerthelper
(`alerts.log`).
...
Mit einem Klick in der [.guihints]#Master control# können Sie Alert&shy;handler global deaktivieren.
...
Neu ist auch die globale Ansicht [.guihints]#Other => Alert handler executions}},# welche alle Ausführungen
...
Ereignis ist, das zu einer [notifications|Alarmierung] führt. So können
...
Emailalarmierung von Checkmk wurde so angepasst, dass Alerthandleralarme,
...
[.guihints]#Alert handlers => Typesof events that are being processed => Allcheck executions!}}# 
...
und Aufrufen von Skripten sind sehr CPU-lastige Operationen. Checkmk kann
...
Folgendes funktio&shy;nierendes Beispiel zeigt die Struktur eine Inlinehandlers:
...
mit Variablen wie `"HOSTNAME"`, `"SERVICEOUTPUT"` usw. Dies entspricht den Umgebungs&shy;variablen,
...
* Der Alerthelper von Checkmk ruft Ihre Funktionen *synchron* auf. Achten Sie darauf, dass keine Wartezustände entstehen!
...
* Das Ganze kann komplett mit der [wato_monitoringagents#bakery|Agent Bakery] aufgesetzt werden.

Die [.guihints]#Linux Remote Alert Handler# bestehen aus folgenden Einzelteilen:

* Der Alerthandler `linux_remote` mit dem Titel [.guihints]#„Linux via SSH“# auf dem Checkmk-Server.
...
* Regeln in [.guihints]#Monitoring agents => Rules => LinuxAgent => Installremote alert handlers (Linux)}},# welche SSH-Schlüssel generieren.
...
soll. Da Sie mit der [wato_monitoringagents#bakery|Agent Bakery] arbeiten, legen Sie das
...
[.guihints]#Monitoring Agents => Installremote alert handlers (Linux)# an. In den Eigenschaften
sehen Sie den Remotehandler [.guihints]#Restart FOO service}},# den Sie gerade angelegt haben.
...
automatisch ein SSH-Schlüssel&shy;paar für den Aufruf des Handlers „ausgewürfelt“,
...
wird vom Checkmk-Server später benötigt, um das so installierte Skript ohne Eingabe
...
Backen Sie nun neue Agenten mit ICON[button_bake_agents.png]. In der Liste
...
einer entsprechenden Regel. Wählen Sie als Handler dieses Mal [.guihints]#Linux via SSH# aus,
...
zurückgemeldet wurde. Und Checkmk hat eine Alarmierung ausgelöst, da
...
[alert_handlers#files_agent|hier].
...
=== Dateien und Verzeichnisse auf dem Checkmk-Server
...
|`var/log/alerts.log`
|Logdatei mit allen Ereignissen rund um die Alerthandler (vom Alerthelper geschrieben).


|`var/log/cmc.log`
|Logdatei des Cores. Auch hier landen teilweise Informationen zu Alerthandlern.


|`local/share/check_mk/alert_handlers/`
|Legen Sie hier Ihre selbst geschriebenen Alerthandler ab.
...
|Remote-Alerthandler, die auf Linux-Systemen ausgeführt werden sollen.
