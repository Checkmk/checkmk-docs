...
Neu ab {v14}i1]: Wenn Sie im Skript in der zweiten Zeile
...
{v14}i1] ist das so geregelt, dass nach Ablauf des Timeouts
...
Ab Version {v14}i1] verhindert {CMK} die gleichzeitige
...
Bis zur Version {v128} werden Ausgabe und Exitcode der
Alerthandler lediglich in `alerts.log` protokolliert. Ab Version
{v14}i1] werden diese Informationen sauber ausgewertet,
...
Ab Version {v14}i1] erzeugen Alerthandler Einträge in
...
Neu in Version {v14}i1] ist auch, dass das Ausführen eines
...
Version {v14}i1] bringt einen Alerthandler mit,
