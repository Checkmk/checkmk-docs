...
Alert Handler sind sehr ähnlich zu link:notifications.html[Alarmierungen] und werden
auch ähnlich konfiguriert, aber es gibt ein paar wichtige Unterschiede:

* Alert Handler sind unabhängig von link:basics_downtimes.html[Wartungszeiten], Alarmierungsperioden, link:basics_ackn.html[Quittierungen] und ähnlichen Einschränkungen.
...
Bei der Auswahl des Alert Handlers können Sie ihm Aufrufargumente mitgeben
...
des Alert Handlers und zeigt dazu jeweils die in der zweiten Zeile
der Skripten hinterlegten Titel an:

image::alert_handler_arguments.png[]

Nach dem Speichern der Regel ist der Alert Handler sofort wirksam und wird
...
* Alert Handler sind erst wirklich nützlich, wenn Sie auf dem Zielhost ein Kommando ausführen. Für Linux bietet {CMK} eine fertige Lösung, die link:alert_handlers.html#linux_remote[weiter unten] erklärt wird.
...
Mit einem Klick in der [.guihint]#Master control# können Sie Alert Handler global deaktivieren.
...
von Alert Handlern global darstellt.


[#notification]
== Alarmierung über Alert Handler

Neu in Version {v14} ist auch, dass das Ausführen eines
Alert Handlers -- genauer gesagt die _Beendigung_ der Ausführung -- ein
...
E-Mailalarmierung von {CMK} wurde so angepasst, dass Alert Handler-Alarme,
...
|var/log/alerts.log |Logdatei mit allen Ereignissen rund um die Alert Handler (vom Alerthelper geschrieben).
