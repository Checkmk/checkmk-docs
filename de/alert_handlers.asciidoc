...
{cee-only}
Alert Handler sind von Ihnen selbst geschriebene Skripte, die {CMK}
...
* Alert Handler werden auch schon beim ersten Wiederholversuch aufgerufen (falls Sie mehrere Check-Versuche konfiguriert haben).
* Alert Handler sind unabhängig von Benutzern und Kontaktgruppen.
* Alert Handler gibt es nur in den kommerziellen Editionen.
...
Alert Handler sind Skripte, die auf dem {CMK}-Server
ausgeführt werden. Diese müssen im Verzeichnis
`~/local/share/check_mk/alert_handlers/` liegen und können in jeder
von Linux unterstützten Sprache geschrieben sein, z.B. in BASH, Python oder Perl.
Vergessen Sie nicht, die Skripte ausführbar zu machen (`chmod +x ...`).
...
. Legen Sie das obige Skript unter `~/local/share/check_mk/alert_handlers/debug` an.
...
Ereignisse Alert Handler auslösen sollen. Das Ganze funktioniert analog zu den xref:notifications#[Benachrichtigungen], nur etwas vereinfacht.
...
3 eingestellt werden. Denn wenn der Handler direkt nach dem zweiten Versuch
...
Herunterfahren des Monitoring-Servers), werden alle noch laufenden Alert Helper
...
Sekunden. Nach Ablauf dieser Zeit wird der Handler beendet. Seit Version
...
Seit Version {v14} verhindert {CMK} die gleichzeitige
...
=== Exit-Status und Ausgabe von Handlern
...
Seit Version {v14} erzeugen Alert Handler Einträge in
der Monitoring-Historie. Damit haben Sie eine bessere Nachvollziehbarkeit,
...
Monitoring-Plugins. Das heißt, sie sollen eine Zeile Text ausgeben und als Exitcode
...
werden zudem alle Ausführungen von Alert Handlern zusammen dargestellt.
...
== Handler bei jeder Check-Ausführung
...
Problemen). Einfache Check-Ausführungen ohne Zustandsänderung lösen keinen
Alert Handler aus.
...
spielend 1000 Checks pro Sekunde durchführen -- aber 1000 Alert Handler-Skripte
...
Sie einfach im gleichen Verzeichnis ab wie die normalen Handlerskripte.
Folgendes funktionierendes Beispiel zeigt die Struktur eines Inlinehandlers:
...
jeder Check-Ausführung aufgerufen und bekommt im Argument `context` ein Python-Dictionary
...
und `omd_site` sind gesetzt auf das Home-Verzeichnis bzw. den Namen
der {CMK}-Instanz.

Die Funktionen `handle_init()` und `handle_shutdown()` werden
von {CMK} beim Starten bzw. Stoppen des Monitoring-Kerns aufgerufen und
...
{CMK}-Agent installiert den SSH-Schlüssel nur auf Systemen, auf denen dieser
...
Setzen Sie auch eine sinnvolle *Bedingung* in der Regel, denn
...
mit dem Host auf. Um den aufwendigen manuellen Austausch des Host Key zu vermeiden, wird
...
Regel werden die SSH-Schlüssel für den Zugriff generiert und auch vom
Alert Handler verwendet! Den Public Key für die Installation in `authorized_keys`
...
|`~/var/log/alerts.log` |Log-Datei mit allen Ereignissen rund um die Alert Handler (vom Alert Helper geschrieben).
|`~/var/log/cmc.log` |Log-Datei des Cores. Auch hier landen teilweise Informationen zu Alert Handlern.
|`~/local/share/check_mk/alert_handlers/` |Legen Sie hier Ihre selbst geschriebenen Alert Handler ab.
|`~/var/check_mk/core/history` |Log-Datei der Monitoring-Historie, wird vom Core geschrieben und auch ausgewertet.
|`~/local/share/check_mk/agents/linux/alert_handlers/` |Remote Alert Handler, die auf Linux-Systemen ausgeführt werden sollen.
|===

[#files_agent]
=== Dateien und Verzeichnisse auf dem überwachten Linux-System
