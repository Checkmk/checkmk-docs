...
[.relateddoc]
notifications, commands, wato_monitoringagents

== Einleitung

=== Soll das Monitoring eingreifen?

image::master_control_alert_handlers.png[left, right, width=280]
...
* Alerthandler sind unabhängig von [basics_downtimes|Wartungszeiten], Alarmierungsperioden, link:basics_ackn.html[Quittierungen] und ähnlichen Einschränkungen.
...
Das Scharfschalten des Handlers geht über das WATO-Modul ICON[icon_alert_handlers.png] [.guihint]#Alert handlers#.
Gehen Sie wie folgt vor:

. Legen das obige Skript unter `~/local/share/check_mk/alert_handlers/debug` an.
. Machen Sie es mit `chmod +x debug` ausführbar.
. Rufen Sie das WATO-Modul ICON[icon_alert_handlers.png] [.guihint]#Alert handlers# auf.
...
== Wie Alerthandler ausgeführt werden#execution
...
image::alert_handlers_options.png[center, width=580]
...
image::master_control_alert_handlers_off.png[left, width=280]

Mit einem Klick in der [.guihint]#Master control# können Sie Alert&shy;handler global deaktivieren.
...
== Alarmierung über Alerthandler#notification
...
== Remoteausführung unter Linux#linux_remote
...
== Relevante Dateien und Verzeichnisse#files
