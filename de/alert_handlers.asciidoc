...
Alert Handler sind sehr ähnlich zu xref:notifications[Alarmierungen] und werden
auch ähnlich konfiguriert, aber es gibt ein paar wichtige Unterschiede:

* Alert Handler sind unabhängig von xref:basics_downtimes[Wartungszeiten], Alarmierungsperioden, xref:basics_ackn[Quittierungen] und ähnlichen Einschränkungen.
...
Wie bei den xref:notifications[Alarmierungen] bekommt das Skript alle Informationen
...
analog zu den xref:notifications[Alarmierungen], nur etwas vereinfacht.
...
Hilfsprozess -- der Alerthelper --  welcher vom xref:cmc[CMC] automatisch gestartet wird. Um
...
Das [.guihint]#Alert handler log level# hingegen beeinflusst das Logging im Alerthelper
(`alerts.log`).
...
Ereignis, das zu einer xref:notifications[Alarmierung] führt. So können
...
xref:alert_handlers#files_agent[hier].
