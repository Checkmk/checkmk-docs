...
[.relateddoc]
monitoring_basics,  wato_hosts,  wato_rules
...
* über das Symbol ICON[icon_services.png] in der Liste der Hosts in einem Ordner in WATO
* über den Eintrag ICON[icon_services.png] [.guihint]#Edit services}} im Kontextmenü des Services link:wato_services.html#discovery_check[{{Check_MK Discovery#] eines Hosts
...
Der übliche Weg ist nun einfach das Hinzufügen der Services über ICON[button_monitor.png]. Danach ein
...
Ein Klick auf ICON[button_monitor.png] fügt abermals alle fehlenden Services hinzu, so dass
...
und ebenfalls mit ICON[button_monitor.png] speichern.
...
erscheinenden Knopf ICON[button_fix_missing_vanished.png]. *Achtung:* Der Grund für das Verschwinden
...
die Häkchen in den Checkboxen und deaktivieren Sie sie mit ICON[button_disable.png]
oder setzten Sie sie mit ICON[button_undecided.png] auf den Status _Unentschieden/Neu._
...
. Mit der link:wato_hosts.html#search[Hostsuche] Hosts suchen und beim Suchergebnis ICON[button_discovery.png] drücken
...
|Hosts, bei denen eine frühere Serviceerkennung per Bulkoperation fehlgeschlagen ist (z.B. weil der Host zu dem Zeitpunkt nicht erreichbar war), werden von WATO mit dem Symbol ICON[icon_inventory_failed.png] markiert. Diese Option erlaubt, die Erkennung nur genau für diese Hosts zu wiederholen.
...
eines Hosts über das Symbol ICON[icon_check_parameters.png]. Wenn Sie die Parameter
...
ICON[icon_rulesets.png] [.guihint]#Host & Services parameters|Parameters for discovered services|Discovery - automatic service detection#.
...
dafür ist das WATO-Modul ICON[icon_static_checks.png] [.guihint]#Manual Checks#.
...
Die meisten Regelsätze im Modul ICON[icon_static_checks.png] [.guihint]#Manual Checks# werden Sie nie
...
des Discovery Checks über den Eintrag ICON[icon_services.png] [.guihint]#Edit services#.
