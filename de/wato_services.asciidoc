...
Die Services sind das eigentliche Fleisch im Monitoring-System. Jeder Einzelne
...
* über den Eintrag icon:icon_services[] [.guihint]#Edit services# im Kontextmenü des Services xref:wato_services#discovery_check[[.guihint]#Check_MK Discovery#] eines Hosts
...
sich die Arbeit mit xref:hosts_setup#bulk_operations[Bulk-Aktionen] von
...
drücken.
. Mit der xref:hosts_setup#search[Host-Suche] Hosts suchen, alle gefundenen mit
...
Bei der dritten Variante können Sie die Service-Erkennung auch rekursiv in allen
...
Die Besonderheit hier: Wenn ein geclusterter Service von einem Node auf den anderen wechselt, könnt er kurzzeitig als verschwunden angesehen und entsprechend ungewollt gelöscht werden.
Verzichten Sie wiederum auf diese Option, würden wiederum tatsächlich verschwundene Services niemals gelöscht.
...
////
ML: Das Senden von Befehlen per echo in eine "Kommando-Pipe" ist imho dermaßen unüblich, dass man da vielleicht nen Link setzen sollte, oder?
...
und xref:alert_handlers#[Alert Handlers] nutzen, stehen sie als
