...
:revdate: 2020-08-27
:title: Elemente eines Hosts erkennen und überwachen
...
[.border]
image::services_illu.png[left, width=45%]
...
|`df`
|`/var`</tr> <tr><td class=tt>lnxsrv015`<td class=tt>cpu.util`
...
== Services eines Hosts in WATO#discovery
...
* über die Knöpfe icon:button_save_and_services.png] und ICON[button_host_services[] in den Details eines Hosts in WATO
...
=== Fehlende Services hinzufügen#available
...
die so abge&shy;wählten Services werden von {CMK} dann wieder als [.guihint]#missing# angemahnt.
...
== Serviceerkennung für viele Hosts gleichzeitig#bulk_discovery
...
|Hosts, bei denen eine frühere Serviceerkennung per Bulkoperation fehlgeschlagen ist (z.B. weil der Host zu dem Zeitpunkt nicht erreichbar war), werden von WATO mit dem Symbol icon:icon_inventory_failed[] markiert. Diese Option erlaubt, die Erkennung nur genau für diese Hosts zu wiederholen.
...
== Checkparameter von Services#parameters
...
=== Überwachung von Prozessen#processes
...
ein Service erzeugt
([.guihint]#Grab user from found processes#).
...
Weitere Details zur Prozesserkennung finden Sie in der icon:icon_help[] Onlinehilfe
dieses Regelsatzes.
...
=== Überwachung von Switchports#switches
...
== Services manuell einrichten#manual_checks
...
== Der Discovery Check#discovery_check
...
=== Automatisches Prüfen auf nicht überwachte Services#discovery_auto
...
== Passive Services#passive_checks
...
{c-start}
{OM}echo "[$(date +%s)] PROCESS_SERVICE_CHECK_RESULT;myhost;BAR;2;Something bad happened" > ~/tmp/run/nagios.cmd
{c-end}
...
== Serviceerkennung auf der Kommandozeile#commandline
...
[notifications|Alarmierungen] und link:alert_handlers.html[Alerthandler] zu schalten.
...
Stellen: [views|Ansichten,] NagVis-Karten, link:notifications.html[Alarmierungen]
und link:alert_handlers.html[Alerthandler.] Bei neuen Ansichten ist hier wichtig,
...
== Mehr über Check-Plugins#checkplugins
