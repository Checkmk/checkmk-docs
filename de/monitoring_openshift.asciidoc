...
Die Überwachung von OpenShift-Cluster ist nur mit der {CCE} möglich.

== Einleitung

{CMK} unterstützt Sie bei der Überwachung Ihrer OpenShift-Cluster. Ab Version {v22} können Sie mit der {CCE} die folgenden Objekte überwachen:
...
Da in OpenShift-Clustern standardmäßig bereits Prometheus installiert ist, kann der {CMK} genau diese Prometheus-Instanz innerhalb Ihrer OpenShift-Umgebung abfragen und die so gewonnenen Daten für Sie in gewohnter {CMK}-Manier aufbereiten.
...
Den Service-Account mit der zugehörigen Rolle und der sogenannten RoleBinding, können Sie durch die Angabe einer von uns vorgefertigten und link:https://github.com/Checkmk/checkmk_kube_agent/blob/main/deploy/kubernetes/checkmk-serviceaccount.yaml[auf GitHub veröffentlichten Datei] vornehmen.
Prüfen Sie deren Inhalt und führen Sie anschließend den folgenden Befehl aus:

[{shell-raw}]
----
{c-user} oc apply -f https://raw.githubusercontent.com/Checkmk/checkmk_kube_agent/main/deploy/kubernetes/checkmk-serviceaccount.yaml
...
Diese Adresse inklusive des angegebenen Ports gehört in der Kubernetes-Regel später in das Feld [.guihint]#API server connection > Endpoint#.
...
Dem String `prometheus-k8s-openshift-monitoring.apps.myopenshift.example.com` müssten Sie dann im Feld [.guihint]#Prometheus API endpoint# nur noch das Protokoll voranstellen.
...
Lassen Sie die Shell mit diesen Informationen geöffnet oder kopieren Sie den Token an einen Ort, an dem Sie während der folgenden Einrichtung in {CMK} zugreifen können.
...
Ändern Sie in diesem Fall die Zahl in den eckigen Klammern hinter `.secrets` in eine 1:
...
Lassen Sie die Shell mit diesen Informationen geöffnet oder kopieren Sie das Zertifikat - inklusive der Zeilen `BEGIN CERTIFICATE` und `END CERTIFICATE` - an einen Ort, an dem Sie während der folgenden Einrichtung in {CMK} zugreifen können.

Sollte die Ausgabe leer sein, gilt der gleiche Hinweis, wie unter xref:get_token[Token auslesen].
...
Das xref:get_token[Passwort (Token) des Service-Accounts] können Sie am besten im Passwortspeicher von {CMK} hinterlegen.
...
Fügen Sie das Passwort in den {CMK}-Passwortspeicher ein mit [.guihint]#Setup > General > Passwords > Add password# z.B. unter der ID und dem Titel `My OpenShift Token`:
...
In einer OpenShift-Umgebung, in der überwachbare und überwachte Objekte kontinuierlich kommen und gehen, empfiehlt es sich auch die Option [.guihint]#Automatically delete hosts without piggyback data# zu aktivieren.
...
Tragen Sie hier die Adresse ein, welche Sie im Punkt xref:get_kubernetes_endpoint[Kubernetes-API-Endpunkt auslesen] ermittelt haben.
...
Aktivieren Sie die Option [.guihint]#Enrich with usage data#, wählen Sie im folgenden Dropdown-Menü [.guihint]#Use data from OpenShift (Cloud Edition)# und tragen Sie den [.guihint]#Prometheus API endpoint# ein.
...
{CMK} erzeugt Labels für Objekte wie Cluster, Deployments oder Namespace während der Service-Erkennung automatisch.
...
Die {CCE} wird mit sechs eingebauten Dashboards für Kubernetes ausgeliefert.
...
[{shell-raw}]
----
{c-user} oc delete -f https://raw.githubusercontent.com/Checkmk/checkmk_kube_agent/main/deploy/kubernetes/checkmk-serviceaccount.yaml
