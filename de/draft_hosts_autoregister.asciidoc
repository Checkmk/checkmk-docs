...
Da die automatische Host-Erstellung vor allem für die Cloud-Umgebungen benötigt wird, ist die Autoregistrierung nur in der {CPE} verfügbar.
...
Bei der Konfiguration des Ordners können Sie übrigens festlegen, ob die Agenten der dort zu erstellenden Hosts im xref:glossar#pull_mode[Pull-Modus] oder im xref:glossar#push_mode[Push-Modus] der {PE} arbeiten sollen.
...
Diese Rolle ist nur in der {PE} verfügbar.
...
Diese Option gibt es nur in der {PE}.
...
Den Regelsatz [.guihint]#Agent registration# gibt es nur in der {PE}.
...
Bei Auswahl von [.guihint]#Stop and do create hosts# ist die Regelauswertung abgeschlossen und die beiden folgenden Kästen werden nicht mehr betrachtet.
...
// TK: Screenshot drinlassen oder weg wg. trivial?
// MFS: Drin lassen, gute Orientierungshilfe.
...
Hierfür nutzen Sie den Regelsatz [.guihint]#Agent controller auto-registration# der Agentenbäckerei, der nur in der {PE} vorhanden ist.
...
ob der Agent Controller bereits existierende Verbindungen zu einem {CMK}-Server löschen ([.guihint]#no#) oder behalten soll ([.guihint]#yes#).
...
Nach der Installation des Agentenpakets setzt der Agent Controller den Befehl zur Registrierung ab.
// TK: Nach oder Während der Installation? Wann genau? Und ist das überhaupt wichtig?
// MFS: Nach der Installation, während des regulären Push-Zyklus – aus diesem Grund kann es ~1 Minute dauern, bis der Host erscheint
...
// TK: Enthält Registrierungsanfragen?
