...
speziellen Host-Typen gibt. Der Cluster-Host ist ein virtueller Host, der mit
...
In unserem Beispiel sind die beiden Knoten-Hosts „mssql_node01” und „mssql_node02“ bereits als link:wato_hosts.html[Hosts] angelegt und eingerichtet. Wie es
...
Geben Sie als [.guihint]#Hostname# `mssql_cluster01` ein und tragen Sie unter
[.guihint]#Nodes# die beiden Knoten-Hosts ein.

Im Kasten [.guihint]#Network Address# können Sie als [.guihint]#IPv4 Address# die Cluster-IP-Adresse (falls verfügbar) eintragen. Stellen Sie sicher, dass im Kasten [.guihint]#Data Sources# die gleichen Datenquellen ausgewählt sind wie bei den beiden Knoten-Hosts.
...
unterschiedlichen Ordnern organisiert sind). Sie erhalten die Seite [.guihint]#New rule: Clustered services#:

image::cs_rule_cs.png[]

Im Kasten [.guihint]#Conditions# aktivieren Sie [.guihint]#Services# und machen dort zwei Einträge: `MSSQL` für alle MS SQL-Services, deren Name mit „MSSQL“ beginnen und `Filesystem D:` für das Laufwerk. Die Eingaben werden als link:regexes.html[reguläre Ausdrücke] interpretiert.
...
suchen Sie den Regelsatz [.guihint]#Clustered services for overlapping clusters# und klicken Sie ihn an. Wählen Sie unter [.guihint]#Create rule in folder# wieder den Ordner aus, der die Knoten-Hosts enthält.
...
Cluster `mssql_cluster01` ein:

image::cs_rule_cs_overlapping.png[]

Im Kasten [.guihint]#Conditions# aktivieren Sie [.guihint]#Explicit hosts# und tragen
den ersten Knoten-Host `mssql_node01` ein. Unter [.guihint]#Services# machen Sie zwei Einträge: `MSSQL` für alle MS SQL-Services und `Filesystem D:` für das Laufwerk:
...
Führen Sie abschließend die Service-Erkennung als 3.
