...
// IGNORE Programmier Versionsnummer] [und miteinzupacken unpaketierter
...
* In den {CEE} haben Sie zusäzlich den Setup-Menüeintrag [.guihint]#Extension Packages#
...
----
// MA: Hier fehlt noch ein Hinweis, warum dort die Versionsnummer angegeben wurde, oder? Also für Fälle, wo das auch wichtig ist, weil es zwei Versionen desselben MKPs gibt. In Kapitel 3.4 wird das (implizit) aufgegriffen. Hier ist es wahrscheinlich erstmal nicht wichtig, oder?
...
// MA: Ah -- hier kommt jetzt der Hinweis, wann und warum man die Versionsnummer mit angibt. Der Satz sollte imho oben im Kapitel 3.2 stehen.
...
// MA: Den folgenden Absatz verstehe ich nicht. Also speziell, wo der Pfad `~/usr/local` herkommt. Er wurde vorher bisher nie explizit erwähnt.
Wenn Sie die Archive neu packen, achten Sie darauf, keine Pfade miteinzupacken, die nicht Teil der Ordnerhierarchie unter `~/usr/local` sind. Auf der obersten Ebene dürfen nur die oben zu sehenden Manifeste und Tar-Dateien enthalten sein.
...
|`enable` | Name des Paketes und Versionsnummer |Aktiviert ein Paket je nach Versionskompatibilität für lokale Verwendung oder Verteilung an Remote-Sites.
