// -*- coding: utf-8 -*-
// IGNORE Kompatibilitäts p1 zeilen endoflife AlmaLinux LTS Lifecycle STS abgekündigten
include::global_attr.adoc[]
= Update-Matrix für Version {v21}
:revdate: 2023-11-02
:title: Update-Matrix für Version {v21}
...
xref:update_major#[Update auf Version {v21}]
...
Sollten Sie auf eine ältere oder neuere {CMK}-Version als {v21} aktualisieren wollen, lesen Sie diesen Artikel bitte für die {CMK}-Version, die _Ziel_ der Aktualisierung ist.
...
Die Mindestanforderungen sind im xref:update_major#[Artikel zum Update auf {v21}] beschrieben.
...
Für ein Update von Debian 9 mit {CMK} {v20}p1 auf Debian 12 mit {CMK} {v21} ergibt sich so die folgende Update-Reihenfolge:

. Unter Debian 9 führen Sie das Update von {CMK} {v20}p1 xref:update#[auf die aktuelle Patch-Version] von {v20} durch.

. Weiterhin unter Debian 9 aktualisieren Sie die aktuelle Patch-Version von {CMK} {v20} auf die xref:update_major#[aktuelle Patch-Version von {v21}].

. Führen Sie nun das Upgrade von Debian 9 auf 10, 11 und schließlich auf 12 durch, so wie es im Artikel zum xref:release_upgrade#[Linux-Upgrade auf dem {CMK}-Server] beschrieben ist.
