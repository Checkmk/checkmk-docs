...
= Update-Matrix für Version {v16}
:revdate: 2023-11-02
:title: Update-Matrix für Version {v16}
...
// xref:update_20#[Update auf Version {v16}]
...
Dies kann es erforderlich machen, vor einem Update von {CMK} ein oder mehrere Upgrades Ihrer Linux-Distribution vorzunehmen.
...
Sollten Sie auf eine neuere {CMK}-Version als {v16} aktualisieren wollen, lesen Sie diesen Artikel bitte für die {CMK}-Version, die _Ziel_ der Aktualisierung ist.
...
// Die Mindestanforderungen sind im xref:update_20#[Artikel zum Update auf {v16}] beschrieben.
...
Für ein Update von Debian 7 mit {CMK} {v15}p1 auf Debian 10 mit {CMK} {v16} ergibt sich so die folgende Update-Reihenfolge:

. Unter Debian 7 führen Sie das Update von {CMK} {v15}p1 xref:update#[auf die aktuelle Patch-Version] von {v15} durch.

. Führen Sie nun das Upgrade von Debian 7 auf 8, 9 und schließlich auf 10 durch.

. Unter Debian 10 aktualisieren Sie die aktuelle Patch-Version von {CMK} {v15} auf die aktuelle Patch-Version von {v16}.
...
Da sowohl Checkmk {v15} als auch {v16} das Supportende erreicht haben, werden die Tabellen hier keine Aktualisierungen mehr erhalten.
