...
// TK: Die {CMK}-Versionsnummern am oberen Rand der Grafiken im Kapitel xref:matrix[Kompatibilitäts-Matrix] zeigen immer diejenigen an, _ab_ der eine Veränderung vorgenommen wurde, also eine Linux-Version neu unterstützt oder nicht mehr unterstützt wird.
...
Ein stumpfes Ende kennzeichnet dagegen eine {CMK}-Version, die Ihr Support-Ende erreicht hat.
...
Die Aufgabe ist es nun, in der Grafik für Ihre Linux-Distribution tendenziell von links oben nach rechts unten zu gelangen.
...
Wir streben dabei an, jede beim Erscheinen von {CMK} unterstützte Distributionsversion über den gesamten Produkt-Lifecycle von {CMK} zu unterstützen, sofern dem nicht technische Gründe entgegenstehen.
...
// TK: Das betrifft doch nur Ubuntu, oder? Also: Ubuntu STS-Versionen ...
