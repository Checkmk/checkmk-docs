...
In den {CEE} können Sie die SAML-Einstellungen direkt in {CMK} vornehmen. Als Identity Provider (IdP) fungiert beispielsweise Azure AD.

In xref:further_IdPs[weiteren Abschnitten] bekommen Sie Hinweise zur Konfiguration weiterer IdPs, etwa Active Directory Federation Services (ADFS) und Micro Focus NetIQ Access Manager. Diese sind insbesondere für die {CRE} relevant, da hier keine SAML-Einstellungen in {CMK} unterstützt werden.
...
* RSA-SHA256 (https://www.w3.org/2001/04/xmldsig-more#rsa-sha256^)
* RSA-SHA384 (https://www.w3.org/2001/04/xmldsig-more#rsa-sha384^)
* RSA-SHA512 (https://www.w3.org/2001/04/xmldsig-more#rsa-sha512^)
* ECDSA-SHA256 (https://www.w3.org/2001/04/xmldsig-more#ecdsa-sha256^)
* ECDSA-SHA384 (https://www.w3.org/2001/04/xmldsig-more#ecdsa-sha384^)
* ECDSA-SHA512 (https://www.w3.org/2001/04/xmldsig-more#ecdsa-sha512^)

{CMK} selber nutzt RSA-SHA512 für die Signierung seiner Anfragen.

Sollte der IdP für seine Antwort einen anderen Algorithmus verwenden, so wird seine Antwort von {CMK} abgewiesen. 
