// -*- coding: utf-8 -*-
// IGNORE ADFS ECDSA FS NetIQ SHA256 SHA384 SP1 SP4 SSO Url UserPrincipalName libxmlsec1-openssl mellon mod_auth_mellon myadfs saml IdP IdPs Attributangaben libxmlsec1 |
...
// ML: Vielleicht Mattias' schönen CA-Blog-Artikel verlinken, https://checkmk.com/de/blog/how-become-your-own-certificate-authority.
...
Vorausgesetzt, es gibt nicht bereits einen Nutzer mit der gleichen ID, der für eine andere Verbindung genutzt wird.
Sollte bereits ein Nutzer mit gleicher ID vorhanden sein, so wird die aktuelle Erstellung des Nutzers abgelehnt.

// ML: "... Nutzer ..., der für eine ... Verbindung genutzt wird." Nutzer, die für Verbindungen genutzt werden klingt merkwürdig. Ich würde den Halbsatz mit der Verbindung verwerfen.
...
* Die Endpunkte der „{CMK}-Service-Provider-Metadaten“ müssen beim IdP registriert worden sein.

// ML: Das Wort Endpunkte kommt gar nicht mehr vor und die Formulierung ist irgendwie ... weiß auch nicht. Ich würde erwarten, dass Endpunkte die Metadaten des Service Providers Checkmk liefern. Sind damit vielleicht die beiden URLs zu saml_metadata.py und saml_acs.py gemeint? ... Nach kurzem Gucken würde ich vermuten ja.
// ML: Mal als Vorschlag: "{CMK}s SAML-Endpunkte für ID/Metadaten und Antworten (Assertion Consumer Service) müssen beim IdP registriert worden sein (wie, zeigen wir unten.)"
...
// ML: Vielleicht nen rotes Kästchen um die beiden Angaben im Bild oben - weil das so riesig ist.
...
// ML: Im folgenden Satz vielleicht das "suchen Sie weitere ..." rauswerfen, weil schon im Satz oben drin. Vielleicht einfach "Und zwar in der Ansicht ... :"
...
// ML: Ich vermute, das hier muss eine Hierarchieebene weiter runter. Fall ich das richtig sehe, würde ich zudem den Titel in sowas wie "SAML in der {CMK}-Weboberfläche aktivieren" umbenennen, da das Hauptkapitel ja schon "SAML in EE einrichten" heißt.

// ML: Das erste Bild in diesem Kapitel ist korrekt, die anderen sind falsch: Das sind einzelne Kästen, die als Ganzes rein müssten und nicht mit Rand. Beim ersten passt es, weil es Ausschnitt samt Header ist.
...
Der [.guihint]#Name# wird hinterher für die xref:login[Benennung des Anmelde-Buttons] genutzt.

// ML: Der xref-Link führt ins Leere - war mir aber nicht sicher, wohin der gehen sollte.
...
Eigene Zertifikate werden im Ordner `omd/sites/mysite/etc/ssl/saml2/custom` abgelegt. Sie können rotiert werden, z.B. automatisiert mit Hilfe eines Skripts.
...
//ML: Fürs Verständnis sollte das reichen -- ich würde den Satz vermutlich ganz streichen, denn dass ich Dateien (egal welche) rotieren/austauschen kann, scheint mir völlig selbstverständlich. Es sei denn, CMK macht was Spezielles mit hin- und weg-rotierten Dateien ... 
//ML: Was ich im Screenshot hier schön fände, wären eingetragene Daten - weil dann klar ist, dass der Key selbst dorthin gehört, nicht zum Beispiel der Pfad zur Key-Datei; aber nur ein Nice-to-have (und vielleicht für andere auch selbstverständlich?).
...
Wichtig ist hierbei, dass [.guihint]#User ID attribute# unbedingt eindeutig sein muss, z.B. die E-Mail-Adresse. {CMK} benötigt hier für jede Angabe den kompletten [.guihint]#claim name# aus Azure AD, also die mit http beginnende Adresse. Beispielsweise für den Vornamen in obigem Beispiel `\http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`.

// ML: Finde ich ein wenig verwirrend. Das klingt, als dürfte User ID attribute auch der Claim name der Mail-Adresse sein - gemäß Screenshot wird aber http..../claims/userID verlangt (und per Default wird user_id vorgegeben). Auch das givenname-Beispiel leuchtet mir nicht ein, weil das in dem Dialog gar nicht vorkommt.
