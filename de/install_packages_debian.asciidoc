...
[{shell}]
----
{c-root} scp check-mk-raw-2.0.0b7_0.focal_amd64.deb root@mymonitoring.mydomain.org:
----
...
[{shell}]
----
{c-root} dpkg-sig --verify check-mk-raw-2.0.0b7_0.focal_amd64.deb
----

Anschließend können anschließend das {CMK}-Paket folgendem Befehl installieren. Achten Sie dabei darauf den vollständigen Pfad zu der .deb-Datei an `apt install` zu übergeben:

[{shell}]
----
{c-root} apt install /tmp/check-mk-raw-2.0.0b7_0.focal_amd64.deb
----
...
----
{c-root} omd version
OMD - Open Monitoring Distribution Version 2.0.0b7.cre
----
