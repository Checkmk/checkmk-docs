...
Tritt dies auf, ist der wahrscheinlichste Grund, dass die installierte Version von `dpkg-sig` nicht mit dem Kompressionsformat des Paketes umgehen kann.
...
Die angezeigte Warnung bezieht sich auf den Public Key von {CMK}, nicht die Signatur des Paketes selbst.
Grund hierfür ist, dass der verwendete Schlüssel mit keinem anderen Schlüssel signiert ist, dem der Nutzer vertraut.
