...
`\http://[MONITORING-SERVER]/[SITE]/check_mk/host_inv_api.py?host=[HOST]`

Für das folgende Beispiel sieht das dann so aus:

`\http://klapp-0242/mysite2/check_mk/host_inv_api.py?host=mycmkserver`
...
`\http://klapp-0242/mysite2/check_mk/host_inv_api.py?host=mycmkserver&output_format=json`
...
`\http://klapp-0242/mysite2/check_mk/host_inv_api.py?host=mycmkserver&output_format=xml`
...
`\http://klapp-0242/mysite2/check_mk/host_inv_api.py?request={"hosts": ["mycmkserver", "myhost456"]}&output_format=json`
...
Nun wollen Sie aber vielleicht nicht alle Inventurdaten abfragen, sondern suchen nur gezielt nach einzelnen Informationen. Dann geben Sie sogenannte _Inventurpfade_ an, um die gewünschten Angaben zu definieren. Sie bekommen dann nur von denjenigen Hosts Informationen angezeigt, die diese Pfade / Informationen haben.
...
`\http://klapp-0242/mysite2/check_mk/host_inv_api.py?host=mycmkserver&request={"paths":[".hardware.memory.total_ram_usable", ".hardware.memory.total_swap"]}&output_format=json`
...
Für jeden Host gibt es dort eine Datei in unkomprimierter (z.B. `mycmkserver`) und eine in komprimierter Variante (z.B. `mycmkserver.gz`).
