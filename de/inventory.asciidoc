...
// TK: Einen neuen Service gibt es nur, wenn nicht bereits eine der Standardregeln zugeschlagen hat.
// TK: Bei meiner Linux-Kiste ist der Service eh schon da wegen des Labels "cmk/check_mk_server:yes"
// TK: Daher neuen Absatz geschrieben:
...
sehr sinnvoll, dass Sie das Intervall, in dem der Inventurcheck ausgeführt
...
Service.
// TK: Das passt nicht zum folgenden: Warum soll ich etwas anpassen, was eh schon "richtig" vorkonfiguriert ist, d.h. 1 Tag statt 1 Minute?
...
// TK: Hier könnte man ruhig die {v16} streichen, auch wenn der alte im Windows-Agenten Artikel noch drinnen ist.
Einzelheiten finden Sie im link:agent_windows.html#plugins[Artikel zum Windows-Agenten.]
// TK: Ich bin ratlos: Windows-Plugin installiert: In meiner CEE Site sehe ich 16 Inventureinträge, in meiner RAW-Site 2606 Einträge. 
...
// TK: Für später: Die Inventur hat im Monitor-Menü ein eigenes Topic. Dieser Einstieg wird hier aber nur sehr versteckt erwähnt.
...
// TK: Die 3 folgenden Kommentare können alle weg...
////
...
// TK: Woraus bezieht sich "sie"? Ansichten, Hosts, Filter?
...
[.guihint]#CPU Related Inventory of all Hosts.# 
// TK: Gibt's nicht. Ist "CPU inventory of all hosts" gemeint?
...
=== Zugriff via Web-API
// TK: Web-API ist als Begriff nicht so dolle, weil wir den Begriff für die "richtige" Web-API verwenden.
// TK: Wie wärs mit "Host Inventory Web API" oder "Host Inventory API" oder einfach nur "API"?
...
// TK: Der letzte Satz gehört in das Kapitel Statusdaten.
