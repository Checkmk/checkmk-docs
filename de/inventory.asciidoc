...
*Hinweis:* Bei der hier erwähnten Web-API handelt es sich NICHT um die (abgekündigte) {CMK} Web-API. Diese Inventur-eigene Web-API ist von der Abkündigung nicht betroffen.
...
`\http://mycmkserver01/mysite/check_mk/host_inv_api.py?host=myhost123&output_format=xml`
...
Geben Sie die URL in die Adressleiste Ihres Browsers ein.
Sie sehen dann sofort ein Ergebnis, weil Sie bereits bei {CMK} angemeldet sind.
...
Sollte ein Fehler bei der Abfrage auftreten, z. B. weil der angegebene Host nicht gefunden wurde, so wird der _result code_ auf 1 gesetzt und eine entsprechende Fehlermeldung ausgegeben:

`{"result": "Found no inventory data for this host.", "result_code": 1}`

==== Mehrere Hosts abfragen

Sie können auch die HW/SW-Inventurdaten mehrerer Hosts gebündelt in einer Ausgabe abfragen. Erweitern Sie dafür die Abfrage auf alle gewünschten Hosts:
`http://mycmkserver01/mysite/check_mk/host_inv_api.py?request={"hosts": ["myhost123", "myhost456"]}`

Das Ergebnis diesr Abfrage sieht dann im wesentlichen genauso aus, wie die obige Ausgabe. Der Unterschied besteht darin, dass auf der obersten Ebene die Namen der Hosts als Schlüssel verwendet werden und die Angaben zu den Hosts dann darunter in den Verzeichnisbäumen folgen:
...
Nun wollen Sie aber vielleicht nicht alle Inventurdaten abfragen, sondern suchen nur gezielt nach einzelnen Informationen. Dann geben Sie sogenannte _Inventurpfade_ an, definieren also die gewünschten Angaben. Sie bekommen dann nur von denjenigen Hosts Informationen angezeigt, die diese Pfade / Informationen haben.
...
`http://mycmkserver01/mysite/check_mk/host_inv_api.py?host=&request={"paths": [".hardware.memory.total_ram_usable", ".hardware.memory.total_swap"]}&output_format=json`

Sie bekommen dann exakt die gewünschten Angaben zurück:
...
Für jeden Host gibt es dort eine Datei in unkomprimierter (z.B. `myhost123`) und eine in komprimierter Variante (z.B. `myhost123.gz`).
