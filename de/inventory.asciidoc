...
=== Die Inventur für die gewünschten Hosts einschalten#activate
...
Regelsuche und dem Wort `inventory`. Verwechseln Sie das nicht mit dem Regelsatz [.guihint]#Hardware/Software-Inventory (Linux, Windows, Solaris, AIX)#. 
...
[.guihint]#Activate Changes# einen [wato_services#active_checks|aktiven
...
[cols=<th style="width: 40%">Kasten</th>,<th style="width: 35%">Plugin</th>,options="header"]
|===
...
|Windows
|[.guihint]#Windows Agent - Plugins#

|`mk_inventory.vbs`

|Linux
|[.guihint]#Linux/Unix Agents - Plugins#
...
[CEE] Falls Sie für die Konfiguration Ihrer Agenten die link:wato_monitoringagents.html#bakery[Agent Bakery]
...
== Mit den Inventurdaten arbeiten#operating
...
über den Knopf ICON[icon_inv.png] [.guihint]#Inventory:#

[.border]
image::inventory_contextbutton.png[]

Alternativ gehen Sie zu dem Service [.guihint]#Check_MK HW/SW Inventory#, zum ICON[icon_menu.png]
Menüsymbol und dort zum Eintrag ICON[icon_inv.png] [.guihint]#Show Hardware/Software Inventory of this host#:
...
[.guihint]#Hardware}}, ICON[icon_networking.png] {{Networking}} und ICON[icon_software.png] {{Software#
...
ist z.B. die Tabelle [.guihint]#ICONlink:icon_software.png] Software.html[ICON[icon_packages.png] Packages#:
...
Tabelle auch als link:views.html[Ansicht] gibt -- mit den üblichen Möglichkeiten
...
Wenn Sie über den oben genannten Link hierhergekommen sind, dann ist im Filter [.guihint]#Hostname (exact match)#
...
Alle tabellenartigen Inventuransichten finden Sie auch über das [.guihint]#Views#-Element in der Seitenleiste
unter dem Eintrag [.guihint]#Inventory}}. Diejenigen, die mit dem Wort {{Search# beginnen, geben erst
dann Daten aus, wenn Sie den Knopf [.guihint]#Search# drücken (nachdem Sie eventuell einige der Filter
ausgefüllt haben).
...
== Historischer Verlauf der Inventurdaten#history
...
mit dem Knopf ICON[icon_inv.png] [.guihint]#Inventory history#:
...
== Externer Zugriff auf die Daten#external
...
== Inventur in verteilten Umgebungen#distributed
...
`var/check_mk/inventory` abgelegt. link:inventory.html#statusdata[Statusdaten]
