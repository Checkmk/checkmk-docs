...
und in verschiedenen anderen xref:views[Ansichten] dargestellt und sind auch über eine API verfügbar.
...
geht das auch hier mithilfe einer xref:wato_rules[Regel.] Den Regelsatz finden
...
oder mit der xref:wato_monitoringagents#bakery[Agentenbäckerei] (nur {EE}) machen.
...
erfahren Sie im xref:agent_linux#plugins[Artikel zum Linux-Agenten.]
...
{cee-only}
Falls Sie in den {EE} für die Konfiguration Ihrer Agenten die xref:wato_monitoringagents#bakery[Agentenbäckerei]
...
Das Besondere an diesem Teil der Inventurdaten: Sie können die Pakete über [.guihint]#Host > Inventory > Software packages# in einer separaten Ansicht aufrufen. Dort finden Sie dann icon:icon_filter[] Filter speziell für die Suche in Paketen (im Bild stark gekürzt):
...
Wenn Sie möchten, können Sie sich informieren lassen, wann immer
...
nichts dagegen), müssen Sie lediglich die entsprechende Option aktivieren in der Regel,
die Sie xref:activate[am Anfang] unter [.guihint]#Do hardware/software inventory# angelegt haben:
...
in der xref:inventory#history[Historie!] Dies würde quasi zu ständigen
...
Die URL dazu lautet `host_inv_api.py?host=`, gefolgt vom Host-Namen, z.B.:
...
authentifizieren Sie sich am besten als xref:wato_user#automation[Automationsbenutzer.]
...
in xref:distributed_monitoring[verteilten Umgebungen.] Hier werden die
Inventurdaten zunächst durch die lokalen Instanzen ermittelt und dort unterhalb
von `var/check_mk/inventory` abgelegt. 


Der xref:distributed_monitoring#livestatusproxy[Livestatus-Proxy-Daemon] überträgt
...
per xref:livestatus[Livestatus] von den Remote-Instanzen geholt werden.
