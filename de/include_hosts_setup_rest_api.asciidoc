...
Schließlich finden Sie im Artikel zur REST-API xref:rest_api.html#examples[Beispiele], wie Sie auf der Kommandozeile Aktionen für Hosts und Ordner ausführen können, z.B. die Ordnerstruktur und die Hosts eines Ordners anzuzeigen, einen Host in einem bestimmten Ordner zu erstellen und vieles mehr.
