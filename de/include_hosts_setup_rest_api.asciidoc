...
- In den ersten 3 Anfragen hab ich noch mit Hervorhebungen gearbeitet (## für gelben Hintergrund). Das funktioniert aber später nicht mehr, wenn die wichtigen Texte in doppelten Anführungen stehem: Trotzdem so lassen oder auf Hervorhebungen ganz verzichten? -> SP: Ich würde es so lassen, finde es gut.
...
Die in diesem Kapitel enthaltenen Beispiele orientieren sich an dem Beispiel-Code für das Kommandozeilenprogramm cURL, das in der REST-API-Dokumentation enthalten ist. 
...
//SP: Ausführung ist auch für ganz frische Hosts möglich, aber der Informationsgehalt ist niedrig. Hinweis fände ich gut. Eventuell inkl. Verweis auf die Agenteninstallation.
