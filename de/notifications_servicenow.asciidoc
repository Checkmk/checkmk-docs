include::global_attr.adoc[]
= Benachrichtigungen per ServiceNow
:revdate: draft
:title: Benachrichtigungen per ServiceNow (notifications)
:description: Sie können Benachrichtigungen von {CMK} an das Incident-Management-System ServiceNow senden lassen. Wie Sie die Anbindung durchführen, erfahren Sie hier.

{related-start}
xref:notifications#[Benachrichtigungen]
{related-end}


== Funktion

{cee-only}
Die Anbindung von link:https://www.servicenow.com/[ServiceNow^] steht nur für die {EE} zur Verfügung.
Über die Plattform ServiceNow lassen sich unterschiedlichste Workflows abbilden und automatisieren (Stichwort ITIL), so auch die Weiterleitung von {CMK}-Benachrichtigungen an Benutzer.
Die Integration läuft über die Instanz-URL und Login-Daten.


== Konfiguration ServiceNow

Eine spezielle Konfiguration innerhalb von ServiceNow ist nicht erforderlich, es müssen lediglich Instanz-URL (gegebenenfalls in Form einer Custom-URL) und Benutzerdaten bekannt sein.


== Konfiguration {CMK}

Wie Sie Benachrichtigungen im Allgemeinen in {CMK} einrichten, haben Sie bereits im Artikel über xref:notifications#[Benachrichtigungen] erfahren.
Für die Anbindung von ServiceNow gehen Sie in {CMK} nun wie folgt vor:

. Wählen Sie als [.guihint]#Notification Method# den Eintrag [.guihint]#Servicenow (Enterprise only).#
. Geben Sie unter [.guihint]#ServiceNow URL# Ihre Instanz-URL ein.
. Fügen Sie Benutzername und Passwort des _erstellenden_ Benutzers hinzu.
. Wählen Sie den [.guihint]#Management type# aus, ServiceNow kennt zwei Typen: _case_ und _incident_. Abhängig von Ihrer Auswahl ändern sich die darunter angezeigten Optionen.

=== Den Managementtyp _Case_ ausgestalten

Haben Sie [.guihint]#Case# ausgewählt, so können Sie aus verschiedenen weiteren Optionen auswählen, um den Inhalt der Benachrichtigung zu gestalten.

image::notifications_servicenow_case.png[alt="Ansicht der Auswahloptionen für den Typ Case".]

=== Den Managementtyp _Incident_ ausgestalten

Haben Sie [.guihint]#Incident# ausgewählt, so gibt es ein paar Besonderheiten bei der Ausgestaltung der Benachrichtigung.

image::notifications_servicenow_incident.png[alt="Ansicht der Auswahloptionen für den Typ Incident".]

. Unter [.guihint]#Caller ID# ergänzen Sie den Benutzernamen des _betroffenen_ Benutzers.
Es wird empfohlen, als erstellenden und betroffenen Benutzer denselben Benutzernamen zu verwenden. In der Inline-Hilfe von {CMK} finden Sie dazu genauere Informationen.
. Alle weiteren Optionen dienen auch hier der Gestaltung des Inhalts der Benachrichtigung.
. Für die beiden ServiceNow-eigenen Optionen [.guihint]#Urgency# und [.guihint]#Impact# finden Sie in der Inline-Hilfe von {CMK} Links auf die
jeweiligen Einträge in der ServiceNow-Dokumentation.

=== Benachrichtigungen testen

Zum Testen können Sie die xref:commands#[Kommandos] [.guihint]#Fake check results# und [.guihint]#Custom notification# verwenden.

