...
// TK: Checks ist (noch) nicht eingeführt. Muss das hier sein?
...
// TK: Besser so: So ist es uns möglich über xref:glossar#livestatus[Livestatus]-Abfragen vom Host Informationen über Host-Gruppen zu erhalten, die der {CMK}-Server zur Verfügung stellt.
// TK: Nach meinem Gefühl sollten wir ein paar mehr Sätze zum Setup der Beispielkonfiguration spendieren...

Im folgenden Beispiel gehen wir von einem Unternehmen mit mehreren Standorten aus:
...
Öffnen Sie eine Kommandozeile für Ihre {CMK}-Instanz.
Melden Sie sich als Instanzbenutzer an und fragen Sie die Informationen zu den Host-Gruppen ab:
// TK: Ersetzen durch: Melden Sie sich als Instanzbenutzer an und fragen Sie die Informationen zu den Host-Gruppen mit folgendem Kommando ab: 
...
// TK: Bis jetzt haben Sie sich mit dem `lq`-Befehl als Instanzbenutzer die Informationen anzeigen lassen.
...
// Beim alten Kommando "cmk -L | head -n 20" gibts einen "BrokenPipeError: [Errno 32] Broken pipe". Daher umgeschrieben.
[{shell}]
----
{c-omd} cmk -L
...
Mit `echo` wird dabei die Ausgabe des `GET`-Kommandos an das Kommando `unixcat` weitergeleitet.
Das Ziel der Ausgabe, der Unix-Socket `~/tmp/run/live`, wird dahinter definiert.
// TK: Hier ist ~/tmp/run/live` falsch, da wir als root arbeiten und nicht als Instanzbenutzer, daher: /omd/sites/$SITE/tmp/run/live
...
Nehmen Sie den xref:intro_setup_monitor#linux[Host ins Monitoring] auf (z.B. als `localhost`) und rufen Sie die Agentendaten dann mit `cmk -d` ab.
...
Sie können Ihr Check-Plugin mit jedem auf dem Linux-System installierten Texteditor
bearbeiten.

Legen Sie also unser Beispiel-Check-Plugin hier an. Konvention ist, dass der Dateiname den Namen der Agentensektion wiedergibt. _Pflicht_ ist, dass die Datei mit `.py` endet, denn ab Version {v20} von {CMK} handelt es sich bei den Check-Plugins immer um echte Python-Module.

Als erstes müssen Sie die für die Check-Plugins nötigen Funktionen aus
...
//SP: Hier müsste noch hin, wie man auf die Ausgabe darunter überhaupt kommt.
...
In unserem Beispiel bedeutet dies, dass jede Zeile Element für Element durchgegangen wird. Der Inhalt aller Zeilen wird so in ein Dictionary umgewandelt. Aus jeder Zeile wird dafür Element 0 (die Host-Gruppe) gefolgt vom Text _members_ und dahinter dem Element 1 (den Mitgliedern der Host-Gruppe) genommen.
(Python beginnt mit der Zählung bei Null, d.h. das erste Element ist Element 0.)

Mit dem `return parsed` wird dieses Dictionary zurückgeben.
...
//SP: WAs ist das Ziel des Check-Plugins? Was soll vom Service angezeigt/geprüft werden?
...
* Ist die Variable `hosts` leer - sind also keine Hosts in der Host-Gruppe `check_mk`, so geht stattdessen der Status des Service auf {OK}. Dann wird zudem ein passender Hinweistext ausgegeben.
...
//SP: Hier muss stattdessen der Link zu Github stehen (vgl. Kap. 3.2)
...
=== Das Check-Plugin in {CMK} testen und aktivieren
...
// TK Altkommentare aus dem Metrikkapitel:
...
In unserem Beispiel sind die Zahl der Hosts in der Host-Gruppe (`num_hosts`) und die Zahl der Hosts im Zustand {UP} (`num_hosts_up`) die Messwerte und der Prozentsatz der Hosts im Zustand {UP} (`hosts_up_perc`) ein daraus berechneter Wert.
...
Metriken werden von der Check-Funktion ermittelt und einfach als zusätzliches Ergebnis zurückgegeben.
Dazu dient das Objekt `Metric`, welches mindestens die beiden Argumente `name` und `value` benötigt.

Wir sind faul und nehmen als Namen der Metrik den Variablennamen, in dem der Prozentsatz als Wert gespeichert ist:
// COMMENT[Hinweis, welchen Namen man für seine Metrik nehmen soll!] > TK: Ja, welche denn?

[{python}]
----
    yield Metric(name="hosts_up_perc", value=hosts_up_perc)
----

Weiterhin gibt es noch zwei optionale Argumente.
Mit dem Argument `levels` können Sie eine Information zu Schwellwerten für {WARN} und {CRIT} mitgeben, und zwar in Form eines Paares von zwei Zahlen.
Die Schwellwerte werden dann üblicherweise im Graphen als gelbe und rote Linien eingezeichnet.
Die erste Zahl steht für die Warnschwelle, die zweite für die kritische.
...
Wenn die gleichen Schwellwerte von 80 und 90 (Prozent) verwendet werden wie im xref:partial_results[vorherigen Abschnitt] bei der Erweiterung der Check-Funktion, sieht das so aus:

[{python}]
----
    yield Metric(name="hosts_up_perc", value=hosts_up_perc, levels=(80, 90))
----

Hinweise:

* *Wichtig:* Die Angabe von `levels` dient hier lediglich als Information für die Darstellung des Graphen.
Für die _Überprüfung_ der Schwellwerte ist die Funktion `check_levels` verantwortlich.

* Falls nur eine der beiden Schwellwerte definiert ist, tragen Sie für die andere einfach `None` ein, also z.B. `levels=(None, 90)`.

* Es sind auch Gleitkommazahlen (_float_) erlaubt, aber keine Strings.

Analog zu den Schwellwerten können Sie dem Graphing-System auch die Information über den möglichen Wertebereich mitgeben.
Damit ist der kleinste und größte mögliche Wert gemeint.
Das geschieht im Argument `boundaries`, wobei auch hier optional für eine der beiden Grenzen `None` eingesetzt werden kann.
...
Analog können Sie jetzt auch die Metrik `services_ok_perc` definieren und beide Metrikdefinitionen ans Ende der Check-Funktion anfügen.
...
----
    ...
    yield Metric(name="hosts_up_perc", value=hosts_up_perc, levels=(80, 90), boundaries=(0, 100))
    yield Metric(name="services_ok_perc", value=services_ok_perc, levels=(80, 90), boundaries=(0, 100))
----

*Hinweis:* Es ist natürlich möglich, nicht nur die beiden berechneten Werte sondern _alle_ Messwerte als Metriken zu definieren, in unserem Beispiel also alle Messwerte aus der Host-Gruppentabelle.
...
Mit der erweiterten Check-Funktion sind die beiden Graphen im Monitoring sichtbar.
In der Service-Übersicht zeigt nun das Symbol icon:icon_service_graph[alt="Symbol zur Anzeige der Graphen des Services."], dass es zum Service Graphen gibt.
...
Und zum Abschluss wieder ein *Hinweis:*
Wenn, wie im obigen Beispiel, die Funktion `check_levels` für die Rückgabe verwendet wird, können Sie sich die separate Rückgabe der Metriken sparen und dies im Funktionsaufruf erledigen
-- durch Hinzufügen der Argumente `metric_name` und `boundaries`.
Die sechs Zeilen von oben reduzieren sich so wieder auf vier:
...
link:https://github.com/Checkmk/checkmk-docs/blob/master/examples/devel_check_plugins/check_plugin_advanced_myhostgroups.py[GitHub^]
finden Sie die erste Variante mit der Rückgabe der Metriken per `Metric`-Objekt.
...
* Es erscheint nicht automatisch ein „Perf-O-Meter“, also die grafische balkenartige Vorschau der Metrik in der Service-Übersicht (z.B. in der Ansicht, die alle Services eines Hosts darstellt).
