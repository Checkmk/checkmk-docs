...
Im Artikel zur Einführung in die xref:draft_intro_devel#[Programmierung von Erweiterungen] für {CMK} erfahren Sie, welche Möglichkeiten Sie haben.

Dieser Artikel zeigt Ihnen, wie Sie echte Check-Plugins für den {CMK}-Agenten entwickeln können — mit allem was dazugehört.
Dabei zeigen wir Ihnen, wie Sie die Check-API für die Plugin-Programmierung nutzen.
Für SNMP-basierte Check-Plugins gibt es einen xref:draft_devel_check_plugins_snmp#[eigenen Artikel.]
...
Die erste Zeile der Ausgabe enthält die Spaltennamen der abgefragten Ta belle `hostgroups`.
...
// TK: Tja, wir halten uns nicht an unsere eigenen Regeln, da wir im Beispiel keinen Präfix verwenden. Oder zählt „my“ als Präfix? 
...
An diesem Punkt bietet die Parse-Funktion durch die von ihr erzeugte andere Struktur deutliche Vorteile.
...
Der entscheidende Unterschied besteht darin, dass sich bei einem `yield` gemerkt wird, wie weit die Funktion in einer Liste/Tabelle etc. gekommen ist.
Beim nächsten Aufruf wird _nach_ der letzten `yield`-Anweisung weiter gemacht - und nicht wieder am Anfang begonnen.
Dadurch wird nicht nur der erste Treffer in einer Tabelle ausgelesen (wie es beim `return` der Fall wäre), sondern sukzessive alle Treffer.
// TK: Ist im obigen Absatz “Tabelle” wirklich der korrekte Begriff? Ich würde ihn ersatzlos streichen.
...
Das erreichen wir durch Erweiterung des im vorherigen Kapitel erstellten xref:agentplugin[Agentenplugins.]
...
// TK: Uff, das überfordert mich. Wenn zip so kompliziert ist, kann man das nicht anders = einfacher lösen? Kannst du das übernehmen?
// MFS: Einrückungen der Leerzeilen
// TK: Ich bin den Artikel durchgegangen. Sollte jetzt ok sein. Aber 4 Augen sehen mehr...
...
// TK: siehe oben. Wenn Du noch was findest, bitte korrigieren.
.~/local/lib/check_mk/base/plugins/agent_based/myhostgroups.py
[{python}]
----
register.check_plugin(
...
Immer, wenn wir ein Item gefunden haben, geben wir das mit `yield` zurück, wobei ein Objekt vom Typ `Service` erzeugt wird, das den Host-Gruppennamen als Item übergeben bekommt.
...
`state` legt den Zustand des Services fest (im Beispiel `OK`) und `summary` den Text, der in dem [.guihint]#Summary# des Services angezeigt wird.
...
----
    ...
    yield from check_levels(hosts_up_perc, levels_lower=(90.0, 80.0), label="UP hosts", notice_only=True)
----
...
[{python}]
----
    services_ok_perc = 100 * num_services_ok / num_services
    yield from check_levels(services_ok_perc, levels_lower=(90.0, 80.0), label="OK services", notice_only=True)
----
...
// MFS: Ja!
// TK: Kannst Du das schreiben? Ich bin nicht sicher, ob ich das richtig verstanden habe.
...
// TK: Hab einen Satz weiter unten hinzugefügt: Durch diese Erweiterung ...

[{python}]
----
    yield from check_levels(hosts_up_perc, levels_lower=(90.0, 80.0), metric_name="hosts_up_perc", label="UP hosts", boundaries=(0, 100), notice_only=True)
----
...
----
    ...
    services_ok_perc = 100 * num_services_ok / num_services
    yield from check_levels(services_ok_perc, levels_lower=(90.0, 80.0), metric_name="services_ok_perc", label="OK services", boundaries=(0, 100), notice_only=True)
----
...
// TK: Ich find das didaktisch richtig im Kontext des Beispiels:
// TK: Wir haben mit check_levels aufgehört und nehmen das jetzt sofort wieder auf, wenns um Metriken geht.
// TK: Dass man Metriken auch ohne Funktion zurückgeben kann, wird ja nicht verschwiegen, kommt nur später.
// TK: Man kann es natürlich auch so machen, wie Du vorschlägst, dafür müsste man aber alles umschreiben und auch den Screenshot neu ziehen.
...
// TK: Hinweis aufgenommen am Ende der Einleitung (auch im Metrik-Kapitel weiter unten). 
...
// TK: Und was importiere ich da jetzt gleich? rulespec_registry + CheckParameterRulespecWithItem sind Funktionen. Was ist RulespecGroupCheckParametersApplications?
...
// TK: Sehr gut - aaaber: Die haben dann nix mit den check_default_parameters von unten zu tun, oder? Die muss man dann manuell konsistent halten?
// TK: Und der Screenshot muss auch neu gezogen werden...
////
...
// TK: Jetzt habe ich oben aber schon die ganzen Schwellwerte auf float geändert. Heisst das, hier muss Float() statt Integer() verwendet werden?
...
// MFS: Wirkt sich doch nicht aus, oder? Um die Änderungen testen zu können, muss ich wie gehabt den Apache neustarten. Oder hab ich was falsch verstanden?
...
// TK: Ok, hab jetzt aber auf omd restart als Abkürzung verzichtet.
...
// TK: hier: done, sonst auch (hoffentlich)
...
Parameters({'hosts_up_lower': (70, 60), 'services_ok_lower': (75, 65)})
----
// TK: Die Beispielausgaben sehen dann mit Float wohl auch anders aus.
...
register.check_plugin(
    ...
    discovery_function = discover_myhostgroups_advanced,
    check_function = check_myhostgroups_advanced,
    check_default_parameters = {"hosts_up_lower": (90.0, 80.0), "services_ok_lower": (90.0, 80.0)},
    check_ruleset_name = "myhostgroups_advanced",
)
----
...
// TK: Frage von oben damit beantwortet :-)
...
// TK: Habs mir fast gedacht: Frage: Will man das?
...
// TK: Einen Satz weiter unten hinzugefügt (Wählen Sie einen eindeutigen Namen, ...). Überschrieben wird ja nix, nur verschattet (wie es bei CMK sonst so schön heisst).
...
// TK: Echt? Du meinst, hier muss stehen, dass man Sekunden nicht als Float eingeben darf? 
...
// TK: Nunja, Oktette hab ich mir nicht ausgedacht. Dann lassen wir Oktette weg. Bytes pro Sekunde reicht doch aus.
...
// TK: Ich verstehe nicht, was Du meinst. Einfach so ändern, dass es stimmt ;-)
...
Die Frage ist, wie Sie reagieren sollen, wenn die Ausgaben vom Agenten nicht die Form haben, die Sie eigentlich erwarten -- egal ob vom {CMK}-Agenten oder per xref:draft_devel_check_plugins_snmp#[SNMP] erhalten.
