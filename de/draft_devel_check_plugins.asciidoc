...
// TK: Die nächste Zeile muss noch in die Hunspell Dicts
// IGNORE C64 Gibibyte IEC Kibibyte Kibibytes Mebibyte Netzwerker Oktette Reißverschlussverfahren Tebibyte ValueSpec aufmultiplizieren gettext herauslaufen zurückgibt
...
// MFS: Hir viellecht noch zwei print() einbauen und darauf verweisen, dass diese dann später bei Aktivierung sichtbar werden?
////
...
Weiter unten finden Sie mehr Informationen zu xref:existing_rule_set[vorhandenen Regelsätzen.]
...
Im Weiteren schreiben Sie dann z. B. anstelle von `"Threshold for Warning"` ein `pass:[_]("Threshold for Warning")` für den Funktionsaufruf.
...
Wenn Sie die Datei für den Regelsatz angelegt haben, sollten Sie erstmal ausprobieren, ob alles soweit funktioniert -- noch ohne Verbindung zum Check-Plugin.
Dazu müssen Sie erstmal den Apache der Instanz neu starten, damit die neue Datei gelesen wird.
...
Dazu gibt es z. B. bei der Service-Erkennung eines Hosts, auf der Seite [.guihint]#Services of host#, im Menü [.guihint]#Display# den Eintrag [.guihint]#Show check parameters.#
...
Das kann eigentlich nur dann der Fall sein, wenn der Check etwas prüft, für das {CMK} in gleicher Form bereits Check-Plugins hat, z. B. das Überwachen einer Temperatur oder anderer mit Sensoren gemessener Werte.
...
// TK: Ursprüngliche Idee: Die noch übrigbleibenden Themen in das Kapitel Weitere Komponenten der Check-Entwicklung" zu stecken.
// TK: Das finde ich bei momentan nur 2 Themen (Metriken für Experten und Zahlen) überstrukturiert.
...
Vorteil an dieser Darstellung ist, dass sich damit sehr einfach rechnen lässt, also z. B. den Zeitraum, wenn Start- und Endzeit bekannt sind.
...
Einsatzgebiet ist z. B. die Taktrate einer CPU:
...
Da Computer so etwas meist in Zweierpotenzen organisieren, also z. B. in Einheiten zu 512, 1024 oder 65 536 Bytes, hatte sich dabei von Beginn an eingebürgert,
