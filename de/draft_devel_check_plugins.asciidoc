...
Im beschriebenen Beispiel - einer einfachen Überwachung innerhalb von {CMK} - gehen wir von einem Unternehmen mit mehreren Standorten aus:
...
// MFS: Wollen wir uns wirklich auf den Befehl konzentrieren oder ein wenig allgemeiner auf die Informationen, die wir einsammeln wollen? Also eher direkt: Informationen fürs Monitoring auslesen
...
// MFS: Ich würde sogar noch weiter gehen und die Zwischenüberschrift ändern zu: "Informationen auslesen und filtern"
...
// MFS: SNMP hier besser komplett weglassen, da hierfür keine Agent Kompononte nötig. 
// Auch Programmiersprachen würde ich hier keine erwähnen. 
// Eher darauf hinweisen, dass die Programmiersprache weitgehend beliebig ist: Wenn Anwendungsentwickler Plugins schreiben, 
// Wichtig ist nur, dass man an die gewünschten Daten rankommt.
// Zudem verwenden wir hier ja eigentlich LQL und nicht Shell.
...
// MFS: diese beiden Sätze würde ich komplett streichen. 
// Ausgangsituation ist doch eher, dass ein Nutzer bereits weiss, wie er auf der Kommandozeile an bestimmte Rohdaten kommt.
// Hier geht es eher um das Filtern und die Integration in den Agenten.
...
// MFS: Nicht "hat", präziser: "im flüchtigen Speicher vorhält"...
...
// MFS: Das ist auch in Kombination etwas schwammig. Ich würde die Dinge beim Namen nennen, etwa so:
...
// MFS: Springen in der Ansprache "für uns aktuell nicht relevant" ... "Also schränken Sie"
// MFS: Hier wäre sinnvoll darauf hinzuweisen, dass viele verschiedene Parameter entscheiden, ob es sinnvoll ist, eine Vorfilterung auf dem Host vorzunehen (Umfang der reduzierten Daten, Aufwand, evtl. braucht man sie doch später noch...)
// MFS: Wir filtern hier, weil es erstens in LQL leicht möglich ist und zweitens, weil es hilft, den Überblick zu bewahren.
...
// MFS: Eher "Ein für unser Beispiel ausreichender Name für das neue Check-Plugin,..."
...
// MFS: Sollen wir darauf verweisen, dass hier noch der Test mit 
// time /usr/lib/check_mk_agent/plugins/myhostgroups
// sinnvoll ist?
// Läuft das eigene Plugin länger als ein paar Sekunden, sollte man drüber nachdenken, es asynchron auszuführen.
...
from .agent_based_api.v1 import check_levels, Metric, register, Result, Service, State
----
// MFS: Wir sortieren alphabetisch.
// Sollten wir nach Reihenfolge der Benutzung umsortieren?
...
// MFS: Hat hier auch nix mit Compiler oder Interpreter zu tun, es geht darum, innerhalb von Checkmk Daten für die Verwendung in Checkmk aufzubereiten.
...
// MFS: Nein, mit Kommas und Anführungszeichen ist nur die menschenlesbare Darstellung.
...
// TK: ~/local am Pfadanfang eingefügt, import gelöscht, denn das wurde ja schon vorher erklärt.
// MFS: Auch Leerzeilen müssen korrekt eingerückt sein.
.~/local/lib/check_mk/base/plugins/agent_based/myhostgroups.py
[{python}]
----
def parse_myhostgroups(string_table):
...
// MFS: Bei Checks, die pro Host nur einen Service erzeugen...
