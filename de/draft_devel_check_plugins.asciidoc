...
// SP: Was sollte ich noch ergänzen? Soll ja auch nicht zu langatmig werden.
...
Hinweis: Sie könnten auch `echo` verwenden, z.B. falls die Variante mit `su` und `lq` aus irgendwelchen Gründen nicht funktionieren sollte. Lesen Sie bei Bedarf hierzu xref:livestatus#network[Livestatus über das Netzwerk nutzen.]
...
Ein lauffähiges Grundgerüst, das Sie im Folgenden Schritt für Schritt weiter ausbauen werden, sieht so aus:
...
Dabei ist die gute Nachricht, dass die Check-API hinsichtlich der Fehlerbehandlung Ihnen bereits viel Arbeit abnimmt.
...
Tritt eine Exception im normalen Monitoring oder während der Serviceerkennung im Setup auf, enthält das _Summary_ Hinweise auf den eben erstellten Absturzbericht (_crash report_).
...
Der Traceback hilft Ihnen als Programmierer zu entscheiden, ob ein Fehler im Programm vorliegt (beispielsweise der Aufruf einer nicht vorhandenen Funktion) oder Agentendaten, die nicht wie erwartet verarbeitet werden konnten.
...
Wenn Sie ihr Check-Plugin auf der Kommandozeile ausführen, erhalten Sie keinen Hinweis auf die ID des erzeugten Absturzberichtes.
...
Tritt der Fehler beim Aufruf mit `--debug` beim nächsten Mal nicht mehr auf – zum Beispiel, weil eine neue Agentenausgabe bereitsteht –, können Sie die letzten Crash-Reports auch im Dateisystem einsehen:
...
. `agent_output` enthält die vollständige Agentenausgabe, die zum Zeitpunkt des Crashes aktuell war.

[#error_invalid_agent]
=== Ungültige Agentenausgabe

Die Frage ist, wie Sie reagieren sollen, wenn die Ausgaben vom Agenten nicht die Form haben, die Sie eigentlich erwarten -- egal ob vom {CMK}-Agenten oder per SNMP erhalten.
...
Erstellen Sie dann eine Datei im Verzeichnis `/var/lib/check_mk_agent/spool`, welche die von Ihrem Check-Plugin erwartete Agentensektion (oder _erwarteten Agentensektionen_) inclusive Header enthält und auf Newline endet.
