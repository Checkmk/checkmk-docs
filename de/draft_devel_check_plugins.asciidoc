...
In diesem Fall sieht der Sektions-Header so aus:
...
Die Variable `COLUMNS`, die aktuell die Gruppennamen und die zugehörigen Mitglieder enthält. Sowie die Variable `SITE`, die den Namen der {CMK}-Instanz enthält.
...
Da die Spalten der Tabelle mit dem Semikolon getrennt werden, legen Sie im Sektions-Header mit dem Zusatz `sep(59)` fest, dass das Semikolon als Trennzeichen (_separator_) für die Daten in der Agentenausgabe genutzt wird. 59 steht für das ASCII-Zeichen Nummer 59, also das Semikolon. Ohne diesen Zusatz würde als Standard das Leerzeichen (ASCII-Zeichen 32) als Trennzeichen verwendet werden.
...
{CMK} weiß nur, dass das Item, das vorher da war, nun weg ist -- und genau das steht in der Meldung.
...
Dazu erweitern wir die `if`-Abfrage um den `else`-Zweig:
...
So haben nunmehr wir in der Check-Funktion den Fall der verschwundenen Host-Gruppen übernommen.
...
...
Der Unterstrich zeigt an, dass uns die beiden andere Spalten
...
...
Jetzt gehen wir wieder alle Zeilen der Reihe nach
...
Daneben gibt es noch das Feld [.guihint]#Details,# in dem alle Details zum Zustand angezeigt werden, wobei die Idee ist, dass alle Informationen des Summary auch in den Details enthalten sind.

Im Monitoring wird in der Service-Übersicht, auf der Seite [.guihint]#Services of Host,# nur die Summary angezeigt:
