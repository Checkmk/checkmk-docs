...
`\n` schreibt jede Host-Gruppe in eine neue Zeile. Hier existieren aktuell drei Host-Gruppen, zwei Niederlassungs-Gruppen (die bereits früher angelegt wurden) sowie die Gruppe check_mk. Diese enthält zwei Hosts namens SQL-Server und localhost.
...
auf (z.B. als `localhost`) und rufen Sie die Agentendaten dann mit `cmk -d` ab.
...
dafür gemacht? Sie haben ein dreizeiliges Skript mit dem Pfad
...
Eine Ausgabe Ihrer Hostgruppen-Informationen sähe in etwa so aus:
[{shell}]
----
[["check_mk", "myhost1,myhost2,myhost3,myhost4"],["foo", "myhost11,myhost22,myhost33,myhost44"]]
----

Es wird ein einziger langer Datensatz ausgelesen. Jede eckige Klammer enthält die Informationen zu einer Hostgruppe: erst den Namen der Hostgruppe in Anführungszeichen, dann dahinter (durch ein Komma abgetrennt) in weiteren Anführungszeichen die zugehörigen Hosts (ebenfalls durch Kommas getrennt). Diese Informationen können Sie zwar alle ansprechen, aber jeweils nur über ihre Position im Datensatz. Bei größeren Datenmengen gestaltet sich dies komplex.
...
Da das im xref:agentbased[vorherigen Kapitel] erstellte Check-Plugin `myhostgroups` zur Überprüfung der Standardgruppe `check_mk` weiterhin funktionieren soll, bleibt dieses Check-Plugin so, wie es ist.
...
Wenn dann später der Host überwacht wird, dann wird die Check-Funktion für jeden Service -- und damit für jedes Item -- separat aufgerufen.
...
Wenn eine Host-Gruppe leer ist oder nicht (mehr) existiert, dann soll der zugehörige Service {CRIT} anzeigen.
In allen anderen Fällen wird der Service auf {OK} gesetzt und alle Hosts der Host-Gruppe werden in der [.guihint]#Summary# des Services aufgelistet.
Die komplette Funktion dazu:

[{python}]
----
def check_myhostgroups_advanced(item, section):
    attributes = section.get(item, False)
    if not attributes:
        yield Result(state=State.CRIT, summary="Group is empty or has been deleted")
    else:
        yield Result(
            state=State.OK, summary=f"Hosts in this group: {attributes['hosts']}"
        )
----
