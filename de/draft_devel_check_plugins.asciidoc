...
[#tools]
== Die Werkzeuge für die Programmierung
...
Und trotzdem kann es immer wieder vorkommen, dass ein Gerät, eine Anwendung oder einfach nur eine bestimmte Metrik, die für Sie wichtig ist, noch von keinem dieser Plugins erfasst ist 
...
Welche Möglichkeiten haben Sie also, hier dennoch eine sinnvolle Überwachung selbst zu implementieren?
Nun -- natürlich können Sie sich an unseren link:https://checkmk.com/de/produkt/support[Support^] wenden und ein geeignetes Plugin entwickeln lassen.
Aber Sie können sich auch selbst helfen.
...
Check-Plugins werten die Daten aus, die die xref:glossar#agent[Monitoring-Agenten] bereitstellen.
...
Einen xref:glossar#special_agent[Spezialagenten] benötigen Sie, wenn Sie weder mit dem normalen {CMK}-Agenten noch per SNMP an die Daten kommen, welche für das Monitoring relevant sind.
...
// TK: Die Artikelliste schreckt eher ab (sie ist eh unter Related Articles verfügbar). Stattdessen die Artikel dort verlinken, wo's gerade nötig ist.
////
...
//SP: "Ein erstes einfaches Check-Plugin" wird aktuell durch Silke bearbeitet
[#agentbased]
== Ein erstes Check-Plugin schreiben

Nach dieser langen Einleitung wird es Zeit, dass wir unser erstes einfaches
Check-Plugin programmieren. Als Einstieg dient eine einfache Überwachung
innerhalb von Checkmk:

Im folgenden Beispiel gehen wir von einem Unternehmen mit mehreren Niederlassungen aus. Jede dieser Niederlassungen wird in Checkmk durch eine xref:glossar#host_group[Host-Gruppe] abgebildet. Jede Niederlassung hat vor Ort ein eigenes Service-Team. Damit bei Problemen jeweils das richtige Service-Team verständigt werden kann, gilt, dass jeder Host einer Niederlassung - also auch einer Host-Gruppe - zugewiesen sein muss. Ziel unseres Beispiels ist nun, eine Prüfung aufzusetzen mit der kontrolliert werden kann, dass für keinen Host vergessen wurde, eine Host-Gruppe zuzuweisen.
...
Am Anfang jeder Check-Programmierung steht: die Recherche! Das bedeutet, dass Sie herausfinden müssen, wie Sie überhaupt an die Informationen kommen, die Sie für die Überwachung brauchen. Bei Linux sind das oft Kommandozeilenbefehle, bei
Windows hilft die PowerShell, VBScript oder WMI und bei xref:snmp#[SNMP] müssen Sie die
...
Vorgehen. Eine ausführliche Beschreibung verschiedener Möglichkeiten und Herangehensweisen würde für diesen Einstieg zu weit führen.

Für das gewählte Beispiel genügt zunächst ein xref:livestatus#[Abruf der Statusdaten via Livestatus,] also der in Tabellen organisierten Strukturdaten von Checkmk. Dazu dient der `lq`-Befehl in Kombination mit einem `GET`-Befehl. `lq` steht für _Livestatus Query Language_ und aktiviert die Livestatusabfrage in Checkmk, das nachfolgende `GET` holt die dahinter spezifizierten Informationen ab. 
...
;check_mk;SQL-Server,myself;SQL-Server|0|1,myself|0|1;check_mk;;;2;0;0;0;0;0;2;61;2;0;2;56;0;3;56;0;5;0;3;0;2;2
----

Die erste Zeile der Ausgabe enthält die Spaltennamen der abgefragten Tabelle hostgroups. Als Trennzeichen fungiert der Strichpunkt. Inder zweiten Zeile folgen dann, ebenfalls durch Strichpunkte getrennt, die Inhalte sämtlicher Spalten. 

Die Ausgabe ist bereits für dieses kleine Beispiel relativ unübersichtlich und enthält Informationen, die für uns aktuell nicht relevant sind. Also schränken Sie die Abfrage ein - auf die Gruppennamen und die darin befindlichen Hosts:
...
`\n` schreibt jede Host-Gruppe in eine neue Zeile. Hier existiert aktuell nur eine Host-Gruppe, die Gruppe check_mk. Diese enthält zwei Hosts namens SQL-Server und myself.

Diese Host-Gruppe check_mk stellt eine Besonderheit innerhalb der Host-Gruppen dar. Sie haben diese nämlich nicht selbst angelegt. Und Sie können auch keinen Host aktiv in diese Gruppe aufnehmen. Woher also stammt diese Host-Gruppe? Da in Checkmk per Definition jeder Host einer Gruppe angehören muss, weist Checkmk jeden Host, den Sie keiner Gruppe zuweisen, der "imaginären" Gruppe check_mk zu. Sobald Sie den Host einer Ihrer eigenen Host-Gruppen zuweisen, wird er aus der Gruppe check_mk entfernt. Hier ist also, im Gegensatz zu Ihren selbst angelegten Gruppen, keine Zuweisung zu mehreren Host-Gruppen möglich.

Genau diese Eigenschaften der Gruppe check_mk werden nun für unser Beispiel genutzt. Da jeder Host einer Niederlassung zugeordnet sein soll, müsste die Host-Gruppe check_mk leer sein. Ist sie nicht leer, so besteht Handlungsbedarf, sprich darin befindliche Hosts müssen den Host-Gruppen und damit den Niederlassungen zugeordnet werden.


[#includecommand]
=== Den Befehl in den Agenten einbauen

Bis jetzt haben Sie sich mit dem `lq`-Befehl die Informationen nur im Terminal anzeigen lassen. Das ist hilfreich, um sich einen Einblick in die Daten zu verschaffen.

Damit Sie diese Daten vom {CMK}-Server aus abrufen können, muss der
neue Befehl Teil vom {CMK}-Agenten auf dem überwachten System werden. Sie könnten nun direkt die Datei `/usr/bin/check_mk_agent` im {CMK}-Agenten editieren
und das einbauen. Das hätte aber den Nachteil, dass Ihr neuer Befehl bei einem Softwareupdate des Agenten wieder verschwindet, weil die Datei ersetzt wird.
...
Und noch eins ist wichtig: Die Daten können nicht einfach so
ausgegeben werden. Sie brauchen noch einen *Sektions-Header* (_section header_).
Das ist eine speziell formatierte Zeile, in der der Name des
...
Ein eindeutiger Name für das Beispiel-Plugin, der alle Kriterien erfüllt, ist zum Beispiel `myhostgroups`. In diesem Fall muss der Sektions-Header so aussehen:
...
Den können Sie einfach mit `echo` ausgeben. Ergänzen Sie dann den
„Shebang“ (das ist eine Abkürzung für _sharp_ und _bang_, wobei letzteres
eine Abkürzung für das Ausrufezeichen ist), an dem Linux erkennt, dass es
das Skript mit der Shell ausführen soll, dann sieht das Plugin
so aus:
...
Um das Skript erweiterbar zu halten, werden Variablen verwendet. Die Variable COLUMNS, die aktuell die Gruppennamen und die zugehörigen Mitglieder enthält. Sowie die Variable SITE, die die Checkmk-Instanz enthält.

Mit dem ersten echo-Befehl rufen Sie die Informationen zu den Host-Gruppen auf, die wir uns oben mit dem ersten lq-Befehl bereits angesehen hatten. Das nachfolgende `sep(59)` gibt an, dass die einzelnen Spalten der Tabelle hostgroups durch das ASCII-Zeichen Nummer 59, also den Strichpunkt, getrennt sind.

Der zweite echo-Befehl `echo -e` holt aus der Tabelle "hostgroups" die Spalten "name" und "members". Diese werden mit dem Skript `unixcat` in die Datei "live" geschrieben. 

Eines ist noch sehr wichtig: Machen
...
Sie können das Plugin direkt von Hand ausprobieren, indem Sie den
...
. Plugin solo ausprobieren. Das haben Sie gerade gemacht.
...
////
SP: noch zu klären: Brauche ich das?
...
dafür gemacht? Sie haben lediglich ein dreizeiliges Skript mit dem Pfad
`/usr/lib/check_mk_agent/plugins/myhostgroups` erzeugt und ausführbar
gemacht!

Alles was nun folgt, geschieht nur noch auf dem {CMK}-Server: Dort schreiben Sie das eigentliche Check-Plugin.
...
//SP: Das hier muss noch überarbeitet werden.
Im einfachsten Fall überspringen Sie das explizite deklarieren der Sektion.
Wenn Sie eine Parse-Funktion implementieren möchten (wozu Ihnen professionelle
Entwickler immer raten würden), finden Sie im xref:devel_check_plugins#parsefunction[Abschnitt über Parse-Funktionen]
mehr Informationen.
...
annehmen soll. Da Ihr Check keine Parameter hat und es auch immer nur
...
xref:devel_check_plugins#parsefunction[Parse-Funktion] definiert haben, zerlegt
...
def check_myhostgroups(section):
    for group in section:
        if group == "check_mk":
            yield Result(
                state=State.CRIT,
                summary=f"Default group is not empty: {section['check_mk']['hosts']}",
            )
            return
...
Anzahl der Hosts pro Gruppe (Spalte `num_hosts`), Anzahl der Services aller Hosts in der Gruppe (`num_services`) und Anzahl der Services im Zustand {OK} (`num_services_ok`).

Jetzt müssen diese neuen Spalten nur noch vom Agenten geliefert werden.
Das erreichen wir durch Erweiterung des im vorherigen Kapitel erstellten xref:includecommand[Agentenplugins.]

Editieren Sie als root-Benutzer das Skript des Agentenplugins.
Da das Skript die konfigurierbaren Werte bereits in Variablen gesteckt hat, reicht es aus, nur die mit `COLUMNS` beginnende Zeile zu ändern und dort die zusätzlichen Spalten einzutragen:
...
COLUMNS="name members num_hosts num_services num_services_ok"
SITE="mysite"
...
[{shell-raw}]
----
{c-root} /usr/lib/check_mk_agent/plugins/my_hostgroups
<<<my_hostgroups:sep(59)>>>
check_mk;localhost;1;61;52
Munich;checkmk.com;1;1;1
Oberammergau;internet,microcontroller;2;5;2
----

Am Ende jeder Zeile stehen nun, durch Semikolon abgetrennt, die drei neuen Werte.

Mit dieser Änderung liefert das Agentenplugin nun andere Daten als vorher.
An dieser Stelle ist es wichtig, sich zu vergewissern, dass die xref:check_function[Check-Funktion] des Check-Plugins auch mit den geänderten Daten immer noch das tut, was sie soll.

Die vorgenommene Erweiterung ist aber unkritisch, denn die Daten werden in der gleichen Struktur geliefert wie vorher.
Es werden nur zusätzliche Spalten bereitgestellt, die in der Check-Funktion (noch) gar nicht ausgewertet werden.

Nachdem das Agentenplugin aktualisiert ist, können Sie sich wieder als Instanzbenutzer anmelden.
Alles weitere wird durch Änderung des Check-Plugins erledigt.
...
Ein sehr üblicher Fall ist aber natürlich auch, dass es von einem Check mehrere Services auf einem Host geben kann.
...
Das ergibt dann als Service-Name dann z.B. `Filesystem /var` oder `Filesystem C:`.
...
Um das ganze praktisch auszuprobieren, werden wir für jede existierende Host-Gruppe einen eigenen Service erzeugen.

Zur Erinnerung:
{CMK} zerlegt die Zeilen der vom Agentenplugin gelieferten Sektion anhand des Trennzeichens (im Beispiel `;`) in eine Liste von Zeilen, die ihrerseits wiederum Listen von Worten sind.

Die vom Agenten gelieferten Beispieldaten:
...
sehen in {CMK} dann so aus:

[{python}]
----
[['check_mk', 'localhost', '1', '61', '52'], ['Munich', 'checkmk.com', '1', '1', '1'], ['Oberammergau', 'internet', 'microcontroller', '2', '5', '2']]
----
