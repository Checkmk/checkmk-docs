...
[#real_plug-in]
=== Checks erstellen
// TK: Tue mich extrem schwer, eine kurze Überschrift zu finden für den Inhalt der Tabelle...
// TK: Der Begriff "Checks" ist (noch) nicht eingeführt. Stört das hier?
// TK: Hieß früher „Muss es immer ein Check-Plugin sein?“ Vielleicht ist der doch besser als der neue.
...
////
MFS: Das gesamte Kapitel 1 taugt wenig als Heranführung, gehört mit besseren Entscheidungshilfen in einen eigenen Artikel umgewandelt.
Aber das hatten wir schon.
IMHO sollte das zeitnah erfolgen, auch weil so hier deutlich herausgehoben werden muss, wie der arbeitsteilige Aufbau des Plugins ist:

1. Das Agenten-Plugin sammelt in der Regel nur die Daten ein, die später ausgewertet werden. Eine detaillierte Auswertung findet hier nicht statt. Allenfalls eine Vorfilterung ist üblich. Möglicherweise liegen die Daten aber auch schon vor, bspw. als CSV-Datei im Dateisystem, dann genügt es, die zu übertragen.
2. Das Plugin auf dem Checkmk-Server wertet diese Daten aus.
////
...
// MFS: Zur ersten Spalte evtl. ergänzen, dass Prefixing sehr häufig genutzt wird und auch in der Liste deutlich zu sehen ist, bspw. Herstellername (Hardware), Programmname (Software), Firmenname (intern genutzte Checks), Betriebssystemname...
...
// MFS: Meines Wissens nicht nur das Leerzeichen, sondern auch Vielfache, die von \s gematcht werden.
...
// MFS: "...welche Namensräume so verfügbar gemacht wurden..."
...
(Python beginnt mit der Zählung bei Null, das heißt das erste Element ist Element 0.)
...
// MFS: Wir arbeiten hier auf dem Dictionary, also "holt den zum Schlüssel check_mk gehörenden Wert in die Variable attr"...   
Die Funktion `check_myhostgroups()` holt als erstes alle Inhalte aus der Zeile, die mit `check_mk` beginnt in die Variable `attr`.
// MFS: Auch hier "Schlüssel-Wert"!
...
// MFS: Erklärung ist korrekt. Aber die Entwicklung mag F-Strings nicht, weil darin Funktionen aufgerufen werden. str.format()? https://realpython.com/python-f-strings/#option-1-formatting
