...
// TK: Im gesamten gereviewten Text geändert: 1 Satz pro Zeile

Nach dieser langen Einleitung wird es Zeit, dass wir unser erstes einfaches Check-Plugin programmieren.
Als Einstieg dient eine einfache Überwachung innerhalb von Checkmk:
// TK: Checkmk > {CMK}, auch im folgenden öfter
// TK: Hier deutlicher machen, dass wir im Bsp. davon ausgehen, dass jeder, der sich für die Programmierung mit CMK interessiert, einen CMK-Server aufgesetzt hat und diesen als Host auch überwacht.
// TK: Im Bsp sind Host + CMK-Server identisch, denn nur dann können wir über den Host via Livestatus-Abfragen die Informationen ermitteln, die wir benötigen.

Im folgenden Beispiel gehen wir von einem Unternehmen mit mehreren Niederlassungen aus.
// TK: Niederlassung > Standort?
Jede dieser Niederlassungen wird in Checkmk durch eine xref:glossar#host_group[Host-Gruppe] abgebildet.
Jede Niederlassung hat vor Ort ein eigenes Service-Team.
Damit bei Problemen jeweils das richtige Service-Team verständigt werden kann, gilt, dass jeder Host einer Niederlassung - also auch einer Host-Gruppe - zugewiesen sein muss.
Ziel unseres Beispiels ist nun, eine Prüfung aufzusetzen mit der kontrolliert werden kann, dass für keinen Host vergessen wurde, eine Host-Gruppe zuzuweisen.
...
Eine ausführliche Beschreibung verschiedener Möglichkeiten und Herangehensweisen würde für diesen Einstieg zu weit führen.
// TK: Besser so: Kommando hängt vom Problem ab. Wir nutzen im Bsp. die Tatsache, dass Host gleich CMK-Server ist, um per Livestatus an die Daten der Host-Gruppen zu kommen.

Für das gewählte Beispiel genügt zunächst ein xref:livestatus#[Abruf der Statusdaten via Livestatus,] also der in Tabellen organisierten Strukturdaten von Checkmk.
// TK: Strukturdaten? Wasistdas? Besser so: aktuellen Daten, die {CMK} über die überwachten Hosts und Services hat.

Dazu dient der `lq`-Befehl in Kombination mit einem `GET`-Befehl.
`lq` steht für _Livestatus Query Language_ und aktiviert die Livestatus-Abfrage in Checkmk, das nachfolgende `GET` holt die dahinter spezifizierten Informationen ab. 
// TK: `lq` steht für _Livestatus Query_

Öffnen Sie eine Kommandozeile für Ihre Checkmk-Instanz und fragen Sie die Informationen zu den Host-Gruppen ab:
// TK: Melden Sie sich als Instanzbenutzer an ... 
...
// TK: Formatieren als `hostgroups`, wie auch später `check_mk`, `SQL-Server`, `localhost`, `COLUMNS`, `SITE`, `name`, `members`, `echo`, `lq`

Als Trennzeichen fungiert der Strichpunkt.
In der zweiten Zeile folgen dann, ebenfalls durch Strichpunkte getrennt, die Inhalte sämtlicher Spalten.
// TK: Etwas kompliziert beschrieben
// TK: Strichpunkt > Semikolon?
...
Also schränken Sie die Abfrage ein - auf die Gruppennamen und die darin befindlichen Hosts:
// TK: Hosts befinden sich in der Gruppe, nicht im Namen
...
`\n` schreibt jede Host-Gruppe in eine neue Zeile.
// TK: \n ist nicht für die Ausgabe sondern für die Kommandoeingabe. Eher so:
// Die Livestatus-Schnittstelle erwartet alle Befehle und Header in jeweils einer eigenen Zeile. Markieren Sie daher jeden Zeilenumbruch in einer mehrzeiligen Eingabe mit einem `\n`.
Hier existieren aktuell drei Host-Gruppen, zwei Niederlassungs-Gruppen (die bereits früher angelegt wurden) sowie die Gruppe check_mk.
...
Da in Checkmk per Definition jeder Host einer Gruppe angehören muss, weist Checkmk jeden Host, den Sie keiner Gruppe zuweisen, der "imaginären" Gruppe check_mk zu.
// TK: imaginär: Hmm, das Ding existiert ja doch: weist {CMK}} jeden Host beim Erstellen der speziellen Gruppe `check_mk` zu.

Sobald Sie den Host einer Ihrer eigenen Host-Gruppen zuweisen, wird er aus der Gruppe check_mk entfernt.
Hier ist also, im Gegensatz zu Ihren selbst angelegten Gruppen, keine Zuweisung zu mehreren Host-Gruppen möglich.
// TK: Unklar formuliert: Was heisst "Hier"? Vielleicht so ähnlich: `check_mk` realisiert eine Einbahnstraße aus der es nur einen Weg heraus, aber keinen hinein gibt.
...
Bis jetzt haben Sie sich mit dem `lq`-Befehl die Informationen nur im Terminal anzeigen lassen.
// TK: nur im Terminal anzeigen lassen > nur auf der Kommandozeile ausführen lassen
Das ist hilfreich, um sich einen Einblick in die Daten zu verschaffen.

Damit Sie diese Daten vom {CMK}-Server aus abrufen können, muss der neue Befehl Teil vom {CMK}-Agenten auf dem überwachten System werden.
// TK: System > Host
Sie könnten nun direkt die Datei `/usr/bin/check_mk_agent` im {CMK}-Agenten editieren und das einbauen.
// TK: Sie könnten nun direkt den {CMK}-Agenten in der Datei `/usr/bin/check_mk_agent` editieren ....
...
Das ist eine speziell formatierte Zeile, in der der Name des neuen Checks steht.
// TK: Wieso Check - und nicht, wenn schon, Check-Plugin? Außerdem werkeln wir doch gerade am Agentenplugin?
// TK: Ich find das voll verwirrend, wie hier Agentenplugin, Sektions-Header, Check und Check-Plugin durcheinandergehen.
// TK: Am einfachsten ist es, wenn Sektions-Header und Check-Plugin gleich heißen - ist aber nicht verpflichtend.
// TK: Das ist aber der Grund, warum im alten Text die Namen der Check-Plugins durchgeorgelt werden, um einen Namen zu finden, den man dann als Sektions-Header einträgt.
An diesem Sektions-Header kann {CMK} später erkennen, wo die Daten des Plugins beginnen und die des vorherigen aufhören.

Also brauchen Sie jetzt erst einmal einen sinnvollen Namen für Ihren neuen Check.
// TK: für Ihr neues Check-Plugin.
...
Ein eindeutiger Name für das Beispiel-Plugin, der alle Kriterien erfüllt, ist zum Beispiel `myhostgroups`.
// TK: Ein eindeutiger Name für das neue Check-Plugin ...
In diesem Fall muss der Sektions-Header so aussehen:
// TK: In diesem Fall sieht dann der Sektions-Header so aus:
...
// TK: Vorschlag: 
// TK: 1) Nun haben Sie alle Informationen zusammen, um das Skript mit dem Agentenplugin zu erstellen. Legen Sie eine neue Datei `myhostgroups` im Verzeichnis `/usr/lib/check_mk_agent/plugins` an:
// TK: 2) Dateiinhalt zeigen
// TK: 3) Dann den Inhalt Zeile für Zeile durchgehen.

Den können Sie einfach mit `echo` ausgeben.
Ergänzen Sie dann den „Shebang“ (das ist eine Abkürzung für _sharp_ und _bang_, wobei letzteres eine Abkürzung für das Ausrufezeichen ist), an dem Linux erkennt, dass es das Skript mit der Shell ausführen soll, dann sieht das Plugin so aus:
...
Um das Skript erweiterbar zu halten, werden Variablen verwendet.
Die Variable COLUMNS, die aktuell die Gruppennamen und die zugehörigen Mitglieder enthält. Sowie die Variable SITE, die die Checkmk-Instanz enthält.
// TK: Das sind keine Sätze.
// TK: Die Variable SITE enthält nicht die Checkmk-Instanz, sondern deren Namen.

Mit dem ersten echo-Befehl rufen Sie die Informationen zu den Host-Gruppen auf, die wir uns oben mit dem ersten lq-Befehl bereits angesehen hatten.
// TK: Nein: Mit dem ersten echo-Befehl geben Sie den Sektions-Header in der vorgeschriebenen Syntax aus.

Das nachfolgende `sep(59)` gibt an, dass die einzelnen Spalten der Tabelle hostgroups durch das ASCII-Zeichen Nummer 59, also den Strichpunkt, getrennt sind.
// TK: Besser motivieren: 
// TK: Da die Spalten der Tabelle mit dem Semikolon getrennt werden, legen wir im Sektions-Header mit dem Zusatz `sep(59)` fest, dass das Semikolon als Trennzeichen (_separator_) für die Daten in der Agentenausgabe genutzt wird.
// TK: 59 steht für das ASCII-Zeichen Nummer 59, also das Semikolon.
// TK: Ohne diesen Zusatz würde als Standard das Leerzeichen (ASCII-Zeichen 32) als Trennzeichen verwendet werden.

Der zweite echo-Befehl `echo -e` holt aus der Tabelle "hostgroups" die Spalten "name" und "members".
Diese werden mit dem Skript `unixcat` in die Datei "live" geschrieben.
// TK: Mit der zweiten Befehlszeile wird das `lq`-Kommando, das Ihnen als Instanzbenutzer zur Verfügung steht, ersetzt, so dass es in einem Skript funktioniert, dass vom `root`-Benutzer ausgeführt wird.
// TK: Mit `echo` wird die Ausgabe des `GET`-Kommandos an das Kommando `unixcat` weitergeleitet, dass als Ziel den Unix-Socket `~/tmp/run/live` hat. (wenn das mal so stimmt...und verständlich ist...)
// TK: Dabei sorgt die Option `echo -e` dafür, dass das `\n` im `GET`-Kommando korrekt interpretiert wird. 

Eines ist noch sehr wichtig:
...
Sie können das Plugin direkt von Hand ausprobieren, indem Sie den kompletten Pfad als Befehl eingeben:
// TK: Sie können das Agentenplugin ...
...
Wie immer sind Test und Fehlersuche am wichtigsten.
// TK: Soso, wie immer halt...
Am besten gehen Sie in drei Schritten vor:

. Plugin solo ausprobieren.
// TK: Plugin > Agentenplugin
...
// TK: Den Satz hinter das Kommando schieben.
...
//SP: Ab hier müssen die Servernamen noch angepasst werden.
// TK: Mit [{shell-raw},highlight=2-3] kann man die beiden relevanten Zeilen hiliten.
[{shell-raw}]
----
<<<local:sep(0)>>>
<<<myhostgroups:sep(59)>>>
check_mk;SQL-Server,myself
...
// TK: Den Satz hinter das Kommando schieben: Hier sollte die gleiche Ausgabe kommen wie beim vorherigen Kommando.
...
[{shell-raw}]
----
{c-root} cmk -d localhost | grep -A5 '^<<<myhostgroups'
<<<myhostgroups:sep(59)>>>
check_mk;SQL-Server,localhost,myself
<<<check_mk:cached(1691578038,3600)>>>
AgentUpdate: last_check 1665481638.3074498
----
// TK: Ich hab hier {shell} ersetzt durch {shell-raw}, sonst funktioniert hier nix.
// TK: Aber: Prompt ist noch falsch (siehe Pull Request von dieser Woche): {c-root} > {c-omd} 

Wenn das funktioniert, ist Ihr Agent vorbereitet!
Und was haben Sie dafür gemacht?
Sie haben ein dreizeiliges Skript mit dem Pfad `/usr/lib/check_mk_agent/plugins/myhostgroups` erzeugt und ausführbar gemacht!
// TK: Das Biest hat jetzt 7 Zeilen.
...
.~/local/lib/check_mk/base/plugins/agent_based/myhostgroups_check.py
[{python}]
----
...
[{shell}]
----
[["check_mk", "myhost1,myhost2,myhost3,myhost4"],["foo", "myhost11,myhost22,myhost33,myhost44"]]
----
...
Mit dem `return` wird die neue Liste zurückgeben. Diese enthält nun vorangestellt jeweils die Host-Gruppe, gefolgt von jeweils einem Eintrag _members_ mit den "Mitgliedern". 
[{shell}]
----
{"check_mk": {"members": "myhost1,myhost2,myhost3,myhost4"}, "foo": {"members": "myhost11,myhost22,myhost33,myhost44"}}
----
...
//SP: Beschreibung der fehlenden Section. Was steht da? Was macht das? Wie muss man es lesen?
...
=== Das Plugin in Checkmk aktivieren
...
def parse_myhostgroups(string_table):
    ...
    for line in string_table:
        ...
        parsed[line[0]] = dict(zip(column_names, line[1:6]))
    return parsed
----
...
Die Check-Funktion bekommt zusätzlich zur Sektion das Argument `item` mit dem jeweils gesuchten Item.
...
In allen anderen Fällen, d.h., wenn eine Host-Gruppe leer ist oder nicht (mehr) existiert, soll der Service {CRIT} anzeigen.
// TK: leer?
...
`state` legt den Zustand des Services fest (im Beispiel `OK` oder `CRIT`) und `summary` den Text, der in der [.guihint]#Summary# des Services angezeigt wird.
Er ist rein informell und wird von {CMK} nicht weiter ausgewertet.

// plus Link zum Testen!





// TK: alter Inhalt aus: 3. Checks mit mehr als einem Service pro Host (Items)
// Single vs. Multi-Items (Prio 1)
...
=== Summary und Details
// TK: alter Inhalt aus: 6. Summary und Details
