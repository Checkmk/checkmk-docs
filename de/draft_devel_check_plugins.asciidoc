...
Zuerst werden die zu verarbeitenden Spalten unter ihrem Namen als Variable `column_names` definiert.
...
// TK: Bitte nochmal gegenlesen, ob das richtig beschrieben ist
...
// TK: Hier wird die if-then-else Logik umgestellt (didaktisch ungünstig), damit die Funktion für die folgenden Kapitel vorbereitet ist.
...
Durch die beiden Schrägstriche (`//`) wird eine Ganzzahldivison ausgeführt, die sicherstellt, das das Ergebnis ein Integer-Wert ist.
// Ganzzahldivison: in English integer division
...
// MFS: Ja!
// TK: Kannst Du das bitte schreiben?
...
*Hinweis:* Wenn Sie den Benutzern des Regelsatzes statt leerer Eingabefelder bereits Werte vorgeben wollen, können Sie das tun, indem Sie die `Integer()` Funktion um das Argument `default_value` erweitern.
...
----
            ("hosts_up_lower", Tuple(
                title=_("Lower percentage threshold for host in UP status"),
                elements=[Integer(title=_("Warning"),default_value=90), Integer(title=_("Critical"),default_value=80)],
                )
            ),
----
...
// TK: Bitte gegenlesen, ob das verständlich ist.
// TK: Nicht getestet, ob das auch in der GUI funktioniert
...
// TK: Überschrieben wird hier ja nichts, nur verschattet (wie es bei CMK sonst so schön heisst). 
...
// TK: Bitte ergänzen, was hier nötig ist...
