...
=== Terminologie
Zwar ist hier immer die Rede von einem Check-Plugin und davon, dieses zu schreiben. Aber genaugenommen benötigen Sie immer zwei Plugins, das xref:glossar#agent_plugin[Agentenplugin] auf der Host-Seite und das xref:glossar#check_plugin[Check-Plugin] auf der {CMK}-Seite. Beide müssen geschrieben und aufeinander abgestimmt werden, nur so funktionieren sie hinterher reibungsfrei.
...
// SP: Was sollte ich dann noch ergänzen? Soll ja auch nicht zu langatmig werden.
...
Das Ziel in diesem Beispiels ist nun, eine Prüfung aufzusetzen mit der kontrolliert werden kann, dass für keinen Host vergessen wurde, eine Host-Gruppe zuzuweisen.
...
Um auf die Livestatus-Schnittstelle zuzugreifen, kommt der Befehl "lq" zum Einsatz.
Diesem können Sie direkt eine Abfrage in LQL (Livestatus Query Language) übergeben, die mit "GET" eingeleitet wird.
...
Die Ausgabe ist bereits für dieses kleine Beispiel relativ unübersichtlich und enthält Informationen, die hier aktuell nicht relevant sind.
Generell sollten Sie die Interpretation von Informationen {CMK} überlassen.
Gleichzeitig kann eine Vorfilterung auf dem Host jedoch den Umfang der zu übertragenden Daten reduzieren, wenn diese später nicht gebraucht werden.
...
Sobald Sie einen Host einer oder mehrerer Ihrer eigenen Host-Gruppen zuweisen, wird er aus der Gruppe `check_mk` entfernt.
...
Die Ausgabe zeigt nur die ersten Zeilen der sehr langen Liste. Durch die Verwendung von Präfixen ist die Zugehörigkeit vieler Check-Plugins hier bereits gut zu erkennen. Auch für Ihre eigenen Check-Plugins empfiehlt sich daher die Verwendung von Präfixen.
Die zweite Spalte zeigt übrigens an, wie das jeweilige Check-Plugin seine xref:agents[Daten] bezieht.

Ein für unser Beispiel passender Name für das neue Check-Plugin, der alle Kriterien erfüllt, ist zum Beispiel `myhostgroups`.
...
// MFS: Variablen in Shell eher klein, FULL CAPITAL nur für Umgebungsvariablen.
// MFS: Variablenzugriff ist so besser lesbar und sauberer abgegrenzt: ${blubb}
// MFS: Warum schwenken wir jetzt auf den rohen Weg um und greifen mit Pipe und Co direkt auf den Socket zu?
// Wäre es nicht besser, beim obigen Beispiel zu bleiben?
// Das erfordert dann halt die Environmont des Site Users, also machen wir su:
////
./usr/lib/check_mk_agent/plugins/myhostgroups
[{file}]
----
...
// TK2SP: Link in en ist: https://en.wikipedia.org/wiki/ASCII#Printable_characters
...
// MFS: Der jetzt folgende Absatz könnte dann deutlich gestrafft werden und bessere Referenzen auf das oben erhalten.
Mit dem zweiten echo-Befehl wird der `lq`-Befehl, der Ihnen als Instanzbenutzer zur Verfügung steht, ersetzt, so dass es in einem Skript funktioniert, das vom `root`-Benutzer ausgeführt wird.
...
Nehmen Sie den xref:intro_setup_monitor#linux[Host ins Monitoring] auf (zum Beispiel als `localhost`), melden Sie sich als Instanzbenutzer am und rufen Sie dann die Agentendaten mit `cmk -d` ab:
...
Das `agent_based` ist für alle Plugins, die sich auf den {CMK}-Agenten beziehen (also zum Beispiel nicht Benachrichtigungs-Plugins).
...
// MFS: Ja, das stimmt, die Dateien werden ja nicht über Shell und Kernel ausgeführt, stattdessen sucht der laufende Checkmk-Prozess nach seinen Dateien.

// MFS: Was haltet Ihr davon, an dieser Stelle das Grundgerüst vorzustellen? Die Leser haben dann eine Vorlage, die wir weiter ausbauen. Die hier funktioniert bereits. Damit kann dann auch die Serviceerkennung gezeigt werden, ohne dass Programmierfehler reingrätschen.
////
...
erst den Namen der Host-Gruppe, dann dahinter die zugehörigen Hosts.

Diese Informationen können Sie zwar alle ansprechen, aber jeweils nur über ihre Position im Datensatz. Sie müssten also immer angeben, die wievielte eckige Klammer und den wievielten Inhalt innerhalb der jeweiligen Klammer Sie brauchen.
Bei größeren Datenmengen gestaltet sich dies komplex und es wird immer schwieriger, den Überblick zu behalten.

An diesem Punkt bietet die Parse-Funktion durch ihre andere Struktur deutliche Vorteile. Durch sie wird der Code leichter lesbar, die Zugriffe werden performanter und Sie behalten viel einfacher den Überblick.
...
// MFS: Hier viellecht noch zwei print() einbauen und darauf verweisen, dass diese dann später bei Aktivierung sichtbar werden?
////
print("Raw data: " + string_table)
print("Resulting dict: " + parsed)
////
//SP: ich würde den Code ungern unnötig aufblähen. Worin liegt der Vorteil dieser Printfunktionen? Sind sie wirklich notwendig?

Mit `def` geben Sie in Python an, dass nachfolgend eine Funktion definiert wird.
`parsed = {}` erzeugt das „Dictionary“ mit der verbesserten Datenstruktur.
In unserem Beispiel bedeutet dies, dass jede Zeile Element für Element durchgegangen wird.
Aus jeder Zeile wird die Host-Gruppe gefolgt von den Mitgliedern der Host-Gruppe genommen und zu einem Eintrag für das _Dictionary_ zusammengesetzt.
...
`yield` hat in Python die gleiche Aufgabe wie `return` – beide geben einen Wert an die aufrufende Funktion zurück. Der entscheidende Unterschied besteht darin, dass sich bei einem `yield` "gemerkt" wird, wie weit die Funktion in einer Liste/Tabelle etc. gekommen ist. Beim nächsten Aufruf wird _nach_ der letzten `yield`-Anweisung weiter gemacht - und nicht wieder am Anfang begonnen. Dadurch wird nicht nur der erste „Treffer“ in einer Tabelle ausgelesen (wie es beim `return` der Fall wäre), sondern sukzessive alle „Treffer“.
...
// MFS: Die Entwicklung mag F-Strings nicht, weil darin Funktionen aufgerufen werden. str.format()? https://realpython.com/python-f-strings/#option-1-formatting
...
`--detect-plugins` beschränkt die Befehlsausführung auf dieses Check-Plugin und durch `localhost` auf eben diesen Host:
...
////
Marker für redaktionelle Bearbeitung:
Stand 6. August 2023
Silke arbeitet oberhalb besprochene Änderungen ein
Mattias beginnt unterhalb das Review
Der Diff-Algo müsste damit eindeutig zuordnen können und wir können später problemlos mergen...
////
...
// MFS: zip() kannte ich bislang nicht. Das hat aber einige Feinheiten, die hier erwähnt werden sollten: 1. Beide Eingaben könnon beliebig iterierbare Objekte sein, dennoch hielte ich es aus didaktischen Gründen für besser, statt eines Tupels ein Array zu verwenden, 2. Auf das Slicing des Arrays sollte verwiesen werden, 3. Hinweis, dass Casting vom zip Objekt nach dict nötig ist.
// MFS: Einrückungen der Leerzeilen
...
// MFS: Formatierung im Artikel konsistent
.~/local/lib/check_mk/base/plugins/agent_based/myhostgroups.py
[{python}]
----
register.check_plugin(
...
// MFS: den Host-Gruppennamen als Item übergeben bekommt
...
----
// MFS: Brauchen wir da noch ein "else" für leere Gruppen? Achso, kommt noch...

Das Check-Ergebnis wird geliefert, indem wir ein Objekt der Klasse `Result` per `yield` zurückgeben.
...
// MFS: informativ, nicht informell...
...
// MFS: mach doch mit dem oben weiter, if then else ...
...
// MFS: Achtung: Wir sollten nie davon ausgehen, dass int / int = float ist, das ist in Python zufälligerweise so, da wir uns aber keinen Kopf machen wollen, sollte die Ermittlung des Prozentwertes so aussehen:
// hosts_up_perc = 100 * num_hosts_up / num_hosts
////
mattias@barium:~$ python3
...
// MFS: Und wo kommt die Funktion check_levels her? Aus dem API oder müssen wir die selbst noch schreiben?
// MFS: Hier ist es zudem sinnvoll, zu schreiben, dass die Funktion ein Objekt vom Typ Result zurückliefert.
// MFS: Bei Vergleichen tendiere ich dazu, nur die selben Datentypen zu vergleichen, also entweder den errechneten Prozentsatz nach int zu casten oder im Schwellwerttupel explizit als float anzugeben: levels_lower=(90.0, 80.0)

Der letzte Parameter nutzt den im xref:notice[vorherigen Abschnitt] für das Objekt `Ŗesult()` bereits vorgestellten Parameter `notice`.
...
// TK: Hier auf den Performance-Gewinn bei mehreren Services durch die Parse-Funktion hinweisen?
// MFS: Ja!
...
// MFS: und hier noch der Hinweis, dass check_levels nun zwei Objekte yieldet: Service und Metric.
...
// MFS: Ist an der Stelle erst mal nicht so wichtig, ggf. auch auf vendor_device Prefix verweisen.
// MFS: Schwieriger die Frage, wenn man explizit bereits vorhandene Metriken verwenden möchte...
...
Falls nur eine Grenze des Wertebereichs definiert ist, tragen Sie für die andere einfach `None` ein, also zum Beispiel `boundaries=(0, None)`.
...
// MFS: Das ist mir jetzt von der Didaktik etwas verdreht. Oben zeigen wir zuerst das Result-Objekt und stellen dann die Convenience-Funktion vor. Hier erweitern wir gleich die Convenience-Funktion so dass sie auch das noch nicht eingeführte Metric-Objekt zurückgibt.
// MFS: Lass uns doch einfach das etwas weiter unten gezeigte Metric-Objekt hochziehen, um die vier ohne Schwellwerte auszugeben:
////
...
////
// MFS: Und erst danach check_levels erweitern...
...
// MFS: Sollte an der Stelle bereits klar sein.
...
// TK: In dem (alten) Kapitel wird fast nur von Check geschrieben und nicht von Check-Plugin.
// TK: Ist Check die Abkürzung für Check-Funktion? („Falls Ihr Check kein Item hat...“) oder Check-Plugin oder mal so, mal so oder doch ganz anders?
// MFS: In der Regel wird Check-Funktion gemeint. Wo mehr Präzision erforderlich ist, weise ich darauf hin.
// TK: Bitte sorgfältig den Code reviewen und auch die verwendeten Namen für Variablen & Co - die sind evtl. nicht so passend, wie sie sein sollten.
// TK: Nach Fertigstellung hab ich gesehen, dass es ja auch noch eine ValueSpec für Prozentsätze gibt. Muss das Beispiel wohl nochmal umgeschrieben werden?
// TK: Muss hier erwähnt werden, dass Regelsätze (wie auch später die Metriken für Experten) nicht von der Check-API abgedeckt sind?
// MFS: Ja, hier muss darauf verwiesen werden, dass wir das GUI API verwenden.

Im erweiterten Check-Plugin `myhostgroups_advanced` haben wir den Zustand {WARN} erzeugt, wenn nur 90{nbsp}% der Hosts {UP} sind, und {CRIT} bei 80{nbsp}%.
...
Dazu benötigen wir einen xref:glossar#rule_set[Regelsatz.]
...
Um einen neuen Regelsatz zu erstellen, legen wir eine neue Datei im Verzeichnis `~/local/share/check_mk/web/plugins/wato` an.
Der Name der Datei sollte sich an dem des Check-Plugins orientieren und er muss wie alle Plugin-Dateien die Endung `.py` haben.
...
// TK: Hmmm, wo denn genau? Das steht nicht im Altkommentar...
// MFS: Hat mit der Art zu tun, wie und wo die Rulespecs verarbeitet werden.
...
// TK: Das macht CMK für *mein* Plugin? Und wo stehen die Texte dann?
// MFS: Kannst Du im Prinzip ausprobieren: Wenn Du einen englischen Text verwendest, der schon in Weblate übersetzt ist, erhältst Du in der GUI den deutschen. Also das Beispiel mit einem Text zeigen, der in Weblate ist.
// MFS: Mit neuen Plugins gehn das natürlich erst, wenn diese Teil von Checkmk geworden sind.
Falls Sie den Check nur für sich selbst bauen, können Sie darauf auch verzichten und brauchen diesen Importbefehl nicht.
// MFS: Import Statement...

Als nächstes importieren Sie sogenannte _ValueSpecs._
// TK: Sind ValueSpecs eigentlich ein CMK-Ding?
// MFS: Nein. Findet man auch in anderen Frameworks, die Labels, Eingabefelder, Werte(-bereiche), Validierung und Testspezifikation (!) also für automatisierte Komponententests in einer Datenstruktur zusammenfassen.
...
Das `Dictionary` benötigen Sie auf jeden Fall, denn seit {CMK}-Version {v20} müssen Check-Parameter Python-Dictionaries sein.
// TK: Wohl präziser: mit der neuen Check-API statt {CMK}-Version {v20}
// MFS: Vielleicht: ...werden Check-Parameter in Python-Dictionaries gespeichert...
`Integer` ist für die Eingabe einer Zahl ohne Kommastellen verantwortlich, `TextInput` für einen Unicode-Text und `Tuple` für eine geordnete, eindimensionale Menge von Objekten.

Als nächstes werden noch Symbole importiert, die beim Registrieren benötigt werden:
// TK: Symbole? Ist das ein Python-Spezialding? Ich dachte, man kann nur Klassen und Funktionen importieren?
// MFS: Das hat jemand geschrieben, der sonst eher C programmiert. "Klassen und Funktionen" ist besser.
...
Da unser Check-Plugin `myhostgroups_advanced` mehrere Services erzeugt, importieren wir `CheckParameterRulespecWithItem`.
...
Zunächst deklarieren wir ein Eingabefeld, mit dem der Benutzer das _Item_ des Checks angeben kann.
...
// TK: Versteh kein Wort. Warum soll ich Items/Services auswählen, wenn es keine Discovery gibt?
// MFS: Da geht es um "Enforced Services", das ist natürlich in unserem Beispiel blöd zu erklären, aber bei manchen Servern möchte man auf allen Hosts in einem bestimmten Ordner bspw. ein "Filesystem /var/lib/blahblubb/tmp" haben. Mit den "Enforced Services" findet man die, die es nicht haben.
Das Feld erstellen wir mit `TextInput`.
...
Auch hierfür legen wir eine Funktion an, welche das ValueSpec erzeugt.
...
////
TK: Ursprünglich wollte ich zuerst einfach loslegen und für hosts_up zwei unterschiedliche Werte definieren (und erst anschließend das Tupel einführen), etwa so:
        elements=[
            ("hosts_up_lower_warn", Integer(title=_("Warning if this percentage of hosts is UP"))),
            ("hosts_up_lower_crit", Integer(title=_("Critical if this percentage of hosts is UP"))),
        ],
Aber das hätte das eh schon sehr lange Kapitel noch länger gemacht...
////

// MFS: Sollen wir hier auch noch default_value zeigen? Die dienen zum Vorausfüllen der Formularfelder:
////
def _parameter_valuespec_eset_license_levels():
    return Dictionary(
        elements=[
...
Alle Schwellwerte sollen Ganzzahlen sein, also verwenden wir hier `Integer`.
// MFS: An das oben gesagte denken: Wenn hier Integer verwendet wird, sollte auch oben Integer verwendet werden. Gut möglich, dass unsere Hilfsfunktionen, die Metric und Result yielden, irgendwann nur noch dieselben Datentypen vergleichen.

Zu guter Letzt registrieren wir jetzt mithilfe der importierten und selbst definierten Dinge den neuen Regelsatz.
...
* Der `check_group_name` stellt die Verbindung zu den Check-Plugins her, welche diesen Regelsatz verwenden wollen.
// TK: Ich bin unsicher: check_group_name legt hier doch zuerst einmal den Namen des Regelsatzes fest, oder?
// MFS: Ja, der Variablenname ist doof gewählt.
// TK: Ein Check-Plugin, das diesen Regelsatz verwenden will, muss beim Registrieren diesen Namen als check_ruleset_name verwenden. Richtig?
// MFS: Exakt.
Er darf auf keinen Fall identisch sein mit einem bereits existierenden Regelsatz, weil dieser damit überschrieben würde.
// TK: Ja super. Und wie find ich das wieder raus?
// MFS: Das ist einfach ein Problem, das Checkmk hat – es gibt keine saubere Trennung der Namensräume. Hier kann man eigentlich nur darauf verweisen, dass man für eigene Rulespecs die Prefixes vendor_product nehmen sollte. Eine Liste der von uns selbst verwendeten Rulespecs gibt es meines Wissens nicht. Allerdings müsste die Entwicklung eine haben oder aus dem Code erzeugen können. Auf jeden Fall auf dem Schirm behalten, zumindest leicht wiederverwendbare müssen identifiziert und erklärt werden.
...
// TK: Es scheint nur für Gruppen zu funktionieren, die CheckParameters im Namen haben. Wollte das Ding eigentlich im letzten Kasten Various parken, ist mir aber nicht gelungen.
// MFS: Ja, nur Checkparameters. Andere Rulespecs sind für weitere interne Verwendung, die nicht Scope dieses Artikels ist.

* Der `match_type` ist immer `"dict"`.
// In älteren {CMK}-Versionen gab es auch Parameterregeln mit anderen Typen.
...
// MFS: Vielleicht hier darauf hinweisen, dass wir uns jetzt ja in der GUI und damit auf einer anderen Ebene als vorher (Monitoringkern) befinden?
...
----
// TK: Kann man eigentlich mit omd 2 Services in einem Befehl starten?
// MFS: Ich mache immer ein omd restart – also ganze Instanz. Gerade, wenn man immer GUI-Code und Check-Code miteinander editiert, macht das vom Zeitbedarf keinen Unterschied mehr.
...
Damit die Regel zum Greifen kommt, müssen wir dem Check-Plugin erlauben, Check-Parameter entgegenzunehmen und ihm sagen, welche Regel benutzt werden soll.
Dazu fügen wir zwei neue Zeilen in die Registrierungsfunktion ein:

// MFS: Wieder konsistente Formatierung des Codes, name = "myhostgroups_advanced", ...
...
Im einfachsten Fall übergeben wir ein leeres Dictionary `{}`.

Als zweites übergeben wir der Registrierungsfunktion noch den `check_ruleset_name`, also den Namen, den wir oben mittels `check_group_name` an den Regelsatz vergeben haben.
...
Damit das klappen kann, müssen wir die Check-Funktion so erweitern, dass sie das Argument `params` erwartet, welches sich zwischen `item` und `section` schiebt:
...
Parameters({'hosts_up_lower': (70, 60), 'services_ok_lower': (75, 65)})
Parameters({'hosts_up_lower': (70, 60), 'services_ok_lower': (75, 65)})
----
...
Nun passen wir unsere Check-Funktion weiter an, so dass die übergebenen Parameter ihre Wirkung entfalten können.
Wir holen uns die beiden Tupel mit dem in der Regel gewählten Namen aus den Parametern:
...
// MFS: Hinweise zur Division siehe oben.
[{python}]
----
    hosts_up_perc = num_hosts_up / num_hosts * 100
...
Falls eine Regel konfiguriert ist, können wir nun die Host-Gruppen unseres Beispiels mit den über die GUI gesetzten Schwellwerten überwachen.
...
// MFS: Hier ist noch "Foobar Sector %s" drin...
[{python},highlight=6]
----
register.check_plugin(
    name = "foobar",
...
----
// MFS: Der Hinweis ist sehr gut. Falls wir oben die default_values zeigen, wäre noch sinnvoll darauf zu verweisen, dass diese mit check_default_parameters synchron gehalten werden sollten, Checkmk hat dafür keinen Automatismus.
...
// TK: Ursprüngliche Idee: Die restlichen Themen in ein Kapitel "Weitere Komponenten der Check-Entwicklung" stecken
// TK: Da es aber momentan nur 2 Themen sind (Metriken für Experten und Zahlen) finde ich das überstrukturiert.
[#metrics_advanced]
== Darstellung von Metriken anpassen

Im xref:metrics[obigen Beispiel] haben wir das Check-Plugin `myhostgroups_advanced` Metriken für alle gemessenen und berechneten Werte erzeugen lassen.
Dazu haben wir zwei Wege vorgestellt.
// MFS: Oben habe ich vorgeschlagen, die Beispiele zu vertauschen (das einfachere zuerst), dann müsste hier analog getauscht werden.
...
Dann haben wir die gemessenen Metriken mit dem Objekt `Metric()` erzeugt -- für die Anzahl der Services im Zustand {OK} zum Beispiel so:
...
// TK: Ich finde, dass es wahrscheinlicher ist, eine passende Metrikdefinition als einen Regelsatz zu finden. Daher hier 1. Abschnitt -- und bei den Regelsätzen letzter.
// MFS: Definitiv. Und aus "können" sollte daher "sollten"werden!

Bevor Sie eine neue Metrikdefinition erstellen, können Sie zunächst prüfen, ob {CMK} nicht bereits eine geeignete Definition mitbringt.
...
----
// TK: Was macht in der metric_info das "_l" beim Titel? Wenn man das "l" bei den eigenen Metrikdefinitionen verwendet, passiert gar nix.
// MFS: Die Übersetzungsthematik von oben. Ohne allzutief zu graben: Das wird ein Wrapper für _() sein, mit dem bereits übersetzte Strings in Metriknamen verwendbar sind.
// TK: Wenn ich testweise "util" bei einer meiner Metriken in der Check-Funktion eintrage, sind alle alten Werte weg. Warum ist das so? Wann passiert das?
// MFS: Da werden neue RRDs für die "neue" Metrik angelegt...
...
// TK: Letzten Satz streichen wg. Nullinformation?
// MFS: Nö, das ist so ein Fall, wo die Redundanz sogar willkommen ist.
...
// MFS: Hier wieder Hinweis, dass man vorhandene "kaputt machen" kann, indem man sie versehentlich überschreibt.

Für unser Beispiel wollen wir einen eigene Metrik für die Anzahl der Services im Zustand {OK} definieren.
...
Für unser Beispiel wollen wir die beiden Metriken `num_services_ok` und `num_services` in einem Graphen darstellen.
Die Metrikdefinitionen dazu sehen wie folgt aus:

.~/local/share/check_mk/web/plugins/metrics/myhostgroups_advanced_metrics.py
[{python}]
----
ffrom cmk.gui.i18n import _
...
// TK: Folgenden Altsatz find ich merkwürdig, da wir doch gerade diese Möglichkeiten beschrieben haben -- wenn auch nicht alle.
// MFS: ich tendiere fast dazu, diesen Absatz rauszunehmen. Stacked wäre in unserem Beispiel Gutwerte + Schlechtwerte = Gesamtwerte. Ich habe keinen Plan, was da noch an Anpassungen folgen wird.
// Leider gibt es im Handbuch noch keine Beschreibung der Möglichkeiten dieser Definition. 
Sie können statt `line` auch `area` oder `stack` ausprobieren - und Sie finden sehr viele Beispiele in den Dateien im Verzeichnis `~/lib/check_mk/gui/plugins/metrics`.
...
----
// TK: Dass man auch hier den Apache neustarten muss, um die Wirkung in der GUI zu sehen, hab ich mir geschenkt.
...
Allerdings wird der Zustand des Services nicht nur durch den Prozentsatz der Services im Zustand {OK} sondern auch durch Prozentsatz der Hosts im Zustand {UP} bestimmt.
...
In der [.guihint]#Summary# und den [.guihint]#Details# eines Services werden oft Zahlen ausgegeben.
Um Ihnen eine schöne und korrekte Formatierung möglichst einfach zu machen,
und um auch die Ausgaben von allen Check-Plugins zu vereinheitlichen, gibt es Hilfsfunktionen für die Darstellung von verschiedenen Arten von Größen.
Alle diese sind Unterfunktionen vom Modul `render` und werden folglich mit `render.` aufgerufen.
Zum Beispiel ergibt `render.bytes(2000)` den Text `1.95 KiB`.

Allen diesen Funktionen ist gemein, dass Sie ihren Wert in einer sogenannten _kanonischen_ oder natürlichen Einheit bekommen.
...
Und zukünftige Versionen von {CMK} können die Darstellung möglicherweise ändern oder sogar konfigurierbar für den Benutzer machen.
// MFS: Hier würde ich schreiben, dass an einigen Stellen bereits in andere Einheitensysteme gewechselt werden kann, beispielsweise kann bei Temperaturen Fahrenheit eingestellt werden.
...
Die Angaben erfolgen immer _Unix-Zeit_, also in Sekunden ab dem 1. Januar 1970, 00:00:00 UTC -- dem Beginn der _Unix-Epoche._
Dies ist auch das Format, mit dem die Python-Funktion `time.time()` arbeitet.

Vorteil an dieser Darstellung ist, dass sich damit sehr einfach rechnen lässt, also zum Beispiel den Zeitraum, wenn Start- und Endzeit bekannt sind.
...
`render.date()` gibt dabei nur das Datum aus, `render.datetime()` fügt noch die Uhrzeit hinzu.
...
Das liegt daran, dass wir dieses Handbuch in der Zeitzone von Deutschland schreiben, und die ist der Standardzeit UTC eine Stunde voraus (zumindest während der Normalzeit, denn der 1. Januar liegt
...
// MFS: Nimmt diese Funktion auch das Timedelta-Objekt an? Nachgeschaut: Nein. Dann müssen wir darauf hinweisen, dass nur Sekunden als Integer zulässig sind.
...
Überall wo es um Arbeitsspeicher, Dateien, Festplatten, Dateisysteme und dergleichen geht, ist die kanonische Einheit das _Byte_.
...
// MFS: Wenn man Oktette einführt, sollte man das oben tun. Historisch gabe es bspw. Systeme, die 7 Bit oder 12 Bit Wortbreite hatten. Da ist Oktett statt Byte pragmatischer. Heute irrelevant.
Eingabewert ist wieder Bytes pro Sekunde (_Oktette_ würde der Netzwerker sagen):
...
// MFS: Bei den beiden folgenden stimmen Beschreibung und Beispielausgabe nicht überein.
|`nicspeed()` |Oktette pro Sekunde |Geschwindigkeit von Netzwerkkarten |`100 MBit/s`
|`networkbandwidth()` |Oktette pro Sekunde |Übertragungsgeschwindigkeit |`23.50 GBit/s`
...
=== Exceptions und Absturzberichte

//// 
MFS:
Hier ist mir die ganze Aufteilung zu nah am Ursprungtext.
IMHO sollte die Aufteilung so sein:

8.
Die Einleitung mit Erwähnung der vielfältigen Gründe für Exceptions (Programmierfehler, unerwartete Daten, falsche Daten) direkt unter der 8, weiß nicht, ob es dafür die Bulletlist braucht.
Meiner Ansicht nach zunächst ohne die sehr wertende Aussage, dass man Exceptions nicht abfangen soll, oder dass man im Regelfall nicht abfangen soll, außer man gewinnt so mehr Erkenntnis, Verweis auf 8.3.
Zuallererst geht es darum, Programmierfehler zu finden.

8.1. Exceptions und Absturzberichte in der GUI
Im wesentlichen wie bislang, aber die ganzen Empfehlungen raus

8.2. Exceptions und Absturzberichte auf der Kommandozeile
Erklärt eigentlich sehr gut, mir fehlt lediglich der Hinweis, dass bei nicht reprodizierbaren Crashes der Weg in die GUI unnötig ist:
ls -lahtr ./var/check_mk/crashes/check/

8.3. Ungültige Agentenausgabe
Haben wir eigentlich oben schon behandelt.
ABER: Unvorhergesehene Eingaben, sind der einzige Platz, an dem man ggf. abfangen und auswerten sollte.
Hier also die Erklärung, wann abfangen.
Ich würde es an dieser Stelle aber tatsächlich mit einem etwas umfangreicheren Beispiel behandeln und zwar, wenn man JSON in einen Dict geparst hat und dann versucht auf Schlüssel zuzugreifen, sollte man nicht blubb['key'] nehmen, sondern blubb.get('key') und dann auf None prüfen.
Man kann dann direkt auf CRIT setzen und sagen, welcher Key in der Agentenausgabe fehlt.
Oder: Einer meiner Virenscanner liefert einen leeren String, wenn er noch kein Signaturupdate erhalten hat.
Ich kann darauf prüfen und muss nicht erst die Parse-Funktion für Datum nach ISO8601 in eine Exception laufen lassen.

8.4. Fehlende Items
Haben wir eigentlich oben schon behandelt.
Wenn wir das hier nochmal bringen wollen, dann mit Beispiel entsprechend modernisiert.
////
...
Und zwar meist mit einem Absturzbericht (_crash report_).
...
* alle Informationen über den Absturz erhalten, wie Auflistung der aufgetretenen Fehler im Programm (_traceback_), aktuelle Werte lokaler Variablen, Agentenausgabe, und vieles mehr;
...
Aber die ganzen Umstände, wie es dazu kam (zum Beispiel die Daten vom Agenten), wären nicht verfügbar.


=== Exceptions auf der Kommandozeile ansehen

Falls Sie ihr Check-Plugin auf der Kommandozeile ausführen, werden keine Absturzberichte erzeugt.
...
Falls das aber eigentlich nicht sein darf ... dann tun Sie am besten so, als ob die Zeile immer aus drei Worten besteht, also zum Beispiel mit:
...
Es gibt noch viele weitere Aspekte und Themen rund um die Entwicklung von eigenen Plugins.
{CMK} hat sehr viele Schnittstellen für eigene Erweiterungen und ist dadurch sehr flexibel erweiterbar.
Wir arbeiten daran, dass diese Schnittstellen nach und nach im Handbuch beschrieben werden.
...
////
In diesem Artikel werden in Zukunft noch weitere Aspekte der Plugin-Entwicklung besprochen werden.
