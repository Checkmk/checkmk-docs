...
Wer sich für die Check-Plugin-Programmierung in {CMK} interessiert, hat mit großer Wahrscheinlichkeit auch schon einmal einen {CMK}-Server aufgesetzt.
...
Mit `echo` wird dabei die Ausgabe des `GET`-Kommandos an das Kommando `unixcat` weitergeleitet. Das Ziel der ausgabe, der Unix-Socket `~/tmp/run/live`, wird dahinter definiert.
...
Nehmen Sie den xref:intro_setup_monitoring#linux[Host ins Monitoring] auf (z.B. als `localhost`) und rufen Sie die Agentendaten dann mit `cmk -d` ab.
...
Dies ist an dieser Stelle sehr viel sinnvoller als im vorherigen Abschnitt, da wir beim Erstellen der Check-Funktion nicht wissen, welchen Zustand der Service im Monitoring haben wird.
