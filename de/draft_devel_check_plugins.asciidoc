...
Mit dem ersten echo-Befehl rufen Sie die Informationen zu den Host-Gruppen auf, die wir uns oben mit dem ersten GET-Befehl bereits angesehen hatten.
Der zweite echo-Befehl schränkt die zu verwertenden Daten auf die Gruppennamen und Mitglieder ein. 
// TK: Die beiden Sätze stimmen so nicht.
// TK: Hilft ja nix, aber ein paar Takte zu "| /omd/sites/$SITE/bin/unixcat /omd/sites/$SITE/tmp/run/live" müssen auch noch rein.
// TK: Und die sep(59) ist auch nicht selbsterklärend.
////

Bis jetzt haben Sie sich mit dem GET-Befehl die Informationen nur im Terminal anzeigen lassen. Das ist hilfreich, um sich einen Einblick in die Daten zu verschaffen.
