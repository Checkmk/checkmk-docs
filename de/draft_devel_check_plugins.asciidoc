...
Das Ziel in diesem Beispiels ist nun, eine Prüfung aufzusetzen, mit der kontrolliert werden kann, ob für keinen Host vergessen wurde, eine Host-Gruppe zuzuweisen.
...
// ML: Hinweis auf neuen Code und unten ... eingefügt - sonst sieht das wie die ganze Datei aus.
...
Alle Schwellwerte sollen Ganzzahlen sein, also verwenden Sie hier `Integer`.
...
Der erste Eintrag unter `metrics` legt fest, welche Metrik für den Titel des Graphens genommen wird. 
...
dass ein _Kilobyte_ nicht 1.000, und damit das Tausendfache der Einheit ist, sondern 1.024 (2 hoch 10) Bytes.
...
Da bei jeder Größenordnung der Unterschied zwischen 1.000 und 1.024 immerhin 2,4{nbsp}% ausmacht, und diese sich aufmultiplizieren,
wird so aus einer Platte der Größe 1 GB (1.024 mal 1.024 mal 1.024) auf einmal 1,07 GB.
...
Demnach ist heute offiziell ein Kilobyte 1.000 Bytes und ein _Kibibyte_ 1.024 Bytes (2 hoch 10).
...
So gibt es speziell für Festplatten und Dateisysteme die Funktion `render.disksize()`, welche die Ausgabe in 1.000er-Potenzen macht.
...
Hier bekommen Sie die Ausgabe in klassischen 1.024er-Potenzen in der neuen offiziellen Schreibweise:
...
Dafür gibt es die Funktion `render.iobandwidth()`, die in {CMK} mit 1.000er-Potenzen arbeitet:
