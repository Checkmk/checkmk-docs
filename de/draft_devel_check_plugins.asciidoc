...
// TK: Dateiname ändern myhostgroups_check.py > myhostgroups.py
...
// TK: Hilft ja nix, aber da muss noch stehen, was die Parse-Funktion eigentlich tut.

Mit dem `return` wird die neue Liste zurückgeben.
Diese enthält nun vorangestellt jeweils die Host-Gruppe, gefolgt von jeweils einem Eintrag _members_ mit den "Mitgliedern".
...
// Die beiden Kommentare sollten mit dem neuen Kap. "Das Plugin in {CMK} aktivieren" weiter unten obsolet sein.
////
...
// TK: ggf. Titel ändern zu: Das Plugin in {CMK} testen und aktivieren
