...
Melden Sie sich als Instanzbenutzer an und fragen Sie die Informationen zu den Host-Gruppen mit folgendem Kommando ab: 
...
Legen Sie also unser Beispiel-Check-Plugin hier an.
...
// TK: Namensgebung: Wie nennt man die Quelle, aus der mit from importiert wird? Und wie die Dinger, die man mit import reinholt?
Als erstes müssen Sie die für die Check-Plugins nötigen Funktionen und Objekte aus anderen Python-Modulen importieren.
...
// TK: Ist das hier eine interessante Information, dass man die Inhalte aus ~/lib/check_mk/base/plugins/agent_based/agent_based_api/v1 holt?
Wie Sie sehen, steckt hier auch eine Versionsnummer der Check-API.
...
Als erstes bauen Sie eine Parse-Funktion auf.
Diese hat die Aufgabe, die „rohen“ Agentendaten zu „parsen“, d.h. zu analysieren und zu zerteilen, und in eine logisch aufgeräumte Form zu bringen, die für alle weiteren Schritte einfach zu verarbeiten ist.
...
// TK: Nachteil dieser und Vorteil der neuen Struktur klarer und konkreter benennen. oder: Wie die Parse-Funktion unser Leben leichter macht.

Ziel ist es nun, die von {CMK} gelieferte Datenstruktur so zu transformieren, dass man jeden der einzelnen Werte durch einen Namen (oder Schlüssel) ansprechen kann:
...
Mit `return parsed` wird dann das oben gezeigte Dictionary zurückgegeben.
...
// TK2SP: Ersetzt: in einem geparsten Format (dazu später mehr). > in einem durch die xref:parsefunction[Parse-Funktion] aufbereitetem Format.
...
In der Service-Übersicht zeigt nun das Symbol icon:icon_service_graph[alt="Symbol zur Anzeige der Graphen des Services."], dass es Graphen zum Service gibt.
...
Dabei beschränken wir uns auf die Angabe der beiden obligatorischen Angaben von Metrikname und Wert.
