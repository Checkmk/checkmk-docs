...
Check-Plugin programmieren. Als Beispiel dient eine einfache Überwachung
innerhalb von Checkmk.

Das Check-Plugin soll einen neuen Service anlegen, welcher prüft, ob es in Ihrem Checkmk Hosts gibt, die keiner Host-Gruppe zugewiesen sind. Finden sich ein oder mehrere Hosts, auf die dies zutrifft, so soll der Service kritisch werden.
