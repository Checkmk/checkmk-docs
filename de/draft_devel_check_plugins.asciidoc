...
Stattdessen werten das Check-Plugin Daten aus, die {CMK} von dem zu überwachenden Gerät per SNMP abruft, und die der auf dem Gerät vorinstallierte SNMP-Agent standardmäßig bereitstellt.
...
Für das gewählte Beispiel genügt zunächst ein GET-Befehl.
// TK: Das ist nicht genug. Hier muss noch das lq Kommando eingeführt werden für die Livestatus-Abfrage

Öffnen Sie eine Kommandozeile für Ihre Checkmk-Instanz und fragen Sie die Informationen zu den Host-Gruppen ab:
// TK: Hier evtl. stattdessen das Kommando absetzen, das alle Spalten der Host-Gruppentabelle auflistet:
...
Hier existiert aktuell nur eine Host-Gruppe, die Gruppe check_mk. Diese enthält zwei Hosts namens SQL-Server und myself.
...
// TK: Ab hier bist Du schon im (alten) Unterkapitel "Den Befehl in den Agenten einbauen"
// TK: Da stehen nützliche Dinge zum Dateinamen des Skripts, zum Verzeichnis, in dem es abgelegt werden muss, zum Sektions-Header, zum Shebang ...
Bis jetzt haben Sie sich mit dem GET-Befehl die Informationen nur im Terminal anzeigen lassen. Das ist hilfreich, um sich einen Einblick in die Daten zu verschaffen. Nun wird diese Abfrage so umgebaut und in ein Skript übertragen, dass die Ergebnisse elektronisch weiterverarbeitet werden können.

// TK: {shell} kommt mit den dreifach spitzen Klammern nicht zurecht, also {shell-raw} nehmen ...
// TK: ... {shell}/{shell-raw} sind aber beide nicht richtig. Für Dateiinhalt gibt es {file} ;-)
[{shell}]
----
#!/bin/bash
...
Das können wir aber auch ganz einfach hier mit der Option `-1` (für *einspaltige* Ausgabe) erzwingen:
