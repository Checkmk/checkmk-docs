...
check_mk;localhost;1;1;60;48
Munich;checkmk.com;1;1;1;1
Oberammergau;internet,microcontroller;2;2;5;2
----
...
Beim xref:declare_section[Erstellen der Parse-Funktion] haben wir nur zwei Spalten der Host-Gruppentabelle berücksichtigt.
...
Da das im xref:agentbased[vorherigen Kapitel] erstellte Check-Plugin zur Überprüfung der Standardgruppe `check_mk` weiterhin funktionieren soll, bleibt dieses Check-Plugin so, wie es ist.
...
Mit `sections=["myhostgroups"]` wird festgelegt, dass das neue Check-Plugin die gleichen Daten nutzt wie das alte: die durch die Parse-Funktion aufbereitete Sektion des Agentenplugins.
...
Wie zuvor bekommt sie das Argument `section`.
