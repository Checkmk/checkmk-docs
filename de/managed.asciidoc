...
In einem zentralen Setup wird {CMK} in der {FE} und {SE} daher alle Konfigurationsdaten an alle beteiligten Instanzen verteilen, 
da sie prinzipiell auch überall liegen könnten, bzw. benötigt werden.
Zentral verwaltete Passwörter müssen auch für die Remote-Instanzen zur Verfügung gestellt werden, Host und Services von Kontaktgruppen können über mehrere Instanzen
verteilt sein.
...
Mit der {CME} bindet ein Anbieter (= ein Provider) in seiner Zentralinstanz für jeden Kunden über das xref:distributed_monitoring#[verteilte Monitoring] eine oder mehrere lokale Instanzen ein, die nur diesem Kunden gehören.
...
Dem Kunden stehen also über die ihm zugeordnete Instanz nur seine eigenen Konfiguration mit den Host- und Servicedaten zur Verfügung.
...
*Wichtig*: Sie müssen die Option [.guihint]#Managed Services# bei der link:https://checkmk.com/de/produkt/preise[Lizenzierung^] auswählen, 
...
Neben der Instanz selbst können Sie -- wie bereits in der Einleitung erwähnt -- auch Elemente anderer Komponenten aus dem [.guihint]#Setup# einem Kunden zuzuweisen.
...
Bei dem Upgrade einer bestehenden Umgebung von der {FE} oder {SE} auf die {ME}, gibt es einige Besonderheiten zu beachten.
