...
xref:distributed_monitoring#distr_wato[zentralen Setup] werden sich die Benutzer
in der Regel auf der _Zentralinstanz_ einloggen, um dort
Konfigurationen vorzunehmen oder auf die Monitoring-Daten zuzugreifen. Die
Benutzer können sich zwar zusätzlich auch auf den _Remote-Instanzen_
...
xref:wato_user#roles[Rollen] und xref:wato_user#contact_groups[Kontaktgruppen]
...
sind. Dass sie dabei eventuell über die Existenz von anderen Hosts und
Services informiert sind, ist kein Problem.

In einem zentralen Setup wird {CMK} in der {FE} und {SE} daher alle
...
Passwörter müssen auch für die Remote-Instanzen zur Verfügung gestellt
...
Remote-Instanzen verteilt werden. Das heißt, sensible Daten eines Kunden
...
Zusätzlich ist es nicht mehr erforderlich, dass ein Kunde zentral eine
Konfiguration vornehmen kann -- der Sinn einer Dienstleistung ist es ja
...
Mit der {CME} bindet ein Anbieter (= ein Provider) in seiner Zentralinstanz
für jeden Kunden über das xref:distributed_monitoring#[verteilte Monitoring] eine
oder mehrere lokale Instanzen ein, die nur diesem Kunden gehören.
// TK: Warum heisst es hier lokale Instanzen und nicht Remote-Instanzen?
Einzelne Elemente im [.guihint]#Setup# werden dann diesen Instanzen zugewiesen. {CMK}
...
Dienstleister weiterhin bequem über das zentrale Setup seiner eigenen Instanz
...
eigenen Konfiguration mit den Host- und Servicedaten zur Verfügung. Er muss sich
...
link:https://checkmk.com/de/produkt/preise[Lizenzierung^] auswählen, sobald Sie
...
Schritt: Wählen Sie unter [.guihint]#Setup > Users > Customers# den Knopf [.guihint]#Add Customer#
...
entsprechenden Komponenten in {CMK} mit diesem Kunden. Die Zentralinstanz,
an die alle weiteren Instanzen der Kunden ihre Daten schicken wird auch
_Provider-Instanz_ genannt. Die Trennung der Daten funktioniert
nur, wenn Sie für jeden Kunden eine eigene Instanz anlegen und diese mit
ihrer Provider-Instanz xref:distributed_monitoring#connect_remote_sites[verbinden]. Die Einrichtung
...
gewohnt über die
xref:distributed_monitoring#sitespecific[instanzspezifischen globalen Einstellungen]
konfigurieren.
...
aus dem [.guihint]#Setup# einem Kunden zuzuweisen. Dabei wird ein Element
...
In der xref:glossar#ec[Event Console] können Sie sowohl einzelne Regeln, als auch ganze
...
anders, als bei Ordnern -- nicht von den einzelnen Regeln
...
einzelnen Kunden _nicht_ zugewiesen werden. Dennoch gibt es ein paar Worte
...
Sie können BI-Aggregate keinem spezifischen Kunden zuordnen. Daher
werden alle Aggregate und deren Regeln auf alle Instanzen übertragen.
Die Benennung der Regeln, Pakete und Aggregate sollten aus diesem Grund so
...
Auch für xref:glossar#host_tag[Host-Merkmale] (_host tags_) gilt, dass sie keine vertraulichen Informationen
enthalten dürfen, da die Merkmale an alle Instanzen verteilt werden.


==== Benachrichtigungen

Regeln zu xref:glossar#notification[Benachrichtigungen] enthalten oft Kontaktgruppen und sehr spezifische
...
verzichten Sie hier insbesondere auf explizite Host- und Service-Namen,
...
Die im vorhergehenden Kapitel erwähnten Punkte wie Benachrichtigungen mit Kontaktgruppen oder Host-Merkmale können Informationen über organisatorische Strukturen anderer Kunden offen legen.
...
Hosts gelistet sind. Diese xref:glossar#view[Ansicht] funktioniert also wie die Ansicht [.guihint]#All hosts# 
-- mit dem Unterschied, dass hier nur die Elemente eines bestimmten Kunden angezeigt werden.


=== Seitenleiste

Das Snapin [.guihint]#Customers# der xref:glossar#sidebar[Seitenleiste] funktioniert genauso, wie das ähnlich aussehende
...
// TK: Site status snapin hat jetzt Enable all und Disable all Knöpfe. Daher dieses Snapin in der CME checken.
...
Zum anderen können Sie auch ganz xref:views#new[neue Ansichten] auf
...
es einige Besonderheiten zu beachten. Wenn Sie nur eine einzelne
Instanz umstellen möchten, ist der Umstieg sehr einfach: Sie führen einfach
wie gewohnt ein xref:update#detailed[Update] der Instanz durch und haben danach
...
können dann in Ruhe eine Umgebung für die Managed Services aufbauen.

Wenn Sie eine bestehende Umgebung umstellen möchten, bei der Sie bereits
Remote-Instanzen bei einem Kunden eingerichtet haben, sind wenige Details
mehr zu beachten:
...
wie bereits beschrieben, dem [.guihint]#Provider# zugeordnet. In einem bestehenden
xref:distributed_monitoring#[verteilten Monitoring] bedeutet das aber auch, dass
...
Um die Änderungen komplett zu unterbinden, können Sie diese im Setup für
den Zeitraum der Updates sperren. Sie gelangen zu der Konfiguration über
