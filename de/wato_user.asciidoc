...
Das Ändern geschieht mit dem Befehl `cmk-passwd`, welches ab dem Patch-Release 2.1.0p16 von {CMK} zur Verfügung steht.
...
Sollten Sie ein Patch-Release vor 2.1.0p16 einsetzen, müssen Sie für die Änderungen des Passworts `htpasswd` bemühen.
Wir empfehlen, dass Sie ihr Passwort durch Verwendung der Optionen -B -C 12 mithilfe von bcrypt verschlüsseln.

[{shell}]
----
{c-omd} htpasswd -B -C 12 etc/htpasswd cmkadmin
New password: *secret*
