...
verteilten Monitoring mit Hunderten von Instanzen: Sofern ein Benutzer nur einen Teil dieser Instanzen für seine Hosts benötigt, wird die GUI auch nur die autorisierten Instanzen kontaktieren, um Ansichten aufzubauen -- was wiederum der Performance enorm zugutekommt.
...
|`user` |Darf nur die eigenen Hosts und Services sehen, in der Weboberfläche nur in für ihn xref:folder_permissions[freigegebenen Ordnern] Änderungen machen und darf generell nichts machen, was andere Benutzer beeinflusst. |Der normale {CMK}-Benutzer, der das Monitoring nutzt und auf Benachrichtigungen reagiert.
|`agent_registration` |Die Berechtigung, neue Hosts zu registrieren -- sonst nichts. |Gedacht für einen Automationsbenutzer, um xref:hosts_autoregister#[Hosts automatisch zu erstellen.] Diese Rolle gibt es nur in der {CCE}.
...
Übrigens: Beim Erzeugen einer neuen {CMK}-Instanz wird nur ein Benutzer der Rolle `admin` angelegt (`cmkadmin`).
...
Editieren Sie dazu als Instanzbenutzer die Datei `etc/htpasswd` und entfernen Sie in der Zeile des betroffenen Benutzers, hier [.guihint]#myuser#, das Ausrufezeichen:
...
Diese Zwei-Faktor-Authentifizierung basiert auf dem Internetstandard FIDO2/WebAuthn. Authentifiziert wird klassisch auf Basis von Wissen (Passwort) und Besitz (Authentifikator). Sie können Hardware-Authentifikatoren wie den YubiKey oder einen anderen USB-Token verwenden.
...
Hat nun einer dieser Benutzer keinen Zugang mehr zu {CMK}, also z. B. seinen Token verloren oder beschädigt, so können Sie die Zwei-Faktor-Authentifizierung gezielt für diesen Benutzer löschen.
...
In der Ansicht des Benutzers schalten Sie über [.guihint]#User > Disable two-factor authentication# die Zwei-Faktor-Authentifizierung für diesen Benutzer wieder aus.

image::wato_user_2fa_disable.png[alt="Deaktivierung einer Zwei-Faktor-Authentifizierung.", width=39%]
...
`cmk-passwd` wird in einer aktuellen Version von {CMK} immer eine sichere Verschlüsselungsmethode wählen, um Ihre Passwörter zu speicher.
Aktuell verwendet `cmk-passwd` dafür bcrypt.
Unverschlüsselte und schwach verschlüsselte Passwörter (bspw. mit MD5) erlauben keinen Login in die GUI.
