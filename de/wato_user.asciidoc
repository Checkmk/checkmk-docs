...
ML: Infos zum Review:
Erstmal nur de.
Bilder neu, Pfade angepasst, WATO ersetzt, diverser Kleinkram, Kapitel zu Nutzereinstellungen gestrichen und auf Intro verlinkt.
Unter 3.1 "Inventar-Sichtbarkeit" neu geschrieben.
Kommentare zu Kleinigkeiten unter 11.2 (Verhinderung von Mehrfachanmeldungen) und 13 - können gegebenenfalls gelöscht werden.
////
...
per link:wato_rules.html[Regel] allen Linux-Hosts zugeordnet. Eine direkte Zuordnung
...
[{image-border}]
image::wato_user_users.png[alt=Nutzerliste mit Eigenschaften des Admin-Users.]
...
image::wato_user_identity.png[alt=Eingabemaske für neue Nutzeridentität.]
...
image::wato_user_security.png[alt=Eingabemaske für Sicherheitseinstellungen eines Nutzers.]
...
image::wato_user_contact_groups.png[alt=Eingabemaske für Kontaktgruppen des Nutzers.]
...
image::wato_user_notifications_fallback.png[alt=Eingabemaske für Alarmierungseinstellungen.]
...
image::wato_user_personal_settings.png[alt=Eingabemaske für persönliche Nutzereinstellungen.]
...
immer in der icon:icon_main_help[] Onlinehilfe.

[#user_config_interface]
=== Oberflächeneinstellungen

image::wato_user_interface_settings.png[alt=Eingabemaske für Oberflächeneinstellungen des Nutzers.]

Auch die Oberflächeneinstellungen können Nutzer selbst über [.guihint]#User > Edit profile# anpassen.
Besonders interessant ist die Option [.guihint]#Set custom show mode:# In vielen Menüs und Listen von
{CMK} werden standardmäßig nicht alle Einträge angezeigt, sondern erst per Klick auf [.guihint]#more#.
Wenn Sie immer alles sehen wollen, können Sie dies hier mit [.guihint]#Enforce show more#
erzwingen.
...
image::wato_user_contact_groups_list.png[alt=Liste mit Kontaktgruppen.]
...
image::wato_user_contact_groups_new.png[alt=Eingabemaske für Name und Alias von Kontaktgruppen.]
...
Zusätzlich können Sie die Sichtbarkeit des xref:inventory#[Hardware-/Software-Inventars] festlegen. Standardmäßig
ist das komplette Inventar sichtbar, es lässt sich aber auch komplett unterdrücken oder
gezielt freischalten. Um beispielsweise ausschließlich einige ausgewählte Inventurdaten
zum Prozessor sichtbar zu machen (Modell und Architektur), können Sie dies über die Option
[.guihint]#Allowed to see the following entries# und die *internen Inventur-Pfade* erledigen.

image::wato_user_contact_groups_inventory_00.png[alt=Eingabemaske für die Sichtbarkeit von Inventurdaten.]

Gehen Sie dazu wie folgt vor:

Rufen Sie zunächst das [.guihint]#HW/SW Inventory# eines (betroffenen) Hosts auf
und aktivieren Sie die Anzeige der internen Inventur-Pfade über [.guihint]#Display > Modify display options#
und die Option [.guihint]#Show internal tree paths.#

image::wato_user_contact_groups_inventory_01.png[alt=Option zum Anzeigen der Inventur-Pfade.]

Nun sehen Sie im Inventar selbst die internen Bezeichnungen, so heißt der interne Pfad
für den Bereich [.guihint]#Processor# beispielsweise [.guihint]#hardware.cpu#. Die
Bezeichnungen für CPU-Modell und -Architektur, [.guihint]#model# und [.guihint]#arch#,
finden Sie darunter in den CPU-Daten.

image::wato_user_contact_groups_inventory_02.png[alt=Inventurdaten mit hervorgehobenen internen Pfaden.]

Im Bereich [.guihint]#Permissions# von Kontaktgruppen geben Sie nun den CPU-Pfad bei [.guihint]#Path#
und die konkreten CPU-Datensätze bei [.guihint]#Attributes# an.

image::wato_user_contact_groups_inventory_03.png[alt=Eingabemaske für die Sichtbarkeit von Inventurdaten mit CPU-Filter.]

Das Resultat: Nutzer der Kontaktgruppe sehen nur noch ein abgespecktes Inventar.

image::wato_user_contact_groups_inventory_04.png[alt=Inventar mit ausgewählten Datensätzen.]


Die Zuordnung von Hosts und Services geschieht wie folgt:
...
Hosts, was wir link:wato_user.html#folder_permissions[weiter unten] im Detail zeigen.
...
[.guihint]#Setup > Host monitoring rules.# In diesem Regelsatz finden
...
image::wato_user_contact_groups_rules_list.png[alt=Zuordnungen von Hosts zu Kontaktgruppen.]
...
image::wato_user_contact_groups_rules_new.png[alt=Dialog zum Zuordnen von Hosts zur Kontaktgruppe Windows-Servers.]
...
In Ihren link:wato_user.html#personal_settings[persönlichen Einstellungen]
...
* Die [.guihint]#Overview# in der Seitenleiste
...
image::wato_user_authorization_settings.png[alt=Dialog mit Authorisierungseinstellungen.]
...
werden. Das geschieht durch eine Alarmierungsregel, die bei neuen Instanzen
...
|user |Darf nur seine Hosts und Services sehen, in der Weboberfläche nur in für ihn freigegebenen Ordnern Änderungen machen und darf generell nichts machen, was andere Benutzer beeinflusst. |Der normale {CMK}-Benutzer, der das Monitoring nutzt und auf Alarme reagiert.
...
image::wato_user_roles_permissions.png[alt=Liste mit Freigaben für angelegte Nutzerrolle.]
...
image::wato_user_roles_new_role.png[alt=Basiseigenschaften einer angelegten Rolle.]
...
Er erzeugt folgende Darstellung,
...
image::wato_user_roles_matrix.png[alt=Matrix mit Nutzerrollen im Vergleich.]

[#personal_settings]
== Persönliche Einstellungen

Einen kleinen Teil der Benutzereinstellungen kann jeder Benutzer für sein Profil
selbst verwalten. Eine genaue Beschreibung aller Optionen finden Sie im Kapitel
xref:user_interface#user_menu[zur Benutzeroberfläche.]
...
übertragen. Nur so ist sichergestellt, dass insbesondere das neue Passwort auch
...
* Verwalten von Hosts per link:rest_api.html[REST-API]
...
. Ermitteln Sie die eigentlich anzuzeigende URL (z.B. die des Dashboards) mit Ihrem Browser -- am besten ohne Navigation, was über [.guihint]#Display > This site without navigation# geht.
...
// ML: Die curl-Aufrufe und die Meldungen sind komplett neu, mit Windows und Linux durchgespielt -, aber guck vielleicht nochmal drüber, vielleicht kann man da auch noch ein wenig an der Ausgabe kürzen.
...
Ergebnis ein „`302 Found`“ und eine Weiterleitung auf die angegebene „`Location`“:


[{shell-raw}]
----
{c-omd}  curl -v 'http://localhost/mysite/check_mk/login.py?_origtarget=/mysite/check_mk/dashboard.py?name=mydashboard&_username=myuser&_password=mypassword&_login=1'
...
Bei falschen Login-Daten bekommen Sie die HTTP-Meldung „`200 OK`“,
sehen aber lediglich den HTML-Code der Anmeldeseite:

[{shell-raw}]
----
{c-omd} curl -v 'http://localhost/mysite/check_mk/login.py?_origtarget=/mysite/check_mk/dashboard.py?name=mydashboard&_username=myuser&_password=NOT&_login=1'

< HTTP/1.1 200 OK

<!DOCTYPE HTML>
<html><head><meta content="text/html; charset=utf-8" http-equiv="Content-Type" /><link href="themes/modern-dark/images/favicon.ico" rel="shortcut icon" type="image/ico" /><title>Checkmk Local site mysite</title><link href="themes/modern-dark/theme-2.0.0-2021.08.13.css" rel="stylesheet" type="text/css" /><script type="text/javascript" src="js/main_min-2.0.0-2021.08.13.js"></script></head><body data-theme="modern-dark" class="main login"><div id="main_page_content"><script type="text/javascript">
...
die Quere kommen, können Sie Berechtigungen für das {CMK} link:wato.html[Setup] auf der Basis von Ordnern vergeben.
...
grundsätzlich eine Berechtigung für die  Konfigurationsumgebung, allerdings mit einigen wichtigen
Einschränkungen:
...
machen können! Das [.guihint]#Setup#-Menü der Navigationsleiste sieht für normale Anwender so aus:

image::wato_user_setupmenu.png[width=65%, align=center, alt=Setup-Menü aus Nutzersicht.]
...
image::wato_user_password_policy.png[alt=Eingabemaske für Passwort-Regeln.]
...
image::wato_user_forced_password_change.png[alt=Eingabemaske für erzwungene Passwort-Neuvergabe.]
...
image::wato_user_login_failures.png[alt=Eingabemaske für automatische Login-Deaktivierung.]
...
image::wato_user_login_idle_timeout.png[alt=Eingabemaske für Login-Timeouts.]
...
image::wato_user_limit_login.png[alt=Eingabemaske für Session-Limits.]

Bei dem Versuch einer parallelen zweiten Anmeldung sehen Sie dann
folgenden Fehler:

image::wato_user_another_session_is_active.png[width=60%, align=center, alt=Gesperrter Login-Dialog mit Hinweis auf laufende Session.]
...
Nach dem Aktivieren dieser globalen Option *kann* es Ihnen passieren, dass sie vom System
abgemeldet werden und sich nicht mehr einloggen können - wenn noch eine alte Session auf
einem anderen System als aktiv gilt. Sollten Sie kein weiteres Administratorkonto für die
Korrektur haben, können Sie die Option händisch aus der zugehörigen Textdatei entfernen.
Anschließend können Sie sich wieder normal in der Weboberfläche einloggen. Die globalen
Einstellungen finden Sie als Instanzbenutzer in der Datei [.guihint]#etc/check_mk/multisite.d/wator/global.mk#.
////
...
können Sie über [.guihint]#Setup > Users > Custom user attributes > Add user attribute#
...
image::wato_user_custom_macro.png[alt=Eingabemaske für individuelle Nutzerattribute.]
...
das völlig getrennt von der Alarmierung funktioniert. Das Tool finden Sie
...
image::wato_user_notify_users.png[alt=Eingabemaske für Benutzeralarmierungen.]
...
|[.guihint]#Open Window in the user interface# |Beim nächsten Seitenaufruf des Benutzers wird ein Popupfenster mit der Nachricht geöffnet.
|[.guihint]#Show hint in the 'User' menu# |Der Benutzer wird durch ein Zahlensymbol in der Navigationsleiste auf die Nachricht hingewiesen.
...
|[.guihint]#Show notification in dashboard element 'User notifications'# |Die Nachricht wird in einem link:dashboards.html#adddashlet[Dashlet] des Typs [.guihint]#User notifications# angezeigt.
|===

////
ML: Ticket wird folgen: Aktiviert man Popup UND User-Menu-Hinweis, erscheint letzterer erst nach Bestätigung des Popups.
Die Bestätigung löscht allerdings die Nachricht selbst und der Hinweis im User-Menü führt ins Leere.
////
...
|etc/check_mk/multisite.d/wato/users.mk |Enthält die über die Konfigurationsumgebung eingerichteten Benutzer. Hier sind nur diejenigen Daten über den Benutzer gespeichert, die sich rein mit der GUI befassen. Manuelle Änderungen in dieser Datei werden sofort wirksam.
|etc/check_mk/conf.d/wato/contacts.mk |Kontaktinformationen der über die Konfigurationsumgebung verwalteten Benutzer. Hier sind alle Daten abgelegt, die für die Konfiguration des Monitoringkerns relevant sind. Nur Benutzer, die auch Kontakte sind, sind hier aufgeführt. Manuelle Änderungen hier benötigen anschließend ein `cmk -O` (Core reload), um wirksam zu werden.
