...
|`agent_registration` |Die Berechtigung, neue Hosts zu registrieren -- sonst nichts. |Gedacht für einen Automationsbenutzer, um xref:draft_hosts_autoregister#[Hosts automatisch zu erstellen.] Diese Rolle gibt es nur in der {CCE}.
...
* die Webdomaine wird durchgängig im gleichen Format eingegeben, also beispielsweise immer +https://www.mycompany.com/mysite+.
...
Danach werden Sie automatisch ausgeloggt.
