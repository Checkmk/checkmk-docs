...
link:user_interface.html[Benutzeroberfläche] hat. Er hat eine oder meh&shy;rere
*[.guihint]#Rollen}}.* Aus den Rollen ergeben sich {{*Berechtigungen* (Permissions)#.
...
== Benutzerverwaltung über WATO#user_config

Die Benutzerverwaltung finden Sie im link:wato.html[WATO]-Modul ICON[icon_users.png] [.guihint]#Users#.
In einer frisch angelegten Instanz sieht diese Seite so aus:

image::wato_users.png[]
...
image::users_authorized-sites_classic.png[]
...
werden soll, der per Email [notifications|alarmiert] werden soll (link:install_packages.html#smtpconfig[SMTP-Konfiguration] notwendig). Analog ist das Feld
...
Über [.guihint]#Autorized sites# dürfen Sie optional beschränken, auf welche der vorhandenen Sites
...
Kontakt&shy;gruppe [.guihint]#Everything# angelegt, die immer alle Hosts und alle
Services enthält (in früheren Versionen hieß diese Gruppe [.guihint]#Everbody#,
...
ICON[icon_contactgroups.png] [.guihint]#Contact groups#. Folgende Abbildung zeigt
dieses Modul mit vier angelegten Kontaktgruppen:

image::contact_groups.png[]
...
image::folder_contact_groups.jpg[]
...
image::host_contact_groups2.jpg[center, width=500]
...
[.guihint]#Assignment of services to contact groups# Services zu Kontakt&shy;gruppen zuordnen --
...
Einträge [.guihint]#Contact groups}} und {{Contacts#, welche die letztendliche Zuordnung
für dieses Objekt auflisten.


image::host_status_contact-groups.png[]


== Sichtbarkeit von Hosts und Services#visibility
...
dort [.guihint]#Hosts}} auf {{Strict - Must be explicit contact of a service#
...
image::authorization_settings.jpg[]
...
|`user`
|Darf nur seine Hosts und Services sehen, in WATO nur in für ihn freigegebenen Ordnern Änderungen machen und darf generell nichts machen, was andere Benutzer beeinflusst.
|Der normale {CMK}-Bentuzer, der das Monitoring nutzt und auf Alarme reagiert.


|`guest`
|Darf alles sehen aber nichts ändern.
|Gedacht zum einfachen „Gucken“, wobei sich alle Gäste einen gemeinsamen Account teilen. Auch nützlich für öffentliche Statusbildschirme, die an der Wand hängen.

|===


Die Rollen werden im WATO-Modul ICON[icon_roles.png] [.guihint]#Roles & Permissions# verwaltet:
...
image::role_matrix.jpg[]


== Persönliche Einstellungen#personal_settings

image::sidebar_bottom.png[left, width=200]
...
== Automationsbenutzer (für Webservices)#automation
...
== Berechtigungen in WATO#wato_permissions
...
* Es sind lediglich Änderungen an Hosts, Services, [wato_rules|Regeln] und link:bi.html[BI-Aggregaten] erlaubt.
...
image::wato_snapin_user.png[center, width=220]
...
der Status&shy;ober&shy;fläche dann für die Kontaktgruppe [.guihint]#Linux# nicht sichtbar
...
image::forced_password_change.png[center, width=500]
...
image::limit_login.png[]

Bei dem Versuch einer parallelen zweiten Anmeldung sehen Sie dann
folgenden Fehler:

image::another_session_is_active.png[center, width=300]
...
=== Passwort auf der Kommandozeile ändern#change_passwd
...
noch ein Feld [.guihint]#Pager# zur Verfügung. Wenn Ihnen das nicht ausreicht und Sie noch
mehr Informationen zu einem Benutzer speichern möchten, können Sie mit dem Knopf
[.guihint]#Custom macros# ICON[button_custom_macros.png] eigene Felder erzeugen, die dann
...
image::notify_users.jpg[]

Dabei haben Sie die Wahl zwischen drei Nachrichtenarten:


|===
|[.guihint]#Send an email#
|Versendet eine Email. Damit erreichen Sie aber nur Benutzer, bei denen auch eine Emailadresse konfiguriert ist.

|[.guihint]#Popup message in the GUI (shows up alert window)#
|Beim nächsten Seitenaufruf des Benutzers wird ein Popupfenster mit der Nachricht geöffnet.

|[.guihint]#Send hint to message inbox (bottom of sidebar)#
|Der Benutzer wird durch ein Zahlensymbol ICON[notify_users4.png] am unteren Ende der Seitenleiste auf die Nachricht hingewiesen und kann diese dann abrufen. 
|===
...
|`etc/auth.secret`
|Diese Datei enthält ein zufälliges Geheimnis, mit dem Anmeldecookies signiert werden. In verteilten Umgebungen soll diese Datei in allen Instanzen gleich sein. Wenn Sie alles mit WATO einrichten, sorgt dieses automatisch dafür. Wird diese Datei geändert, so werden alle Anmeldungen sofort ungültig und Benutzer müssen sich neu anmelden. Diese Datei ist mit den Rechten `660` versehen, da ein Lesezugriff von Dritten das Fälschen einer Anmeldung ermöglichen würde.


|`etc/auth.serials`
|Seriennummern der Passwörter pro Benutzer. Jede Änderung des Passworts erhöht die Seriennummer und macht damit alle aktuellen Sitzungen ungültig. Damit ist sichergestellt, dass eine Passwortänderung einen Benutzer zuverlässig ausloggt.


|`etc/check_mk/multisite.d/wato/users.mk`
|Enthält die über WATO eingerichteten Benutzer. Hier sind nur diejenigen Daten über den Benutzer gespeichert, die sich rein mit der GUI befassen. Manuelle Änderungen in dieser Datei werden sofort wirksam.


|`etc/check_mk/conf.d/wato/contacts.mk`
|Kontaktinformationen der über WATO verwalteten Benutzer. Hier sind alle Daten abgelegt, die für die Konfiguration des Monitoringkerns relevant sind. Nur Benutzer, die auch Kontakte sind, sind hier aufgeführt. Manuelle Änderungen hier benötigen anschließend ein `cmk -O` (Core reload), um wirksam zu werden.


|`var/check_mk/web`
|Jeder Benutzer, der sich mindestens einmal an der GUI angemeldet hat, hat hier ein Verzeichnis, in dem Dinge wie selbst erstellte Ansichten und Berichte, aktuelle Konfiguration der Seitenleiste und vieles anderes in einzelnen kleinen Dateien mit der Endung `.mk` gespeichert sind. Diese Dateien haben Pythonformat.
