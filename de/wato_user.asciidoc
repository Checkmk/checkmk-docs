...
Welche Bedeutung die zahlreichen Berechtigungen haben erfahren Sie aus der icon:icon_help[] Onlinehilfe.
...
(Symbol icon:icon_perm_yes.png]) bzw. entfernt (Symbol ICON[icon_perm_no[]) wurden.
...
können Sie mit dem icon:button_random[] Würfel automatisch erstellen lassen:
...
[.guihint]#Custom macros# icon:button_custom_macros[] eigene Felder erzeugen, die dann
...
|Der Benutzer wird durch ein Zahlensymbol icon:notify_users4[] am unteren Ende der Seitenleiste auf die Nachricht hingewiesen und kann diese dann abrufen. 
|===
