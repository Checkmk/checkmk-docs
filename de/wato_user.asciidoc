...
TK: Infos vom Reviewer :-)
1) Kleinigkeiten direkt erledigt, inkl. guihints mit Bold aufgelöst (in der Einleitung), alt-Texte in Anführungszeichen gesetzt, einige Links korrigiert, ersetzt und neue hinzugefügt, icon_help.png ersetzt, Terminologie auf Stand gebracht - insbesondere (aber nicht nur) Alarm/Alarmierung > Benachrichtigungen. Daher musste ich Benachrichtigung in Kap. 13 zur Meldung machen.
2) Du hast in diversen Screenshots die Navileiste mit drin: Ist meiner Meinung nach überflüssig (bis auf wato_user_setupmenu.png) - und der Breadcrump-Pfad oben zeigt eh, wie man da hingekommen ist.
3) Warauf Du nochmal schauen solltest, steckt in den Kommentaren.
////
...
Wenn ein Kontakt nur zum Zwecke der link:notifications.html[Benachrichtigung] angelegt wurde
...
werden soll, der per E-Mail link:notifications.html[benachrichtigt] werden soll (link:notifications.html#smtp[SMTP-Konfiguration] notwendig). Analog ist das Feld
...
link:wato_user.html#roles[weiter unten]) macht dies jedoch wenig Sinn.
...
link:notifications.html#fallback[keine Benachrichtigungsregel greift.]
...
// TK: Unschön finde ich den Sprung Benutzerverwaltung > Inventory und wieder zurück.
// TK: Eleganter wäre es im inventory im Kap. "3. Mit den Inventurdaten arbeiten" ein neues Unterkapitel anzulegen.
// TK: Arbeitstitel: Interne Pfadinformationen anzeigen - und wofür man sie gebrauchen kann
// TK: Dann Link dahin und der Leser kann in der Benutzerverwaltung bleiben.

Zusätzlich können Sie die Sichtbarkeit des mit der xref:inventory#[Hardware-/Software-Inventur] gefundenen Inventars festlegen. Standardmäßig
...
Um die geforderten Pfadinformationen eingeben zu können, müssen Sie diese zuerst aus den Inventurdaten auslesen.
...
Zurück im Bereich [.guihint]#Permissions# von Kontaktgruppen geben Sie nun den CPU-Pfad bei [.guihint]#Path#
...
// TK: Uff. Die beiden nächsten Absätze sind sehr wirr und quasi unverständlich.
Der eigentliche Sinn dieser Option ist das Setzen von Berechtigungen für das Pflegen von
Hosts, was wir xref:wato_user#folder_permissions[weiter unten] im Detail zeigen.
// TK: Den Anker folder_permissions gab es nicht, ich hab ihn für das Kap. "Benutzern das Verwalten von Hosts ermöglichen" gesetzt.
...
* link:views.html[Tabellenansichten] von Hosts und Services
* link:dashboards.html[Dashboards]
* Das Snapin xref:user_interface.html#overview[Overview] der Seitenleiste
...
Details erfahren Sie im link:notifications.html[Artikel über die Benachrichtigungen.]
...
|===
// TK: Ich hab immer dann, wenn "freigegebene Ordner" vorkommt (wie in der 1. Tabellenzeile), verlinkt, damit klar wird, was mit der "Freigabe" gemeint ist.
...
Automationsbenutzer `automation` und dem Automationspasswort aus der obigen
Abbildung:
...
// TK: Hat alles funktioniert: Ich hab in den URLs den Server geändert und den HTML-Code gekürzt (auch wenn das nicht gern gesehen wird...)
...
// TK: Warum jetzt Administrator-Kollegen und nicht mehr "Sie"? Es geht doch gar nicht um die Zusammenarbeit mehrerer Admins.
...
machen können! Das [.guihint]#Setup#-Menü der Navigationsleiste sieht für normale Benutzer so aus:

image::wato_user_setupmenu.png[width=65%, align=center, alt="Setup-Menü aus Benutzersicht."]
// TK: Braucht es diesen Screenshot? Es wird hier doch gar nicht deutlich, dass der user nichts/wenig machen kann.
// TK: Wenn er drinbleiben soll, dann aber die Überschrift mit "Problem dashboard" wegschneiden.

[#folder_permissions]
=== Benutzern das Verwalten von Hosts ermöglichen

Die Berechtigung für das Anlegen, Editieren und Entfernen von Hosts
erhält ein Benutzer über link:wato_user.html#contact_groups[Kontaktgruppen.] Der Ablauf ist wie folgt:
...
== Passwortänderung, Passwortrichtlinien
// TK: Wie wär es denn mit dem simplen Titel "Passwörter"?
...
entfernen Sie in der Zeile des betroffenen Benutzers das Ausrufezeichen:
...
cmkadmin:$apr1$bhGavmqd...
----
// TK: Schöner wärs, myuser rauszuschmeissen und das Ausrufezeichen bei cmkadmin zu setzen. Dann passt es auch zum Text.
// TK: Übrigens: Wie hilitet man hier?
...
// TK: Im screenshot steht noch 1.6.0p11
...
Einstellungen finden Sie als Instanzbenutzer in der Datei `etc/check_mk/multisite.d/wator/global.mk`.
////
// TK: Mirco, mach es nicht so spannend: Wie heißt denn jetzt die Einstellung? 
...
// TK: Den htpasswd-Inhalt hab ich rausgeworfen: War eh veraltet.
...
Denn dazu muss der Wert dem Monitoring-Kern (z.B. link:cmc.html[CMC]) in einer Variablen
...
Im link:notifications.html[Artikel über Benachrichtigungen] gehen wir sehr ausführlich
...
TK: Hmmm, anderes Verhalten in 2.0.0p9: Die Nummer im User-Menü ist bei mir zu sehen, während das Popup angezeigt wird.
Nach Popup-Bestätigung verschwindet die Nummer, aber erst nach einigen Sekunden.
Dafür funktioniert bei mir was anderes nicht: Wenn ich nur den User-Menu-Hinweis wähle, erscheint die Nummer und ich kann mir die Meldung
in der Meldungsliste ansehen. Ich kann sie aber nicht löschen: Klick auf Trash tut nix und Nummer bleibt. Achja...
////
...
// TK: Ich würde umformulieren und die letzten beiden Listeneinträge auskommentieren: Mir ist kein Ticket bekannt für diese 2 Themen.
