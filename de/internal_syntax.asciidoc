...
aber mit einem Icon icon:brokenlink[] als fehlend markiert).
...
- link:intro.html#editions[Das offizielle Handbuch zu {CMK}]
...
|H1: |Überschrift Ebene 1 |Diese Überschrift taucht im Inhaltsverzeichnis des Artikels auf und wird automatisch durchnummeriert. Es definiert ein _Kapitel_. Ein Kapitel braucht nicht unbedingt mit `H2:` weiter untergliedert zu sein. Falls es das ist, darf zwischen `H1:` und `H2:` kein Text stehen.  *Achtung:* Die Überschrift darf nicht so lang sein, dass es im Inhaltsverzeichnis (Kasten rechts oben) einen Umbruch gibt!
|H2: |Überschrift Ebene 2 |Diese Überschrift wird mit zwei Ziffern nummeriert und taucht ebenfalls im Inhaltsverzeichnis auf.
|H3: |Zwischenüberschrift |Keine Nummerierung. Dient nur der Gliederung. Zwischen einem `H2:` und dem folgendem `H3:` darf ein Textkörper stehen. Ein `H3:` ist aber _nur_ nach einem `H2` erlaubt.
|===
...
|<td class=right> |Rechtsbündige Darstellung -- vor allem für Zahlen. dass `right`-Attribut darf auch in `<th>` verwendet werden.
|===
...
|<i>text</i> |_Kursivschrift_ im Fließtext. Dies wird verwendet bei der Einführung oder erstmaligen Verwendung von Begriffen und bei einer milden Hervorherbung.
...
|<tt>omd config</tt>` |Zitate von der _Kommandozeile_, Eingaben, die der Benutzer 1:1 in der GUI eingeben muss: Melden Sie sich als `omdadmin` mit dem Passwort `omd` an. Dieser Zeichsatz symbolisiert, dass hier eine _exakte_ Übereinstimming wichtig ist. Der Zeichensatz ist daher auch optimiert für die Erkennung von Unterschieden zwischen `O` und `0` und dergleichen.  Achtung: <tt> in `H1:`, `H2:` und `H3` sind nicht erlaubt!
|{{Service description}} |Zitat eines Textes aus der {CMK}-Benutzeroberfläche. Dies wird aktuell [.guihint]#kursiv# dargestellt, aber das kann sich später eventuell ändern. Das ist also *nicht* das gleiche wie <i>_Kursivschrift_</i>, auch wenn es erstnmal gleich aussieht.  Wenn etwas gleichzeitig ein Zitat *und* eine exakte Übereinstimmung bedeutet (z.B. das Zitieren einer ID), dann gewinnt `<tt>`
|{{Global settings|Foo|Bar}} |Navigationspfad in der {CMK}-Benutzeroberfläche. Darstellung ist aktuell [.guihint]#Global settings => Foo => Bar#. Der Pfad beginnt meist mit dem WATO-Modul. Wir verwenden auch im Deutschen Handbuch immer die Texte der englischen GUI.
|===
...
|<br> |Zeilenumbruch: nur einsetzen bei ernsten Layoutproblemen!
|&shy; |Weiches Trennzeichen, Trennvorschlag
...
Dieses tolle Feature geht natürlich nur bei der {EE}. Und wenn Sie die {EE}
...
|(CMK) |Produktname {CMK}. Bitte anstelle des ausgeschriebenen Check_MK immer dieses Makro verwenden, damit wir später eine Schreibung anpassen können, wenn nötig.
| (OK) (WARN) (CRIT) + (PEND) (UNKNOWN) + (UP) (DOWN) (UNREACH) |Bezeichnet Monitoringzustände. Die Darstellung ist: {OK} {WARN} {CRIT} {PEND} {UNKNOWN} {UP} {DOWN} {UNREACH}
|(CRE) |Die CRE in Langform: {CRE}
|(CSE) |Die CEE in Langform: {CSE}
|(CFE) |Die CFE in Langform: {CFE}
|(CME) |Die CME in Langform: {CME}
|(CEE) |Alle {EE} in Langform: {CEE}
|(RE) |{RE} für häufige Nutzung im Textfluss
|(SE) |{SE} für häufige Nutzung im Textfluss
|(FE) |{FE} für häufige Nutzung im Textfluss
|(ME) |{ME} für häufige Nutzung im Textfluss
|(EE) |Die {EE} im Allgemeinen, also die {SE}, {FE} oder {ME}, im Textfluss für häufige Nutzung.
|ICON[icon_help.png] |Dient dem Einbetten von kleinen Bildern aus der GUI, die so skaliert und positioniert werden, dass sie in den normalen Textfluss passen. Wenn die Icons zur Illustration eingesetzt werden, sollen sie immer _vor_ dem Begriff stehen, z.B. wie bei der icon:icon_help[] Onlinehilfe. Man kann damit auch Screenshots von Knöpfen wie icon:button_activate_changes[] einbinden.
|(NEW) |(NEW) Ein Hinweis auf einen neuen Artikel. Dieser Hinweis ist für das Inhaltsverzeichnis (`index`) vorgesehen.
|VERSION[1.4.8i2]` |Hinweis auf eine bestimmte {CMK}-Version: Ab Version VERSION[1.4.8i2]
...
Mehr zu Konventionen zu Screenshots weiter link:internal_syntax.html#screenshots[unten].
...
image::matrix_view_1.jpg[align=center, width=60%]
...
image::snapin_site_status.png[float=left, width=30%]
...
Hinweise zum Erstellen von Screenshots findest du link:internal_syntax.html#screenshots[weiter unten].
...
image::folder_monitored_on.png[align=center, width=90%]
...
ausgezeichnet. Das erscheint dann . Ganze Zeilen auskommentieren
...
image::new_host_3.png[align=center, width=500]
...
wird das border-Attribut gesetzt, da link:internal_syntax.html#whiteborder[weiße Randbereiche]
entstehen.

[.border]
image::border_example_dialog.png[]
...
icon:button_frameurl[] icon:button_pageurl[] aus. Diese sollen _nicht_
in den Screenshot.
...
image::bi_all_aggr_ex5.png[]

* Schrift viel zu klein - kaum leserlich bei der Standardbreite im Handbuch
* Auch hier hätte wohl der Inhalt in [.guihint]#Tree# vollkommen gereicht
...
[cols="30,20,20,~"]
|===
|Schriftgröße im Bild |Bildbreite |Bereich |Rand bei border-Attribut 

|16 px |1200 |1180 - 1220 |32 px
|17 px |1275 |1250 - 1300 |34 px
|18 px |1350 |1330 - 1370 |36 px
|19 px |1425 |1400 - 1450 |38 px
|20 px |1500 |1480 - 1520 |40 px
|21 px |1575 |1550 - 1600 |42 px
|22 px |1650 |1630 - 1670 |44 px
|23 px |1725 |1700 - 1750 |46 px
|24 px |1800 |1780 - 1820 |48 px
|25 px |1875 |1850 - 1900 |50 px
|26 px |1950 |1930 - 1970 |52 px
|27 px |2025 |2000 - 2050 |54 px
|28 px |2100 |2080 - 2120 |56 px
|29 px |2175 |2150 - 2200 |58 px
|30 px |2250 |2230 - 2270 |60 px
|31 px |2325 |2300 - 2350 |62 px
|32 px |2400 |2380 - 2420 |64 px
|33 px |2475 |2450 - 2500 |66 px
|===
...
image::ec_performance.png[align=center, width=42%]
...
gemäß Tabelle ein <b>Breite von
1575 px* und folglich an allen Seiten *42 px weißen Rand:*

[.border]
image::border_example_list.png[]
...
Für die Inhalte der Screenshots gelten die link:internal_syntax.html#beispiele[Konventionen für Beispiele].
...
[cols="8,8,84"]
|===
|Kürzel |Makros |Schreibung lang / kurz 
...
[{shell}]
----
{c-root} aptitude install texlive-latex-recommended texlive-math-extra texlive-lang-german texlive-latex-extra gv texlive-pstricks texlive-generic-recommended
----
...
----
 <table>
 <tr>
 <td class="tt" [hilite]#style="width:20%"#>Updated</td>
...
[#presentation]
=== Sonderfälle für die Trainings-Artikel
...
. Epic: Große Aufgabe, die fortwährend andauert (z.B. Legacy-Dokumentation loswerden). So ein Ticket beinhaltet normalerweise Subtickets.
. Story: Normale Aufgabe, z.B. einen Artikel schreiben. Oder etwas anderes, was eher Tage als Stunden Zeit braucht.
. Task: Kleine Aufgabe oder Bug, etwas, das man in kurzer Zeit erledigen kann.
...
setzt den Status auf [.guihint]#Ready for Translation#.
