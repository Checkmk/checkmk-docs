...
// IGNORE Aliasbezeichnung Aliasnamen Anwendungsfälle Balancer Fernsteuerungs Filialnummer Integrationen Inventarnummern
// IGNORE Lüfterdrehzahlen Rescan Umbenennungsaktion agentenlos gepingt gescannt
...
// MFS: Evtl. ...setzen Sie im Regelfall zusätzlich im... um zu verdeutlichen, dass es durchaus Fälle gibt, wo Smart Ping trotz Push benutzt werden will.
...
// MFS: Sollen wir hier für die Suche noch "Lights Out Management/LOM" nachtragen?
Die Bezeichnung [.guihint]#Management board# steht für separate Steckkarten oder erweiterte BIOS-Funktionalität (Baseboard Management Controller/BMC, Management Engine/ME) zur Überwachung und Verwaltung der Hardware neben dem installierten Betriebssystem.
...
Dagegen bringt Sie [.guihint]#Save & go to connection tests# zum Verbindungstest. Damit können Sie zuerst einmal testen, ob Sie mit den festgelegten Einstellungen überhaupt Daten von dem Host bekommen -- sei es per Agent oder was auch immer Sie vorher konfiguriert haben.
...
[#hosts_autoremove]
== Hosts automatisch entfernen lassen
...
// TK: Unter den Bulk-Aktionen (siehe oben) gibt es "Detect network parent hosts": Ist das die gleiche Funktion unter anderem Namen oder was anderes?
