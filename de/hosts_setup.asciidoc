...
Folgendes Beispiel zeigt die Vererbung der Merkmale „Criticality“ (mit den Werten `prod` und `test`) und „Checkmk agent / API integrations“ (mit den Ausprägungen `tcp` und `no-agent`).
