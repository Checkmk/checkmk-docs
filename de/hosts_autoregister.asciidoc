...
Im folgenden Beispiel wird neben dem vordefinierten Label `cmk/hostname-simple` auch noch das benutzerdefinierte Label `my_autoregister_label` geprüft: 
...
Sie können auch hier mit [.guihint]#Add some label# eigene Labels definieren.
...
// TK: Ist das folgende hier nicht zu versteckt und/oder kommt zu spät?
Automatisch erstellte Hosts können Sie auch wieder xref:hosts_setup#hosts_autoremove[automatisch entfernen lassen.]
Damit bietet die {CE} die beiden wichtigsten Komponenten für die Lebenszyklusverwaltung (_lifecycle management_) von Hosts.
...
// MFS: Im Artikel zum Windows-Agenten ist mit 5.7 unter dem Anker #test mittlerweile ebenfalls ein Hinweis auf die Logdatei, welche Erhellung verschafft.
...
// MFS: Im Windows-Artikel sind nun auch die analogen Pfade enthalten:
...
// MFS: Well, zu Debugging-Zwecken zumindest während der Beta sinnvoll.
|`/var/lib/cmk-agent/pre_configured_connections.json` |Enthält eine vorkonfigurierte und per Agentenbäckerei in das Agentenpaket integrierte Verbindung zu einer Instanz für die Autoregistrierung.
// TK: Der Plural in registered_connections.json ist wohl falsch, da ja nur eine Verbindung eingebacken werden kann.
// MFS: Eingebacken ja. Manuell können mehrere enthalten sein. Ich kann mir mittelfristig im Distributed Monitoring Fälle vorstellen, wo ein Host selbst seinen Monitoring-Server findet - nämlich den, der für ihn erreichbar ist. Das bereits in der Konfigurationsdatei vorwegzunehmen, ist nachbvollziehbar.
