...
= Monitoringagenten
:revdate: draft
:title: Monitoringagenten - Mit {CMK} Geräte im Netzwerk überwachen
...
Die Komponente, die diese Auskunft gibt, ist immer ein aktives Stück Software -- also ein Agent.
...
// TK: Hier sollte Check_MK durch Checkmk ersetzt werden.
...
In der {RE} führen Sie die Menüeinträge [.guihint]#Linux#, [.guihint]#Windows# und [.guihint]#Other operating systems# direkt zu den Download-Seiten, auf denen Sie die vorkonfigurierte Agenten und Agenten-Plugins finden, im folgenden Beispiel zur Download-Seite für [.guihint]#Linux, Solaris, AIX#:
...
Über das Menü [.guihint]#Related# können Sie übrigens auch die oben für die {RE} genannten Download-Seiten aufrufen.
...
Alle weitere Konfiguration geschieht über Konfigurationsdateien und die Installation von Plugins durch Download von der gleichen Seite sowie manuelles Kopieren in die entsprechenden Verzeichnisse.
...
Details zu bestimmten Agenten finden Sie deswegen in eigenen Artikeln:
...
* Verwendung des xref:inventory#[Hardware-/Software-Inventur] (Plugin nötig)
...
Bei Linux haben Sie dabei die Wahl zwischen den Paketformaten RPM (für für Red Hat, CentOS, SLES) und DEB (für Debian, Ubuntu.) sowie einem sogenannten „Tarball“ im TGZ-Dateiformat, der einfach als `root` unter `/` ausgepackt wird. 
...
image::agent_bakery_agentlist.png[alt="Liste mit zwei Konfiguration der Agenten zum Download."]

In der Spalte [.guihint]#Hosts# finden Sie eine Liste von Hosts, die der jeweiligen Konfiguration zugeordnet sind.
Aus Platzgründen ist diese Liste eventuell nicht vollständig sein. 
Eine Sonderrolle nehmen die beiden Namen [.guihint]#VANILLA# und [.guihint]#GENERIC# ein.
Diese beiden Pseudo-Hosts sind immer vorhanden und haben folgende Funktion:
...
Sie erreichen die Agentenpakete für einen Host übrigens auch bequem über die Eigenschaften des Hosts, indem Sie in [.guihint]#Setup > Hosts > Hosts# den Host anklicken und im Menü [.guihint]#Hosts# den Eintrag icon:icon_agents[alt="Symbol zur Anzeige der Monitoringagenten."] [.guihint]#Monitoring Agent# auswählen:
...
Sehr viele Regeln befassen sich mit der Installation verschiedener [.guihint]#Plugins#. 
...
Bei der Regel, welche das Plugin aktiviert, finden Sie auch gleich die Einstellungen für die Konfiguration des Plugins.
Hier als Beispiel die Regel für die Überwachung von MySQL:

image::agent_rule_mysql.png[alt="Regel für das MySQL Datenbasis Plugin des Agenten."]
...
Wenn der Host bereits in das Monitoring aufgenommen ist, können Sie die Daten auch mit dem Befehl `cmk -d` abrufen. 
Dieser verwendet dann im [.guihint]#Setup# konfigurierte IP-Adresse, berücksichtigt eine eventuell umkonfigurierte Port-Nummer und auch den Fall eines Spezialagenten:
