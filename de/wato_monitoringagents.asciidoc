...
// IGNORE Hashs Programmier erstmal umkonfigurierte
...
// MFS: Muss hier schon der Push-Modus hin?
...
// MFS: Auf die Spezialagenten folgt ein Spezialfall, der die Spezialagenten damit zum Normalfall macht?
// MFS: Mir fällt auch nix besseres ein, vielleicht "Sonderfall"?
Ein Spezialfall ist schließlich die Überwachung von Netzwerkdiensten wie HTTP, SMTP oder IMAP.
...
// MFS: OMG, ist diese Grafik inkonsistent. IMHO: Zunächst belassen, bis alle Fragen hinsichtlich Grafiken in CI und Konsistenz geklärt sind, dann beide neu erstellen. 
[{image-border}]
...
// TK: Fehlt hier für den Push-Modus noch ein Pfeil zu den Fetchern?
// MFS: Nach meinem Verständnis rufen die Fetcher bei Konfiguration für verschlüsselte Kommunikation den Agent Receiver auf, der dann die Daten liefert, egal, ob Push- oder Pull-Modus.
// MFS: Oder zieht der Agent Receiver auch bei unverschlüsselter Kommunikation die Agentenausgabe?
// MFS: Das sollte leicht herauszufinden sein, indem man mal den Agent Receiver abdreht und schaut, ob gar nix mehr ankommt oder nur Agentenausgabe von unverschlüsselten Hosts.
// MFS: Im Prinzip gehört dann noch für Legacy-Agenten rein:
// MFS: [ Fetcher ] -> [[ Internet Superserver (xinetd, BusyBox inetd…) ] -> [ Agentenskript]]
