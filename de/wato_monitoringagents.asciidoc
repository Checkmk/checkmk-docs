...
|Individuelle Agentenkonfiguration |Per xref:glossar#agent_bakery[Agentenbäckerei] können Agenten für einzelne oder Gruppen von Hosts individuell konfiguriert und die Agentenpakete für die Installation erstellt werden. |{EE}
|xref:agent_deployment#[Automatische Agenten-Updates] |Das Paket aus der Agentenbäckerei wird zuerst manuell oder per Skript installiert und wird von da an automatisch aktualisiert. |{EE}
...
{cee-only}
Wenn Sie eine der {EE} oder die {CE} verwenden, dann können Sie mit der Agentenbäckerei (_Agent Bakery_) Agenten individuell paketieren.
