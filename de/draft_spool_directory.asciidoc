...
// IGNORE OpenWRT Programmes Softlinks inhalt unpriviligierten Agentenplugin
...
MFS: Das Spool-Directory kann helfen, dass man sich das Lesen großer Datenmengen erspart.
Beispielsweise indem ein Programm selbst seinen Zustand in eine Spool-Datei schreibt.
Das Beispiel mit dem Check-Plugin ist eigentlich ein sehr schönes:
...
// MFS: Alphanumerisch sollte es je nach Shell treffen… Ich kann allerdings nicht ausschließen, dass manche Implementierungen (jetzt oder künftig) die Inode-Reihenfolge verwenden.
// MFS: Aus diesem Grund würde ich die schwammigere Formulierung beibehalten wollen.
...
// MFS: Zusammenspiel besser erläutert.
...
Die Zeile _Current user count: …_ schreibt der Prozess dabei nicht nach jedem Login/Logout, sondern in festen Zeitabständen.
Ist die Zahl der Zeilen in der Logdatei klein genug, um sie schnell einzulesen, können Sie einen xref:localchecks#[lokalen Check] programmieren.
Dieser liest jedes Mal die ganze Logdatei Zeile für Zeile und setzt immer beim Vorkommen der Zeile _Current user count: …_ die Nutzerzahl auf den angezeigten Wert.
...
//SK: Ich habe hier den Link eimgefügt, weil sich ein Leser fragen könnte, was "asynchrone Agentenplugins" noch mal genau sind und der Aschnitt doch recht gut in agent_linux versteckt ist.
Mit steigender Beliebtheit Ihres Dienstes läuft der lokale Check immer länger, so dass irgendwann auch die Ausführung als xref:agent_linux#async_plugins[asynchrones Agentenplugin] nicht mehr praktikabel ist.
...
Zunächst soll es sich nicht selbst beenden, wenn es am Ende der Datei angekommen ist, sondern eine Spool-Datei schreiben, die den aktuell ermittelten Zustand des Dienstes bei vollständiger Auswertung der Logdatei enthält:
...
//SK: Der folgende Abschnitt kommt so rüber, als würde "ihr Programm" irgendwie automagisch eine gewisse Zeit warten und auch automagisch nur neue Log-Einträge auswerten.
// Ihr Programm wartet dann eine gewisse Zeitspanne -- ob wenige Sekunden oder mehrere Minuten, sollten Sie von der Frequenz neu hinzukommender Log-Einträge abhängig machen.
// Es wertet dann nur die neu hinzugekommenen Zeilen aus, berechnet den Zustand neu und schreibt die Spool-Datei frisch, wenn geänderte Zahlen vorliegen.
// Um abzufangen, dass dieses Programm auch abstürzen kann, sollten Sie die Logdatei entsprechend benennen, beispielsweise `1800_frobolator.txt`, wenn 30 Minuten ohne Login/Logout auf Probleme des Dienstes oder des auswertenden Programmes hindeuten.
...
Um abzufangen, dass dieses Programm auch abstürzen kann, sollten Sie die Spool-Datei entsprechend benennen, beispielsweise `1800_frobolator.txt`, wenn 30 Minuten ohne ohne neu schreiben der Spool-Datei auf Probleme des Dienstes oder des auswertenden Programmes hindeuten.
...
Viele Serverapplikationen bieten zudem die Möglichkeit, statt oder zusätzlich zum Schreiben normaler Logdateien, einem anderen Programm Log-Ausgaben per Pipe zu übergeben.
...
Zu beachten ist, dass hier die Altersprüfung über Dateinamen nicht funktioniert, weil das Alter des Softlinks oder der benannten Pipes selbst ausgewertet wird und nicht das Alter der geschriebenen Daten.
...
// MFS: Lass uns das mal drin lassen, das ist ein Fallstrick, bei dem Redundanz gegen überlesen hilft
