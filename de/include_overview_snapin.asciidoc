...
Damit sind z.B. Meldungen aus Syslog, SNMP-Traps und Logdateien gemeint.
...
Übrigens werden Probleme von Services hier nicht mitgezählt, deren Host {DOWN} oder in einer Wartungszeit ist.
...
Schließlich zeigt die letzte Spalte ganz rechts die Hosts oder Services, deren Information „veraltet“ (_stale_) ist, da über sie zurzeit keine aktuellen Monitoring-Daten vorliegen. 
Wenn z.&ThinSpace;B. ein Host aktuell gar nicht erreichbar ist, kann {CMK} auch keine Neuigkeiten über dessen Services ermitteln. 
