...
die Sie unter unter `cmk/post_rename_site/plugins/actions` im link:https://github.com/tribe29/checkmk/tree/master/cmk/post_rename_site/plugins/actions[Git-Repository^] finden.
...
Mit `omd update` wird übrigens auch ein xref:update#updatedemo[Upgrade] auf eine andere Edition durchgeführt, z.B. von der {RE} auf die {SE}.
