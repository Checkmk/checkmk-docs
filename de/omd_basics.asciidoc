...
|`CORE` |`nagios` ({RE}), +
 `cmc` ({EE}) |Auswahl des Monitoring-Kerns. In den {CEE} wird neben dem xref:cmc#[{CMK} Micro Core (CMC)] auch noch der Nagios-Kern angeboten. In der {CRE} gibt es nur `nagios` als Monitoring-Kern.
