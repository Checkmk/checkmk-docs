...
= Die Monitoring-Instanzen (Sites)
:revdate: draft
:title: Die Monitoring-Instanzen (Sites) - Instanzen mit {CMK} verwalten
...
Hilfe zu einem dieser `omd`-Befehle erhalten Sie wiederum, indem Sie hinter dem Befehl die Option `--help` anfügen, z.B. `omd create --help`.
...
// TK: Der folgende Absatz nutzt bereits den "Instanzbenutzer", der erst später eingeführt wird. Unschön. Trotzdem drinlassen? Die Unterscheidung root vs. Instanzbenutzer war mir bisher in den Tiefen des Artikels untergegangen.
Die `omd`-Befehle können Sie als `root`-Benutzer oder als xref:site_user[Instanzbenutzer] ausführen.
Unter `root` haben Sie mehr Möglichkeiten.
So kann nur `root` eine Instanz erstellen.
Sobald die Instanz existiert, sollte sie mit weiteren `omd`-Befehlen nur noch als Instanzbenutzer konfiguriert werden.
...
Dort können Sie auch regelmäßige Backup-Aufträge erstellen. Im Artikel zu xref:backup#[Backups] erfahren Sie, wie das geht.
...
// TK: Ein Kapitel mit nur einem Eintrag kann man wohl auch weglassen, oder?
