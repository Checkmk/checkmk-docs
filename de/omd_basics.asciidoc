...
`omd init` holt dann die beiden ausgesetzten Schritte nach, fügt also die Standarddateien ein und erstellt die Apache-Konfiguration.
...
|`CORE` |`nagios` ({RE}), +
 `cmc` ({EE}) |Auswahl des Monitoring-Kerns. In den {CEE} kann statt des xref:cmc#[{CMK} Micro Core (CMC)] auch der Nagios-Kern ausgewählt werden. In der {CRE} gibt es nur `nagios` als Monitoring-Kern.
...
// ML: Neu 2/3
[#cleanup]
== Ungenutzten Versionen deinstallieren
...
OMD behält dabei neben den genutzten Versionen auch die Standardversion; sofern nicht manuell anders konfiguriert die zuletzt installierte Version von {CMK}, hier also [.guihint]#2.1.0p19.cme.#
////
...
Über den Befehl `omd disable --kill mysite`, ausgeführt als `root`, passiert Folgendes:

. Instanz [.guihint]#mysite# wird gestoppt.
. Prozesse, die auf das tmpfs zugreifen, werden gestoppt.
. Das tmpf wird ausgehängt.
. [.guihint]#/omd/apache/mysite.conf# wird geleert.
. Apache wird neu gestartet.

In diesem Status wird das Home-Verzeichnis der Instanz, hier [.guihint]#/omd/sites/mysite,# von keinem Prozess mehr referenziert.
Praktisch ist dies vor allem in einem xref:appliance_cluster#[Cluster,] da sich das Home-Verzeichnis nun auf einen anderen Node verschieben lässt.
////
