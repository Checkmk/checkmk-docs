...
// SK: *Wichtig:* Es ist nicht möglich eine neue Instanz mit einem Namen zu erstellen, den es auf Ihrem Server bereits als Namen eines "normalen" Nutzers gibt.
// SK: Warum "normal"? Über --reuse kann man ja schon einen bestehenden Nutzer verwenden. Dieser MUSS aber sein leeres(!) home directory aber in /omd/sites/ haben. Sonst funktioniert --reuse nicht.

//SK: Der folgende Satz kann m.E. raus. Selbst mit omd -f wird keine Site mit einem Namen angelegt, den es bereits gibt. Ausnahme natürlich: --reuse. Aber das ist was anderes. Ich wüsste nicht, wie es zu so einer "doppelten Belegung" kommen sollte. Womöglich war das früher mal so.
...
|`--reuse` |OMD geht davon aus, dass der neue Benutzer bereits existiert und legt ihn nicht an. Das Home-Verzeichnis dieses Nutzer muss sich unterhalb von `/omd/sites/` befinden und leer sein.
...
// SK: Für diesen Befehl fehlt mir seit Jahr und Tag das Verständnis. Es ist keine Abkürzung und macht nicht einfacher. Nur ein weiterer Befehl den man sich merken könnte. Ich würde nicht "klarer machen, dass die beiden Kommandos identisch sind", sondern omd su weglassen.
...
// SK: Ich würde eher hier klar machen, dass wir jetzt 'woanders' sind. "Sobald sie als Instanzbenuzter angemeldet sind..."
...
// SK: In dem folgenden Absatz ist nicht immer klar, wer jetzt gerade welchen Befehl ausführt. Root oder Instanzbenutzer?
*Hinweis:* 
...
//SK: Hier ist schon sehr wichtig wer omd version ausführt. Als Instanzbenutzer bekomme ich natürlich die Version der Instanz angezeigt. Das ergibt sich über die Datei .profile im Verzeichnis der Instanz. Root bekommt hier die zuletzt installierte bzw. durch omd setversion ausgewählte Version angezeigt.
...
|`CORE` |`nagios` ({RE}), `cmc` ({EE}) |Auswahl des Monitoring-Kerns. In den {CEE} wird neben dem xref:cmc#[{CMK} Micro Core (CMC)] auch noch der Nagios-Kern angeboten. In der {CRE} gibt es nur `nagios` als Monitoring-Kern.
...
* Wenn Sie das Backup als Instanznutzer erstellen erhält nur der Instanzbenutzer und seine Gruppe Lese- und Schreibzugriff auf das tar-Archiv.
...
// SK: Hier könnte wir nochmal auf den Punkt hinweisen, dass ein Restore in einer Site mit einem anderen Namen erst aber der 2.1.0 korrekt funktioniert. In der 2.0.0 kann steht man sonst wieder in einem "Unknown site".
Dies ist sogar als Instanzbenutzer möglich.
// SK: Ggf. bekommt die Instanz sogar eine andere Version von Checkmk. Nämlich die Version, die die Site im Backup hat.
// SK: Wiederholung von oben: Die Version der Site im Backup muss auf dem Server installiert sein.
