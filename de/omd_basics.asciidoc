...
Die einzelnen Migrationsschritte werden in der Folge ganz automatisch über die so genannten _rename actions plugins_ abgearbeitet, die Sie im Git-Repository unter link:https://github.com/Checkmk/checkmk/tree/master/cmk/post_rename_site/plugins/actions[cmk/post_rename_site/plugins/actions^] finden.
