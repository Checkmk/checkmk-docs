...
|`C:\ProgramData\checkmk\agent\check_mk.user.yml` |Konfigurationsdatei für Ihre individuelle Anpassungen, z.B. um eine Einstellung oder eine Erweiterung auf einem Host zu testen. Diese Datei wird als letztes eingelesen und überschreibt gegebenenfalls Werte aus der Konfigurationsdatei der Agentenbäckerei.
