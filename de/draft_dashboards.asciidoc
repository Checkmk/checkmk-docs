...
ifdef::env-github,env-browser[:relfilesuffix: .asciidoc]
:imagesdir: ../images
...
// ML: Einige Bilder sind noch mit eingeklappter Aktionsleiste - ändere ich, nachdem alles andere durch ist.
...
// ML: Hier zoomt es überall - aber wenn das ein Bug sein sollte, raus damit. 
// Entfernt: Bei der [.guihint]#Host overview# kann es schnell mal zu Hunderten Hexagons kommen, daher können Sie in diesem Dashlet mit dem Mausrad zoomen. 
...
Zur Verdeutlichung des Prinzips hier ein Beispiel mit einem Host-Matrix-Dashlet mit manueller Höhe und Breite in der Mitte sowie dem Anker oben links. Den Anker erkennen Sie an der grünen Ecke, die Einstellungen für aktuelle Höhe und Breite finden Si in der Mitte der Dashlets.
...
// ML: Beim verteilten Monitoring zeigt das Dashlet tatsächlich die Site-Overview, NICHT die Host-Overview - habe es unten erläutert und hier querverlinkt. Wird es damit verständlich?
...
// ML: Eigentlich wohl zeilenweise, war mir nicht konsistent genug zum Beschreiben, teils buggy. Wenns doch rein soll würde ein Hinweis auf zeilenweise wohl genügen.
...
// ML: Jein,  es geht darum, DASS etwas gesetzt werden muss, weil obligatorisch - was die GUI nicht als required anzgeigt. Insonfern Mini-Bugchen, das ich aber dennoch dokumentieren wollte ;) Mit der Fehlermeldung verschwinden nämlich auch die sonstigen Einstellungen und man fängt von vorn an. Vielleicht ein Ticket, die Max-/Min-Einstellungen als required zu markieren?
...
// ML: Daher auch nur der Konjunktiv ;)
...
// ML: Ich lasse sie erstmal drin, da ich sie als Referenz durchaus nützlich finde - zumal das Handbuch ja auch für Interessierte Noch-nicht-Nutzer gedacht ist. Bezüglich der Übersetzung: Stimmt schon, nur sind Begriffe wie Scatterplot und Gauge imho nicht unbedingt gängige Vokabeln - Streudiagramm und Tachometer dürften dann aber selbsterklärend sein. Soll jemand Drittes nochmal über die Tabelle gucken.
// ML: Die ntop-Dashlets sind nicht drin, weil ich keine ntop-Subscription habe - könntest Du die nachtragen? Danke!
...
// ML: Nein, bei Static Text, Custom URL, Sidebar Element, User Notifications, Percentage of service problems geht das nicht - habe ein wenig ergänzt.
...
Ein wenig mehr als Sie vielleicht vermuten würden kann auch das Dashlet [.guihint]#Static text#: Es eignet sich für Hinweise und simple Beschriftungen, kann aber auch zum Verlinken sonstiger Dashboards oder anderer {CMK}-Bereiche verwendet werden, wie Sie auch im folgenden Beispiel fürein Top-Down-Dashboard sehen.
