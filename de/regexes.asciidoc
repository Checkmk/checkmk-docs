...
:revdate: 2018-02-08
:title: Reguläre Ausdrücke in {CMK} - {CMK} mit Text-Muster konfigurieren
:description: In {CMK} können Sie an vielen Stellen mit regulären Ausdrücken (regular expressions) eine Gruppe von Objekten erfassen und dadurch eine hohe Dynamik erreichen.
...
sind: `\ . * + ? { } ( ) [ ] | & ^` und `$`.  
Durch den `\` Backslash *wird das folgende Sonderzeichen als normales Zeichen interpretiert*.
...
definieren, sprich eine ODER-Verknüpfung verwenden: `1|2|3` matcht also
