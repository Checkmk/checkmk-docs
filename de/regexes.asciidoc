...
Soll eine Match-Gruppe keine Zeichengruppen „einfangen“, beispielsweise wenn sie nur zur Strukturierung dient, kann mit `?:` die Umwandlung in eine _non-capturing Match Group_ erfolgen: `(?:/local)?/share`
// TK: Ungünstig, dass ein bereits übersetzter Begriff wieder Englisch wird, weil ein Adjektiv davorsteht. Vielleicht so:
// TK: nicht einfangende Match-Gruppe (_non-capturing match group_) ... 
...
Für die Arbeit mit {CMK} relevant ist vor allem `(?i)`, welches bei Ausdrücken, die sonst unter Berücksichtigung von Groß- und Kleinschreibung ausgewertet werden, auf Matching ohne deren Berücksichtigung umschaltet (_case-insensitive_).
// TK: Schwer zu lesen - und schwer zu verbessern. Vielleicht so:
// TK: ... welches bei Ausdrücken, die sonst mit Groß- und Kleinschreibung ausgewertet werden, auf ein Matching umschaltet, das Groß- und Kleinschreibung nicht berücksichtigt (_case-insensitive_).
In sehr seltenen Fällen werden Sie für die Arbeit mit mehrzeiligen Strings link:https://www.regular-expressions.info/modifiers.html[`(?s)` und `(?m)`^] benutzen wollen.
// TK: ... mit mehrzeiligen Strings auch ...

Beachten Sie bei Verwendung von Inline Flags, dass Python seit Version 3.11 Inline Flags entweder am Anfang eines regulären Ausdruckes oder unter Angabe des Geltungsbereiches erwartet.
Da {CMK} in einigen Fällen aus Gründen besserer Performance reguläre Ausdrücke intern kombiniert, raten wir, diese Schreibweise _nicht mehr_ zu benutzen:

`(?i)somestring`

Verwenden Sie stattdessen _immer_ die Schreibweise mit Gültigkeitsbereich – der sich im Zweifel auf den gesamten regulären Ausdruck erstreckt.
Hierbei handelt es sich um eine Variante der _non-capturing xref:matchgroups[Match Group]_:

`(?i:somestring)`

// TK: Du erklärst nicht, oder nur implzit, wie man den Geltungsbereich (oder Gültigkeitsbereich oder egal?) festlegt.
// TK: Vorschlag folgt: 1) Ich hab die „falsche" Schreibweise in den Text gezogen, sonst zu hervorgehoben 2) Ob dringend musst Du entscheiden :-)
////
...
|`pass:[(?i:]_A_)` | Ändert den Auswertungsmodus des Unterausdrucks _A_ per xref:inlineflags[Inline Flag] auf _case-insensitive_.
...
Diese werden im Monitoring-Kern in C mit der Regex-Funktion der C-Bibliothek
ausgewertet. Sie finden eine komplette Referenz dazu in der Linux Man Page zu
`regex(7)`:
