...
// MA: Das ist so nicht korrekt. In der Config (Setup-Menü) wird in der Regel _immer_ case-sensitive gearbeitet. In der Suche des Eintrags _Monitor_ wiederum trifft die Aussage zu. Hier müssen wir die Aussage entweder rausnehmen, oder klarer beschreiben.
...
// MA: In der Regel haben wir standadisierte Funktionen, um Eingabefelder zur Verfügung zu stellen und diese kümmern sich auch um die Auswertung. Wie sich also ein Feld verhält, liegt nicht allein beim Ersteller des Plugins. Der Absatz muss genauer werden, denn momentan sagt er nur "Es könnte etwas ungewöhnlich sein, aber ob Du das herausfinden kannst, ist ein Glücksspiel". Das sollten wir etwas verlässlicher, oder positiver formulieren.
...
// MA: Würde ich entfernen. Hier ist imho klar, dass es sich um ein Beispiel handelt.
//Beispiel:
...
Auftreten*. Sehr oft möchte man definieren, dass irgendwo im Ausdruck
eine beliebige Folge von Zeichen vorkommen darf. In regulären Ausdrücken
schreiben Sie dann `pass:[.*]` (Punkt Stern).
...
// MA: Für den Aha-Effekt hätte ich erwartet, dass hier ein Verweis auf 2.2 kommt. Also in dem Sinne: "Sie Können auch den . (Punkt) mit den Abkürzungen kombinieren, um zum Beispiel eine beliebige Folge von Zeichen zu suchen." - Sprich, den Kontext schnel aufrecht erhalten, dass man das vorher gelernte direkt hier anwenden kann.
...
// MA: Wie oben beschrieben, ist das so nicht ganz korrekt. Abgesehen davon, erläutern wir hier ja nicht das spezifische Verhalten von Checkmk, sondern die Funktionsweise von regulären Ausdrücken. Auch wenn wir das natürlich meist bezogen auf Checkmk machen.
...
// MA: Finde den Vorschlag von TK gut.
...
Beispiel bereits gezeigt. Dann weiß der Interpreter der regulären Ausrücke,
// MA: Wer ist denn dieser ominöse Interpreter? Ich würde hier eher von "der Ausdruck wird dann entsprechend ausgewertet" schreiben.
...
`pass:[^]` (Zirkumflex), für das Ende (Suffix-Match) das `pass:[$]` (Dollarzeichen). Ist
...
Der Grund dafür ist, dass dies meistens das ist, was Sie brauchen.
...
// MA: Hier müssen wir unterscheiden, ob wir konfigurieren, oder anzeigen wollen. In der Konfiguration macht ein Präfix-match sehr viel Sinn (und ist auch das, was man braucht). In der Anzeige macht es keinen Sinn.
...
// MA: Weshalb erwähnen wir hier die EC (Event Console) und andere Stelle nicht? Mir ist die Aufgabe der Erläuterung nicht klar, außer, dass sie zeigt, wie inkonsistent wir in CMK beim matching von regexp sind. ^^
...
// MA: Warum muss ich tricksen? Das klingt so, als wäre die Präfix-Match ein Unfall gewesen und jetzt wollen wir die Leute anleiten, wie sie einen Workaround bauen können. Aber das hat ja schon seinen Sinn. Ich würde hier also eher Richtung "Falls Sie doch einmal einen Infix brauchen, können Sie..." gehen.
...
// Ma: Wenn ich das richtig sehe, werden hier Matching-Gruppen benutzt, obwohl diese erste danach eingeführt werden?
...
// MFS: Evtl. noch erklären, dass es wirklich "capturing groups" heisst?
...
// MA: Uff. Die Nennung der Beispiel suggeriert, dass sie vollständig ist. Wie stellen wir denn sicher, dass diese Liste vollständig ist und auch bleibt?
...
Folgende Abbildung zeigt eine solche Umbenennung die Umbenennung mehrerer
...
// MA: Grundsätzlich bin ich kein Freund von Screenshots, in denen die Inline-Hilfe an ist. Das macht es sehr unübersichtlich und im Zweifel sogar redundant. Der Screenshot sollte meines Erachtens verständlich sein, ohne dass die Inline-Hilfe zu sehen ist. Das es sie gibt, sollt der Leser ab Kapitel 5 schon wissen.
...
Folgende Zeichen müssen durch link:#escaping[Backslash maskiert werden],
...
// MA: Die Erläuterungen zu \w und \W sind mir noch etwas zu unspezifisch. Und ich frage mich, ob wir \u nicht weglassen sollten... Jemand, der damit etwas anfangen kann, wird wahrscheinlich wissen, dass es das gibt. Alle anderen sollten es eventuell eher nicht benutzen.
...
Online-Dienste wie link:https://regex101.com/[regex101.com] bereiten Matches
grafisch auf und erklären darüber die Reihenfolge der Auswertung in Echtzeit.

image::regexes_testing.png[]

// MA: In dem Screenshot sind mir noch zu viele Elemente drin, die mir zu sehr vom eigentlich zu Zeigenden ablenken. Die Schriftgröße ist auch nicht optimal. Relevant scheint mir hier doch der Teil mit "Regular Expression" und "Explanation" zu sein. Gerade Explanation ist hier sehr interessant, warum die Seite so hilfreich ist. Wie sie im gesamten aussieht, kann man sich ja schnell selbst anschauen.
...
// MA: Ich würde hier keine Zwei-Klassen-Gesellschaft aufmachen. Eher Richtung "Wenn Sie es ganz genau wissen wollen, ..."
...
in der Online-Hilfe von Python zum Modul `re`, oder ausführlicher in der
link:https://docs.python.org/3/library/re.html[Online-Dokumentation] von
Python:
