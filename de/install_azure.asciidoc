// -*- coding: utf-8 -*-
// IGNORE % Benachrichtigungs ED25519 MTAs Nullmailer SMB Snakeoil WebDAV burstable p2
...
Falls Sie Azure-Nutzer sind, hatten Sie schon immer die Möglichkeit, ein im Marketplace link:https://learn.microsoft.com/en-us/azure/virtual-machines/linux/quick-create-portal?tabs=ubuntu[vorhandenes Ubuntu-Image^] mit {CMK} zu versehen und so "Monitoring in der Cloud" einzurichten.
// TK: Anders als bei AWS, ist das doch kein fertiges Image, sondern die Anleitung, wie ich mir ein Image baue. Also eher so:
// TK: ... die Möglichkeit, im Marketplace ein link:https://learn.microsoft.com/en-us/azure/virtual-machines/linux/quick-create-portal?tabs=ubuntu[vorhandenes Ubuntu-Image zu erstellen^], mit {CMK} zu versehen und ...
{CMK} {v22} geht den nächsten Schritt und bietet link:https://azuremarketplace.microsoft.com/de-de/marketplace/apps/tribe29gmbh1665582614827.checkmk003?tab=Overview[vorinstallierte Images^] auf Basis von Ubuntu 22.04 _(Jammy Jellyfish)_, bei denen alle Abhängigkeiten bereits erfüllt sind.
// TK: vorinstallierte Images: Plural? Ist doch nur eines, oder?
...
// TK: Der Source-Text ist nicht ganz passend, da im Linkziel erklärt wird, wie ein RSA-Schlüsselpärchen erstellt wird. Aber akzeptabel wg. Konsistenz zu AWS.
// TK: Pärchen ist zwar süss, aber warum hier Diminutiv? 
...
// TK: Das weiß man dann als Experte schon, wo die Inbound port rules zu finden sind? Keine rhetorische Frage.
...
// TK: Große Zahlen mit Leerzeichen statt mit Punkt gliedern.
...
// TK: Du meinst wirklich 25 - und nicht 25 %?
...
{CMK} bietet eine komfortable xref:backup#[Backup-Funktion], die Sie direkt im Setup unter [.guihint]#Setup > Maintenance > Backups  > Backup targets# konfigurieren.
// TK: Im GUI-Pfad sollte m.E. der Klickweg stehen und nicht der Seitentitelweg. Noch schöner wärs, wenn es da keinen Unterschied gäbe.
...
// TK: Credentials wollten wir ja nicht verwenden: Azure-Zugangsdaten / AWS-Zugangsdaten?
...
// TK: ... _vor_ dem Umzug und dem Backup ...?
