...
|===
//SP: müsste hier nicht nach neuestem Wording unterschieden werden in Enterprise Edition einerseits und Cloud Edition andererseits, d.h. in zwei getrennte Aufzählungspunkte? Oder alternativ jeweils beide Begriffe in die Überschriften gesetzt werden? Gilt auch für die nachfolgenden Abschnitte.
...
// TK: Ersetzt ohne Test: check-mk-raw:2.1.0-latest > check-mk-raw:2.2.0-latest
[{shell}]
----
...
Achten Sie nur darauf, dass Sie den Namen des geladenen Images (`Loaded image`) aus der vorherigen Kommandoausgabe im folgenden Startkommando verwenden, also in diesem Beispiel `checkmk/check-mk-enterprise:2.2.0p1`:
