...
betreiben möchten. Auch {CMK} kann man in einer Docker-Umgebung einsetzen. Ein
Anwendungsfall kann sein, eine dynamisch erstellte Containergruppe zu überwachen
und {CMK} zu einem Teil dieser Gruppe zu machen. Wird die Containergruppe nicht
mehr benötigt, kann auch die Instanz von {CMK} wieder entfernt werden.
...
Weg. Da jegliche Virtualisierung zum einen eine verringerte Performance mit sich
bringt und zum anderen das Monitoring prinzipiell möglichst wenige physische
Abhängigkeiten haben sollte, eignen sich {CMK}-Container zum Beispiel nicht, um
damit die gesamte Infrastruktur zu überwachen. In einem in sich abgeschlossenen
Container-Cluster kann es aber durchaus zielführend sein, in dieses einen
{CMK}-Container zu integrieren, um es direkt von innen heraus zu überwachen.
Prüfen Sie also auch und besonders hier, ob das Werkzeug Docker/Container für
Ihre konkrete Anforderung das richtige ist.

Um Ihnen die Bereitstellung als Container so einfach wie möglich zu machen,
stellen wir für jede {CMK}-Edition eigene Images zur Verfügung, welche neben
{CMK} das Linux-Betriebssystem Debian enthalten. Diese Images erhalten Sie wie
folgt:

// SK: Die folgende Tabelle und der vorangegange Satz, suggerieren dem Neuling, er müsse das Image manuell herunterladen. Mit dem ersten Befehl aus 2. passiert das aber automagisch.

[cols=2]
|===

|{CRE} |link:https://hub.docker.com/r/checkmk/check-mk-raw/[Docker Hub] oder link:https://checkmk.com/de/download?edition=cre&version=stable&dist=docker[{CMK}-Download-Seite]
|{CEE} |link:https://checkmk.com/de/download/[{CMK}-Download-Seite] (für die {FE}),
link:https://customer.checkmk.com/[{CMK}-Kundenportal]
|===

In diesem Artikel führen wir Sie durch die Einrichtung von {CMK} in Docker und
zeigen Ihnen einige Tricks, die das Leben mit {CMK} in Docker einfacher machen.

Weitere Informationen finden Sie im Artikel xref:managing_docker#[{CMK}-Server
im Docker-Container.]
...
// SK: Ich kann mir gut vorstellen, dass hier jemand ankommt, der mit dem Thema Docker noch nicht viel zu tun hat, weshalb ich einen Hinweis auf diese Vorraussetung gut fände. Ggf. in einem neuen zweiten Kapitel namens (System-)Vorraussetzung. Der Docker-Vollprofi braucht eigentlich auch unsere Anleitung nicht. Das Problem ist allerdings das apt install docker.io nur für Ubuntu und Debian Gültigkeit hätte und darüberhinaus auch bereits veraltet wäre. Ich fürchte, dass wir hier eigentlich nur nach docs.docker.com verlinken können. https://docs.docker.com/engine/install/

{cre-only}
Der Start mit Docker mit der {CRE} ist einfach. Ein passendes Image steht Ihnen
direkt über Docker Hub zur Verfügung. Das Ganze geht mit einem einzigen Kommando
auf der Linux-Konsole.Dabei wird nicht nur ein Docker-Container mit {CMK}
erzeugt, sondern auch gleich eine Monitoringinstanz mit dem Namen `cmk`
eingerichtet und gestartet. Diese ist sofort bereit zur Anmeldung mit dem
Benutzer `cmkadmin`.
...
// SK: -dit ist ein Kürzel für -d -i -t -> -d = detach; -i = interactive; -t = psuedo-TTY. Passt scho.
...
// SK: Das Zeile zu --ulimit kann komplett entfernt werden. Ist intern geklärt.
...
// SK: Ich komme über dieses http://containerID/cmk auch nicht auf die GUI von Checkmk. Ich finde im Netz auch keine Info dazu, dass das eine Standardadresse einer Container-GUI wäre. Eine Anfrage bei den Kollegen läuft.
...
// SK: Ich finde, dass man dieses Beispiel mit weggelassenem -v /omd/sites auch noch mal in eine Box packen könnte. Der Übergang in den nächsten Satz, bei dem ja etwas anderes erreicht werden soll, war mir schon immer zu nahtlos. Muss aber nicht sein...

// {c-root} docker container run -dit -p 8080:5000 --ulimit nofile=1024 --tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 --name monitoring -v /etc/localtime:/etc/localtime:ro --restart always checkmk/check-mk-raw:2.0.0-latest
...
////
SK: Der Vollständigkeit halber könnte man es natürlich noch hier rein packen.
Checkmk Container und Image entfernen

Alle Container `docker container ls -a` auflisten

[{shell}]
----
docker container ls -a
CONTAINER ID        IMAGE                                 COMMAND                 CREATED            STATUS                       PORTS                              NAMES
7496d3c43af5        checkmk/check-mk-raw:2.0.0-latest     "/docker-entrypoint.…"  3 hours ago        Up 3 hours (healthy)         6557/tcp, 0.0.0.0:8080->5000/tcp   monitoring
----

Container ID übernehmen und per docker rm entfernen
docker rm 7496d3c43af5

Images mit `docker images` auflisten

[{shell}]
----
docker images
REPOSITORY                                TAG                 IMAGE ID            CREATED             SIZE
checkmk/check-mk-raw                      2.0.0-latest        5d48834cf7b2        5 days ago          1.02GB
----

Images über die Image ID entfernen
[{shell}]
----
docker rmi 5d48834cf7b2
----
////