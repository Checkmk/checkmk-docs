...
{v16} offiziell in einer Docker-Umgebung einsetzen. Ein
...
in dem Artikel link:managing_docker.html[{CMK}-Server im Docker-Container.]
...
|--tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 |Ab Version {v16} kann für eine optimale Performance ein temporäres Dateisystem direkt im RAM des Docker-Node genutzt werden. Mit dieser Option geben Sie den Pfad zu diesem Dateisystem an. Wenn Sie die ID der Instanz ändern, so muss auch dieser Pfad entsprechend angepasst werden.
...
* Mit `--rm` übergeben direkt zum Start den Befehl, dass auch der Datenspeicher für den Container beim Stoppen entfernt werden soll. Auf diese Weise sparen Sie sich das manuelle Aufräumen, wenn Sie viele kurzlebige {CMK}-Container haben. *Wichtig:* Beim Stoppen wird auch der Container selbst komplett entfernt!
