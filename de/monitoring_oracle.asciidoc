...
Die entsprechenden Check-Plugins wurden bereits soweit vorbereitet, dass Sie auch als xref:clustered_services#[_Clustered Services_] konfiguriert werden können.
...
|SQLS_SECTION_NAME |Der Sektionssname, wenn Sie ein eigenes Check-Plugin für die individuellen SQL-Abfragen geschrieben haben. |ja
...
xref:devel_check_plugins#[{CMK}-Check programmiert] und in die Monitoring-Instanz einbindet. Danach ordnen
Sie lediglich die individuellen SQL-Abfragen mit den Variablen diesem Check-Plugin zu.
