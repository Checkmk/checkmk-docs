// -*- coding: utf-8 -*-
// IGNORE Datafiles ORACLE_HOME RMAN Sqlnet TNSALIASes pre sqlnet tnsnames
...
// TK: Das folgende Plugin gibts nicht mehr:
...
Auf Systemen mit systemd müssen Sie das Plugin daher xref:agent_linux.html#async_plugins[asynchron ausühren lassen.]
...
Bei der Einrichtung über die xref:#bakery[Agentenbäckerei] erledigen Sie das freilich über eine Option in der Regel.
////
...
|Linux, Solaris, UNIX mit systemd |`/usr/lib/check_mk_agent/plugins/ZAHL` |`/etc/check_mk/`
|AIX |`/usr/check_mk/lib/plugins/` |`/usr/check_mk/conf/`
|AIX mit systemd |`/usr/check_mk/lib/plugins/ZAHL` |`/usr/check_mk/conf/`
|===
...
| [.guihint]#Host uses xinetd or systemd (Linux/AIX/Solaris only)# |Muss für Systeme mit xinetd/systemd aktiviert werden; das Plugin wird dann asynchron im festgelegten Intervall ausgeführt.
...
Er setzt voraus, dass Sie wissen, wie man ein xref:devel_check_plugins#[agentenbasiertes Check-Plugin programmiert] und in die Monitoring-Instanz einbindet.
