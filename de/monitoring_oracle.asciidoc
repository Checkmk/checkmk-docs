...
Sie, dass die Einrichtung mittels der xref:monitoring_oracle#bakery[Agent Bakery]
...
sofern Sie die xref:monitoring_oracle#bakery[Agent Bakery] nutzen.
...
xref:wato_monitoringagents#bakery[Agent Bakery] stark vereinfacht werden, da
...
|[.guihint]#ORACLE_HOME to use for remote access# |Hier können Sie bestimmen, wo das Agenten-Plugin das Programm `sqlplus` findet. Sie müssen es dann angeben, wenn Sie eine xref:monitoring_oracle#remote_instances[entfernte Instanz] überwachen wollen, aber `sqlplus` nicht über die Umgebungsvariablen gefunden werden kann.
...
der xref:agent_windows#plugins[Konfigurationsdatei des Agenten] auf die
Datei im xref:agent_windows#agent_architecture[Installationspfad] verweisen.
...
Die entsprechenden Checkplugins wurden bereits soweit vorbereitet, dass Sie auch als xref:clustered_services#[_Clustered Services_] konfiguriert werden können.
Legen Sie also einen Cluster-Host in {CMK} an und ordnen Sie diesem Host die einzelnen Oracle-Hosts zu, denen die primäre und die Standy-Instanzen zugeordnet sind.
...
xref:localchecks#[Local-Checks] vor allem mit Blick auf die Performance-Werte
...
* `long`: Wenn der Service einen xref:wato_services#discovery_auto[[.guihint]#Long Output#] haben soll, können Sie diesen hier angeben. Auch dieses Schlüsselwort können Sie mehrmals verwenden, um mehrere Zeilen im [.guihint]#Long Output# zu erzeugen.
...
den neuen Service. Danach sehen Sie ihn bei den anderen Services in der Übersicht des Hosts. In diesem Fall unter dem Namen [.guihint]#ORA XE SQL MyCustomSQL.SQL#:
...
xref:devel_check_plugins#[{CMK}-Check programmiert] und in die Monitoringinstanz einbindet. Danach ordnen
