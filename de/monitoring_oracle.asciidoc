...
link:https://checkmk.de/cms_check_plugins_catalog.html.html[Katalog der Check-Plugins] nachlesen. Wir erweitern
...
anderen Einstellungen, die Sie unter link:monitoring_oracle.html#adv_linux[unixoiden Systemen]
...
das richtige Verzeichnis des Agenten. Wie der Pfad für Agenten-Plugins
korrekt lautet, wird in den Artikeln zu link:agent_linux.html#plugins[Linux] und
...
Das _Oracle-Wallet_ wiederum legt die Zugangsdaten verschlüsselt
...
Bentuzerdaten auch gefunden werden, müssen Sie diese einmalig in das
...
link:monitoring_oracle.html#files_cmk[{CMK}-Server] und auch auf dem Oracle-Host.
...
[cols="25,~,~"]
|===
|OS |Pluginpfad |Konfigurationspfad 
...
In der link:monitoring_oracle.html#first_steps[allgemeinen Einrichtung] haben Sie
...
Sie die entsprechenden Variablen direk. Um nur bestimmte Instanzen
einzuschränken, können Sie die Variable `EXCLUDE__{SID}_` nutzen,
die Sie im link:monitoring_oracle.html#skip_include[vorherigen Abschnitt] bereits
...
// TODO: Genaue Funktionalität herausfinden und hier beschreiben.
//AN:tns_config
...
sein. Wie Sie wahrscheinlich schon bemerkt haben, folgen auf die Portangabe nun
weitere Werte. Diese sind teilweise
optional und teilweise notwendig, um eine Verbindung aufbauen zu können:

Der erste zusätzlich verwendete Wert _(PIGGYBACKHOST)_ bei der Instanz
...
diese Angabe nicht; die Zurordnung zu einem anderen Host ist _optional._

Der zweite neue Wert _(SID)_ ist die Angabe des Instanznamens. Da
...
Der dritte Wert _(VERSION)_ ist _notwendig_ und ebenfalls dem Umstand
...
Der vierte und letzte Wert _(TNSALIAS)_ ist wieder _optional_ und
...
die Agent Bakery konfigurieren. Zum einen dann, wenn es sich um veraltete
...
Über [.guihint]#WATO => Monitoring Agents => Rules => ORACLE Databases (Linux, Solaris, AIX)# legen Sie eine Regel an.
...
Natürlich können Sie auch hier das Wallet nutzen, indem Sie in dem Dropdown-Menü
...
|[.guihint]#Instances to monitor# |Diese Option fasst mehrere Optionen der Konfigurationsdatei zusammen, mit denen Sie Instanzen link:monitoring_oracle.html#skip_include[aus- oder einschließen] können.
|[.guihint]#Add pre or postfix to TNSALIASes# |Mit dieser Optionen können Sie den link:monitoring_oracle.html#tns_config[TNSALIAS] entweder global oder für eine spezifische Instanz erweitern.
...
|[.guihint]#Exclude some sections on certain instances# |Wenn Sie mit `EXCLUDE_{SID}` nicht die gesamte Instanz, sondern nur ein paar Sektionen ausschließen wollen, können Sie das über diese Option machen. Eine genauere Beschreibung finden Sie bei den Optionen zu den link:monitoring_oracle.html#skip_include[Instanzen] und link:monitoring_oracle.html#sections[Sektionen.]
...
link:monitoring_oracle.html#linux[unixoiden Systemen] angelehnt, enthält aber nur
...
Um das Plugin `mk_oracle.ps1` zu aktivieren, kopieren Sie das
Plugin von `C:\Program Files (x86)\checkmk\service\plugins\`
in das oben beschriebene Pluginverzeichnis. Alternativ können Sie in
der link:agent_windows.html#plugins[Konfigurationsdatei des Agenten] auf die
Datei im link:agent_windows.html#agent_architecture[Installationspfad] verweisen.
hilfreich - arbeitet man ja nicht unbedingt oft mit.]
...
die Richtilinien umzustellen. Setzen Sie daher eine permanente Ausnahme nur
...
{cee-only}
Zur Zeit steht die Einrichtung über die Agent Bakery nur für
link:monitoring_oracle.html#linux[unixoide Systeme] zur Verfügung.
 +
 +
...
Haben Sie ADG lizenziert und aktiviert, müssen Sie keinerlei Verändungen in
...
Rolle _sysdba._ Durch diese Berechtigung ist der Benutzer auch dann
in der Lage zumindest einen Teil der Daten zu holen, wenn die primäre Instanz
ausfällt und auf dem Standby-Server die Instanz noch nicht von _MOUNTED_
...
dass Sie auch als link:clustered_services.html[_Clustered Services_] konfiguriert
werden können. Legen Sie also einen Cluster-Host in {CMK} an und ordnen Sie
...
link:monitoring_oracle.html#cluster_adg[(Active) Data Guard] dem Cluster-Host
...
|SQLS_SECTIONS |Die selbst definierten Sektionsfunktionen, die von dem Plugin ausgeführt werden sollen. *Wichtig:* Diese Variable kann nur außerhalb einer Sektionsfunktion _(global)_ gesetzt werden. |nein
...
|SQLS_SECTION_NAME |Der Sektionssname, wenn Sie ein eigenes Checkplugin für die individuellen SQL-Abfragen geschrieben haben. |ja
...
Haben Sie ein solches Checkplugin geschrieben, sind Sie komplett frei in der
Syntax der Pluginausgabe und können ganz eigene Wege gehen. Da dieser
Weg der umfangreichste und auch schwierigste ist, ist er hier nur der
Vollständigkeit halber erwähnt. Er setzt voraus, dass Sie wissen, wie man einen
link:devel_check_plugins.html[{CMK}-Check programmiert] und in die Monitoringinstanz einbindet. Danach ordnen
Sie lediglich die individuellen SQL-Abfragen mit den Variablen diesem Checkplugin zu.
...
Das Plugin akzeptiert unter Windows keine Paramter. Um hier also die
Verbidung zu testen, limitieren Sie temporär die abzurufenden Sektionen auf
...
komplette Ausgabe des Loggings.
