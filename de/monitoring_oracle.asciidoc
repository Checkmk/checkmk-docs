...
|ONLY_SIDS |Hier kann bestimmt werden, welche Instanzen überwacht werden sollen. Alle anderen werden ignoriert. Natürlich werden die Instanzen weiterhin auch nur dann überwacht, wenn sie von dem Plugin gefunden werden. Die Option eignet sich also sehr gut, wenn die Menge der zu überwachenden Instanzen kleiner ist, als die Anzahl derer, die ignoriert werden sollen.
...
Um das Plugin `mk_oracle.ps1` zu aktivieren, kopieren Sie
...
die Möglichkeit, eigene SQLs zu definieren und mit dem
...
|SQLS_DIR |Der Pfad, in dem die eigenen SQLs abgelegt wurden. Kann global oder pro Sektion gesetzt werden. |nein
...
|SQLS_ITEM_NAME |Bestimmt einen individuellen Identifikator für den Servicenamen. Normalerweise werden die SID und der Dateiname der SQLs genommen. *Wichtig:* Diese Variable kann _nicht_ zusammen mit der Variable `SQLS_SECTION_NAME` benutzt werden und kann auch nur innerhalb einer Sektionsfunktion gesetzt werden. |ja
|SQLS_ITEM_SID |tbd |ja
|SQLS_MAX_CACHE_AGE |Erfüllt dieselbe Aufgabe, wie link:monitoring_oracle.html#sections[`CACHE_MAXAGE`] -- allerdings für die eigenen SQLs. |ja
...
eigene Sektionsnamen für ein oder mehrere SQLs ausgeben lassen. Das setzt
