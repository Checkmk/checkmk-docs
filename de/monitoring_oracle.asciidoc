...
Damit sich das Plugin auf die entfernte Datenbank verbinden kann,
...
|[.guihint]#Exclude some sections on certain instances# |Wenn Sie mit `EXCLUDE_{SID}` nicht die gesamte Instanz, sondern nur ein paar Sektionen ausschließen wollen, können Sie das über diese Option machen. Eine genauere Beschreibun finden Sie bei den Optionen zu den link:monitoring_oracle.html#skip_include[Instanzen] und link:monitoring_oracle.html#sections[Sektionen].
...
Rolle _sysdba_. Durch diese Berechtigung ist der Benutzer auch dann
