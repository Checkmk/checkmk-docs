...
als Item bekommt. Der Unterstrich zeigt an, dass uns die beiden andere Spalten
in der Ausgabe erstmal egalsind, denn bei der Discovery ist es schließlich unerheblich,
wieviele Slots belegt sind. Insgesamt sieht das dann so aus:
...
[{shell}]
----
{c-omd} cmk --detect-pluigns=foobar -vI myhost123
  [green]#3# foobar
...
Dabei gilt die Konvention, dass der Check beim
