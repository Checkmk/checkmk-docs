...
:revdate: draft
:title: Eigene Checkplugins schreiben
:description: 
...
|Logmeldungen auswerten |_Meldungen_ überwachen per link:ec.html[Event Console] |Keine Entwicklung notwendig sondern nur aufstellen von Regeln in der Event Console |Geht nur, wenn passende Logmeldungen vorhanden sind, kein gesicherter aktueller Status, kein Erfassen von Metriken, keine konfigurierbaren Schwellwerte
|Echtes {CMK}-Plugin |Wird in diesem Artikel erklärt |Fügt sich zu 100% in {CMK} ein, automatische Serviceerkennung, zentrale Konfiguration der Schwellwerte über die grafische Oberfläche, sehr performant, unterstützt link:snmp.html[SNMP], automatische Host- und Servicelabels möglich, unterstützt link:inventory.html[HW/SW-Inventur], Unterstützung durch Standardbibliotheken von {CMK} |Erfordert mehr Einarbeitungszeit sowie Kenntnisse in der Programmsprache Python
|===
...
richtigen OIDs finden (dazu gibt es ein link:devel_check_plugins.html#snmp[eigenes Kapitel]).
...
link:devel_check_plugins.html#check_levels[Kapitel] erklärt.
...
prüft z.B. der Service [.guihint]#Memory used# unter Linux nicht nur den RAM- und Swap-Verbrauch,
...
Artikel über das link:snmp.html[Monitoring via SNMP].
...
|matches(oid, regex) |not_matches(oid, regex) |Der Wert der OID matcht auf den link:regexes.html[regulären Ausdruck] `regex`, und zwar hinten und vorne geankert, also mit einem exakten Match. Wenn Sie nur einen Teilstring benötigen, ergänzen Sie einfach vorne bzw. hinten noch ein `pass:[.*]`
...
Dieser Abschnitt ist nocht nicht fertig!

==== Anlegen einer WATO Regelkette

Um Ihr Checkplugin parameterisieren, benötigen Sie zunächst eine geeignete Maske für die WATO Regelkette. Die ausgelieferten befinden sich in dem Verzeichnis cmk/gui/plugins/wato/check_parameters/.

Falls Sie dort eine geeignete Regel finden, können Sie diese benutzen (Beispiel: memory_simple.py, "Memory levels for simple devices not running more complex OSs", Levels für RAM/Swap).

Wir wollen für unser Beispiel eine eigene Maske entwerfen. Dazu erzeugen wir eine Datei im Verzeichnis '~/local/share/check_mk/web/plugins/wato'
...
*  Mit Hilfe der Registrierungsfunktion "rulespec_registry.register" geben wir eine Rulespec bekannt. Da unser Beispiel Check ein Item bekommen hat wählen wir "CheckParameterRulespecWithItem". Falls unser Checkplugin keine Items erzeugt können wir "CheckParameterRulespecWithoutItem" wählen, und das Keyword Argument "item_spec" weg lassen.

*  check_group_name: Hier geben wir unserer Regel einen eindeutigen Namen. Über ihn wird das Plugin diese Regel später finden.

*  group: Dieses Argument steuert, wo unsere Regel später im Regelkatalog zu finden sein wird.

*  match_type: Dieses Argument steuert, wie {CMK} sich verhält, falls mehrere Regeln für einen Bestimmten Service greifen. Bei 'merge' werden alle greifenden Regeln miteinander kombiniert. In der Regel sollten sie diesen Matchtype wählen.

*  item_spec: Bei einem plugin mit Items kann der User, der die Regel anlegt, mittels einer Regex einschränken, für welche der Items seine Regel greifen soll. Zu diesem Zweck übergeben wir ein Textfeld. Als Titel wählen wir eine gurze Beschreibung des Items.

*  parameter_valuespec: Hier übergeben wir eine Funktion, die die eigentliche Maske zum anlegen der Parameter erzeugt. Diese Funktion muss bei Regelketten für Checkplugins immer ein Dictionary zurück geben. In unserem Fall enthält das Dictionary genau ein Element: Eine ganze Zahl. Wenn die Anzahl der freien "Slots" in unserem Beispiel auf oder unter die hier angegehene Zahl fällt, soll unser Service in den WARN Zustand wechseln.

*  title: Hier geben wir noch einen griffigen Titel für unsere Regel an.

Jetzt: omd restart apache, eine Regel im Hauptmenü anlegen, damit wir das gleich im Checkplugin auspobieren können.


====  Die Regel im Checkplugin benutzen

Damit die Regel zum Greifen kommt, müssen wir dem Checkplugin erlauben Checkparameter entgegen zu nehmen, und ihm sagen welche Regel benutzt werden soll.

Dass wir ein Checkplugin mit Parametern programmieren wollen, teilen wir {CMK} mit, indem wir bei der Registrierung "check_default_parameters" setzen. Im einfachsten Fall übergeben wir ein leeres Dictionary.

Nun wird {CMK} versuchen der Checkfunktion Parameter zu übergeben. Damit das klappen kann, müssen wir die Checkfunktion so erweitern, dass sie als zweites das Argument 'params' in der Signatur hat.

Als drittes übergen wir der Registrierungsfunktion noch den "check_ruleset_name", also den Namen, den wir oben mittels "check_group_name" an die Regelkette vergeben haben.
So weiß {CMK} aus welcher Regelkette die Parameter bestimmt werden sollen.
...
Durch das print Statement können wir überprüfen, dass die Parameter ankommen:


Nun passen wir unsere Checkfunktion an, so dass der übergebene Parameter seine Wirkung entfalten kann. Wir holen uns den Wert mit dem in der Regel gewählten Key aus den Parametern:
...
Falls eine Regel konfiguriert ist, können wir nun die "freien Slots" in unserem Beispiel überwachen.
Wenn allerdings keine Regel definiert ist, wird diese Checkfunktion crashen: Da die default Parameter des Plugins nicht befüllt sind, wird das Plugin bei Abwesenheit einer Regel einen "KeyError" werfen.

Dieses Probem können wir beheben, indem wir bei der Registrierung einen passenden Parameter einfügen:
...
register.check_plugin(
    ...
    discovery_function = discover_foobar,
    check_function = check_foobar,
    check_default_parameters={"warning_lower"},
    check_ruleset_name="foobar",
)
----

Sie sollten Defaultwerte immer auf diese Weise übergeben (und den Fall fehlender Parameter nicht im Checkplugin abfangen), da diese Defaultparameter auch in der GUI angezeigt werden können (Beispiel: "Discovery -> View -> Show Check parameters").

Ein einzelner Wert als Parameter ist in {CMK} allerdings sehr unüblich. Da Services in den Zuständen OK, WARN, CRIT sein können, ist es naheliegend die Parameter immer als Tuple zu definieren, also Schwellen für den WARN und den CRIT Zustand. Dazu passen wir die WATO Regel wie folgt an:
...
Beachten Sie, das eine solche Änderung des Datentyps eine incompatible Änderung darstellt: Existierende Regeln können jetzt nicht mehr von der GUI geladen werden.

Hier noch ein Überblick über einige der wichtigsten Valuespecs:


Float

Nicht so spannend, genau wie Integer.


Percentage

Oft möchte man Schwellen nicht in absoluten Zahlen, sonden in Prozent angeben:
...
Bei dieser Valuespec würde das Checkplugin die paramerter '{"levels_percent": (80.0, 90.0)}' übergeben bekommen.


MonitoringState

Wenn man einen nicht numerischen Zustand erfasst, möchte man für bestimmte Zustände einen konfigurierbaren Servicestate auslösen. Hier kann man einstellen, welchen Zustand  der Service bekommen soll, falls kein Backup konfiguriert bzw vorhanden ist:
...
Bei dieser Valuespec würde das Checkplugin die paramerter '{"no_backup_configured_state": 1, "no_backup_found_state": 1}' übergeben bekommen.
Sie können die Zahl einfach in ein 'State' Objekt umwandeln:
...
Hier kann man ein Alter angeben:
...
(Siehe: Ruleset "AntiVirus last update age")
...
(Siehe: Ruleset "Plesk Backups").

Beachtes Sie die freundliche Hilfe!


Hinweis: Dropdown statt Checkbox!


=== Definition von Perf-O-Metern, Graphtemplates, Metriken

In userem obigen Beispiel haben wir das Plugin "foobar" die Metrik "fooslots" erzeugen lassen. In der GUI können wir sie bereits sehen, allerdings ist die
Darstellung etwas dürftig, da wir dem Metriksystem noch nichts über die Metrik mitrgeteilt haben.

Bevor wir das tun sollten wir -- wie bei der WATO Regel auch -- zunächst prüfen, ob {CMK} nich bereits eine geeignete Metrik mitbringt.
Die vordefinierten Metriken finden wir in dem Verzeichnis "cmk/gui/plugins/metrics/". In der Datei "fs.py" finden wir beispielsweise eine Metric für freien Platz eines Dateisystems:
...
Falls diese für unser Plugin geeignet ist, müssen wir lediglich in unserem Aufruf der Metric Klasse den Namen "fs_free" zu benutzen.

In unserem Beispiel wollen wir einen eigene Metrik für unsere "fooslots" definieren. Dazu legen wir eine weitere Datei in "~/local/share/check_mk/web/plugins/metrics" an:
...
*  key: der Name der Metrik, wie im Aufruf von "Metric" verwendet
*  title: ein verständlicher Titel für den Graphen
*  unit: Hier können sie eine Einheit angeben, für die korrekte Beschriftung der y-Achse. Zur Auswahl stehen etwa 30 Einheiten wie "count", "%", "s" und "bytes". Eine vollstängige Liste findet man (derzeit) in "cmk/gui/plugins/metrics/unit.py".


Im Graph der GUI können wir nun den angepassten Titel und die Farbe sehen.

Auf analoge Weise kann man üer einen "graph_info" eintrag mehrere Metriken zu einem Graphen kombinieren.
...
Auch mal "stack" statt "line" ausprobieren!


==== Darstellung der Metriken im Perf-O-meter


Möchten wir zu inserer Metrik noch ein Perf-O-Meter in der Servicezeile anzeigen, benötigen wir eine weitere Datei in '~/local/share/check_mk/web/plugins/perfometer'
...
Wir benutzen hier "logarithmic", da uns keine maximal möglich Anzahl von "Fooslots" bekannt ist. Bei anderen Werten (wie z.B. Prozent) könnten wir ein Perf-O-Meter auch linear darstellen:
...
* Eine Manual Page (Man Page), welche das Checkplugin für den Anwender dokumentiert.
