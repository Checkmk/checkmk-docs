...
// COMMENT[Diese Ausgabe wird sich noch ändern.]
...
Entwickler immer raten würden), finden Sie im link:devel_check_plugins.html#parsefunction[Abschnitt über Parsefunktionen]
mehr Informationen.
...
link:devel_check_plugins.html#parsefunction[Parsefunktion] definiert haben, zerlegt
...
// Das ist ein API-Problem: warum heisst es nicht auf beiden Seiten gleich?!

Entscheidend ist das Schlüsselwort `check_group_name`,
...
ankommt, wird weiter unten gezeight.


=== Eine eigene Regelkette definieren

// das klingt komisch - nach dem Motto "damit braucht man bei uns nicht rechnen"
Falls es noch keine passende Regelkette gibt (der wahrscheinlichere Fall),
müssen wir uns selbst eine neue erzeugen. Dazu legen wir
...
// 'praktisches und universelles Werkzeug' klingt komisch, wie wenn mir jemand was unterjubeln will :)
...
in Python-Datenstrukturen. In folgendem Beispiele werden `Dictionary`,
...
Damit über die Modulgrenze hinaus sichtbar wird, sollte er mit einem
...
// Fehlt ein hinweis auf Transform? - Das wäre ja auch eine Ausnahme zu "Das `return Dictionary(...)` ist vorgeschrieben."

// ich finde hier sollte nicht erst ein Beispiel mit einem Int gemacht werden, wenn
// später gesagt wird, dass man ein Tuple verwenden sollte. Besser gleich auf den
// Grund hinweisen, warum eben kein einzelner Int verwendet wird
...
// Dinge => Symbole ? Dann weiss man, wonach man googlen muss
...
// warum kann ich das dann nicht weglassen? Und was ist ein `match_type`?
...
// Warum?
// Ist aus meiner Sicht wichtig, weil es ja suggeriert, dass hier auch etwas generisches
// stehen kann.


==== Test
// das klingt negativ und belehrend, vllt eher "Bevor Sie mit der Checkfunktion weiterarbeiten, sollten Sie ..." 
Wenn Sie diese Datei angelegt haben, sollten Sie erstmal ausprobieren, ob alles soweit
funktioniert und nicht gleich mit der Checkfunktion weiterarbeiten. Dazu muss zunächst die
Apache-Instanz neu gestartet werden, damit die neue Datei gelesen wird.
...
Inhalt der Variable `params` mit mittels `print()`
(oder `pprint`, wenn Sie es etwas komfortabler haben wollen)
ausgeben zu lassen. Legen Sie verschiedene Regeln an, probieren
Sie, welche Werte bei `params` ankommen:

// hier fehlt noch ein Hinweis, wie man den `print()`-Befehl zum Einsatz bringt
// => cmk <opts> <host> [|grep ..]
// apache-Restart NICHT erforderlich
...
// warum hat die int-konvertierung nicht die parse()-Funktion vorgenommen?
// vllt sollte es hier bereits einen Hinweise auf `check_levels` geben
[{file}]
----
def check_foobar(item, params, section):
...
// ist das so? Werden da nicht die Defaults der ValueSpec angezeigt?
...
// hinweis auf weiter unten wäre hier schon hilfreich
...
// ist hier ein hinweis auf die float/int prozent/absolut Situation hilfreich?
...
// hier vllt ein Hinweis auf TB / TiB und dass am besten immer in Byte gerechnet werden
// sollte, da die render-Funktionen damit rechnen
...
// ein Thema ist nicht flexibel und extrem flexibel erzeugt bei mir komische Bilder :)
// "ValueSpecs sind sehr flexibel und deren vollständige Behandlung würde.."

Das Thema ValueSpecs ist extrem flexibel und umfangreich und
würde diesen Artikel sprengen. Bitte schauen Sie sich die
Beispiele der von {CMK} mitgelieferten Regeldefinitionen in
...
// Außerdem wird zufällig eine Metrik für das Perf-O-Meter gewählt
