...
[.guihint]#Output of check plugin#. Ab VERSION[1.7.0] heißt diese [.guihint]#Summary# --
...
Daneben gibt es noch das Feld [.guihint]#Details#, welches früher {{Long output
of check plugin (multiline)}} hieß. Hier werden alle Details des Zustandes
...
[{shell}]
----
{c-omd} cmk -II --checks=foobar myhost123
...
[{shell}]
----
{c-omd} cmk --debug -II --checks=foobar myhost123
...
Discovery natürlich aus. {CMK} geht deswegen etwas zielsicherer vor.

==== Detection

Die Serviceerkennung teilt sich in zwei Phasen auf. Zunächst geschieht die
_SNMP-Detection_. Diese hat die Aufgabe, zu ermitteln, welche Plugins
...
`Cisco NX-OS(tm) n5000, Software (n5000-uk9),...`. Ausgehend von
diesem Text kann man für sehr viele Plugins schon definitiv entscheiden,
ob diese hier Sinn ergeben.

Ergebnis der SNMP-Detection ist dann eine Kandidaten-Liste von Checkplugins.

==== Discovery

Im zweiten Schritt werden für jeden dieser Kandidaten die für das jeweilige
Plugin eigentlich Monitoringdaten mit SNMP-Walks geholt. Diese werden dann
der Discovery-Funktion des Checks in dem Argument `section` bereitgestellt,
welche dann daraus wie gewohnt die zu überwachenden Items ermittelt.
...
dann Dinge wie 1=up, 2=down, festgelegt sind) und manchmal auch noch
einen nützlichen Kommentar.

{CMK} liefert eine ganze Reihe von MIB-Dateien mit aus, aber nur soweit
diese frei verfügbar sind. Diese sind sehr allgemeiner Art, und wenn Sie
selbst einen Check für ein Gerät entwickeln wollen, welches von {CMK}
noch nicht unterstützt wird, so ist die Wahrscheinlichkeit, dass dessen
MIB bereits dabei ist, gleich Null.

Versuchen Sie also, die nötigen MIB-Dateien irgendwo auf den Webseiten
vom Hersteller oder sogar auf dem Management-Interface des Geräts
zu finden und installieren Sie diese in der {CMK}-Instanz nach
`local/share/check_mk/mibs`. Dann können Sie in SNMP-Walks OID-Nummern
in Namen umrechnen lassen und so schneller finden, wo die für das
Monitoring interessanten Daten sind. Auch enthalten die MIBs, wenn sie sorgfältig
...
{CMK} kann es sehr einfach für Sie erledigen. Nehmen Sie dazu zunächst
...
die Service [.guihint]#SNMP Info# und [.guihint]#Uptime# gefunden werden und wahrscheinlich
auch noch mindestens ein [.guihint]#Interface#.
...
Wie bereits erwähnt, kann so ein Komplettwalk Minuten oder sogar Stunden
dauern (auch wenn letzteres eher selten ist). Werden Sie also nicht
nervös, wenn es hier etwas dauert. Der Walk wurde nun gespeichert
in der Datei `var/check_mk/snmpwalks/mydevice01`. Es handelt
sich dabei um eine gut lesbare Textdatei, die etwa so beginnt:
...
In jeder Zeile steht eine OID und danach deren Wert. Die allerste ist auch gleich die
...
Die Datei `translated` ist jetzt wie der ursprüngliche Walk, hat aber in jeder
...
Sie sehen also somit z.B., dass `.1.3.6.1.2.1.1.4.0` übersetzt
`SNMPv2-MIB::sysContact.0` bedeutet. Dies ist ein wichtiger Hinweis
auf die Bedeutung der OID. Der Rest ist dann Übung, Erfahrung und natürlich
experimentieren.
...
Die Registrierung der SNMP-Sektion wird erledigt durch den Aufruf von
`register.snmp_section()`. Dies benötigt mindestens drei Argumente:
den Namen der Sektion, die Angaben für die SNMP-Detection und die benötigten
OID-Zweige für das eigentlich Monitoring. Hier ist ein Beispiel für ein
...
die Discoveryfunktion überhaupt ausgeführt werden soll. Dies ist hier
der Fall, wenn der Wert der OID `.1.3.6.1.2.1.1.1.0` (also die
`sysDescr`) mit dem Text `foobar device` beginnt (wobei
Groß-/Kleinschreibung grundsätlich nicht unterschieden wird). Neben
...
|matches(oid, regex) |not_matches(oid, regex) |Wert der OID matcht auf den link:regexes.html[regulären Ausdruck] `regex`, und zwar hinten und vorne geankert, also mit einem exakten Match. Wenn Sie nur einen Teilstring benötigen, ergänzen Sie einfach vorne bzw. hinten noch ein `.*`
...
erfüllt ist. Hier ist ein Beispiel, wo verschiedene Wert für die `sysDescr`
erlaubt sind:
...
Übrigens: wer sich ein bisschen mit link:regexes.html[regulären Ausdrücken] auskennt, der kann
spezielle dieses Beispiel auch in einer Zeile formulieren:
...
Hier wird die OID `.1.3.6.1.4.1.4455.1.3` nur bei solchen Geräten
geholt, die `foo` in ihrer `sysDescr` haben.
...
* Falls die Detection eigentlich zulässige Geräte _nicht_ erkennt, werden dort auch keine Services gefunden.

=== Die OID-Bereiche für das Monitoring

Die wichtigste Stelle der SNMP-Deklaration ist die Angabe, welche OIDs für
das Monitoring geholt werden sollen. Betrachten wir erst den einfachen Fall -- nämlich den,
dass Sie nur aus einer Stelle aus dem Baum Daten benötigen. Hier geben Sie zwei Dinge
an:
