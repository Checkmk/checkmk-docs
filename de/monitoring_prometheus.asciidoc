...
:description: Mit dieser Erweiterung integrieren Sie Prometheus Instanzen in {CMK}. Details zur Konfiguration erfahren Sie hier.
...
Mit dem Erscheinen des link:https://blog.checkmk.com/feature-pack-2-comes-with-prometheus-integration#how-to-install-feature-pack[Feature Pack 2]
können Sie Prometheus in {CMK} ab der Version {v16}p12 integrieren. Die
Integration der wichtigsten Exporter für Prometheus wird dabei über einen
link:datasource_programs.html#specialagents[Spezialagenten] zur Verfügung gestellt.

Nach der link:mkps.html#feature_packs[Aktivierung des Erweitungspakets] für Prometheus
stehen Ihnen die folgenden Exporter zur Verfügung.

*  link:https://checkmk.de/cms_check_plugins_catalog.html#cAdvisor[cAdvisor (Container Advisor)]
...
Prometheus-eigenen link:monitoring_prometheus.html#promQL[Abfragesprache PromQL].
...
bestätigen Sie Ihre Auswahl über [.guihint]#Save & Finish#.
...
Sie in WATO über [.guihint]#Host & Service Parameters > Datasource Programs > Prometheus#.
...
Node Exporter. Wählen Sie nun [.guihint]#Add new Scrape Target# und im sich öffnenden
...
link:monitoring_prometheus.html#dcd_cadvisor[dynamischen Hostkonfiguration] hat.
...
die gleiche, wie auch für den link:monitoring_prometheus.html#node_exporter[Node Exporter]
oder den link:monitoring_prometheus.html#cadvisor[cAdvisor] oben
...
link:wato_hosts.html#services[Serviceerkennung] auf dem Prometheus-Host durchzuführen.
...
sich die Einrichtung einer link:dcd.html[dynamischen Hostkonfiguration] an. Die Daten der
...
in der link:monitoring_prometheus.html#cadvisor[Prometheus-Regel]) an.
Auf diese Weise können Sie nicht mehr vorhandene Container dennoch
mit der Option [.guihint]#Delete vanished hosts# in der dynamischen Hostkonfiguration
