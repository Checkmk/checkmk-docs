...
Aber erinnern wir uns: Im Einleitungsartikel zur
link:wato.html[{CMK}-Konfiguration] haben wir gesehen, dass die
...
Das [.guihint]#Bulk renaming# erlaubt dabei über intelligente Operationen
systematische Namensanpassungen. Im Feld [.guihint]#Hostname matching# geben Sie
zunächst optional einen regulären Ausdruck an, der auf *den Anfang* der Hosts
matcht, welche Sie umbenennen möchten -- hier im Beispiel also alle Hosts, die
mit `mysrv` beginnen. Dann fügen Sie eine oder mehrere Operationen ein, die
*der Reihenfolge nach* auf die Hostnamen angewendet werden sollen. In folgendem
Beispiel wird von allen Hosts zunächst alles ab dem ersten `.` abgeschnitten und
danach die Endung `.servers` angefügt:
...
sowie link:notifications.html[Alarmierungen] und link:alert_handlers.html[Alert Handler]
...
link:alert_handlers.html[Alert Handler] nutzen.  Wichtig ist dabei lediglich die Angabe
