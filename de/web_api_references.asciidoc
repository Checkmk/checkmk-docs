...
:description: Alle Möglichkeiten Checkmk per HTTP-API (oder Web-API) zu konfigurieren, sind in diesem Artikel beschrieben. Er ist damit eine Erweiterung des einführenden Artikels zur Web-API.
...
Um die Verzeichnisse im WATO zu verwalten, bietet {CMK} ab Version VERSION[1.5.0] die folgenden Kommandos:
...
SLAs können ab VERSION[1.5.0b8] mit dem Befehl `get_sla` abgerufen werden. Für eine solche Abfrage, können die folgenden Zeitdaten verwendet werden:
