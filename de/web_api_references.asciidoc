...
:description: Alle Möglichkeiten {CMK} per HTTP-API (oder Web-API) zu konfigurieren, sind in diesem Artikel beschrieben. Er ist damit eine Erweiterung des einführenden Artikels zur Web-API.
...
Um die Verzeichnisse im WATO zu verwalten, bietet {CMK} ab Version {v15} die folgenden Kommandos:
...
ob es sich um Slave-Instanzen handelt, die über die abgerufene konfiguriert
