...
//SK: Ich weiß, dass dieser Punkt bereits unten als 6. angesprochen wird. Es kommt leider immer wieder dazu, dass dies übersehen wird. Sobald wir die Admonitions für Asciidoctor gestylet haben, können die # # wieder weg und durch eine schicke Box ersetzt werden.
#*Wichtig:* Bitte beachten Sie, dass das Mangement-Interface aller {CMK}-Racks *im Auslieferungszustand deaktiviert* ist and bei Bedarf xref:appliance_rack_config#ipmi[manuell über die Weboberfläche aktiviert werden muss].
