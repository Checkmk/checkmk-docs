...
// TK: "Derzeit" klingt nach: Da kommt noch was. Ist mir aber nicht bekannt.
// TK: `deb` oder `rpm` Pakete: Sonst schreiben wir DEB- oder RPM-Pakete (Dateityp in Grossbuchstaben)
// TK: Vorschlag: Der Agent Controller läuft nur mit dem Init-System `systemd` auf einer x86/64-Plattform und setzt die Installation durch ein DEB- oder RPM-Paket voraus.
...
Diese Voraussetzungen erfüllen zwar die größte Zahl der Linux-Server und -Desktops, doch sind Jahre lang _hochaktualisierte Systeme_, ältere _virtuelle Maschinen_ mit i686 Instanzen, _Distroless Container_ oder _Embedded Linux_ Systeme eben keine Randerscheinungen, sondern normale Bestandteile vieler Systemlandschaften, für die es Überwachungsbedarf gibt.
// TK: "hochaktualisiert": Ich weiß was Du meinst, der Begriff ist aber merkwürdig - klingt nach topaktuell. Besser in einem Nebensatz erklären.
Dank der Modularität von {CMK} können Sie solche Linux-Hosts dennoch einfach ins Monitoring aufnehmen.
// TK. Bei uns ist ja fast alles "einfach"... aber das jetzt auch? Ich würds weglassen.

Da der verschlüsselte Transport der Agentendaten per Agent Controller hier nicht in Frage kommt, erklären wir in diesem Artikel sowohl die Konfiguration des unverschlüsselten Transports über einen _Internet Superserver_ als auch die Einrichtung eines _SSH-Tunnels_.
// TK: Klarer machen, dass es ein entweder-oder ist.

// MFS: Das habe ich entfernt, ich möchte nicht dafür bürgen, dass Bash 2.04 unterstützt wird, ein alter Printserver mit Ubuntu 12.04 auf i686 ist dagegen nicht abwegig
// TK: OK!
////
...
== Installation
// TK: Hier klarmachen, dass es Alternativen sind: Wenn ich Pakete installieren kann, bin ich mit 2.1 fertig, sonst 2.2, und ohne Tarball 2.3

[#frompackage]
=== Installation aus Paketen

Die Installation aus `deb`- oder  `rpm`-Paketen beschreiben wir im Artikel xref:agent_linux#[Linux überwachen]. Da Sie sehr wahrscheinlich über die Verlinkung innerhalb dieses Artikels hierher gefunden haben, verzichten wir an dieser Stelle auf die Wiederholung der Vorgehensweise.
// TK: Wegen SKs Kommentar: Vielleicht hier doch zusätzlich die Installation im Zack-Zack-Schnelldurchlauf mit 2 Kommandos für Super-Experten? Paket per REST-API holen und ausführen und fertig?
...
// TK: Kriegen wir aber nicht. Hier sollte ein hilite auf die Linux TGZ rein.
...
// TK: Hier klarmachen, dass man als letzte Alternative nur das Agentenskript zum Laufen bringt und auf alles andere verzichtet.
...
// TK: Jetzt schon wieder check_mk_agent? Lass den Test hier weg oder verweis ins vorherige Kapitel.
...
// MFS: Mir sind keine Distributionen bekannt, die auf `timeout` verzichten. Daher würde ich auf diesen Abschnitt gerne verzichten.
////
...
Merken Sie sich diesen Programmnamen.
// TK: ..., da sie ihn im weiteren Verlauf noch benötigen - unabhängig von der gewählten Zugriffsmethode. 

Eine Besonderheit ist, wenn hier `cmk-agent-ctl` erscheint:
// TK: Das klingt zu sehr danach wie: Das kann eigentlich nicht richtig sein.
// TK: Vielleicht so: Wenn als Programmname hier `cmk-agent-ctl` ausgegeben wird, können Sie sich freuen.
// TK: Das kann aber nur sein, wenn man das Paket installiert hat, oder? Dann hinschreiben.
Dann ist Ihr Linux (vor allem die verwendete systemd-Version) doch aktuell genug für die Verwendung des Agent Controllers, wie im Artikel xref:agent_linux#[Linux überwachen] beschrieben, und Sie können die Registrierung des Agenten vornehmen.


// TK: Das führt jetzt vielleicht zu weit, aber trotzdem:
// TK: Wenn man den Inhalt von 4. Konfiguration des Zugriffs in die Bestandsaufnahme zieht, könnte man die Folgekapitel ab "Unverschlüsselt: Einrichtung von (x)inetd" eine Ebene höher ziehen und hätte die allerletzten 3 Unterkapitel im TOC: Ebene 5 verwenden wir ja nicht.
// TK: Alternativ alles auf eine Ebene ziehen: == Entscheidung über die Zugriffsmethode, == Unverschlüsselter Zugriff mit (x)inetd, == Verschlüsselter Zugriff mit SSH-Tunnel
...
Sollte ein "klassischer" `inetd` aktiv sein, ist es meist sinnvoll, xref:otherinetd[diesen einzurichten].
// TK:..., ist es meist sinnvoller, diesen zu verwenden und xref:otherinetd[einzurichten] statt auf `xinetd` zu wechseln.


[#xinetd]
==== xinetd einrichten

{cee-only} Für die Konfiguration eines bereits vorhandenen `xinetd`, der das Verzeichnis `/etc/xinetd.d/` zur Konfiguration verwendet, liegt im entpackten TGZ-Archiv ein Skript bei, welches in zwei Schritten zuerst die Konfiguration installiert und dann den `xinetd` die neue Konfiguration einlesen lässt.
// TK: Das gilt doch auch für die Paketinstallation und nicht nur für den Tarball?
...
----
// TK: *grep 6556/ /etc/services* oder *grep 6556 /etc/services*?
...
// MFS: Leserechte sind zulässig und manchmal sogar erwünscht. Der folgende satz ist damit falsch:
...
// TK: Die beiden vorherigen Sätze umstellen: Zuerst Verbindungstest, dann Aktivieren der Änderungen und damit "Im Monitoring"
...
// TK: Hier fehlt, dass sonst die Kommandoausgabe den Prozess anzeigt, um den man sich kümmern muss. Wie, wird in den folgenden Abschnitten gezeigt.
...
// TK: Den Test an den Schluss ziehen ("Erfolgskontrolle"), da es ja für alle 3 gilt.
