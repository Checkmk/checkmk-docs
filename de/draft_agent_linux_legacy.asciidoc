...
// Eine Besonderheit ist, wenn hier nach Installation aus DEB- oder RPM-Paket `cmk-agent-ctl` erscheint:
Wenn als Programmname hier nach Installation aus DEB- oder RPM-Paket  `cmk-agent-ctl` ausgegeben wird, können Sie sich freuen:
...
Sollte ein "klassischer" `inetd` aktiv sein, ist es meist sinnvoll, deisen zu nutzen und xref:otherinetd[einzurichten] statt auf `xinetd` zu wechseln.
