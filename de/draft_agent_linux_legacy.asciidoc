...
Diese Voraussetzungen erfüllt zwar die größte Zahl der Linux-Server und Desktops, doch sind Jahre lang "hochaktualisierte" Systeme, ältere virtuelle Maschinen mit i686 Instanzen, Distroless Container oder Embedded Linux Systeme eben keine Randerscheinungen, sondern normale Bestandteile vieler Systemlandschaften, für die es Überwachungsbedarf gibt.

Falls eine oder mehrere der folgenden Aussagen zutrifft…

* Ihr System nutzt ein anderes Init-Systems als `systemd`
* Die Rechnerarchitektur ist eine andere als x86/64, beispielsweise i686 (32 Bit) oder ARM
* Pakete werden weder als `deb` noch `rpm` verwaltet

…wird zwar der Agent Controller nicht unterstützt, dennoch können Sie Ihren Linux-Host ins Monitoring aufnehmen.

Da der verschlüsselte Transport der Agentendaten per Agent Controller nicht in Frage kommt, erklären wir in diesem Artikel sowohl die Konfiguration des unverschlüsselten Transports über einen _Internet Superserver_ als auch die Einrichtung eines _SSH-Tunnels_.
...
{cee-only} Für die komfortable distributionsunabhängige Installation benötigen den Linux-Agenten im TGZ-Archivformat ("Tarball"), den Sie in den {EE} im Setup-Menü über [.guihint]#Agents > Windows, Linux, Solaris, AIX# herunterladen können.
...
[#manual]
=== Manuelle Installation des Scriptes
...
Vergessen Sie bitte nicht die letzten beiden Befehle:
...
Uns interessiert in diesem Fall der Programmname innerhalb der Klammer, hier `xinetd` – notieren Sie ihn.

Eine Besonderheit ist, wenn hier `cmk-agent-ctl` erscheint:
Dann ist Ihr Linux (vor allem die verwendete Systemd-Version) doch aktuell genug für die Verwendung des Agent Controller wie im Artikel xref:agent_linux#[Linux überwachen] beschrieben und Sie können die Registrierung des Agenten vornehmen.
...
* Oder ist Ihnen höhere Sicherheit einen gewissen Mehraufwand wert?
...
Sollten Sie zum Schluss kommen, dass eine unverschlüsselte Datenübertragung ein kalkulierbares Risiko ist, steht die Einrichtung einen _Internet Superservers_ an.
...
Ist kein Prozess aktiv, installieren Sie bitte einen `xinetd` über das Paketmanagement Ihrer Distribution.
...
Falls der oben ausgeführte Befehl `ss -tulpn` keinen Prozess gefunden hat, der an TCP Port 6556 lauscht, sind Sie mit der Einrichtung des SSH-Tunnels fertig.

===== Xinetd

Um bei `xinetd` Port zu schließen, deaktivieren Sie den xinetd-Dienst von {CMK}, indem Sie den Wert von `disabled` auf `yes` setzen.
...
Ist es `inetd`, der Zugriff auf Port 6556 regelt, passen Sie die Konfigurationsdatei `/etc/inetd.conf` an.
Suchen Sie dort die relevante Zeile:

[{shell}]
----
{c-root} grep -n 6556 /etc/inetd.conf
----
...
Prüfen Sie anschließend mit `telnet` oder `nc`, ob noch Zugriff möglich ist. 
