...
= Linux überwachen im Legacy-Pull-Modus
:revdate: draft
:title: Linux überwachen im Legacy-Pull-Modus
:description: Der Linux-Agent kann im Legacy-Pull-Modus betrieben werden, wenn er die Voraussetzungen für den verschlüsselten Pull-Modus nicht erfüllt.
...
Seit der {CMK} Version {v21} beherrscht der neue Linux-Agent den registrierten, TLS-verschlüsselten und komprimierten *Pull-Modus*.
Allerdings muss dafür auf dem Host, auf dem er installiert werden soll, der Superserver `systemd` laufen.
Im Artikel xref:agent_linux#[Linux überwachen] erfahren Sie, wie Sie den Agenten im Pull-Modus installieren, konfigurieren und erweitern können.

Um den Agenten auch auf Hosts ohne `systemd` zu betreiben, unterstützt der Linux-Agent den sogenannten *Legacy-Pull-Modus*.
In diesem Modus arbeitet der neue wie der alte Agent bis zur Version {v20}, d.h. ohne Registrierung, TLS-Verschlüsselung und Komprimierung.
// TK: Das sollte konkreter werden: kein systemd, aber xinetd (ggf. mit Nachinstallation), aber auch manuelle Installation, wenn kein Paket installiert werden kann.
// MFS: Konkretisiert
Dieser Artikel zeigt Ihnen, wie Sie den Linux-Agent im Legacy-Pull-Modus einrichten, er deckt dabei sowohl die Situation ab, dass Sie per Paketmanager installieren konnten und jetzt die Konfiguration nur abschließen oder kontrollieren müssen, als auch die Verwendung unter eher exotischen Umgebungen, die eine manuelle Installation erfordern. 

Dabei werden hier nur die Aufgaben behandelt, die im Legacy-Pull-Modus anders als im Pull-Modus durchzuführen sind.
...
* xref:agent_linux#disable_sections[Sektionen deaktivieren]
* xref:agent_linux#agent_plugins[Agent um Plugins erweitern]
...
////
TK: Die Übersicht hab ich weggelassen, auch wenn die manuelle Installation hier erwähnt wird.
MFS: Jepp, weg mit!
...
Falls bislang kein Internet-Supersver konfiguriert ist, installieren Sie bitte das Paket `xinetd` vor der Installation des Agenten, dies erspart Ihnen einen Konfigurationsschritt.
Das RPM/DEB-Paket installieren Sie dann genauso wie es im xref:agent_linux#installation[Installationskapitel] für den Agenten im Pull-Modus beschrieben ist.


Nach der Installation ist der Agent sofort aktiv und kann auf TCP Port 6556 abgefragt werden.

////
TK: Der Abschnitt könnte eine Einleitung für die manuelle Installation sein

==== Exotische Linux-Distributionen

Auch wenn der {CMK}-Server nur Enterprise-Linux-Distributionen unterstützt, die deren Hersteller noch unterstützt, ist der {CMK}-_Agent_ hier viel genügsamer.
...
Auch hier gilt: , stellen Sie bitte _vor_ der Installation des {CMK}-Agenten sicher, dass `xinetd` auf dem System vorhanden ist.
Andere Internet-Superserver (GNU inetutils, OpenBSD inetd, Busybox inetd) erfordern manuelle Konfiguration.

Sie benötigen aus der Seite der Agentendateien den Linux-Agenten im TGZ-Archivformat.
Das TGZ-Archiv enthält die komplette Verzeichnisstruktur des Linux-Agenten fürs Entpacken des Agenten im Wurzelverzeichnis des überwachten Hosts.

// MFS: Hier muss Screenshot Download des Tarballs hin!
// image::agent_linux_agents_manual.png[]

Damit alle Pfade stimmen, ist der Parameter `-C` ("change directory") beim Entpacken unerlässlich, wird verwenden zudem `--no-overwrite-dir`, damit Berechtigungen bereits vorhandenen Verzeichnisse nicht verändert werden. 
...
Wird hier eine alte Versionsnummer ausgegeben, haben Sie vermutlich eine ältere Version als Script als `/usr/local/bin/check_mk_agent` installiert. Verschieben Sie dieses alte Script, beispielsweise indem Sie `.bak` anhängen.
...
Für die Konfiguration eines bereits vorhandenen `xinetd`, der das Verzeichnis `/etc/xinetd.d/` zur Konfiguration verwendet, liegt ein Script bei, welches in zwei Schritten zuerst die Konfiguration installiert und dann den `xinetd` die neue Konfiguration einlesen lässt. Das Script müssen Sie mit vollem Pfad aufrufen:
...
Auch hier gilt: War bereits eine ältere Version des {CMK}-Agenten installiert, prüfen Sie die Pfade, welche in der Datei `/etc/xinetd.d/check-mk-agent` eingetragen sind: Installationsordner des Agentenskriptes ist nun `/usr/bin`. Falls Ihr `xinetd` das alte Konfigurationsschema mit lediglich einer großen `/etc/xinetd.conf` nutzt, übertragen Sie die Beispielkonfiguration aus `/etc/check_mk/xinetd-service-template.cfg` in die `/etc/xinetd.conf`.

////
MFS: /usr/bin/waitmax ist Teil des Tarballs.

Falls Sie eine sehr alte Distribution haben (vor 2010 veröffentlicht), welche den Befehl `timeout` nicht kennt, dann laden Sie noch das kleine Programm `waitmax` von der Agentenseite (ebenfalls im Kasten [.guihint]#Agents#) und installieren Sie es ebenfalls nach `/usr/local/bin`.
...
Waitmax wurde als Teil von {CMK} zu einer Zeit entwickelt, als `timeout` noch nicht verbreitet war.
Es hat fast die gleiche Aufrufsyntax:
...
////
TK: Bereits von SK auskommentierte Befehle zur Nachinstallation von xinetd für RPM:
...
TK: Bereits von SK auskommentierte Befehle zur Nachinstallation von xinetd für DEB:
...
////
MFS: Auch das kann weg, Konfiguration wird ja mitgeliefert 

Legen Sie nach der Installation die folgende Datei an:
...
Tragen Sie hier unter `only_from` die IP-Adressen Ihrer {CMK}-Server ein, die auf den Agenten zugreifen dürfen.
Danach braucht es nur noch ein Aktivieren und der Agent ist bereit:
...
Sobald Sie den Agent installiert und konfiguriert haben, stellen Sie sich sicher die Frage, wie Sie ausprobieren können, ob Sie alles richtig gemacht haben.
...
Aber natürlich gibt es noch weitere Diagnosemöglichkeiten, wenn man direkt auf dem Zielsystem selbst eingeloggt ist.

Da der „Agent“ im Grunde nichts als ein einfaches Programm ist, das Daten über Ihr System beschafft und diese als lose formatierten Text ausgibt, können Sie ihn auch als Programm aufrufen, und zwar
ganz einfach so:
...
Diese Ausgabe beweist natürlich nicht, dass der Agent auch über das Netzwerk erreichbar ist.
Aber Sie können so testen, ob in der Ausgabe alle gewünschten Daten enthalten sind – beispielsweise, ob alle installierten Plugins Daten liefern.
...
Das ist über die Direktive `only_from` der Konfigurationsdatei Ihres `xinetd` schnell zu erreichen. Tragen Sie durch Leerzeichen getrennt IP-Adressen oder Adressbereiche (in der Form `12.34.56.78/29` oder `1234::/46`) ein. Zulässig sind auch Hostnamen: in diesem Fall wird geprüft, ob der durch _Rückwärtsauflösung_ der IP-Adresse des anfragenden Hosts ermittelte Hostnamen mit dem eingetragenen übereinstimmt:
...
Aber dann ist es sehr wahrscheinlich, dass er die Antwort nicht bekommt -- weil diese zum echten Monitoringserver geht.
Oder er bekommt sie tatsächlich, aber der {CMK}-Server bekommt keinerlei Daten und wird sehr bald einen Fehler melden.


////
TK Erstmal auskommentiert
==== systemd

Da die meisten Distributionen zu systemd gewechselt sind, der nicht nur die Aufgaben des Init-Systems, sondern auch Netzwerkfunktionalität bereitstellt, arbeiten Linux-Distributoren daran, den guten alten xinetd abzuschaffen.
Der vorpaketierte Linux-Agent (nicht der aus der Agentenbäckerei!) installiert sich daher bereits mit Mitteln des systemd, wenn das Zielsystem auf Systemd basiert und _kein_ bereits installierter xinetd verfügbar ist.

Bei der Verwendung von systemd lässt sich natürlich auch einschränken, welcher Rechner bzw. welche IP den {CMK}-Agenten abfragen darf.
Das Pendant zum von xinetd bekannten _only_from_ heißt hier _IPAddressAllow_.
...
Wenn Sie noch nicht zu den von systemd überzeugten gehören, gibt es aber einen einfachen Ausweg:
Es ist selbst bei systemd-basierten Systemen (noch) möglich, xinetd zu verwenden.
Dieser läuft dann als Dienst unter systemd.
Installieren Sie dazu das Paket `xinetd` und danach den {CMK}-Agenten _erneut_.
...
Kopieren Sie jetzt den Inhalt der Public Keys in diese Datei.
...
Die Abfrage nach dem [.guihint]#key fingerprint# kommt übrigens nur beim ersten Mal.
...
Nach einem Speichern und einem xref:wato#activate_changes[Activieren der Änderungen] sollte alles funktionieren!
...
[#encryption]
=== Eingebaute (symetrische) Verschlüsselung
...
Auch ohne Agentenbäckerei geht der erste Schritt dennoch über [.guihint]Setup# und das Anlegen einer Regel im Regelsatz [.guihint]#Setup > Agents > Access to Agents > Checkmk agent > Encryption#.
...
Die Verschlüsselung funktioniert mit einem gemeinsamen Passwort, das Sie hier angeben und sowohl auf dem {CMK}-Server als auch auf dem Agenten im Klartext gespeichert werden muss („Shared secret“).
...
// TK: Ich hab die Zeielen auskommentiert, die auf jeden Fall nur im Hauptartikel relevant sind. Bei einigen war ich mir unsicher.
...
|===
|Pfad |Bedeutung
|`/usr/bin` |Installationsort des Agentenskripts `check_mk_agent` auf dem Zielsystem.
...
// TK: Datei gibts nicht mehr in /etc/xinetd.d
