...
Dieser Artikel zeigt Ihnen, wie Sie den Linux-Agent im Legacy-Pull-Modus einrichten. 
...
Sie installieren das RPM/DEB-Paket genauso wie es im xref:agent_linux#installation[Installationskapitel] für den Agenten im Pull-Modus beschrieben ist.
...
Sehr gut eignet sich `/usr/local/bin/`, da es sich im Suchpfad befindet und für eigene Erweiterungen gedacht ist.
Auch hier können Sie wieder direkt mit `wget` arbeiten:
...
Das ist einfach gemacht und war schon im Beispiel weiter oben zu sehen:
