...
// TK: SLA funtioniert zur Zeit (so halbwegs) nur im Master wg. Crash beim SLA an Service binden (SUP-8782, CMK-9084)
// TK: Als SLA-Newbie war ich mit inhaltlichen Änderungen sehr vorsichtig und hab sie mit Kommentaren gekennzeichnet.
...
// TK: Da stand vorher nur ein lapidarer Satz, mit dem ich die Pointe nicht verstanden hatte, daher umgeschrieben.
...
{cee-only} Wenn Sie eine der {EE} einsetzen, verfügt {CMK} über eine separate Funktion für Service Level Agreements (SLA).
...
// TK: Da stand vorher eine zu kurze Beschreibung, die auch nicht zum Screenshot passte.
...
.Symbole in der SLA-Spalte zeigen den Status der verschiedenen SLA-Perioden: ganz rechts wird die aktuelle angezeigt
...
.Mit ID und Titel werden die allgemeine Eigenschaften des SLAs festgelegt
image::sla_general_properties.png[alt="Dialog zur Festlegung der allgemeine Eigenschaften eines SLAs."]
...
Und auch hier dürfen Sie wieder über die [.guihint]#Levels for SLA monitoring# verfeinern und bestimmen, bei wieviel verbleibenden Vorfällen vor dem Brechen des SLA Sie mit einem {WARN} beziehungsweise {CRIT} gewarnt werden.
...
// TK: Den Screenshot sla_view_context.png hab ich hier rausgenommen und durch 1 Link ersetzt. Verwirrt an dieser Stelle, wenn man das Ergebnis sieht, bevor man mit der Konfig überhaupt angefangen hat.
...
// TK: Den 1. Eintrag sieht man momentan gar nicht, sondern nur nach dem Patchen wie in CMK-9316 beschrieben.
...
// TK: Aktuell heisst der 2. Eintrag nicht mehr "SLA - Column specific" sondern "SLA - <SLA_Name>". Warum? Bringt keinen Informationsgewinn und verwirrt eher. Da die Umbennung evtl. mit CMK-9316 wieder zurückgenommen wird, hab ich den ursprünglichen Namen erstmal dringelassen.
// TK: Hier gibt es einen neuen Folge-Bug (auch im Daily build vom master 2022.01.13): Selbst wenn ich die Regel lösche, wird mir weiterhin "Hosts/Services: SLA - <SLA_Name>" angeboten. "Hosts/Services: SLA - Column specific" sehe ich erst dann wieder, wenn ich auch das SLA gelöscht und wieder ganz von vorne angefangen habe.
...
// TK: Ich sehe keinen Unterschied zwischen den ersten beiden horizontalen Balken Availability und SLA rawdata. Brauch ich Brille?
...
// TK: Und wofür soll die Markierung gut sein? Ich kann nicht sehen, dass sich in den Daten irgendwas verändert.
...
Der Status eines BI-Aggregats kann über den Regelsatz [.guihint]#Check State of BI Aggregation# als ganz normaler
Service überwacht werden.
Dieser erscheint dann beispielsweise als [.guihint]#Aggr My Bi# in den Ansichten und kann wiederum über die bereits xref:link_sla_service[oben] genutzte Regel [.guihint]#Assign SLA definition to service# mit einem SLA verknüpft werden.
...
Im Feld [.guihint]#Aggregation Name# tragen Sie den Titel einer Top-Level-Regel aus Ihrem BI-Paket ein.

image::sla_view_bi_modern.png[width=97%]
...
* xref:basics_downtimes#[Wartungszeiten]: [.guihint]#Setup > Hosts > Host monitoring rules > Recurring downtimes for hosts# und [.guihint]#Setup > Services > Service monitoring rules > Recurring downtimes for services# (beide Regeln nur in den {CEE})
...
Um über eine Statusänderung bereits vorher informiert zu werden, müssen Sie die xref:sla#create_sla[SLA-Levels konfigurieren.]
