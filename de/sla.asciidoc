...
Unter [.guihint]#SLA-Settings}} setzen Sie nun die {{SLA period# auf den gewünschten
...
Datensatz „{OK}, [.guihint]#Minimum}}, {{90#“. Wird dieser Wert unterschritten,
...
Anschließend setzen Sie unter [.guihint]#Conditions}} im Bereich {{Services#
...
Abfrage geben Sie [.guihint]#All services}} als {{Datasource# an. Die folgende Abfrage,
...
[.guihint]#Hosts: Hostname}} und {{Services: Service description#
hinzu, um eine Basis für die Ansicht zu haben.

In der Spaltenauswahl finden Sie auch zwei SLA-spezifische Spalten:
[.guihint]#Hosts/Services: SLA - Service specific}} und {{Hosts/Services: SLA - Column specific#.
...
[.guihint]#monthly}} in Ihrer SLA-Definition gewählt haben und hier {{Last Year#
...
setzen Sie [.guihint]#Starting from period number}} auf {{0}} und {{Loocking back#
auf [.guihint]#4#.

[.guihint]#Layout options}}: Standardmäßig steht diese Option auf {{Only Display SLA Name#.
...
* [.guihint]#Display a summary for each SLA period}} zeigt eine grafische Zusammenfassung unter dem Label {{Aggregated result#.
* [.guihint]#Display a summary over all SLA periods}}: Zeigt eine textliche, prozentuale Zusammenfassung über alle SLAs unter dem Label {{Summary#.
...
optional unter [.guihint]#Grouping}} die Spalte {{Host: Hostname# hinzu -- das sorgt
...
einen Filter unter [.guihint]#Host}} für den {{Hostname# setzen:
...
== Weitere Ansichten#moreslaviews

=== Spalten-spezifische SLA-Anzeige#slacolumn
...
de-/markieren. Das funktioniert in den Zeilen [.guihint]#Result}} und {{Aggregated results#.
...
// Was Sie im BI-Modul über den Punkt [.guihint]#Aggregations|New Aggregation}} erstellen, ist *nicht gemeint!* Dabei handelt es sich nur um Ansichtsoptionen: Dort wird festgelegt, dass und wie ein Aggregat auch tatsächlich in einer Ansicht auftaucht (beispielsweise beschränkt auf einen einzelnen Host). Daher gibt es auch kein Feld für einen Aggregatsnamen, sondern nur für {{Aggregation Groups}}. Diese Aggregatsgruppen tauchen als einzelne Bereiche mit separaten Überschriften beispielsweise unter {{Views|Business Intelligence|All Aggregations}} auf. Also nochmal: Die {{Aggregations# aus folgendem Bild sind _nicht gemeint:_
//
...
* Servicezeiten: [.guihint]#WATO|Monitoring Configuration|Service Period for hosts}} bzw. {{... for services#
