...
In {CMK} können Sie xref:availability#[Verfügbarkeiten] auswerten und für
diese auch eine rudimentäre xref:availability#thresholds[SLA-Auswertung]
...
{cee-only} Wenn Sie eine {EE} einsetzen, verfügt {CMK} aber auch über eine
separate Funktion für Service Level Agreements. Das SLA-Feature baut auf
den Verfügbarkeitsdaten auf und erlaubt nun eine wesentlich detailliertere
...
Daten können auf die in WATO definierten xref:timeperiods#[Zeitperioden], also
...
* Zeitperiode: Aktive xref:timeperiods#[Zeitperioden] aus der WATO-Konfiguration; etwa nur Werktage.
...
eine xref:views#[Ansicht.] So ließen sich zum Beispiel die letzten fünf Wochen,
...
|Scheduled Downtimes |Berücksichtigung geplanter xref:basics_downtimes#[Wartungszeiten.]
...
|Service Time |Berücksichtigung von xref:availability#serviceperiod[Servicezeiten.]
|Notification Period |Berücksichtigung von Benachrichtigungszeiten.
|Short Time Intervals |Ignorieren von Intervallen unterhalb einer gegebenen Dauer, um kurzzeitige Störungen zu ignorieren (ähnlich dem Konzept xref:availability#softstates[Soft states]).
...
fest. Sofern Sie in WATO xref:timeperiods#[Zeitperioden] festgelegt
haben, können diese, wie xref:availability#serviceperiod[oben] bereits für
...
xref:sla#moreslaviews[Spalten-spezifische SLA-Anzeige.]
...
xref:regexes#[Regulären Ausdrücken] arbeiten und die SLA-Definition wie in diesem
...
erledigen, sehen Sie xref:slacolumn[weiter unten.]
...
erstellen Sie noch eine xref:views#new[neue Ansicht] für die SLAs. Für
...
für alle Services anzeigen zu lassen. Dazu xref:slacolumn[später]
