...
* Prozentsatz-SLA: Prozentsatz, zu dem ein Service-Status ({OK}, {WARN}, {CRIT}, {UNKNOWN}) ober-/unterhalb eines gegebenen Werts liegt.
...
// TK: Versteh ich nicht. Die Beschreibung ist zu kurz. Die Darstellung im View sollte ausführlicher sein, ggf. nach dem Screenshot image::sla_view.png[]
...
Das Ergebnis wird dann so aussehen wie das xref:image_sla_view[Bild am Ende diese Kapitels.]
// TK: Den Screenshot sla_view_context.png hab ich hier rausgenommen. Verwirrt an dieser Stelle, wenn man das Ergebnis sieht bevor man mit der Konfig überhaupt angefangen hat.
...
Die folgende Abfrage, ob Informationen eines einzelnen Objekte gezeigt werden sollen, bestätigen Sie mit dem Standardwert [.guihint]#No restrictions to specific objects.# 

Geben Sie unter [.guihint]#General Properties# eine ID ein (hier `MySLAView_Demo`), einen Titel ein (etwa `My SLA Demo View`), und wählen Sie letztlich noch ein Thema aus wie [.guihint]#Workplace#, wenn Sie später die SLA-Ansichten unter einem eigenen Eintrag im xref:user_interface#monitor_menu[Monitor-Menü] sehen möchten.
...
// TK: Aktuell heisst der 2. Eintrag nicht mehr "SLA - Column specific" sondern "SLA - <SLA_Name>"? Warum? Bringt keinen Informationsgewinn und verwirrt eher. Da die Umbennung evtl. mit CMK-9316 wieder zurückgenommen wird, hab ich den usrprünglichen Namen dringelassen.
// TK: Hier gibt es einen neuen Folge-Bug (auch im Daily build vom master 2022.01.13): Selbst wenn ich die Regel lösche, wird mir weiterhin "Hosts/Services: SLA - <SLA_Name>" angeboten. "Hosts/Services: SLA - Column specific" sehe ich erst dann, wenn ich auch das SLA gelöscht und wieder ganz von vorne angefangen habe.
...
Nach Auswahl im [.guihint]#Monitor#-Menü bekommen Sie als Ergebnis eine Ansicht mit fünf Statussymbolen als Einzelresultate des Prozentsatz-SLAs und dazu eine Zusammenfassung in der Form [.guihint]#100 Prozent# für das Ausfall-SLA -- natürlich nur in den Zeilen der Dateisystem-Services, die {CMK}-Zeilen bleiben leer.
...
Eine Service-spezifische Ansicht hat einen Nachteil:
...
In dem oben fertiggestellten Beispiel wurden die eingangs erwähnten drei Schritte abgearbeitet -- SLA erstellen, an Service binden, in Ansicht
einbauen.
...
// TK: Aktuell heisst der Eintrag (noch?) "Hosts/Services: SLA - <SLA_Name>", siehe CMK-9316.
...
image::sla_view_columnspecific.png[]
...
betrachten.
Ein Klick auf die Zelle mit den SLA-Daten bringt Sie direkt zur Detailseite der SLA-Ergebnisse des betroffenen Services.
...
// TK: Was ist der Unterschied zwischen den ersten beiden vertikalen Balken (Availability und SLA rawdata)?
...
Wenn Sie, wie im Beispiel beschrieben, die SLA-Levels gesetzt haben und das SLA noch vor dem Brechen auf {CRIT} geht, wird das hier über einen orangen und nicht über en sonst üblichen roten Balken angezeigt. 
Rot werden die Balken dann beim Brechen des SLA.
Sobald Sie den Mauszeiger auf den Ergebnisbalken ziehen, sehen Sie per Tooltip auch gleich die einzelnen Ereignisse, die für den Status verantwortlich sind.
Im folgenden Bild ist der Status etwa {WARN}, weil nur noch vier von fünf erlaubten Ausfällen übrig sind.
Auch die Meldung [.guihint]#SLA broken# würde in diesem Tooltip erscheinen.
...
// TK: Und wofür soll das gut sein? Ich kann nicht sehen, dass sich in den Daten irgendwas verändert.
// Im obigen Bild ist beispielsweise die aktuelle Periode ganz rechts hervorgehoben.
...
image::sla_view_details_spec.png[]
...
Und auch die SLAs stehen den Aggregaten (der obersten Ebene) zur Verfügung.
Über einen kleinen Umweg:
Der Status eines xref:bi#[BI-Aggregats] kann über den Regelsatz [.guihint]#Check State of BI Aggregation# als ganz normaler
Service überwacht werden.
Dieser erscheint dann beispielsweise als _Aggr MySLA_ in den Host-Ansichten und kann wiederum über die bereits xref:link_sla_service[oben] genutzte Regel [.guihint]#Assign SLA definition to service# mit einem SLA verknüpft werden.

Sie finden die Regel unter [.guihint]#Setup > Services > Other services > Check State of BI Aggregation#.
Die Regel ist darauf ausgelegt, BI-Aggregate auch auf entfernten {CMK}-Servern abzufragen.
Daher müssen Sie hier für die Verbindung die URL zum Server und einen xref:wato_user#automation[Automationsbenutzer] angeben -- und natürlich das gewünschte BI-Aggregat im Feld [.guihint]#Aggregation Name#:
Hier tragen Sie den Titel einer Top-Level-Regel aus Ihrem BI-Paket ein.

[{image-border}]
image::sla_view_bi_modern.png[]

Vorsicht, hier besteht *Verwechslungsgefahr:*
In der BI-Konfiguration erstellen Sie die eigentliche Aggregation, also die Logik, über Regeln -- und eine der obersten Regeln wird hier eben über ihren Titel als „Aggregation“ angegeben.
...
SLA selbst, Ansichts- und Service-Optionen, Zeitperioden, Regeln und natürlich Verfügbarkeitsdaten.
...
* xref:basics_downtimes#[Wartungszeiten]: [.guihint]#Setup > Hosts > Host monitoring rules > Recurring downtimes for hosts# und [.guihint]#Setup > Services > Service monitoring rules > Recurring downtimes for services# (nur {CEE})
...
Nachdem Sie die Konfigurationen geprüft haben, können Sie die Funktion des SLA über manuelle (gefälschte) Statusänderungen und Wartungszeiten
...
In solchen Fällen können Sie zur xref:slacolumn[Spalten-spezifische Anzeige] wechseln.
