...
xref:draft_devel_check_plugins#[Native {CMK}-Plugins]  bestehen immer aus zwei Komponenten: Ein xref:glossar#agent_plugin[Agentenplugin]  sammelt Rohdaten und filtert diese vor,  wertet sie aber nicht aus.
...
Lassen Sie ein Programm direkt eine Textdatei schreiben, welche das Format des Agenten-Outputs enthält.
