...
:description: Um {CMK} für das Monitoring einzurichten, muss der erste Host aufgenommen, seine Services konfiguriert und die Änderungen aktiviert werden. Um diese Themen geht es im aktuellen Kapitel des Leitfadens für {CMK} Einsteiger, der kompakt durch die ersten Schritte führt, um das IT-Monitoring mit {CMK} optimal einzurichten.
...
{cee-only}
In den {EE} gelangen Sie mit [.guihint]#Setup => Agents => Windows, Linux, Solaris, AIX#
...
{cre-only}
Die {RE} verfügt über keine Agentenbäckerei. Hier kommen Sie über
...
//SK: Wurde die Datei erst heruntergeladen, müsste sie per scp oder sonstwie wieder nach /root auf dem {CMK}-Server kopiert werden.
//SK: Wir gehen im Beispiel aber doch explizit davon aus, dass wir mit dem {CMK}-Server starten wollen. Warum zeigen wir dem Nutzer nicht einfach, wo der generische Agent liegt (/omd/sites/mysite/share/check_mk/agents)
...
//SK: Und wenn das hier doch so bleiben soll: Wo ist dieses Verzeichnis /root? Ja, in diesem Fall auf dem {CMK}-Server, aber wenn ich den in Schritt 5.1 erstmal auf meine Kiste runtergeladen habe, dann stehe ich hier im Regen.
//SK: Die alte Überschrift lautete hier "{CMK}-Agenten auf dem Zielhost installieren". Da hatte man wenigstens noch einen Anhaltspunkt.
...
//SK: Kommuniziert denn der "neu erstellte Host" mit dem Agenten? Kommuniziert nicht {CMK} mit dem Agenten auf dem Host?
...
//SK: Auch hier: Nimmt {CMK} "in weiteren Kästen" Kontakt auf? Oder erzählt mir {CMK} in diesen Kästen nur von der versuchten Kontaktaufnahme?
...
. In den Eigenschaften des Hosts, im Kasten [.guihint]#Data sources# aktivieren Sie  [.guihint]#{CMK} Agent# und wählen Sie [.guihint]#No agent# aus.
