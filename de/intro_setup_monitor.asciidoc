...
Da eine Ordnerstruktur nur in seltenen Fällen die Komplexität der Wirklichkeit abbilden kann, bietet {CMK} mit den Host-Merkmalen (_host tags_) eine weitere ergänzende Möglichkeit zur Strukturierung: doch dazu in einem xref:intro_finetune.html#host_tags[anderen Kapitel] mehr. Weiterführende Informationen unter anderem zur Ordnerstruktur finden Sie im xref:wato_hosts#[Artikel über Hosts].
...
image::empty_main_directory.png[alt="Ordneransicht 'Main directory' ohne Ordner und Hosts."]
...
Aktionen aus den Menüs als Knöpfe zum direkten Anklicken angeboten werden. 
...
Aktionen zum Erstellen eines Ordners auswählen (z.B. den Aktionsknopf icon:icon_newfolder[alt="Symbol zum Erstellen eines Ordners."] [.guihint]#Add subfolder#). Auf der neuen Seite
[.guihint]#Create new folder# tragen Sie im ersten Kasten [.guihint]#Basic settings# den Ordnernamen ein:

image::folder_basic_settings.png[alt="Dialog mit Eigenschaften beim Erstellen eines Ordners: die 'Basic settings' mit Titel."]

Im obigen Bild ist der xref:intro_gui#show_less_more[Show-less-Modus] aktiv und es wird nur derjenige Eintrag angezeigt, der zum Erstellen eines Ordners unbedingt notwendig ist. Bestätigen Sie die Eingabe mit [.guihint]#Save#.
...
image::three_empty_folders.png[alt="Ansicht 'Main directory' mit drei Ordnern, einer davon aufgeklappt mit Symbolen für Ordneraktionen."]
...
//TK: Ungünstige Kapitelstruktur, weil die 5 notwendigen Schritte wg. des Einschubs 5.4. "Diagnose" auf unterschiedlichen Ebenen liegen (5.1, 5.2, 5.3, 6 und 7)
...
xref:wato_monitoringagents#bakery[Agentenbäckerei] (_agent bakery_) bietet, mit
der Sie sich individuell konfigurierte Agentenpakete „backen“ können. Zusätzlich
...
{cre-only}
Die {RE} verfügt über keine Agentenbäckerei. Hier kommen Sie über [.guihint]#Setup > Agents > Linux# direkt zur Download-Seite, auf der Sie vorkonfigurierte Agenten und Agenten-Plugins finden:

image::agent_download_page.png[alt="Liste der Linux-Agenten zum Download in der {RE}."]
...
//TK2SK: Verstehe die Kritik, aber auch nur zum Teil: Habe die folgenden Abschnitte überarbeitet, um einige der Punkte zu berücksichtigen.
//TK2SK: Wer es bis hierhin geschafft hat, musste sich bereits durch die CMK-Linux-Installation kämpfen (ist zwar nicht im intro, aber verlinkt und wird vorausgesetzt) und da geht es ums gleiche Thema. Das ist also nicht neu für den Leser - und das Bsp. ist doch so gewählt, dass alle Aktionen auf dem gleichen Host ablaufen, von daher sollte scp gar nicht nötig sein.
//TK2SK: Download-Seite muss m.E. sein. Ich weiß nicht, ob man das Verzeichnis mit den generischen Agenten hier erwähnen soll (für mich interessant, ich kannte es auch nicht), aber das ist für den Leser vielleicht doch Information-Overkill. Da bin ich mir unsicher...
...
//TK2SK: Falls xinetd nachinstalliert werden muss: Muss der Agent danach neu installiert werden?
...
image::host_allsettings_less.png[alt="Dialog mit Eigenschaften beim Erstellen eines Hosts."]

Wie schon beim Anlegen der drei Ordner weiter oben ist immer noch der
xref:intro_gui#show_less_more[Show-less-Modus] aktiv. Daher zeigt {CMK} im Formular nur die wichtigsten und zum Erstellen eines Hosts notwendigen Host-Parameter an. Falls es Sie interessiert, können Sie sich den Rest ansehen, indem Sie bei jedem der geöffneten Kästen die drei Auslassungszeichen
icon:button_showmore[alt="Symbol zum Wechsel in den Show-more-Modus."] anklicken und die beiden zugeklappten Kästen am Ende der Seite öffnen. Wie am Anfang erwähnt, ist {CMK} ein komplexes System,
...
Am besten ist es, wenn der Host unter seinem Namen im DNS auflösbar ist. Dann sind Sie mit diesem Formular auch schon fertig. Falls nicht, oder falls Sie kein DNS verwenden möchten, können Sie die IP-Adresse aber auch von Hand im Feld [.guihint]#IPv4 Address# eintragen.

*Hinweis:* Damit {CMK} immer stabil und performant laufen kann, unterhält es einen eigenen Cache für die Auflösung der Hostnamen. Daher führt der Ausfall des DNS-Dienstes nicht zum Ausfall des Monitorings. Die Details zu Hostnamen, IP-Adressen und DNS finden Sie im xref:wato_hosts#dns[Artikel über die Verwaltung der Hosts].
...
Bereits bei der Erstellung eines Hosts bietet {CMK} an, nicht nur die Eingaben (Hostname und IP-Adresse) auf der Seite [.guihint]#Add host# zu sichern, sondern zusätzlich die Verbindung zum Host zu testen. In der Aktionsleiste der Seite [.guihint]#Add host# finden Sie unter anderem den Knopf [.guihint]#Save & go to connection tests#. Klicken Sie auf diesen.
...
image::host_diagnostics.png[alt="Ergebnis des Verbindungstests zum Host mit ping und mit Ausgabe des Agenten."]
...
image::intro_setup_monitor_services_of_host.png[alt="Liste der auf dem Host gefundenen Services zur Aufnahme ins Monitoring."]
...
Der Knopf [.guihint]#Fix all# macht alles auf einmal: fehlende Services hinzufügen und überflüssige entfernen.


[#activate_changes]
== Änderungen aktivieren

{CMK} speichert alle Änderungen, die Sie vornehmen, zunächst nur in einer vorläufigen „Konfigurationsumgebung“, die das aktuell laufende Monitoring noch nicht beeinflusst. Erst durch das „Aktivieren der aufgelaufenen Änderungen“ werden diese in das Monitoring übernommen. Mehr über die Hintergründe dazu erfahren Sie im xref:wato#[Artikel über die Konfiguration von {CMK}].
...
image::activate_changes.png[alt="Liste der aufgelaufenen Änderungen zur Aktivierung."]
...
xref:intro_setup_monitor#download_agent[Linux-Agenten] (in der {CRE} knapp daneben
...
. Erstellen Sie in {CMK} den Host für das SNMP-Gerät wie xref:intro_setup_monitor#create_host[oben beschrieben], diesmal im dafür vorgesehenen Ordner [.guihint]#Network#.
...
//TK2SK: Hier geht es doch um die Community-Einstellung der SNMP-Geräte. Falls die nicht "public" ist, muss ich das in CMK entsprechend eintragen. Da ist es doch egal, welche Default-Einstellung CMK hat. Oder meinst Du was anderes?
