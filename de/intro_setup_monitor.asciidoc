...
:description: Um Checkmk für das Monitoring einzurichten, muss der erste Host aufgenommen, seine Services konfiguriert und die Änderungen aktiviert werden. Um diese Themen geht es im aktuellen Kapitel des Leitfadens für Checkmk Einsteiger, der kompakt durch die ersten Schritte führt, um das IT-Monitoring mit Checkmk optimal einzurichten.
...
link:intro_gui.html[Was zuletzt geschah]
...
*Host*: Ein Host ist in {CMK} ein Server, eine virtuelle Maschine (VM), ein
...
{UNREACH}.
//SK: Das gefiel mir vorher mit "in der Regel" besser. Jetzt steht hier, dass jeder Host eine IP hat, außer wenn er keine hat.
...
nicht nur für Ihre eigene Übersicht nützlich, sie kann zusätzlich auch für die
Konfiguration von {CMK} verwendet werden: Alle Konfigurationsparameter von Hosts
können in einem Ordner definiert werden, die dann automatisch an die dort
enthaltenen Unterordner und Hosts *vererbt* werden.
//SK: Meiner Meinung, sollte hier deutlicher gemacht werden, dass Ordner für die Konfiguration großer Umgebungen elementar ist. Dieser Halbsatz mit "kann zusätzlich auch" untergräbt ein wenig, wie wichtig eine gute Ordnerstruktur VON BEGINN AN ist.
...
//SK: Ich verstehe, warum das Folgende hier steht. So richtig günstig finde ich es trotzdem nicht. Die Überschrift lautet hier "Ordner erstellen" und jetzt werden mir als erstes grundlegende Funktionen der GUI erklärt. Wenn ich das später nochmal Suche, werde ich schwer überlegen müssen, wo ich das gelesen habe.
//SK: Im Sinne der schrittweisen Einführung verstehe ich es allerdings.
...
machen.
//SK: Ich bin gar kein großer Fan von "Sinn machen". Ich plädiere immer für "Sinn ergeben".
...
//SK: Reminder für später. Bitte stehen lassen: Hier fehlt noch alt-text für die Knöpfe "Show suggested actions" und "Hide suggested actions"
...
Legen Sie diese drei Ordner nacheinander an, indem Sie eine der angebotenen
Aktionen zum Erstellen eines Ordners auswählen (z.B. den Aktionsknopf
icon:icon_newfolder[] [.guihint]#Add subfolder#) und auf der neuen Seite
[.guihint]#Create new folder# im ersten Kasten [.guihint]#Basic Settings# den jeweiligen Namen
eintragen:
//SK: Den vorherigen Satz musste ich dreimal lesen.
...
*Tipp:* Wenn Sie mit der Maus auf den oberen Bereich eines der drei
Ordner-Symbole zeigen, werden Ihnen Symbole angezeigt um schnell Aktionen mit
dem Ordner auszuführen (die Eigenschaften ändern, den Ordner verschieben oder
ihn löschen).
//SK: Tatsächlich wird hier ja die einzige Möglichkeit beschrieben einen Ordner zu löschen.
//SK: Ich erinnere noch gut, wie das zum ersten Mal machen wollte und fast in den Tisch gebissen hätte.
//SK: Vielleicht könnte man das aus diesem Tipp rausziehen und als Übung einen vierten Ordner erstellen, den man anschließend wieder löscht. Muss aber nicht...
...
//SK: Hab ich was verpasst? Machen wir die Klick-Pfade jetzt mit ">"?
{cee-only}
In den {EE} gelangen Sie mit [.guihint]#Setup# > [.guihint]#Agents# > [.guihint]#Windows, Linux, Solaris, AIX#
...
{cre-only}
Die {RE} verfügt über keine Agentenbäckerei. Hier gelanden Sie über
[.guihint]#Setup# > [.guihint]#Agents# > [.guihint]#Linux# direkt zur Download-Seite, auf der Sie
vorkonfigurierte Agenten und Agenten-Plugins finden. (Diese finden Sie in den
{EE} über [.guihint]#Setup# > [.guihint]#Agents# > [.guihint]#Other operating systems#.)
//SK: Das wird mir auch noch mal jemand erklären müssen, warum die Plugins in "Agents|Other operating systems" liegen...
...
//SK: Vorher erklären wir jeden Button und jetzt "nehmen wir an, dass die Datei in [...] /root" liegt. Das kann je nach Anweder schon zu grauen Haaren führen.
//SK: Wurde die Datei erst heruntergeladen, müsste sie per scp oder sonstwie wieder nach /root auf dem Checkmk-Server kopiert werden.
//SK: Wir gehen im Beispiel aber doch explizit davon aus, dass wir mit dem Checkmk-Server starten wollen. Warum zeigen wir dem Nutzer nicht einfach, wo der generische Agent liegt (/omd/sites/mysite/share/check_mk/agents)
//SK: und kopieren oder installieren von dort? Anschließend weisen wir darauf hin, dass er denn auf andere Hosts natürlich per geeignetem Werkzeug (scp oder was auch immer) kopieren muss.
//SK: Und wenn das hier doch so bleiben soll: Wo ist dieses Verzeichnis /root? Ja, in diesem Fall auf dem Checkmk-Server, aber wenn ich den in Schritt 5.1 erstmal auf meine Kiste runtergeladen habe, dann stehe ich hier im Regen.
//SK: Die alte Überschrift lautete hier "Checkmk-Agenten auf dem Zielhost installieren". Da hatte man wenigstens noch einen Anhaltspunkt.
...
//SK: Wenn wir schon sagen, dass die Verwendung von -U hier "am besten" ist, könnte wir vielleicht auch kurz sagen warum dem so ist.
//SK: Die Option `-U` steht bei rpm für Upgrade und sorgt dafür, dass die Installation auch dann fehlerlos durchläuft, wenn bereits eine alte Version den Agenten installiert ist.
...
//SK: In den beiden folgenden Code-Beispielen habe ich den Hostname 'linux' durch 'mycmkserver' ersetzt (analog zu dem ersten Screenshot in 5.3).
...
//SK: Statt weiter unten ins Thema DNS einzusteigen, wäre mir hier ein Hinweis zur Wichtigkeit guter und wohlüberlegter Hostnamen lieber.
//SK: Eine spätere Änderung von Hostnamen ist zwar für den User nicht kompliziert aber unsere Inlihe-Hilfe zu dem Thema spricht Bände.
//SK: Vielleicht wäre ein entsprechender Hinweis an dieser Stelle hier möglich.
//SK: Zitat der Inline-Hilfe: ¨Das Umbenennen von Hosts ist eine komplexe Operation, denn der Name des Hosts wird an vielen Stellen als eindeutiger Schlüssel verwendet. Deswegen muss auch das Monitoring dabei vorübergehend angehalten werden. Sie können keinen Host umbenennen, solange sie nicht-aktivierte Änderungen haben¨ 

Aber nun zu den Eingaben, um den ersten Host zu erstellen. Sie müssen nur ein
einziges Feld ausfüllen, nämlich [.guihint]#Hostname# bei den [.guihint]#Basic Settings#. Diesen
Namen können Sie frei vergeben. Er dient im Monitoring an allen Stellen als
interne ID (oder Schlüssel) und eindeutige Bezeichnung für den Host.

Falls der Host unter seinem Namen im DNS auflösbar ist, sind Sie mit diesem
Formular bereits fertig. Falls nicht, oder falls Sie kein DNS verwenden möchten,
können Sie die IP-Adresse aber auch von Hand im Feld [.guihint]#IPv4 Address# eintragen.

//SK: Ist der folgende Exkurs ins Thema DNS und DNS-Cache hier hilfreich?
...
Der DNS-Cache wird automatisch jeden Tag um 00:05 Uhr erneuert. Falls Sie den
Cache neu aufbauen wollen, damit eine Änderung in Ihrem DNS sofort wirksam wird,
bietet Ihnen {CMK} auch diese Möglichkeit, und zwar in den Eigenschaften eines
Hosts mit dem Knopf icon:icon_update[] [.guihint]#Update site DNS cache#. Genauere
Informationen dazu finden Sie im
link:wato_hosts.html#dns[Artikel über die Verwaltung der Hosts].

Um aber die Eigenschaften eines Hosts aufrufen zu können, muss der erste Host
zuerst einmal komplett erstellt werden -- und soweit sind wir noch nicht, auch
wenn wir kurz davor stehen.
...
//SK: Kommuniziert denn der "neu erstellte Host" mit dem Agenten? Kommuniziert nicht Checkmk mit dem Agenten auf dem Host?
Durch die Ausgabe im Kasten [.guihint]#Agent# erhalten Sie die Gewissheit, dass der neu
erstellte Host in {CMK} erfolgreich mit dem Agent kommunizieren kann, den Sie
zuvor auf dem Host manuell installiert hatten.

//SK: Auch hier: Nimmt Checkmk "in weiteren Kästen" Kontakt auf? Oder erzählt mir Checkmk in diesen Kästen nur von der versuchten Kontaktaufnahme?
In weiteren Kästen versucht {CMK} per SNMP Kontakt aufzunehmen. Das führt in
diesem Beispiel erwartbar zu SNMP-Fehlern, ist aber sehr nützlich für
Netzwerkgeräte, die wir weiter unten besprechen werden.
...
unter [.guihint]#Setup# > [.guihint]#Agents# > [.guihint]#Windows#). Sobald Sie das MSI-Paket
...
. Erstellen Sie in {CMK} den Host für das SNMP-Gerät wie link:intro_setup_monitor.html#create_host[oben beschrieben], diesmal im dafür vorgesehenen Ordner [.guihint]#Network#.
...
Benutzung sind. Dies können Sie später nach Belieben anpassen. Auch zeigt es
Ihnen in je einem Service, der immer {OK} ist, die allgemeinen Informationen zu
dem Gerät sowie seine Uptime an.
//SK: Dieser letzte Satz hat mich verwirrt, weil ich trotz der Verwendung von "in je einem Service" gedacht habe, dass Uptime und allgemeine Infos in einem Service stehen. Was keinen Sinn ergeben würde.
...
link:intro_tools.html[Hier geht es weiter]
