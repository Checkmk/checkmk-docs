...
doch dazu in einem xref:intro_finetune.html#host_tags[anderen Kapitel] mehr.
...
Legen Sie den ersten der drei Ordner an, indem Sie eine der angebotenen Aktionen zum Erstellen eines Ordners auswählen, z.B. den Aktionsknopf icon:icon_newfolder[alt="Symbol zum Erstellen eines Ordners."] [.guihint]#Add subfolder#.
...
Nach erfolgter Einrichtung des Hosts in der Konfigurationsumgebung wird die Aufnahme komplettiert durch die Konfiguration der Services und die Aktivierung der Änderungen.
...
Nachdem der Agent auf dem Host installiert ist, können Sie den Host in die xref:glossar#configuration_environment[Konfigurationsumgebung] von {CMK} aufnehmen
...
Schließen Sie also die Erstellung des Hosts in der Konfigurationsumgebung vorerst ab, indem Sie auf [.guihint]#Save & go to folder# klicken.
...
Detailliertere Informationen über den Linux-Agenten können Sie im xref:agent_windows#[Artikel über die Überwachung von Linux] nachlesen.
