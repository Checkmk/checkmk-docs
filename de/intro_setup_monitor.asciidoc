...
image::empty_main_directory.png[Screenshot des leeren Main directory]
...
Die Aktionsleiste können Sie mit dem Knopf icon:button_hide_toolbar[Icon mit Pfeil nach oben] rechts neben dem [.guihint]#Help#-Menü ausblenden und mit icon:button_show_toolbar[Icon zum Einblenden der Aktionsleiste] wieder einblenden. 
Bei ausgeblendeter Aktionsleiste werden die Symbole in der Menüleiste rechts neben icon:button_show_toolbar[Icon mit Pfeil nach unten, zum Einblenden der Aktionsleiste] angezeigt.
...
image::folder_basic_settings.png[Basic settings des Ordners]
...
image::three_empty_folders.png[Screenshot mit drei Ordnern im Main directory, einer davon aufgeklappt mit hervorgehobenen Icons]
...
image::agent_download_cee.png[Seite zum Download des Linux-Agenten in den {EE} mit hervorgehobenen Icons]
...
image::agent_download_page.png[Download des Linux-Agenten in der {RE}]
...
image::host_allsettings_less.png[Eigenschaften beim Erstellen eines Hosts]
...
image::host_diagnostics.png[Verbindungstest zum Host per ping und mit Ausgabe des Agenten]
...
image::intro_setup_monitor_services_of_host.png[Auf dem Host gefundene Services]
...
image::host_snmp_configuration.png[Eigenschaften beim Erstellen eines Hosts per SNMP]
...
image::snmp_diagnostics.png[Test der Verbindung zum Host per SNMP]
