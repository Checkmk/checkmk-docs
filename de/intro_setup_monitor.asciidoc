...
.Die Download-Seite der {RE} für die Linux-Pakete
...
Für das Kommando benötigen Sie die Namen des {CMK}-Servers (im Beispiel `mycmkserver`), der {CMK}-Instanz (`mysite`) und des Hosts (`localhost`), wie er xref:create_host[soeben] in {CMK} eingerichtet wurde.
...
fehlende Services hinzufügen, verschwundene entfernen -- und zusätzlich gefundene Änderungen an Host-xref:glossar#label[Labels] übernehmen.
...
Gerade wenn Sie komplexere Modifikationen an Ihrem System vornehmen, kann es passieren, dass Sie doch im Laufe eines Bearbeitungsprozesses Änderungen zurücknehmen wollen.

Nutzen Sie hierfür den Menüeintrag [.guihint]#Changes > Revert changes#

image::intro_setup_monitor_activate_pending.png[alt="Auswahl der Funktion 'Revert pending changes'.", width=41%]

Damit können Sie alle seit dem letzten [.guihint]#Activate on selected sites# aufgelaufenen Änderungen verwerfen:

image::intro_setup_monitor_revert.png[alt="Rückgängig machen bestätigen."]
