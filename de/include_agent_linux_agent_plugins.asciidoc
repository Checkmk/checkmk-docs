...
[#agent_plugins]
== [[plugins]]Agent um Plugins erweitern

=== Was sind Agentenplugins?

Das Agentenskript `/usr/bin/check_mk_agent` enthält eine ganze Reihe von Sektionen, welche Überwachungsdaten für diverse Checks liefern, die dann von der Service-Erkennung automatisch gefunden werden.
...
Bevor Sie ein Plugin im Agenten installieren, werfen Sie einen Blick in die entsprechende Datei.
...
Sobald das Plugin ausführbar und im richtigen Verzeichnis ist, wird es vom Agenten aufgerufen und es entsteht eine neue Sektion in der Agentenausgabe.
Diese trägt üblicherweise den gleichen Namen wie das Plugin.
Komplexe Plugins (z.B. `mk_oracle`) erzeugen sogar eine ganze Reihe von Sektionen.
