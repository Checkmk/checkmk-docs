...
// TK: Da Du im nächsten Abschnitt einen Service als Beispiel nimmst, wäre hier ein Screenshot für die Service-Regel besser - und mit dem Wert 60.00, den Du unten verwendest.
...
// TK: Hm, wer kriegt die Nummer 1? Die 1. Benachrichtigung oder die 1. der weiteren Benachrichtigungen? Naja, ist auch nicht so wichtig...
...
// TK: Eskalation statt Eskalierung, auch im übernächsten Satz.
// TK: statt "eine zusätzliche Benachrichtigungsregel" "zusätzlich eine Benachrichtigungsregel", denn die Regel aus dem vorherigen Abschnitt ist eine "normale" Regel und keine Benachrichtigungsregel.
Tragen Sie hier als Bereich für den Zähler 3 bis 99999 ein, so greift diese Regel ab der dritten Benachrichtigung.
Diese Eskalierung kann dann entweder durch die Wahl einer anderen Methode (z.B. SMS) erfolgen oder durch die Benachrichtigung von anderen Personen (Kontaktauswahl).

image::notifications_escalation.png[alt="Regel zur Einstellung der Häufigkeit von Benachrichtigungen."]
// TK: Auch hier fände ich es schöner, die gezeigten Werte passend zum Text abzubilden.
// TK: Du könntest hier die Werte für die Eskalationsstufe 3 (Firmenleitung) eintragen: 11 to 999999, 11, 24 (wenn ichs recht verstehe...) und den Text vor und nach dem Screenshot entsprechend anpassen.

Mit der Option [.guihint]#Throttle periodic notifications# können Sie die Rate der wiederholten Benachrichtigungen nach einer bestimmten Zeit reduzieren. So können Sie z.B. am ersten Tag jede Stunde eine E-Mail senden lassen und später das Ganze auf einmal am Tag beschränken.
// TK: also hier: So können Sie z.B. zunächst jede Stunde eine E-Mail senden lassen ...
...
Hierzu ein Beispiel:
// TK: Hierzu ein Beispiel umgesetzt mit einer Regel für periodisch wiederholte Benachrichtigungen und drei Benachrichtigungsregeln:
...
// TK: Hier kann man dann hinzufügen, dass die Werte für diese Eskalationsstufe im vorherigen Bild zu sehen sind.
...
Ab der sechsten Alarmierung um 14 Uhr bekommt nun zusätzlich auch der Teamleiter eine Benachrichtigung -- aber auch dieser kann keinen der Mitarbeiter zur Problemlösung bewegen.
// TK: Alarmierung steht auf dem Index: Die sechste Benachrichtigung um 14 Uhr bekommt nun zusätzlich auch der Teamleiter...
So werden um 15, 16, 17 und 18 Uhr weiter E-Mails an die Mitarbeiter und an den Teamleiter verschickt.
...
// TK: Für "Entwarnung" diese Anführungszeichen verwenden: „Entwarnung“ (einfügen mit ALT Gr+V/ALT Gr+B)
...
// TK: Im vorherigen Satz fehlt das Verb, also besser per Doppelpunkt andocken.
...
{cee-only}
In den {CEE} finden Sie in der Log-Datei `~/var/log/cmc.log` ein Protokoll des Monitoring-Kerns.
