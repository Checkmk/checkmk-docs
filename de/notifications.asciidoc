...
//SK: Hier könnte meines Erachtens auch in Großbuchstaben und Schriftgröße 40 stehen: "VERY IMPORTANT - TAKE YOUR TIME TO GET THIS RIGHT"
...
// SK: Diese Änderung wurde 2016 vorgenommen und betrifft somit nur Nutzer die mit oder vor der 1.2.6 angefangen haben. Ich würde es auch rausnehmen. MA ist dagegen.
...
*Wichtig:* Anders als bei den xref:wato_rules#[Regeln für Host- und Services]
...
Bedingungen legen fest, wann eine Regel Anwendung findet. Ohne die unten gesondert besprochene Ausnahme, gibt es hierbei zwei wichtige Dinge zu beachten:

. Solange keine Bedingung definiert ist, greift die Regel bei *jedem* Monitoring-Ereignis.
. Sobald Sie auch nur eine einzige Bedingung auswählen, greift die Regel nur, wenn auch wirklich *alle* Bedingungen erfüllt.

Der folgende Auszug aus der kontextsensitiven Hilfe zu der Option [.guihint]#Match services# verdeutlich das Verhalten sehr gut:

> Anmerkung: Auf Host-Benachrichtigungen trifft diese Regel nie zu, wenn diese Option benutzt wird.

Die Bedingungen, die Sie im folgenden Screenshot sehen, können also niemals zu einer Benachrichtigung führen, die Ihnen mitteilt, dass ein Host in den Status {DOWN} gewechselt ist.

image::notifications_rule_part4.png[]

Den folgenden Hinweis geben wir im Handbuch immer wieder mal. Im Zusammenhang mit der Konfiguration Ihrer Benachrichtigungen, ist er allerdings nochmal besonders hervorzuheben:

*Hinweis:* Blenden Sie die kontextsensitive Hilfe mit [.guihint]#Help > Show inline help# ein, um Einzelheiten über die Auswirkung der verschiedenen Bedingungen zu erfahren.

//SK: Stimmt es, dass sich die "Bedingungen mit den statischen Eigenschaften" befassen? Gerade das Beispiel in der Klammer direkt hinter "aktuellen Zustand" hinkt doch stark. Ich kann schließlich nicht nur dann eine Benachrichtigung schicken, sobald ein Service Crit ist. Ich kann auch ganz explizit nur dann Benachrichtigen, wenn ein Service con CRIT nach UNKNOWN geht. Die Übergänge (Transitions) werden hier unterschlagen.
...
// Wenn ein Monitoring-Ereignis auch nur eine der konfigurierten Bedingungen nicht erfüllt, kommt die Regel nicht zur Anwendung. Eine Besonderheit dabei sind die Bedingungen [.guihint]#Match host event type# und [.guihint]#Match service event type#:

==== Die Ausnahme

Die beiden Bedingungen [.guihint]#Match host event type# und [.guihint]#Match service event type# bilden zu dem vorherigen eine wichtige Ausnahme:
...
//SK: Sobald wir andernorts mal beschreiben, was diese "Service-Level" eigentlich sind, könnten wir das hier mal verlinken.
...
// SK: Bezieht sich das cee-only hier nicht auf die "nachvollziehbaren Benachrichtigung"?
{cee-only}
...
// SK: Kann es sein, dass es hier um die Failed Notifications im Overview Snap-In geht?
