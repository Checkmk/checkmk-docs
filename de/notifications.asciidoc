...
Sie können {CMK} so einrichten, dass es in einem festen Intervall immer weitere Benachrichtigungen versendet, solange bis das Problem entweder icon:icon_ack[alt="Ausrufezeichen als Symbol der Quittierung."] xref:basics_ackn#[quittiert] oder behoben wurde.
...
Dazu bietet {CMK} auf der Seite [.guihint]#Notification configuration# eine eingebaute Analysefunktion, welche Sie mit dem Menüeintrag [.guihint]#Display > Show analysis# erreichen.
...
|*Kürzel* |*Host Event Type* |*Auslöser der Regel*
|rd |UP ➤ DOWN |Hoststatus wechselte von {UP} auf {DOWN}.
|ru |UP ➤ UNREACHABLE |Hoststatus wechselte von {UP} auf {UNREACH}.
|dr |DOWN ➤ UP |Hoststatus wechselte von {DOWN} auf {UP}.
|du |DOWN ➤ UNREACHABLE |Hoststatus wechselte von {DOWN} auf {UNREACH}.
|ud |UNREACHABLE ➤ DOWN |Hoststatus wechselte von {UNREACH} auf {DOWN}.
|ur |UNREACHABLE ➤ UP |Hoststatus wechselte von {UNREACH} auf {UP}.
|?r |any ➤ UP |Hoststatus wechselte von einem beliebigen Status auf {UP}.
|?d |any ➤ DOWN |Hoststatus wechselte von einem beliebigen Status auf {DOWN}.
|?u |any ➤ UNREACHABLE |Hoststatus wechselte von einem beliebigen Status auf {UNREACH}.
...
|x |Acknowledgement of problem |Ein Problem wurde quittiert.
|as |Alert handler execution, successful |Eine Alarmierungsausnahme wurde erfolgreich durchgeführt.
|af |Alert handler execution, failed |Eine Alarmierungsausnahme scheiterte.
|===

[cols="10,31,~", options="header"]
|===
3+|Service-Event Typen
|*Kürzel* |*Service Event Type* |*Auslöser der Regel*
|rw |OK ➤ WARN |Servicestatus wechselte von {OK} auf {WARN}.
|rr |OK ➤ OK |Servicestatus wechselte von {OK} auf {OK} .
|rc |OK ➤ CRIT |Servicestatus wechselte von {OK} auf {CRIT}.
|ru |OK ➤ UNKNOWN |Servicestatus wechselte von {OK} auf {UNKNOWN}.
|wr |WARN ➤ OK |Servicestatus wechselte von {WARN} auf {OK}.
|wc |WARN ➤ CRIT |Servicestatus wechselte von {WARN} auf {CRIT}.
|wu |WARN ➤ UNKNOWN |Servicestatus wechselte von {WARN} auf {UNKNOWN}.
|cr |CRIT ➤ OK |Servicestatus wechselte von {CRIT} auf {OK}.
|cw |CRIT ➤ WARN |Servicestatus wechselte von {CRIT} auf {WARN}.
|cu |CRIT ➤ UNKNOWN |Servicestatus wechselte von {CRIT} auf {UNKNOWN}.
|ur |UNKNOWN ➤ OK |Servicestatus wechselte von {UNKNOWN} auf {OK}.
|uw |UNKNOWN ➤ WARN |Servicestatus wechselte von {UNKNOWN} auf {WARN}.
|uc |UNKNOWN ➤ CRIT |Servicestatus wechselte von {UNKNOWN} auf {CRIT}.
|?r |any ➤ OK |Service wechselte von einem beliebigen Status auf {OK}.
|?w |any ➤ WARN |Service wechselte von einem beliebigen Status auf {WARN}. 
|?c |any ➤ CRIT |Service wechselte von einem beliebigen Status auf {CRIT}. 
|?u |any ➤ UNKNOWN |Service wechselte von einem beliebigen Status auf {UNKNOWN}. 
|===
