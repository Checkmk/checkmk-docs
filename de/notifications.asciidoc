...
// TK: Das passt im wesentlichen zu den Typen, wie sie im Kap. "Umgebungsvariablen" unter NOTIFY_NOTIFICATIONTYPE beschrieben sind. Allerdings gibt es dort keinen Eintrag für die Event Console.
...
* Ein durch ein icon:icon_commands[] xref:commands#[Kommando] manuell ausgelöste Benachrichtigung
* Die Ausführung eines icon:icon_alert_handlers[] xref:alert_handlers#[Alert Handlers]
* Ein Ereignis, das von der icon:icon_event_console[] xref:ec#[Event Console] zur Benachrichtigung übergeben wurde

{CMK} verfügt über ein regelbasiertes System zur Konfiguration der Benachrichtigungen, mit dem Sie auch sehr anspruchsvolle Anforderungen umsetzen können. 
...
Dazu müssen Sie erstmal die Schwellwerte und alle anderen Einstellungen soweit im Griff haben, dass im Normalfall alle Zustände {OK} / {UP} sind oder mit anderen Worten: alles „grün“ ist.
...
Je nach Ihrer Linux-Distribution kann es sich dabei z.B. um Postfix, Qmail, Exim, Sendmail oder Nullmailer handeln. 
Die Konfiguration erfolgt mit den Mitteln Ihrer Linux-Distribution.

In der Regel beschränkt sich die Konfiguration auf das Eintragen eines „Smarthosts“ (auch SMTP-Relay-Server genannt), an den alle E-Mails
...
Bei der {CMK}-Appliance können Sie den Smarthost bequem über die xref:appliance_usage#cma_webconf_system_settings[Weboberfläche]
konfigurieren.

Sie können das Versenden von E-Mails auf der Kommandozeile mit dem Befehl `mail` testen. 
Da es für diesen Befehl unter Linux zahlreiche unterschiedliche Implementierungen gibt, liefert {CMK} zur
Vereinheitlichung die Variante aus dem Projekt link:http://heirloom.sourceforge.net/mailx.html[Heirloom mailx] mit aus -- direkt im Suchpfad des Instanzbenutzers als `~/bin/mail`. 
...
Dies geht mit dem xref:commands#[Kommando] [.guihint]#Fake check results,# das Sie in der Service-Liste im Menü [.guihint]#Commands# finden:
...
Allerdings ist der erzeugte Benachrichtigung dann von einem anderen Typ und kann sich -- abhängig von Ihren Benachrichtigungsregeln -- anders verhalten.
...
Das ist sicher erstmal sinnvoll, aber in der Praxis tauchen viele weitergehende Anforderungen auf, z.B.:
...
*Hinweis:* Wenn Sie die Seite [.guihint]#Notification configuration# das erste Mal aufrufen, wird Ihnen eine Warnung zur nicht konfigurierten „fallback email address" angezeigt. Die Warnung können Sie im Moment ignorieren. Wir gehen darauf xref:fallback[weiter unten] ein.
...
*Wichtig:* Anders als bei den xref:wato_rules#[Regeln für Host- und Services] geht die Auswertung hier auch nach einer zutreffenden Regel weiter! 
Die nachfolgenden Regeln können weitere Benachrichtigungen hinzufügen. 
Auch können Sie Benachrichtigungen wieder xref:cancel[löschen], welche vorherige Regeln generiert haben. 
...
Sie können aber recht einfach xref:scripts[eigene Skripte] in beliebigen Programmiersprachen schreiben, um speziellere Anforderungen umzusetzen, z.B. die Weiterleitung der Benachrichtigungen an ein eigenes Ticketsystem.
...
Dann werden Benachrichtigungen dieser Methode aus früheren Regeln wieder verworfen.
...
Der häufigste Fall bei der Festlegung der Kontakte ist, alle Benutzer zu benachrichtigen, die als xref:wato_user#contacts[Kontakt] für den jeweiligen Host/Service eingetragen sind. 
...
Die beiden Optionen mit [.guihint]#Restrict by ...# arbeiten etwas anders. 
...
Die Bedingungen befassen sich dabei mit den statischen Eigenschaften des Objekts (z.B. ob der Service-Name den Text `/tmp` enthält), mit
dem aktuellen Zustand (z.B. ob der Service gerade von {OK} nach {CRIT} gewechselt hat) oder mit ganz anderen Dingen (z.B. ob die xref:timeperiods#[Zeitperiode] „Arbeitszeit“ gerade aktiv ist).

Wenn ein Monitoring-Ereignis auch nur eine der konfigurierten Bedingungen nicht erfüllt, kommt die Regel nicht zur Anwendung. 
Eine Besonderheit dabei sind die Bedingungen [.guihint]#Match host event type# und [.guihint]#Match service event type#:
...
Falls Sie aber *beide* Bedingungen aktivieren, matcht die Regel, falls der Ereignistyp in einer der beiden Checkboxlisten aktiviert ist. 
...
eine bestimmte Kontaktzuordnung hat. Damit kann man Dinge umsetzen wie „Host-Benachrichtigungen in der Kontaktgruppe Linux sollen nie per SMS versendet werden“.
...
An die werden Benachrichtigungen verschickt, auf die keine einzige Benachrichtigungsregel greift.

Die Fallback-Adresse wird nur dann verwendet, wenn _keine Regel_ greift, nicht wenn kein Benachrichtigung erzeugt würde! 
Wie wir im vorheigen Kapitel gezeigt haben, ist das explizite Löschen von Benachrichtigungen ja erwünscht und kein Konfigurationsfehler.
...
Aber damit stellen Sie sicher, dass immer eine Regel greift und lassen die Warnung verschwinden.
...
Da Sie als Administrator einfachen Zugriff auf die persönlichen Einstellungen der Benutzer haben, können Sie das Abschalten stellvertretend für den Benutzer machen -- auch wenn diesem oben genannte Berechtigung fehlt. 
...
// TK: Der alte Titel "Wann genau Alarme erzeugt werden" ist nicht korrekt, da es in vielen Unterkapiteln z.B. darum geht, wie man Benachrichtigungen abschaltet oder umkonfiguriert
// TK: Andererseits fehlen hier in der Einleitung genannte Ereignisse die Benachrichtigungen erzeugen: Quittierung, Kommando, Alert Handler, Event Console
...
Während sich ein Host oder Service in einer xref:basics_downtimes#[Wartungszeit] befindet, sind für dieses Objekt die Benachrichtigungen
unterdrückt. D
as ist -- neben einer korrekten Berechnung von Verfügbarkeiten -- der wichtigste Grund, warum man überhaupt eine Wartungszeit im Monitoring
...
* Endet die Wartungszeit eines Objekts, das _während_ der Wartungszeit in einen Problemzustand gewechselt hat, dann wird dieses Problem exakt beim Ablauf der Wartung _nachträglich_ benachrichtigt.
...
Ein Objekt, welches sich gerade *nicht* in seiner Benachrichtigungsperiode befindet, wird durch ein graues Pause-Zeichen icon:icon_outofnot[] markiert.
...
Dabei wird nur der jeweils letzte Zustand benachrichtigt, auch wenn es außerhalb der Periode mehrere Zustandswechsel gab.
...
Natürlich wäre es sehr lästig, wenn alle Probleme von aktiven Checks in so einem Zustand benachrichtigt würden. 
Denn wenn z.B. ein Webserver nicht erreichbar ist -- und dies auch schon benachrichtigt wurde -- wäre es wenig informativ, wenn nun auch für jeden einzelnen HTTP-Dienst, den dieser bereit stellt, eine E-Mail generiert würde.
...
{cre-only}
Wenn Sie die {RE} verwenden (oder eine der {EE} mit Nagios als Kern), dann kann es in seltenen Fällen bei einem Host-Problem trotzdem zu einer Benachrichtigung eines aktiven Services kommen. 
...
Sobald alle Parents auf {DOWN} gehen, werden die nicht mehr erreichbaren Hosts auf den Zustand {UNREACH} gesetzt und die Benachrichtigungen
für diese wird unterdrückt. 
Das Problem mit dem Router selbst wird hingegen durchaus benachrichtigt.
...
Über die Regelsätze [.guihint]#Enable/disable notifications for hosts# bzw. [.guihint]#Enable/disable notifications for services# können Sie Hosts und Services bestimmen, für die grundsätzliche keine Benachrichtigungen erzeugt werden sollen. 
...
Es ist grundsätzlich möglich, bei einzelnen Hosts und Services die Benachrichtigungen auch per xref:commands.html[Kommando] vorübergehend abzuschalten.
...
Mit zugewiesener Berechtigung können Sie Benachrichtigungen von Hosts und Services mit dem Kommando [.guihint]#Commands > Notifications# abschalten (und später auch wieder einschalten):
...
Sie legen hier einen Zeitraum fest. 
Eine Benachrichtigung wird dann solange zurückgehalten, bis diese Zeit abgelaufen ist. 
...
Bei Systemen mit einem hohen Service-Level kann es sinnvoll sein, es nicht bei einer einzelnen Benachrichtigung zu belassen, falls das Problem über einen längeren Zeitraum weiterhin besteht. 
Sie können {CMK} so einrichten, dass es in einem festen Intervall immer weitere Benachrichtigungen versendet, solange bis das Problem entweder icon:icon_ack[] xref:basics_ackn#[quittiert] oder behoben wurde.
...
// TK: Hier könnte Ihre Grafik stehen :-) Was da genau gezeigt werden soll, wäre noch zu besprechen...
...
Wenn alle xref:conditions[Bedingungen] erfüllt sind und es grünes Licht für eine Benachrichtigung gibt, erzeugt der Kern eine
...
Neben den Daten aus der Rohbenachrichtigung enthält jeder Benutzerbenachrichtigung folgende zusätzliche Informationen:
...
[cols="10,~"]
|===
|icon:icon_analyze[] |Test der Regelkette, in dem für jede Regel geprüft wird, ob das Monitoring-Ereignis alle Bedingungen der Regel erfüllen würde. Im Anschluss an die Regeln wird dann die daraus resultierende Benachrichtigungstabelle angezeigt.
|icon:icon_toggle_context[] |Anzeige des kompletten Benachrichtigungskontexts.
|icon:icon_reload_cmk[] |Wiederholung der Rohbenachrichtigung, als wäre das Monitoring-Ereignis jetzt aufgetreten. Ansonsten ist die Anzeige gleich wie bei der Analyse. Damit können Sie nicht nur die Bedingungen der Regel überprüfen, sondern auch testen, wie eine Benachrichtigung dann aussieht.
|===
...
{cee-only}
Eine mächtige Zusatzfunktion der {EE} ist der Benachrichtigungs-Spooler._
...
Bei Verwendung der xref:syncsmtp[nachvollziehbaren Zustellung per SMTP] oder anderer Skripte, welche Netzwerkverbindungen aufbauen, sollten Sie *auf jeden Fall* asynchrone Zustellung einstellen. 
...
isoliertes Problem eine ganze Flut von (Folge-)Benachrichtigungen losgetreten hat.
...
Fall nur eine einzige E-Mail versendet wird, welche alle 43 Benachrichtigungen nacheinander
...
Die Funktionsweise dieser _Sammelbenachrichtigung_ ist sehr einfach. Wenn eine Benachrichtigung
auftritt, so wird dieser zunächst eine kurze Zeit lang zurückgehalten.
Weitere Benachrichtigungen, die während dieser Zeit kommen, werden dann gleich mit
...
aber mit einer Sammelbenachrichtigung. Wird in einer Regel die Sammelbenachrichtigung
...
benachrichtigt werden sollen. Üblicherweise wird die Option [.guihint]#Host# gewählt,
die dafür sorgt, dass nur Benachrichtigungen vom gleichen Host zusammengefasst werden.
...
* Sie können die Größe des Topfs limitieren ([.guihint]#Maiximum bulk size#). Bei Erreichen dieses Maximums wird die Sammelbenachrichtigung sofort verschickt.


=== Sammelbenachrichtigungen und Zeitperioden

Was ist eigentlich, wenn eine Benachrichtigung innerhalb der Benachrichtigungsperiode liegt,
die Sammelbenachrichtigung, die ihn enthält -- die ja etwas später kommt -- dann aber schon außerhalb
liegt? Und auch der umgekehrte Fall ist ja möglich ...

Hier gilt ein ganz einfaches Prinzip: Alle Konfigurationen, die Benachrichtigungen auf
Zeitperioden eingrenzen, gelten immer nur *für die eigentliche Benachrichtigung*. Die
später folgende Sammelbenachrichtigung wird immer *unabhängig* von sämtlichen
...
Dazu gehört, dass Benachrichtigungen _zuverlässig_ und _zeitnah_ ankommen. Nun
...
der Regel frühestens in 15-30 Minuten. Aber dann kann die Benachrichtigung
...
Fehler dann einfach unter und Benachrichtigungen bleiben aus.
...
Doch was tun bei gescheiterten Benachrichtigungen? Diese wiederum per E-Mail zu benachrichtigen
...
dazu führen kann, dass das Benachrichtigungsskript sehr lange läuft und am
Ende in einen Timeout gerät. Deswegen ist es unbedingt ratsam, dass Sie den
Benachrichtigungs-Spooler verwenden und eine xref:async[asynchrone Zustellung] von Benachrichtigungen einstellen.
...
pro Benachrichtigungsmethode einstellen:
...
parallel laufen und so gleichzeitig mehrere Benachrichtigungen versenden darf.
...
Benachrichtigungsskript einen Fehlerstatus zurückgeben können, erfahren Sie im Kapitel über 
...
In verteilten Umgebungen -- also solchen mit mehr als einer {CMK}-Instanz -- stellt sich die Frage, was mit Benachrichtigungen geschehen soll, die auf Remote-Instanzen erzeugt werden. Hier gibt es grundsätzlich zwei Möglichkeiten:
...
Sie selbst eigene Benachrichtigungsskripte schreiben können. Sie können diese
...
automatisch gefunden und bei den Benachrichtigungsregeln als Methode zur Auswahl angeboten.
...
Im Falle einer Benachrichtigung wird Ihr Skript mit den Rechten des Instanzbenutzers
...
das Monitoring-Ereignis, den zu Kontakt und die Parameter, die
in der Benachrichtigungsregel angegeben wurden.

Texte, die das Skript in die *Standardausgabe* schreibt (mit `print`,
`echo`, etc.), erscheinen in der Logdatei des Benachrichtigungsmoduls `var/log/notify.log`.


=== Nachvollziehbare Benachrichtigungen

{cee-only}
Benachrichtigungsskripte haben die Möglichkeit, über den Exit Code mitzuteilen,
...
Benachrichtigungsskripts zusammen mit dem Status in die Benachrichtigungshistorie des
Hosts/Services eingetragen und ist somit über die GUI sichtbar.

Die Behandlung von Benachrichtigungsfehlern aus Sicht des Benutzers wird im
...
Damit das Skript verwendet wird, müssen Sie es in einer Benachrichtigungsregel
...
aller {CMK}-Umgebungsvariablen, die Informationen über die Benachrichtigung
...
der Art der Benachrichtigung und auch von der verwendeten {CMK}-Version und -Edition
...
Wenn Ihr Skript xref:bulk[Sammelbenachrichtigungen] unterstützen soll,
müssen Sie es speziell dafür präparieren, da hier dem Skript _mehrere
Benachrichtigungen auf einmal_ übergeben werden. Aus diesem Grund funktioniert dann
...
dann sendet das Benachrichtigungsmodul die Benachrichtigungen auf der _Standardeingabe:_
...
die Daten gesendet werden. Dazu können Sie z.B. das folgende Benachrichtigungsskript nutzen:
...
Im Auslieferungszustand stellt {CMK} bereits eine ganze Reihe Skripte bereit zur Anbindung an beliebte und weit verbreitete Instant-Messaging-Dienste, Incident-Management-Plattformen und Ticketsysteme. Wie Sie diese Skripte nutzen können, erfahren Sie in den folgenden Artikeln:
...
|`var/log/mknotifyd.state` |Aktueller Zustand des Benachrichtigungs-Spoolers. Das ist hauptsächlich bei einer xref:distributed_monitoring#notifications[Benachrichtigungen in verteilten Umgebungen] relevant.
