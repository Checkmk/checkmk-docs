// -*- coding: utf-8 -*-
// IGNORE % Benachrichtigungs → ➤ Pre diktengleiche
...
Linux-Distribution kann es sich dabei z.B. um Postfix, Qmail, Exim 
...
. In die jeweilige Kontaktgruppe darf nur ein einziger Benutzer eingetragen werden, ansonsten werden pro Ereignis mehrere Benachrichtigungen an das Zielsystem geschickt.

. Wenn der erste Punkt erfüllt ist, dieser Benutzer (z.B. cmkadmin) aber in mehreren Benachrichtigungsregeln verwendet wird, die vom selben Typ sind (z.B. ServiceNow), dann greift jeweils nur die letzte Regel. Es empfiehlt sich also, für jede dieser Benachrichtigungsregeln einen eigenen funktionalen Benutzer anzulegen.
