...
[.relateddoc]
basics_ackn, commands, wato_user
...
* Die ICON[icon_ack.png] link:basics_ackn.html[Bestätigung eines Problems] durch einen Benutzer ([.guihint]#Acknowledgement#)
* Eine durch ein ICON[icon_commands.png] link:commands.html[Kommando] von Hand ausgelöste Alarmierung
* Die Ausführung eines ICON[icon_alert_handlers.png] link:alert_handlers.html[Alerthandlers]
* Ein Ereignis, das von der ICON[icon_mkeventd.png] link:ec.html[Event Console] zur Alarmierung übergeben wurde
...
[.guihint]#Maximum number of check attempts,}} verwenden Sie {{Delay ... notifications#
...
== Einfache Alarmierung per Email#simple_mail
...
=== Einrichten des Mailversands unter Linux#smtp
...
=== Test#notification_testing

Um die Alarmierung zu testen, können Sie einen {OK}-Service einfach von Hand auf
{CRIT} setzen. Dies geht mit dem ICON[icon_commands.png] link:commands.html[Kommando] [.guihint]#Fake check results.#
...
== Alarmierung per Regeln steuern#rules
...
WATO-Modul ICON[icon_notifications.png] [.guihint]#Notifications# verwalten Sie
...
image::wato_sidebar_notifications.png[center, width=42%]
...
setzen können: Bei den [.guihint]#Host- &amp; Service&shy;parameters}} finden Sie unter {{Monitoring Configuration|Notifications#
...
sind die Bedingungen [.guihint]#Match host event type}} und {{Match service event type#:
...
Ein Hinweis noch zu den Bedingungen [.guihint]#Match contacts}} und {{Match contact groups#:
...
[.guihint]#Cancel previous notifications#.  Die Kontakt&shy;auswahl selektiert die
...
=== Was ist, wenn keine Regel greift?#fallback
...
== Benutzerdefinierte Alarmierung#personal
...
== Wann genau Alarme erzeugt werden#conditions
...
image::icon_downtime.png[left, width=50]
...
image::icon_outofnot.png[left, width=50]
...
werden mit der Zeit ICON[icon_stale.png] [.guihint]#stale#.
...
[CEE] Der link:cmc.html[CMC] verhält sich intern übrigens geringfügig anders
...
image::icon_notif_man_disabled.png[left, width=50]
...
image::notifications_disabled.png[center, width=240]
...
[.guihint]#Monitoring configuration|Delay host notifications}} und {{Delay service notifications.#
...
image::icon_flapping.png[left, width=50]
...
* Die [.guihint]#Master control}} hat einen Hauptschalter für die Erkennung von Unstetigkeiten {{(Flap Detection).#
* Über die Regelsätze [.guihint]#Monitoring configuration|Enable/disable flapping detection for hosts}} bzw. {{... services# können Sie Objekte von der Erkennung ausklammern.
...
|CMC
|Der link:cmc.html[{CMK} Micro Core] ist der Kern der {EE} und erfüllt die gleiche Aufgabe wie Nagios in der {CRE}.
|`var/log/cmc.log` 

|Alarmierungsmodul
|Das Alarmierungsmodul wertet die Alarmierungsregeln aus, um aus einem Rohalarm fertige Alarme zu erzeugen. Es ruft die Alarmierungsskripten auf.
|`var/log/notify.log`

|Alarmspooler
|Der Alarmspooler (nur {EE}) dient der asynchronen Zustellung von Alarmen und dem zentralisierten Alarmieren in verteilten Umgebungen.
|`var/log/mknotifyd.log`

|Alarmierungsskript
|Für jede Alarmierungsmethode gibt es ein link:notifications.html#scripts[Skript,] welches die eigentliche Zustellung durchführt (z.B. eine HTML-Email generiert und versendet).
...
=== Asynchrone Zustellung durch Alarmspooler#async
...
link:notifications.html#scripts[Alarmierungsskript] fertig ausgeführt wurde. Sollte
...
<td>Jeder Alarm wird in einer Spooldatei unter `var/check_mk/notify/spool`
abgelegt. Es kann sich kein Stau bilden. Bei einem Stopp des Monitorings bleiben
...
Seit Version VERSION[1.6.0p] ist die asynchrone Zustellung per Default
...
== Sammelalarmierung (Bulk notifications)#bulk
...
== Nachvollziehbare Zustellung per SMTP#syncsmtp
...
. Die Auswertung der Regeln ergibt einen ICON[icon_alert_notify.png] Alarm an den Benutzer [.guihint]#hh# mit der Methode `mail`.
...
image::failed_notifications_to.png[center, width=240]
...
[.border]
image::plugin_timing_settings.png[center, width=480]
...
== Alarmierung in verteilten Systemen#distributed
...
|`2` und höher
|Ein endgültiger Fehler ist aufgetreten. Die Alarmierung wird nicht wiederholt. Auf der GUI wird ein Alarmierungsfehler angezeigt. Der Fehler wird in der Historie des Hosts/Services angezeigt. Beispiel: der SMS-Dienst meldet den Fehler „Ungültige Authentifizierung“.
...
* Bei der Alarmierung per [.guihint]#HTML Email}} gibt es eine Checkbox {{Information to be displayed in the email body}} und dort den Punkt {{Complete variable list (for testing)#.
...
|`NOTIFY_WHAT`
|Bei Hostalarmen das Wort `HOST`, sonst `SERVICE`. Damit können Sie Ihr Skript so intelligent machen, dass es in beiden Fällen sinnvolle Informationen meldet.
...
|`NOTIFY_CONTACTEMAIL`
|E-Mail-Adresse des zu alarmierenden Kontakts


|`NOTIFY_CONTACTPAGER`
|Eintrag im Feld [.guihint]#Pager# des Benutzerprofils des Kontakts. Da das Feld meist nicht für einen bestimmten Zweck belegt ist, können Sie es einfach nutzen, um eine für die Alarmierung nötige Information pro Benutzer zu speichern.
...
|`NOTIFY_LONGDATETIME`
|Datum und Uhrzeit in der nicht-lokalisierten Defaultdarstellung des Linuxsystems, also z.B. `Tue Jun 09 12:31:06 CET 2020`
...
|`NOTIFY_HOSTOUTPUT`
|Ausgabe des Hostcheck-Plugins (also z.B. „`Packet received via smart PING`“). Diese Ausgabe ist nur bei Hostalarmen interessant, aber auch bei Servicealarmen vorhanden.
...
|Der Typ des Alarms (siehe in der Einleitung dieses Artikels). Er wird durch eines der folgenden Worte ausgedrückt:
 `PROBLEM` - Normales Host- oder Serviceproblem<br> `RECOVERY` -- Host-/Service geht wieder {UP} / {OK}<br> `ACKNOWLEDGEMENT (&#8230;)` -- link:basics_ackn.html[Quittierung] eines Problems<br> `FLAPPINGSTART` -- Ein Host-/Service beginnt unstetig zu sein<br> `FLAPPINGSTOP` -- Ende der Unstetigkeit<br> `DOWNTIMESTART` -- Beginn einer geplanten link:basics_downtimes.html[Wartung].<br> `DOWNTIMEEND` -- Normales Ende einer Wartung<br> `DOWNTIMECANCELLED` -- Vorzeitiger Abbruch einer Wartung<br> `CUSTOM` -- Alarm, der per link:commands.html[Kommando] manuell ausgelöst wurde<br> `ALERTHANDLER (&#8230;)` -- Alerthandlerausführung {CEE}
...
|`NOTIFY_SERVICEDESC`
|Der Name des Services, der alarmiert wird. Bei Hostalarmen ist diese Variable nicht vorhanden.
...
|`var/log/cmc.log`
|Logdatei des link:cmc.html[CMC]. Falls das Notificationdebugging eingeschaltet ist, finden Sie hier genaue Angaben, warum Alarme (nicht) erzeugt wurden.
...
|Aktueller Zustand des Alarmspoolers. Das ist hauptsächlich bei einer link:distributed_monitoring.html#notifications[verteilten Alarmierung] relevant.


|`var/nagios/debug.log`
|Debuglogfile von Nagios. Schalten Sie Debugmeldungen in `etc/nagios/nagios.d/logging.cfg` in der Variable `debug_level` ein.
...
|Bei temporären Fehlern verschiebt der Alarmspooler die Dateien hierher und probiert es erst nach ein paar Minuten nochmal.
...
|Ablageort für eigene Alarmierungsskripten. Möchten Sie ein mitgeliefertes Skript anpassen, so kopieren Sie es von `share/check_mk/notifications` hierher und behalten dessen Dateinamen bei.


|`share/doc/check_mk/treasures/notifications`
|Hier finden Sie einige Alarmierungsskripten, die Sie geringfügig anpassen und verwenden können.

|===


=== Logdateien des SMTP-Diensts#maillog
