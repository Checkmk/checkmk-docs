...
Die Bedingungen befassen sich dabei mit den statischen Eigenschaften des Objekts (z.B. ob der Service-Name den Text `/tmp` enthält oder ein Host sich in einer bestimmten Host-Gruppe befindet), 
...
. Sobald Sie auch nur eine einzige Bedingung auswählen, greift die Regel nur, wenn auch wirklich *alle* ausgewählte Bedingungen erfüllt sind. 
Alle ausgewählten Bedingungen werden mit UND verknüpft. 
Von dieser wichtigen Regel gibt es nur eine Ausnahme, auf die wir später eingehen werden und jetzt nicht betrachten.
...
Nehmen wir an, eine Benachrichtigung soll erfolgen, wenn ein Monitoring-Ereignis eintritt für einen Service, der mit dem Namen `NTP` beginnt, auf einem Host im [.guihint]#Main directory#:

image::notifications_rule_part4.png[]

Nehmen wir weiter an, dass diese Bedingung nun *erweitert* wird, indem auch alle Zustandsänderungen eines Hosts auf den Zustand {DOWN} benachrichtigt werden sollen:

image::notifications_rule_part4_match_host_types.png[]

Das Ergebnis dieser Benachrichtgungsregel mit den drei Einzelbedingungen ist, dass *nie* eine Benachrichtigung erfolgen wird, weil kein Monitoring-Ereignis die Zustandsänderung eines Hosts *und* den Service-Namen mit `NTP` enthalten wird.
...
Blenden Sie die kontextsensitive Hilfe mit [.guihint]#Help > Show inline help# ein, um Einzelheiten über die Auswirkung der verschiedenen Bedingungen zu erfahren.
...
Damit kann man Dinge umsetzen wie „Host-Benachrichtigungen in der Kontaktgruppe Linux sollen nie per SMS versendet werden“.
...
Bei Systemen mit einem per xref:sla#[SLA] festgelegten sehr hohen Service Level kann es sinnvoll sein, es nicht bei einer einzelnen Benachrichtigung zu belassen, falls das Problem über einen längeren Zeitraum weiterhin besteht.
