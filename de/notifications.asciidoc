...
[{image-left}]
image::icon_notifications.png[alt="Glocke als Symbol für Benachrichtigungen", width=80.]
...
[{image-left}]
image::icon_downtime.png[alt="Darstellung eines Pylons zur Symbolisierung einer Wartungszeit", width=50.]
...
[{image-left}]
image::icon_outofnot.png[alt="Symbolische Darstellung einer Benachrichtigungsperiode", width=50.]
...
[{image-left}]
image::icon_notif_man_disabled.png[alt="Symbol der deaktivierten Benachrichtigung", width=50.]
...
[{image-left}]
image::icon_flapping.png[alt="Darstellung einer Perlenkette als Symbol des Flapping-Zustands", width=50.]
...
Aus der Benachrichtigungskette sehen Sie, welche Regeln auf ein Monitoring-Ereignis angewendet wurden:
...
Wurde eine Regel nicht angewendet, so erhalten Sie eine entsprechende Information, wenn Sie die Maus über den grauen Kreis bewegen (Mouse-Over-Text).

image::notifications_mouseover.png[alt="Darstellung des Hinweistextes bei einer nicht-angewendeten Regel"., width=72%]

Dieser Mouse-Over-Text verwendet für die Begründung einer nicht-angewendeten Regel Abkürzungen. Diese beziehen sich auf die xref:rule_conditions[Bedingungen], die Sie für die Regeln [.guihint]#Match host event type# bzw. [.guihint]#Match service event type# eingestellt haben:
