...
{CMK} verfügt über ein regelbasiertes System, mit dem Sie aus diesen Monitoring-Ereignisse Benutzerbenachrichtigungen erstellen lassen können, und mit dem Sie auch sehr anspruchsvolle Anforderungen umsetzen können.
...
Sobald alle Parents auf {DOWN} gehen, werden die nicht mehr erreichbaren Hosts auf den Zustand {UNREACH} gesetzt und die Benachrichtigungen für diese werden unterdrückt. 
...
In *Umgebungsvariablen,* die mit `NOTIFY_` beginnen, bekommt es alle Informationen über den betreffenden Host/Service, das Monitoring-Ereignis, den zu Kontakt und die Parameter, die in der Benachrichtigungsregel angegeben wurden.
...
// TK: Sind die Exit Codes wirklich ein CEE-Feature?
{cee-only}
...
// TK: Ich hab den folgenden Satz auskommentiert, weil ich finde, dass im Linkziel Exit Codes nicht mal erwähnt werden 
