...
Wenn Sie HTML-E-Mails verschicken, haben Sie möglicherweise den Wunsch, zusätzliche Informationen hinzuzufügen oder flexibel eine Reply-To-Adresse auf einen Kontakt für Rückfragen zu setzen.
Hierfür ist die Regel [.guihint]#Setup > Services > Service monitoring rules > Parameters for HTML Email# vorgesehen.
Mit dieser Regel können Sie eine Reihe von Parametern wie Antwortadresse, zusätzliche Felder mit Details oder Freitext als HTML formatiert hinzufügen.

Beachten Sie bei der Verwendung von HTML-Tags, dass aus Sicherheitsgründen nur eine kleine Menge an HTML-Tags zugelassen ist.
Diese sind:
...
Daneben ist `link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a[a^]` zugelassen, wenn es mit deinem Attribut `href` (verpflichtend) und `target` (optional) kombiniert ist.
