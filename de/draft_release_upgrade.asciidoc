//encoding: utf-8
// IGNORE TBD p22 p23 pN BTRFS Unmount Netzwerkshare Nullmailer
...
Das Release-Upgrade spielen wir hier exemplarisch am Beispiel des Wechsels von Ubuntu 20.04 auf 22.04 durch.
...
Ähnliches können Sie bei Verwendung von _Copy-on-Write-Dateisystemen_ wie ZFS oder BTRFS mit _Snapshots_ erreichen.
Allerdings müssen Sie die Entscheidung für so ein Dateisystem schon bei der Installation des {CMK}-Servers getroffen haben.

Minimal empfehlen wir:
...
* Sicherung des Inhaltes von `/opt/omd/sites` nach Stoppen aller Instanzen und Unmount der instanzspezifischen `/tmp` Dateisysteme. Dies kann im Zuge des Updates durchgeführt werden.
...
Diese Methode ist oft beim Einsatz von {CMK} auf dedizierter Hardware mit großen Datenbeständen sinnvoll, wo Backup und Restore die Downtime deutlich erhöhen würde.
...
. Nicht benutzte {CMK}-Versionen können Sie deinstallieren.
...
Diese Methode bietet sich häufig beim Einsatz von {CMK} auf in virtualisierten Umgebungen an, wo es möglich ist, einen zweiten Server mit der neuen Distributionsversion vorbereitend _hochzuziehen_ und mit diesem im Parallelbetrieb bereits erste Tests durchzuführen.
...
Die Schritte, welche hier seriell dargestellt werden, können Sie in vielen Fällen parallelisieren, was die Downtime minimieren hilft.
...
. Führen Sie unter der alten Distributionsversion ein Backup der Site durch. Unser Beispiel verwendet als Ziel ein Netzwerkshare, welches später auch auf der neuen Installation bereitstehen wird.
...
. Bereiten Sie nun das Zielsystem vor und konfigurieren Sie dort benötigte Software, beispielsweise den _Nullmailer_.
Verwenden Sie denselben Hostnamen und dieselbe IP-Adresse wie das Ausgangssystem.

. Installieren Sie die gleiche {CMK}-Version mit exakt identischem Patchlevel.
Im Falle von Ubuntu würde an dieser Stelle der folgende Befehl genügen:
