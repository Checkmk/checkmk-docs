...
Zum anderen ergänzt er das Skript so, dass neue Funktionen hinzugefügt werden können wie die TLS-Verschlüsselung der Kommunikation, Datenkomprimierung -- und auch die Umkehrung der Kommunikationsrichtung, den neuen *Push-Modus*. Mehr dazu im nächsten Kapitel.
...
Der Artikel, den Sie gerade lesen, behandelt Installation, Konfiguration und Erweiterungen des neuen Linux-Agenten im neuen *Registrierungs-Modus*.
...
Präziser gesagt, wird dem bestehenden Shellskript  mit dem Agent Controller eine neue Komponente zur Seite gestellt.
Der Agent Controller (`cmk-agent-ctl`) ist dem eigentlichen Agentenskript (`check_mk_agent`) vorgeschaltet, fragt diesen ab und kommuniziert an dessen Stelle mit dem {CMK}-Server.
