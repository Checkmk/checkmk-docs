...
:description: Für die Alarmierung in Checkmk muss der Versand per E-Mail vorbereitet und aktiviert werden. Anschließend wird die Alarmierung getestet und feinjustiert. Um diese Themen geht es im aktuellen Kapitel des Leitfadens für Checkmk Einsteiger, der kompakt durch die ersten Schritte führt, um das IT-Monitoring mit Checkmk optimal einzurichten.
...
* Aktivieren Sie die Alarmierung für andere Benutzer erst dann, wenn Sie die Fehlalarme (<I>false positives_) auf ein Minimum reduziert haben. Was Sie dafür tun können, haben wir im link:intro_finetune.html[Kapitel über die Feinjustierung des Monitoring] beschrieben.
...
Sie finden [.guihint]#Fake check results# auf einem ähnlichen Weg wie die Kommandos zum link:intro_monitor.html#ack[Quittieren von Problemen] und zur link:intro_monitor.html#downtimes[Festlegung von Wartungszeiten]: Öffnen Sie eine Statusansicht mit einer Service-Liste (am schnellsten über die [.guihint]#Tactical overview#), blenden Sie die Checkboxen ein und wählen die einen Service aus -- am besten einen, der gerade {OK} ist. Klicken Sie im Menü [.guihint]#Commands# > [.guihint]#Fake check results#:
...
zum Ausbau des Monitoring, von denen wir im link:intro_extend.html[nächsten Kapitel] einige kurz vorstellen werden inklusive der Verweise in die Artikel, die die detaillierten Informationen zu den Themen bieten.
