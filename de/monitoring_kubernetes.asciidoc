...
Vor allem die link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/charts/checkmk[Anleitungen zur Installation des Agenten^] ist eine direkte Quelle, um die aktuell empfohlenen Wege nachzulesen.
...
Aktuell empfehlen wir die Installation des Kubernetes-Monitorings mit Hilfe des Tools `helm`, da es sich auch für weniger versierte Nutzer eignet. Machen Sie dafür zuallererst das Repository bekannt. In dem folgenden Beispiel nutzen wir den Namen `k8scmk`, um später einfacher auf das Repository zugreifen zu können. Sie können aber selbstverständlich auch jeden anderen Namen nutzen:
...
Wenn Sie bereits sehr viel Erfahrung mit Kubernetes mitbringen und Kontrolle über alle Details der Einrichtung haben wollen, eignet sich auch die Einrichtung des Kubernetes-Monitorings mit den von uns vorgefertigten Manifesten. Laden Sie dafür zuerst die von uns vorgefertigten Manifeste aus unserem entsprechenden link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/kubernetes[Repository bei GitHub^] herunter.
