...
Wenn Sie die Einrichtung des Kubernetes-Monitoring mit den von uns vorgefertigten Manifesten angehen wollen, so müssen Sie diese zuerst aus unserem entsprechenden link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/kubernetes[Repository bei GitHub^] herunterladen. Sie benötigen mindestens die folgenden fünf Dateien:
...
Wenn Sie nicht gleich das gesamte Repo klonen wollen -  was Ihnen selbstverständlich frei steht -, können Sie mit dem folgenden Befehl gezielt die fünf benötigten Dateien herunterladen:
...
Dieser erlaubt die Kommunikation von extern und wird in der Datei `service.yaml` fest auch den TCP-Port 30035 gesetzt.
Sollte dieser Port in Ihrem Cluster schon vergeben sein, ändern sie den Port bitte entsprechend.

Sobald Sie diese Einstellungen vorgenommen habe, können Sie diese Manifest-Dateien gesammelt auf Ihr Cluster anwenden.
...
Des weiteren können Sie auch noch alle Service innerhalb des Namespace wie folgt prüfen:
...
Als nächstes geht es in der GUI von {CMK} an die Einrichtung des Spezialagenten und einer Regel für die automatische Erzeugung von Host für Ihre Kubernetes-Objekte.
Für die Einrichtung des Spezialagenten müssen aber zuerst noch eine Voraussetzungen erfüllt werden:
...
Wenn Sie als Namespace für das Monitoring Ihres Kuberentes-Cluster das vorgegebene `checkmk-monitoring` beibehalten haben, schneidet die folgende Befehlszeile das Passwort direkt aus der Ausgabe von `kubectl get secrets` aus:
...
Damit {CMK} der CA des Service-Accounts vertrauen kann, müssen Sie dieses in {CMK} hinterlegen. Auslesen können Sie das Zertifikat - sofern Sie als Namespace `checkmk-monitoring` beibehalten haben - mit dem folgenden Befehl:
...
Da dieser Host ausschließlich über den Spezialagenten Daten erhält, setzen sie die Option [.guihint]#IP address family# auf [.guihint]#No IP#.

==== Dynamische Host-Konfiguration einrichten

Um eine Trennung zwischen den zahlreichen Kubernetes-Objekten und Ihrer restlichen Monitoring-Umgebung zu gewährleisten, bietet es sich an zuerst über [.guihint]#Setup > Hosts > Add folder# einen Ordner anzulegen, in welchem die xref:dcd#[dynamische Host-Konfiguration] automatisch alle notwendigen Hosts anlegen kann. Einen solchen Ordner zu erzeugen bzw. zu nutzen ist aber sicherlich als optional zu betrachten.
...
Klicken Sie als nächstes auf [.guihint]#Add new element# und wählen Sie unter [.guihint]#Create hosts in# den zuvor angelegten Ordern aus.

In einer Kubernetes-Umgebung, in der überwachbare und überwachte Objekte naturgemäß kommen und gehen, empfiehlt es sich auch die Option [.guihint]#Automatically delete hosts without piggyback data# zu aktivieren. Was genau diese Option bewirkt und unter welchen Umständen Hosts dann tatsächlich gelöscht werden, erklären wir im Abschnitt xref:dcd#_automatisches_löschen_von_hosts[Automatisches Löschen von Hosts]
im Artikel zur dynamischen Host-Konfiguration.

Tragen Sie nun noch unter [.guithint]#Restrict source hosts# den zuvor angelegten xref:source-host[Piggyback Quell-Host] ein und aktivieren Sie die Option [.guihint]#Discover services during creation#.
...
Wenn Sie hier beispielsweise `mycluster` eintragen, werden die Namen der Hosts aller Pods aus diesem Cluster später mit `pod_mycluster` beginnen. Der nächste Teil des Host-Namen wird dann immer der Namespace sein, in dem dieses Kubernetes-Objekt existiert.

Wählen Sie unter [.guihint]#Token# nun den xref:token[zuvor angelegten Eintrag] aus dem Passwortspeicher von {CMK} aus.

Unter [.guihint]#API server connection > Endpoint# verlangt {CMK} nun die Eingabe der URL (bzw. IP) über welche ihr Kubernetes API server erreichbar ist.
Die Angabe des Ports ist hier ebenfalls Pflicht.
Wie Sie diese IP am einfachsten herausfinden können - falls Sie sie nicht bereits zur Hand haben - hängt von Ihrer Kubernetes-Umgebung ab.
...
Wenn Sie die CA Ihres Clusters - xref:certimport[wie oben beschrieben] - in {CMK} hinterlegt haben, können Sie unter [.guihint]#SSL certificate verification# [.guihint]#Verify the certificate# auswählen.
...
Falls Sie den Port in der Datei `service.yaml` angepasst haben, so ändern Sie den Port hier bitte entsprechend.
Die URL bzw. die IP des Nodeport sollte sich über die Beschreibung des Pods `cluster-collector` herausfinden lassen.
Führen Sie dazu einfach den folgenden Befehl aus und schauen Sie dort in der Zeile die mit `Node:` beginnt nach:
...
*Wichtig:* Unter [.guithint]#Conditions > Explicit hosts# müssen Sie nun erneut den xref:source-host[zuvor angelegten Host] eintragen:
...
Speicher Sie anschließend die Regel und führen Sie eine Serviceerkennung auf diesem Host durch.
