...
Das im Folgenden beschriebene Feature von {CMK} ist Teil des Beta-Release der Version {v21}.
Sowohl an dieser Beschreibung als auch an dem Feature selbst kann es noch Änderungen geben.
Beachten Sie dazu die link:https://github.com/tribe29/checkmk-docs/commits/master/de/monitoring_kubernetes.asciidoc[Änderungen an diesem Artikel auf GitHub^] bzw. unsere link:https://checkmk.com/de/werks?search=kubernetes&product=cmk&cmk_version%5B%5D=2.1[Werks bezüglich Änderungen am Feature^] selbst.

Da die Integration von Kubernetes in {CMK} nativ in Kubernetes selbst eingebaut wurde,
nutzen wir auch direkt die _README_-Dateien in den GitHub-Repositories selbst.
Vor allem die link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/charts/checkmk[Anleitungen zur Installation des Agenten^] ist eine direkte Quelle,
um die aktuell empfohlenen Wege nachzulesen.
////
...
Das Kubernetes-Monitoring in {CMK} wurde von Grund auf neu geschrieben. Der Umfang der überwachbaren Daten ist stark gewachsen. Da die technische Grundlage für das Kubernetes-Monitoring in {CMK} {v21} grundlegend anders ist, ist eine Übernahme oder auch eine Weiterschreibung bisheriger Monitoring-Daten Ihrer Kubernetes-Objekte nicht möglich.
// MA: Hier fehlt mir eine Beschreibung, _wie_ wir nun Kuberentes überwachen. Also: Was ich richte ich eigentlich in Kapitel 3 ein? Hint: Wahrscheinlich die Definition, welche Pods mit welchen Container, etc. im Cluster laufen sollen.
...
////
Für die Einrichtung der Überwachung geben wir Ihnen im Folgenden zwei verschiedene Methoden zur Auswahl.
Da der erfolgreiche Einsatz von Kubernetes ein gerüttelt Maß an Erfahrung vorraussetzt, haben Sie hierzu vermutlich schon Ihre eigene Präferenz.
Womöglich gibt es auch eine Vorgabe oder Policy, welchen Weg Sie hier beschreiten wollen oder müssen.
////

////
Sie können die Einrichtung des Monitoring mit den von uns link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/kubernetes[vorgefertigten Manifesten^] im Format YAML durchführen.
Oder Sie verwenden die von link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/charts/checkmk[uns angebotenen Helm Charts^], um die Einrichtung dem "Packet Manager für Kubernetes" zu überlassen.
////


== Voraussetzungen im Cluster schaffen

//==== Einrichten per Helm chart
// MA: Übergangsweise, damit wir den Hinweis auf die bevorzugte Installationsweise der Entwickler in der Doku haben. Hier fehlen noch ein paar Dinge, wie z.B. erwartbare Ausgaben zu den Kommandozeilen.

Um Ihr Kubernetes-Cluster in {Cmk} überwachen zu können, schaffen Sie zuerst die Voraussetzungen in Ihrem Cluster. Vor allem, indem Sie dem Cluster mitteilen, welche Pods/Container bereitgestellt werden sollen und wie die Konfiguration derselben aussehen muss.

=== Einrichten des Helm-Repository

Aktuell empfehlen wir die Installation des Kubernetes-Monitoring mit Hilfe des Tools `helm`,
da es sich auch für weniger versierte Nutzer eignet und die Verwaltung der Konfigurationen standardisiert. Helm ist eine Art Paketmanager für Kubernetes. Sie können darüber _Repositories_ einbinden und die darin enthaltenen Helm-Charts wie Pakete ganz einfach Ihrem Cluster hinzufügen.
 Machen Sie dafür zuallererst das Repository bekannt. In dem folgenden Beispiel nutzen wir den Namen `tribe29`, um später einfacher auf das Repository zugreifen zu können. Sie können aber selbstverständlich auch jeden anderen Namen nutzen:
...
Nachdem Sie das frisch geklonte Repository mit dem Kommando `helm repo update` optional aktualisiert haben,
können Sie mit dem zweiten Schritt die restliche Installation angehen.

=== Anpassungen an der Konfiguration vornehmen

Helm erzeugt die notwendigen Konfigurationsdateien komplet selbstständig. Um bestimmte Parameter über alle Konfigurationen hinweg selbst bestimmen zu können, geben Sie dabei eine Steuerdatei mit, die sogenannte `values.yaml`. Als Ausgangsbasis empfehlen wir die von uns bereitgestellte link:https://raw.githubusercontent.com/tribe29/checkmk_kube_agent/main/deploy/charts/checkmk/values.yaml[Vorlage^]. Kopieren Sie sich diese und passen Sie sie an die eigene Umgebung an. Da wir im Vorfeld nicht wissen können, wie Ihr Kubernetes-Cluster aufgebaut ist, haben wir die sicherste Variante gewählt, wie die {CMK}-Kollektoren gestartet werden: Sie stellen standardmäßig keine Ports bereit, um von außen erreicht werden zu können. Damit Sie später auf die Kollektoren zugreifen können, passen Sie diese Einstellungen entsprechend an. Der Einfachheit halber, nehme wir unsere Vorlage als Ausgangsbasis und passen diese entsprechend an. Wir unterstützen standardmäßig zwei Kommunikationspfade: Die Abfrage über _Ingress_ und die Abfrage über _NodePort_. Je nachdem, welche Variante Sie in Ihrem Cluster unstützen, ist die Konfiguration unterschiedlich.

==== Kommunikation per Ingress bereitstellen

Falls Sie link:https://kubernetes.io/docs/concepts/services-networking/ingress/[Ingress^] verwenden, um die Zugriffe zu Ihren Diensten zu steuern, passen Sie entsprechend die bereits vorbereiteten Teile in der `values.yaml` an. Zur besseren Übersicht ist in dem folgenden Beispiel nur der relevante Ausschnitt gezeigt. Den Wert `enabled` setzen Sie auf `true`. Die restlichen Werte passen Sie entsprechend Ihrer Umgebung an:
...
Sie können den Zugriff auf die Dienste auch direkt über einen Port bereitstellen. Das ist dann notwendig, wenn Sie kein Ingress verwenden. Auch hier wird nur der relevante Ausschnitt in dem nachfolgenden Beispiel gezeigt. Sie setzen dabei den Wert `type` auf `NodePort` und entfernen die Auskommentierung zu dem Wert `nodePort`:
...
| helm upgrade --install | Dieser Teil stellt den Basisbefehl, um dem Kubernetes-Cluster die Konfiguration zu übermitteln
| --create-namespace | In Kuberentes geben Sie immer an, zu welchem _Namespace_ die Konfiguration hinzugefügt werden soll. Diese Option benötigen Sie, falls es den Namespace noch nicht gibt. Helm wird ihn in diesem Fall mit anlegen.
...
| checkmk | `checkmk` steht hier als Beispiel für den Namen des Helm-Charts. Idealerweise belassen Sie diesen Namen auch so, da Sie nur dan automatisch davon profitieren, dass die Kubernetes-Objekte kurze Namen bekommen
...
| -f values.yaml | Zuletzt geben Sie die angepasste Konfigurationsdatei an, die Sie in dem zweiten Schritt erstellt haben. Sie enthält alle Anpassungen, die in der durch `helm` erstellen Konfigurationsdateien berücksichtigt werden sollen.
|===

Nachdem Sie das Kommando ausgeführt haben, ist Ihr Kubernetes-Cluster vorbereitet, um mit {CMK} überwacht zu werden. Das Cluster wird nun selbstständig darum kümmern, dass die notwendigen Pods und die darin enthaltenen Container laufen und erreichbar sind.

// Wahrscheinlich nicht mehr notwendig:
// Mehr Informationen finden Sie in der Einleitung bereits erwähnten _README_-Datei der link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/charts/checkmk[Helm Charts^].


=== Alternative: Einrichten per Manifest

Normalerweise ist es nicht sinnvoll, dass Sie die Manifeste (Konfigurationsdateien) selbst anpassen. Zum einen, da Sie dafür genaue Kenntnisse der Architektur der _{CMK} Kubernetes Collectors_ benötigen und zum anderen, da die händische Anpassung sehr viel fehleranfälliger ist. So richten Sie in `helm` die Kommunikation über TLS einmalig ein, anstatt sie an allen relevanten Stellen in den Manifesten selbst einzutragen. Wenn Sie jedoch kein `helm` nutzen, oder die Kontrolle über alle Details der Einrichtung haben wollen, können Sie diesen Weg dennoch auch gehen. Laden Sie dafür zuerst die von uns vorgefertigten Manifeste aus unserem entsprechenden link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/kubernetes[Repository bei GitHub^] herunter. Sie benötigen mindestens die folgenden fünf Dateien:
...
Im Einzelnen dienen diese Manifeste den folgenden Zwecken:
...
Wenn Sie nicht gleich das gesamte Repository klonen wollen -  was Ihnen selbstverständlich frei steht -, können Sie mit dem folgenden Befehl gezielt die fünf benötigten Dateien herunterladen:
...
// TK: Hier wird mir nicht klar genug, dass Du durch die String-Ersetzung die Manifestdateien an den kubernetes-collector koppelst.
// TK: Bytheway: Was ist ein kubernetes-collector überhaupt? Und welchen nehm ich denn da?
...
== Voraussetzungen in {CMK} schaffen
...
// TK: Du importierst doch das Zertifikat, um der CA zu vertrauen, oder?

Damit {CMK} der Certificate Authority (CA) des Service-Accounts vertrauen kann, müssen Sie das CA-Zertifikat in {CMK} hinterlegen. Auslesen können Sie das Zertifikat - sofern Sie als Namespace `checkmk-monitoring` beibehalten haben - mit dem folgenden Befehl:
...
Erzeugen Sie in Checkmk auf gewohnte Weise einen neuen Host und nennen Sie diesen beispielsweise `mykubernetesclusterhost`.
...
Um eine Trennung zwischen den zahlreichen Kubernetes-Objekten und Ihrer restlichen Monitoring-Umgebung zu gewährleisten, bietet es sich an zuerst über [.guihint]#Setup > Hosts > Add folder# einen Ordner anzulegen, in welchem die xref:dcd#[dynamische Host-Konfiguration] automatisch alle notwendigen Hosts anlegen kann. Einen solchen Ordner zu erzeugen bzw. zu nutzen ist aber optional.

Unbedingt notwendig hingegen ist die Einrichtung eines Konnektors fürs die anfallenden Piggyback-Daten. Über [.guihint]#Setup > Hosts > Dynamic host management > Add connection# gelangen Sie zur Seite für die entsprechende Einrichtung.
...
In einer Kubernetes-Umgebung, in der überwachbare und überwachte Objekte naturgemäß kommen und gehen, empfiehlt es sich auch die Option [.guihint]#Automatically delete hosts without piggyback data# zu aktivieren.
// TK: naturgemäß? Naja...
Was genau diese Option bewirkt und unter welchen Umständen Hosts dann tatsächlich gelöscht werden, erklären wir im Kapitel xref:dcd#_automatisches_löschen_von_hosts[Automatisches Löschen von Hosts]
...
Wenn Sie hier beispielsweise `mycluster` eintragen, werden die Namen der Hosts aller Pods aus diesem Cluster später mit `pod_mycluster` beginnen. Der nächste Teil des Host-Namens wird dann immer der Namespace sein, in dem dieses Kubernetes-Objekt existiert.
...
Die Angabe des Ports ist hier ebenfalls notwendig, wenn Dienst nicht über einen virtuellen Host bereitgestellt wurde.
...
//Zu schwammig? Ja, Kubernetes ist halt ein großer Schwamm.
...
// TK. Wie oben: "Wenn": Gibt doch eh keine Alternative.
...
Sollten Sie sich dazu entscheiden auch die [.guihint]#Pods of CronJobs# zu überwachen, so beachten Sie bitte die xref:user_interface#inline_help[Inline-Hilfe] zu diesem Punkt.
...
*Wichtig:* Unter [.guihint]#Conditions > Explicit hosts# müssen Sie nun erneut den xref:source-host[zuvor angelegten Host] eintragen:
// TK: erneut?
...
// TK: Wenn: siehe oben. erstellten Accounts, Service und so weiter > die erstellten Objekte?
