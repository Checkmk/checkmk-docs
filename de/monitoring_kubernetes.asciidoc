...
Ein vollständige Auflistung aller verfügbaren Check-Plugins für die Überwachung von Kubernetes finden Sie in unserem link:https://checkmk.com/de/integrations?tags=kubernetes[Katalog der Check-Plugins.^]
...
Aktuell empfehlen wir die Installation des Kubernetes-Monitoring mit Hilfe des Tools `helm`, da es sich auch für weniger versierte Nutzer eignet. Machen Sie dafür zu allererst das Repository bekannt. In dem folgenden Beispiel nutzen wir den Namen `k8scmk`, um später einfacher auf das Repository zugreifen zu können. Sie aber selbstverständlich auch jeden anderen Namen nutzen:

[{shell}]
----
{c-user} helm repo k8scmk https://tribe29.github.io/checkmk_kube_agent
----

Nachdem, sie das frisch geklonte Repository mit `helm repo update` aktualisiert haben, können mit dem zweiten Schritt die restliche Installation vornehmen. Hier vergeben Sie einen sogenannten _Release_Namespace_ -- im Beispiel ist das 'my_release' und einen _Release_Name_. Wenn Sie letzteren auf `checkmk` setzen, profitieren Sie automatisch davon, dass die Kubernetes-Objekte kurze Namen bekommen. Zuletzt geben sie noch den Pfad an, wo `helm` die notwendigen Konfigurationsdateien findet:

[{shell}]
----
{c-user} helm upgrade --install --create-namespace -n my_release checkmk k8scmk/checkmk
----

Alternativ können Sie auch am Ende des Kommandos angeben, wenn sie eine bestimmte Version nutzen möchten. Hier könnten Sie zum Beispiel `-- version 1.0.0-beta.2` angeben, oder auch bestimmte Marker, wie das `--devel` in dem folgenden Beispiel:

[{shell}]
----
{c-user} helm upgrade --install --create-namespace -n my_release checkmk k8scmk/checkmk --devel
----
...
==== Einrichten per Manifest

Wenn Sie bereits sehr viel Erfahrung mit Kubernetes mitbringen und Kontrolle über alle Details der Einrichtung haben wollen, eignet sich auch die Einrichtung des Kubernetes-Monitoring mit den von uns vorgefertigten Manifesten. Laden Sie dafür zuerst die von uns vorgefertigten Manifeste aus unserem entsprechenden link:https://github.com/tribe29/checkmk_kube_agent/tree/main/deploy/kubernetes[Repository bei GitHub^] herunter. Sie benötigen mindestens die folgenden fünf Dateien:
...
Nachdem nun alle Vorraussetzungen im Cluster und in {CMK} geschaffen sind, können Sie sich der Konfiguration des Spezialagenten widmen.
...
Die Angabe des Ports ist hier ebenfalls Pflicht.
...
Bei einem Vanilla-Kubernetes ist erneut `kubectl` ein guter Anlaufpunkt.
Mit dem folgenden Befehl können Sie sich alle Endpoints Ihrer Umgebung anzeigen lassen.
...
Je nachdem auf welchen Kontext `kubectl` bei Ihnen gerade eingestellt ist, können Sie `--all-namespaces` gegebenenfalls weglassen.
// TK: --all-namespaces steht ja gar nicht im Kommando
...
Zu guter Letzt können Sie noch auswählen, ob Sie nur bestimmte Namespaces innerhalb Ihre Clusters überwachen möchten oder ob explizite Namespaces aus der Überwachung ausgeschlossen werden sollen.
