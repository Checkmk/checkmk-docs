...
Sie können darüber _Repositories_ einbinden und die darin enthaltenen Helm-Charts wie Pakete ganz einfach Ihrem Cluster hinzufügen.
Machen Sie dafür zuallererst das Repository bekannt.
// TK: Was ist "das Repository"?
...
Nachdem Sie das frisch geklonte Repository mit dem Kommando `helm repo update` optional aktualisiert haben, können Sie mit dem zweiten Schritt die restliche Installation angehen.
// TK: Wenn es frisch geklont ist, macht ein Update jetzt keinen Sinn, oder? Später wohl schon?
// TK: restliche Installation? Weiter geht es aber mit der Konfiguration.
// TK: Vielleicht so:
////
Nachdem Sie das Repository geklont haben, können Sie mit dem zweiten Schritt die Konfiguration angehen.
...
Helm erzeugt die notwendigen Konfigurationsdateien komplett selbstständig.
...
// TK: Den Einschub "- was Ihnen selbstverständlich frei steht -" hab ich gelöscht - war mir zu gönnerhaft.

// TK: Zur Abwechslung mal was technisches: Ich hab in den beiden folgenden Kommandos shell zu shell-raw gemacht, weil das https zwar im Preview zu sehen ist, aber online rausfliegt und in einen Link umgesetzt wird.
