...
[{image-left}]
image::influxdb-logo.png[width=100]
...
[{image-left}]
image::graphite-logo.png[width=150]
...
// InfluxDB wurde nur teilweise aus der "alten" GUI entfernt.
...
Wählen Sie im xref:user_interface#setup_menu[Setup-Menü] [.guihint]#Setup > Custom > InfluxDB connections#, um sich die Liste der existierenden InfluxDB-Verbindungen anzuzeigen. Im Auslieferungszsutand ist die Liste leer. Erstellen Sie also eine neue Verbindung mit 
...
.Die Auswahl der Metriken erfolgt per Regel
image::influxdb_send_metrics_rule.png[alt="Regel zur Auswahl der Metriken zum Versand über die InfluxDB-Verbindung."]
...
Im folgenden wählen Sie die Metrikdaten aus, die von {CMK} an den InfluxDB-Server gesendet werden, wie zum Beispiel den Service-Zustand.
...
// TK: Viele Fragen ohne funktionierenden Versand zur InfluxDB: 
// TK: Metrics of service: Was passiert, wenn ich das nicht auswähle? Wird dann nichts gesendet? Welche Metriken gibt es denn noch (Metrics of host?)
// TK: Service state: State of service kann ich unten unter Additional values for metric nochmal auswählen. Warum 2x?
// TK: Wo finde ich die tags in der InfluxDB-GUI?
...
////
TK: InfluxDB-GUI Screenshot nachliefern
[#influxdb_display]
...
Neben den offensichtlichen Angaben zum Graphite-Server (Name und Portnummer des Carbon-Plaintext-Protokolls) können Sie mit [.guihint]#Optional variable prefix# einen Präfix konfigurieren, der jedem Host-Namen vorangestellt wird, um z.B. eindeutige Namen zu erzwingen.
{CMK} setzt den Metrikpfad für das Plaintext-Protokoll auf `<Host>.<Service>.<Metrik>`.
// TK: Inline help sagt: Graphite graphs will be named with the schema HOST / SERVICE / VARIABLE.
// Mit [.guihint]#C-style name mangling# haben Sie eine Option, um die Zerlegung der Daten zu beeinflussen (mehr Informationen dazu finden Sie in der Inline-Hilfe).
// TK: Die Option C-style name mangling würde ich nicht beschreiben: Die Inline help hilft (zumindest mir) nicht wirklich weiter.
...
[#graphite_metrics]
=== Metriken auswählen
// TK: Die Überschrift ist nicht korrekt, weil ich bei Graphite gar keine Metriken auswähle, sondern nur zusätzliche Werte, die mitgeliefert werden.

Den Regelsatz [.guihint]#Send metrics to Graphite# finden Sie mit der Suchfunktion im [.guihint]#Setup#-Menü.
...
.Auch bei Graphite erfolgt die Auswahl der Metriken per Regel
image::influxdb_graphite_rule.png[alt="Regel zur Auswahl der Metriken zum Versand über die Graphite-Verbindung."]

// TK: Den Trick mit den 2 Regeln aus SUP-5442 abgeschrieben.
Da ohne einschränkende Regel alle Daten gesendet werden, empfiehlt es sich zuerst eine Regel zu erstellen, die die Weiterleitung global abschaltet und anschließend eine spezifiche Regel für die Hosts und Services zu erstellen, deren Metriken an den Graphite-Server übermittelt werden sollen. Ordnen Sie die spezifische vor der globalen Regel an:
...
////
TK: Graphite-GUI Screenshot nachliefern
[#graphite_display]
...
Folgendes Beispiel zeigt die Meldungen im Fall einer nicht funktionierenden Verbindung zum InfluxDB-Server:
// TK: Meldungsteil muss noch überarbeitet werden, wenn die InfluxDB-Verbindung mal wieder steht.
// TK: Momentan wird das GET ewig wiederholt...

.~/var/log/cmc.log
[{file}]
----
2021-12-15 15:29:35 [5] [influxdb https://influx.dev.tribe29.com:8086] Successfully initiated connection
2021-12-15 15:29:45 [3] [influxdb https://influx.dev.tribe29.com:8086] GET failed [10]
2021-12-15 15:29:46 [3] [influxdb https://influx.dev.tribe29.com:8086] GET failed [10]
----
...
Der {CMK} Micro Core versucht in so einer Situation von sich aus immer wieder, die Verbindung aufzubauen. 
Metriken, die während einer Zeit anfallen, zu der keine Verbindung zu Graphite besteht, werden nicht zwischengespeichert, sondern gehen verloren (bzw. sind dann nur in den RRDs von {CMK} verfügbar).
