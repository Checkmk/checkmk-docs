...
// TK: Ich find es besser, dass InfluxDB und Graphite im Titel stehen
:revdate: draft
...
InfluxDB bietet beginnend mit seiner V2.0 eine neue, einheitliche API -- die InfluxDB v2 API -- mit der unter anderem Metrikdaten in die Datenbank geschrieben werden können.
...
// MA: Der alte Weg _muss_ sogar gegangen werden, wenn man noch InfluxDB 1.8 oder früher einsetzt. Das sollte aber imho in dem Influx-Teil erwähnt werden. Die config unterscheidet sich ja nicht wesentlich von Graphite.
// TK: Passt das so?
*Wichtig:* Die in diesem Kapitel beschriebene Konfiguration gilt nur für die Anbindung einer InfluxDB beginnend mit V2.0, da erst ab dieser Version die InfluxDB v2 API verfügbar ist.
Falls Sie eine InfluxDB +<=+ V1.8 einsetzen, dann richten Sie diese stattdessen so ein wie im xref:graphite[Kapitel] zur Graphite-Anbindung beschrieben, da InfluxDB bis zur V1.8 das Carbon-Protokoll von link:https://docs.influxdata.com/influxdb/v1.8/supported_protocols/graphite/[Graphite^] unterstützt.
...
Öffnen Sie das xref:user_interface#setup_menu[Setup-Menü] und schalten Sie den xref:intro_gui#show_less_more[Show-more-Modus] an -- denn nur dann wird der Menüeintrag [.guihint]#Setup > Custom > InfluxDB connections# angezeigt. Wählen Sie diesen aus, um sich die Liste der existierenden InfluxDB-Verbindungen anzeigen zu lassen. Da Sie hier wahrscheinlich noch nichts konfiguriert haben, wird die Liste leer sein.
...
Erstellen Sie also zuerst eine neue Verbindung mit icon:icon_new[] [.guihint]#Add connection#:
...
Falls mit der aktuellen Instanz weitere Instanzen verbunden sind, wie das beim xref:distributed_monitoring#[verteilten Monitoring] der Fall ist, können Sie unter [guihint]#Site restriction# die Verbindung auf bestimmte Instanzen eingrenzen.
Alle Instanzen, die Daten empfangen sollen, müssen in der rechten Spalte [.guihint]#Selected# eingetragen sein.
...
// TK: Ich finde den Eintrag "All sites" verwirrend, sobald ich anfange die Liste zu verändern: 
// TK: Wenn ich 2 Sites habe (Site1 und Site2) und Site1 nach rechts schiebe, dann steht links Site2 und rechts Site1 und All sites. Bekommt Site2 nun Daten? All Sites ist ja noch ausgewählt. Und bekommt Site1 die Daten doppelt? 
// TK: Oder ist es so gedacht: Sobald ich einschränken will, muss ich zuerst "All sites" nach links schieben und dann die einzelnen Sites nach rechts rüberziehen?
...
// TK: Danke für den Hinweis: Die Regel-Icons hatte ich komplett übersehen.

[#influxdb_connection_list]
.Aktiv und/oder mit Regel? Die Verbindungsliste zeigt es
image::influxdb_connection_list.png[alt="Liste der InfluxDB-Verbindungen."]
...
Aus der ersten Spalte können Sie ablesen, ob die Verbindung icon:icon_perm_yes[alt="Symbol zur Anzeige einer aktiven Verbindung."] aktiviert oder icon:icon_perm_no[alt="Symbol zur Anzeige einer deaktivierten Verbindung."] deaktiviert ist.
...
Bei einer regellosen Verbindung führt ein Klick auf icon:button_rulesets_disabled[alt="Knopf zur Erstellung einer neuen Regel."] direkt in die Seite zur Erstellung der Regel.
...
Mit [guihint]#Organisation# entscheiden Sie, welche Benutzer die Daten erhalten, und mit [guihint]#Bucket#, wie lange die Metriken vorgehalten werden.
...
Durch Ankreuzen der [guihint]#Service state# Checkbox wird der xref:monitoring_basics#services[Service-Zustand] als eigene Metrik gesendet.

Nach Aktivierung von [guihint]#Additional values per metric# können Sie den Metriken eine Reihe von Meta-Daten mitgeben, die der Darstellung dienen, z.B. den Zustand des zugehörigen Services ([guihint]#State of service#) und die Schwellwerte ([guihint]#Thresholds#).
...
In der letzten Liste [guihint]#Tags to use# werden den Metriken Meta-Daten wie xref:glossar#host_tag[Host-Merkmale] oder xref:glossar#label[Labels] hinzugefügt, die hauptsächlich der Filterung dienen und dafür optimiert sind. Die standardmäßig ausgewählten [guihint]#Host name# und [guihint]#Service name# sind dabei Pflichteinträge, die nicht abgewählt werden können.
...
Erstellen Sie anschließend eine spezifische Regel für die Hosts und Services, deren Metriken an den Graphite-Server übermittelt werden sollen. Ordnen Sie die spezifische vor der globalen Regel an:
...
// TK: Das stand aber bereits im Absatz davor und steht jetzt immernoch da: Ich hab jetzt Performance-Gründe aufgenommen und ein Wichtig davorgeklebt.
