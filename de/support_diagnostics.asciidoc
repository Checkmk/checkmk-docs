...
[.guihint]#Site# auswählen, von welcher Sie die im Folgenden auszuwählenden Daten
sammeln wollen.
...
aller Dateien, die im Verzeichnis `~/local/` Ihrer Site liegen. Dies kann
...
Und falls es sich bei der oben ausgewählten Site um den Knoten eines Clusters
...
Fehler in der Hostkonfiguration zu finden.
...
Wenn Sie die Site jetzt einfach einige Minuten weiter laufen lassen oder einen
...
|--checkmk-overview  |Informationen aus [.guihint]#HW/SW Inventory-Knoten Software > Applications > Checkmk# der {CMK}-Server.
