...
Mit Version {v20} kann das Leben nun viel einfacher sein, da Sie ein in {CMK} integriertes Werkzeug haben, das die benötigten Informationen generiert -- die _Support Diagnostics_.
...
Würden Sie es dabei belassen -- also keine der weiteren Optionen auswählen -- und über [.guihint]#Collect dump# die Sammlung der Daten starten, erhielten Sie eine Datei, die nur die folgenden Daten enthielte:
...
Dies kann mitunter hilfreich sein, wenn Ihre lokale Anpassungen zu einem kürzlich durchgeführten Update von {CMK} inkompatibel sind. Auch alle installierten MKPs werden hier mit erfasst.
...
Nach der Aktivierung der Option [.guihint]#{CMK} Configuration files# haben Sie die Möglichkeit, mehr oder weniger vertrauliche Daten aus diesem Teil des Pakets zu entfernen -- über die Vorgabe [.guihint]#Pack only Low sensitivity files#.
Alle Dateien, die Sie hier auswählen können, stammen aus dem Verzeichnis `~/etc/checkmk` und seinen Unterverzeichnissen.
Eine genaue Auflistung sehen Sie direkt unter dem Dropdown-Menü.
...
image::support_diagnostics_sensitivity_levels.png[]
...
Die [.guihint]#Support diagnostics# nehmen Ihnen hier die Arbeit ab, eine ganze Reihe an Reporte manuell als PDF-Dateien zu erzeugen.
