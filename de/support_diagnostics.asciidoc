...
Sollten Sie in {CMK} mal mit Problemen konfrontiert sein, die Sie nicht selbsttätig -- unter Zuhilfenahme dieses Handbuchs -- lösen können, so bieten sich
Ihnen mit dem offiziellen link:https://checkmk.com/support.html[{CMK}-Support^] und dem link:https://forum.checkmk.com/[{CMK}-Forum^] zwei exzellente Anlaufstellen.
...
Die grundlegendsten Informationen stellen hierbei sicherlich die von Ihnen eingesetzte xref:cmk_versions#[Version] und link:https://checkmk.com/de/produkt/editionen[Edition^] von {CMK} dar.
Je nach Situation können jedoch wesentlich mehr Informationen erforderlich sein, um Ihrem Problem auf die Schliche zu kommen. In der Vergangenheit hat Ihnen der {CMK}-Support gesagt, welche Informationen Sie ihm zur Verfügung stellen müssen.
...
// TK: Ich weiß, das gehört nicht zum Thema, aber mit https://checkmk.com/de/werk/13752 gibt es jetzt eine neue Option Licensing Information. Bitte neuen Screenshot ziehen :-)
...
Würden Sie es dabei belassen -- also keine der weiteren Optionen auswählen -- und über [.guihint]#Collect diagnostics# die Sammlung der Daten starten, erhielten Sie eine Datei, die nur die folgenden Daten enthielte:
...
----
{
    ...
    "version": "2.2.0-2023.04.18"
}
----
// TK: Ich hab den Dateiinhalt aktualisiert.
...
Wählen Sie hier [.guihint]#Local Files#, erstellt {CMK} zusätzlich eine Übersicht aller Dateien, die im Verzeichnis `~/local/` Ihrer Instanz liegen.
Dies kann mitunter hilfreich sein, wenn Ihre lokalen Anpassungen zu einem kürzlich durchgeführten Update von {CMK} inkompatibel sind. Auch alle installierten MKPs werden hier mit erfasst.
...
Nach der Aktivierung einer der Optionen [.guihint]#{CMK} Log files# oder [.guihint]#{CMK} Configuration files# haben Sie die Möglichkeit, mehr oder weniger vertrauliche Daten aus dem jeweiligen Teil des Pakets zu entfernen -- über die Vorgabe [.guihint]#Pack only Low sensitivity files#.
Alle Dateien, die Sie hier auswählen können, stammen aus dem Verzeichnis `~/etc/checkmk` und seinen Unterverzeichnissen.
Eine genaue Auflistung sehen Sie direkt unter dem Menü.
...
// TK: In dem Screenshot bitte nur 1 Dateiliste zeigen (es geht ja nur ums Prinzip der Auswahl) und auch ein paar Dateien in die rechte Liste verschieben.
...
Die [.guihint]#Support diagnostics# nehmen Ihnen hier die Arbeit ab, eine ganze Reihe von Berichten manuell als PDF-Dateien zu erzeugen.
...
Im Unterpunkt [.guihint]#Global Settings# stehen Ihnen alle `global.mk`-Dateien der einzelnen Komponenten Ihrer {CMK}-Instanz, wie beispielsweise des Dynamic-Configuration-Daemons oder auch des Livestatus-Proxy-Daemons, zur Auswahl.
...
// TK: Warum hast Du den folgenden Abschnitt gelöscht? Warum ist das nicht mehr relevant?
////
...
Geben Sie auf dieser Seite am einfachsten `logging` in das Feld [.guihint]#Filter settings# ein und setzen Sie dann beispielsweise das Log-Level für den [.guihint]#Core# auf [.guihint]#Debug.#
...
Auch nach der Aktivierung einer der Optionen [.guihint]#Business Intelligence,# [.guihint]#CMC ({CMK} Microcore)# oder [.guihint]#Licensing information# haben Sie wieder die Möglichkeit, mehr oder weniger vertrauliche Daten aus dem jeweiligen Teil des Pakets zu entfernen -- über die Vorgabe [.guihint]#Pack only Low sensitivity files#.
Für den CMC ist die Auswahl unterteilt in [.guihint]#Core files# und [.guihint]#Log files#.

image::support_diagnostics_cmc.png[alt="Auswahl der Dateien für den CMC."]
// TK: Hier fehlt auch die Licensing information
...
|`--performance-graphs` |Performance-Graphen (bspw. CPU load, CPU utilization) des {CMK}-Servers.
|===
...
Im Besonderen die Informationen, die aus dem [.guihint]#HW/SW Inventory# des {CMK}-Servers stammen, wurden in älteren Versionen des Agenten noch gar nicht bereitgestellt.
...
Sollten Sie in einem erstellten Dump gewisse Daten vermissen, prüfen Sie bitte, ob Ihre {CMK}-Server über das Label `cmk/check_mk_server:yes` verfügen.
