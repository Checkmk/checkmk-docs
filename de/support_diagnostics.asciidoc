...
// TK: Mittlerweile steckt in der tar.gz nicht nur besagte general.json, sondern auch noch: pip_freeze.json, perfdata.json, hwinfo.json, general.json, file_size.csv, environment.json.
// TK: Damit nicht weiterhin der Eindruck vermittelt wird, dass nur die eine Datei general.json erzeugt wird, hab ich folgenden Absatz hinzugefügt.
...
Mit der Option [.guihint]#Crash Dumps# können Sie aus den verschiedenen Kategorien der Absturzberichte -- z.B. `base`, `check`, `gui` oder `rest_api` -- den jeweils letzten dem Dump hinzufügen lassen.
// TK: Ist crash dump = crash report = Absturzbericht?
...
Eine genaue Auflistung sehen Sie direkt unter der Liste.
