...
Übrigens erhalten Sie genauere Informationen zu den genannten und anderen wichtigen Snapins link:intro.html#monitoring_tools[im Kapitel zu den Monitoring-Werkzeugen] weiter unten.
...
image::quicksearch.png[width=50%]
...
* Die Suchanfrage können Sie in einem link:intro.html#bookmarks[Lesezeichen] speichern.
...
*Tipp:* Die [.guihint]#Quicksearch#-Suche steht Ihnen zusätzlich im Symbolmenü [.guihint]#Monitor# zur Verfügung. Für die Suche in der Konfigurationsumgebung können Sie die Suchfunktion im Symbolmenü [.guihint]#Setup# nutzen.
...
jenes mit dem Titel [.guihint]#Views#. Dieses Snapin ist standardmäßig nicht in der Seitenleiste enthalten. Sie können es aber einfach durch Anklicken von icon:button_sidebar_add_snapin[] aus der Liste der verfügbaren Snapins in die Seitenleiste aufnehmen.
...
All die *globalen* Statusansichten sind über das Snapin [.guihint]#Views# erreichbar. Die Ansichten sind dort zu Themen (_topics_) zusammengefasst, die einzel auf- und zuklappbar sind:
...
* Sie können zu anderen Ansichten navigieren, indem Sie bestimmte Zellen anklicken (im obigen Beispiel etwa den Host-Namen oder einen Service im Zustand {WARN}).
* Durch einen Klick auf einen Spaltentitel können Sie nach dieser Spalte sortieren.
* Der Toolbar-Knopf icon:view_button_filters[][.guihint]#Filter view# öffnet eine Suchleiste am rechten Rand, über die Sie die gezeigten Objekte filtern können.
...
50{nbsp}GByte des Dateisystems belegt sind, was ca. 76{nbsp}% ausmacht. Die Angaben sehen Sie in der Spalte [.guihint]#Status detail#. Der wichtigste Wert davon -- die Prozentangabe -- wird außerdem auf der rechte Seite in der Spalte [.guihint]#Perf-O-Meter# visualisiert.
...
Noch interessanter ist aber, dass {CMK} automatisch den Zeitverlauf aller solcher Messwerte standardmäßig für bis zu vier Jahren aufbewahrt. Innerhalb der ersten 48 Stunden werden die Werte minutengenau gespeichert. Dargestellt werden die Zeitverläufe in Graphen wie diesem, wie er z.B. für den Service [.guihint]#Check_MK# in den {CEE} dargestellt wird:
...
auf dem Nagios Addon PNP4Nagios und ist nicht interaktiv.
...
Sie haben Hosts aufgenommen, und wir haben uns wichtige Werkzeuge angesehen: Jetzt können wir loslegen mit dem eigentlichen Monitoring. Denn der Sinn von {CMK} ist es ja nicht, sich ständig mit der Konfiguration zu befassen, sondern eine Unterstützung beim IT-Betrieb
zu leisten.
...
In diesem Kapitel befassen wir uns nur mit den ersten und dem letzten Punkt. Die Alarmierung behandeln wir später link:intro.html#notifications[in einem eigenen Kapitel] -- aus guten Gründen, wie Sie noch sehen werden.
...
Rufen Sie zunächst eine Statusansicht auf, in der das Problem enthalten ist. Am einfachsten ist der Einstieg über die vordefinierten Statusansichten im Symbolmenü [.guihint]#Monitor# > [.guihint]#Problems# > [.guihint]#Host & service problems#, [.guihint]#Host problems# oder [.guihint]#Service problems#. Zu den letzten beiden gelangen Sie übrigens fast noch schneller, wenn Sie in der [.guihint]#Tactical overview# die Zahl der Probleme anklicken.

//Dazu klicken Sie sich bis zu den Details des Hosts/Services durch -- also der Ansicht mit dem Titel:
//LI:[.guihint]#Status of Host myhost123# im Falle eines Hosts und
//LI:[.guihint]#Service myhost123, FOO Service# im Falle eines Services.
...
Es ist gar nicht so selten, dass Sie eine Reihe (zusammengehöriger) Probleme auf einmal quittieren wollen. Das geht einfach, indem Sie sich  mit einem Klick auf den Toolbar-Knopf icon:checkbox_enable[] [.guihint]#Show checkboxes# eine neue erste Spalte in der Liste einblenden lassen, die eine Checkbox vor jeder Zeile  enthält. Die Checkboxen sind alle nicht markiert, denn die Auswahl treffen Sie: Markieren Sie die Checkbox für jeden gewünschten Host oder Service.
...
Klicken Sie nun auf den Toolbar-Knopf icon:view_button_commands[] [.guihint]#Acknowledge problems#. Dies blendet am Beginn der Seite den folgenden Bereich ein:

[{image-border}]
image::command_acknowledge.png[]

//Dieses öffnet eine Reihe von Eingabefeldern, über die Sie zahlreiche Aktionen auf dem dargestellten Host/Service ausführen können. Gleich das oberste Feld ist das gesuchte:
...
Die Einrichtung von Wartungszeiten ist sehr ähnlich zum Ablauf der Quittierung von Problemen: Sie starten wieder mit einer Statusansicht, in der das gewünschte Objekt (Host oder Service) enthalten ist, für das Sie eine Wartungszeit einrichten wollen. Zum Beispiel können Ihnen wieder die vordefinierten Statusansichten gute Dienste leisten: im Symbolmenü [.guihint]#Monitor# > [.guihint]#Overview# > [.guihint]#All hosts#, [.guihint]#Host search# oder [.guihint]#Service search#.

In der dann angezeigten Liste blenden Sie mit dem Toolbar-Knopf icon:checkbox_enable[] [.guihint]#Show checkboxes# die Checkboxen ein und wählen alle relevanten Einträge aus.

Klicken Sie nun auf den Toolbar-Knopf icon:view_button_commands[] [.guihint]#Schedule downtimes#. Dies blendet am Beginn der Seite den folgenden Bereich ein:
...
* Einen Überblick über aktuell laufende Wartungszeiten erhalten Sie im Symbolmenü [.guihint]#Monitor# > [.guihint]#Overview# > [.guihint]#Scheduled downtimes#.
...
Zum einen finden Sie  die Regelsätze im Symbolmenü [.guihint]#Setup# unter den Themen der Objekte, für die es Regelsätze gibt ([.guihint]#Hosts#, [.guihint]#Services# und [.guihint]#Agents#) in verschiedenen Kategorien. Für Services gibt es die folgenden Einträge mit Regelsätzen: [.guihint]#Monitoring rules#, [.guihint]#Discovery rules#, [.guihint]#Enforced services#, [.guihint]#HTTP, TCP, Email, ...# und [.guihint]#Other services#. Wenn Sie einen dieser Einträge auswählen, werden Ihnen die zugehörigen Regelsätze auf der Hauptseite aufgelistet. Das können eine Handvoll sein, aber auch sehr, sehr viele wie bei den [.guihint]#Monitoring rules#. Daher haben Sie auf der Ergebnisseite die Möglichkeit zu filtern: im Menü [.guihint]#View# > [.guihint]#Filter#.
...
Klicken Sie auf den Namen [.guihint]#Oracle Tablespaces#, so landen Sie in der Übersichtsseite des Regelsatzes:
...
Mehr zu den [.guihint]#Service labels#, dem letzten Parameter im obigen Bild, erfahren Sie übrigens im link:labels.html[Artikel zu Labels].
