...
=== Erster Überblick über die Oberfläche#guioverview
...
=== Hosts und Services, Agenten#hosts
...
=== Ordnerstruktur für Hosts#folders
...
Die Verwaltung von Ordnern und Hosts finden Sie im Modul icon:icon_folder.png[] [.guihint]#WATO|Hosts#,
...
wenn Ihnen das nicht gefällt, können Sie ihn mit dem Knopf icon:icon_edit.png[] [.guihint]#Folder properties#
...
drei Ordner [.guihint]#Windows}}, {{Linux}} und {{Network#. Legen Sie diese drei Ordner
an, indem Sie jeweils auf den Knopf icon:icon_newfolder.png[] [.guihint]#New folder#
...
=== Aufnehmen der ersten Hosts#linux
...
Gehen Sie also wieder in das Modul icon:icon_folder.png[] [.guihint]#WATO|Hosts# und wechseln Sie
dort in den Ordner [.guihint]#Linux,# indem Sie einfach die Grafik des Ordners anklicken.
Klicken Sie dort auf den Knopf icon:icon_new.png[] [.guihint]#New host.#
...
das Feld [.guihint]#Host name}} bei den {{Basic Settings.# Diesen Namen können
...
Knopf icon:icon_update.png[] [.guihint]#Update DNS cache# in dem Fenster der
...
Eigenschaften des Hosts mit dem Knopf icon:icon_diagnose.png[] [.guihint]#Diagnostic#
...
* wenn Sie in den Host-Eigenschaften oder auf einer anderen Seite des Hosts oben auf den Knopf icon:icon_services.png[] [.guihint]#Services# klicken.
...
=== Änderungen aktivieren#activatechanges
...
. In den Eigenschaften des Hosts im Kasten [.guihint]#Data sources}} setzen Sie {{Check_MK Agent}} auf {{No agent.#
. Im gleichen Kasten aktivieren Sie den Punkt [.guihint]#SNMP}} und wählen {{SNMP v2 or v3.#
...
=== Lesezeichen (Bookmarks)#bookmarks_snapin
...
=== Master Control#master_control_snapin
...
=== Anpassen der Seitenleiste#sidebar_customizing
...
== Finetuning des Monitorings#finetuning
...
=== Die regelbasierte Konfiguration#rules
...
=== Host-Merkmale (Host-Tags)#hosttags
...
Die Definition der Tag-Gruppen finden Sie im WATO-Modul icon:icon_tag.png[] [.guihint]#WATO|Tags#.
...
=== Schwellwerte für Dateisysteme#filesystems
...
* Die Parameter erlauben Schwellwerte, die von der Größe des Dateisystems abhängen. Wählen Sie dazu [.guihint]#Levels for filesystems|Levels for filesystem used space|Dynamic levels}}. Mit dem Knopf {{Add new element# definieren Sie jetzt pro Plattengröße eigene Schwellwerte.
...
Sie in [.guihint]#WATO|Host & Service parameters}} nach dem Regelsatz {{Host check command.# Legen Sie
...
neben [.guihint]#1 - up}} auch {{2 - down:#
...
Die Regelkette heißt [.guihint]#Network interfaces and switch ports.}} Aktivieren Sie die Option {{Operational state,#
deaktivieren Sie hier [.guihint]#Ignore the operational state}} und aktivieren Sie bei den {{Allowed states#
die Zustände [.guihint]#1 - up}} und {{2 - down# (und eventuell weitere Zustände).

=== Hosts, die regelmäßig gebootet werden#reboothosts
...
[CRE] In der {CRE} definieren Sie zunächst eine [.guihint]#Timeperiod,#
...
Ketten [.guihint]#Notification period for hosts}} und {{Notification period for services# für
...
=== Services dauerhaft ignorieren#disabled
...
=== Sporadische Fehler in den Griff bekommen#sporadic
...
=== Neue und weggefallene Services#discovery
...
[cols=<th style="width: 25%">Rolle</th>,<th style="width: 10%">Kürzel</th>,options="header"]
|===
...
|`admin`
|Ein Benutzer mit dieser Rolle darf alles. Seine Hauptaufgabe ist das generelle Konfigurieren von {CMK}, nicht das Operating. Dies schließt natürlich auch das Anlegen von Benutzern und Anpassen von Rollen ein.

|Normal monitoring user

|`user`
|Diese Rolle ist für einen „normalen“ Benutzer im _Operating._ Der darf grundsätzlich nur solche Hosts und Services sehen, für die er zuständig ist. Zudem besteht die Möglichkeit, dass Sie ihm das Recht geben, seine Hosts in WATO selbst zu verwalten.
...
=== Kontakte und Zuständigkeiten#contacts
...
icon:icon_contactgroups.png[] [.guihint]#Contact groups.# Eine Kontaktgruppe mit dem
...
Mit icon:icon_new.png[] [.guihint]#New contact group# legen Sie eine neue Gruppe
...
Knopf icon:icon_rulesets.png[] [.guihint]#Rules}} im Modul {{Contact groups.# Aber
...
. Aktivieren Sie im Kasten [.guihint]#Basic settings}} die Checkbox {{Permissions.#
. Wählen Sie im Kasten [.guihint]#Available}} eine oder mehrere Gruppen aus und verschieben Sie diese mit dem Pfeil nach rechts in das Feld {{Selected.#
...
=== Anlegen von Benutzern#users

Die Verwaltung der Benutzer finden Sie im WATO-Modul icon:icon_users.png[] [.guihint]#Users:#
...
Und falls Sie über den Knopf icon:icon_ldap.png[] [.guihint]#LDAP Connections#
...
Einen neuen Benutzer legen Sie mit dem Knopf icon:icon_new.png[] [.guihint]#New user# an. Diese Maske
...
Im ersten Feld geben Sie wie immer eine ID [.guihint]#(Username}}) und einen Titel {{(Full name)# ein -- hier
...
=== Mailversand vorbereiten#notify_init
...
* Wenn der Zustand in letzter Zeit zu oft zwischen verschiedenen Zuständen gewechselt hat und der Service deswegen als icon:icon_flapping.png[] _unstetig (flapping)_ markiert wurde. Dies kann schnell passieren, wenn Sie mittels [.guihint]#Fake check results# den Zustand ständig wechseln!
...
in der Statusoberfläche gehen und dort auf den Knopf icon:icon_history.png[] [.guihint]#History# klicken.
...
== Best Practises, Tipps & Tricks#best_practices
...
. Setzen Sie ferner [.guihint]#Data sources|Check_MK Agent}} auf {{No agent.#
...
. Aktivieren Sie außerdem die Option [.guihint]#Alternative address to ping}} und wählen Sie dort {{Ping all IPv4 addresses# aus.
...
. Wählen Sie dort im Feld [.guihint]#Host check command}} die Option {{Use the status of a service...# und tragen Sie den Service-Namen `Internet connection` ein, den Sie im 7. Schritt definiert haben.
...
. Wählen Sie bei [.guihint]#Data sources|Check_MK Agent}} die Einstellung {{No agent# und speichern Sie ohne Serviceerkennung.
. Legen Sie jetzt eine Regel im Regelsatz [.guihint]#Active Checks (HTTP, TCP, etc.)|Check HTTP service}} an, die für diesen Host greift (z.B. mit {{Explicit hosts# oder einem passenden Host-Tag).
...
=== Intelligente Schwellwerte für Dateisysteme#magicfactor
