...
[.relateddoc]
user_interface, dashboards, graphing
...
tabellarischen [.guihint]#Ansichten}}, welche in der englischen Oberfläche mit {{Views#
...
Seitenleistenelemente [.guihint]#Tactical Overview}} und {{Views#. In der Tactical
...
image::sidebar_quicksearch.png[center, width=280]
...
Seite [.guihint]#Services of host ...# dieses Hosts. Dort finden Sie dann wieder die Knöpfe
...
|Öffnet den Dialog mit den [.guihint]#Filtern#. Damit können Sie die gezeigten Daten weiter einschränken. Sobald Sie einen Filter gesetzt haben, ändert sich das Symbol auf ICON[button_filters_set_lo.png], damit klar ist, dass unter Umständen nicht alle Daten angezeigt werden.
 ...
 Änderungen der Filter werden nicht gespeichert, sondern wieder zurückgesetzt, sobald Sie die Ansicht verlassen.</td></tr> <tr><td>ICON[view_button_options.png]
|Öffnet die _Anzeigeoptionen_, mit denen Sie z.B. das Format von Zeitangaben (relativ oder absolut) einstellen können. Welche Optionen Ihnen angeboten werden, hängt von den jeweils in der Ansicht angezeigten Information ab.</td></tr>
 <tr><td>ICON[icon_commands.png]
|Hier können Sie link:commands.html[Kommandos] auf den gezeigten Objekten ausführen (z.B. Eintragen von Wartungszeiten). Die Kommandos sind detailliert in einem link:commands.html[eigenen Artikel] beschrieben.</td></tr>
 <tr><td>ICON[icon_checkbox.png]
|Damit schalten Sie Checkboxen ein oder aus. Mit den Checkboxen können Sie die Kommandos auf ausgewählte Datensätze einschränken.</td></tr>
 <tr><td>ICON[view_button_columns.png]
|Dieses Rädchen können Sie durch Klicken oder mit dem Mausrad drehen. Es legt die Anzahl der Spalten für die gewählte Ansicht fest. Mehrspaltige Ansichten helfen, auf breiten Monitoren den verfügbaren Platz auszunutzen. Bei Ansichten, die nur einen Datensatz anzeigen, ist diese Einstellung wirkungslos.</td></tr>
 <tr><td>ICON[view_button_refresh.png]
|Dieses Rädchen legt fest, in welchem Intervall die Ansicht neu geladen wird. Sie können damit das automatische Neuladen auch ganz abschalten. Bitte beachten Sie, dass es in diesem Fall sein kann, dass Sie über ein inzwischen aufgetretenes Problem nicht informiert werden!</td></tr>
|===
...
Sie die ICON[view_button_options.png] Anzeigeoption [.guihint]#Time stamp format# auf
...
[.guihint]#Check_MK Discovery}} und {{Check_MK HW/SW Inventory#.
...
|Dieses Rädchen legt fest, in welchem Intervall die Ansicht neu geladen wird. Sie können damit das automatische Neuladen auch ganz abschalten. Bitte beachten Sie, dass es in diesem Fall sein kann, dass Sie über ein inzwischen aufgetretenes Problem nicht informiert werden!</td></tr>
|===
 <table> <tr>   <th>Format</th>   <th>Vorgehen</th> </tr>
 <tr>   <td>PDF
...
image::views_limit1.png[center, width=80%]
...
image::views_limit2.png[center, width=80%]

Sie können die beiden Stufen unter [.guihint]#WATO|Glboal Settings}} unter {{User interface#
anpassen:

image::views_limit3.png[]

== Ansichten anpassen#edit
...
. Bei einer bestehenden View über den Knopf ICON[context_button_edit_view.png] (welcher evtl. hinter ICON[context_button_dots.png] versteckt ist).
. Im Seitenleistenelement [.guihint]#Views# über den Knopf ICON[button_view_snapin_edit.png]. Dort können Sie mit ICON[context_button_new.png] ganz neue Ansichten erzeugen oder mit ICON[button_clone.png] bestehende anpassen:
...
ersten Anpassen einer View (egal ob mit ICON[context_button_edit_view.png] oder
...
. Danach gelten Ansichten, die ein Adminbenutzer angepasst und freigegeben hat (genau gesagt jemand mit dem Recht [.guihint]#Modify builting views#).
...
Ihre Version? Dies wird über die [.guihint]#Unique ID}} in den {{General Properties#
...
* Als [.guihint]#Icon# für den Knopf ist ICON[icon_status.png] ausgewählt.
...
Tabelle, die nach Spalten sortierbar ist  --  oder [.guihint]#Single dataset#  --  welches
die Legende links hat und meist für einzelne Datensätze verwendet wird. Sie können
[.guihint]#Single dataset# aber auch für Ansichten mit mehr als einem Objekt verwenden.
Die Ansicht [.guihint]#All hosts}} sieht nach einer Umstellung auf {{Single dataset# etwa so aus:
...
hat. Bestes Beispiel ist die Ansicht [.guihint]#All hosts}}. Die Spalte {{Hostname#
...
dass bei einem der drei Hosts die Services [.guihint]#CPU load}} udn {{Memory# nicht
...
// Man sieht auch, dass bei drei von fünf Servern der Service [.guihint]#NTP Time#
...
Hier erscheinen dann unter [.guihint]#Column:}} die Spalten für Services und es wurde {{Perf-O-Meter# ausgewählt.
Der Eintrag [.guihint]#Titel}} bestimmt dann die Überschrift der Spalte. Unter {{Of Service# ist der
...
Die mitgelieferten Ansichten [.guihint]#Host search}} und {{Service search# (und
...
aufgebaut. In den meisten Fällen liegen Sie mit [.guihint]#All services}} oder {{All hosts# richtig.
...
|[.guihint]#Event Console#, Host- und Service-Events
|link:ec.html[Event Console]

|[.guihint]#Inventory#, diverse
|link:inventory.html[Inventory]

|[.guihint]#The Logfile#
|link:livestatus.html#logs[Livestatus-Daten]
...
==== Host- und Servicegruppen
Die Datenquellen
[.guihint]#Hostgroups}} und {{Servicegroups# liefern je Zeile die
...
[.guihint]#Hostgroups, merged}} und {{Servicegroups, merged# genau das Gleiche.
...
_Join_ der Tabelle [.guihint]#Hosts}} mit der Tabelle {{Hostgroups# sprechen. 
...
Tabelle [.guihint]#Services}} mit der Tabelle {{Hostgroups# und
...
image::service_filter_cpumemfs.png[center, width=300]
...
* Als [.guihint]#Basic layout}} ist {{Matrix# eingetragen.
* Bei [.guihint]#Grouping}} ist als einzige Spalte der {{Hostname# eingetragen.
* Bei [.guihint]#Columns}} sind die {{Service description}} und das {{Service Perf-O-Meter# eingetragen.
...
[.guihint]#Host icons}} und {{WATO folder - just folder name# hinzufügen:
...
==  Views in fremde Webseiten einbetten#embed_views
...
das ICON[icon_menu.png] [.guihint]#Action menu#, aus dem Sie Host- beziehungsweise Service-Aktionen
...
Sie [.guihint]#ID}}, {{Icon}} und einen {{Titel# fest; der Titel wird übrigens später
