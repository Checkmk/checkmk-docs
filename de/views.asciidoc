...
dass bei einem der drei Hosts die Services [.guihint]#CPU#, [.guihint]#Memory# und [.guihint]#Uptime# nicht
...
Unter [.guihint]#Of Service# wählen Sie zunächst entweder aus [.guihint]#Explicit match#
oder [.guihint]#Regular expression match#.
...
Wenn Sie nicht genau wissen, wie der Service heißt, oder er auf jedem Host ein wenig anders ist,
geben Sie einen regulären Ausdruck an, der pro Host auf exakt *einen* Service matcht.
Das ist vor allem dann nützlich, wenn Sie eine Gruppe von Hosts betrachten wollen,
bei denen es eine vergleichbare Gruppe an Services gibt,
von denen jeweils immer nur einer auf einem Host vorkommen kann.
In dem nachfolgenden Beispiel ist das die [.guihint]#CPU load# oder die [.guihint]#Processor queue#,
...
In beiden Varianten is so eine Darstellung natürlich nur dann nützlich,
wenn die Tabellenansicht eine Liste von ähnlichen Hosts anzeigt,
welche auch alle über die gewählten Services verfügen oder sich ähnelnde Services haben.
...
{CMK} bietet viele unterschiedliche Daten zu einem Host oder einem Service. Die meisten davon können Sie mit den Datenquellen [.guihint]#All services# oder [.guihint]#All hosts# darstellen. Die xref:inventory[HW/SW-Inventur] bietet darüber hinaus zahlreiche xref:inventory#table[Tabellen] (Datenquellen), die Sie auch in eigenen Ansichten anzeigen und individuell strukturieren können. Ab {v22} können Sie solchen Ansichten auch Spalten aus anderen Tabellen hinzufügen, sofern diese eine gemeinsame Spalte teilen.
...
In dem folgenden Beispiel werden wir einmal eine sehr kompakte Ansicht erstellen, in der zwei HW/SW-Inventur-Tabellen miteinander verknüpft werden. Erstellen Sie zum Anfang xref:views#new[eine neue Ansicht] und wählen als xref:views#datasource[Datenquelle] die grundlegende Inventur-Tabelle aus. Hier suchen Sie nach [.guihint]#Inventory table: Checkmk ➤ Checkmk sites# und bestätigen anschließen mit icon:button_continue[]:

image::views_create_view_3.png[]

Die darauf folgende Seite können Sie überspringen, da es nicht nötig ist, den Datenquelle auf einen einzelnen Host oder eine einzelne Instanz einzuschränken. Der nächste Schritt erfolgt also direkt in der Hauptseite der Konfiguration, indem Sie zunächst die [.guihint]#Unique ID# und einen [.guihint]#Title# eintragen.
...
image::views_create_view_4.png[]

Diese Makros können Sie nun in speziellen [.guihint]#Columns# verwenden. Neben den xref:views#columns[regulären], die bereits in der Anpassung von Ansichten verwendet wurden, stehen jetzt sowohl die ebenfalls vorher beschriebenen Spalten der xref:views#joined_columns[Service-Datenquelle] ([.guihint]#Joined columns#) zur Verfügung, als auch die [.guihint]#Joined inventory columns#.

Die ersten beiden Spalten nutzen die Standard-Daten aus der Datenquelle. Da Daten aus der HW/SW-Inventur immer zu einem bestimmten Host und Instanz gehören, stehen Ihnen diese Tabellen ebenfalls zur Verfügung, wenn Sie eine solche Datenquelle ausgewählt haben.

image::views_create_view_5.png[]
...
image::views_create_view_6.png[]

Daten aus anderen HW/SW-Inventur-Tabellen anzuzeigen funktioniert ganz ähnlich. Lediglich die zu verwendende Tabelle ist vorher auszuwählen. In dem nachfolenden Beispiel verwenden Sie die Tabelle [.guihint]#Checkmk ➤ Checkmk versions#, wodurch das Dropdown-Feld [.guihint]#Display the column# automatisch auf eine sinnvolle Auswahl eingeschränkt wird.

Wie Checkmk die richtige Zeile in der Tabelle findet, von der die Spalte angezeigt werden soll, bestimmen Sie danach. Fügen Sie dafür mit icon:button_add_new_match_criteria[] ein Kriterium in Form einer Spalte hinzu, über das {CMK} später die passende Zeile bestimmen kann. In dem obigen Beispiel ist das die [.gui]#Version#. Denn diese ist in der Tabelle einzigartig und kann nur einmal vorkommen. Das bedeutet umgekehrt auch, dass Sie zwar beliebige Tabellen der HW/SW-Inventur miteinander verknüpfen können, Sie dabei aber auch immer sicherstellen müssen, dass die beiden Tabellen immer eine gemeinsame Spalte haben und diese Spalte auch immer einzigartige Einträge enthält. In dem obigen Beispiel wäre die Spalte [.guihint]##Sites# also kein geeignetes Kriterium, da es mehrere Editionen mit derselben Anzahl an Sites geben könnte.

image::views_create_view_7.png[]

Selbstverständlich können Sie bei Bedarf auch bei den [.guihint]#Joined inventory columns# den Titel der Spalte anpassen, wenn der automatische nicht passen sollte. Nachdem alle Einstellungen vorgenommen wurden, speichern Sie mit icon:button_save_and_go_to_view[] die neue Ansicht ab und werden direkt zu dem Ergebnis geleitet. Je nachdem, wie viele Instanzen bei Ihnen überwacht werden, werden auch entsprechend für jede eine Zeile in der Ansicht eingefügt.

image::views_create_view_8.png[]

Wie Sie hier sehen können, gibt es für jede Spalte immer einen Standard-Titel, wenn dieser nicht von Ihnen vorgegeben wird. In der dritten Spalte zu dem [.guihint]#Perf-O-Meter# haben den Titel nicht vergeben, so dass von {CMK} der verwendete reguläre Ausdruck als Titel heranzgezogen wurde. Sie können das jederzeit in den Einstellungen ändern und wie bei der Spalte zu der [.guihint]#Edition# anpassen.
