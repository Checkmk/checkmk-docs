include::../attributes/global_attr.adoc[]
...
// ML: 2.1 und 2.4 von (alten) Icons auf Menüpunkte umgeschrieben.
...
|[.guihint]#Commands# |Hier können Sie xref:commands#[Kommandos] auf den gezeigten Objekten ausführen (z.&thinsp;B. Eintragen von Wartungszeiten). Die Kommandos sind detailliert in einem xref:commands#[eigenen Artikel] beschrieben.
|[.guihint]#Display > Filter# |Öffnet den Dialog mit den [.guihint]#Filtern#. Damit können Sie die gezeigten Daten weiter einschränken. Sobald Sie einen Filter gesetzt haben, ändert sich das Symbol auf icon:button_filters_set_lo[], damit klar ist, dass unter Umständen nicht alle Daten angezeigt werden. Umgekehrt haben manche Views schon Filter vorbelegt (z.&thinsp;B. die Liste aller Probleme). Hier können Sie durch Entfernen der Filter wiederum mehr Daten anzeigen lassen. Änderungen der Filter werden nicht gespeichert, sondern wieder zurückgesetzt, sobald Sie die Ansicht verlassen.
...
|[.guihint]#Reports > This view as PDF# |Nur {EE}: Hierbei wird ein sogenannter _Instant-Report_ erzeugt, der die aktuelle Tabellenansicht in ein PDF-Dokument überführt, abzüglich Icons, Navigation und so weiter. Das Aussehen können Sie über spezielle Schablonen im link:reporting.html[Berichtsmodul] anpassen.
...
anzeigt. Wenn es um einen ganz.&thinsp;B.stimmten Service von einem einzigen Host gehen
...
[cols="5,~"]
|===
|Aus |Was wird angezeigt?

| t |Titelzeile mit Überschrift, Menüleiste, Breadcrumbs, Kontext-Knöpfen.
| b |Kontext-Knöpfe und Menüpunkte, die zu anderen Ansichten verlinken.
| f |Knopf/Menüpunkt, der die icon:view_button_filters[] Filter öffnet.
| c |Knopf/Menüpunkt, der die icon:view_button_commands[] Kommandobox öffnet sowie Icons zum Ausführen von Kommandos.
| o |Die Option [.guihint]#Display > Modify display options# wird ausgegraut.
| d |Die Option [.guihint]#Display > Modify display options# wird entfernt.
| e |Menüpunkt [.guihint]#Display > Customize View#.
| r |Javascript-Code für das automatische Aktualisieren.
| s |Abspielen von xref:views#alarm_sounds[Alarmtönen] bei den Service-Zuständen {WARN} und {CRIT}.
| i |Links, die auf andere Ansichten zeigen
| m |Link-Ziel ist standardmäßig der HTML-Frame `main` -- mit dieser Option werden Links in neuen Tabs geöffnet.
| l |Links in Spaltentiteln.
...
`allhosts` so aus:
 ...
 /myserver/mysite/check_mk/view.py?view_name=allhosts&display_options=tbdezocf
...
Diese Aktion ruft schlicht die Standard-Host-Ansicht für den jeweiligen Host auf der Seite _mysite_
aufruft.
