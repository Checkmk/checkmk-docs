...
Wir unterscheiden zwischen *globalen* Ansichten und solchen, die einen
*Kontext* benötigen. Globale Ansichten können immer direkt aufgerufen
...
desjenigen Hosts, dessen Status angezeigt werden soll. Solche Ansichten können nur
...
|[.guihint]#Display > Filter# |Öffnet den Dialog mit den [.guihint]#Filtern#. Damit können Sie die gezeigten Daten weiter einschränken. Sobald Sie einen Filter gesetzt haben, ändert sich das Symbol auf icon:button_filters_set_lo[], damit klar ist, dass unter Umständen nicht alle Daten angezeigt werden. Umgekehrt haben manche Ansichten schon Filter vorbelegt (z.B. die Liste aller Probleme). Hier können Sie durch Entfernen der Filter wiederum mehr Daten anzeigen lassen. Änderungen der Filter werden nicht gespeichert, sondern wieder zurückgesetzt, sobald Sie die Ansicht verlassen.
...
Bei einer größeren Monitoring-Umgebung sind nicht mehr alle Ansichten sinnvoll.
...
. Bei einer bestehenden View über den Menüeintrag [.guihint]#Display > Customize view# -- womit ein Klon der Ansicht angelegt wird.
...
ersten Anpassen einer Ansicht wird automatisch eine Kopie der Ansicht für Sie erzeugt.
...
der Ansicht. Welche davon wird für wen sichtbar? Dies wird durch folgende
Regeln bestimmt:
...
gesteuert. Ändern Sie einfach die ID Ihrer Ansicht auf einen anderen Wert. Damit gilt diese nicht mehr als Klon der mitgelieferten Ansicht sondern beginnt ein eigenes
Leben.

Die ID ist übrigens auch die entscheidende Angabe in der URL, über die
Ansichten aufgerufen werden. Das Schema ist ganz einfach. So wird z.B. die
globale Ansicht mit der ID `allhosts` wie folgt aufgerufen:
...
=== Integration einer Ansicht in die Navigation

Wie und ob eine Ansicht im [.guihint]#Monitor#-Menü der Navigationsleiste
...
* [.guihint]#Topic in 'Monitor menu':# Unter diesem Thema wird die Ansicht einsortiert. Sie können auch eigene Themen definieren.
...
Der nächste Kasten [.guihint]#View Properties# bestimmt das generelle Aussehen der Ansicht:
...
das Aktualisierungsintervall -- beide Werte können in der Ansicht selbst [.guihint]#Display > Modify display options#
...
für sich anpassen. In der Konfiguration der Ansicht haben Sie aber mehr
...
Die Gruppierung in einer Ansicht konfigurieren Sie analog zu den Spalten.
...
Ein wichtiger Aspekt von Ansichten ist die _Selektion_ der Daten. Welche Hosts oder
...
UND-verknüpft. Die Filterkriterien, die bei einer Ansicht dann tatsächlich
zum Einsatz kommen, werden aus drei Quellen zusammengestellt:

. Filter, die direkt in der Ansicht hinterlegt und mit Kriterien gefüllt sind
. Filter, die der Benutzer interaktiv in der Ansicht mit icon:icon_filter[] gesetzt hat
. Filter, die per Variablen über die URL gesetzt werden

Die Filter, die Sie beim Editieren einer Ansicht im Kasten [.guihint]#Context / Search filters# zusammenstellen,
...
mit Kriterien vorbelegen und so die in der Ansicht gezeigten Daten einschränken (Punkt 1 von oben).

Falls Sie eine Ansicht mit *Kontext* anlegen oder editieren, so tritt anstelle der Filter
des entsprechenden Objekts nur ein optionales Eingabefeld. In diesem gilt dann immer
ein _exakter_ Vergleich (Groß-/Kleinschreibung beachten). Nehmen wir als Beispiel die Ansicht
`host`, welche alle Services eines _bestimmten_ Hosts anzeigt. Der Hostname wird
der Ansicht über einen Kontext mit auf den Weg gegeben. Sie können sich aber eine Ansicht bauen,
welche den gezeigten Host quasi *hart kodiert* direkt in der Ansicht festlegt:

image::views_edit_view_context_filter.png[]

Damit können Sie diese Ansicht wieder ohne Kontext aufrufen und auf Wunsch
...
Das Erzeugen einer neuen Ansicht mit icon:button_add_view[] geht im Prinzip genauso wie
das Editieren einer bestehenden Ansicht  --  mit einem kleinen Unterschied: Sie müssen vorher
...
Hier entscheidet sich, ob Ihre neue Ansicht einen Kontext haben oder ob es
...
In den ausgelieferten Ansichten gibt es eine, die dieses Layout verwendet  -- und
zwar [.guihint]#Search performance data#. Sie finden diese Ansicht
...
Wenn Sie in die Definition der Ansicht gucken, können Sie sehen, wie diese konstruiert ist:
...
Alarmtöne werden aber nicht in allen Ansichten abgespielt, sondern nur, wo dies in den [.guihint]#View Properties# aktiviert ist:
...
Elemente der Ansicht ergeben allerdings in so einem Kontext keinen Sinn oder sind
...
Bestandteile der Ansicht im HTML-Code *nicht* generiert werden sollen.

*Wichtig:* Auch die Navigation von {CMK} ist beim Einbetten meist eher unerwünscht.
Bevor Sie also in einer Ansicht manuell Optionen setzen, lassen Sie die Seite über
...
eigentliche Tabelle darstellen möchten, so sieht ein Link auf die Ansicht
`allhosts` so aus:
