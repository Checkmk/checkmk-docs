...
Kontext benötigt z.&thinsp;B. die Ansicht [.guihint]#Status of Host ...#  --  nämlich die Angabe
...
// TK: Warum nicht die Views, "die einen Kontext benötigen" kontext-abhängige Ansichten nennen? Mit dem Begriff kann man dann später weitermachen.
// TK: Verständnisfrage: Eine kontext-abhängige Ansicht ist immer eine, die beim Aufruf leer ist und mich in der Filterleiste zur Eingabe des Kontextes auffordert? Wahrscheinlich nicht...

Die globalen Ansichten erreichen Sie am einfachsten über die
Seitenleistenelemente [.guihint]#Overview# und [.guihint]#Views#. Im 
...
// TK: Statt des Snapin Views hier das Monitor-Menü als Defaultzugang vorstellen, auch mit Screenshot (monitor_menu_less.png). Snapin View nur als Randbemerkung.
...
wie z.&thinsp;B. die link:dashboards.html[Dashboards], welche unter dem Thema [.guihint]#Overview#
einsortiert sind. Dashboards können aber Views _enthalten._

image::views_snapin_views.png[width=42%, align=center]

// TK: Ab hier wirds verwirrend, da öfter nicht klar ist, auf welchen Screenshot sich ein Text bezieht.
// TK: Bsp: Der folgende Absatz beschreibt den Screenshot darunter und die Detailansicht wird erst im übernächsten Screenshot gezeigt. Besser erst den Screenshot zeigen, danach die Beschreibung.
...
Zellen sind anklickbar und bringen Sie zu Detailansichten:
// TK: Globale und kontext-abhängige Ansichten hatten wir schon: Was ist denn jetzt eine Detailansicht?
...
// TK: Auch den folgenden Einschub über die Suche finde ich mehr verwirrend als hilfreich - ganz davon abgesehen, dass hier auch die Suche im Monitor-Menü und nicht Quicksearch gefeatured werden sollte. 
// TK: Daher Passus streichen und stattdessen oben beim Monitor-Menü hinzufügen, dass man auch nach Host- und Service-Namen suchen kann (siehe user_interface.html#search_monitor) und als Sucheergebnis die entsprechenden Ansichten öffnen kann.
...
== Ansichten verwenden
// TK: Da die alte Beschreibung an der 1.6 GUI geklebt hat, passt es in diesem Kapitel an sehr vielen Stellen nicht mehr.
// TK: Das fängt schon im ersten Satz mit den sechs Symbolen an, die es nicht mehr gibt ... oder Icons am Kopf der Ansicht oder Fuß der Seite und vieles mehr. Die Unterkapitel 2.1 Optionen, Filter und Kommandos und 2.4. Export müssten komplett umgeschrieben werden. 2.1 geordnet nach den neuen Menüs und einzelne Einträge herausgehoben, die in der 1.6. noch in der Button-Leiste steckten.
...
|icon:icon_dashboard_filter[] |Öffnet den Dialog mit den [.guihint]#Filtern#. Damit können Sie die gezeigten Daten weiter einschränken. Sobald Sie einen Filter gesetzt haben, ändert sich das Symbol auf icon:button_filters_set_lo[], damit klar ist, dass unter Umständen nicht alle Daten angezeigt werden.  Umgekehrt haben manche Views schon Filter vorbelegt (z.&thinsp;B. die Liste alle Problem). Hier können Sie durch Entfernen der Filter auch mehr Daten anzeigen lassen.  Änderungen der Filter werden nicht gespeichert, sondern wieder zurückgesetzt, sobald Sie die Ansicht verlassen.
|icon:view_button_options[] |Öffnet die _Anzeigeoptionen_, mit denen Sie z.&thinsp;B. das Format von Zeitangaben (relativ oder absolut) einstellen können. Welche Optionen Ihnen angeboten werden, hängt von den jeweils in der Ansicht angezeigten Information ab.
|icon:icon_commands[] |Hier können Sie link:commands.html[Kommandos] auf den gezeigten Objekten ausführen (z.&thinsp;B. Eintragen von Wartungszeiten). Die Kommandos sind detailliert in einem link:commands.html[eigenen Artikel] beschrieben.
...
also z.&thinsp;B. [.guihint]#16 hrs#. Sie können das im Menü über
...
* Welche *Datenquelle* wird dargestellt (z.&thinsp;B., Hosts, Services, Events der Event Console etc.)?
...
. Bei einer bestehenden View über den Menüeintrag [.guihint]#Display > Customize view#.
. Im Menü [.guihint]#Customize > Visualization > Views#. Dort können Sie mit icon:button_add_view[] ganz neue Ansichten erzeugen oder mit icon:icon_clone[] bestehende anpassen:
// TK: Der einführende Satz klingt so, als ob es 2 Wege zur gleichen Ergebnisseite gibt. Aber mit 1. landet man direkt im Clone view und bei 2. in Edit Views. Solltest Du kurz erwähnen, dass bei 1. was anderes passiert.
...
Views aufgerufen werden. Das Schema ist ganz einfach. So wird z.&thinsp;B. die
...
////
ML: Obsolet, dürfte oben unter xref:menucontext[menucontext] genügend erklärt sein.
...
// TK: Der Absatz muss auch umgeschrieben werden. Rädchen sind gone.
...
Das Feld [.guihint]#Link# bietet eine Auswahl von allen Ansichten.
...
// TK: Unter Link kann man auch Link to dashboard auswählen
// TK: {CMK} {v16} rausnehmen
...
// TK: Tooltip kaum zu sehen, nur Ausschnitt zeigen?
...
völlig andere Informationen als z.&thinsp;B. bei USVs.
...
z.&thinsp;B. zuerst nach dem Service-Status, und bei gleichem Status nach dem Namen
...
Snapin [.guihint]#Overview# erreichen. Wie Sie sehen können, ist diese Tabelle
gruppiert nach dem Service-Status (_State_): zuerst alle {CRIT}, dann {UNKNOWN}, dann {WARN}:
...
anzeigt.
// TK: Ist der Gruppentitel im Screenshot zu sehen?
...
// TK: Ich lade jetzt mal hier einen Tipp zum Gruppieren ab, den ich bei meinem 1. CMK-Kurs aufgeschnappt habe und ziemlich pfiffig finde:
// Global Settings > Grouping of services in table views: Title to show for the group: OK und Grouping expression: .*
// Damit werden alle Services mit OK-Zustand in eine Gruppe gesteckt und verborgen und nur die Nicht-OK-Einträge sind sichtbar.
// Zitat aus Kursunterlage: "Das tolle an der Zusammenfassung: Nur OK-Services werden eingeklappt angezeigt, WARN- und CRIT-Services sehen Sie immer außerhalb der Gruppe."
// TK: Was hälst Du davon? Passt nicht wirklich an diese Stelle - aber an welche sonst? Zur Not, einfach den Kommentar stehenlassen.
...
abgeschickt wurde.
//TK: Ist das dann auch eine kontext-abhängige Ansicht?
...
|[.guihint]#Alert Statistics# |link:livestatus.html[Statusstatistiken]
...
* Als Verknüpfung in einer Spalte (siehe oben, z.&thinsp;B. Klick auf Hostname in einer Ansicht)
...
zwar [.guihint]#Metrics > Search performance data#. 
// TK: Wo steckt die denn? Hab ich nicht gefunden...
...
Wenn Sie z.&thinsp;B. mehr Informationen über den Host anzeigen möchten, fügen Sie einfach
weitere Spalten im Bereich [.guihint]#Grouping# hinzu. So sieht z.&thinsp;B. die Tabelle von
...
konstruierten Matrixansicht auf einen Blick feststellen können, ob z.&thinsp;B. bei
...
der nicht {OK} ist). Diese primitive Art der Alarmierung ist z.&thinsp;B. interessant
...
Webseiten einbetten  --  z.&thinsp;B. über einen `&lt;iframe&gt;`. Etliche
...
|===
// TK: Wann hat eigentlich zuletzt jemand diese Optionen ausprobiert? Ich hab ein paar Versuche gemacht mit H/h, T/t, O/o, Z/z, E/e: Es passiert NIE das, was beschrieben ist, sondern irgendetwas undefiniertes, dass zum Teil richtig scheußlich aussieht (d.h. ohne jede Formatierung). Übrigens auch schon in der 1.6. Keine Ahnung, was wir damit machen sollen ... wohl zuerst mal auskommentieren.

Wenn Sie also die z.&thinsp;B. alle Bedienelemente und Knöpfe abschalten und nur die
...
//TK: Obacht: Der Link wird ohne http:// angezeigt, ist aber scharf.
...
// TK: Vielleicht bereits hier den Screenshot von unten zeigen?
