...
[.relateddoc]
check_plugins_catalog, localchecks,  mkps
...
* Eine link:dev_guidelines.html#manpage[Manpage]
* Bei Plugins mit Checkparametern eine link:dev_guidelines.html#wato[WATO-Definition] für den zugehörigen Regelsatz
* link:dev_guidelines.html#metrics[Metrikdefinitionen] für Graphen und Perf-O-Meter, falls der Check Metrikdaten ausgibt
...
* Wenn es nur obere oder nur untere Schwellwerte gibt (häufigster Fall), dann sollen die Eingabefelder in WATO mit [.guihint]#Warning at ______}} und {{Critical at ______# beschriftet werden.
