...
{cee-only}
Ein Bericht in {CMK} ist eine Zusammenstellung unterschiedlicher Elemente (link:views.html[Tabellenansichten], link:availability.html[Verfügbarkeitstabellen], link:graphing.html[Verlaufsgraphen], Fließtext, Logos) zu einem druckfähigen PDF-Dokument.
...
Wie bei link:views.html[Tabellenansichten] und link:dashboards.html[Dashboards] ist es nicht möglich, mitgelieferte Berichte zu verändern.
