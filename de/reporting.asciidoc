...
== Berichte in Checkmk
// MA: Insgesamt ist das Kapitel sehr lang mit sehr vielen Unterpunkten. Was sind denn Berichte außerhalb von Checkmk? Werden die hier auch beschrieben? 
// MA: Hinweis: {CMK} und nicht Checkmk
...
// MA: Der Titel geht mir noch ins Ohr. Was soll das Kapitel denn leisten? Klingt für mich erstmal so, als wäre es in der Einleitung gut aufgehoben.
...
Berichte können, abhängig von ihrem Kontext und ihrer Bestimmung an verschiedenen Punkten in {CMK} aufgerufen werden. So lassen sich die dafür vorgesehenen Berichte beispielsweise aus dem Hauptmenü aufrufen.
...
image::reporting_main_menu.png[alt="Auswahl der Berichte aus dem Hauptmenü."]
...
image::reporting_exportmenu_original.png[alt="Ansicht der Berichte im Menü Export."]


==== Schnellaufruf der Berichte - Das Snapin Reporting in der Seitenleiste
Über das Snapin [.guihint]#Reporting# in der xref:user_interface#sidebar[Seitenleiste] erhalten Sie raschen Zugriff auf alle Berichte, die für die Anzeige in Hauptmenü und Seitenleiste vorgesehen wurden, und den xref:scheduler[Berichtsplaner] [.guihint]#(Scheduler):#
...
// MA: Hier kann ich nicht mehr folgen. Was sind denn Berichtseigenschaften? Heißt "Kontext vorgegeben" und "immer mit korrekten Informationen gefüllt", dass der Bericht statisch ist? Als sind die Informationen korrekt, weil sie immer gleich sind - unabhängig davon, von wo und wie der Bericht erstellt wird?

* Kontext nicht vorgegeben: Aufruf über das Menü [.guihint]#Export# in den verschiedenen Ansichten, der Kontext wird durch die (gefilterte) Ansicht geliefert.
...
Denken Sie nun nochmal an das oben erwähnte Snapin [.guihint]#Reporting#: Hierüber gelangen Sie ohne weitere Vorauswahl / Einschränkung etc. direkt zu den gerenderten Berichten - sinnvoll sind hier also eigentlich nur solche Berichte, die einen _eingebauten_ Kontext haben.
...
Berichte, deren Kontext erst über (gefilterte) Ansichten entsteht, können Sie daher getrost über die Option [.guihint]#Hide this report in the monitor menu# aus dem Hauptmenü sowie dem Snapin ausschließen.

image::reporting_general_properties_view2.png[alt="Auswahl zum Ausblenden des Berichts in Hauptmenü und Snapin."]
...
*Tipp:* Die in {CMK} vorhandenen xref:macros[Makros] (=Textbausteine) können Sie zum Beispiel auch nutzen, um noch eine Fußzeile mit Seitenzahl und Namen des Berichts zu erstellen. Wählen Sie dazu das Seitenelement [.guihint]#One line of text# und geben Sie im zweiten Schritt die Position am unteren Seitenende sowie den Text inklusive der gewünschten Makros an: 
...
Öffnen Sie mit [.guihint]#Scheduled reports > Add schedule# einen neuen Plan, wählen Sie dann den Bericht aus und klicken Sie icon:icon_save[alt="Symbol zum Fortfahren."] [.guihint]#Continue#.
...
Statt den Bericht zu versenden, können Sie diesen auch lokal ablegen. Wählen Sie dafür die [.guihint]#Action - Store locally#:

image::reporting_general_options_local.png[alt="Parameter für die lokale Ablage eines Berichts."]

Legen Sie dann einen Dateinamen und optional einen Berichtstitel fest. Um eine Historie der Berichte aufbauen zu können - anstatt bei jeder Erstellung des Berichts die Vorgängerversion zu überschreiben, bietet {CMK} diverse xref:macros[Macros] (=Textbausteine) an.
...
|Table of available text makros |Übersicht aller verfügbaren Makros |Name und aktueller Output des xref:macros[Makros] werden angezeigt
