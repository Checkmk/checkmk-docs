//encoding: utf-8
// IGNORE % Acceptor Aufträge Benachrichtigungs Sicherheits p22 p23 spooler unhandled Betaphase SP1 SP5 Mindest Monitoringdaten TBD pN
...
Während der Betaphase wird der Großteil der von {v21} unterstützten Distributionen unterstützt, um Upgrades zu erleichtern.
In der finalen {CMK} Version {v22} werden einige veraltete Distributionen dann nicht mehr unterstützt werden.
Falls Sie auf Nummer sicher gehen wollen, verwenden Sie zur (Neu-) Installation eines {CMK}-Servers eine Linux-Distribution von Debian, RHEL (und Derivaten Alma sowie Rocky Linux), SLES oder Ubuntu mit Long Term Support über das Jahr 2023 hinaus.

Während des gesamten Maintenance-Zeitraums der {v22} unterstützt werden Debian 10 und 11, RHEL 8 und 9, SLES 12 ab SP5 und 15 ab SP1, sowie Ubuntu 20.04 und 22.04.
Die Support-Zeiträume weiterer Versionen werden während der Betaphase bekanntgegeben.
...
Mit {CMK} {v22} werden keine Agentenpakete für Windows 6.0 (Vista und Server 2008 R1) mehr bereitgestellt.
Auf betroffenen Systemen kann der Agent von {v21} weiter genutzt werden.
...
*Was müssen Sie tun?* Identifizieren Sie die betroffenen Systeme und aktualisieren Sie die installierten Agenten auf das letzte verfügbare Patchlevel der {v21}.
Falls automatische Agenten-Updates konfiguriert sind, xref:agent_deployment#deactivate_automatic_updates[deaktivieren Sie diese] vor dem Update der {CMK}-Site.
...
*Betrifft Sie das?* Da es für {CMK} bei einem Update nicht möglich ist, die Kompatibilität von lokalen Anpassungen vollständig sicherzustellen, sollten Sie Ihre {CMK}-Instanz vor einem Update daraufhin überprüfen, ob lokale Dateien bei Ihnen verwendet werden und gegebenenfalls welche.

*Was müssen Sie tun?* Verschaffen Sie sich einen Überblick über nicht paketierte lokale Dateien Ihrer {CMK}-Instanz, indem Sie als Instanzbenutzer das folgende Kommando ausführen:
...
// {v20} Thema - auch gültig für {v21}
// MFS: Wie ist hier die Situation für {v22}?
[#check-api]
=== Die Check-API
...
Mit link:https://checkmk.com/de/werk/13320[Werk #13320^] wird aus dieser Empfehlung nun eine Voraussetzung -- und eine Sperre eingeführt, die zum Beispiel ein direktes Update von Version {v20} auf {v22} verhindert.
...
Dieses Mindest-Patchlevel kann bei {v22}pN höher liegen.
...
Beachten Sie, dass auch beim Update im verteilten Monitoring gilt: Hauptversionen dürfen nicht ausgelassen werden, sodass z.B. eine Verbindung zwischen einer {v20} und einer {v22} Instanz nicht unterstützt wird.
...
Am eigentlichen Update der Software hat sich in der {CMK} {v22} nichts Grundlegendes geändert, d.h. Sie installieren die neue Version, führen das Update der {CMK}-Instanz durch, kümmern sich um Konflikte (falls es denn welche geben sollte) und überprüfen und bestätigen die inkompatiblen Änderungen.
...
Seit {CMK} {v21} benötigen Hosts zur Durchführung der xref:agent_registration[TLS-Registrierung] Zugriff auf folgende Ports des {CMK}-Servers: Port 443 (respektive 80) für REST-API-Anfragen und Port 8000 für den neuen Agent Receiver.
Sprechen Sicherheitsrichtlinien dagegen, diese Ports verfügbar zu machen, können Sie bei Nutzung des _Pull-Mode_ ({CMK}-Site holt Monitoringdaten beim Host ab) eine xref:agent_linux#proxyregister[Registrierung im Auftrag] direkt auf dem {CMK}-Server durchführen.

{CMK} {v22} {CE} führt zudem einen _Push-Mode_ ein, mit dem Hosts Monitoringdaten zur Site schicken können.
Um diesen nutzen zu können, muss Port 8000 des {CMK}-Servers vom überwachten Host aus erreichbar sein.
...
Die Web-API (auch HTTP-API genannt) ist abgekündigt und wurde in {v22} xref:web-api[entfernt.]
