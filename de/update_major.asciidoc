...
Die Aktualisierung von Python von 3.10 auf 3.11 betrifft auch in einer Site nachinstallierte Module.
...
*Was müssen Sie tun?*
Finden Sie zunächst heraus, ob und – wenn ja –, welche Python-Module im Site-Kontext installiert sind, hierfür suchen Sie die `dist-info` und `egg-info` Ordner:
...
*Was müssen Sie tun?*
Finden Sie zunächst heraus, ob die bislang installierten Module bereits mit der neuen {CMK}-Version ausgeliefert werden, zum Beispiel:
...
Testen Sie anschließend die Check-Plugins und Spezialagenten, die auf im Site-Kontext installierte Module angewiesen sind.
