// -*- coding: utf-8 -*-
// IGNORE TBD p22 p23 pN STS mTLS p16 p20 p7
...
// TK: Ich bin mit dem ganzen Kapitel ziemlich unzufrieden:
// TK: Es ist ein Mix aus allgemeinen Überlegungen mit einer abschließenden Prozedur, die denn Update-Vorgang vorwegnimmt, obwohl wir ja noch in den Vorbereitungen sind.   
...
// TK: Für das Upgrade von FE nach SE gibt es ja nix zu beachten, da FE = SE mit Limitierung ist.
...
// TK: Hier fehlt der Fall, dass man den Upgrade ja auch machen soll, wenn man von der FE ohne Limits in die CE durchstarten will.
...
Im konkreten Fall führen Sie gleichzeitig mit dem _Update_ auf die Version {v22} ein _Upgrade_ auf eine andere Edition durch.
...
* Debian 9 _Stretch_
* Ubuntu in STS Versionen 21.10 _Impish_ und 22.10 _Kinetic_
* SLES 12 SP3 und SP4
...
Kontaktieren Sie gegebenenfalls den Hardwarehersteller für Updates.

[#agentlinux]
=== Der Agent für Linux

In {CMK} {v22} unterstützt der xref:agent_linux#[Linux-Agent] nicht länger Python 2.6 für Plugins.
// Auf betroffenen Systemen können die Agentenplugins von {v21} aber weiter genutzt werden.
// TK: Was heißt das? Der Linux-Agent der {v21} darf nicht auf {v22} aktualisiert werden - dann funktionieren auch die Python 2.6 Plugins weiter? 

*Betrifft Sie das?*
Nur, wenn bei Ihnen sehr alte Linux-Systeme im Einsatz sind (Python 2.6 ist von Oktober 2008 und wurde ab Juli 2010 von Python 2.7 abgelöst) _und_ Sie auf diesen in Python geschriebene Agentenplugins nutzen.
...
Sie können dann die einzelnen Komponenten Agentenscript, Plugins und lokale Checks – nach Kompatibilitätstests – manuell aktualisieren.
// TK: Die Deaktivierung ist auch in diesem Artikel beschrieben: xref:automatic_agent_updates
// Aktualisierungen des Agentenskripts sind immer noch manuell möglich.
// TK: Will man das wirklich? In einen 2.1 Agenten das Agentenskript der 2.2 packen? Und was hat man davon?
...
// TK Auch? nicht? Bei dem Satz hab ich nur große Fragezeichen.
...
// TK: Kann ich nicht testen, da ich keine ollen Plugins habe.
...
// TK: Im obigen Link taucht der Begriff Inline Flag aber gar nicht auf. Da heissen die Dinger Mode Modifier.
...
// TK: führten oder führen?
// TK: Was wurde denn genau abgekündigt? Inline Flags an sich oder nur, wenn sie *nicht* am Anfang verwendet werden? 
...
Benutzernamen in {CMK} dürfen nur Buchstaben, Ziffern und die folgenden Sonderzeichen enthalten: Bindestrich (`-`), Unterstrich (`_`), Punkt (`.`), At-Zeichen (`@`) und Dollarzeichen (`$`).
...
// TK: Ich würde hier *nicht* Check-API nennen, da dies ja eine offiziell unterstützte API ist, also nur: Dieses Thema kann Sie betreffen ...
// TK: Mein Verständnis: Alle APIs, für die API-Dokumentation im Help-Menü angeboten wird, werden unterstützt (REST-API, Check-API, Bakery-API,...). Alle anderen sind intern.
...
// TK: Als  "_intern_ gelten" ist schon richtig, siehe meine vorherigen Kommentare.
...
* Aufsplittung von GUI-Imports bezüglich Datasource Programs und aktiven Checks in mehrere Dateien (link:https://checkmk.com/werk/14716[Werk pass:q[#]14716^])
* Aufsplittung von GUI-Imports bezüglich der Verwendung des Passwortspeichers (_password store_) in mehrere Dateien (link:https://checkmk.com/werk/14719[Werk pass:q[#]14719^])
* Änderung des Geltungsbereichs der `Transform`-Klasse (link:https://checkmk.com/werk/14821[Werk pass:q[#]14821^])
* Feinere Granulierung des Zugriffs auf Rechte eingebauter Benutzer (link:https://checkmk.com/werk/14824[Werk pass:q[#]14824^])
* Ende der Unterstützung des Dashlet-Konfigurationsformats von {CMK} vor Version {v16} (link:https://checkmk.com/werk/14297[Werk pass:q[#]14297^])
...
Eine Auflistung finden Sie in den {EE} über [.guihint]#Setup > Maintenance > Extension packages.#
...
// TK: Folgenden Satz umformuliert:
...
{cee-only}
Wenn Sie in den {EE} die automatischen Agenten-Updates verwenden, sollten Sie überlegen, diese vor dem Update von {CMK} vorübergehend zu deaktivieren, um den Wechsel auf die neuen Agenten bei den Hosts später kontrolliert vollziehen zu können.
...
Insbesondere, wenn Sie die Aktualisierung im laufenden Betrieb anstreben, sollten Sie die Hinweise im allgemeinen xref:update#updatedistributed[Artikel zu Updates und Upgrades] lesen.
...
In {CMK} {v22} ist die Laufzeit der Zertifikate einstellbar und auf einen Standardwert von fünf Jahren eingestellt (siehe link:https://checkmk.com/de/werk/14964[Werk pass:q[#]14964^]).
...
// TK: Das liest sich so, als ob der Agent Controller auch ohne Verschlüsselung verwendet werden kann. Vielleicht besser:
// TK: ..., welche die TLS-Verschlüsselung mit dem Agent Controller verwenden ...
...
// TK: plus Hinweis, dass wir diesen Weg - obwohl nicht unterstützt - weiterhin beschreiben werden?
...
// TK: Zuerst wollte ich "Keine Konfiguration von Check-Plugins mit mk-Dateien" schreiben, aber im Werk wird explizit und immer wieder von den "default levels" geschrieben
...
// TK: Da die Check-API ja in unserem update_major 2.2. herausgefallen ist, nunmehr Link in die 2.1. Ok?
...
Der Prometheus-Agent bietet einige Checks als Datenquellen, die nicht mehr aktiv unterstützt werden.
...
Zudem wird in den Regeln [.guihint]#Prometheus# und [.guihint]#Alertmanager# die Angabe von IP-Adresse/Hostname, Port und Pfadprefix durch ein einziges Eingabefeld [.guihint]#Custom URL# abgelöst (siehe link:https://checkmk.com/de/werk/14573[Werk pass:q[#]14573^]).
