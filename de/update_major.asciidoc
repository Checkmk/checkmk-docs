...
Falls Sie verteiltes Monitoring mit einer xref:distributed_monitoring#distr_wato[zentralen Konfiguration] betreiben, aus der auch Erweiterungspakete an die Remote-Instanzen verteilt werden, haben Sie die Möglichkeit, in der {CMK} {v20} Zentralinstanz Pakete für die Remoteinstanzen – in kompatiblen Versionen für {v20} und {v21} vorzuhalten.

Bei der Verteilung werden dann automatisch die zur Remote-Instanz passenden MKPs verteilt.
Um dieses Feature nutzen zu können, müssen die {v20} Instanzen mindestens auf Patchlevel {v20}p33 gebracht werden, die {v21} Instanzen auf {v21}p21.

Sind alle Instanzen auf den angegebenen Mindest-Patchlevels, können Sie beginnen, auf der Zentralinstanz zusätzlich xref:#mkp_backport[Pakete für {v21}] hinzuzufügen.
...
*Betrifft Sie das?* Dies betrifft Sie, wenn Sie von der Funktion Gebrauch gemacht haben, MKPs xref:#mkp_backport[in verschiedenen Versionen] vorzuhalten.
