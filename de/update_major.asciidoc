...
// Es ist ein Mix aus allgemeinen Überlegungen mit einer abschließenden Prozedur, die denn Update-Vorgang vorwegnimmt, obwohl wir ja noch in den Vorbereitungen sind.   
...
// TK: Immer? Doch nur, wenn ich eine Distro habe, die oben nicht gelistet ist.
...
Unterstützt werden Chromium ab Version 101, Firefox ab Version 102 und Safari ab 16.
...
* Aufsplittung von GUI-Imports bezüglich der Verwendung des Passwort Store in mehrere Dateien (link:https://checkmk.com/werk/14719[Werk pass:q[#]14719^])
* Änderung des Scopes der `Transform`-Klasse (link:https://checkmk.com/werk/14821[Werk pass:q[#]14821^])
* Feinere Granulierung des Zugriffs auf Rechte eingebauter Nutzer (link:https://checkmk.com/werk/14824[Werk pass:q[#]14824^])
* Ende der Unterstützung des Dashlet-Konfigurationsformats von {CMK} vor {v16} (link:https://checkmk.com/werk/14297[Werk pass:q[#]14297^])
...
Insbesondere, wenn Sie die Aktualisierung im laufenden Betrieb anstreben, sollten Sie die xref:update#updatedistributed[Hinweise im allgemeinen Artikel zu Updates und Upgrades] lesen.
...
{CMK} {v21} nutzte für den Aufbau der mTLS-Verbindung Zertifikate mit de facto unbegrenzter Gültigkeit.
...
// TK: Da die Check-API ja in der 2.2. herausgefallen ist, nunmehr Link in die 2.1. Ok?
