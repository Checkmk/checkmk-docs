...
Bei Nutzung eines xref:#dist_setup[zentralen Setups im verteilten Monitoring] können Sie mit diesem Feature auf der Zentralinstanz Pakete für aktuellere Remote-Instanzen bereitstellen.
// TK: Zu viel information im obigen Satz und etwas verwirrender Link. Vorschlag:
////
...
// TK: Folgenden Satz hinzufügen, damit klarer wird, was man vor dem Update tun muss:
// TK: Sind diese Voraussetzungen erfüllt, können Sie auf der Zentralinstanz zusätzlich Pakete für {v21} hinzufügen.
...
Details der überarbeiteten Paketverwaltung auch im Setup zeigt der xref:mkps#[Artikel zur Verwaltung von Erweiterungspaketen (MKPs)]. 
// TK: "auch im Setup" würde ich hier weglassen.
...
Falls Sie verteiltes Monitoring mit einer xref:glossar#distributed_setup[zentralen Konfiguration] betreiben, aus der auch xref:glossar#mkp[{CMK}-Erweiterungspakete (MKPs)] an die Remote-Instanzen verteilt werden, haben Sie die Möglichkeit, in der {CMK} {v20} Zentralinstanz Pakete für die Remote-Instanzen – in kompatiblen Versionen für {v20} und {v21} vorzuhalten.
// TK: Was meinst Du hier mit "in kompatiblen Versionen"? Ggf. einfach weglassen.
Es werden dann automatisch die zur Remote-Instanz passenden MKPs verteilt.

Um dieses Feature nutzen zu können, müssen die {v20} Instanzen mindestens auf Patch-Version {v20}p33 gebracht werden, die {v21} Instanzen auf {v21}p21.
// TK: Muss es nicht konkreter so heißen:
// Um dieses Feature nutzen zu können, muß die Zentralinstanz auf Patch-Version {v20}p33 gebracht werden, und die Remote-Instanzen auf {v21}p21.

Sind auf allen beteiligten Instanzen die angegebenen Patch-Versionen installiert, können Sie beginnen, auf der Zentralinstanz zusätzlich xref:mkp_backport[Pakete für {v21} hinzuzufügen.]
Aktualisierte Remote-Instanzen erhalten und aktivieren dann automatisch das passende Paket.
...
Die Web-API (auch HTTP-API genannt) ist abgekündigt und wird in der nächsten {CMK}-Version xref:web-api[entfernt werden.]
