...
Manchmal ist es nicht so einfach, zu entscheiden, welches Werkzeug für das konkrete Problem am geeignetsten ist, denn:
...
Dieser Artikel soll eine Übersicht über die verschiedenen Strukturierungselemente in {CMK} geben und ihre Gemeinsamkeiten und Unterschiede erläutern.
Sie sollen dann alle Informationen zur Hand haben, um die für Ihre Umgebung passende Struktur aufzusetzen.
Wir wissen, dass momentan noch Aspekte fehlen, die für dieses Thema wichtig sind und werden versuchen die Lücken noch zu stopfen.
