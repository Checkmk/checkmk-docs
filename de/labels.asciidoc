...
{CMK} kann mit dem xref:dcd#[Dynamic host management] solche dynamischen
...
////
MA: An der Stelle hat mich Jens Dunkelberg von Abraxas mal auf etwas sehr interessantes
MA: aufmerksam gemacht: Labels können da sein - Hostmerkmale sind immer da. Und der
MA: Umstand, dass Labels so dynamisch sind und quasi ad hoc hinzugefügt werden können,
MA: eignen sie sich hervorragend für Checkmk-User. Merkmale eignen sich dagegen sehr gut
MA: für Eigenschaften, die *immer* da sein sollen und zudem von dem Checkmk-Admin
MA: (global) vorgegeben werden. Wir sollten uns hier klarer positionieren.
////
...
// MA: Alternativ kann man dan Service auch manuell anstoßen, so dass er direkt wieder OK wird.
...
////
MA: Checkmk schlägt inzwischen bereits bekannte Labels beim Eintippen vor. So kann man
MA: schnell erkennen, ob es das Label überhaupt gibt.
////
...
// MA: Der zweite Punkt ist nicht korrekt. Man kann Service-Labels als Condition in Regeln verwenden.
