// -*- coding: utf-8 -*-
// IGNORE hw kube Vorschlägen umkonfigurieren unmonitored
...
Falls das im Einzelfall Schwierigkeiten bereitet, ist es eventuell günstiger, wenn Sie mit Host-Merkmalen arbeiten 
- denn diese arbeiten mit definierten Werten anstelle von freier Texteingabe.
...
Sie können die Suche durch Hinzufügen weiterer Labels verfeinern (die dann wieder UND-verknüpft werden) 
und die Suche nach Labels natürlich auch mit den anderen verfügbaren Suchparametern kombinieren.
...
// und zudem in anderer Reihenfolge realisiert als bei den {CEE} und der {CRE}.
// TK: Was meinst du mit "in anderer Reihenfolge"? Ist die Erwähnung von CEE + CRE hier notwendig?
// MFS: Denke so passt es besser.
...
// So kann zwar beispielsweise das Betriebssystem leicht ermittelt werden, aber es bleibt dem {CMK}-Server verborgen, für welche Abteilung der neu im Monitoring angemeldete Host nun arbeitet.
...
Während der Registrierung können Sie anhand dieser Labels bestimmen, ob ein Host überhaupt im Monitoring angelegt werden soll, und – falls dem so ist – die Ordnerzugehörigkeit, aber auch andere Host-Attribute beeinflussen.
Nach abgeschlossener Registrierung kann nicht mehr auf Agenten-Labels zugegriffen werden.

// TK: Was Du hier motivierst und beschreibst, sind nur die benutzerdefinierten Agenten-Label ("Custom label").
// TK: Es gibt aber auch noch die (zurzeit 2) vordefinierten Agenten-Label cmk/os-family und cmk/hostname-simple
// TK: Ersteres hast Du oben kurz gestreift ("Betriebssystem leicht ermittelt"), letzteres ist für das Auslesen des Host-Namens und die darauf aufbauende Erstellung des Host-Namens in CMK very wichtig.
// TK: Diese beiden könnten/müssten dann auch in einem neuen Kapitel 7.2 Agenten-Label beschrieben werden.
// TK: Bin mir dabei nicht sicher, ob das wirklich notwendig ist, denn beide werden ja in hosts_autoregister bereits beschrieben: da, wo es wichtig ist.
Zwei vordefinierte Agenten-Labels werden immer bei der Registrierung übertragen:

* `cmk/os-family` enthält die Betriebssystemfamilie (derzeit _Windows_ oder _Linux_)
...
// TK: Etwas ungenau, da man ja auf beiden Seiten (Instanz und Agentenbäckerei) das gleiche Label eintragen muss. Führt aber hier zu weit, daher einfacher: 
// Sie können diese benutzerdefinierten Agenten-Label frei vergeben, beispielsweise:
...
// Soll ein bestimmtes Agenten-Label nach abgeschlossener Registrierung zu einem bestimmten Host-Label führen, ist das in den Regeln für die Autoregistrierung zu beachten.
// TK: Das versteh ich nicht. Meines Wissens kann ich aus einem Agenten-Label kein Host-Label zaubern.
// MFS: Es geht über Afenten-Label -> Anlegen in (temporärem) Ordner -> Host-Label -> verschieben, Label bleibt erhalten
// MFS: Zu doller Edge-Case? Weg damit?
// TK: Was passiert (aber unabhängig von Agenten-Labels) ist, dass der Host nach erfolgreicher Autoregistrierung automatisch das Host-Label cmk/agent_auto_registered:yes erhält
// MFS: So besser?
...
// TK: cmk/agent_auto_registered:yes müsste in Kap. 7.1 aufgenommen werden. 
// TK: Out of scope, aber egal: In 7.1 fehlt auch noch das Host-Label cmk/site. Weiss aber nicht, wo das herkommt.
...
|`cmk/vsphere_object` |`vm`, wenn der Host ein VMware vCenter ist; `server`, wenn der Host ein ESXi Host-System ist
|===
