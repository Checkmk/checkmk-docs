...
=== Bedingungen in Alarmierungen

Auch in link:notifications.html[Alarmierungen] können Sie Labels
als Bedingungen nutzen. Das funktioniert analog zu anderen
link:notifications.html#rule_conditions[Bedigungen], so dass Sie sich hier nicht
umgewöhnen müssen. Geben Sie einfach ein, welches Label oder welche Labels
ein Host oder Service haben muss, damit durch die Benachrichtigungsregel
eine Alarmierung ausgelöst wird. Auch hier werden mehrere Labels durch
_UND_-Verknüpfung verbunden.
