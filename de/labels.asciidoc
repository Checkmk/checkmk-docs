...
Die per xref:discovery_check[Discovery Check] gefundenen Labels können von jedem Spezialagenten (z.B. Kubernetes), einem xref:datasource_programs#[Datenquellenprogramm] oder der xref:inventory#[HW-/SW-Inventur] kommen. 
Der einfachste Weg, Label direkt zu beeinflussen, ist die Ergänzung eines xref:glossar#agent_plugin[Agentenplugins], 
