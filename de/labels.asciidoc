...
in der Schreibweise von Tags können nicht vorkommen, ebenso wenig Hosts,
...
Deswegen wurde mit den xref:automatic_labels[{CMK}-Labels] ein Konzept eingeführt,
...
* Labels sind nach dem Prinzip `Schlüssel:Wert` aufgebaut. Pro Schlüssel darf ein Host nur einen Wert haben. Also kann ein Host, der das Label `foo:bar` hat, nicht gleichzeitig `foo:bar2` haben. (Im Abschnitt xref:automatic_labels[Übersicht der automatisch generierten Labels] finden Sie eine Übersicht der Labels, die {CMK} automatisch generiert.)
...
Auf der Seite mit den Host-Eigenschaften, die angezeigt wird, wenn Sie im xref:user_interface#setup_menu[Setup] einen xref:hosts_setup#create_hosts[Host erstellen oder bearbeiten,] können Sie diesem beliebig viele Labels verpassen:
...
Führen Sie hier am besten die xref:hosts_setup#bulk_operations[Bulk-Aktion] zur Service-Erkennung durch ([.guihint]#Discover services#) und wählen Sie als Modus [.guihint]#Only discover new host labels# -- oder alternativ [.guihint]#Add unmonitored services and new host labels#, wenn Sie bei der Gelegenheit auch gleich neue Services aufnehmen wollen.
...
// MA: Können da wirklich zwei Doppelpunkte vorkommen? Also z.B. 'cmk/docker_image:docker.io/library/ngingx:latest'?
|cmk/docker_image_name |Name des Docker-Images, z.B. `nginx`
|cmk/docker_image_version |Version des Docker-Images, z.B. `latest`
|cmk/docker_object:container |Wird gesetzt, wenn der Host ein Docker-Container ist
// MA: Schwierig, weil hier in der Spalte 'Schlüssel' auch der Wert gesetzt wurde. Vielleiht eher aus dieser und der folgenden Zeile eine gemeinsame machen und die beiden möglichen Werte beschreiben?
|cmk/docker_object:node |Wird gesetzt, wenn der Host ein Docker-Knoten ist
|cmk/kubernetes/annotation/{key}:{value} |Diese Bezeichnungen werden für jede Kubernetes-Beschriftung ausgegeben, die ein gültiges Kubernetes-Label ist (über die Regel 'Kubernetes' konfigurierbar).
// MA: Die Bedeutung/Verwendung der Variablen {key} und {value} wurden noch nicht eingeführt, oder?
|cmk/kubernetes |Für alle Kubernetes-Objekte, Wert: `yes`
// MA: Die Beschreibung hier unterscheidet sich von der in der ersten Zeile. Weshalb?
...
|cmk/kubernetes/node |Name des zugehörigen Kubernetes-Knotens. Checkmk Hosts vom Typ Pod oder Node erhalten dieses Label.
|cmk/kubernetes/object |Kubernetes Objekttyp
|cmk/kubernetes/statefulset |Name des StatefulSet
|cmk/kubernetes_object:endpoint |Wird gesetzt, wenn Host ein Kubernetes Endpunktobjekt ist
// MA: Gleiches Problem, wie oben. Wert ist der Spalte für den Schlüssel
|cmk/os_family |Betriebssystem, vom Agenten als „AgentOS“ gemeldet (z.B. „windows“ oder „linux“)
|cmk/vsphere_object |`vm`, wenn der Host ein VMware vCenter ist, `server`, wenn der Host ein ESXi Hostsystem ist
// MA: Hier ist es genau so umgesetzt, wie oben von mir vorgeschlagen.
|===


=== Check-Plugins, in denen Labels registriert werden

// MA: Mit dem Absatz kann ich nichts anfangen. Welche Nutzerfrage soll er mir erklären?
Die folgenden Plugins werden verwendet, wenn mit dem {CMK} Spezialagenten für Prometheus die sogenannten Kube-state-metrics (Überwachung von Kubernetes) abgeholt werden:
...
Befüllt werden damit die Schlüssel cmk/kubernetes und cmk/kubernetes_object.
