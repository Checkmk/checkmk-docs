// -*- coding: utf-8 -*-
// IGNORE Vertipper Vorschlägen beeinflußen hw kube umkonfigurieren unmonitored
...
Die {CCE} verfügt über die Möglichkeit, xref:hosts_autoregister#[Hosts automatisch ins Monitoring] aufnehmen zu lassen.
Hierfür ist die gesamte Kette der Registrierung des Agenten, das Anlegen des Hosts und die Service-Erkennung automatisiert und zudem in anderer Reihenfolge realisiert als bei {CEE} und {CRE}.
...
Aus diesem Grund wurde das Konzept der flüchtigen Agenten-Labels (_Agent Labels_) eingeführt.
...
Während der Registrierung können Sie anhand dieser Labels die Ordnerzugehörigkeit, aber auch andere Host-Attribute beeinflußen.
