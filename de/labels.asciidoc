...
////
Sie können die Suche durch Hinzufügen weiterer Labels verfeinern und die Suche nach Labels natürlich auch mit den anderen verfügbaren Suchparametern kombinieren.

Die Label-Suche geht aber noch einen Schritt weiter: Zum einen können Sie nach Hosts fahnden, die die gesetzten Filter erfüllen, über die Vorgabe [.guihint]#is,# oder eben nicht erfüllen, über [.guihint]#is not.#

Zum anderen stehen Ihnen hier die booleschen Operatoren [.guihint]#NOT, AND, OR# zur Verfügung, um die exakte Teilmenge Ihrer Hosts zu definieren.
Wichtig: Die Operatoren werden in genau dieser Reihenfolge -- NOT, AND, OR -- abgearbeitet, unabhängig davon, an welcher Position sie in der Filterliste stehen.
Dies entspricht dem Standard der booleschen Algebra; beispielsweise würde `A OR B AND C NOT D AND E` folgender Klammersetzung entsprechen: `A OR (B AND (C NOT D)) OR E.`

image::labels_filter_boolean.png[alt="Filterleiste mit per logischen Operatoren verknüpften Label-Filtern.",width=60%]

Die Filter im obigen Bild würden also Hosts herausfiltern, die ein Linux-System sind und nicht „name1“ heißen sowie Windows-Systeme, die nicht headless und nicht französisch sind.
////

Hier können Sie nach Hosts fahnden mit diesem Label (über die Vorgabe [.guihint]#is#) oder ohne dieses Label (mit der Option [.guihint]#is not#).

Die Label-Suche geht aber noch einen Schritt weiter:
Um die Ergebnisliste Ihrer Hosts weiter zu verfeinern, stehen Ihnen zusätzlich die booleschen Operatoren [.guihint]#not, and, or# zur Verfügung.
Dabei ist [.guihint]#not# die Abkürzung für [.guihint]#and not#.

*Wichtig:* Die Operatoren werden in genau dieser Priorität -- [.guihint]#not, and, or# -- abgearbeitet, also _nicht_ notwendigerweise in der Reihenfolge, in der sie in der Filterliste stehen.
Dies entspricht dem Standard der booleschen Algebra. +
Beispielsweise würde `A and B not C or D` der Klammersetzung `pass:[((A and B) (not C)) or D]` entsprechen.
...
oder Sie erstellen mit [.guihint]#Add to query# eine neue Gruppe, was den nunmehr komplexeren Filter besser lesbar macht:
