...
{CMK} kann mit dem xref:dcd#[Dynamic Configuration Daemon] solche dynamischen
...
Regelbedingungen, Suchen, Auswertungen und andere Aufgaben verwenden.
...
* Labels werden mit einem Konnektor an xref:grafana#[Grafana] weitergereicht und können dort für Filterung, Aggregation, etc. verwendet werden.
...
Einem Host können auf drei verschiedenen Wegen Labels zugeordnet werden. 
Die erste davon ist einfach: 
Auf der Seite mit den Host-Eigenschaften, die angezeigt wird, wenn Sie im xref:user_interface#setup[Setup] einen xref:wato_hosts#labels[Host erstellen] oder bearbeiten, können Sie diesem beliebig viele Labels verpassen:
...
Da Labels ohne Vorgaben festgelegt werden können, und {CMK} daher nicht wissen kann, welche Labels
...
// TK: Umformuliert: wenn es bis jetzt noch nicht passiert ist, wird es wohl auch nicht mehr kommen.
//* Aktuell können Sie über Servicelabels noch keine Bedingungen formulieren. Dies wird jedoch in Kürze auch möglich sein.
* Sie können  über Service-Labels keine Bedingungen formulieren.


[#labels_grafana]
== Labels in Grafana

//TK: Umformuliert ("Aktuell wird für Grafana eine Datasource entwickelt")
Sie können eine link:https://github.com/tribe29/grafana-checkmk-datasource[Datasource] für Grafana nutzen, 
mit der Sie aus Grafana direkt auf die historischen Metriken von
{CMK} zugreifen können. Wenn Sie diese verwenden, erhält Grafana automatisch die
Information über alle Host- und Service-Labels. Damit können Sie {CMK}-Metriken in
