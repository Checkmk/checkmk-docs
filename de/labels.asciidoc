...
[#agent_plugins]
=== Label per Agenten-Plugin setzen
...
Der einfachste Weg, Label direkt zu beeinflussen, ist die Ergänzung eines Agentenplugins, das analog zu xref:localchecks#[lokalen Checks] eine Sektion `labels` erzeugt.
Sie können auf diese Weise Labels detaillierter als alleine durch Auswertung der HW-/SW-Inventur Labels vergeben – beispielsweise nach Nuancen der verbauten Hardware (wie CPU-Features) oder tatsächlich laufenden Prozessen (statt nur installierter Software).
Die Label-Ausgabe ist hierbei als Python-Dict zu formatieren.

.Beispiel-Agentenausgabe
[{file}]
----
<<<labels>>>
{"cpu/vendor": "zilog"}
...
|cmk/check_mk_server |Für alle {CMK}-Server, Wert: `yes`
...
|cmk/kubernetes/annotation/{Schlüssel}:{Wert} |Diese Bezeichnungen werden für jede Kubernetes-Beschriftung ausgegeben, die ein gültiges Kubernetes-Label ist (über die Regel 'Kubernetes' konfigurierbar).
...
Die Kubernetes-bezogenen Schlüssel cmk/kubernetes und cmk/kubernetes_object können Sie alternativ auch über die folgenden Plugins füllen, wenn mit dem {CMK} Spezialagenten für Prometheus die sogenannten Kube-state-metrics abgeholt werden:
