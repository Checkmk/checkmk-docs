...
Hierfür ist die gesamte Kette der Registrierung des Agenten, das Anlegen des Hosts, die Service-Erkennung und die Aktivierung der Änderungen automatisiert und zudem in anderer Reihenfolge realisiert als bei den {CEE} und der {CRE}.
...
Aus diesem Grund wurde das Konzept der flüchtigen Agenten-Labels eingeführt.
...
Sie können diese beim Backen eines Paketes frei vergeben, beispielsweise:
...
Während der Registrierung können Sie anhand dieser Labels die Ordnerzugehörigkeit, aber auch andere Host-Attribute beeinflussen.
