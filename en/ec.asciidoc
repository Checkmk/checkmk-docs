...
* Messages can xref:ec#cancelling[cancel] current events.
* 'Artificial' events can be created if particular messages xref:ec#expect[do not] come.
...
Possible defined xref:ec#actions[Actions] will actually be executed.
...
views, events and xref:ec#archive[event history] are available as data sources. Detailed information
...
xref:ec#actions[actions] on events.
...
More information concerning notifications through the EC will be explained xref:ec#notifications[below].
...
xref:ec#archive[Archive], all of this information can be accessed later at any time.
...
* If the host is not *not* found, the contact groups – xref:ec#contactgroups[if configured there] – will be adopted from the rule that generated the event.
...
// MFS: ✂ -- ✂ -- ✂ -- ✂ -- ✂ -- ✂ -- ✂ -- ✂ -- ✂ --
// This block is deprecated and should be removed during translation:
...
// MFS: To be consistent, we probably should use "rule packs"
...
messages, if in the xref:ec#globalsettings[Settings for the EC] you switch
...
* The rewriting of events (xref:ec#rewriting[Rewriting])
* The automatic cancelling of events (xref:ec#cancelling[Cancelling])
* The counting of messages (xref:ec#counting[Counting])
...
For more on the rule packs, see xref:ec#rulepacks[later below].
...
The contact groups are also used for the xref:ec#visibility[visibility], and from Version
{v14} also used for event
xref:ec#notifications[Notification]. Here you can assign contact groups explicitly
by using Rule Events. Details for this can be found in the xref:ec#visibility[section on operation].
...
With the rewriting, the xref:ec#matchgroups[Matchgroups] described above are
...
is captured by one member of a xref:ec#matchgroups[matchgroup].
...
This does _not_ work! This placeholder only functions with xref:ec#rewriting[rewriting].


==== Executing actions when cancelling

When cancelling an event you can also allow xref:ec#automatic_actions[actions]
...
If you work with rewriting and xref:ec#cancelling[cancelling] in the same rule,
...
* Host
* Application
* xref:ec#matchgroups[Match groups]
...
automatic xref:ec#cancelling[cancelling] of events. Set a delay of
...
* Creation of {CMK}-xref:ec#notifications[notifications]
...
xref:ec#visibility[visibility of events]. If such a host can be found,
...
|[.guihint]#Date / Time# |With events, the xref:ec#counting[counting], is the time of the _last_ occurrence of a message associated with an event.
...
xref:ec#commands[Commands]. More interesting is the automatic execution of
...
With the use of xref:ec#counting[counting] or
...
As described in the section on xref:ec#setup[setting up] the Event Console,
...
|CANCELLED |The event was automatically xref:ec#cancelling[cancelled] following an OK-message.
...
in xref:ec#globalsettings[Settings for the EC].
...
. In the xref:ec#globalsettings[EC's global settings], activate the [.guihint]#Force message archiving# option.
...
* Work with xref:ec#rulepacks[rulepacks] to bundle related rules. The first rule in each pack should immediately exit the pack if the common basic condition is not satisfied
...
|per Host |The host contacts, which are identified in exactly the same way as with the xref:ec#notifications[notification of events] in {CMK}.
...
As shown xref:ec#archive[above] the Event Console has an archive of all events and
...
performance data in curves, and which also warns of xref:ec#overflow[overflow].
...
|history |Access to the xref:ec#archive[archive]. A query to this table directs to an access of the archive's text data. Definitely use a filter for the time range of the desired entry to avoid a full accessing all of the files.
...
|ACTION |Executes a user-defined action on an event. Arguments are event-ID, user-ID and actions-ID. The special `@NOTIFY` ID corresponds to a xref:ec#notifications[notification] over {CMK}.
|===
...
|var/mkeventd/history/ |The xref:ec#archive[EC-Archive's] storage location
