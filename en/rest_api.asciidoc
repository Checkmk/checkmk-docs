...
:revdate: draft
:title: Configuration via the Checkmk REST-API
:description: The Checkmk REST-API is a new interface with many advantages for transferring and executing tasks -- which you normally perform in Checkmk via the GUI -- to the Checkmk server via a command or script with HTTP requests.
...
As of Version VERSION[2.0], there is a new application programming interface in
...
In the above example, you can see that there are pending changes in Checkmk,
