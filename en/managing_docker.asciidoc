...
:description: Following the start of a docker container is its ongoing maintenance. This article describes what peculiarities should be noted when {CMK} runs in a container.
...
In {CMK}, when xref:intro_setup#create_site[setting up a site] a random password for the default user `cmkadmin` is created. 
