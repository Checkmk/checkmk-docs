...
|[.guihint]#Reports > This view as PDF# |Only in the commercial editions: This creates a so-called _instant report_, which converts the current view into a PDF document, minus icons, navigation and so on.
...
Here then under [.guihint]#Column# the columns for services will appear, from which you select [.guihint]#Perf-O-Meter# in this example.
...
As of {CMK} {v22} you can also add columns from other tables to such views, as long as they have a column in common.
...
image::views_create_view_3.png[alt="Selecting the 'Checkmk sites' data source."]
...
image::views_create_view_5.png[alt="Dialog for selection of standard columns."]
...
image::views_create_view_6.png[alt="Using a macro to select a 'Joined column'."]
...
image::views_create_view_7.png[alt="Using a macro to select a 'Joined inventory column'."]
