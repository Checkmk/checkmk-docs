...
Alternatively you can use the sidebar elements [.guihint]#Overview# and
[.guihint]#Views.# In the overview each number is clickable and takes you to a
...
In views that relate to a specific host or service, you will find all sorts of entries in the menu [.guihint]#Host# respectively [.guihint]#Service#, which will then take you to other views with the same context -- i.e. of the same host or service.

image::views_context_buttons.png[]

You can also access host- and service-specific views by searching the monitor menu:
...
Views of a single host, on the other hand, contain the menu items [.guihint]#Host# and [.guihint]#Services# with all actions for the current host and all services of the current view.
...
|[.guihint]#Display > Filter# |Opens the dialogue with the [.guihint]#filters#. With this you can further limit the displayed data. As soon as you set a filter the symbol on icon:button_filters_set_lo[] changes so that it is clear that under certain circumstances not all data will be will be displayed.  Conversely, some views already have predefined filters, (e.g. the list of all problems). Here, by removing the filters you can also have more data displayed.  Changes to filters are not saved, rather they are reset when you quit the view.
|[.guihint]#Display > Show checkboxes# |With this you switch checkboxes on or off. With the checkboxes you can restrict the commands to selected data sets.
|[.guihint]#Display > Modify display options > Number of columns# |Defines the number of columns for the selected view. Views with multiple columns allow the space available on wider monitors to be exploited. For views that only display a single data set this setting has no effect.
...
|[.guihint]#Display > Modify display options > Time stamp format# |Timestamps can be displayed absolute, relative, mixed, together or as Unix Timestamp.
|===
...
|[.guihint]#Data > Export JSON# |For the JSON export, the cells also end up in quotes, separated by breaks and commas, enclosed by square brackets.
|[.guihint]#Reports > This view as PDF# |Only {EE}: This creates a so-called _Instant-Report_, which converts the current table view into a PDF document, minus icons, navigation and so on. Its layout can be customised with special templates in the link:reporting.html[report module].
...
. From an active view via the menu item [.guihint]#Display > Customize view#-- wherewith a clone of the view is created.
...
[.guihint]#Service problems# view, which is simply reached via the snapin
[.guihint]#Overview#.
...
with a click on icon:icon_filter[]. Secondly, you can predefine
...
*Important:* The {CMK} navigation is usually rather undesirable when embedding, too.
So before you manually set options in a view, let the page be
displayed without navigation and sidebar via
[.guihint]#Display > This page without navigation.#

Every component is coded with a letter and can be excluded accordingly.
...
| t |Title bar (with heading, breadcrumbs, countdown symbol for page update), menu bar and actions bar are removed.
| b |Menu items and action buttons that link to other views are removed.
| f |Menu item [.guihint]#Display > Filter# and its corresponding icon:view_button_filters[] button are removed.
| c |Menu item [.guihint]#Commands# and its corresponding buttons are removed.
| o |The options [.guihint]#Number of columns# and [.guihint]#Refresh interval# are removed.
| d |Menu item [.guihint]#Display > Modify display options# is removed; otherwise callable via [.guihint]#Display > Modify display options.#
|e |Menu item [.guihint]#Display > Customize View# is removed.
| r |The Javascript code for the automatic refresh and the corresponding icon are removed.
| s |The playing of xref:views#alarm_sounds[alarm sounds] for the {WARN} and {CRIT} service states is suppressed.
| i |Links to other views are removed, e.g. the links of the column [.guihint]#Service# in service views.
| m |With this option links are assigned the `main` HTML-frame as their target -- and are opened in a new tab.
| l |Links in column headings are removed.
...
settings under [.guihint]#Steup > General > Global settings > User Interface > Custom icons and actions#.
