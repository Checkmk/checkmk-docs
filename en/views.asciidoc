...
|[.guihint]#Display > Modify display options > Time stamp format# |Timestamps can be displayed absolute, relative, mixed, together or as a Unix Timestamp.
|===
...
. From an active view via the menu item [.guihint]#Display > Customize view# -- with which a clone of the curent view is created.
...
is ‘greyed-out’ -- overlaid by your new version in effect. You can return to
