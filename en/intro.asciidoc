...
=== Selecting a {CMK} Edition#editions
...
=== The first overview of the interface#guioverview
...
=== Hosts and services, agents#hosts
...
=== Host folder structures#folders
...
the ICON[icon_folder.png] [.guihint]#WATO|Hosts# module, which you can reach via
...
[.guihint]#Windows}}, {{Linux}} and {{Network#.  Create these three folders by
clicking the ICON[icon_newfolder.png] [.guihint]#New folder# button and in the first
...
Now go back to the ICON[icon_folder.png] [.guihint]#WATO|Hosts# module and there
switch to the [.guihint]#Linux# folder by simply clicking on the folder’s graphic.
Click on ICON[icon_new.png] [.guihint]#New host#.
...
ICON[icon_update.png] [.guihint]#Update DNS cache# button in the Host Properties
...
in the Host Properties, by using the ICON[icon_diagnose.png] [.guihint]#Diagnostic#
...
* by clicking on the ICON[icon_services.png] [.guihint]#Services# button in the host properties, or at the top of any other page for the host
...
=== Activating changes#activatechanges
...
. In the host’s properties in the [.guihint]#Data sources}} box, set {{Check_MK Agent}} to {{No agent#.
. In the same box activate [.guihint]#SNMP}}, and select {{SNMP v2 or v3#.
...
=== The Tactical Overview#tactical_overview
...
=== The Master Control#master_control_snapin
...
=== Customizing the sidebar#sidebar_customizing
...
[.guihint]#Tactical Overview}} -- the one titled {{Views#.  A view is a status display that
...
with [.guihint]#Quicksearch}}, and the {{Services|Service Search# view is somewhat
more flexible.
...
== Fine tuning Monitoring#finetuning
...
=== Rules-based Configuration#rules
...
[.guihint]#Criticality}} and {{Networking Segment# are left alone.  It is
...
=== Thresholds for file systems#filesystems
...
* The parameters allow thresholds that depend on the size of the file system. Select [.guihint]#Levels for filesystems|Levels for filesystem used space|Dynamic levels}}. With the {{Add new element# button you can now define your own threshold values appropriate to each drive's capacity.
...
it in [.guihint]#WATO|Host & Service parameters}} under the {{Host check command#
...
[.guihint]#Network interface port states to discover}} alongside {{1 - up# and
[.guihint]#2 - down#:
...
[.guihint]#Network interfaces and switch ports}}. Activate the {{Operational state# option,
uncheck [.guihint]#Ignore the operational state}}, and in {{Allowed states# check
the states [.guihint]#1 - up}} and {{2 - down# (and possibly other states if needed).


=== Hosts that are rebooted regularly#reboothosts
...
[.guihint]#Notification period for hosts}} and {{Notification period for services# rule chains for
...
=== Permanently ignore services#disabled
...
=== Getting a grip on sporadic errors#sporadic
...
=== New and discontinued services#discovery
...
=== Contacts and Responsibilities#contacs
...
ICON[icon_contactgroups.png] [.guihint]#Contact groups# WATO module.  A contact
...
Use ICON[icon_new.png] [.guihint]#New contact group# to create a new group.  Here,
...
the ICON[icon_rulesets.png] [.guihint]#Rules}} button in the {{Contact groups#
...
. In the [.guihint]#Basic settings}} box check the {{Permissions# checkbox.
. Select one or more groups in the box [.guihint]#Available}}, and move them to the right with the arrow buttons, into the {{Selected# field.
...
If you have discovered the ICON[icon_ldap.png] [.guihint]#LDAP Connections#
...
Create a new user with the ICON[icon_new.png] [.guihint]#New user# button.  This form
...
name of the user.  The [.guihint]#Email address}} and {{pager address# fields are
...
=== Preparing email dispatching#notify_init
...
* If a service has recently been changing between different states too often, and the service has thus been marked as ICON[icon_flapping.png] _flapping_! This can happen quickly if you constantly change the state using [.guihint]#Fake check results#!
...
and click on ICON[icon_history.png] [.guihint]#History#.  There you will find all
...
== Best Practices, Tips & Tricks#best_practices
...
. Also set [.guihint]#Data sources|Check_MK Agent}} to {{No agent#.
...
. Also enable [.guihint]#Alternative address to ping}}, and select {{Ping all IPv4 addresses#.
...
. In the [.guihint]#Host check command}} field, select {{Use the status of a service ...#, and enter the service name `Internet connection` which you defined in step 7.
...
. In [.guihint]#Data sources|Check_MK Agent}}, select {{No agent# and save it without service detection.
. Now create a rule in the [.guihint]#Active Checks (HTTP, TCP, etc.)|Check HTTP service}} rule set for this host (eg with {{Explicit hosts# or an appropriate host tag).
...
=== Intelligent file system thresholds#magicfactor
