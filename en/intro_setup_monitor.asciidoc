...
For the command you need the names of the {CMK} server (`mycmkserver` in the example), the {CMK} site (`mysite`) and the host (`localhost`) as set up in {CMK} xref:create_host[in the preceeding chapter].
