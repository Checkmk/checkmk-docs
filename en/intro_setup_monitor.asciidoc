...
It won't be able to report its own total failure of course, but this is still useful since not only does it give you an overview of CPU and RAM usage, but also a number of metrics and checks concerning the {CMK} system itself.
...
After installing the agent on the host, you can add the host in {CMK}'s configuration environment  
...
Therefore, {CMK} only shows the most important host attributes in the form -- those that are necessary to create a host.
...
Detailed information on host names, IP addresses and DNS can be found in xref:wato_hosts#dns[the article on host administration].
...
We have shortened the server certificate to be confirmed here in this example for clarity:
...
By the way, you can find more possibilities for diagnostics in the xref:agent_linux#test[Linux agent] article.
...
If you click [.guihint]#Monitor undecided services#, all the services will be directly added to the monitoring and all the [.guihint]#Undecided# services will become [.guihint]#Monitored# services.
