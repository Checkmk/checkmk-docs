...
image::empty_main_directory.png[View of main directory without folders and hosts.]
...
the action bar with the icon:button_hide_toolbar[Icon for hiding the action bar.] button to the right of the
[.guihint]#Help# menu and show it again with icon:button_show_toolbar[Icon for showing the action bar.]. When the
action bar is hidden, the icons are displayed in the menu bar to the right of
icon:button_show_toolbar[Icon for showing the action bar.].
...
create a folder (e.g. the icon:icon_newfolder[Icon for creating a folder.] [.guihint]#Add subfolder# button).
On the new page [.guihint]#Create new folder# enter the folder name in the first box [.guihint]#Basic settings#:

image::folder_basic_settings.png[Properties when creating a folder: Basic settings with title.]
...
image::three_empty_folders.png[Main directory with three folders, one of them expanded with icons for folder actions.]
...
image::agent_download_cee.png[List of Linux agents for download in the {EE}.]
...
image::agent_download_page.png[List of Linux agents for download in the {RE}.]
...
image::host_allsettings_less.png[Properties when creating a host.]
...
icon:button_showmore[Icon for switching to Show more mode.] at each of the open boxes and opening the two collapsed
...
image::host_diagnostics.png[Result of the connection test to the host with ping and with agent output.]
...
image::intro_setup_monitor_services_of_host.png[Services found on the host to add to monitoring.]
...
image::activate_changes.png[List of accumulated changes for activation.]
...
image::host_snmp_configuration.png[Properties when creating a host via SNMP: the data sources.]
...
image::snmp_diagnostics.png[Result of the connection test to the host via SNMP.]
