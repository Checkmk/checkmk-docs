...
In {CMK}, a host is usually a server, a virtual machine (VM), a network device, an appliance -- or generally something with an IP address that is monitored by {CMK}.
However, there are also hosts without an IP address, for instance Docker containers.
Each host always has one of the states {UP}, {DOWN} or {UNREACH}.
...
Each service can have one of the states {OK}, {WARN}, {CRIT} or {UNKNOWN}.

In order for {CMK} to be able to request data from a host, an *agent* is usually necessary.
...
Create the first of the three folders by selecting one of the actions offered to create a folder, e.g. the icon:icon_newfolder[alt="Icon for creating a folder."] [.guihint]#Add subfolder# button.
...
After adding the host, the setup is completed by configuring the services and activating the changes, which we will describe next.
...
For the following installation example, we assume that the downloaded package file is located in the `/root` directory, i.e. in the home directory of the `root` user.
If you have downloaded the file to another directory, replace the `/root` directory definition with the actual directory in the following installation command.
...
////
*Important:* The agent requires either the background program (_daemon_) `systemd`, which is standard on newer Linux distributions, or the auxiliary daemon `xinetd`.
You can see which daemon is running on your computer from the output when you install the agent:
...
If you have neither `systemd` nor `xinetd`, simply install `xinetd` later.
This can be done on RedHat/CentOS with:
...
////
The {CMK} agent for Linux is an executable programme (shell script) that you can test very easily by invoking the `check_mk_agent` command:
...
To test the accessibility of the agent from outside, you can try a connection on port 6556 from another computer via `telnet`.
Here the agent should respond with this same output:
...
*Note:* By default, the agent is accessible from the entire network and can be queried without a password.
However, since the agent does not accept any commands from the network, a would-be attacker cannot gain access.
Information such as the list of current processes is visible, however.
You can find out how to secure the agent in the article on the xref:agent_linux#security[Linux agent].
////

[#create_host]
=== Creating a host

After installing the agent on the host, you can add the host in the xref:glossar#configuration_environment[configuration_environment] of {CMK} 
...
So, complete the creation of the host in the configuration environment for now by clicking [.guihint]#Save & go to folder#.
...
For the command you need the names of the {CMK} server (`mycmkserver` in the example), the {CMK} site (`mysite`) and the host (`mycmkserver` as set up xref:create_host[just] in {CMK}).
...
More detailed information about the Linux agent can be found in the xref:agent_windows#[article about Linux monitoring].
...
// *Note:* You may need to configure the xref:agent_windows#firewall[Firewall settings in Windows] to allow {CMK} access over the network.
...
image::host_snmp_configuration.png[alt="Dialog with properties when creating a host via SNMP: the 'Monitoring agents'."]
