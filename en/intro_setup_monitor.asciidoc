...
image::empty_main_directory.png[alt=Main directory without folders and hosts]
...
image::folder_basic_settings.png[alt=Properties when creating a folder: Basic settings with title]
...
image::three_empty_folders.png[alt="Main directory with three folders, one of them expanded with icons for folder actions"]
...
image::host_allsettings_less.png[alt=Properties when creating a host]
...
image::intro_setup_monitor_services_of_host.png[alt=Services found on the host to add to monitoring]
...
image::host_snmp_configuration.png[alt=Properties when creating a host via SNMP: the data sources]
