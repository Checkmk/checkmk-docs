// -*- coding: utf-8 -*-
// IGNORE AssumeRole ReadOnlyAccess STS Storages Balancer preselected
...
How you choose the so-called '(filter) scope' in AWS is also up to you.
In most cases it will make sense though to have the filter apply to all objects in the bucket.
...
If necessary, perform another xref:wato_services#discovery[Service Discovery] on the AWS host.
...
{CCE-only}
For a convenient start to monitoring AWS, the {CMK} {CE} includes the two built-in xref:glossar#dashboard[dashboards], [.guihint]#AWS EC2 instances# and [.guihint]#AWS S3#.
