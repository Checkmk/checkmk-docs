...
to AWS as the root user, and navigate in [.guihint]#All services# to [.guihint]#Security, Identity, & Compliance > IAM# (Identity and Access Management).
...
the data relating to the costs and the [.guihint]#Route53# service:

image::aws_rule_1.png[]

You also see the option [.guihint]#Use STS AssumeRole to assume a different IAM role#.
...
rather they become so-called xref:piggyback#[piggyback hosts].
...
optionally with the new xref:dcd#[Dynamic Configuration Daemon (DCD)].
