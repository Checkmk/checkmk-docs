...
:description: The current chapter of the guide for {CMK} beginners briefly introduces the {CMK} manual in general and in particular the guide that provides a concise guide through the first steps for setting up IT monitoring with {CMK} in an optimum way.
...
system. In the link:intro_bestpractise.html[last chapter] we will show you some very
...
link:intro_setup.html[We will continue with the setup of {CMK}]
