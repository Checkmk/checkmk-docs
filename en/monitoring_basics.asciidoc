...
|===
|Role |Description 
|`admin` |May see and do everything, has all permissions.
|`user` |May only see what they are a contact for. May manage hosts in folders assigned to them. May not make global settings.
|`agent_registration` |May only register a host's xref:wato_monitoringagents#agents[{CMK} agent] with the {CMK} server -- nothing else.
|`guest` |May see everything, but may not configure anything or intervene in monitoring.
|===
