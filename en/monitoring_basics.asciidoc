...
:revdate: 2019-12-12
:title: Understanding (CMK) terminology
...
For the processing of states, the link:ec.html[{CMK} Event Console] is also
...
|{UNREACH}
|orange
|The _path_ to the host is currently blocked to monitoring, because a router or switch in the path has failed.
...
{UNREACH} _no_ link:notifications.html[alerting] takes place.
...
Contacts and link:wato_user.html#contact_groups[contact groups] offer the possibility of assigning persons to hosts
...
|icon:icon_roles[]
|`admin` <td>May view all, has all privileges
...
|icon:icon_roles[]
|`guest` <td>May view all, but may not configure and may not influence monitoring
...
image::tactical_overview.png[center, width=43%]
...
link:basics_ackn.html[own article].
...
criteria, and decide whom should be alerted and in what form (email, SMS, etc.)

All particulars concerning alerts can be found in it's
link:notifications.html[own article].

=== Flapping hosts and services#flapping
...
image::icon_timeperiods.png[left, width=8%]
...
== Check interval, check attempts and check period#checkintervall
...
a green double-arrow (icon:icon_reload[]) is shown, but a gray four-way-arrow
(icon:icon_reload_cmk[]) for most others. The services with the green
