...
Open the properties of a host with [.guihint]#Setup > Hosts# and then in the [.guihint]#Hosts > Service Configuration# menu the list of the services for this host.
...
.We have omitted the [.guihint]#Status detail# table column, which is not relevant here
...
// This block replaces the following section "Users and roles"
// start translation
////
[#users_roles]
== Benutzer und Rollen
...
As soon as someone _acknowledges_ (confirms) the problem it will be flagged as _handled_, and no surprise, _unhandled_ problems are those which have not yet been attended to.
...
Further details on acknowledgments can be found in its own article, xref:basics_ackn#[Acknowledging problems].
...
All particulars concerning notifications can be found in its own article xref:notifications#[Notifications].
...
[#stale]
=== Outdated hosts and services (Stale)
...
Whenever there is a stale host or service, this will also be shown in the xref:user_interface.html#overview[[.guihint]#Overview#] snapin, which will be extended by the column [.guihint]#Stale#.

But what exactly does the _stale_ state mean?
In general, a host or service is marked as _stale_ when {CMK} no longer receives up-to-date information about its status over a longer period of time:
...
The services remain in their last status, but are marked as _stale_ after a certain time has elapsed.
