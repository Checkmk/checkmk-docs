...
|`0` |Status |The status of the service is given as a number: `0` for {OK}, `1` for {WARN}, `2` for {CRIT} and `3` for {UNKNOWN}. Alternatively, the status can also be xref:localchecks#dynamic_state[calculated dynamically]: then the number is replaced by a `P`.
...
|`myvalue=73;80;90` |Metrics |Metric values for the data. More information about the construction can be found in the chapter on xref:localchecks#metrics[metrics]. Alternatively a minus sign can be coded if the check produces no metrics.
...
Insert the output to be tested into the `echo` command, making sure to escape the quotes for the service name with `\`:
...
Therefore, no newline characters are allowed in the output -- unless they are masked, for example for a xref:locaclchecks#multi-line[multiline output] in {CMK}.

How it can be checked whether the local script will be correctly invoked by the agent can be seen in the xref:localchecks#diagnose[Error analysis].
...
A list of path names can be found in xref:localchecks#folders_script[Files and directories] below.
...
More on rule-creation can be found in the chapter xref:localchecks#bakery[Distribution via the Agent Bakery].
...
Should a problem arise during the service discovery, the xref:localchecks#diagnose[Error analysis] can be of help.
...
The cached data is written to a xref:localchecks#folders_cache[cache directory].
...
Alternatively, on Windows you can also do the configuration in the xref:localchecks#bakery[Agent Bakery].
...
*Important:* On Linux, you can configure asynchronous execution analogously as described in the xref:localchecks#cache[previous chapter] by now creating a directory under `custom/mycustompackage/lib/local/` with the number of seconds of the execution interval and placing the script there. 
Under Windows you can use rule sets for this (and for other options), which you can find in the Agent Bakery under [.guihint]#Agent rules > Windows Agent#.
...
* Is the script in its correct xref:localchecks#folders_script[directory]?
...
The output of the local check must conform to the syntax as described in the chapters xref:localchecks#syntax[Creating the script] and xref:localchecks#extended_functions[Extended functions]. 
...
Under Windows the output can be diverted to a text file, in which the expected outputs can likewise be searched for in the `local`-section using Notepad, for example:
