...
An availability of link:availability.html#bi[BI aggregations] is also possible.
...
|[.guihint]#in scheduled downtime# |[.guihint]#Downtime# |The object was in a period of scheduled downtime icon:icon_downtime[] link:basics_downtimes.html[Downtime]. This state will also be assumed for services whose hosts are in a scheduled downtime.
...
link:views.html[view] of hosts, services or link:bi.html[BI aggregations].
...
* Also in the timeline, you can use the [.guihint]#Availability > Change display options# or [.guihint]#Availability > Change computation options# menu items to customize the link:availability.html#options[options] for display and evaluation.
...
* Under the availability of BI aggregations, with the icon:button_timewarp[] magic wand symbol you can time travel to the state of the aggregate for the time slice in question. More on this link:availability.html#bi[below].
...
link:views.html[view] of the monitoring history with a pre-filled filter for the relevant
...
The link:availability.html#states[states] described in the introduction can be customized
...
Availability views can be embedded in link:reporting.html[reports].
