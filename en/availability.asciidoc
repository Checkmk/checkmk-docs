...
image::icon_availability.png[float=left, width=100]
...
[cols="<th style="width: 30%">State</th>,<th style="width: 13%">Abreviation</th>,~",options="header"]
|===
...
|[.guihint]#unmonitored#
|N/A
|Time ranges during which the object was not being monitored. There are two possible reasons for this: the object was not in the monitoring’s configuration, or the monitoring itself was not running during the specified time range.


|[.guihint]#out of service period#
|
|The object was outside its service period icon:icon_outof_serviceperiod[] – in other words when its availability was ‘irrelevant’. You can learn more about service periods link:availability.html#serviceperiod[further below].

|[.guihint]#in scheduled downtime#
|[.guihint]#Downtime#
|The object was in a period of scheduled downtime icon:icon_downtime[] link:basics_downtimes.html[Downtime]. This state will also be assumed for services whose hosts are in a scheduled downtime icon:icon_hostdowntime[].
...
|[.guihint]#flapping#
|
|Phases in which the state is icon:icon_flapping[] link:monitoring_basics.html#flapping[flapping] -- i.e., those phases during which many changes of state have been experienced over a short time span.
...
[views|view] of hosts, services or link:bi.html[BI-Aggregates].
...
image::button_timeline.png[float=left, width=48]
...
image::button_history.png[float=left, width=48]
...
image::avail_button_options.png[float=left, width=48]
...
image::avoption_time_range2.png[float=left, width=300]
...
image::avoption_time_range.png[float=left, width=300]
...
image::avoption_only_outages.png[float=left, width=300]
...
image::avoption_labelling.png[float=left, width=300]
...
image::avoption_visual_levels.png[float=left, width=300]
...
image::avoption_outage_statistics.png[float=left, width=300]
...
image::avoption_count_max.png[float=left, width=300]
...
image::avoption_format_time.png[float=left, width=300]
...
image::avoption_format_timestamps.png[float=left, width=300]
...
image::avoption_summary_line.png[float=left, width=300]
...
image::avoption_timeline.png[float=left, width=300]
...
image::avoption_group.png[float=left, width=300]
...
image::avoption_availability.png[float=left, width=300]
...
image::avoption_status_grouping.png[float=left, width=300]
...
image::avoption_host_status_grouping.png[float=left, width=300]
...
image::avoption_status_classification.png[float=left, width=300]
...
image::avoption_downtimes.png[float=left, width=300]


With the [.guihint]#Scheduled Downtimes# option you can specify how
link:basics_downtimes.html[scheduled downtimes] affect the availablity analysis:<br><br><br><br>
...
image::avoption_phase_merging.png[float=left, width=300]
...
[.guihint]#Maximum number of check attempts#] from 1 to 3 to allow more failures before
a notification is triggered. Thus the concept of [.guihint]#Soft states# has been
...
image::avoption_short_times.png[float=left, width=300]
...
* Assign these to the objects with the link:.guihint].html#Host & Service parameters|Monitoring configuration[Service period for hosts# or respectively the [.guihint]#... for services# rule sets.
* Activate the changes.
* Use the [.guihint]#Service time# Availability-option to control the behaviour:

image::avoption_service_time.png[float=left, width=300]
...
image::avoption_notification_period.png[float=left, width=300]
...
image::avoption_query_time_limit.png[float=left, width=300]
...
image::avoption_limit_data.png[float=left, width=300]
...
[cols="<th style="width: 20%">Core</th>,<th style="width: 40%">Current file</th>,~",options="header"]
|===
...
image::avail_statehist_cache.png[align=center, width=550]
...
[cols="<th style="width: 50%">File path</th>,~",options="header"]
|===
...
|Here the link:availability.html#annotations[annotations] and retrospectively-amended scheduled downtimes for outages are stored. The file is in the Python-format and can be manually edited
