...
Linux-supported language, e.g., BASH, Python or Perl. Don't forget to make the
...
allows direct access and shows the title which is logged in the second line of the script. Additionally you may add arguments, which you enter in the text fields. These will be interpreted as command line arguments in the script. On your shell you may access them with `$1`, `$2`, etc.

image::alert_handler_arguments.png[]

*Important*: After saving the rule the alert handler will be immediately active and will
run with every change of status for any host or service!
...
At the end of this time the handler will be stopped. Since version
...
Since version {v14} {CMK} precludes the simultaneous running of alert
...
canceled and identified as 'failed'.
...
Don't forget to reset the small switch to green as soon as practicable! Otherwise you
...
Be careful with this setting! Starting processes and calling up scripts consumes a lot
of CPU resources. {CMK} can easily run 1000 checks per second - but Linux certainly could not
...
initialization - for example when establishing a connection to a data base.
...
Every version since version {v14} includes a built-in alert handler that enables
...
* The scripts ('remote handler')written by you on the target system.
...
In the characteristics, the remote handler [.guihint]#Restart FOO service# you have just defined can be seen.
...
One can also utilize another user as the `root` - naturally only when they have the appropriate rights
...
Be aware that your system could be set up so that an SSH-access as `root` is not
...
Set also a sensible *condition* in the rule, otherwise an SSH-connection will be
