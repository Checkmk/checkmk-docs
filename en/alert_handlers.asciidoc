...
Alert handlers are very similar to xref:notifications[Alarms] and are similarly configured,
but there are a few important differences:

* Alert handlers are independent of xref:basics_downtimes[Downtimes], Alarm periods, xref:basics_ackn[Acknowledgements] and similar controls.
...
As with xref:notifications[Alarms] the script obtains all information for
...
A list of the file paths can be found xref:alert_handlers#files_agent[here].
