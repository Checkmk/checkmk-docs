= Alert handlers
:revdate: 2017-02-13
[.seealso][#notifications] [#commands] [#wato_monitoringagents]MT:Responding to problems automatically
MD:Some problems occur only rarely and with a short duration. Checkmk can then run scripts to collect ad hoc information for further analysis.
...
[CEE] Alert handlers are scripts you write yourself, that are run for you by (CMK) if
...
* Alert handlers really become useful when they execute a command on the target host. (CMK) offers a ready solution for Linux that will be explained later [alert_handlers#linux_remote|see below].
...
* The `linux_remote` alert handler with the title [.guihints]#„Linux via SSH“# on the (CMK)-Server.
