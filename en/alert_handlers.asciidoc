...
= Alert handlers
:revdate: 2017-02-13
:title: Responding to problems automatically with the alert handler
:description: Some problems occur only rarely and with a short duration. With the alert handler, {CMK} can then run scripts to collect ad hoc information for further analysis.
...
trigger a notification link:alert_handlers.html#notification[below]).
...
Also new in version {v14} is that the execution of an alert
