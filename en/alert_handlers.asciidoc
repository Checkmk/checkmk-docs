...
* Alert handlers are independent of [basics_downtimes|Downtimes], Alarm periods, link:basics_ackn.html[Acknowledgements] and similar controls.
...
== Setting up alert handlers#setup
...
The handler activation is performed using the ICON[icon_alert_handlers.png] [.guihint]#Alert handlers# WATO module.

Proceed as follows:

. Save the script in `~/local/share/check_mk/alert_handlers/debug`.
. Make it executable with `chmod +x debug`.
. Call up the ICON[icon_alert_handlers.png] [.guihint]#Alert handlers# WATO module.
...
== How alert handlers are executed#execution
...
== Notification via alert handlers#notification
...
== Remote execution under Linux#linux_remote
...
== Relevant files and directories#files
