= Alert handlers
:revdate: 2017-02-13
[.seealso][#notifications] [#commands] [#wato_monitoringagents]:title: Responding to problems automatically
...
image::bilder/master_control_alert_handlers.png[align=left,right]
...
* Restarting a service that has crashed<br>
* The triggering of a garbage collector if a Java-VM is running out of memory<br>
...
=== Alert handlers in Checkmk

[CEE] Alert handlers are scripts you write yourself, that are run for you by Checkmk if
a problem is detected - or more precisely - if a host or service changes its status.

Alert handlers are very similar to [notifications|Alarms] and are similarly configured,
but there are a few important differences:

* Alert handlers are independent of [basics_downtimes|Downtimes], Alarm periods, [basics_ackn|Acknowledgements] and similar controls.
...
You can naturally - as always in Checkmk - use rules to define the conditions
for which a particular handler should be executed.

[CRE] A further tip for (CRE) users: monitoring can also be enabled to execute
...
<a href="https://www.google.com/search?q=nagios+event+handlers">Google</a>.
...
Alert handlers are scripts that are run on the Checkmk server. They must be stored in
...
As with [notifications|Alarms] the script obtains all information for
...
The handler activation is performed using the ICON[icon_alert_handlers.png] [.guihints]#Alert handlers# WATO module.

Proceed as follows:

. Save the script in `~/local/share/check_mk/alert_handlers/debug`.
. Make it executable with `chmod +x debug`.
. Call up the ICON[icon_alert_handlers.png] [.guihints]#Alert handlers# WATO module.
. There, define a new rule with ICON[button_new_rule.png].
...
image::bilder/alert_handler_rule.png[]

In the mask for selecting the handler you can pass arguments
({{Call with the following parameters}}).# These are interpreted as
...
image::bilder/alert_handler_arguments.png[]
...
To test, manually set a service, for example, [.guihints]#Fake check results# to (CRIT).
Now the file should have been created with the variables.
Here are its first twenty lines:

[source,bash]
----
OM:head -n 20 ~/tmp/alert.out
...
* Alert handlers really become useful when they execute a command on the target host. Checkmk offers a ready solution for Linux that will be explained later [alert_handlers#linux_remote|see below].
...
This functions completely analogous to [notifications|Notifications],
...
image::bilder/alert_handler_rule_condition.jpg[]
...
a rule [.guihints]#Maximum number of check attempts for service# to set the minimum
...
Please note: Unlike other places in Checkmk *every* alert handler rule
...
and it is started by the [cmc|CMC].
...
From version VERSION[1.4.0i1] Checkmk precludes the simultaneous running of alert
...
monitoring history. Furthermore you can trigger a notification [alert_handlers#notification|below]).

=== Global settings

There are a number of global settings for executing alert handlers:

image::bilder/alert_handlers_options.png[align=center,width=580]

The [.guihints]#Logging of the alert processing in the core# is concerned with the phase up until the
CMC sends a message to the alert handler (`cmc.log`). On the other hand the
[.guihints]#Alert handler log level# influences the logging in the alert helper
(`alerts.log`).

=== Master control

image::bilder/master_control_alert_handlers_off.png[align=left,width=280]

With a click in the [.guihints]#Master control# you can deactivate alert handlers globally.
...
image::bilder/alert_handler_history.png[]

Also new is the global view [.guihints]#Other => Alert handler executions}},# which provides a global display of all executing
alert handlers.
...
that triggers a [notifications|notification]. This way you can be informed that a handler has completed its task.
There are additionally two new types of event that you can filter in a notification rule:

image::bilder/alert_handler_notif_condition.png[]
...
With the
[.guihints]#Alert handlers => Typesof events that are being processed => Allcheck executions!# global option
...
of CPU resources. Checkmk can easily run 1000 checks per second - but Linux certainly couldn't
handle 1000 alert handler scripts per second.

In order to make this usefully possible, Checkmk provides the option to write alert
...
home directory and the Checkmk-Instance's name respectively.
...
* The Checkmk alert helpers call up their functions *synchronously*. Make sure that no wait states occur!
...
* This can all be implemented using the [wato_monitoringagents#bakery|Agent bakery].

The [.guihints]#Linux Remote Alert Handler# consists of the following individual elements:

* The `linux_remote` alert handler with the title [.guihints]#„Linux via SSH“# on the Checkmk-Server.
...
* Rules in [.guihints]#Monitoring agents => Rules => LinuxAgent => Installremote alert handlers (Linux)# that generate SSH keys.
...
the [wato_monitoringagents#bakery|Agent bakery], install the script
*on the Checkmk-Server* (not on the target system!).
...
[source,bash]
----
OM:cd local/share/check_mk/agents/linux/alert_handlers
OM:chmod +x restart_foo
----
...
Define a rule under [.guihints]#Monitoring Agents => Installremote alert handlers (Linux)}}.# 
In the characteristics, the Remotehandler [.guihints]#Restart FOO service# you have just defined can be seen.
Select this for installation:

image::bilder/install_remote_handlers.png[]
...
image::bilder/install_remote_handlers2.png[]

The public key is intended for the agent. The private key will later be required by the Checkmk-Server
...
for the required action. The Checkmk-Agent will only install the SSH-key on systems where this user already exists.

==== Baking agents

Now bake new agents with ICON[button_bake_agents.png]. In the list of ready agents an entry should
now appear in which your remote handler and SSH-key can be seen:

image::bilder/baked_remote_handlers.png[]
...
creation of an appropriate rule. This time choose [.guihints]#Linux via SSH# as the handler,
enter the user for whom the SSH-key should be installed, and select your remote handler:

image::bilder/alert_rule_foo.png[]
...
image::bilder/alert_foo_failing.png[]
...
Likewise that Checkmk has triggered a notification which was stopped by an alert handler.
...
A list of the file paths can be found [alert_handlers#files_agent|here].
...
=== Files and directories on the Checkmk-Server
[cols=53, options="header"]
|===
...
|`var/log/alerts.log`
|Log file with all events relevant to the alert handler (logged by the Alerthelper)


|`var/log/cmc.log`
|Log file for the core. Some alert handler information is also stored here
...
|Here the monitoring history's log file is stored and also evaluated by the core
...
|SSH-configuration for the user `root`


|`~harri/.ssh/authorized_keys`
|SSH-configuration for a user `harri`
