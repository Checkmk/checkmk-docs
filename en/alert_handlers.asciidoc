...
:description: Some problems occur only rarely and with a short duration. {CMK} can then run scripts to collect ad hoc information for further analysis.
...
New from {v14}i1]: if a comment is inserted into the second line
...
Up until version {v128} you need to specify the name of the
...
From version {v14} the mask for selecting the handler
...
{v14}i1] it is regulated that at the end of a timeout a
...
From version {v14}i1] {CMK} precludes the simultaneous running of alert
...
Up until version {v128} the alert handler's outputs and exit codes were
simply logged in `alerts.log`. Since version {v14}i1] this
...
Since version {v14}i1] alert handlers have created entries in the monitoring history.
...
Also new in version {v14}i1] is that the execution of an alert
...
Version {v14}i1] includes a built-in alert handler that enables
