...
:description: Here you will learn how to piggyback monitoring data from one host when querying another host using {CMK}.
...
also be very impractical for monitoring if all information for all VMs always
...
image::piggyback_settings.png[alt="Piggyback data redirection is set in the agent settings."]
...
link:dcd.html[dynamic configuration] and automatically create
...
For this there is the special [.guihint]#Hostname translation for piggybacked hosts# link:wato_rules.html[rule set], which you can find in the Setup menu under [.guihint]#Setup > Agents > Agent access rules#.
...
image::piggyback_hostname_translation.png[alt="Options of 'hostname translation', removing the domain part, conversion to lowercase or explicit mapping."]
...
Note that in xref:distributed_monitoring#[distributed environments] currently the source host and the piggybacked hosts must be monitored in the same site. 
