...
With commands influencing hosts, services and other objects, you can intervene in the monitoring process.
The commands are most often used for xref:basics_ackn#[acknowledging problems], and for setting xref:basics_downtimes#[scheduled downtime].
...
When you select one of the commands, an area with the parameters of the command is displayed below the menu bar or action bar. If you select [.guihint]#Acknowledge problems# it looks like this:

image::commands_acknowledge.png[]

Some commands need further information, such as a comment for acknowledgement. 
...
If you first wish to make a selection there, you have three options:
...
|Fake check results |Manually determine the results of checks, i.e. the output, performance data or simply the status of a host or service. This is useful, for example, to test the effects of different outputs and statuses on monitoring, views or or even xref:notifications#notification_testing [notifications].
