...
and in various other xref:views#[views], and are also available via an API.
...
possible with the help of a xref:wato_rules#[rule]. This rule set can be found under
...
The rule you have just created will generate an xref:active_checks#[active check] for each host at the
...
The new service can be found on the hosts in the service list, and it looks something like this:
...
details see the xref:agent_linux#plugins[Linux agent articles].
...
{cee-only}
Of course if you use the xref:wato_monitoringagents#bakery[Agent Bakery] to
...
* Include in xref:reporting#[reports]
* Export as PDF or as CSV
* Integrate with xref:dashboards#[dashboards]
...
the xref:inventory#history[history]! This would almost always lead to constant
changes and render this function useless.
xref:inventory#statusdata[Status data] are not
...
From a script the best way to authenticate is as xref:wato_user#automation[automation user].
...
xref:distributed_monitoring#[distributed environments]. Here the inventory data are
first determined by the local sites and stored there locally under
`var/check_mk/inventory`. 

The xref:distributed_monitoring#livestatusproxy[Livestatus proxy daemon] will
...
the remote sites via xref:livestatus#[Lsivestatus].
