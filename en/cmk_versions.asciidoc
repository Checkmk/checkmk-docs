...
It begins following the release of the preceding version – for example `2.1.0` – with the start of development of the new features for the coming version – for example `2.2.0`.
...
{cee-only}
Innovation versions are a special feature of the {CEE} and give you the possibility 
...
Over the following 1-2 months, 3-4 releases will be produced, each identified in sequence by an `i` number (e.g., `2.0.0i2`).
...
The release of a new stable version, (e.g. `2.2.0`), begins with a beta phase.
...
On the stable branch a series of beta versions – identified by a lowercase `b` in their names – will be published, (e.g., `2.2.0b1`, `2.2.0b2`, etc.).
...
Following the beta phase, a stable version is released – for example, `2.2.0`.
...
These will be corrected by a series of patch releases (`2.2.0p1`, `2.2.0p2`, etc.).
The time intervals between these patches will initially be quite short (around a week), and later they will be much longer (a few months). 

In addition to the master, daily builds are also published on the stable branch, which provide prompt bug fixes.
These carry the name of the branch, plus the date, e.g., `2.2.0-2023.03.30`.
...
This suffix enables the same versions of various link:https://checkmk.com/product/editions[{CMK} editions^] to be distinguished.
This makes it possible to have, for example, version `2.2.0` of the {CRE} and the {CSE}, installed simultaneously.
This is in fact sometimes very sensible – namely if you wish to migrate from the CRE to the CEE.
...
We will actively maintain this stable version with patch releases.
...
This means that two versions are served in parallel with patch releases for 6 months:
...
{cee-only}
Since daily builds and beta versions are only recommended for testing purposes and the product lifecycle is correspondingly short,
only the current and former stable versions are listed in the following table. 
...
|{v22} |2023-05-23 |6 months after release of {v23}footnote:stable_version[Currently, no release date has been set.] |18 months after release of {v23}footnote:stable_version[]
|{v21} |2022-05-24 |2023-11-23 |2024-11-23
