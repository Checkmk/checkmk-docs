...
In addition to general information about the costs that are incurred by your Google Cloud and the current status of link:https://checkmk.com/integrations/gcp_health[Google services in your region^], you can monitor the following Google Cloud products with all editions of {CMK} from version {v22} onwards:
...
A complete listing of all available check plug-ins for monitoring GCP can be found in our link:https://checkmk.com/integrations?tags=gcp[Catalog of check plug-ins^].


[#implementation_gcp]
== Practical implementation of a GCP monitoring system
...
[#create_host]
=== Creating a host for GCP in {CMK}
...
[#agent_rule]
=== Creating a rule for GCP agents
...
=== The [.guihint]#Exceptions# service
...
In the event of an error, this service becomes critical by default and in its [.guihint]#Summary# will provide an indication of where the problem lies.
