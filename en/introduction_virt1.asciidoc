...
With the virtual appliance _{CMK} virt1_ you can run {CMK} as a guest on an existing virtualisation platform (_hypervisor_) such as VMware ESXi.
VirtualBox is also supported and is particularly suitable for trying out.

The operating system and a Web-GUI for administration are already pre-installed in the appliance.
The appliance is set up via a dialog, configured via the Web-GUI and is ready for for immediate use after a few simple steps, 
...
As an alternative, you can download the virtual appliance of the {FE} from the link:https://checkmk.com/download[download page].

The following file types are available for download for the virtual appliance:

* OVA file: The {CMK} virt1 virtual appliance for import into the hypervisor. OVA stands for Open Virtualization Archive and is the archive format of the Open Virtualization Format (OVF). The OVA file can be found in the download area at the product {CMK} appliance. You will need this OVA file for the installation.

* CFW file: The firmware of the appliance. You can use this to update the appliance version, e.g. from version 1.4.13 to 1.4.14. An xref:appliance_usage#cma_webconf_firmware[update of the firmware] is done with the appliance Web-GUI. The CFW file can also be found at the product {CMK} appliance. You do not need the CFW file for the initial installation, as the OVA file already contains the firmware.

* CMA file: The {CMK} software for installation in the appliance. Starting with appliance version 1.4.14, no {CMK} software is pre-installed in the appliance. You also carry out the xref:appliance_usage#manage_cmk[installation of the {CMK} software] with the appliance Web-GUI. The CMA file is offered to you in the download area after you have selected the {CMK} edition, version and the appliance platform. You will only need the CMA file *after* the installation is complete -- when you perform the basic configuration of the appliance.
