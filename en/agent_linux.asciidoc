...
|Simple installation of a standard agent with a manual configuration over configuration files
|X
|X

|RPM/DEB-Packet from the link:wato_monitoringagents.html#bakery[Agent Bakery]
|Configuration via the GUI -- individual configuration of each host is possible
|
|X

|link:agent_deployment.html[automated updates]
|The packet from the agent bakery is initially installed manually or by using a script, and subsequently from there updated automatically.
|
|X

|Manual installation
|The individual files, without a packet, are copied to the target system and then `xinetd`, `systemd`, SSH or a user’s own access method are manually installed.
|X
|X
...
link:wato_monitoringagents.html#bakery[Agent Bakery]. From here, with the [.guihint]#Unpackaged agents#
button, or (in Version VERSION[1.4.0]) [.guihint]#Agent files# button
...
{c-start}
{c-root}rpm -U check-mk-agent-1.4.0p4-1.noarch.rpm
{c-end}
...
{c-start}
{c-root}dpkg -i check-mk-agent_1.4.0p4-1_all.deb
{c-end}
...
{c-start}
{c-root}wget http://mycmkserver/mysite/check_mk/agents/check-mk-agent-1.4.0p4-1.noarch.rpm
{c-end}
...
{c-start}
{c-root}rpm -U http://mycmkserver/mysite/check_mk/agents/check-mk-agent-1.4.0p4-1.noarch.rpm
{c-end}
...
This process is described in its link:agent_deployment.html[own article].<br><br>
...
link:wato_services.html#available[available]. It will look like this (here in the new
...
|The user's own link:localchecks.html[‘local checks’].
...
|`/etc/check_mk/mrpe.cfg`
|Configuration file for link:agent_linux.html#mrpe[MRPE] -- for the execution of standard, Nagios-compatible check plug-ins.


|`/etc/check_mk/encryption.cfg`
|Configuration for the link:agent_linux.html#encryption[encryption] of the agent data.
