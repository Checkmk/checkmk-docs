...
xref:wato_monitoringagents#bakery[Agent Bakery]. From here, with the [.guihint]#Agent files# button
...
standard settings can be found here (the package for xref:agent_windows[Windows]
...
scripts for the xref:agent_linux#manual[manual installation] to `/usr/bin/`.
...
{cee-only}
The {CEE} utilises the xref:wato_monitoringagents#bakery[Agent-Bakery] via a
...
* A relevant article in this handbuch (e.g., on the monitoring of xref:monitoring_oracle[Oracle])
...
xref:wato_monitoringagents#bakery[Agent Bakery]. This not only takes care of the installation
...
|`/etc/check_mk/mrpe.cfgv` |Configuration file for xref:agent_linux#mrpe[MRPE] -- for the execution of standard, Nagios-compatible check plug-ins.
|`/etc/check_mk/encryption.cfg` |Configuration for the xref:agent_linux#encryption[encryption] of the agent data.
