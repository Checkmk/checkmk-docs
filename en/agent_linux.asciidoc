...
The `cmk-agent-ctl` *Agent Controller* is the component within the agent that is responsible for transporting the data collected by the agent script.
...
To register a host for monitoring, this host must be able to reach the REST API of the {CMK} server (port 443 or 80) and the _Agent Receiver_ (port 8000 for the first site, 8001 for the second...).
...
After entering the command `cmk-agent-ctl register` the Agent Controller first runs a request to the REST API of the {CMK} server to ask for the port of the Agent Receiver.
As second step a connection to the Agent Receiver is established to request the certificate.
You can simulate the first request using a program like `curl`. On the host that is to be added to the monitoring, run:
...
In most cases this is just `8000`.
...
In this case you might change routing or firewall settings to enable access, respectively add the internal CA to the trust chain of the host.
...
Communication is now directly to the Agent Receiver wirthout any detour:
...
Equivalent to port 443 (respectively 80) mentioned above you can now change routing or firewall settings in a way the host to be registered can reach port 8000 (or 8001...) of the {CMK} server.

Should security policies prohibit access to the agent receiver, there is still the possibility to use xref:proxyregister[proxy registration] on the {CMK} server.
This is also possible using a pipeline where the output of `cmk-agent-ctl proxy-register` is handed over as input to `ssh hostname cmk-agent-ctl import`:
