...
This means that in addition to the registered, encrypted and compressed *pull mode*, an equivalent *push mode* now becomes possible.
More on this in the next chapter.

Push mode will be a feature included in (and exclusive to) the new {CMK} Plus Edition (CPE), which will be introduced with the next {CMK} version.
TLS encryption and compression will be available for all {CMK} editions starting with version {v21} -- as long as both the {CMK} server and the agent have at least version {v21}.
...
With the new agent introduced in {v21}, more software components are involved in the communication than previously.
We will therefore briefly introduce the components and their interactions here.
The architectures are identical for both the Linux agent and the xref:agent_windows#agent_architecture[Windows agent].
...
The `check_mk_agent` *agent script* is and remains responsible for the collection of the monitoring data.
For data collection, it calls existing system commands in sequence.
...
* can be easily extended, because you can write plug-ins in any programming or scripting language and have them executed by the agent script.
* transparent, because it is a shell script in which you can see what commands it calls.

The `cmk-agent-ctl.exe` *Agent Controller* is the component within the agent that is responsible for transporting the data collected by the agent script.
...
The `agent-receiver` *Agent Receiver* is the component of the {CMK} server that serves as the general endpoint for the Agent Controller's communication, e.g. for registering the connection.
It will continue to receive data sent by the Agent Controller in push mode.
In pull mode, data is exchanged with the site's xref:cmc_differences.html#fetcher_checker[{CMK} fetchers].
...
In such a case, registration is not possible and the only available communication path remains the *legacy mode*.
...
The reason is that a mutual trust relationship is missing -- trust and encryption are mandatory from {CMK} {v21}.
Although unencrypted communication in legacy pull mode is possible immediately after the initial installation of the {v21} agent (also as an update of an older agent), this will be deactivated at the latest when the agent is next updated.
Therefore, perform the registration promptly after installation/update.
This chapter shows how to do this.

Registration and thus establishing the mutual trust relationship is done through a {CMK} user with access to the xref:rest_api#[REST API.]
For this, the xref:glossar#automation_user[automation user], which is automatically created with every {CMK} installation and whose password you can randomly generate, is a good choice.
...
On a host connected to the {CMK} server, you can revoke the trust connection with this command:
...
The Universally Unique Identifier (UUID) to be specified is the one output by the `cmk-agent-ctl status` command. 
...
All of the diagnostic options that are available from the {CMK} server side are described in the general article on xref:wato_monitoringagents#diagnostics[agents].
...
If it has been verified that the agent script and its installed plug-ins are executing as expected locally, you can next check from the {CMK} server via `netcat` (or `nc`) whether port 6556 is reachable:
...
*Note:* If the communication between agent and {CMK} server is _still_ unencrypted (as in legacy pull mode), or is and will remain unencrypted (as in xref:agent_linux_legacy#[legacy mode]), this command will give you the full unencrypted agent output instead of the `16`.
