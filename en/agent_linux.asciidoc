...
So, on the one hand, the Linux agent takes over the agent script, and thus also its advantages.
...
To secure the TLS connection, {CMK} uses a self signed certificate, that is automatically replaced shortly before expiring.
Hosts that are monitored are made familiar with the new certificate at the right time, which means upon expiration no manual interaction is necessary.
You can adjust the lifetime of certificates using the global setting [.guihint]#Agent Certificates > Lifetime of certificates#.
