...
The Agent Controller is connected upstream of the agent script, queries it and communicates with the {CMK} server in its place.
To do this, it registers with the *Agent Receiver*, a process that runs on the {CMK} server.

On the one hand, the Linux agent takes over the agent script and thus gains script’s advantages.
On the other hand, the agent enhances the script in such a way that new functions can be added, such as TLS encryption of the communication or data compression.
...
{cee-only}
The {EE} and the {CE} have a software module, the xref:glossar#agent_bakery[Agent Bakery], for automatically packaging customized agents.
...
and manual registration, as described in the following xref:registration[chapter], is _no longer_  necessary.
...
*Attention:* The push mode and auto-registration in the {CE} are necessarily dependent on the Agent Controller and are therefore not usable in the legacy mode, a matter to which we refer several times in this chapter.
...
You can display the command help with `cmk-agent-ctl help`, also for specific available subcommands, for example with `cmk-agent-ctl help register`.
...
The Agent Receiver tells the Agent Controller in which mode the controller should work at the time of registration.
...
The name of the host stored in the `$HOSTNAME` environment variable is used.
The subsequent confirmation of the certificate corresponds to that shown in the last section.
...
In the case of a registration in the {CE} push mode, the following will apply:
If the registration has been successful, the minute-by-minute transfer of the agent output will also be successful.

Should security policies prohibit access to the Agent Receiver, there is still the option to use xref:proxyregister[registration by proxy] on the {CMK} server.
...
Information lines such as the following indicate connection problems:
...
If a host has been configured for xref:hosts_autoregister#rule_autoregister_bakery[auto-registration] in the {CCE} with the [.guihint]#Agent controller auto-registration# rule set, and the [.guihint]#Keep existing connections# option has been set to [. guihint]#no#, every time the `cmk-agent-ctl-daemon` service is restarted (for example, when a host is restarted), all other connections will be removed -- except the connection configured for auto-registration.
This affects, for example, hosts on which connections to multiple sites were set up before the baked agent package was installed, or connections were added manually after the agent package was installed.

You can temporarily change this behavior by setting the `keep_existing_connections` variable to `true` in the `/var/lib/cmk-agent/pre_configured_connections.json` file on the host.
You can achieve a permanent change across an update of the agent packages by setting [.guihint]#Keep existing connections# to [.guihint]#yes# in the above rule set.
...
In the third and last step, use the [.guihint]#Enforce agent data encryption# rule to specify that the Checkmk server only accepts data encrypted via TLS.
...
The `/etc/check_mk/encryption.cfg` configuration file is edited automatically for you and included in the agent packages.
