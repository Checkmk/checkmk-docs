...
that a certain host delivers incorrect values and you want to suspend the
retrieval of this data for a short time.

As a user of one of the {CEE} you can simply create a rule via
[[.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent rules > Disabled sections (Linux agent)#, 
which will then be taken into account by the
...
suited, as explained in the link:cmk_commandline.html#cmk[article on the command line].
...
you have to add the option _-i_ and point it to the appropriate key. Ideally,
use `$OMD_ROOT` as the substitute for the path to the instance directory
...
will also be runable in an site with a different name:
...
Information on how to resolve such errors can be found in the dedicated
Knowledge base article link:https://kb.checkmk.com/display/KB/Executing+the+Linux+agent+over+ssh[Executing the Linux agent over ssh].
...
|`/etc/check_mk/mrpe.cfgv` |Configuration file for link:agent_linux.html#mrpe[MRPE] -- for the execution of standard, Nagios-compatible check plug-ins.
|`/etc/check_mk/encryption.cfg` |Configuration for the link:agent_linux.html#encryption[encryption] of the agent data.
|`/etc/check_mk/exclude_sections.cfg` |Configuration file for link:agent_linux.html#disabled_sections[disabling certain sections] of the agent
