...
Attention: in the [.guihint]#Linux/Unix-Agents# section can be found the straight agent scripts
for the link:agent_linux.html#manual[manual installation] to `/usr/bin/`.
Only use these if the packaged agents do not function on your system, or if you wish to
monitor link:agent_unix.html[Unix derivatives] other than Linux.
...
[.guihint]#Monitoring Agents => Generic Options => Execute MRPE Checks# is responsible for this.
...
. Create a rule in [.guihint]#Monitoring Agents => Generic Options => Deploy custom files with agent#.
...
by creating a [.guihint]#Host & Service Parameters => Access to agents => Encryption# rule in the rule set.
...
For such a situation, at the host’s characteristics in the [.guihint]#Host tags# submenu in WATO,
set the [.guihint]#Agent type# to {{Dual: {CMK} Agent + SNMP}}.
...
* The host has been configured in {CMK} for an additional monitoring per link:agent_linux.html#snmp[SNMP], with [.guihint]#Agent type# set to [.guihint]#Dual: Check_MK Agent + SNMP#.
