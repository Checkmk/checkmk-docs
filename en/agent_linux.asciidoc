...
The target system is now ready. Now only the configuration of {CMK} itself
needs to be done. This is performed via the 
[.guihint]#Datasource programs > Individual program call instead of agent access# link:wato_rules.html[rule set].
...
its link:datasource_programs.html[own article].
...
used in [.guihint]#Datasource programs#.
...
by creating a [.guihint]#Host & Service Parameters > Access to agents > Check_MK Agent > Encryption# rule in the rule set.
