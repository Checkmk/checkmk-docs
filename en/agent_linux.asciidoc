...
scripts for the xref:manual[manual installation] to `/usr/bin/`.
...
How that is achieved will be described xref:ssh[below].
...
and partly also using special xref:plugins[plug-ins].
...
information and delivering this via xref:snmp[SNMP].
...
* The host has been configured in {CMK} for an additional monitoring per xref:snmp[SNMP].
...
|/etc/check_mk/mrpe.cfg |Configuration file for xref:mrpe[MRPE] -- for the execution of standard, Nagios-compatible check plug-ins.
