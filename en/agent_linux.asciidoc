...
The xref:rest_api#[REST API] of {CMK} provides the following ways to download agent packages as of release {v20}p18:
...
As always, for details on how the agent download endpoints work, please refer to the API documentation available in {CMK}, which can be found via [.guihint]#Help > APIs > REST API documenation#.
...
Additionally, these are available via the download page for the agents in the Setup menu (as
described at the beginning of this article) in the [.guihint]#Plugins# menu box:
...
During its installation the agent detects if the running version of Systemd already knows this option and configures itself accordingly.
...
Only after this confirmation, {CMK} will be able to access the agent in the background:
...
For very old target systems, it is also possible that keys with elliptical curves (ed25519 and ecdsa) are not known.
...
submenu in the [.guihint]#Setup#, set the [.guihint]#Check_MK Agent# to one of the options starting with
...
This system continuously logs data on the condition of the HDD or SSD, and with the `smart` plug-in {CMK} can retrieve these values and evaluate the most important of them. In order that the plug-in functions after an installation, the following preconditions must be met:
...
If -- as shown in the screenshot -- a `cmd_timeout` occur occasionally, set the smart plugin to asynchronous execution at intervals of a few minutes.
...
|`/etc/check_mk/exclude_sections.cfg` |Configuration file for xref:disabled_sections[disabling certain sections] of the agent
