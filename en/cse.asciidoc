...
In this article we will introduce you to the {CSE}, which is the most commonly used edition in professional environments.
...
|CMC |The {EE} by default uses the xref:cmc#[{CMK} Micro Core], which is more performant than other monitoring cores by reducing to essential functionality and optimizing it.
...
|Flap detection  |In the {EE}, a xref:notifications#flapping[fine-tuning of unsteadiness detection] is possible for services that oscillate around thresholds for a long time.
|===
...
|===
|Function |Description
|Standard dashboard |By default the {EE} use a xref:dashboards#usage[dashboard], which is not available in the {RE}. The latter uses the problem dashboard instead.
|Measured values and graphing |Advanced xref:graphing#[graphing functions] are only available in the {EE}. These include: Scope of dashlets, PDF export, graph collections, customizations (_graph tunings_), embedding in reports or dashboards, and custom graphs.
|Kubernetes dashboards |The xref:monitoring_kubernetes#dashboards[Kubernetes dashboards] are available only in the {EE}.
|Scheduled downtimes |Only in the {EE} can regularly recurring xref:basics_downtimes#[scheduled downtimes] be specified. In addition, convenient xref:commands#commands[changing or removing one or more scheduled downtimes] is possible via _commands_.
|PDF export of availability data |xref:availability#[Availability data] can also be exported as a PDF in the {EE}.
|PDF export of views |Direct PDF export of xref:views#[views] in monitoring is available only in the {EE}.
|Reporting |xref:reporting#[Reporting] is available only in the {EE}.
|Extended availability (SLAs) |The xref:sla#[extended availability / service level agreements] included only in the {EE} facilitate the control of contractual agreements regarding the availability of services.
|Acknowledgement with validity period | The xref:basics_ackn#[acknowledgement of problems] can be assigned a maximum validity period in the {EE}.
|===
...
|===
|Function |Description
|Notification spooler| The {EE} include the xref:notifications#async[notification spooler], a component that allows more efficient and flexible delivery of notifications.
|Trackable delivery |xref:notifications#syncsmtp[Smarthost settings] can be used in the {EE} to ensure that notifications arrive more reliably than by just handing them over to the {RE}'s local mail system.
|Centralized notifications |In distributed monitoring, the {EE}'s notification spooler can be configured to send xref:distributed_monitoring#notifications[notifications from  remote sites for all targets].
|===
...
|===
|Function |Description
|Agent Bakery |The {EE} have a mechanism to automatically create custom agent packages with plug-ins and configuration files with the xref:wato_monitoringagents#bakery[Agent Bakery]. Agent configuration settings can be made using the xref:bakery_api#[Bakery API] in the setup GUI.
|Agent updates |The xref:agent_deployment#[automatic update] of agent packages after configuration changes is only possible in the {EE}.
|SNMP monitoring |Thanks to a dedicated xref:snmp#[SNMP engine] _(inline SMTP)_, the {EE} are more performant than the {RE} with the use of `snmpget`/`snmpbulkwalk`.
|{CMK} fetchers |In the {EE}, the {CMK} fetchers provide efficient xref:wato_monitoringagents#[fetching of monitoring data.]
|===
...
|===
|Function |Description
|SAML |Since {CMK} {v22}, the {EE} can natively xref:saml#[authenticate against SAML], up until {v21} and continuing in the {RE}, this is only possible at the Apache level.
|Grafana |The xref:grafana#[Grafana plug-in] allows filter cascades when used with the {EE}.
|InfluxDB and Graphite |Only the {EE} provide an interface to connect to xref:metrics_exporter#[external metrics databases].
|Jira |The xref:notifications_jira#[notification method for Jira] is only available in the {EE}.
|ServiceNow |The xref:notifications_servicenow#[notification method for ServiceNow] is only available in the {EE}.
|===
...
|MKP management |Management of xref:mkps#[{CMK} extension packages] is only possible on the command line in the {RE}, and in the {EE} additionally is available via the Setup GUI.
|Predictive monitoring |xref:predictive_monitoring#[Predictive monitoring] with dynamic thresholds based on values determined over a longer period of time can be used In the {EE}.
|===