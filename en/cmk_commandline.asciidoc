...
|`PATH` |Directories in which executable programs will be searched for. For example, {CMK} keeps the site's `bin/` here. If duplicate names are foundgeschützt {CMK} programs have priority – this is important, e.g., for the `mail` command, a special version of which is provided with a {CMK} installation.
...
Alternatively it is possible to also customise the the log levels on the command line in configuration files.
...
* For `cmc_log_levels` the amount of logged data increases with the incrementation of the count. 
...
and stopping sites, for the basic configuration of components, and for
xref:update#[software updates], `cmk` is the most important command.
...
which was introduced to make typing the command quicker. The command includes
a built-in, very detailed online help, that can be called up with `--help` or, more briefly, `-h`:
...
|`cmk -v`|`cmk --verbose` |Prompts `cmk` to produce a detailed dump of its current activity
...
The {CEE} utilise the xref:cmc#[{CMK} Micro Core] (CMC) as its monitoring core, the
{CRE} uses Nagios. An important task for the `cmk` is the generation of a
...
[cols="15,25,~",options=header]
|===
|Short |Longform |Function 

|`cmk -R` |`cmk --restart` |Generates a new configuration for the core and restarts the core (analogous to `omd restart core`). This is the method provided for Nagios.

|`cmk -O` |`cmk --reload` |Generates the configuration for the core and loads this without a restart of the active processing (analogous to `omd reload core`). This is the recommended variant with the CMC. 
...
|`cmk -U` |`cmk --update` |Generates the configuration for the core _without_ activating it. 
...
|`cmk -N` |`cmk --nagios-config`|For diagnostic purposes, this outputs the configuration to be generated on the standard output, without altering the actual configuration file. Here you can enter a host's name simply in order to view the host's configuration (z.B. `cmk -N myserver123`).
|===
...
[{shell}]
----
{c-omd} cmk --check -nv myserver123
...
|`--no-tcp` |This is like `--cache`, however it will interrupt with an error if a cache file is absent or not current. Thus in any situation you can suppress an access to the agent.

|`--usewalk` |For SNMP hosts: instead of accessing the SNMP agent this uses a stored SNMP walk, that has been previously pulled with `cmk --snmpwalk myserver123`. These walks are stored in `var/check_mk/snmpwalks`.
...
`cmk -d` retrieves and displays the outputs from a host's {CMK} agent.
This is not always the same as a `telnet` to port 6556 in a target host,
since here possible settings for 
xref:datasource_programs#[datasource programs], an encryption of the
agent's output and other factors are taken into account. The agent data is thus
retrieved with `cmk -d` in the same way as with the monitoring.
...
Three `cmk` command options give you access to information about these plugins.
...
Using `cmk -m` with no further options will access a complete catalogue of all chcheck plug-in eck man pages.
...
|===
|Domain |Configuration directory |Main file |Changes aktivated 
