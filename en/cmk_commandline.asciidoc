...
|`cmk -m` |xref:info_plugins[Accessing the check plug-ins catalogue]
|`cmk -M df` |xref:info_plugins[Showing a check plug-in manual page] (here of plugin `df`)
2+|*Special topics*
...
* Do not use this command in monitored production hosts which use log file monitoring. Log messages are only sent once by agents, and it can happen that a manual `cmk -nv` ‘catches’ these and that they will then be lost from the monitoring. In such a situation use the `--cache` option.
...
As no hosts fulfil this condition, the output remains empty:
...
Using `cmk -m` with no further options will access a complete catalogue of all check plug-ins man pages.
