...
|Physical location |`/omd/versions/1.6.0p18.cee/ |`/omd/sites/mysite/
...
`/omd/versions/1.6.0p18.cee`, and they are accessible via symbolic links
...
|share/ |The main part of the installed software. Very many components are located in `share/check_mk` – among others, well over 1,400 Check plug-ins.
...
|etc/ |Configuration files. These can be edited either by hand or by using {CMK}'s link:wato.html[Setup]. Note: The scripts in `etc/init.d` are actually also ‘configuration’ files, since they are found in `etc/`. This is based on the same pattern as found in every Linux system under `/etc/init.d/`. We do advise against changing this script however, since this can lead to link:update.html#conflicts[conflicts] during a software update. Changes to the scripts are not necessary.
...
For the `cmc_log_levls` the amount of logged data increases with the
incrementation of the count. There are 8 levels ranging from 0 for [.guihint]#Emergency#
to 7 which stands for [.guihint]#Debug#.

For `notification_log` you can choose between the values 10 for
[.guihint]#Full dump of all variables and command#, 15 for [.guihint]#Normal logging# and 20
for [.guihint]#minimal logging#.

The `alert_logging` can be set to 10 for [.guihint]#Full dump of all variables#
or 20 for [.guihint]#Normal logging#.

With the three values `'terse'`, `'full'` and `None` for
`cmc_log_rrdcreation` you can decide whether the creation of RRDs should
be logged and how.
...
The amount of logged data increases inversely as the count decreases. The lowest
Loglevel is 50 ([.guihint]#Critical#) and while the most data will be logged at 10
which represents [.guihint]#Debug#.
...
The {CEE} utilise the link:cmc.html[{CMK} Micro Core] (CMC) as its monitoring core, the
...
which objects it should provide using the GUI's link:livestatus.html[Livestatus].
...
|cmk -N | |For diagnostic purposes, this outputs the configuration to be generated on the standard output, without altering the actual configuration file. Here you can enter the host's name simply in order to view the host's configuration (z.B. `cmk -N myserver123`).
|===
...
monitoring core or the GUI. Simply enter the `cmk` command and the name
of a host configured in the monitoring directly. Furthermore, you should always
add both of the following options:


[cols=2]
|===

|-v |Check results output: Without this option we will only see the output from the service [.guihint]#Check_MK# itself, and not the results from the other service.
...
services, host tags and other attributes. Because the list of services is so
...
{CMK} provides a large number of ready to use plug-ins as standard. In every
release a few new ones are added, and Version 1.6.0 already includes around
1,500 plug-ins. Three of the call types give access to the list of available
plug-ins. At the same time, any self-written plug-ins stored in `local/`
will also be listed.
...
The link:wato.html[Setup menu] is a great configuration tool. There are however many reasons
