...
The command `cmk -d` retrieves and displays the outputs from a host's {CMK} agent.
With `cmk -d` agent data is retrieved in the same way as with the monitoring.
It is neither validated nor processed.
Thus the data showed exactly matches the data that is turned over to the _agent controller_ (when TLS encryption is enabled) or to a tunneling program in case  xref:datasource_programs#[datasource programs] are configured.
