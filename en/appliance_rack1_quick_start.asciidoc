...
//SK: I know that we repeat as step 6 below, but we really need to hammer this home.
#*Important:* Please take note that, ex-factory, the management interface of all {CMK} racks is *initially deactivated* and needs to be xref:appliance_rack_config#ipmi[enabled manually in the web interface].
...
Please take note that, ex-factory, the management interface supplied with all Checkmk racks is initially deactivated. You must take this into consideration before delivering a rack to a remote location, for example.
