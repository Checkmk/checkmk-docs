...
// 
// Das Skript `mk-job` ist -- wie in {CMK} oft üblich -- ein einfaches Shell-Skript, welches Sie jederzeit überprüfen können. Sie haben also auch bei den wichtigen Jobs auf Ihrem Host jederzeit die maximale Transparenz und Kontrolle.
// 
// 
...
// ----
// 
// Falls Sie das Skript unter AIX oder Solaris installieren wollen,
...
// stellen Sie mit einem Editor Ihres Vertrauens dem Kommando in der Zeile das Skript `mk-job` zusammen mit einem Namen voran.
...
// ob sie das Skript `mk-job` korrekt eingebunden haben,
