...
There is a [.guihint]#Site status# snapin for the side bar which can be added using icon:button_sidebar_add_snapin[alt="Icon to show all snapins."].
...
{cee-only}
With the _Livestatus proxy_ the {CEE} features
...
you can also alter the notification spooler’s log to a higher log level.
