...
If you are already using Livestatus - and hence the option [.guihint]#LIVESTATUS_TCP# is set to [.guihint]#on# - {CMK} will never activate the option `LIVESTATUS_TCP_TLS` automatically during an update (`omd update`), because otherwise the connection would fail if your configuration is to complete yet.
So after the update, to make use of the new feature in your monitoring sites, stop the site and activate the option mentioned:
