...
== Distributed monitoring with Livestatus#livestatus

=== Basic principle

==== Central status

[livestatus|Livestatus] is an interface integrated into the link:cmc.html[monitoring core]
...
=== Installing a distributed monitoring#distr_wato_config
...
. Integrate the slave instances into the master using the ICON[icon_sites.png] [.guihint]#Distributed monitoring# WATO-module
...
Set [.guihint]#LIVESTATUS_TCP}} to {{‘on’# and enter an available port number for
...
When using the link:cmc.html[CMC] as the Checkmk’s core ({EE}), the core’s version number
...
|This instance has outstanding changes. The configuration matches the master, but not all changes have been activated. With the [.guihint]#Restart# button a targetted activation for this instance can be performed.

|ICON[icon_need_replicate.png]
|The WATO-configuration for this instance is not synchronous and must be carried over. A restart will then of course be necessary to activate it. Both functions can be performed with the [.guihint]#Sync & Restart# button.
...
|The instance is currently not reachable. Livestatus queries are running in a _Timeout_. This delays the page loading. Status data for this instance is not visible in the GUI.

|ICON[button_sitestatus_down.png]
|The instance is currently not reachable, but this is due to the setting up of a statushost or is known through the link:distributed_monitoring.html#livestatusproxy[Livestatus proxy] (see link:distributed_monitoring.html#wan[below]). The inaccessability does *not* lead to Timeouts. Status data for this instance is not visible in the GUI. 

|ICON[button_sitestatus_disabled.png]
|The livestatus connection to this instance has been temporarily deactivated by the (master’s) administrator. The setting matches the _‘Temporarily disable this connection’_ check box in the settings for this connection.
...
* Historic [graphing|graphing] will be lost. This can be avoided by manually moving the relevant RRD-files. The location of the files can be found in link:distributed_monitoring.html#files[Files and directories].
...
=== Connecting Livestatus with encryption#livestatus_tls
...
[notifications|Notifications], link:alert_handlers.html[Alert handlers] and everything else.
...
If you monitor with {CMK} link:clusters.html[HA-Cluster], the cluster’s individual nodes
...
=== Instance-specific global settings#sitespecific
...
ICON[icon_sites.png] [.guihint]#Distributed monitoring# WATO-module:
...
* If you use the [.guihint]#Check event state in Event Console}} rule set, switch this back to {{Connect to the local Event Console#.
* If you use the [.guihint]#Logwatch Event Console Forwarding# rule set, switch this likewise to the local Event Console.
* In the Event Console [.guihint]#Settings}}, switch the {{Access to event status via TCP }} back to {{no access via TCP#.
...
== Unstable or slow connections#wan
...
[CRE] With the [.guihint]#Use persistent connections# check box you can prompt the GUI
...
=== The livestatus proxy#livestatusproxy
...
* A dump of the configuration is also possible on a [cmc|CMC-core] -- the importing of which requires Nagios. If the [cmc|CMC] is running on your master use link:distributed_monitoring.html#cmcdump[CMCDump].
...
=== Implementing CMCDump#cmcdump

CMCDump is for the [cmc|{CMK} Micro Core] what link:distributed_monitoring.html#livedump[Livedump]
...
* ICON[icon_ack.png] link:basics_ackn.html[acknowledged], if present
* Whether the object is currently in a ICON[icon_downtime.png] link:basics_downtimes.html[planned maintenace].
...
== Notifications in distributed environments#notifications
...
[cols=<th style="width: 44%">Path</th>,options="header"]
|===
...
|This is generated on the slaves by the distributed WATO and it ensures that the slave only monitors its own hosts.


|`etc/nagios/conf.d/`
|Storage location for customer-created Nagios-configurations files with hosts and services. These are required for the use of link:distributed_monitoring.html#livedump[Livedump] on the master.
...
|`etc/check_mk/conf.d/`
|The configuration of hosts and rules for {CMK}. Store configurations files that are generated by link:distributed_monitoring.html#cmcdump[CMCDump] here. Only the `wato/`subdirectory is managed by, and will be visible in WATO.
...
|`var/check_mk/rrds/`
|Location of the Round-Robin-Database for archiving the performance data when using the {CMK}-RRD-format (the default with the {EE})
