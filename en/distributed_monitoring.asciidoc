...
[cols=2]
|===
|icon:status_label_online[width=80px] |The site is reachable via Livestatus.
|icon:status_label_dead[width=80px] |The site is currently not reachable. Livestatus queries are running in a _Timeout_. This delays the page loading. Status data for this site is not visible in the GUI.
|icon:status_label_down[width=80px] |The site is currently not reachable, but this is due to the setting up of a status host or is known through the xref:livestatusproxy[Livestatus proxy] (see xref:wan[below]). The inaccessibility does *not* lead to Timeouts. Status data for this site is not visible in the GUI.
|icon:status_label_disabled[width=80px] |The livestatus connection to this site has been temporarily deactivated by the (central site’s) administrator. The setting matches the _‘Temporarily disable this connection’_ check box in the settings for this connection.
|===
...
These will be flagged with the icon:status_label_disabled[width=80px] status.
With this you can also disable a icon:status_label_dead[width=80px] site that is generating timeouts, thus avoiding superfluous timeouts:
...
It is annoying when a site doesn’t answer at all. To tolerate brief outages (e.g., due to restarting a site or a lost TCP-Packet), the GUI waits for a given time before a site is declared to be icon:status_label_dead[width=80px], and then continues processing the responses from the remaining sites.
...
The GUI will then be aware of unreachable sites and can immediately exclude and flag them as  icon:status_label_down[width=80px].
...
[cols=2]
|===
|icon:status_label_unreach[width=80px] |The computer on which the remote site is running is just now unreachable to the monitoring because a router is down (the status host has an {UNREACH} state).
|icon:status_label_waiting[width=80px] |The status host that monitors the connection to the remote system has not yet been verified by the monitoring (it still has a {PEND} state).
|icon:status_label_unknown[width=80px] |The status host’s state has an invalid value (this should never occur).
|===
