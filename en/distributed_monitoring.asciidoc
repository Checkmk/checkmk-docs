...
* The {CMK} version (e.g. {v20}) of the remote and central sites is the same -- xref:mixed_versions[mixed versions] are supported only for easier updates.
...
retrieved from an ESXi host to the individual virtual machines. For the same reason
...
{cre-only}
If you are installing the {CRE} (or the CEE with a Nagios core), use the `livedump` tool.
