...
:description: Here you will learn how to set up read-only connections or even a central configuration to other {CMK} sites.
...
* Organisation: Various different groups should be able to administer their own sites independently.
...
configuration using WATO. For situations with very separated networks,
...
In the case of multiple, or all sites needing to be administered by the same
...
and then depending on their tasks, be automatically distributed to the remote sites.
...
. Integrate the remote sites into the central site using the icon:icon_sites[] [.guihint]#Distributed monitoring# WATO-module
...
No special requirements are placed on the central site. This means that a long-established
site can be expanded into a distributed monitoring without requiring additional modifications.

==== Installing remote sites and enabling livestatus via the network

The remote sites are then generated as new sites in the usual way with
`omd create`. This will naturally take place on the (remote) server
intended for the respective remote site.

*Special notes*:

* For the remote sites, use IDs _unique_ to your distributed monitoring.
* The remote's {CMK}-version is permitted to diverge from the central site’s version to a maximum of one patch level (denoted by the numeral following the `‘p’` for stable versions). Other versions _may be_ compatible, but not necessarily. Information on the {CMK} version-numbering system can be found in its link:cmk_versions.html[own article].
...
on the central site. The required WATO-module is icon:icon_sites[]
[.guihint]#Distributed monitoring#, and this serves to manage the connections to the
individual sites. For this function the central site itself counts as a site and is already present in the list:

image::distributed_monitoring_1.png[]

Using icon:button_new_connection[], now define the connection to the first remote site:

image::dm_basic_settings.jpg[]

In the [.guihint]#Basic settings# it is important to use the remote site’s EXACT name
...
the status of the remote sites via livestatus.
...
recommended, provided that the remote site’s Apache supports HTTPS.
...
To this end, via HTTP the central site exchanges a randomly-generated password with the
remote site, through which all future communication will take place.
The `cmkadmin` access on the remote site will subsequently no longer be used.

To login use the access data `cmkadmin` and the according password of the remote site:
...
likewise the WATO-synchronisation states of the individual sites:
...
|icon:icon_need_restart[] |This site has outstanding changes. The configuration matches the central site, but not all changes have been activated. With the [.guihint]#Restart# button a targetted activation for this site can be performed.
|icon:icon_need_replicate[] |The WATO-configuration for this site is not synchronous and must be carried over. A restart will then of course be necessary to activate it. Both functions can be performed with the [.guihint]#Sync & Restart# button.
|===
...
|icon:button_sitestatus_online[] |The site is reachable via Livestatus.
|icon:button_sitestatus_dead[] |The site is currently not reachable. Livestatus queries are running in a _Timeout_. This delays the page loading. Status data for this site is not visible in the GUI.
|icon:button_sitestatus_down[] |The site is currently not reachable, but this is due to the setting up of a statushost or is known through the link:distributed_monitoring.html#livestatusproxy[Livestatus proxy] (see link:distributed_monitoring.html#wan[below]). The inaccessability does *not* lead to Timeouts. Status data for this site is not visible in the GUI.
|icon:button_sitestatus_disabled[] |The livestatus connection to this site has been temporarily deactivated by the (central site’s) administrator. The setting matches the _‘Temporarily disable this connection’_ check box in the settings for this connection.
|===


Clicking on the icon:button_activate_changes[] button will now synchronise
all sites and activate the changes. This is performed in parallel,
so that the overall time equates to the time required by the slowest site.
Included in the time is the creation of a configuration snapshot for the
respective site, the transmission over HTTP, the unpacking of the snapshot
on the remote site, and the activation of the changes.

*Important:* Do not leave the page before the synchronisation has been
completed on all sites – leaving the page will interrupt the synchronisation.
...
When *migrating* hosts from one site to another there are a
...
from one site and _freshly-installed_ on the other site:
...
From version {v16} Livestatus connections between the central site and
a remote can be encrypted. For newly-created sites nothing further needs to done,
...
feature in your monitoring sites, stop the site and activate the
option mentioned:
...
the site then immediately uses the new encryption feature.  So that
you can still access the site from the central site, in the second step
activate the [.guihint]#Encryption# option in the site connection properties
...
individual sites, and it also provides the option of clicking on the status
...
icon:button_sitestatus_dead[] site that is generating timeouts,
...
setting up a new remote site. Share livestatus with
TCP, then add the site to the icon:icon_sites[]
...
Before integrating the site into the distributed WATO as described above,
you should be aware that in doing so the site’s entire local
...
to the central site in order that they can be carried over.
...
Next, move the hosts and rules into the central WATO on the central site.
...
into a remote site's sub-directory using WATO.
...
the tethered site to the same location in the central site.
...
So that the attributes of the central site’s parent folder’s functions are
...
all sites have a common and (apart from the hosts) the same configuration.
What is the situation however, when individual sites require different
global settings?
...
particularly small or a particularly large site.
...
is to operate only a single site in the Event Console – and that one
within the central site. Here is to where you direct all host events.

This setup has the disadvantage that the hosts’ events must be sent to another
site, rather than to the site which is actively monitoring them.
...
running a distributed Event Console. Then every site will run its own
event processing which captures the events from all of the hosts being
monitored from the site. The events will thus _not_ be sent to the
central system, rather they will remain at the sites and be only centrally-retrieved.
...
i.e., the {CMK} site from which the object is to be monitored.
NagVis cannot find the host or service  automatically,
above all for performance reasons.
Therefore if you move hosts to a different remote site you will need to update
...
reliable access to all of the connected sites.
The one snag with this is that a view can only be displayed when _all_
sites have responded. The process is always that first a Livestatus query
...
It is annoying when a site doesn’t answer at all. To tolerate brief outages
(e.g., due to restarting a site or a lost TCP-Packet), the GUI waits for a given
time before a site is declared to be icon:button_sitestatus_dead[],
...
The idea is simple: The central site actively monitors the connection to
each individual remote site. At least we will then have a monitoring system available!
The GUI will then be aware of unreachable sites and can immediately exclude
...
A failed connection to a remote site can now only lead to a brief hangup
...
|icon:button_sitestatus_unreach[] |The computer on which the remote site is running is just now unreachable to the monitoring because a router is down (the status host has an {UNREACH} state).
...
* the remote sites have no permanent network connection and no fixed IP-addresses
...
the central site and are _administered decentrally_.
...
Livedump/CMCDump can help by generating a copy of the remote sites’ configuration
which can then be loaded into the central site.
...
* Actualisation of the states and performance data in the central site will be delayed.
...
* State changes that occurr more quickly than the actualisation interval will be invisible to the central site.
...
* Commands on the central site such as Downtimes and Acknowledgements _cannot_ be transmitted to the remote site.
...
may not be visible, a link:notifications.html[notification] through the central site is not ideal.
If however the central site is utilised as a purely _display site_ – as a central
...
* A dump of the configuration is also possible on a link:cmc.html[CMC-core] -- the importing of which requires Nagios. If the link:cmc.html[CMC] is running on your central site use link:distributed_monitoring.html#cmcdump[CMCDump].
...
If you implement a distributed WATO these rules are the same on all sites.
...
which can be individually activated for each remote site.
...
to be the ‘central’. This task can be taken on by any {CMK} site.

If a remote site has been set to ‘forwarding’, all notifications wiil be
...
for the local site (= central site).
...
received. With a standard loglevel one should see something like this on the central site:
...
Now on the _central site_ an incoming notification should appear in
...
|etc/check_mk/multisite.d/sites.mk |Here WATO stores the configuration for the connections to the individual sites. If the interface ‘hangs’ due to an error in the configuration, so that it becomes inoperable, you can edit the disruptive entry directly in the file. If the livestatus proxy is activated however, it will subsequently be necessary to edit and save at least one connection over WATO, since only with this action will a suitable configuration be generated for this daemon.
...
|etc/nagios/conf.d/ |Storage location for customer-created Nagios-configurations files with hosts and services. These are required for the use of link:distributed_monitoring.html#livedump[Livedump] on the central site.
