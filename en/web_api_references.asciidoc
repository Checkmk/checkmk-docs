...
:description: All of the options for configuring Checkmk via HTTP-API (or Web-API) are described in this article. It is an extension of the introductory article on the Web-API.
...
[{shell}]
----
{c-root} curl "http://myserver/mysite/check_mk/webapi.py?action=edit_users&_username=automation&_secret=myautomationsecret" -d 'request={"users":["customAutomation"]}'
{"result": null, "result_code": 0}
----
...
[{shell}]
----
{c-root} curl "http://myserver/mysite/check_mk/webapi.py?action=get_site&_username=automation&_secret=myautomationsecret&output_format=python" -d 'request={"site_id":"mySlave"}'
...
[{shell}]
----
{c-root} curl "http://myserver/mysite/check_mk/webapi.py?action=delete_site&_username=automation&_secret=myautomationsecret" -d 'request={"site_id":"mySlave"}'{"result": null, "result_code": 0}
----
...
[{shell}]
----
{c-root} curl "http://myserver/mysite/check_mk/webapi.py?action=login_site&_username=automation&_secret=myautomationsecret" -d 'request={"site_id":"mySlave","username":"cmkadmin","password":"cmk"}'
{"result": null, "result_code": 0}
----
...
[{shell}]
----
{c-root} curl "http://myserver/mysite/check_mk/webapi.py?action=logout_site&_username=automation&_secret=myautomationsecret" -d 'request={"site_id":"mySlave"}'{"result": null, "result_code": 0}
----
...
From VERSION[1.5.0b8] SLAs can be retrieved with the `get_sla` command. For such a query, the following time data can be used:
...
A user is not necessarily allowed to access all instances. The following command can be used to find out which instances are available to the automation user. It does not require any options:
...
A typical Web-API command always targets a single specific {CMK} instance.
...
link:web_api_references.html#get_graph[`get_graph`]. Here too you can optionally specify a particular instance:

[{shell}]
----
{c-root} curl "http://myserver/mysite/check_mk/webapi.py?action=get_metrics_of_host&_username=automation&_secret=myautomationsecret" -d 'request={"hostname":"myslavehost345", "site_id":"myslave1"}'
----
