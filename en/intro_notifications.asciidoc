...
:description: For the notifications in {CMK}, their dispatch by email has to be prepared and activated. Next, the notification is tested and fine-tuned.
...
* You can set notifications to specific xref:timeperiods[time periods], for example, to take on-call rosters into account.
...
* Do not activate notifications for other users until you have reduced the false alarms (_false positives_) to a minimum. We have described what you can do for this in the xref:intro_finetune[chapter on fine-tuning monitoring].
...
Linux can be found in the xref:notifications#smtp[article on notifications].
...
earlier in the xref:intro_users#create_users[chapter on user management], for instance
...
test the notifications. It is easier to do this with [.guihint]#Fake check results#,
...
xref:intro_monitor#ack[acknowledge problems] and xref:intro_monitor#downtimes[set scheduled downtimes].
...
image::fake_check_results.png[alt="Dialogue for manually setting the status of a host/service."]
...
* if the status has changed too often recently and the service has therefore been flagged as icon:icon_flapping[alt="Icon for indicating a flapping status."] _flapping_. By the way, this can also happen quickly if you have changed the status frequently using [.guihint]#Fake check results#.
...
details for this in the xref:notifications[article on notifications].
...
practical experience. The question "Why didn't {CMK} notify here?" will therefore
...
find all notification events for this service listed chronologically from the most
recent to the oldest:

image::service_notifications_broken_alarm.png[alt="List of notification events for a service."]
