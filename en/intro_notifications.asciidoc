...
image::fake_check_results.png[Dialog for manually setting the status of a host/service.]
...
* if the status has changed too often recently and the service has therefore been flagged as icon:icon_flapping[Icon for indicating a flapping status.] _flapping_. By the way, this can also happen quickly if you have changed the status frequently using [.guihint]#Fake check results#.
...
image::service_notifications_broken_alarm.png[List of notification events for a service.]
