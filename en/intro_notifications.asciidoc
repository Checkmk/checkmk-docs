...
//
// H1:Grundlegendes zur Alarmierung#notifications
//
...
//
// LI:Sie können per SMS, Pager, Slack und anderen Internetdiensten alarmieren.
// LI:Sie können Alarmierungen an bestimmten link:timeperiods.html[Zeitfenstern] festmachen, z.B. um Bereitschaftsdienste zu berücksichtigen.
// LI:Sie können Eskalationen definieren, falls der zuständige Kontakt nicht schnell genug aktiv wird.
// LI:Benutzer können selbständig Alarme „abonnieren“ oder abbestellen, wenn Sie das zulassen möchten.
// LI:Sie können generell über Regeln festlegen, wer wann über was alarmiert werden soll.
//
// Bevor Sie jedoch mit der Alarmierung beginnen, sollten Sie noch Folgendes beachten:
//
// LI:Die Alarmierung ist ein optionales Feature. Manche Anwender verzichten auf die Alarmierung, da Sie einen Leitstand haben, der rund um die Uhr besetzt ist und nur mit der Statusoberfläche arbeitet.
// LI:Aktivieren Sie die Alarmierung zunächst nur für sich selbst und machen Sie sich für *alles* zuständig. Beobachten Sie mindestens ein paar Tage, wie groß das Volumen an Alarmen ist.
// LI:Aktivieren Sie die Alarmierung für andere Benutzer erst dann, wenn Sie die Fehlalarme (_false positives_) auf ein Minimum reduziert haben. Was Sie dafür tun können, haben wir im link:intro_finetune.html[Kapitel über die Feinjustierung des Monitoring] beschrieben.
//
//
// H1:Mailversand vorbereiten#notify_init
//
...
//
// C+:
// OM:echo "Testcontent" | mail -s Test harry.hirsch@example.com
// C-:
//
...
//
//
// H1:E-Mail-Alarmierung aktivieren#mail_activate
//
...
//
// LI:Dem Benutzer ist eine E-Mail-Adresse zugeordnet.
// LI:Der Benutzer ist für Hosts oder Services zuständig -- über die Zuweisung von Kontaktgruppen.
//
...
//
// LI:wenn die Alarmierung im Snapin link:intro_tools.html#master_control[[.guihint]#Master Control#] ausgeschaltet ist;
// LI:wenn ein Host oder Service sich in einer Wartungszeit befindet;
// LI:wenn ein Host {DOWN} ist und daher keine Alarmierungen seiner Services ausgelöst werden;
// LI:wenn der Status in letzter Zeit zu oft gewechselt hat und der Service deswegen als icon:icon_flapping[] „unstetig“ (_flapping_) markiert wurde. Dies kann übrigens schnell passieren, wenn Sie mittels [.guihint]#Fake check results# den Zustand häufig gewechselt haben.
//
//
// H1:Alarmierung feinjustieren#finetune
//
...
//
// BI:service_notifications_broken_alarm.png
//
