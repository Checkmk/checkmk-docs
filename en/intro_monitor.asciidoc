...
So you have added hosts and looked at some important tools -- and now we are ready to start with the actual monitoring.
...
It is true that the views available by default, such as the [.guihint]#Overview# snapin, already show you very precisely how many and what problems currently exist.
...
You can click on the problematic host or service in the list and then, on the page with its details, perform the acknowledgement only for this individual host or service.
...
*Important:* If you perform an action on a page with a list without checkboxes, then this action will be performed for *all* of the listed entries.
...
Enter a comment and click [.guihint]#Acknowledge# -- and with the confirmation of the 'Are you sure?' question{nbsp}...

image::intro_really_acknowledge.png[alt="Dialog for confirmation of acknowledgement."]
...
* Acknowledgements can run automatically.
The [.guihint]#Expire acknowledgement after# option serves this purpose, but it is only effective in the {CEE}.

For more information on all parameters for acknowledgement actions, see the xref:basics_ackn#[article on acknowledgement].
...
In the list that is displayed, use [.guihint]#Show checkboxes# to show the checkboxes and then select all of the appropriate entries.

Now click [.guihint]#Schedule downtimes#.
This will display the following panel at the top of the page:

image::intro_command_downtime.png[alt="Dialog for setting a scheduled downtime."]

There is a whole range of options for the scheduled downtimes.
You must enter a comment in each case.
By clicking the buttons, you can select the beginning and end of the downtime.
For example, the [.guihint]#2 hours# button declares the selected objects as 'in maintenance' for two hours from the current time.
In contrast to the acknowledgements, scheduled downtimes will always have an end time that is set beforehand.
...
* In views, a service is marked with the icon:icon_downtime[alt="Icon for displaying the scheduled downtime for services."] guiding cone.
A host, including the services sent to downtime with it, is marked with the icon:icon_derived_downtime[alt="Icon for displaying the scheduled downtime for hosts."] blue pause icon.
...
For a detailed description of all of the above-mentioned and other aspects, see the xref:basics_downtimes#[article on scheduled downtimes]
