...
It is true that the views available by default, or e.g. the [.guihint]#Overview# snapin, already show you very precisely how many and what problems currently exist.
...
.The dialog for the scheduled downtime of a host in the {RE}
...
* In views, the selected host or service    is marked with the icon:icon_downtime[alt="Icon indicating a scheduled downtime."] guiding cone.
If a host with all its services is sent to downtime, the services get the icon:icon_derived_downtime[alt="Icon indicating a derived scheduled downtime for a service."] blue pause icon.
