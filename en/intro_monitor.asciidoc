...
//
// H1:Wichtige Aufgaben im Monitoring#monitoring
//
...
//
// LI:die Quittierung von Problemen
// LI:das Senden von Alarmen im Falle von Problemen
// LI:das Setzen von Wartungszeiten
//
...
//
// LI:Im [.guihint]#Overview# wird das Problem in der Spalte [.guihint]#Unhandled# beim Host oder Service nicht mehr gezählt.
// LI:Das Standard-Dashboard listet das Problem ebenfalls nicht mehr auf.
// LI:Das Objekt (Host oder Service) wird in Statusansichten mit dem Symbol icon:icon_ack[] markiert.
// LI:Ein Eintrag erfolgt in die Objekt-History, so dass die Aktion später nachvollzogen werden kann.
// LI:Wiederholte Alarmierungen (falls konfiguriert) werden gestoppt.
//
...
//
// BI:really_acknowledge.png
//
// &#8230; gelten alle vorher ausgewählten Probleme als quittiert.
//
// Abschließend noch einige Hinweise:
//
// LI:Mit dem Knopf [.guihint]#Remove acknowledgement# können Sie eine Quittierung auch wieder entfernen.
// LI:Quittierungen können automatisch ablaufen. Dazu dient die Option [.guihint]#Expire acknowledgement after#.
//
// Weitere Informationen zu allen Quittierungsparametern erhalten Sie im link:basics_ackn.html[Artikel über die Quittierung].
//
//
// H1:Wartungszeiten einrichten#downtimes
//
...
//
// LI:Wenn Sie einen Host in die Wartung schicken, werden automatisch auch alle seine Services mitgeschickt. Sparen Sie sich daher die Arbeit, dies doppelt zu tun.
// LI:Die *flexiblen* Wartungszeiten beginnen tatsächlich erst dann, wenn das Objekt in einen anderen Zustand als {OK} wechselt.
// LI:Wenn Sie die {CEE} nutzen, können Sie auch *regelmäßige* Wartungszeiten definieren (z.B. wegen eines obligatorischen Reboots einmal in der Woche).
// LI:Einen Überblick über aktuell laufende Wartungszeiten erhalten Sie in [.guihint]#Monitor > Overview > Scheduled downtimes#.
//
// Die Auswirkungen einer Wartungszeit sind folgende:
//
// LI:Im [.guihint]#Overview# tauchen die betroffenen Objekte (Hosts und Services) nicht mehr als Probleme auf.
// LI:Die Objekte werden in Statusansichten mit dem Symbol icon:icon_derived_downtime[] markiert.
// LI:Die Alarmierung über Probleme ist während der Wartung abgeschaltet.
// LI:Zu Beginn und Ende einer Wartungszeit wird eine spezielle Alarmierung ausgelöst.
// LI:In der link:availability.html[Verfügbarkeitsanalyse] werden geplante Wartungszeiten gesondert berücksichtigt.
//
