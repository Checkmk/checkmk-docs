...
With the `\` backslash *the following special character is interpreted as normal character*.
...
A question mark is the abbreviation for `{0,1}` – i.e. something that appears once, or never.
It thus designates the preceding character as optional:
