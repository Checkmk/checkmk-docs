...
:revdate: 2019-06-18
:title: Successfully connecting Nagstamon to {CMK}
:description: Nagstamon is a tool which enables administrators to immediately see problems on the desktop without a browser. These instructions help with connecting Nagstamon to your {CMK} installation.
...
link:http://nagstamon.ifw-dresden.de/[Nagstamon] is a small desktop tool
that was originally developed for Nagios. In today's version, the client also
supports {CMK}. It translates a {CMK} view in a very compact form and summarizes
the problems.
...
The simplest method is to connect Nagstamon directly to the {CMK} web UI.
To ensure that this integration works seamlessly, {CMK} delivers two special views,
...
the [.guihint]#Settings# menu item, where you can create a new connection
via [.guihint]#New server ...#.

In the dialog that appears, select the entry [.guihint]#{CMK} Multisite# under
[.guihint]#Monitor type#, assign a human-readable name and enter the URL of the
connection in the [.guihint]#Monitor URL# field.
...
You can then use the [.guihint]#Show more options# option for fine-tuning and if you
have configured your own here views in {CMK}, change to them.
