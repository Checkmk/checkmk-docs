...
For the time being, please refer to the https://docs.checkmk.com/1.6.0/en/notifications_jira.html[article for {CMK} {v16}], which is still valid in most parts. +
By the way, https://docs.checkmk.com/latest/de/notifications_jira.html[this article in German] is already updated -- if that is an alternative for you.
...
Diese Fehlermeldung weist darauf hin, dass die ID nicht korrekt ist, welche Sie in der Benachrichtigungsregel für das monierte Feld (hier: Project ID) eingetragen haben.
