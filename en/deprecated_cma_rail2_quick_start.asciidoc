...
= Quick start guide for Checkmk rail2
:revdate: 2019-04-04
:title: 
:description: 
...
* Select the item link:cma_basic_site_management.html[instance administration] on the menu now shown and click on _Create new instance_ on the following page.
...
* In order to make things such as name resolution and sending emails possible, you must make further settings in the web interface. You can read more about this in the chapter on link:cma_basic_configuration.html[basic configuration on the web interface].

Find out what to do next in the article link:check_mk_getting_started.html[{CMK} Getting Started].
