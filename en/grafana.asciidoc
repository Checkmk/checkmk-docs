...
== Check prerequisites in {CMK}

If you created your site with {CMK} {v21} -- that is, ran the command `omd create` with version {v21} -- the {CMK} Web API is disabled by default in the global settings.
In this case, you need to re-enable the Web API by unchecking [.guihint]#Setup > General > Global Settings > Site management > Disable Web API#.

image::grafana_reenable_web_api.png["Enabling Web API in {CMK} {v21"}."]

Starting with version {v22} of {CMK}, the plugin will support our REST API.
