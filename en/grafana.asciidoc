...
== Introduction
image::grafana_logo.png[float=left, width=20%]
...
image::grafana_new_dashboard.png[align=center, width=370]
...
|$title |The metric’s title as it would be represented in Checkmk.
|$site |The Checkmk instance on which the host and its metric is monitored.
|$host |The host that the metric is associated with.
|$service |The service to which the metric is assigned in Checkmk.
|===
