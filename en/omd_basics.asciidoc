...
Once you are logged in as a site user, you usually don't need to include a site name with `omd` commands, since such a command is applied to the site you are logged in as.

If you have several {CMK} versions installed on your {CMK} server, the corresponding OMD version is also installed with each of these versions.
...
* As a site user, you always use the `omd` commands of the {CMK} version currently installed on the site, which you can display with `omd version`.

* As a `root` user, you run the commands of the default version that is also used when creating a site.
Usually this is the last version installed on the server.
...
|`CORE` |`nagios` ({RE}), +
`cmc` ({EE}) |Selection of the monitoring core. In the {CEE} there is the xref:cmc#[{CMK} Micro Core (CMC)] and the Nagios core. In the {CRE} there is only `nagios` as monitoring core.
...
To update the monitoring software installed on the site to a higher version, the command `omd update` is used.
This is presented in detail in the article on the xref:update#detailed[update of {CMK}].
...
* If you create the backup as a site user, only the site user and his group get read and write access to the tar archive.
...
Restoring a backup is as simple as backing up itself.
The `omd restore` command restores a site  from a backup -- in the {CMK} version that was used to backup the site.
Therefore, for the restore to work, this version must be installed on the server. 
