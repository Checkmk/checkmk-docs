...
The site must be stopped for this procedure.
The site will not be recreated (which would require `root`-permissions), rather it will be completely emptied and then refilled:
After the restore, the site must be restarted:
