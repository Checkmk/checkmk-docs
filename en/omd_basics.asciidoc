...
which you can find  at `cmk/post_rename_site/plugins/actions` in the link:https://github.com/tribe29/checkmk/tree/master/cmk/post_rename_site/plugins/actions[Git repository^].
...
By the way, `omd update` is also used to xref:update#updatedemo[upgrade] to another edition, e.g. from the {RE} to the {SE}.
