...
|`CORE` |`nagios` ({RE}), +
`cmc` (commercial editions) |Selection of the monitoring core. In the commercial editions the Nagios core can be selected instead of the xref:cmc#[{CMK} Micro Core (CMC)]. The {CRE} has only `nagios` as its monitoring core.
