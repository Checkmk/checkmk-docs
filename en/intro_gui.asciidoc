...
Nevertheless, you should familiarise yourself with the basic elements of the interface.
...
*Note:* The [.guihint]#Main dashboard# shown above with problems and statistics of all hosts and services is that of the {RE}.
The {EE} offer by default a different graph-enhanced xref:dashboards#usage[Main dashboard].
...
image::intro_mainpage_default.png[alt="{CMK} main page with 'Main dashboard'."]
...
You set the general behaviour in the settings for your user profile ([.guihint]#User > Edit profile > Show more / Show less > Set custom show mode#) by setting one of the two modes as the default -- or do without both modes and face the unfiltered complexity.
