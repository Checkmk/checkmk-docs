...
image::empty_dashboard.png[alt={CMK} startpage page with Main dashboard still empty]
...
[{image-left}]
image::navbar.png[alt={CMK} navigation bar,width=70]
...
image::mainpage_default.png[alt={CMK} main page with Main dashboard]
...
If you click on icon:button_sidebar_add_snapin[alt=Icon for displaying all snapins] at the very bottom of the sidebar, the main page will show you all of the snapins that are currently
...
* Expand or collapse: Click *in* the displayed title of the snapin. Alternatively, point to the title bar and then click icon:button_show_snapin[alt=Icon for showing the snapin] or icon:button_hide_snapin[alt=Icon for hiding the snapin].
* Show more or less: Some snapins (like [.guihint]#Overview#) offer you two views -- either with only the most important information or with all information. Point the mouse at the title bar and click icon:button_showmore[alt=Icon for switching to Show more mode] or icon:button_showless[alt=Icon for switching to Show less mode] to switch between the two modes.
...
* Remove from sidebar: Point the mouse at the title bar and click icon:button_sidebar_close_snapin[alt=Icon for removing the snapin from the sidebar].
...
image::monitor_menu_showmore.png[alt=First line of the Monitor menu with search input field and Show more symbol]
...
icon:button_showmore[alt=Icon for switching to Show more mode] and show less with icon:button_showless[alt=Icon for switching to Show less mode].
...
image::setup_menu_search.png[alt=Setup menu with search results]
