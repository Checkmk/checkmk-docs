...
in the {EE} these are the xref:cmc_differences.html#fetcher_checker[{CMK} fetchers].
...
|TLS encryption |Data is exchanged in encrypted form via TLS. |All editions as of version {v21}
...
|Individual agent configuration |Per xref:glossar#agent_bakery[Agent Bakery], agents can be individually configured for single or groups of hosts and the agent packages can be created for installation. |{EE}
|xref:agent_deployment#[Automatic agent updates] |The package from the Agent Bakery is first installed manually or via script and is automatically updated from then on. |{EE}
...
{cee-only}
If you use one of the {EE} you can package personalized agents with the _Agent Bakery_.
