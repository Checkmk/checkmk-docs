...
For a monitoring system to receive more information from an endpoint other than that it is accessible, help is required from the target system. 
...
All manageable network devices and appliances have a built-in link:snmp.html[SNMP agent].
...
But even in this case there is usually an additional agent in use which provides further server data to the monitoring.
...
SNMP traps. 
For this entire subject {CMK} has the _Event Console_ which is described in its xref:ec#[own article].
...
This can even be done individually and differently for each host.


== Installation via the download page

The agents are accessed via via [.guihint]#Setup > Agents#.
n the {RE}, the menu items [.guihint]#Linux#, [.guihint]#Windows# and [.guihint]#Other operating systems# will take you directly to the download pages where you will find the pre-configured agents and agent plug-ins, in the following example to the download page for [.guihint]#Linux, Solaris, AIX#:

image::agent_download_linux_cre.png[alt="List of Linux agents for download in the {RE}."]

In the {EE}, the menu item [.guihint]#Windows, Linux, Solaris, AIX# will take you to a page that also gives you access to the Agent Bakery. 
...
For this reason details for specific agents can be found in their own articles:
...
* Utilization of the xref:inventory#[hardware/software inventory] (plug-in required)
...
For Linux you have a choice between the pacpackageket formats RPM (for Red Hat, CentOS, SLES), and DEB (for Debian, Ubuntu), as well as a so-called  'tarball' that is simply unpacked as `root` under `/`. 
...
The [.guihint]#Agent rules# button takes you to a page which lists all rule sets that affect the agents:
...
The result -- you now have two configurations:
...
|[.guihint]#GENERIC# |A virtual host to which *all* rules with no defined additional conditions apply. This entry is especially useful for installing agents on hosts that have not yet been incorporated in the monitoring.
|===
...
In such a case the affected services go into an {UNKNOWN}:
...
The best way is directly from the {CMK} site that is also actively monitoring the agent. 
...
This uses the IP address configured in the [.guihint]#Setup#, allows for a possibly reconfigured port number, and also the case of a special agent:
