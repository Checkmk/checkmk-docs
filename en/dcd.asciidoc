...
{cee-only} The commercial editions of {CMK} support you in this process with a smart tool
...
By default, the xref:wato_user#automation[automation user] exists in {CMK}.
...
Clicking [.guihint]#Credentials: Use "Checkmk Automation" user# will take you to the REST API connection editing page:

image::dcd_connection_RESTAPI.png[alt="Changing the automation user."]
...
It can however not yet run before you have activated the changes – only then does it start functioning.
...
With [.guihint]#Setup > General > Audit log# you open a page with the list of all changes that have been made in
