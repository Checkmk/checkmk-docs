...
:revdate: 2019-10-07
:title: Managing dynamic infrastructures
:description: {CMK} can manage ephemeral infrastructures by dynamically adding and deleting hosts in a fully-automated procedure. 
...
From version {v16} the {CEE} supports your {CMK} in this
...
In version {v16} of {CMK} the DCD is at first equipped with only one connector:
...
The first setting – [.guihint]#Prevent host deletion right after initialization#
– affects a complete reboot of the {CMK} server itself. In this situation
piggyback data for all hosts will at first be missing until the hosts are
queried for the first time. To avoid the senseless deletion and reappearance of
hosts (which is also accompanied by repeated alarms for known problems),
deletions will by default be generally waived during the first 10 minutes. This
time limit can be customized here.

The option [.guihint]#Keep hosts while piggyback source sends piggyback data
only for other hosts for# handles the case that there is still piggyback data,
but not for some of the automatically created hosts. Here you can specify which
amount of time such hosts should be retained for, before WATO automatically
removes them. This is the normal case when e.g. virtual machines or cloud
services are no longer available. If you want the corresponding objects from
{CMK} to then disappear promptly, then set a correspondingly short time period
here.

The [.guihint]#Set period how long outdated piggyback data is treated as valid#
deals with the case where a host whose monitoring data has been automatically
created a number of hosts, no longer provides piggyback data. The existing data
is then considered valid for the period set here. valid for the time period set
here. This can be the case, for example, if access to AWS and Co. no longer
works. Or, of course, if you have removed the special agent from the
configuration. removed from the configuration. The automatically created hosts
then remain the set time in the system before they are removed from WATO.
