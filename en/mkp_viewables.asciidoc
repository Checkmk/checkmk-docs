//encoding: utf-8
// IGNORE CLI Viewables unpackaged check_mk gui mydashboard packages_local ~ enabled_packages state_host
include::global_attr.adoc[]
= MKPs for GUI extensions
:revdate: draft
:title: {CMK} extensions packages (MKPs) for dashboards, views, reports
:description: Dashboards, views, and reports can be easily packaged and shared with others - even via the {CMK} Exchange.

{related-start}
xref:devel_check_plugins#[Writing own check plug-ins]
xref:dev_guidelines#[Guidelines for check plug-ins]
xref:mkps#[Extension packages (MKP)]
{related-end}
...
== Einleitung
{cee-only} Die xref:mkps#[{CMK}-Erweiterungspakete] (MKPs) können neben funktionalen Erweiterungen auch solche für die GUI verpacken, also Dashboards, Ansichten oder Berichte.
...
Über icon:icon_new_mkp[] [.guihint]#Create Packages# erstellen Sie nun das Paket.
...
. Listen nicht paketierter Dateien: +
`mkp find`
...
{cre-only} Ein Hinweis für Nutzer der {RE}: Auch hier können Sie GUI-Erweiterungen wie alle anderen MKPs installieren.
Da die {EE} jedoch einige exklusive visuelle Elemente (etwa Dashlets) enthalten, könnten Sie über eine Meldung folgender Art stolpern:
[.guihint]#Dashlet type 'state_host' could not be found. Please remove it from your dashboard configuration.#
