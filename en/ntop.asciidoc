...
The ntopng integration is a chargeable add-on available for the {CSE}.
...
({CMK} communicates with ntopng via its REST API.)
...
{cee-only}
To use the integration of ntopng in {CMK}, you need the {CSE} with the add-on for ntopng integration and an ntopng in a Professional or Enterprise version >= 5.6,
