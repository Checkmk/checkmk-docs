...
After you have link:wato.html#activate_changes[activated the changes], all {CMK} users will see the user interface changes for ntopng as described in the link:ntop.html#ntop_info_show[chapter on displaying the ntopng information], for example the extension of the [.guihint]#Monitor# menu. However the ntopng data will only be visible to those {CMK} users who have a user account on the ntopng counterpart. The other {CMK} users will be able to open the ntopng-specific pages, but will only receive blank pages or an error message that the login data is invalid.
