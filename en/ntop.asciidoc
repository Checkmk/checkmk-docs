...
:description: ntopng is the powerful and resource-saving software for monitoring and analysing network traffic. Through the integration of ntopng, the functional scope of {CMK} is extended by two important tasks -- the monitoring of the network data flow and the in-depth analysis of the network traffic.
...
. {CMK} and ntopng are installed and configured as independent systems on (usually) different servers.
...
Once the requirements described in the chapter link:ntop.html#ntop_connect[connecting to ntopng] have been met, you will now have access to the ntopng data in {CMK}. To do this, log in to {CMK} under a user account that is assigned to an ntopng user account.
...
* [.guihint]#Flows#: Table of the data flows that have the selected host as a start or end point. For more information, see the chapter link:ntop.html#ntop_flows_dashboard[Flows Dashboard].

* [.guihint]#Engaged Alerts#, [.guihint]#Past Alerts#, [.guihint]#Flow Alerts#: tables of active alerts, past alerts and flow alerts. For more information, see the chapter link:ntop.html#ntop_alerts_dashboard[Alerts Dashboard].
...
To use the ntopng-specific entries, you must link:views.html#edit[Customise the view]. In the menu, select [.guihint]#Display > Customize view#. On the [.guihint]#Clone view# page, the columns of the table contents can be defined.
...
You can switch between the alarm tables with the following tabs:
...
In all tabs, there are two bar charts above the table with a breakdown by date and time, which you can use to filter the table: By dragging with the mouse, select a time interval in one diagram -- or successively in both. The filter defined in this way is immediately applied to the alarm table. To switch off the filter, click outside the marking in the diagram.

*Note:* This dashboard is similar in structure to the three alarm-specific tabs on the [.guihint]#Network statistics and flows# page (see the chapter link:ntop.html#ntop_nw_statistics[Network statistics for a host]).
...
*Note:* This dashboard is similar in structure to the [.guihint]#Flows# tab on the [.guihint]#Network statistics and flows# page (see the chapter link:ntop.html#ntop_nw_statistics[Network statistics for a host]).
