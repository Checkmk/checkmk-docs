...
This happens on the one hand in new xref:glossar#dashboard[dashboards] in which the ntopng information is prepared for {CMK} users.
...
In each host view, you can use the [.guihint]#Ntopng Host# filter to create the intersection of {CMK} and ntopng hosts - like this:
...
Select the filter [.guihint]#Ntopng Host# from the list, set it to [.guihint]#yes# and activate it with [.guihint]#Apply filters#.

The view will then only display hosts that are in {CMK} _and_ in ntopng:
...
The network data of the selected host is then presented on this page from different perspectives on the following tabs:
...
You can add the following ntopng-specific entries with [.guihint]#Add column#:
...
*Note:* This dashboard is similar in structure to the three alert-specific tabs on the [.guihint]#Network statistics and flows# page (see the section xref:ntop_nw_statistics[Network statistics for a host]).
...
*Note:* This dashboard is similar in structure to the [.guihint]#Flows# tab on the [.guihint]#Network statistics and flows# page (see the section xref:ntop_nw_statistics[Network statistics for a host]).
...
Each host is clickable and then opens the corresponding ntopng page with the host details.
...
*Note:* This dashboard is similar in structure to the link:https://www.ntop.org/guides/ntopng/web_gui/dashboard.html#professional-enterprise-dashboard[Traffic Dashboard^] in the ntopng interface.
