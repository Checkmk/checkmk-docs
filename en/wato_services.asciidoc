...
menu of the service link:wato_services.html#discovery_check[[.guihint]#Check_MK Discovery#] of a host
...
link:wato_hosts.html#bulk_operations[Bulk Operations]. Firstly, choose the hosts
...
. Search for hosts with link:wato_hosts.html#search[Host search], select all hosts
...
link:wato_services.html#enforced_services[enforced services] or -- and this is
...
is a collection of link:wato_rules.html[rule sets] which have exactly the same
...
[.guihint]#Periodic service discovery# link:wato_rules.html[rule set]. In a site
...
First, you need to notify the core of the service. This is done with the
...
link:views.html[views] and NagVis maps, and to switch targeted
link:notifications.html[notifications] and link:alert_handlers.html[alert
