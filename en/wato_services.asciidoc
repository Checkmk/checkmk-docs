...
== Host services in WATO#discovery
...
* via the ICON[icon_services.png] symbol in the list of hosts in a folder in WATO
* via the ICON[icon_services.png] [.guihint]#Edit services}} entry in a host’s service’s context menu link:wato_services.html#discovery_check[{{Check_MK Discovery#]
...
The usual method is to simply save with ICON[button_monitor.png], followed
by an [.guihint]#Activate changes# -- after which the host will be in the monitoring.


=== Adding missing services#available

For a host that is already being monitored this list looks different. Instead
of [.guihint]#Undecided services (currently not monitored}} you will see {{Monitored services#.
...
A click on
ICON[button_monitor.png] simply adds all of the missing services
...
and again save them with ICON[button_monitor.png].
...
ICON[button_fix_missing_vanished.png] button that appears in such a case.
...
disable them with ICON[button_disable.png] or set them to the status
_Undecided/New_ with ICON[button_undecided.png]. And naturally,
...
=== Special conditions with SNMP#snmp
...
== Bulk Discovery -- simultaneous discovery on multiple hosts#bulk_discovery
...
. Search for hosts with link:wato_hosts.html#search[Host search], and then press ICON[button_discovery.png] in the search results
...
|Hosts for which an earlier service discovery via bulk operations has failed (e.g. because the host was not accessible at the time), are flagged with the ICON[icon_inventory_failed.png] symbol.  This option allows the discovery to be repeated only for these hosts.
...
The [.guihint]#Performance Options}} are predefined so that a {{Full Scan#
...
== Check parameters in services#parameters
...
parameter page.  This can be accessed via the ICON[icon_check_parameters.png]
...
=== Monitoring of processes#processes
...
[.guihint]#Windows Service Discovery}} (_what_) and {{Windows services# (_how_)
...
=== Monitoring of switch ports#switches
...
* The use of the [.guihint]#Description}} or the {{Alias# in service names
* The restriction or _expansion_ of the types or names of interfaces being monitored


== Setting-up services manually#manual_checks
...
for this is the ICON[icon_static_checks.png] [.guihint]#Manual Checks# WATO
...
ICON[icon_static_checks.png] [.guihint]#Manual Checks# module, they are only present
...
== The discovery check#discovery_check
...
=== Automatic check for unmonitored services#discovery_auto
...
== Service discovery on the command line#commandline
...
such as [.guihint]#Filesystem}} and {{myservice# to create a set of file systems.
...
== More on Check plug-ins#checkplugins
