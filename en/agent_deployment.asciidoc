...
ICON[icon_agents.png] package for the host from the WATO at [.guihint]#Monitoring Agents#.
...
while the [.guihint]#Update Check}} and {{Update Check Output# fields come from
...
=== Viewing the local configuration#show_config
...
== Agent updates in distributed monitoring#distr_wato
...
== FAQ
=== Typical errors and their solutions#faq
