...
However, as soon as imported certificates from one of the mentioned sources are available to the agent updater, all other CA certificate authorities are ignored! In case of problems with the configuration of HTTPS, please consult the xref:agent_deployment#faq4[following FAQ] below.
...
xref:agent_deployment#create_signature_key[step 1.1]. To do this, click [.guihint]#Sign agents#, select the desired key, enter the passphrase that matches the key and click [.guihint]#Sign#. All agents signed in this way are marked with a corresponding icon [icon_signature_key.png]. If you have created multiple keys, the signature
