...
////
MFS: Hi Anthony, most of the article is up to date. Only the section "bake agents"
and "sign agents" are a single option now "bake and sign agents" so this part has
be translated compleately. Please also proofread the complete article: I have
identified many passages that use "European Pidgin English". These have to be
rectified by some one whose native language is English.
////
...
// MFS: There is a new section, please translate
////
...
// MFS: Baking and signing is done in a single step, so the following 2 sections have to be replaced by a complete translation of the german section following afterwards and commented out.
[#bakery]
=== Baking agents
...
This way you can be notified about a problem with the updates.
...
link:distributed_monitoring.html[central configuration] and a connection which allows the
...
site -- accessible from the host -- in the entire distributed monitoring system.
...
link:agent_deployment.html#fixed_update_server[rule set for the Agent Updater].
...
To be able to address a {CMK} instance via HTTPS, the monitoring server must
...
link:agent_deployment.html#update_server_information[Update Server Information].
...
is stored on the server, i.e. is at the end of the certificate chain. The latter
