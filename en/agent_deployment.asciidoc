...
In the case of distributed monitoring with a centralized configuration, the agent updater receives its relevent update server by default from the {CMK} site to which it connects.
...
===== The update server's {CMK} site name
...
* Linux (deb, rpm, tgz): You do not have to manually edit anything for these systems; the agent updater is passed as a 64bit binary. You can also optionally select a 32bit binary for legacy systems, or the old Python script. *Important*: For the binary you will need the _glibc_ package (minimum the 2.5 version). Linux distributions have generally met these requirements since 2006.
...
* Solaris: You do not have to modify anything here either. {CMK} will use the Python script even if you leave the default value on the 64bit binary.
...
* Under Windows: The agent updater is integrated into the Windows agent `C:\Program Files (x86)\checkmk\service\check_mk_agent.exe`.
...
If you have so far followed these instructions from the beginning and have installed a baked agent, not all of the following settings will be asked for.
These settings are shown here for reference purposes only.
...
In this list you will also find documention on how the hash of an agent starts, which agent is intended for a host ([.guihint]#Target Agent#),
...
At various points in this article, there are references to securing the respective connections via HTTPS. At this point, a central summary is provided showing everything that needs to be done for complete security via HTTPS. Both the connection from the remote site to the central site and from the host to the {CMK} site (regardless of whether it is a single-site setup or a distributed setup) can and should be secured via TLS, i.e. via HTTPS.
...
suitable CA or self-signed certificates. This can be realized as follows:
...
HTTPS connections and terminates a connection if this is not possible. Certificates for
...
so that the registry information does not get lost. A new installation will find the file and continue using it. If this situation is undesirable, after a deinstallation simply delete the `cmk-update-agent.state` file manually.
...
_OpenSSL_ tool. Due to the chain’s length, for clarity here only a relevant section is shown and the omitted sections marked by _[...]_:
