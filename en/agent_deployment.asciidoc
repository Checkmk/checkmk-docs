...
To set up the updates, follow these steps: First open [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX# and select [.guihint]#Agents > Automatic updates#.
...
the affected hosts should be ‘moved’ to another {CMK} site. In this case,
...
In a distributed setup with central configuration, the site where you want to register may also be different from the central site where this rule is configured.


[#update_protocol]
==== Protocol for fetching updates
If -- as we recommend -- you use HTTPS you must ensure that the agent updater also has a CA certificate available to verify the connection. *Important*: Depending on the configuration of the server, HTTPS (including forwarding from HTTP to HTTPS) may be forced. In such a case, configuring this rule to HTTP has no effect.


[#certificates_for_https]
==== Encrypted communication for updates

CA or self-signed certificates configured here are available to the agent updater for the verification of HTTPS connections. Alternatively, in the case of a distributed setup with central configuration, certificates can also be made available to the agent updater from the {CMK} site, or these can be imported directly during the connection via command line parameters.
...
However, as soon as imported certificates from one of the mentioned sources are available to the agent updater, all other CA certificate authorities are ignored! In case of problems with the configuration of HTTPS, please consult the link:agent_deployment.html#faq4[following FAQ] below.
...
* Windows: The 64bit binary is also the default -- if necessary you can select the 32bit binary. The script is not available here and will be ignored accordingly. *Note*: As of Version {v16} of {CMK}, the plug-in is always delivered as a 32-bit executable code. Any existing setting in this rule is ignored.
...
Next, you can already bake the agents configured in this way. To do this, navigate again to [.guihint]#Setup > Agents > Windows, Linux, Solaris AIX# and then click on [.guihint]#Bake agents#. This is because the created and adapted rules will only then found in the installation packages after you create/bake them again. Once this process has been completed you will receive a confirmation:
...
Next, sign the agents with the key created in
link:agent_deployment.html#create_signature_key[step 1.1]. To do this, click [.guihint]#Sign agents#, select the desired key, enter the passphrase that matches the key and click [.guihint]#Sign#. All agents signed in this way are marked with a corresponding icon [icon_signature_key.png]. If you have created multiple keys, the signature
...
From now on, the agent will report at the end of the configured update interval and look for a new version of the agent.
...
The video at the following link, which was made at the {CMK} conference #3 (2017), also offers step-by-step instructions. This is not the current version, but the basic procedure has not changed:
link:https://www.youtube.com/watch?v=S7TNo2YcGpM&#t=12m47[The New Automatic Agent Updates]
...
* The link:agent_deployment.html#show_config[host secret], which was automatically assigned during registration.
...
//
//
// H2:Funktionsweise#functionality
//
...
//
//
// H2:Konfiguration#configuration_distributed_monitoring
//
...
//
//
// AN:connection_to_central_bakery
// H3:Connection to central agent bakery
//
...
//
//
// AN:connection_to_remote_bakery
// H3:Connection to remote agent bakery
//
...
//
//
// H1:HTTPS-Handling#https_handling
//
...
//
//
// H2:HTTPS verwenden#https_usage
//
...
//
//
// H2:Zertifikate bereitstellen#provide_certificates
//
...
//
//
// AN:connection_to_cmk_server
// H3:Verbindung von Host zu {CMK}-Server
//
...
//
//
// AN:connection_from_remote_to_central_site
// H3:Verbindung von Remote Site zu Central Site
//
...
link:agent_deployment.html#certificates_for_https[HTTPS configuration].
