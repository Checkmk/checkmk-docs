...
In the case of distributed monitoring with a centralized configuration, the agent updater receives its relevant update server by default from the {CMK} site to which it connects.
...
* Windows: For Windows hosts {CMK} will always deploy a 32-bit-executable. This rule is being ignored for Windows hosts. *Note:* Should you find a 64-bit binary of the agent updater on any of your Windows hosts, this version dates back to an older version of {CMK} and is not up-to-date.
* Solaris: You do not have to modify anything here either. {CMK} will use the Python script even if you leave the default value on the 64-bit binary.
...
After this last setting, your rule could look like the following screenshot. Save all your entries by clicking on icon:icon_save[] [.guihint]#Save#.
...
==== Registering agents under Linux
...
==== Registering agents under Windows
...
The prerequisite for this is a setup with a xref:distributed_monitoring#[central configuration] 
...
In this case, however, be sure to follow the notes on mixed-version monitoring environments in xref:distributed_monitoring#mixed_versions[distributed_monitoring].
...
This is independent of whether it is a single site setup or a distributed setup.
...
*Certificate via the command line:*
The Agent Updater can be invoked with the `--trust-cert` command line argument.
...
*Caution:* If a certificate is imported in this way, the user must ensure the authenticity of the server themselves, since the certificate does not come from an independent source.
...
*Caution*: The certificate check will actually be completely disabled for this call.
The communication will still be encrypted -- so using this argument is "better than nothing".
...
_OpenSSL_ tool. Due to the chain’s length, for clarity here only a relevant section is shown and the omitted sections marked by `[...]`:
