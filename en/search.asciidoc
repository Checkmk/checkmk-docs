...
// IGNORE Asciidoctor js Lunr performant
...
Such a solution is performant and it will be the most efficient way to deliver the User guide to you for many years to come.
...
[cols="30,~"]
|===
|`find=searchterm` |At least one search term is required. You can separate multiple search terms with `+`.
...
For example `+` will be replaced by `%2B` and `-` by `%2D`.
