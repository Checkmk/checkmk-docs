...
A big advantage of this procedure over the widely-used 'copy & paste' approach of data base oriented configuration systems is that you can predefine attributes for hosts that will be registered *in the future*.
...
|4. |Via xref:wato_rules#[rule set] [.guihint]#Hosts with dynamic DNS lookup during monitoring# you determine hosts for a dynamic DNS. The result is similar to 3., except that the  [.guihint]#Hostname#  field is used for DNS query.  |during check execution
|===
