...
=== Folders and their hierarchies#folder
...
You create new folders via the ICON[icon_newfolder.png] button. The
options are the same as when creating new hosts, which we will explain in
detail below.

== Creating hosts in WATO#create_hosts

You can manage folders and hosts via the ICON[icon_folder.png] [.guihint]#Hosts#
WATO module:

[.border]
image::wato_hosts.png[]

The create host ICON[icon_new.png] button, the clone
ICON[icon_insert.png] button and the edit host ICON[icon_edit.png] button
...
* in the ICON[icon_menu.png] menu, by selecting the [.guihint]#Check_MK Discovery}} service with the ICON[button_services.png] {{Edit Services# entry
...
* The usual method when creating a *new* host is to use the [.guihint]#Save manual check configuration}} button, which adopts all services to be found for monitoring ({{Available (missing) services#).
...
== Importing hosts from CSV data#import
...
hosts with the [.guihint]#Perform automatic service discovery# option:
...
and what the states of [monitoring_basics#hosts|Hosts] and link:monitoring_basics.html#notifications[Notifications]
...
automatically using a scan. The [.guihint]#Parent Scan}} can be found in {{WATO|Hosts#
...
ICON[icon_parentscan.png] [.guihint]#Parent scan# icon.
...
the [.guihint]#Include all subfolders}} and {{Scan all hosts# options.
...
and likewise customise [notifications|notifications] and link:alert_handlers.html[alert handlers].
...
in rules for [notifications|notifications] and link:alert_handlers.html[alert handlers].
