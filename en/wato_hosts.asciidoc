...
|[.guihint]#Normal {CMK} agent# |The host should be monitored via the   link:wato_monitoringagents.html[{CMK} agents]   (which must be installed of course).   Select this setting also in the case of _special agents_, such as e.g., ESX-Monitoring
...
Starting with version {v16} of {CMK} there is the new concept of
...
But let us remind ourselves: in the link:wato.html[{CMK} configuration}] introduction article we
