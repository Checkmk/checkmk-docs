...
It’s not just about registering the correct master data (e.g., host name, IP address) -- settings for the monitoring (e.g., alarms, threshholds, etc.) also need to be attended to.
...
The hosts A, B and _ are in the folder ‘Servers‘ and D, E and F in 'Network' :
...
image::wato_folders_step_4.png[alt="alt="Illustration of a folder structure with assigned attributes on different levels.",width=500px]
...
Everywhere in {CMK} the [.guihint]#Hostname# field serves to explicitely identify the host. 
...
You assign a label to the host by first selecting [.guihint]#Labels#. Then click in the [.guihint]#Add some Label# field, enter the label definition and finish it with [Enter]. Use the crosses to remove labels.
...
* The usual method when creating a *new* host is the [.guihint]#Monitor undecided services# button, which adopts all services to be found for monitoring: [.guihint]#Undecided services (currently not monitored#)

* If you open an existing host’s page and find services that are not currently being monitored, then the [.guihint]#Fix all# button is useful. 
This adds the missing services, removes services that have disappeared and takes over host labels that have been found.

* The [.guihint]#Full service scan# button enables fresh, complete data to be obtained from a target device. 
{CMK} works with cached data to enable the rapid loading of pages for a normal monitoring’s displays. 
With SNMP devices the button starts an active search for new check plug-ins and can possibly find further services.
...
These always apply for hosts that are located directly in a folder.
...
* [.guihint]#Edit attributes# - Changes one or more attributes on all selected hosts. 
The attribute is thereby entered explicitely in the hosts. 
+
...
The search always preceeds from the current folder recursively through all subfolders. 
...
This means that you can use the xref:bulk_operations[bulk actons] here, which are offered in the [.guihint]#Hosts# menu in the [.guihint]#On selected hosts# section, e.g. to move all hosts found to a specific folder.

If you don’t like the results, you can adjust and refine the seach on the results page with [.guihint]#Refine search#.
...
The exact internal IDs can be found under [.guihint]#Setup > Hosts > Tags#.
...
If the monitoring is a new installion, which from the very beginning has been planned with an orderly folder and parents structure, there should be no real problems with the inheriting of parents via folders. 
...
image::hosts_parents_list.png[alt="List of host with display of parents."]
...
Renaming hosts -- on the face of it a simple matter -- turns out to be an astoundingly-complex operation on closer inspection. 
The reason for this is that {CMK} uses the host’s name as the unique key for the host – and this is used in numerous locations. 
These include file names, log data, configuration rules, BI agreggations, reports, dashboards and much more.
...
n the [.guihint]#Hostname matching# field, first optionally specify a regular expression that matches the *beginning* of the names of the hosts you want to rename -- in this example, all hosts whose names begin with `lnx`. 
...
On the one hand, in the in the filter bar of xref:views#[table views], there is the [.guihint]#WATO Folder# filter, that you can use to restrict the current view to only those hosts below a particular folder:

image::hosts_filter_wato_folder.png[alt="View filter bar with 'WATO Folder' filter.",width=60%]

On the other hand, you can also use the [.guihint]#Folders# snapin in the xref:user_interface.html#sidebar[sidebar] to restrict the view to a folder:

image::hosts_folders_snapin.png[alt="Folders snapin.",width=50%]

This element functions in conjunction with the [.guihint]#Views# snapin. 
Once selected, a folder is retained even if you select another view. 
