...
Here the site `mysite1` uses version `1.6.0p13.cee`,
...
The second step will now be an update of the site from `1.6.0p10.cre`
...
If you are planning to update the {CMK} version {v16} to {v20}, you should first read the xref:update20[Update to version 2.0.0] article, in which we have compiled the most important issues you should consider before and after such an update.

But even if you have already installed a {v20} version and want to update to a new stable patch version of {v20}, some of the points mentioned in the article may be relevant, for example, the chapter on xref:update_20#local_files[local files] of your site.
...
xref:wato[Activate Changes] will be automatically executed in order to generate a valid configuration for the core.
...
Updating a {CMK} docker container is a breeze. Just follow the appropriate instruction in link:managing_docker.html#update_easy[{CMK} in a Docker container].
...
The fundamentals remain valid as for a xref:update#detailed[normal update of {CMK}].
...
. Change the desired sites to that version in the link:appliance_usage.html#site_management[site management] of the appliance.
...
* There are incompatible scripts in the local part of the site directory structure, i.e. in `~/local`.

See the article on updating {CMK} version {v16} to {v20} for advice on handling xref:update20##incompatible[incompatible changes], xref:update20#local_files[local files] and xref:update20#incompatible_mkps[incompatible and obsolete MKPs].
