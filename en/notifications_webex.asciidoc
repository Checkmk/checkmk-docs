...
In order to be able to receive notifications from {CMK} in Cisco Webex Teams,
first you have to install the appropriate extension package. This extension
packge was added to {CMK} in the
link:https://blog.checkmk.com/feature-pack-2-comes-with-prometheus-integration#how-to-install-feature-pack[Feature Pack 2]
and delivered within {CMK} {v16}p12. If you are running at least this patch
level, you can install the extension via WATO > Extensions Packages and
clicking the install button icon:install_extension[] next to [.guihint]#cisco_webex_teams_notifications#.
