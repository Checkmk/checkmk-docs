...
[.guihint]#Site# from which you want to collect the data to be selected in the
to be collected.

=== General information

Under [.guihint]#General information# you will find basically only a hint, that
the information about the {CMK} version and the used operating system will be
will be included in any case. If you were to leave it at that -- i.e. select
none of the options -- and start the collection of the data via
[.guihint]#Collect dump#, you would get a file that only contains the
following data:
...
your configuration of OMD. These correspond exactly to what you would get
on the command line with the command `omd config show`.
...
about *all* sites running on your {CMK} server.
It also creates a list of all installed {CMK} versions.
And in case the site selected above is the node of a cluster
this fact will also be recorded here.

After activating the [.guihint]#{CMK} Configuration files# option you have the
possibility to remove more or less confidential data from this part of the of
the package -- via the [.guihint]#Pack only Low sensitivity files# preference.
All files you can select here come from the directory `~/etc/checkmk` and its
subdirectories. You can see a detailed list directly below the dropdown menu.
With the option [.guihint]#Select individual files from list# you even have the
option to include only specific files into the dump.

With this option you can also see which files have which confidentiality level
so High (H) for files with e.g. passwords, Medium (M), if they contain addresses
...
with it. Especially in case of problems with the performance of a {CMK} site
...
[.guihint]#global.mk#-files of the individual components of your {CMK} site,
...
corresponding configuration files and a selection of log files. In case of
difficulties with your notifications you or in last site the {CMK} support can
often find the cause in these logs.

*Note:* In order to find as much detailed information as possible in the logs
about about the behavior of {CMK} in the logs, it may be necessary to change the
log level in {CMK}. log level in {CMK} for a short time. The corresponding
settings can be found via [.guihint]#Setup > General > Global settings.# On this
page, simply enter `logging'. on this page, type `logging` into the
...
to run for a few minutes or repeat a reproducible error, the chances increase
that information about this error show up in the log.
