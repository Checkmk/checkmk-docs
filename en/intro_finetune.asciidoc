...
'_On all production Oracle servers, tablespaces DW20 and DW30 will at 90{nbsp}%fill level be_ {WARN} _and at 95{nbsp}% be_ {CRIT}'.

You can then search for an appropriate rule set -- in this example via the rule search: [.guihint]#Setup > General > Rule search#.
This opens a page in which you can search for 'Oracle' or for 'tablespace' (case-insensitive) and find all the rule sets that contain this text in their name or in their description (not shown here):
...
*Note:* A detailed description of all the other parameters and a detailed explanation of the important concept of rules can be found in the xref:wato_rules#[article on rules].
...
*Tip:* If you disable an individual service in the service configuration of a host by clicking on icon:icon_move_to_disabled[alt="Icon for disabling a service."], a rule is automatically created for the host in this very rule set.
