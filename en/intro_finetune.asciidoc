...
//
// H1:Fehlalarme -- der Tod jedes Monitorings#false_positives
//
...
//
// LI:Nehmen Sie nicht zu viele Hosts auf einmal ins Monitoring auf.
// LI:Sorgen Sie dafür, dass alle Services, bei denen nicht wirklich ein Problem besteht, zuverlässig auf {OK} sind.
// LI:Aktivieren Sie die Alarmierung per E-Mail oder SMS erst, wenn {CMK} eine Zeit lang zuverlässig ohne oder mit sehr wenigen Fehlalarmen läuft.
//
...
//
//
// H1:Regelbasiert konfigurieren#rules
//
...
//
//
// H1:Regeln erstellen#create_rules
//
...
//
// BI:ruleset_search_tablespace.png
//
...
//
// BI:ruleset_oracle_tablespaces.png
//
...
//
// LI:Der Name dieser Bedingung passt sich dem Regeltyp an. Wenn hier [.guihint]#Explicit Services# steht, geben Sie die *Namen* der betroffenen Services an. Ein solcher könnte z.B. `Tablespace DW20` lauten -- also inklusive des Worts `Tablespace`. Im gezeigten Beispiel hingegen möchte {CMK} von Ihnen lediglich den Namen des Tablespaces selbst wissen, also z.B. `DW20`.
//
// LI:Die eingegebenen Texte werden immer gegen den Anfang gematcht. Die Eingabe von `DW20` greift also auch auf einen fiktiven Tablespace `DW20A`. Wenn Sie das verhindern möchten, hängen Sie das Zeichen `$` an das Ende an, also `DW20$`, denn es handelt sich hier um sogenannte link:regexes.html[reguläre Ausdrücke].
//
...
//
// BI:ruleset_ora_one_rule.png border
//
...
//
//
// H1:Host-Merkmale (host tags)#host_tags
//
// H2:So funktionieren Host-Merkmale
//
...
//
// BI:new_taggroup_choices.png
//
// Hinweise:
//
// LI:Die IDs müssen über alle Gruppen hinweg eindeutig sein.
// LI:Auch Gruppen mit nur einer einzigen Auswahl sind erlaubt und sogar sinnvoll. Diese erscheinen dann als Checkboxen. Jeder Host hat dann das Merkmal --  oder eben nicht.
// LI:An dieser Stelle können Sie die Hilfsmerkmale (_auxiliary tags_) erstmal ignorieren. Sie erhalten alle Informationen zu Hilfsmerkmalen im speziellen und zu Host-Merkmalen im allgemeinen im link:wato_rules.html[Artikel über Regeln].
//
// Sobald Sie mit [.guihint]#Save# die neue Host-Merkmalsgruppe gesichert haben, können Sie sie nutzen.
//
//
// H2:Ein Merkmal einem Host zuordnen
//
...
//
//
// H1:Dateisystem-Schwellwerte anpassen#filesystems
//
...
//
// LI:Legen Sie Ihre eigenen Regeln an im Regelsatz [.guihint]#Filesystem (used space and growth)#.
// LI:Die Parameter erlauben Schwellwerte, die von der Größe des Dateisystems abhängen. Wählen Sie dazu [.guihint]#Levels for filesystems > Levels for filesystem used space > Dynamic levels#. Mit dem Knopf [.guihint]#Add new element# definieren Sie jetzt pro Plattengröße eigene Schwellwerte.
// LI:Noch einfacher geht es mit dem [.guihint]#Magic factor#, den wir im link:intro_bestpractise.html#magic_factor[letzten Kapitel] vorstellen.
//
//
// H1:Hosts in die Wartung schicken#hosts_downtime
//
...
//
//
// H1:Abgeschaltete Hosts ignorieren#hosts_down
//
...
//
// BI:rule_status_of_cmk_services.png
//
//
// H1:Switchports konfigurieren#switchports
//
...
//
//
// H1:Services dauerhaft deaktivieren#services_disabled
//
...
//
//
// H1:Durch Mittelwert Ausreißer abfangen#average_value
//
...
//
//
// H1:Sporadische Fehler kontrollieren#sporadic_errors
//
...
//
//
// H1:Liste der Services aktuell halten#discovery
//
