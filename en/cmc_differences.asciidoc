...
The fetchers thus take over the network communication with the {CMK} agent, SNMP agents and the xref:datasource_programs#specialagents[special agents.]  
...
It is active by default, i.e. the option [.guihint]#Global settings > Monitoring Core > Use separate fetchers and checkers# is switched on.
...
which in turn is based on the link:http://www.rrdtool.org/RRDtool[RRDtool.] 
