...
The details can be found in the article on xref:cmc_migration#[migration to the CMC].
...
Furthermore, these are jammed for quite a long time if a host is not reachable, and long timeouts must be endured.
...
Host checks not only serve to trigger notifications in the case of a total host failure, but also to suppress notifications for service problems during the host's down time. 
...
For this, it is sufficient if directly after a host failure, the service check (and not the host check) is executed first by chance. 
Even though it is down, the host would still be considered {UP} and the notification for service would be sent -- spuriously.
...
In this situation, _after the start_ of the service check a TCP RST packet (_connection refused_) will be received from this server.
...
On hosts where these are required you can simply set up an xref:active_checks#[active check] via ping with the [.guihint]#Check hosts with PING (ICMP Echo Request# rule set.


=== Unpingable hosts

Not all hosts are checkable by ping in practice. 
...
There are firewalls that answer TCP connection packets to inaccessible hosts with a TCP RST packet packet.
...
A lesson from the poor performance of Nagios in larger environments is that creating processes is an expensive operation.
The _size of the parent process_ is primarily decisive here.
For every execution of an xref:active_checks#[active check] the complete Nagios process must first be duplicated (_forked_) before it is replaced by the new process -- the check plug-in.
...
* The {CMK} fetchers +
retrieve the required information from the monitored hosts, i.e. the data of the [.guihint]#Check_MK# and [.guihint]#Check_MK Discovery# services. 
The fetchers thus take over the network communication with the {CMK} agents, SNMP agents and the xref:datasource_programs#specialagents[special agents.]  
Gathering this information takes some time, but consumes little memory (about 30 MByte per process).
So many of these processes can be configured without problems. 
The limiting factor here is the available memory of the {CMK} server.
...
A checker process consumes at least about 90 MByte -- however, a multiple of this may be necessary, depending on how the checks are configured. 
...
As a rule, this number corresponds to the number of processing cores of your server. 
Since the checkers are not IO-bound, they are most effective if each checker gets its own CPU. 
// ML: Wieder: "... its own core." fände ich plausibler.
...
It is active by default, i.e. the [.guihint]#Global settings > Monitoring Core > Use separate fetchers and checkers# option is switched on.

Before version {v20}, there was only _one_ auxiliary process type that was responsible for both -- the so-called {CMK} helpers. 
...
In larger installations, however, many {CMK} helpers were needed for many hosts to be monitored, which then consumed a lot of memory,
because each {CMK} helper held the entire {CMK} configuration in memory. 

With the new fetcher/checker model in version {v20}, both tasks can now be split between two separate pools of processes: 
...
As a result, a CMC consumes up to four times less memory for the same performance (checks per second)!
...
These values are set under [.guihint]#Global settings > Monitoring Core# and can be adjusted by you:
...
If a pool is 100 % utilised, the checks will not be executed in time, the latency will grow and the states of the services will not be up-to-date.
...
However, be careful to create only as many auxiliary processes as are really needed, as each process consumes resources. 
...
As an alternative to the [.guihint]#Core statistics# snapin, you can also have {CMK} analyse your configuration with [.guihint]#Setup > Maintenance > Analyze configuration#. 
The advantage of this is that {CMK} immediately gives you an assessment of the state of the auxiliary processes:

image::cmc_differences_analyze_configuration.png[alt="Dialogue with the result of the configuration analysis for auxiliary processes."]

By clicking on icon:icon_toggle_details[alt="Icon for showing and hiding detailed information."] you can have a suitable help text shown for each entry. 
Very handy: 
If one of the auxiliary processes is not [.guihint]#OK#, you can also open from the help text the corresponding option in the [.guihint]#Global settings# to change the value. 
...
This is very convenient for the user since a new host will be queried as soon as the configuration is activated. 
...
. The creation and updating of the xref:graphing.html#rrds[Round Robin Databases (RRDs).]
. The graphical representation of the data in the GUI.
