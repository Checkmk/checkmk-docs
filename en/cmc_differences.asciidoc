...
Host checks not only serve to trigger notifications in the case of a total host failure, but also to suppress notifications of service problems during the host's down time. 
...
The division of the two different 'collect' and 'execute' tasks between {CMK} fetcher and {CMK} checker has existed since {CMK} version {v20}. 
It is active by default, i.e. the [.guihint]#Global settings > Monitoring Core > Use separate fetchers and checkers# option is activated.
...
Very handy: 
If one of the auxiliary processes is not [.guihint]#OK#, from the help text you can also open the corresponding option in the [.guihint]#Global settings# to change the value. 
