...
== Activating the Apache Module

An HTTPS securing of the {CMK} interface requires the Apache module `mod_ssl`.
...
With this procedure, certificates can be validated not only at the domain level, but also at organization level (Organization Validation) and higher (Extended Validation), as is mandatory in some industries for regulatory reasons.
...
You are then given the option of selecting an e-mail address: from those stored for the Admin-C (owner of the domain) or for Tech-C (technically responsible for the domain), or a generic e-mail address such as `webmaster@domain.com`.
A confirmation link will then be sent to this address.

==== Zertifikat erhalten     Receiving certificates
...
The `pip3 install certbot` command also installs all of the required dependent Python modules. 
...
So that certificate management is not a stumbling block when agents are automatically updated in the {CEE}, in the xref:glossar#agent_bakery[Agent Bakery] we have provided the option of passing an xref:agent_deployment#certificates_for_https[own CA certificate] that is only used for agent updates.
...
You can read the exact procedure in the article on the link:https://docs.microsoft.com/de-de/troubleshoot/windows-server/windows-security/import-third-party-ca-to-enterprise-ntauth-store#method-1---import-a-certificate-by-using-the-pki-health-tool[Microsoft Knowledge Base PKI^].
Alternatively, you can distribute certificates via link:https://docs.microsoft.com/de-de/mem/intune/protect/certificates-configure[Intune^].


[#configsample]
== Configuring HTTPS connections for a site
...
`SSLCertificateChainFile` contains the intermediate certificate or, if applicable, strings of intermediate certificates.
...
Apache works with virtual hosts to provide a variety of content under a single IP address.
