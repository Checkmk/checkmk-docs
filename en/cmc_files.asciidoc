...
:description: This overview compiles all important files and directories relating to the {CMK} micro core (CMC).
...
All paths relate to the site’s home directory.
For example: if a site is named `mysite`, then all of the paths are derived from `/omd/sites/mysite`.
