...
You have already learned how to set up {CMK} notifications in general in the article on xref:notifications.html[notifications].
...
. Select the [.guihint]#Management type#: ServiceNow has two types, _case_ and _incident_. Depending on your selection, the options displayed below will change. Each of the options offered in {CMK}, both for the _case_ type and the _incident_ type corresponds to the setting of the same name in ServiceNow.
