...
{cee-only}
In {CMK}, a report is a compilation of diverse elements -- link:views.html[views], link:availability.html[availability tables], link:graphing.html[graphs], text, logos, etc., into a printable PDF document.
...
Via the [.guihint]#Reporting# snapin in the xref:user_interface#sidebar[sidebar] you get quick access to all reports and the xref:scheduler[report scheduler].
...
As with link:views.html[table views] and link:dashboards.html[dashboards], supplied reports cannot be changed – an existing report can be cloned, and the clone can then be customised as desired.
...
Clicking the icon:icon_clone[alt=] will clone the report and take you to editing the basic settings for the new report:
...
At a minimum, enter values for [.guihint]#Unique ID# and [.guihint]#Title#.
...
*Tip:* In the preview, you will see red lines and error messages instead of real data -- because, of course, the report in the editor lacks context, here, for example, the specification of a host, which would only be revealed by calling the report from an appropriate view.
...
.Preview with real data of a filtered host
...
The following dialogs are used to set detail settings such as position, file name and size.
...
When all of the settings have been defined, complete this process by clicking [.guihint]#Save#. 
...
After that, only what you already know comes up -- further customization of the report is the same as xref:customize_existing_reports[customizing existing reports.]] shown above.
...
In the [.guihint]#General Options# box you specify basic settings can be defined like title, creation time, report time frame, recipient and so on:
...
In the list you will also find information about when the report was last generated/sent, whether an error occurred, when it will be generated/sent the next time, and more (slightly shortened here):
