...
The permissions needed to register and create hosts are contained in the [.guihint]#Agent registration user# xref:wato_user#roles[role].
This allows you to create a special user who by definition is not allowed to do anything other than register hosts.
...
The permissions to register _new_ hosts are available in this role only in the {CE}.

.The automation password and role are used to create an auto-registration user
