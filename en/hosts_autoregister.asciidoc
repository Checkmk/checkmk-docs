...
You can find it under [.guihint]#Setup > Agents# (if the xref:intro_gui#show_less_more[Show more mode] is active) or by searching the [.guihint]#Setup# menu.
...
For this, you use the [.guihint]#Agent controller auto-registration (Cloud Edition)# rule set in the Agent Bakery, which, as its title suggests, is only available in the {CE}.
