...
In order for these cloud objects, once they have been created, to be included without delay as hosts in the {CMK} monitoring, a solution without manual intervention is required.

{CMK} provides the answer with its _auto-registration_ -- which enables hosts to be created automatically.
As automatic host creation is mainly needed for the cloud environments referred to above, auto-registration is only available in the {CCE}.
...
By the way, when configuring the folder, you can specify whether the agents for the hosts to be created there should work in the {CE}'s xref:glossar#pull_mode[pull mode] or xref:glossar#push_mode[push mode].

Then comes the {CMK} agent: Here, the information for a registration request is specified by an agent rule and then packed into an agent package by the Agent Bakery.
...
[#new_user]
=== Creating a user for the auto-registration

The permission needed to register and create hosts is contained in the [.guihint]#Agent registration user# xref:wato_user#roles[role] -- and only that one permission and no other.
This allows you to create a special user who by definition is not allowed to do anything other than register new hosts.

Start the creation of an xref:glossar#automation_user[automation user] with [.guihint]#Setup > Users > Users > Add user# 
as described in the article on xref:wato_user#user_config[user management].
First enter the user name and its full name in the [.guihint]#Identity# box.
Important here are the two options in the [.guihint]#Security# box.
Select [.guihint]#Automation secret for machine accounts# and enter the automation password or have it generated randomly.
Under [.guihint]#Roles#, assign only the [.guihint]#Agent registration user# role to the user.
This role is only available in the {CE}.

.The automation password and role are used to create the new auto-registration user
image::hosts_autoregister_new_user.png[alt="Dialogue for creating a new user with the 'Agent registration user' role."]

By specifying the above options, you ensure that the new user can only be used for auto-registration.
Save your entries with [.guihint]#Save.#
...
Alternatively, you can use an existing folder if you configure it as described below.
...
This option is only available in the {CE}.
...
image::hosts_autoregister_new_folder.png[alt="Dialogue for creating a new folder for auto-registration."]
...
The [.guihint]#Agent registration# rule set is only available in the {CE}.
This rule can be found under [.guihint]#Setup > Agents#, where the creation of a new rule can be started with [.guihint]#Add rule#. 
In the first box, [.guihint]#Properties#, as usual with {CMK}, you first assign an ID and a title.
...
*Note*: Every host that has been successfully registered automatically receives the `cmk/agent_auto_registered:yes` host label appended by {CMK}.
You cannot use this label here, however, because it is not an agent label and is only assigned _after_ the registration.
...
In the [.guihint]#Hostname computation# box, you now specify which name the host should receive.
In the [.guihint]#Hostname template# box, enter the template.
...
For this, you use the [.guihint]#Agent controller auto-registration# rule set in the Agent Bakery, which is only available in the {CE}.
You can find the rule set in [.guihint]#Setup > Agents > Windows, Linux, Solaris, AIX > Agent > Agent rules.#
Create a new rule.
...
the xref:new_user[predefined automation user] for auto-registration.
...
After installing the agent package, the Agent Controller evaluates the `/var/lib/cmk-agent/pre_configured_connections.json` file
...
You can track the individual steps in this process in [.guihint]#Setup > General > Audit log#.
...
Connection: myhost/mysite
	UUID: 1a53b797-f0d3-4869-a037-804eee8a0d05
	Local:
		Connection type: push-agent
		Connecting to receiver port: 8000
		Certificate issuer: Site 'mysite' local CA
		Certificate validity: Mon, 09 Jan 2023 16:48:17 +0000 - Sat, 12 May 3021 16:48:17 +0000
	Remote:
		Connection type: push-agent
...
The {CE} thus provides the two most important components for the _lifecycle management_ of hosts.
...
There are many possible grounds for a rejection.
...
|`/var/lib/cmk-agent/pre_configured_connections.json` |Contains a pre-configured connection integrated into the agent package via the Agent Bakery to a site for the auto-registration in the {CE}.
|===
...
|`C:\ProgramData\checkmk\agentregistered_connections.json` |Contains a pre-configured connection integrated into the agent package via the Agent Bakery to a site for the auto-registration in the {CE}.
|===
