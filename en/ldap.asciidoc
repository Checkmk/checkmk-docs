...
|[.guihint]#Show more / Show less# |show_mode |String | `default_show_less, default_show_more, enforce_show_more`| Set default of xref:ldap#other_attr["Show more / Show less" mode]. With `enfore_show_more` all features, filters, input fields etc will always be shown. The button to switch between the two modes will be removed
