...
:description: With (CMK) you can monitor a vCenter, its ESXi hosts and also the VMs without requiring extra agents. Details of the configuration are described here.
...
link:check_plugins_catalog.html[Catalog of check plug-ins] in
...
* You must have defined the ESXi-server as the host in {CMK}, and configured it as an agent {{({CMK} Agent)}}. Tip: Select the host name so that it is the same as that known to the server itself.


Once the prerequisites have have been satisfied you can create a link:wato_rules.html#create_rules[Rule] in the
[.guihint]#Datasource programs|Check state of VMWare ESX via vSphere# rule set.
This will be assigned to the defined host, so that instead of the
standard agent the link:wato_monitoringagents.html#special_agent[Special Agent]
...
Finally, go back to the host configuration, and execute a link:wato_hosts.html#services[Service discovery].
...
the link:monitoring_vmware.html#diagnose[Diagnostic options], as described later in this article.
...
performed with the help of the [.guihint]#Access to agents|Hostname translation for piggybacked hosts# rule.
...
link:monitoring_vmware.html#datasource_combination[data retrieval] has been activated.
...
The piggyback names can however be link:monitoring_vmware.html#rename_piggyback[altered]
in a number of ways.
...
|`tmp/check_mk/cache/`
|Here the respective latest agent output from all hosts is temporarily saved. The content of a host’s file is identical to the [.guihint]#cmk -d myhost# command. 
