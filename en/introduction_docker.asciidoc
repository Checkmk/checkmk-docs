...
One application case may be to monitor a dynamically-created container group and to make {CMK} a part of this group. 
...
But it may be a good one, to integrate a {CMK} container in a self-contained container cluster, because in this case you would be able to monitor this cluster from the inside. 
So verify especially in this case if the tool Docker/Container is the best to fit your actual requirements.

In order to make the setting-up as easy as possible for you, we supply each {CMK} edition inclusive of its own specific image, which contain the Linux operating system Debian in addition to {CMK}:

[cols=2]
|===

|{CRE} |link:https://hub.Docker.com/r/checkmk/check-mk-raw/[Docker Hub]
|{CEE} |link:https://checkmk.com/download/[{CMK} download page] (for the {FE}), 
link:https://customer.checkmk.com/[{CMK} customer portal]
|===

*Note:* Deployment in Docker Hub allows you to download and install in one command, as we will show in the xref:install_cre[chapter on installing the {RE}].
...
To execute the commands presented in this article, you need a working installation of the link:https://docs.docker.com/engine/install/[Docker Engine] and basic knowledge of its use.
...
[{shell}]
----
{c-root} docker container run -dit -p 8080:5000 --tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 -v monitoring:/omd/sites --name monitoring -v /etc/localtime:/etc/localtime:ro --restart always checkmk/check-mk-raw:2.0.0-latest
...
|--tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 |For optimal performance, you can use a temporary file system directly in the RAM of the Docker node. The path of this file system is specified with this option. If you change the site ID this path must also be adjusted accordingly.
...
|checkmk/check-mk-raw:2.0.0-latest |The {CMK} image label in the <repository>:<tag> format. The exact labels can be read out with the command `docker images`.
|===
...
*Note:* The URL displayed in the log for accessing the web interface with the ID of the container is only known within the container and is not suitable for access from outside in the web browser.
...
[{shell}]
----
{c-root} docker container run -dit -p 8080:5000 --tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 --name monitoring -v /etc/localtime:/etc/localtime:ro --restart always checkmk/check-mk-raw:2.0.0-latest
----
...
[{shell}]
----
{c-root} docker container run --rm -dit -p 8080:5000 --tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 -v /omd/sites --name monitoring -v /etc/localtime:/etc/localtime:ro checkmk/check-mk-raw:2.0.0-latest
----
...
Download the desired edition and version from the link:https://checkmk.com/download/[{CMK} download page] (for the {FE}) or from the link:https://customer.checkmk.com/[{CMK} customer portal].
...
Just take care that you use the name of the `Loaded image` from the previous command output in the following start command, so in this example `checkmk/check-mk-enterprise:2.0.0p5`:

[{shell}]
----
{c-root} docker container run -dit -p 8080:5000 --tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 -v monitoring:/omd/sites --name monitoring -v /etc/localtime:/etc/localtime:ro --restart always checkmk/check-mk-enterprise:2.0.0p5
