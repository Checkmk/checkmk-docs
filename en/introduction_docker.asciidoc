...
|{CEE} |link:https://checkmk.com/download?method=docker&edition=cce&version=stable[{CMK} download page^] (for the {CE}), link:https://portal.checkmk.com/[{CMK} customer portal^]
|===
...
|`checkmk/check-mk-raw:2.2.0-latest` |The {CMK} image label in the `<repository>:<tag>` format.
...
Just take care that you use the name of the `Loaded image` from the previous command output in the following start command, so in this example `checkmk/check-mk-enterprise:2.2.0p1`:

[{shell}]
----
{c-root} docker container run -dit -p 8080:5000 -p 8000:8000 --tmpfs /opt/omd/sites/cmk/tmp:uid=1000,gid=1000 -v monitoring:/omd/sites --name monitoring -v /etc/localtime:/etc/localtime:ro --restart always checkmk/check-mk-enterprise:2.2.0p1
