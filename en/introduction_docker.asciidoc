...
In order to make the setting-up as easy as possible for you, we supply each {CMK} edition inclusive of its own specific image. 
In addition to {CMK} as the Linux operating system, Debian is also included. 
These images are sourced as follows:
...
|--ulimit nofile=1024 |By manually-setting the user limit (`ulimit`) for `nofile`, you are able to reduce the amount of file descriptors a process is able to open. 
// That is especially useful in this case, as {CMK} still uses Python 2 which uses a very high default. This can significantly slow the process down.
...
Download the desired edition and version from the link:https://checkmk.com/de/download/[{CMK} download page] (for the {FE}) or from the link:https://customer.checkmk.com/[{CMK} customer portal].
