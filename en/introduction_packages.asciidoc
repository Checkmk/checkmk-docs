...
On the {CMK} link:download.php?[download pages] you can choose from:
...
link:check_mk_getting_started.html[Introduction to monitoring with {CMK} guide].

== Upgrading from the {FE} to the full version

The {FE} can be easily upgraded to the full version at any time by link:subscription.php[purchasing a subscription].
