...
this is explained in the link:wato_hosts.html[article on the hosts]. How to
...
set that defines the link:wato_services.html#parameters[parameter for the check
...
link:wato_services.html#enforced_services[article about services]. A small
...
only the rulesets in which you have defined at least one rule. This is often a
convenient way to get started if you want to make adjustments to your to your
existing rules.Incidentally, some of the rules are created already when creating
...
link:wato_rules.html#auxtags[auxiliary tags] are another possibility.
...
| [.guihint]#Checkmk Agent / API integrations#|agent|Defines what type of data the host receives from its link:wato_monitoringagents.html[agents].| cmk-agent, all-agents, special-agents, no-agent
