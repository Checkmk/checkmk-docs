...
Another method is via the icon:context_button_rulesets[] button in the details for an existing
...
The icon:button_check_parameters[] symbol takes you directly to the rule set that defines
...
Cloning creates an identical copy of the rule that you can then edit with icon:button_edit[].
...
icon:button_top[], icon:button_bottom[], icon:button_up[],
and icon:button_down[] buttons. The sequence is important because rules positioned higher in
...
* The [.guihint]#Comment# field can be used for a longer description. It only appears in a rule’s edit mode. Via the icon:button_insertdate[] symbol you can insert a date stamp and your login name in the text (here, for example, `2016-05-06 mk:`).
* The [.guihint]#Documentation-URL# is intended for a link to internal documentation that you maintain in another system (e.g., a CMDB). It will appear as the clickable icon:button_url[] symbol in the rules table.
...
[.guihint]#Folder#, [.guihint]#Explicit hosts# and [.guihint]#Explicit services# are unchanged, and are as explained above.
...
icon:button_top[], icon:button_bottom[], icon:button_up[] and
icon:button_down[] buttons -- has only a visual function --
...
using icon:button_sidebar_addsnapin[] in the bottom left of the sidebar.
