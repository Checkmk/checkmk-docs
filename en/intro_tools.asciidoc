...
image::view_service_filter.png[View with open filter bar.]
...
image::filesystem_c.png[Data of a service for a Windows server file system.]
...
image::service_metrics.png[Service details with the table of all measured values.]
...
The chronology is displayed in graphs like this one, as it is shown for the [.guihint]#Check_MK# service in the {CEE}:

image::example_graph.png[Service details with a time series graph of the measured values.]
...
* With the icon:resize_graph[Icon for resizing a graph.] icon in the bottom right corner you can resize the graph.
