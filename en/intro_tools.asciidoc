...
{CMK} has its own very powerful module for this purpose -- the link:ec.html[Event Console],
which however will not be discussed in this Beginner's guide.
...
which we will discuss in the link:intro_monitor.html#ack[next chapter].
...
link:intro_gui.html#show_less_more[see less or more information].
...
link:intro_tools.html#dashboards[Dashboard] or as a link:intro_tools.html#views[View] -- each of
which we will cover in more detail in the following chapters.
...
{CMK} provides several preconfigured dashboards. 
...
If one of these dashboards is displayed, you can quickly switch to the others: in the menu bar to the right of the [.guihint]#Help# menu you will find the icons for the available dashboards.

{cre-only}
The [.guihint]#Main# dashboard of the {EE}, which shows the state of the site, is not included in the {RE}. In the {RE}, the [.guihint]#Problem# is also the [.guihint]#Main# dashboard.
...
* You can reduce the number of table entries with the *filter bar* -- which is not really necessary in the example shown, but which is very helpful with long tables.
...
Your personal bookmarks are stored under [.guihint]#My Bookmarks#. Internally, {CMK}
organises the bookmarks as lists. You can assign an individual topic to each
bookmark in a list. You can access the management of the bookmark lists with the [.guihint]#Edit# button.
