...
image::monitor_menu_less.png[alt=Monitor menü in Show less mode]
...
icon:button_sidebar_add_snapin[alt=Icon for displaying all snapins] from the list of available snapins.
...
image::view_service_problems.png[alt=Service problems view]
...
image::master_control.png[alt=Master control snapin,width=50%]
...
image::filesystem_c.png[alt=Service for a Windows server file system]
