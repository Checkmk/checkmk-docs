// -*- coding: utf-8 -*-
// IGNORE HardDisk 82545EM CFW CMA E1000 MiB NIC OVF Proxmox VMXNET VMware virtualized
...
For the {SE} and the {ME} you will require a valid subscription and the associated access data.
...
. activating the disks and selecting one as boot disk
...
- [.guihint]#HardDisk:# Delete default entry!
...
Then import the two virtual disks into the virtual machine (here the [.guihint]#VM ID# `103`) and your memory (default: `local-lvm`):
...
Next, you must activate the hard disks imported in this way in Proxmox.
After the import process, they are initially `Unused`.
...
Finally, you must activate the device with `disk-0` in its description as a boot disk via [.guihint]#Options > Boot Order#.
