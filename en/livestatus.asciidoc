...
|json |The output will like wise be generated as a list, but only a json-compatible formate will be used.
|CSV |Formats the output conforming to link:https://tools.ietf.org/html/rfc4180[RFC-4180].
|csv |See link:livestatus.html#csv[customising `csv`]. This is the standard format if no other is specified, and it is based on the official  CSV-Format.
|===
