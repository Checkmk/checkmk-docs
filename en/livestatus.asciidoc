...
[.relateddoc]
livestatus_references, cmk_commandline, web_api
...
link:livestatus_references.html#tables[tables], and this article also contains a general
...
reference in link:livestatus_references.html#commands[Commands].
...
link:livestatus_references.html#hdr[here].
...
the chapter link:livestatus.html#files[Files and directories].
...
This can be re-inserted in the output with the link:livestatus.html#columnheader[ColumnHeaders] header.
...
The available operators can found in the link:livestatus_references.html#operators[Operators]
...
article covering link:regexes.html#characters[Regular expressions].


== Complex queries

=== Filters for lists#filter
...
in link:livestatus_references.html.html#list_operators[Operators for lists].
...
|See link:livestatus.html#csv[customising `csv`]. This is the standard format if no other is specified, and it is based on the official  CSV-Format.
...
The [.guihint]#Tactical Overview# receives its data by retrieving statistics for hosts,
...
By the way, such statistics can be combined with all link:livestatus.html#filter[filters].
...
link:livestatus.html#combining[`Filter`] header, with statistics the output will be
...
operators can be found link:livestatus_references.html#stats[here].
...
== Activating column headers (ColumnHeaders)#columnheader
...
Note here the difference between link:wato_user.html#visibility[`strict` and `loose`]
permission settings:
...
A full list of the Wait-headers can be found link:livestatus_references.html#header[here].
...
A further application is to combine this with a link:livestatus.html#commands[command].
...
== Status codes (ResponseHeader)#response_header
...
* Bytes 1-3: The status code. The complete table of possible codes can be found link:livestatus_references.html#response[here].
...
*Important*: In an error situation the link:livestatus.html#output_format[output format]
...
link:livestatus.html#network[netwerk], you may possibly want to keep the channel open to
...
By the way – following a link:livestatus.html#commands[command] a Livestatus connection
...
[.guihint]#Glodal Settings|Monitoring Core|Maximum concurrent Livestatus connections#.

Always combine `KeepAlive` with the link:livestatus.html#response_header[`ResponseHeader`],
...
in the [.guihint]#Global Settings#:
...
|[.guihint]#History log rotation: Regular interval of rotations#
|Here it can be defined within which time range the history should be continued in a new file. 

|[.guihint]#History log rotation: Rotate by size (Limit of the size)#
...
|[.guihint]#Maximum number of parsed lines per log file#
...
used by the interface's link:availability.html[availability] display.
...
more detail in the link:livestatus.html#columns[Command reference].
...
== Using Livestatus in network environments#network
...
Because it will additionally be used in the link:livestatus.html#logs[Logs] to track the
...
message, and with only a simple entry in the link:livestatus.html#files[`cmc.log`]!
...
link:livestatus_references.html#commands[Command reference].
...
|In this directory a number of examples of Livestatus queries can be found which you can try out. The examples are based on the `lq` script command – e.g.: `lq &lt; 1.lql`. </tr>
 <tr>

|`share/doc/check_mk/livestatus/api/python`
|the API for Python is in this directory, as well as a number of examples. Also read the `README` in this directory. </tr>
 <tr>

|`share/doc/check_mk/livestatus/api/perl`
|The API for Perl can be found here. Here as well there is a `README`. The usage examples are located in the `examples` sub-directory. </tr>
 <tr>

|`share/doc/check_mk/livestatus/api/c++`
|There are also example codes for the C++ programming language. The code for the API itself is likewise located in an uncompiled form here, so that you have the best insight into the API's functionality. </tr>
