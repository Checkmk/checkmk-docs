...
The output preceeding the four hostnames is written to STDERR by the `openssl` command.
...
In case you want to make the encrypted remote Livestatus port as local unencrypted port, you can use the program link:https://www.stunnel.org/[Stunnel^].
...
To see if the expected output is complete in the event of connection failures, we recommend evaluating the xref:responseheader[response header].
...
Other suitable APIs are listed in the chapter covering xref:files[Files and Directories].
