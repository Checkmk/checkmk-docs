...
from Version VERSION[1.6.0p12] you can integrate Prometheus in {CMK}.
...
Via the selection menu [.guihint]#Entity level used to create Checkmk piggyback hosts#
...
Select either [.guihint]#Both}} or {{Container#, and also define the name under which
...
Then select [.guihint]#Add new PromQL query}} and use the {{Metric label# field to
...
[.guihint]#Blocked}} -- are then combined in a service called {{Processes#.
...
== Dynamic host configuration#dcd
...
select [.guihint]#Piggyback data}} as the connector type, and use {{Add new element# to
...
=== Special feature in interactions with cAdvisor#dcd_cadvisor
