...
//
// LI:Unterschiedliche Benutzer können unterschiedliche *Berechtigungen* haben.
// LI:Sie können die *Zuständigkeit* eines Benutzers auf bestimmte Hosts und Services beschränken, so dass nur noch diese im Monitoring sichtbar sind.
// LI:Benutzer können eigene Lesezeichen anlegen, ihre Seitenleiste individuell einrichten und auch andere Dinge für sich anpassen.
// LI:Sie können ein Konto löschen, wenn ein Mitarbeiter die Firma verlässt, ohne dass das die anderen Konten beeinflusst.
//
...
//
//
// H1:Rollen für Berechtigungen#roles
//
...
// <td>Ein Administrator kann alle Funktionen in {CMK} ausführen. Seine Hauptaufgabe ist die generelle Konfiguration von {CMK}, nicht das Monitoring. Dies schließt auch das Anlegen von Benutzern und Anpassen von Rollen ein.</td>
// </tr>
//
// <tr>
// <td>Normal monitoring user</td>
// <td class=tt>user</td>
// <td>Diese Rolle ist für den *Operator* gedacht, der das Monitoring durchführt. Der Operator sollte grundsätzlich nur solche Hosts und Services sehen, für die er zuständig ist. Zudem besteht die Möglichkeit, dass Sie als Administrator ihm die Berechtigung geben, seine Hosts selbst zu verwalten.</td>
// </tr>
//
// <tr>
// <td>Guest user</td>
// <td class=tt>guest</td>
// <td>Ein Gastbenutzer darf alles sehen, aber nichts ändern. Diese Rolle ist z.B. nützlich, wenn Sie einen Statusmonitor an die Wand hängen möchten, der immer eine Gesamtübersicht des Monitoring zeigt. Da ein Gastbenutzer nichts ändern kann, ist es auch möglich, dass mehrere Kollegen ein Konto mit dieser Rolle gleichzeitig verwenden.</td>
// </tr>
//
//
...
//
//
// H1:Kontaktgruppen für Zuständigkeiten#contact_groups
//
...
//
// LI:Jeder Benutzer kann Mitglied von beliebig vielen Kontaktgruppen sein.
// LI:Jeder Host und jeder Service ist Mitglied von mindestens einer oder mehreren Kontaktgruppen.
//
// Hier ist ein Beispiel für so eine Zuordnung:
//
// BI:contactgroup_example.png center width:50%
//
...
//
// LI:Ein Benutzer der Rolle `user` sieht im Monitoring genau die Objekte, die sich in seinen Kontaktgruppen befinden.
// LI:Gibt es ein Problem mit einem Host oder Service, werden (per Default) alle Benutzer alarmiert, die in mindestens einer seiner Kontaktgruppen sind.
//
...
//
//
// H1:Kontaktgruppen erstellen#create_contact_groups
//
...
//
// BI:host_group_assignment_rule.png
//
...
//
//
// H2:Mit Host-Eigenschaften zuzuordnen
//
...
//
//
// H1:Services zuordnen#assign_services
//
...
//
// BI:wato_new_user_contact_groups.png
//
