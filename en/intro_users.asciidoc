...
and you can simply continue reading in the link:intro_notifications.html[next chapter on notifications].
...
guide in the link:wato_user.html[article on user administration].
...
You can learn how to customise the predefined roles in the link:wato_user.html#roles[article on user administration].
...
service in monitoring and possibly be alerted if there is a problem?
...
image::contactgroup_example.png[Illustration of the relationship between users, hosts/services and contact groups.,width=50%]
...
* If there is a problem with a host or service, all users who are in at least one of its contact groups are alerted (by default).
...
image::wato_new_contact_group.png[Dialog for setting the properties for the new contact group.]
...
image::host_group_assignment_rule.png[Dialog for assigning contact groups to hosts in a rule.]
...
image::host_permissions.png[Dialog for assigning contact groups to hosts in the host properties.]
...
image::wato_module_users.png[List of {CMK} users]
...
access (e.g. via script or link:rest_api.html[REST-API]) and it is described in detail in
the link:wato_user.html#automation[article on user administration].

You create a new user with the [.guihint]#Add user# button on the page with this same title:

image::wato_new_user_identity.png[Dialog for setting the identity for the new user.]
...
[.guihint]#Pager address# fields are optional and are used for alerting via email and SMS respectively.

*Note:* Do not enter an email address here for now.
First read the notes in the link:intro_notifications.html[chapter on notifications].

image::wato_new_user_security.png[Dialog for defining the security settings for the new user.]
...
image::wato_new_user_contact_groups.png[Dialog for selecting contact groups for the new user.]
...
Details on this can be found in the link:ldap.html[article on the LDAP/Active Directory].


link:intro_notifications.html[Continue with the {CMK} notifications]
