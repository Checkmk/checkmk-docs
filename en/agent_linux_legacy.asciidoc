...
Either DEB or RPM packages for Debian, Ubuntu, RHEL, SLES (and their derivatives), a TGZ archive for all other distributions ({CEE}) or likewise a shell script ({CRE}) for any other distributions.
...
// This block replaces the following section "Further security options"
// start translation
////
[#further_options]
