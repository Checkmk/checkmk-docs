...
Currently, only `systemd` on the x86/64 platform is supported in this regard, and package management for `deb` or `rpm` packages is required for setup.

If all of the following requirements have been met...

* Your Linux uses `systemd` from version 220 or later as its init system
* The computer architecture is x86/64
...
Since using encrypted transport of the agent data via Agent Controller is out of the question here, we explain in this article how to do either the unencrypted transport via an _Internet Superserver_ or the configuration of _SSH as an encrypted tunnel_.

The issues for which the agent mode does not matter can be found in the article on Linux agent with agent controller:

* xref:agent_linux#disabled_sections[Disable Sections]
* xref:agent_linux#plugins[Extend agent with plug-ins]
* xref:agent_linux#e2e_monitoring[Include classic (Nagios) check plug-ins]
* xref:agent_linux#hw_monitoring[Monitor hardware]
...
On RHEL, SLES and related distributions, the RPM package is installed under `root` with the command `rpm -U`:
...
The installation of the DEB package on Debian, Ubuntu or related distributions is done under `root` with the command `dpkg -i`:
...
The TGZ archive contains the complete directory structure for the Linux agent when unpacking the agent in the monitored host's root directory.
...
If there is no output, port 6556 is not yet open. If a line has been output, then port 6556 is open.
...
If after installation from a DEB or RPM package the program name `cmk-agent-ctl` is output here, you can be pleased:
...
{cee-only} For configuring an existing `xinetd` that uses the `/etc/xinetd.d/` directory for configuration, both the TGZ archive and the DEB and RPM packages come with a script that first installs the configuration in two steps and then lets `xinetd` read the new configuration.
...
In the example below, you use the `ssh-keygen -t ed25519` command as the instance user:
...
The private key is called `id_ed25519` and is readable only by the instance user (`-rw-------`) -- and that's a good thing!
...
The next step must now take place on (each of) the Linux server(s) monitored via SSH.
Log in there as `root` and create the subdirectory `.ssh` in its home directory (`/root`), if it does not already exist.
With the following command the access rights will be set correctly to 700 right away:
...
Be precise!
Every space counts.
Also make sure that *there are never two* spaces in a row.
...
Create a rule here for the affected hosts and enter `ssh -T root@$HOSTADDRESS$` or `ssh -C -T root@$HOSTADDRESS$` (for additional compression of the agent data) as command:
...
After a save and an xref:glossar#activate_changes [Activate changes] the host is added to the monitoring.
...
It is best to use `$OMD_ROOT` here as a replacement for the path to the instance directory (`/omd/sites/mysite`).
The full command could then be `ssh -i $OMD_ROOT/.ssh/my_key -T root@$HOSTADDRESS$`, and thus the configuration would also be executable in an instance with a different name:
...
To close the port at `xinetd`, disable {CMK}'s xinetd service by setting the value of `disabled` to `yes`.
...
However, since an attacker cannot send commands and cannot do anything with encrypted output data, the goal of eavesdropping security is usually sufficiently fulfilled.
...
*Note:* Since symmetric encryption uses the same key for encryption and decryption, an attacker who intercepts an update packet created by the Agent Bakery which contains the key can decrypt the contents of the packet.
