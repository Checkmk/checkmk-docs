...
image::cpu_single_core.png[alt=Define the monitoring of CPU utilization of individual cores for Windows servers]
...
image::windows_services_rule.png[alt=Define the Windows server services to be monitored depending on their status]
...
image::windows_service_discovery.png[alt=Define the names of the Windows server services to be monitored]
...
image::service_internet.png[alt=Service for monitoring the Internet connection to several IP addresses]
...
image::http_service.png[alt=Service for monitoring the HTTP/HTTPS services on a host]
...
image::magic_factor.png[alt=Set the magic factor for file system thresholds]
