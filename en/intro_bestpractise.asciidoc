...
the average load on CPU performance over the course of the last minute.
...
image::cpu_single_core.png[Dialog to define the monitoring of CPU utilization of individual cores for Windows servers.]
...
image::windows_services_rule.png[Dialog to define the Windows server services to be monitored depending on their status.]
...
image::windows_service_discovery.png[Dialog to define the names of the Windows server services to be monitored.]
...
image::service_internet.png[Data of service for monitoring the Internet connection to several IP addresses.]
...
*Important:* Since a service is basically not alerted when its host is {DOWN},
...
image::http_service.png[Data of service for monitoring the HTTP/HTTPS services on a host.]
...
link:intro_finetune.html#filesystems[chapter on fine-tuning monitoring] -- and hinted then
...
image::magic_factor.png[Dialog for setting the magic factor for file system thresholds.]
...
image::magic_factor_services.png[List with two file system services and their thresholds.]
