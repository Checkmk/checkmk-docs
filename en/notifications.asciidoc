...
. Wenn der erste Punkt erfüllt ist, dieser Benutzer (z.B. `cmkadmin`) aber in mehreren Benachrichtigungsregeln verwendet wird, die vom selben Typ sind (z.B. ServiceNow), dann greift jeweils nur die letzte Regel.
...
|`tt` |link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tt[Teletype (monotype font)^] |Deprecated. Do not use this tag!
