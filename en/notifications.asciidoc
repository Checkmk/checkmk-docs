...
Notification means that users are actively informed in case of problems or other events in the monitoring of {CMK}.
...
{CMK} provides a simple interface to write scripts for your own notification methods.

The starting point for any notification to a user is an event reported by the monitoring core.
We call this a *monitoring event* in the following to avoid confusion with the events processed by the xref:ec#[Event Console].
...
{CMK} utilises a rule-based system that allows you to create user notifications from these monitoring events -- and can also be used to implement very demanding requirements.
...
Acceptance for the new monitoring will quickly fade if every day the inbox is flooded with hundreds of useless emails.

The following procedure has proven to effectively tune notifications:

*Step 1*: Fine-tune the monitoring by, on the one hand, fixing the actual problems newly uncovered by {CMK} and, on the other hand, eliminating false alarms.
Do this until everything is ‘normally’ {OK} / {UP}.
See the xref:intro_finetune#filesystems[Beginner's guide] for some recommendations to reduce typical false alarms.
...
Before you receive such an email from {CMK}, a few preparations are necessary, which are described below.
...
In the snapin xref:user_interface#master_control[Master control] snapin of the sidebar you can temporarily deactivate the detection of flapping ([.guihint]#Flap Detection#).

Alternatively, you can also send a [.guihint]#Custom notification#-- also by command:
...
{CMK} is configured ‘by default’ so that when a monitoring event occurs a notification email is sent to every xref:wato_user#contact_groups[contact] of the affected host or service.
...
* The escalation of problems when no xref:basics_ackn#[acknowledgement] has been received beyond a certain time
...
With [.guihint]#Setup > Events > Notifications# you enter the configuration.

*Note:* When you call up the [.guihint]#Notification configuration# page for the first time, you will see a warning about the unconfigured "fallback email address". You can ignore the warning for the moment. We will go into this xref:fallback[below].

In the notification configuration, you manage the *chain of notification rules,* which determine _who_ should be notified _how_.
...
* A notification method (_How_ to notify?), e.g. HTML email, and optionally, additional parameters for the method.
...
|===
|Who (contact) |How (method) |Parameters of the method
...
Now for each entry in this table, the xref:scripts [notification script] which actually executes the user notification appropriate to the method is invoked.
...
As usual, you can edit the rule icon:icon_edit[], copy icon:icon_clone[], delete icon:icon_delete[] or create a new rule.
...
So you can, for example, define a global subject for your email, but also with an individual notification rule define an alternative subject.
...
*Note:* For the following notification methods for forwarding to other systems, you will get more detailed information in separate articles:
...
xref:notifications_victorops#[VictorOps], and
xref:notifications_webex#[Cisco Webex Teams]


==== Selecting contacts

The most common procedure is for notifications to be sent to all users who have been registered as xref:wato_user#contacts[contact] for the respective host/service.
...
If, in the method, you have selected [.guihint]#Cancel previous notifications#, the notifications will only be deleted for the contact selected here.
...
* the object's static attributes – e.g., whether the service name contains the `/tmp` text or a host is in a particular host group, 
* with the current state or the change of state – e.g., whether the service has just changed from {OK} to {CRIT}, 
* or with other things – e.g., whether the 'working time' xref:timeperiods#[time period] is currently active.

There are two important points to consider when specifying conditions:

. If no conditions have been defined the rule will take effect for *every* monitoring event.
. As soon as you select even a single condition, the rule will only take effect if *all* selected conditions are met. 
...
This means that you should pay very close attention to whether the conditions you have selected can be fulfilled at the same time, so that a notification is triggered for the desired case. 

Let's say you want a notification to be sent when a monitoring event occurs for a service starting with the name `NTP` on a host in the [.guihint]#Main directory#:

image::notifications_rule_part4.png[]

Let's further assume that this condition is now *extended* by also notifying all state changes of a host to the {DOWN} state:

image::notifications_rule_part4_match_host_types.png[]

The result of this notification rule with the three individual conditions is that *never* a notification will occur because no monitoring event will contain the state change of a host *and* the service name with `NTP`.

We often give the following hint in this User guide. However, in the context of configuring your notifications, it is worth emphasizing again:
...
The following excerpt from the context-sensitive help for the [.guihint]#Match services# option illustrates the behavior very well: "_Note: Host notifications never match this rule if this option is being used._".


[discrete]
==== The exception to the AND operation

Only if a monitoring event meets all configured conditions, the notification rule will be applied. 
As mentioned above, there is one important exception to this general rule: for the conditions [.guihint]#Match host event type# and [.guihint]#Match service event type#:

image::notifications_rule_part4_match_event_types.png[]

Should you select *only* [.guihint]#Match host event type#, the rule will match
no single service event. Similarly, this applies to the selection of [.guihint]#Match service event
type# and all host events. Should you activate *both* conditions however, the rule
will match if the event type is activated in *any* of the two checkbox lists.
In this exceptional case these conditions will thus not be linked with a logical ‘AND’, but rather with an ‘OR’. In this way you can simply administer host and service notifications with a single rule.
...
*Note:* Alternatively, you can also make a user the recipient in their xref:wato_user.html#user_config_notifications[personal settings]. 
...
If you want to prevent customisation altogether, you can revoke the xref:wato_user#roles[permission] [.guihint]#General Permissions > Edit personal notification settings# of the user role `user`.

As an administrator, you can display all user rules if you select in the menu [.guihint]#Display > Show user rules#:
...
Only if you explicitly assign this right to the role `user`, a user will see the corresponding checkboxes in his personal settings:
...
{cee-only}
The xref:cmc#[CMC] operates internally in a slightly different manner to Nagios by the way.
...
A subsequent notification rule that ‘subscribes’ to notifications for such services will be ineffective, as they are not generated at all.
...
It is also possible to temporarily disable notifications for individual hosts or services via xref:commands.html[command].
...
With assigned permission, you can disable (and later enable) notifications from hosts and services with the command [.guihint]#Commands > Notifications#:
...
Since commands -- in contrast to rules -- require neither configuration permissions nor an xref:wato#activate_changes[activate changes], they can be a quick workaround to react quickly to a situation.
...
In the [.guihint]#Master control# snapin of the sidebar you will find a master switch for [.guihint]#Notifications#:
...
Each site in xref:distributed_monitoring#[distributed monitoring] has one of these switches.
...
Only if three successive attempts return a not-{OK}-state will the service switch to the hard state and a notification is triggered.
...
Show the context sensitive help with [.guihint]#Help > Show inline help# for details about the adjustable values.


=== Periodically repeated notifications and escalation

For some systems, it may be useful not to leave it at a single notification if the problem persists over a long period of time, e.g., for hosts whose xref:wato_rules#hosttags[host tag] [.guihint]#Criticality# is set to [.guihint]#Business critical#.
{CMK} can be set up so that successive notifications are issued at fixed intervals, until either the problem is icon:icon_ack[] xref:basics_ackn#[acknowledged] or solved.
...
To get started, we will show you how to view the history of notifications at the host and service level in {CMK} to track the notification process.

A monitoring event that causes {CMK} to trigger a notification is, for example, the change of state of a service.
You can manually trigger this state change with the command [.guihint]#Fake check results# for testing purposes, as we have already shown in the chapter on xref:notification_testing[test] of a simple notification by email.
...
. The monitoring core generates a icon:icon_alert_cmk_notify[] _raw notification_. This is passed by the core to the notification module, which performs the evaluation of the notification rules.
...
To help in the correct understanding of the contexts for all of the various setting options and basic conditions, and to enable an accurate problem diagnosis when a notification appears or does not appear as expected, here we will describe all details of the notification process with all components involved.
...
|Nagios |Monitoring core in the {CRE} that detects monitoring events and generates raw notifications. |`var/log/nagios.log +
var/nagios/debug.log`
|xref:cmc#[{CMK} Micro Core (CMC)] |Monitoring core of the {EE} that performs the same function as Nagios in the {CRE}. |`var/log/cmc.log`
...
More interesting however are the messages you receive in the `var/nagios/debug.log` file which you get if you set the `debug_level`
...
Should you need to analyze a larger amount of raw notifications, you can easily increase the number stored for analysis via [.guihint]#Global settings > Notifications > Store notifications for rule analysis#:
...
* Synchronous delivery: The notification module waits until the notification script has finished executing. If this takes a long time to execute, more notifications will pile up. If monitoring is stopped, these notifications are lost. In addition, if there are many notifications in a short period of time, a backlog may build up to the core, causing monitoring to stall.
...
In distributed environments -- i.e., those with more than a single {CMK} site -- the question arisesof what to do with notifications generated on remote sites.
...
Notification can occur in very manifold and individual ways.
...
Some more example scripts are included with the software in `share/doc/check_mk/treasures/notifications`.
...
Additionally, in all cases the the standard output of the notification script, together with the status will be entered in the host’s/service’s notification history and will therefore be visible in the GUI.
...
|`NOTIFY_CONTACTEMAIL` |Email address of the contact.
...
|`var/nagios/debug.log` |The Nagios debug log file. Switch on the debug messages in `etc/nagios/nagios.d/logging.cfg` with the variable `debug_level`.
