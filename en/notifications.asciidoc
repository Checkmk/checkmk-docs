...
Note that in the [.guihint]#Insert HTML section between body and table# field, for security reasons only a small set of HTML tags is allowed.
These are:
...
An object that is not currently in a notification period will be flagged with a grey pause icon icon:icon_outofnot[alt="Icon of an inactive notification period."].
...
image::notifications_overview_failed.png[alt="Display of failed notifications in the 'Overview' snapin.",width=50%]
