...
= Notifications
:revdate: 2016-11-29
:title: Notifications - Alerts (alarms) via Email, SMS, Ticketsystems and more
...
from the _Heirloom mailx_ project directly in the site user’s
search path (as `~/bin/mail`). The best way to test this is as a site user,
...
In a freshly-generated {CMK} site, initially there is only the
...
A main switch for notifications can be found in the [.guihint]#Master control#:
...
Each site in link:distributed_monitoring.html[distributed monitoring] has one of these switches. 
Switching off the central site’s notifications still allows remote sites to activate notifications -- even though these are directed to the central site and displayed there.
...
{CMK} site -- the question arises: _what should happen with notifications
that are generated on remote sites? _

In such a situation there are basically two possibilities:

. Local delivery
. Central delivery on the central site (only CEE)
...
In the case of a notification your script will be called up with the site
...
* `&gt; $OMD_ROOT/tmp/foobar.out` writes the result to the `tmp/foobar.out` file within the site.
...
|OMD_SITE |The site name, e.g., `mysite`
