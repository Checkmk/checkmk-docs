...
Conditions determine when a rule will be used. Without the exception discussed separately below, there are two important things to note here:
...
The following excerpt from the context-sensitive help for the [.guihint]#Match services# option illustrates the behavior very well:

> Note: Host notifications will never match this rule, if this option is being used.

So the conditions you see in the following screenshot can never result in a
notification telling you that a host has changed to {DOWN} state.

image::notifications_rule_part4.png[]


The following note is repeated in this here official guide from time to time. However, in connection with the configuration of your notifications, it should be emphasized again:

*Note:* Show context sensitive help with [.guihint]#Help > Show inline help# to get details about the effect of the different conditions.
...
The two conditions [.guihint]#Match host event type# und [.guihint]#Match service event type# are the important exception, to what we previously talked about:
