...
* the ICON[icon_ack.png] link:basics_ackn.html[Confirmation of a problem] ([.guihint]#Acknowledgement#) by a user
* an event arising from a manually-triggered notification ICON[icon_commands.png] link:commands.html[Command]
* the execution of an link:alert_handlers.html[Alert handler] ICON[icon_alert_handlers.png] 
* an event passed for notification ICON[icon_mkeventd.png] from the link:ec.html[Event Console]
...
== Simple notifications by email#simple_mail
...
=== Setting up mail dispatching in Linux#smtp
...
=== Testing#notification_testing

To test the notification you could simply set an {OK}-service to {CRIT} manually.
This is done with the [.guihint]#Fake check results# ICON[icon_commands.png] link:commands.html[command].
...
== Controlling notifications with rules#rules
...
such demands. Using the ICON[icon_notifications.png] [.guihint]#Notifications# WATO-Modul
...
[.guihint]#Match host event type}} and {{Match service event type# conditions:
...
A further tip regarding the [.guihint]#Match contacts}} and {{Match contact groups# conditions:
...
=== Cancelling notifications#cancel
...
=== What happens if no rule is applicable?#fallback
...
== User-defined notifications#personal
...
== When exactly notifications are generated#conditions
...
These will be flagged with the [.guihint]#stale# time icon ICON[icon_stale.png].
...
[CEE] The link:cmc.html[CMC] operates internally in a slightly different manner to
...
*  The [.guihint]#Master control}}  has a main switch for controlling the detection of flapping ({{Flap detection#).
* You can exclude objects from detection by using the [.guihint]#Monitoring configuration|Enable/disable flapping detection for hosts}} rule sets, or respectively, the {{... services# rule sets.
...
ICON[icon_notifications.png] [.guihint]#Notifications# WATO module.
...
=== Asynchronous delivery via the notification spooler#async
...
* The link:notifications.html#scripts[notification method] must support bulk notifications. This is currently only the case for [.guihint]#ASCII email}} and {{HTML email#.
...
== Traceable delivery per SMTP#syncsmtp
...
. The rules evaluation results in a notification ICON[icon_alert_notify.png] to user [.guihint]#hh# with the `mail` method.
...
== Notification in distributed systems#distributed
...
* For notifications per [.guihint]#HTML email}} there is a check box {{Information to be displayed in the email body}} with the option {{Complete variable list (for testing)#.
...
=== The SMTP-service’s log files#maillog
