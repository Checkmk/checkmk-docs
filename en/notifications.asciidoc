...
Daneben ist `link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a[a^]` zugelassen, wenn es mit dem Attribut `href` (verpflichtend) und `target` (optional) kombiniert ist.
Links müssen hierbei entweder relative Pfade enthalten (also mit `./` oder `../`) oder eines der URL-Schemas `http`, `https` oder `mailto` verwenden.
Von der Verwendung relativer Pfade raten wir wegen der sehr unterschiedlichen Handhabung durch E-Mail-Clients ab.
