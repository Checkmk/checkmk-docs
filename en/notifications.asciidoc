...
Depending on your Linux distribution, this could utilize, for example, Postfix, Qmail, Exim, Sendmail or Nullmailer.
...
image::notifications_fake_check_results.png[alt="Dialog for manual state change."]
...
In the xref:user_interface#master_control[Master control] snap-in on the sidebar you can temporarily deactivate the detection of flapping ([.guihint]#Flap Detection#).

Alternatively, you can also send a [.guihint]#Custom notification# -- also using a command:

image::notifications_custom.png[alt="Dialog for a custom notification."]
...
| `a` | link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a[Anchor/Link^] | This is permitted if it is combined with the `href` (mandatory) and `target` (optional) attributes. Links must either contain relative paths (i.e. with `./` or `../`) or use one of the URL schemes `http`, `https` or `mailto`. We advise against the use of relative paths because of their very different handling by email clients.
...
| `tt`| link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tt[Teletype (monospaced font)^] | Deprecated, please do not use anymore!
...
| `br` | link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/br[Break^] |
| `nobr` | link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/nobr[Text without line break^] | Deprecated, please do not use anymore!
| `pre` | link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/pre[Pre-formatted^] | Spaces and tab stops are shown
...
| `li` | link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/li[List Item^] | Only use within these list environments:
