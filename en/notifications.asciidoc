...
`link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/ul[ul^]` and
`link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/ol[ol^]`.

In addition, `link:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a[a^]` is permitted if it is combined with the `href` (mandatory) and `target` (optional) attributes.
Links must either contain relative paths (i.e. with `./` or `../`) or use one of the URL schemes `http`, `https` or `mailto`.
We advise against the use of relative paths because of their very different handling by email clients.
