...
[.relateddoc]
wato_monitoringagents, datasource_programs, agent_linux,  agent_windows
...
by means of link:wato_monitoringagents.html#agents|agents] or [snmp[SNMP] are only two of
...
originally from
<a href="https://www.monitoring-plugins.org">www.monitoring-plugins.org</a>.
...
[.guihint]#WATO|Host & Service Parameters}} entry for {{Active Checks (HTTP, TCP, etc.)#.
...
unclear you can also refer to the ICON[icon_help.png] online help for
...
already-mentioned ICON[icon_help.png] online help --
...
macros can be found as always in the ICON[icon_help.png] online help. After you
...
== Running Active Checks on a host (MRPE)#mrpe
...
[agent_windows#mrpe|Windows] and link:agent_linux.html#mrpe[Linux].
...
|`~/lib/nagios/plugins/`
|Here you will find all plug-ins that are delivered with {CMK}. No distinction is made between plug-ins that are written by <a href="https://www.monitoring-plugins.org">www.monitoring-plugins.org</a> and those that are written specifically for {CMK}.


|`~/local/lib/nagios/plugins/`
|You can store your own plug-ins here. They are then dynamically read in, and will also survive an update of the {CMK} instance.
