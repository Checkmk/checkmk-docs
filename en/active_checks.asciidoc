...
{CMK} provides its _active checks for such situations. These checks allow you
...
you will find many more in your site. To simplify the example overview here,
...
Place the plug-in in the local path of your site and make it executable in one go:
...
* ... are always executed by the {CMK} site's server. Exceptions are link:active_checks.html#mrpe[MRPEs], which are executed directly on a host.
