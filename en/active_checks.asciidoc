...
|`~/lib/nagios/plugins/`
|Here you will find all plug-ins that are delivered with {CMK}. No distinction is made between plug-ins that are written by link:https://www.monitoring-plugins.org[www.monitoring-plugins.org] and those that are written specifically for {CMK}.
