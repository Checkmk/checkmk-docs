...
////
MFS: Hi Anthony, I already updated links, screenshots, listings and order of
clicking in the GUI. However there are two added sections. I'll mark where to
include those sections to make translation easier.
////
...
// MFS: At this place the german article contains a section hinting on the main task of active checks, providing E2E monitoring. Please translate and insert.
// German as reference:
////
...
for the service name `Certificate expiry`. In the following example the label has been
...
macros can be found as always in the icon:icon_help[] online help. After activating the changes, you can see the new service on the assigned host:
...
// MFS: At this place I added an in depth explanation where indirect execution of active checks makes sense. Please translate and insert.
// German as reference:
////
