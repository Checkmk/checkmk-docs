...
= The spool directory
:revdate: draft
:title: The spool directory - Adding file contents to agent output
:description: The spool directory can be used by regularly running programs to store information about their state, which is then added to the agent output.
...
Um abzufangen, dass dieses Programm auch abstürzen kann, sollten Sie die Spool-Datei entsprechend benennen, beispielsweise `1800_frobolator.txt` -- wenn 30 Minuten ohne Aktualisierung der Spool-Datei auf Probleme des Dienstes oder des auswertenden Programmes hindeuten.
