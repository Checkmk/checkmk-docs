...
:description: TThe {CMK} HTTP-API (or Web-API) is an interface to configure {CMK} over HTTP, and this article is an introduction to using this API.
...
*Note:* Starting with version {v20} there is a new API in {CMK}, the REST API, which should replace the Web API.
