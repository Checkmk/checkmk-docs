...
:description: TThe Checkmk HTTP-API (or Web-API) is an interface to configure Checkmk over HTTP, and this article is an introduction to using this API.
...
The Web-API enables configuration tasks that are normally performed manually in the link:wato.html[WATO web interface] to be automated using HTTP-invocation. The complete WATO functionality is in fact not available via API, but the most important things, such as the managing of hosts, folders, instances, users, groups and more is already possible. A full list of all API functions can be found link:web_api_references.html[here].
...
A basic knowledge of Python and its link:web_api.html#format[Data structures] are needed
...
*Note:* Starting with version VERSION[2.0] there is a new API in {CMK}, the REST API, which should replace the Web API.
...
On a newly-created site the [.guihint]#automation# user already exists. It can be found, like other users, in [.guihint]#WATO => Users#. Make sure that the roles and the associated permissions for the automation user are set to allow the execution of your commands
...
link:web_api_references.html[Command reference] the input and answer are often spread
...
rules and much more. Have a look in the link:web_api_references.html[Command reference]
...
A host can be created in {CMK} with the link:web_api_references.html#hosts[`add_host`] command.
...
link:web_api_references.html#discover_services[the type of Service Discovery] as required.
...
* link:omd_https.html[{CMK} over HTTPS]: Use the Web-API exclusively over Hypertext Transfer Protocol Secure (HTTPS), otherwise user names, passwords and also configuration data will be transmitted over the web in plain text.
...
As explained link:web_api.html#result_code[above], a query returns an error code
...
Because every available link:wato_user.html#roles[role] can be assigned to an automated user,
the link:wato_user.html#wato_permissions[contact groups] must be adapted as appropriate
...
|etc/check_mk/conf.d/wato/ |The directories stored here are the directories which are displayed with their [.guihint]#Hosts# in WATO.
