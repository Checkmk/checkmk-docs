...
As under link:monitoring_oracle.html#adv_user_linux[Linux], you can also define not only
...
link:monitoring_oracle.html#skip_include[Linux].
...
|EXCLUDE__{SID}_ |Since the link:monitoring_oracle.html#skip_include[`SKIP_SIDS`] option is not available to you, you can only _exclude_ instances with this option and thus define a negative list. You do this by setting the value of the variable to `ALL`. In addition, you can also exclude only certain sections for the specified instance and in this way define a negative list for individual sections of the instance. *Note*: An (`+ASM`) cannot be completely deactivated with this option.
|===
...
same way as with link:monitoring_oracle.html#sections[Linux],
