...
|Linux, Solaris, UNIX with systemd |`/usr/lib/check_mk_agent/plugins/<number>` |`/etc/check_mk/`
|AIX |`/usr/check_mk/lib/plugins/` |`/usr/check_mk/conf/`
|AIX with systemd |`/usr/check_mk/lib/plugins/<number>` |`/usr/check_mk/conf/`
|===
...
completeness. It assumes that you know how to xref:devel_check_plugins#[program a check plug-in] and
integrate it into the monitoring site. After that you simply assign the
