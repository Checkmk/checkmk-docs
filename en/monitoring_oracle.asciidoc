...
that you can set in link:monitoring_oracle.html#adv_linux[unix-type systems] or under
...
// AN:tns_config
// H3:Configuring TNSALIAS and TNSADMIN
//
...
to monitor the database. To monitor remote databases, there are, the following requirements:
...
Alternatively, you can refer to the file in the link:agent_windows.html#agent_architecture[installation path] in the link:agent_windows.html#plugins[agent's configuration file].
...
//
// LI:Sie können beliebig viele individuelle Zugangsdaten definieren. Diese werden immer dem Standard gegenüber bevorzugt.
// LI:Jede Option wird in einer Liste definiert. Die Reihenfolge der Einträge ist nicht beliebig und darf daher nicht vertauscht werden.
...
// LI:Werden ab einem bestimmten Punkt keine optionalen Felder mehr gebraucht, können sie weggelassen werden. Siehe `ASMUSER`, bei dem nur die Rolle des Benutzers angegeben wurde.
// LI:Soll dem Benutzer keine spezielle Rolle zugewiesen, aber `HOST` oder `PORT` angepasst werden, tragen Sie an der Stelle lediglich zwei Anführungszeichen (`""`) ein.
//
//
...
link:monitoring_oracle.html#cluster_adg[(Active) Data Guard], you also assign the
...
|SQLS_MAX_CACHE_AGE |Performs the same task as link:monitoring_oracle.html#sections[`CACHE_MAXAGE`] -- but for the own SQL queries. |ja
...
completeness. It assumes that you know how to link:devel_check_plugins.html[program a {CMK} check] and
...
|share/check_mk/agents/windows/plugins/mk_oracle.ps1 |The plugin for Windows that fetches the data on the Oracle host.
|share/check_mk/agents/plugins/mk_oracle |The plugin for unix-type systems, which fetches the data on the Oracle host.
