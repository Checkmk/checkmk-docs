...
:revdate: draft
:title: Monitoring Oracle databases efficiently
...
link:https://checkmk.com/cms_check_plugins_catalog.html.html[Catalogue of Check Plug-ins].
We extend this plug-in regularly, so it is always worth looking in the catalogue.
Among others, {CMK} can monitor the following values (some of the plug-ins only from
Version VERSION[1.6.0]):

* link:https://checkmk.com/cms_check_oracle_instance.html.html[Instance state]
* link:https://checkmk.com/cms_check_oracle_version.html.html[Version]
* link:https://checkmk.com/cms_check_oracle_jobs.html.html[Jobs]
* link:https://checkmk.com/cms_check_oracle_locks.html.html[Locks]
* link:https://checkmk.com/cms_check_oracle_processes.html.html[Processes]
* link:https://checkmk.com/cms_check_oracle_sessions.html.html[Number of active sessions]
* link:https://checkmk.com/cms_check_oracle_recovery_area.html.html[Recovery area]
* link:https://checkmk.com/cms_check_oracle_logswitches.html.html[Log switch activity]
* link:https://checkmk.com/cms_check_oracle_tablespaces.html.html[General tablespace information]
* link:https://checkmk.com/cms_check_oracle_performance.html.html[Tablespaces performance]
* link:https://checkmk.com/cms_check_oracle_longactivesessions.html.html[Long active sessions]
* link:https://checkmk.com/cms_check_oracle_undostat.html.html[Undo retention]
* link:https://checkmk.com/cms_check_oracle_recovery_status.html.html[Checkpoint and online backup state]
* link:https://checkmk.com/cms_check_oracle_sql.html.html[Custom SQLs]
* link:https://checkmk.com/cms_check_oracle_rman.html.html[RMAN backup status]
* link:https://checkmk.com/cms_check_oracle_rman_backups.html.html[RMAN backups]
* link:https://checkmk.com/cms_check_oracle_asm_diskgroup.html.html[ASM disk groups]
* link:https://checkmk.com/cms_check_oracle_dataguard_stats.html.html[Apply and transport lag of Oracle Data-Guard]
* link:https://checkmk.com/cms_check_oracle_crs_res.html.html[Oracle Clusterware: Cluster resources]
* link:https://checkmk.com/cms_check_oracle_crs_version.html.html[Oracle Clusterware: Version]
* link:https://checkmk.com/cms_check_oracle_crs_voting.html.html[Oracle Clusterware: Voting]
...
link:monitoring_oracle.html#bakery[Agent Bakery] is currently only available for Linux,
Solaris and AIX.
...
databases. Provided that you are not using link:monitoring_oracle.html#rac[RAC], create a
...
You can find out exactly how the login to a specific instance functions in the
...
performed in the config using a special user and with the prefix `C##`.

[#user_config]
=== Create the configuration file and user
...
link:monitoring_oracle.html#linux[Linux, AIX, Solaris] or
for link:monitoring_oracle.html#windows[Windows]. In the following example the file for
unix-like systems can be seen:
...
that you can set in link:monitoring_oracle.html#adv_linux[unix-like systems] or under
link:monitoring_oracle.html#adv_windows[Windows] are optional.


=== Getting the script and configuration on to the host
...
link:agent_linux.html#plugins[Linux] and link:agent_windows.html#plugins[Windows]. Solaris and AIX
...
enter this once in the wallet. In the following example we thus add the user
...
So that the agent plug-in can access these files you can either specify the path under Linux, Solaris and AIX using
the link:monitoring_oracle.html#tns_config[`TNSALIAS` extended variable].
...
link:monitoring_oracle.html#files_cmk[{CMK}-Server], and also on the Oracle host.
...
reduces maintenance effort, since SQLs are not duplicated, and on the other hand
...
In link:monitoring_oracle.html#first_steps[general setup] you have already learned about
...
|ONLY_SIDS |Here you can determine which instances are to be monitored -- all other instances will then be ignored. Of course the instances will still only be monitored if they are found by the plug-in. This option is very useful if the number of instances to be monitored is smaller than the number of instances to be ignored.
|SKIP_SIDS |All instances that are coded here separated by spaces are completely ignored and not monitored. This option is very suitable if the number of instances to be ignored is smaller than the number to be monitored.
|EXCLUDE__{SID}_ |With this option you can partially exclude instances. Again, you can use the value `ALL` to exclude all sections, and thus do the same as if you were adding them to `SKIP_SIDS`. However you can also disable only certain sections of certain instances, and thus only monitor a subset of what is possible. *Important*: An ASM (`+ASM`) cannot be completely disabled with this option.
|===
...
In case of doubt -- that is, as the default behavior -- the instance will also
then be monitored accordingly.
...
The operational purposes are also diverse, and it is especially practical when
...
link:monitoring_oracle.html#skip_include[previous section].
...
|SYNC_SECTIONS |Sections that are to be queried synchronously, i.e. every time the agent runs. Since the query interval is 60 seconds by default, the SQLs used must be run through correspondingly fast. If the variable is not specified, all sections are queried.
...
As can be seen in the example, the ASM instances will only query the
`instance` section, and additionally, on all regular instances a minimal
set of default asynchronous sections has been specified.
...
normally asynchronous section to a synchronous section -- you must specify
...
[#tns_config]
==== Configuring TNSALIAS and TNSADMIN

Coming soon. This can be used to add a post or prefix to an instance's TNSALIAS.

You can additionally set the `TNSADMIN` to specify the path to existing
...
Under unix-type systems, not only can you retrieve local instances, but also
connect to and retrieve from remote instances. This, for example, is
advantageous if you do not have access to the underlying system, but still want
to monitor the database. To monitor remote databases, there are, however,
...
access data in the configuration file. This procedure is similar to that you
...
noticed, for the values after the port specification there are other values that
have been newly-set.
...
the query to the specified host. If this is present as a host in Checkmk,
the new services will be installed there accordingly instead of on the host
where the plug-in is running. You do not see this specification on the second
...
The third value (=_VERSION_) is also required, and is due to the fact that
...
which SQLs can be passed to the instance.
In the example, both instances use version `11.2`.

The fourth and last value (=_TNSALIAS_) is again optional and can be used
...
with the link:wato_monitoringagents.html#bakery[Agent Bakery], because syntax errors in
...
that require a major intervention and require clear expertise.
Accordingly, the link:monitoring_oracle.html#custom_sqls[Custom SQLs] cannot be
configured directly in the Agent Bakery.

With [.guihint]#WATO => Monitoring Agents => Rules => ORACLE Databases (Linux, Solaris, AIX)# you
can create a rule.
...
In the link:monitoring_oracle.html#user_config[simplest configuration] it will look
something like this:
...
|[.guihint]#Instances to monitor# |This option combines several options of the configuration file, allowing you to link:monitoring_oracle.html#skip_include[include or exclude] instances.
|[.guihint]#Add pre or postfix to TNSALIASes# |This option allows you to extend the link:monitoring_oracle.html#tns_config[TNSALIAS] either globally or for a specific instance.
|[.guihint]#Sections - data to collect# |All available sections are listed under this option, and these can be configured individually at the global level. They therefore correspond to both variables `SYNC_SECTIONS` and `ASYNC_SECTIONS`, as well as their counterpart in the case of an _ASM_ from the section where the link:monitoring_oracle.html#sections[data to be fetched] is to be found.
|[.guihint]#Exclude some sections on certain instances# |If you do not want to use `EXCLUDE_{SID}` to exclude the entire instance, but only to exclude a few sections, you can do so using this option. A more detailed description can be found in the options for the link:monitoring_oracle.html#skip_include[Instances] and link:monitoring_oracle.html#sections[Sections].
...
|[.guihint]#Remote instances# |Configure link:monitoring_oracle.html#remote_instances[remote instances] with this option. It contains all elements of the configuration that you already know. To specify the variable's ID, via [.guihint]#Unique ID# you can select from various options. These only affect the specified ID and must therefore be unique.
|[.guihint]#ORACLE_HOME to use for remote access# |Here you can determine where the agent plug-in can find the `sqlplus` program. You must then specify this program if you want to monitor a link:monitoring_oracle.html#remote_instances[remote instance], but `sqlplus` cannot be found via the environment variables.
|[.guihint]#TNS_ADMIN to use for sqlnet.ora and tnsnames.ora# |If you have already created the corresponding files in another location, you can use this option -- link:monitoring_oracle.html#tns_config[as described above] -- to provide the file path to the files. Otherwise, this is searched for under `/etc/check_mk/`.
|===
...
On Windows, {CMK} uses Powershell as the scripting language. The functionality
is similar to the plug-in under link:monitoring_oracle.html#linux[unix-type systems],
...
To activate the `mk_oracle.ps1` plug-in, either copy the plug-in from
`C:\Program Files (x86)\checkmk\service\plugins\` into the plug-in
directory described above. Alternatively, you can specify the file in the
installation path for the agent configuration file.


=== Special features under Windows and Powershell

Windows normally prevents the execution of scripts if they have not been signed.
You can now work around this problem very easily by modifying the policies for
running Powershell scripts for the user who is running the checkmk agent:
...
general functionality of the checkmk agent. To avoid compromising the security
...
Note that the configuration script is also added to the exceptions here.
...
In the link:monitoring_oracle.html#user_config[general setup] you have already learned
about the first variables to get monitoring data from your Oracle instances.
Further options that are also available to you under Windows are described
...
compatible link:monitoring_oracle.html#linux[unix-type operating system].
...
link:monitoring_oracle.html#linux[unix-type operating systems].
...
the instance(s). *Important*: How this works may differ from this example.
...
password file to the standby server. In the plugin's configuration file,
set the role of the user to `sysdba`:
...
On the monitored host you need to perform certain actions, and on the {CMK} side
it is also necessary that you customize the services and assign them to a
_cluster host_ -- regardless of whether you use ADG or DG.
The corresponding check plug-ins have already been prepared to the extent that
they can also be configured as link:clustered_services.html[_Clustered Services_].
Create a cluster host in {CMK}, and to this cluster host assign the
individual Oracle hosts to which the primary and the standalone instances are
assigned.
...
link:monitoring_oracle.html#cluster_adg[_(Active) Data Guard_], you also assign the
...
[#custom_sqls]
== Monitoring own SQLs (Custom SQLs)

=== Why specify own SQLs?

With its official plug-in {CMK} already provides a large number of SQLs with
...
monitoring of a specific database, from Version VERSION[1.6.0] onwards {CMK}
provides the possibility of defining your own SQLs and having them queried with
the `mk_oracle` plug-in. These SQLs are then automatically recognized and
monitored as own services in the web interface.

*Important*: It is only possible to query own SQLs under Linux,
AIX and Solaris. Under Windows this option is _not_ available.


=== Simple integration of own SQLs

==== Writing SQLs to be queried

The easiest way to connect such SQLs is to use the
link:check_oracle_sql.html[[.guihint]#Custom SQLs for Oracle DBs# check plug-in]. To do this,
...
link:localchecks.html[local check], especially with regard to performance values.
...
* `long`: If the service should have a link:wato_services.html#discovery_auto[[.guihint]#Long Output#], you can specify it here. You can also use this keyword multiple times to create multiple lines in the [.guihint]#Long Output#.
...
The possibilities for monitoring your own SQLs of course go beyond the simple
...
|SQLS_SECTIONS |The self-defined section functions to be executed by the plug-in. *Important*: This variable can only be set outside a section function. (=_global_) |no
|SQLS_SIDS |The instances that are to execute the section(s). Can be set globally or per section. |no
|SQLS_DIR |The file path under which your own SQLs were stored. Can be global or per section. |no
|SQLS_SQL |The file that contains the instructions for a section. |no
|SQLS_PARAMETERS |In development |ja
|SQLS_SECTION_NAME |The section name when you have written your own check plug-in for the individual SQLs. |ja
...
|SQLS_ITEM_NAME |Determines an individual identifier for the service name. Normally the SID and the file name of the SQLs are taken over. *Important*: This variable can _not_ be used together with the variable `SQLS_SECTION_NAME`, and can also only be set within a section function |ja
|SQLS_ITEM_SID |In development |ja
|SQLS_MAX_CACHE_AGE |Performs the same task as link:monitoring_oracle.html#sections[`CACHE_MAXAGE`]. |ja
...
|SQLS_TNSALIAS |Sets an individual link:monitoring_oracle.html#tns_config[TNSALIAS] for the sections. |ja
|===
...
for one or more SQLs. However, this requires that you have also written an
...
for the plugin's output and can go your own way. Since this method is the most
comprehensive, and also the most difficult, it is only mentioned here for
completeness. It assumes that you know how to program a {CMK} check and
integrate it into the monitoring instance. After that you simply assign the
individual SQLs with the variables to this check plug-in.
...
Since this call is more likely to be made in the event of an error, you will of
course receive an output, and in this output you will also receive the
connection string used for the connection, and the first 100 characters of the
error message returned by the instance. With the help of this information,
...
_not_ masked -- so everything and every variable is readable in plain text.
...
=== On an Oracle-Host under Linux, AIX und Solaris
...
=== On an Oracle-Host under Windows
...
|share/check_mk/agents/plugins/mk_oracle |The plug-in that fetches the data on the Oracle host.
