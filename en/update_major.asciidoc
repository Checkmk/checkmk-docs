...
More recent Ubuntu STS versions will not receive support during the whole lifecycle of {CMK} {v22}.
// Neuere Ubuntu STS Versionen erhalten nicht mehr über den gesamten Support-Zeitraum Unterstützung.
However, we will provide the possibility to seamlessly perform updates up to the next LTS 24.04.
// Nutzern dieser Versionen geben wir jedoch die Möglichkeit, nahtlos auf die nächste LTS 24.04 zu aktualisieren.
For details please read the xref:update_matrix#ossupport[OS Support Policy].
