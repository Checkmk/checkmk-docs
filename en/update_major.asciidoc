// -*- coding: utf-8 -*-
// IGNORE Alertmanager facto p16 p20 p22 p23 p7 pre
...
However, the functionality of the {FE} lives on in the {CE}: the time-limited trial and the permanent use of one of the {EE} on a small scale.
...
If in your case the 25 host limit in the {FE} was sufficient, but the 750 free services of the {CE} were not, you can either purchase a subscription and upgrade to one of the {EE} -- or choose to downgrade to the free {RE}.
...
* Ubuntu in the Short Term Support (STS) versions 21.10 _Impish_ and 22.10 _Kinetic_
* SLES 12 SP3 and SP4
...
To find out which Linux distribution versions are supported by {CMK}, see the xref:install_packages#supported_distributions[installation article] and the link:https://checkmk.com/download[download page^] after you have selected the {CMK} version and your Linux distribution.
...
*Does this affect you?*
The Agent Bakery and thus the Bakery API are only relevant for users of the {EE}. +
...
With this comes link:https://github.com/python/cpython/issues/66683[a stricter interpretation^] of xref:regexes#[regular expressions].

*Does this affect you?*
Only if you use link:https://www.regular-expressions.info/modifiers.html[modifiers^] as 'inline flags' in regular expressions -- that is, not at the beginning of the regular expression, but in the middle of it.
This kind of usage has been marked as _deprecated_ since Python 3.6 and up until now has generated a warning when tested in the Python prompt.
This usage is no longer allowed in Python 3.11.

*What do you need to do?*
Identify regular expressions that use inline flags.
It is usually sufficient to check for the `(?` string.
Modify the affected regular expressions so that inline flags appear only at the beginning.
xref:regexes#testing[Test] the modified regular expressions with the prompt in the Python interpreter.
...
* Changed scope of `transform` class (link:https://checkmk.com/werk/14821[Werk pass:q[#]14821^]).
...
As of version {v22}, SAML authentication is built into the {CMK} {EE} and the configuration is done through the {CMK} user interface, as described in the article on xref:saml#saml_cee[SAML].
...
This deprecated way of configuring plug-ins has long been undocumented -- and in any case no longer works for all of those plug-ins that have been switched to the link:https://docs.checkmk.com/2.1.0/en/update_major.html#check-api[Check API] that was introduced with version {v20}.
