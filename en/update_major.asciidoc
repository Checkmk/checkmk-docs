...
This will affect you if your {CMK} server has one of the following Ubuntu versions installed that are still supported in {v20}:
...
During the beta phase, most of the distributions supported by {v21} will be supported to facilitate upgrades.
In the final {CMK} version {v22}, some obsolete distributions will no longer be supported.
If you want to be on the safe side, use a Linux distribution of Debian, RHEL (and Alma and Rocky Linux derivatives), SLES or Ubuntu with Long Term Support (LTS) beyond 2023 to (re)install a {CMK} server.

Debian 10 and 11, RHEL 8 and 9, SLES 12 from SP5 and 15 from SP1, and Ubuntu 20.04 and 22.04 will be supported throughout the {v22} product lifecycle.
Support for other Linux versions will be announced during the beta phase.

*What do you need to do?*
Before updating {CMK}, first perform a version upgrade of the Linux distribution.
...
For example, if you were using {CMK} {v21}p23 before upgrading the distribution, now install the {v21}p23 that matches your new Linux distribution version.
...
*What do you need to do?*
Identify those Windows hosts in the monitoring that use the Windows server operating systems 2008 R2 and client operating systems 7 or higher on x86_64.
...
If automatic agent updates are configured, xref:agent_deployment#deactivate_automatic_updates[disable them] before updating the {CMK} site.
...
However, before updating {CMK}, it certainly does not hurt to make sure of the operating system's currency beforehand, by using `apt` (for Debian and Ubuntu), `yum` and `dnf` (for Red Hat Enterprise Linux (RHEL) based systems) or `zypper` (for SUSE Linux Enterprise Server).
...
The update to {v22} requires at least {v21}p23.
This minimum patch version may be higher for {v22}pN.
...
Since version {v21} there are version checks to ensure that the sites involved are using compatible versions.
To summarize:
...
The following table shows the possible combinations when updating from {v21} to {v22}:
...
Note that the following also applies when updating in a distributed monitoring:
major versions must not be omitted, so that, for example, a connection between a {v20} and a {v22} site is not supported.
...
*Does this affect you?*
This check is useful if there are check plug-ins in your local files that were programmed conforming with the old Check API that was valid until version {v16}.

*What do you need to do?*
Run the following command as a a site user:
...
*Does this affect you?*
This affects you if you used the {CMK} special agent to monitor Kubernetes in version {v20}.
...
*What do you need to do?*
Completely redeploy Kubernetes monitoring as described in the xref:monitoring_kubernetes#[Monitoring Kubernetes] article.
////
