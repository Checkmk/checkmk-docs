...
you will probably need to read the short section on SNMP in the xref:intro_setup_monitor#snmp[Beginner's guide].
...
But SNMP has a second variation: _SNMP traps_. These are spontaneous push messages sent by devices at configured addresses via UDP (Port 162) in push mode. Traps have many disadvantages over active requests, which is the reason they are not very important for monitoring. Some of the disadvantages are:
...
. Enter the addresses of your {CMK} servers as the permitted IP addresses. It may also be useful to provide a test site of {CMK} here. Important: If you have multiple redundant {CMK} servers, do not forget to also specify the IP address(es) used after a failover. In the case of the {CMK} appliance in particular, this uses the IP address of the active node as the source IP address for outgoing connections -- and not the service IP address. In a xref:distributed_monitoring#[distributed environment] the IP address of the remote site from which the device is monitored is critical.
...
(except for xref:snmp#v3[SNMP Version 3]). Anyone who can listen to packets can therefore very easily identify the community. On the other hand you have limited access to read-only access,
...
We will discuss version 3 xref:snmp#v3[below]. If you do not have very high security requirements,
...
the [.guihint]#Access to Agents > SNMP credentials of monitored hosts# xref:wato_rules#[Rule Set].
...
the usual next step is the xref:wato_services#[configuration of services]. There are some peculiarities with this, because internally the service discovery is done very differently in SNMP devices compared to hosts, which are monitored with the {CMK} agent -- {CMK} can simply look at the agent’s output and find the items of interest using the individual check plug-ins. With SNMP a little more work is necessary.
...
otherwise SNMP would not function. In general {CMK} is preset so that it includes all ports that are active at the time of service discovery (operational status ‘up’) in the monitoring.
...
xref:intro_finetune#switchports[configuring switch ports].
...
no reply to the `sysDescr` for example. These devices are as good as dead in a diagnosis, and they will not deliver any results to a service discovery if you don’t help them with a special configuration.
To do this, for affected hosts create a rule under [.guihint]#Access to agents > Hosts without system description OID# with the [.guihint]#Positive match (Add matching hosts to the set# option.
...
* In the [.guihint]#Access to agents > Legacy SNMP devices using SNMP v2c# rule set, create a rule for the host, and set the value typically to [.guihint]#Positive match (Add matching hosts to the set)#.
...
you can manually limit {CMK} to polling only specified ports. Details can be found below in xref:snmp#performance[Performance].
...
* You can develop your own plug-ins. Information on writing your own plug-ins can be found in xref:devel_check_plugins#[several articles] in the manual.
...
For such cases, there is the [.guihint]#Bulk walk: Limit SNMP OID Ranges# rule set. This allows you to statically limit the list of queried data (e.g., ports). In the rule’s value, for each particular check plug-in you specify which indexes of the respective table are to be fetched.

The usual check type for switch ports is called [.guihint]#SNMP interface check with 64bit counters#.
...
Depending on the [.guihint]#Network interface and switch port discovery# setting, this does not automatically mean that these two ports really are monitored.
...
image::snmp_download_walk.png[alt="Download SNMP walk in the context menu of the host in the monitoring overview."]
