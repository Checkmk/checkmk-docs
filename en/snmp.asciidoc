...
the usual next step is the xref:wato_services#[configuration of services]. There are some peculiarities with this, because internally the service recognition is done very differently in SNMP devices compared to hosts, which are monitored with the {CMK} agent -- {CMK} can simply look at the agent’s output and find the items of interest using the individual check plug-ins. With SNMP a little more work is necessary.
...
xref:intro_finetune#switchports[configuring switchports].
