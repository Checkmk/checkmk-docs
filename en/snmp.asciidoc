...
//// 
MFS: Notes for translation
I updated screenshots and the paths to click. Added sections are marked and
have to be translated completely. Besides this, please read ans smoothe rough
edges, parts of the article seems to be translated be some non native speaker.
////
...
// MFS: New section! @Anthony: Please translate.
////
Sollte allerdings für eine Hard- oder Software-Komponente (beispielsweise ein
RAID-Controller) kein {CMK}-Agenten-Plugin zur Überwachung, dafür jedoch eine
SNMP-Schnittstelle existieren, können Sie selbstverständlich über SNMP
zusätzliche Monitoring-Daten erheben. Achten Sie in diesem Fall auf hinreichend
lange Abfrageintervalle.
////

Monitoring SNMP devices with {CMK} is very easy. If you just want to get started quickly with SNMP,
you will probably need to read the short section on SNMP in the link:intro_setup_monitor.html#snmp[Beginner's guide].
...
But SNMP has a second variation: _SNMP Traps_. These are spontaneous push messages sent by devices at configured addresses via UDP (Port 162). Traps have many disadvantages over active requests, which is the reason they are not very important for monitoring. Some of the disadvantages are:
...
However if you want or need to work with traps, the link:ec.html#snmp[Event Console] provides a solution.
...
. Enter the addresses of your {CMK} servers as the permitted IP addresses. It may also be useful to provide a test site of {CMK} here. Important: If you have multiple redundant {CMK} servers, do not forget to also specify the IP address(es) used after a failover. In the case of the {CMK} appliance in particular, this uses the IP address of the active node as the source IP address for outgoing connections -- and not the service IP address. In a link:distributed_monitoring.html[distributed environment] the IP address of the remote site from which the device is monitored is critical.
...
(except for link:snmp.html#v3[SNMP Version 3]). Anyone who can listen to packets can therefore very easily identify the community. On the other hand you have limited access to read-only access,
...
Now in the properties of the host (or folder), in the [.guihint]#Data sources# box set [.guihint]#Check_MK Agent# to [.guihint]#No agent#.
...
We will discuss version 3 link:snmp.html#v3[below]. If you do not have very high security requirements,
...
the [.guihint]#Access to Agents > SNMP credentials of monitored hosts# link:wato_rules.html[Rule Set].
This will allow you to assign the credentials based on host tags, link:labels.html[labels] and similar properties.
...
To do this save with the [.guihint]#Save & go to connection test# menu entry. Here is an example of the diagnostics for a switch.
...
the usual next step is the link:wato_services.html[configuration of services]. There are some peculiarities with this, because internally the service recognition is done very differently in SNMP devices compared to hosts, which are monitored with the {CMK} agent -- {CMK} can simply look at the agent’s output and find the items of interest using the individual check plug-ins. With SNMP a little more work is necessary.
...
However {CMK} has a smarter approach. Initially, it only retrieves the very first two records (OIDs) from the device -- the `sysDescr` and `sysObjectID`. Thereafter, as needed, further queries are invoked. Based on the results, each of the nearly 1,000 supplied SNMP check plug-ins decides whether the device actually supports this plug-in. {CMK} calls this phase the _SNMP scan_,
...
The [.guihint]#Full scan# button forces an SNMP scan and fetches fresh data via SNMP.
...
otherwise SNMP would not function. In general {CMK} is preset so that it includes all ports that are active at the time of service detection (operational status ‘up’) in the monitoring.
You can influence this with the [.guihint]#Parameters for discovered services > Discovery -- automatic service detection > Network Interface and Switch Port Discovery# set of rules.

By the way, in the Beginner's guide you will find a chapter on best practices when
link:intro_finetune.html#switchports[configuring switchports].

The second is the [.guihint]#SNMP Info# service which displays the same four pieces of information that
...
To do this, for affected hosts create a rule under [.guihint]#Access to agents > Hosts without system description OID# with [.guihint]#Positive outcome#.
...
With such a host, some simple SNMP checks will work -- such as [.guihint]#SNMP Info# or [.guihint]#SNMP Uptime#,
...
* In the [.guihint]#Access to agents > Legacy SNMP devices using SNMP v2c# rule chain, create a rule for the host, and set the value typically to [.guihint]#Positive match (Add matching hosts to the set)#.
...
you can manually limit {CMK} to polling only specified ports. Details can be found below in link:snmp.html#performance[Performance].

=== Only the standard services are found

You have included an SNMP device in the monitoring but {CMK} recognizes only the [.guihint]#SNMP Info#
and [.guihint]#SNMP Uptime# services and the interfaces. This can be due to a number of causes:
...
* You can develop your own plug-ins. Information on writing your own plug-ins can be found in link:devel_check_plugins.html[several articles] in the manual.
...
On a few devices you must use SNMP v3 and link:snmp.html#contexts[Contexts] to get the data you want.
...
either under the host tags or the [.guihint]#SNMP credentials of monitored hosts# rule set.
...
* Then you need another rule in the rule set [.guihint]#SNMPv3 contexts to use in requests#. Here you select the check plug-in for which contexts are to be activated, and then the list of contexts that should be queried in the monitoring.
...
and monitoring with SNMP consumes more CPU and memory than with {CMK} agents.

{cee-only}
While the {RE} makes SNMP requests in the classic way via the
...
If you are curious about the difference, you can use the [.guihint]#Hosts not using Inline-SNMP# rule set to turn
...
With the [.guihint]#Normal check interval for service checks# rule set, which you apply specifically to
the [.guihint]#Check_MK# services of hosts, you can extend the general interval of one minute to, for example, 2 or 5 minutes.

Especially for SNMP checks, there is also the rule set [.guihint]#Check intervals for SNMP checks#.
This allows you to lower the interval for _individual_ check plug-ins. It is important to know that you can never set the interval to faster than the interval for general monitoring by the [.guihint]#Check_MK# service.
...
Try the [.guihint]#Bulk walk: Number of OIDs per bulk# experimental rules chain to see if a higher value performs better.
...
For such cases, there is the [.guihint]#Limit SNMP OID ranges# rule chain. This allows you to statically limit the list of queried data (e.g., ports). In the rule’s value, for each particular check plug-in you specify which indexes of the respective table are to be fetched.

The usual plug-in for switchports is called [.guihint]#SNMP interface check with 64bit counters#.
...
Depending on the [.guihint]#switch port discovery# setting, this does not automatically mean that these two ports really are monitored.
...
the device on another {CMK} server, even if this other server has no actual network connection to the device.
...
[.guihint]#Check_MK# host’s service context menu, and also the icon:icon_agent_output[] [.guihint]#Download SNMP walk# option in the host’s menu:

[{image-border}]
image::snmp_download_walk.png[]

The creation of the walk takes a few seconds in the best case, but a few minutes are not uncommon.
When the build is done you can download the file via the [.guihint]#Result# line.


=== Creating a Walk from the Command line

Alternatively, you can also create walks from the command line.
Log on to the site from which the device is being monitored. The creation of the walk is simply
...
If you want to use this walk on a different (or the same) {CMK} site for a simulation,
then save the walk file with the name of the host on this site under `var/check_mk/snmpwalks`.

Now create a rule in the [.guihint]#Simulating SNMP by using a stored SNMP walk# rule that accesses the affected host(s).
