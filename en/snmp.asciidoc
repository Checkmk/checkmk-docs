...
= Monitoring via SNMP
:revdate: 2019-11-20
:title: Monitoring of SNMP devices with (CMK)
:description: (CMK) provides strong and flexible support for monitoring all possible devices with SNMP. Here is how SNMP works and how it works with CMK.

[.relateddoc]
wato_monitoringagents
...
you will probably need to read the short section on SNMP in the link:intro.html#snmp[Getting Started Guide].
...
[cols=<th style="width: 10%">Version</th>,<th style="width: 10%">Features</th>,<th style="width: 10%">{CMK}</th>,options="header"]
|===
...
|*v2c*
|Bulk queries,<br> 64-Bit-Counter
|yes
...
|Security
|no
|Version 2 offers even better security options in addition to the features of v2c. Version 2 of SNMP is not found in practice, therefore CMK does not support this protocol version. If you need security use link:snmp.html#v3[version 3] instead.
...
== Setting-up SNMP in {CMK}#snmphost


=== Preparing a device#enable_snmp
...
. Enter the addresses of your {CMK} servers as the permitted IP addresses. It may also be useful to provide a test instance of {CMK} here. Important: If you have multiple redundant {CMK} servers, do not forget to also specify the IP address(es) used after a failover. In the case of the {CMK} appliance in particular, this uses the IP address of the active node as the source IP address for outgoing connections -- and not the service IP address. In a link:distributed_monitoring.html[distributed environment] the IP address of the slave site from which the device is monitored is critical.
...
Now in the properties of the host (or folder), in the [.guihint]#Data sources}} box set {{Check_MK Agent}} to {{No agent#.
...
In the same box, also activate [.guihint]#SNMP}} and as the SNMP Protocol select {{SNMP v2 or v3#.
...
To do this save with the [.guihint]#Save & Test# button. Here is an example of the diagnostics for a switch.
...
link:intro.html#switchports[best practices when monitoring switch ports].
...
Finally there is the [.guihint]#SNMP Uptime# service, which shows you when the device was last restarted.
...
To do this, for affected hosts create a rule under [.guihint]#Access to agents|Hosts without system description OID}} with {{Positive outcome#.
...
With such a host, some simple SNMP checks will work -- such as [.guihint]#SNMP Info}} or {{SNMP Uptime#,
...
* In the [.guihint]#Access to agents|Legacy SNMP devices using SNMP v2c}} rule chain, create a rule for the host, and set the value typically to {{Positive match (Add matching hosts to the set)#.
...
|`authNoPriv`
|User-based authentication with a name ([.guihint]#Security name#) and a password, but no encryption.
...
== Performance and Timing#performance
...
=== Creating a Walk via the GUI#snmpwalks

You can create an SNMP walk directly from the GUI. This function can be found in the
[.guihint]#Check_MK}} host’s service context menu, and also the ICON[icon_agent_output.png] {{Download SNMP walk# option in the host’s menu:
...
[cols=<th style="width: 20%">Option</th>,options="header"]
|===
...
|`--oid`
|This option is similar to `--extraoid`, but _only_ retrieves the specified OID. This is of interest for testing purposes. Note, however, that the walk will be incomplete.


|`-v`
|The `v` stands for _verbose_ and will output some interesting information during the walk.
