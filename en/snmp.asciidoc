...
|*v3* |Security, + 
Context |yes |xref:snmp#v3[Version 3] is used when encrypting SNMP traffic. With v2c and v1 this runs in plain text – including in the community. In practice, version 3 is rather less common, because this version requires significantly more computing power, and also the cost of the configuration is significantly higher than with v2c. The _Contexts_ are a concept in which different information is visible in the same area of the SNMP data structure (OID), depending on the context ID. This would be used for partitioning of Fibre Channel switches for example.
|===
...
SNMP v2c requires the entry of the Community as discussed above.
...
In the same box, enable the [.guihint]#SNMP# option and set the value to [.guihint]#SNMP v2 or v3# or [.guihint]#SNMP v1#, as described above:
...
==== SNMP v2c works, but bulk requests fail
...
SNMP v3 recognizes various levels of security:
...
{cee-only}
While the {RE} makes SNMP requests in the classic way via the `snmpget` or `snmpbulkwalk` commands, the commercial editions have a built-in SNMP engine that performs SNMP requests very efficiently without generating any extra processes.
...
.1.3.6.1.2.1.1.4.0 Zoe Zhang <zoe@checkmk.com>
