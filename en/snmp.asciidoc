...
* In the link:.guihint].html#Access to agents[Legacy SNMP devices using SNMP v2c# rule chain, create a rule for the host, and set the value typically to [.guihint]#Positive match (Add matching hosts to the set)#.
...
image::CME.png[float=left,height=60]
