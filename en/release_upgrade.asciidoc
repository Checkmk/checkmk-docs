// -*- coding: utf-8 -*-
// IGNORE BTRFS Deregister reinstallation unmounting virtualized
...
xref:update_matrix#[Update matrix for version {v21}]
{related-end}
...
If you want to combine a release upgrade with an xref:update_major#[version update of {CMK}] or you have to do so (for example because an older {CMK} version is not provided for a more recent version of your Linux distribution), use  our xref:update_matrix#[update matrix] to determine the suitable order of the steps needed.
...
mysite    2.1.0p2.cee    default version
----
...
[{shell}]
----
{c-root} apt-get remove --purge -y check-mk-enterprise-2.1.0p1
Reading package lists... Done
Building dependency tree       
Reading state information... Done
The following packages will be REMOVED:
  check-mk-enterprise-2.1.0p1*
...
Removing check-mk-enterprise-2.1.0p1 (0.focal) ...
(Reading database ... 89444 files and directories currently installed.)
Purging configuration files for check-mk-enterprise-2.1.0p1 (0.focal) ...
...
[{shell}]
----
{c-root} apt-get remove -y check-mk-enterprise-2.1.0p2
Reading package lists... Done
Building dependency tree       
Reading state information... Done
The following packages will be REMOVED:
  check-mk-enterprise-2.1.0p2*
...
----
...
[{shell}]
----
{c-root} apt install /tmp/check-mk-enterprise-2.1.0p2_0.jammy_amd64.deb
----
