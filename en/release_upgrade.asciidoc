// -*- coding: utf-8 -*-
// IGNORE BTRFS Deregister reinstallation unmounting virtualized Checkmk Deutsch ZFS checkmk localdev
...
xref:update_matrix#[Update matrix for version {v20}]
{related-end}
...
If you want to combine a release upgrade with a xref:update_20#[version update of {CMK}] or you have to do so (for example because an older {CMK} version is not provided for a more recent version of your Linux distribution), use  our xref:update_matrix#[update matrix] to determine the suitable order of the steps needed.
...
* Creating a backup using the tools provided by {CMK}, either via the command line using the xref:omd_basics#[`omd` command] or via the xref:backup#[Setup].
...
. Create an xref:omd_basics#[archive] (in other contexts a backup) of the site.
