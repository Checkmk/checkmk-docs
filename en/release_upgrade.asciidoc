...
If you want to combine a release upgrade with an update of {CMK} or you have to do so (for example because an older {CMK} version is not provided for a more recent version of your Linux distribution), use  our xref:update_matrix#[update matrix] to determine the suitable order of the steps needed.
