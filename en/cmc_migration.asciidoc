...
If you wish to carry over the event history (e.g., for xref:availability#[availability]), copy the necessary files on the command line:
...
A carry over of scheduled downtimes, etc. from CMC to Nagios is not possible.
...
You manage these with the  [.guihint]#Integrate Nagios plugins# rule set, which you can find in [.guihint]#Setup > Services > Other services# -- and by the way, you can also use them without CMC.
...
The CMC utilises the xref:cmc_differences.html#smartping[Smart Ping] as the standard for host checks.
