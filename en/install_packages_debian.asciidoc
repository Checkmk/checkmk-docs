...
You can do that for example with the program _WinSCP_ or -- if you are able to use SSH connections -- with the command line tool `scp`.
...
In some constellations signature check fails with the following error:
...
In this case, revert to `gpg` to verify the signature:
...
The warning presented is just about the {CMK} public key, not about the signature on the package itself.
The reason is that the Checkmk public key is not signed by any other key the user is trusting.
