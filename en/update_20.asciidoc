...
[#checkmk_versions]
=== CheckMK Update Paths

These are the supported Update Path to CheckMK 2.x

CheckMK Support Team please write down from wich CMK Version/Patchlevel a direct update is supported.
Should a update must done from the latest 1.6.0 Patchlevel or is there a minimum 1.6.0 Patchlevel to update from?
...
Last but not least, the new check API scores with improved documentation. There is an introductory article on link:devel_check_plugins.html[Programming your own check plug-ins] with the new check API, and you can access the always up-to-date plug-in API reference documentation in {CMK} {v20} via the {CMK} interface -- in the new [.guihint]#Help# menu in the navigation bar. 
...
On the page mentioned above you can simply add a few hosts or host groups to the correspodning field and reactivate the master switch.

image::update_20_activate_update_on_selected_hosts.png[]

Just remember to remove these restrictions to explicit hosts and host groups once you are done testing the new agent and are satified with the results.

[#update_distributed_monitoring]
=== Updating in with a Distributed Monitoring
...
There is a third option: Updating during full operation. You may need this option for example if you manage a bigger amount of instances and the first two options are not suitable. this may be the case for technical or organisational reasons.

This procedure is *not recommended and performed at your own risk!*

Operating with different versions gives you the basics functionality. But some features of {CMK} may work in this case with restrictions or even not at all. Before you decide to use this procedure, you need to test the update for you individuel set of features. In this scenario it is even more important that you take care, that your backup is running and working, because you have to expect bigger problems. You will avoid such problems at best if you update the central instance as *last of all*.
...
The synchronisation of configuration files between sites in distributed monitoring has changed in version {v20}. In version {v16} *all* configuration files are synchronised between the central site and the remote sites; in version {v20} this happens incrementally, so that only those files that have changed will be synchronised.

To enable the updating of all sites utilising the distributed monitoring to version {v20} during operation, the synchronisation will also work for a transitional period in a mixed environment, as the incremental synchronisation is performed only if both instances are running on version {v20}. Otherwise the old method will be used. Please read absolutely the hints regarding the xref:live_update[Update during operation].
