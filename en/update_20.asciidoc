...
To enable the updating of all sites utilising the distributed monitoring to version {v20} during operation, the synchronisation will also work for a transitional period in a mixed environment, meaning *from* a central instance with version {v20} *to* a remote instance with Version {1.6}. Please read absolutely the hints regarding the xref:live_update[Update during operation].
