...
For more information, see the articles on link:backup.html#[Backups] and on link:omd_basics.html#backup[Backing-up and restoring sites].
...
An update to version {v20} currently does not require a specific {v16} patch release. However, there are good reasons to start the jump to {v20} only from the latest {v16} patch release, because e.&ThinSpace;g. a {v16} patch may contain fixes that make it easier to update to the {v20} version.
