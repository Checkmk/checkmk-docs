...
When reinstalling {CMK}, follow the xref:install_packages#_installation_instructions[detailed installation instructions] for your respective distribution.
...
Should you not be able to assign all packages via the check of the installed MKPs and the comparison with our catalogue, we recommend the xref:dry_run[dry run of the update] described below in order to identify incompatibilities and then remove these from your production monitoring before the update.
