...
The {CRE} is free, 100{nbsp}% Open Source and incorporates Nagios as its core. 
...
The {CSE} is primarily aimed at professional users and offers a number of interesting features beyond the scope of the {RE},
...
As a {SE} user, you can optionally receive professional support link:hhttps://checkmk.com/product/pricing[from our team^] or from one of our link:https://checkmk.com/partners/find-partner[partners^].

The {CCE} extends the functionality of the {SE} with features that are useful in cloud environments such as Amazon Web Services (AWS) and Microsoft Azure, 
...
More details on the {CE} can be found in xref:cce#[this article].

You can test the {CE} without obligation, because it is not limited for the first 30 days (in license state "Trial").
On a small scale, i.e. with one site and up to 750 services, you can also run {CMK} with the {CE} permanently without licensing in the license state "Free".
Entering a license key after 30 days at the latest is only necessary if you want to use the {CE} without limitations.

The {CME} is a multi-tenant capable extension of the {SE} and has all of the necessary functions for operating individual, self-contained sites for each of multiple clients with {CMK} via a xref:glossar#distributed_monitoring[distributed monitoring]. 
If you want to provide these services for your clients, this is your edition. 
For more details on the concept of managed services, see the introduction xref:managed#[of this article].

A list with descriptions of the differences between the various editions can be found on our link:https://checkmk.com/product/editions[website^].

{cee-only}
In this User Guide, whenever we discuss functions that apply only to the commercial editions -- i.e. to the {SE}, {CE} or {ME} -- we mark this with the icon as in this paragraph.
...
{cee-only}
The virtual appliance is available for all commercial editions, for the {CE} also in the license states "Trial" and "Free".
...
{cee-only}
Since the commercial editions have more features than the {RE}, you will see more services there. 
