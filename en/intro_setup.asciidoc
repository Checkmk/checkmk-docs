...
{cee-only}
In this User Guide, whenever we discuss functions that apply only to the {EE} -- i.e. to the {SE}, {CE} or {ME} -- we mark this with the icon as in this paragraph.
...
{cee-only}
The virtual appliance is available for all {EE}, for the {CE} also in the licence statess "Trial" and "Free".
...
The physical appliance is available for all {EE}. 
