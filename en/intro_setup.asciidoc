...
// This block replaces the following section "Selecting a {CMK} edition"
// start translation
////
[#editions]
== Edition auswählen
...
The {FE} contains all the features of the {SE}, is free of charge and unlimited for the first 30 days. 
...
a large number of sophisticated dashlets to build into xref:glossar#dashboard[dashboards], reporting, and much more.
...
The {CME} (*CME*) is a multi-client-capable extension of the {SE} which, via xref:glossar#distributed_monitoring[distributed monitoring], has all the necessary functions for operating an individual {CMK} site for each one of multiple independent customers.
...
You can find a breakdown of the differences between the editions on our link:https://checkmk.com/product/editions[homepage^].
...
{cee-only}
Since the {EE} have more features than the {RE}, you will see more services there. 
