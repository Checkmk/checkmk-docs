...
without obligation or if you want to use {CMK} on a small scale with up to two
sites, each with up to ten monitored hosts. The {FE} contains all of the
features of the {SE} and is free of charge. Both the {FE} and the {RE} can be
...
Created new site mysite with version 2.0.0b8.cee.
