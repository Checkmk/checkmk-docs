...
The {CSE} (CSE) is primarily aimed at professional users and offers a number of interesting features beyond the scope of the {RE},
...
The {CCE} (CCE) extends the functionality of the {SE} with features that are useful in cloud environments such as Amazon Web Services (AWS) and Microsoft Azure, 
...
The {CFE} (CFE) is the right version for you if you want to test the {SE} or {CE} without obligation, or if you want to use {CMK} on a small scale for one xref:glossar#site[site] with up to 25 monitored hosts. 
The {FE} contains all of the features of the {CE}, is free of charge and is not limited in its number of sites and hosts for the first 30 days. 
Both the {FE} and the {RE} can later be upgraded directly to the {SE} or the {CE} without further detours.

The {CME} (CME) is a multi-tenant capable extension of the {SE} and has all of the necessary functions for operating individual, self-contained sites for each of multiple clients with {CMK} via a xref:glossar#distributed_monitoring[distributed monitoring]. 
...
{cee-only}
In this manual, whenever we discuss functions that apply only to one of the {EE} -- i.e. to the CFE, CEE or CME -- or to the CCE, we mark this with the icon as in this paragraph.
