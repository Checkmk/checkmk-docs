...
You can test the {CE} without obligation, because it is not limited for the first 30 days.
On a small scale, i.e. with one site and up to 750 services, you can also run {CMK} with the {CE} permanently without licensing.
...
{cee-only}
In this User Guide, whenever we discuss functions that apply only to one of the {EE} -- i.e. to the CSE, CCE, CFE or CME -- we mark this with the icon as in this paragraph.
