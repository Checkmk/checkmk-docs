...
* For all participating hosts (this always means the cluster host and all its associated node hosts), the data sources must be configured identically, i.e. in particular, some may not be configured via a Checkmk agent and others via SNMP. {CMK} ensures that a cluster host can only be created if this requirement is met.
...
Complete the creation with [.guihint]#Save & go to folder# and link:wato.html[activate the changes].
...
Under [.guihint]#Setup => Hosts => Hosts#, in the host list at the cluster host entry click the icon:icon_services_edit[] icon to edit the services.
...
[.guihint]#Local checks in Checkmk clusters# rule set to influence the result by choosing between [.guihint]#Worst state# and [.guihint]#Best state#.
