...
:description: A group of networked hosts that work together to accomplish the same task is called a cluster. A cluster provides one or more services to the outside world: the cluster services. {CMK} helps you set up the cluster and to select these cluster services for monitoring.
...
* For all participating hosts (this always means the cluster host and all its associated node hosts), the data sources must be configured identically, i.e. in particular, some may not be configured via a {CMK} agent and others via SNMP. As of version 1.6.0p12, {CMK} ensures that a cluster host can only be created if this requirement is met.
