...
//
//
//H2:Das Monitoring eines Clusters
//
...
//
//NL:Cluster erstellen
//NL:Cluster-Services auswählen
//NL:Service-Erkennung für alle beteiligten Hosts durchführen
//
//Wie Sie dabei vorgehen, wird im nächsten Kapitel anhand der folgenden
//Beispielkonfiguration beschrieben:
//
//BI:cs_example_cluster.png border
//
...
//
//LI:„mssql-node01“ ist der aktive Knoten mit laufender Datenbankinstanz.
//LI:„mssql-node02“ ist der passive Knoten.
//LI:„mssql-cluster01“ ist das Cluster, dem beide Knoten angehören.
//
...
//
//
//H1:Cluster und Cluster-Services einrichten
//
//H2:Cluster erstellen
//
...
//
//LI: Der Cluster-Host ist ein virtueller Host, der mit Cluster-IP-Adresse konfiguriert werden soll, wenn diese existiert. In unserem Beispiel gehen wir davon aus, dass der Name des Cluster-Hosts über DNS auflösbar ist.
//
//LI:Cluster-Hosts können auf die gleiche Art und Weise konfiguriert werden wie „normale“ Hosts, zum Beispiel mit link:wato_rules.html#hosttags[Host-Merkmalen] oder link:wato_hosts.html#Hostgruppen[Host-Gruppen].
//
//LI:Für alle beteiligten Hosts (damit sind stets der Cluster-Host und alle zugehörigen Knoten-Hosts gemeint), müssen die Datenquellen identisch konfiguriert werden, d.h. insbesondere dürfen nicht einige per Checkmk-Agent und andere per SNMP konfiguriert sein. Ab Version 1.6.0p12 stellt {CMK} sicher, dass ein Cluster-Host nur dann erstellt werden kann, wenn diese Voraussetzung erfüllt ist.
//
//LI:In einem link:distributed_monitoring.html[verteilten Monitoring] müssen alle beteiligten Hosts derselben {CMK}-Instanz zugeordnet sein.
//
//LI:Nicht alle Checks funktionieren in einer Cluster-Konfiguration. Bei denjenigen Checks, die die Cluster-Unterstützung implementiert haben, ist der Parameter `node_info` auf `True` gesetzt. Sie können sich diese Checks auflisten lassen, indem Sie im Plugin-Verzeichnis `share/check_mk/checks` das folgende Kommando ausführen: `grep "node_info" * | grep True | cut -d: -f1 | uniq`
//
//In unserem Beispiel sind die beiden Knoten-Hosts „mssql-node01” und „mssql-node02“ bereits als link:wato_hosts.html[Hosts] angelegt und eingerichtet. Wie es
//so weit kommen konnte, können Sie im link:agent_windows.html[Artikel zur Überwachung von Windows-Servern] nachlesen -- und dort im Kapitel zur Erweiterung des Standard Windows-Agenten mit Plugins, für unser Beispiel den link:check_plugins_catalog.html#Microsoft%20SQL%20Server[MS SQL Server-Plugins].
//
//Starten Sie die Erstellung des Clusters mit [.guihint]#WATO => Hosts => New cluster#:
//
//BI:cs_create_cluster.png
//
//Geben Sie als [.guihint]#Hostname# `mssql-cluster01` ein und tragen Sie unter
//[.guihint]#Nodes# die beiden Knoten-Hosts ein.
//
//*Hinweis:* Falls Sie es mit einem Cluster ohne Cluster-IP-Adresse zu tun haben, müssen Sie einen nicht ganz komfortablen Umweg gehen: Sie können im Kasten [.guihint]#Network Address# für die [.guihint]#IP Address Family# den Eintrag [.guihint]#No IP# auswählen. Um aber zu verhindern, dass der Host im Monitoring auf {DOWN} geht, müssen Sie für diesen das stardardmäßig eingestellte „Host Check Command“ über die gleichnamige Regel ändern: von [.guihint]#Smart PING# bzw. [.guihint]#PING# auf z.B. den Status eines der Services, die dem Cluster-Host im nächsten Kapitel zugewiesen werden. Mehr Informationen zu den Host-Regelsätzen finden Sie im link:wato_rules.html[Artikel über Host- und Serviceparameter].
//
//Schließen Sie die Erstellung mit [.guihint]#Save & Finish# ab und aktivieren Sie die
//Änderungen.
//
//
//H2:Cluster-Services auswählen
//
...
//Seite [.guihint]#Properties of host# klicken Sie [.guihint]#Clustered Services#. Sie landen auf der Seite des Regelsatzes [.guihint]#Clustered services#, auf der Sie eine neue Regel erstellen können.
//
...
//
//
//H2:Service-Erkennung durchführen
//
...
//Ergebnis führen.
//
//Klicken Sie [.guihint]#Start# um die link:wato_services.html#bulk_discovery[Serviceerkennung für viele Hosts] zu beginnen. Nach erfolgreichem Abschluss, erkennbar an der Meldung `Bulk discovery successful`, verlassen Sie die Seite und aktivieren die Änderungen.
//
//Um herauszufinden, ob die Auswahl der Cluster-Services zum gewünschten Ergebnis
//geführt hat, können Sie sich alle Services auflisten, die nunmehr dem Cluster zugewiesen sind: Unter [.guihint]#WATO => Hosts# klicken Sie in der Host-Liste beim Eintrag des Cluster-Hosts auf icon:icon_services[]. Auf der folgenden Seite
//[.guihint]#Services of host# werden unter [.guihint]#Monitored services# alle Cluster-Services
//aufgelistet:
//
//BI:cs_cluster_monitored_services.png border
//
...
//
//BI:cs_node_monitored_services.png border
//
//*Tipp:* Falls Sie link:localchecks.html[lokale Checks] in einem Cluster ausführen, in der die Regel [.guihint]#Clustered services# angewendet ist, können Sie mit dem Regelsatz [.guihint]#Settings for local checks# das Ergebnis beeinflussen, indem Sie zwischen [.guihint]#Worst state# und [.guihint]#Best state# auswählen.
//
//
//H1:Überlappende Cluster
//
...
//
//BI:cs_example_cluster_overlap.png border
//
//In dieser
...
//
//LI:„mssql-node01“ ist der erste aktive Knoten mit der laufenden Datenbankinstanz „MSSQL Instance1“.
//LI:„mssql-node02“ ist der zweite aktive Knoten mit der laufenden Datenbankinstanz „MSSQL Instance2“.
//LI:„mssql-cluster01“ und „mssql-cluster02“ sind die beiden Cluster, denen beide Knoten angehören.
//
...
//suchen Sie den Regelsatz [.guihint]#Clustered services for overlapping clusters# und klicken ihn an. Wählen Sie unter [.guihint]#Create rule in folder# wieder den Ordner aus, der die Knoten-Hosts enthält.
//
//Tragen Sie als [.guihint]#Assign services to the following cluster# das Cluster `mssql-cluster01` ein:
//
//BI:cs_rule_cs_overlapping.png
//
...
//
//BI:cs_rule_cs_overlapping_conditions.png
//
...
//tragen Sie jetzt `MSSQL Instance2` ein für alle MS SQL-Services der zweiten Datenbankinstanz. Der zweite Knoten-Host, auf dem die zweiten Datenbankinstanz standardmäßig läuft, greift auf sein Speichermedium unter einem anderen Laufwerk zu, im folgenden Beispiel über das Laufwerk E:
//
//BI:cs_rule_cs_overlapping_conditions2.png
//
//Sichern Sie auch diese Regel und aktivieren Sie dann die beiden Änderungen.
//
//Führen Sie abschließend die Service-Erkennung als 3. und letzten Schritt genauso aus wie oben beschrieben: als [.guihint]#Bulk discovery# für alle beteiligten Hosts.
//
