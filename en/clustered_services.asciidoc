...
:description: A group of networked hosts that work together to accomplish the same task is called a cluster. A cluster provides one or more services to the outside world: the cluster services. Checkmk helps you set up the cluster and to select these cluster services for monitoring.
...
* Cluster hosts can be configured in the same way as 'normal' hosts, for example with link:wato_rules.html#hosttags[host tags] or link:wato_hosts.html#hostgroups[host groups].
* For all participating hosts (this always means the cluster host and all its associated node hosts), the data sources must be configured identically, i.e. in particular, some may not be configured via a Checkmk agent and others via SNMP. As of version 1.6.0p12, {CMK} ensures that a cluster host can only be created if this requirement is met.
* In a link:distributed_monitoring.html[distributed monitoring] all participating hosts must be assigned to the same {CMK} site.
* Not all checks work in a cluster configuration. Those checks that have had cluster support implemented have the `node_info` parameter set to `True`. You can list these checks by executing the following command in the plug-in directory `share/check_mk/checks`: `grep "node_info" * | grep True | cut -d: -f1 | uniq`.

In our example, the two node hosts 'mssql-node01' and 'mssql-node02' have already
been created and set up as link:wato_hosts.html[hosts]. To find out how to get this far,
see the link:agent_windows.html[article on monitoring Windows servers] -- and there in the
...
Start the creation of the cluster with [.guihint]#WATO => Hosts => New cluster#:
...
see the link:wato_rules.html[article on host and service parameters].

Complete the creation with [.guihint]#Save & Finish# and activate the changes.
...
Start with [.guihint]#WATO => Hosts# and click the cluster name.
On the [.guihint]#Properties of host# page click [.guihint]#Clustered services#. You will land
...
drive. The entries are interpreted as link:regexes.html[regular expressions].
...
Under [.guihint]#WATO => Hosts#, first select all of the hosts that are involved and then
click the [.guihint]#Bulk discovery# button. On the [.guihint]#Bulk discovery# page,
the first option [.guihint]#Add unmonitored services and new host labels# should
produce the desired result.

Click [.guihint]#Start# to begin the
link:wato_services.html#bulk_discovery[Service discovery for multiple hosts].
...
Under [.guihint]#WATO => Hosts#, click icon:icon_services[] in the host list at the
cluster host entry. On the following page [.guihint]#Services of host# all cluster
...
*Tip:* If you run link:localchecks.html[local checks] in a cluster where the
[.guihint]#Clustered services# rule has been applied, you can use the
[.guihint]#Settings for local checks# rule set to influence the result by choosing
between [.guihint]#Worst state# and [.guihint]#Best state#.
...
Let's start with the first rule: Under [.guihint]#WATO => Host & Service Parameters# find
...
way as described above -- as a [.guihint]#Bulk discovery# for all of the associated hosts.
