...
been created and set up as xref:glossar#host[hosts]. To find out how to get this far,
...
the link:https://checkmk.com/de/integrations?tags=mssql[MS SQL Server plug-ins].
...
*Note:* If you are dealing with a cluster without a cluster IP address, you will need to take a not-so-comfortable detour, 
...
This example implements a 100{nbsp}% overlapping cluster because the two nodes belong
to both clusters.
...
For our example with 100{nbsp}% overlap we need two of these rules: The first rule
