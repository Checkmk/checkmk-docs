...
Den Service-Account mit der zugehörigen Rolle und der sogenannten RoleBinding, können Sie durch die Angabe einer von uns vorgefertigten und link:https://github.com/Checkmk/checkmk_kube_agent/blob/main/deploy/kubernetes/checkmk-serviceaccount.yaml[auf GitHub veröffentlichten YAML-Datei^] vornehmen.
...
Aktivieren Sie die Option [.guihint]#Enrich with usage data#, wählen Sie im folgenden Menü [.guihint]#Use data from OpenShift (Cloud Edition)# und tragen Sie den [.guihint]#Prometheus API endpoint# ein, den Sie im Abschnitt xref:get_prometheus_endpoint[Prometheus-API-Endpunkt auslesen] ermittelt haben.
