...
Monitoring OpenShift clusters is only possible with the {CCE}.


== Introduction

{CMK} helps you to monitor your OpenShift clusters. Starting with version {v22}, you can use the {CCE} to monitor the following objects:
...
Additionally, using the xref:dashboards[Kubernetes dashboards] in the {CCE} is only useful if the appropriate workload data is available.
...
The service account with its associated role and the so-called RoleBinding can be created by specifying our ready-made link:https://github.com/Checkmk/checkmk_kube_agent/blob/main/deploy/kubernetes/checkmk-serviceaccount.yaml[YAML file published on GitHub^].
...
To ensure separation between objects in different Kubernetes clusters, it is usually convenient to create a folder per cluster via [.guihint]#Setup > Hosts > Add folder#, in which the xref:dcd#[dynamic host configuration] can create all of a cluster's hosts automatically.
However, creating and using such a folder is optional.

Next, set up a connector for the incoming piggyback data.
...
What exactly this option does and under which circumstances hosts are then actually deleted is explained in the xref:dcd#automatic_deletion_of_hosts[automatic deletion of hosts] chapter in the article on dynamic host configuration.

Finally, enable the [.guihint]#Discover services during creation# option.

The [.guihint]#Connection Properties# section of this new connector might look like the following:

image::monitoring_openshift_connection_properties.png[alt="Example settings for a dynamic host configuration."]
...
Enable the [.guihint]#Enrich with usage data# option, select [.guihint]#Use data from OpenShift (Cloud Edition)# in the following menu, and enter the [.guihint]#Prometheus API endpoint# that you determined in the xref:get_prometheus_endpoint[Retrieve the Prometheus API endpoint] section.
...
Now activate all of the changes you have made and let the dynamic host configuration do the work from now on.
...
The {CCE} comes with six built-in dashboards for Kubernetes.
