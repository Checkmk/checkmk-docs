...
[.guihint]#Host & Services Problems}} and {{Network Topology#. You find them in
...
[.guihint]#Host Statistics}} and {{Service Statistics}} in the {{Main Overview# dashboard),
...
* In every corner there is an an anchor element icon:dashlet_anchor_off.png]. Only one anchor can be active ICON[dashlet_anchor_lo[]. In the standard installation the active anchor is at top left.
* Indicators show active icon:dashlet_anchor_lo.png] and inactive ICON[dashlet_anchor_off[] anchors. These indicators show the element’s size mode: grey (fixed size), yellow (element grows automatically), red (maximum size).

== Creating a new Dashboard#new

To create a new dashboard, in the [.guihint]#Views}} element click on {{EDIT# in the sidebar.
