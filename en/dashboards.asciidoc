...
Even away from the dashboard and dashlet configuration, there are important settings in {CMK}, namely permissions. Under [.guihint]#Setup > Users > Roles & permissions > Edit role user# you can simply filter for _dashboard_ to list all options. Here for a xref:wato_user#role[role] you can specify in detail which standard dashboards its assigned users can see and what exactly they are allowed to do with other dashboards.
