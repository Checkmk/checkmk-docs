...
[.relateddoc]
omd_basics,  wato_services, commands
...
|The buffered agent data can be found here. The directory is empty if agent data has never been retrieved. Because all such data is located in `tmp/` in a RAM disk, this directory will be empty after a restart.
