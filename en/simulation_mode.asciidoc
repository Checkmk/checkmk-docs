...
for example, when the xref:omd_basics#cpmv[copy] of an instance is to be xref:update[updated]
...
* Hosts and services on other instances in a distributed monitoring will continue to be monitored as usual. Only in a distributed Setup will the simulation mode be passed on to the other remote sites.
