...
H2:Schedule downtimes on hosts

For example, if you clicked icon:commands[] commands in the [.guihint]#Services of Host# view, the scheduled downtime would actually apply directly to all or all selected services. If this is not desired, you do not have to navigate to the [.guihint]#Status of Host# view first, but can use this option to set the downtime directly to the host.
