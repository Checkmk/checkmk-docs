...
|`agent_registration` |The permission to register new hosts -- nothing else. |Intended for an automation user for xref:hosts_autoregister#[creating hosts automatically.] This role exists only in the {CCE}.
...
If one of these users no longer has access to {CMK}, for instance if he has lost or damaged his token, you can delete the two-factor authentication for this user. 
To do this, open the relevant entry in the user administration by clicking on icon:icon_edit[alt="Icon for editing."].
In the user's view, disable the two-factor authentication for this user via [.guihint]#User > Disable two-factor authentication#.

image::wato_user_2fa_disable.png[alt="Disabling two-factor authentication.", width=39%]
...
// This block replaces the following section "Changing a password using the command line"
// start translation
////
[#change_passwd]
...
We strongly recommend that you encrypt your password with bcrypt by using the options `-B -C 12`.
Unencrypted passwords do not allow login to the GUI.
