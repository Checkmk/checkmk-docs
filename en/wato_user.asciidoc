...
== User management with WATO#user_config

User management can be found in the [.guihint]#Users# link:wato.html[WATO] module ICON[icon_users.png].
...
receive [notifications|notifications] by email (link:install_packages.html#smtpconfig[SMTP configuration] necessary).
...
this group was called [.guihint]#Everbody#, but the name was thought to be misleading).
...
Contact groups are administered with the ICON[icon_contactgroups.png] [.guihint]#Contact GSroups# WATO module.
...
via link:wato_hosts.html#folder|Folder] and via [wato_rules[Rules].
...
== Visibility of hosts and services#visibility
...
If here you change [.guihint]#Hosts}} to {{Strict - Must be explicit contact of a service#
...
Roles are managed in the WATO ICON[icon_roles.png] [.guihint]#Roles & Permissions# module:
...
the ICON[button_clone.png] [.guihint]#Clone# button. The new role is not simply a copy,
...
== Automation user (for Webservices)#automation
...
== WATO permissions#wato_permissions
...
*  Only changes to hosts, services, [wato_rules|rules] and link:bi.html[BI-Aggregates] are allowed.
...
The [.guihint]#Enforce change: Change password at next login or access}} option in the {{Security#
...
=== Changing a password using the command line#change_passwd
...
with the [.guihint]#Custom macros# ICON[button_custom_macros.png] button you can create your own fields
...
|The user will receive a ICON[notify_users4.png] numeral at the bottom of the side bar advising that a message is in their inbox - which the user can then open at will. 
|===
...
|This file contains a random secret for signing login cookies. The file should be the same in all instances in distributed environments. When you configure everything with WATO it will take care of this file automatically. If this data is changed all logins will immediately be void and users must log themselves in anew. The file is furnished with the `660` permission, as read access could enable a third party to falsify a login.
