...
they will be appear in the table with the 
icon:icon_user_locked[] icon. They will no longer be able to log in,
...
If the user was logged in at the time of the lockout, they will be
...
By default the complete inventory is visible, but it can also be completely suppressed or selectively enabled. 
For example, to display only some selected inventory data on the processor (model and architecture), you can do this with the 
[.guihint]#Allowed to see the following entries# option and by specifying *internal inventory paths*.

image::wato_user_contact_groups_inventory_00.png[alt="Dialog for the visability of inventory data."]

To be able to enter the required path information, you must first read it out of the inventory data.
To do this, proceed as follows:

First call up the [.guihint]#HW/SW Inventory# for the (relevant) host, and activate the display of the internal inventory paths via [.guihint]#Display > Modify display options# and the [.guihint]#Show internal tree paths# option.

image::wato_user_contact_groups_inventory_01.png[alt="Option for displaying the inventory paths."]

Now you see the internal designations in the inventory itself, so for example the internal path for 
the area [.guihint]#Processor# is called [.guihint]#hardware.cpu#. 
The names for the CPU model and architecture -- [.guihint]#model# and [.guihint]#arch# -- can be found below in the CPU data.

image::wato_user_contact_groups_inventory_02.png[alt="Inventory data with highlighted internal paths."]

Back in the [.guihint]#Permissions# area within contact groups, you now enter the CPU path at [.guihint]#Path#
and the specific CPU records at [.guihint]#Attributes#.

image::wato_user_contact_groups_inventory_03.png[alt="Dialog for the visibility of inventory data using a CPU filter."]

The result -- users in this contact group will only see a slimmed-down inventory:

image::wato_user_contact_groups_inventory_04.png[alt="Inventory with selected records."]
...
The fact that normal users (the `user` role) only see those objects for which they are a contact is all the more important the larger
your monitoring environment is. 
