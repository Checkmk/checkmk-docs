...
which is generated automatically when a site is created.
...
User management can be found in [.guihint]#Setup > Users > Users#.
In a newly-created site this page will appear as below:

[{image-border}]
image::wato_user_users.png[]

Here `cmkadmin` can be seen -- the only user which is automatically
generated when a new site is created. In {CMK}-Appliance this user can have
...
For a new site the contact group [.guihint]#Everything# is automatically generated,
...
In this rule set you will find a predefined rule which is generated when a
site is created, and which assigns all of the hosts to the [.guihint]#Everything# contact group.
...
This is handled by a notifications rule which is automatically created for a new site.
...
Incidentally -- when a new {CMK} site is created only a single user with the
...
settings will be immediately passed on to all monitoring sites.
...
* Managing hosts with the link:rest_api.html[REST-API]
...
If the script that opens the URL is running directly in the monitoring site you can
...
permissions based on folders in the link:wato.html[{CMK} configuration environment].
...
As the site user, also edit the `etc/htpasswd` file by removing the
...
Apache installation. This does *not* ask for the existing password. We recommend
encrypting your password using MD5 by using the `-m` option. Unencrypted and
bcrypt encrypted passwords will not work, when trying to login to the GUI.
...
with user management. As always all information here is relative to the site's directory.
...
|etc/auth.secret |This file contains a random secret for signing login cookies. The file should be the same in all sites in distributed environments. When you configure everything with WATO it will take care of this file automatically. If this data is changed all logins will immediately be void and users must log themselves in anew. The file is furnished with the `660` permission, as read access could enable a third party to falsify a login.
