...
:revdate: 2016-10-20
[.seealso][#wato] [#wato_rules] [#ldap]:title: Authorization and user configuration
...
permissions in Checkmk. Before we go into the details, however,
we’ll first need to explain a few terms.

In Checkmk a *{{User*# is one with access to the
...
In Checkmk-Appliance this user can have a different name because you can specify
...
As always in Checkmk, the ID of a dataset -- (here [.guihints]#Username}})# -- cannot be
...
are controlled by scripts which access Checkmk per HTTP, and which are authenticated via the URL.
...
By default Checkmk notifies all contacts of an affected host or service when problems occur.
...
A new version of Checkmk can alter these default values however (this occurs very rarely).
...
When connecting Checkmk to other systems it is often desired that specific tasks normally
...
To simplify this procedure Checkmk offers the concept of the _Automation user_.
...
found in the Checkmk _Treasures_, and with which script-controlled planned downtimes
...
Test this by logging out of Checkmk in your browser, and then pasting the contructed
...
generally guidelines for working with passwords. Checkmk offers a number of approaches
...
to Checkmk from two browsers in parallel. This option is at the same time linked with
...
In the [notifications|article on notifications] we very comprehensively cover how Checkmk can
...
the actual Checkmk-System.

To this end Checkmk offers a small built-in messaging system that functions
...
The following summary shows which files and directories on the Checkmk-Server are associated
