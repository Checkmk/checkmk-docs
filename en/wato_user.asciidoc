...
|`agent_registration` |The permission to register a host's xref:wato_monitoringagents#agents[{CMK} agent] (more precisely, its Agent Controller) with the {CMK} server -- nothing else. |Intended for an automation user to perform registration with minimal privileges. In the {CCE}, this role contains additional permissions for xref:hosts_autoregister#[creating hosts automatically].
