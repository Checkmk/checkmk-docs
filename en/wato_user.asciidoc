...
editing an existing user with icon:icon_edit[] consists of four sections.
...
Contact groups are administered with the icon:icon_contactgroups[] [.guihint]#Contact GSroups# WATO module.
...
The functions of the numerous permissions can be found in the icon:icon_help[] online help.
...
(icon:icon_perm_yes.png] Symbol), or respectively, removed (ICON[icon_perm_no[] Symbol).
...
icon:button_random[] randomising die:
...
|The user will receive a icon:notify_users4[] numeral at the bottom of the side bar advising that a message is in their inbox - which the user can then open at will. 
|===
