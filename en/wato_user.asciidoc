...
This two-factor authentication is based on the FIDO2/WebAuthn Internet standard. Authentication is conventionally based on knowledge (a password) and possession (an authenticator). You can use hardware authenticators such as the YubiKey or another similar USB token.
...
The change is made with the `cmk-passwd` command, which is available starting with the patch release 2.1.0p16.
This will *not* ask you for the existing password.
`cmk-passwd` will - in an up-to-date version of {CMK} - always use a secure method to encrypt your passwords.
Currently `cmk-passwd` will use bcrypt for that.
Unencrypted passwords do not allow login to the GUI.
Passwords with a weak encryption like MD5 will not allow login as of {CMK} {v22}.
...
If you are using a patch release *before 2.1.0p16* you have to rely on `htpasswd`.
