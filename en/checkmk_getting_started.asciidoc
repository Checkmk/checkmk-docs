...
After you have installed {CMK} on a xref:install_packages#[Linux server], you will learn here what still needs to be done to create, start and fill a site with life by adding the first devices to the monitoring - and to complete everything in just a few steps.

*Note:* If you are running {CMK} with one of our appliances or in a Docker container instead of on a Linux server, the site management runs differently than described below:
{CMK} appliances are administered via a special web interface that also covers the creation of sites, which is explained in the xref:appliance_usage#site_management[article about the appliance]. 
...
You will learn how to do this in the next paragraph but one.
...
Now log in with the user name `cmkadmin` and the random password or with the one you changed at the beginning. 
...
For detailed instructions on installing the agent, see xref:intro_setup_monitor#install_agent[Beginner's guide].
...
image::checkmk_getting_started_services_undecided.png[alt="Services found on the host to add to monitoring"]
