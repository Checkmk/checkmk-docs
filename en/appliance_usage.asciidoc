// -*- coding: utf-8 -*-
// IGNORE +49 Infos User_Memory pre support@checkmk CFW CIFS CMA FQDNs NFS SSHFS cma netmask snapin unmount
...
The [.guihint]#{CMK} Appliance# snapin is available for all sites on the appliance:

In all appliance sites, the [.guihint]#{CMK} Appliance# snapin is available to all administrators in the xref:user_interface#sidebar[sidebar]:

image::cma_site_sidebar_2.png[alt="The snapin {CMK} Appliance.",width=50%, align=center]

With the entries in this snapin, you can always switch from a site to the appliance's web interface.
...
*Note:* Make sure that you select the firmware that matches the edition installed on your appliance. 
An update of the {CE} in the license state "Trial" or "Free" to a full version of the {EE} is of course also possible.
...
* Change in the first digit (_major release_) of the version number: You must manually back up your appliance's data and import it again following the update. An update without data migration is not possible.
* Update to a higher number in the second digit (_minor release_): The update can be carried out without data migration. A prior backup is nevertheless recommended.
* Change of the third digit (_patch_) in the version number: The update can be carried out without data migration. A prior backup is nevertheless recommended.
...
Note on the *Update from 1.4 to 1.5:*
If you update the firmware from 1.4 to 1.5, you must also optionally update all installed {CMK} versions or reinstall them in the same version (but for a newer platform).
The reason for this is that 1.5 is based on a new Debian version.
Under [.guihint]#Site Management# you can check whether there are any incompatible firmware versions.
Incompatible versions can no longer be started.

[{image-border}]
image::cma_sites_incompatible_versions.png[alt="Display of incompatible firmware versions."]
...
.The iDrac port gives access to the seperate managment interface
...
Access to the _root_  system user is possible, but is not recommended or supported, as it is very easy to damage the configuration or the software.
...
Up to firmware version 1.4, when root access is activated you will see the addition *tainted* with the version information in the console -- simply as a small warning.
...
To be able to guarantee the stability of the system platform, a fixed amount of the working memory is reserved for the device's operating system.
Depending on your appliance configuration, the amount of reserved memory is as follows:

* Single unit (without cluster configuration): *100 MB*
* Clustered: *300 MB*
...
A [.guihint]#User_Memory# service is created on the device, which shows you the current and historical utilization.
...
The current versions of the firmware can be found on our link:https://checkmk.com/download[website.^]
To access the {EE} and {ME} you will require the access data from your current support contract.
