...
[#init_start]
=== Initializing at first start

At this point you should have set up the appliance on a xref:cma_rack1_quick_start#[rack] or installed and started in a xref:introduction_virt1#[virtual machine], either VirtualBox or VMWare ESXi.
...
This language will be set for the whole device.
Afterwards, a message appears prompting you to initialise the data volume:

image::cma_boot_preinit_init_uservol_2.png[]

Confirm this dialogue and wait for device startup to be resumed and finished.
The status view is now displayed on the local console:

image::cma_console_welcome_2.png[]

This view shows you general status information and the most important configuration options of your device.
...
In the configuration menu, select [.guihint]#Device Password# in order to set the password. 
...
If you have already connected the device with your network correctly, you will now see in the [.guihint]#Status# box that the network connection is active: [.guihint]#LAN: UP#.


==== Protecting access to the console

When you started the appliance, you may have noticed that there was no password prompt.
Anyone who has direct access to the rack or the management interface of the virtualisation solution is able to change the basic configuration via the console

Anyone with physical access to the rack or the management interface of the virtualisation solution could change the
change the basic settings via the console. 
...
Here you can see the login screen of the web interface:

image::cma_webconf_login_2.png[width=50%]

After you have logged in with the previously set device password, the main will open. 
From here you can access all of the features of the web interface.

[{image-border}]
image::cma_webconf_index_2.png[]

Select [.guihint]#Device Settings# to view the most important device settings and change them if necessary:

image::cma_webconf_settings_2.png[]

By clicking on the parameter name you will be taken to the respective page for adjusting the respective setting.

If you have DNS servers available in your environment you should first now configure one or more of these so that the resolution of host names can be used. 
...
This completes the basic configuration of the device and you can proceed with the installation of the {CMK} software and the setup of the first monitoring site.


[#manage_cmk]
== Administrating {CMK} software versions

Starting with appliance version 1.4.14, {CMK} software is no longer pre-installed in the appliance.

The {CMK} software for installation in the appliance is offered to you as a CMA file. 
Download the CMA file -- either in the link:https://customer.checkmk.com/[customer portal] for the {SE} and {ME} or on the link:https://checkmk.com/download[download page] for the {FE}.
You will find the CMA file after selecting the {CMK} edition, version and platform appliance.

Select in the main menu [.guihint]#Check_MK versions#, on the following page the CMA file from from your hard disk with the help of the file selection dialogue and confirm your selection by clicking on
[.guihint]#Upload & Install#.

Now the {CMK} software is uploaded to the device. 
...
This allows several sites to be run in different versions, and for individual sites to be changed to newer or older versions independently of one another. 
This means that you can install a new version for example and try it out initially in a test site in order to then update your production site if the testing is successful.
...
[#site_management]
== Administrating monitoring sites
...
On this page you have access to all monitoring sites of this device. 
...
On the following page you can specify the initial configuration of the site.
...
You will be offered all versions that are installed in the xref:manage_cmk [management of {CMK} software versions].
...
If necessary, you can adjust them later via the site editing page.
...
In the login dialogue of the site, enter the access data you specified when creating the site.
...
image::cma_site_sidebar_2.png[width=50%]

The entries of this snapin will take you from your sites directly to the web interface of the appliance.
...
First install the desired new version as described in the chapter on xref:manage_cmk[management of {CMK} software versions].

Then list the sites on the web interface of the appliance ([.guihint]#Main Menu > Site Management#):

[{image-border}]
image::cma_webconf_site_list_2.png[]

Make sure that the site is not running, i.e. if the [.guihint]#Status# is equal to [.guihint]#running#, stop the site ([.guihint]#Control > Stop#). 
Then, under [.guihint]#Actions#, click on the Update icon icon:icon_cma_site_update[].
...
With the [.guihint]#Back# button you return to the list of sites in which the new {CMK} version is entered in the column [.guihint]#Version#. You can now start the site again.
...
Open the main menu of the web interface and click on [.guihint]#Site Management#. Then click on the [.guihint]#Migrate Site# button, which will take you to this page:

image::cma_webconf_site_migrate_start_2.png[]

On this page you first need to configure under [.guihint]#Source Host# the host address (host name, DNS name or IP address) of the source system which you want to migrate the site from. 
Next you need to enter the site ID of the site you want to be migrated under [.guihint]#Source site ID#.
...
Additionally you have the option to skip the carrying-over of performance data (measurements, graphs) and historical data during the migration.
...
After you have filled in the parameters and confirmed with [.guihint]#Start#, the progress of the migration is displayed:
...
You can update the software of your device, i.e. the firmware of the appliance, to a newer version during operation -- or also change back to an older version. 
...
The CFW file can be found in the download area at the product Checkmk Appliance.

*Note:* Be sure to select firmware that matches the installed edition of your appliance. 
However, if you want to upgrade from the {FE} to a full version of the {EE}, this is also possible by selecting the firmware of the full version and upgrading to it.  
...
Depending on the version differences between the current version and the one to be installed various messages will appear telling you what to do with your data during the update.

* Change of the first digit (major release) of the version number: You must back up the data of your device manually and restore it after the update. An update cannot be performed without data migration.
* Update to a higher number in the second digit (minor release): The update can be carried out without data migration. You are advised to back up your data beforehand anyway.
* Downgrade to lower number in the second digit: You must back up the data of your device manually and restore it after the update. An update cannot be performed without data migration.
* Change of the third digit (patch) of the version number: The update can be carried out without data migration. You are advised to back up your data beforehand anyway.

At this point, you can cancel the dialogue with [.guihint]#No# and first make a backup, if necessary.
...
During the restart, the selected firmware is installed. 
This will cause restarting to take much longer than usual. 
It will normally take less than 10 minutes however. 
Afterwards, another reboot is automatically carried out, which completes the firmware update.
The status view of the console shows the newly installed firmware version.
