...
you have logged in you can set up Checkmk in the usual manner. The snap-in
[.guihints]#Checkmk Appliance# is available in all monitoring instances and for all
...
to a Checkmk appliance. The Checkmk appliance offers a migration dialog which
...
[#manage_cmk]
== Administrating Checkmk versions

It is possible to install several Checkmk versions on the device at the same
...
To administrate the Checkmk versions select the [.guihints]#Checkmk versions# item in
the main menu of the web interface.

The dialogue box which then appears will list all installed Checkmk versions.
...
Using this dialogue box you can also upload new Checkmk versions onto the
device in order to use them in new instances or to update existing instances.

To do this, download the desired Checkmk version from our <a
href="https://checkmk.com/download.php">website</a> onto your computer
(search for _Checkmk appliance_ in the _Distribution_ column).
...
The Checkmk version will now be uploaded onto the device. Depending on the
...
=== Changing access to Checkmk agents

A Checkmk agent is installed on the device and in the basic setting can only
...
It is also possible to make the Checkmk agent accessible from another device,
meaning the device can also be monitored by another Checkmk system (e.g. in
a distributed environment by a central server). For this purpose, you can
configure a list of IP addresses that are allowed to contact the Checkmk agent.
...
Uncompressed backups are first supported from Checkmk-version 1.2.8p5. If you
...
From Checkmk version 1.4.0i1, for every configured backup job the [.guihints]#Service Discovery}}# 
...
In order to build a cluster you will first need two compatible Checkmk
...
The configuration of a cluster with two Checkmk rack1 is shown in the
...
about this dialogue, click on the icon beside the Checkmk logo in the top
...
interface of the Checkmk rack1) which you can adapt to the individual devices
at any time.
...
The monitoring instances and Checkmk versions are also synchronised between the
...
notifications sent over it by Checkmk (in the event of critical problems
for example).
...
In the case of a software fault – whether firmware or Checkmk monitoring
