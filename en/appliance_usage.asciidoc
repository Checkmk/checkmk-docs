...
After a major update of the firmware from version xref:majorfirmwareupdate[1.4 to 1.5] you may see a note about incompatibilities of the installed {CMK} versions. In this case you have to install a {CMK} version compatible with firmware version 1.5 - which may have the exact same version number, but is intended for the newer firmware. The background is a new operating system as a basis for the 1.5 firmware.
...
Once you have logged in you can set up {CMK} in the usual manner -- the first steps are described in the article on xref:intro_setup_monitor#[Setting up monitoring.]
...
*Caution:* When updating to new {CMK} major versions, you should follow the xref:update_major#[related article.]
It is important that you always update to the next major version only and do not skip any.
...
*Note:* This article is currently being adapted. If you want to perform a firmware update to platform version 1.5, refer to link:https://checkmk.com/werk/9484[Werk 9484^] in the change log.
...
Note about the *major update:* If you update the firmware from 1.4 to 1.5, you must also optionally update all installed {CMK} versions or reinstall them in the same version (but for a newer platform).
The reason: 1.5 is based on a new Debian version.
Under [.guihint]#Site Management# you can check if versions are incompatible. Incompatible versions can no longer be started.

[{image-border}]
image::cma_sites_incompatible_versions.png[Incompatible versions]
...
In contrast to the firmware update within compatible versions described below, e.g. 1.5.1 to 1.5.2, you have to proceed a bit different when updating from 1.4 versions to the current 1.5 versions.
The reason: 1.5 is based on a new Debian version.
In short, this means that you have to take the cluster completely offline for a short time - so you will get a downtime.
For regular updates, it is sufficient to put individual nodes of the cluster into maintenance mode to perform the update.
...
- Update the nodes one by one as described xref:cma_webconf_firmware[above.]
- Reconnect the nodes to the cluster via [.guihint]#Clustering > Reconnect Cluster.#
- Check if your instances are using compatible {CMK} versions and update as needed as described xref:update_site[above.]
