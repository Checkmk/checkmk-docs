...
[cma_virt1_basic_virtualbox|VirtualBox] or link:cma_virt1_basic_esx.html[VMWare ESXi].
...
the [.guihint]#device information}} box, and {{web interface: on}} in the {{access#
...
== Administrating monitoring instances#site_management
...
== Device settings#cma_webconf_system_settings
...
== Protecting the appliance-GUI via TLS#ssl
...
In the last section [.guihint]#Signing method}} you now select {{Create a self-signed certificate#.
...
[.guihint]#Shutdown device}} under the point {{Control device# in the main menu. The
...
and in the next menu open the [.guihint]#Backup target}}. Then create a {{New backup target#.
...
select the [.guihint]#Restore}} item on the {{Device backup# page, and on the next
...
The interface designations [.guihint]#LAN1}}, {{LAN2# etc., used in the diagram
correspond to the designations of the physical interfaces on the device. In
the operating system, [.guihint]#LAN1}} corresponds to the device {{eth0}}, {{LAN2#
...
in the device. Only the interface [.guihint]#eth0}} (corresponding to {{LAN1#) –
