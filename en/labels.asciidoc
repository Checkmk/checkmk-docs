...
// This block replaces the rest of the current section
// start translation
////
...
You can refine the search by adding more labels and, of course, combine the label search with the other available search parameters.

However, the label search goes one step further: On the one hand, you can search for hosts that meet the set filters, using the default [.guihint]#is,# or do not meet them, using [.guihint]#is not.#

On the other hand, the Boolean operators [.guihint]#NOT, AND, OR# are available here to define the exact subset of your hosts.
Important: The operators are processed in exactly this order -- NOT, AND, OR -- regardless of their position in the filter list.
This corresponds to the standard Boolean algebra; for example, `A OR B AND C NOT D OR E` would correspond to the following bracketing: `A OR (B AND (C NOT D)) OR E.`

image::labels_filter_boolean.png[alt="Filter bar with label filters linked by logical operators.",width=60%]

So the filters shown in the image above would filter out hosts that are a Linux system and are located in Munich and are not web servers.
