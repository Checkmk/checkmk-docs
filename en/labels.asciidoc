...
This corresponds to the standard Boolean algebra; for example, `A OR B AND C NOT D AND E` would correspond to the following bracketing: `A OR (B AND (C NOT D)) OR E.`

image::labels_filter_boolean.png[alt="Filter bar with label filters linked by logical operators.",width=60%]

So the filters shown in the image above would filter out hosts that are a Linux system and not named 'name1' as well as Windows systems that are not headless and not French.
