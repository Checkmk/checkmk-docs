...
From version {v16} {CMK} includes the new concept of [.guihint]#Labels#.
...
You can use labels as conditions in link:notifications.html[notifications], too. The
usage is the same as in other link:notifications.html#rule_conditions[conditions], so
you don't need re-educate yourself to use them. Just insert the label or the
labels that a host or service needs to have, to trigger a notification through
this rule. As it is in rules, labels will be combined through _AND_, too.
