...
and can also receive information about any labels/tags that are already present. These labels/tags can then
...
Labels may be present, host tags always are.
...
Individual {CMK} users can implement the specific requirements in their area of responsibility with labels.
For a manageable local structure, labels can fill in the gaps left vacant by the global administration, showing their advantages: They are quickly and easily created -- and also deleted again.
...
Labels can be assigned to a host in different ways. 
...
The suggestions do not distinguish between host and service labels: all matching labels are offered.
