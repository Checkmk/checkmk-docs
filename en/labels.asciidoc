...
:revdate: draft
:title: Labels
:description: In this article you will learn how to use familiar structuring markers from other environments in {CMK} with host and service labels.
...
container systems like xref:monitoring_aws#[Amazon Web Services (AWS)], xref:monitoring_azure#[Microsoft Azure] and
...
{CMK} can create such dynamic hosts automatically with the xref:dcd#[Dynamic Configuration Daemon],
...
* Labels are passed to xref:grafana#[Grafana] over a connector and can be used there for filtering, aggregation, etc.
...
Labels can be assigned to a host in three different ways. 
The first of these is simple: 
On the host properties page, which is displayed when you create or edit a xref:wato_hosts#labels[host in the Setup], you can arbitrarily
define as many labels as desired:

image::labels_host_properties.png[alt="Dialog with properties of a host to define labels."]
...
There is a [.guihint]#Host labels# rule set for this function, which you can quickly find via xref:user_interface#search_setup[search in the Setup menu].
...
the new labels by calling the service configuration of the host in the Setup
...
that it no longer reminds you for new labels. 
...
. In the last place are automatic generated labels.
...
Since labels can be defined without any defaults, and {CMK} cannot know exactly which labels really exist, it cannot recognize typing errors.
If that causes isolated problems it may be more effective if you work with host tags,
...
Select [.guihint]#Match host labels# and simply enter which labels a host or service must have, to trigger a notification through this rule. 
...
The labels are also visible in the monitoring environment, e.g. in the status view of a host:

image::labels_host_status.png[alt="Dialogue with the host status and the assigned labels."]

Here you can see the labels in the different colours depending on how they were created: 
Explicit labels in violet, labels created by rules in red and labels created by a discovery check in yellow-ochre.

The colour dots of the labels not only stand out visually in the view, they are also practical because they can be clicked on and then lead you to a search for all hosts with this label:
...
* You cannot formulate conditions via service labels.


[#labels_grafana]
== Labels in Grafana

You can use a link:https://github.com/tribe29/grafana-checkmk-datasource[Datasource] for Grafana, 
with which you can directly access the historical metrics of {CMK} directly from Grafana. 
