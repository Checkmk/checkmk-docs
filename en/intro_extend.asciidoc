...
Based on the availability data, the xref:sla#[SLA] software module included in the {CEE} enables a much more detailed evaluation for aid in complying with service level agreements -- and such agreements can even be actively monitored.
...
PDF reports can only be created in the {CEE}.


[#agent_updater]
== Automated updating of the agents

If you monitor many Linux and Windows servers, you can use the xref:agent_deployment#[Agent Updater] included in the {CEE} to have a centralized method for keeping your monitoring agents and their configurations up to date.


[#devel]
== Developing your own plug-ins

Even though {CMK} offers nearly 2000 check plug-ins, it can still sometimes be that a specific plug-in is missing.
For an introduction to developing your own plug-ins, see the xref:devel_check_plugins#[article on writing check plug-ins].
