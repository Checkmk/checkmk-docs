...
:description: The Bakery API provides an interface to extend agent packages of the Agent Bakery, e.g. with agent plugins and their configuration. 
...
Please be patient. +
By the way, https://docs.checkmk.com/latest/de/bakery_api.html[this article in German] is already updated -- if that is an alternative for you.
...
{cee-only} In den {CEE} können Sie die Bakery-API nutzen, um eigene, sogenannte _Bakery Plugins_ zu schreiben, die Funktionen in die Agentenpakete der xref:wato_monitoringagents#bakery[Agentenbäckerei] (_Agent Bakery_) einbinden. 
...
Sie haben die Einführung in die xref:devel_check_plugins#[Check-Plugin-Programmierung] mit der Check-API gelesen und davon inspiriert Ihr eigenes agentenbasiertes Check-Plugin mit zugehörigem Agentenplugin geschrieben.
...
Auch dafür finden Sie in der {CMK} Exchange ein ausführlich kommentiertes Beispiel: das link:https://exchange.checkmk.com/p/hello-bakery[Hello bakery!^] MKP, welches eng an das in diesem Artikel vorgestellte Beispielszenario angelehnt ist.
...
* Für den {CMK}-Agenten wird ein Plugin bereitgestellt mit dem Namen `hello_bakery`.
...
Die zugehörigen Dateien liegen vor und heißen `hello_bakery` (für Linux), `hello_bakery.solaris.ksh` (für Solaris) und `hello_bakery.cmd` (für Windows). +
...
Mehr dazu erfahren Sie in der Einführung zum xref:devel_check_plugins#[Schreiben eigener Check-Plugins.]
...
Das Linux-Plugin liest die Konfiguration aus der Konfigurationsdatei `hello_bakery.json` und das Solaris-Plugin aus der Datei `hello_bakery.cfg`.
Das Windows-Plugin liest die Einträge `hello_bakery.user` und `hello_bakery.content` aus der YAML-Konfigurationsdatei des Windows-Agenten ein. +
...
* Unter Linux und Solaris soll nach der Installation des Agenten per Paketmanager-Routine in das Syslog geschrieben werden, dass `hello_bakery` installiert wurde. 
Nach der Deinstallation des Agenten soll analog in Syslog geschrieben werden, dass `hello_bakery` deinstalliert wurde. +
...
In der `get_hello_bakery_windows_config` Funktion greifen wir über das `conf` Argument auf die Konfiguration zu, die über den xref:ruleset[Regelsatz] in der Setup-GUI festgelegt wird: 
das Zeitintervall für die erneute Ausführung bei zwischengespeicherter Ausgabe (`interval`) und die beiden Variablen, mit denen das Plugin konfiguriert werden kann (`user`, `content`).
Dabei gehen wir davon aus, dass die Konfiguration des Regelsatzes als ein `dict` geliefert wird. Über das `TypedDict` der Klasse `HelloBakeryConfig` können wir uns einen typisierten Zugriff darauf einrichten.
...
In der Funktion `get_hello_bakery_plugin_files` wird zuerst die Python-Datei `hello_bakery` als `Plugin` definiert, d.h. als ausführbare Datei, die vom {CMK}-Agenten als Agentenplugin ausgeführt werden soll. 
Dann wird mit `PluginConfig` die zu generierende Konfigurationsdatei `hello_bakery.json` für das Linux-Agentenplugin mit den Einträgen `user` und `content` festgelegt.
...
Im allgemeinen Fall kann die Konfiguration innerhalb der Funktionen `files_function`, `scriptlets_function` und `windows_config_function` beliebig ausgewertet werden und auf die zurückgegebenen xref:artefacts[Artefakte] Einfluss nehmen, um z.B. den Benutzernamen in die Konfiguration zu schreiben.
...
Sie finden zu diesem Thema eine Einführung im Artikel zum xref:devel_check_plugins#parameters[Schreiben eigener Check-Plugins].
...
Die aus diesem Regelsatz resulierende GUI zeigt das folgende Bild:
...
|===
|Pfad |Bedeutung
|`local/lib/check_mk/base/cee/plugins/bakery/` |Verzeichnis für das Bakery-Plugin (im Beispiel `hello_bakery.py`).
...
|`local/share/check_mk/agents/` |Verzeichnis für mitgelieferte Programme oder Shell-Skripte für unixoide Betriebssysteme (im Beispiel `some_binary`).
