...
|Excluded services / hosts |Services or hosts that are not included in the license size. These are e.g. services / hosts that are only used for internal testing scenarios within your organization. To mark services or hosts as excluded, set labels of type `licensing:excluded` according to the description in xref:labels#create_labels[Creating labels].
