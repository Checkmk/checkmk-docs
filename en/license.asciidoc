...
// This block replaces the complete old article
// start translation
////
==  Einleitung

{cee-only} Nutzen Sie die {CSE}, die {CME} oder die {CCE} (oberhalb der Grenzen des Lizenzstatus „Free“, d.h. mit mehr als 750 Services oder mehr als einer Instanz), so sind Sie verpflichtet, regelmäßig eine Übersicht über den Einsatz von {CMK} in Ihrem Unternehmen an die {comfull} zu liefern.
...
{cee-only} If you use one of the {CEE}, you perform a license audit at least once a year, i.e. you create an overview of the use of {CMK} within your company and pass it on to {com}.
...
As an administrator of {CMK}, you then have the license usage information conveniently transmitted directly from {CMK} to the {com} customer portal.
