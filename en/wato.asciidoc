...
xref:wato_monitoringagents#bakery[Agent Bakery] (only in the {EE}), which can be used to configure, package and automatically update agents,
...
xref:alert_handlers#[alert handler] (only in the {EE}).
...
xref:mkps#[{CMK} extension packages (MKPs)] are only available in the {EE}.

|[.guihint]#Exporter# |The export of data, i.e. metrics to 
xref:metrics_exporter#[InfluxDB (and Graphite)].
This topic is only available in the {EE}.
|===
