...
* detecting state changes and generating xref:notifications#[notifications] from these.
...
This offers numerous useful functions and has been proven over many years by millions of users worldwide.
This inherent flexibility is one of the reasons for the success of Nagios.

Alternatively, the core from _Icinga_ can also be utilised. 
...
{cee-only}
Since {CMK} is being used in ever-larger environments, and in order to overcome the limitations of Nagios as described above, in 2013 we commenced a new development of our own core specifically for the {CEE}.
The _{CMK} micro core_ -- or _CMC_ -- has not simply been created as a fork from Nagios, rather it has a complete code basis of its own.
...
This applies for xref:active_checks#[active checks] as well as {CMK}-based checks. 
...
* *Additional features* -
The CMC makes use of numerous additional features, such as, e.g., recurring xref:basics_downtimes#[scheduled downtimes] and xref:basics_ackn#[acknowledgements] with automatic expiry times.

Other elements have also been optimised. 
...
* The low CPU consumption enables virtualisation to substitute for hardware in many cases.
* The shock-free activation of changes allows frequent configuration changes.
* Demands such as Cloud monitoring can thus be satisfied, where servers are added and removed in quick succession.
...
The two graphics below show the CPU load and utilization for a {CMK} server before and after changing from Nagios to the CMC. 
...
In the above case a smaller site that was not restructured runs on the same server.
...
Likewise the {CEE} will continue to have Nagios as an optional core -- but only to support a migration from the {RE} to the {EE}.


=== Can I switch between Nagios and the CMC?

Switching between the two cores is easy as long as your configuration was created exclusively with the xref:wato#setup_menu[Setup menu] of the {CMK} Web interface. 
Details on this can be found in the xref:cmc_migration#[Migration to the CMC] article.
By default the {EE} create new sites with the CMC as the core.


=== Is the CMC freely-available?

The CMC is included as a component in the link:https://checkmk.com/product/editions[{CEE}].
The {FE} is cost-free.
The {SE} and the {ME} are available via subscriptions.
