...
{cee-only}
Since {CMK} is being used in ever-larger environments, and in order to overcome the limitations of Nagios as described above, in 2013 we commenced a new development of our own core specifically for the commercial editions.
...
Likewise the commercial editions will continue to have Nagios as an optional core -- but only to support a migration from the {RE} to the commercial editions.
...
The {CE} can be tested without obligation for a limited period of time and operated permanently on a small scale without licensing.
