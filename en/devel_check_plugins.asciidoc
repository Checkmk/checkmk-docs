...
plug-ins in version VERSION[2.0] of {CMK}.


=== What has changed compared to the old API?

Do you already have experience with developing check plug-ins for {CMK} Version
VERSION[1.6.0] or earlier? If so, here is a concise overview of all of the
changes introduced in the new Check API available from Version VERSION[2.0]:
...
VERSION[1.6.0] of {CMK} will be supported with some minor restrictions for a few
...
But we can also easily force this here with the `-1` option:
...
can get rid of them quite easily with a `grep`. There we take the `-v`
option for negative logic. Here you can now see much more clearly that in our
...
Here we have again omitted the `-1`, because `ls` now writes into a pipe and outputs a single column by itself. And `grep` needs the
`--` because otherwise it would interpret the word `-part` as the
four options `-p`, `-a`, `-r` and `-t`.

And by the way: Why don't we simply use `usb` so that only USB
devices are transferred? Well, of course we could do that. But for one thing,
...
monitored. We could of course simply edit the `/bin/check_mk_agent` file
...
`/usr/lib/check_mk_agent/plugins/usbstick` and made it executable!
...
`.py`, since from {CMK} version VERSION[2.0] onwards the plug-ins
...
to its status {OK}, {WARN}, and so on. Up until version VERSION[1.6.0] this was
called [.guihint]#Output of check plugin#. As of version VERSION[2.0] this is now called
...
API -- the one up to version VERSION[1.6.0] of {CMK}?
