...
// H1:Threshold values and check parameters#parameters
//
//
// H2:Eine Regelkette für das Setup
//
...
// Zahl 10 direkt in der Checkfunktion fest einprogrammiert, oder hart codiert,
...
//
// NL:Es gibt bereits eine passende Regelkette. Das kann eigentlich nur dann der Fall sein, wenn Ihr neuer Check etwas prüft, für das {CMK} in gleicher Form bereits Checkplugins hat, z.B. das Überwachen einer Temperatur. Dafür gibt es bereits eine Regelkette, die Sie direkt verwenden können.
// NL:Es gibt keine passende Regelkette. Dann müssen Sie eine neue anlegen.
//
// H2:Verwenden von vorhandenen Regelketten
//
...
//
// F+:lib/check_mk/gui/plugins/wato/check_parameters/memory_simple.py
...
//
//
// H2:Eine eigene Regelkette definieren
//
...
// Sie den Unterstrich. Dieser ist eine Markierung für alle übersetzbaren
// Texte. Im weiteren schreiben Sie dann z.B. anstelle
// von `"Threshold for warn"` ein `_("Threshold for warn")`.
// Das Übersetzungssystem von {CMK}, welches auf Gettext basiert, findet
...
// auch verzeichten und brauchen den folgenden Importbefehl nicht:
//
// F+:local/share/check_mk/web/plugins/wato/foobar_parameters.py
...
// in Pythondatenstrukturen. In folgendem Beispiele werden `Dictionary`,
...
// Python-Dictionaries sein müssen. Früher konnte es z.B. auch ein Tupel (Paar)
...
// Damit keine globale Variable erzeugt wird, sollte er mit einem
...
// LI:Falls Ihr Check kein Item verwendet, lautet die innere Funktion `CheckParameterRulespecWithoutItem`. Die Zeile `item_spec` entfällt dann.
// LI:Wie oben erwähnt stellt der `check_group_name` die Verbindung zu den Checks her, welche diese Regel verwenden sollen. Er darf auf keinen Fall identisch sein mit einer bereits existierenden Regel, weil diese damit überschrieben würde.
// LI:Die `group` legt fest, in welcher Kategorie im Setup die Regelkette auftauchen soll. Die meisten dieser Gruppen sind in der Datei `lib/check_mk/gui/plugins/wato/utils/__init__.py` definiert. Dort finden Sie auch Beispiele, wie Sie eine eigene neue Gruppe anlegen können.
// LI:Der `match_type` ist immer `"dict"`. In älteren {CMK}-Versionen gab es auch Parameterregeln mit anderen Typen.
// LI:`title` legt den Titel der Regelkette fest, wird aber nicht direkt als Text, sondern als ausführbare Funktion angegeben, welche den Text zurückliefert (deswegen das `lambda:`).
//
//
// H3:Test
//
// Wenn Sie diese Datei angelegt haben, sollten Sie erstmal ausprobieren, ob alles soweit
// funktioniert und nicht gleich mit der Checkfunktion weiterarbeiten. Dazu brauchen
// Sie erstmal einen Neustart vom Apache der Instanz, damit die neue Datei gelesen wird.
// Das macht der Befehl:
//
// C+:
// OM:omd restart apache
// C-:
//
...
// verwenden.
//
// H2:Die Regel im Checkplugin benutzen
//
// Damit die Regel zum Greifen kommt, müssen wir dem Checkplugin erlauben,
// Checkparameter entgegen zu nehmen und ihm sagen, welche Regel benutzt
...
// F-:
//
// Beachten Sie, das eine solche Änderung des Datentyps eine inkompatible
...
//
//
// H1:Angepasste Darstellung von Metriken
//
// H2:Der Sinn von Metrikdefinitionen
//
// In unserem obigen Beispiel haben wir das Plugin `foobar` die Metrik `fooslots`
// erzeugen lassen. Solche Metriken werden in der grafischen Oberfläche von {CMK} sofort
// verfügbar, ohne dass Sie etwas dafür tun müssten. Pro Metrik wird bei den
// Servicedetails automatisch ein Graph erzeugt.
//
// Allerdings gibt es dabei ein paar Einschränkungen:
//
// LI:Es erscheint nicht automatisch ein „Perf-O-Meter“, also die grafische balkenartige Vorschau des Messwerts, wenn der Service in der Listendarstellung angezeigt wird (z.B. in der Ansicht, die alle Services eines Hosts darstellt).
// LI:Es werden nicht automatisch passende Metriken in einem Graphen kombiniert, sondern jede erscheint einzeln.
// LI:Die Metrik hat keinen richtigen Titel, sondern es wird der interne Variablenname der Metrik gezeigt.
// LI:Es wird keine Einheit verwendet, die eine sinnvolle Darstellung erlaubt (z.B. GB anstelle von einzelnen Bytes)
// LI:Es wir zufällig eine Farbe ausgewählt.
//
...
//
// H2:Vorhandene Metrikdefinitionen verwenden
//
...
//
// F+:local/share/check_mk/web/plugins/metrics/foobar_metric.py
// from cmk.gui.i18n import _
...
// LI:Der Schlüssel (hier `"fooslots"`) ist der Metrikname und muss dem entsprechen, was die Checkfunktion ausgibt.
// LI:Das Importieren und verwenden des Unterstrichs für die Internationalisierung ist optional, wie bereits bei den Regeln besprochen.
// LI:Welche Unit-Definitionen es gibt, erfahren Sie in der Datei `lib/check_mk/gui/plugins/metrics/unit.py`.
// LI:Die Farbdefinition verwendet eine Palette. Zu jeder Palettenfarbe gibt es `/a` und `/b`. Dies sind zwei Schattierungen der gleichen Farbe. In den vorhandenen Definitionen werden Sie auch viele direkte Farbkodierungen wie `"#ff8800"` finden. Diese werden nach und nach abgeschafft und alle durch Palettenfarben ersetzt werden, da diese ein einheitlicheres Aussehen bieten und auch leichter an die Themes der Oberfläche angepasst werden können.
//
// Diese Definition sorgt jetzt dafür, dass Farbe, Titel und Einheit der Metrik
// nach unsere Wünschen angezeigt werden.
//
// H2:Graphen mit mehreren Metriken
//
...
// Hinweise dazu:
//
// LI:Leider gibt es im Handbuch noch keine Beschreibung der Möglichkeiten dieser Definition. Aber Sie finden sehr viele Beispiele in den Dateien im Verzeichnis `lib/check_mk/gui/plugins/metrics`.
// LI:Probieren Sie anstelle von `line` auch mal `area` oder `stack`.
//
//
// H2:Darstellung der Metriken im Perf-O-Meter
//
//
...
//
//
// F+:local/share/check_mk/web/plugins/perfometer/foobar_perfometer.py
...
//
//
// H2:Die Parsefunktion schreiben
//
...
// </td>
// <td>Ablageort für selbst geschriebene Checkplugins</td>
// </tr>
//
// <tr><td class=tt>
// local/share/check_mk/web/plugins/wato
// </td>
// <td>Ablageort für Ihre Regelketten für Checkparameter</td>
// </tr>
//
// <tr><td class=tt>
// local/share/check_mk/web/plugins/metrics
// </td>
// <td>Ablageort für eigene Metrikdefinitionen</td>
// </tr>
//
// <tr><td class=tt>
// local/share/check_mk/web/plugins/perfometer
// </td>
// <td>Ablageort für eigene Definitionen von Perf-O-Metern</td>
// </tr>
// <tr><td class=tt>
// local/share/check_mk/mibs
// </td>
// <td>Legen Sie hier SNMP-MIB-Dateien ab, die automatisch geladen werden sollen.</td>
// </tr>
//
// <tr><td class=tt>
// lib/check_mk/gui/plugins/wato/check_parameters
// </td>
// <td>Hier finden Sie die Regelkettendefinitionen von allen mitgelieferten
// Checkplugins von {CMK}</td>
// </tr>
//
// <tr><td class=tt>
// lib/check_mk/gui/plugins/wato/utils/__init__.py
// </td>
// <td>In dieser Datei sind die Gruppen der Setupoberfläche definiert,
// in welchen Sie neue Regelketten ablegen können.</td>
// </tr>
//
// <tr><td class=tt>
// lib/check_mk/gui/plugins/metrics/
// </td>
// <td>Hier finden Sie die Metrikdefinitionen der mitgelieferten Plugins</td>
// </tr>
//
// <tr><td class=tt>
// lib/check_mk/gui/plugins/metrics/unit.py
// </td>
// <td>In dieser Datei sind die vordefinierten Einheiten für Metriken.</td>
// </tr>
//
// <tr><td class=tt>
// /usr/lib/check_mk_agent/plugins
// </td>
// <td>Dieses Verzeichnis bezieht sich auf einen überwachten Linux-Host. Hier
// erwartet der {CMK}-Agent für Linux Erweiterungen des Agenten (Agent-Plugins).</td>
// </tr>
//
