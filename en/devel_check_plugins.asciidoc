...
|Run the plug-in via _MRPE_ from the [agent_windows#mrpe|Windows] or link:agent_linux.html#mrpe[Linux] agent. 
...
|log messages
|Monitor _messages_ with the link:ec.html[Event Console]
...
|SNMP
|When monitoring via link:snmp.html[SNMP] you do not need an extension of an agent, but evaluate the data that your device retrieves data from your device via SNMP, which provides this by default. {CMK} supports you and takes over all details and special features of the SNMP protocol.
