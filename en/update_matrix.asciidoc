...
If you want to update to a version of {CMK} older than {v22}, please read this article for the {CMK} version that is to be the _target_ of the update.
...
The minimum requirements are described in the xref:update_major#update_patch[article on updating to {v22}].
...
For an update from Debian 9 with {CMK} {v21}p1 to Debian 12 with {CMK} {v22}, this will result in the following update sequence:

. Under Debian 9, update {CMK} {v21}p1 xref:update#[to the current patch version] of {v21}.
...
. On Debian 12, update the current patch version of {CMK} {v21} to the xref:update_major#[current patch version of {v22}].
...
Ubuntu STS versions will in future no longer be supported.
Since {CMK} {v21} for Ubuntu 22.10 had already been released when this _OS Support Policy_ was published, in order to facilitate updates we have decided to slowly phase out support for Ubuntu's STS versions.
This means that Ubuntu 23.10 will be the last STS version supported by {CMK}.
Affected users can then seamlessly switch to an LTS version with the update to 24.04.
