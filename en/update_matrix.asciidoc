...
= Update matrix for version {v16}
:revdate: 2023-11-07
:title: Update matrix for version {v16}
...
// xref:update_20#[Update to version {v16}]
...
This may make it necessary to perform one or more upgrades of your Linux distribution before performing an update of {CMK}.
...
If you want to update to a version of {CMK} newer than {v16}, please read this article for the {CMK} version that is to be the _target_ of the update.
...
The minimum requirements are described in the xref:update#[article on updating].
...
For an update from Debian 7 with {CMK} {v15}p1 to Debian 10 with {CMK} {v16}, this will result in the following update sequence:

. Under Debian 7, update {CMK} {v15}p1 xref:update#[to the current patch version] of {v15}.

. Now upgrade from Debian 7 to 8, 9 and finally to 10.

. On Debian 10, update the current patch version of {CMK} {v15} to the current patch version of {v16}.
...
Since both {CMK} {v15} and {CMK} {v16} reached their end of support, the tables on this page will receive no further updates.
