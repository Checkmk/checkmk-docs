...
= Update matrix for version {v21}
:revdate: draft
:title: Update matrix for version {v21}
...
xref:update_major#[Update to version {v21}]
...
Sollten Sie auf eine ältere {CMK}-Version als {v21} aktualisieren wollen, lesen Sie diesen Artikel bitte für die {CMK}-Version, die _Ziel_ der Aktualisierung ist.
...
Die Mindestanforderungen sind im xref:update_major#update_patch[Artikel zum Update auf {v21}] beschrieben.
...
Für ein Update von Debian 9 mit {CMK} {v21}p1 auf Debian 12 mit {CMK} {v21} ergibt sich so die folgende Update-Reihenfolge:
...
. Unter Debian 12 aktualisieren Sie die aktuelle Patch-Version von {CMK} {v21} auf die xref:update_major#[aktuelle Patch-Version von {v21}].
...
Ab {v21} schafft die xref:ossupport[OS Support Policy] klare Regeln.
