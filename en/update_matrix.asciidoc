...
If you want to update to a version of {CMK} older or newer than {v21}, please read this article for the {CMK} version that is to be the _target_ of the update.
...
The minimum requirements are described in the xref:update_major#update_patch[article on updating to {v21}].
...
For an update from Debian 9 with {CMK} {v20}p1 to Debian 12 with {CMK} {v21}, this will result in the following update sequence:

. Under Debian 9, update {CMK} {v20}p1 xref:update#[to the current patch version] of {v20}.

. Still on Debian 9, update the current patch version of {CMK} {v20} to the xref:update_major#[current patch version of {v21}].

. Now upgrade from Debian 9 to 10, 11 and finally to 12 as described in the xref:release_upgrade#[Linux upgrade on the {CMK} server] article.
