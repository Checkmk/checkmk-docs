...
In the following short article we would like to introduce the {CCE}, one of the commercial xref:intro_setup#editions[editions].
...
In addition, the {CE} also provides cloud-specific xref:glossar#check_plugin[check plug-ins].
In this way you can continue to use the {SE} when transitioning from a data center to a cloud.
...
* The {CE} is available through the xref:install_azure#[Microsoft Azure] and xref:install_aws#[AWS] marketplaces.

* Advanced check plug-ins: +
The {CE} includes additional cloud-specific check plug-ins for monitoring xref:monitoring_aws#[Amazon Web Services (AWS)], xref:monitoring_azure#[Microsoft Azure] and the xref:monitoring_gcp#[Google Cloud Platform (GCP)].

* Cloud-specific dashboards: +
In the {CE}, there are specific dashboards for AWS, Microsoft Azure and GCP.

* Monitoring OpenShift: +
The xref:monitoring_openshift#[monitoring of OpenShift] with the special agent for Kubernetes is exclusive to the {CE}.
...
*Note:* In the {CMK} user interface, elements that only exist in the {CE} are indicated by the [.guihint]#(Cloud Edition)# suffix, e.g. rules, dashboards or parameter values.
...
|OpenShift dashboards |The six built-in xref:monitoring_openshift#dashboards[dashboards for Kubernetes] can be used in the {CE} to monitor OpenShift clusters.
|===
...
|Push mode |Only in the {CE} the xref:wato_monitoringagents#agents[{CMK} agent] for Linux and for Windows can be configured and used in xref:glossar#push_mode[push mode].
...
|Auto-registration |The {CE} provides the ability to create hosts automatically during registration with the so-called xref:hosts_autoregister#[auto-registration].
...
|Agent Bakery |In the {CE} you can additionally use the Agent Bakery to provide agent packages with a configuration for auto-registration.
...
|Agent registration permissions | In the {CE} the xref:wato_user#predefined_roles[predefined role] `agent_registration` contains additional permissions for creating hosts automatically.
|===
...
|Availability in the Azure and AWS Marketplaces |The {CE} is available for installation in the marketplaces of link:https://azuremarketplace.microsoft.com/en-us/marketplace/apps/tribe29gmbh1665582614827.checkmk003?tab=Overview[Microsoft Azure^] and link:https://aws.amazon.com/marketplace/pp/prodview-gddkal2hfn7yo[AWS^].
The xref:install_azure#[Azure] and xref:install_aws#[AWS] articles will guide you through the image setup.

|Testing the Edition |With the available Enterprise xref:intro_setup#editions[Editions], i.e. {SE}, {CE} and {ME}, you can test the {CE} in full functionality for 30 days without obligation.
On a small scale, i.e. with one site and up to 750 services, you can also run {CMK} with the {CE} on a permanent basis without requiring a subscription.
|===
...
|Check plug-ins for AWS |In the {CE} there are additional check plug-ins for the AWS Cloud products
...
|Check plug-ins for Azure |In the {CE} there are additional check plug-ins for the Azure products
...
|Check plug-ins for GCP |In the {CE} there are additional check plug-ins for the Google Cloud products
