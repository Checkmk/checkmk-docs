...
. During xref:appliance_usage#init_start[initialisation at first start], you will be prompted at the console to partition the data volume. Confirm.
...
Then select [.guihint]#{CMK} versions# in the web interface of the appliance. In one step, upload the software package you have just downloaded to the appliance and install it -- as described in detail in xref:appliance_usage#manage_cmk[administrating {CMK} software versions].
