// -*- coding: utf-8 -*-
// IGNORE Corosync DRBD LDAPS SNMPTRAP VLAN
...
|8000 |TCP |Agent Controller TLS registration (all editions), agents in xref:glossar#push_mode[push mode] ({CE}) |If multiple sites are running on the {CMK} server, additional ports (8001, 8002...) may be needed.
|===
