...
Diesr Artikel soll Ihnen einen Überblick geben, welche Ports für welche Funktionalität benötigt werden.
...
|8000 |TCP |Agent Controller TLS-Registrierung |Wenn mehrere Instanzen auf dem {CMK}-Server laufen, sind eventuell weitere Ports (8001, 8002…) nötig
|===
...
|6555 |TCP |Benachrichtigungs-Spooler (_notification spooler_) |Der xref:distributed_monitoring#notifications[Benachrichtigungs-Spooler] dient dem zentralen Versand von Benachrichtigungen, hier beim Verbindungsaufbau durch die Remote-Instanz _(optional aktivierbar)_
|===
...
// These two sentences are copied from appliance_usage.asciidoc – when translating you might copy and paste from there as well
