...
|8000 |TCP |Agent Controller TLS registration |If multiple sites are running on the {CMK} server, additional ports (8001, 8002...) may be needed.
|===
...
You can combine two {CMK} appliances ('nodes') into a xref:appliance_usage#_failover_cluster[failover cluster].
...
|7789 |TCP |DRBD |Synchronization of DRDB (Distributed Replicated Block Device)
|===
...
You may need some additional outgoing ports from the {CMK} server:
