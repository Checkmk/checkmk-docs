...
|{OK} |green |The check is completely ok. All checked values are considered optimal.
|{WARN} |yellow |The check is basically ok, but {CMK} has identified potential for improvement.
...
.7+|[.guihint]#Performance# |[.guihint]#Check helper usagefootnote:Cee_only[This check is only present in the {CEE}.]# .5+.^|xref:cmc_differences#aux_processes[{CMK} auxiliary processes]
