...
- the control of cronjobs—when xref:monitoring_jobs#[the `mk-job` plugin] is not sufficient—,
...
// MFS: Begin translation
// MFS: This new subchapter is necessary since several users ran into the troubles described mainly using Windows scripting languages.
// MFS: kodiert = encoded, Kodierungen = encodings
////
[#charset]
=== Zeichensatz
...
* Sorgen Sie insbesondere auf _älteren_ Windows- und Linux-Systemen dafür, dass Spool-Dateien _nicht_ in den Kodierungen link:https://de.wikipedia.org/wiki/Windows-1250[Windows 1250 bis 1258^] oder link:https://de.wikipedia.org/wiki/ISO_8859[ISO 8859^] geschrieben werden.
...
* Verzichten Sie auf das link:https://de.wikipedia.org/wiki/Byte_Order_Mark[_BOM_^].
...
Auch hier nutzt Windows häufig die Zeichenfolge `CRLF` (`0x0D 0A` oder `\r\n`).
...
* `0xFF FE` zeigt eine UTF-16 kodierte Datei an: Ändern Sie in diesem Fall Ihre Skripte auf Ausgabe in UTF-8.
* `0xEF BB BF` zeigt UTF-8 mit BOM an.
