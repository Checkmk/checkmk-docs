...
:description: Each Linux distribution (RHEL-based, SLES, Debian and Ubuntu) has its own special features which need to be considered when installing {CMK}.
...
Alongside this there is the pleasant side benefit of the availability of completely-free distributions of Linux with Debian, Ubuntu and CentOS, which are enterprise-capable and supported by {CMK}.

If you do not want to deal with Linux under any circumstances, it is also possible to run {CMK} as xref:appliance_install_virt1#[a virtual appliance], link:https://checkmk.com/product/appliances[a physical appliance^], or as
...
We support the following Linux distributions during the full product lifecycle of {CMK} {v22}:

* Red Hat Enterprise Linux (RHEL) and binary compatible derivatives (CentOS, AlmaLinux, Rocky Linux, Oracle Linux...) from Version 7.x
* SUSE Linux Enterprise Server (SLES) Version 12 from SP4 or Version 15 from SP1
* Debian from Version 10.0
* Ubuntu Version 16.04, 18.04, 20.04 and 22.04
...
As a far-sighted approach we thus recommend choosing a recent LTS (long term support) distribution at least upon fresh install of a {CMK} server.
...
For this reason we will assume that the Linux system on which the {CMK} is to be installed has been installed with the developer’s default settings, and correctly-connected to the network, so that it is accessible via HTTP and SSH from your workplace.
