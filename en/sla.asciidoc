...
{cee-only} If you're using one of the {EE} {CMK} has a separate function for Service Level Agreements (SLA).
...
* Outage SLA: Maximum number of outages – more precisely, occurrences of the {CRIT}, {WARN}, and {UNKNOWN} statuses over a given time frame.
...
But it can also be adjusted so that it goes to {CRIT} if, for example, the permitted {CRIT} status for the service has reached 80 percent – and before that point it could change to a {WARN} flag.
...
For example: "Over each monthly monitoring period, the status of the service _MyService_ should be at least 90 percent {OK}."
...
|[.guihint]#Service Status Grouping# |Reclassification of a status.
|[.guihint]#Only show objects with outages# |Show only objects with given failure rates.
|[.guihint]#Host Status Grouping# |Interpret the {UNREACH} host status as {UNREACH}, {UP} or {DOWN}.
...
According to the SLA, per SLA period the service is now allowed the specified status on a maximum of five occasions, and each occurrence for a maximum of two minutes, without the SLA being flagged as broken.
...
[.guihint]#General information#: Here you can see the raw availability data, and thus the SLA calculations as an overview of the status for each period, and below it the aggregated results of the SLA’s requirements.
...
Once you get that, move the mouse pointer to the result bar where, via a tooltip, you will be able to see the individual events that are responsible for the status.
In the following picture the status is {WARN} – because only two out of the three permitted outages remain:
...
=== An SLA status change is not shown

In its simplest form, an SLA status only changes in the GUI views when its requirements are broken. 
