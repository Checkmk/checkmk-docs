...
In {CMK} you can evaluate link:availability.html[availability], and
for these also configure a rudimentary link:availability.html#thresholds[SLA evaluation].
...
Under [.guihint]#SLA-Settings}} set the {{SLA period# to the desired period,
...
|`Scheduled Downtimes`
|Consideration of planned link:basics_downtimes.html[Scheduled Downtimes.]
...
|`Service Time`
|Consideration of link:availability.html#serviceperiod[Serviceperiod.]
...
|Ignore intervals shorter than a given duration, so that brief interruptions are ignored (similar to the concept of link:availability.html#softstates[Soft states]).
...
they can also be used in the SLAs like link:availability.html#serviceperiod[above] as already mentioned under general availability. Select a desired Period under [.guihint]#Active in timeperiod}}, or as here in the example {{Always# to define the requirements for a 24/7 operation.
...
record to ‚{OK}, [.guihint]#Minimum}}, {{90#‘. If this value falls short,
...
the [.guihint]#Computation Type}} to {{Maximum number of service outages#.
...
Next under [.guihint]#Conditions}} in the {{Services# section set
...
[.guihint]#All services}} as {{Datasource#. For the following query,
...
[.guihint]#Services: Service state}}, {{Hosts: hostname#
...
[.guihint]#Hosts/services: SLA - service specific}} and {{hosts/services: SLA - column specific#.
...
[.guihint]#monthly}} in your SLA definition and here {{Last Year#, you receive
...
For five periods/results set [.guihint]#Starting from period number}} to {{0#,
and [.guihint]#Looking back}} to {{4#.

[.guihint]#Layout options}}: By default, this option is set to {{Only Display SLA Name#.
...
* [.guihint]#Display a summary for each SLA period}} shows a graphic summary under the {{Aggregated result# label.
* [.guihint]#Display a summary over all SLA periods}}: Shows a textual, percentage summary of all SLAs under the {{Summary# label.
...
in the last step – still in [.guihint]#Context/Search Filters}} – under {{host# set a filter
...
== Further views#moreslaviews

=== Column-specific SLA displays#slacolumn
...
This works in the [.guihint]#Result}} and {{Aggregated results# lines.
...
* Service times: [.guihint]#WATO|Monitoring Configuration|Service Period for hosts}} and {{... for services# respectively
