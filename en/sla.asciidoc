...
For the following query, whether to show information from a single object, just confirm without making a selection.

Under [.guihint]#General Properties#, enter an ID (here `MySLAView_Demo`), a title (like `My SLA Demo View`), and finally select a theme like [.guihint]#Workplace# if you want to see all SLA views later under a separate item in the xref:user_interface#monitor_menu[Monitor menu].
All other values can be left unchanged while testing.
...
The latter shows _a fixed_ SLA definition for each service in the view - the better alternative to an SLA for all services as mentioned above.
More on this xref:slacolumn[later].
...
In this example the [.guihint]#SLA period range# option is used to count the number of times displayed reporting periods can be set directly:
For five periods/results set [.guihint]#Starting from period number# to `0`, and [.guihint]#Looking back# to `4`.

[.guihint]#SLA type#: At this point define for the display of _Outage/Percentage SLAs_ whether summaries (texts) or respectively individual results (icons) of the reporting periods should appear.
...
As a result you will get a view with five status icons as single results from the Percentage SLA, and a summary in the form of [.guihint]#100 percent# for the Outage SLA.
...
the status of a xref:bi#[BI aggregation] can be monitored through the [.guihint]#Check State of BI Aggregation# rule set as a fully normal service.
...
Therefore you need to connect the URL to the server and specify an xref:wato_user#automation[ automation user].
...
by applying xref:commands#[commands] to the objects in a view.
...
In such cases, you can change the display via xref:slacolumn[column-specific].
