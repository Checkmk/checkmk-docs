...
In {CMK} you can evaluate xref:availability[availability], and for these also
configure a rudimentary xref:availability#thresholds[SLA evaluation].  Now the
...
{cee-only} But {CMK} has a separate function for Service Level Agreements if
you're using one of the {EE}. The SLA feature builds on the availability data
and now allows a much more detailed evaluation.  Two different requirements
can be implemented:
...
data can be converted to the xref:timeperiods[Time periods] defined in WATO,
...
* Time period: Active xref:timeperiods[Time periods] from the WATO configuration (business days, more or less)

For each SLA period an independent result is generated. How many of these individual results you see in a table can be configured via xref:views[views].
...
|Scheduled Downtimes |Consideration of planned xref:basics_downtimes[Scheduled Downtimes.]
...
|Service Time |Consideration of xref:availability#serviceperiod[Serviceperiod.]
|Notification Period |Consideration of Notification Periods.
|Short Time Intervals |Ignore intervals shorter than a given duration, so that brief interruptions are ignored (similar to the concept of xref:availability#softstates[Soft states]).
...
Provided these have been specified in WATO xref:timeperiods[time periods],
they can also be used in the SLAs like xref:availability#serviceperiod[above] as already mentioned under general availability. Select a desired Period under [.guihint]#Active in timeperiod#, or as here in the example [.guihint]#Always# to define the requirements for a 24/7 operation.
...
xref:sla#moreslaviews[Column-specific SLA display] below.
...
with xref:regexes[regular expressions], and as in this example link the
...
column-specific SLA view can be seen in xref:sla#slacolumn[below].
...
Now create another xref:views#new[new view] for the SLAs.
...
for all services as mentioned above. More on this xref:sla#slacolumn[later].
...
In such cases, you can change the display via xref:sla#slacolumn[column-specific].
