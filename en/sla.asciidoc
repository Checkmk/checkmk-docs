...
A complete hour outage would however cause a lot of damage.

{cee-only} But {CMK} has a separate function for Service Level Agreements (SLA) if you're using one of the {EE}.
...
// Because of this proximity to views, the SLA feature is also embedded in the views configuration.
...
* Time period: Active xref:timeperiods#[time periods] from the Setup configuration (business days, more or less)
...
As usual with {CMK}, between the data source (SLA definitions) and the output (view) there is still a rule to assign for SLA-specific services - but this is not a must.
...
|===
|Option |Function 
|[.guihint]#Scheduled Downtimes# |Consideration of planned xref:basics_downtimes#[scheduled downtimes.]
...
As long as you have specified xref:timeperiods#[time periods], these can also be used with the SLAs, as already mentioned above for xref:availability#serviceperiod[general availability].
Select a desired time period under [.guihint]#Active in timeperiod#, or as here in the example `24X7 - Always` to define the requirements for a 24/7 operation.
...
How to use the configuration created up to this point without such an SLA service connection is described below under xref:moreslaviews[Column-specific SLA displays.]
...
How and why it is better to do that with a column-specific SLA view can be seen in xref:slacolumn[below].
...
Create a new view with [.guihint]#Customize > Vizualization > Views > Add view#.
...
The latter shows a fixed SLA definition for each service in the view - the better alternative to an SLA for all services as mentioned above. More on this xref:slacolumn[later].
...
If you want to group the view by individual hosts, optionally under [.guihint]#Grouping# add the column [.guihint]#Hosts: Hostname# - which ensures a visual separation of the hosts.
...
After selection in the [.guihint]#Monitor# menu you get as result a view with five status icons as single results from the percentage SLA, and a summary in the form of [.guihint]#100 percent# for the outage SLA.
...
A service-specific view has a disadvantage:
...
In the example completed above, those three steps mentioned at the beginning were executed - create SLA, bind to service, install in view.
...
A click on the cell with the SLA data takes you directly to the detail page of the SLA results from the affected service.
...
In the following picture the status is nearly {WARN} – because only four out of the five allowed outages are left – and the [.guihint]#SLA The message [.guihint]#SLA broken# would also appear in this tooltip.
...
// In the image above, for example, the current period is highlighted on the far right.
...
The state of a xref:bi#[BI aggregation] can be monitored via the [.guihint]#Check State of BI Aggregation# rule set as a normal service.
This then appears, for example, as _Aggr MySLA_ in the host views and can in turn be associated to an SLA via the [.guihint]#Assign SLA definition to service# rule already used xref:link_sla_service[above].

You will find the rule under [.guihint]#Setup > Services > Other services > Check State of BI Aggregation#.
The rule is designed to also query BI aggregates on remote {CMK} servers.
Therefore you need to connect the URL to the server and specify an xref:wato_user#automation[automation user] -- and of course the desired BI aggregation in the [.guihint]#Aggregation Name# field:
Here you enter the title of a top-level rule from your BI pack.

[{image-border}]
image::sla_view_bi_modern.png[]

Caution – *there is a risk of confusion here:*
In the BI configuration, you create the actual aggregation, i.e. the logic, via rules -- and one of the top rules is specified here precisely via its title as "Aggregation".
...
* xref:basics_downtimes#[Scheduled downtimes]: [.guihint]#Setup > Hosts > Host monitoring rules > Recurring downtimes for hosts# and [.guihint]#Setup > Services > Service monitoring rules > Recurring downtimes for services# (nur {CEE})
...
In such cases, you can switch to xref:slacolumn[column-specific display].
...
To be informed of a status change beforehand, you must xref:sla#createsla[configure the SLA levels].
