...
In the following picture the status is nearly {WARN} – because only four out of the five allowed outages are left – 
and the the message [.guihint]#SLA broken# would also appear in this tooltip.
...
This then appears, for example, as [.guihint]#Aggr My Bi# in the views and can in turn be associated to an SLA via the [.guihint]#Assign SLA definition to service# rule already used xref:link_sla_service[above].
...
In the [.guihint]#Aggregation Name# field, enter the title of a top-level rule from your BI package.
