...
* Calculation of the link:availability.html[availability] of an application.
* link:notifications.html[Notifications] in the event of a problem, or even a failure of an application.
...
== Configuration Part 1: The first aggregation#first
...
link:localchecks.html[dummy-local-checks] you will quickly get matching services
to play with.
...
Get started: configure BI through the [.guihint]#Business Intelligence# WATO module.
...
For this example it is best to create a new package -- with the [.guihint]#New BI Pack# button --
...
With each entry is a symbol for editing the properties (icon:button_edit[]),
and a symbol to get to the actual _content_ of the
Package (icon:icon_bi_rules[]), which is where you want to go now.
...
Pre-selected is [.guihint]#Worst -- takes worst of all node states#. That would mean that
...
The [.guihint]#New Aggregation# button takes you to a mask for creating a new aggregation.
...
However, it is important that you leave [.guihint]#Rule to call}} set to {{Call a rule#,
and at [.guihint]#Rule:# to select the rule you have just created (and before that the
...
If you now create the aggregation with [.guihint]#Create#, you will be done!
...
== BI in Operation Part 1: The Status View#statusgui
...
status interface. The easiest way to do this is via the [.guihint]#Views# element in the
sidebar using the [.guihint]#Business Intelligence|All Aggregations# entry:

image::bi_status_gui_1.png[]

==== Creating views for BI
In addition to the ready-made BI views, you can also custom-make your own.
To do so, select one of the BI data sources in link:views.html#1. Data source[Create a new view].
...
Use the icon:tree_black_open[] black triangle to expand and collapse views of subtrees.
...
and icon:icon_availability.png]. With the first icon -- ICON[icon_showbi[] -- you come
...
icon:icon_availability[] will take you to the calculation of the availability.
More on this later.
...
icon:icon_assume_1.png] ({WARN}), ICON[icon_assume_2[] ({CRIT}) and
icon:icon_assume_3.png] ({UNKNOWN}), and back to ICON[icon_assume_none[].
...
For this purpose, the icon:icon_commands[] link:commands.html[commands]
...
the [.guihint]#All aggregations}} page -- in our example above that is simply on the {{Main# heading.
...
== Configuration Part 2: Multi-level trees#multilevel
...
This time leave the [.guihint]#Aggregation Function# in the third box as
...
== BI in operation Part 2: Alternative displays#displayoptions
...
for these is the so-called [.guihint]#Display options#, which you can access via the icon:icon_painteroptions[]
...
is the [.guihint]#Boxes# diplay. Here each node is a coloured box which can be
...
=== Visualising BI aggregations#visualization
...
BI aggregations in the link:bi.html#aggregationfunction[Aggregation Function] section
(this can be reached directly via the node’s context menu).
Using the icon:icon_bi_visualization_rotate.png] and ICON[icon_bi_visualization_resize[] icons
...
in the [.guihint]#Rule Properties}} menu you can assign the {{Hierarchical}}, {{Radial#
...
choosing the right style. Here in the image it is a tree in the [.guihint]#Radial style#,
...
== Configuration Part 3: Variables, Templates, Searches#config3
...
These two subnodes should be [.guihint]#Switch 1}} or {{Switch 2#.
...
But that is no problem, because BI always interprets the service name as a prefix match using regular expressions -- completely-analogous to the well-known link:wato_rules.html[rules] service.
...
The subject of link:regexes.html[regular expressions] is again worth a closer look.
...
Alongside [.guihint]#Infrastructure}} there should be a node named {{Mystery Servers#.
...
* With [.guihint]#Service:# the sophisticated link:regexes.html[regular expression] `CPU|Memory` which uses a vertical bar to allow alternative service names (prefixes) is used, and this matches all services that begin with `CPU` or `Memory`. This saves a doubling of the configuration!
...
you can omit this and instead use the regular expression [.guihint]#srv-mys-(.*)#
...
[.guihint]#Create notes based on service search#. Here is an example:
...
== The predefined host aggregation#hostaggr
...
As usual, access the rules by clicking icon:icon_bi_rules[]:
...
icon:icon_bitree[] for this rule takes you to a tree view.
...
Back in the list of rules, with the [.guihint]#Aggregations# button you can access
...
More precisely, for BI there are two link:wato_user.html#roles[permissions]:
...
In the following example [.guihint]#Permitted Contact Groups}} the {{The Mystery Admins# contact
...
By the way, with [.guihint]#Public|Allow all users to refer to this pack#
you can allow other users to at least [.guihint]#use# the rules contained here
...
[.guihint]#WATO|Roles & Permissions#:
...
== BI in Operation Part 3: Maintenance times, acknowledgments#operating

=== The General Idea

How does BI actually manage icon:icon_downtime[] link:basics_downtimes.html[maintenance times]?
...
reminded of what the real idea behind maintenance times is -- i.e. the symbol icon:icon_downtime[]:
...
aggregation really is threatened -- only then will {CMK} mark it as icon:icon_downtime[] ‘in maintenance’.
...
The option for this is called [.guihint]#Escalate downtime based on aggregated WARN state#:
...
has been icon:icon_ack[] link:basics_ackn.html[acknowledged] the information is also
...
Exactly as with hosts and services, you can also access the BI link:availability.html[availability]
...
What is actually happening in BI in a link:distributed_monitoring.html[distributed environment]?
...
== Notifications, BI as a service#biasservice

=== Aktive Checks or Data Source Programs

image::icon_notifications.png[left, width=80]

Can you actually link:notifications.html[notify] of status changes in BI aggregations?
...
We will start with the ‘link:datasource_programs.html[data source program]’ method,
because this is always good if you wish to generate more than a handful of
aggregations as services.
You will find the appropriate rule set under [.guihint]#Datasource Programs|Check state of BI Aggregations#:
...
The whole thing is then performed via the link:piggyback.html[piggyback mechanism].
...
[.guihint]#Host & Service Parameters|Active checks|Check State of BI Aggregation# ruleset:
...
* The user must be an link:wato_user.html#automation[Automation user] – only such users may call the Web API. The `automation` user offers itself here as it is always created automatically for such purposes.
* At [.guihint]#password}} enter the user’s {{Automation secret for machine accounts#, which you will find in the configuration mask of the user properties.
