...
:description: With the {CMK} {ME} you can operate isolated sites for multiple customers via distributed monitoring.
...
xref:distributed_monitoring#distr_wato[centralized Setup], as a rule a user will
log in to the _central site_ in order to work with configurations
or to access the monitoring data.
Users can additionally log in to the _remote sites_ since only they are
responsible for the hosts and services monitored beyond that position.
The authorization concept in {CMK} which uses xref:wato_user#roles[Roles] and
xref:wato_user#contact_groups[Contact groups] to control the visibility and
...
With a centralized setup, in the {FE} and {SE} {CMK} will therefore distribute all configuration
files to all participating sites since in principle they could be located or
be required anywhere, as the case may be. Centrally-managed passwords must also
be made available for the remote sites as the hosts and services in
contact groups can be distributed over multiple sites.
...
specified remote sites. This means that sensitive customer data may
...
from needing to perform such work. The customer also does not need a centralized
overview since they only need access to their own data.

With the {CME}, via the xref:distributed_monitoring#[distributed monitoring] in their
central site, for each customer a provider links only the one or more local
sites which belong to that customer. Individual elements in [.guihint]#Setup# will then
be assigned to these sites. When distributing configuration data {CMK} will
now only send data of a general nature or data that has been approved for the site
to a customer's site. The service provider can still easily carry out a configuration
over the central Setup in their own site.
...
as in a normal distributed environment with the only difference, that you must use the {CME} on all involved sites:
...
to the customer over the site assigned to that customer.
...
link:https://checkmk.com/product/pricing[licensing^] must be selected if 
...
in [.guihint]#Setup > Users > Customers# select the [.guihint]#Add Customer# button, and assign an
...
to this customer. The central site to which all of the customer's other
sites send their data is also known as the _provider site_.
Currently separation of the data only functions if each customer has their own
site assigned to them and this is in turn xref:distributed_monitoring#connect_remote_sites[connected]
to your provider site. The setup in this case differs at a single point:
...
a site {CMK} always knows which host belongs to which customer.

*Note*: The customer site's [.guihint]#Global settings# can as usual be configured
over the xref:distributed_monitoring#sitespecific[site specific global settings].


=== Further assignments

Alongside the site itself – as mentioned in the introduction – you can also
assign other elements from the [.guihint]#Setup# to a customer. In doing so an element
...
In the xref:glossar#ec[Event Console] you can assign individual rules as well as complete
rule packs to a customer.
...
If a rule pack has not been assigned to any customer,
the individual rules can be assigned to a customer as applicable.


=== Non-customizable components

All components that have not been discussed in the preceding can _not_ be assigned
...
BI aggregations cannot be assigned to a specific customer. Therefore all aggregations
and their rules will be assigned to all sites. For this reason the naming of
rules, packs and aggregations should be as generalized as possible,
...
Likewise xref:glossar#host_tag[host tags] may not contain confidential
information since the tags are distributed to all sites.


==== Notifications

Rules for xref:glossar#notification[notifications] often contain contact groups and very specific conditions under
which the notifications should be triggered and sent.
Since these rules are also distributed to all sites, you should especially
avoid using explicit host and service names, contact addresses and other sensitive data.


==== Customization of global users

Note that all customizations of global users will be passed on to all of the
customer's sites. Global users are therefore unsuitable for specialized views,
...
The topics mentioned in the last chapter like notifications with contact groups or host tags might only leak information on organizational structures of other customers.
...
This xref:glossar#view[view] functions like the [.guihint]#All hosts# view, with the difference here being
that only the specific customer's elements are shown.


=== Sidebar

The [.guihint]#Customers# snapin of the xref:glossar#sidebar[sidebar] functions in exactly the same way as the similar looking
[.guihint]#Site status# snapin. Here the status of the individual customer's sites can be
...
*all* of a customer's sites.


=== Filtering and building views

Of course you can also use the filters and data sets for your own views in the same way as they are used in the dashlet and the snapin.
...
And on the other hand you can build completely xref:views#new[new views] based on
...
number of particulars to be aware of. If you only want to switch a single site
the transition is very easy: simply perform a xref:update#detailed[update] of the
site in the usual way, after which all of the important tasks will have been completed.
...
being function as before. Then in your own good time you can construct an
environment for the Managed Services.

If the upgrade is to an existing environment in which already remote sites
...
and for assigning sites, users, etc. to them.
As already mentioned these will in fact be assigned to the [.guihint]#Provider#.
In an existing xref:distributed_monitoring#[distributed monitoring] this however also
means that all other sites with this data can not yet use it.
...
To securely prevent any changes from occurring, these can be disabled
for the duration of the update process. This lock is activated in the
[.guihint]#Setup > General > Read only mode#:
...
sites or if it is a customer-specific user.
