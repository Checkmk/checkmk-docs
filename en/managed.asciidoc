...
be made available for the _remote sites_ as the hosts and services in
...
specified _remote sites. This means that sensitive customer data may
...
site assigned to them and this is in turn xref:distributed_monitoring#[connected]
...
All components that have not been discussed in the preceding can not be assigned
...
rules, packets and aggregations should be as generalized as possible,
...
Rules for notifications often contain contact groups and very specific conditions under
...
The topics mentioned in the last chapter like contact groups or host tags might leak information on organizational structures of other customers.
Noncompliance poses a moderate security risk.

Distribution of CA (_certificate authority_) root certificates is another issue however:
In case distribution of CA certificates would result in a certificate destined for _Customer A_ to be trusted by the computers of _Customer B_, a rogue admin of  _Customer A_ would be able to conduct a _Man in the Middle_ attack on encrypted communication of _Customer B_.
For this reason, CA certificates added with the global [.guihint]#Trusted certificate authorities for SSL# will _not_ be synchronized for the remote sites.
Use the corresponding site specific setting instead.
...
New on the Dashboard [.guihint]#Main Overview# is the [.guihint]#Customers# element in which links
...
[.guihint]#Site Status# Snapin. Here the status of an individual customer's sites can be
...
If the upgrade is to an existing environment in which already deleted sites
