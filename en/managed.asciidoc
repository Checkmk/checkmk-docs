...
With a centralized setup, under the {FE} and {SE} {CMK} will therefore distribute all configuration files to all participating sites, 
...
The [.guihint]#Customers# snapin of the xref:glossar#sidebar[sidebar] functions in exactly the same way as the similar looking [.guihint]#Site status# snapin.
...
In contrast to the [.guihint]#Site status# snapin, with this snapin a single click hides *all* of a customer's sites.
...
When upgrading an existing environment from the {FE} or {SE} to the {ME} there are a number of particulars to be aware of.
