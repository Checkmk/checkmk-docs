...
xref:distributed_monitoring#distr_wato[centralised Setup], as a rule a user will
...
The authorisation concept in {CMK} which uses xref:wato_user#roles[Roles] and
...
With a centralised setup, in the {FE} and {SE} {CMK} will therefore distribute all configuration
...
customer's sites. Global users are therefore unsuitable for specialised views,
