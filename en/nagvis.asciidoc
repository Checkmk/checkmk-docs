...
These dynamic icons can then show the current status of each of the hosts and services.
...
Here can be found over 100 options for the basics, such as data formats and language settings, from the displaying of objects on maps, to the weighting of the available host and service statuses.
...
although that is simpler to achieve with a right-click on an icon on the map, and then selecting the [.guihint]#Make root# shortcut menu entry.
...
Right-click on the icon to open its shortcut menu, and select [.guihint]#Unlock#.

image::nagvis2_regularmap_icon-menu_locked.png[]

Now you can move the icon, or again open the shortcut menu.
...
Once the editing of a host/icon is complete, again open its shortcut menu and select [.guihint]#Lock#.
...
If you want to open the shortcut menu of an iframe container, you must place the mouse cursor directly over the edge so that it changes its shape for scaling.
