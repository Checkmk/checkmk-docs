...
image::sidebar_add_snapin2.png[width=280]
...
use of NagVis, and especially its configuration options.  NagVis itself has
...
to know NagVis' procedures and its most important elements, we will show
...
Here a number of new options will be seen which will enable the object to be cloned, deleted or edited, or to acknowledge a problem directly from here by choosing the [.guihint]#Acknowledge# option.
See link:basics_ackn.html[acknowledge.]
...
the alarms with [.guihint]#event_on_load#, and at the bottom check the [.guihint]#event_sound#
...
Use the simple link:livestatus.html[Livestatus query] [.guihint]#Filter: description ~ CPU load\n#.
