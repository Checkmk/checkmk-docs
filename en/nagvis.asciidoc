...
:revdate: 2018-06-26
:title: Creating maps in (CMK) with NagVis
...
In the article on link:monitoring_basics.html#parents[Monitoring basics]
you saw the map of a network's topology based on
link:monitoring_basics.html#parents[Parent-Child-Relationships].  NagVis is responsible
...
precise link:livestatus.html[Livestatus], provides backend data such as hosts, services,
...
Provided the link:monitoring_basics.html#parents[Parent-Child-Relationships] have
...
using the icon:button_sidebar_addsnapin_lo[] button at the lower left of
the navigation.  Here select the [.guihint]#NagVis Maps# snapin, and finally start
NagVis with the icon:button_view_snapin_edit[] button.

image::sidebar_add_snapin2.png[center, width=280]
...
link:regexes.html[Regular expressions]; to put it more accurately, in the form of
valid link:livestatus.html[Livestatus-Filter-Definitions.]  In this way new hosts
...
type via [.guihint]#Options|Manage Maps|Create Map}}.  In {{ID# enter
the internal name [.guihint]#myautomap}}, and in {{Alias# your name for the display
[.guihint]#My Automap#.
...
[.guihint]#Create Map}} give the map the name {{mymap}} and the alias {{My Map#. For
the [.guihint]#Map Type}} select {{Regular map}} and verify it with {{Create# button.
...
[.guihint]#Appearance}} tab. Here activate the {{map_image# option and select the
...
Go to the [.guihint]#Obj. Defaults}} tab. Here you can activate the {{label_show#
...
[.guihint]#Acknowledge# option. See link:basics_ackn.html[acknowledge.]
...
select [.guihint]#Lock}}.  By the way, with {{Edit Map|Lock/Unlock all# the editing
...
alarm and blinking host icons.  For this, call up [.guihint]#Edit Map|Map Options#
and switch to the [.guihint]#Events# tab.  Here, at the top of the display, activate
the alarms with [.guihint]#event_on_load}}, and at the bottom check the {{event_sound#
...
[.guihint]#Worldmaps}}, and the _non-interactive_ variants are called {{Geomaps#.
...
a [.guihint]#Worldmap}} – with {{Options|Manage Maps|Create Map}}. As {{Map Type#
select the [.guihint]#Geographical Map (interactive)}} option – the ID {{(mygeomap)#
...
define a new alias [.guihint]#NRW}} with {{Edit Map|Map Options#.
...
[.guihint]#Create Object}}-dialogue you only need to specify the {{mygeomap_nrw# map from
the selection menu.
...
and allocate an ID [.guihint]#(mydynamicmap)}} and alias {{(My Dynamic Map)#.

image::nagvis2_dynmap_create.png[]

Next open the map options with [.guihint]#Edit Map|Map Options# and select the
[.guihint]#Dynmap}} tab.  Here activate the {{dynmap_object_types# option and choose
...
Use the simple link:livestatus.html[Livestatus-Abfrage] [.guihint]#Filter: description ~ CPU load\n#.
...
Next, switch to the [.guihint]#Appearance}} tab and set the {{view_type}} to {{line#.
As the [.guihint]#line_type}} activate the {{--%+BW-><-%+BW--# variant – with this
...
In the [.guihint]#General}} tab choose {{myhost# as the host, and a suitable network
interface as the service.

Switch to the [.guihint]#Appearance}} tab and set {{view_type}} to {{gadget#.  Directly
...
purely to the table should serve as an link:views.html#embed_views[embedded view].
...
In the [.guihint]#General}} tab set the {{view_type}} to {{iframe#.
...
Subsequently the link:views.html#embed_views[Embed a view] options are
...
maps [.guihint]#mymap1}}, {{mymap3}} and {{mymap#, and with an interval of 30 seconds:
