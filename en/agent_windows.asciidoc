...
Immediately after the installation of the {v21} agent (even as an update of an older agent), only unencrypted communication in legacy pull mode is possible.
...
For a host that is connected to the {CMK} server, you can revoke the trust with this command:
...
If you delete this soft link, the host must be registered again.
Here you can also delete unused soft links, for example, from multiple registrations of the same host.
