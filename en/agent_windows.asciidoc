...
In {CMK} version {v21}, with the *Agent Controller* a new component has been added to this *agent program*.
The Agent Controller is upstream of the agent program, queries it and communicates with the {CMK} server in its place.
...
From {CMK} version {v22} onwards, the {CCE} also features the xref:glossar#push_mode[push mode], which facilitates the monitoring of hosts that are located behind firewalls.
...
// In this mode the agent works only as an agent program, i.e. without Agent Controller and thus without registration at the {CMK} server.
// For the legacy mode some special requirements have to be considered, which are summarized in the xref:post_install[installation chapter].
...
In the Version column you will find the beginning of the version number, which can be displayed on the Windows system with the `ver` and `systeminfo` commands.

[#windows_versions]
[cols="70,~",options="header"]
|===
|Edition |Version
// MFS: Support for NT 6.0 has been dropped with the start of 2.2.0 Beta
// |Windows Vista |6.0 |without
// |Windows Server 2008 (R1) |6.0 |without
|Windows 7 |6.1
|Windows Server 2008 R2 |6.1
|Windows 8 |6.2
|Windows Server 2012 (R1) |6.2
|Windows 8.1 |6.3
...
*Important*: Editions not listed in this table are not officially supported.
This also includes _Windows Embedded_, for example.
However, for monitoring older Windows versions like e.g. Windows Server 2008 (R1), you can use a *legacy agent* on your own risk.
Legacy agents are agents of older {CMK} versions without agent controller.
Of course this means extended functionality of the agent controller like TLS encryption or compression will be missing.
Legacy agents are link:https://download.checkmk.com/legacy-agents/[available for download here^].
For the legacy agents some special requirements have to be considered, which are summarized in the xref:post_install[installation chapter].
...
{CMK} provides several ways of installing the Windows agent -- from a manual installation of the software package to the fully automated deployment including its update function.
Some of these installation methods are only available in the {CEE}:
...
In the {CRE} you can find the agent's Windows package via [.guihint]#Setup > Agents > Windows#.
In the {EE}, you first get to the xref:wato_monitoringagents#bakery[Agent Bakery] in the [.guihint]#Setup# menu via [.guihint]#Agents > Windows, Linux, Solaris, AIX#, where you find the baked packages.
...
{cee-only}
The {EE} have a software module, the xref:glossar#agent_bakery[Agent Bakery], for automatically packaging customized agents.
...
In the {CE} you can additionally use the Agent Bakery to provide agent packages with a configuration for auto-registration,
...
The case is different when using a *legacy agent* on a very old xref:windows_versions[Windows system].
Without Agent Controller, registration is not possible.
...
An exclusively encrypted data transmission is can only be activated once a trust relationship has been established.

An exception to this are the packages preconfigured for the xref:hosts_autoregister#[auto-registration] and downloaded via the Agent Bakery in the {CCE}.
...
For this, a good choice is the xref:glossar#automation_user[automation user] which is automatically created with every {CMK} installation and whose password you can let 
Checkmk generate randomly for you by clicking on the icon:icon_random[alt="Symbol for rolling a password."] icon.

*Note:* Since there is no Agent Controller, and thus no registry and TLS encryption, on very old xref:windows_versions[Windows systems], 
you will need to use other encryption methods if required.
In this case we recommend using the built-in (symmetric) encryption with the [.guihint]#Symmetric encryption (Linux, Windows)# rule and/or set up an xref:ssh[SSH tunnel].
...
In the {CCE} you will find the [.guihint]#Checkmk agent connection mode# option in the properties of the host in the section on xref:hosts_setup.html#monitoring_agents[monitoring agents].
...
Whether the host is configured for the pull mode (all editions) or the push mode (only the {CE}) makes no difference for the command examples.
...
The {CCE} provides the facility to create hosts automatically at registration.
The following requirements must be met for the xref:hosts_autoregister#[auto-registration]:

* A user with the permission to register hosts must exist.
* At least one folder must be configured for automatically created hosts.
...
The biggest difference here is the modified sub-command `register-new` in combination with the specification of the user for the auto-registration.
Both of these together request the creation of a new host on the {CMK} site.
...
In the {CCE} you can switch hosts from push to pull mode and vice versa.
This may be necessary in individual cases if changes to the network topology are pending, or a downgrade to the {CEE} -- in which only the pull mode is possible -- is to be carried out.
...
The agent program is a simple Windows executable that obtains data on your system and outputs it as loosely formatted text.
...
In the case of a registration in the push mode of the {CE} the following applies:
...
If in pull mode it has been verified that the agent program and its installed plug-ins are executed correctly,
...
When using the push mode of the {CCE}, it is usually not necessary to change its settings.
If you use a very strict firewall configuration, the _Outbound Rules_ for connections to the monitoring server must be configured so that at least xref:ports#cmk_incoming_host_outgoing[port 8000] (for easier registration, additionally ports 80 or 443) is reachable.
...
For push hosts under the {CCE} this soft link points to the folder with the agent output, for pull hosts it points to a non-existent file with the name of the host as used in the monitoring.
...
If a host in the {CCE} has been configured for xref:hosts_autoregister#rule_autoregister_bakery[auto-registration] with the [.guihint]#Agent controller auto-registration# rule set and the [.guihint]#Keep existing connections# option is set to [. guihint]#no#, whenever the `cmk-agent-ctl-daemon` service is restarted (for example, when a host is restarted), all other connections will be removed -- except the connection configured for auto-registration.
...
When auto-registering a host in the {CCE}, typically about two minutes pass before the host appears in the monitoring.
