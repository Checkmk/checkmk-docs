...
|MSI package from the link:wato_monitoringagents.html#bakery[Agent Bakery]
|Configuration over the GUI -- an individual configuration of each host is possible.
|
|X

|link:agent_deployment.html[Automatic Updates]
...
These can be found in WATO under ICON[icon_agents.png] [.guihint]#Monitoring Agents#.
If you are using one of the {EE} and a version that is older than VERSION[1.4.0],
take the detour via the [.guihint]#Unpackaged agents}} or {{Agent files# button:

image::packaged_agents.png[]

In [.guihint]#Packaged Agents}} you can find the required file {{check_mk_agent.msi#.
...
image::auto_delete_legacy_agent.png[center, width=80%]
...
agent in the [.guihint]#Windows Firewall with Advanced Security}} ({{WF.msc#).
...
== Installation of the old agent#legacy
...
of your {CMK} server to Version VERSION[1.6.0] does not automatically require
an agent update.
...
== Testing and fault diagnosis#testing
...
== Incorporation of classic check plug-ins#mrpe
...
[.border]
image::agent_windows_plugins.png[]
...
<a href="https://www.python.org/doc/">Python Software Foundation</a>.


=== Customising the execution of a special plug-in#customizeexecution
...
|`cache_age`
|`*60*`
|Specifies in seconds how long an output is valid. If `async` is activated, a cache of ??? seconds is automatically created.
...
[CEE] The plug-ins supplied by {CMK} can be configured via the
...
From {CMK} version VERSION[1.4.0] the Windows Agent (and also its Linux
...
in WATO. In versions older than VERSION[1.5.0] the box is called [.guihint]#Host tags#
and will be altered to [.guihint]#Dual: Check_MK Agent + SNMP#. Services which are
...
|The directory for your own link:localchecks.html[local-scripts]
