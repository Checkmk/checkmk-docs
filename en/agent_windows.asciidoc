...
Compared to monitoring with WMI or SNMP this has several benefits -- {CMK} is:
...
because, for example, to function it needs no additional libraries.
{CMK} is also delivered with a basic configuration which is sufficient for most
applications.

In operation the agent consists of a Windows service which starts the supplied
program -- the agent. When invoked this agent collects data from the local system
...
The following table lists these specifically:
...
xref:wato_monitoringagents#bakery[Agent Bakery], or configure them manually on each
host -- you first need the installation file.
In the {CRE} you go via [.guihint]#Setup > Agents > Windows#
to the [.guihint]#Packaged Agents#. Here you will find the [.guihint]#check_mk_agent.msi# file.

image::agent_windows_packaged_agents.png[width=60%]

In the {EE} you find the baked agent packages via [.guihint]#Setup > Agents >
Windows, Linux, Solaris, AIX#. In order to download the installer, simply click the icon in the column [.guihint]#Windows#.
...
If you do not find such an entry in the Windows Firewall settings, you can add it at this point specifically.
...
{cee-only}
For the agent on Windows the {CEE} also have  the possibility of configuring it individually via the xref:wato_monitoringagents#bakery[Agent Bakery] using the [.guihint]#Setup# menu.
...
=== Special features of the agent up to Version {v15}
...
Simply call the installation package for the new agent (`check_mk_agent.msi`), and follow the instructions as usual.
...
a group. Once you are sure that the conversion works correctly, you can
accordingly deinstall the old agent automatically. You will then save yourself
the manual deinstallation, and can simply replace the old agent with the new one.

*Important*: After you have removed the old agent, its directory may not
have been completely deleted. This is not an error, but a normal behavior if
...
In such cases, simply delete the old agent’s installation directory following
the deinstallation after making sure that it contains no important files.


==== Adapting firewall rules for the new agent

If you do not make a fresh install of the standard agent and are instead
migrating from the legacy agent, you may need to adapt your existing
firewall rule. You don’t need to add a new rule as described
xref:agent_windows#firewall[above] however, instead you can simply modify your existing rule and point it to the new agent. We will assume in the following example that your rule is called ‘Check_MK’:
...
to a specific part. Here, for example, it is being checked whether the options for the `ps` section have been defined correctly:
...
The value shown here is also the default value in seconds, so alter this value only if you want to set a shorter or longer interval:
...
It is not a requirement that the file be stored in the agent’s directory,
...
After you have set up the MRPE plug-in, it will be immediately become active  without needing a reboot of the agent, and it will be added to the output.
...
correctly -- via a service discovery also automatically create new services.
...
As already mentioned, as standard {CMK} provides a whole series of plug-ins for Windows. You can find these on the monitored host in the agent’s installation
...
If the plug-in reaches this timeout, it will make a second attempt to get a
result.
...
traffic. On newer Windows versions the native SSH is additionally optional, so that an encryption over the entire duration of the connection can be guaranteed, as is already familiar under Linux.
...
ability to simply ignore requests from unfamiliar IP addresses.
...
configuration file, and that only the relevant excerpt is shown here:
...
Of course an attacker can very easily fake their IP address and thus get a connection to the agent, however it is very unlikely that they will get an answer -- because any answer will be routed to the real monitoring server.
...
It makes sense to first start with [.guihint]#Enable#. Once you are confident that all of the agents are using encryption, switch to [.guihint]#Enforce# to find any hosts which still send data in clear text.
...
The necessary entry is then automatically created in the xref:agent_windows#files[configuration file for the Agent Bakery].
The `/etc/check_mk/encryption.cfg` file will be automatically-created and built into the agent packages for you.
...
Services which are available via both SNMP and a {CMK} agent (e.g. CPU utilization, file systems, network adapters) will then be fetched automatically from the {CMK} agent and not via SNMP.
...
From the command line, you have several options for removing the agent.
If you still have the most recently installed MSI package, you can use it for uninstallation as follows:
...
The process can sometimes take quite a long time and will not provide any status messages, but will output very long lists.
...
Since the various routines in Windows only remove files that also got there through the installation process, it is perfectly normal that some files could remain in the xref:agent_windows#files[the agent's directories].
