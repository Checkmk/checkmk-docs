...
However, we provide a link:https://download.checkmk.com/legacy-agents/[download repository^] with the archived legacy agents of older {CMK} versions.
You might try these on your own risk with Windows versions not included in the list.
For the legacy mode some special requirements have to be considered, which are summarized in the xref:post_install[installation chapter].
...
The case is different if one of the legacy agents is installed on a very old xref:windows_versions[Windows system].
