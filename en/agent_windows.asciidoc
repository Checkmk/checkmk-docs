...
However, if you receive no information at all or only error messages about timeouts during link:wato_services.html#discovery[service discovery] or during a link:wato_monitoringagents.html#diagnosticpage[connection test] to the host, you should check the [.guihint]#Inbound Rules# of the Windows firewall on the host.
...
[cols="15,~"]
|====
|Rule Type |Leave the selection here at [.guihint]#Program#.
|Program |Enter [.guihint]#This program path# `%ProgramFiles% (x86)\checkmk\service\check_mk_agent.exe` or use the [.guihint]#Browse# button to select the `check_mk_agent.exe` file.
|Action |Leave the selection at [.guihint]#Allow the connection#.
|Profile |This point depends heavily on the configuration of your network. However, in most cases it is recommended to enable only [.guihint]#Domain# and [.guihint]#Private# here.
...
{cee-only}
The {CEE} also have for the agent on Windows the possibility to configure it individually via the link:wato_monitoringagents.html#bakery[Agent Bakery] using the [.guihint]#Setup# menu.
For a detailed description see the general chapter on the link:wato_monitoringagents.html[agents].
...
The necessary entry is then generated automatically in the link:agent_windows.html#files[Agent Bakery configuration file].
...
* A corresponding article in this User manual -- for example, that on monitoring link:monitoring_oracle.html[Oracle]
...
The necessary entry is then automatically created in the link:agent_windows.html#files[configuration file of the agent bakery].
...
Since the various routines of Windows only remove files that also got there through the installation process, it is perfectly normal that files remain in the link:agent_windows.html#files[directories of the agent].
...
|`C:\ProgramData\checkmk\agent\local` |The directory for your own link:localchecks.html[local checks].
