...
However, we provide a link:https://download.checkmk.com/legacy-agents/[download repository^] with the archived agents of older {CMK} versions.
...
The case is different if one of the archived agents is installed on a very old xref:windows_versions[Windows system].
