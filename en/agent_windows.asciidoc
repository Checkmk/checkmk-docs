...
In the chapter xref:test[Testing and troubleshooting] you must omit the test for calling the Agent Controller, because it is not available for a legacy agent.
...
{CMK} generate randomly for you by clicking on the icon:icon_random[alt="Symbol for rolling a password."] icon.
Alternatively, by assigning the `agent_registration` role, you can create an automation user that only has permission to register agents.
This is described in the xref:hosts_autoregister#setup_autoregister[automated host creation] article of the {CE}.
