...
Supports CMK- and KNW- prefixed tickets.

Usage: python read_ticket.py <TICKET_KEY>
...
def collect_linked_keys(issue: Issue) -> list[tuple[str, str, str]]:
    """Return (link_type_description, direction, key) tuples for CMK- and KNW- project links."""
    ...
    if parent and is_allowed_key(parent.key):
...
def main() -> None:
    ...
    if not is_allowed_key(key):
        sys.stderr.write(
            f"Error: Only CMK- and KNW- tickets are supported, got: {key}\n"
        )
        sys.exit(1)
