...
:CMKTITLEDE: Die offiziellen Gebrauchshinweise und Richtlinien zur Nutzung Ihrer Datenverarbeitungs- und speicherungseinrichtung
...
// command line macros
:shell: source,subs="attributes+,+macros,+quotes"
:powershell: source,powershell,subs="attributes+,+macros,+quotes"
:shell-raw: source,subs="attributes+"
:c-user: user@host:~$
:c-omd: OMD[mysite]:~$
:c-local: OMD[central]:~$
